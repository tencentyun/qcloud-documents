## DML语句
- 【禁止】SELECT 语句必须指定具体字段名称，禁止写成select *，因为select * 会造成不必要的IO和网络开销，无法使用覆盖索引，表结构变更业务会受影响
- 【禁止】insert 语句指定具体字段名称，不要写成 insert into t1 values(…)，因为后期如果表字段做了变更，但应用层没有及时更新的话，系统会报错
- 【建议】事务涉及的表必须全部是 innodb 表。否则一旦失败不会全部回滚，且易造成主从库同步中断
- 【禁止】事务中，不要使用SELECT … FOR UPDATE 语法，它会扩大意向锁范围，较大程度影响数据库的并发事务效率
...
- 【禁止】 索 引 列 不 要 使 用 函 数 或 表 达 式 ， 否 则 无 法 利 用 索 引 
...
##  多表连接
- 【禁止】禁止在业务的更新类 SQL 语句中使用 join，比如 update t1 join t2…
...
- 【建议】事务中，不要使用 SELECT … FOR UPDATE 语法，它会扩大意向锁范围，较大
程度影响数据库的并发事务效率

...
注意：
1. 
支持列压缩属性的数据类型包括BLOB，TEXT，VARCHAR，VARBINARY和JSON。其中BLOB包含 TINYBLOB , MEDIUMBLOB , LONGBLOG ；TEXT包含 TINYTEXT , MEDUUMTEXT , LONGTEXT 
2. 
只有字段长度超过innodb_min_column_compress_length(默认256)才压缩

...
概述：该工具也可以用来修改DB的一些参数，修改后将会固化到配置文件上。
...
## 开启允许创建myisam表
修改reject_create_table_myisam，创建myisam表
在主DB机器上操作（DB端口号和实例ID以实例为准）：
...
## 修改单个事务量的大小
如果mysqlagent版本为12.7的，可以使用# strings mysqlreport|grep gcc_命令查看当前mysqlagent版本， 修改的方式为：
在主DB上执行，执行后会同步到备机上，并固化为配置文件里。
`# vim /data/tdsql_run/4028/mysqlagent/conf/modify_zk.xml`
在`<connect_timeout conf="connect_timeout"/>`下面一行加下面内容不能保存
`<binlog_write_threshold  conf="binlog_write_threshold"  param="binlog_write_threshold"/>`运行参数修改工具
```

...
|---------|---------|---------|---------|---------|
| timestamp  |  | int  |取当前时间（秒）即可|Y|
| eventId  |  | int  |取随机值即可，标识一次网络请求|Y|
|interfaceName  |接口名称|string| 混流接口名称请填写：Mix_StreamV2|Y|
|app_id||int|直播APPID|Y|
|interface||string|固定填写mix_streamv2.start_mix_stream_advanced|Y|
|mix_stream_session_id||string|填写80字节以内，仅含字母、数字以及下划线的字符串|Y|
|output_stream_id||string|指定输出流ID|Y
|output_stream_type||int|指定输出流类型。当输出流为输入流list中的一条时，填写0；当输出流为一条新的流，并非输入流list中的流时，该值为1。不填默认为0。|N|
|mix_stream_template_id||int|输入模版ID。目前两输入源支持10、20、30、40；三输入源支持310、390、391；四输入源支持410；五输入源支持510、590；六输入源支持610。不填默认为0。|N|
|input_stream_id||string| 指定输入流ID|Y|
| image_layer || int  |图层标识号，目前支持1-6。|Y|  
|input_type ||int |输入源标识，0表示输入源为流，3表示输入源为画布。|N| 
|image_width  ||  int| 输入画面的宽度。建议在0-3000以内。|N| 
|image_height || int 输入画面的高度。建议在0-3000以内。 |N|
|location_x || int| x偏移：相对于大主播背景画面左上角的横向偏移。|  N| 
|location_y | | int |y偏移：相对于大主播背景画面左上角的纵向偏移。 | N |
|color||  string| 当输入源为画布时，必须输入颜色。|N|
...
模版410效果图：
![](https://mc.qcloudimg.com/static/img/c7e1ea8da4076b40f1572ac39df0e355/t410.png)

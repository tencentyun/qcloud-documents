...
<td> 否
<td> String
<td> 是否开启实时视频录制，每个片段不超过5分钟 0表示普通 1表示小视频。<br>（1）若开启实时视频录制，调用接口则同步开始录制，此时如果传入任务开始时间参数，任务开始时间参数无效。
（2）开启实时视频录制的同时如果传入了任务结束时间，则按照任务结束时间结束录制，若没传入，则30分钟后自动结束录制。
（3）开启实时视频录制的同时如果传入了任务结束时间，实时录制时间与任务结束时间超过30分钟，则30分钟后会自动结束录制，实时视频录制建议控制台在5分钟以内。</font>
<tr>

...
不过在创建 LivePush 对象之前，还需要您指定一个**LivePushConfig**对象，该对象的用途是决定 LivePush 推流时格格环节的配置参数，比如推流用多大的分辨率、每秒钟要多少帧画面（FPS）以及Gop（表示多少秒一个I帧）等等。
...
- **推荐开启硬编**
iOS平台的机型数量并不像Android那么浩瀚，而且硬件质量也都非常过关，所以硬件加速在iOS平台是非常推荐的，尤其是 SDK 1.6.2 版本以后，只有开启硬件加速才能使用更好的美颜特效。故，如没有特殊原因，推荐您启用。
...
### step 11: 横屏推流
- **横屏采集**
通过对 LivePushConfig 中的 **homeOrientation** 设置项进行配置，可以实现横屏推流。

- **横屏预览**
有些客户可能想要在横屏推流时对主播端的预览画面也做相应的旋转调整，您可以通过 TXLivePush 中的 setRenderRotation 接口设置摄像头预览画面中的显示方向。
...
####  警告事件 
SDK发现了一些问题，但SDK还会坚强的活下去。只是抱着拒绝黑盒的设计原则，我们将其悉数暴露出来，其中PUSH_WARNING_NET_BUSY 相对比较有用(Step 10)，其它的了解一下即可，千万不要“小题大做”哦。

...
| x5_type           | String | 无   | 通过video属性“x5-video-player-type”声明启用同层H5播放器，支持的值：h5 (该属性为TBS内核实验性属性，非TBS内核不支持，具体介绍参照 [TBS文档](http://x5.tencent.com/guide?id=4000) )                            |
...
| WMode             | String  | window | Window模式不支持其他页面元素覆盖在flash播放器上面，如需要可以修改为opaque 或其他flash wmode的参数值，可以在 Flash 播放器上进行覆盖其他元素。<br> <font color="red">备注：该选项只对PC平台Flash播放器生效  </font>                                                                  |

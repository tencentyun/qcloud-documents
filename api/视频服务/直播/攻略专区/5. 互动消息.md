...
[**腾讯云通信**](https://cloud.tencent.com/product/im.html) 服务提供了一套完整的成体系的即时通讯解决方案，本节我们将介绍如何使用腾讯云 IM 通讯服务实现上述的几个功能点。

## 5.2 开通服务
在使用云通信服务之前，首先要将这个服务开通，可以按照如下步骤操作：

进入[云通讯管理控制台](https://console.cloud.tencent.com/avc)，如果还没有服务，直接单击【直接开通云通信】即可。新认证的腾讯云账号，云通信的应用列表是空的，如下图：
...
上图的列表中，SDKAppID 这一列显示的是腾讯云通信业务 ID，这个 ID 在对接各平台版本的 IM SDK 时都会用到，右侧有一个【应用配置】按钮，单击这里进入下一步的配置工作，如下图所示。
...
该模式对接步骤如下：
> 1. 在腾讯云通信 [管理控制台](https://console.cloud.tencent.com/avc)，将集成模式选为**独立模式**，并下载签名用的公私钥。
...
## 5.4 消息收发
腾讯云通信服务支持多种消息类型的格式，例如文本、图片、表情、语音甚至小文件，这部分可以在 [云通信文档专区](https://cloud.tencent.com/doc/product/269) 进行了解。

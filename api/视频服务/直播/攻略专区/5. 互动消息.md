...
访客模式的接入流程非常简单：点点鼠标 + 写几行对接代码就能搞定。

（1）首先，在 **5.2 **中集成模式选项里选择**托管模式**，这样能让腾讯云为访客模式的匿名账号提供后台支持。
（2）在客户端代码中对接 IM SDK 的访客登录模式即可，这里包括 TLS（TLS 的全称是 “Tencent Login Service” ，是腾讯云 IM SDK 的核心组件）和 IM SDK 两处的函数调用。

如果您已经有小直播的代码，这部分逻辑在小直播的源代码（iOS平台：位于TCIMPlatform.h 中的 guestLogin 函数；Android平台：位于TCLoginMgr.java 中的 guestLogin 函数）中已经帮您封装出来了，直接拷贝使用即可。

如果您想自己手动编写对接代码也很简单，如下是各平台的对接详解：
...
### 5.3.2 独立模式
访客模式在接入方便性上很有优势，一个GuestLogin函数就能解决问题，但您可能会想：“访客登录毕竟是使用了腾讯云的匿名账号帮我的App用户，是否可以让消息的发送人和接收人都实打实的是我自己后台系统的账号呢？”
...
针对这种比较简单的场景，我们在小直播中相应地采用了一种非常简单的办法，即：**统一采用文本消息通道收发消息，因为有多字段（如消息类型、用户头像、昵称等）拼装需求的情况，我们采用了json格式对数据进行组装。**
...
```
其中，**userAction** 是我们在小直播中定义的消息类型，一共有 5种，他们分别是：

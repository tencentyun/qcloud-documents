...
### 5.3.2 独立（安全）模式
访客模式在接入方便性上很有优势，一个GuestLogin函数就能解决问题，但它并没有很好地机制确保100%的安全性。因为访客账号毕竟只是一堆“傀儡”，真正消息发送者的信息都被放在具体的消息体中。如果攻击者破解了App，其实是可以伪造其它用户身份发消息的。

` “那对于安全性级别要求很高的场景，怎样才可以让消息的收发人都实打实的是我自己App的账号呢？” `

独立模式可以做到，代价就是相比于访客模式，它的对接步骤要繁琐很多，毕竟它要面对的问题是安全信任问题，所以<font color='red'>如果您没有非常迫切的安全性需求，不推荐使用该模式。</font>

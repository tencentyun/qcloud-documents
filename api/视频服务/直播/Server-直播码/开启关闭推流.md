...
- **接口**
 ...
  对一条直播流执行**禁用、断流和允许推流操作**。禁用，表示不能再继续使用该流id推流；如果正在推流，则推流会被中断，中断后不可再次推流。断流表示中断正在推的流，断流后可以再次推流。允许推流表示启用该流id，允许用该流id推流。
...
| Param.n.status | 开关状态 | int | 0表示关闭； 1表示开启；2表示断流 | Y|

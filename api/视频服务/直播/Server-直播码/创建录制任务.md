...
| Param.s.end_time | 任务结束时间 | string   |标准的date_time，需要urlencode，如2017-01-01%2010:10:01。任务开始时间和结束时间差不可超过30分钟。  | Y |
|Param.n.task_sub_type  | 是否开启实时视频录制| int| 默认0，1表示开启实时视频录制。
（1）若开启实时视频录制，调用接口则同步开始录制，此时如果传入任务开始时间参数，任务开始时间参数无效。
（2）开启实时视频录制的同时如果传入了任务结束时间，则按照任务结束时间结束录制，若没传入，则30分钟后自动结束录制。
（3）开启实时视频录制的同时如果传入了任务结束时间，实时录制时间与任务结束时间超过30分钟，则30分钟后会自动结束录制。|N|

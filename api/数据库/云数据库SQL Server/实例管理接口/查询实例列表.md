...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见<a href='/doc/api/449/6436' title='公共请求参数'>公共请求参数</a>页面。其中，此接口的Action字段为GetInstanceList。
...
| regionId | 否 | Int | 地域ID，请使用[查看地域ID列表](/doc/api/253/1266)  |
| zoneId | 否 | Int | 可用区ID，请使用[查看可用区ID列表](/doc/api/253/1266)|
| status | 否 | Int | 实例状态值，请使用[查看实例状态列表](/doc/api/253/1266) |
...
| status | Int | 实例状态，请使用[查看实例状态列表](/doc/api/253/1266) |
| vip | String | 实例访问ip |
| vport | Int | 实例访问端口 |

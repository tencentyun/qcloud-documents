...
3. 上传前需要对文件内容进行分片，每个分片的大小为2MB。第1个分片的ID为1，第2个分片的ID为2，以此类推... 
4. 每个分片的大小必须是 2MB，最后一个分片的大小可以小于2MB。
5. 上传前须调用<a href='/document/product/236/8377' title='查询导入文件列表'>查询导入文件列表</a>接口，查看是否已经存在同名文件：

...
| nextStatus | String | 正在进行的状态，可能的取值同data.tasks.status | 
| nextMsg | String | 正在进行状态对应的消息 | 
| progress | String | 当前进度的百分比，取值范围为[0-100]| 
| message | String | 任务的错误信息| 
| checkWorkId | String | 校验任务 ID | 
| checkFlag | String |校验标签，0-未检验，1-校验通过，2-校验未通过| 
| stepAll | String | 总共的步骤数量 | 
| stepNow | String | 当前进行的步骤 | 
| taskType | String | 任务类型，0-立刻执行，1-定时执行 | 
| migrateOption | String | 迁移类型，0-迁移结构和数据，1-只迁移结构| 
| startTime | String | 任务开始执行时间 | 
| endTime | String | 任务结束执行时间| 
| updateTime | String | 任务上一次更新状态的时间 | 
| submitTime | String | 任务提交的时间 | 
| srcType | String | 源实例类型：1-CVM实例，2-外网实例，3-VPC实例，10-灾备实例 | 
| srcId | String | 迁移源 ID | 
| uInstanceId | String | 迁移目的地 CDB 实例 ID| 

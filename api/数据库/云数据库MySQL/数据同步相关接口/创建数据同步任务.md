...
1. 通过本接口创建数据同步任务之后，需使用[开始校验数据同步任务](/document/product/236/7931)接口发起校验，主要内容包括：源MySQL和目标实例是否满足同步条件；
2. 发起校验之后，您可以使用[查询数据同步任务校验详情](/document/product/236/7934)接口来查询实时的校验结果；
...
| srcInfo | 是 | Object | 源MySQL实例（主实例）信息|
| dstInfo | 是 | Object | 目的MySQL实例（灾备实例）信息|
...
&jobName=myJobName
&srcInfo.cdbUInstanceId=cdb-c1nl9rpv

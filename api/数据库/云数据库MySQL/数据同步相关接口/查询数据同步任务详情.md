## 1. 接口描述
本接口(GetCdbDataSyncTaskInfo)用于查询数据同步任务信息，返回数据同步任务的详细信息。
接口请求域名：<font style='color:red'>cdb.api.qcloud.com </font>


## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见<a href='/document/product/236/6921' title='公共请求参数'>公共请求参数</a>页面。其中，此接口的Action字段为GetCdbDataSyncTaskInfo。

| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| jobId | 是 | Int | 数据同步任务ID，请使用[DescribeProject](/doc/api/229/1330)接口查询任务ID |
...
| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| jobId | String | 数据同步任务 ID |
| jobName | String | 数据同步任务名称 |
| jobType | String | 任务类型，可能的值有：0-立刻执行；1-定时执行 |
| appId | String | 用户的 AppId | 
| cvmId | String | CVM 实例 ID | 
| srcIp | String | 同步源 IP |
| srcPort | String | 同步源 PORT |
| status | String | 同步状态，可能的返回值：<br>0-初始化<br>1-准备运行中<br>2-运行中<br>3-运行成功<br>4-运行失败<br>5-任务强行中止<br>6-被清理掉<br>提交完成请求 |
...
| migrateOption | String | 同步类型，可能的值有：0-同步结构和数据；1-只同步结构 |
...
| srcId | String | 同步源 ID |
| uInstanceId | String | 同步目的地 CDB 实例短 ID |

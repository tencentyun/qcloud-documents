## 1. 接口描述
本接口(CreateCdb)用于创建包年包月的实例，可通过传入实例规格、MySQL 版本号、购买时长和数量等信息创建云数据库实例。
...
| protectMode | 否 | Int | 数据复制方式，购买主实例时可指定该参数，不填默认为0，购买只读实例或者灾备实例时系统会强制将该参数置为0 |
| deployMode | 否 | Int | 多可用区域，购买主实例时可指定该参数，不填默认为0，购买只读实例或者灾备实例时系统会强制将该参数置为0 |
| slaveZone | 否 | Int | 备库_1区域Id，购买主实例时可指定该参数，不填默认为zoneId的值，购买只读实例或者灾备实例时系统会强制将该参数置为zoneId的值 |
| backupZone | 否 | Int | 备库_2区域Id，购买主实例时可指定该参数，不填默认为0，购买只读实例或者灾备实例时系统会强制将该参数置为0 |

...
| protectMode | 否 | Int | 数据复制方式，购买主实例时可指定该参数，默认为0，购买只读实例或者灾备实例时系统会强制将该参数置为0 |
| deployMode | 否 | Int | 多可用区域，购买主实例时可指定该参数，默认为0，购买只读实例或者灾备实例时系统会强制将该参数置为0 |
| slaveZone | 否 | Int | 备库_1的区域ID，购买主实例时可指定该参数，默认为zoneId的值，购买只读实例或者灾备实例时系统会强制将该参数置为zoneId的值 |
| backupZone | 否 | Int | 备库_2的区域ID，购买主实例时可指定该参数，默认为0，购买只读实例或者灾备实例时系统会强制将该参数置为0 |

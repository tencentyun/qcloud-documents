...
| 参数名称 | 必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| cdbInstanceId | 否 | String | | cdbInstanceId | String | 支持实例长ID和短ID，其中实例长ID为API专用的实例ID（已废弃），实例短ID同云数据库控制台显示的实例ID相同（<font style="color:red">推荐使用</font>）。请使用[查询实例列表](/doc/api/253/1266)接口查询云数据库实例ID。|
...
其中100002的值表示该可用区下支持的实例信息，其参数构成如下：

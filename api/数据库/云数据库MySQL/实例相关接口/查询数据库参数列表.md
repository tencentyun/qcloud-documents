...
| data | Array | 返回数据） |
| data.enum_value | Array | 参数的可选枚举值，如果非枚举参数，则为空| 
| data.max | Int | 最大允许值 | 
| data.min | Int | 最小允许值 | 
| data.name | String | 参数名 | 
| data.need_reboot | Int | 修改参数后是否需要重启DB以使参数生效，0否，1是 | 
| data.type | String | 参数类型，如integer整数、enum枚举、string字符串等 | 

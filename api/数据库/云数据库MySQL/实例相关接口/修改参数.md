...
| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| cdbInstanceIds.n (cdbInstanceIds 为数组，此处入参需要填写数组元素 ) | 是 | String | 一个或者多个实例ID，支持实例长ID和短ID，其中实例长ID为API专用的实例ID，实例短ID同云数据库控制台显示的实例ID相同（<font style="color:red">推荐使用</font>）。请使用[查询实例列表](/doc/api/253/1266)接口查询云数据库实例ID。|

...
| slaveZone | 否 | Int | 备库1的可用区ID，默认为zoneId的值，购买主实例时可指定该参数，购买只读实例或者灾备实例时系统会强制将该参数置为zoneId的值|
| backupZone | 否 | Int | 备库2的可用区ID，默认为0，购买主实例时可指定该参数，购买只读实例或者灾备实例时系统会强制将该参数置为0|

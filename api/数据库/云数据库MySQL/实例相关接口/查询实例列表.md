...
| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| cdbInstanceId | String | 实例长ID，实例唯一标识，API专用的实例ID。实例创建成功后才返回此ID，创建中的实例只返回实例短ID。可用于查询实例信息或管理实例 | 
| uInstanceId | String | 实例短ID，实例唯一标识，同云数据库控制台实例列表页显示的实例ID相同。功能同cdbInstanceId字段。<font style="color:red">推荐使用</font> | 

## 1. 接口描述
本接口(DescribeCdbInstances)用于查询云数据库实例列表，支持通过项目ID、实例UUID/实例ID、访问地址、实例状态等来筛选实例。
接口请求域名：<font style="color:red">cdb.api.qcloud.com</font>

1. 可根据项目ID、实例UUID、实例ID、访问地址或实例状态作为过滤条件来查询实例列表；
...
| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| uInstanceId | String | 实例ID，与云数据库控制台页面显示的实例ID相同，可用于查询或操作实例。<font style="color:red">推荐使用</font> |
| cdbInstanceId | String | 已废弃，推荐使用 uInstanceId 替代。实例UUID，实例创建成功后才生成实例UUID，创建中的实例只包含 uInstanceId。 |

...
4. 接口仅支持查询主实例和灾备实例列表，暂不支持查询只读实例列表，您可使用[查询只读实例列表](/doc/api/253/6417)接口来查询只读实例列表。
...
| status.n | 否 | Int | 实例状态，默认为1，支持传多个状态值，值包括：<br>0-发货中<br>1-正常运行<br>4-销毁中<br>5-隔离中|
...
| status | Int | 实例状态，可能返回值：<br>0-创建中<br>1-运行中<br>2-创建失败<br>4-删除中<br>5-隔离中<br>6-下线中<br>7-已下线 | 

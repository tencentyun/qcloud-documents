...
| status| Int | 只读实例状态，可能返回值：0-创建中；1-运行中；2-创建失败；4 -删除中；5-隔离中；6-下线中；7-已下线|
| vip| String| 只读实例访问IP |
| vport| Int | 只读实例访问端口 |
| vpcId| Int | 只读实例所在私有网络ID |
| subnetId| Int | 只读实例所在私有网络子网ID |
...
| cdbInstanceId | String | 只读实例长ID，只读实例唯一标识，可用于查询实例信息和操作实例 | 
| uInstanceId | String | 只读实例短ID，只读实例唯一标识，功能同cdbInstanceId字段。<font style="color:red">推荐使用</font> | 
| cdbInstanceName | String | 只读实例名称 | 
| status | Int | 只读实例状态，可能返回值：0-创建中；1-运行中；2-创建失败；4 -删除中；5-隔离中；6-下线中；7-已下线 | 
| taskStatus | Int | 只读实例任务状态，可能返回值：0-当前没有任务；1-升级任务；2-数据导入；3-开放slave读；4-外网访问开通中；5-批量操作；6-回档；7-外网访问关闭中；8-修改密码；9-改名；10-参数修改任务；11-初始化任务；12-自建迁移| 
| cdbVersion | String | 只读实例数据库引擎版本，可能返回值：5.5和5.6 | 
| cdbType | String | 只读实例规格描述，如：CUSTOM | 
| typeName | String | 只读实例类型名称，如：高IO版 | 
| initFlag | Int | 只读实例初始化标记，可能返回值：1-已初始化；0-未初始化 | 
| autoRenewFlag | Int | 只读实例自动续费标记，可能返回值：1-自动续费；0-不自动续费 | 

...
| jobId | Int | 任务ID |
| type | Int | 任务类型，可能返回的值：9-只读升级任务；11-主实例或灾备实例任务 |
...
| detail | Object | 任务结束时间，时间格式如：2017-02-05 18:19:08 |
其中， detail 参数的构成如下：
...
| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| memory | String | 升级前的内存大小，单位：MB |
| volume | String | 升级前的硬盘大小，单位：GB |
...
| slaveZoneFirst | String | 升级前的备库_1的区域ID |
| slaveZoneSecond | String | 升级前的备库_2的区域ID |
| cdbType | String | 升级前的实例规格 |
其中， after 参数的构成如下：

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| memory | String | 升级后的内存大小，单位：MB |
| volume | String | 升级后的硬盘大小，单位：GB |
...
| slaveZoneFirst | String | 升级后的备库_1的区域ID |
| slaveZoneSecond | String | 升级后的备库_2的区域ID |
...
```
{
    ...
    "data": {
        ...
        "status": "0",
        "progress": "0",
        "startTime": "2017-02-05 18:19:08",
        "endTime": "",
        "detail": [
            {
                "cdbInstanceId": "cdb-0pzitle8",
                "dealName": "20170205110003",
                "before": {
                    "memory": 360,
                    "volume": 10,
                    "engineVersion": "5.6",

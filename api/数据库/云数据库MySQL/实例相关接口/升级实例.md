...
| protectMode | 否 | Int | 数据复制方式，升级主实例时可指定该参数，升级只读实例或者灾备实例时指定该参数无意义 |
| deployMode | 否 | Int | 多可用区域，升级主实例时可指定该参数，升级只读实例或者灾备实例时指定该参数无意义 |
| slaveZone | 否 | Int | 备库_1区域Id，升级主实例时可指定该参数，升级只读实例或者灾备实例时指定该参数无意义 |
| backupZone | 否 | Int | 备库_2区域Id，升级主实例时可指定该参数，升级只读实例或者灾备实例时指定该参数无意义 |

...
| protectMode | 否 | Int | 数据复制方式，升级主实例时可指定该参数，升级只读实例或者灾备实例时系统会强制将该参数置为0 |
| deployMode | 否 | Int | 多可用区域，升级主实例时可指定该参数，升级只读实例或者灾备实例时系统会强制将该参数置为0 |
| slaveZone | 否 | Int | 备库_1区域Id，升级主实例时可指定该参数，升级只读实例或者灾备实例时系统会强制将该参数置为实例的可用区Id的值|
| backupZone | 否 | Int | 备库_2区域Id，升级主实例时可指定该参数，升级只读实例或者灾备实例时系统会强制将该参数置为0|

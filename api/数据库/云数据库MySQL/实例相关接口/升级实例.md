...
| slaveZone | 否 | Int | 备库1的可用区ID，默认为zoneId的值，升级主实例时可指定该参数，升级只读实例或者灾备实例时系统会强制将该参数置为实例的可用区Id的值|
| backupZone | 否 | Int | 备库2的可用区ID，默认为0，升级主实例时可指定该参数，升级只读实例或者灾备实例时系统会强制将该参数置为0|

## 1. 接口描述
本接口(GetCdbDeviceMonitorInfo)用于查询云数据库物理机当天的监控信息，<font style="color:red">**暂只支持最高配置实例查询**</font>。
...
| 参数名称 | 必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| cdbInstanceId | 是 | String | 支持实例长 ID 和短 ID，其中实例长 ID 为 API 专用的实例 ID（已废弃），实例短 ID 同云数据库控制台显示的实例 ID 相同（<font style="color:red">推荐使用</font>）。请使用[查询实例列表](/doc/api/253/1266)接口查询云数据库实例 ID|

...
创建实例接口结果返回了短订单ID（dealIds）、长订单ID（dealNames）和实例短ID，其中短订单ID（dealIds）用于调用云API，长订单号（dealNames）用于反馈订单问题给官方客服，实例短ID用于管理云数据库实例。您可使用[查询实例列表](/doc/api/253/1266)接口查看刚创建的实例。
上述请求的结果如下：

## 1. API Description
This API (ModifyCdbDataMigrationTask) is used to modify a data migration task. It can only be performed for the non-running tasks. The relevant statuses include Not Configured (status = 0), Configured (status = 1), Verification Failed (status = 3), Verification Succeeded (status = 4), Migration Failed (status = 8) and Migration Succeeded (status = 9).
Domain for API request: <font style='color:red'>cdb.api.qcloud.com </font>

1. After modifying a data migration task with this API, you need to use API [Start Verification of Data Migration Task](/document/product/236/7726) to initiate a verification to verify whether the source MySQL and the destination instance meet the migration conditions.
...
4. If the verification fails, you can use this API (ModifyCdbDataMigrationTask) to further modify the task parameters, and then return to Step 1 to initiate the task verification again.

## 2. Input Parameters
The following request parameter list only provides API request parameters. Common request parameters need to be added when the API is called. For more information, refer to <a href='/doc/api/372/4153' title='Common Request Parameters'>Common Request Parameters</a>. The Action field for this API is ModifyCdbDataMigrationTask, and the Region field for this API is the region value of the destination instance.

| Parameter Name | Required | Type | Description |
|---------|---------|---------|---------|
| jobId | Yes | Int | ID of data migration task to be modified. Please use API "Query Data Migration Task List" to query the task ID |
...
| srcInfo | Yes | Object | Source instance information depending on the migration task type ||
| dstInfo | Yes | Object | Destination instance information. It only includes attribute cdbUInstanceId with a format of cdb-c1nl9rpv. It is identical to the instance ID displayed in the Cloud Database console page and can be obtained via API [Query List of Instances](/doc/api/253/1266). Its value equals the uInstanceId field value in the output parameter. |
...
| dbInfo | Yes | Array | Information on the source database table to be migrated. If you need to migrate the entire instance, this field can be left empty. The composition of each element is shown in the table below |
...
| table | No | Array | The name of the table in the current database. If you need to migrate all the tables in the current database, this field can by left empty |

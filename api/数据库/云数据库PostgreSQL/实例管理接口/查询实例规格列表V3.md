## 1. 接口描述
本接口(DescribeSpecList)用于查询实例规格列表。
接口请求域名：<font style='color:red'>postgres.api.qcloud.com </font>

## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见<a href='/doc/api/238/7328' title='公共请求参数'>公共请求参数</a>页面。其中，此接口的Action字段为DescribeSpecList。
...
输入
<pre>
https://postgres.api.qcloud.com/v3/index.php?Action=DescribeSpecList
&Version=2017-03-20

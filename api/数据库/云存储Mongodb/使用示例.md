...
## 2. 查询副本集实例价格
创建实例会扣除涉及的费用，可以通过[查询副本集实例价格(包年包月)](http://www.qcloud.com/document/product/240/8311)接口查询涉及的费用。 我们这里假设购买一个广州二区高IO版实例， 查询价格的输入参数如下： 
...
## 3. 创建副本集实例
接下来，我们按照上面的规格，调用[创建副本集实例（包年包月）](http://www.qcloud.com/document/product/240/8308)接口, 输入参数如下： 

## 1. 接口描述
本接口（InquiryMongoDBReplSetPrice）用于获取副本集实例价格(包年包月),支持新购、续费、升级实例时价格查询。
...
| zoneId | 是 | Int | 可用区ID，请使用[查询副本集实例价格(包年包月)](http://www.qcloud.com/document/product/240/8311)接口获取可创建的可用区 |
...
| instanceId | 是 | String | 待操作的实例ID，可通过 [DescribeMongoDBReplSet](http://www.qcloud.com/document/product/240/8312) 接口返回值中的 instanceId 获取。|

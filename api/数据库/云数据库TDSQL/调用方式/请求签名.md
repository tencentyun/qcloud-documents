...
SecretId： AKIDz8krbsJxxxLPx3gnPhESA
SecretKey： Gu5t9xGARNxxxCN3Cozk1qA
...
| SecretId | 密钥 ID  | AKIDz8krbsJxxxLPx3gnPhESA | 
...
```php
$secretKey = 'Gu5t9xGARNxxxCozk1qA';
$srcStr = 'GETcvm.api.qcloud.com/v2/index.php?Action=DescribeInstances&InstanceIds.0=ins-09dx96dg&Nonce=11886&Region=ap-guangzhou&SecretId=AKIDz8krbsJxxxLPx3gnPhESA&SignatureMethod=HmacSHA256&Timestamp=1465185768';
...
```
0EEm/HtGRr/VJXTAD9tcxxxv1GdM8s=
```

同理，当您指定签名算法为 **HmacSHA1** 时，生成签名串的代码如下：
```php
$secretKey = 'Gu5t9xGARNxxxCN3Cozk1qA';
$srcStr = 'GETcvm.api.qcloud.com/v2/index.php?Action=DescribeInstances&InstanceIds.0=ins-09dx96dg&Nonce=11886&Region=ap-guangzhou&SecretId=AKIDz8krbsJxxxLPx3gnPhESA&SignatureMethod=HmacSHA1&Timestamp=1465185768';
...
如上一步生成的签名串为`0EEm/HtGRr/VJXTAD9tcxxxv1GdM8s=`，则其编码后为`0EEm%2FHtGRr%2FVJXTAD9tcxxxv1GdM8s%3D`。因此，最终得到的签名串请求参数 (Signature) 为：`0EEm%2FHtGRr%2FVJXTAD9tcxxxv1GdM8s%3D`，它将用于生成最终的请求URL。

## 1. 接口描述
本接口（CloseTdsqlExtranetAccess）用于关闭外网地址。
接口请求域名：**_tdsql.api.qcloud.com_**

## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](/doc/api/309/7016)页面。其中，此接口的 Action 字段为 CloseTdsqlExtranetAccess。
...
| FlowId | Integer | 任务流程号。如果外网地址已经关闭，则不会生成新的流程| 
## 4. 错误码表
...
输入
<pre>
https://tdsql.api.qcloud.com/v2/index.php?Action=CloseTdsqlExtranetAccess

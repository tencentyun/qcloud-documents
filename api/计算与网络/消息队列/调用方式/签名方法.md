...
由上表可以看出，请求参数中的公共请求参数只有5个：Action、SecretId、Timestamp、Nonce 和 Region，而不是在“公共请求参数”中所述的6个，事实上，第 6 个参数 Signature（签名串）正是由其他参数（包括指令请求参数）共同生成的，具体步骤如下：
### 2.1. 对参数排序
...
如上一步生成的签名串为 `gb1xVzX634nRQnsdgRH4SqurJSg=`，则其编码后为 `gb1xVzX634nRQnsdgRH4SqurJSg%3d`。因此，最终得到的签名串请求参数(Signature)为：`gb1xVzX634nRQnsdgRH4SqurJSg%3d`，它将用于生成最终的请求 URL。

...
### 订阅消息示例
如下为订阅消息 Demo，投递消息均为 POST 方法，因此只需重写 do_POST 方法即可。   本示例会打印收到的 HTTP POST 请求内容，并将 post data json 反序列化，最终遍历打印请求数据。
```
#!/usr/bin/python
...
def start_server(port):
	http_server = HTTPServer(('10.104.3.102', int(port)),TestHTTPHandle)

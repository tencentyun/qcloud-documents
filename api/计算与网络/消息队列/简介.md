...
| (Batch)ReceiveMessage | (Batch) Receive Message | 消费消息 | ReceiveMessage操作仅仅会把消息从active状态置为inactive状态，使得该消息不能被其他消费者消费到，这样做是为了防止异常情况下，用户至少能够消费一次消息。但消费完后要显式调用(Batch)DeleteMessage操作才能把消息从队列中删除掉，否则消息过了visibilityTimeout指定的时间后，重新变为active状态，又可被其他消费者消费到，造成意料之外地重复消息。|

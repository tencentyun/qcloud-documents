...
| isVpcGateway | Yes | Int | Whether it is [Internet Gateway](https://intl.cloud.tencent.com/document/product/215/8119) or not. 0: Non-Internet gateway; 1: Internet gateway. The Internet gateway can be used only when the instance has an Internet IP and is in VPC. |
| storageSize | Yes | Int | Data disk size (GB). The increment is 10. The value of 0 means that no data disk is needed. For the maximum size of different disks, please see API [Overview of Hard Disk Products](https://intl.cloud.tencent.com/document/product/213/4953). |

...
* <font style="color:red">当前云主机状态为正常或关机，才能调用此接口。</font>具体可以参考 [查看实例列表](/doc/api/213/831) 页面中的实例状态列表。
...
| password| 实例密码。未设置则为随机产生，并通过站内信下发。linux 实例密码必须 8 - 16 位，至少包括两项 [ a - z，A - Z ]、[ 0 - 9 ]  和  [ ( )  & # 96；~ ! @ # $ % ^ & * - + = & #124； { }  [ ] ： ； ' < > ，. ?  /  ]中的特殊符号。Windows实例密码必须 12 - 16位，至少包括三项 [ a - z ]，[ A - Z ]，[ 0 - 9 ] 和 [ ( )  & #96； ~ ! @ # $ % ^ & * - + = { } [ ] ： ； ' ， . ? / ] 中的特殊符号|String|否|  

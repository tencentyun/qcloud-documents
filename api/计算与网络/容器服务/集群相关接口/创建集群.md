...
| ignoreClusterCIDRConflict | 否| Int|是否忽略clusterCIDR冲突错误，1：忽略冲突（继续创建），0：不忽略冲突（并返回错误），默认为0。|
| zoneId| 是| String|可用区。请填写[查询可用区](https://www.qcloud.com/document/api/213/9455)接口中返回的Zone字段。|
| goodsNum| 是| Int|购买节点数量。最大100|
...
| cvmType| 否| String|节点的类型。<br>PayByHour：按量计费  <br>PayByMonth：包年包月<br>默认按量计费<br>|
|bandwidthType| 是| String|带宽的类型。<br>包年包月主机：PayByMonth：按带宽使用时长计费，PayByTraffic：按流量计费，PayByPackage：按带宽包计费。<br> 按量计费主机：PayByHour：按带宽使用时长计费，PayByTraffic：按流量计费，PayByPackage：按带宽包计费。<br>网络计费模式的区别可以参看[购买网络带宽](https://www.qcloud.com/doc/product/213/509)。|
| bandwidth| 是| Int| 公网带宽(Mbps)，当按流量计费时为公网带宽峰值,如果是带宽包用户，当该值为65535时表示流量不做限制 |
...
| rootSize| 是| Int| 系统盘大小（GB）。<br>linux系统调整范围为20-50G，步长为1。|
| storageSize| 是| Int| 数据盘大小(GB)。步长为10，0表示不要数据盘。关于磁盘的最大大小请参考[硬盘产品简介](https://www.qcloud.com/doc/product/213/498)。|
| password| 否| String| 节点密码。未设置则为随机产生，并通过站内信下发。节点密码必须8到16位，至少包括两项[a-z，A-Z]、[0-9] 和 [( ) &#96; ~ ! @ # $ % ^ & * - + = &#124; { } [ ] : ; ' < > , . ? / ]中的特殊符号。|
| keyId| 否| String| [密钥](https://www.qcloud.com/doc/product/213/503)ID。关联密钥后可使用密钥登录节点，keyId 可通过接口[查询密钥](http://www.qcloud.com/doc/api/229/%E6%9F%A5%E8%AF%A2%E5%AF%86%E9%92%A5)获取，密钥与密码不能同时指定，同时 Windows 操作系统不支持指定密钥。|
...
|mountTarget|否|String|数据盘要挂载到主机的目录|
|dockerGraphPath|否|String|docker graph目录|

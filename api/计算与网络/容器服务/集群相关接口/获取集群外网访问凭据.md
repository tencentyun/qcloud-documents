...
>
## 接口描述
本接口（DescribeClusterSecurityInfo）用于获取集群外网访问凭据。
...
| userName | 用于登录集群的用户名 | String|
| password | 用于登录集群的密码 | String|
| certificationAuthority | 集群 CA 证书 | String |
| clusterExternalEndpoint | 外网访问地址 | String |

## 示例
### 输入
```
 https://domain/v2/index.php?Action=DescribeClusterSecurityInfo
  &clusterId=cls-xxxxx
  &其它公共参数
```
### 输出

...
|ip|IP过滤，支持外网和内网IP，支持模糊查询，默认为空，不过滤|String|否|
...
|lanIp|实例内网IP|String|
|wanIp|实例外网IP|String|
|instanceId|实例ID查询，实例ID形如：ins-11112222|String|
|instanceCreateTime|实例创建时间，形如：2018-05-30 23:30:35|Date|
|usable|是否可用|Bool|
|unusableReason|不可用原因描述：“already in Cluster”：已经添加到集群中，过滤掉、“cvm status error”：CVM状态不是运行中，过滤掉、“windows and Local disk”：cvm限制 windows 本地盘不支持重装，过滤掉|String|

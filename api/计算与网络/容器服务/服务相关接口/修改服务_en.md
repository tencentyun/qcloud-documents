...
| delayTime    | Int    | After the container starts up, the time to wait before enabling health check (in second). Default is 0 (enable immediately). If ``type`` is ``readyCheck``, the container is considered unready within the delayTime period after it launches, during which requests are not forwarded to this container when you access the corresponding service  |

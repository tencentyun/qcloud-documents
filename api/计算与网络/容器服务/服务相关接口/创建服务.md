...
|---------|---------|---------|---------|
| name   | 是 | String      |容器卷名称|
| volumeType   | 是 | String      |容器卷类型，目前支持hostPath、cbsDisk和nfsDisk， **注意：对于cbsDisk类型，服务的实例数目只能是1，因为cbs盘不支持多个节点同时挂载**|
...
| nfsPath | 否 | String | 当volumeType为nfsDisk时必填。此字段为挂载nfs盘的路径，如 127.0.0.1:/exports|
...
| privileged | 否 | Bool |容器启动参数，传入true为开启，false为关闭，区分大小写<br>详细解释可参考[服务运行命令和参数设置](https://www.qcloud.com/document/product/457/9100)|
| workingDir | 否 | String |容器启动参数<br>详细解释可参考[服务运行命令和参数设置](https://www.qcloud.com/document/product/457/9100)|

...
>如：Node1的label是：NodeType = S1.SMALL1， Node2的label是：NodeType = S1.SMALL2， Node3的label是：NodeType =  S1.MEDIUM2. 创建服务是指定nodeAffinity参数是 NodeType In {S1.SMALL1, S1.SMALL2}, 那么该服务下的容器仅在Node1和Node2下调度。

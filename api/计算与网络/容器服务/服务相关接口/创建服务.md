...
| nodeAffinity |  节点亲和性，不支持1.4.6集群 | Object Array  |否 |
| podAffinity |  Pod亲和性，不支持1.4.6集群 | Object Array  |否 |
...
| memory | 容器最大能使用内存的量 ( 单位：MiB )，建议最小为 4 MiB<br>不传参数或传0表示不做限制，需求量与最大使用量默认一致<br>详情请参考 [服务资源限制设置](/doc/product/457/9099) | Int |否 |
...
节点亲和性调度使用kubernetes的nodeAffinity硬调度(requiredDuringSchedulingIgnoredDuringExecution)方式调度，当Pod发现没有满足条件的节点可以供调度使用时，会出现调度失败的情况
...
Pod亲和性调度使用kubernetes的podAffinity硬调度(requiredDuringSchedulingIgnoredDuringExecution)方式调度，当Pod发现没有满足条件的节点可以供调度使用时，会出现调度失败的情况

...
| 参数名称 | 描述 | 必选  | 类型 |
|---------|---------|---------|---------|
| clusterId   |集群ID，请填写[查询集群列表](https://www.qcloud.com/document/api/457/9448)接口中返回的clusterId字段。| 否    | String |
| autoScalingGroupId   |伸缩组ID。| 否    | String |
| status   |伸缩组状态, 状态列表参见下文。| 否    | String |
|offset| 偏移量,默认0| 否| Int|
|limit| 最大输出条数，默认20| 否| Int|
...
| 字段 |描述 | 类型|
|---------|---------|---------|
|autoScalingGroupId |伸缩组ID |String|
...
|minSize|伸缩组最小值|Int|
|maxSize|伸缩组最大值|Int|
|instanceNum|伸缩组内的cvm数量|Int|
|desiredCapacity|伸缩组内期望的cvm数量|Int|
|label |伸缩组内cvm的label |Array|
|launchConfigurationId |启动配置ID |String|

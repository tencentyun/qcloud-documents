...
| instanceId   |参考cvm，必须是集群内的节点。请填写[查询集群节点列表](https://www.qcloud.com/document/api/457/9449)接口中返回的instanceId字段。会根据此节点的配置（包括vCPU、内存、机型、VPC、子网、系统盘、数据盘类型和大小、带宽、带宽计费模式、是否分配公网IP等）生成扩容时的启动配置| 是    | String |
|minSize|伸缩组最小值|是|Int|
|maxSize|伸缩组最大值，伸缩组的最大值弹性伸缩本身有限制，详见弹性伸缩说明文档。|是|Int|
| password| 节点密码。未设置则为随机产生，并通过站内信下发。密码必须8到16位，至少包括两项[a-z，A-Z]、[0-9] 和 [( ) &#96; ~ ! @ # $ % ^ & * - + = &#124; { } [ ] : ; ' < > , . ? / ]中的特殊符号| 否| String|

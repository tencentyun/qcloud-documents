...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见 [公共请求参数页面](/document/product/386/6718)。其中，此接口的 Action 字段为 ModifyBmForwardLocation。
...
rules 描述当前监听器的具体信息，n 为下标，rules 包含字段如下

| 参数名称 | 必选  | 类型 | 描述 |
|---------|---------|---------|---------|
|rules.n.domainId|是|String|转发域名实例 ID，可通过接口 [DescribeBmForwardRules](/document/product/386/9285) 查询。|
|rules.n.locationId|是|String|转发路径实例 ID，可通过接口 [DescribeBmForwardRules](/document/product/386/9285) 查询。|
...
| -12003 | InvalidForwardRule.NotExist | CCDB 中不存在该转发规则 |

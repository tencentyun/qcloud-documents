...
为了避免出现IP冲突导致网络异常的情况，当您需要使用某些子网下的特定的IP资源时（物理机IP除外），需要调用此接口进行IP注册。
如果指定的IP资源（事先知道了具体要使用哪些IP）在可用IP资源列表中，则会对该IP资源进行注册，否则，对已经调用ApplyIps接口申请或者调用RegisterBatchIp接口注册的IP资源进行注册，会返回IP资源冲突错误，除非您把IP资源退还了。
已经注册的IP资源，可用IP资源列表中将不再返回此IP。

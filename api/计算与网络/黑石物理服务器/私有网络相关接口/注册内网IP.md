## 功能描述
RegisterBatchIp 用于指定子网 IP 进行 IP 注册。

为了避免出现 IP 冲突导致网络异常的情况，当您需要使用某些子网下的特定的IP资源时（物理机 IP 除外），需要调用此接口进行 IP 注册。
如果指定的 IP 资源（事先知道了具体要使用哪些 IP）在可用 IP 资源列表中，则会对该 IP 资源进行注册，否则，对已经调用 ApplyIps 接口申请或者调用RegisterBatchIp 接口注册的 IP 资源进行注册，会返回 IP 资源冲突错误，除非您把 IP 资源退还了。
已经注册的 IP 资源，可用IP资源列表中将不再返回此 IP。

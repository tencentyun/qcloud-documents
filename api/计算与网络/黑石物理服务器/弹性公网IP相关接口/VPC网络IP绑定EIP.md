...
| 参数名称 | 类型 | 描述 | 必选 |
|---------|---------|---------|---------|
| eipId | String | EIP实例ID，格式形如：eip-testid | 是 |
| vpcId | Int | IP所属的VPC的ID，可通过[查询私有网络列表](/document/product/386/6646)返回的字段vpcId获得| 是 |
| vpcIp | String | VPC内IP，此IP地址必须通过[申请内网IP接口](/document/product/386/7337)申请获得或者通过[注册子网IP](/document/product/386/7925)接口注册，否则无法绑定EIP| 是 |

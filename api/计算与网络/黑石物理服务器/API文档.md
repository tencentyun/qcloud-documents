...
本文档主要介绍如何调用黑石物理服务器 API， 并给出调用示例。您可通过API创建、销毁、重启黑石物理服务器等，所有API列表请对参考。[API概览页](/api/372/API%E6%A6%82%E8%A7%88)。
...
| 术语 | 全称  | 中文 | 说明 |
|---------|---------|---------|---------|
| Instance | Instance |[实例](https://www.qcloud.com/doc/product/213/501#1.-.E9.95.9C.E5.83.8F.E7.AE.80.E4.BB.8B) | 指代一台黑石物理服务器。
...
1. 调用[BuyDevice](), 传入zoneId，deviceClassCode等参数，即可购买一台黑石物理服务器
2. 调用[EipBmApply](),传入goodsNum，payMode等参数，即可申请与goodsNum数量相符的eip
3. 调用[EipBmBindRs]()，传入eipId，instanceId等参数，即可以将指定的EIP绑定到指定的服务器上

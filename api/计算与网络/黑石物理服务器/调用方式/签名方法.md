...
以[绑定服务器EIP]()(EipBmBindRs)请求为例，当用户调用这一接口时，其请求参数可能如下:
...
2) 请求主机:[绑定服务器EIP]()(EipBmBindRs)的请求域名为：eip.api.qcloud.com。实际的请求域名根据接口所属模块的不同而不同, 详见各接口说明。
...
如上一步生成的签名串为Bb7vNsvjCnRUOVInFHD8UVw2O4c=，则其编码后为Bb7vNsvjCnRUOVInFHD8UVw2O4c%3D。因此，最终得到的签名串请求参数(Signature)为：Bb7vNsvjCnRUOVInFHD8UVw2O4c%3D，它将用于生成最终的请求URL。

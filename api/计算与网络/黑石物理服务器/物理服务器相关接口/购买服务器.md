...
| passwd             | 否    | String | 用户设置的linux root或Windows Administrator密码。密码校验规则:：<li> Windows机器密码需12到16位，至少包括三项（[a-z],[A-Z],[0-9]和[()\`~!@#$%^&*-+=_&#124;{}[]:;'<>,.?/]的特殊符号）; <li> Linux机器密码需8到16位，至少包括两项（[a-z,A-Z],[0-9]和[()\`~!@#$%^&*-+=_&#124;{}[]:;'<>,.?/]的特殊符号）。 |

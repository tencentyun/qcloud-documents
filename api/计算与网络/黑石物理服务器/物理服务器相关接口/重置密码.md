 ...
 * 本接口是异步操作，需要通过另外接口查询操作完成状态[DescriptionOperationResult]()。
 * 接口支持批量修改服务器密码
...
| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| instanceIds.n| 是 | String| 待操作的服务器ID。可通过 [DescribeDeviceList]() API返回值中的 instanceId 获取（此接口支持同时传入多个ID。此参数的具体格式可参考API[简介]()的`id.n`一节）。|

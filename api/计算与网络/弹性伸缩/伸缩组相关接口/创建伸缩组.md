...
| subnetIds.n  | 否 | String |伸缩组的子网Id。当vpcId不为0时，此参数必传。可调用<a href="/doc/api/245/查询子网列表" title="查询子网列表">查询子网列表</a>(DescribeSubnetEx)接口查询。|
| zoneIds.n | 否 | String |伸缩组所在地域Id。当vpcId为0时，此参数必传。可调用<a href="/doc/api/404/4433" title="查询可用区">查询可用区</a>(DescribeAvailabilityZones)接口查询。|
...
| InsufficientBalance | 账户余额不足，无法创建伸缩组 |

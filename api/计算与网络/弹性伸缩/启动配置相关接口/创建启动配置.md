...
| mem | 是 | Int | 内存大小，单位为GB，各个地域可选的内存大小不一样，详情见下表。|
| storageType | 是 | Int | 数据盘类型。只有两种取值：若为1，表示本地盘；若为2，表示云硬盘。|
| storageSize | 是 | Int | 数据盘大小，单位为GB。若为本地盘，可选范围在0-500G；若为云硬盘，可选范围在0-4000G。|
...
目前启动配置仅支持标准型云主机，启动配置支持云主机的CPU和内存大小见下表：

...
| cpu | 是 | Int | CPU核数，各个地域可选的CPU核数不一样，详情见文档[CVM实例配置](https://www.qcloud.com/doc/product/213/2177)。|
| mem | 是 | Int | 内存大小，单位为GB，各个地域可选的内存大小不一样，详情见文档[CVM实例配置](https://www.qcloud.com/doc/product/213/2177)。|
...
目前启动配置支持标准型、高IO型、内存型三种云主机（以每个地区实际开放机型为准），启动配置支持云主机的CPU和内存大小详情见[CVM实例配置](https://www.qcloud.com/doc/product/213/2177)。
...
|InvalidSecurityGroupId|安全组Id不正确|
|InvalidParameter.Conflict|1C1G只能配置云盘|
|InvalidParameter.storageType|存储类型错误|
|InvalidParameter.storageSize|数据盘大小超出范围|
|InvalidParameter.rootSize|系统盘大小超出范围|
|Conflict.cpuVsMem|此cpu和mem的机型已售空或不存在|

...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，详情请参见 <a href="https://cloud.tencent.com/document/product/215/4772" title="公共请求参数">公共请求参数</a> 页面。其中，此接口的 Action 字段为 DescribeAccountVpcAttributes。
 
...
| data.supportedPlatforms | String | only-vpc：默认 VPC 用户，默认 VPC 用户所有带网络属性的云资源都是在私用网络下的；classic：基础网络用户，基础网络用户所有带网络属性的资源都是在基础网络下的。|

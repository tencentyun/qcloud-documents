...
本接口（UpgradeNatGateway）用于升级 NAT 网关规格。
接口请求域名：vpc.api.qcloud.com


## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，详情请参见<a href="https://cloud.tencent.com/document/product/215/4772" title="公共请求参数"> 公共请求参数 </a>页面。其中，此接口的 Action 字段为 UpgradeNatGateway。

| 参数名称 | 必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| vpcId | 是 | string | 私有网络 ID 或者统一 ID，建议使用统一 ID，例如 vpc-xdf54d|
| natId | 是 | string | 高可用网关统一 ID，例如 nat-xdf54d |
...
| message | String | 错误信息 |
| billId | string | 订单 ID，升级结果可调用<a href="https://cloud.tencent.com/doc/api/245/%e6%9f%a5%e8%af%a2NAT%e7%bd%91%e5%85%b3%e7%9a%84%e7%94%9f%e4%ba%a7%e7%8a%b6%e6%80%81?viewType=preview" title="查询NAT网关的升级状态"> 查询 NAT 网关的升级状态 </a>查询 |

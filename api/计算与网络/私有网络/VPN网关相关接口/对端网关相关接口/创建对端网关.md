...
| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| userGwName | 是 | String | 对端网关名称，可任意命名，但不得超过60个字符。同一个开发商下名称不可重复。 |
...
| InvalidUserGw.InUse | 对端网关公网IP已被使用。同一个账户下对端网关公网IP不能重复。 |
| UserGwLimitExceeded | 已经达到指定区域对端网关资源申请数量上限。如果需要更多资源，请联系客服申请。更多vpc资源限制信息详见<a href="https://www.qcloud.com/doc/product/215/537" title="VPC使用限制">VPC使用限制</a>。 |

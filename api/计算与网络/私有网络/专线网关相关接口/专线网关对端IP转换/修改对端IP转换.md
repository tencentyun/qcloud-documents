...
本接口（ModifyPeerIPTranslationNatRule）用于修改专线网关对端 IP 转换规则。
接口请求域名：vpc.api.qcloud.com

本接口用于修改指定专线网关的对端 IP 转换规则。

## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，详情请参见<a href="https://cloud.tencent.com/document/product/215/4772" title="公共请求参数"> 公共请求参数 </a>页面。其中，此接口的 Action 字段为 ModifyPeerIPTranslationNatRule。
...
| InvalidOriginalIP.Duplicate | 无效的原始 IP， 原始IP重复。原始 IP 已经在该网关的本端 IP 专线规则中，原始 IP 不可重复。|

...
| DiskIds.N | array of Strings | 否 | 按照一个或者多个云硬盘ID查询。云硬盘ID形如：`disk-11112222`。此参数的具体格式可参考API简介的id.N一节）。参数不支持同时指定`DiskIds`和`Filters`。 |
| Filters.N | array of Filter objects | 否 | 过滤条件，详见云硬盘过滤条件表。参数不支持同时指定`DiskIds`和`Filters`。 |
| Offset | Integer | 否 | 偏移量，默认为0。关于`Offset`的更进一步介绍请参考 API 简介中的相关小节。 |
| Limit | Integer | 否 | 返回数量，默认为20，最大值为100。关于`Limit`的更进一步介绍请参考 API 简介中的相关小节。 |
...
| portable | Integer | 否 |（过滤条件）按是否为弹性云盘过滤。取值范围：<br><li>1表示弹性云盘<br><li>0表示非弹性云盘。 |
...
| disk-type | String | 否 |（过滤条件）按照云盘介质类型过滤。取值范围：<br><li>CLOUD_BASIC：表示普通云硬盘<br><li>CLOUD_PREMIUM：表示高效云硬盘<br><li>CLOUD_SSD：SSD表示SSD云硬盘。|
...
| assigned | Integer | 否 |（过滤条件）按照云盘是否挂载到云主机上过滤。取值范围：<br><li>0表示未挂载<br><li>1表示已挂载。 |

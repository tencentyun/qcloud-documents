...
3. 登录云服务器初始化弹性云盘：初次使用新创建的弹性云盘时，需要进行分区、格式化等一系列操作。具体操作方式请见：[初始化场景介绍](https://cloud.tencent.com/document/product/362/33065)。注意，对于linux系统，分区并不是必要的，您可以跳过分区直接进行格式化操作。

...
* Other fields than the fixed ones are defined by specific APIs. For more information on the fields returned by specific APIs, please see the API documentation. In this example, TotalCount and InstanceStatusSet are defined by the API DescribeInstancesStatus. Because the user who initiated the request does not have a CVM instance, the returned value for TotalCount is 0 and the InstanceStatusSet list is empty.

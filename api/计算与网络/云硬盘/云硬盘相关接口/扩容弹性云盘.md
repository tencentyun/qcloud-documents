...
使用限制：<br>
1. 只支持扩容弹性云盘。云硬盘类型可以通过 [DescribeCbsStorages（查询云硬盘信息）](https://cloud.tencent.com/doc/api/364/2519)接口查询，见输出参数中`portable`字段解释。随云服务器创建的云硬盘需通过 [ResizeInstance（调整实例配置（包年包月））](/doc/api/229/1306) 或 [ResizeInstanceHour（调整实例配置（按量计费））](/doc/api/229/1344) 接口扩容。

...
| 名称      |    类型 |  是否必选 | 描述|
|---------|---------|---------|---------|
|VpcId|String|是|私有网络ID。该参数可以通过调用[DescribeVpcEx](https://www.qcloud.com/document/api/215/1372)的返回值中的unVpcId字段来获取。|
|SubnetId|String|是|私有网络子网ID。该参数可以通过调用[DescribeSubnetEx](https://www.qcloud.com/document/api/215/1371)的返回值中的unSubnetId字段来获取。|

...
<br>
> **注意：**<br>
> 本模块底下的云服务器接口，均为改版后的API接口，后续云服务器相关的所有新增功能都会在此模块底下的接口里面增加，<font face="微软雅黑" color=red size=5>推荐使用</font>。现有旧接口的功能依然保持，对于使用旧接口的用户详情可参见：[云服务器API（旧接口）简介](/document/api/213/568)。
...
| Region | Region |[地域](/document/product/213/497#1.-.E5.9C.B0.E5.9F.9F) | 表示资源所在的地域，每个地域包含一个或多个可用区。|
| Zone | Zone |[可用区](/document/product/213/497#2.-.E5.8F.AF.E7.94.A8.E5.8C.BA) | 指腾讯云在同一地域内电力和网络互相独立的物理数据中心。目标是能够保证可用区间故障相互隔离（大型灾害或者大型电力故障除外），不出现故障扩散，使得用户的业务持续在线服务。|
| Instance | Instance |[实例](/doc/product/213/4939) | 指代一台云服务器。|

...
注：本接口为改版后的API接口。如需了解旧接口相关信息，请参考：[SyncImages(旧版)](/document/api/213/1336)
...
| ImageIds.N |  array of Strings |是 | 镜像ID ，镜像ID可以通过[DescribeImages](/document/api/213/9418)获取；必须满足限制：<br><li>镜像ID对应的镜像状态必须为`NORMAL`，<br><li>镜像大小小于50GB。<br>镜像状态请参考[镜像数据表](/document/api/213/9452#image_state)。
| DestinationRegions.N |  array of String |是 | 目的同步地域；必须满足限制：<br><li>不能为源地域<br><li>必须是一个合法的Region<br><li>暂不支持部分地域同步<br>具体地域参数请参考[Region](/document/product/213/6091)
...
请求参数
<pre>
https://image.api.qcloud.com/?Action=SyncImages
&Version=2017-03-12

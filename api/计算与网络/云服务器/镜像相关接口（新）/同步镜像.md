...
| DestinationRegions |  array of String |是 | 需要同步到的地域，值不能和公共请求参数中的 Region 相同（即不能与源地域相同） ；必须满足下列限制：<br> * 必须是一个合法的Region<br> * 不能是当前地域<br> * 某些地域暂不支持同步。
...
|InvalidRegion.NotFound|无效的Region:找不到对应的Region
|InvalidRegion.Unavailable|无效的Region: 此Region当前不可用

...
| ImageIds.N |  array of Strings |是 | 镜像ID列表 ，镜像ID可以通过[DescribeImages](/document/api/213/9418)获取；必须满足下列限制： <br><li>镜像ID对应的镜像状态必须为`NORMAL`，<br><li>镜像ID对应的镜像没有共享给其它账户。<br>镜像状态请参考[镜像数据表](/document/api/213/9452#image_state)。

...
注：本接口为改版后的API接口。如需了解旧接口相关信息，请参考：[DeleteImages(旧版)](/document/api/213/1274)

本接口（DeleteImages）用于删除一个或多个镜像

接口请求域名：<font style="color:red">image.api.qcloud.com</font>

* 当[镜像状态](/document/api/213/9452#image_state)为`创建中`和`使用中`时, 不允许删除。
...
| ImageIds |  array of Strings |是 | 镜像ID ，镜像ID可以通过[DescribeImages](/document/api/213/9418)获取；必须满足下列限制： <br><li>镜像ID对应的镜像状态必须为`NORMAL`，<br><li>镜像ID对应的镜像没有共享给其它账户。<br>镜像状态请参考[镜像数据表](/document/api/213/9452#image_state)。
...
请求参数
<pre>
https://image.api.qcloud.com/?Action=DeleteImages
&Version=2017-03-12

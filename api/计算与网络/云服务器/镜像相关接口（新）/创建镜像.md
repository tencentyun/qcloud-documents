...
注：本接口为改版后的API接口。如需了解旧接口相关信息，请参考：[CreateImage(旧版)](/document/api/213/1273)
...
| InstanceId |  String |是 | 用于制作镜像的实例ID 。
| ImageName |  String |是 | 镜像名称；需要满足下列要求：<br><li>合法的镜像名称，<br><li>不得超过20个字符，<br><li>镜像名称不得重复。
| ImageDescription |  String |否 | 镜像描述；需要满足下列要求：<br><li>合法的镜像描述，<br><li>不得超过60个字符。<br>不指定该参数时镜像描述为空。
...
请求参数
<pre>
https://image.api.qcloud.com/?Action=CreateImage
&Version=2017-03-12

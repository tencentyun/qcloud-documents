...
注：本接口为改版后的API接口。如需了解旧接口相关信息，请参考：[重启实例](https://www.qcloud.com/document/api/213/1247)。
...
* 支持强制重启。强制重启等同于传统断电重启，可能导致实例操作系统未写入磁盘的数据丢失等情况，详细请见[强制重启]()。
...
| ForceReboot| Boolean| 否 |是否在正常重启失败后选择强制重启实例。取值范围：<br><li>TRUE：表示在正常重启失败后进行强制重启<br><li>FALSE：表示不在正常重启失败后进行强制重启<br><br>默认取值：FALSE。更多信息详见：强制重启存在的隐患。|

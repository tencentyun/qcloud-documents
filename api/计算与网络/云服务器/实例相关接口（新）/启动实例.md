...
注：本接口为改版后的API接口。如需了解旧接口相关信息，请参考：[启动实例](/document/api/213/1249)。
...
* 支持批量操作。每次请求批量实例的上限为100。在执行批量启动实例操作前，如果存在不允许操作的实例，操作会以特定[错误码](/document/api/213/9386#4.-.E9.94.99.E8.AF.AF.E7.A0.81)返回。

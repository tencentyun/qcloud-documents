...
* 只支持主动退还按量计费类型的实例。包年包月实例如有误创建需要退还的情况通过[工单系统](https://console.qcloud.com/workorder/category/create?level1_id=6&level2_id=7)提工单由后台运维人员进行支持。
* 支持批量操作，将多个按量付费的实例退还。每次请求批量实例的上限为100。如果批量实例存在不允许操作的实例，操作会以特定[错误码](https://www.qcloud.com/document/api/213/9388#4.-.E9.94.99.E8.AF.AF.E7.A0.81)返回。

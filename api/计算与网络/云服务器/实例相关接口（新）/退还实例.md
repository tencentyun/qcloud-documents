...
注：本接口为改版后的API接口。如需了解旧接口相关信息，请参考：[退还实例（按量计费）](/document/api/213/1347)。
...
* 支持批量操作，将多个按量付费的实例退还。每次请求批量实例的上限为100。如果批量实例存在不允许操作的实例，操作会以特定[错误码](/document/api/213/9388#4.-.E9.94.99.E8.AF.AF.E7.A0.81)返回。

...
|InstanceType|String|否|实例机型。不同实例机型指定了不同的资源规格，具体取值可通过调用接口[DescribeInstanceTypeConfigs](https://www.qcloud.com/document/api/213/9391)来获取或参见[CVM实例配置](https://www.qcloud.com/document/product/213/2177)描述，也可以调用查询实例资源规格列表接口获得最新的规格表。若不指定该参数，则默认机型为S1.SMALL1。|

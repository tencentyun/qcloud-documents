...
|SecurityGroupIds.N|array of Strings|否|实例所属安全组。若不指定该参数，则默认不绑定安全组。<font style="color:red">不绑定安全组将暴露所有端口到公网和内网，实例的所有业务（如80、443等端口）将均可被访问，但会有一定的安全风险，建议选择按需新建的安全组</font>。当前仅支持购买的时候指定一个安全组。|

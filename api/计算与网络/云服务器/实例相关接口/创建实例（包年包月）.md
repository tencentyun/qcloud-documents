...
|vpcId| 否| String|[私有网络](https://www.qcloud.com/doc/product/215/535)ID。私有网络下为必填，不填则为基础网络。
...
|storageType| 否| Int| 硬盘类型。1:本地盘、2:云硬盘、3:SSD本地盘；默认为1。关于硬盘类型的选择请参考[硬盘产品简介](https://www.qcloud.com/doc/product/213/498)，可选硬盘类型受到实例类型（InstanceType）限制。另外允许购买的最大容量也因硬盘类型的不同而有所差异。|

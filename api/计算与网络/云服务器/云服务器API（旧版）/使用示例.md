...
| imageId | 镜像ID，这里我们使用 Windows 2008（具体imageId对应何种操作系统可见[镜像列表](https://cloud.tencent.com/doc/api/229/1272)）。 | img-lkxqa4kj |
| cpu | CPU核数。 | 1 
|mem | 内存（GB）。| 1
...
| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
|instanceId |是 |String |待操作CVM的实例ID。可通过<a href="https://cloud.tencent.com/document/api/213/831" title="DescribeInstances">DescribeInstances</a>接口返回字段中的 unInstanceId 获取。|

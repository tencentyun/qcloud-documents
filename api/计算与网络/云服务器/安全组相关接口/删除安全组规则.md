...
|---------|---------|---------|---------|
| sgId | 是 | String | 安全组ID。 |
| version | 否 | Int | 用于指定要操作的安全组的版本。版本，即用户每次更新安全规则版本会自动加1。入参此version若不是最新的，将帮你使操作失败以防止你想要更新的路由规则已过期（如规则位置已变动）；若不传version则需要您自己注意确认好此次删除的规则在您传入的indexes位置。建议用户使用本参数，防止待删除的规则的indexes在调用本规则之前发生变化 |

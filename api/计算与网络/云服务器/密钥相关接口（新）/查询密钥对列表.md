...
| KeyIds.N  | array of Strings | 否 | 密钥对ID，密钥对ID形如：`skey-11112222`（此接口支持同时传入多个ID进行过滤。此参数的具体格式可参考`API`[简介](/doc/api/229/568)的`id.N`一节）。参数不支持同时指定`KeyIds`和`Filters`。<br> 密钥对ID可以通过登录[控制台](https://console.cloud.tencent.com/cvm/sshkey)查询。|

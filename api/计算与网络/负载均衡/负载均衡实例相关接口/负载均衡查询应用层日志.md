...
接口说明：该接口只能查询 COS 日志，不支持查询 CLS 日志。如需查询 CLS 日志请参见 [搜索日志](https://cloud.tencent.com/document/product/614/16875) ，该接口可以查询三天之内的负载均衡的转发日志，包括转发给后端 RS 的日志，以及由于后端 RS 异常，直接从负载均衡返回的日志，请求的时间间隔不超过一天。 

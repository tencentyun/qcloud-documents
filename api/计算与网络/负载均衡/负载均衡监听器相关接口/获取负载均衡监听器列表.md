...
|listenerIds.n|是|String|负载均衡监听器ID，可使用listenerId 或 unListenerId，推荐使用unListenerId。|
...
|httpHash|String|公网(有日租)的HTTP、HTTPS协议监听器的轮询方法。wrr表示按权重轮询，ip_hash表示根据访问的源IP进行一致性哈希方式来分发。|
|httpCode|Int|公网(有日租)的HTTP、HTTPS协议监听器的健康检查返回码。具体可参考[创建监听器](/doc/api/244/1255)中对该字段的解释。|
|httpCheckPath|String|公网(有日租)的HTTP、HTTPS协议监听器的健康检查路径。|
|SSLMode|String|公网(有日租)的HTTPS协议监听器的认证方式。|
|certId|String|公网(有日租)的HTTPS协议监听器服务端证书ID。|
|certCaId|String|公网(有日租)的HTTPS协议监听器客户端证书ID。|

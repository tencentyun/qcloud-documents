## 1. 接口描述
 DeleteForwardLBSeventhListeners 接口用来删除应用型负载均衡实例的一个或多个监听器。
 
接口访问域名：lb.api.qcloud.com

## 2. 请求参数
   以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](/doc/api/244/4183)页面。其中，此接口的Action字段为 DeleteForwardLBSeventhListeners。
...
|listenerIds.n|是|String|应用型负载均衡监听器ID，可通过 DescribeForwardLBListeners 接口查询。|

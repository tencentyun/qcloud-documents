...
## 2. 请求参数
   ...
   以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](/doc/api/244/4183)页面。其中，此接口的Action字段为 RegisterInstancesWithForwardLBSeventhListener。
...
|locationIds.n|否|String|应用型负载均衡监听器的转发规则ID。|

...
消息队列 CKafka 向用户提供弹性的存储和网络资源。用户可以使用本文档介绍的 API，并参照相应的示例，对消息队列 CKafka 进行相关操作：如创建、销毁、升配等；支持的全部操作可参见 [API 概览](/document/api/597/10076)。
...
>	
> topic.0=test1&topic.1=test2&topic.2=test3
>	
> 以此类推（以下标 0 开始）。
...
1. 通过使用 [创建主题](/document/product/597/10096) API，提供实例ID、主题名称、分区个数，副本个数等一些必要的信息，即可立刻创建出一个topic。

2. 如需修改主题配置，可以使用 [修改主题属性接口](/document/product/597/10098) API 调整主题的配置。

## 使用限制 
* 消息队列 CKafka  API 调用配额为：100 次/分钟；且单一 API 不超过 100 次/分钟。

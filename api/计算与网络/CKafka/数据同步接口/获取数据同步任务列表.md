...
| connectorList::status | String |connector 当前状态。 该状态不是实时展示，有一定的有延迟，详细的可能需要通过 GetConnectorStatus 接口。 <br>UNASSIGNED：任务还未分配；RUNNING：代表 connector 正在运行；PAUSED：代表 connector 已经暂停；FAILED：代表任务失败；DESTROYED：代表任务销毁。 |

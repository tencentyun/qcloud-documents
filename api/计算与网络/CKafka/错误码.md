## 功能说明
若 API 调用失败，则最终返回结果中的错误码 code 不为0，message 字段会显示详细错误信息。用户可以根据 code 和message在错误码列表查询具体的错误信息。
示例如下：
```shell
{
    "code": 5100,
...
| 4000   | 10000      | vpcid not exist                                             | vpc 不存在                         | 您指定的 vpc 不存在，请您确认vpc 的合法性                    |
| 4000   | 10001      | subnetId not exist                                          | 子网Id 不存在                      | 您指定的子网 ID 不存在，请您确认子网 ID 的合法性             |
| 4000   | 10004      | not support zone                                            | 不支持该地域                       | 还未支持该地域，请您重新选择地域                             |
| 4100   | 10005      | subnet  not belong to zone                                  | 子网不属于该zone                   | 子网不属于这个可用区，请您确认子网的可用区                   |
...
| 4000   | 10017      | task id not exist                                           | 任务不存在                         | 您指定的任务ID不存在，请您确认任务的合法性                   |

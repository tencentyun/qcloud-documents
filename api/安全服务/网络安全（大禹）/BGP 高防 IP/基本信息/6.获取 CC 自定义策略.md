...
| rulelist | Array | 当 smode 的值是 matching 时，需要携带此参数 </br>"rulelist":[</br>{"key":"host","operate":"include","value":"test1"},</br>{"key":"cgi","operate":"include","value":"test2"}</br>]</br>其中，</br>key 的值为：host、cgi、ua、referer；</br>operate 的值为：include （包含）、not_include （不包含）、equal （等于）；</br>value 的值为：字符串，不超过 31 个字符 |

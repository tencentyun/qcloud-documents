...
| 参数名称 | 是否必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| filtering. name | 可选 | String | 以 BGP 高防 IP 的名称进行关键字查询，支持模糊搜索 |
| filtering.ip | 可选 | String | 以 BGP 高防 IP 进行关键字查询，支持模糊搜索 |
| line | 可选 | Integer | BGP 高防 IP 的线路类型（1 / 2 ）</br>1 表示 BGP</br>2 表示三网 |
| sorting.field | 可选 | String | 取值范围： bandwidth / overloadCount</br>bandwidth 表示以带宽进行排序</br> overloadCount 表示以超峰次数进行排序 |
...
| region | 必选 | String | BGP 高防 IP 的地域</br>目前有三个地区可供选择：</br>gz：广州</br>sh：上海</br>bj：北京 |
...
| records | [obj,…] | Array | 攻击详情数组，数组元素如下：</br>{</br>"id": "bgpip-00000001", </br>"name": "服务包1",</br>"region": "gz/sh/bj",</br>"line": "1/2",</br>"boundIP":</br>"1.2.3.4",</br>"bandwidth": 10000, </br>"elasticLimit" : 100000, </br>"overloadCount" : 100, </br>"status":"idle/attacking/blocking/creating",</br>"expire": "2016-03-02 01:23:45",</br>"locked": "yes/no"</br>"transTarget":"qcloud/nqcloud/blackstone/finance",</br>"transRules": "12"</br>} |

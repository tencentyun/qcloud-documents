...
#### 3.1.2 添加验证码展示控件
```
	<!--显示验证码的地方预留的的空间：宽>=300px，高>=310px，否则会导致验证码显示异常-->
...
|函数名         |  描述 |
| ------------- |:-------------|
| capInit(iframe_div, options)|初始化并显示验证码,参数如下：<br> 1、iframe_div（必填）：嵌入验证码iframe的元素。<br> 2、options: {callback:xxx,showheader:xxx, themeColor:xxxxxx,type:"embed"},json格式对象<br>callback: 验证码页面关闭回调函数。用户验证之后，会调用该函数，传入json格式验证参数，如：{ret:xxx,ticket:"xxx",randstr:"xxx"}，ret=0表示用户验证完成，业务可以校验ticket；ret=1表示用户未验证验证码，此时没有ticket和randstr参数。这两个参数需要提交给业务后台，具体填哪个字段参考后面后台server开发部分。<br>showHeader：类型boolen，是否显示验证码页面的header(返回和帮组)，false不显示，true显示。只对手机页面有效<br>themeColor：设置页面的主题色彩，值为16进制色彩，比如ff572d。设置后页面里的按钮和图标会变成设置的颜色<br>type:pc端可选选项，"embed"表示没有触发按钮，直接出验证码|

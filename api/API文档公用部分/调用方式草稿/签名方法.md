...
3. 在 [云 API 访问密钥管理](https://console.cloud.tencent.com/capi) 页面，单击【新建密钥】即可以创建一对 SecretId/SecretKey。
>**注意：**
...
以腾讯云 CVM 为例，，当用户调用腾讯云 CVM 的 [查看实例列表](/document/product/213/9388 ) (DescribeInstances)接口时，其请求参数为：
...
* **请求主机：**即主机域名，请求域名根据接口所属的产品或所属产品的模块不同而不同，（如腾讯云 CVM 的查询实例列表(DescribeInstances)的请求域名为：`cvm.api.qcloud.com`。）具体产品请求域名详见各接口说明。
...
此步骤生成签名串。
>**注意：** 
>计算签名的方法有两种：HmacSHA256 和 HmacSHA1 这里要根据您指定的签名算法（即 SignatureMethod 参数）生成签名串。当指定 SignatureMethod 为 HmacSHA256时，需要使用 HmacSHA256 计算签名，其他情况请使用 HmacSHA1 计算签名。
...
>**注意：**
>如果用户的请求方法是 GET，则对所有请求参数的参数值均需要做 UR L编码；此外，部分语言库会自动对 URL 进行编码，重复编码会导致签名校验失败。

Tencent Cloud API will verify every access request. Every request needs to contain a signature in public request parameters for identity verification. The signature is generated by your security credential, which include SecretID and SecretKey. If you do not have a security credential, you need to apply for it on Tencent Cloud official website. Otherwise, you cannot call Tencent Cloud APIs.
...
3) In [Cloud API Key Managemen](https://console.qcloud.com/capi), click "New" to create a SecretId/SecretKey pair.

...
3. 在[云 API 访问密钥管理](https://console.cloud.tencent.com/capi) 页面，点击【新建】即可以创建一对 SecretId/SecretKey，每个帐号最多可以拥有两对 SecretId/SecretKey。
...
以腾讯云 CVM 为例，当用户调用腾讯云 CVM 的 [查看实例列表](/doc/api/229/查看实例列表 ) （DescribeInstances） 时，其请求参数可能如下:
...
由上表可以看出，请求参数中的公共请求参数只有 5 个：Action、SecretId、Timestamp、Nonce 和 Region，而不是在“公共请求参数”中所述的 6 个，事实上，第 6 个参数 Signature（签名串）正是由其他参数（包括指令请求参数）共同生成的，具体步骤如下：
...
- 请求主机:[查看实例列表](https://cloud.tencent.com/document/product/213/9388 ) (DescribeInstances) 的请求域名为：cvm.api.qcloud.com。实际的请求域名根据接口所属模块的不同而不同, 详见各接口说明。

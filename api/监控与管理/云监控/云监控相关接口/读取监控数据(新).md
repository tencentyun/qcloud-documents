...
| 指标名称 | 含义 | 单位 |维度|
|---------|---------|---------|---------|
|invisibleMsgNum|队列不可见消息数量|条|queueId|
|visibleMsgNum|队列可见消息数量|条|queueId|
|sendMsgReqCount|发送消息请求量|次|queueId|
|sendMsgNum|发送的消息数量|条|queueId|
|recvMsgReqCount|接收消息请求量|次|queueId|
|recvMsgNum|接收的消息数量|条|queueId|
|recvNullMsgNum|接收空消息的数量|条|queueId|
|batchRecvNullMsgNum|批量接收空消息的数量|条|queueId|
|delMsgReqCount|删除消息的请求量|次|queueId|
|delMsgNum|删除消息的数量|条|queueId|
|sendMsgSize|发送的消息大小|KB|queueId|
|batchSendMsgSize|批量发送的消息大小|KB|queueId|
|batchSendMsgReqCount|批量发送消息的请求量|次|queueId|
|batchRecvMsgReqCount|批量接收消息的请求量|次|queueId|
|batchDelMsgReqCount|批量删除消息的请求量|次|queueId|

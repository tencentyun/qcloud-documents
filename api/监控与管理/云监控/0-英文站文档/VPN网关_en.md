...
VPN connection is a method to connect peered IDC and VPC through public network encrypted tunnel.

This API (GetMonitorData) is used to query the monitoring data of VPN gateway product. The values for input parameters are as follows:
namespace: qce/vpngw
Value of dimension name: vpnGwId
dimensions.0.name=vpnGwId
dimensions.0.value is VPN gateway ID
...
| dimensions.0.name  | Yes    | String   | vpnGwId    | Input parameter is VPN gateway ID                               |
| dimensions.0.value | Yes    | String   | Specific VPN gateway ID | Enter specific vpnGwId                              |

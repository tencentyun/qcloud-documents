...
After the VPN gateway and the peer gateway are established, the VPN tunnel used for encrypted communication between VPC and external IDC can be established.

This API (GetMonitorData) is used to query the monitoring data of VPN tunnel product. The values for input parameters are as follows:
namespace: qce/vpnx
Value of dimension name: vpnConnId
dimensions.0.name=vpnConnId
dimensions.0.value is VPN tunnel ID
...
| dimensions.0.name  | Yes    | String   | vpnConnId  | Input parameter is VPN tunnel ID                               |
| dimensions.0.value | Yes    | String   | Specific VPN tunnel ID | Enter specific vpnConnId                            |
...
| delay        | Latency    | Second    | vpnConnId |

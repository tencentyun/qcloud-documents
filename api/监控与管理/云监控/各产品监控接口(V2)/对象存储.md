...
对象存储服务 COS（Cloud Object Storage）是腾讯云提供的一种基于 Web 的安全稳定、海量、便捷、低延迟、低成本的云端存储服务，有关产品信息请参考 [对象存储产品页](https://cloud.tencent.com/product/cos) 介绍。
...
> 存储类指标支持的最小统计周期（period）为 300，并支持使用 3600、86400 为拉取周期。
...
> 流量类指标支持的最小统计周期（period）为 60，并支持使用 300、3600、86400 为拉取周期。
...
> 数据读取类指标支持的最小统计周期（period）为 60，并支持使用 300、3600、86400 为拉取周期。

| 指标名称（metricName） | 含义         | 单位 |
| ---------------------- | ------------ | ---- |
| std_retrieval          | 标准数据读取 | B    |
| ia_retrieval           | 标准数据读取 | B    |
| nl_retrieval           | 标准数据读取 | B    |

#### 请求类

> 请求类指标支持的最小统计周期（period）为 60，并支持使用 300、3600、86400 为拉取周期。

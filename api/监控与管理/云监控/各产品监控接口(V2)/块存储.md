...
接口调用频率限制为：50次/秒，500次/分钟。 单请求最多可支持批量拉取 10 个实例的监控数据，单请求的数据点数限制为 1440 个。

若您需要调用的指标、对象较多，可能存在因限频出现拉取失败的情况，建议尽量将请求按时间维度均摊。

查询块存储监控数据，入参取值如下：
Namespace：qce/block_storage
Dimensions.N.diskId：块存储ID，调用[DescribeDisks](https://cloud.tencent.com/document/product/362/16315)接口获取的 DiskId 字段。

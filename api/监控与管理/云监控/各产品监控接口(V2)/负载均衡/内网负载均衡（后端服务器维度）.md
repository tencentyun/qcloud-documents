...
qce/lb_rs_private 是内网负载均衡后端服务器维度的命名空间，您可以在此空间查询到内网负载均衡后端服务器维度相关监控数据。

qce/lb_rs_private 支持以下2种维度组合的查询方式，四种入参取值如下：
...
| 参数名称               | 维度名称             | 维度解释          | 格式                            |
| ------------------ | ---------------- | ------------- | ----------------------------- |
| dimensions.0.name  | vip              | 负载均衡 VIP       | String类型维度名称：vip              |
| dimensions.0.value | vip              | 负载均衡 VIP       | 具体 IP 地址，如111.111.111.11        |
| dimensions.1.name  | vpcId            | 负载均衡所在私有网络的 ID | String 类型维度名称：vpcId            |
...
| dimensions.4.name  | lanIp            | 负载均衡绑定机器的 IP  | String 类型维度名称：lanIp            |

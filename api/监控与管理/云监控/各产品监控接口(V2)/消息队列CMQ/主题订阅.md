...
| dimensions.0.value | 是    | String   | 具体的 CMQ 主题实例id  | 输入具体 topicId，如 topic-i4p4k0u0。            |
| dimensions.1.name  | 否    | String   | subscriptionId | 入参为 CMQ 订阅实例id，当2.2表中指标对应的维度为topicId,subscriptionId时，该字段必选 。                           |
| dimensions.1.value | 否    | String   | 具体的 CMQ 订阅实例id  | 输入具体 subscriptionId，如 test1， 当2.2表中指标对应的维度为topicId,subscriptionId时，该字段必选 。              |

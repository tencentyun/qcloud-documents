...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，详情请参见 [公共请求参数](https://cloud.tencent.com/doc/api/231/4473) 页面。其中，此接口的 Action 字段为 GetCertificates。
...
| 参数名称  | 类型   | 描述                                 |
| -------- | ------ | ----------------------------------- |
| total    | Int    | 返回的 HTTPS 证书总数。|
| hosts    | Array  | HTTPS 证书数组，详细说明见下文 **hosts 字段说明**。          |

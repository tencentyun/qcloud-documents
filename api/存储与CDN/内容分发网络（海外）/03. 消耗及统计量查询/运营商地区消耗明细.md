...
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](https://cloud.tencent.com/doc/api/231/4473)页面。其中，此接口的Action字段为GetCdnOverseaProvIspDetailStat。

| 参数名称     | 是否必选 | 类型     | 描述                                       |
| -------- | ---- | ------ | ---------------------------------------- |
| date     | 是    | String | 查询日期，格式必须为：2016-11-20                    |
| hosts.n  | 是    | String | 域名，支持多个，如hosts.0=www.test.com&hosts.1=www.test2.com，最多可查询5个 |
| provs.n  | 否    | String | 指定地区，地区通过编码区分，编码参考下文中境外 CDN日志地区编码映射表      |
| isps.n   | 否    | String | 指定运营商，运营商通过编码区分，编码参考下文中境外 CDN日志地区编码映射表    |
...
73：印度、1195：印度尼西亚、1176：新加坡、57：泰国、144：越南、3701：马来西亚、2588：菲律宾、2026：中国台湾、1044：日本、3379：韩国、1200：z中国香港、3839：加拿大、669：美国、-2：境外其他地区、-3：未知；

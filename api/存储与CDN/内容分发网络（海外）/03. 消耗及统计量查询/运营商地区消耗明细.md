...
| statType | 是    | String | 指定查询类型，支持七种数据的查询，bandwidth对应带宽数据，单位为bps；requests对应请求数据，单位为个；flux对应流量数据，单位为Byte；statuscode对应状态码，单位为个；dlspeed对应下载速度，dltime对应下载用时，hitrate对应命中率，单位为百分比 |

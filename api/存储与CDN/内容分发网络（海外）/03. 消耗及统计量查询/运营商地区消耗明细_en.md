...
| statType | Yes    | String | Specifies query type. You may query data of seven types: bandwidth (bandwidth), measured in bps; number of requests (requests); traffic (flux), measured in Byte; number of status codes (statuscode); download speed (dlspeed), measured in bit/s; download time cost (dltime), measured in seconds; hit rate (hitrate), in terms of percentage |

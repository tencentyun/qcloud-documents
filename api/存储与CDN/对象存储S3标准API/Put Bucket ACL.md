...
使用API写入Bucket的ACL表，您可以通过Header：『x-cos-acl』『x-cos-grant-read』『x-cos-grant-write』『x-cos-grant-full-control』传入ACL信息，也可以通过body以XML格式传入ACL信息，但是只能选择Header和Body其中一种，不然返回冲突。

Put Bucket ACL是一个覆盖操作，传入新的ACL将覆盖原有ACL。
...
| 参数名称                     | 是否必选 | 类型     | 描述                                       |
| ------------------------ | ---- | ------ | ---------------------------------------- |
| X-cos-acl                | 否    | String | 定义Bucket的ACL属性，有效值：private, public-read, public-read-write，默认值：private |

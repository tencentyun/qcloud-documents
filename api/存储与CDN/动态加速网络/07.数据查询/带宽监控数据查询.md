## 接口描述
本接口用于查询指定时间区间的访问带宽明细数据，可以根据此数据观察域名带宽变化情况。
...
| 参数 |  必选  |   类型   | 示例  |  描述  |
| ------ | ----------| ----------| ------- |
| metrics | 是  | String | bandwidth | 统计指标，JSON 格式</br>例如：bandwidth |
| projects | 否 | String | default |项目列表，根据项目查询数据时使用</br>按 JSON 格式提交 |
| hosts |    否     | String  | test.qcloud.com | 域名列表，根据域名查询数据时使用</br>按 JSON 格式提交 |
...
|参数|类型|描述|
|------ | ----------| ----------|
|code|Int|公共错误码</br>0：表示成功</br>其他值：表示失败</br>详情请参见 [公共错误码](https://cloud.tencent.com/document/product/570/13937 ) 页面 |
...
| bandwidth | Unsigned | 统计带宽明细值</br>数据单位：bps |

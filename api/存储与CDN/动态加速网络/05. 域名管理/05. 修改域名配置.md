...
| 参数名称     | 是否必选     | 类型     |描述|
| -------- | ------ | ---------------------------------------- | -------------- |
|type|是|Int	|0：全部文件缓存配置<br>1：按文件后缀类型缓存配置<br>2：文件夹/目录路径缓存配置<br>3：具体 URL 缓存配置，例如`folder/*.jpg`类型的正则匹配<br>4：首页缓存设置 |
...
启用缓存，.txt 类型缓存1天，/test;/abc 目录缓存1小时，/test/ 目录下所有 jpg 文件缓存1分钟，首页缓存1秒，其他文件不缓存。
```
host: dsa.qcloud.com
hostId: 1234

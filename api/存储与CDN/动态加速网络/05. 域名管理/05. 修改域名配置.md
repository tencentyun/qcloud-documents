...
>2. 一次仅支持修改一个域名对应的配置。
>3. 一次可修改同一个域名的多个配置项。
>4. 该接口调用频次上限为100次/分钟，超出上限则会返回错误，请勿高频调用。
...
| 参数名称   | 必选 | 类型     | 描述                                       |
| ------ | ---- | ------ | ---------------------------------------- |
| hostId | 是    | Int    | 域名接入 DSA 后的标识 ID</br>您可通过 [查询域名列表信息](https://cloud.tencent.com/document/product/570/13940) 接口查询域名的 hostId  |
...
```
**注意事项**
> 当设置自定义头部时，请**采用 POST 方式提交**，防止 URL 长度越界问题。
...
|https| Object | HTTPS 配置信息，详情 [HTTPS 字段说明](#httpsinfo2) 章节  |
|message|String|信息描述|

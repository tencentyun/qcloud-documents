...
## 输入参数 
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上 [公共请求参数](https://cloud.tencent.com/document/product/570/13932)，见公共请求参数页面。其中，此接口的 Action 字段为 CreateDsaHost。
...
| 参数名称   | 是否必选 | 类型     | 描述         |
| ------ | ---- | ------ | ---------------------------------------- |
| type | 是 | Int | Https开关，0-表示关闭，2-表示启用，并且采用HTTP回源，4-表示启用，并采用协议跟随回源，默认关闭 Https 配置 |
...
**注意事项**
>当启用Https配置，且采用自有证书时，由于需要上传证书信息，请**采用 POST 方式提交**。

...
Complete Vault Lock实现完成文件库锁定。如果当文件库已锁定处于 `Locked`状态时，再次发起该请求，则返回204 成功。如果当文件库已锁定处于 `Locked` 状态但锁定 ID无效，则返回 `AccessDeniedException` 。如果当文件库锁定处于 `InProgress` 状态但锁定 ID无效，则会返回 `InvalidParameter` 
...
```http
POST /<UID>/vaults/<VaultName>/lock-policy/<LockID> HTTP 1.1

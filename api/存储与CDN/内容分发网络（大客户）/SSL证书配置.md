...
| httpsType   | 是    | Int    | 配置类型设置，若设置为 0 ，表示清除https配置，此时无需填写证书及私钥参数；若设置为1，表示开启并http回源；若设置为2，表示开启并协议跟随回源；开启时需要传递证书及私钥； |
...
| 参数名称     | 描述                                       | 类型     |
| -------- | ---------------------------------------- | ------ |
| code     | 公共错误码，0表示成功，其他值表示失败。详见错误码页面的[公共错误码](https://www.qcloud.com/doc/api/231/5078#1.-.E5.85.AC.E5.85.B1.E9.94.99.E8.AF.AF.E7.A0.81)。 | Int    |

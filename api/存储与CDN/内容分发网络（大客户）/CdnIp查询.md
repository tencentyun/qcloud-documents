## 1. 接口描述
本接口（QueryCdnIp）用于查询指定 IP 是否为 CDN 节点。

接口请求域名：cdn.api.qcloud.com

**接口说明：**
+ 支持多 IP 查询，一次最多可查询20个；


## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](https://www.qcloud.com/doc/api/231/4473)页面。其中，此接口的 Action 字段为 QueryCdnIp。
...
+ 多个 IP 时，查询方式如下，多个 IP 时使用英文逗号分隔：
```
ips=1.1.1.1,2.2.2.2
```

...
本接口（GetCdnHostsIntervalHyStat）用于查询某段时间内的域名级回源带宽/流量数据。
...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](https://www.qcloud.com/doc/api/231/4473)页面。其中，此接口的Action字段为GetCdnHostsIntervalHyStat。
...
Action=GetCdnHostsIntervalHyStat

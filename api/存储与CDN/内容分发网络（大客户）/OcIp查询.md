## 1. 接口描述
本接口（GetCdnIps）用于查询指定域名在腾讯云CDN资源平台中所有的边缘节点IP。
...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](https://cloud.tencent.com/doc/api/231/4473)页面。其中，此接口的Action字段为 GetCdnIps。

...
## 2. 输入参数
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](https://cloud.tencent.com/doc/api/231/4473)页面。其中，此接口的Action字段为GetHostInfoById。

| 参数名称  | 是否必选 | 类型   | 描述                  |
| ----- | ---- | ---- | ------------------- |
| ids.n | 是    | Int  | 需要查询的域名ID，支持查询一个或多个 |

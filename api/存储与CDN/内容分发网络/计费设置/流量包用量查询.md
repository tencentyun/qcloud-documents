## 接口描述
**GetPackage**  用于查询账号下 CDN（国内）流量包使用情况。 
...
## 入参说明
以下请求参数列表仅列出了接口请求参数，正式调用时需要加上公共请求参数，见[公共请求参数](https://cloud.tencent.com/doc/api/231/4473)页面。其中，此接口的Action字段为 GetPackage。
...
| flux_byte   | Int    | 流量包总量，单位为 Byte                                      |
| flux_used   | Int    | 已抵扣流量包用量，单位为 Byte                                |

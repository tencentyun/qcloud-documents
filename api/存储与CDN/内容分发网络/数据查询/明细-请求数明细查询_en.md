<font style="color:Brown">The API GetCdnHostsDetailStatistics is used to query bandwidth, traffic, number of requests, number of hit requests, hit traffic, status codes and other data based on the input parameter statType. This document describes how to query the number of requests.</font>

## API Description

**GetCdnHostsDetailStatistics** can be used to query the number of requests within the specified time range for a domain name.

Domain name for API request: <font style="color:red">cdn.api.qcloud.com</font>

**Note:**

+ You can submit a maximum of 10 domain names at a time to query the number of real-time requests for each domain name, and the total number of real-time requests for all the domain names.
+ The statistic value of traffic is generated at an interval of 5 minutes, with the delay of real-time data being 5-10 minutes.
+ You can query the data with a time range of 60 days between a start and end time (inclusive).
+ By default, the data granularity is 5 minutes for the data within a time range of 3 days, 1 hour for the data within 4-7 days (total requests), and 1 day for the data within 8 or more days (total requests).
...
| statType | Yes | String | Statistic type<br/>"requests" indicates number of requests |
...
| requests | Array | Number of requests |
| requests_total | Array | Total number of requests for multiple domain names |

<font style="color:Brown">The API GetCdnHostsDetailStatistics is used to query bandwidth, traffic, number of requests, number of hit requests, hit traffic, status codes and other data based on the input parameter statType. This document describes how to query the details of hit traffic.</font>

## API Description

**GetCdnHostsDetailStatistics** can be used to query the details of hit traffic within the specified time range for a domain name.

Domain name for API request: <font style="color:red">cdn.api.qcloud.com</font>

**Note:**

+ Hit traffic is the traffic data that is directly returned by nodes but not obtained by origin-pull from the origin server.
+ You can submit a maximum of 10 domain names at a time to query the real-time hit traffic for each domain name, and the total real-time hit traffic for all the domain names.
+ The statistic value of hit traffic is generated at an interval of 5 minutes, with the delay of real-time data being 5-10 minutes.
+ You can query the data with a time range of 60 days between a start and end time (inclusive).
+ By default, the data granularity is 5 minutes for the data within a time range of 3 days, 1 hour for the data within 4-7 days (total hit traffic), and 1 day for the data within 8 or more days (total hit traffic).
...
| statType | Yes | String | Statistic type<br/>"hit_flux" indicates details of hit traffic |
...
| requests | Array | Data of hit traffic (in byte) |
| requests_total | Array | Data of total hit traffic for multiple domain names (in byte) |

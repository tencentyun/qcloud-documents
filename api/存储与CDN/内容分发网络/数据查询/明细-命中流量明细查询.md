<font style="color:Brown">接口 GetCdnHostsDetailStatistics 通过入参 statType 不同可指定查询带宽、流量、请求数、命中请求数数据、命中流量、状态码等明细数据，此文档对命中流量明细查询方式进行了详细说明</font>

## 接口描述

**GetCdnHostsDetailStatistics** 查询域名指定时间区间的命中流量明细数据。
...
| requests_total | Array  | 多域名总命中流量明细，单位为 byte |

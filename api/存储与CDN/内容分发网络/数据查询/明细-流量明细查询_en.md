<font style="color:Brown">The API GetCdnHostsDetailStatistics is used to query bandwidth, traffic, number of requests, number of hit requests, hit traffic, status codes and other data based on the input parameter statType. This document describes how to query the details of traffic.</font>

## API Description

**GetCdnHostsDetailStatistics** can be used to query the details of traffic within the specified time range for a domain name.

Domain name for API request: <font style="color:red">cdn.api.qcloud.com</font>

**Note:**

+ You can submit a maximum of 10 domain names at a time to query the real-time traffic for each domain name, and the total real-time traffic for all the domain names.
...
| statType | Yes | String | Statistic type<br/>"flux" indicates details of traffic |
...
| flux | Array | Details of traffic (in byte) |
| flux_total | Array | Data of total traffic for multiple domain names (in byte) |

<font style="color:Brown">The API DescribeCdnHostInfo is used to query peak bandwidth, total traffic, total number of requests, and average hit rate of requests based on the input parameter statType. This document describes how to query the total traffic.</font>

## API Description

**DescribeCdnHostInfo** can be used to query the total traffic within a specified time range for a domain name.

Domain name for API request: <font style="color:red">cdn.api.qcloud.com</font>

**Note:**

+ You can submit multiple domain names or projects at a time to query the total traffic for the submitted domain names or the domain names under each project.
+ The statistic value of traffic is generated every 5 minutes, and the total traffic is the sum of the statistic values within the specified time range.
...
+ Parameter projectId is required to query the total traffic for a domain name.
...
| statType | Yes | String | Statistic type<br/>"flux" indicates total traffic |
...
| host_type | String | Domain name type<br/>"cname" indicates the domain name accessing a self-owned origin<br/>"cos" indicates the domain name accessing to a COS origin |
| host_value | Int | Total traffic (in byte) |

<font style="color:Brown">The API GetCdnHostsDetailStatistics is used to query bandwidth, traffic, number of requests, number of hit requests, hit traffic, status codes and other data based on the input parameter statType. This document describes how to query the details of status codes.</font>

## API Description

**GetCdnHostsDetailStatistics** can be used to query the details of status codes within the specified time range for a domain name.

Domain name for API request: <font style="color:red">cdn.api.qcloud.com</font>

**Note:**

+ You can submit a maximum of domain names at a time to query the real-time status codes for each domain name, and the total real-time status codes for all the domain names.
+ The statistic value of status codes is generated at an interval of 5 minutes, with the delay of real-time data being 5-10 minutes.
+ Default status codes include: 200, 206, 302, 304, 401, 403, 404, 405, 416, 500, 502
+ You can query the data with a time range of 60 days between a start and end time (inclusive).
+ By default, the data granularity is 5 minutes for the data within a time range of 3 days, 1 hour for the data within 4-7 days (total), and 1 day for the data within 8 or more days (total).
...
| statType | Yes | String | Statistic type<br/>"status_code" indicates details of status codes |
...
| status_code | Array | Status codes |
| status_code_total | Array | Total status codes for multiple domain names |

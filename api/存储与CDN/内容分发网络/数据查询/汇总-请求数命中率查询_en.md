<font style="color:Brown">The API DescribeCdnHostInfo is used to query peak bandwidth, total traffic, total number of requests, and average hit rate of requests based on the input parameter statType. This document describes how to query the average hit rate of requests.</font>

## API Description

**DescribeCdnHostInfo** can be used to query the average hit rate of requests within a specified time range for a domain name.

Domain name for API request: <font style="color:red">cdn.api.qcloud.com</font>

**Note:**

+ You can submit multiple domain names or projects at a time to query the average hit rate of requests for the submitted domain names or the domain names under each project.
+ The statistic value of hit rate of requests is generated every 5 minutes, and the query result is the average of the statistic values within the specified time range.
...
+ Parameter projectId is required to query the hit rate of requests for a domain name. 
...
| statType | Yes | String | Statistic type<br/>"cache" indicates hit rate of requests |
...
| host_value | Int | Hit rate of requests (in %) |

<font style="color:Brown">The API GetCdnHostsDetailStatistics is used to query bandwidth, traffic, number of requests, number of hit requests, hit traffic, status codes and other data based on the input parameter statType. This document describes how to query the number of hit requests.</font>

## API Description

**GetCdnHostsDetailStatistics** can be used to query the number of hit requests within the specified time range for a domain name.

Domain name for API request: <font style="color:red">cdn.api.qcloud.com</font>

**Note:**

+ Number of hit requests is the statistics of requests that are directly returned by nodes but not pulled from origin server.
+ You can submit a maximum of 10 domain names at a time to query the number of real-time hit requests for each domain name, and the total number of real-time hit requests for all the domain names.
+ The statistic value of the number of hit requests is generated at an interval of 5 minutes, with the delay of real-time data being 5-10 minutes.
+ You can query the data with a time range of 60 days between a start and end time (inclusive).
+ By default, the data granularity is 5 minutes for the data within a time range of 3 days, 1 hour for the data within 4-7 days (total hit requests), and 1 day for the data within 8 or more days (total hit requests).
...
| statType | Yes | String | Statistic type<br/>"hit_requests" indicates the number of hit requests |
...
| codeDesc | String | Error message or error code at business side.<br/>For more information, please see [Business Error Codes](https://cloud.tencent.com/document/product/228/5078#2.-.E6.A8.A1.E5.9D.97.E9.94.99.E8.AF.AF.E7.A0.81). |
...
| requests | Array | Number of hit requests |
| requests_total | Array | Total number of hit requests for multiple domain names |

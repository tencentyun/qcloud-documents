...
腾讯云对象存储 COS 目前提供了 [COS 签名工具](https://cos5.cloud.tencent.com/static/cos-sign/) 供用户生成签名，如有需要可前往使用；同时各语言 SDK 提供**预签名 URL**方便用户获取携带签名的 URL，并自行处理相关的请求，详情请参阅各语言 SDK 的**预签名 URL**章节。

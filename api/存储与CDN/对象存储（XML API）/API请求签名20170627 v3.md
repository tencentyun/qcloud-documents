...
![签名流程图](//mc.qcloudimg.com/static/img/386bbd613921c7d7b801bed37520c712/image.png)
...
q-header-list|参数[*SignedHeaderList*]|必填。HTTP头部列表，请查看（==此处链接到目录SignedHeaderList ==）
q-url-param-list|参数[*SignedParameterList*]|必填。HTTP参数列表，请查看（== 此处链接到目录SignedParameterList ==）
q-signature|参数[Signature]|必填。HTTP内容签名，请查看（== 此处链接到目录Signature ==）
...
参数|说明
---|---
[q-key-time]| 需填写与（==此处链接到目录 签名内容 中的表格q-key-time ==）一致的字段。
...
[q-sign-time]|需填写与（==此处链接到目录 签名内容 中的表格q-sign-time ==）一致的字段。

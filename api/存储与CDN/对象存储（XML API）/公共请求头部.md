...
| Content-MD5          | RFC 1864 中定义的经过 Base64 编码的请求体内容 MD5 哈希值，此头部用于完整性检查，验证请求体在传输过程中是否发生变化。<br>此头部的值应使用 MD5 哈希值的 16 字节二进制形式转换为 Base64 编码，最终的 Base64 编码长度应为 24 个字符，请注意在编写代码时使用正确的方法和参数，例如`ZzD3iDJdrMAAb00lgLLeig==`。<br>针对有请求体的 PUT 和 POST 请求（除 POST Object），强烈建议携带此头部。 | string  | 否。                                                         |

...
> 注：本文所描述的API请求签名，如果您使用SDK进行开发，则已包含在内。**仅在您希望通过原始API进行二次开发时，需要根据本文所描述步骤进行操作**。
...
![流程图](//mc.qcloudimg.com/static/img/840056b5d73169f7335e11ef679d26c6/image.png)


## 准备工作
1. 腾讯云访问APPID、密钥ID、密钥Key，即APPID、SecretID 和SecretKey。 
...
<span id="signaturesplit">### 签名内容组成</span>
其中，组成签名内容的键值(key=value)描述如下：

键（key）|值(value)	|描述
---|---|---
q-sign-algorithm|sha1|必填。签名算法，目前仅支持sha1，即为sha1 。
q-ak|参数[*SecretID*]|必填。  帐户ID，即SecretID，在控制台 [云API密钥](https://console.qcloud.com/capi) 页面可获取，如AKIDQjz3ltompVjBni5LitkWHFlFpwkn9U5q 。
q-sign-time|参数[*SignTime*]|必填。本签名的有效起止时间，通过Unix时间戳描述起始和结束时间<sup>注</sup>，以秒为单位，格式为[*start-seconds*];[*end-seconds*]，如1480932292;1481012298 
q-key-time|参数[*KeyTime*]|必填。与q-sign-time值相同。
q-header-list|参数[*SignedHeaderList*]|必填。HTTP头部列表，请查看[SignedHeaderList 内容组成]
q-url-param-list|参数[*SignedParameterList*]|必填。HTTP参数列表，请查看[SignedParameterList 内容组成](#signature)
q-signature|参数[Signature]|必填。HTTP内容签名，请查看 [signature内容组成](#signature)


> 注：关于q-sign-time 和 q-key-time
> 1. Unix时间戳是从格林威治时间1970年01月01日00时00分00秒(北京时间1970年01月01日08时00分00秒)起至现在的总秒数。
> 2. 结束时间戳需要大于起始时间戳，否则将会导致签名马上过期。


<span id="signedheaderlist">### SignedHeaderList 内容组成</span>
由HTTP请求中全部或部分头部字段组成。
HTTP请求头部由一个或多个key:value对组成。

SignedHeaderList内容组成的伪代码为：

```
SignedHeaderList=LOWCASTING(key1);LOWCASTING(key2)
```
其中，请用对应的开发语言来替代LOWCASTING函数。

伪函数 | php | java
---|---|---
LOWCASING| strtolower| str.toLowerCase

如HTTP请求如下：

```
GET /?prefix=abc&max-keys=20
Host:testbucket-1254000000.cn-north.myqcloud.com
Content-Length:2222
```

则SignedHeaderList=host;content-length 。

<span id="signedparamlist">### SignedParameterList 内容组成</span>

由HTTP请求中URL中的全部或部分查询字符串组成。
URL中查询字符串可能为空，或者由一个或多个key=value对组成。

SignedParameterList内容组成的伪代码为：

```
SignedParameterList=LOWCASING(key0);LOWCASING(key1)
```
...
GET /?prefix=abc&max-keys=20
```
则 SignedParameterList=prefix;max-keys 或者 SignedParameterList=prefix 。


<span id="signature">### Signature 计算</span>
...
参数|说明
---|---
[q-key-time]| 需与[签名内容](#signaturesplit)中所填写的q-key-time保持一致
[HttpMethod]| HTTP请求方法。仅支持小写，即可为get，post，put，delete，head，options。**如**HTTP请求 “GET http://bucket1-1254000000.cn-north.myqcloud.com ”，其 HttpMethod 为 get。
[HttpURI]| HTTP请求URI部分。即从“/”虚拟根路径开始部分。**如**HTTP请求 “GET /testfile”，其 HttpURI 为 /testfile
[HttpParameters]|HTTP请求参数部分。仅支持小写，即为URI 中“?”之后由“&”连接的部分,此部分由多对key=value组成，key和value均需转为小写，且字典顺序排序。**如**HTTP请求 “GET /?prefix=abc&max-keys=20" ，其 HttpParameters 为 max-keys=20&prefix=abc
[HttpHeaders]|HTTP请求头部信息。需由key:value转化为key=value格式，且key需转化为小写,value需进行URLEncode转换，并多个key=value对之间以字典顺序排序，如有多组key=value，可用“&”连接。**如** Host: bucket1-1254000000.cn-north.myqcloud.com Content-Type: image/jpeg ，其 HttpHeaders 为 content-type=image%2Fjpeg&host=bucket1-1254000000.cn-north.myqcloud.com
[q-sign-algorithm]|sha1。目前仅支持sha1加密算法。
[q-sign-time]|需与[签名内容](#signaturesplit)中所填写的q-sign-time保持一致。

...
| detect-type        | 审核类型，拥有 porn（涉黄识别）、ads（广告识别）等。用户可选择多种识别类型，例如 detect-type=porn,ads 表示对图片进行涉黄及广告审核。<br/>注：不设置时会审核biz-type中开启的所有场景。 | String | 否       |
...
| Category   | String          | 该图片命中的审核类别结果。<br/>注意：该字段可能返回空        |

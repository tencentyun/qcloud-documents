...
<span id=1></span>
- 支持根据不同的业务场景 [配置自定义审核策略](https://cloud.tencent.com/document/product/436/55206)。
...
| biz-type    | 审核策略，不填写则使用默认策略。可在控制台进行配置，详情请参见 [设置审核策略](#1) | String | 否       |
...
| large-image-detect | 对于超过大小限制的图片，可通过该参数选择是否需要压缩图片后再审核，压缩为后台默认操作，会产生额外的 [基础图片处理用量](https://cloud.tencent.com/document/product/436/58963#.E5.9F.BA.E7.A1.80.E5.9B.BE.E7.89.87.E5.A4.84.E7.90.86.E8.B4.B9.E7.94.A8)，取值为：0（不压缩），1（压缩）。默认为0。 注意：最大支持压缩32MB的图片。 | String | 否   |
...
| 参数名称      | 类型      | 描述                                                         |
| ------------- | --------- | ------------------------------------------------------------ |
| JobId | String | 图片审核的任务 ID，您可以通过该 ID 主动查询图片审核结果。 |
...
| CompressionResult | Int  | 该参数表示当前图片是否被压缩处理过，值为 0（未经过压缩处理），1（已经过压缩处理）。 |

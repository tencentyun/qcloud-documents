腾讯云对象存储（COS）服务的 XML API 是一种轻量级的、无连接状态的接口。调用此套接口可以直接通过 http/https 发出请求和接受响应，从而实现与腾讯云对象存储后台进行交互的操作。此套 API 的请求和响应内容都为 XML 格式。
>**注意：**
>为了能更高效的使用腾讯云对象存储服务的 XML API，请在查阅其他的 API 文档之前，先详细阅读 [请求签名](https://www.qcloud.com/document/product/436/7778?!preview=true&lang=zh) 。

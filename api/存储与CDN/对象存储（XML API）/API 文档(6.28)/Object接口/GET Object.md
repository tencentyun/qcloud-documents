...
>- 当通过 COS 控制台 [设置回源](https://cloud.tencent.com/document/product/436/13310) ，但未开启**同步回源**时，需注意，COS 从用户配置的源站拉取数据时，发起 GET Object 请求将返回 302 并重定向到设置的回源地址（如果该回源地址是不受信任的，强烈建议在使用 SDK 或自行调用 API 时，不要跟随 302，而用户应该把获取到的回源地址进行二次校验，否则可能产生 SSRF 等安全风险，例如回源到一个内网地址）。

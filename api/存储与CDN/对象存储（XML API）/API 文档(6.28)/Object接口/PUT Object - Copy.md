...
本案例演示将对象从高频存储转换为归档存储，该使用方法也适合高频存储与低频存储之间的互相转换，如果希望将归档存储的对象转换为其他存储类型，需要首先使用 [POST Object restore](https://cloud.tencent.com/document/product/436/12633) 将归档存储的对象回热才能使用该接口请求转换存储类型。

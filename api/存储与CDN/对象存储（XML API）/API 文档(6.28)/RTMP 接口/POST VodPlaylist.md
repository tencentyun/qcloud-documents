...
POST VodPlaylist 接口用于为指定通道（Live Channel）生成一个可供点播例用的播放列表。COS 会查询请求参数中时间范围内由该 LiveChannel 推流生成的 ts 文件，由此生成一个 m3u8 播放列表，并上传至该存储桶内。
...
| PlaylistName | 指定生成的用于点播的 m3u8 文件的名称，必须以`.m3u8`结尾，<br/>长度范围为[6, 128]，默认值：playlist.m3u8。<br/>说明：生成的播放列表名（对象名）为`ChannelName/PlaylistName` | String | 是       |

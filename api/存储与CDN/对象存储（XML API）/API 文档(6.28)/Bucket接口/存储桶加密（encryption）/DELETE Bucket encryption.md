...
要执行此接口，必须拥有 PutEncryptionConfiguration 权限。默认情况下，Bucket 的持有者直接有权限使用该 API 接口，Bucket 持有者也可以将权限授予其他用户。
...
以下示例表示从存储桶 examplebucket-1250000000中删除默认 SSE-COS 加密配置。

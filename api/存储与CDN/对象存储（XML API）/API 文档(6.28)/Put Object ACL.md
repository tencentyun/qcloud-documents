## 功能描述
Put Object ACL 接口用来对某个 Bucket 中的某个的 Object 进行 ACL 表的配置，您可以通过 Header：`"x-cos-acl"`，`"x-cos-grant-read"`，`"x-cos-grant-write"`，`"x-cos-grant-full-control"` 传入ACL 信息，或者通过 Body 以 XML 格式传入 ACL 信息。
>注：
...
|节点名称（关键字）|父节点|描述|类型|必选|
|:---|:-- |:--|:--|
| ID | AccessControlPolicy.Owner |  Object 资源持有者 ID，</br>格式：`qcs::cam::uin/<OnwerUin>:uin/<SubUin>` 如果是根帐号，`<OnwerUin>` 和 `<SubUin> `是同一个值 |  String |是|
...
| 节点名称（关键字）          |父节点 | 描述                                    | 类型        |必选|
| ------------ | ------------------------------------- | --------- |:--|
| Grantee | AccessControlPolicy.AccessControlList.Grant | 说明被授权者信息。type 类型可以为 RootAcount， SubAccount；</br>当 type 类型为 RootAcount 时，可以在 uin 中填写 QQ，也可以填写 anonymous（指代所有类型用户）。</br>当 type 类型为 RootAcount 时，uin 代表根账户账号，Subaccount 代表子账户账号  | Container    |是|

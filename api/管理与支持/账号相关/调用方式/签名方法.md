...
3. 在 [云 API 访问密钥管理](https://console.cloud.tencent.com/capi) 页面，单击【新建】即可以创建一对 SecretId/SecretKey，每个账号最多可以拥有两对 SecretId/SecretKey。
...
### 2.2. 拼接请求字符串
此步骤生成请求字符串。
将把上一步排序好的请求参数格式化成“参数名称”=“参数值”的形式，如对 Action 参数，其参数名称为 “Action”，参数值为 “scribeInstances”，因此格式化后就为 Action=DescribeInstances。
...
- 请求主机：查看实例列表的请求域名为：cvm.api.qcloud.com。实际的请求域名根据接口所属模块的不同而不同, 详见各接口说明。
- 请求路径：API 的请求路径固定为 /v2/index.php。
- 请求字符串：上一步生成的请求字符串。
...
>!用户的请求方法是 GET，则对所有请求参数值均需要做 URL 编码。

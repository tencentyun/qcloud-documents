...
使用 application/json 格式，参数可选择 url 和 image， 此时 image 须为经 base64 编码后的 String，参数类型为 String；
使用 multipart/form-data 格式， 参数可选择 url 和 image， 此时 image 须为 Binary 格式， 参数类型为 Binary。
...
| image    | 否  | Binary 或 String | 图片二进制内容或经 base64 转码后的字符串                                |

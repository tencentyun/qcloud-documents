...
| border_check_flag  | 否   | Int | 切边检测告警开关(1:开启 0:关闭，默认0，如果置1，需要同时将enable_recognize_warn_code置为1)。      |
| ret_portrait_flag  | 否   | Int | 是否返回人像照片(1:是 0:否，默认0)      |
| enable_reshoot  | 否   | Int | 是否返回翻拍告警(1:是 0:否，默认0，选择是，需要同时将enable_recognize_warn_code置为1)     |
| enable_detect_copy  | 否   | Int | 是否返回复印件检测告警(1:是 0:否，默认0，选择是，需要同时将enable_recognize_warn_code置为1)     |
| enable_recognize_warn_code  | 否   | Int | 是否返回多重告警开关(1:是 0:否，默认0)     |

...
### 服务简介
图片标签V2版接口，用于根据用户上传的图像，识别并返回图片内包含的各种元素，以及属于这些元素的可能性。
...
| Content-Type   | application/json |  根据不同接口选择：<br/>1. 使用 application/json 格式，参数为 url ，其值为图片链接；2. 使用 multipart/form-data 格式，参数为 image，其值为图片的二进制内容。                                    |

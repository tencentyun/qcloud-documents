## 1. 接口描述
本接口提供图像文字识别 + 翻译功能，目前支持中文到英日韩，英日韩到中文的翻译。
...
| 参数名称 |必选 | 类型 |描述|
|---------|---------|---------|---------|
| data| 是 |  String |将待识别和翻译的图片转换为base64编码的字符串|
| ocrType | 否 | Int32 | 默认为0。<br> 0：对图片进行识别并按句子翻译 <br>1：对图片进行识别并按单词翻译|
| source | 是 | 	String |源语言（目前支持中文到英日韩，英日韩到中文的翻译）|
| target | 是 | 	String |目标语言（目前支持中文到英日韩，英日韩到中文的翻译）|
...
| jp | 日语 ||
| kr | 韩语 ||
|auto| 自动识别源语言| 只能用于source字段。当source为auto时，target目前只支持填入en或zh|
...
| message | String | 错误信息 |
| result | String | 图片识别并翻译后的文本，Json 格式。<br> 例如：[{ sourceText: 'hello',targetText: '你好'，x: 70, y: 29, w: 619, h: 79}] |

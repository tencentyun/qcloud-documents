...
### 2.2. Construct a request string
...
If another programming language is used, the original signature string in the above example can be used for verification, as long as the signature generated is the same as the one in the example.
...
When calling the API 3.0 in practice, you should use the corresponding Tencent Cloud SDK 3.0 which encapsulates the signature process, so that you can only focus on the specific APIs provided by the product during development. For more information, please see [SDK Center](https://cloud.tencent.com/document/sdk). The following programming languages are supported:

...
在开始迁移任务前，您需要先为您的源数据开启公网访问，并将 迁移任务 的 IP 地址添加到源数据库的白名单中，以便 迁移任务 可以与需要访问的数据库连通。如果未依照以下指引将 迁移任务相应地域的 IP 地址添加到源数据库的白名单中，将会因无法连通导致迁移失败。


#### 操作指引
根据源数据库所在地域，在下方获取对应地域 迁移任务 的 IP 地址，将其加入到源数据库的白名单中。
...
根据以下的配置校验项，检查您的源 Apollo 数据库配置正确：

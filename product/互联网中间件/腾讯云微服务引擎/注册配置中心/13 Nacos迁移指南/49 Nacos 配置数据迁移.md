...
Nacos 配置数据迁移是通过数据库层面的数据导入，实现将自建的 Nacos 集群数据库同步至 TSE Nacos 集群，在迁移过程中，新部署的服务暂不可用，存量服务不受影响。如果您需要不影响现网业务注册发现的平滑迁移，请配合 [Nacos 双注册双发现热迁移](https://cloud.tencent.com/document/product/1364/80035) 一起使用。

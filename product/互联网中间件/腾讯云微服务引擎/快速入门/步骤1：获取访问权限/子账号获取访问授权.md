...
1. 要将角色（及其许可策略）传递至 TSE 服务，用户必须具有**传递角色**至服务的许可且TSE需要获得访问其他云产品资源的能力，即子账号关联 tse_PassRole 和 QcloudAccessForTSERole 策略，详细操作参见 [授予 tse_PassRole 和 QcloudAccessForTSERole 策略](#步骤一：授予 tse_PassRole 和 QcloudAccessForTSERole 策略)。
2. 使用 TSE 的权限，您可以通过授予子账号 QcloudTSEFullAccess 策略给予子账号使用 TSE 的全量权限，详细操作参见 [授予 QcloudTSEFullAccess 策略](#步骤二：授予 tse_PassRole QcloudTSEFullAccess策略)。
...
### 步骤二：授予 tse_PassRole QcloudTSEFullAccess策略
1. 使用主账号登录 [访问管理控制台](https://console.cloud.tencent.com/cam)
2. 在左侧导航栏，单击**策略**，进入策略管理列表页。

...
1. TSE需要获取访问用户其他云产品资源的权限，如查看用户子网所在的可用区信息等场景。因此，需要将角色（及其许可策略）传递给 TSE 服务，即子账号关联 tse_PassRole 和 QcloudAccessForTSERole 策略，详细操作参见 [步骤一：授予 tse_PassRole 和 QcloudAccessForTSERole 策略](#步骤一：授予 tse_PassRole 和 QcloudAccessForTSERole 策略)。策略中，具体接口和使用场景详见[附录](#附录)
2. 子账号使用 TSE ，主账号需要授予相关的权限：**全量权限** 或 **指定资源的权限**。根据您的业务需要，您可自行选择赋予权限的范围，详细操作参见：
全量权限：子账号 QcloudTSEFullAccess 策略给予子账号使用 TSE 的全量权限[步骤二：授予全量权限或指定资源权限](步骤二：授予 全量权限 或 指定资源权限)；
指定资源权限：授予指定资源的权限，进行[资源级授权](这里需要「资源级授权」的链接)。


### 步骤一：授予 tse_PassRole 和 QcloudAccessForTSERole 策略

#### 一、新建 tse_PassRole 策略
...
#### 三、将 QcloudAccessForTSERole 策略绑定到用户
...
:::
::: 指定资源权限
详细操作请参见：资源级授权[资源级授权](这里需要「资源级授权」的链接)
:::

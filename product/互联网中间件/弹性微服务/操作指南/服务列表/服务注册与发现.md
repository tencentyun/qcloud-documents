...
如果选择了自动注入注册中心，在用户提交部署时，TEM 会自动把注册中心默认参数以 properties 文件形式，保存到环境中名为 `tse-config` 的 [ConfigMap](https://iwiki.woa.com/download/resources/com.oa.iwiki.cherry/res/ConfigMaps) 中，并通过 [VolumeMounts](https://kubernetes.io/docs/concepts/storage/volumes/#configmap) 的形式，挂载到应用的 `/config/tse-default-spring-cloud-config.properties` 目录下。

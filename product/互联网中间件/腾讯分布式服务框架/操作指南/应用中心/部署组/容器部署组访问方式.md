...
>**注意**:
> 1. 一个账号再单地域创建的公网负载均衡 LB 实例有数量限制，参考负载均衡 [使用约束](https://cloud.tencent.com/document/product/214/6187)。如需要扩大负载均衡实例限额，请提工单处理。
> 2. 目前 TSF 不支持 VPC 内网访问方式，如果用户需要设置 VPC 内网访问方式，可以在 TKE 控制台上进行设置。

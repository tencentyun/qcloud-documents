...
- **健康检查**：存活检查用于判断何时重启实例，就绪检查用于判断判定何时 Pod 变为 Ready状态，会影响滚动更新。详情参考 [Kubernetes 配置存活和就绪检查](https://kubernetes.io/zh/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/)。
- **环境变量**：设置应用容器设置的环境变量。

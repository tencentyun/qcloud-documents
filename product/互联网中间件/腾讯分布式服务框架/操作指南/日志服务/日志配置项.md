...
![](https://main.qcloudimg.com/raw/f6e9cd3d967015cc06097a0a01f02093.png)
   ...
   - **日志格式**：当“日志类型”为“自定义logback”、“自定义log4j”、 “自定义log4j2” 及 “单行/多行文本”时，需设置 [日志格式](#rzgs)
	   - 设置日志格式后，您可以通过“格式解析”功能，检验当前的设置是否正确
		   - 单击击“格式解析”，打开“日志格式解析”弹框，复制部分日志内容粘贴在“日志内容”区域，点击“解析”按钮，查看“解析结果”区域的输出是否符合预期
...
#### 4. 自定义 log4j
如果应用程序使用 logback 日志配置，设置日志格式为 logback，然后设置日志格式（即日志解析规则，对应 logback 中的 pattern）。可参考 Logback 中关于 [Pattern](https://logback.qos.ch/manual/layouts.html) 的介绍。参考模板如下：
- Pom：
	```xml
...
- Log4j2.xml（日志文件名应为log4j2.xml。请注意日志输出路径）：
```xml
		<?xml version="1.0" encoding="UTF-8"?>

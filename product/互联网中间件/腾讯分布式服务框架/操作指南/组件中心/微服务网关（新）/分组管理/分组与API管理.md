...
3. 填写分组名称和访问 context， 选择鉴权类型。
   ...
   - 托管 API 类型：当前微服务网关可以托管两种不同类型的 API：微服务 API，即部署在 TSF 上的微服务的 API；外部 API，即后端服务未部署在 TSF 上或未注册到 TSF 的注册中心的服务的 API。托管外部 API 时，需要用户手动填写后端服务的 Host 地址。
...
3. 支持两种导入方式：导入某个微服务下全部 API 和导入某个微服务下部分 API。

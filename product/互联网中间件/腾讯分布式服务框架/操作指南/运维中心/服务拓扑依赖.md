...
5. 选择之后将在下方空白处出现对应的服务依赖调用关系，单击右下角【查看图例】可查看相关说明。
   ...
   - 服务间带箭头曲线上的数字表示两个服务间调用的平均耗时。
     ...
     在选中时间范围内，consumer-demo 调用了 provider-demo 服务，调用成功比例为 100% （绿色部分），其中平均每次调用耗时3.23ms。      
   >?
...
| Tolerating<br><nobr>（可容忍）</nobr>| 应用响应时间大于 T，但同时小于或等于 4T。| 例如，应用设定的T值为1s，则4 × 1=4s为应用响应时间的容忍上限。|

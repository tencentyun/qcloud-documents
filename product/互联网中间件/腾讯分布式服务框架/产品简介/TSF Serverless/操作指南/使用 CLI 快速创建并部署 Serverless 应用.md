# 使用 CLI 快速创建并部署 Serverless 应用

## 操作场景
本文指导您如何使用 CLI 工具创建并部署 Serverless 应用。
...
## 相关说明
- 在安装 TCCLI工具时，**需要预先[配置秘钥和地域相关的用户信息](https://cloud.tencent.com/document/product/440/34012)**。
- 在安装 COSCMD时，**无需配置COS的相关信息**。下述脚本会自动完成对COSCMD的配置。


## 操作步骤

### 1. 使用 CLI 快速创建 Serverless 应用
参考以下脚本，快速创建并部署一个 Serverless 应用。在以下脚本里，我们会创建一个Serverless应用、上传程序包、创建部署组。

首先新建 shell 脚本，文件名称可自定义：
...
# 这里需要按照您实际部署的region进行调整，例如ap-shanghai、ap-beijing
REGION="ap-guangzhou"
...
# 当前需要额外上传一份spec.yaml（注意控制台上传程序包不需要该文件），后续我们会对此进行优化
echo "
...
# 填写应用ID，您可以通过前一个脚本或者控制台查看您的应用ID
...
# 获取部署组（若应用下有多个部署组，默认获取更新时间最新的部署组）的ID和状态

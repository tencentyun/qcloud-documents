...
当前 TSF Serverless 主要支持东西向调用场景（Java），兼容南北向调用。南北向调用场景下（Python，Nodejs，GO等环境）暂时不支持东西向功能。
...
- stop.sh**（必填）**：停止脚本。我们会通过运行停止脚本来终止程序运行。需要注意的是，停止脚本需要放在**压缩包的根路径下**。以下是一个示例停止脚本：
```bash
#! /bin/bash
...
您需要**在项目的根路径（start.sh 所在路径）下**执行打包命令：
```bash
zip code.zip * -r
```

...
<parent>
		...
		<version>0.0.1-SNAPSHOT</version> <!-- 调整为 SDK 最新版本号 -->
</parent>
...
##### 2. 修改配置文件
由于轻量级服务注册中心（原生的 consul）的 metadata 只能支持512个字节，因此您需要关闭 TSF 的 API 上报能力。
在配置文件 `application.yml` 中关闭 swagger：
```
tsf:
  swagger:
    enabled: false
```
如果省略了该步骤，在本地运行 Demo 时将会报错，错误信息中包含 `Value is too long (limit 512 characters)`。
...
<!-- consul sdk依赖的版本控制参考 Demo 或之前 pom 说明-->
```

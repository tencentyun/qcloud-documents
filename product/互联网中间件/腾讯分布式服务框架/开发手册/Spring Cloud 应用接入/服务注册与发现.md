...
在 IDE 中启动，通过 VM options 配置启动参数 `-Dtsf_consul_ip=127.0.0.1 -Dtsf_consul_port=8500`，通过 main 方法直接启动。其中 IP 和 port 取值为轻量级服务注册中心的地址。
...
其中 127.0.0.1 和 8500 为轻量级服务注册中心地址，在本地调试时 tsf.app_id 和 tsf.group_id 可以填任意值。

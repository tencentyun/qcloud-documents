...
由于轻量级服务注册中心（原生的consul）的metadata 只能支持512个字节，因此 TSF 的API 上报能力需要关闭。在 配置文件 `application.yml` 中关闭 swagger。
...
如果省略了该步骤，在本地运行 demo 时会报错，错误信息中包含： `Value is too long (limit 512 characters)`。

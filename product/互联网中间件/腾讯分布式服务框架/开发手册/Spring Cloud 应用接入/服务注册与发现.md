...
> **注意：**由于轻量级服务注册中心（原生的 consul）的 metadata 只能支持512个字节，因此需要关闭 TSF 的 API 上报能力 `-Dtsf.swagger.enabled=false`，如果没有这个启动参数，在本地运行 Demo 时将会报错，错误信息中包含`Value is too long (limit 512 characters)`。

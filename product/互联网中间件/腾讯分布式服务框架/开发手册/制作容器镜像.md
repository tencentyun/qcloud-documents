...
ENV workdir /app/
# 其中provider-demo-0.0.1-SNAPSHOT.jar 来自于第一步下载并编译的程序包，需要注意这个 jar 包要和您的 dockerfile 位于同一级目录
...
>! 对于私有化的 TSF ，需要注意对于 1.13 版本之前的版本，要支持 stdout 日志，需要在启动命令中将 stdout 及 stderr 重定向到一个文件中。将上文的`CMD`一行替换成：
```

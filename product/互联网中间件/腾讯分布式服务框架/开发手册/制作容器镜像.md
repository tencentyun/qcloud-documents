...
`start.sh` 不是必须的文件，如果用户不需要通过控制台来设置  JVM 启动参数设或者显示 stdout 日志，可以不使用 `start.sh` 脚本。
...
```
|-- apis			// 存放 API 定义的目录
|	|-- user.yaml	// user服务的API定义
|-- spec.yaml 		// 服务描述文件
|-- start.sh 		// 启动脚本
|-- userService.py	// 业务代码
```

关于 API 定义、服务描述文件和启动脚本的格式说明 [Mesh Demo 介绍](<https://cloud.tencent.com/document/product/649/30436>)。

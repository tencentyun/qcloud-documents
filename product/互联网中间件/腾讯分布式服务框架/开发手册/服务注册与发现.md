...
本地开发调试时，需要启动轻量级注册中心。轻量级注册中心包含了 TSF 服务发现注册功能，详细文件及下载见[轻量级服务注册中心]()。
...
<parent>
 ...
 <dependencies>
    ...
    <!--目前需通过dependencyManagement中显示指定tsf consul各sdk的版本，配置较为繁琐-->
    <!--后续会通过命名规范、仓库发布、版本管理来优化这里的体验-->
    <!--现可通过直接复制添加demo中的dependencyManagement进行相应配置-->
...
如果您的工程不想将parent设置为spring-boot-starter-parent, 也可以通过如下方式添加 dependencyManagement ，设置 scope=import，来达到依赖管理的效果。
...
在resource中的application.properties 文件中配置应用名与监听端口号。
...
> 需确认com.ecwid.consul consul-api的依赖版本为 `1.3.0`。

因为在这里要演示FeignClient的使用，所以与`provider-demo`相比，`pom.xml`文件中的依赖增加了一个`spring-cloud-starter-feign`。
...
本地开发调试时，需要使用轻量级注册中心，轻量级注册中心包含了TSF服务注册发现服务端的轻量版，详细文档请参考见。
...
在IDE中启动，通过VM options配置启动参数 `-Dtsf_consul_ip=127.0.0.1 -Dtsf_consul_port=8500`，通过main方法直接启动。其中ip和port取值为轻量级服务注册中心的地址。
...
将打包好的 FatJar 程序包上传到TSF控制台，进行部署操作，无需关心额外配置。部署相关操作可参考腾讯云官网上 TSF 的[使用手册](https://cloud.tencent.com/document/product/649)。

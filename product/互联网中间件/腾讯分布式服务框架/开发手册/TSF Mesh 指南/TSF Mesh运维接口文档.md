# 1 运维接口
service mesh微服务架构的核心是在用户的服务侧同机(虚拟机)或同Pod(容器)部署一个网络代理来让用户实现无侵入的接入微服务。因为代理是以本地额外的服务实现的，本地应用无感知，为了快速定位出现的问题，代理测提供了大量的http运维接口。

# 2 代理的组件说明
本地代理分成三个组件pilot-agent, mesh-dns和envoy，功能如下
pilot-agent:
> 管理整个代理侧环境，如iptables规则的更新，本地应用和服务的配置管理，同时负责mesh-dns和envoy组件的生命周期管理

mesh-dns:
> 托管本地的dns请求，将mesh结构内的流量导入本地代理

envoy:
> 负责服务发现和路由，http请求的负载均衡等，负责处理流入本地服务和从本地服务流出的所有流量

# 3 iptables规则说明
为了将流入和流出的流量导入到本地代理中，tsf mesh使用了iptables作流量转发，一般规则如下(vm环境在本地，容器环境在sidecar容器中)：
iptables -t nat -L -n
```
...
**DNS_OUTPUT:**
> 1 托管本地的dns请求到mesh-dns进程
> 2 第一条RETURN规则非常重要，不再托管从mesh-dns转发出来的dns请求
> 3 后面两条是托管本地的53端口(即dns原生请求端口)的流量

**ISTIO_INBOUND:**
> 1 托管访问本地服务的流量
> 2 可以看到托管的流量非常细粒度，只托管注册到mesh框架的9.77.7.28:8089的流量

**ISTIO_OUTPUT：**
> 1 托管从本地流出的流量
> 2 第一条RETURN规则非常重要，不再托管从代理转发出来的请求
> 3 第二条是将访问{特定ip}-从dns中获得，的流量导入到本地的代理中(即引用的ISTIO_REDIRECT规则)

# 4 本地接口
如果在本地调用`netstat -lntp | grep 127.0.0.1`，会出现三个listen服务
```
...
```
其中主要的接口如下：
> /clusters: 查看各个部署组下的实例信息和健康信息，比如查看本地服务节点的健康信息`curl -s 127.0.0.1:15000/clusters | grep "in#"`

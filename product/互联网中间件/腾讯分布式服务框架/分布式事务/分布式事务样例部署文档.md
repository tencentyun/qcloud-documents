...
```
5. 配置数据库地址。 
由于完整版本 Demo 需要连接 MySQL 数据库，因此您需要对每个服务配置数据库的地址。您可以使用 TSF 的 [应用配置](https://cloud.tencent.com/document/product/649/15539) 功能进行配置，每个服务的配置文件是各个服务的 resources 中的 application.yml，例如 couponService 的配置文件如下：
```
spring:
  application:
    name: "couponService"
...
server:
  ...
  address: 0.0.0.0
```
只需要按照实际修改 MySQL 的 url 即可。
6. 在安装了 mainService 的机器上通过 HTTP 请求 mainService。
（如果 mainService 所在的实例有公网 IP，也可以直接通过公网 IP 访问。）
```
    curl -X POST \

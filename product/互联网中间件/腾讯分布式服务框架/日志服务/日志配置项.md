...
2. 单击左侧导航栏 **日志服务>日志配置**。
3. 单击【新建配置】，在“创建日志配置”页面中设置您的日志配置项信息。

![](https://main.qcloudimg.com/raw/f6e9cd3d967015cc06097a0a01f02093.png)
   ...
   - **日志格式**：当“日志类型”为“自定义logback”、“自定义log4j”、 “自定义log4j2” 及 “单行/多行文本”时，需设置日志格式
	   - 设置日志格式后，您可以通过“格式解析”功能，检验当前的设置是否正确
		   - 点击“格式解析”按钮，打开“日志格式解析”弹框，复制部分日志内容粘贴在“日志内容”区域，点击“解析”按钮，查看“解析结果”区域的输出是否符合预期

	   ![](https://main.qcloudimg.com/raw/a3805a64b5ff8568dd7074e026d6c129.png)
	   ![](https://main.qcloudimg.com/raw/8672e4cfbe2ca61d0e1a036b6ca9e4a8.png)
   - **备注**：选填，日志配置项的描述信息。
4. 单击【提交】按钮。

### 日志格式详解
说明：请尽量**保持以年-月-日做日志开头**（如：2019-09-21 11:09:48.395，因为分割日志时以这种格式作为分割）。否则，可能导致日志显示分行有问题，甚至显示不出来日志。
...
3. 自定义 Logback
	- 如果应用程序使用 logback 日志配置，设置日志类型为 logback，然后设置日志格式（即日志解析规则，对应 logback 中的 pattern）。可参考 Logback 中关于 [Pattern](https://logback.qos.ch/manual/layouts.html) 的介绍。
	- 参考模板：
		- Pom：
		...
		- Logback.xml（*注：使用时，应用应该依赖有LogbackTraceConverter类的sdk）
...
4. 自定义log4j
	- 如果应用程序使用 logback 日志配置，设置日志格式为 logback，然后设置日志格式（即日志解析规则，对应 logback 中的 pattern）。可参考 Logback 中关于 [Pattern](https://logback.qos.ch/manual/layouts.html) 的介绍。
	- 参考模板：
		- Pom：
		...
		- Log4j.xml（*注：使用时，应用应该依赖有Log4JPatternLayout类的sdk。请注意日志输出路径）：
...
5. 自定义log4j2
	- 如果应用程序使用 log4j2 日志配置，设置日志格式为 log4j2，然后设置日志格式。
	- 参考模板：
		- Pom：
		...
		- Log4j2.xml（*注：日志文件名应为log4j2.xml。请注意日志输出路径）：
...
6. 单行/多行文本
	- 当日志为自定义格式时，可选择日志类型“单行/多行文本”，日志日期时间部分提供四种可选的格式，然后设置剩余部分内容的格式
		- 日期格式支持：
...
1. 登录 [TSF 控制台](https://console.cloud.tencent.com/tsf/index)。
2. 单击左侧导航栏 **日志配置**。
3. 单击日志配置项列表右侧的【操作】-->【更多】-->【删除】按钮。
     ![](https://main.qcloudimg.com/raw/9fb9ffe1c8133eafc7fb51575718c207.png)
4. 在删除确认框中单击【确认】。

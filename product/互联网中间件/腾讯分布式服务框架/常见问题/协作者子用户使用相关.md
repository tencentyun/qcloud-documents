...
### 当子账号使用 TSF 平台 弹窗提示 “您没有权限执行此操作”如何解决？

子账号使用 TSF 平台时，可能报错 “您没有权限执行此操作”，举例如下：

![](https://main.qcloudimg.com/raw/7e7d98a5cfe1a21c597eeacc61f2ba41.jpg)

在使用 TSF 平台时，需要调用其他云产品的接口获取一些信息，包含 VPC、CVM、API网关、镜像仓库、云监控、TKE等云产品。

建议对子账号进行如下策略的授权：

| 策略名称                  | 策略说明                                                     |
| ------------------------- | ------------------------------------------------------------ |
| QcloudAccessForTSFRole    | 腾讯微服务平台(TSF) 操作权限含获取云服务器(CVM)实例信息、重置CVM实例等；含获取私有网络(VPC)信息等；含获取容器服务(TKE)集群信息等。含创建、查询API网关服务以及创建查询API信息等。 |
| QcloudMonitorFullAccess   | 云监控（MONITOR）全读写访问权限，包括查看用户组的权限        |
| QcloudAPIGWReadOnlyAccess | API网关（API Gateway）只读访问权限                           |

...
在 CVM 或 TSF 中部署刚刚打包好的示例工程 jar 包。请确保 CVM 或 TSF 中用于部署的机器，与数据库在同一 VPC 下。若使用腾讯云CVM 与 云数据库MySQL，您可参考 [一键连接检查工具](https://cloud.tencent.com/document/product/236/33206) 检查二者是否处于同一 VPC 下。

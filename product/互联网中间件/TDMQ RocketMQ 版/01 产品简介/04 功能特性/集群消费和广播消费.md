本文主要介绍消息队列TDMQ RocketMQ版中集群消费和广播消费的概念定义和适用场景。
...
同一个 Group ID 所标识的所有 Consumer 都会各自消费某条消息一次。例如某个 Topic 有9条消息，一个Group ID 有3个 Consumer 实例，那么在广播消费模式下每个实例都会各自消费9条消息。
...
- 请确保同一个Group ID下所有Consumer实例的订阅关系保持一致。

本文主要介绍消息队列 TDMQ RabbitMQ 版中延迟消息的概念，应用场景和使用方式。
...
1. 声明 Exchange 并指定Exchange的路由类型。
   ...
   | 参数              | 说明                                                         |
   | :---------------- | :----------------------------------------------------------- |
   | x-delayed-type    | Exchange 的类型，指定路由规则。取值说明如下：<br><li>direct</li><li>fanout</li><li>topic</li><br/>具体说明请参考 [Exchange](https://cloud.tencent.com/document/product/1495/61826)。 |
   | ExchangeName      | Exchange的名称，在控制台 Exchange 列表获取。                 |

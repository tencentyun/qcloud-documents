...
## 前提条件
已具备2个可使用节点，其中1个节点机型至少配置8核16G内存。另1个节点无特殊要求，作为 installer 节点使用。本文均以操作系统为 CentOS 7.6 的腾讯云云服务器为例。可参考 [创建实例](https://tcloud-doc.isd.com/document/product/213/4855) 进行创建。
...
主要参数信息如下：
	...
	- **master节点**：
	 ...
	 - **SSH端口**：请确保目标机器安全组开放 SSH 端口和 ICMP 协议，否则无法远程登录和 PING 通。本文以22端口为例。

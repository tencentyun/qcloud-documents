...
## 前提条件
已具备可使用节点，本文以操作系统为 CentOS 7.6 的腾讯云云服务器为例。可参考 [快速配置 Linux 云服务器](https://tcloud-doc.isd.com/document/product/213/2936) 进行创建。

## 操作步骤
### 平台安装
1. 执行以下命令，下载 tke-installer 安装器至节点的 `/data` 目录下。
```
...
```
3. 按照以下格式访问 TKE Stack 控制台，开始控制台安装。
```
http://云服务器公网 IP:8080/index.html
```

### 控制台安装
1. 在 TKE Stacke “基本设置”页面，参考以下提示进行设置并单击【下一步】。如下图所示：
...
主要参数信息如下：
	...
	- **容器网络**：将为集群内容器分配在容器网络地址范围内的 IP 地址，您可以自定义三大私有网段作为容器网络。根据您选择的集群内服务数量的上限，自动分配适当大小的 CIDR 段用于 Kubernetes service。根据您选择 Pod 数量上限/节点，自动为集群内每台服务器分配一个适当大小的网段用于该主机分配 Pod 的 IP 地址
		...
		- **Pod数量上限/节点**：分配给每个 Node 的 CIDR 的大小。
		- **Service数量上限/集群**：分配给 Sevice 的 CIDR 大小。
	- **master 节点**：
	 - **访问地址**： Master 节点的内网 IP，**请配置至少8核16G内存及以上的机型**。
	 - **SSH端口**：请确保目标机器安全组开放 SSH 端口和 ICMP 协议，否则无法远程登录和 PING 通。
...
![](https://main.qcloudimg.com/raw/1413a363eb8b50b0120c0aa13cef87c1.png)
	- **镜像仓库类型**：请按需设置，本文以【TKE提供】为例。
		...
		- **第三方仓库**：已配置好的外部镜像仓库。若选择【带伞房仓库】，TKE Stack 将不会再安装镜像仓库，而是使用您提供的镜像仓库作为默认镜像仓库服务。
...
7. 在“配置预览”页面，确认 TKE Stack 控制台所有配置是否正确，并单击【安装】。
如下图所示，则安装成功。
![](https://main.qcloudimg.com/raw/83eee3292843650f62fae194d415671c.png)
8. 单击【查看指引】，参考弹出的“操作指引”窗口中的步骤配置域名，访问 TKE Stack 控制台。


## 下一步操作
请参考 [步骤2：创建并导入集群]() 完成集群创建操作。

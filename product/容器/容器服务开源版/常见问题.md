...
容器服务开源版（Tencent Kubernetes Engine Stack，TKE Stack）是一款集强壮性和易用性于一体的开源容器编排引擎。容器服务开源版基于 Docker 和 Kubernetes 实现了容器应用的全生命周期管理功能。您可以使用与公有云容器服务相同的控制台界面，方便地管理云上云下不同基础设施上的业务和资源，敏捷、高效地构建和发布应用程序。

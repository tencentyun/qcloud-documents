...
5. Redis集群：支持Codis集群，Codis可以兼容100多个客户端，并且对客户端屏蔽了集群中的节点数、分片等细节。
6. 分布式：用户的存储分布在多台物理机上，彻底摆脱单机容量和资源限制；

## 版本说明

社区版Redis引擎兼容Redis2.8版本协议和命令，社区版Redis4.0版本即将上线，自研CKV引擎兼容Redis3.2版本协议和命令。

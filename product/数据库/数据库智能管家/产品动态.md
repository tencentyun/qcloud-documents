...
<tr>
<td><a href="https:增加链接跳转到Redis特有诊断优化文件夹" target="_blank">DBbrain 支持 Redis</a></td>
<td>DBbrain 支持 Redis，支持功能包括：实例概览、实例管理、数据库巡检、异常告警、监控大盘、性能监控（性能趋势、实时性能）、实时会话、内存分析（Key分析、大Key分析）、访问分析（热Key分析、延迟分析、命令行分析）、慢日志分析、实时日志分析、健康报告等。</td>
<td>2020-07</td><td>-</td></tr>
</tbody></table>

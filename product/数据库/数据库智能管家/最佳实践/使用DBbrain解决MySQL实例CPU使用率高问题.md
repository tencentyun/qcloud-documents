...
### 二、使用慢 SQL 分析功能排查导致 CPU 使用率过高的 SQL
1. 登录 [DBbrain 控制台](https://console.cloud.tencent.com/dbbrain/analysis)，在左侧导航选择【诊断优化】，在上方选择【慢 SQL 分析】页。
3. 在左上角选择（可输入和搜索）目标实例，切换至目标实例。
4. 在页面中选择要查询的时间，若此实例在该时间段中有慢SQL，图示中会以柱形图的方式显示慢SQL产生的时间点和个数。单击柱形图，下方的列表就会显示其对应的所有慢SQL信息（模板聚合之后的SQL），右方会显示该时间段内SQL的耗时分布。
...
1. 登录腾讯云数据库管理控制台。
2. 在左侧导航栏中，选择 数据库智能管家DBbrain >SQL透视，进入SQL透视页。
3. 在左上角选择（可输入和搜索）目标实例，切换至目标实例。
...
6. SQL分析详情页下方表格中展示了聚合后的SQL执行的信息，包括执行次数、总延迟、最大延迟、最小延迟、总影响行数、最大影响行数、最小影响行数等。可根据各项信息的组合排序，识别出待优化的SQL。下面列举一个示例：

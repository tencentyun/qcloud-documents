...
>?异常告警目前仅支持云数据库 MySQL（不含基础版）。


## 异常告警列表
登录 [DBbrain 控制台](https://console.cloud.tencent.com/dbbrain/monitor)，在左侧导航选择【异常告警】页。
在异常告警列表中，展示了数据库实例基本信息（实例名/ID、内网 IP、可用区）、风险等级、诊断项、持续时长及操作等。在搜索栏，用户可根据实例 ID、实例名、内网 IP、诊断项等进行搜索，同时也支持按地域及时间进行筛选。

...
---|---|---|---
[qps] 表示每秒响应的查询次数 | [read] 表示 InnoDB 存储引擎表的读取记录行数 | [running] 表示活跃的连接数，即正在执行 sql 的连接 |[in] 表示进入实例的网络流量
[tps] 表示每秒处理的事务个数 | [insert] 表示 InnoDB 存储引擎表的写入记录行数| [connected] 表示连接在实例上的空闲连接，即未执行 sql 的连接| [out] 表示流出实例的网络流量
...
## 内嵌 phpMyAdmin
在导航栏单击【前往PMA】可进入腾讯 DMC 内置的 phpMyAdmin 对数据库进行操作，phpMyAdmin 支持大多数 MySQL功能，包括：
- 浏览和删除数据库、表、视图、字段以及索引。
- 创建、复制、删除、重命名以及更改数据库、表、字段、索引。
- 维护服务器、数据库和表、以及有关的服务器配置。
- 执行、编辑 SQL 语句。
- 管理 MySQL 用户帐户以及配置权限。
- 管理存储过程和触发器。

...
>?
>- 慢 SQL 分析目前支持云数据库 MySQL（不含单节点 - 基础型）、云原生数据库 TDSQL-C（TDSQL-C for MySQL）、自建数据库 MySQL、云数据库 MongoDB（请参考 [MongoDB 慢 SQL 分析](链接到MongoDB 特有诊断优化文件夹下的对应链接))。Redis 的慢日志分析与 MySQL 和 TDSQL-C 不同，请参见 [Redis 慢日志分析](https://cloud.tencent.com/document/product/1130/58851)。

...
- 修复实例启动后大事务回滚与 shutdown 操作并发时可能导致实例 crash 的问题。
- 修复 undo space truncate 失败重试时相关页面 io 重试多次失败导致实例退出的问题。
- 修复只读实例关闭时统计信息更新访问快照缓存导致 crash 的问题。
- 修复 undo space truncate 可能存在 truncate log 落后于 truncate 操作的问题。
- 修复官方 bugfix，分区表扫描 ICP 检查错误导致查询慢的问题。
- 修复只读实例中前项扫描遇到索引分裂日志部分回放导致 crash 的问题。

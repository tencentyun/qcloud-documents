...
>?如不满足 TDSQL-C MySQL 版内核版本要求，可先升级数据库内核版本，详细操作请参见 [升级内核小版本](https://cloud.tencent.com/document/product/1003/61541)，如需了解数据库代理各个内核版本发布时间，请参见 [数据库代理内核版本发布时间](https://cloud.tencent.com/document/product/1003/61514#DLNHFBSJ)。
>
...
<td><strong>问题修复</strong><li>修复了某些情况 select for update 语句路由错误的问题。</li><li>更改了 select @@read_only 语句路由，现在 select @@read_only 会被路由到主库，避免某些框架使用 read_only 标记，错误判断数据库代理不可写的问题。</li><li>修复了部分场景下数据库实例 HA 引起数据库代理节点异常的问题。</li></td></tr>
<tr>
<td>1.3.5</td>
<td>TDSQL-C MySQL 版 5.7 ≥ 2.0.20/2.1.6<br>TDSQL-C MySQL 版 8.0 ≥ 3.1.6</td>

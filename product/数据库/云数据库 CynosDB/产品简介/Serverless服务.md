Serverless 是腾讯自研云原生数据库 TDSQL-C MySQL 版的无服务器架构版，是全 Serverless 架构的云原生数据库。Serverless 服务支持按实际计算和存储资源使用量收取费用，不用不付费，将腾讯云云原生技术普惠用户。
>?仅云原生数据库 TDSQL-C MySQL 版支持 Serverless 模式。

...
| 查询数据库中的账号 | ```SELECT user,host,password FROM mysql.user_view;```<br>或<br>```show grants for xxx``` | 
| 权限回收 | 收回全部权限<br>```REVOKE ALL PRIVILEGES,GRANT OPTION FROM 'username'@'host';```<br>收回指定权限<br>```REVOKE UPDATE ON *.* FROM 'username'@'host';``` | 

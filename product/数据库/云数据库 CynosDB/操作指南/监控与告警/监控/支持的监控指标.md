...
<td>每秒执行事务数</td><td>tps</td><td>个/N</td></tr>
...
<td>键缓存使用率</td><td>key_cache_use_rate</td><td>%</td></tr>
<td>键缓存内未使用的块数量</td><td>key_blocks_unused</td><td>个</td></tr>
<td>键缓存内使用的块数量</td><td>key_blocks_used</td><td>个</td></tr>
<td>键缓存读取数据块次数</td><td>key_read_requests</td><td>个/N</td></tr>
<td>硬盘读取数据块次数</td><td>key_reads</td><td>个/N</td></tr>
<td>数据块写入键缓冲次数</td><td>key_write_requests</td><td>个/N</td></tr>
<td>数据块写入磁盘次数</td><td>key_writes</td><td>个/N</td></tr>
<td rowspan="1">其他</td>
<td>打开文件总数</td><td>open_files</td><td>个</td></tr>
<td rowspan="3">部署监控</td>
<td rowspan="3"></td>
...
- 客户端发送和接收流量：通过观察监控指标-资源监控-发送或接收客户端流量数据的波动情况，进而检查是业务峰值带来的问题还是数据库的问题，从而进行业务优化或者数据库配置的调整。

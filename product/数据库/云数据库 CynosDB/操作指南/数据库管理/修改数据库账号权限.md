TDSQL-C MySQL 版支持授权和修改数据库的账号权限，由于 root 帐号默认拥有 TDSQL-C MySQL 版集群下所有数据库的读写权限，所以不支持对 TDSQL-C MySQL 版集群下的数据库进行 root 帐号的授权修改，您可以在数据库管理页面对已创建的数据库授权其他帐号对其进行读写、只读等的权限设置。

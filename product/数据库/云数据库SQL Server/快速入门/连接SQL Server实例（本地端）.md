## 操作场景
本文为您介绍借助具有外网 IP 的 Linux 云服务器进行端口映射，并通过 SQL Server Management Studio（SSMS）连接到实例运行简单查询的操作。

## 操作步骤
考虑到数据的安全，云数据库 SQL Server 尚未开放实例外网 IP，如有需求可以利用 SSH2 的端口映射在外网连接实例，并对其进行配置和管理。本文以 SecureCRT 为例。
...
3. 在本地使用 SSH 工具（如 SecureCRT 或 PuTTY 等）登录 Linux 云服务器，请参见 [登录 Linux 实例](/doc/product/213/5436)。

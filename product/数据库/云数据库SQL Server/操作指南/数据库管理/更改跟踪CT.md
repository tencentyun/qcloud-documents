...
更改跟踪（CT）可以应用跟踪到具体一个数据库中的具体表甚至是具体列，在用户对启用了更改跟踪的表进行了增加、修改和删除操作时，系统自动将该操作生成一个版本号，记录下操作的时间戳、操作的类型、受影响数据的主键等信息。

启用更改跟踪，它只记录表中的行是否有过更改，而不记录更改的历史数据，故对数据操作的性能影响不是很大。这些信息将记录到 SQL Server 系统表中的，系统自动负责清理和维护。

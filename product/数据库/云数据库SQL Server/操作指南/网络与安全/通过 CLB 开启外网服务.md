云数据库 SQLServer 支持内网和外网两种地址类型，默认提供内网地址供您内部访问实例，如果需要使用外网访问，可通过负载均衡 CLB 绑定外网地址，绑定 CLB 开启外网服务必须配置安全组规则才能访问。

以下为您介绍通过绑定 CLB 开启外网服务，并通过 SQL Server Management Studio（SSMS）连接到实例，运行简单查询的操作。
## 步骤1：新购负载均衡

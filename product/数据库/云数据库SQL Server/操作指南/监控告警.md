为方便用户查看和掌握实例的运行信息，云数据库 SQL Server 提供了丰富的性能监控项与便捷的监控功能（自定义视图、时间对比、合并监控项等）。用户可登录 [SQL Server 控制台](https://console.cloud.tencent.com/sqlserver)，进入实例管理页的【实例监控】查看监控信息。

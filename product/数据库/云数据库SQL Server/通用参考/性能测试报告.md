## 测试工具
本次性能测试使用的是 HammerDB 内置的 TPC-C 基准测试负载。TPC-C 是一种典型的 OLTP 工作负载，其模拟了拥有多个仓库的商品批发销售公司为大量客户发货商品的场景，其中仓库数的调整在测试中能够体现数据库所能够支持的数据规模的能力。
...
## 测试方法
1. 准备TPC-C 工作负载。
![img](file:///C:/Users/V_GYYU~1/AppData/Local/Temp/msohtmlclip1/01/clip_image002.gif)![img](file:///C:/Users/V_GYYU~1/AppData/Local/Temp/msohtmlclip1/01/clip_image004.jpg)
如图所示：
Number of Warehouses 为设置仓库个数，将影响生成测试库的大小。
2. 设置测试脚本。
![img](file:///C:/Users/V_GYYU~1/AppData/Local/Temp/msohtmlclip1/01/clip_image006.jpg)
如图所示：
Minutes of Rampup Time 为压测预热时间。
Minutes Test Duration 为压测运行时间。
Total Transactions per User 为每个用户的总事务数。建议设置较高的值保证在压测时，User不会因为事务运行完而退出。
3. 设置自动化测试脚本。
![img](file:///C:/Users/V_GYYU~1/AppData/Local/Temp/msohtmlclip1/01/clip_image008.jpg)
Minutes per Test in Virtual User Sequence 为自动化测试的间隔时间，在这个时间内程序要完成创建虚拟用户，预热，运行测试，停止测试等工作。应选择比Minutes of Rampup Time + Minutes Test Duration 长。
Active Virtual User Sequence 为自动化测试每一次迭代产生的虚拟用户数，这里虚拟用户数可以理解为并发连接数。
4. 开始测试以及输出结果。
点击 Autopilot/Autopilot 开始测试
![img](file:///C:/Users/V_GYYU~1/AppData/Local/Temp/msohtmlclip1/01/clip_image010.jpg)
测试结果输出在hammerdb.log文件中
![img](file:///C:/Users/V_GYYU~1/AppData/Local/Temp/msohtmlclip1/01/clip_image012.jpg)

 
## 测试环境和参数设置
- 实例所在物理机
具有80核心767GB内存30TB存储的高性能物理机
- 测试实例规格
本次测试的版本覆盖2008企业版，2012企业版，2016企业版，2017企业版。规格覆盖1C2G, 1C4G, 1C8G, 2C16G, 4C32G, 8C64G, 12C96G, 16C128G, 32C256G。几乎覆盖当前售卖的所有规格。
- 负载生成环境
安装HammerDB的机器，与数据库实例型号一致，能保证能充分压测出SQL Server实例性能。
- TPCC基准参数
Number of Warehouses = 100，设置仓库个数为100，将生成测试数据库大小约55G
Minutes of Rampup Time = 2，设置测试前预热时间2分钟。
Minutes Test Duration = 5， 设置测试时间5分钟。
- 虚拟用户数
虚拟用户数即并发连接连接数。在不同规格的实例上，本文分别测试了不同的并发连接数。
...
## 测试结果
**性能指标**
HammerDB中的TPM是通过SQL Server性能计数器batch requests/sec 得到的，因此TPM实际上是指 batch requests per minute。本文测试以性能计数器的batch requests/sec值作为QPS来展示。

**性能数据**
![img](file:///C:/Users/V_GYYU~1/AppData/Local/Temp/msohtmlclip1/01/clip_image014.gif)
- SQL Server 2008 企业版

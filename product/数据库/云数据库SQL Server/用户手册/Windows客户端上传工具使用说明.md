...
注：conf.json文件请存储为“ UTF8 无 BOM 格式”（在windows下建议用notepad++转换编码）
...
4、在Windows命令行中进入解压后的“ Windows 客户端上传工具”目录，调用 bin 目录下的 upload-tool.exe 完成上传操作。upload-tool.exe 有两个参数 –r 和 –p。–p 表示备份文件在本地的绝对路径；–r表示中转存储所处的地域（请选择您的腾讯云数据库所在地域）；

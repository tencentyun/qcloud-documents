为方便用户查看和掌握实例的运行信息，TDSQL PostgreSQL版 提供了丰富的性能监控项。用户可登录 [TDSQL PostgreSQL版 控制台](https://console.cloud.tencent.com/tbase)，单击实例 ID 进入实例管理页面，在系统管理页查看监控。
>?
...
## 监控指标
### 实例维度
腾讯云云监控从实例维度为 TDSQL PostgreSQL版 实例提供以下监控指标：
...
### 节点维度
腾讯云云监控同时从节点维度为 TDSQL PostgreSQL版 提供以下监控指标：

| 指标中文名 | 指标英文名 | 单位 |指标说明|
|---------|---------|---------|---------|
|CPU 利用率|cpu_used_pct|%|节点的 CPU 使用率的最大值|
|内存利用率|mem_used_pct|%|节点的内存使用率的最大值|
|IO 吞吐量|iops|次/s|节点的磁盘吞吐率|
|缓存命中率|cache_hit_pct|%|数据缓存命中率|
...
|其他请求数|other_requests|次|除了读和写以外的请求总数，按分钟累加|
|错误请求数|error_requests|次|实例上记录的所有错误的请求数之和，按分钟累加|
|残留两阶段事务数目|two_phase_commit_trxs|个|节点的中 prepared 阶段在10分钟之前的事务数之和|
|容量使用率|capacity_used_pct|%|节点的容量使用率|
|容量已使用|capacity_usage|GBytes|节点的已使用容量|
|剩余 XID 数量|xid_remain|Count|节点的所有 CN、DN 上剩余 XID 的最小值|
|主备 XLOG 同步差异|xlog_diff|Bytes|节点的同步大小差异，代表同步延迟，越小越好|

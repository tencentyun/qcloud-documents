为方便用户查看和掌握实例的运行信息，TDSQL 提供了丰富的性能监控项。用户可登录 [TDSQL 控制台](https://console.cloud.tencent.com/tbase)，单击实例名进入实例管理页面，在系统管理页查看监控。
>?

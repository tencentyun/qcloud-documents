## 创建数据库
要创建一个数据库，你必须是一个超级用户或者具有特殊的 CREATEDB 特权，默认情况下，新数据库将通过克隆标准系统数据库 template1 被创建。可以通过写 TEMPLATE name 指定一个不同的模板。特别地，通过写 TEMPLATE template0 您可以创建一个干净的数据库，它将只包含你的 TDSQL PostgreSQL 所预定义的标准对象。

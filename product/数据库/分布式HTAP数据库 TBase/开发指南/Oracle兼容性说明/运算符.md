## 算术运算符
您可以使用算术运算符对 TDSQL PostgreSQL版 所支持数据类型的值进行操作。本文介绍 TDSQL PostgreSQL版 支持的算术运算符。
...
这里除法与 Oracle 运算结果精度稍有差异，TDSQL PostgreSQL版 运算结果小数点后为16位 - 20位，Oracle 运算结果小数点后最多9位，自动省略末尾0，小于1时自动省略个位数。
...
## 连接运算符
TDSQL PostgreSQL版 兼容 Oracle 的 SQL 连接符”||”，支持字符与字符、字符与数字、数字与数字类型的连接运算。
...
## 集合运算符
TDSQL PostgreSQL版 兼容 Oracle 的集合运算符 UNION、UNION ALL、INTERSECT、MINUS。
...
## 其它运算符
### (+) 
TDSQL PostgreSQL版 兼容 Oracle 的 SQL（左/右）外连接 (+) 写法，等同于 left | right join。

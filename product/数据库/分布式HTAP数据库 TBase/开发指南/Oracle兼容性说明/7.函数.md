## 7.1	数学 ##
### 7.1.1	MOD (character varying, integer) ###
...
## 7.2	日期时间 ##
### 7.2.1	ADD_MONTHS (timestamp, text)  ###
...
### 7.2.3 NUMTOYMINTERVAL

`NUMTOYMINTERVAL` 时间计算函数有 2 个参数。第 1 个通常为数字，第 2 个参数为单位，可选有 'year'、'month'（忽略大小写）。
...
### 7.2.6	MONTHS_BETWEEN ###
返回两个日期之间的月份数。

在 Oracle 中计算月份的差值，如果均是月底或是日期相同，都会按整月计算。如果计算结果不是整数，Oracle 会把 31 作为分母进行计算。

#### 使用限制

Oracle 和 TDSQL PostgreSQL 版（Oracle 兼容）的该函数实现相同，但精度不同，Oracle 有效数字为 9 位，TDSQL PostgreSQL 版（Oracle 兼容）为15位。
...
TDSQL PostgreSQL 版（Oracle 兼容）兼容 `EXTRACT` 函数用法。
...
1. 暂不支持 Oracle 用法中的参数 `timezone_region`、`timezone_abbr`；

2. 暂不支持 Oracle 用法中的日期计算，如：
...
### 7.4.1	NVL (character varying, integer) ###
...
### 7.5.1	DECODE (numeric, integer, text, numeric) ###
...
### 7.5.2	DECODE (bigint, integer, integer, integer) ###
...
### 7.5.3	DECODE(bigint, integer, integer, integer, integer) ###
...
### 7.5.4	DECODE(bigint, bigint, bigint, double precision) ###
...
### 7.5.5	DECODE(bigint, integer, integer, double precision) ###
...
### 7.5.6	DECODE (numeric, bigint, text, bigint, text) ###
...
其中：
`expr`：需要进行分析的表达式（数字或字符串等，可以是各个类型的值）。
...
3. `16`：以十六进制返回结果的值。
4. `17`：以单字符的形式返回结果的值。
5. `1000`：以上 4 种加 1000，表示在返回值中加上当前字符集：
   + `1008`：带字符集名称的八进制符号。
   + `1010`：带字符集名称的十进制符号。
   + `1016`：带字符集名称的十六进制符号。
   + `1017`：带字符集名称的单个字符。
...
1. `Typ` 表示当前的 `expr` 值的类型，类型查询SQL：`select typname from pg_type where oid=?；`。
...
3. 具体的存储值。返回的数值就是 TDSQL PostgreSQL 版（Oracle 兼容）在数据库内部的存储形式。对于非汉字的普通字符串，可以理解为就是它的 ASCII 码。
...
## 7.7	转换 ##
### 7.7.1	TO_CLOB ###
TDSQL PostgreSQL 版（Oracle 兼容）兼容 Oracle 的 `TO_CLOB` 函数，将字符串、数字转换为 `clob` 类型。
...
#### 示例
    ...
    ---------
     clob
    (1 row)
确认函数输出类型 `11899` 对应 `clob`。

### 7.7.2	ROWIDTOCHAR ###
Oracle 中 `ROWIDTOCHAR` 函数转换 `rowid` 为 `varchar2` 类型。TDSQL PostgreSQL 版（Oracle 兼容）兼容该函数，返回类型为 `text`。
...
TDSQL PostgreSQL 版（Oracle 兼容）中 `rowid` 长度为20位。
...
### 7.8.1	EMPTY_BLOB ###
TDSQL PostgreSQL 版（Oracle 兼容）兼容 Oracle 的 EMPTY_BLOB 函数，用于初始化 BLOB 字段，然后再使用输出流向字段中写数据。
...
### 7.8.2	EMPTY_CLOB ###
TDSQL PostgreSQL 版（Oracle 兼容）兼容 Oracle 的 EMPTY_CLOB 函数，用于初始化 CLOB 字段，然后再使用输出流向字段中写数据。

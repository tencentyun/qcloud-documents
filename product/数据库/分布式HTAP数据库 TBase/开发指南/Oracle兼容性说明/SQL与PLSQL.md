...
### 使用限制
- connect by 用法较多，当前 TDSQL PostgreSQL版 能支持常用的写法，某些写可能不支持。
...
此处 DBLINK 是指从 TDSQL PostgreSQL版 访问 Oracle，如需 Oracle 访问 TDSQL PostgreSQL版，可通过透明网关实现，TDSQL PostgreSQL版 访问 TDSQL PostgreSQL版 可用 postgres_fdw 扩展创建外部表实现。
...
- Oracle 字符集与 TDSQL PostgreSQL版 可能会有差异，当前 TDSQL PostgreSQL版 数据库字符集为 GBK 或 GB18030 时，DBLINK 访问 Oracle 端都会设置为 ZHS16GBK。
- 由于占⽤了关键字“@”，导致原 TDSQL PostgreSQL版 ⽀持的“@”操作符失效，如需使⽤，⽤ abs 函数代替之 。
...
PostgreSQL 只支持相同字段类型 UNION ALL 合并，TDSQL PostgreSQL版 则支持不同字段类型的合并。
...
## 支持只读事务时可以获取序列的下一个值
Oracle 支持只读事务时可以获取序列的下一个值，TDSQL PostgreSQL版 兼容 Oracle 该用法。
...
TDSQL PostgreSQL版 兼容 UNPIVOT 用法。
...
TDSQL PostgreSQL版 兼容 MERGE INTO 语法。
...
TDSQL PostgreSQL版 兼容 DELETE 时省略 FROM 关键字用法。
...
TDSQL PostgreSQL版 兼容常用 Oracle 的 HINT 用法，同时支持基于 pg_hint_plan 的 HINT 用法。

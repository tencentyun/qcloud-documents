...
1. TDSQL PostgreSQL 版序列当前为 `bigint` 类型，最大值为 `9223372036854775807`，超过该值报错。
2. 兼容 Oracle 用法，TDSQL PostgreSQL 版（Oracle 兼容）支持只读事务时获取序列下一个值。
3. Oracle 中 `NOCYCLE` 为不循环生成，TDSQL PostgreSQL 版（Oracle 兼容）中为 `NO CYCLE`，写法稍有差异。
4. 暂不支持 `Oracle` 中的参数 `NOCACHE`、`ORDER`、`NOORDER`。
5. 默认不循环生成，达到最大值时将报错；如需循环生成创建时可加参数 `CYCLE`。
...
## 3.2	同义词 ##
同义词是 Oracle 中数据库对象的一种别名，支持的对象有：表、视图、序列、运算符、函数、存储过程、包、物化视图、Java class模式对象、用户自定义对象类型以及其他同义词。同义词分为公共同义词、私有同义词。

#### 使用限制

1. 暂不支持同义词修改，如需修改，可先删除、再创建。
1. 暂需要超级用户创建、删除公有同义词，不支持普通用户操作。
1. 同义词名和被代表对象名不能重复。
1. 同义词不能循环解释。
1. 对象不存在时，仍能创建同义词，所以创建同义词后，须验证同义词是否可用。
...
1. 权限
   ...
   + 创建私有同义词，需要超级用户或者具有对应模式的 CREATE 权限。（Oracle：需要 CREATE ANY SYNONYM/CREATE SYNONYM 权限才可以创建同义词，在 TDSQL PostgreSQL 版（Oracle 兼容）中将同义词看成普通对象，简化创建同义词的权限管理。）
...
3. PUBLIC：创建公共同义词，所有用户都能够使用这种同义词，但是需要有被代表对象的访问权限。
   ...
   + 创建与模式同名同义词，导致 PLSQL 失效。Oracle 行为，TDSQL PostgreSQL 版（Oracle 兼容）不支持。不指定 PUBLIC 关键字，语句创建私有同义词。
   + 私有同义词在定义模式下名称唯一，并且也不能与其他对象（同义词支持的表、视图、函数等）重名。
   + 其他用户在私有同义词对应的对象如果有权限，那么是允许使用这些私有同义词的。

4. schema ：指定同义词存放的模式名。TDSQL PostgreSQL 版（Oracle 兼容）行为：会按照 search_path 值创建对象；如果search_path 中只有 PUBLIC 合法，则会创建公共同义词。（Oracle：如果不指定，存放到当前用户的模式下）
   + 指定PUBLIC关键字，就不能指定schema

5. synonym：同义词名
6. FOR 子句：指定被代表的数据库对象，对象不能是包里的。支持如下对象：
   + 表、对象表（TDSQL PostgreSQL 版（Oracle 兼容）： typed table， create table…of…）
   + 视图（对象视图属于 Oracle 特性，TDSQL PostgreSQL 版（Oracle 兼容）不支持）
   + 序列
   + 存储过程、函数、包
   + 物化视图
   + Java class schema object（Java存储过程）
   + 用户自定义类型
   + 同义词

7. schema：对象所在的模式，不指定 TDSQL PostgreSQL 版（Oracle 兼容）使用 search_path 查找对象（Oracle如果不指定，默认为当前用户模式）。Oracle 要求对远程函数存储过程定义同义词，必须指定模式，TDSQL PostgreSQL 版（Oracle 兼容）不做此限制。
...
- 如果模式属主是当前用户，则可以删除这个模式下的同义词，或者可以删除其他模式下，同义词属主与当前用户相同的同义词对象。（Oracle行为：需要有 DROP ANY SYNONYM 权限。如果删除公共同义词，需要有 DROP PUBLIC SYNONYM 权限。）
- PUBLIC：指定 PUBLIC 可以删除公共同义词。PUBLIC 和 schema 不能同时存在。
- schema：同义词所在的 schema，如果不指定，在当前 search_path 下查找同义词。（Oracle：删除当前用户模式下的同义词）
- 删除物化视图中的表所对应的同义词，不会使物化视图失效。（Oracle：如果删除的同义词被物化视图引用，删除同义词会标识物化视图失效）
- FORCE：TDSQL PostgreSQL 版（Oracle 兼容）不支持，TDSQL 会在 Transform 阶段将同义词替换为实际对象，后续不会再依赖同义词。（Oracle：如果物化视图被其他对象引用，会将对象标识为 UNUSED。）
...
## 3.3	伪列 ##
### 3.3.1	ROWNUM ###
ROWNUM 是 Oracle 中对结果集加的一个伪列，即先查到结果集之后再加上去的一个列 (强调：须要先有结果集)。简单的说 ROWNUM 是对符合条件结果的序列号。它总是从 1 开始排起的。

TDSQL PostgreSQL 版（Oracle 兼容）支持 ROWNUM 伪列用法。
...
`rownum between` 如果大于 1，查询结果为空，因为 ROWNUM 总是从1开始。 
...
1. 数据对象编号，表明此行所属的数据库对象的编号，每个数据对象在数据库建立的时候都被唯一分配一个编号，并且此编号唯一。数据对象编号占用大约 32 位。
2. 对应文件编号，表明该行所在文件的编号，表空间的每一个文件标号都是唯一的。文件编号所占用的位置是 10 位。
3. 块编号，表明该行所在文件的块的位置块编号需要 22 位。
4. 行编号，表明该行在行目录中的具体位置行编号需要 16 位。
...
TDSQL PostgreSQL 版（Oracle 兼容）兼容 Oracle 的 ROWID，由 `<xc_node_id, sequence_id>` 10个字节组成，同样可以保证行更新后，`ROWID` 值不变，但不支持 Oracle 的 DBMS_ROWID 包调用。

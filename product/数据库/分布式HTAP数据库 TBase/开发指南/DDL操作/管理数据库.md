...
## 创建数据库
创建数据库，必须是一个超级用户或者具有特殊的 CREATEDB 特权，默认情况下，新数据库将通过克隆标准系统数据库 template1 创建。您可以通过写 TEMPLATE name 指定一个不同的模板，通过写 TEMPLATE template0 您可以创建一个干净的数据库，它将只包含您的 TDSQL 版本所预定义的标准对象。

...
### 如何购买 TDSQL-H LibraDB？
TDSQL-H LibraDB 目前在邀测阶段，请联系 [在线支持](https://cloud.tencent.com/online-service?from=sales&source=PRESALE)，以获取进一步信息和体验服务。

### 为何新建的库表（新写入的数据）不见了？

初次使用 TDSQL-H LibraDB 的用户，常常反馈库表或是数据丢失。这是由于没有正确的创建库表。创建库表需要遵循以下原则：

- 必须使用 Replicated 引擎族，即您选择的引擎，需要支持副本。（请注意这里不是指Replacing引擎族）
- 必须使用 Distributed 表做数据操作。使用ON CLUSTER default_cluster语法。

这是因为，您所连上的网络地址，是按照负载均衡原则，随机接入集群任意节点的。如果只使用不带副本的本地表，当下一个会话被派发到其他节点时，是无法获取到上一个节点的库表。

为了方便初涉 LibraSQL分析引擎的客户，提供操作Demo如下

```sql
# 建立online_smoke Database
...
# 创建本地表online_smoke_local的Distributed表

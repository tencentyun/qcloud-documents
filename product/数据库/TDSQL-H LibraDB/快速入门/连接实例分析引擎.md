...
```
 - hostname：替换为目标 LibraSQL 的内网地址，在 [控制台](https://console.cloud.tencent.com/txln/instance) 实例**分析引擎**页的**连接信息**可查看。
 - username：替换为用户名。
...
```
7. 在新建的 tcp.d 目录下创建 conf 文件新建一个 tcp转发配置。将云服务器的9000端口和8123端口转发到内网的10.0.0.10。

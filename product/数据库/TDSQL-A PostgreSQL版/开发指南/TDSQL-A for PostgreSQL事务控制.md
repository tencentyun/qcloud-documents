...
CREATE TABLE
postgres=# insert into t_repeatable_read values(1,'tdapg');
INSERT 0 1
...
postgres=# insert into t_row_lock values(1,'tdapg'),(2,'pgxz');       
INSERT 0 2
...
postgres=# set lock_timeout to 1;
SET
postgres=#  update t_row_lock set mc='postgresql' where mc='tdapg';        
UPDATE 1
postgres=# 
```
上面 session1 与 session2 分别持有 mc=pgxz 行和 mc=tdapg 的行锁。
...
postgres=# set lock_timeout to 1; 
SET
postgres=# select * from t_row_lock where mc='tdapg' for update;
 id |  mc  
----+------
  2 | pgxz
(1 row)
...
mysql> select * from t_row_lock where mc='tdapg' for update;

...
--搜索路径只配置为"$user", public，其中"$user"为当前用户名，即上面的current_user值“dbadmin”。
```
...
上面出错就是因为模式tdapg_schema没有配置在search_path搜索路径中。

...
## 未指定 shard key 建表
如果未指定 shard key 建表方式，系统默认使用第一个字段作为表的 shard key。
```
...
CREATE TABLE
```
TDSQL-A PostgreSQL版 支持存在1级和2级分区混用，用户不需要都平级。
...
CREATE TABLE
postgres=# insert into t_rep values(1,'tdapg'),(2,'pgxz');
INSERT 0 2
...
[tdapg@VM_0_37_centos pgbench]$ pgbench -h 127.0.0.1 -p 11008 -d postgres -U tdapg -c 4 -j 1 -n -M prepared -T 60 -r -f insert_t_native_range.sql > insert_t_native_range.log 2>&1
[tdapg@VM_0_37_centos pgbench]$ tail insert_t_native_range.log
duration: 60 s
...
postgres=# 
[tdapg@VM_0_37_centos pgbench]$ pgbench -h 127.0.0.1 -p 11008 -d postgres -U tdapg -c 4 -j 1 -n -M prepared -T 60 -r -f insert_t_time_range.sql > insert_t_time_range.log 2>&1
[tdapg@VM_0_37_centos pgbench]$ tail -20 insert_t_time_range.log
client 3 receiving
client 0 receiving
client 1 receiving
client 2 receiving

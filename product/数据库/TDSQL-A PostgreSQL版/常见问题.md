### TDSQL-A PostgreSQL版和TDSQL PostgreSQL版数据库什么区别？
TDSQL-A PostgreSQL版和TDSQL PostgreSQL版都是腾讯云基于PostgreSQL研制的分布式数据库，二者都是腾讯云原分布式数据库TBase的不同产品系列。
TDSQL-A PostgreSQL版和TDSQL PostgreSQL版的主要差别是TDSQL-A PostgreSQL版是OLAP增强版，有自研列式存储和向量化执行引擎，主打OLAP场景；TDSQL PostgreSQL版则更专注在高并发的OLTP场景及混合HTAP场景。二者的很多功能特性都是相通和相互继承的，比如Oracle兼容性、比如多级安全特性、冷热分离能力等，而且也会在后续的演进中互相继承功能特性。

### TDSQL-A PostgreSQL版和TDSQL Clickhouse版数据库什么区别？
二者同属腾讯云分析型数据库TDSQL-A系列。分别基于开源PostgreSQL及开源Clickhouse进行研制，二者在语法协议上也是分别兼容PostgreSQL及MySQL协议。TDSQL-A PostgreSQL较之Clickhouse版具备更完善的SQL复杂查询能力，包括多表的复杂查询能力，同时具备完善的事务逻辑，能保证事务的ACID。

### TDSQL-A PostgreSQL版和腾讯云数据仓库 PostgreSQL什么区别？
TDSQL-A PostgreSQL版是腾讯云自主研发的分布式分析型数据库，兼容PostgreSQL协议，具有自研列式存储和向量化执行引擎，内核做了深度的优化；并同时和和TDSQL PostgreSQL版（原tbase v2/v5版本）共代码主线，继承和适配全套企业级特性，包括高可用、安全能力、管控能力、内核高性能等。在公有云和私有云均可以售卖。
云数据仓库 PostgreSQL (原Snova数仓)是腾讯云基于Greenplum的云上数仓，仅提供公有云版本。

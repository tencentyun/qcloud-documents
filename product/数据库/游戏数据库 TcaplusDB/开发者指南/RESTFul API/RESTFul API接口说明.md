...
确保您已经在 [腾讯云TcaplusDB](https://cloud.tencent.com/product/tcaplus/app) 创建了集群，并且已经获取对应的集群信息（包括AppId(集群接入ID)，ZoneId(表格组ID)，AppKey（集群访问密码））。当前 Tcaplus RESTful API 只支持通过 protobuf 定义的表。

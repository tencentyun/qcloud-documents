...
## 准备工作
已在 [TcaplusDB  控制台](https://cloud.tencent.com/product/tcaplus/app) 创建集群，并且经获取对应的集群信息，包括 AppId（集群接入 ID），ZoneId（表格组 ID），AppKey（集群访问密码）。当前 Tcaplus RESTful API 只支持通过 protobuf 定义的表。

...
TcaplusDB 支持回档，包括全区全服、单张表、千亿条记录回档N条记录，支持冷备时间回档（最近的凌晨01：05：00）、精确时间回档（秒级别）、模糊回档（项目组指定回档的规则），精确时间回档的速度请参照300GB数据、200GBUlog流水，耗时2小时左右，回档的原理，冷备时间回档即替换引擎文件，精确时间回档，将冷备的引擎文件 + 执行Ulog流水，执行到需要回档的时间点即可，基于 key 的回档需要项目组配合，先对这些 key 封号，TcaplusDB 回档完毕后，再对这些 key 解封。
...
### TcaplusDB 备份文件存放的时间？
TcaplusDB 备份的引擎文件保存25天，Ulog 流水保存15天，海外环境，各个 TcaplusDB 环境是不一样的，需要具体的 TcaplusDB 环境咨询 TcaplusDB。
...
TcaplusDB 推荐项目组自己实现超时机制，这样可以对超时的请求进行重试等处理。

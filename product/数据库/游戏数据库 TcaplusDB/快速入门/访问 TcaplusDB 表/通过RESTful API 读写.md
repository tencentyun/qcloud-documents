...
通过创建应用、创建部署单元、创建表格后，您就有了属于自己的 TcaplusDB 业务数据表，通过获取到的访问点信息就能够对业务数据表进行读写访问。本示例将分别采用 SetRecord 和 GetRecord 接口对目标表进行写和读操作。
>?相关操作需要在用户腾讯云账号下申请的云服务器 CVM 中进行。

本例中，接入点信息如下，并且在部署单元ID为1的部署单元中创建了表 tb_online。
- 接入ID：2

TcaplusDB 支持2种表定义格式：Protobuf（Protocol Buffers）表、TDR（Tencent Data Representation）表。
...
| tcaplus_field_cipher_suite | 如需使用字段加密功能，需要设置，如果用户需要指定自己的加密算法，请参考 API 中的例子 | option(tcaplusservice.tcaplus_field_cipher_suite) = "DefaultAesCipherSuite"; | 否   |
...
message player {  //使用 message 定义表，message 名即表名
    ...
    // 必须是指定了主键选项（tcaplusservice.tcaplus_primary_key）的 message 才能被识别为一张 TcaplusDB 业务数据表，否则此message只是一个结构体
    // 主键选项指定了主键字段名列表，字段名用逗号分割，最多四个字段能够被指定为主键
    option(tcaplusservice.tcaplus_primary_key) = "uin,name,region";

...
| int32        | int32      | int        | int         | int32    | fixnum/<br>bignum<br>(根据需要) | int        | integer        | int       | 使用可变长度编码。负数编码效率低，如果您的字段可能具有负值，请改用 sint32。 |
...
| fixed32      | uint32     | int        | int/long    | uint32   | fixnum/<br>bignum<br>(根据需要)| uint       | integer        | int       | 始终为四个字节。如果值通常大于2^28^，则比 uint32 更有效。     |

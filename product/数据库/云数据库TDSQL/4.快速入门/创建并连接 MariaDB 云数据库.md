...
## 一、创建 MariaDB 云数据库实例
在此步骤中，您会使用腾讯云 [控制台](https://console.cloud.tencent.com/) 创建 MariaDB 云数据库实例。
...
3. 在 TDSQL（MariaDB）云数据库购买界面中，选择配置属性。确认无误后，单击【立即购买】。
...
8. 在 MariaDB 云数据库实例详情页，单击【账号管理】>【创建账号】，弹出创建账号页面。填写相关信息创建账号，确认无误后，单击【确定】。此步骤填写的 **账号名** 和 **密码** 将在连接 MariaDB 云数据库时使用，请妥善保管。

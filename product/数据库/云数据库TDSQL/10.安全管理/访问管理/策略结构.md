...
- **版本 version**是必填项，目前允许值为"2.0"(该值实际代表 CAM 接受的云 API 版本)。
- **语句 statement**是用来描述一条或多条权限的详细信息。该元素包括 effect、action、resource，condition 等多个其他元素的权限或权限集合。一条策略有且仅有一个 statement 元素。
 1. **操作 action** 用来描述允许或拒绝的操作。操作 action 实际填入的是以 "mariadb:" 前缀描述，[云数据库MariaDB API](https://cloud.tencent.com/document/api/237/16144) 为后缀的一串字符串 。该元素是必填项。
...
>!由于历史原因，云数据库 MariaDB 的接口关键词为 mariadb；详情请参考 [产品更名详情](http://bbs.qcloud.com/thread-55032-1-1.html)。
...
- **account**：资源拥有者的根账号信息，如 uin/653339763。
...
"resource": ["*"]
```
4. 如果您想要在在一条指令中同时指定多个资源，请使用英文逗号将它们隔开，如下所示为指定两个资源的例子：
```

...
>!如果提示和之前安装的版本冲突，您可能需要先移除以前安装的包，例如`yum remove mariadb-libs`。
...
```
- 安装 lz4 解压软件，更多详情请参考 [如何解压备份文件和日志文件](https://cloud.tencent.com/document/product/237/2088)
```
yum install -y lz4
percona-xtrabackup
...
>?lz4 默认安装在`mysqlagent/bin`目录下，也可以将其放置在`/usr/bin`目录下，当环境变量引用。
...
修改数据库参数文件`(/etc/my.cnf.d/server.cnf)`，具体参数数值请参考解压文件中`backup-my.cnf`的参数。
>!不能直接用 backup-my.cnf 替换参数文件。
...
参考 [停止数据库，清空数据文件](#.E5.81.9C.E6.AD.A2.E6.95.B0.E6.8D.AE.E5.BA.93.EF.BC.8C.E6.B8.85.E7.A9.BA.E6.95.B0.E6.8D.AE.E6.96.87.E4.BB.B6)。
...
下图为利用工具`./innobackupex`对备份文件进行 apply，直到 apply 正常结束。工具使用 demo 如下：
```
...
>?apply 时需要通过`--keyring-file-data=key_file`来指定密钥，同时该参数指定的路径要填绝对路径。这里以及后文中用到的 xtrabakcup 为腾讯云专有云数据库 TDSQL 自带的 xtrabakcup，默认放置在专有云安装包目录下的 xtrabackup 目录。

### 利用工具`./innobackupex`将备份拷贝到数据目录

下图为利用工具`./innobackupex`对备份文件进行移动。工具使用 demo 如下：
>?建议使用 MySQL 启动用户权限。
...
下图为利用工具`keyring_file`将生成的密钥问配置到 MySQL，注意红框中的配置。
>?建议使用 MySQL 启动用户权限运行`keyring_file`。
>

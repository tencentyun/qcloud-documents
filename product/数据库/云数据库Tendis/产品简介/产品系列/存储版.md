云数据库 Tendis 存储版支持标准架构和集群架构：
- 存储版（标准架构）基于腾讯 KV 存储引擎 Tendis，将数据直接存储到磁盘，通过多副本保障服务的可用性和数据的可靠性，适用于大容量 KV 存储场景。
- 存储版（集群架构）兼容 Redis 4.0 版本绝大部分命令，使用门槛低，可以充分利用 Redis 丰富的数据结构和操作命令给业务带来高效的存取方案。
![](https://main.qcloudimg.com/raw/9e076d4a36bd2b6b5b15a45ecf6533ef.png)

## 存储版特点
#### 低成本
- 数据存储在磁盘，相对 Redis 的全内存方案，成本降冷80%以上，同时提供存储级别的数据可靠性。

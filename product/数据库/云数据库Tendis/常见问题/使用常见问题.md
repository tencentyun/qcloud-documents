...
云数据库 Tendis 支持使用 [DMC](https://bj-dmc.cloud.tencent.com/v2/qcloudLogin/login) 来进行可视化管理，也可以通过外网连接使用第三方组件来访问。

### 扩容和缩容，会中断业务吗？
Tendis 存储版和混合存储版在扩容中都是无损的，磁盘的扩容在资源充足的情况下可以在1分钟内完成，缩容涉及到数据搬迁，会有连接闪断。

### 如何添加监控报警？
可通过自定义监控告警实现，请参见 [监控告警](https://cloud.tencent.com/document/product/1363/50909)。

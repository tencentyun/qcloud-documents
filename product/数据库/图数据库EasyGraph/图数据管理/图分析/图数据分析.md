本文为您概述介绍在 KonisGraph 实例图数据管理平台的图分析中，如何新建图数据分析。
...
2. 用户可选择【图数据分析】，选择适合的【数据类型】，【导入图数据文件】单击【下一步】。
图数据分析当前支持的文件类型参照【图数据分析的文件类型】。

- 新建图数据分析
![](https://main.qcloudimg.com/raw/90ee287911125a8280b48a83bb70cd99.png)
- 导入数据文件
![](https://main.qcloudimg.com/raw/e33498049bd40dc8821792bcda9d064a.png)
...
Gremlin CSV 格式遵循 RFC 4180 CSV 规范。有关更多信息，请参阅 Internet Engineering Task Force (IETF) 网站上的[ CSV 文件的一般格式和 MIME 类型](https://datatracker.ietf.org/doc/html/rfc4180?spm=a2c4g.11186623.2.7.36f46d84aFdUEZ)。
...
| :--------- | :----- | :---------- | :---------- |
|~id|id|N|边唯一ID|
|~from|from/source|Y|源点ID|
|~to|to/target|Y|目标点ID|
|~label|label|N|边的标签，只支持单个标签|
||color|N|边的颜色|
||size|N|边的大小|

##### 属性列标题
可通过以下语法指定属性列，类型名称不区分大小写，列标题中不允许使用空格，因此，属性名称也不能包含空格，属性不支持List和Set数据类型，只能有一个值
```
attribute:type

```
>?
为了简化文件，type也可以省略，除系统列标题以外的标题都将被识别为属性列标题。
?
...
|小数类型|float|32位IEEE 754浮点|
|小数类型|double|64位IEEE 754浮点|
...
与上述CSV格式相似，但仅需提供 Edge List CSV 一个文件即可，系统将根据边的source和target自动生成点列表。

本文为您概述介绍在 KonisGraph 实例图数据管理平台的图分析中，如何进行图可视化分析的【统计】。

## 统计概述
图可视分析系统的图统计模块支持了对图的度，连通分量，PageRank的统计。

![](https://main.qcloudimg.com/raw/12e9b1ab733258eaa9c90a717c24e112.png)


## 平均度
点击“平均度”运行，将会计算出图的平均度，以及每个节点总度数、出度、入度的散点分布图。
...
## 连通分量
1. 点击“连通分量”运行，可以对图类型进行配置，
2. 点击“确定”之后将会计算出图的连通分量，以及散点分布图。
...
1. 点击“PageRank”运行，可以对PageRank算法进行精度值、阻尼系数的配置，
2. 点击“确定”之后将会计算出每个节点的PageRank的散点分布图。
>?PageRank会将“@PageRank”注入到点的属性上，并可以通过“@PageRank”进一步进行外观配置、查询。
>
|属性名称|描述|
| :--------- | :----- |
|@pageRank|点的PageRank|

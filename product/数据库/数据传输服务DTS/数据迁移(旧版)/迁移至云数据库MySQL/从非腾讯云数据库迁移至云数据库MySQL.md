...
> ?源数据库的帐号长度不能超过16位。如果帐号超过16位需要用户重新创建一个符合要求的帐号。
...
### 4. 校验迁移任务
 在创建完迁移任务后，您需要对迁移任务信息进行校验，单击**下一步：校验任务**进行校验，只有所有校验项通过后才能启动迁移任务，单击**启动任务**。
 任务校验存在 3 种状态：

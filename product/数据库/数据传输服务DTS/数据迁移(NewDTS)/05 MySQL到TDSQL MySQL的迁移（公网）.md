本文介绍使用 DTS 数据迁移功能，从 MySQL 迁移数据至腾讯云分布式数据库 TDSQL MySQL 的操作指导。
...
- 需要具备源数据库的权限。
  ...
  - “整个实例”迁移，需要的帐号权限如下：
    ...
    GRANT INSERT, UPDATE, DELETE, DROP, SELECT, CREATE ON `__tencentdb__`.* TO '迁移帐号'@'%'; 
...
<table>
        ...
        <tr> 
                ...
                <td>
                    ...
                    <li>源数据库实例为分布式数据库时，需要提前在目标库建立分表，否则这些表被迁移后都将是单表。</li>
                </td>
        </tr>
        <tr> 
                <td>其他要求</td>

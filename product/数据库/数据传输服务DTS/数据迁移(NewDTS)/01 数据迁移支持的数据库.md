...
>- 云数据库 MariaDB 指腾讯云数据库 MariaDB，支持三种内核MariaDB、MySQL 和 Percona。用户在使用时不需要区分哪种内核，如果源数据库为腾讯云 MariaDB，不论源数据库的内核是MariaDB、Percona 还是 MySQL，在设置源数据库或目标数据库类型时，都选择 MariaDB。

...
- 当源实例非腾讯云 SQL Server 实例时（公网/CVM 自建实例、其他云厂商实例）或腾讯云基础版 SQL Server 实例时，需使用具有 sysadmin 权限的帐号进行迁移，且需要能够运行 xp_cmdshell 存储过程，当源实例为腾讯云高可用版和集群版 SQL Server 时，无权限限制。
- 迁移账号需要为 localsystem。

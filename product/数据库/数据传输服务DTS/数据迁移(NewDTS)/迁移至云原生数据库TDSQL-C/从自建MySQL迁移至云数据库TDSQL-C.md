云原生数据库 TDSQL-C（Cloud Native Database TDSQL-C）简称 TDSQL-C 是腾讯云自研的新一代高性能高可用的企业级分布式云数据库，融合了传统数据库、云计算与新硬件技术的优势，100%兼容 MySQL。通过 DTS 数据迁移功能，可以实现从公网自建 MySQL 迁移至云原生数据库TDSQL-C（兼容 MySQL 版）。
...
- TDSQL-C 的存储空间需须是源端自建 MySQL 数据库所占用存储空间的1.2倍以上。
...
- 增量同步：在全量数据迁移的基础上，DTS 会读取并解析源端自建 MySQL 数据库的 binlog 信息，将源端自建 MySQL 中的增量更新同步到目标 TDSQL-C。通过增量数据同步完成自建应用在不停服的情况下平滑迁移到腾讯云。

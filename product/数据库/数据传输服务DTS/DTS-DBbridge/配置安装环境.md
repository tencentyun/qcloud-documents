...
至此，数据迁移的基础环境配置就已经完成了。若需要使用增量同步功能，还需要为数据库启动归档模式；否则，则可以忽略下述操作
...
```
也可以将已有用户指定为XStream管理员。

3. 为XStream管理员赋予CREATE SESSION的权限
```
GRANT CREATE SESSION TO xstrmadmin;
```

4. 执行DBMS_XSTREAM_AUTH包中的GRANT_ADMIN_PRIVILEGE存储过程
```
BEGIN
...
#### 使用已有用户作为Xstream管理员，以用户test为例（二选一）
1. 为已有用户test赋予CREATE SESSION的权限
```
GRANT CREATE SESSION TO test;
```
2. 执行DBMS_XSTREAM_AUTH包中的GRANT_ADMIN_PRIVILEGE存储过程
```
BEGIN
   DBMS_XSTREAM_AUTH.GRANT_ADMIN_PRIVILEGE(
      grantee                 => 'test',

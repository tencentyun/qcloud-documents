...
>? 此处的角色策略需要同时添加源数据库所属服务的对应策略，否则在后续 DTS 任务时，无法拉取到源数据库实例信息。例如源数据库为云数据库 MySQL，则添加 QcloudAccessForMySQLRole（云数据库(MySQL)操作权限），如下以云数据库 MySQL 为例，其他场景请用户根据实际情况选择配置。
>

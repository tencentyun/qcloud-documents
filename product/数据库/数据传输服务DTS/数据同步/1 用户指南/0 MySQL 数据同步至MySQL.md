...
本文为您介绍使用DTS从MySQL数据库同步数据至MySQL数据库的过程。

## **前提条件**

- 所支持的数据库及版本请参见文档&quot;0腾讯云数据同步产品介绍&quot;。
...
![](https://main.qcloudimg.com/raw/94b1751564b042ad7c9c6dc3c3bf2e29.png)

购买数据同步任务参数说明：

| 参数         | 描述                                                         |
| ------------ | ------------------------------------------------------------ |
| 计费模式     | 支持包年包月和按量计费。目前免费，将来开始计费前1个月会通过邮件和站内信方式提前通知用户 |
| 源实例类型   | 选择MySQL（包括云数据库MySQL及自建MySQL）或TDSQL-C MySQL     |
| 源实例地域   | 选择源实例所在地域                                           |
| 目的实例类型 | 选择MySQL（含云数据库MySQL及自建MySQL）或TDSQL-C MySQL       |
| 目的实例地域 | 选择目的实例所在地域                                         |
| 同步任务规格 | 目前只支持标准版                                             |
...
3. 在 [数据同步列表](https://console.cloud.tencent.com/dts/replication)页面，单击&quot;操作&quot;列的【配置】，进入配置同步任务页面。
4. 在配置同步任务页面，配置源端实例、帐号密码，配置目标端实例、帐号和密码，测试连通性后，单击【下一步】。

![](https://main.qcloudimg.com/raw/d5ed42367196f718c62a90c3a3a37088.png)
...
| 参数       | 描述                                                         |
| ---------- | ------------------------------------------------------------ |
| 源实例类型 | 购买时所选择的云数据库实例类型，不可修改                     |
| 源实例地域 | 购买时选择的云数据库实例所在地域，不可修改                   |
| 服务提供商 | 支持普通（包括腾讯云MySQL数据库及自建MySQL数据库）、AWS、阿里云 |
| 接入类型   | 根据服务提供商选择会有所不同，如服务提供商选择普通，接入类型可选云数据库、云主机自建、公网、云联网；若服务提供商选择其他云厂商，接入类型可选公网 |

3）目标实例设置

| 参数         | 描述                             |
| ------------ | -------------------------------- |
| 目标实例类型 | 所选择的目标实例类型，不可修改   |
| 目标实例地域 | 选择的目标实例所在地域，不可修改 |
| 接入类型     | 选择目标数据库类型               |

**接入类型的说明**

在源实例及目标实例设置中，根据接入类型选择的不同，会要求填写不同的参数，对应情况见下表：

| 服务提供商 | 接入类型 | 实例ID |云主机实例|主机地址|端口|账号|密码|
| ------- | ----- | ---- |------- |------ |-- |-- |--- |
| -    | 云数据库 | √      |×|×|×|√|√|
| - | 云主机自建 | × |√|×|√|√|√|
| - | 公网 | × |×|√|√|√|√|
...
- 对AWS支持的数据库版本有：RDS MySQL 5.6、5.7、8.0； Aurora MySQL 5.6、 5.7

- 对阿里云支持的数据库版本有：RDS 5.6、5.7、8.0； PolarDB 5.6、5.7、8.0

5. 在设置同步选项和同步对象页面，将对数据初始化选项、数据同步选项、同步对象选项进行设置，在设置完成后单击【保存并下一步】

![](https://main.qcloudimg.com/raw/272026696de9d8dd15b0034f7bf8f0dd.png)

- 数据初始化选项

| 参数| 描述|
| ------ | ---- |
| 初始化类型 | 结构初始化：同步任务执行时会先将源实例中表结构初始化到目标实例中。<br/>全量数据初始化：同步任务执行时会先将源实例中数据初始化到目标实例中。<br/>默认两者都勾上，可根据实际情况取消。 |
| 已存在同名表   | 前置校验并报错：存在同名表则报错，流程不再继续忽略并继续执行：全量数据和增量数据直接追加目标实例的表中 |

- 数据同步选项

| 参数| 描述|
| ------ | ---- |
| 冲突处理机制                                                 | 冲突报错：在同步时发现表主键冲突，报错并结束数据同步。<br>冲突忽略：在同步时发现表主键冲突，保留原主键记录，继续后续数据同步。<br>冲突覆盖：在同步时发现表主键冲突，用新记录覆盖原主键记录。 |
| 同步操作类型   | 支持操作：Insert、Update、Delete、DDL     |

- 同步对象选项
| 参数| 描述|
| ------ | ---- |
| 源实例库表对象 | 选择待同步的对象，支持库级别和表级别                             |

...
本文主要介绍通过 DTS 数据迁移功能从阿里云 PolarDB MySQL 迁移至腾讯云数据库 MySQL。DTS 支持结构迁移、全量数据迁移以及增量数据迁移，可以实现在不停服的情况下，平滑迁移数据到腾讯云数据库 MySQL。
...
- 云数据库 MySQL 的存储空间需要大于源端阿里云 PolarDB MySQL 数据库所占用存储空间的1.2倍。
...
<tr>
<td>账号</td><td>阿里云 PolarDB MySQL 的数据库帐号，帐号权限需要满足要求。</td></tr>
<tr>

...
>
![](https://main.qcloudimg.com/raw/5c3a9b015df21c0b558274ab5f1b0a22.png)


### 5. 启动迁移
1. 校验通过后，在校验任务页面单击【启动任务】立即开始迁移数据。
>!如果您设定了迁移任务的定时时间，则迁移任务会在设定的时间开始排队并执行，如果没有设置定时任务，则迁移任务会立即执行。
2. 迁移启动后，返回迁移任务列表，可查看对应的迁移进度信息。鼠标指向步骤后的下图提示符时，可显示迁移当前所处阶段及其开始时间。
...
### （可选）撤销迁移
在迁移过程中，如果您需要终止迁移任务，可以在 DTS 任务的“操作”列，选择【更多】>【撤销】。

...
![image1](https://main.qcloudimg.com/raw/f96867195a68e3353bf3ec716fe95097.png)

>注意：
- 目前我们对订阅的消息内容默认会保留最近1天，预计后面会提供保留最近3天。
- 另外如果订阅整库的话，后续新增的表也是会在原有订阅通道出现，不需要对原订阅通道进行新增配置操作。
- 目前只支持CDB For MySQL5.6,后续会很快支持CDB For MySQL5.7 。
- 数据订阅暂不支持view，触发器和外键。

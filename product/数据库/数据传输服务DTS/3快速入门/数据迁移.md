...
目前 DTS 不支持 TencentDB for MySQL 基础版的迁移，另外**实例有关联灾备实例的暂不支持数据传输服务，请 [提交工单](https://console.cloud.tencent.com/workorder/category) 进行迁移**。通过增量迁移的时间建议不超过15天，请确保主从同步之后单击迁移完成按钮。

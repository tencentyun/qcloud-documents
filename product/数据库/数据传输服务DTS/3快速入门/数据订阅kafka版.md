数据传输服务 DTS 提供基于 binlog 的增量数据订阅功能，仅需几步简单操作，即可订阅云数据库 TencentDB 的增量更新数据。通过数据订阅功能您可以轻松实现源库的增量数据变更订阅，方便您搭建云数据库和异构系统之间的数据同步，如缓存更新，ETL 实时同步，业务异步解耦等。

## 前提条件
- 已准备好待订阅的腾讯云数据库 MySQL，支持同步的 MySQL 版本包括 MySQL 5.6、MySQL 5.7、MariaDB 10.0.10、MariaDB 10.1.9、Percona 5.7.17。
- 已在源端 MySQL 实例中开启 binlog。
- 已在源端 MySQL 实例中创建订阅帐号，需要帐号权限如下：REPLICATION CLIENT、REPLICATION SLAVE、PROCESS 和全部对象的 SELECT 权限。
...
![](https://main.qcloudimg.com/raw/3f5278d5a8ab0a3db529209f1bccf27a.png)
4. 在配置数据库订阅页面，选择相应配置，单击【下一步】。
 - MySQL 实例：选择 MySQL 实例，目前支持 MySQL 5.6、MySQL 5.7 高可用版的主实例。只读实例和灾备实例不支持数据订阅。
...
![](https://main.qcloudimg.com/raw/1a3cbca6818ff3625a8f36bfe832c4a6.png)
...
![](https://main.qcloudimg.com/raw/eb4721d5d43875b182a390eeb4f66018.png)

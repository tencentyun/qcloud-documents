...
## 3. 导出数据
mysqldump 导出数据：
在 [DSQL 控制台](https://console.cloud.tencent.com/dcdb) 的参数设置中设置 net_write_timeout 参数：set global net_write_timeout=28800
```
mysqldump --compact --single-transaction --no-create-info -c -uxxx  -pxxx -hxxx.xxx.xxx.xxx -Pxxxx db_name table_name  > data.sql
```
>?db_name 和 table_name 参数根据实际情况选择，如果导出的数据要导⼊到另外⼀套 DSQL 环境的话，必须加上 -c 选项，-c 与 db_name 之间需添加空格。
>

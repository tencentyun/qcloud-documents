## 1.数据导入概述
由于分布式数据库的分布式架构对用户透明，一般情况下，只需要预先建好表结构。可以使用 mysqldump、或其他 Navicat、SQLyog 等 MySQL 客户端进行迁移。笔者对迁移步骤总结如下：
...
- 云服务器
	- 建议配置 CPU 2核，内存 8GB，磁盘 500GB 以上（取决于数据量大小）
	- linux
	- 安装 MySQL 客户端
...
### 3.3 上传文件至云服务器某目录
上传前，您开启 CVM 外网访问地址，并请参考 CVM 上传文件说明：[Linux机器通过SCP上传文件](https://cloud.tencent.com/document/product/213/2133)，即可将文件上传,您至少需要上传刚刚导出的：
- 表结构sql：table.sql
- 数据sql：data.sql
...
>注意：必须要设置主键，必须指定 shardkey，必须注意表名大小问题，建议删除多余注释，否则建表可能不成功。
...
> 注意：需先导入建表语句，再导入数据。也可以通过 mysql 的 source 命令直接导入 sql。

...
>注意：导出数据必须通过mysqldump工具导出，并且加上 -c 参数，因为这样导出的数据行都带有列名字段，不带列名字段的sql会被DCDB for TDSQL拒绝掉。-t 参数的意义是不导出表结构，只导出表数据。

### 3.3 上传文件至云服务器某目录
上传前，您开启CVM外网访问地址，并请参考CVM上传文件说明：[Linux机器通过SCP上传文件](https://www.qcloud.com/document/product/213/2133)，即可将文件上传,您至少需要上传刚刚导出的：
- 表结构sql：table.sql
- 数据sql：data.sql

...
**DCDB supports all character sets and character sequences of MySQL on the backend storage. The following examples are just for illustration, which does not mean that the DCDB's supported character sets and character sequences are different from those of MySQL.**
```
...
>Note: Setting parameters via commands is not supported for DCDB. To set DCDB's parameters, go to the cloud console.

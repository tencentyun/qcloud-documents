...
**分表：**即自动水平拆分的表，水平拆分是基于分表键（shardkey）采用类似于一致性 hash 方式，根据 hash 后的值分配到不同的节点组中的一种技术方案，该能力是几乎所有分布式数据库的核心特性。DCDB 的设计目标是**期望开发者完全无需关注后端分表策略**，像使用普通数据库一样使用 DCDB，因此我们在设计上隐藏了分表的细节方案。
...
```
由于在 DCDB 下，shardkey 对应后端数据库的分区字段，因此必须是主键以及所有唯一索引的一部分，否则无法建表；
```
...
> **注意**：某些分表方案可以支持“非主键或唯一索引”成为 shardkey，但此类方案会导致数据不一致，因此 DCDB 默认禁止了“非主键或唯一索引”成为 shardkey。

分布式TDSQL支持通用的DDL语句，如建库，建表，修改表结构等。


## 建表

分布式TDSQL可以创建三种类型的表，分表，小表以及单表。分表适用于表在单个数据库存不下的情况，此时对sql会有一定的限制，具体参考<SQL兼容性>；单表主要用于数据量能在单个数据库存储，同时sql比较复杂的情况
...
删除表和mysql的语法完全一样，TDSQL会根据表的类型，自动去一个或者多个后端数据库删除表

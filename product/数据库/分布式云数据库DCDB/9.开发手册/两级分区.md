...
>!分区使用的是小于`<`符号，因此如果要存储当年的数据的话（2017），需要创建` <2018 `的分区，用户只需创建到当前的时间分区，TDSQL 会自动增加（立即生效）后续分区，默认往后创建3个分区，二级分区只支持年/月/日的分片自动创建，不支持自动删除，以 YEAR 为例，TDSQL 会自动创建2018，2019，2020的分区，后续会自动增加。

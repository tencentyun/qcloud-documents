## 概述
DCDB 支持分表，通过分表键（shardkey）把一个大表水平拆分到多个数据库，下面详细介绍分表的方法。
 ...
 **例子：**
假设业务有两张表，一个用于记录用户的基本信息，一个用于用户的订单信息，此时若选取用户 ID 作为分表键，则理论数据分布和访问将会比较均衡，同时单个用户对应的基本信息和订单信息均会在一个后端数据库中，方便后续的 join 等操作。

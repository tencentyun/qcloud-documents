## 1. 概述
DCDB for Percona、MariaDB 支持分表，通过分表键（shardkey）把一个大表水平拆分到多个数据库，下面详细介绍分表的方法。
 ...
 **例子：**
如果业务有两张表，一个用于记录用户的基本信息，一个用于用户的订单信息，此时如果选取用户 ID 作为分表键，则理论数据分布和访问都会比较均衡，同时单个用户对应的基本信息和订单信息都会在一个后端数据库中，方便后续的 join 等操作。

## 3. 分表键选择的限制

普通的分表创建时在最后面指定分表键（shardkey）的值，该值为表中的一个字段名字，用于后续 sql 的路由选择。
分表键的限定要求如下：
...
4. 不要 `update` shardkey 字段的值，如必须则先 `delete`，再`insert` 。

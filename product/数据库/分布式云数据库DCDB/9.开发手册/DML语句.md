...
**SELECT**：建议在条件中带上 shardkey 字段，否则 DCDB 无法判断 SQL 应该路由至哪些节点，需要进行全表扫描，然后在网关聚合，容易影响执行效率：
```
...
```
**INSERT/REPLACE**：字段必须包含 shardkey，因为 DCDB 无法判断该条 SQL 应该发往哪个节点，乱发会导致数据异常，因此 DCDB 将会直接拒绝执行无 shardkey 的 INSERT/REPLACE 语句：
```

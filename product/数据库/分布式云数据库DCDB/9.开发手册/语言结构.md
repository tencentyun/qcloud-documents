...
**DCDB 支持所有 MySQL 使用的文字格式，此处只是列举，并不代表与 MySQL 有差异：**
```
	String Literals
	Numeric Literals
...
String Literals 是一个 bytes 或者 characters 的序列，两端被单引号'或者双引号"包围，目前DCDB不支持 ANSI_QUOTES SQL MODE，双引号"包围的始终认为是 String Literals，而不是 identifier。

...
2. 在实例管理页面，选择**账号管理**页，单击**创建账号**。
![](https://main.qcloudimg.com/raw/bfbaa009dfde227aecb026a97c5b1b2c.png)
3. 在弹出的对话框，输入账号名、主机、密码等，确认无误后，单击**确认，下一步**。
...
5. 返回账号列表，单击**修改权限**可以修改用户权限，单击**克隆账号**可以完全复制当前账号权限来新建一个账号，单击**更多**可以重置密码和删除账号。
...
2. 打开 SQLyog，选择**文件** > **新连接**，输入对应的主机地址、端口、用户名和密码，单击**连接**。
 ...
 - 用户名：输入创建的账号名。
 - 密码：输入账号对应的密码，如忘记密码，可至 [控制台](https://console.cloud.tencent.com/tdsqld/instance-tdmysql) 进行修改。
 - 端口：输入地址对应的端口。

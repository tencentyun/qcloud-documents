...
- 支持 range 的分片机制。
- 支持联合字段的 shard key。
- 分片实例下所有数据集合必须使用分片，建议把不使用分片的数据放到单独的副本集实例下。
...
<table class="table-striped">
	<tbody>
		...
		<tr>
			...
			<td >必须带 shardkey 字段，批量 insert 时 shard key 必须一致</td>
			<td>支持</td>
		</tr>		
		<tr>
			<td rowspan="1">update</td>
			<td >更新字段不能是 shardkey</td>
			<td>支持</td>
		</tr>		
		<tr>
    ...
    <tr>
		...
		<tr>
		  <td rowspan="1">user 管理命令</td>
			<td rowspan="1">-</td>
			<td>-</td>
			<td>不支持</td>
		</tr>
			<tr>
		  <td rowspan="1">role 管理命令</td>
			<td rowspan="1">-</td>
			<td>-</td>
			<td>不支持</td>
		</tr>

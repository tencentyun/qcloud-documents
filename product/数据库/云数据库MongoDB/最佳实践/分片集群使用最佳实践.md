...
在一个分片集群内部，MongoDB 会把数据分为 chunks，后台进程 balancer 负责 chunk 的迁移，从而均衡各个 shard server 的负载，每个 chunk 包含一部分数据，chunk 的产生和引入会导致 balance 的产生。
>?系统初始就1个 chunk，chunk size 默认值64MB，

chunck 迁移时会造成集群的读写性能下降，因此需要通过适当配置 balance 活动窗口来避免 balance 对业务高峰期的影响，也可以通过命令来关闭 balance。

下面介绍几种命令管理 balance，若某些指令无权限执行，请 [提交工单](https://console.cloud.tencent.com/workorder/category) 联系我们处理。
...
停止 balance 后，没有迁移进程正在迁移，可执行下列命令：
```
use config
...
如果驱动没有命令 sh.startBalancer()，可执行下列命令：
```
use config

...
创建一个名为 **MyNewDB** 的数据库，并插入文档。

```
> use myFirstDB
switched to db runoob
> db.myFirsrDB.insert({"test":"MyfirstDB"})
...
| options字段 | 类型 |      | 描述                                                         |
| ----------- | ---- | ---- | ------------------------------------------------------------ |
| capped      | BOOL | 否   | 指是否设置集合的最大字节数。如果为 true，需设置size参数。默认为false。 |
...
创建集合 FirstCol，最大字节数为 6142800 B, 文档最大个数为 10000 个，示例如下：
...
MongoDB使用**remove()**删除集合中的文档。示例如下：

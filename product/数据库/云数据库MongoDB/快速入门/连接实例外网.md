## 操作场景
云数据库 MongoDB 暂时不支持外网访问，您可以通过具备外网 IP 的云服务器 CVM 进行端口转发，来实现外网访问 MongoDB 实例。
...
4. 在访问端通过外网地址（本示例即26.xx.xx.2:10001）连接内网 MongoDB 实例，连接命令与内网连接命令一致，命令请参见 [连接实例](https://cloud.tencent.com/document/product/240/7092)。

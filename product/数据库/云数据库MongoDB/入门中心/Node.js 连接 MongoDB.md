...
## 先决条件
1. 在腾讯云[选购一台 CVM](https://buy.qcloud.com/cvm)，本文档以 Windows Server 2012 R2 版本为示例。
2. 在腾讯云[选购 MongoDB](https://buy.qcloud.com/mongodb)。
3. 在 CVM 中[下载 Node.js](https://nodejs.org/en/download/)。

## 安装驱动
进入[腾讯云控制台](https://console.qcloud.com/cvm/index)，登录刚刚购买的 CVM ，在左下角【开始】处单击右键，再单击【命令提示符】， 打开 cmd 命令行，进入 nodejs 根目录。
```
cd C:\Program Files\nodejs
```
使用 npm 命令安装驱动。
...
## 相关说明
腾讯云 MongoDB 默认提供了 “rwuser” 和 “mongouser” 两个用户名分别支持 “MONGODB-CR” 和 “SCRAM-SHA-1” 两种认证方式，对于这两种认证方式，连接 URL 需要做不同的处理，详情请参考[连接示例](https://www.qcloud.com/document/product/240/3563)。
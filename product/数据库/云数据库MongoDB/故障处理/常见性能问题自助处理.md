...
## 连接数超限
### 问题描述
针对连接数超限问题，用户可使用 [控制台](https://console.cloud.tencent.com/mongodb) 的连接数管理和重启功能来进行排查处理。

### 解决方案
#### 提升连接数
1. 登录 [MongoDB 控制台](https://console.cloud.tencent.com/mongodb)，单击实例名进入实例管理页面。
2. 选择【数据库管理】>【连接数管理】页，查看连接的来源 IP 和对应的连接数，进行业务端的排查。
 - 若您的连接数达到80%，影响到新连接的建立，可通过控制台一键【提升连接数】功能在6小时内提升连接数的上限至150%。
...
#### 优化业务侧服务
通过对业务端的排查可对业务侧服务进行优化。

#### 重启实例
连接数管理页面提供了数据库侧的【重启】功能，必要时您可对 Mongos 进行重启，以解决连接数超限的问题。
>?
>- 4.0 版本副本集实例无 Mongos。

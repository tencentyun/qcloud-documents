...
## 数据结构
MongoDB是面向文档的NoSQL（非关系型）数据库，其数据结构由 字段（Field）和值（Value） 组成，类似于JSON对象，示例如下：
...
- 集合 （Collection） ：  一个MongoDB集合可以包含多个文档。例如，您可以将不同数据结构的文档插入在集合中。
...
- 数据库（Database）：一个MongoDB数据库中可以包含多个集合。您可以创建多个数据库。
...
您可以由浅入深逐步了解云数据库MongoDB相关名词、系统架构、使用限制、进阶操作等。 

Tencent Cloud Database MongoDB provides the capabilities of NoSQL Database, making it a service easier to be deployed, managed, and expanded compared with self-built MongoDB databases;
At the same time, it has the same feature with the public cloud in terms of "Request on Demand and Pay by Usage" which improves its cost efficiency. For more information, please see the following table:
...
| Service availability | 99.95%, high industry standard, professional team on a 7/24 basis, one-to-one instruction, QQ remote assistance | Deal with failures and build master/slave or RAID by yourself |
| Data reliability | 99.9996%, perfect auto data backup and lossless recovery mechanism, real-time hot backup, recover data to any time point within 5 days (Note: If the data operated between the two backups exceeds the oplog size, you cannot roll it back to the time between the two backups) | Self-protection, reliability depends on hardware failure rate and on the database management skills of technical personnel |
...
3) Highly available service: master-slave or even more hot backups, automatic disaster recovery, failover transparent to users; support the same "priorly read slave" feature as the native MongoDB to ensure high concurrent read capability.

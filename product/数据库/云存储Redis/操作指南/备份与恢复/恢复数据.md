## 操作场景
云数据库 Redis 单机版（社区）、主从版（社区）、主从版（CKV）和集群版（CKV）支持从备份中恢复整个实例。
	
>!
>- 仅 Redis 单机版（社区）、主从版（社区）、主从版（CKV）和集群版（CKV）支持恢复实例，Redis 集群版（社区）支持 [克隆数据](https://cloud.tencent.com/document/product/239/32545)。
>- 恢复整个实例会中断实例对外提供的服务。
>- 实例恢复后数据将被覆盖，无法恢复。

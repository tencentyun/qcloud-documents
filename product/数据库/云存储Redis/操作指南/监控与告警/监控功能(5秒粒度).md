...
| 网络监控 | 读平均时延         | Proxy    | latency_read       | ms    | Proxy 到 Redis Server 的读命令平均执行时延，读命令分类，请参见 [命令分类](#mlfl)  |
| 网络监控 | 写平均时延         | Proxy    | latency_write      | ms    | Proxy 到 Redis Server 的写命令平均执行时延，写命令分类，请参见 [命令分类](#mlfl)  |
| 网络监控 | 其他命令平均时延   | Proxy    | latency_other      | ms    | Proxy 到 Redis Server 的读写命令之外的命令平均执行时延  |
| 请求监控 | 总请求  | Node     | commands      | 次/秒 | QPS，命令执行次数                              |
| 请求监控 | 读请求    | Node     | cmd_read      | 次/秒 | 读命令执行次数，读命令分类，请参见 [命令分类](#mlfl)          |
| 请求监控 | 写请求    | Node     | cmd_write    | 次/秒 | 写命令执行次数，写命令分类，请参见 [命令分类](#mlfl)           |
| 请求监控 |  其他请求           | Node     | cmd_other          | 次/秒 | 读写命令之外的命令执行次数 |

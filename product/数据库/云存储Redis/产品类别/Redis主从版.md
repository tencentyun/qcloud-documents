...
### Redis主从版简介
Redis主从版是最通用的Redis版本，采用主从节点部署架构，提供数据持久化和备份，适用于对数据可靠性、可用性都有要求的场景。主节点提供日常服务访问，备节点提供 HA 高可用，当主节点发生故障，系统会自动切换至备节点，保证业务平稳运行。
...
### 使用限制
 ...
 1. Redis主从版支持0.25～60GB规格，需要更大的主从版规格可以选择CKV主从版；
 2. Redis主从版的性能最大支持10万QPS，需要更高的QPS请选择Redis集群版，或者CKV集群版；

### 兼容性
云数据库 Redis 标准版在 Redis 2.8基础上进行开发，兼容 Redis 协议命令。自建的 Redis 数据库可以平滑迁移至 Redis 标准版。并且提供数据传输工具（DTS）可以进行增量的 Redis 迁移，保证业务平稳过渡。命令兼容性如下表：
**主从版 Redis 支持命令**

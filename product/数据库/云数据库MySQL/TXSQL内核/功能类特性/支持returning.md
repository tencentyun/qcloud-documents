## 功能介绍
在某些使用场景下，用户需要在 DML 操作后返回刚操作的数据行。实现这个需求一般有两种办法：一是在开启事务的后在 DML 语句后紧跟一条 SELECT 语句；二是使用触发器等较为复杂的操作实现。前者主要会增加一条 SELECT 语句的开销，后者则会令 SQL 的实现变得更加复杂并且不够灵活（需要创建触发器）。
...
4. INSERT/REPLACE的RETURNING 语句若需要返回 last_insert_id()，则该 last_insert_id() 的值为该语句执行成功之前的值。若用户需要获得精确的 last_insert_id() 值，建议使用 RETURNING 直接返回该表的自增列 ID。

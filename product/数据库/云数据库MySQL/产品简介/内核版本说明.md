TXSQL 是腾讯云数据库团队维护的 MySQL 内核分支，100%兼容原生 MySQL 版本，TXSQL 提供了类似于 MySQL 企业版的诸多功能，如企业级数据透明加密、升级、线程池、并行查询、备份恢复等功能。
TXSQL 在 binlog 复制、InnoDB 存储引擎以及 Server 层面进行了大量优化，拥有更好的性能、稳定性和可维护性。

云数据库 MySQL 使用 TXSQL 内核，为用户提供 MySQL 全部功能的同时，另外提供了企业级的容灾、恢复、监控、性能优化、读写分离、透明数据加密、审计等高级特性。
   
云数据库 MySQL 支持自动升级内核版本，内核版本的升级可实现新功能使用、性能提升、问题修复等功能。

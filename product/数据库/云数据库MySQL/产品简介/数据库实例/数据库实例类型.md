云数据库 MySQL实例，或简称实例，是在腾讯云中独立运行的数据库环境，是用户购买云数据库MySQL服务的基本单位，用户可通过控制台来创建、修改和删除云数据库MySQL实例。各实例之间相互独立、资源隔离，相互之间不存在CPU、内存、IO等抢占问题，每个实例拥有其自己的特性，例如数据库类型、版本等，系统有相应的参数来控制实例行为。

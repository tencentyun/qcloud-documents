...
腾讯云数据库 MySQL 默认支持 InnoDB 存储引擎，并在 MySQL 5.6 以上的版本中，不再支持 MyISAM 存储引擎。云数据库 MySQL 不支持 MyISAM 引擎的主要原因有如下几个：
...
- MyISAM 的发展在向 InnoDB 转移，在最新的 8.0 版本中 MySQL 系统表均已采用 InnoDB。


更多关于 InnoDB 和 MyISAM 引擎的信息，请参见 [InnoDB 简介](https://dev.mysql.com/doc/refman/5.7/en/innodb-introduction.html) 和 [MyISAM 简介](https://dev.mysql.com/doc/refman/5.7/en/myisam-storage-engine.html)。

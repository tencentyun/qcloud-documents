> 后续本文档将逐步归口至 [数据传输服务>教程>数据导入](https://www.qcloud.com/document/product/571/8850) 进行管理



在线热迁移提供 CVM 自建 MySQL 数据库到 CDB 的在线热迁移功能，用户可在不停服的情况下对数据进行迁移，具有公网 IP/Port 的本地 IDC 或其他云上 MySQL 数据库迁移至与数据库CDB 服务。

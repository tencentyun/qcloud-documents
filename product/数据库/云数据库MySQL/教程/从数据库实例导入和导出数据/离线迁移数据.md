...
### 1.1. windows 迁移数据库
1.<span></span> 从云数据库数据控制台下载备份文件，记录完整的路径名称。
本例中的完整路径为： F:\download\cdb147691_backup_20170717050142

![][image-1]

2.<span></span> 通过 MySQL 命令行工具进行还原。

![][image-2]

3.<span></span> 登录 MySQL 数据库可以查看到备份数据已经还原到服务器内。
...
3.<span></span> 可以查看到备份的数据库表已导入到目标服务器对应的数据库中。

## 2. 命令行工具导出
1. 使用 MySQL 工具 mysqldump 生成待导入的 SQL 文件，方式如下：
><div style="margin-bottom:0">**注意：**</div>
...
1.<span></span> 登录 CVM 主机，使用 MySQL 工具 mysqldump 生成待导入的 SQL 文件。

![][image-5]

2.<span></span> 通过 MySQL 命令行工具进行还原，本例将数据还原到 CVM 服务器上。

![][image-6]

3.<span></span> 可以查看到备份的数据库已导入到目标服务器对应的数据库中。

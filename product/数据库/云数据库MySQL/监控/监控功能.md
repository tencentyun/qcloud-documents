...
|---------|---------|---------|---------|
|每秒执行操作数|QPS|次/秒|实例维度|com_update+com_delete+com_insert+com_replace+select_count四个指标之和|
...
|磁盘使用空间|real_capacity|MB|实例维度|仅包括数据文件，不含日志空间|
|磁盘占用空间|capacity|MB|实例维度|包括数据文件和日志文件|

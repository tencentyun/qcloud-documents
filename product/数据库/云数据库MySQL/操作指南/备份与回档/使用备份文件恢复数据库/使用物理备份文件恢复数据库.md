...
### 解包备份文件
1. 由于备份文件先经过 qpress 压缩，后经过 xbstream 打包（xbstream 为 Percona 的一种打包/解包工具），所以下载备份文件后，应该先用 xbstream 将其解包。xbstream 工具可以通过添加 Percona 的 yum 源安装或者直接下载二进制包。
 - Percona yum 源安装
```
...
```
**特别注意 : 这里安装要选择xtrabackup-24的版本，而不是xtrabackup-22的老版，只有24以上版本的xtrabackup才能支持后面的xtrabackup --decompress的qpress解压参数**
...
```
>?xtrabackup --decompress会调用qpress工具,使用--decompress参数前需要安装qpress。

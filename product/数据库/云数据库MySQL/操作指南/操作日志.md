...
您也可以在命令行界面（CLI）或者使用云数据库 API 查看、下载数据库日志，请参见 [查询备份日志](https://cloud.tencent.com/document/product/236/15842) 与 [查询二进制日志](https://cloud.tencent.com/document/product/236/15843)。
...
您可单击【本地下载】直接下载，也可以选择复制下载地址，并登录到云数据库所在 VPC 下的 CVM（Linux 系统） 中，运用 wget 命令进行下载。
>?
>- wget 命令格式：wget -c '日志文件下载地址' -O 自定义文件名.log
>示例如下：
```
wget -c 'http://szx.dl.cdb.tencentyun.com:303/cfdee?appid=12510&time=157691&sign=aIGM%3D' -O test.log
```
>- 若日志大小为0KB，则无法下载。
<table>
<thead>

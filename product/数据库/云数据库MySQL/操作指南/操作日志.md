...
您可以在控制台的操作日志页面查看实例的慢日志明细、错误日志明细、回档日志，及下载慢日志。也可以在命令行界面（CLI）或者使用云数据库 API 查看、下载数据库日志，请参见 [查询备份日志](https://cloud.tencent.com/document/product/236/15842) 与 [查询二进制日志](https://cloud.tencent.com/document/product/236/15843)。

>?仅云数据库 MySQL 高可用版和单节点高 IO 版实例支持操作日志管理功能，基础版实例不支持。
>

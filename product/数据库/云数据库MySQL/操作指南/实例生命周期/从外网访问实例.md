## 操作场景
如果您的云服务器与云数据库部署在同一地域上，则无需申请外网地址。如果在不同地域或者在腾讯云以外的系统上，则需开启外网地址来连接腾讯云云数据库 MySQL 数据库（**中国香港及国外地区 MySQL 实例暂不支持开通外网**），本文为您介绍如何开启外网访问地址及登录实例。

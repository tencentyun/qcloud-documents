## 操作场景
[腾讯云 MySQL](https://cloud.tencent.com/product/cdb-overview) 支持用户创建一个或多个只读实例组成只读实例 RO 组，适用于用户的读写分离和一主多从应用场景，可显著提高用户数据库的读负载能力。
>!
> - 创建只读实例之前需要先创建云数据库主实例。请参考腾讯云  MySQL 产品文档 [购买与续费](/doc/product/236/5160) 中的步骤新建云数据库主实例。
> - 使用腾讯云 MySQL 之前，需要对数据库进行初始化。请参考腾讯云  MySQL 产品文档 [初始化 MySQL 数据库](/doc/product/236/3128) 中的步骤初始化 MySQL 数据库。

## 操作步骤
### 创建只读实例 RO 组
1. 登录 [云数据库MySQL控制台](https://console.cloud.tencent.com/cdb/ )，实例列表中，选择需要创建只读实例的云数据库，单击【管理】，进入云数据库主实例管理界面。
![](https://main.qcloudimg.com/raw/dbaf189318e65d01b1ad1aa469c3d09e.png)
2. 在管理界面中，在【实例架构图】内单击【添加只读实例】，进入只读实例管理界面。
...
![](https://main.qcloudimg.com/raw/955a03a5fcc93f858ad3160d19fe4e1a.png)
...
1. 在 [云数据库MySQL控制台](https://console.cloud.tencent.com/cdb)，选择需要设置只读实例 RO 组的云数据库主实例，单击【管理】，进入云数据库主实例管理界面。
![](https://main.qcloudimg.com/raw/7ac1dfa3f5f3ab47b05dcf3b333e38e5.png)

2. 在只读实例 RO 组管理页面，单击【配置】，进入只读实例 RO 组配置界面。
![](https://main.qcloudimg.com/raw/d8ca5831397e4af4afdc4e0f6f0de719.png)
3. 在只读实例 RO 组配置界面，可以对只读实例 RO 组进行详细配置。

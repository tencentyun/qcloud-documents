...
![](https://main.qcloudimg.com/raw/83e46e7f30ca7871fe408ce38823b664.png)
	...
	- <span id="rid-of-delay">延时超限剔除：是否启动剔除策略。被剔除的实例权重自动设置为 0，且新增实例状态：停服同步中。若只读实例延迟超过阈值被剔除会向用户发出警告，且实例状态为停服同步中、权重为0，当只读实例延迟时间小于阈值时会重新加入到RO组。同时，不管实例是否启用延时超限剔除功能，当只读实例故障被剔除后，待实例修复也会重新加入到RO组。</span>

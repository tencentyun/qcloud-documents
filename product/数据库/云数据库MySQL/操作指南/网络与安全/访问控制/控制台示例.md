## 操作场景
您可以使用访问管理 CAM 策略让用户拥有在 TencentDB 控制台中查看和使用特定资源的权限。该部分的示例能够使用户使用控制台的特定部分的策略。
...
```
以上策略是通过让用户分别对 TencentDB、私有网络 VPC、安全组、对象存储 COS、密钥管理服务 KMS 和 Monitor 中所有资源进行 CAM 策略授权来达到目的。

### TencentDB 的只读策略
如果您只想让用户拥有查询 TencentDB 实例的权限，但是不具有创建、删除和修改的权限，您可以对该用户使用名称为：QcloudCDBInnerReadOnlyAccess 的策略。

>!建议配置 TencentDB 的只读策略。
...
```
以上策略是通过让用户分别对 TencentDB 中所有以单词“Describe”开头的操作进行 CAM 策略授权来达到目的。
...
- TencentDB 中所有以单词“Describe”开头的操作。
- VPC 中所有以单词“Describe”开头的操作、所有以单词“Inquiry”开头的操作和所有以单词“Get”开头的操作。
- 安全组中所有以单词“DescribeSecurityGroup”开头的所有操作。
- COS 中所有以单词“List”开头的操作、所有以单词“Get”开头的操作、所有以单词“Head”开头的操作和名为“OptionsObject”的操作。
- Monitor 中所有的操作。

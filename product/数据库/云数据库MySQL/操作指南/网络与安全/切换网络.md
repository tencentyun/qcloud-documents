...
## 注意事项
- 将基础网络切换为私有网络后，仅相同私有网络下客户端可以互通。可以通过对私有网络网段[配置](https://cloud.tencent.com/document/product/215/36515)，将私有网络 IP 保持与基础网络 IP 地址不变。
...
### RO 组切换网络
1. 登录 [MySQL 控制台](https://console.cloud.tencent.com/cdb)，在实例列表，单击实例 ID 或**操作**列的**管理**，进入实例详情页面。
2. 在只读实例的 **RO 组信息**中，根据网络间切换类型（基础网络转 vpc / vpc 转 vpc），单击**更换子网**或**转 VPC 网络**。

...
### Windows 迁移数据库
>!云数据库 MySQL 物理备份是基于 Linux 系统下的文件拷贝，不适用于 Windows 系统。Windows 系统下请使用云数据库 MySQL 的逻辑备份。

1. 以 db_blog 这个数据库为例。进入 [云数据库数据控制台](https://console.cloud.tencent.com/cdb) 找到需要导出数据的实例，选择【管理】 > 【备份管理】进入备份管理页面，在【备份列表】中找到需要下载的备份文件，选择【下载】 > 【部分下载】。

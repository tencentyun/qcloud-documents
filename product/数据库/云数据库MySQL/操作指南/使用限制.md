...
由于资源的有限性，为了隔离用户间性能影响，TencentDB for MySQL 对各种类型的 MySQL 实例做了数据量限制。下面从技术角度谈谈 TencentDB for MySQL 在大数据量下的单个实例以及单个表使用的影响：

**大数据量表**：单表数据量过大后，MySQL 对单表资源的管理成本（数据、索引等）变更，将直接影响表的处理效率。例如，当一个业务表（innodb）数据量达到了 5G 后，更新操作延迟明显增大，直接影响了业务的响应时间，后来不得不通过迁移分表来缓解这个问题。

**大数据量实例**：云数据库默认存储引擎是 Innodb，当实例中的数据及索引页都能被 innodb 的 cache buffer 缓存住时，MySQL 实例能够支持很大的并发访问。如果实例数据量过大，会导致 cache buffer 频繁的数据换出换进，MySQL 瓶颈很快转到 IO 上，访问吞吐量直趋下降（例如，某云数据库实例本可以支持 8000 次访问，当数据量为 cache buffer 大小的两倍时，仅能支持 700 次每秒左右的访问）。

### 连接数限制

TencentDB for MySQL 的连接数上限为 MySQL 的系统变量 max_connections，当 TencentDB for MySQL 实例连接数量超过 max_connections 时，新的连接将无法建立。
云数据库大型默认值为 3000，其它规格为 800。用户可以根据需要自行调整 max_connections 的值。
但是，连接数越多，消耗系统资源也越多。如果连接数超过实际系统的负载承受能力范围，必然影响系统服务质量。
关于 max_connections 可以参考 MySQL 官方手册。
...
1. 对于使用 Linux 云服务器的开发者，可以通过云数据库导出工具获取慢查询日志，详见：云数据库数据导出。
...
#### 1. 数据库账户权限
TencentDB for MySQL 不再向用户提供实例 super user 权限，需要 super user 权限才能修改的参数可通过控制台"参数配置"功能和提交工单两种方式修改，小部分参数无法修改。

#### 2. 数据库引擎
出于性能和安全性考虑建议尽量采用 InnoDB 存储引擎，TencentDB for MySQL 5.6 以上版本不再支持 MyISAM 引擎。

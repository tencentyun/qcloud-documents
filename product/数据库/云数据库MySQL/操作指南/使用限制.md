...
云数据库默认连接数可在 [MySQL 控制台](https://console.cloud.tencent.com/cdb) 的参数设置页（点进对应实例>数据库管理>参数设置）查看。用户可以根据需要自行调整 max_connections 的值。但连接数越多，消耗系统资源也越多。如果连接数超过实际系统的负载承受能力范围，必然影响系统服务质量。
...
### 云数据库的 binlog 保存时间说明
云数据库 MySQL binlog 日志文件可以保留7天 - 732天，默认为7天（实例管理页>备份恢复>自动备份设置里可配置保留时间）。 
...
1. 通过SQL语言执行如下语句可修改云数据库实例的默认字符集编码：
```
...
2. 通过控制台修改：MySQL 实例列表，单击实例 ID / 名称进入实例详情页，在基本信息里找到字符集，单击修改的图标即可修改字符集。
![](https://qcloudimg.tencent-cloud.cn/raw/1b778cef2b848abfda8a45d3b7ef2a57.png)
![](https://qcloudimg.tencent-cloud.cn/raw/f5ca7dd40c06fdc7082ed4c259342e0d.png)
3. 执行如下语句可修改当前连接的字符集编码：
```
...
SET names utf8;
```
4. 对于 PHP 程序，可通过如下函数设置当前连接的字符集编码：
```
...
```
5. 对于 Java 程序，可通过如下方式设置当前连接的字符集编码：
```
...
## 数据库帐户权限
云数据库 MySQL 不再向用户提供实例 super user 权限，需要 super user 权限才能修改的参数可通过 控制台 点进对应实例>数据库管理>[参数设置](https://cloud.tencent.com/document/product/236/42500)功能修改。

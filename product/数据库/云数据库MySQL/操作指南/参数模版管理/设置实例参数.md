...
### 方式一：通过参数设置页面导入
1. 登录 [MySQL 控制台](https://console.cloud.tencent.com/cdb)，在实例列表，单击实例 ID，进入实例管理页面。
2. 选择**数据库管理** > **参数设置**页，单击**自定义模板**（若之前没有设置过常用的自定义模板，可通过MySQL 控制台，左侧导航栏的参数模板进入找到自定义模板，点击创建模板即可预先设置参数模板，之后即可通过第2步从自定义模板导入）。
...
3.点击**选择文件**找到需要的参数文件，然后点击**导入并覆盖原有参数**。
...
## 如何恢复默认模板
1.登录 [MySQL 控制台](https://console.cloud.tencent.com/cdb)，在实例列表，单击实例 ID，进入实例管理页面。
2.选择**数据库管理** >**参数设置**，点击**默认模板**，之后选择**系统默认模板**并点击**导入覆盖原有参数**即可。
![](https://qcloudimg.tencent-cloud.cn/raw/9b293d78e824e28cc8345508aed97fe4.png)
3.在弹出的对话框，选择参数任务的执行方式，单击确定。
>?
>- 若选择**立即执行**，所选实例的参数变更任务会立即执行并生效。
>- 若选择**维护时间内**，所选实例的参数变更任务会在实例的 [维护时间](https://cloud.tencent.com/document/product/236/10929) 内执行并生效。
>

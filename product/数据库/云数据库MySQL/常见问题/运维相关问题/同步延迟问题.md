...
2. 若只读实例、灾备实例承载了大量的分析类业务导致实例负载过高，需将其实例规格升级至合适的配置或者对其性能 [低效的 SQL 进行优化](https://cloud.tencent.com/document/product/1130/39081)。
...
2. 对应解决方案如下：
 - 大事物运行，阻塞 DDL，继而阻塞所有同表的后续操作，根据 DBbrain 的异常诊断提示找到大事物的 ID，然后 kill 掉
 - 未提交事物，阻塞 DDL，继而阻塞所有同表的后续操作，根据 DBbrain 的异常诊断找到未提交事物的 ID，然后 kill 掉，并排查程序，及时提交事务。

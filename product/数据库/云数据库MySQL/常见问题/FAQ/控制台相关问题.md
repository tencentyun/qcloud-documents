...
### 怎样查看 MySQL 数据表?
您可以登录云数据库 MySQL  [管理控制台](https://console.cloud.tencent.com/cdb) ，选中目标实例，单击操作栏中的【登录】按钮，跳转到DMC数据管理工具，登陆后单击【前往PMA】，来到 MySQL 数据库操作工具。在这里您可以查看对应数据库和表的结构，并且可以运行 sql 语句，如下图所示：
...
实例监控中 max_connections 数值是个定制，表示最大允许的连接个数；当前打开连接数表示当前时刻实际的连接数，是个随时间变化的值，具体如下图：

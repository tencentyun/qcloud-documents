...
实例监控中 max_connections 数值是个定制，表示最大允许的连接个数；【当前打开连接数】表示当前时刻实际的连接数，是个随时间变化的值，具体如下图：
...
### MySQL 的临时实例什么时间会有？
临时实例上线，腾讯云会有公告通知，您可以关注 [腾讯云官网](https://cloud.tencent.com/) 。

### 如何获知磁盘空间不足？
监控中心对云数据库的磁盘空间进行了监控，当云数据库的使用空间超过 90% 的时候就会触发短信和邮件告警，这里只需要在云监控中配置好对应的告警接收人即可（如何配置请参考 [告警功能](https://cloud.tencent.com/document/product/236/8457)），当磁盘空间不足的时候就能收到告警。

...
### 如何设置开启 MySQL 的定时器功能？
数据库 TencentDB for MySQL 的参数设置中可以修改参数，实现开启定时器功能。
...
数据库 TencentDB for MySQL 的参数设置中可以增加超时连接时间。
...
数据库 TencentDB for MySQL 的参数设置中可以调整 group_concat 参数。
进入 [云数据库控制台](https://console.cloud.tencent.com/cdb)，找到需要修改的实例单击【管理】按钮，进入数据库管理页面，接着单击【参数设置】。在控制台参数设置中修改 group_concat 参数 。

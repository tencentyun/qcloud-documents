...
云数据库 MySQL 最大连接数默认为800，如出现连接数过高时，建议先排查原因并解决，不建议直接调大连接数。
您可登录 [控制台](https://console.cloud.tencent.com/cdb)，单击实例名进入实例管理页，选择【数据库管理】>【参数设置】页，找到 max_connections 参数进行修改。

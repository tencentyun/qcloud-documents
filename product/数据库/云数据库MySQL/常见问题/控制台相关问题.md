...
### 如何获知磁盘空间不足？
监控中心对云数据库的磁盘空间进行了监控，当云数据库的使用空间超过90%时，会触发短信和邮件告警，您只需要在腾讯云可观测平台中配置好对应的告警接收人（如何配置请参见 [告警功能](https://cloud.tencent.com/document/product/236/8457)），当空间不足的时候就能收到告警。

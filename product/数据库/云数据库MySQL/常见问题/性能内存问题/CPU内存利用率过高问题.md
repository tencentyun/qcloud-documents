...
## 现象描述
云数据库 MySQL 出现响应变慢、无法获取连接、超时等现象。
用户在哪里（如控制台）发现什么数据，或者什么现象，代表CPU使用率过高？
——产品帮忙补充下
...
>?us% 的定义，是指用户线程占用 CPU 的时间多少，这代表着用户线程占用了大量的时间。
>
- **慢查询**
进行长时间的计算，例如：order by，group by，临时表，join 等。这一类问题可能是查询效率不高，导致单个 SQL 语句长时间占用 CPU 时间。
- **计算量大**
单纯的数据量比较多，导致计算量巨大。
...
### 系统线程
在实际的环境中，系统线程遇到问题的情况会比较少，一般来说，多个系统线程很少会同时跑满，只要服务器的可用核心数大于等于 4 ，一般也不会遇到 CPU 100%，当然有一些 bug 可能会有影响，如下图所示：
...
## 解决思路
一般故障场景，基本是用户线程繁忙导致，因此本文主要介绍用户线程导致的 CPU 利用率过高，提供对应的解决方案。
- 慢查询：建议使用 DBbrain 来排查和优化，详情请参见 [慢查询](#mcx)。
- 计算量大：若数据量比较大，即使索引和执行计划没什么问题，也会导致 CPU 100%，处理措施详情请参见 [计算量大](#jsld)。
- 高 QPS：无论是 row_examined/rows_sent 的比值，还是 SQL 的索引、执行计划等都不会有什么明显问题，只是 QPS 指标会比较高，处理措施详情参见 [高 QPS](#gqps)。
...
### [计算量大](id:jsld)
若数据量比较大，即使索引和执行计划没什么问题，也会导致 CPU 100%，而且结合 MySQL one-thread-per-connection 的特性，并不需要太多的并发就能把 CPU 使用率跑满。
这一类查询是比较容易排查的，因为执行时间一般会比较久，在 processlist 里面就会非常显眼，反而是 slowlog 里可能找不到，因为没有执行完的语句是不会记录的。

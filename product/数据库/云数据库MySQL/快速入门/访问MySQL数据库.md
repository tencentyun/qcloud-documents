连接到 MySQL 数据库的方式有两种：
- **内网访问**：使用在同一个可用区的 CVM 来访问自动分配给数据库的内网地址。这种方式使用内网高速网络，延迟低。此台 CVM 需要与数据库同一地域下的基础网络中，或者同一个 VPC 中，VPC 更多信息请参见<a href="https://cloud.tencent.com/document/product/215/20046" target="_blank"> VPC 概述</a>。

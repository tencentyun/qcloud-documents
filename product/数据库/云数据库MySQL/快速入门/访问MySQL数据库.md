...
| 方式|  说明|
|---------|---------|
| 内网访问 |使用云服务器 CVM 直接访问云数据库的内网地址，这种访问方式使用内网高速网络，延迟低。<li>云服务器和数据库须是同一账号，且同一个[ VPC](https://cloud.tencent.com/document/product/215/20046) 内（保障同一个地域），或同在基础网络内。<li>内网地址系统默认提供，可在 [MySQL 控制台](https://console.cloud.tencent.com/cdb) 的实例列表或实例详情页查看。| 

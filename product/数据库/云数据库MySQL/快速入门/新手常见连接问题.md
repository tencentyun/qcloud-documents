...
| 可能原因                   | 说明                          | 可能原因                     | 说明                              |
| ------------------- | -------------------- |-------------------- |-------------------- |
| [网络问题1](#cvmj)   | 云服务器（CVM）采用私有网络（VPC），MySQL 采用基础网络 | [帐号所授权的主机地址问题](#sjkzhzjpzyw) | 数据库帐号限制了访问的具体主机地址   | [密码问题](#mmwt) | 忘记密码或输入错误密码  |
| [网络问题2](#cjmv) | CVM 采用基础网络，MySQL 采用 VPC  | [连接语法问题](#ljyfwt) | 连接命令有误  |
| [网络问题3](#cmvbt)  | CVM 与 MySQL 在同一地域内，但属于不同的 VPC 网络   | [IP 和端口问题](#idyw)  | 命令行或配置文件中的 IP 和端口有误  |
| [网络问题4](#dywt)   | CVM 与 MySQL 不在同一个地域内，属于不同的 VPC 网络 | MySQL 实例状态 |  MySQL 实例隔离中，可通过 <a href="https://console.cloud.tencent.com/mysql/recycle">回收站</a> 恢复
...
#### [CVM 与 MySQL 不在同一个地域内，属于不同的 VPC 网络](id:cmvbt)

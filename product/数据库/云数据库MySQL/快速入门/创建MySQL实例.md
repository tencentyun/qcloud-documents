...
1. 登录 [MySQL 购买页](https://buy.cloud.tencent.com/cdb)，根据实际需求选择各项配置信息，确认无误后，单击**立即购买**。
 ...
 - **数据库版本**：云数据库 MySQL 目前支持以下版本：MySQL 8.0、MySQL 5.7、MySQL 5.6、MySQL 5.5，各个版本相关特性，请参见 [官方文档](https://cloud.tencent.com/document/product/236/30969)。
...
>?1.主备机处于不同可用区，可能会增加2ms - 3ms的同步网络延迟；
>2.购买云服务时建议选择最靠近您的地域，可降低访问时延、提高下载速度；
...
>?安全组入站规则需要放通 MySQL 实例的3306端口。MySQL 内网默认端口为3306，同时支持自定义端口，若修改过默认端口号，安全组中需放通 MySQL 新端口信息。
 ...
 - **标签**：便于分类管理实例资源，[了解详情](https://cloud.tencent.com/document/product/236/30971)。
 - **实例名**：可选择创建后命名或立即命名。

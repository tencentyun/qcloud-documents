...
1. 登录 [MySQL 购买页](https://buy.cloud.tencent.com/cdb)，根据实际需求选择各项配置信息，确认无误后，单击**立即购买**。
 ...
 - **地域**：选择您业务需要部署 MySQL 的地域。建议您选择与云服务器同一个地域，不同地域的云产品内网不通，购买后不能更换。
  - **架构**：提供双节点、三节点，各架构介绍请参见 [数据库架构](https://cloud.tencent.com/document/product/236/17136)。
...
>?主备机处于不同可用区，可能会增加2ms - 3ms的同步网络延迟。
 - **隔离策略**：支持通用型、独享型，详情请参见 [隔离策略](https://cloud.tencent.com/document/product/236/53253)。
...
2. 支付完成后，返回实例列表，会看到实例显示发货中（大概需要3min - 5min中，请耐心等待），待实例状态变为未初始化，即可进行初始化操作。

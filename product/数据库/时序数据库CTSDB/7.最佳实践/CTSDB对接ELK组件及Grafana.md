## 概述 
腾讯云数据库 CTSDB 是一款分布式、可扩展、支持近实时数据搜索与分析的时序数据库，且兼容 ELK 生态组件，您可以非常方便的使用 ELK 组件与 CTSDB 对接。ELK 组件提供了丰富的数据处理功能，包括数据采集、数据清洗、可视化图形展示等。常用的 ELK 生态组件包括 Filebeat、Logstash、Kibana。同时，CTSDB 也支持 Grafana 作为可视化平台。常见架构图如下： 
...
3. **启动 **
Filebeat 启动时可以指定配置文件路径，若不指定则默认使用filebeat.yml
...
#### Logstash 使用流程  
1. **安装** 
Logstash 安装较为简单，具体参考 https://www.elastic.co/guide/en/logstash/current/installing-logstash.html
2. **配置** 
...
```
grok filter 插件在 Logstash 默认可用的，其能够将非结构化的数据解析为结构化的数据，具体使用参考文档 https://www.elastic.co/guide/en/logstash/6.2/plugins-filters-grok.html
- 启动 Logstash、Filebeat，并观察 CTSDB 中对应表的数据：
```
...
####  Grafana  
1. 安装 
Grafana 的安装过程可以参考官方文档 http://docs.grafana.org/installation/
2. 配置
Grafana 的配置项较多，可以使用默认的配置，具体配置说明参考 http://docs.grafana.org/installation/configuration/
3. 运行 
...
## 小结  
以上为 ELK 生态组件及 Grafana 对接 CTSDB 的详细使用过程，如在使用过程中遇到问题需要解决，欢迎提工单联系我们。

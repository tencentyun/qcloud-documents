## 性能概述 ##
时序数据库 CTSDB 由节点组成，单节点的规格和分片数量决定了 CTSDB 实例的处理能力。理论上讲：<br>
CTSDB 实例读写并发性能=∑（某节点的性能 * 节点数量）。<br>
因此，节点规格越高，节点数量越多，实例的读写能力越强。而单节点的性能主要跟CPU和内存配置相关。**实例的具体性能随着单节点配置、节点数量和写入字段数量等而改变**。本文档给出的测试数据是在指定参数下的参考值，仅作为选型参考依据，实际情况需要业务真实的测试。
## 性能测试 ##
### 测试方法 ###
## 测试工具 ##
...
>  其中，{ctsdb_ip_port} 为腾讯云CTSDB访问端口，{user}和{passwd}分别为用户名和密码。

...
实例名主要用于对 TDSQL-C PostgreSQL版 实例进行区分和管理，默认实例名和实例 ID 保持一致，也可以通过控制台对实例名字进行修改。

## 在实例列表中修改实例名
1. 登录 [TDSQL-C PostgreSQL版 控制台](https://console.cloud.tencent.com/cynosdb?dbType=POSTGRESQL)，在集群列表，找到需要的集群，单击集群 ID，进入集群管理页面。

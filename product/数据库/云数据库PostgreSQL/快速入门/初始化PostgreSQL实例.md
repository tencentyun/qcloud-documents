...
3. 选择状态为【待初始化】的 PostgreSQL 实例，在操作列单击【初始化】。
![](https://main.qcloudimg.com/raw/6fc7151ff30dc818ad97f08417001317.png)
4. 在弹出的初始化对话框中，配置初始化相关参数，单击【确定】开始初始化。
 - **设置管理员用户名**：帐号名需要1-16个字符，由字母、数字或特殊字符组成；以字母开头，字母或数字结尾；特殊字符为_；不区分大小写，不能为: postgres，不能以pg_开头。
 - **密码**：密码需要8-32个字符，至少包含英文、数字和符号_+-&=!@#$%^*()中的2种
 - **确认密码**：再次输入密码。
 - **支持字符集**：支持 LATIN1 、UTF8 字符集，默认字符集编码格式是 UTF8。
5. 返回实例列表，目标 PostgreSQL 实例的状态变为【运行中】，说明已初始化成功。

### 1. Creating Recovery Directory
mkdir recovery
Note: recovery is only a sample directory, which can be replaced with your own directory.

### 2.	Downloading Full Backup File
Go to PostgreSQL management page - "Tencent Cloud Console" -> "Cloud Products" -> "Relational Databases" -> "PostgreSQL Instance List" -> "Management" -> "Backup Management", click "Download" to get the download address of the backup file you need. (The server and the database must be in the same network)

![](https://mc.qcloudimg.com/static/img/a5de09aeecb4bf1d9ce20423c0c8ddd5/1.png)

### 3.	Decompressing Full Backup File
...
### 4. Installing posgresql of the Same Version (skip this step if it is already installed)
...
### 7.	Applying Incremental Backup File [Optional]
This is an optional step. If this step is skipped, the data of database when the full backup starts will be used.
Note: If xlog file is put under pg_xlog folder, pg will automatically replay xlog log. For example, if the full backup is done at 12:00 and all xlogs between 12:00 and 13:00 are put under pg_xlog folder, the database can be restored to the one at 13:00.
Download incremental backup file (xlog)
![](https://mc.qcloudimg.com/static/img/775b3a63d1fa37e1815ab13c100f8b40/4.png)


Decompress it to pg_xlog folder

...
详细操作参考 [数据迁移服务](https://cloud.tencent.com/document/product/571/16309)
 
# 使用psql命令
如果你的数据已经在 PostgreSQL 上，你可以 psql 命令将数据轻松的迁移到 CDB for PostgreSQL 中。
迁移数据的工作主要分为两大部分：
> （1）使用 pg_dump 命令进行逻辑备份，创建转储数据；
> （2）将上一步的备份数据恢复到 CDB for PostgreSQL 上。
准备操作：你应该已经完成 PostgreSQL 实例的准备，如未完成，请参见申请实例和连接实例的操作指引。

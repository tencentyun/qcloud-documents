...
TDE 主要用于保护静态数据（[data at rest](https://en.wikipedia.org/wiki/Data_at_rest)），防止磁盘被盗窃导致数据泄漏。

TDE 不保护传输中的数据（[data in transit](https://en.wikipedia.org/wiki/Data_in_transit)）和使用中的数据（[data in use](https://en.wikipedia.org/wiki/Data_in_use)）。

...
| 资源类型                       | 授权策略中的资源描述方法                                     |
| :----------------------------- | ------------------------------------------------------------ |
| [实例](#PostgreSQLCorrelation) | ` qcs::postgres:$region:$account:instanceId/$DBInstanceId ` <br>` qcs::postgres:$region:$account:instanceId/*  ` |

[PostgreSQL 实例相关](#PostgreSQLCorrelation)分别介绍了当前支持资源级权限的 PostgreSQL API 操作，以及每个操作支持的资源和条件密钥。设置资源路径时，您需要将`$region`、`$account`等变量参数修改为您实际的参数信息，同时您也可以在路径中使用 \* 通配符。相关操作示例可参见 [控制台示例](https://cloud.tencent.com/document/product/596/42905)。
...
### PostgreSQL 实例相关
**资源路径：**
` qcs::postgres:$region:$account:instanceId/$DBInstanceId ` <br>` qcs::postgres:$region:$account:instanceId/*  `
...
| DeleteServerlessDBInstance      | 删除ServerlessDB实例     |
| DescribeServerlessDBInstances   | 查询ServerlessDB实例列表 |

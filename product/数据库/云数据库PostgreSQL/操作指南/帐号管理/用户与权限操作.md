## 云数据库默认角色
腾讯云数据库 PostgreSQL 未开放superuser 角色属性和pg_execute_server_program、pg_read_server_files、pg_write_server_files角色供用户使用。但因为部分操作必须要求 superuser 进行，所以云数据库 PostgreSQL 提供了 pg_tencentdb_superuser 角色用以替代superuser。

...
>?PostgreSQL for Serverless 目前为免费内测阶段，可直接通过云 API 接口 [CreateServerlessDBInstance](https://cloud.tencent.com/document/product/409/42762) 创建实例。

## 系统架构
用户连接数据库后，通过 PostgreSQL for Serverless Proxy 层统一进行请求转发，然后进行数据操作。当用户请求数增加时，数据库将自动响应。当前设定单用户最高支持 QPS 40000/s。
![](https://main.qcloudimg.com/raw/1e6fb33d59eafdf0a84bffe49a09d4bd.png)

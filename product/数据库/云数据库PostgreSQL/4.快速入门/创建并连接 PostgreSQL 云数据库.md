...
> **注意：**
> 在创建 PostgreSQL 云数据库实例之前，您必须拥有一个腾讯云账户。如果您没有腾讯云账户，请在 [注册页面](https://cloud.tencent.com/register) 填写相关信息注册腾讯云账户。

## 一、创建 PostgreSQL 云数据库实例
在此步骤中，您会使用腾讯云 [控制台](https://console.cloud.tencent.com/) 创建 PostgreSQL 云数据库实例。
...
3. 在 PostgreSQL 云数据库购买界面中，选择配置属性。确认无误后，单击【立即购买】。
![](//mc.qcloudimg.com/static/img/90a26e59a3f499409592b1389dc976bb/image.png)
 ...
 - 数据库版本。提供 PostgreSQL 9.3.5 和 PostgreSQL 9.5.4 两个版本。不同可用区可能有所不同，具体以实际情况为准。
 - 实例规格和所需的硬盘。
 - 购买数量和购买时长。
4. 再次进入腾讯云云数据库 [控制台](https://console.cloud.tencent.com/cdb)，选择【PostgreSQL】，查看刚才创建的云数据库实例。状态显示是 **待初始化**。
...
6. 配置初始化参数，确认无误后，单击【确定】。初始化需要一定的时间，请耐心等待。当状态显示 **运行中**，表示云数据库初始化成功。
 ...
 - 支持的字符集。提供 UTF8 和 LANTIN1 两种字符集。
...
8. 在 PostgreSQL 云数据库实例详情页，开通外网地址。（此步骤不是必须。如果需要从外网访问云数据库必须开通外网地址。）
...
### 1. 下载并安装 pgAdmin
您可以从 pgAdmin 的官方 [网站](https://www.pgadmin.org/download/) 下载并安装 pgAdmin。本文档以 pgAdmin4 为例进行说明。
### 2. 新建数据库连接
安装好 pgAdmin4 后，打开客户端。右键单击【Servers】>【Create】>【Server】。
![](//mc.qcloudimg.com/static/img/d858eff4877223e5e8749a0a0e3aa193/image.png)
在弹出的 **Create-Server** 对话框中填写名称、主机 IP 地址、端口号、用户名和密码，新建数据库连接并单击【Save】保存。
- Name。数据库连接名称。
...
PostgreSQL 云数据库实例不需要手动删除，到期后没有续费将会自动停止。

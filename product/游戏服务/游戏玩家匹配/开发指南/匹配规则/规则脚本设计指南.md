...
>?measurements、referenceValue、operation、maxDistance、minDistance、maxLatency 和 distanceReference 的参数类型和是否必填与规则类型有关，不同的规则类型对应的参数类型和是否必填都不一致，详情可参见  [规则类型说明](https://cloud.tencent.com/document/product/1294/49604)。

...
<table class="typical">
	...
	<tr>
		...
		<td >当一个票据含有多个玩家时的处理方式</td>
		<td >否</td>
		<td ><ul><li>合法取值："each"，"min"，"max"，"avg"，"any"。（当用于比较的元素是字符串类型时，此处合法取值为"="，"!="） <li>不传默认取"each" <li>关于 partyAggregation 的实现机制和取值说明，详情请参见 <a  href= https://cloud.tencent.com/document/product/1294/49599>多人组队匹配</a></ul></td>
	</tr>
</tbody></table>
...
>?GPM 仅按地域延迟进行玩家撮合，不更新玩家传入的地域信息，也不为玩家进行地域选择。按延迟规则匹配完成后，对局中的玩家至少有一个共同地域满足规则条件，GSE 才将为玩家选择对战服地域。
>
- #### 规则属性说明
<table class="typical">
	<tbody>
	...
	<tr>
		...
		<td >合法取值："min"，"avg"。<ul><li>"min"表示一个匹配中所有玩家到一个地域的最小值 <li>"avg"表示一个匹配中所有玩家到一个地域的平均值</ul></td>
	</tr>
	</tr>
		<tr>
		<td>partyAggregation</td>

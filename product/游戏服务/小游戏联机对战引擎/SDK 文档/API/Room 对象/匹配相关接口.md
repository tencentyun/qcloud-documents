...
- 房间匹配指按照传入的参数（maxPlayers、roomType）搜索现存的房间，如果已经存在与房间属性 maxPlayers、roomType 对应相同的房间，同时满足房间允许加入（isForbidJoin 为 false）、房间为非私有（isPrivate 为 false）、房间人数未满等条件，则将调用者加入到该房间，并且房间内成员（不含调用者）会收到 onJoinRoom 广播。如果不存在这样的房间，会直接使用 maxPlayers、roomType 这两个参数作为房间属性为调用者创建一个新房间。

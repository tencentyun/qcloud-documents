...
设置过滤条件
where 可接收对象作为参数，表示筛选出拥有和传入对象相同的 key-value 的文档。例如筛选出所有类型为计算机的、内存为8g的商品：
...
如果要表达更复杂的查询，可使用高级查询指令，例如筛选出所有内存大于8g的计算机商品。
```js
...
注意 `eq` 指令比对象的方式有更大的灵活性，可以用于表示字段等于某个对象的情况：
...
##### or
表示需满足所有指定条件中的至少一个。如筛选出价格小于4000或在6000 - 8000之间的计算机。
...
如果要跨字段 “或” 操作：（如筛选出内存8g或 cpu 3.2 ghz 的计算机）。

...
您可以在开发环境输入 `MGOBE.Debugger.exclude=[“onRecvFrame”]` 取消接口的日志打印。
...
### 使用帧同步的游戏，操作杆的状态在客户端按固定频率去发还是只发有操作的帧？

建议操作的时候发帧，玩家A操作摇杆，就发一条帧消息，告诉每个客户端这个玩家方向变了。客户端拿到这个帧广播后，修改玩家A的方向。但是操作的频率会很高，可能导致发帧率很高，建议在操作摇杆的时候，通过一定的时间间隔或者方向变化伐值处理发帧。
...
### cocosCreate 打包安卓端显示90006报错，需要证书如何处理？

需要安装证书，参考链接：https://blog.csdn.net/u013654125/article/details/94394864
cc.url.raw("resources/***.cer") 作为 cacertNativeUrl，这个 cer 文件，可以在这个链接下载：
...
- 直接导入 SDK 即可，不用区分小游戏还是小程序，主要是导入 SDK 方式存在区别。小程序导入 SDK 使用两种方法：
```
const MGOBE = require("../MGOBE/MGOBE.js");
import * as MGOBE from "../MGOBE/MGOBE.js";
```
- 小程序里暂不支持 import "./MGOBE.js" 这种方式。

...
## 操作步骤
请在 Util.js 文件添加以下工具类方法。

1. 设置一个对象，用以保存一些常用属性和方法。
```
// 模拟全局对象
export const Global = {};
```

2. 生成 openId，在实际项目中需要使用微信平台的接口获取 openId。
```
// 生成测试 openId
export const mockOpenId = () => `mock_openId_${Math.ceil(Math.random() * 100)}_${(new Date()).getMilliseconds()}`;
```

3. 生成 userName，在实际项目中需要使用微信平台的接口获取玩家昵称。
```
// 生成测试 userName
export const mockName = () => `user_${Math.ceil(Math.random() * 100)}`;
```

4. 获得 canvas 对象上下文，以及画布宽度、高度信息。
```
// 获得 canvas 上下文
...
5. 最后还需要准备基本的点击事件管理方法，使用一个数组 clickHandlers 保存全部点击区域和点击回调，当监听到点击事件发生时，遍历该数组并执行点击回调。
```
	// 点击事件回调函数数组

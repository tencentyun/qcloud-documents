...
2. 在 Util.js 文件中，添加如下代码，用于生成 playerId，在实际项目中，需要使用微信平台的接口获取 openId。
```
// 生成测试 playerId
export const mockPlayerId = () => `playerId_${Math.ceil(Math.random() * 100)}_${(new Date()).getMilliseconds()}`;
```

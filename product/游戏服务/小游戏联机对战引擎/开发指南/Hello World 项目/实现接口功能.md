...
```
  matchRoom() {
    ...
    const playerInfo = {
      playerName: Util.mockName(),
    };

    Global.name = playerInfo.playerName;
...
```
  onRoomUpdate() {
    // 更新 按钮
    const playerId = Global.gameInfo.playerId;
    const player = Global.room.roomInfo.playerList.find(player => player.playerId === playerId);
    ...
    Global.room.roomInfo.playerList.forEach((player, i) => {
      msg += `${i}: ${player.playerId} ${player.customPlayerStatus === 1 ? "已准备" : "未准备"}\n`;
    });

    this.msgBox.setText(msg);

    if (Global.room.isMyRoom() && !Global.room.roomInfo.playerList.find(player => player.customPlayerStatus !== 1)) {
      // 全部玩家准备好就跳转
...
4. 编译代码，玩家单击【快速加房】进入 RoomView 后，单击【准备】，按钮和文本框都将更新。如果房间内的玩家都是【已准备】状态，页面将跳转到 GameView。
 ![房间信息更新](https://main.qcloudimg.com/raw/75a945d0807684604c4dbeec605d6775.png)
...
4. 使用两个设备进行“快速加房”操作（如模拟器+手机预览），两个玩家有可能匹配在一起，这时页面上将弹出“新玩家加入”提示。
 ...
 手机预览效果图如下所示：
 ![手机预览效果](https://main.qcloudimg.com/raw/1e8503301bce7640e6402929b67db507.jpg)
...
```
  onInit() {
    ...
    if (Global.room.roomInfo.frameStatus !== Global.ENUM.FrameStatusType.START) {
...
```
  onInit() {
    ...
    // 设置广播回调
    Global.room.onFrame = this.onFrame.bind(this);
  }
...
```
  ...
  stopFrameSync() {
    ...
    if (Global.room.frameStatus === Global.ENUM.FrameStatusType.STOP) {
      return;
    }
...
```
  onRoomUpdate() {
    if (Global.room.roomInfo.frameStatus === Global.ENUM.FrameStatusType.STOP &&

...
1. 在 MainView.js 的 onInit 方法中为 button 绑定点击事件，示例代码如下：
```
  onInit() {
...
2. 为 MainView 类添加 matchRoom 方法。示例代码如下：
```
  matchRoom() {
...
2. 0和1表示玩家“未准备”、“已准备”两种状态。customStatus 为 RoomView 类的属性，用来保存玩家状态。示例代码如下：
```
...
3. 为 RoomView 类添加 changeState 方法。示例代码如下：
```
  // 修改用户状态
...
1. 使用 timer 属性保存计时器，当页面触发 onDestroy 事件时，需要清除定时器。checkRoom 方法可以在页面 onInit 时进行调用。

2. 为 RoomView 类添加 timer 属性。示例代码如下：
```
export default class RoomView extends BaseView {
...
5. 编译代码，玩家单击【快速加房】进入 RoomView 后，单击【准备】，按钮和文本框都将更新。如果房间内的玩家都是【已准备】状态，页面将跳转到 GameView。
 ![房间信息更新](https://main.qcloudimg.com/raw/d0f1ba9073e0f0cfe653db34cf387d5a.png)

### 处理其他玩家加房
1. 当其他玩家进入房间时，SDK 会触发 joinRoomBroadcast 回调。因此可以在进入页面时为房间绑定 joinRoomBroadcast 广播回调函数。修改 RoomView.js 中的 onInit 方法。示例代码如下所示：
```
onInit() {
    // ...
...
2. 添加 joinRoomBroadcast 方法，这里做一个 toast 提醒玩家即可。示例代码如下所示：
```
  // 加房广播
...
```
 ...
 >!Global.room.joinRoomBroadcast 指定了一个回调函数，当页面跳转到其他页面时，这个回调函数引用还在。如果该函数依赖于页面脚本的上下文，有可能在触发广播时报错。
3. 请离开页面时置空 Global.room.joinRoomBroadcast。示例代码如下所示：
```
  onDestroy() {
    // 离开页面时一定要清除定时器
    clearInterval(this.timer);
...
3. 编译代码后，在模拟器中进行加房操作进入 RoomView 后，单击【退出房间】，即可跳转到 MainView。

### 实现开始帧同步

在 RoomView 中，全部玩家单击【准备】后，页面自动跳转到 GameView 页面。此时需要在 GameView 中检查房间状态，如果房间已开启帧同步，则激活该房间接收广播，否则调用开始帧同步接口。之后继续将玩家状态改为【未准备】，避免出现回到 RoomView 中又自动跳转 GameView 的情况。
...
3. 在 GameView 类中添加 changeState 方法。示例代码如下：
```
  // 修改用户状态
  changeState(customStatus) {
    const changeUserStatePara = { customStatus };
...
### 实现发送帧消息
玩家在游戏过程中发送的帧消息都是由一些指令组成，比如“跳跃”、“发射子弹”等操作。这里实现一个简单的指令，每个玩家发送一个随机数出去。在 GameView 类中添加一个 sendFrame 方法。示例代码如下：
```
  // 玩家发送帧消息
  sendFrame() {
    const data = {
...
1. 在 GameView 类的 onInit 中为 room 对象绑定帧消息广播回调，并实现 frameBroadcast 方法。示例代码如下：
```
  onInit() {
...
2. 在 frameBroadcast 中添加代码，实现记录帧广播消息，并定时发送帧消息。示例代码如下：
```
  frameId = 0;
  frameItems = [];
...
3. 帧同步停止后，页面需要跳转到 RoomView。检查房间帧同步状态有两种方法：
 - 使用监听 stopGameBroadcast 广播。
 - 使用定时器去不断检查 room.roomInfo.status 的值。

4. 利用定时器同时检查房间状态和房间成员状态，如果房间停止帧同步并且房间成员状态全部为0 ，就跳转到 RoomView。示例代码如下：
```
  checkRoom() {
    clearInterval(this.timer);
...
5. 在 onInit 中调用 checkRoom。示例代码如下：
```
  onInit() {
    // ...
...
6. 在 onDestroy 中清除定时器，并置空 frameBroadcast 回调。示例代码如下：
```
  onDestroy() {
    // 离开页面时一定要清除定时器
    clearInterval(this.timer);
...
至此，这个 HelloWorld 示例结束。请在本文档下载示例代码。更多 MGOBE API 可以参考 [API 文档](https://cloud.tencent.com/document/product/1038/33331)。

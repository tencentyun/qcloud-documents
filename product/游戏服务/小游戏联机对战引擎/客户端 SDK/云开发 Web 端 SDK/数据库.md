...
#### 设置过滤条件
where 可接收对象作为参数，表示筛选出拥有和传入对象相同的 key-value 的文档。筛选出所有类型为计算机的、内存为8g的商品。示例代码如下：
...
如果要表达更复杂的查询，可使用高级查询指令，例如筛选出所有内存大于8g的计算机商品。示例代码如下：
```js
...
单个 doc 的监听，也可以采用 doc('docId').watch() 形式，示例代码如下：
```js

...
游戏服务器引擎（Game Server Engine，GSE）部分支持资源级权限，即表示针对支持资源级权限的 GSE 操作，您可以控制何时允许用户执行操作或是允许用户使用的特定资源。下表将向您介绍 GSE 可授权的资源类型。
...
| API 操作                        | 资源路径                                                     | 说明                                                 |
| :------------------------------ | :----------------------------------------------------------- | :--------------------------------------------------- |
| DeleteGameServerSessionQueue    | `qcs::gse:$region:$account:gameServerSessionQueue/*`<br>`qcs::gse:$region:$account:gameServerSessionQueue/$Name` | 删除游戏服务器会话队列                               |
| StartGameServerSessionPlacement | `qcs::gse:$region:$account:gameServerSessionQueue/*`<br>`qcs::gse:$region:$account:gameServerSessionQueue/$GameServerSessionQueueName` | 开始将游戏服务器会话放置请求发送到游戏服务器会话队列 |
| UpdateGameServerSessionQueue    | `qcs::gse:$region:$account:gameServerSessionQueue/*`<br>`qcs::gse:$region:$account:gameServerSessionQueue/$Name` | 更新游戏服务器会话队列的属性                         |

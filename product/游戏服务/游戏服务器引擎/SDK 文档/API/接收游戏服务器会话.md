...
当游戏开发者通过调用 CreateGameServerSession 等相关云 API 接口生成 GameServerSession 后，GSE 会通过 OnStartGameServerSession 接口，将 GameServerSession 相关信息回传给游戏进程，游戏进程收到请求后，需要保存该 GameServerSession 对应的信息，并需要在其实现里调用 GSE 的[ActivateGameServerSession ](#激活游戏服务器会话)接口。

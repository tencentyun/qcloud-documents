...
GSE服务器接收到后，将服务器实例状态改为ACTIVE， 如果注册了回调函数每1分钟调用1次onHealthCheck，每1分钟调用1次onHealthCheck，onHealthCheck 1分钟内返回有效。

#### 参数描述

|参数名|类型/值|描述|
|:---|---|---|
|onStartGameServerSession|[std::function <void(TencentCloud::Gse::Model::GameServerSession) > onStartGameServerSession](http://www.baidu.com)|创建游戏会话后的回调函数|
|onProcessTerminate|[std::function<void()>onProcessTerminate](http://www.baidu.com)|通知请结束进程|
|onHealthCheck |[std::function<bool()> onHealthCheck](http://www.baidu.com)|定时健康检查函数|
|port|int 类型|游戏进程监听的端口号|
|logParameters|[TencentCloud::Gse::Server::LogParameters](http://www.baidu.com)|要上传的日志路径|

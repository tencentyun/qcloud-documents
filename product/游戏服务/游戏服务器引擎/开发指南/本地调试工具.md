...
使用 GSE Local 可以在本地设备上运行托管服务 GSE 的有限版本，并在其上测试您的游戏集成。此工具在对您的游戏集成进行迭代开发时将会使用到，其替代方法是将每个新生成的游戏包上传到 GSE 并配置服务器舰队来托管游戏。
...
2. 在运行时，游戏服务器每分钟将运行状况发送到 GSE（onHealthCheak回调）。
...
本地通过 curl 模拟客户端的调用。具体参数可参考 [云API接口](https://cloud.tencent.com/document/product/1165/42076)。
...
[测试游戏服务器](#test) 、 [测试游戏服务器和客户端](#test4) 中涉及到调用游戏会话和玩家会话的 API，都是通过 curl 来进行调用 GSE Local，在游戏服务中调用时，可以通过代码调用，本地调试需通过调用 `http://127.0.0.1:8080/capi`，验证您的游戏服务已成功进行任意或所有以下 API 调用：

...
### 1. 集成 gRPC 框架。
游戏服务器和 GSE 通过 gRPC 通信，游戏服务器程序集成 gRPC 框架，实现多语言接入，生成游戏服务器可执行文件。各语言的具体操作请您参考[gRPC-C++教程](#gRPC-C++教程)、[gRPC-C#教程](#gRPC-C#教程)、[gRPC-Go教程](#gRPC-Go教程)、[gRPC-Java教程](#gRPC-C++Java)、[gRPC-Lua教程](#gRPC-Lua教程)、[gRPC-Nodejs教程](#gRPC-Nodejs教程)。其他语言请您参考[gRPC官方文档](http://doc.oschina.net/grpc)。
...
### 3. 调用云 API ，获取服务端地址（IP:port或域名）。
通过创建游戏服务器会话、放置游戏服务器会话，即可获取服务端地址（IP:port或域名）。
#### 创建游戏服务器会话
两种创建方式：
* 通过服务器舰队创建
* 通过别名创建
...
* 当一个游戏服务器会话支持多局游戏或一个服务（如登陆服）时，调用云 API：
...
调用云API的具体操作请参见 [创建游戏服务器会话](#创建游戏服务器会话)文档。
...
调用云API的具体操作请参见 [放置游戏服务器会话](#放置游戏服务器会话)文档。

视频内容审核，是一项使用 AI 对视频内容进行智能审核的功能。对视频执行内容审核任务后，执行结果将包括审核评分、审核建议和嫌疑视频片段。根据结果中的“审核建议”，您可以决定视频是否允许公开，有效规避违规视频带来的法律风险和品牌伤害。可以对视频画面、ASR 文字和 OCR 文字三种对象进行智能审核，审核操作包括鉴黄、鉴暴和鉴政。
...
针对常见的操作组合，MPS 提供了 [预置视频内容审核模板](链接到https://cloud.tencent.com/document/product/862/37042的视频内容审核模板)。另外，您还可以调用 [服务端 API](https://cloud.tencent.com/document/product/862/37609) 创建和管理自定义视频内容审核模板。

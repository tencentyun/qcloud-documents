...
<span id="que4"></span>
### 如何接入 X-P2P？
必须接入 SDK。仅支持终端间分享；移动端、OTT 端支持动态加载，Flash、HTML5 支持远程动态加载。与通用播放器完全兼容，不需要更改播放接口，不需要弹幕对齐、用户分类等开发工作，仅需使用若干行代码即可迅速集成上线。

<span id="que5"></span>
### X-P2P 和传统 CDN 视频分发方案有什么区别？

相对传统 CDN 价格降低 50%、潮汐式的 P2P 节点与固定的 CDN Edge 节点之间智能调度使弱网传输能力更强。通过搭配弹性伸缩、多机热备和异地容灾，避免单点故障，使服务更加稳定。

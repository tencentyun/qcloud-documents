...
### step 2: 创建Player对象
接下来创建一个**Player**对象，它是所有SDK调用接口的承载者，同时，您需要使用**setPlayerView**接口，将Player对象与我们刚刚添加到界面上的**TXCloudVideoView**控件进行关联。
```java
...
```
跟直播场景中不同的是，这里的startPlay中的参数为**PLAY_TYPE_VOD_MP4**，即为MP4在线点播。

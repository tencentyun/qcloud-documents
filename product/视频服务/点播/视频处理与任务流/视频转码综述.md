...
云点播内置了一个任务流 `[QCVB_ProcessUGCFile]()` 用于 UGC 短视频鉴黄场景，当用户使用该任务流并指定进行 AI 鉴黄时，鉴黄操作将优先执行，并根据鉴黄结果及模版配置，来决定是否进行后续处理（转码、打水印和截图等）。流程图如下：
...
人工审核并不需要单独触发，开发者只需要在使用任务流 `QCVB_ProcessUGCFile()` 时，选择 AI 鉴黄模版 10，这样当视频涉黄时，任务流就会终止。开发者可以根据[鉴黄完成事件通知]()来查看涉黄内容的截图。如果确实涉黄，开发者可以或者[删除视频](https://cloud.tencent.com/document/product/266/7838)；如果确认不涉黄，那么可以重新发起任务流来进行转码等处理（此时 `QCVB_ProcessUGCFile()` 的 AI 鉴黄模版参数填 0 即可，表示不需鉴黄）。流程图如下：

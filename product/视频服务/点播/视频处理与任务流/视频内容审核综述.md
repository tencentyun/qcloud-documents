...
当您需要进行人工复核时，您可以依据 segment 字段，定位到嫌疑度高的视频片段（segment 字段结构如下表）。

| 字段名 | 类型 | 含义 |
| --- | --- | --- |
| startTimeOffset | Float | 嫌疑片段起始的偏移时间 |
| endTimeOffset | Float | 嫌疑片段结束的偏移时间 |
| confidence | Float | 嫌疑片段的评分（0到100） |
| suggestion | Float | 嫌疑片段风险建议，有 pass，review，block 三种 |
| url | String | 嫌疑图片 URL |
| picUrlExpireTime | String | 嫌疑图片 URL 失效时间 |

...
<dx-tabs>
::: 场景1
自适应码流模板若是预置的10模板，可在播放器签名中设置 pcfg=default。也可以不指定，则为默认读取的配置。通过签名校验工具进行确认：
...
1. 此时务必在播放器签名中设置 pcfg=AdpatInstance（AdpatInstance 即为自定义播放器的配置名称。）
![](https://main.qcloudimg.com/raw/c761d0d2a66a5a20c7ea4521fb7321f7.png)
2. 同时需要在播放代码中设置 psign 参数。例如：
<dx-codeblock>
:::  plaintext

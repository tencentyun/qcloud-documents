伪直播移依托直播录制的能力，将直播录制文件增加“限制观看时间”和“同步观看进度”两种主要能力，使之达成直播效果，有效降低直播风险与直播成本。
...
- 伪直播进行过程中无法大幅度地快进。常见于在线教学视频、直播晚会和广电等行业。
...
## 实践步骤
### 步骤1：上传视频到云点播
在 [云点播控制台](https://console.cloud.tencent.com/vod/media) （非管理员）左侧导航栏，选择【媒资管理】>【视频管理】，您可以单击【上传视频】。
![](https://main.qcloudimg.com/raw/5e3fc022bd8518b80d5b1aee147f5c9c.png)
您可根据业务情况选择合适的方法将视频文件上传到云点播，更多上传方式可参见 [媒体上传综述](https://cloud.tencent.com/document/product/266/9760) 和 [直播录制转点播](https://cloud.tencent.com/document/product/266/50701)。
...
2. 当转码完成后，您可以在控制台 [媒资管理](https://cloud.tencent.com/document/product/266/36451) 查看 HLS 的 URL，或者通过接收 [事件通知](https://cloud.tencent.com/document/product/266/33478#.E7.BB.93.E6.9E.9C.E8.8E.B7.E5.8F.96) 的方式获取 HLS 的 URL。
...
|`plive`|`5e344f00`|表示该场伪直播的开始时间（北京时间），以 unix 时间戳的形式表示。如1577808000表示2020-01-01 00:00:00这个时间点。|
|`exper`|`0`|试看时长|
|`rlimit`|`0`|观看 IP 数限制|
| `us`     | `test`| 生成的随机字符串                                   |
...
## 体验伪直播
在支持 HLS 播放的播放器中（如 Safari 浏览器、VLC、PotPlayer 等等。）访问上述防盗链链接，即可直接体验。
>!Chrome 浏览器默认不支持 HLS，需要安装插件。

## 使用说明
### 防盗链签名生成工具
为了方便开发者调试，我们提供了 [防盗链签名生成工具页面](https://vod-1252229716.file.myqcloud.com/plive/fake_live.html)。按页面内容填写参数后即可查看签名计算的中间结果和最终防盗链链接。

### 使用限制
伪直播本质上是点播，因此并不具备标准直播的一些能力，如：
- 不支持对“一场”伪直播进行数据统计。
- 无法感知“一场”伪直播的开始和结束。
- 不支持对正在进行的伪直播做暂停/终止等操作。
- 不支持对已分发出去的伪直播链接进行禁用。
- 不支持动态改变视频内容（如实时转码、打水印等）。

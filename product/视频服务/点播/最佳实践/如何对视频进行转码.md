...
VOD 后台在视频上传完成和转码任务完成后均会发起 [事件通知](https://cloud.tencent.com/document/product/266/33779) 给开发者预先配置好的事件通知接收服务。开发者可以利用事件通知机制来对新上传的视频发起转码，也可以通过事件通知来自动获取转码结果（上文展示的方法是人工在控制台查看转码结果）。事件通知的使用方法在单独的最佳实践文档 [接收事件通知](https://cloud.tencent.com/document/product/266/45366) 中有详细介绍。
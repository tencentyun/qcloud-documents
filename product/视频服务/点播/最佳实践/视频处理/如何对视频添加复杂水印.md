...
SVG 宽高没有限制，云点播会自动计算出能包含完整元素的最小矩形，并以此作为 SVG 水印原始大小（宽高）。例如，最小矩形如下图虚线包围的部分。

![最小矩形](https://main.qcloudimg.com/raw/6eee7b479365b0aebacbabae4ba862a5.jpg "最小矩形")
>?推荐使用 SVG 调试工具来绘制 SVG，完成之后导出 XML。对于精细化设置（例如元素对齐属性、字体大小等），可手工修改 XML。确认最终效果，得到 XML 代码。

#### SVG 调试方法

1. 您可通过 SVG 调试工具，所见即所得。
2. 将 SVG 保存为后缀 `.html` 文件后使用浏览器打开查看效果。

>?若不同元素间有对齐需求，建议灵活运用各元素的对齐类属性，通过改变元素内容验证是否有效（例如图文混排，增减文本长度，查看图片和文字是否始终满足对齐的需求）。具体可以参考本文下面的示例。
...
- 文字向上和向右对齐品牌 Logo；
- 文字字体为黑体，颜色白色，带有高斯模糊阴影，阴影颜色为黑色。品牌 Logo 不缩放时文字大小为 50 像素。
...
将上面这段代码保存至后缀为 `.html` 的文件，在浏览器打开可看到水印效果图：
![水印效果](https://main.qcloudimg.com/raw/7b139cbc39463ef83db0a49e4800a8e6.png "水印效果")

##### SVG 说明

`<svg>`：定义 SVG 画布。

- 属性 `width="1000px" height="1000px"`：宽高均为 1000px，这里只要能够包含 SVG 里完整元素即可。

`<filter id="filter">`：定义要用到的滤镜。
`<feGaussianBlur>`：高斯模糊滤镜。

- 属性 `stdDeviation="2"`：模糊操作的标准差为 2。

`<feOffset>`：滤镜元素偏移。

- 属性 `dx="0" dy="3"`：设置滤镜元素「向下」偏移 3 个像素。

`<image id="img_watermark">`：品牌 Logo 图片。

- 属性 `xlink:href="data:image/png;base64,{图片数据的 base64}"`：引用本地图片。
- 属性 `height="185px" width="256px"`：原始图片的宽高。
- 属性 `x="74.4%" y="0px"`：根据需求，图片在上面，所以 y 轴偏移设置为 0px，通过不断 ***调整*** `x`值确认效果，最终确定`x="74.4%"`符合预期。

`<text id="text_watermark_shadow">`：实现文字阴影效果。

- 属性 `font-family="SimHei" font-style="Regular" font-size="50px"`：设置字符的字体及大小。
- 属性 `text-anchor="end"`：设置字符的对齐方式，文本字符串的末尾即当前文本的初始位置。
- 属性 `x="100%"`：设置元素沿 x 轴偏移至「最右侧」。
通过设置`text-anchor`、`x` 属性，我们将文字的最后一个字符「固定」在 SVG 画布的「最右侧」，方便调整品牌 Logo 的位置（标签`<image id="img_watermark">`的 `x` 属性）。
- 属性 `style="opacity:1;"`：设置透明度为不透明。
- 属性 `fill="black"`：设置填充颜色为黑色。
- 属性 `filter="url(#filter)`：使用 `id` 为 filter 的滤镜。
- 属性 `y="245px"`：设置元素 y 轴偏移大小，由于品牌 Logo 的高度为 185 像素，文字大小为 50 像素（文字大小近似为高度），所以 `y`设置为不小于 `185+50=235`，通过 ***调整*** ，我们认为 245 效果合适。

`<text id="text_watermark">`：文字。
与 `<text id="text_watermark_shadow">` 相比，大部分属性一样。区别在于没有使用滤镜（无 `filter` 属性），且文字为白色（属性 `fill="white"`）。

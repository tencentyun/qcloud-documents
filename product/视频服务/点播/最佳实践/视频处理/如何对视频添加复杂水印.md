...
>?若不同元素间有对齐需求，建议灵活运用各元素的对齐类属性，通过改变元素内容验证是否有效（例如图文混排，增减文本长度，查看图片和文字是否始终满足对齐的需求）。完整教程参考[SVG 教程](https://developer.mozilla.org/zh-CN/docs/Web/SVG/Tutorial)。具体案例可参见本文下面的 [示例](#example)。
...
验证效果：
- 方式1：将上面这段代码复制到 [在线 HTML 所见即所得编辑工具](https://www.w3schools.com/graphics/tryit.asp?filename=trysvg_myfirst) 页面左侧的文本输入框，显示实时效果在页面右侧展示，如下：

...
[](id:editor)
>?
>- 网上提供很多基于 Web 且免费的在线调试工具，您可通过此类工具绘制满意的 SVG，完成后即可导出 XML。
>- 对于精细化设置（例如元素对齐属性、字体大小等），可手工修改 XML。确认最终效果，得到 XML 代码。

## 操作步骤
### 步骤1：调试 SVG 水印效果
1. 通过 [SVG 调试工具](#editor)，调试满意的图形，操作过程所见即所得。将 SVG 保存为后缀 `.html` 文件后使用浏览器打开查看效果。

>?若不同元素间有对齐需求，建议灵活运用各元素的对齐类属性，通过改变元素内容验证是否有效（例如图文混排，增减文本长度，查看图片和文字是否始终满足对齐的需求）。具体介绍可参见本文下面的 [示例](#example)。
...
## 示例 品牌 Logo+用户 ID 水印[](id:example)

以下将通过具体的案例如何添加复杂水印，给出详细的操作说明。

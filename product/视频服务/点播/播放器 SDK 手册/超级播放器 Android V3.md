...
Android 超级播放器 SDK 是一款用于播放云点播视频的播放器组件，简单几行代码就能实现类似腾讯视频强大的播放功能。

* 基础功能：横竖屏切换、清晰度选择、手势和小窗等；
* 高级功能：视频缓存、软硬解切换、倍速播放、视频缩略图、DRM 加密播放等。

相比系统播放器，超级播放器支持格式更多，兼容性更好，功能更强大。同时还具备首屏秒开、低延迟的优点。
...
经过以下的步骤，您将以最简单的方式集成 Android 超级播放器 SDK，并实现视频播放。
...
通过 [ProcessMedia](https://cloud.tencent.com/document/product/266/33427) ，对上传的视频发起 [转自适应码流](https://cloud.tencent.com/document/product/266/34071) 任务。 API 参数中的`MediaProcessTask.AdaptiveDynamicStreamingTaskSet.Definition`建议填10，表示转 HLS 格式的自适应码流。
...
数字版权管理（Digital Rights Management，DRM），是通过技术手段对内容加密，保护版权内容的安全，适用于音乐、电影等带版权的多媒体内容。云点播提供了商业级 DRM 加密，详情请参考 [如何对内容做版权保护](https://cloud.tencent.com/document/product/266/34105#.E5.95.86.E4.B8.9A.E7.BA.A7-drm) 方案文档。

Android 超级播放器，可以播放 [方案](https://cloud.tencent.com/document/product/266/34105#.E5.95.86.E4.B8.9A.E7.BA.A7-drm) 中的两种方式加密的输出：

* 基于 Widevine 加密的 Dash 格式；
* 基于 SimpleAES 加密的 HLS 格式。

云点播提供了基于 Web 超级播放器的 [DRM 入门教程](https://cloud.tencent.com/document/product/266/34690)，**强烈建议**您先通过该教程的学习。

...
<dx-tabs>
::: 返回窗口模式[](id:window)
返回按钮可返回窗口播放模式，单击后 SDK 处理完全屏切换的逻辑后会触发的代理方法为：
...
在这里拿到 SPDefaultControlView 对象，播放器 view 初始化的时候去给 SPDefaultControlView 的弹幕按钮设置事件，弹幕内容和弹幕 view 需要用户自己自定义，详细参见 SuperPlayerDemo 下的CFDanmakuView、CFDanmakuInfo、CFDanmaku。
...
* 当超级播放器设置为手动播放模式`PLAY_ACTION_MANUAL_PLAY`时，需用户单击播放按钮后视频才开始播放。在单击播放按钮前将展示封面；在单击播放按钮后到视频首帧加载出来前也将展示封面。

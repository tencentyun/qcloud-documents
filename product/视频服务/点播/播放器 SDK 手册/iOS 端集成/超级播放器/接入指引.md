...
若超级播放器组件满足不了您的业务的个性化需求，且您具有一定的开发经验，可以集成 [播放器 SDK](https://cloud.tencent.com/document/product/881/20215)，自定义开发播放器界面和播放功能。
...
3. 下载 SDK + Demo 开发包，腾讯云视立方 iOS 超级播放器项目为 [SuperPlayer_iOS](https://github.com/tencentyun/SuperPlayer_iOS)。

4. 导入 `TXLiteAVSDK_Player.framework` 到工程中，并勾选 `Do Not Embed`。
   :::
...
超级播放器支持全屏播放，在全屏播放场景内，同时支持锁屏、手势控制音量和亮度、弹幕、截屏、清晰度切换等功能设置。功能效果可在【[腾讯云视立方 App](下面下载的二维码)>播放器>超级播放器】中体验，点击界面右下角按钮即可进入全屏播放界面。

<img src="切换全屏ios.png" style="zoom:25%;" />
...
<img src="全屏.png" style="zoom:25%;" />

1. 返回窗口模式

返回按钮可返回窗口播放模式，点击后sdk处理完全屏切换的逻辑后会触发的代理方法为
...
锁屏操作可以让用户进入沉浸式播放状态。点击后由sdk自己处理，无回调。
...
在这里拿到SPDefaultControlView对象，播放器view初始化的时候去给SPDefaultControlView的弹幕按钮设置事件，弹幕内容和弹幕view需要用户自己自定义，详细参考SuperPlayerDemo下的CFDanmakuView、CFDanmakuInfo、CFDanmaku。
...
超级播放器提供播放过程中截取当前视频帧功能，您可以把图片保存起来进行分享。点击截屏按钮后，由sdk内部处理，无截屏成功失败的回调，截取到的图片目录为手机相册。

5. 清晰度切换

用户可以根据需求选择不同的视频播放清晰度，如高清、标清或超清等。点击后触发的显示清晰度view以及点击清晰度选项均由sdk内部处理，无回调。

### 2、悬浮窗播放

超级播放器支持悬浮窗小窗口播放，可在切换到其它应用时，不打断视频播放功能。功能效果可在【[腾讯云视立方 App](下面下载的二维码)>播放器>超级播放器】中体验，点击界面左上角返回按钮，即可体验悬浮窗播放功能。

<img src="切换浮窗3ios.png" style="zoom:25%;" />



```objective-c
// 如果在竖屏且正在播放的情况下点击返回按钮会触发接口
...
超级播放器支持用户自定义视频封面，用于在视频接收到首帧画面播放回调前展示。功能效果可在【[腾讯云视立方 App](下面下载的二维码)>播放器>超级播放器>自定义封面演示】视频中体验。

<img src="自定义封面.png" style="zoom:15%;" />

* 当超级播放器设置为自动播放模式`PLAY_ACTION_AUTO_PLAY`时，视频自动播放，此时将在视频首帧加载出来之前展示封面；
* 当超级播放器设置为手动播放模式`PLAY_ACTION_MANUAL_PLAY`时，需用户点击播放按钮后视频才开始播放。在点击播放按钮前将展示封面；在点击播放按钮后到视频首帧加载出来前也将展示封面。

视频封面支持使用网络url地址或本地file地址，使用方式可参见下述指引。若您通过FileID的方式播放视频，则可直接在云点播内配置视频封面。
...
功能效果可在【[腾讯云视立方 App](下面下载的二维码)>播放器>超级播放器>视频列表轮播演示】视频中体验。

<img src="lunbo1.png" style="zoom:25%;" />

```objective-c
    //步骤1:构建轮播数据的NSMutableArray
...
超级播放器支持视频试看功能，可以适用于非VIP试看等场景，开发者可以传入不同的参数来控制视频试看时长、提示信息、试看结束界面等。功能效果可在【[腾讯云视立方 App](下面下载的二维码)>播放器>超级播放器>试看功能演示】视频中体验。

![shikan1.png](shikan1.png)

![shikan2.png](shikan2.png)
...
超级播放器支持在播放界面添加不规则跑动的文字水印，有效防盗录。全屏播放模式和窗口播放模式均可展示水印，开发者可修改水印文本、文字大小、颜色。功能效果可在【[腾讯云视立方 App](下面下载的二维码)>播放器>超级播放器>动态水印演示】视频中体验。

![shuiyin.png](shuiyin.png)
...
更多完整功能可直接运行工程 Demo，或扫码下载移动端Demo腾讯云视立方 App体验。

...
> 注意：
> - 任务流状态变更通知模式由发起任务流时的参数notifyMode决定。Finish：只有当任务流全部执行完毕时，才发起一次事件通知；Change：只要任务流中每个子任务的状态发生变化，都进行事件通知。 默认为Finish。。
...
| input.disableHigherBitrate | Integer | 是否禁止从较低码率转为较高码率。0：不禁止；1：禁止。默认不禁止。 |
...
| output.totalCount | Integer | 所有雪碧图的小图总和 |

...
| data.metaData | Object | 视频元信息，该字段一定存在，字段信息参见[metaData（频元信息）]  |
| data.drm | Object | 文件加密信息，用户在发起任务流时若指定了加密，该字段才存在。 字段信息参见[drm（视频加密信息）]  |
...
任务列表，目前有Trancode（转码任务），SampleSnapshot（采样截图任务），SampleSnapshot（雪碧图截图任务）。

视频内容识别，即借助 AI 对视频内容进行审核，是一种离线任务。使用视频内容识别，可以识别出视频画面中的文字、片头片尾以及语音中的文字。根据视频内容识别的结果，您可以准确高效地管理视频。视频内容识别包括以下功能：
...
* 使用的素材库：对于审核，使用公共库还是自定义库。
* 指定过滤分数：审核的置信度达到多少分的结果才返回。
* 指定过滤标签：审核的标签在什么范围内的结果才返回。

针对常见的操作组合，云点播提供了 [预置视频内容识别模板](https://cloud.tencent.com/document/product/266/33476#.E9.A2.84.E7.BD.AE.E8.A7.86.E9.A2.91.E5.86.85.E5.AE.B9.E6.99.BA.E8.83.BD.E8.AF.86.E5.88.AB.E6.A8.A1.E6.9D.BF)。另外，您还可以调用 [服务端 API](https://cloud.tencent.com/document/product/266/34791) 创建和管理自定义视频内容识别模板。
...
`Type`为`FaceRecognition`的结果显示，`Output.ResultSet`中包含了两个识别出的人物，分别是`张三`和`李四`。`SegmentSet`表示审核出现在视频中的时间段（由`StartTimeOffset`和`EndTimeOffset`确定）和在画面中的坐标（由`AreaCoordSet`确定）。

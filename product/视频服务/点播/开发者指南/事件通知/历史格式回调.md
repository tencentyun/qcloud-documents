云点播在 2019 年对事件通知的格式进行了改版：
* 改版后新注册的用户的回调格式为 3.0 版本（现行格式）；
* 改版前注册的部分用户可能仍然在使用 2.0 版本（历史格式）。

本文的目的是提供 2.0 格式和 3.0 格式回调的对照。

阅读本文前，在控制台的 [回调配置](/document/product/266/14058#.E5.9B.9E.E8.B0.83.E9.85.8D.E7.BD.AE) 中，如果：
* 仅可见【回调 URL】，说明您的普通回调默认即是 3.0 格式的，无需阅读本文；
* 同时可见【2.0 回调 URL】和【3.0 回调 URL】，说明您是正在使用 2.0 格式普通回调的用户，请继续阅读后面的内容。

如果您仍在使用 2.0 格式的普通回调，建议您将回调版本逐步迁移到 3.0 格式，2.0 格式回调的文档将不再维护。

3.0 格式和 2.0 格式的回调对照表如表所示：

| 事件通知 | 3.0 格式 | 2.0 格式 |
| -- | -- | -- |
| 视频上传完成 | [链接](【超链接到“视频上传完成”一文】) | [链接](【超链接到“2.0格式回调列表”的“视频上传完成”】) | 
| 拉取上传完成 | [链接](【超链接到“拉取上传完成”一文】) | [链接](【超链接到“2.0格式回调列表”的“拉取上传完成”】) | 
| 视频删除完成 | [链接](【超链接到“视频删除完成”一文】) | [链接](【超链接到“2.0格式回调列表”的“视频删除完成”】) |  
| 任务流状态变更 | [链接](【超链接到“任务流状态变更”一文】) | [链接](【超链接到“2.0格式回调列表”的“任务流状态变更”】) | 
| 视频编辑完成 | [链接](【超链接到“视频编辑完成”一文】) | / | 
| 微信发布完成 | [链接](【超链接到“微信发布完成”一文】) | / | 
| 视频转码完成 | [链接](【超链接到“视频转码完成”一文】) | [链接](【超链接到“2.0格式回调列表”的“视频转码完成”】) | 
| 指定时间点截图完成 | [链接](【超链接到“指定时间点截图完成”一文】) | [链接](【超链接到“2.0格式回调列表”的“指定时间点截图完成”】) | 
| 视频截取雪碧图完成 | [链接](【超链接到“视频截取雪碧图完成”一文】) | [链接](【超链接到“2.0格式回调列表”的“视频截取雪碧图完成”】) | 
| 视频剪辑完成 | [链接](【超链接到“视频剪辑完成”一文】) | [链接](【超链接到“2.0格式回调列表”的“视频剪辑完成”】) | 
| 视频拼接完成 | [链接](【超链接到“视频拼接完成”一文】) | [链接](【超链接到“2.0格式回调列表”的“视频拼接完成”】) | 
...
| status | String | 任务状态，有PROCESSING，SUCCESS，FAIL 三种。 |
...
| output.suggestion | String | 鉴黄结果建议，有 pass，review，block 三种。 |
...
| status | String | 任务状态，有 PROCESSING，SUCCESS，FAIL三种。 |
| errCode | Integer | 错误码。 0: 成功, 其他值: 失败,其中30009为原文件异常失败；30010为系统失败或未知。 |
| message | String | 错误信息。 |
| input | Object | 任务的输入信息。 |
...
| status | String | 任务状态，有 PROCESSING，SUCCESS，FAIL 三种。 |
| errCode | Integer | 错误码。 0: 成功, 其他值: 失败,其中30009为原文件异常失败；30010为系统失败或未知。 |
| message | String | 错误信息。 |
| input | Object | 任务的输入信息。 |
...
| output.container | String | 容器类型，例如 m4a，mp4 等。 |
...
| -- | -- | -- |
| version | String | 事件通知版本号，固定为 "4.0"。 |
| eventType | String | 事件类型，固定为 TranscodeComplete。 |
| data | Object | 具体回调数据。 |
| data.status | Integer | 错误码, 0: 成功, 其他值: 失败。 |
| data.message | String | 错误信息。  |
...
data.picInfo 数组中每个元素均为 Object，每个参数含义如下：
...
| data.definition | Integer | 雪碧图规格，参见[雪碧图截图模板](/document/product/266/11702#.E9.9B.AA.E7.A2.A7.E5.9B.BE.E6.88.AA.E5.9B.BE.E6.A8.A1.E6.9D.BF)。 |
...
data.fileInfo 数组中每个元素均为 Object，每个参数含义如下：

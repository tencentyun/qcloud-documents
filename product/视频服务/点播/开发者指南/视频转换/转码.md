转码是将视频码流转换成另一个视频码流的过程，是一种离线任务。通过转码，可以改变原始码流的编码格式、分辨率、码率等参数，适应不同终端和网络环境的播放。使用转码功能，可以实现：
...
* 调用服务端 API [ProcessMedia](https://cloud.tencent.com/document/product/266/33427) 发起任务：在请求中的`MediaProcessTask.TranscodeTaskSet`参数指定 [转码模板](《链接到“转码模板”》) 的模板 ID；
* 调用服务端 API [ProcessMediaByUrl](https://cloud.tencent.com/document/product/266/33426) 发起任务：在请求中的`MediaProcessTask.TranscodeTaskSet`参数指定 [转码模板](《链接到“转码模板”》) 的模板 ID；

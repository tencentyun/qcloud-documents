...
4. **如何实现类似暂停上传/恢复上传 的功能呢？ **
文档中已经提到，sdk底层已经自动实现了断点续传的功能，因此暂停的本质即是调用uploader.cancle()这个方法；同理，暂停之后的恢复上传也是调用初始的tcVod.upload方法，唯一区别在于恢复上传时调用该方法的参数，应该是之前缓存起来的参数（例如可以在启动上传时全局变量存储一份参数，上传完成后再清掉）

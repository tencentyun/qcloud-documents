...
3. **将明文串转为最终签名**（以部分 Java 代码为例）
   ...
   mac.init(secretKey);
	...
	2. 将明文串 original 使用utf-8编码成字节数组，然后把 signatureTmp 与该数组进行合并，最后把合并后的结果进行 [Base64](https://tools.ietf.org/html/rfc4648) 编码，得到最终签名 signature：
```java
String signature = base64Encode(byteMerger(signatureTmp, original.getBytes("utf8")));
```
>?**byteMerger 和 base64Encode 分别是数组合并和 Base64 编码的方法，详细请参见 [Java 示例代码](https://cloud.tencent.com/document/product/266/10638#java-.E7.AD.BE.E5.90.8D.E7.A4.BA.E4.BE.8B)**。

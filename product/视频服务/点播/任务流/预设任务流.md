...
## QCVB_CommonProcessFile

简单的ProcessFile，支持转码，水印，设置封面，采样截图。
QCVB_SimpleProcessFile(transcodeDefinition, watermarkDefinition, coverBySnapshotDefinition, sampleSnapshotDefinition, aiReviewDefinition,manualReviewDefinition)
...
| coverBySnapshotDefinition | 是 |Integer | 使用截图设置视频封面模板号，参见[截图参数模板](/document/product/266/8097)。 |
| sampleSnapshotDefinition | 是 |Integer | 采样截图模板号，参见[采样截图参数模板](/document/product/266/9050) |
| aiReviewDefinition | 是 |Integer | 暂时只有默认模板10 |
| manualReviewDefinition | 是 |Integer | 暂时只有默认模板10 |
...
### 使用控制台配置的规格转码示例
```
QCVB_CommonProcessFile(1, 1, 10, 10)
```
以下示例的含义是：
1. 对视频进行转码，转码模板为控制台配置的模板；
1. 转码过程需要设置水印，水印模板号为1，即默认模板；
...
### 指定视频规格转码示例
```
QCVB_CommonProcessFile({210,220}, 10, 10, 10)
```
以下示例的含义是：
1. 对视频文件进行转码，目标输出模板为210，220；
1. 转码过程需要设置水印，水印模板号为10；
...
### 指定视频鉴黄并指定规格转码示例
```
QCVB_CommonProcessFile({210,220}, 10, 10, 10, 10, 10)
```
以下示例的含义是：
1. 对视频进行Ai鉴黄，采样截图的模板号为10；
1. 对视频进行人工鉴黄，采样截图的模板号为10，若用户最终判定涉黄，后续操作不会执行；
1. 对视频文件进行转码，目标输出模板为210，220；
1. 转码过程需要设置水印，水印模板号为10；
1. 对视频设置封面，封面图片为视频首帧，截图模板号为10；
1. 对视频进行采样截图，采样截图的模板号为10；

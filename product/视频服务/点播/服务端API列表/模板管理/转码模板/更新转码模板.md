...
| video.codec | 否 | Integer | 视频流的编码格式，可填 libx264（H264编码），libx265（H265编码） |
| video.fps | 否 | Integer | 帧率，单位：hz |
| video.resolutionSelfAdapting| 否 | Integer | 分辨率开启自适应：1为开启，0为关闭。 则width的值用于较长边，height的值用于较短边 |
| video.width | 否 | Integer | 视频流宽度的最大值，若填0，当视频的高度非0，则按比例缩放，当视频的高度也为0，则表示同源。单位：px |
| video.height | 否 | Integer | 视频流高度的最大值。单位：px |

...
| **参数名称** | **必填** | **类型** | **描述** |
|---------|---------|---------|---------|
| definition | 是 | Integer | 采样截图模板号，参见TODO |
...
| dataKey | 否 | Array | 如果秘钥来源为UserDefined，则通过该参数传入加密秘钥，类型为String |
| encyptedDataKey | 否 | Array | 如果秘钥来源为UserDefined，则通过该参数传入加密秘钥，类型为String | 
| getKeyUrl | 是 | String | 播放器获取秘钥的URL |

注意：如果使用自定义秘钥，dataKey和encyptedDataKey必须一一对应。

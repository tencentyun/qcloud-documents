...
| **参数名称** | **必填** | **类型** | **描述** |
|---------|---------|---------|---------|
| samplingType | 是 | String | 采样类型。Time：依照时间间隔进行采样；Percent：依照总时长百分比进行采样 |
...
| **参数名称** | **必填** | **类型** | **描述** |
|---------|---------|---------|---------|
| encryptType | 否 | String | 加密强度。SingleKey：整个视频文件单秘钥；MultiKey：整个视频文件多秘钥。 |
| keyType | 是 | String | 秘钥类型，备选参数：UserDefined，BuildInKMS，QCloudKMS |
...
&screenshot.samplingType=Time
&screenshot.definition=1
&screenshot.interval=10
&imageSprite.definition=10
&drm.encryptType=SingleKey
&drm.keyType=BuildInKMS

...
3. 目标文件与源文件使用相同的ts分片；
4. 源文件删除会导致目标文件不可播放，但目标文件已经转码成功的文件仍可继续播放；
...
| startTimeOffset | 是 | Integer| 目标文件开始相对源文件的时间偏移，单位(秒)，大于0表示从头部开始计算，小于0表示从尾部开始计算 |
| endTimeOffset | 是 | Integer| 目标文件结束相对源文件的时间偏移，单位(秒)，大于0表示从头部开始计算，小于0表示从尾部开始计算 |

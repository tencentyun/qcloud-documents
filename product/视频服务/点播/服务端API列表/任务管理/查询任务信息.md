...
注意：仅支持查询三天（72小时）的任务的信息，查询更早的任务将返回 NotFound。
...
| message | String | 错误信息 |
| status | String | 任务状态，取值：WAITING/PROCESSING/FINISHED |
| type | String | 任务类型，目前有 procedure（任务流），transcode（转码），pull（转拉），clip（剪辑） |
| createTime | Integer | 任务创建时间（Unix时间戳） |
| beginProcessTime | Integer | 任务开始执行的时间（Unix时间戳） |
| finishTime | Integer | 任务信息最近更新时间（Unix时间戳） |
...
当type为pull，data内容与[事件通知-URL拉取视频上传完成](/document/product/266/7831)回调包体中data字段相同

当type为clip，data内容与[事件通知-URL拉取视频上传完成](/document/product/266/10157)回调包体中data字段相同

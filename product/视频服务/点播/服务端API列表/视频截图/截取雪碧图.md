...
## 功能说明
1. 对视频文件进行截图，返回雪碧图；
2. 视频截图结束之后，如果APP配置了服务端事件通知，则会产生截图完成事件通知。

## 事件通知
文件截图成功可触发[事件通知-视频按时间点截图完成](/document/product/266/7832)。APP后台可据此监听点播系统中的视频截图行为。
...
| row | 是 | Integer | 指定需要截图的时间点 |
| colunm | 是 | Integer | 指定需要截图的时间点 |
| screenshotInterval | 是 | Integer | 指定需要截图的时间点 |
| fileId | 是 | String | 指定需要截图的时间点 |

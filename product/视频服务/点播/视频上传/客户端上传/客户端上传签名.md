...
生成签名需要APP服务器确定4个必选参数，用户构造签名的明文串，参数如下：

| 参数名称 | 类型 | 说明 |
| --- | --- | --- | --- |
| secretId | String | 构造签名明文串的参数，[云 API 密钥](https://console.qcloud.com/capi)中的 Secret ID。 |
| currentTimeStamp  | Integer | 构造签名明文串的参数，当前 Unix 时间。 |
| expireTime | Integer| 构造签名明文串的参数，签名到期 Unix 时间。<br/>```expireTime = currentTimeStamp + 签名有效时长```<br/>签名有效时长最大取值为7776000，即90天。 |
...
* 包含 secretId， currentTimeStamp，expireTime 和 random 四个参数

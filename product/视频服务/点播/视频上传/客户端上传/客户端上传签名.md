...
按照 URL QueryString 的格式要求生成签名明文串 Original<sup>注意 1</sup>，格式如下：

```secretId=[secretId]&currentTimeStamp=[currentTimeStamp]&expireTime=[expireTime]&random=[random]```

### 第三步：将明文串转为最终签名

生成签名明文串 Original 后，用已获取的 secretKey 对明文串进行 [HMAC-SHA1](https://www.ietf.org/rfc/rfc2104.txt) 加密，得到 SignatureTmp<sup>注意 2</sup>：

...
SDK 自动支持断点续传功能，无需做额外操作。当上传意外终止时，用户再次上传该文件，可以从中断处继续上传，减少重复上传时间。断点续传的有效时间是 1 天，即同一个视频上传被中断，那么 1 天内再次上传可以直接从断点处上传，超过 1 天则默认会重新上传完整视频。
...
| error        | 上传失败回调 | Object   | 回调的 result 对象中：<br  />type 字段表示上传失败的文件种类；<br  />video 字段表示视频或者 cover 字段表示封面  |
...
2. 一个 10MB 文件，上传了 50%，断点续传是从 20% 开始吗？
上传是按分片上传的，断点续传是以已经上传完成的分片为准。比如说正在上传 3 个分片，已完成 2 个分片，那么断点续传的时候就是以完成 2 个分片的状态为准的。

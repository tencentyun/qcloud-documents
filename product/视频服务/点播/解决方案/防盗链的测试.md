# 防盗链的测试
## 为什么需要防盗链测试
云点播提供了[防盗链](/document/product/266/11243)功能，开发者可以根据实际需要，对视频播放 URL 使用的域名合理设置防盗链，实现对用户视频播放行为的控制。
...
例如，开发者希望对视频播放 URL 的有效期进行控制，就需要启用 [KEY 防盗链](/document/product/266/14047)：
...
* **点播默认域名**：现网环境中，用户播放点播视频使用的正式域名。“预置点播域名”和“自定义域名”都能被设置成“点播默认域名”（设置方法参见[这里](/document/product/266/14056#.E8.87.AA.E5.AE.9A.E4.B9.89.E5.9F.9F.E5.90.8D)）。
...
![](https://main.qcloudimg.com/raw/839532397701b5107f70d4c8402494c5.jpeg)
...
![](https://main.qcloudimg.com/raw/4ab4d998fcb5fab8510477a9c4eb015e.jpeg)

为了避免防盗链测试影响现网用户，点播提供了一个“预置点播测试域名”，与现网中使用的点播默认域名隔离。开发者测试防盗链时，仅操作测试域名的防盗链配置。

点播还提供了一个“测试域名代理”（IP 为 122.152.250.73），开发者需要修改测试客户端的 HOST 表，将点播默认域名解析到这个代理上。这样，测试客户端的视频播放请求，将经过代理转发到测试域名（上图中的红色路径），而现网用户的播放请求仍然通过正式域名获取视频内容（上图中的黑色路径）。

因此，开发者可以自由修改测试域名的防盗链配置，以及测试 APP 后台派发的视频播放 URL，而不必担心影响到现网用户。

![](https://main.qcloudimg.com/raw/82225f7a8d0a9ad62b83b6b848666c13.jpeg)
...
用户（例中 appid 为125xxx655）登录点播控制体台的[【域名管理】](https://console.cloud.tencent.com/video/domainsetting)后，将看到以下两种域名：
...
选择预置点播测试域名（125xxx655-test.vod2.myqcloud.com），点击【设置】链接，进入【Key 防盗链】，打开【启用 Key 防盗链】，并使用【生成 KEY】生成一个防盗链 KEY。点击【确定】保存，等待配置生效。

**2. 获取一个原始播放 URL**

![](https://main.qcloudimg.com/raw/24b6096fcd35611e17c28891abb8286e.png)

视频的原始播放 URL，是指没有带[防盗链参数](/document/product/266/14047#.E9.98.B2.E7.9B.97.E9.93.BE.E5.8F.82.E6.95.B0)的 URL 地址，可以从[云视频管理](https://console.cloud.tencent.com/video/videolist)获得（如上图所示）。例子中使用的 URL 为 
...
修改测试终端的 HOST 表（Windows 为 C:\Windows\System32\drivers\etc\hosts，Mac 为 /private/etc/hosts），添加一条记录```122.152.250.73 125xxx655.vod2.myqcloud.com```，然后保存。
...
按照 KEY 防盗链的[生成规则](/document/product/266/14047#.E9.98.B2.E7.9B.97.E9.93.BE-url-.E7.94.9F.E6.88.90.E6.96.B9.E5.BC.8F)生成带有防盗链参数的 URL，地址为 https://125xxx655.vod2.myqcloud.com/ca7xxx655/cfbxxx349/PkxxxIA.mov?t=5bd6be00&sign=18cxxx9deb ，就能成功播放视频了，执行```curl```返回的 HTTP 状态码为 200。
...
然后打开点播默认域名的【Key 防盗链】，把测试域名的 KEY 粘贴在【防盗链 KEY】文本框中，点击【确定】保存。

进入 [云点播控制台]( https://console.cloud.tencent.com/vod/overview)，选择左侧导航栏的【视频处理设置】>【模板设置】。模板设置内置了五种类型的模板，分别为**视频转码模板**、**音频转码模板**、**水印模板**、**截图模板**、**转动图模板**和**审核模板**。每种模板都可以添加至任务流设置中，用于视频处理设置。 //更新
# 需要新图
...
+ 模板名称：仅只支持中文、英文、数字、空格、下划线(_)、短横线(-)和句点(.)七种格式，长度不能超过64个字符。
+ 审核项： 可选审核项为三大类，图像识别、语音识别、文字识别。选择后的项会出现在右侧已选择中。
    a. 图像识别包含涉黄、涉暴、涉政三个子项。
        i) 涉黄：包含对色情、低俗、情谜行为、性感的子项审核
        ii) 涉暴：包含对武装分子、武器枪支、血腥画面、爆炸火灾、暴恐旗帜、暴恐人物、警察部队、人群聚集的子项审核
        iii)涉政：包含对政治任务、违规图标、娱乐明星、体育明星的子项审核
    b. 语音识别包含涉黄、涉政两个子项。
    c. 文字识别包含涉黄、涉政两个子项。
  每一个审核项都可以配置置信度默认阈值和置信度疑似阈值，用来调节审核的力度。如不填写，则按照腾讯云点播的默认值进行填写和审核。
+ 置信度确认阈值：腾讯云点播对用户上传的审核视频经过计算后会得出一个审核分数，当分数超过用户给出的值，则该项审核状态为确认，如无特殊需求，建议用户使用默认值。
+ 置信度疑似阈值：腾讯云点播对用户上传的审核视频经过计算后会得出一个审核分数，当分数超过用户给出的值，则该项审核状态为疑似，用户可以在视频审核页面发起人工复审，如无特殊需求，建议用户使用默认值。
+ 抽帧间隔： 表示审核视频每间隔几秒进行抽则帧检查，默认值为1，最小值为0.5，抽帧间隔单位为秒，默认值为1，最小值为0.5，

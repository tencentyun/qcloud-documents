...
您可以在**Web播放器管理**页面 [新增播放器](点击跳转到下面新增播放器部分内容)，或者对已有播放器进行如下操作：
...
- 贴片：您可以选择在播放器播放文件前、暂停时和播放结束后，对视频进行自定义贴片，且贴片内容支持跳转。贴片内容支持 gif/jpg/png/bmp 格式（gif 文件按第一帧处理），大小1MB以内即将支持视频贴片。
- 广告（敬请期待）。

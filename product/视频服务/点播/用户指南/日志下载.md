...
腾讯云点播支持下载接入域名访问情况的详细信息，为您提供以小时为维度的 CDN 日志，您可以下载最近30天内的 CDN 日志。
## 下载 CDN 日志
登录 [点播控制台](https://console.cloud.tencent.com/video/cdnlog)，在左侧菜单栏中选择【日志下载】>【CDN日志】。选择需要获取日志的时间范围和域名，单击【确定】，即可获取日志下载链接。
...
由于通过应用层统计的 TCP 协议丢包、三次握手以及重传等机制所消耗的带宽或流量比传输层小，因此日志中记录的带宽或流量数据均为应用层（HTTP 协议）回包数据。

...
>!
	...
	- 直接操作字符串的方式生成 Original 容易出错，建议您尽量使用标准的类库来构造 Original。
3. **将明文串转为最终签名**
	1. 用已获取的 SecretKey 对明文串 Original 进行 [HMAC-SHA1](https://www.ietf.org/rfc/rfc2104.txt) 加密，得到 SignatureTmp：
	```
	SignatureTmp = HMAC-SHA1(secretKey.getBytes("utf-8"), Original.getBytes("utf-8")) 
	```
	>? **SignatureTmp 是使用utf-8编码、通过 HMAC-SHA1 加密出来的字节数组**。
	2. 将明文串 Original 使用utf-8编码成字节数组，然后把 SignatureTmp 与编码后的 Original 进行拼接，最后把拼接后的结果进行 [Base64](https://tools.ietf.org/html/rfc4648) 编码，得到最终签名 Signature：
```
Signature = Base64(mergeByteArray(SignatureTmp, Original.getBytes("utf-8"))
```

## 签名生成示例
云点播还提供了**签名生成示例代码**和**签名工具**，便于您参考和验证：

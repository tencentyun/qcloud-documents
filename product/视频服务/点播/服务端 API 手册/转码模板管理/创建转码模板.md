...
| fps | 是 | Float | 视频帧率，单位：Hz。当 fps 不为0时，可以取的值有24、25、30等（在[1, 60]区间）。当 fps 为0时，表示帧率和原始视频保持一致。 |
...
| bitrate | 是 | Integer | 视频流的码率，单位：kbps。当 bitrate 不为0时，可以取的值在[128, 35000]区间。当 bitrate 为0时，表示视频码率和原始视频保持一致。 |
...
| sampleRate | 是 | Integer | 音频流的采样率，可填32000、44100和48000三个值，单位：Hz。|
...
> - video 中的 fps、bitrate、width 和 height，以及 audio 中的 bitrate 和 sampleRate，这些个字段可以通过支持填入负数，表示该字段禁止低转高。例如，video.bitrate 赋值为-512的模板表示，若原视频码率低于512k，转码后视频码率将不会提升（即原视频码率为256k，则转码后视频码率仍为256k；如果原视频码率为1024k，则转码后视频码率为512k）。

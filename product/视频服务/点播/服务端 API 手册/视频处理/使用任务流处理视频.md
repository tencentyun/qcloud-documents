...
| 参数名称 | 必填 | 类型 | 说明 |
|---------|---------|---------|---------|
| inputType | 是 | String | 输入视频的方式，可以取的值为 SingleFile，MultiFile，Stream 和 PullFile<ul><li>SingleFile：直接指定要处理的文件 ID</li><li>MultiFile：指定文件列表和剪辑参数生成操作的文件 ID</li><li>Stream：指定流 ID 和剪辑参数生成操作的文件 ID</li><li>PullFile：指定要拉取的视频文件 url</li> |
...
| procedure | 是 | String | 任务流模板，参考[点播预置任务流模板](/document/product/266/11700#.E7.82.B9.E6.92.AD.E9.A2.84.E7.BD.AE.E4.BB.BB.E5.8A.A1.E6.B5.81.E6.A8.A1.E6.9D.BF) |
...
| startTimeOffset | 否 | Integer | 剪辑的文件内起始偏移 |
| endTimeOffset | 否 | Integer | 剪辑的文件内结束偏移 |
...
&ampfile.id=12345
&ampfile.startTimeOffset=10
&ampfile.endTimeOffset=25
...
#### 通过指定剪辑文件 ID 列表请求示例
以下示例的含义是：
1.把 fileId 为12345的文件按视频内区间（30，100）剪辑，fileId 为67899的文件按视频内区间（300，400）剪辑；
...
&ampfileList.0.id=12345
&ampfileList.0.startTimeOffset=30
&ampfileList.0.endTimeOffset=100
&ampfileList.1.id=67899
&ampfileList.1.startTimeOffset=300
&ampfileList.1.endTimeOffset=400
&ampprocedure=SomeProcedure
&ampCOMMON_PARAMS
</pre>
...
#### 通过录制流信息请求示例
以下示例的含义是：
1.把streamId为12345的所有 fileId 按录制时间区间（15020876538，15020876638）剪辑；
2.然后再把剪辑的结果拼接成目标文件，格式是mp4；
3.最后对目标文件发起任务流。
...
&ampstream.startTimeStamp=15020876538
&ampstream.endTimeStamp=1502087638
&ampprocedure=SomeProcedure
&ampCOMMON_PARAMS
</pre>

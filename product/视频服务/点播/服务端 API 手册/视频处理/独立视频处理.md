...
发起任务后，系统将从 API 中指定的输入文件 Bucekt 获取源视频文件，并将处理后的结果写入指定的输出文件 Bucket（Bucket 需要在准备工作中完成[配置操作]()）。
...
| mediaCheck | 否 | Object| 参见[视频鉴黄参数参数](#mediacheck.EF.BC.88.E8.A7.86.E9.A2.91.E9.89.B4.E9.BB.84.E5.8F.82.E6.95.B0.EF.BC.89) |
| mediaProcess| 否 | Object | 参见[视频处理参数](#mediaprocess.EF.BC.88.E8.A7.86.E9.A2.91.E5.A4.84.E7.90.86.E5.8F.82.E6.95.B0.EF.BC.89) |
| taskAttribute | 否 | Object | 参见[任务属性配置](#taskAttribute.EF.BC.88.E4.BB.BB.E5.8A.A1.E5.B1.9E.E6.80.A7.E9.85.8D.E7.BD.AE.E5.8F.82.E6.95.B0.EF.BC.89) |
| COMMON_PARAMS | 是 |  | 参见[公共参数](/document/api/213/6976) |

#### input（输入文件信息参数）
| 参数名称 | 必填 | 类型 | 说明 |
|---------|---------|---------|---------|
| bucket | 是 | String | 输入文件 Bucket（需要先完成[配置操作]()） |
| path | 是 | String | 输入文件路径 |

#### output（输出文件信息参数）
| 参数名称 | 必填 | 类型 | 说明 |
|---------|---------|---------|---------|
| bucket | 是 | String | 输出文件 Bucket（需要先完成[配置操作]()） |
| path | 是 | String | 输出文件目录 |

#### mediaCheck（视频鉴黄参数）
| 参数名称 | 必填 | 类型 | 说明 |
|---------|---------|---------|---------|
| pornCheckDefinition | 否 | Integer| 视频鉴黄模版号 |
...
| hlsMasterPlaylistDefinition | 否 | Array| HLS Master Playlist 的模板列表 |
| hlsMasterPlaylistDefinition.n | 否 | Integer | HLS Master Playlist 的模板号 |
...
| disableHigherBitrate | 否 | Integer | 是否禁止从较低码率转为较高码率。<br/>0：不禁止；1：禁止。<br/>默认为不禁止。|
...
| 参数名称 | 必填 | 类型 | 说明 |
|---------|---------|---------|---------|
| definition| 是 | Integer| 指定时间点截图模版号，参见[指定时间点截图参数模板](/document/product/266/11702#.E6.8C.87.E5.AE.9A.E6.97.B6.E9.97.B4.E7.82.B9.E6.88.AA.E5.9B.BE.E6.A8.A1.E6.9D.BF) |
| positionType | 是 | string| 截图方式，取值为 Time 或者 Percent 两种：<ul><li>Time：依照时间点截图</li><li>Percent：依照百分比截图。</li></ul>|
| position | 是 | Integer| 截图时间点：<ul><li>对于依照时间点截图，该值表示指定视频第几秒的截图作为封面</li><li>对于依照百分比截图，该值表示使用视频百分之多少的截图作为封面</li></ul> |
...
| notifyDefinition| 否 | Integer | 回调模板号 |

### 请求示例
输入文件在名为 `myinputbucket` 的 Bucket 中，绝对路径为 `/input/F62A55F5-C4D6-4AEA-934F-7A6BFF3D8BCF.MOV`

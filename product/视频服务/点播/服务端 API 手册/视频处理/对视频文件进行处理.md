...
| notifyMode | 否 | String | 任务流状态变更通知模式。<ul><li>Finish：只有当任务流全部执行完毕时，才发起一次事件通知；</li><li>Change：只要任务流中每个子任务的状态发生变化，都进行事件通知；</li><li>None：不接受该任务流回调。 默认为Finish。</li></ul> | 
...
&amptranscode.disableHigherBitrate=1

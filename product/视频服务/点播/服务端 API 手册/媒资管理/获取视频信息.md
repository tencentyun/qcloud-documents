...
1. 该接口可以获取单个视频的多种信息，包括：
    ...
    5. 转动图结果信息（animatedGraphicsInfo）：对视频转动图（如 gif）后，动图相关信息；
    6. 采样截图信息（sampleSnapshotInfo）：对视频采样截图后，相关截图信息；
    7. 雪碧图信息（imageSpriteInfo）：对视频截取雪碧图之后，雪碧的相关信息；
    8. 指定时间点截图信息（snapshotByTimeOffsetInfo）：对视频依照指定时间点截图后，各个截图的信息。
    9. 视频打点信息（keyFrameDescInfo）：对视频设置的各个打点信息。
2. 可以指定回包只返回部分信息。
*本接口只支持点播4.0*
...
| infoFilter.n | 否 | String | 指定需要返回的信息，可同时指定多个信息，n 从0开始递增。如果未填写该字段，默认返回所有信息。备选项：basicInfo（基础信息）、元信息（metaData）、加密信息（drm）、transcodeInfo（转码结果信息）、animatedGraphicsInfo（转动图信息）、imageSpriteInfo（雪碧图信息）、snapshotByTimeOffsetInfo（指定时间点截图信息）、sampleSnapshotInfo（采样截图信息）、keyFrameDescInfo(视频打点信息)。 |

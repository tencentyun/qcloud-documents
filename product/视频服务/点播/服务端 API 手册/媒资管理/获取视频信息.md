...
1. 该接口可以获取单个视频的多种信息，包括：
    ...
    4. 内容审核结果信息（contentReviewInfo）：对视频发起内容审核后的审核结果，<!--包括鉴黄结果、暴恐结果等-->目前仅支持鉴黄结果；
...
| segments.suggestion | Float | 嫌疑片段鉴黄结果建议，有 pass，review，block 三种 |
...
| segments.suggestion | Float | 嫌疑片段暴恐结果建议，有 pass，review，block 三种 |
...
| sampleSnapshotInfo.sampleType | String | 采样方式，Percent为根据百分比间隔采样，Time为根据时间间隔采样 |

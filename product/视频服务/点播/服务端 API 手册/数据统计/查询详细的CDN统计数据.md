...
1. 只能查询最近90天的数据。
2. 统计数据可以是每5分钟汇总的统计数据，也可以是每天汇总的统计数据，根据请求的起始日期和结束日期的时间跨度以及请求的间隔时间参数来决定。
...
| interval      | 否   | Integer | 统计数据的间隔时间，单位为分钟。如果 startDate 不等于 endDate 将忽略该参数，按天返回每天汇总的统计数据。如果 startDate 等于 endDate ，interval 可以取的值有<ul><li>5：每5分钟汇总一个统计数据</li><li>30：每30分钟汇总一个统计数据</li><li>60：每小时汇总一个统计数据</li><li>120：每2小时汇总一个统计数据</li></ul> 默认为最小间隔 |
...
| message       | String  | 错误信息                                                                   |
| data          | Object  | 结果数据                                                                   |
| data.interval | Integer | 返回数据的时间间隔，单位为分钟。如 interval=5，表示数据是每5分钟汇总的数据 |

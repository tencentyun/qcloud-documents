## 接口名称
GetTaskInfo
<font color=red>本接口已不再维护，新接口 [DescribeTaskDetail](https://cloud.tencent.com/document/product/266/33431) 为您提供更完备的功能，请尽快迁移。</font>
...
| message | String | 错误信息 |
| type | String | 任务类型，目前有：<ul><li>Procedure（任务流）</li><li>transcode（转码）</li><li>clip_media（剪辑）</li><li>concatFile（拼接）</li><li>snapShotByTime（截图）</li><li>imageSprite（雪碧图）</li> </ul>|
...
- 当type为clip_media，data内容与 [事件通知-剪辑完成](/document/product/266/10157) 回调包体中data字段相同。
- 当type为concatFile，data内容与 [事件通知-拼接完成](/document/product/266/7384) 回调包体中data字段相同。
- 当type为snapShotByTime，data内容与 [事件通知-截图完成](/document/product/266/8105) 回调包体中data字段相同。
- 当type为imageSprite，data内容与 [事件通知-雪碧图完成](/document/product/266/8104) 回调包体中data字段相同。

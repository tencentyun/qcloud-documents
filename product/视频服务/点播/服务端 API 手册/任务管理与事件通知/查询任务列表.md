## 接口名称
GetTaskList
<font color=red>本接口已不再维护，新接口 [DescribeTasks](https://cloud.tencent.com/document/product/266/33430) 为您提供更完备的功能，请尽快迁移。</font>

## 功能说明
1. 查询任务列表；
2. 当列表数据比较多时，单次接口调用无法拉取整个列表，这时可以通过参数 next 分批拉取；
3. 只能查询到三天（72小时）内的任务。
...
| 参数名称 | 必填 | 类型 | 说明 |
|---------|---------|---------|---------|
| fileId | 否 | String | 文件 ID，该字段若填写，则只会查询该文件的任务列表。 |
| status      | 是    | String | 任务状态，取值 WAITING/PROCESSING/FINISH。               |
| maxCount      | 否    | Integer | 本次查询返回多少个数据，范围在10 - 100之间，不填默认为10。                         |
| next        | 否    | String | 分批拉取时使用： 当列表数据比较多时，单次接口调用无法拉取整个列表，这时会返回拉取到的最后一个数据的 ID，下次请求携带该 ID，将会从该 ID 下一个开始拉取。                       |
...
| data | Object | 任务信息数据。 |
| data.next | String | 当列表比较多时，单次接口调用无法拉取全部列表，这时会返回拉取到的最后一个数据的 ID，下次请求携带该 ID，将会从该 ID 下一个开始拉取。 如果为空表示已经拉完最后一个数据。 |

...
```
SDK 内置了摄像头拍摄的实现，如果您想快速地实现Web端的拍摄，可以直接使用`camera`参数配置。
注意：`config`中的`camera`参数表示将由 SDK 来采集当前设备的摄像头流作为输入，
当设置了`camera`参数，SDK 还提供一系列基础的摄像头设备管理方法。

如果您无需管理摄像头，也可传入其他视频或`canvas`，则可以设置`config`中的`input`参数，
`input`参数支持传入媒体流`MediaStream`与图片`HTMLImageElement|String(图片url)`。
可以将加载完成的`video`或绘制完成的`canvas`转成`MediaStream`作为input。

```javascript
// 以下为设置input类型为stream的示例
...
SDK输入为camera类型时，其挂载的camera对象支持以下方法：
<table>
...
<td>switchDevice</td>
<td>切换设备</td>
<td>type:string, deviceId:string</td>
<td>
Promise
</td>
</tr>
...
> - 其他场景（包括input是camera时）均返回 `MediaStream` 类型，其中的 `video` 轨道是 ArSdk 实时处理的画面轨，如果媒体流有音频则 `audio` 轨道保持不变。
...
```
注意：initLocalPlayer获取到的播放器默认静音，如果开启静音则可能产生回声；

SDK initLocalPlayer后，其player对象支持以下方法：
<table>
<thead><tr><th>方法名</th><th>说明</th><th>入参</th><th>返回值</th></tr></thead>
<tbody>
<tr>

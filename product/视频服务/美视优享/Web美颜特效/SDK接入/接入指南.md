...
```
SDK 内置了摄像头拍摄的实现，如果您想快速地实现 Web 端的拍摄，可以直接使用 `camera` 参数配置。
>! `config`中的 `camera` 参数表示将由 SDK 来采集当前设备的摄像头流作为输入，当设置了 `camera` 参数，SDK 还提供一系列基础的摄像头设备管理方法。

如果您无需管理摄像头，也可传入其他视频或 canvas，则可以设置 config 中的 input 参数，input 参数支持传入媒体流 MediaStream 与图片 `HTMLImageElement|String(图片url)`。可以将加载完成的 video 或绘制完成的 canvas 转成 MediaStream 作为 input。
...
SDK 输入为 camera 类型时，其挂载的 camera 对象支持以下方法：
<table>
...
sdk.on('ready', () => {
	...
	sdk.setEffect({
		id: effectList[0].EffectId,
		intensity: 0.7,
		filterIntensity: 0.5 // 1.0.18及以上版本支持单独设置effect中滤镜的强度，不传则默认与特效的intensity保持一致
	});
	sdk.setFilter(filterList[0].EffectId, 0.5)
})
sdk.on('error', (error) => {
...
effect:{
	...
	filterIntensity: number // 默认取intensity (1.0.18及以上版本支持)
}</pre>

...
| cameraReady | SDK的camera画面生成时触发，仅在使用内置相机时生效 | -         |
...
#### 延迟初始化
默认情况下初始化config的lazyInit为false，如果设置了lazyInit为true，则sdk此时会先加载需要的资源文件，资源加载
成功后会触发一个resourceReady事件，您可以在resourceReady事件触发后执行sdk.initCore完成初始化过程；
延迟初始化的应用场景：某些页面不一定立刻需要美颜sdk，但您可以先执行资源加载，之后结合业务特性按需初始化sdk再展示效果；
下面是示例代码：
```javascript
// 获取鉴权参数
const authData = {

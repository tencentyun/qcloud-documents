...
- 播放直播和**回看**
	...
	> 
	测试期间建议用一路，节约您的成本，如果您想多路调试又尽量减少录制费用，你可以通过修改小直播后台** RequestLVBAddr.php **文件，将拼接的URL参数 **record=flv|hls** 去掉。或者在终端代码里面将请求到的推流地址中的 **record=flv|hls** 参数删除，来屏蔽录制功能。推流地址带参数启动录制的原理可以参见[录制和回看](https://www.qcloud.com/document/product/454/6852)

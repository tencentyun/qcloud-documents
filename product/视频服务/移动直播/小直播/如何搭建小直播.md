...
> - 在压缩包中有live_demo.nginx和nginx.conf这两个文件，拷贝live_demo.nginx文件到/etc/nginx/ 目录下，将live_demo_service目录拷到 /data 目录下。在/etc/nginx 目录下有一个nginx.conf文件，参照压缩包中的 nginx.conf，主要确认http配置选项下有 **"include live_demo.nginx;"**。确保live_demo.nginx中root参数和小直播源码路径一致。

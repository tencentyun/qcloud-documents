尊敬的客户您好，感谢您关注腾讯云移动直播业务，接下来我们将向您介绍如何快基于腾讯视频云构建您自己的视频直播业务:

## 源码下载

开通视频直播服务后，就可以在 [直播控制台](https://console.qcloud.com/live/mlvbsdkdownload) 中免费下载小直播源码:
![](//mc.qcloudimg.com/static/img/3c136ed282c453aef1e31655f9a47d8b/image.png)

由于视频云各项服务的配置信息都跟具体的客户账号绑定，所以我们暂不提供统一的测试账号，以免您在调试对接过程中被其它腾讯云客户所影响，所以本文的目的旨在推荐您 <font color='blue'>**如何在自己的腾讯云账号上** </font>快速运行小直播。

## 对接攻略

### step1. 开通服务
小直播使用了几项腾讯云成熟的功能服务体系，你如果要快速调试运行起小直播，需要先开通如下服务：

| 服务名称 | 内部简称 | 功能描述 | 试用支持 | 是否可替换|
|:--------:|:---------:|---------|:-----: |:----:|
| [直播](https://www.qcloud.com/doc/api/258/6208#1.1-.E5.A6.82.E4.BD.95.E5.BC.80.E9.80.9A.E8.A7.86.E9.A2.91.E7.9B.B4.E6.92.AD.E6.9C.8D.E5.8A.A1) | LVB | 必选服务，推流和观看都需要使用它，也是唯一需要腾讯云人工审核才能开通的服务。 | 10G 免费试用 | 支持(云商需支持RTMP)|
| [点播](https://www.qcloud.com/doc/api/258/6208#2.1-.E5.A6.82.E4.BD.95.E5.BC.80.E9.80.9A.E8.A7.86.E9.A2.91.E7.82.B9.E6.92.AD.E6.9C.8D.E5.8A.A1) | VOD| 小直播支持将直播过程录制下来并以回放列表的形式进行展现，而视频文件的存储依赖腾讯云点播服务。| 7天免费试用|绑定LVB|
| [云通讯](https://www.qcloud.com/doc/product/454/6559#3.-.E4.BA.91.E9.80.9A.E8.AE.AF.E6.9C.8D.E5.8A.A1.EF.BC.88im.EF.BC.89) | IM | 提供消息的发送、接收、上下线状态、离线消息缓存以及聊天室等基础消息服务，小直播的互动消息、弹幕、点赞等功能都是基于这种互动消息而实现的。| 支持免费试用 | 支持 |
| [对象存储](https://www.qcloud.com/doc/product/454/6559#4.-.E5.AF.B9.E8.B1.A1.E5.AD.98.E5.82.A8.E6.9C.8D.E5.8A.A1.EF.BC.88cos.EF.BC.89) | COS | 小直播中的直播封面、用户头像等细节部分，需要使用图片的上传和下载功能，它是基于腾讯云的对象存储服务构建的。| 支持免费试用 | 支持 |

### step2. 开直播码
腾讯云直播服务(LVB)提供了两种接入模式：适合常规接入的**直播码**模式和适合手工操作的**频道托管**模式，小直播采用了直播码模式。

在开通直播服务后，进入直播管理控制台，在左侧的导航栏里打开**接入管理 - 直播码接入**，即可进入配置页面：
...
| 配置项 | 取值范围            | 具体含义  |
|----------|----------------------|--------------|
| 直播录制 | 开启 OR 关闭 | 全局直播录制开关，开启后每一条直播流都会默认录制为flv格式的视频文件，并落地到点播平台的视频列表里。[参考文档](https://www.qcloud.com/doc/api/258/5691) |
| 推流防盗链key | 32位小写的字符串  | 出于安全考虑，推流地址需要绑定防盗链以避免被其他人抢占，该KEY用于计算防盗链签名。[参考文档](https://www.qcloud.com/doc/api/258/5693) |
| API鉴权key | 32位小写的字符串  | 出于安全考虑，您的服务器在调用腾讯云后台API时，需要提供身份鉴权信息，该KEY用于帮助腾讯云确定您的服务器的合法身份。[参考文档](https://www.qcloud.com/doc/api/258/5956) |
| 回调URL | HTTP协议URL地址 | 腾讯云会把视频的推流断流等事件通知通过这个URL投递给您，尚不支持HTTPS协议。[参考文档](https://www.qcloud.com/doc/api/258/5957)  |

### step3. 配置终端
[直播控制台](https://console.qcloud.com/live/mlvbsdkdownload) 可以下载到小直播的终端源码，代码下载并编译后<font color='red'>并不能立刻体验</font>，因为源码中有些配置信息是留空的，不补全是无法正常运行的，您需要按照如下表格开通服务，并完成相关配置。
...
| 后台服务器地址        | 小直播后台服务器地址(可使用腾讯云主机快速搭建)      | [STEP4](https://www.qcloud.com/doc/product/454/6560#step5.-.E9.85.8D.E7.BD.AE.E5.90.8E.E5.8F.B0)  |
...
> **PUSH_URL_KEY**
>  就是推流反盗链KEY,是和**APP_ID**，**APP_BIZID**相关联，注意区分。**PUSH_URL_KEY**配置错误将会导致，[txSecret](https://www.qcloud.com/document/product/267/5693)计算错误，最终导致推流失败，直播控制台修改了**PUSH_URL_KEY**，业务后台一定要**同步更新**。
> **CALL_BACK_KEY ?** 

...
- **第一步：拷贝 LiteAVSDK 到工程目录**
  将下载到的 arr 文件拷贝到工程的 app/libs 目录下：
...
- **第四步：指定 App 使用的 CPU 架构**
  在 app/build.gradle 的 defaultConfig 中，指定 App 使用的 CPU 架构（目前 LiteAVSDK 企业版支持 armeabi  和 armeabi-v7a 架构，x64 架构还在开发中，计划时间点是 2019 年 6 月份） 。
...
- **第二步：拷贝 SDK 文件到工程目录**
  将解压得到的 jar 文件和 armeabi 文件夹拷贝到 app/libs 目录下。
  ![](https://main.qcloudimg.com/raw/54976997e5e6000393a419b925273872.png)
- **第三步：拷贝特效资源文件到工程目录**
  将解压得到的特效资源文件拷贝到 app/src/main/assets 目录下。
...
申请企业版 license 成功后，您会获得两个字符串：其中一个字符串是 licence URL，另一个字符串是解密 key。在您的 App 调用企业版 SDK 相关功能之前（建议在 Application类中）进行如下设置：
...
在 App 启动后，下载并解压素材包到手机任意目录下（推荐 app 的 data 目录），当解压完成后，即可通过以下接口开启动效效果：
...
- **LiteAVSDK 企业版是否支持 arm64 ？**
  目前企业版 SDK 由于特效处理库尚不支持 arm64 下的一些汇编指令，所以暂时还不支持 arm64 架构，请注意指定 App 的架构配置。x64 架构还在开发中，计划时间点是 2019 年 6 月份
- **Licence 是否正常使用中？**
  在设置 Licence 成功后，稍后一段时间（依据网络情况而定），SDK 会下载 license 文件到手机上。可以通过 TXLiveBase 的 getLicenceInfo() 方法查看 License 信息：
...
打印信息包含 Licence 的生效和过期时间，绑定的 app package name 信息等；若您需要其他协助，可将打印出来的 Licence 信息保存，并联系我们的技术支持。

...
- **API 调用**：腾讯云提供了直播相关的 API接口，包括状态查询和状态管理等功能，供您的后台服务器调用。
- **消息通知**：腾讯云在直播流状态变更、录制文件生成等一系列事件发生时，能够以事件消息（JSON）的形式主动通知您的后台服务器，只需要您在腾讯云注册接收事件通知的回调 URL 即可实现。


## API 调用
腾讯云提供了直播相关的 API接口，包括状态查询和状态管理等功能，供您的后台服务器调用，您可参考[云直播API概览](https://cloud.tencent.com/document/product/267/20456)文档。
...
- **计算示例**
   比如我们现在的时间是2016-08-22 15:16:27， 我们希望有效期是1分钟，也就是2016-08-22 15:16:27以后再收到携带这个 t 的请求或者通知即判定为非法的：
```

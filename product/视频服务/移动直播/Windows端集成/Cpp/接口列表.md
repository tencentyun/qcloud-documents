...
| startPreview(srcType,rendHwnd,rect,userType)    |(新接口)启动视频预览,包括录屏、摄像头、外部源数据   |
...
| openSystemVoiceInput(szPlayerPath)                         |打开系统混音或进程混音     |
| closeSystemVoiceInput()                         |关闭系统混音或进程混音     |
...
#### 4.startPreview(srcType,rendHwnd,rect,userType)
...
#### 13.setVideoResolution(resolution )
...
#### 14.setBeautyStyle(beautyStyle , beautyLevel, whitenessLevel)
...
#### 18.setAutoAdjustStrategy(strategy)
...
#### 19.setVideoBitRateMin(videoBitrateMin) && setVideoBitRateMax(videoBitrateMax)
...
| ------ | -------------------- | ---------------------------------------- |
| index | int | 麦克风设备的索引 |
| name | string |  用于存放麦克风设备的名称的字符串数组|
...
```
int volume = 100；
pusher.enableMic(bEnable);
```

#### 27. enableMic(bEnable)
...
#### 28. openSystemVoiceInput(szPlayerPath)
...
| 参数     | 类型                   | 说明                                       |
| ------ | -------------------- | ---------------------------------------- |
| szPlayerPath | string |szPlayerPath 播放器地址;如果用户此参数填空或不填，表示采集系统中的所有声音;
...
#### 32. setScreenCaptureParam(captureHwnd,rect)
...
| 参数     | 类型                   | 说明                                       |
| ------ | -------------------- | ---------------------------------------- |
| captureHwnd | HWND | 指定捕抓窗口，captureHwnd不为NULL时，捕抓整个窗口大小，captureRect无效，，则录制整个主屏。captureHwnd为NULL时，则捕抓主屏区域：captureRect。 |
| rect | const RECT | 指定捕抓主屏区域RC，captureHwnd为NULL时生效，captureRect为{0}时，则录制整个主屏。 |
...
#### 33. captureVideoSnapShot(filePath,length)

接口详情： long captureVideoSnapShot(filePath,length)
...
| 参数     | 类型                   | 说明                                       |
| ------ | -------------------- | ---------------------------------------- |
| filePath | string | 文件路径，目前只支持 .jpg格式 |
| length | int |  filePath 的长度|
...
#### 15. captureVideoSnapShot(filePath,length)
...
#### 17. speakerDeviceName(index,name)
...
推流视频分辨率
	...
	TXE_VIDEO_RESOLUTION_1080x1920 = 206,

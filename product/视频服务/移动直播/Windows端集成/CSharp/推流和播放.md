...
![](https://mc.qcloudimg.com/static/img/82391e1dccd97d0496b2a357f9239044/image.png)

## 准备工作

- **获取开发包**
  [下载](https://cloud.tencent.com/document/product/454/7873#Windows) SDK 开发包，并按照[工程配置]()指引将 SDK 嵌入您的 APP 开发工程。
...
先创建一个 **ManageLiteAV.TXCSharpLivePusher** 对象，我们后面主要用它来完成推流工作。

创建完成后，在开始推流之前，可以调用ManageLiteAV.TXCSharpLivePusher的设置接口，设置镜像效果、码率、分辨率和填充模式等
...
接下来我们要给摄像头的影像画面找个地方来显示，Windows 使用 HWND 窗口句柄作为基本的界面渲染单位，在 C# 窗体应用程序中，每个控件可以调用它的Handle获取得到HWND，所以您只需要准备一个 HWND 传给 ManageLiteAV.TXCSharpLivePusher 对象的 **startPreview** 接口就可以了。

```c#
// 设置回调，用于获取音视频数据和SDK内部事件
pusher.setCallback(this, (IntPtr)UserDataFlag.PusherFlag);
// 打开摄像头，摄像头索引值可通过ManageLiteAV.TXCSharpLivePusher 对象的 enumCameras 接口获取得到
...
一台电脑中如果存在多个摄像头，可以在不同摄像头之间进行切换，只需要调用ManageLiteAV.TXCSharpLivePusher 对象的 **switchCamera**接口，传入摄像头的索引值，即可完成切换。
...
- TXCSharpLivePusher 的 enumCameras 函数可以获取摄像头的索引值。

- Windows 下开启一个 USB 摄像头需要很长的电路和驱动启动时间，TXCSharpLivePusher 的 startPreview 的最后一个参数为 -1 时 ，代表打开全部摄像头，这时再切换摄像头，反应时间就会很快。
...
结束推流很简单，不过要做好清理工作，因为用于推流的 ManageLiteAV.TXCSharpLivePusher 对象同一时刻只能有一个在运行，所以清理工作不当会导致下次直播遭受不良的影响。
...
#### step 6: 更多功能
本地摄像头的画面镜像、静音、清晰度设置等功能，可以参考接口列表，获取更多详细信息。

### 事件处理

SDK 通过  ManageLiteAV.TXCSharpLivePusher 对象的 setCallback 接口来监听推流相关的事件。
...
先创建一个 **ManageLiteAV.TXCSharpLivePlayer** 对象，我们后面主要用它来完成推流工作。

```c#
player = new ManageLiteAV.TXCSharpLivePlayer();
```
...
```c#
// 设置回调
player.setCallback(this, (IntPtr)UserDataFlag.PlayerFlag);
...
启动播放的功能，只需调用 ManageLiteAV.TXCSharpLivePlayer 的 startPlay 接口即可，其中 PLAY_TYPE_LIVE_RTMP 是常规 CDN 的 RTMP 地址，优点是带宽价格很低，但延迟一般比较大。
...
结束播放时，如果要退出当前的UI界面，调用ManageLiteAV.TXCSharpLivePlayer的 stopPlay 接口。
...
通过 ManageLiteAV.TXCSharpLivePlayer 对象的 setCallback 接口来监听播放相关的事件。

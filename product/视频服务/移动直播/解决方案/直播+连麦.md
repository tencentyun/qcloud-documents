...
| :-------:| :-------:| :-------: | :-------:|
| serverDomain | 腾讯云 RoomService<br> `https://room.qcloud.com/weapp`<br>`/live_room` | 腾讯云 RoomService<br>`https://room.qcloud.com/weapp`<br>`/live_room` <br>需要提前进行 [**账号配置**](#TencentRoom) | [自行部署](#SelfRoom) RoomService<br> `https://[yourcompany]`<br>`/weapp/live_room` |
...
> 1. 方案二使用腾讯云 RoomSerivce （`https://room.qcloud.com/weapp/live_room` ）作为 LiveRoom 的房间后台，但跟方案一的直接使用不同，方案二需要先使用  [RoomTool.zip](http://dldir1.qq.com/hudongzhibo/mlvb/RoomTool.zip)  配置自己的直播和 IM 等服务信息。
> 
> 2. 如果您是终端研发工程师，可以使用  [RoomTool.zip](http://dldir1.qq.com/hudongzhibo/mlvb/RoomTool.zip)  本地生成几组调试用的 UserID 和 UserSig。 
...
LiveRoom 单靠一个终端的组件无法独自运行，它依赖一个后台服务为其实现房间管理和状态协调，这个后台服务我们称之为 RoomService。

...
| :-------:| :-------:| :-------: | :-------:|
| serverDomain | 腾讯云 RoomService<br> `https://room.qcloud.com/weapp/live_room` | 腾讯云 RoomService `https://room.qcloud.com/weapp/live_room` 需要提前 [配置](#TencentRoom) | [自建](#SelfRoom) RoomService `https://[yourcompany]/weapp/live_room` |
...
LiveRoom 单考靠一个终端的组件无法独自运行，它依赖一个后台服务为其实现房间管理和状态协调，这个后台服务我们称之为 RoomService。

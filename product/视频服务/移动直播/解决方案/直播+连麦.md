...
### 3. LiveRoom 内部原理
![](https://main.qcloudimg.com/raw/e3488c2ffdc7e2a63db5316c943452f0.gif)
...
<h4 id="CLIENT_LOGIN"> step2：登录 </h4>
...
userID 和 userSig 是用于登录 RoomService 的用户名和登录密码。由于用户名（userID）是由您来指定的，而且我们不能让您的用户在您的后台和腾讯云各注册一次，所以这里的登录密码是基于**数字签名**方案实现的：
 ...
 为了帮助您在调试期间快速跑通整个流程，可以先使用我们提供的  [RoomTool.zip](http://dldir1.qq.com/hudongzhibo/xiaozhibo/RoomTool.zip)  工具本地生成几组调试用的 UserID 和 UserSig。但产品上线时，UserSig 应当改成由您的后台服务器 [签发](https://cloud.tencent.com/document/product/454/14548)，这是因为客户端持有签名私钥是不安全的。
...
| 云通讯（IM）administrator | 调用 IM REST API 时需要使用[管理员名称](https://cloud.tencent.com/document/product/269/1519#3-.E8.B0.83.E7.94.A8.E6.96.B9.E6.B3.95)，由于 RoomService 使用了 IM REST API 发送房间里的系统消息，所以需要您配置管理员名称来实现消息下发。 |  [DOC](https://cloud.tencent.com/document/product/454/7953#IM_ADMIN)  |
...
![](https://main.qcloudimg.com/raw/06a11a4629781ea6e3e208ffa3fc63e2.png)

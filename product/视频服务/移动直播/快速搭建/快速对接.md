...
| 云通讯 | IM | 提供消息的发送、接收、上下线状态、离线消息缓存以及聊天室等基础消息服务，小直播的互动消息、弹幕、点赞等功能都是基于这种互动消息而实现的。| 支持免费试用 | 支持 |
| 对象存储 | COS | 小直播中的直播封面、用户头像等细节部分，需要使用图片的上传和下载功能，它是基于腾讯云的对象存储服务构建的。| 支持免费试用 | 支持 |
...
### step4. 配置终端
代码下载并编译后<font color='red'>并不能立刻体验</font>，因为源码中有些配置信息是留空的，不补全是无法正常运行的，您需要按照如下表格开通服务，并完成相关配置。
...
### step5. 配置后台
step3 中可以下载到小直播的服务端源码（目前还只有PHP版本），跟终端情况一样，源码中有些配置信息是留空的，不补全是无法正常运行的，您需要按照如下步骤完成相关配置。

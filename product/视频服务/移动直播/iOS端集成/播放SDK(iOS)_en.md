...
- **LVB and VOD**
<font color='blue'>LVB</font>'s video source is generated in real time, and it only makes sense when someone pushes the LVB stream. Therefore, once the VJ stops broadcasting, the LVB URL will become invalid. Since the video is played in real time, no progress bar is displayed on the player during the LVB.
...
Next, we need to look for a place to display the video pictures in a player. In iOS system, View is used as the basic picture rendering unit. Therefore, you only need to prepare a View to adjust the layout.

- **Recommended layout!**
> In the RTMP SDK player, the picture is not directly rendered to the View provided by you, but a subView used for OpenGL rendering is created on that View. However, the size of this subView used for rendering will be adjusted automatically according to the size change of the View provided by you. Therefore, you do not need to configure it.
>![](//mccdn.qcloud.com/static/img/75b41bd0e9d8a6c2ec8406dc706de503/image.png)
>
> If you want to implement UI controls such as bullet screen and flower presenting on the rendered view, we recommend that you create another View at the same level. This is a common design that avoids a lot of problems of picture covering.

- **How to make animation?**
> You can freely make animations for "view". But please note that the target attribute modified for the animation is **transform** attribute, instead of the frame attribute.
>
```objectivec
...
// Use setupVideoWidget to bind the View defining rendering area for the player. Its first parameter frame has been deprecated in 1.5.2 or later version
...
- **setupVideoWidget** 
The parameter frame has been deprecated in 1.5.2 or later version. The full screen will always be filled with the input View. You can modify the size and position of the view by directly adjusting the View.
...
[_txLivePlayer removeVideoWidget]; // Be sure to destroy the View control
```
...
```objectivec
// Specify a TXVideoRecordListener to synchronize the progress and results of the recording
...
```
- The progress of the recording is indicated by time by TXVideoRecordListener's onRecordProgress.
- Recorded file is in the format of MP4, and is indicated by TXVideoRecordListener's onRecordComplete.
- TXUGCPublish is responsible for uploading and publishing the video. For details on how to use it, refer to [UGC Small Video](https://www.qcloud.com/document/product/454/8838).
...
If you have no idea abouthow to handle the PLAY_EVT_PLAY_PROGRESS event during VOD, refer to the sample code - [Work with Playback Progress](https://www.qcloud.com/document/product/454/7896).
...
- **How to verify whether the LVB has ended?**
For **VOD**, we can use the PLAY_EVT_PLAY_END event to verify whether the playback has ended.
...
| NET_STATUS_VIDEO_FPS | The video frame rate of current stream media |

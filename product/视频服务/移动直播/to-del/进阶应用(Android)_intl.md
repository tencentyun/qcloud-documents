...
> If the Ping value from VJ to the IP given by SERVER_IP is very high (for example, exceeding 500 ms), the push quality will be unsatisfactory. ** Tencent Cloud has been providing services on an "Access to the Closest** basis. In case of the above situation, please contact us. Our OPS team will optimize the service.
...
| :-------------- | :-----------------------------------------------| :------: |
| setAudioSampleRate |   Audio sampling rate: count of audio signal captures by the recording device per second   |  44100   |  
| setANS          |   Noise suppression: When this is enabled, background noises can be filtered out (applicable when the sampling rate is below 32,000) |  Off     |
| setHardwareAcceleration |   Video hard-coding: When this is enabled, video capture up to 720 p, 30 fps is supported.   |  On   |  
| setVideoFPS     |   Video frame rate: Number of frames produced by the video encoder per second. Most of the phones don't support encoding above 30 FPS, so you're recommended to set FPS to 20.           |  20      |
| setVideoResolution |   Video resolution: Four types of 16:9 resolutions are available      |  640 * 360 |
| setVideoBitrate |   Video bitrate: Number of data bits produced by the video encoder per second (in Kbps) |  800 |
| setAutoAdjustBitrate |   Bitrate adaptation: Adjust the video bitrate automatically based on the network condition |   Off |
| setAutoAdjustStrategy | If bitrate adaptation is enabled, you can use this API to set the strategy to dynamically adjust the bitrate | STRATEGY_1 |
| setMaxVideoBitrate | Maximum output bitrate: This option takes effect only when bitrate adaption is enabled. |   1,200 |
| setMinVideoBitrate | Minimum output bitrate: This option takes effect only when bitrate adaption is enabled. |   800 |
| setVideoEncodeGop | Keyframe interval (in second): The interval at which one I frame is output | 3 seconds |
| setHomeOrientation | Set the rotation angle of video image, e.g., whether to push in a Landscape mode  |   0: home is on the right; 1: home is at the bottom; 2: home is on the left; 3: home is at the top   |
| setBeautyFilter | Beauty filter level: levels 1 to 9 are supported; the higher the level, the more obvious the effect.  0 means Off  |   Off   |
| setFrontCamera | Front or rear camera by default | Front |
| setWatermark | Watermark image (UIImage object) | Tencent Cloud Logo (demo)  |    
| SetTouchFocus | Whether to enable manual focus | Enabled |
| setPauseImg | Set the pause image used during the push at background | A resource image packed in the Demo | 
...
// Modify the audio sampling rate to 44,100 and fixed video bitrate to 800
...
mLivePushConfig.setAutoAdjustBitrate(false);   //Push at a fixed bitrate
mLivePushConfig.setVideoResolution(VIDEO_RESOLUTION_TYPE_360_640); //Set push resolution
```
...
mLivePushConfig.setMinVideoBitrate(500);//Minimum bitrate for dynamic adjustment
mLivePushConfig.setMaxVideoBitrate(1000);//Maximum bitrate for dynamic adjustment
mLivePushConfig.setVideoBitrate(800);//Initial default bitrate
mLivePushConfig.setAutoAdjustBitrate(true);//Whether to enable dynamic adjustment of bitrate
mLivePushConfig.setAutoAdjustStrategy(AUTO_ADJUST_BITRATE_STRATEGY_2);
mLivePushConfig.setVideoResolution(VIDEO_RESOLUTION_TYPE_360_640);//Default resolution 
```
...
```java
TXLivePushConfig* _config;
mLivePushConfig.setMinVideoBitrate(800);//Minimum bitrate for dynamic adjustment
mLivePushConfig.setMaxVideoBitrate(3000);//Maximum bitrate for dynamic adjustment
mLivePushConfig.setVideoBitrate(1800);//Initial default bitrate
mLivePushConfig.setAutoAdjustBitrate(true);//Whether to enable dynamic adjustment of bitrate
mLivePushConfig.setAutoAdjustStrategy(AUTO_ADJUST_BITRATE_RESOLUTION_STRATEGY_2);
mLivePushConfig.setVideoResolution(VIDEO_RESOLUTION_TYPE_720_1280);//Default resolution 
```
...
Implement VideoCustomProcessListener's onTextureCustomProcess () function to implement custom processing of the video images. The texture specified by textureId is a texture of type GLES20.GL_TEXTURE_2D.
...
SendCustomVideoData is used to populate the SDK with the captured and processed video data. I420 format is supported currently.

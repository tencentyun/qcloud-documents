...
> ?上图为超低延时直播和标准的CDN直播的对比视频，从左至右分别为：源视频、**超低延时直播（延时80ms）**、标准的 CDN 直播。
...
### 方案优势：
 ...
 - **灵活简单**：新的移动直播 V2 接口在设计上更加简单，尤其是在连麦等互动场景上，可以非常灵活的接入业务系统。
...
#### 推流体验
1. 下载视频云工具包，安装登录后，进入连麦演示（新方案）中。
2. 允许相关权限申请，单击【Pusher】，默认选择为 RTC，即超低演示直播模式，随机输入一个`steamId`，例如 `11223344`，然后单击【开始推流】按钮。
3. 开始推流后，也可单击右下侧的菜单按钮，放大窗口，设置如美颜、BGM、切换摄像头等操作。 
 ![](https://main.qcloudimg.com/raw/e2160adc79b02798488fb674de423545.png)

#### 播放体验
1. 下载视频云工具包，安装登录后，进入连麦演示（新方案）中。
2. 允许相关权限申请，单击【Player】，默认选择为 RTC，即超低演示直播模式，输入对应推流时设置`steamId`，例如 `11223344`，然后单击【开始播放】按钮。
3. 开始播放后，也可点击右下侧的菜单按钮，放大窗口，设置诸如静音等操作。
...
![](https://main.qcloudimg.com/raw/70c9154d9a63f76fb247fd808c8c3937.png)
...
在超低延时播放的场景中，一般都伴随着中/高频的连麦需求，包括观众连麦和主播 PK，目前新的 V2 接口可以**非常灵活、简单**的实现类似的互动需求，且最多支持30人同时连麦，功能更强大，稳定性更高，只需如下简单四步：
1. 调用 V2TXLivePusher 开始主播 A 的推流。
2. 所有观众侧调用 V2TXLivePlayer 开始播放主播 A 的推流。
3. 开始连麦，连麦观众 B 调用V2TXLivePusher发起推流。
4. 收到连麦消息后，主播 A 和其他观众调用 V2TXLivePlayer 开始播放观众 B 的推流。

此时我们的主播 A，观众B，其他观众即进入超低延时的实时互动场景中，更详细的步骤调用&amp;代码示例，详见：[连麦互动(新方案)](https://cloud.tencent.com/document/product/454/52751?!editLang=zh&!preview)。

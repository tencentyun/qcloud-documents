...
“小直播”使用了一个叫做 `MLVBLiveRoom` 的 class 来实现房间列表的功能， 其中的 `createRoom` 函数的第二个参数为一个字符串类型的 `roomInfo`，主播端可以定制这个字符串（比如通过 jason 格式组装出自己想要的数据内容），观众端会在 `getRoomList` 时拿到这个字符串。

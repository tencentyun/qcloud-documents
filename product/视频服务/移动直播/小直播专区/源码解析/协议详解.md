...
小直播与业务Server的通信采用Https协议，请求包和回包数据格式采用json，接口名使用Action指定，如Action=get_push_url，请求方式采用POST方式
...
| 参数       | 类型     | 说明                      |
| -------- | ------ | ----------------------- |
| Action   | string | 该接口Action为get_push_url |
| userid   | string | 用户id                    |
| groupid  | string | 群组id                    |
| title    | string | 直播标题                    |

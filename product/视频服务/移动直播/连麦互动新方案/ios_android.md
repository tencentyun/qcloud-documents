...
## 连麦互动新方案体验
视频云工具包是腾讯云开源的一套完整的音视频服务解决方案，包含实时音视频（TRTC）、移动直播（MLVB）、短视频（UGC）等多个 SDK 的能力展示，其中包含连麦互动新方案相关体验UI — **连麦演示( 新方案 )** 。
...
4. 创建成功后，点击右侧【应用信息】，查看应用对应的 `SDKAppID` 信息。
...
#### URL 拼接
具体的推流 URL 字符串，需要开发者按照协议解析中的规则，在工程代码中自行拼接。
...
- **步骤一**：主播 A 开始直播，调用 `V2TXLivePusher` 开始主播 A 的推流。
<dx-codeblock>
::: Java
...
- **步骤二**：所有观众观看主播 A 直播，调用 `V2TXLivePlayer` 开始播放主播 A 的推流。
<dx-codeblock>
::: Java
...
- **步骤三**：**开始连麦**，其中观众 B 调用 `V2TXLivePusher` 发起推流（后续会称呼为连麦观众B）。
<dx-codeblock>
::: Java
...
- **步骤四**：**收到连麦消息后**，主播 A 调用 `V2TXLivePlayer` 开始播放**连麦观众 B** 的推流，此时主播 A 和**连麦观众 B** 即可进入超低延时的实时互动场景中。
<dx-codeblock>
::: Java
...
- **步骤五**：**连麦成功后**，其他观众调用 `V2TXLivePlayer` 同时也开始播放**连麦观众 B** 的推流。
...
- **步骤二**：主播 B 开始直播，调用 `V2TXLivePusher` 开始主播 B 的推流。
- <dx-codeblock>
::: Java
V2TXLivePusher pusherB = new V2TXLivePusherImpl(this, V2TXLiveMode.TXLiveMode_RTC);
...
...
- **步骤三**：**开始 PK**，主播 A 和主播 B 分别调用 `V2TXLivePlayer` 开始播放对方的推流，此时主播 A 和主播 B 即进入超低延时的实时互动场景中。
<dx-codeblock>
::: Java
V2TXLivePlayer playerA = new V2TXLivePlayerImpl(mContext);
...
...
#### 1. 为什么同一台设备不支持使用相同 streamid 同时推流和拉流呢，你们的`TXLivePusher&TXLivePlayer`是可以的啊？

...
### 1.2 直播码接入怎么配？
直播服务开通后，进入[直播控制台>>直播码接入>>接入配置](https://console.qcloud.com/live/livecodemanage) 完成相关配置，即可开启直播码服务：
![](//mc.qcloudimg.com/static/img/32158e398ab9543b5ac3acf5f04aa86e/image.png)

| 配置项 | 取值范围            | 具体含义  |
|----------|----------------------|--------------|
| 直播录制 | 开启 OR 关闭 | 开启直播录制后，只要是直播的视频，都会被后台默认的录制下来。[DOC](https://www.qcloud.com/document/product/454/7917) |
| 推流防盗链key | 32位小写的字符串  | 出于安全考虑，推流地址需要绑定防盗链以避免被其他人抢占，该KEY用于计算防盗链签名。[DOC](https://www.qcloud.com/document/product/454/7915#.E9.98.B2.E7.9B.97.E9.93.BE.E7.9A.84.E8.AE.A1.E7.AE.97.EF.BC.9F) |
| API鉴权key | 32位小写的字符串  | 您的服务器在调用腾讯云后台API时，需要提供身份鉴权信息，该KEY用于帮助腾讯云确定您的服务器的合法身份。[DOC](https://www.qcloud.com/document/product/454/7920#.E5.AE.89.E5.85.A8.E6.A3.80.E6.9F.A5) |
| 回调URL | HTTP协议URL地址 | 腾讯云会把视频的推流断流等事件通知通过这个URL投递给您，尚不支持HTTPS协议。[DOC](https://www.qcloud.com/document/product/267/5957)  |
...
### 1.5 查询推流防盗链KEY
推流防盗链KEY是为了确保只有您的APP用户才可以推流的安全保护手段，推流防盗链KEY可以在开启直播码时指定，亦可随时按您的需要在[直播管理控制台](https://console.qcloud.com/live/livecodemanage)中修改：
![](//mc.qcloudimg.com/static/img/6be1d875f1120a16d3692c60bb4485a9/image.png)

### 1.6 查询API访问鉴权KEY
API访问鉴权KEY是您的后台服务器在调用腾讯云直播码相关的[云端API](https://www.qcloud.com/doc/api/258/5956)时需要用到的，目的是帮助腾讯云确认调用的合法性，API访问鉴权KEY可以在开启直播码时指定，亦可随时按您的需要在[直播管理控制台](https://console.qcloud.com/live/livecodemanage)中修改：
...
## 4. 对象存储服务（COS）
### 4.1 如何开通云通讯服务
新认证的腾讯云账号都可以立刻使用对象存储服务，只需要进入[COS管理控制台](https://console.qcloud.com/cos)创建爱你一个Bucket即可开始启用，**请注意为支持https下载（适配苹果的ATS），请开启CDN加速**。
...
进入[云服务器管理控制台](https://console.qcloud.com/cvm/overview)，点击"购买云服务器"，将进入服务器购买页面：
...
## 6. 云数据库（可选）
### 6.1 如何开通云数据库
进入[云数据库管理控制台](https://console.qcloud.com/cdb)，如果您没开通过CDB(MYSQL)服务，可直接点上面的“新建”按钮：

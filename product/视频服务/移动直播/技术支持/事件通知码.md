...
| 3005 | PUSH_WARNING_READ_WRITE_FAIL   |  超过30s没有数据发送，主动断开连接。。 |
...
> **注意：**
>- **判断直播是否结束：**基于各种标准的实现原理不同，很多直播流通常没有结束事件（2006）抛出，此时可预期的表现是：主播结束推流后，SDK 会很快发现数据流拉取失败（WARNING_RECONNECT），然后开始重试，直至三次重试失败后抛出 PLAY_ERR_NET_DISCONNECT 事件。所以 2006 和  -2301 都要判断，用来作为直播结束的判定事件。
>

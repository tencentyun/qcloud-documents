...
Implement onPreProcessTexture function of TXVideoCustomProcessDelegate for the customized processing of video images. The texture specified by textureId is a texture of type GLES20.GL_TEXTURE_2D.
...
- **Step 1: Do not call TXLivePusher's startPreview API any longer**
...
- **Step 2: Set customModeType via TXLivePushConfig**
...
- - **Step 3: Use sendVideoSampleBuffer to populate SDK with Video data**
SendVideoSampleBuffer is used to populate the SDK with the captured and processed video data. RGBA and NV12 formats are supported now.

```objectiveC
    TXLivePushConfig* config = [[TXLivePushConfig alloc] init];
    config.customModeType    = CUSTOM_MODE_VIDEO_CAPTURE;  // Customize data capture
...
- - **Step 4: Use sendAudioSampleBuffer to populate SDK with Audio data**
...
```objectiveC
    TXLivePushConfig* config = [[TXLivePushConfig alloc] init];
    config.customModeType    = CUSTOM_MODE_AUDIO_CAPTURE;  // Customize data capture
    TXLivePush pusher = [[TXLivePush alloc] initWithConfig:config];
...
- **Capture image data of Player with the onPlayerPixelBuffer callback.**
 ...
 /**
 * Callback for video rendering object

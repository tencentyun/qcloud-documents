在MPS中，有两种发起转码任务的方式：一种是通过设置工作流，实现文件上传后自动触发任务；另一种是通过调用API，对已上传文件主动发起转码任务。第一种方式请参考[设置工作流](./设置工作流.md)进行设置。本文将介绍后一种任务发起方式。
...
如果开发者设置了事件通知CMQ，则将会接收到该任务的完成事件通知。除了通过CMQ接收事件通知消息，也可以通过[查询任务详情](https://cloud.tencent.com/document/product/1090/33530) API 接口查询任务结果，输入参数即为ProcessMedia接口返回的TaskId。
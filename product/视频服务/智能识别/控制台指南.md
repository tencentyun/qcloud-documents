...
### 步骤7：发起视频处理<span id ="p1"></span>

调用 API [ProcessMedia](https://cloud.tencent.com/document/api/266/33427) 发起任务：在请求中的`AiRecognitionTask`参数指定 [视频内容识别模板](https://cloud.tencent.com/document/product/266/34692#sh) 的模板 ID。



### 步骤8：查看处理结果

调用 API [DescribeTaskDetail](https://cloud.tencent.com/document/api/862/37614) 发起视频处理查询任务，查看执行状态和结果的详细信息，在请求的`Taskid`参数中输入在 [步骤7：发起视频处理](#p1) 中返回的`TaskId`（最多可以查询3天之内提交的任务）。

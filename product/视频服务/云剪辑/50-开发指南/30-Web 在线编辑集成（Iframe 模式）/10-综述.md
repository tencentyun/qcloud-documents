...
<img src='https://main.qcloudimg.com/raw/ad1a8b8a92ff8f54cc7bb2eec4981a14.png'>
...
云剪组件用 iframe 的方式嵌入到应用，采用 postMessage 方式与父容器通信，所以`send`必须在`iframe`内剪辑页面加载成功以后可用（监听到`Editor:Ready`事件以后），否则无效或报错。

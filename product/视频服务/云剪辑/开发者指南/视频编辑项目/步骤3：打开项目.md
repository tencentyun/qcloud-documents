...
![](https://main.qcloudimg.com/raw/116e4540f438110d20c69739f5ed186d.png)
在平台`Platform=example`下，用户`UserId=uid_tony`打开视频编辑项目`ProjectId=pid_example`，具体的执行流程如下：
### 获取访问签名
当开发者前端需要在页面上集成视频编辑页面时，需要先生成一个访问签名。访问签名是由开发者后端派发的，用于云剪校验访问者的身份及权限，具体生成方法请参见 [签名算法](https://cloud.tencent.com/document/product/1156/43777?!preview&!editLang=zh)。在生成签名中涉及到一些必选参数，示例参数如下所示：
```
action = OpenProject
platform = example
...
云剪会将上面`id=editor`的`div`标签进行初始化，将云剪视频编辑页以 iframe 的形式植入当中，开发者也可以根据自己的页面结构进行适当的调整，更多细节请参见 [云剪SDK](https://cloud.tencent.com/document/product/1156/43776?!preview&!editLang=zh)。

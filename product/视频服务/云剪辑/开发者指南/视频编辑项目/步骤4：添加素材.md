添加素材指将文件添加到项目中成为素材，项目中可以对素材进行编辑。用户进入云剪编辑页后，云剪只提供了一些效果素材和公共素材。用户若添加自己的素材，素材来源可以是本地文件，也可以是云点播的媒资文件，开发者可根据自己系统的应用场景给用户提供合适的添加方式。
添加素材目前支持上传本地文件及导入云点播媒资两种方式，具体的用法如下文所述。

## 上传本地文件
![](https://main.qcloudimg.com/raw/a5fb00420f2fba3e065a0e62848ec861.png)
### 流程描述
#### 触发添加素材通知
...
#### 获取上传访问签名
当用户触发添加素材事件时，开发者接收到通知，如果允许用户进行上传，需要使用到访问签名。访问签名由开发者后端派发，用于云剪校验访问者的身份及权限，具体签名生成方法请参见 [签名算法](https://cloud.tencent.com/document/product/1156/43777?!preview&!editLang=zh)。在生成签名中涉及到一些必选参数，对应的示例参数如下所示：
<pre><code>
action = Upload
platform = example
...
效果展示：
![](https://main.qcloudimg.com/raw/0b09b2721d112ff787a04b2bd1224410.png)
...
## 导入云点播媒资
![](https://main.qcloudimg.com/raw/dfe5b315c4a6e2a75e62240810c1efba.png)
### 流程描述
#### 导入云点播媒资
...
#### 调用 SDK 导入素材
当开发者获取到具体的素材 ID 后，调用云剪前端 SDK 将该素材导入到当前打开的项目中，素材即可出现在面板中展示面板中，用户即可使用素材进行编辑：
```js
...
```
效果展示：
![](https://main.qcloudimg.com/raw/5ca1b0ca7d3191bf0060be0a71544f32.png)

**添加素材**指将文件添加到项目中成为素材，项目中可以对素材进行编辑。用户进入云剪编辑页后，云剪只提供了一些效果素材和公共素材。用户若添加自己的素材，素材来源可以是本地文件，也可以是云点播的媒资文件，开发者可根据自己系统的应用场景给用户提供合适的添加方式。
...
#### 获取上传访问签名
当用户触发添加素材事件时，开发者接收到通知，如果允许用户进行上传，需要使用到访问签名。访问签名由开发者后端派发，用于云剪校验访问者的身份及权限，具体签名生成方法请参见 [签名算法](https://cloud.tencent.com/document/product/1156/43777)。在生成签名中涉及到一些必选参数，对应的示例参数如下所示：
<pre><code>
action = Upload
platform = example

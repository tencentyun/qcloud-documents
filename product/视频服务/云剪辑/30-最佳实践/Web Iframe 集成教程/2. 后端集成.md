...
1. [创建项目](#CreateProject)：此接口返回 `项目ID` 及打开项目时所需要的签名，同时导入编辑素材。
2. [导出视频](#ExportVideo)：此接口发起视频导出任务，返回 `任务ID`。
3. [查看导出结果](#GetTaskInfo)：此接口用于查询云端视频合成进度，成功后会同时返回视频 URL 及云点播 FileId。

回顾集成架构图，**业务后端** 的位置如红框所示。
<img src="https://qcloudimg.tencent-cloud.cn/raw/e50a43d73308e1ad22e4907d09c628a4.png" width="960">
...
## 接口约定
本教程中约定，后端暴露的接口形式如下：
1. 后端服务的域名为 `api.example.com`。
2. 以 HTTP GET 方式发起请求，通过 QueryString 传递业务参数。
3. 以 `example.com` 域的 Cookie `UserId` 传递用户 ID（暂时不考虑鉴权）。
...
| Definition | String | 视频剪辑预设配置 Id，用于指定导出视频的参数，详情请参考 [导出视频编辑项目](https://cloud.tencent.com/document/product/1156/40353) 接口说明。 |
...
业务中涉及腾讯云服务端 API 调用。需要在路由代码之前增加如下代码，初始化腾讯云API SDK：
```javascript
...
## Step 2：开发创建项目接口（CreateProject）[](id:CreateProject)
主要流程如下：
1. 解析请求，从 Cookie 中获取 `UseId`。解析请求获得项目名称、项目初始媒体列表等请求参数。
2. 调用多媒体创作引擎 [创建项目（CreateProject）](https://cloud.tencent.com/document/product/1156/40350)接口创建在线剪辑项目，返回项目 ID。
3. 调用多媒体创作引擎 [在项目中导入媒体](https://cloud.tencent.com/document/product/1156/40352)接口，导入媒体素材到项目中。
4. 根据 [客户端访问签名](https://cloud.tencent.com/document/product/1156/43784)生成前端打开项目时所需要的签名。
5. 组装前端应答。
...
router.get('/CreateProject', async (ctx, next) => {
    ...
    // 2. 调用多媒体创作引擎服务端 API 创建项目
    const projectId = await doCreateProject({
        userId: userId,
        name: name,
    ...
    // 3. 导入媒体到项目
   doImportMediaToProject({
        projectId: projectId,
        initFileIdList: fileIdList
        })
...
// 根据请求参数，组装到多媒体创建引擎创建项目的参数
function buildCreateProjectParams(params) { 
    return {
        // 平台，填写准备工作中创建的平台 Id,常量
        ...
        Owner: {
            Type: "PERSON",
            // 用户 Id，使用当前调用的用户 Id，用于区分权限。可通过参数指定或者后台指定
            Id: params.userId
        },
        // 项目类别，固定为 VIDEO_EDIT
        Category: "VIDEO_EDIT",
...
function doCreateProject(params) {
    return new Promise((resolve, reject) => {
        client.CreateProject(buildCreateProjectParams(params), function (err, response) {
            if (err) {
                reject(err)
            } else {
...
function buildImportMediaToProjectParams(projectId, vodFileId) { 
    return {
        ...
        // 项目 Id
        ProjectId: projectId,
        // 媒体来源，这里使用云点播文件固定为 'VOD'
        SourceType: "VOD",
        // 云点播 FileId
        VodFileId: vodFileId,
        // 预处理参数，如里视频不能在浏览器直接播放(非 h264 编辑、格式非 mp4/m3u8/webm)需要添加此参数
        PreProcessDefinition: 10
    }
}

// 将媒体导入到多媒体创作引擎，这里忽略了错误处理，实际业务中请做好错误处理
function doImportMediaToProject(projectId, initFileIdList) {
    for(var i = 0; i<= initFileIdList.length; ++i){
        await importMediaToProject(projectId, initFileIdList[i])
    }
}

// 将单个媒体导入到多媒体创作引擎
function importMediaToProject(projectId, vofFileId) {
    return new Promise((resolve, reject) => {
        client.ImportMediaToProject(buildImportMediaToProjectParams(projectId, vodFileId), function (err, response) {
            if (err) {
                reject(err)
            } else {
                resolve(response)
...
// 生成打开项目的签名
function genOpenProjectSign(userId, projectId) {
    let current = parseInt((new Date()).getTime() / 1000)
    let expired = current + 300  // 签名有效期：5分钟
...
>- 示例代码中没有对客户端请求的 `InitFileIdList` 做校验，可能导致用户访问到 VOD 平台中任意一个视频。实际业务中，为避免不必要的访问越权，需要进行校验 FileId 归属。
>- 创建项目后导入媒体，也可以根据实际情况在创建项目完成后分批导入。


### 步骤3：开发导出视频编辑项目接口（ExportVideo）[](id:ExportVideo)
主要流程如下：
1. 从 Cookie 中获取 `UseId`。解析请求获得项目 Id、剪辑预设配置、导出视频名称等请求参数。
2. 调用多媒体创作引擎 [导出视频编辑项目（ExportVideoEditProject）](https://cloud.tencent.com/document/product/1156/40353)接口，发起导出视频任务，返回任务 ID
3. 组装前端应答。
...
router.get('/ExportVideo', async (ctx, next) => {
    ...
    const taskId = await doExportVideoEditProject({
            userId: userId,
            name: name,
            projectId: projectId,
...
function buildExportVideoEditProjectParams(params) { 
    return {
        // 平台，填写准备工作中创建的平台 Id
        Platform: config.platform,
        // 项目 Id，填参数传递到服务端的项目 Id
        ProjectId: params.projectId,
        // 导出模板 Id，具体请参见接口文档。此处填接口传递给服务端的参数
        Definition: params.definition,
        // 导出目标，此处为导出视频到云点播为例，固定填 VOD
...
function doExportVideoEditProject(params) {
    return new Promise((resolve, reject) => {
        client.ExportVideoEditProject(buildExportVideoEditProjectParams(params), function (err, response) {
            if (err) {
                reject(err)
            } else {
...
### 步骤4：开发查看导出任务结果接口（GetTaskInfo）[](id:GetTaskInfo)
整个接口的主要流程如下：
1. 解析请求，从 Cookie 中获取 `UseId`。解析请求获得任务 Id 参数。
2. 调用多媒体创作引擎 [获取任务详情（DescribeTaskDetail）](https://cloud.tencent.com/document/product/1156/40359)接口，获取任务结果，如果任务完成，返回导出的视频 URL 及点播 FileId。
3. 组装前端应答。
...
router.get('/GetTaskInfo', async (ctx, next) => {
    ...
    const taskInfo = await doGetTaskInfo({
            userId: userId,
			taskId: taskId
        })
...
function doGetTaskInfo(params) {
    return new Promise((resolve, reject) => {
        client.DescribeTaskDetail(buildGetTaskInfoParams(params), function (err, response) {
            if (err) {
                reject(err)
            } else {
                let result = {
                    // 任务状态，只状态为 'SUCCESS' 才算成功

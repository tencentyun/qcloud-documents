...
用户点击 **开始** 按钮进入业务流程，`多媒体创作引擎` 的剪辑界面会嵌入到 `container` 元素内。
...
1. 调用 [后端集成](TODO) 中的 [CreateProject](TODO) 接口，指定 **初始素材** 并获取 **打开项目所需签名** 。
...
>?
此处以[准备工作](TODO)中的视频和水印图片为例，假设两个视频的 FileID 分别为 `VideoFileID1`、`VideoFileID2`，水印图片的 FileID 为 `WatermarkFileID`。
...
function initExportEvent(cmeInstance,successCallback){
      /**
        * @on 监听方法,监听来自 CME 的用户事件，用户点击导出按钮时会触发这个事件。
...
集成完毕，点击 开始 按钮，即可看到剪辑器界面，在该页面中剪辑您的视频：
1. 拖动两个视频到轨道。
2. 点击转场，在两个视频之间插入一个效果。
...
点击导出按钮，耐心等待直到看到合成提示，至此已完成 Web Iframe 模式集成在线视频剪辑业务。


## 更多参考

- [SDK API参考](https://cloud.tencent.com/document/product/1156/51217)
- [Web 组件化参考](https://cloud.tencent.com/document/product/1156/51221)
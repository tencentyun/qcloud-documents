...
###  VideoEditProjectTask
字段 | 类型 | 描述
------- | ------- | -------
ErrCode | Int64 |  返回码。
ErrMsg | String |  返回错误信息。
VideoEditProjectInput |  [VideoEditProjectInput](#VideoEditProjectInput) |  创建时间 ,标准ISO格式。
VideoEditProjectOutput | [VideoEditProjectOutput](#VideoEditProjectOutput) | 视频编辑导出任务。
...
Name | String |  项目 ID 。
ClassId | Int64 |  模板 ID 。
ThirdPartyPublishInfoSet |  String | 导出目标。
VODExportInfo | [VODExportInfo](#VODExportInfo) | VOD导出信息。
CMEExportInfo | [CMEExportInfo](#CMEExportInfo) | CME导出信息。
...
### SwitcherPgmOutputConfigForUpdate
字段 | 类型 | 描述
------- | ------- | -------
TemplateId | Int64 |导播台输出的模板 ID。
Width | Int64 |导播台输出的的宽，单位：像素。
Height | Int64 |导播台输出的的高，单位：像素。
Fps | Int64 |导播台输出的的帧率，单位： 帧/秒。
BitRate | Int64 |导播台输出的的码率，单位 bit/s 。

### StreamConnectInfoForUpdate
字段 | 类型 | 描述
------- | ------- | -------

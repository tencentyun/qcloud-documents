...
```
2. 特效自定义：
特效支持添加 `alpha`类型特效， `alpha`类型特效制作请参见 [素材制作与扩展-特效制作教程]()。
...
```
>? 可以看到`sticker3`相比默认数据类型多了一个`zipUrl`字段，里面包含了贴纸的元信息，默认贴纸已在播放器内置所以无需传入。`zipUrl`制作方法请参见 [素材制作与扩展-贴纸制作教程]()。

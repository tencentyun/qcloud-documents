...
该项目是为**短视频小程序解决方案所提供的后台 Demo**。该 **Demo** 主要是用 `TypeScript` 进行开发，使用 [`Koa`](https://koa.bootcss.com/) 框架进行搭建，其中使用了丰富的第三方开源库，如 [`routing-controllers`](https://github.com/typestack/routing-controllers)、 [`class-validator`](https://github.com/typestack/class-validator) 、 [`typeorm`](https://typeorm.io/) 、[`typedi`](https://github.com/typestack/typedi) 等。
...
| wxAppConfig | 微信小程序配置。 |
| wxAppConfig.appId | 微信小程序开发者AppId |
| wxAppConfig.appSecret | 微信小程序开发者AppSecret |
...
| 文件/目录 |  |
| ---- | ----------- |
| [`package.json`](https://docs.npmjs.com/files/package.json) | 该文件定义了这个项目所需要的各种依赖模块，以及项目的运行 `Script` 和配置信息（比如名称、版本、许可证等元数据）。 |
| `src` | 源代码目录。 |
| `src/conf`        | 配置模块。 |
...
- 加载服务配置。
- 连接数据库。
- 通过 [`Koa`](https://koa.bootcss.com/) 加载 日志 和解析 `HTTP BODY` 的中间件，注册控制器。
...
| 模块文件 | 描述 |
| ---- | ----------- |
| [moduleConfig.json](#配置文件)        | 整个**Demo** 服务的配置文件。 |
...
```
- 可以看到 `MediaController` 类通过装饰器（`@JsonController`）进行装饰，确保每个处理方法都能以`JSON`字符串返回，并且 `HTTP Response ` 的头部 `Content-Type` 被设置为 `application/json`。

- 另外， `MediaController` 中定义了一个 `wxampGetVideoList` 方法，`wxampGetVideoList` 方法前通过 `@Post`， 指定 `/WXAMPGetVideoList` 接口路由到此方法进行处理。
- 其中，`wxampGetVideoList` 方法中定义了一个数据传输对象 `GetVideoListDTO`，并通过 `@Body({ validate: false })` 装饰，此处将请求中的 `HTTP Body` 数据注入到 `GetVideoListDTO` 对象中。为了在校验非法的情况下使用自定义的回包，此处通过指定 `validate: false` 促使不在注入的同时进行参数校验，而是延迟到在 `wxampGetVideoList` 方法体中通过 `Validate` 方法进行显示校验。
...
- 最后，`wxampGetVideoList` 方法返回 `Response` 对象， [`routing-controllers`](https://github.com/typestack/routing-controllers) 将其转化为 `JSON` 字符串返回到请求客户端。
...
- 比如，对于 ``Offset`` 字段，通过 `@Min` 限制 `Offset` 的最短长度为 1，并且都定义了校验非法情况下的错误信息 `message` 和错误码 `errorCode`。
...
该模块主要封装了一些业务处理的逻辑，作为一个完整的子功能提供给 `Controller` 类使用。比如 `Token`的生成、上传签名的生成、参数校验错误的解析以及数据库相关的操作等。下面以 `src/service/video.ts` 中的 `SearchVideo` 方法为例进行说明。
...
该模块下主要封装了对于数据库的增删改查操作，用于返回业务逻辑逻辑需要的实体数据。比如根据条件获取视频列表，修改用户信息，查询用户信息，视频播放信息和用户信息聚合等相关操作。下面将以 `src/dao/video.ts` 中 `VideoDao` 为例进行说明。
...
该模块下定义了一些工具方法、错误码以及一些公共类（包括`Response`、`VodError`、`Context`等）。开发者可根据自身业务特点进行扩充。

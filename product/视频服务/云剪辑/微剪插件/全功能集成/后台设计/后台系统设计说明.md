...
## 业务逻辑
本章节说明各项业务功能的交互流程以及核心逻辑，对于其中的实现细节，请参考 [业务后台代码解读](https://tcloud-doc.isd.com/document/product/1156/48626?!editLang=zh&!preview)。
...
1. 用户在 App 上录制、编辑视频，单击【发布】按键，客户端将向业务后台请求客户端上传签名。
...
#### 删除视频
1. 用户在【我】界面单击自己上传过的视频，然后单击【删除】按钮，客户端会向业务后台发出删除请求。
2. 业务后台校验 Token 的合法性，如果不合法，返回错误码；如果合法则删除数据库中的对应记录。
3. 业务后台向 VOD 发起删除请求，清理 VOD 上存储的视频数据。
4. 业务后台返回处理结果给客户端。

### 播放视频
客户端在拉取视频列表时，已经获取到了视频的 URL 并缓存，如果 VOD 已经开启了 Key 防盗链，那么业务后台返回的视频 URL 会带有防盗链签名。当用户单击视频时，若 VOD 并未开启 Key 防盗链，或者防盗链签名尚未过期，则客户端可以直接使用缓存的 URL 进行播放（下图蓝线）。如果开启了 Key 防盗链并且缓存 URL 的防盗链签名已经过期，那么就需要重新获取（下图红线）。
...
#### 上传签名
业务后台在生成上传签名时指定了一些参数（详见 [上传签名参数]( https://cloud.tencent.com/document/product/266/9221#.3Cspan-id-.3D.22p2.22.3E.3C.2Fspan.3E.E7.AD.BE.E5.90.8D.E5.8F.82.E6.95.B0.E8.AF.B4.E6.98.8E)），以实现短视频场景下的典型业务逻辑。
- **任务流参数**：
```

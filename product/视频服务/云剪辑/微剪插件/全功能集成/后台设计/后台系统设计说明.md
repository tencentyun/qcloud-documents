...
![系统架构](https://main.qcloudimg.com/raw/4d170581fb06cde8a89105d6ae1777e5.png)

  Demo 所提供的是客户端和业务后台两部分的源码。

  ## 业务逻辑

  本章节说明各项业务功能的交互流程以及核心逻辑，对于其中的实现细节，请参考 [业务后台代码解读]()。

  ### 小程序用户登录

  登录是客户端和业务后台之间的逻辑，交互流程不涉及 VOD，如下图所示：

  ![登录](https://main.qcloudimg.com/raw/2a6622fd7d9efade11345c89ec471d21.png)

  整体思路可以见[微信小程序官方文档](https://developers.weixin.qq.com/miniprogram/dev/framework/open-ability/login.html)登录部分。

  1. 用户登录时，微信小程序前端调用`wx.login`接口获取登录凭证 code。向业务后台发起登录请求；
2. 业务后台会根据登录凭证 `code`，调用微信接口`auth.code2Session` 换取用户登录态信息，包括用户的唯一标识 `openid` 及本次登录的会话密钥 `session_key` 等，后台将会把 `openid` 作为小程序用户的授权信息。如果获取失败则会返回错误；
  3. 如果业务后台在数据库授权信息（ `UserAuth` 表）中没有找到对应授权信息（即 `openid` ），则自行创建一个全局唯一的用户，昵称和密码通过随机生成，将该用户 ID 与授权信息保存在数据库中；如果找到对应授权信息（即 `openid` ），则会获得对应的用户 ID；
4. 业务后台为用户生成一个 `Token` 并返回。

  ### 用户管理

用户是客户端和业务后台之间的逻辑，交互流程不涉及 VOD，如下图所示：

![用户管理](https://main.qcloudimg.com/raw/ee452c90b63395a3820b9b499bdfb4d5.png)
...
3. 业务后台返回成功。
  ...
  - 用户在 App 上访问用户信息界面，客户端将向业务后台发起请求；
- 业务后台校验请求中 Token 的合法性，如果不合法，返回错误码；如果合法则读取数据库中的对应数据；
  - 业务后台将用户资料返回给客户端。
...
![上传](https://main.qcloudimg.com/raw/4fceb8a9b6aca1d4d8739770e20fe062.png)

  1. 用户在 App 上录制、编辑视频，点击【发布】按键，客户端将向业务后台请求客户端上传签名；
...
11. 业务后台根据发布任务回调中的信息（视频 ID、审核结果等），修改数据库中对应的微信小程序视频发布状态（发布通过或不通过）。
  ...
  媒资管理主要是指对视频信息的增删查改。Demo 中主要涉及的有两个操作：拉取视频列表（分为拉取全局视频、拉取自己上传的视频两种形式）、删除自己上传的视频。如下图所示：

  ![媒资管理](https://main.qcloudimg.com/raw/884c5862795e59b83e72ff92c96f473b.png)
  ...
  1. 用户在【我】界面点击自己上传过的视频，然后点击【删除】按钮，客户端会向业务后台发出删除请求；
  ...
  客户端在拉取视频列表时，已经获取到了视频的 URL 并缓存，如果 VOD 已经开启了 Key 防盗链，那么业务后台返回的视频 URL 会带有防盗链签名。当用户点击视频时，如果 VOD 并未开启 Key 防盗链，或者防盗链签名尚未过期，那么客户端可以直接使用缓存的 URL 进行播放（下图蓝线）。如果开启了 Key 防盗链并且缓存 URL 的防盗链签名已经过期，那么就需要重新获取（下图红线）。

  ![播放视频](https://main.qcloudimg.com/raw/b40fa6f88d8624b594832259fff19b6b.png)
  ...
  业务后台在生成上传签名时指定了一些参数（详见 [上传签名参数]()），以实现短视频场景下的典型业务逻辑。
  ...
  该参数使视频上传完成后，VOD 后台自动对视频发起指定的任务流。该任务流是在部署脚本中自动创建的，其中的任务包括内容审核、截取视频首帧做封面。
  ...
  在 Body 中返回以下形式的 JSON，其中 `RequestId` 是业务后台生成的标识请求的 ID；`Code` 是错误码；`Message` 是错误信息；`Data` 是具体的业务数据（可能为空）。
...
| 参数名称 | 数据类型 | 是否必填 | 说明                                                         |
| -------- | -------- | -------- | ------------------------------------------------------------ |
| Code     | String   | 是       | 小程序前端通过`wx.login`获取登录凭证 code，传递到后端，由后端换取openid, 并作为授权信息保存 |
...
| TokenExpireTime | String   | Token 过期时间，ISO 8601日期格式。                 |
...
| 参数名称    | 数据类型 | 是否必填 | 说明                                                         |
| ----------- | -------- | -------- | ------------------------------------------------------------ |
| Token       | String   | 是       | 用户身份凭证，注册/登录后取得。                              |
| NickName    | String   | 否       | 用户昵称，修改昵称时需填写，全局唯一，由英文字母、数字和下划线组成，4~24个字符。 |
| Avatar      | String   | 否       | 用户头像 ID，修改头像时需填写，不多于32字节。                |
...
| ExpireTime | String   | 上传签名过期时间，ISO 8601日期格式 |
...
| VideoList.Title                   | String   | 视频名称，不多于64字符。                                     |
...
| VideoList.CreateTime              | String   | 创建时间，ISO 8601日期格式。                                 |
| VideoList.AuthorId                | String   | 上传者的用户 ID。                                            |
| VideoList.AuthorNickname          | String   | 上传者昵称。                                                 |
| VideoList.AuthorAvatar            | String   | 上传者头像 ID。                                              |
| VideoList.Width                   | Number   | 视频显示长度，单位px                                         |
| VideoList.Height                  | Number   | 视频显示宽度，单位px                                         |
...
| VideoList                         | Array    | 视频列表。                                                   |
| VideoList.Id                      | String   | 视频 ID，在上传时由 VOD 后台生成，全局唯一。                 |
| VideoList.Title                   | String   | 视频名称，不大于64字符。                                     |
| VideoList.Url                     | String   | 视频播放地址，可带防盗链签名。                               |
| VideoList.UrlExpireTime           | String   | URL 中防盗链签名（如果有）的过期时间，ISO 8601格式。 空字符串表示签名永久有效或未开启防盗链。 |
| VideoList.Cover                   | String   | 封面地址。                                                   |
| VideoList.AnimatedCover           | String   | 动图封面地址。                                               |
| VideoList.CreateTime              | String   | 创建时间，ISO 8601日期格式。                                 |
| VideoList.AuthorId                | String   | 上传者的用户 ID。                                            |
...
| animated_cover             | varchar(256) |          | 视频动图封面 UR。        |

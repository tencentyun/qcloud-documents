## 准备工作
### 步骤1：注册腾讯云账号
打开 [腾讯云官网](https://cloud.tencent.com) 注册腾讯云帐号，并完成实名认证。
### 步骤2：注册小程序账号
...
>? 在内测阶段暂时不收取插件费用，内测结束后具体收费策略会另行通知。

### 步骤5：插件使用
插件提供两个组件，分别为 clip 和 export。
- clip 组件：集成了拍摄、预览、编辑等功能。
- export 组件：提供定制化的导出功能。

>? 其中 clip 和 export 作为插件的出口，由接入方的页面引用来引入完整的插件功能。
...
console.log(plugin)
clip为插件的入口，因此插件内部除导出功能外所用到的参数均从clip传入。
```
### 导出组件

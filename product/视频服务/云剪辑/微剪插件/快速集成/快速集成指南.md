...
### 步骤3：接入组件或者页面[](id:step3)

插件`1.6.0`版本开始，支持通过页面跳转的方式直接跳转至模板页面，原有接入组件方式不变。
#### 3-1：编写页面<span id="step3"></span>
...
```
注意：此方式目前只支持如下方式配置导出页路径，其他的settings暂不支持，若有自定义的需求，请使用上述 3-1 组件的接入方式
```

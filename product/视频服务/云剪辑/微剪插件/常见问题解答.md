...
[](id:cque7)
### 运行微剪对客户端版本和基础库有要求吗？
**iOS 端**要求基础库版本不低于 2.12.0。
**Android 端**要求客户端版本不低于 7.0.18 以及 2.12.2 以上的基础库。
...
[](id:cque11)
### 在微剪选择图片或视频时是否有大小限制？视频选择之后是否经过压缩？
<b>图片选择限制：</b>图片需要小于2048*2048，超过会自动过滤。
<b>视频选择限制：</b>视频是用小程序的`wx.chooseMedia` API 进行选择，选择后自动进行压缩，限制跟随`wx.chooseMedia` API 的限制，微剪未做限制。

[](id:cque12)
### 裁切器组件手柄位置异常或重叠？
请参考裁切器组件的 [使用文档](https://cloud.tencent.com/document/product/1156/50162) 中的 **trackInfo 结构与裁剪示例**部分，手柄位置与下述参数相关，确保参数设置正确。
...
上述参数设置无误，再确认 trackInfo 的 duration 字段的值是否正确，若字段缺失（undefined 或0）请手动纠正即可。
...
```
>!模板功能需要使用文件管理 API 进行包缓存和管理，因此需要调用方小程序暴露接口及文件目录供微剪插件使用。

微剪在1.5.0版本支持转场和动效，可以实现更加平滑的过渡和视频效果，可以用于实现影集模式的视频、图片拼接。

使用快速接入方式接入的

## 转场
### 获取内置转场
获取内置资源方式请参考[高级功能-内置资源](https://cloud.tencent.com/document/product/1156/49439)。
...
```
微剪内置了数十个流畅炫酷的转场效果，开发者可以获取到key值后直接构造数据传递给播放器和导出器，以实现更丰富的编辑效果。
...
在两个片段之间有transition的情况下，假设`transition`的时长为`transitionDuration`秒。
...
获取内置效果的方式请参考[高级功能-内置资源](https://cloud.tencent.com/document/product/1156/49439)，目前支持的动效类型及效果如下：
...
构造一个变焦效果的operation，效果会贯穿整个`clip`播放期间，无需关心时间：
...
let fadeoutOperation = new global.editPage.player.types.ClipOperation({
   ...
   key: "fadeout",
   type: "leave",
   workOnAllSiblings: true, // 设为true则效果生效于整个视频范围，不仅是当前Clip
});
```

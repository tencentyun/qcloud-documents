...
### clip组件
clip组件作为插件快速接入方式的入口组件，只需要将其引用到具体的业务page中即可，如下所示：
...
3.clip组件提供拍摄、相册及模板入口，默认为拍摄。若需要自定义初始模式，比如只保留拍摄和相册入口，且默认跳转相册，可通过注入配置的方式实现，如下所示：

> 具体配置项可参考 [扩展功能](todo.md) 中的配置项说明部分。

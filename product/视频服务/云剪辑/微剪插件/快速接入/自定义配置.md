...
> 注意：用户提供自定义配置时，需要严格遵循上述数据结构，可以缺省某些参数，但是不可以随意修改其名称。插件会提供一份默认的配置项，然后根据用户传入的数据做deep merge。
> 
> ```
...
```
通过配置主题色和按钮默认颜色为"rgb(60,179,113)"，便实现了换肤的功能，颜色相关配置涉及透明度计算，请使用rgb或16进制形式。

<img src="https://cdn-weijian-1258344699.file.myqcloud.com/others/theme.PNG" width="300">
...
```
需要自定素材（增删改）的需求，请参考[自定义素材]() 文档，详细介绍了各种类型素材的自定义方式。

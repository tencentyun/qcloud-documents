...
#### send[](id:send)
云剪组件用 Iframe 的方式嵌入到应用，采用 postMessage 方式与父容器通信，所以`send`必须在 `Iframe`内 云剪辑页面加载成功以后可用（监听到`Editor:Ready`事件以后），否则无效或报错。控制云剪辑的页面行为，实例如下:
```js
  /**
   *
   *    @send 发送命令字方法,
...
```
>? 通过`send` 方法发送**命令字**，命令字相关内容请参阅 [云剪辑指令](https://cloud.tencent.com/document/product/1156/51219)。 
>
#### destroy[](id:destroy)
```js
/**
...
#### on[](id:on)
>? 监听来自云剪辑的用户事件用户事件，与之相对应的有移除事件，了解更多用户事件请参阅 [云剪辑事件](https://cloud.tencent.com/document/product/1156/51218)。

...
#### 调用 SDK 弹出上传面板
开发者前端从后端获取到访问签名后，调用云剪 SDK 填入访问签名并允许弹出上传面板，用户即可以在页面上选择本地文件上传到项目中成为素材：
```js
...
- 对于已经接入云点播且构建了自身的媒资系统的开发者，提供了连接云剪素材体系的渠道。
...
#### 调用 SDK 导入素材
当开发者获取到具体的素材 ID 后，调用云剪前端 SDK 将该素材导入到当前打开的项目中，素材即可出现在面板中展示，用户即可使用素材进行编辑：
```js

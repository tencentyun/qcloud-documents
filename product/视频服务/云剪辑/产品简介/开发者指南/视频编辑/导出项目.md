...
### 流程描述
1. **触发导出事件通知**：App 前端可以通过调用云剪 SDK配置展示 `导出按钮`，当用户在操作云剪编辑页时候点击 `导出按钮`会触发导出事件通知到 App 前端，具体使用请参见 [云剪前端 SDK](https://cloud.tencent.com/document/product/1156/43776?!preview&!editLang=zh)。

- 高级模版，使用 PAG 插件进行制作。利用 Adobe After Effects 软件（后续简称为 AE）可制作出无数种引人注目的动效图形和震撼人心的视觉效果，但生成的文件不支持外部编辑，为了能进行外部编辑，提升设计师创作效率，腾讯研发出 PAG 插件，支持通过 PAG 插件将 AE 文件导出为智能创作平台可识别和使用的图形模版，此模版即为高级模版，模版文件后缀名为 `.pag`。
...
- 确保已经在 AE 上安装了 PA G插件，安装详情请见 [PAG 插件安装](https://cloud.tencent.com/document/product/1156/82274)。

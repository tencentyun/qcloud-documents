## 简介
- 高级模版，使用 PAG 导出插件制作。设计师在AE中制作完成视频后，可以通过 PAG 导出插件导出成技术侧可消费的视频模版。
- PAG 导出插件同时支持**矢量预合成**直接导出和 **BMP 预合成**导出两种方式。两种导出方式适合的使用场景和支持的 AE 功能详见 AE 功能支持列表。

## 前提条件
在开始本教程之前，请您确保已满足以下前提条件：
- 确保已经安装了 AE（Adobe After Effects）（AE2018及以上版本皆可）。
- 确保已经安装 PAGViewer 和 PAG 导出插件。安装详情请见 PAG 导出插件安装。
- **素材准备**：下载 [过场.aep](https://drive.weixin.qq.com/s?k=AJEAIQdfAAoqqgTVnQATwARQb8AOs)，按照本教程内容一起来创作。

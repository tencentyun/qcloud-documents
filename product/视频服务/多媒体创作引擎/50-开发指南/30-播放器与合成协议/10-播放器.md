...
[Web Iframe 接入](TODO: product/视频服务/多媒体创作引擎/3-快速入门/Web Iframe 集成教程/1. 准备工作.md)可以不用关心播放器细节，如果业务需要自行实现剪辑交互，可以使用我们的播放器进行轨道数据播放。

- `window.YJPlayer` 是用于预览[视频合成协议数据](TODO: product/视频服务/多媒体创作引擎/50-开发指南/30-播放器与合成协议/20-合成协议.md)的 Web 播放器组件。
- `window.YJPlayerHelper` 提供辅助函数以便于快速组装[合成协议数据](TODO: product/视频服务/多媒体创作引擎/50-开发指南/30-播放器与合成协议/20-合成协议.md)。

[](id:que1)
###  音视频通话 SDK（TRTC SDK） 的 RoomID 是什么？取值区间值是多少？
RoomID 即房间号，用于唯一标识一个房间。房间号取值区间为1 - 4294967295，由开发者自行维护和分配。

[](id:que2)
###  音视频通话 SDK（TRTC SDK） 进房 UserID 是什么？取值范围是多少？
...
[](id:que7)
###  怎么理解音视频通话 SDK（TRTC SDK）的角色 Role？
...
[](id:que11)
###  音视频通话 SDK（TRTC SDK）最多可以同时创建多少个房间？
...
[](id:que13)
###  音视频通话 SDK（TRTC SDK）的视频服务端最大支持带宽是多少？
没有限制。
...
[](id:que15)
###  音视频通话 SDK（TRTC SDK）开通旁路直播，域名是否需要进行备案使用？
...
[](id:que18)
###  音视频通话 SDK（TRTC SDK）双人视频通话是否支持蓝牙耳机？
支持。
...
[](id:que20)
###  音视频通话 SDK（TRTC SDK）接入 PC 端是否支持屏幕分享功能？
支持，您可以参考如下文档：
...
[](id:que30)
###  音视频通话 SDK（TRTC SDK）是否可以与直播 SDK 通信？
音视频通话 TRTC 与直播 SDK 后台方案架构不同，所以不支持直接相互通信，仅可从 TRTC 后台旁路推流到 CDN。

[](id:que31)
###  音视频通话 SDK（TRTC SDK）进房模式 AppScene 有什么区别？
...
[](id:que32)
###  音视频通话 SDK（TRTC SDK）是否支持音视频通话免提模式？
...
[](id:que37)
###  音视频通话 SDK（TRTC SDK）是否支持类似微信视频通话的悬浮窗、大小画面切换等功能？
...
音视频通话 SDK（TRTC SDK）没有音频和视频通道的区分，只调用 startLocalAudio 不调用 startLocalPreview 时即为纯音频通话模式。

[](id:que39)
###  音视频通话 SDK（TRTC SDK）纯音频通话怎么实现旁路推流和录制？
...
[](id:que40)
###  音视频通话 SDK（TRTC SDK）房间支不支持踢人、禁止发言、静音？
支持。
...
[](id:que44)
###  能用音视频通话 SDK（TRTC SDK）发送自定义消息接口实现聊天室、弹幕等功能吗？
不能，音视频通话 SDK（TRTC SDK）发送自定义消息适用于简单低频的信令传输场景，具体限制请参考 [使用限制](https://cloud.tencent.com/document/product/1449/57127#.E4.BD.BF.E7.94.A8.E9.99.90.E5.88.B6)。

[](id:que45)
###  音视频通话 SDK（TRTC SDK）的播放背景音是否支持循环播放？是否支持调整背景音的播放进度？
...
[](id:que46)
###  音视频通话 SDK（TRTC SDK）有没有房间成员进出房间的监听回调？onUserEnter/onUserExit 是否可以用？
...
[](id:que48)
###  音视频通话 SDK（TRTC SDK）有没有首帧渲染回调？能否监听画面开始渲染，声音开始播放？
支持，通过 onFirstVideoFrame/onFirstAudioFrame 可以监听。

[](id:que49)
###  音视频通话 SDK（TRTC SDK）是否支持视频画面截图功能？
...
[](id:que51)
###  音视频通话 SDK（TRTC SDK）音视频过程中的上下行码率、分辨率、丢包率、音频采样率等信息怎么获取到？
可以通过 SDK 接口 onStatistics() 获取到这些统计信息。

[](id:que52)
###  音视频通话 SDK（TRTC SDK）播放背景音乐接口 playBGM() 是否支持在线音乐？
目前只支持本地音乐，可以先下载至本地再调用 playBGM() 播放。

[](id:que53)
###  音视频通话 SDK（TRTC SDK）是否支持设置本地采集音量？是否支持设置每个远端用户的播放音量？
...
[](id:que66)
###  音视频通话 SDK（TRTC SDK）是什么？
音视频通话 SDK（TRTC SDK）是音视频终端 SDK（腾讯视立方）子产品之一，包含**音视频通话**一个功能模块，与视频产品实时音视频使用相同的底层基础模块。 音视频通话 SDK（TRTC SDK）主打全平台互通的多人音视频通话和低延时互动直播解决方案，致力于帮助开发者快速搭建低成本、低延时、高品质的音视频互动解决方案。

[](id:que67)
###  音视频通话 SDK（TRTC SDK）Demo 怎么体验？
具体请参见 [Demo 体验](https://cloud.tencent.com/document/product/1449/56977)。

[](id:que68)
###  音视频通话 SDK（TRTC SDK）如何快速入门？
音视频通话 SDK（TRTC SDK）为您提供了各个平台的 Demo 源码，您只需花费极少的时间即可快速搭建属于您自己的小应用，具体请参见 [新手入门](https://cloud.tencent.com/document/product/647/49327#.E6.96.B0.E6.89.8B.E5.85.A5.E9.97.A8)。
...
![image](https://qcloudimg.tencent-cloud.cn/raw/6fa1c30fbdef1ba8d40dba183a5f6d74.png)
**具体说明**：

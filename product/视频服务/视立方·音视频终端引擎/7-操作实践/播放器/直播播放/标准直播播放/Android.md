...
### 步骤7：结束播放
结束播放时**需要销毁 view 控件**，尤其是在下次 startLivePlay 之前，否则**会产生大量的内存泄露以及闪屏问题**。

...
腾讯云视立方点播 Flutter 超级播放器项目，请参见 [SuperPlayer Flutter](https://github.com/tencentyun/SuperPlayer/tree/main/Flutter)。 

## 集成指引

1. 在`pubspec.yaml`中增加配置：
```yaml
super_player:
    git:
      url: https://git.code.oa.com/media_cloud_client/media_client_dev
      ref: feature/SuperPlayer_Flutter
      path: platform/flutter/super_player
```
2. 然后更新依赖包。
```yaml
      flutter pub upgrade
...
Android 依赖原生播放器 SDK，把目录下 `example/android/superplayerkit` 文件夹复制到您的工程目录下，在 `setings.gradle` 插入 `include ':superplayerkit'`，当然，您也可以去官网搜索自己的合适版本进行导入。
...
iOS 原生采用 `pod` 方式进行依赖，编辑 `podfile` 文件，指定您的播放器版本：
```xml
...
![](https://main.qcloudimg.com/raw/540866026bf526f404a67f40df31fa7b.png)

### 多清晰度

上面的示例代码只有一种清晰度，如果要添加多个清晰度，也非常简单。以直播为例，打开 [直播控制台](https://console.cloud.tencent.com/live/livemanage)，找到需要播放的直播流，进入详情。

![](https://main.qcloudimg.com/raw/43d681b826018f584d1379c914c64d9e.png)

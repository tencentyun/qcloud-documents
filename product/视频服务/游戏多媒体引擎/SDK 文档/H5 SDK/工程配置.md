...
| Android      | TBS（微信和手机 QQ 的默认 Webview） | 43600                | 微信和手机 QQ 默认内置的浏览器内核为 [TBS](http://x5.tencent.com/)。 |
...
###  3. 进行 H5 鉴权设置
#### 下载程序
单击此链接可以下载我们准备的 authBuffer 示例程序，该程序可以完成对指定的 sdkappid 的鉴权信息签名。
...
>!  由于使用到 async 语法，请确保你的 node 版本在 8 以上。命令行中执行 node -v 以查看版本。

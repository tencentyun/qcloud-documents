...
Tencent Cloud GME SDK의 사용을 환영합니다. Android 개발자가 Tencent Cloud GME 제품 API를 디버깅하고 연결하는 데 도움이 되도록 여기에 Android 개발에 적합한 빠른 액세스 문서를 소개합니다.
...
GME 시작 가이드 문서는 가장 중요한 연결 인터페이스에 대한 정보만 제공하고 더 많은 세부 인터페이스는 [관련 인터페이스 문서](https://cloud.tencent.com/document/product/607/15210)를 참조하십시오.
...
| openID    		|OpenID는 Int64 유형(string으로 전환 후 가져오기)만 지원하고 반드시 10000보다 크며 사용자 ID 식별에 사용합니다.|
...
|매개변수     |유형         |의미|
| ------------- |:-------------:|-------------|
| isEnabled    |boolean     |마이크를 꺼야 하는 경우 매개변수 FALSE를 가져오고, 마이크를 켜야 하는 경우 매개변수 TRUE를 가져옵니다.|
####  인스턴스 코드  
```
...
| roomId    		|String   	|방 번호, 최대 127자 지원(오프라인 음성 방 번호 매개변수는 반드시 0을 써야 함)|

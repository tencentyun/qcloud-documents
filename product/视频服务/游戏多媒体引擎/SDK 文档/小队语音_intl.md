...
| ------------- |:-------------:|-------------
| roomId		|int    		| Indicates the room number, which is limited to 127 characters |
| roomType | ITMG_ROOM_TYPE | Indicates the audio type of the room |
...
| ------------- |------------ | ---- |---- |---- |---- |
| ITMG_ROOM_TYPE_FLUENCY | Fluent | 1 | With high fluency and ultra-low delay, it is suitable for team speak scenarios in such games as FPS and MOBA. | Speaker: voice volume; headphone: media volume | 16k (if there is no special requirement for sound quality) |
| ITMG_ROOM_TYPE_STANDARD | Standard | 2 | With good sound quality and medium delay, it is suitable for voice chat scenarios in casual games such as Werewolf and board games. | Speaker: voice volume; headphone: media volume | 16k or 48k, depending on the requirement for sound quality |
| ITMG_ROOM_TYPE_HIGHQUALITY | HD | 3 | With ultra-high sound quality and high delay, it is suitable for music and voice social Apps, and scenarios demanding high sound quality, such as music playback and online karaoke. | Speaker: media volume; headphone: media volume | 48k is recommended to ensure the best effect |
...
### Team chatting room
#### Description:
Team chatting: Players form a team before the game starts, and the team chat can only be heard by team members;
...
#### Suppose player A is in the global chatting mode, the interaction status between player A and player B varies with the scenario and player B's voice chat mode, as shown below:
...
#### Suppose player A is in the team chatting mode, the interaction status between player A and player B varies with the scenario and player B's voice chat mode, as shown below:
...
| ------------- |:-------------:|-------------
| roomId		|int    		| Indicates the room number, which is limited to 127 characters |
| roomType | ITMG_ROOM_TYPE | Indicates the audio type of the room (only 1 is allowed) |
| authBuffer	| Byte[] | Authentication data |
| teamId | int | The ID of the team that enters the room (0 is not allowed) |
| audioMode | int | 0: global chatting; 1: team chatting |
...
| Parameter | Type | Description |
| ------------- |:-------------:|-------------
| Type_AudioMode | int | 0: global chatting; 1: team chatting |
...
| Parameter | Type | Description |
| ------------- |-------------|-------------
| range | int | Maximum range for receiving audio |

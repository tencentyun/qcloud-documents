...
请使用 VS2015 打开。如果需要用 VS2010 打开的话请自行对工程进行降级。
...
Unity 的 Demo 中设置了  OnApplicationFocus 当程序失去焦点时，会 Pause 暂停语音，如果需要后台可播放声音的话，请将调用 Pause 接口的相关代码去掉。
...
如果是 Unity 引擎，在 PC 端开发的话，可以尝试去路径 %appdata%\Tencent\GME\Unity.exe 下找到日志。

在 iOS 真机上，可以通过让应用程序支持文件共享来取得日志，具体步骤如下：
1. 在应用程序的 Info.plist 文件中添加 UIFileSharingEnabled 键，并将键值设置为 YES。
2. 将您希望共享的文件放在应用程序的 Documents 目录。
3. 一旦设备插入到用户计算机，iTunes 就会在选中设备的 Apps 标签中显示一个 File Sharing 区域。
4. 此后，用户就可以向该目录添加文件或者将文件移动到桌面计算机中。

**日志等级**

提供日志时候如果有调用过 SetLogLevel 设置过日志等级，建议恢复默认日志等级。

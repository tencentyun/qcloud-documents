...
建议升级 Unity 版本，详情请参见 [Unity 官网链接](https://forum.unity.com/threads/undefined-symbols-for-architecture-armv7-query_call_back-callback_func_type.830544/#post-5590516)。
如果您无需升级，则不打包 armv7 架构即可。
...
如果使用过程中有 `Found plugins with same names and architectures` 类似报错，因为 GME SDK 默认提供 x86 和 x86_64 架构的 SDK 版本，请在 plugins 文件夹中删除其中一份。
...
先检查导出来的 apk 里面是否包含 x86 的库文件，如果没有请重新下载 sdk 后导入 x86 架构 sdk 文件后再导出可执行文件。

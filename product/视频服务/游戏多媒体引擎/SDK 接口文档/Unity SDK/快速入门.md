...
|错误码名称|错误码值|原因及建议方案|
|--------|-------|------------|
|AV_ERR_AUTH_FIALD         |7006|鉴权失败 有以下几个原因：<li>AppID 不存在或者错误。<li>authbuff 鉴权错误。<li>鉴权过期。 <li>openID不符合规范。|
|AV_ERR_IN_OTHER_ROOM      |7007|已经在其它房间。|
|AV_ERR_REPEATED_OPERATION  |1001   |已在进房过程中，又重复了此操作。建议在进房回调返回前，不要再调用进房接口。|
|AV_ERR_HAS_IN_THE_STATE    |1003   |已进入房间，又调用一次进房接口。|

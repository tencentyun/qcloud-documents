...
### 流式语音识别的回调
启动流式语音识别后，需要在回调函数 OnEvent 中监听回调消息，事件消息分为两个，ITMG_MAIN_EVNET_TYPE_PTT_STREAMINGRECOGNITION_COMPLETE 是在停止录制并完成识别后才返回文字，相当于一段话说完才会返回识别的文字。

另一个消息为 ITMG_MAIN_EVNET_TYPE_PTT_STREAMINGRECOGNITION_IS_RUNNING ，这个消息在录音过程中就会实时返回识别到的文字，相当于边说话边返回识别到的文字。
...
| type    |ITMG_VOICE_TYPE                      |变声类型，请参考[变声特效](https://cloud.tencent.com/document/product/607/34378#.E5.8F.98.E5.A3.B0.E7.89.B9.E6.95.88)|

...
- 以 Unity 为例，建议将3D文件放在项目的 StreamingAssets 目录下，并参考 SampleCode 中 **copyFileFromAssetsToPersistent** 函数写法，将资源文件拷贝到不同平台的相应目录中。
- 以 Unreal 为例，参考 SampleCode 中 **CopyAllAssetsToExternal** 函数写法，将3D模型文件拷贝后再读取路径。
  </dx-alert>

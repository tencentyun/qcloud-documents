## 2020年01月

<table>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th>
</tr><tr>
<td>资源包使用情况查询功能上线</td>
<td>支持通过资源包进行使用情况、购买时间、到期时间以及使用状态监控，统计分析资源包具体使用情况。</td>
<td>-</td>
</tr><tr>
<td>极速高清转码、标准转码资源包计费上线</td>
<td>标准转码/极速高清转码功能支持预付费资源包计费方式。</td>
<td><a href="https://cloud.tencent.com/document/product/267/39889">标准直播预付费计费说明</a></td>
</tr><tr>
<td>新增快直播服务计费说明</td>
<td>快直播服务计费说明</td>
<td><a href="https://cloud.tencent.com/document/product/267/39172">快直播价格总览</a></td>
</tr>
</table>

## 2019年12月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>新增域名租赁功能</td>
<td>腾讯云为没有准备域名和域名未备案完成的用户提供域名租赁功能。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20381">域名租赁</a></td>
</tr></table>

## 2019年07月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.4.3版本</td>
<td>支持自定义源站。</td>
<td><a href="https://cloud.tencent.com/document/product/267/37155">源站设置</a></td>
</tr></table>

## 2019年06月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.4.2版本</td>
<td>接入云监控，支持通过云监控进行流量/带宽/在线人数的监控告警。<br>统计分析支持海外分区统计。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20392">运营数据</a></td>
</tr></table>

## 2019年05月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.4.1版本</td>
<td>接入标签管理，支持对直播域名设置标签。</td>
<td><a href="https://cloud.tencent.com/document/product/267/40935">域名管理</a></td>
</tr></table>

## 2019年04月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.3.4版本</td>
<td>日志管理，含实时日志分析和日志下载。</td>
<td><a href="https://cloud.tencent.com/document/product/267/33996">实时日志分析</a></td>
</tr><tr>
<td>发布V5.3.3版本</td>
<td>慢直播功能上线。</td>
<td><a href="https://cloud.tencent.com/document/product/267/39171">慢直播价格总览</a></td>
</tr><tr>
<td>发布V5.3.2版本</td>
<td>旧版控制台自助切换新版。
支持纯音频转码设置。</td>
<td><a href="https://cloud.tencent.com/document/product/267/34347">如何切换新版控制台</a></td>
</tr></table>

## 2019年03月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.3.1版本</td>
<td>统计分析接口升级 API 3.0。<br>支持设置带宽封顶阈值。</td>
<td><a href="https://cloud.tencent.com/document/product/267/35971">直播计费带宽和流量数据查询</a></td>
</tr><tr>
<td>发布V5.3.0版本</td>
<td>云直播 CAM 子账号上线，支持接口级别权限管理。<br>支持云直播只读策略和全读写策略。</td>
<td><a href="https://cloud.tencent.com/document/product/267/34301">CAM 访问控制</a></td>
</tr><tr>
<td>发布V5.2.4版本</td>
<td>新版控制台全量上线。<br>回调配置可设置回调 Key。<br>优化录制模板。</td>
<td><a href="https://cloud.tencent.com/document/product/267/35251">录制配置</a></td>
</tr></table>

## 2019年02月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.2.3版本</td>
<td>新增地址生成器工具。<br>推流和播放支持二维码扫描。<br>国际站控制台全量开放。</td>
<td><a href="https://cloud.tencent.com/document/product/267/35257">地址生成器</a></td>
</tr></table>

## 2019年01月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.2.2版本</td>
<td>API 接口升级3.0。<br>优化云直播控制台。<br>新增断流诊断工具。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20455">API 简介</a></td>
</tr><tr>
<td>发布V5.2.1版本</td>
<td>新增功能模板管理（转码、录制、截图、鉴黄及水印）。<br>支持域名粒度模板配置。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20384">录制配置</a></td>
</tr></table>

## 2018年12月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.2.0版本</td>
<td>新版控制台灰度上线。<br>支持流管理。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20380">流管理</a></td>
</tr><tr>
<td>发布V5.1.4版本</td>
<td>支持自助配置播放鉴权。</td>
<td><a href="https://cloud.tencent.com/document/product/267/32463">播放鉴权配置</a></td>
</tr><tr>
<td>发布V5.1.3版本</td>
<td>支持自助配置 HTTPS 功能。</td>
<td><a href="https://cloud.tencent.com/document/product/267/32826">HTTPS 配置</a></td>
</tr><tr>
<td>发布V5.1.2版本</td>
<td>支持自助开通海外加速和全球加速。</td>
<td><a href="https://cloud.tencent.com/document/product/267/14621">直播海外服务介绍</a></td>
</tr></table>

## 2018年11月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.1.1版本</td>
<td>接入方式默认直播码。</td>
<td><a href="https://cloud.tencent.com/document/product/267/14436">频道模式和直播码模式 ID 删除相关问题</a></td>
</tr></table>

## 2018年09月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V5.1.0版本</td>
<td>新增域名管理，支持自定义推流域名和播放域名。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20381">添加自有域名</a></td>
</tr><tr>
<td>发布V5.0.0版本</td>
<td>优化云直播控制台。</td>
<td>-</td>
</tr></table>

## 2018年08月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V4.1.2版本</td>
<td>优化云直播海外申请开通服务。</td>
<td><a href="https://cloud.tencent.com/document/product/267/14621#.E5.BC.80.E9.80.9A.E6.96.B9.E5.BC.8F">直播海外服务介绍</a></td>
</tr></table>

## 2018年05月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V4.1.1版本</td>
<td>优化回调配置和水印配置功能。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20387">水印配置</a></td>
</tr><tr>
<td>发布V4.1.0版本</td>
<td>支持数据概览。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20379">控制台概览</a></td>
</tr></table>

## 2018年03月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V4.0.0版本</td>
<td>极速高清功能上线。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20385">转码配置</a></td>
</tr></table>

## 2017年12月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V3.0.0版本</td>
<td>支持 P2P 加速。</td>
<td>-</td>
</tr></table>

## 2017年08月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V2.2.0版本</td>
<td>直播房间监控。</td>
<td>-</td>
</tr><tr>
<td>发布V2.1.2版本</td>
<td>新增直播录制和点播存储功能。</td>
<td><a href="https://cloud.tencent.com/document/product/267/32739">直播录制</a></td>
</tr></table>

## 2017年05月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>上线 HLS 直播自动拼接功能</td>
<td>对于 HLS 协议而言，用户可以设置一个续流超时时间，主播推流中断后，如果在规定时间内重新推流，断流前后会录制成同一份文件，无需客户调用接口将两个文件拼成一份。</td>
<td><a href="https://cloud.tencent.com/document/product/267/32739#.E8.87.AA.E5.8A.A8.E6.8B.BC.E6.8E.A5.E5.BD.95.E5.88.B6.EF.BC.88.E5.A4.9A.E6.AC.A1.E6.8E.A8.E6.B5.81.E7.BB.AD.E6.B5.81.E5.BD.95.E5.88.B6.EF.BC.89">直播录制</a></td>
</tr></table>

## 2017年04月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>上线直播 HLS 码率自适应功能</td>
<td>针对 HLS 协议的播放实现码率自适应功能，根据用户所处终端网络环境质量，获取最合适的适码率，在保证视频能流畅观看的同时，尽可能的让网络顺畅的用户看到高清、甚至超清的视频。</td>
<td><a href="https://cloud.tencent.com/document/product/267/2823">产品功能</a></td>
</tr></table>

## 2017年02月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V2.1.1版本</td>
<td>截图和录制回调。</td>
<td><a href="https://cloud.tencent.com/document/product/267/35254">回调配置</a></td>
</tr></table>

## 2016年12月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V2.1.0版本</td>
<td>直播流鉴黄和处理。</td>
<td><a href="https://cloud.tencent.com/document/product/267/32741">直播鉴黄</a></td>
</tr></table>

## 2016年09月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V2.0.0版本</td>
<td>直播流处理功能上线。<br>CDN 加速优化更新。</td>
<td><a href="https://cloud.tencent.com/document/product/267/32722">直播海外相关问题</a></td>
</tr></table>

## 2016年08月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V1.1.3版本</td>
<td>上线质量监控数据展示功能。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20382">统计分析</a></td>
</tr><tr>
<td>发布V1.1.2版本</td>
<td>上线频道创建功能。</td>
<td><a href="https://cloud.tencent.com/document/product/267/31157">频道托管</a></td>
</tr></table>

## 2016年06月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V1.1.1版本</td>
<td>支持直播录制和截图。</td>
<td><a href="https://cloud.tencent.com/document/product/267/32739">直播录制</a></td>
</tr></table>

## 2016年01月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V1.1.0版本</td>
<td>新增统计分析功能。</td>
<td><a href="https://cloud.tencent.com/document/product/267/20390">增值功能</a></td>
</tr></table>

## 2015年10月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V1.0.1版本</td>
<td>支持控制台申请接入。</td>
<td><a href="https://cloud.tencent.com/document/product/267/13551">快速入门</a></td>
</tr></table>

## 2015年07月

<table><tr>
<th width="25%">动态名称</th><th width="55%">动态描述</th><th width="20%">相关文档</th></tr><tr>
<td>发布V1.0.0版本</td>
<td>支持直播 CDN 加速。</td>
<td><a href="https://cloud.tencent.com/document/product/267/32722">直播海外相关问题</a></td>
</tr></table>










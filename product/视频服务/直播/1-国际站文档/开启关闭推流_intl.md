...
- **API**
  - **Live_Channel_SetStatus**: This API is used to enable or disable the **pushable** state of an LVB stream.

- **URL**
  - URL for calling API: http://<font color='red'>fcgi.</font>video.qcloud.com/common_access

- **Purpose**
  - This API is used to **ban** an LVB during porn detection. For example, if a VJ is found to play porny or rebellious content, this LVB stream can be disabled at any time.

- **Note**
  - Once an LVB stream is "disabled", Tencent Cloud actively disconnects the push linkage and rejects subsequent push requests from the VJ.
...
| Param.n.status | Status |  int | 0: Disabled; 1: Enabled |  Y|

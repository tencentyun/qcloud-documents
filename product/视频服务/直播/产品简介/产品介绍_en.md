...
#### Pusher and Player SDKs on Mobile Device
...
Push end: All RTMP push devices are supported to push videos to LVB system using Apps integrated with Tencent self-developed LVB SDKs or LVB software from third-parties.
Playback end: Apps integrated with Tencent self-developed LVB SDKs or players from third-parties are supported.
Management components: Users can manage LVB events through API and the console, and query relevant statistical data.
Distribution components: Fast and flexible playback methods are provided for LVB users via the CDN system across the country. Users can use self-developed player SDKs or those provided by Tencent Cloud to integrate with their own business. Users can also watch HLS LVB directly through H5 page.

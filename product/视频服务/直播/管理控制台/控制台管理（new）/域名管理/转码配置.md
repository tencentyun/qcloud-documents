...
**例如：**原播放地址为`http://domain/AppName/StreamName.flv`，domain 关联的转码模板名称为`hd`。若您需获取播放转码后的视频，则需前往【[地址生成器](https://console.cloud.tencent.com/live/addrgenerator/addrgenerator)】录入 StreamName 名称为`XXXX_hd`，生成新的转码播放地址为`http://domain/AppName/XXXX_hd.flv`。

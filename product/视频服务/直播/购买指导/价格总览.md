...
### 录制费用
腾讯云直播服务向每个帐号提供5个免费的频道用于录制。若您的产品需要使用更多的录制频道，我们将收取 30元/路/月 的录制费用（按月并发录制频道峰值路数计费）。另，每50M并发带宽将减免抵扣1个录制频道费用（例：本月带宽为500M，则将减免10个录制频道的费用，加上提供的5个免费录制频道，您共可以减免15个录制名频道的录制费用）。
### 截图鉴黄费用
...
2.报价：
1）截图报价：0.1元／千张
2）图片鉴黄（确定部分）：1.1元／千张； (无需人工审核，算法部分)

...
>?  系统默认提供测试域名`数字.livepush.myqcloud.com`，您可通过该域名进行推流测试，但强烈不建议您在正式业务中使用这个域名作为推流域名。
...
>?添加成功后，系统会为您自动分配一个 CNAME 域名，您可通过[【域名管理】](https://console.cloud.tencent.com/live/domainmanage)的域名列表进行查看。CNAME 域名不能直接访问，您需要在域名服务提供商处完成 CNAME 配置，配置生效后，即可享受云直播服务。

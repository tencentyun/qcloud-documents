...
## 查看 SDK 状态提示信息
如果您使用的是腾讯云移动直播 SDK 来推流，该 SDK 提供了一种状态反馈机制，每隔2秒就会将内部各种状态参数反馈出来，您可以通过注册 [V2TXLivePusherObserver](https://cloud.tencent.com/document/product/454/34770) 监听器，然后通过回调函数 onStatisticsUpdate 来获取这些状态。V2TXLivePusherStatistics 相关状态的说明如下：
...
- **2.2 确认谁在消耗 CPU**
一款直播 App 中使用 CPU 的不可能只有推流 SDK，弹幕、飘星、文本消息互动等都有可能会消耗一定的 CPU，这些都是不可避免的。如果单纯要测试推流 SDK 的 CPU 占用情况，可以使用我们的 [工具包 DEMO](https://cloud.tencent.com/document/product/454/6555) 来观察和评估。
...
### 2. 合理的编码设置
如下是我们推荐的编码设置（更多信息请参见 [设定画面质量](https://tcloud-doc.isd.com/document/product/454/56600?!preview&!editLang=zh)），可以通过 V2TXLivePusher 里的 [setVideoQuality](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__V2TXLivePusher__android.html#a2695806cb6c74ccce4b378d306ef0a02) 接口进行相应档位的设置：
...
### 2. 针对性优化方案
为了能够让您无需了解过多流控处理知识就能优化出较好的播放体验，腾讯云移动直播 SDK 经过多个版本的改进，优化出一套自动调节技术，并在其基础上推出了三种比较优秀的 [延迟控制方案](https://tcloud-doc.isd.com/document/product/454/56598?!preview&!editLang=zh)，可以通过 V2TXLivePlayer [的setCacheParams](https://liteav.sdk.qcloud.com/doc/api/zh-cn/group__V2TXLivePlayer__android.html#a8a4f8f8e220a6e4aa2a04ca3e866efcb) 来设置：

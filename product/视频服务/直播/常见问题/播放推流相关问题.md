...
<span id="que9"></span>
### 直播推流地址最长时效时间是多久?
没有限制，推流地址设置有效时间主要是为了防盗鉴权，不过您可以自己通过拼接规则来生成更长有效期的地址，具体拼装方式请参见 [自主拼装直播 URL 相关问题](https://cloud.tencent.com/document/product/267/32720) 。
...
<span id="que11"></span>
### 为什么不能创建固定的推流地址？
不能，如果创建一个固定的推流地址，会有可能出现盗推的现象，这个是会给您业务造成很高的损失的。
...
<span id="que13"></span>
###  直播时是否可以设置最高码率?
 不可以的，这是推流端自行设置的，但是推流端设置最高码率也要根据你网络的上传速度来决定。码率（或称最大比特率），上限就是你的网络上传速度，如果设太高就会使直播画面丢帧卡顿。 

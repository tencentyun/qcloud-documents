...
<span id="que9"></span>
### 直播录制路数峰值如何计算？
在一场直播过程中只录制一种格式文件，即为一路，一个月内产生的最高的录制路数即为直播录制路数峰值。
**示例：**
<style>#ye{background:#ffe699;}#gr{background:#c6e0b4;}#br{background:#bdd7ee}</style>
<table id="rroad">
<tr><th rowspan=2 width="10%">域名ID（每个域名下<br>只有一路直播推流）</th>
...
- 若需使用标准直播流量资源包抵扣，首先请在 [云直播控制台](https://console.cloud.tencent.com/live/livestat) 确认当前的计费方式是否为为日结流量计费。

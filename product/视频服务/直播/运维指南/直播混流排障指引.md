...
腾讯云云直播混流，若非背景流断开，断掉的流画面会停在最后一帧，若背景流断开，则整个画面都会卡住。在15分钟内该流以同一流 ID 重新推流成功，则自动恢复混流。若两条流均断开，15分钟后混流自动取消。
...
[](id:error4)
#### 错误4：`"Message":"input stream num is not match the template id!"`
...
[](id:error6)
#### 错误6：`"Message":"InnerErrCode:[ -2 ],InnerErrMsg: [ small picture out of the background ]"`

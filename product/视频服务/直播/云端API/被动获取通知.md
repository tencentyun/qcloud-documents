...
|channel_name|	频道名称|	string	|qq空间直播需要|	N|
|play_info|	播放url信息	|array|	qq空间直播需要|	N|
|push_url|推流url|string|Qq空间直播需要|N|
|node	|Upload接入点的ip	|String|	Tgw ip|	Y|
|user_ip	|用户推流ip|	String	|Client_ip	|Y|
|metadata|	推流元信息	|Array|	斗鱼需求，event_type为5|	N|
|errcode	|推流错误码	|Int	|	|N|
|errmsg	|推流失败信息	|String	|	|N|
...
|file_id|file_id	|string|	旧系统没有该字段|	Y|
|file_format	|文件格式	|string|	flv, hls, mp4，旧系统没有该字段	|Y|
...
腾讯云后台目前的消息可靠性保证机制是基于**简单重传**实现的，即：<font color='blue'>如果一条通知消息没有成功发送到您指定的回调URL，腾讯云会反复重试100次。</font> 

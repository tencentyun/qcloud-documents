...
2. 单击【云产品】，选择【管理与审计】栏下的[【访问管理】](https://console.cloud.tencent.com/cam/overview)，进入“访问管理”页面。
3. 选择【访问密钥】>【[API密钥管理](https://console.cloud.tencent.com/capi)】进入“API密钥管理”页面，单击【新建密钥】即可以创建一对 SecretId/SecretKey，每个帐号最多可以拥有两对 SecretId/SecretKey。
![](https://main.qcloudimg.com/raw/fd081bd2fa3c24ec0edcdd11730233ba.png)

## 2. 生成签名串 
有了安全凭证 SecretId 和 SecretKey 后，就可以生成签名串了。下面给出了一个生成签名串的详细过程。
假设用户的 SecretId 和 SecretKey 分别是：
- SecretId: `AKIDz8kr**********************nPhESA` 
- SecretKey: `Gu5t***********************zk1qA`
...
| SecretId | 密钥 ID | `AKIDz8kr**********************nPhESA`  | 
...
由上表可以看出，请求参数中的公共请求参数只有5个：Action、SecretId、Timestamp、Nonce 和 Region，而不是在 [公共请求参数](https://cloud.tencent.com/document/api/213/6976) 中所述的6个。事实上，第6个参数 Signature（签名串）正是由其他参数（包括指令请求参数）共同生成的，具体步骤如下：
### 2.1. 对参数排序
...
```
Action=DescribeInstances&Nonce=11886&Region=gz&SecretId=AKIDz8kr**********************nPhESA&Timestamp=1465185768&instanceIds.0=ins-09dx96dg&limit=20&offset=0
```
...
则示例的拼接结果为：
```
GETcvm.api.qcloud.com/v2/index.php?Action=DescribeInstances&Nonce=11886&Region=gz&SecretId=AKIDz8kr**********************nPhESA&Timestamp=1465185768&instanceIds.0=ins-09dx96dg&limit=20&offset=0
```
...
```
$secretKey = 'Gu5t***********************zk1qA';
$srcStr = 'GETcvm.api.qcloud.com/v2/index.php?Action=DescribeInstances&Nonce=11886&Region=gz&SecretId=AKIDz8kr**********************nPhESA&Timestamp=1465185768&instanceIds.0=ins-09dx96dg&limit=20&offset=0';

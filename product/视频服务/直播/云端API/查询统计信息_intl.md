...
- **API**
   ...
   - **Get_LivePushStat**: This API is used to return only the push statistics to improve query efficiency
   - **Get_LivePlayStat**: This API is used to return only the playback statistics to improve query efficiency
- **URL**
   - URL for calling API: http://<font color='red'>statcgi.</font>video.qcloud.com/common_access
- **Purpose**
...
- **Note**
   ...
   - <font color='red'>If the target stream is not in broadcasting, the output field in the returned result is blank.</font>
...
- **BETA**
   - The statistic API is still in Beta and is not open to all customers. If you call this API when it is not activated, you will receive a prompt "<font color='red'>cmd is invalid</font>". Contact us if there's an urgent need.
...
| Param.s.stream_id | LVB Code |  string |<font color='red'>If stream_id is not set, all streams that are in LVB will be queried</font> |  N   |
...
| --------------- | ---------------- | ------ | ------------------ |
| stream_count    | Total number of the online LVB streams | int    | 1 is returned when stream_id is specified in the request |

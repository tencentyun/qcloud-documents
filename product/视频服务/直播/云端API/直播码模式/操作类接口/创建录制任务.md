...
|Param.n.task_sub_type  | 是否开启实时视频录制| int| 1：开启；0：关闭，推荐使用实时视频录制。<br>a. 实时视频录制创建成功依赖主播活跃推流，调用接口成功则同步开始录制，并忽略任务开始时间参数；<br>b. 实时视频录制任务时长支持最大为30分钟，如果传入任务结束时间与当前时间差大于30分钟，则按30分钟计算。录制时长建议控制在5分钟以内；<br>c. 如果关闭实时录制，则需要传入任务开始时间参数，且结束时间与开始时间差不大于1天。|Y|

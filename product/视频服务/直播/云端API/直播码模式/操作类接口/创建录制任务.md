...
|Param.n.task_sub_type  | 是否开启实时视频录制| int| 建议使用实时视频录制，默认 1。<br>（1）实时视频录制创建成功依赖主播活跃推流，调用接口成功则同步开始录制，此时如果传入任务开始时间参数，任务开始时间参数无效。<br>（2）开启实时视频录制的同时如果传入了任务结束时间，则按照任务结束时间结束录制。若没传入，则 30 分钟后自动结束录制。<br>（3）实时录制任务开始时间与任务结束时间超过 30 分钟，则 30 分钟后会自动结束录制，实时视频录制建议控制在 5 分钟以内。|Y|

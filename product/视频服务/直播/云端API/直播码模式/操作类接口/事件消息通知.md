...
> 
其中 cos_appid 和 cos_bucketname 是您在腾讯云开通 [对象存储服务](https://console.cloud.tencent.com/cos)。
2.回调新信息中增一个图片URL 字段`pic_full_url`以便您直接得到完整的图片 URL，为了不影响原有业务，原有回调信息字段均无变化（即 pic_url 字段依旧存在）。

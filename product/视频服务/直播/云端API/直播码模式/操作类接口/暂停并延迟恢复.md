...
- **说明**
  - 调用该接口可以暂停推某路流（即禁止推流）。如果要恢复主播推流，可再次调用该接口，或者设置一个恢复时间，到达指定时间后，允许推流。最长禁止推流3个月，即禁止推流截止时间最长为当前时间往后3个月，如果超过3个月，则以3个月为准。
...
| Param.n.abstime_end | 禁播截止的时间戳 | int | 禁播截止的绝对时间，请填写UNIX时间戳(十进制)，系统最多禁播三个月。
| Y|

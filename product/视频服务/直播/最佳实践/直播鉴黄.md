## 直播鉴黄
开通直播鉴黄，首先要开通直播截图功能，用户可以通过[云直播控制台](https://cloud.tencent.com/document/product/267/20386)或直播截图鉴黄 API 来实现，本文主要介绍如何通过直播截图鉴黄 API 来实现直播鉴黄功能。

### 一、开通直播鉴黄

由于直播鉴黄是基于直播截图的，因此开通直播鉴黄必须开通直播截图功能，具体操作如下：
...
调用 CreateLiveSnapshotRule，创建带鉴黄功能的直播截图规则，将第 1 步创建的直播截图模板 ID 关联到需要鉴黄的直播 AppId、DomainName、AppName、StreamName 维度下。
...
在开通了直播鉴黄功能后，用户可在鉴黄回调模板中配置注册的回调域名，则腾讯云直播后台会将鉴黄结果回调给用户。

具体操作如下：

#### 1、创建直播鉴黄回调模板

调用 CreateLiveCallbackTemplate，设置 PornCensorshipNotifyUrl 参数为用户的域名，创建直播鉴黄回调模板。
...
直播后台会将鉴黄结果通过 HTTP POST 请求发送到用户注册的域名，鉴黄结果以 JSON 格式存放在 HTTP Body 中，用户可以只通过 confidence 字段来判断直播是否涉黄。
...
| gameDetails | 选填 | Object |   |

AbductionRisk介绍

| **参数** | **是否必填** | **数据类型** | **描述** |
| --- | --- | --- | --- |
| level | 必填 | Number | 风险等级范围 0~4，数字越大风险越高，3 和 4 表示恶意，建议进行处理 |
...
签名计算：`Signature = base64\_encode(hash\_hmac('SHA1', $Body, $SecretKey, true))`，SecretKey 为 SecretId 对应的 SecretKey。
...
（1）调用 DeleteLiveSnapshotRule，通过模板 ID 删除 DomainName、 AppName、StreamName 下对应直播截图规格。

（2）调用 ModifyLiveSnapshotRule，修改 AppName、StreamName 下对应模板规则的直播模板 ID 为未开通鉴黄功能的直播模板 ID。

#### 2、删除或修改鉴黄截图模板来实现

调用 ModifyLiveSnapshotTemplate 修改直播模板的鉴黄标志为 0。

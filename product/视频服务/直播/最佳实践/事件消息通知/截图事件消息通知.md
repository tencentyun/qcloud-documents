直播截图是以固定的时间间隔截取实时直播流的图像，并生成图片存储到 COS 中。而截图回调则是用于推送截图存储图片文件信息，主要包括图片生成事件、图片大小、所在路径以及可下载地址。你需在在回调模板中配置截图回调消息接收服务器地址，并将该模板与推流域名进行关联。当直播流触发截图事件后，腾讯云直播后台会将截图信息回调到您设置的接收服务器中。

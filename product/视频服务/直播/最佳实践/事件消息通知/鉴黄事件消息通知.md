...
| type | 必填     | Array                 | 图片类型，暂时只有一个值，与恶意标签 label 对应。取值如下：<ul style="margin:0"><li/>0：正常图片<li/>1：色情图片<li/>2：性感图片<li/>3：涉政<li/>4：违法<li/>5：暴恐<li/>6：谩骂<li/>7：青少年不适宜<li/>8：广告</ul> |
| score | 必填     | Array                 | type 对应的评分                                              |
| confidence | 必填     | Number                | <li/>涉黄置信度，和 pornScore 相同，取值范围 0 - 100 <br>（**兼容老接口，新接口待屏蔽**） |
| normalScore | 必填     | Number                | <li/>图片为正常图片（非涉黄和性感）的评分，取值范围：100 –  `max(hotScore, pornScore)`<br>（**兼容老接口，新接口待屏蔽**） |
...
| label | 必填     | string                | 恶意标签，与 type 对应，取值可选：<ul style="margin:0"><li/>Normal：正常<li/>Polity：涉政<li/>Porn：色情<li/>Sexy：性感<li/>Illegal：违法<li/>Abuse：谩骂<li/>Terror：暴恐<li/>Ad：广告</ul> |
...
| 名称   | 类型                 | 描述                                                     |
| ---------- | ------------------------ | ------------------------------------------------------------ |
| Scene      | String | 场景识别结果，取值可选：<ul style="margin:0"><li/>Porn：色情<li/>Sexy：性感<li/>Terror：暴恐<li/>Illegal：违法违规<li/>Polity：涉政</ul> |
| Suggestion | String | 建议值，取值可选：<ul style="margin:0"><li/>Block：打击<li/>Review：待复审<li/>Pass：正常</ul> |
| Label      | String | 审核标签，取值可选：<ul style="margin:0"><li/>Normal：正常<li/>Polity：涉政<li/>Porn：色情<li/>Illegal：违法<li/>Terror：暴恐</ul> |
...
| 名称   | 类型           | 描述                                                     |
| ---------- | ------------------ | ------------------------------------------------------------ |
| Scene      | String | 场景识别结果，固定取值 Similar                               |
| Suggestion | String |建议值，取值可选：<ul style="margin:0"><li/>Block：打击<li/>Review：待复审<li/>Pass：正常 </ul> |
| Label      | String | 标签：<ul style="margin:0"><li/>Normal：正常<li/>Polity：涉政<li/>Porn：色情<li/>Illegal：违法<li/>Abuse：谩骂<li/>Terror：暴恐<li/>Ad：广告</ul> |
| SubLabel   | String             | 子标签名称                                                   |
...
| 名称   | 类型              | 描述                                                     |
| ---------- | --------------------- | ------------------------------------------------------------ |
| Scene      | String                | 场景识别结果，取值可选：<ul style="margin:0"><li/>PolityLogo：涉政实体<li/>PolityFace：涉政人物<li/>FaceAttribute：人物属性<li/>OCR：OCR 图片<li/>QrCode：二维码</ul> |
| Suggestion | String                | 建议值：<ul style="margin:0"><li/>Block：打击<li/>Review：待复审<li/>Pass：正常 </ul>|
| Label      | String                | 标签，取值可选：<ul style="margin:0"><li/>Normal：正常<li/>Polity：涉政<li/>Porn：色情<li/>Illegal：违法<li/>Abuse：谩骂<li/>Terror：暴恐<li/>Ad：广告</ul> |
| SubLabel   | String                | 子标签名称                                                   |
...
| 名称   | 类型               | 描述                                                     |
| ---------- | ---------------------- | ------------------------------------------------------------ |
| Scene      | String                 | 识别场景，取值可选：OCR 图片                           |
| Suggestion | String                 | 建议值，Block：打击，Review：待复审，Pass：正常              |
| Label      | String                 | 标签：<ul style="margin:0"><li/>Normal：正常<li/>Polity：涉政<li/>Porn：色情<li/>Illegal：违法违规<li/>Abuse：谩骂<li/>Terror：暴恐<li/>Ad：广告</ul> |
| SubLabel   | String                 | 子标签名称                                                   |
| Score      | Integer                | 所属场景模型命中子标签的分值，取值范围0分 - 100分            |
| Text       | String                 | 文本内容                                                     |
...
| 名称 | 类型        | 描述                                                     |
| -------- | --------------- | ------------------------------------------------------------ |
| Text     | String          | OCR 文本内容                                                 |
| Label    | String          | 恶意标签：<ul style="margin:0"><li/>Normal：正常<li/>Polity：涉政<li/>Porn：色情<li/>Illegal：违法<li/>Abuse：谩骂<li/>Terror：暴恐<li/>Ad：广告</ul> |

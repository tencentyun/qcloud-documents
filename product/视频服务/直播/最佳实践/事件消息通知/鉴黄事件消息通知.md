直播鉴黄是将直播流中可疑的涉黄画面实时截取成图像，并生成图片存储到 COS 中。而鉴黄回调用于推送直播鉴黄图片信息，包括问题图片所属类型、等级评定、截图时间等。在回调模板中配置鉴黄回调消息接收服务器地址，并将该模板与推流域名进行关联后。当您的直播流触发鉴黄事件后，腾讯云直播后台会将涉黄图片信息回调到您设置的接收服务器中。

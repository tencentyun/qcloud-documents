推断流回调用于推送直播流状态信息，包括直播推流成功和直播断流。在回调模板中配置推流回调和断流回调消息接收服务器地址，并将该模板与推流域名进行关联后。当您生成对应的推流地址并开始推送直播流后，腾讯云直播后台会将推流结果回调到您设置的接收服务器中。

本文主要讲解触发推断流回调事件后，腾讯云直播返回的回调消息通知字段。

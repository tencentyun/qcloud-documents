...
| start_time_usec | int | 录制任务开始写文件的时间；不能以该值作为录制内容的开始时间，录制内容的开始时间 = end_time – duration；微秒部分 |
| end_time_usec | int | 录制结束时间 微秒部分 |
| duration     | int64  | 录制文件时长，单位秒 |
| file_size | uint64 | 录制文件大小，单位字节 |
| stream_param | string | 用户推流 URL 所带参数（自定义） |
| video_url    | string | 录制文件下载 URL |
| media_start_time | int | 录制开始拉流收到的首帧pts （并不一定是文件首帧pts）          |

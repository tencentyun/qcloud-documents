...
回调配置主要通过两种方式实现，一种是通过 [云直播控制台](#c_callback)，另一种是通过 [调用服务端 API](api_callback)。
>?直播回调事件消息通知 URL 支持对推流事件、断流事件、录制事件、截图事件、鉴黄事件配置独立回调 URL。




### 直播控制台<span id="c_callback"></span>
1. 进入云直播控制台的【功能模板】>【[回调配置](https://console.cloud.tencent.com/live/config/callback)】。
2. 根据需求 [创建回调模板](https://cloud.tencent.com/document/product/267/20388)，设置对应事件接收回调消息的服务器地址。
3. 在[【域名管理】](https://console.cloud.tencent.com/live/domainmanage)找到您需操作的推流域名，单击【管理】>【模板配置】，将此域名与转码模板进行关联。具体操作请参见 [回调配置](https://cloud.tencent.com/document/product/267/35254)。


### API 调用<span id="api_callback"></span>
1. 调用 [CreateLiveCallbackTemplate](https://cloud.tencent.com/document/api/267/32637) 创建回调模板接口，设置您需要的回调参数信息。
2. 调用 [CreateLiveCallbackRule](https://cloud.tencent.com/document/api/267/32638) 创建回调规则，设置参数推流域名 DomainName 和 TemplateId（第1步返回）。在 AppName 中填写与推流和播放地址中一致的 AppName，表示匹配此域名下所有事件的回调。，以此实现部分直播流开启转码的效果。

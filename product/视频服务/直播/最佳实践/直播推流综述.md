...
### 场景一：PC 端推流
在 PC（Windows/Mac）端推流时候，可根据您的实际情况，选择安装 [OBS](https://obsproject.com/download) 或 [XSplit](https://www.xsplit.com/zh-cn) 进行推流。 OBS 支持 Windows/Mac/Linux 等系统，是一个免费的开源的视频录制和视频实时流软件。XSplit 需收费使用，XSplit 游戏直播有单独的安装包，非游戏直播推荐使用 BroadCaster。
...
>? 如果您没有提前准备推流地址，您在摄像头推流页面单击推流地址右侧的【NEW】，系统会自动创建一个推流地址，同时生成四个播放地址的二维码（您可以扫描该二维码观看视频）。

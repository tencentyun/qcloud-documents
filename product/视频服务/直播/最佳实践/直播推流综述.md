...
### 场景一：PC 端推流
在 PC（Windows/Mac）端推流时候，您可以借助第三方推流软件 OBS（推荐）或 XSplit 向腾讯视频云的推流地址，推送经过压缩编码的现场活动、教学、投影或游戏等音视频流，同时观众可以通过和推流地址相对应的播放地址收看到实时画面。
>?您可根据预算情况，选择安装 [OBS](https://obsproject.com/download) 或 [XSplit](https://www.xsplit.com/zh-cn)。 OBS 支持 Windows/Mac/Linux 等系统，是一个免费的开源的视频录制和视频实时流软件。XSplit 需收费使用，XSplit 游戏直播有单独的安装包，非游戏直播推荐使用 BroadCaster。

![](https://main.qcloudimg.com/raw/67cdbb198cbdaf68d63f2993f12a8d17.png)
本文档以安装 OBS 推流为例实现推流，假设准备好的推流地址为：
```

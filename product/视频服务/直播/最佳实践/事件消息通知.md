...
整体流程：
1. 您在控制台或直接调用云 API 配置事件消息通知 URL 以及录制、截图等相关功能。
- 您进行直播推断流。
- 当直播服务内部有事件发生时，消息将会经由事件消息通知服务统一回调给您。
...
| 字段名称 | 类型 | 说明 |
| --- | --- | --- |
| event\_type | int | 事件通知信息类型：推流事件为1；断流事件为0；混流事件为8；录制事件为100；截图事件为200。 |
| sign | string | 事件通知签名 |
...
| domain | string | 推流域名 |
| path | string | 推流路径 |
| mix\_session\_id | string | 混流会话 ID |
| status\_code | int | 混流状态码 |
| status\_msg | string | 混流状态描述 |
| timestamp | int64 | 混流开始 Unix 时间戳 |
| stream\_list | array | 混流流名称集合，流名称为 string 类型 |

推流域名的 [回调关联配置](https://cloud.tencent.com/document/product/267/35254) 成功后，当直播过程中域名关联模板事件被触发时，腾讯云将主动发送请求到客户服务器，客户服务器负责应答请求。验证通过后，您可被动获取到含直播事件回调信息的 JSON 数据包。
目前，直播事件触发消息通知支持如下事件：
...
#### 推断流事件<span id="DropLiveStream"></span>
...
#### 【直播录制】文件生成事件<span id="Record"></span>
...
#### 【直播截图】文件生成事件<span id="Screenshot"></span>

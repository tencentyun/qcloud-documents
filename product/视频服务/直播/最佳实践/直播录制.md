...
### API 说明
API 文档请参见 [创建录制任务-CreateRecordTask](https://cloud.tencent.com/document/product/267/45983)。
>! 此调用方式暂时不支持海外推流录制。


### 录制示例
- 最简单的情况，您只需要指定 StreamName、DomainName、AppName和EndTime参数，如：
```
...
&<公共请求参数>
```
- 若您创建了2018年09月11日早上08点到10点的录制任务，格式为 FLV，视频录制，分片间隔30分钟，可永久存储。
...
- 若您创建了2018年09月11日早上08点到10点的录制任务，格式为 MP4，纯音频录制，分片间隔1小时，录制文件存储时间为1个月。
> ?

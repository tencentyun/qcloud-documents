...
2. 在 [域名管理页面](https://console.cloud.tencent.com/live/domainmanage) 添加推流域名，点击管理操作，将此域名与录制模板进行关联。

### API 调用

1. 调用 [CreateLiveRecordTemplate](https://cloud.tencent.com/document/product/267/30145)，设置至少一种录制格式，如 FlvParam。
2. 调用 [CreateLiveRecordRule](https://cloud.tencent.com/document/product/267/30145)，设置参数推流域名 DomainName 和 TemplateId（第1步返回）。AppName 及 StreamName 置空，表示通配此域名下所有推流。
...
* 在控制台，或者通过 API [CreateLiveRecordRule](https://cloud.tencent.com/document/product/267/30145) 设置 DomainName和StreamName 参数，将上述录制模板与指定推流域名、流名称进行关联。

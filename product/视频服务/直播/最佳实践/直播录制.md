...
其原理是针对 HLS 录制格式使用 HLS 的 **#EXT-X-DISCONTINUITY** 标签来分割多次推流的音视频数据，此标签的作用是标识前后的音视频数据的时间戳、视频编码、音频编码采样等信息。前后音视频数据可能不同，需要播放器刷新解码器以实现无缝的正常播放。因此使用此功能时，需要播放器支持 **#EXT-X-DISCONTINUITY** 标签，iOS 自带播放器（或者 Safari 直接播放）、安卓的 ExoPlayer、Web 端的 hls.js 播放器都支持此标签，VLC 等播放器不支持此标签。

使用此功能后，在最后一次正常推流结束后，HLS 录制文件在会在设置好的推流中断自动拼接时间（比如5分钟）生成，也就是下一小节描述的文件生成获取会有这个配置时间的延迟。另外，如果主动调用了禁播来中断推流，那么自动拼接将提前结束并生成文件。

如果需要开通此功能请提工单并提供 **Templateld** 与推流中断自动拼接时间（最长可设置5分钟）。

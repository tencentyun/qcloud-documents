...
第 1 步，在[录制配置页面](https://console.cloud.tencent.com/live/config/record)添加录制配置（模板）。

第 2 步，在[域名管理页面](https://console.cloud.tencent.com/live/domainmanage)添加推流域名，点击管理操作，将此域名与录制模板进行关联。

#### 2、API 调用

第 1 步，调用 [CreateLiveRecordTemplate](https://cloud.tencent.com/document/product/267/30145)，设置至少一种录制格式，如 FlvParam。

第 2 步，调用 [CreateLiveRecordRule](https://cloud.tencent.com/document/product/267/30145)，设置参数推流域名 DomainName 和 TemplateId（第1步返回）。AppName 及 StreamName 置空，表示通配此域名下所有推流。
...
您可能已为某个推流域名配置了录制，但是此域名下存在个别推流因业务原因不需要录制。可以这样操作：

- 调用[CreateLiveRecordTemplate](https://cloud.tencent.com/document/product/267/30145)，且不指定任何录制格式。
...
* 在控制台，或者通过 API [CreateLiveRecordRule](https://cloud.tencent.com/document/product/267/30145) 设置DomainName和StreamName参数，将上述录制模板与指定推流域名、流名称进行关联。

需要注意的是：上述方案仅针对个别推流不需要录制的情况。如果您需要操作直播流数较多，我们还是建议您使用另外一个推流域名进行独立管理，主要考虑到：
...
创建了 2018-09-11 早上 8 点到 10 点的录制任务，格式为 FLV，视频录制，分片间隔 30 分钟，永久存储。
...
创建了 2018-09-11 早上 8 点到 10 点的录制任务，格式为 MP4，纯音频录制，分片间隔 1 小时，录制文件存储时间为 1 个月。

需要说明的是，对于同一条直播流，定时任务之间以及定时任务与其它形式的录制任务不冲突。也就是说，您创建的多个定时任务所指定的时间范围可以重叠。而且可以在启用配置录制的基础上再调用 API 创建录制任务。
...
登录点播控制台，在视频管理页面可以浏览已录制所有文件。

 ![](https://main.qcloudimg.com/raw/d3afc2a39fadc9ac889d68cfe52c71ef.png)

#### 录制事件通知

通过控制台或者 API 调用设置录制回调地址，录制文件生成后会以消息方式通知到该回调地址。在收到消息后可根据录制[回调协议内容](https://cloud.tencent.com/document/product/267/5957)进行业务处理。

事件通知机制高效可靠且实时性好，我们推流您使用回调方式获取录制文件。
...
在更新完录制配置之后，我们建议您重新推流并对配置进行验证。配置的生效规则如下：

...
```
   ...
   &<公共请求参数>
```
2. 在控制台，或者通过 API [CreateLiveRecordRule](https://cloud.tencent.com/document/product/267/32615) 设置 DomainName 和 StreamName 参数，将上述录制模板与指定推流域名、流名称进行关联。
...
例如：创建了2018年09月11日早上08点到10点的录制任务，格式为 FLV，视频录制，分片间隔30分钟，可永久存储。
```
...
例如：创建了2018年09月11日早上08点到10点的录制任务，格式为 MP4，纯音频录制，分片间隔1小时，录制文件存储时间为1个月。
```
...
如果只想录制混流画面，您可以通过 API [CreateRecordTask](https://cloud.tencent.com/document/product/267/45983) 实现。需要说明的是，对于 OutputStreamType 为`1`这种混流类型，调用上述 API 需要设置 MixStream 参数为`1`。
...
![img](https://main.qcloudimg.com/raw/a2f1bfba98b97a73eebd91a1a6c8f460.png)

...
当前云直播支持IEFT QUIC和Google QUIC，支持的版本分别为：

IEFT QUIC版本：h3-27、h3-29；

Google QUIC版本为：Q39、Q43、Q46。
...
1. 打开 Chrome浏览器。
   Chrome浏览器目前默认使用IEFT QUIC版本，如果要验证Google QUIC版本，需使用命令行工具指定quic-version打开：
   打开命令行工具，进入chrome安装目录，执行chrome --enable-quic --quic-version=QUIC_VERSION_43。
3. 打开 Chrome 的 QUIC 开关。
   在 Chrome 浏览器地址栏输入 `chrome://flags/#enable-quic `，将开关设置成 Enabled，并重启 Chrome 浏览器。
   
   ![](https://main.qcloudimg.com/raw/b5aee3532ef918518206b607cc2d8f53.png) 
2. 打开 TCPlayer Lite，单击FLV标签，填入 **HTTPS** 的FLV播放地址。单击**PC端播放器预览**，开始播放。
![image](https://user-images.githubusercontent.com/8492932/149660320-6c62b1ee-a813-4bdc-aefa-e9b35e6e7ebb.png)

3. 在 Chrome 的开发者工具中，选择 **Network** 标签页，可以看到请求的 protocol 是已经是QUIC协议，根据QUIC的版本不同，可能显示 `http/2+quic/43`或 `h3-29`等。

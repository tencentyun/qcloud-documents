...
## 使用播放转码
考虑到不同的网络因素，满足您使用不同码率不同分辨率的需求，您可以前往 [转码配置](https://console.cloud.tencent.com/live/config/transcode) 设置不同码率不同分辨率的转码模板（关于转码相关详细介绍可以前往 [最佳实践-直播转码]() 了解）。
...
## 使用 HTTPS 播放
若您的播放域名需要支持 HTTPS，您需要准备好有效的证书内容、有效私钥内容前往 [云直播控制台-域名管理](https://console.cloud.tencent.com/live/domainmanage)，进入【播放域名管理】>【高级配置】>【HTTPS配置】添加配置，添加成功之后会有个生效时间（2小时），在生效后您的直播流就可以支持 HTTPS 协议播放了。
...
### 参数说明
-  txTime
播放 URL 的有效时间，格式为16进制 UNIX 时间。假如当前 txTime 的值大于当前请求的时间则可以正常播放，否则播放会被后台拒绝。
- key
...
- 有效时间
有效时间设置必须大于0，假设 txTime 设置为当前时间，有效时间设置为300s，则播放URL过期时间为当前时间+300s。

### 防盗链计算
防盗链计算需要三个参数，key（随机字符串）、流名称（StreamName） ，txTime（16进制格式）。
假设您设置的 key 为 **somestring**，流名称（StreamName）为 **test**，txTime 为 **5c2acacc**（2019-01-01 10:05:00）。高清码率为：**900kbps**，转码模板 ID 为：**900**。
原始流播放地址：
```
...
> - HLS：由于 HLS 是短链接，会不断的请求 m3u8 获取最新的 ts 分片。假设您设置 txTime 的值为**当前时间+十分钟**，则在十分钟之后 HLS 播放 URL 请求会被拒绝。针对这个问题您可在业务端动态更新 HLS 的请求地址，或者将 HLS 的播放地址过期时间设置久一点。
> - RTMP：目前 RTMP 不支持防盗链。
...
## 如何播放
### 移动端播放
如果您需要集成直播功能到您的移动产品，您可以使用我们直播 SDK，它可以支持您这边播放 RTMP、HTTP-FLV、HLS 直播播放，详情可以参考 [移动直播产品](https://cloud.tencent.com/product/mlvb)。
### Web播放

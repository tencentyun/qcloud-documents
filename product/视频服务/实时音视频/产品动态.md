...
<li/>增加外接音频设备的异常检测。您可以通过监听 onStatistics 回调 TRTCLocalStatistics 中的 audioCaptureState 来实时检测长时间静音、破音、异常间断问题。
<li/>优化 BGM 资源管理，及时释放内存占用。
<li/>推流端退后台暂停视频上行时，播放端能及时收到 onUserVideoAvailable(false) 的通知。
</ul><br>Mac：<ul style="margin:0">
优化屏幕分享时鼠标捕捉的 CPU 和内存占用。
</ul><br>Windows：<ul style="margin:0">
<li/>自定义采集支持输入 RGBA 格式的视频数据。
</ul></td>
<td>2021-05-25</td>

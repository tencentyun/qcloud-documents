...
## 常见问题
### 录制回调通知会不会丢？
&emsp;&emsp;腾讯云后台目前的消息可靠性保证机制是基于简单重传实现。即：如果一条通知消息没有成功发送到您指定的回调 URL，腾讯云会反复重试。那怎么确认消息是已经送达您的服务器了呢？这里是需要您的协助的：当您的服务器成功收到一条 Http 事件通知消息时，请回复： 200 的状态码，以代表您已经成功收到了消息，从而避免腾讯云反复重复通知。

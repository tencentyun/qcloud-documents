...
监听对象为创建课堂时传入的 ILiveRoomOption 对象的一个属性 memberStatusListener ，由于需要将房间页面对象LiveRoomViewController 设置为监听者，所以在创建房间对象时将 option 通过自动以的初始化方法 initWithOption 传到了 LiveRoomViewController 内部。
```objc

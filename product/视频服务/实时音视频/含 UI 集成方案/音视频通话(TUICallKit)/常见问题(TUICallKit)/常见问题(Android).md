...
**支持。**Android 离线推送采用腾讯云的 `TUIOfflinePush` 组件，详情请参见 [TUIOfflinePush](https://cloud.tencent.com/document/product/269/44516)。
...
```
 这是因为 TUICallKit 依赖的 TRTC 等 SDK 部分接口使用了 JNI 实现，Android Studio 在某些条件进行编译 APK 的时候可能并不会把Native 的 .so 库打包进去，导致出现这类报错，重新 clean 一下即可。
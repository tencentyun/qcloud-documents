...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| image | Bitmap | 水印图片，注意：一定要用透明底的 png 格式！！！ |
...
举例：如果当前编码分辨率是 540x960，(x， y， width) 设置为（0.1， 0.1， 0.2） 那么：水印的左上坐标点就是 (540*0.1， 960*0.1)，也就是 (54， 96)，水印的宽度是 540*0.2 = 108px，高度自动计算。
...
- texture 方案：对接需要一定的 OpenGL 基础，但是性能较好，640x360 以上的分辨率请采用该方案。

...
| [setRemoteSubStreamViewFillMode](https://cloud.tencent.com/document/product/647/32264#setremotesubstreamviewfillmode) | 设置辅流画面（屏幕分享）的渲染模式 对应于setRemoteViewFillMode() 于设置远端的主路画面，该接口用于设置远端的辅路（屏幕分享、远程播片）画面 |
...
| API | 描述 |
|-----|-----|
| [enableCustomVideoCapture](https://cloud.tencent.com/document/product/647/32264#enablecustomvideocapture) | 启用视频自定义采集模式，即放弃SDK原来的视频采集流程，改用sendCustomVideoData向SDK塞入自己采集的视频画面          |
...
| API | 描述 |
|-----|-----|
| [onUserEnter](https://cloud.tencent.com/document/product/647/32265#onuserenter) | userid对应的成员的进房通知，您可以在这个回调中调用 startRemoteView 显示该 userid 的视频画面          |
| [onUserExit](https://cloud.tencent.com/document/product/647/32265#onuserexit) | userid对应的成员的退房通知，您可以在这个回调中调用 stopRemoteView 关闭该 userid 的视频画面          |
| [onUserVideoAvailable](https://cloud.tencent.com/document/product/647/32265#onuservideoavailable) | userid对应的远端主路（即摄像头）画面的状态通知          |
| [onUserSubStreamAvailable](https://cloud.tencent.com/document/product/647/32265#onusersubstreamavailable) | userid对应的远端辅路（屏幕分享等）画面的状态通知          |
| [onUserAudioAvailable](https://cloud.tencent.com/document/product/647/32265#onuseraudioavailable) | userid对应的远端声音的状态通知          |
| [onUserVoiceVolume](https://cloud.tencent.com/document/product/647/32265#onuservoicevolume) | userid对应的成员语音音量 通过调用 TRTCCloud enableAudioVolumeEvaluation 来开关这个回调          |
...
| [onSpeedTest](https://cloud.tencent.com/document/product/647/32265#onspeedtest) | ：6.4 服务器测速的回调，SDK 对多个服务器IP做测速，每个IP的测速结果通过这个回调通知          |
...
| API | 描述 |
|-----|-----|
| [onStartPublishCDNStream](https://cloud.tencent.com/document/product/647/32265#onstartpublishcdnstream) | 旁路推流到CDN的回调，对应于 TRTCCloud 的 startPublishCDNStream() 接口 |
...
| [onVideoPostProcessGLDestroy](https://cloud.tencent.com/document/product/647/32265#onvideopostprocessgldestroy) | 自定义预处理，Opengl环境销毁通知回调; 用户可以在此回调里，进行opengl资源回收；保证跟sdk 的opengl 在同一个线程；否则可能会显存泄漏，或 崩溃          |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [appId](https://cloud.tencent.com/document/product/647/32266#appid2) | int | 腾讯云 AppID，在直播控制台-直播码接入可查询到          |
| [bizId](https://cloud.tencent.com/document/product/647/32266#bizid2) | int | 腾讯云直播bizid，在直播控制台-直播码接入可查询到          |

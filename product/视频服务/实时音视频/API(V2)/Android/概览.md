...
| [setListener](31513#setlistener) | 设置回调接口 TRTCCloudListener，用户获得来自 TRTCCloud 的各种状态通知          |
| [setListenerHandler](31513#setlistenerhandler) | 设置驱动回调的线程，默认会采用 Main Thread。 也就是说，如果您不指定 listenerHandler，那么直接在 TRTCCloudListener 的回调函数中操作 UI 界面将是安全的          |
| [setAudioListener](31513#setaudiolistener) | 设置音频相关回调          |
...
| [setVideoEncoderParam](31513#setvideoencoderparam) | 设置视频编码器相关参数，该设置决定了远端用户看到的画面质量（同时也是云端录制出的视频文件的画面质量）          |
| [setNetworkQosParam](31513#setnetworkqosparam) | 设置网络流控相关参数，该设置决定了SDK在各种网络环境下的调控策略（比如弱网下是“保清晰”还是“保流畅”）          |
...
| [setVideoEncoderRotation](31513#setvideoencoderrotation) | 设置视频编码出的（供录制和远程观看的）画面方向          |
...
| API | 描述 |
|-----|-----|
| [startLocalAudio](31513#startlocalaudio) | 开启本地音频流，该函数会启动麦克风采集，并将音频数据广播给房间里的其他用户 |
| [stopLocalAudio](31513#stoplocalaudio) | 关闭本地音频流          |
...
| API | 描述 |
|-----|-----|
| [enableCustomVideoCapture](31513#enablecustomvideocapture) | 启用视频自定义采集模式，即放弃SDK原来的视频采集流程，改用sendVideoSampleBuffer向SDK塞入自己采集的视频画面          |
| [sendCustomVideoData](31513#sendcustomvideodata) | 发送自定义的视频数据          |
| [enableCustomAudioCapture](31513#enablecustomaudiocapture) | 启用音频自定义采集模式，即放弃SDK原来的声音采集流程，改用enableCustomAudioCapture向SDK塞入自己采集的声音数据（PCM格式）          |
| [sendCustomAudioData](31513#sendcustomaudiodata) | 发送客户自定义的音频PCM数据          |
| [setLocalVideoRenderListener](31513#setlocalvideorenderlistener) | 设置本地视频自定义渲染          |
| [setRemoteVideoRenderListener](31513#setremotevideorenderlistener) | 设置远端视频自定义渲染          |
...
| [setBGMVolume](31513#setbgmvolume) | 设置背景音乐的音量大小，播放背景音乐混音时使用，用来控制背景音音量大小          |
...
| API | 描述 |
|-----|-----|
| [startPublishCDNStream](31513#startpublishcdnstream) | 启动CDN发布：通过腾讯云将当前房间的音视频流发布到直播CDN上 |
| [stopPublishCDNStream](31513#stoppublishcdnstream) | 停止CDN发布          |
| [startCloudMixTranscoding](31513#startcloudmixtranscoding) | 启动云端的混流转码：通过腾讯云的转码服务，将房间里的多路画面叠加到一路画面上  【画面1】=> 解码 => =>
                        \
 【画面2】=> 解码 =>  画面混合 => 编码 => 【混合后的画面】
                        /
 【画面3】=> 解码 => =>
 |
| [stopCloudMixTranscoding](31513#stopcloudmixtranscoding) | 停止云端的混流转码          |
...
| [setLogListener](31513#setloglistener) | 设置日志回调          |
| [showDebugView](31513#showdebugview) | 显示仪表盘（状态统计和事件消息浮层view），方便调试          |
| [setDebugViewMargin](31513#setdebugviewmargin) | 设置仪表盘的边距，必须在 用户进房后设置才生效          |
...
| [topMargin](31513#topmargin) | float | 距离顶部的百分比，取值范围为0-1          |
| [rightMargin](31513#rightmargin) | float | 距离右边的百分比，取值范围为0-1          |
| [bottomMargin](31513#bottommargin) | float | 距离底部的百分比，取值范围为0-1          |
...
| API | 描述 |
|-----|-----|
| [TRTCViewMargin](31513#trtcviewmargin) | 构造方法          |
...
| [onMicDidReady](31514#onmicdidready) | 麦克风准备就绪          |
...
| API | 描述 |
|-----|-----|
| [onRecvCustomCmdMsg](31514#onrecvcustomcmdmsg) | 当房间中的某个用户使用 sendCustomCmdMsg 发送自定义消息时，房间中的其它用户可以通过 onRecvCustomCmdMsg 接口接收消息 |
| [onMissCustomCmdMsg](31514#onmisscustomcmdmsg) | TRTC所使用的传输通道为UDP通道，所以即使设置了 reliable，也做不到100不丢失，只是丢消息概率极低，能满足常规可靠性要求。 在过去的一段时间内（通常为5s），自定义消息在传输途中丢失的消息数量的统计，SDK 都会通过此回调通知出来 |
...
| API | 描述 |
|-----|-----|
| [onStartPublishCDNStream](31514#onstartpublishcdnstream) |  |
| [onStopPublishCDNStream](31514#onstoppublishcdnstream) |  |
| [onStartCloudMixTranscoding](31514#onstartcloudmixtranscoding) |  |
| [onStopCloudMixTranscoding](31514#onstopcloudmixtranscoding) |  |
...
| API | 描述 |
|-----|-----|
| [onCapturedAudioData](31514#oncapturedaudiodata) | 本机采集到的声音回调          |
| [onPlayAudioDataBeforeMixing](31514#onplayaudiodatabeforemixing) | 混音前的每一路声音（比如您要对某一路的语音进行文字转换，必须要使用这里的数据，混音后的数据不适合用于语音识别）          |
| [onPlayAudioDataAfterMixing](31514#onplayaudiodataaftermixing) | 经过混合后的声音          |
...
| [roomId](31515#roomid) | String | 房间号码 [必填] 指定房间号，在同一个房间里的用户（userId）可以彼此看到对方并进行视频通话 |
...
| [videoFps](31515#videofps) | int | 视频采集帧率，推荐设置为 15fps 或 20fps，10fps以下会有明显的卡顿感，20fps以上则没有必要，如果大于20，sdk内部会限制到20 |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [preference](31515#preference) | int | 弱网下是“保清晰”还是“保流畅” |
| [controlMode](31515#controlmode) | int | 视频分辨率（云端控制 - 客户端控制） |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [appId](31515#appid) | int | 腾讯云 AppID，在直播控制台-直播码接入可查询到          |
| [bizId](31515#bizid) | int | 腾讯云直播bizid，在直播控制台-直播码接入可查询到          |
| [url](31515#url) | String | 旁路转推的URL          |
| [enableTranscoding](31515#enabletranscoding) | boolean | 是否允许转码混流
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [userId](31515#userid2) | String | 参与混流的userId          |
| [x](31515#x) | int | 图层位置x坐标（绝对像素值）          |
| [y](31515#y) | int | 图层位置y坐标（绝对像素值）          |
| [width](31515#width2) | int | 图层位置宽度（绝对像素值）          |
| [height](31515#height2) | int | 图层位置高度（绝对像素值）          |
| [zOrder](31515#zorder) | int | 图层层次 （1-16） 不可重复          |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [mode](31515#mode) | int | 转码config模式          |
| [videoWidth](31515#videowidth) | int | 视频分辨率：宽          |
| [videoHeight](31515#videoheight) | int | 视频分辨率：高          |
| [videoBitrate](31515#videobitrate2) | int | 视频码率          |
| [videoFramerate](31515#videoframerate) | int | 视频帧率          |
| [videoGOP](31515#videogop) | int | 视频GOP          |
| [audioSampleRate](31515#audiosamplerate) | int | 音频采样率 48000          |
| [audioBitrate](31515#audiobitrate) | int | 音频码率 64K          |
| [audioChannels](31515#audiochannels) | int | 声道数 2          |
| [mixExtraInfo](31515#mixextrainfo) | String | SEI信息          |
| [mixUsers](31515#mixusers) | ArrayList< TRTCMixUser > | 混流配置          |
...
| [audioBitrate](31515#audiobitrate2) | int | 音频码率（Kbps）          |

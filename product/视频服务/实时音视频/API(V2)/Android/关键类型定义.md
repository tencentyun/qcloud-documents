...
很多 Android 手机的前置摄像头并不支持15fps 以上的采集帧率部分过于突出美颜功能的 Android 手机前置摄像头的采集帧率可能低于10fps。
...
Client 模式：客户端控制模式，用于 SDK 开发内部调试，客户请勿使用 Server 模式（默认）：云端控制模式，若没有特殊原因，请直接使用该模式。
...
参与混合的是主路还是辅路画面[TRTC_VIDEO_STREAM_TYPE_BIG, TRTC_VIDEO_STREAM_TYPE_SUB]。

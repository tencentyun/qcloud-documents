...
|-----|-----|-----|
| userId | string | 用户标识。 |
| reason | int | 离开原因代码。<br>区分用户正常离开，或由于网络断线等原因引起异常离开。 |
...
声音数据帧的自定义处理回调（只读）。
>!回调函数会在 SDK 内部线程同步抛出，不要做耗时操作。如无需自定义音频数据回调，请不要调用注册回调函数，减少性能损失。
...
混音前的每一路远程用户的音频数据（例如您需要对某一路的语音进行文字转换，必须使用这里的原始数据，而非混音后的数据）。
```

...
| userId | NSString * | 用户标识（必填），当前用户的 userid，相当于登录用户名。 | 如果一个用户在您的账号系统中的id为“abc”，则 userId 即可设置为“abc”。 |
| userSig | NSString * | 用户签名（必填），当前 userId 对应的验证签名，相当于登录密码。 | 参考文档：[如何计算UserSig](https://cloud.tencent.com/document/product/647/17275)。 |
| roomId | UInt32 | 房间号码（必填），指定房间号，在同一个房间里的用户（userId）可以彼此看到对方并进行视频通话。 | 您可以随意指定，但请不要重复，如果您的用户账号 ID 是数字类型的，可以直接用创建者的用户 ID 来做为 roomId。 |
...
|-----|-----|-----|-----|-----|
| videoResolution | TRTCVideoResolution | 视频分辨率 | 1.视频通话建议选择 360x640 及以下分辨率，resMode 选择 Portrait。<br>1. 手机直播建议选择 540x960，resMode 选择 Portrait。<br>2. Window 和 iMac 建议选择 640x360 及以上分辨率，resMode 选择 Landscape。 | 您在 TRTCVideoResolution 只能找到横屏模式的分辨率，比如：640x360 这样的分辨率。 如果想要使用竖屏分辨率，请指定 resMode 为 Portrait，比如：640x360 + Portrait = 360x640。 |
| resMode | TRTCVideoResolutionMode | 分辨率模式（横屏分辨率 - 竖屏分辨率） | 手机直播建议选择 Portrait，Window 和 Mac 建议选择 Landscape。 | 如果 videoResolution 指定分辨率 640x360，resMode 指定模式为 Portrait，则最终编码出的分辨率为 360x640。 |
| videoFps | int | 视频采集帧率 | 推荐设置为15fps或20fps，10fps以下会有明显的卡顿感，20fps以上则没有必要 | 很多 Android 手机的前置摄像头并不支持15fps以上的采集帧率，部分过于突出美颜功能的 Android 手机前置摄像头的采集帧率可能低于 10fps。 |
...
| 属性 | 类型 | 字段含义 | 推荐取值 | 特别说明 |
|-----|-----|-----|-----|-----|
| preference | TRTCVideoQosPreference | 弱网下是“保清晰”还是“保流畅” | - | 1. 弱网下保流畅 - 在遭遇弱网环境时，画面会变得模糊，且会有较多马赛克，但可以保持流畅不卡顿<br>2. 弱网下保清晰 - 在遭遇弱网环境时，画面会尽可能保持清晰，但可能会更容易出现卡顿 |
| controlMode | TRTCQosControlMode | 视频分辨率（云端控制 - 客户端控制） | 云端控制 | 1. Client 模式：客户端控制模式，用于SDK开发内部调试，客户请勿使用<br>2. Server 模式（默认）：云端控制模式，若没有特殊原因，请直接使用该模式 |
...
>2.&nbsp;upLostRate 是指上行丢包率，例如0.3代表每向服务器发送10个数据包，可能有3个会在中途丢失。
>3.&nbsp;downLostRate 是指下行丢包率，例如0.2代表从服务器每收取10个数据包，可能有2个会在中途丢失。
>4.&nbsp;rtt 是指当前设备到腾讯云服务器的一次网络往返时间，正常数值在10ms - 100ms 之间。
...
| rtt | uint32_t | 延迟（毫秒），代表 SDK 跟服务器一来一回之间所消耗的时间，这个值越小越好，正常数值在 10ms - 100ms 之间。 |
...
| 属性 | 类型 | 字段含义 | 推荐取值 |
|-----|-----|-----|-----|
| pixelFormat | TRTCVideoPixelFormat | 视频像素格式 | TRTCVideoPixelFormat_NV12 |
| bufferType | TRTCVideoBufferType | 视频数据结构类型 | TRTCVideoBufferType_PixelBuffer |
| pixelBuffer | CVPixelBufferRef | bufferType 为 TRTCVideoBufferType_PixelBuffer 时的视频数据。 | - |
| data | NSData * | bufferType 为 TRTCVideoBufferType_NSData 时的视频数据。 | 自定义视频采集不支持。 |
...
| rotation | TRTCVideoRotation | 视频像素的顺时针旋转角度 | 自定义视频采集时不需要填写。 |
...
|-----|-----|-----|
| data | NSData * | 音频数据 |
| sampleRate | TRTCAudioSampleRate | 采样率 |
| channels | int | 声道数 |
...
| streamType | TRTCVideoStreamType | 参与混合的是主路画面（TRTCVideoStreamTypeBig）还是屏幕分享（TRTCVideoStreamTypeSub）画面 |
...
| mixUsers | NSArray< TRTCMixUser * > * | 每一路子画面的位置信息 | - |
...
| 属性 | 类型 | 字段含义 |
|-----|-----|-----|
| upLoss | uint32_t | C -> S 上行丢包率(%)<br>这个值越小越好，比如：0% 的丢包率代表网络很好， 而 30% 的丢包率则意味着 SDK 向服务器发送的每 10 个数据包中就会有 3 个会在上行传输中丢失。 |
| downLoss | uint32_t | S -> C 下行丢包率(%)<br>这个值越小越好，比如：0% 的丢包率代表网络很好， 而 30% 的丢包率则意味着服务器向 SDK 发送的每 10 个数据包中就会有 3 个会在下行传输中丢失。 |
...
此处仅定义了横屏分辨率，如果要使用 360x640 这样的竖屏分辨率，需要同时指定 TRTCVideoResolutionMode 为 Portrait。
...
- 横屏分辨率：TRTCVideoResolution_640_360 + TRTCVideoResolutionModeLandscape = 640x360
- 竖屏分辨率：TRTCVideoResolution_640_360 + TRTCVideoResolutionModePortrait = 360x640。
...
>?1.&nbsp;如果主播的上行网络和性能比较好，则可以同时送出大小两路画面。
>2.&nbsp;SDK 不支持单独开启小画面，小画面必须依附于主画面而存在。
...
在自定义采集和自定义渲染功能，你需要用到如下枚举值来指定您希望以什么类型的容器来包装视频数据。
...
音频采样率用来衡量声音的保真程度，采样率越高保真程度越好，如果你的应用场景有音乐的存在，推荐使用 TRTCAudioSampleRate48000。
...
在 Mac 上，每一种类型的设备都可能有多个，因此 TRTC SDK 的 Mac 版本提供了一系列函数用来操作这些设备。 4.5 混流参数配置模式

...
| [setVideoEncoderParam](31507#setvideoencoderparam) | 设置视频编码器相关参数，该设置决定了远端用户看到的画面质量（同时也是云端录制出的视频文件的画面质量）          |
| [setNetworkQosParam](31507#setnetworkqosparam) | 设置网络流控相关参数，该设置决定了SDK在各种网络环境下的调控策略（比如弱网下是“保清晰”还是“保流畅”）          |
...
| [setVideoEncoderRotation](31507#setvideoencoderrotation) | 设置视频编码输出的（也就是远端用户观看到的，以及服务器录制下来的）画面方向          |
...
| API | 描述 |
|-----|-----|
| [startLocalAudio](31507#startlocalaudio) | 开启本地音频流，该函数会启动麦克风采集，并将音频数据广播给房间里的其他用户 |
| [stopLocalAudio](31507#stoplocalaudio) | 关闭本地音频流          |
...
| API | 描述 |
|-----|-----|
| [enableCustomVideoCapture](31507#enablecustomvideocapture) | 启用视频自定义采集模式，即放弃SDK原来的视频采集流程，改用sendVideoSampleBuffer向SDK塞入自己采集的视频画面          |
| [sendCustomVideoData](31507#sendcustomvideodata) | 发送自定义的SampleBuffer          |
| [enableCustomAudioCapture](31507#enablecustomaudiocapture) | 启用音频自定义采集模式，即放弃SDK原来的声音采集流程，改用enableCustomAudioCapture向SDK塞入自己采集的声音数据（PCM格式）          |
| [sendCustomAudioData](31507#sendcustomaudiodata) | 发送客户自定义的音频PCM数据 |
| [setLocalVideoRenderDelegate](31507#setlocalvideorenderdelegate) | 设置本地视频的自定义渲染回调          |
| [setRemoteVideoRenderDelegate](31507#setremotevideorenderdelegate) | 设置远端视频的自定义渲染回调          |
...
| [setBGMVolume](31507#setbgmvolume) | 设置背景音乐的音量大小，播放背景音乐混音时使用，用来控制背景音音量大小          |
...
| API | 描述 |
|-----|-----|
| [startPublishCDNStream](31507#startpublishcdnstream) | 启动CDN发布：通过腾讯云将当前房间的音视频流发布到直播CDN上 |
| [stopPublishCDNStream](31507#stoppublishcdnstream) | 停止CDN发布          |
| [startCloudMixTranscoding](31507#startcloudmixtranscoding) | 启动云端的混流转码：通过腾讯云的转码服务，将房间里的多路画面叠加到一路画面上  【画面1】=> 解码 => => 
                        \
 【画面2】=> 解码 =>  画面混合 => 编码 => 【混合后的画面】
                        /
 【画面3】=> 解码 => =>
 |
| [stopCloudMixTranscoding](31507#stopcloudmixtranscoding) | 停止云端的混流转码          |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [delegate](31507#delegate) | id< TRTCCloudDelegate > | 设置回调接口 TRTCCloudDelegate，用户获得来自 TRTCCloud 的各种状态通知          |
...
| [audioDelegate](31507#audiodelegate) | id< TRTCAudioDelegate > | 设置音频数据回调，传入nil取消监听          |
...
| [onMicDidReady](31509#onmicdidready) | 麦克风准备就绪          |
...
| API | 描述 |
|-----|-----|
| [onRecvCustomCmdMsgUserId](31509#onrecvcustomcmdmsguserid) | 当房间中的某个用户使用 sendCustomCmdMsg 发送自定义消息时，房间中的其它用户可以通过 onRecvCustomCmdMsg 接口接收消息 |
| [onMissCustomCmdMsgUserId](31509#onmisscustomcmdmsguserid) | TRTC所使用的传输通道为UDP通道，所以即使设置了 reliable，也做不到100不丢失，只是丢消息概率极低，能满足常规可靠性要求。 在过去的一段时间内（通常为5s），自定义消息在传输途中丢失的消息数量的统计，SDK 都会通过此回调通知出来 |
...
| API | 描述 |
|-----|-----|
| [onStartPublishCDNStream](31509#onstartpublishcdnstream) |  |
| [onStopPublishCDNStream](31509#onstoppublishcdnstream) |  |
| [onStartCloudMixTranscoding](31509#onstartcloudmixtranscoding) |  |
| [onStopCloudMixTranscoding](31509#onstopcloudmixtranscoding) |  |
...
| API | 描述 |
|-----|-----|
| [onCapturedAudioData](31509#oncapturedaudiodata) | 本机采集到的声音回调          |
| [onPlayAudioDataBeforeMixing](31509#onplayaudiodatabeforemixing) | 混音前的每一路声音（比如您要对某一路的语音进行文字转换，必须要使用这里的数据，混音后的数据不适合用于语音识别）          |
| [onPlayAudioDataAfterMixing](31509#onplayaudiodataaftermixing) | 经过混合后的声音          |
...
| [roomId](31511#roomid) | NSString *_Nonnull | 房间号码 [必填] 指定房间号，在同一个房间里的用户（userId）可以彼此看到对方并进行视频通话 |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [preference](31511#preference) | TRTCVideoQosPreference | 弱网下是“保清晰”还是“保流畅” |
| [controlMode](31511#controlmode) | TRTCQosControlMode | 视频分辨率（云端控制 - 客户端控制） |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [pixelFormat](31511#pixelformat) | TRTCVideoPixelFormat | 视频像素格式          |
| [bufferType](31511#buffertype) | TRTCVideoBufferType | 视频数据结构类型          |
| [textureId](31511#textureid) | int | 视频纹理ID          |
| [pixelBuffer](31511#pixelbuffer) | CVPixelBufferRef | bufferType为TRTCVideoBufferType_PixelBuffer时的视频数据          |
| [data](31511#data) | NSData * | bufferType为TRTCVideoBufferType_NSData时的视频数据          |
| [timestamp](31511#timestamp) | uint64_t | 视频帧的时间戳，单位毫秒          |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [data](31511#data2) | NSData * | 音频数据          |
| [sampleRate](31511#samplerate) | TRTCAudioSampleRate | 采样率          |
| [channels](31511#channels) | int | 声道数          |
| [timestamp](31511#timestamp2) | uint64_t | 时间戳，单位ms          |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [appId](31511#appid) | int | 腾讯云 AppID，在直播控制台-直播码接入可查询到          |
| [bizId](31511#bizid) | int | 腾讯云直播bizid，在直播控制台-直播码接入可查询到          |
| [url](31511#url) | NSString * | 旁路转推的URL          |
| [enableTranscoding](31511#enabletranscoding) | BOOL | 是否允许转码混流
enableTranscoding = YES : 需要调用startCloudMixTranscoding对多路画面进行混合，发布到CDN上的是混合之后的音视频流enableTranscoding = NO : 发布当前房间里的各路音视频画面，每一路画面都有一个独立的地址，相互之间无影响，调用startCloudMixTranscoding将无效 
         |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [userId](31511#userid2) | NSString * | 参与混流的userId          |
| [rect](31511#rect) | CGRect | 图层位置坐标以及大小，左上角为坐标原点(0,0) （绝对像素值）          |
| [zOrder](31511#zorder) | int | 图层层次 （1-16） 不可重复          |
...
| 属性 | 类型 | 描述 |
|-----|-----|-----|
| [mode](31511#mode) | TRTCTranscodingConfigMode | 转码config模式          |
| [videoWidth](31511#videowidth) | int | 视频分辨率：宽          |
| [videoHeight](31511#videoheight) | int | 视频分辨率：高          |
| [videoBitrate](31511#videobitrate2) | int | 视频码率          |
| [videoFramerate](31511#videoframerate) | int | 视频帧率          |
| [videoGOP](31511#videogop) | int | 视频GOP，单位秒          |
| [audioSampleRate](31511#audiosamplerate) | int | 音频采样率 48000          |
| [audioBitrate](31511#audiobitrate) | int | 音频码率 64K          |
| [audioChannels](31511#audiochannels) | int | 声道数 2          |
| [mixExtraInfo](31511#mixextrainfo) | NSString * | SEI信息          |
| [mixUsers](31511#mixusers) | NSArray< TRTCMixUser * > * | 混流配置          |
...
| [audioBitrate](31511#audiobitrate2) | uint32_t | 音频发送码率（Kbps）          |

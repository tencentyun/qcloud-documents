...
准备工作中的 1.1至1.5 步是在腾讯云实时音视频控制台创建应用并购买套餐，以及在微信公众平台进行配置，以获取 SDKAppId 和签名需要的 public_key 、 private_key，以及获得小程序实时音视频的使用权限，如果之前已经完成这些操作可以直接跳到 1.6 开始。
...
### 1.2 购买套餐
单击上一步中的**购买测试体验套餐包**或者**购买正式套餐包**按钮，为上一步中创建的 SDKAppId 充值一定分钟数的测试用视频通话时长。
...
集成SDK之前还需要搭建一个签名服务，在初始化时进行签发 userSig 和 privateMapKey（如果没有在【控制台】-【实时音视频 】-【您的应用名称】-【帐号信息】中启用权限密钥，可以忽略 privateMapKey）
...
- 请申请域名并做备案
- 请将服务端代码部署到申请的服务器上
- 请将业务server域名、WebRTC音视频鉴权服务域名、推流域名及IM域名配置到小程序控制台request合法域名里面：

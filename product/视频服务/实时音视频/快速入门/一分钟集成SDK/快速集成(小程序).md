...
您可以通过 npm 包的方式进行引用，也可以通过 [SDK 资源](https://sdk-web-1252463788.cos.ap-hongkong.myqcloud.com/trtc/miniapp/download/trtc-wx.js) 下载 `trtc-wx.js` 文件，直接进行引用。
...
[](id:step2)
### 步骤2：开始推流
首先需要进入我们的 TRTC 房间，调用 enterRoom 的方法，之后您可以手动控制开始推流，这里您可以参考 [API enterRoom](https://cloud.tencent.com/document/product/647/17018?!editLang=zh&!preview#enterroom(params)) 查看可以带哪些进房参数。
<dx-codeblock>
::: javascript javascript
enterRoom(options) {
    this.setData({
...
#### 控制是否上行本地音视频流
需要变更 live-pusher 标签上 enable-mic 和 enable-camera 的属性，您可以通过调用 [setPusherAttributes](https://cloud.tencent.com/document/product/647/17018?!editLang=zh&!preview#setpusherattributes(config)) 对状态机中管理的推流状态进行改变，会返回给您更新后的状态值，更新到页面中。
<dx-codeblock>
::: javascript javascript
// 上行音频流
this.setData({
...
如果您想其他的属性，您可以阅读 [API 文档](https://cloud.tencent.com/document/product/647/17018?!preview&!editLang=zh)，对于一些属性不生效的问题，具体请参见 [问题指南](#que)。

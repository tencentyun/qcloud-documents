...
在多人会议和在线教育等场景下，可能更期望将一堂课程（或一次会议）只录制出一个文件，并且需要该文件中包含所有用户的画面和声音。您可以开启录制的同时，使用 [云端混流](https://cloud.tencent.com/document/product/647/16827) 服务将同房间中的 userA、userB 和 userC 的画面和声音混合成一路 （A + B + C）。
...
本文以 iOS 端的 Objective-C 代码为例，实现一个大小画面上下叠加的画面混合效果：
...
-(void)onUserVideoAvailable:(NSString *)userId available:(BOOL)available {
    ...
    broadCaster.zOrder = 0;   // zOrder 为0代表主播画面位于最底层
...
<span id="search"></span>
### 步骤5：查找录制文件
除了接收回调的方式接收录制文件，您也可以在点播系统中查找录制下来的文件。
...
腾讯云点播系统提供了一系列 REST API 来管理音视频文件，您可以通过 [搜索媒体信息](https://cloud.tencent.com/document/product/266/31813) API 查询您在点播系统上的文件。您可以通过请求参数表中的 `Text` 参数进行模糊匹配，也可以根据 `StreamId` 参数进行精准查找。
REST 请求示例：
```
...
### 步骤7：删除录制文件
腾讯云点播系统提供了一系列 REST API 来管理音视频文件，您可以通过 [删除媒体](https://cloud.tencent.com/document/product/266/31764) API 删除某个指定的文件。
REST 请求示例：
```

...
## 前提条件
- 已开通 [云直播](https://cloud.tencent.com/document/product/267) 服务。
- 已开通 [云点播](https://cloud.tencent.com/document/product/266) 服务。

## 注意事项

云端录制属于增值服务，使用该功能将会由 [云直播](https://cloud.tencent.com/document/product/267) 和 [云点播](https://cloud.tencent.com/document/product/266) 收取相关费用，详情请参见 [云端录制相关费用](https://cloud.tencent.com/document/product/647/32574#.E4.BA.91.E7.AB.AF.E5.BD.95.E5.88.B6.E7.9B.B8.E5.85.B3.E8.B4.B9.E7.94.A8)。如果启用混流录制，将会由 [云直播](https://cloud.tencent.com/document/product/267) 收取相关费用，详情请参见 [云端混流转码相关费用](https://cloud.tencent.com/document/product/647/32574#.E4.BA.91.E7.AB.AF.E6.B7.B7.E6.B5.81.E8.BD.AC.E7.A0.81.E7.9B.B8.E5.85.B3.E8.B4.B9.E7.94.A8)。
可能涉及的增值项如下：
- **录制视频**
...
<span id="open"></span>
## 开启云端录制
1. 登录 [实时音视频控制台](https://console.cloud.tencent.com/trtc)，在左侧导航栏选择【功能配置】。
2. 单击目标应用所在行的【功能配置】，进入功能配置页卡。
 >?如果您还没有创建过应用，可以单击【创建应用】，填写应用名称，单击【确定】创建一个新的应用。
3. 单击【启动自动旁路直播】右侧的<img src="https://main.qcloudimg.com/raw/3fc81b259baa4edf112af2f570e6d97f.png"  style="margin:0;">开启旁路直播功能。
4. 单击【启动云端自动录制】右侧的<img src="https://main.qcloudimg.com/raw/3fc81b259baa4edf112af2f570e6d97f.png"  style="margin:0;">。
<span id="Step5"></span>
...
#### 解析消息
腾讯云会将录制和录制相关的事件都通过您设置的回调地址推送给您的服务器。
回调消息示例如下图所示：
...
<span id="traceless"></span>
### 下载录制文件
1. 在[开启云端录制-步骤2](#openstep2) 中将录制文件的类型选择为 FLV 或 MP4（如果纯音频文件则为 ACC），并配置 http 协议的录制回调地址。
2. 通过配置的回调地址[接收录制回调](#callback)，每当收到 event_type 为 100 的回调时间后，解析出其中的 `video_url` 字段，并将其下载到自己的服务器上。
3. 通过云点播的媒资管理 REST API，从腾讯云上 [删除](https://cloud.tencent.com/document/product/266/31764) 此文件。

## 适用场景
在视频面试、远程定损、金融双录、在线教育、秀场直播以及在线医疗等应用场景中，考虑取证、质检、审核、存档和回放等需求，常常需要将整个视频通话或互动直播过程录制和存储下来。
...
- **多路音视频录制成多个文件**
您可以选择将房间中的每一路音视频都录制成一个单独的文件，如果您需要保存原始文件并进行后期合成，可以选择此方案。
![](https://main.qcloudimg.com/raw/d642e231c63746ff73d4553e03f19bd2.gif)

<span id="mixRecord"></span>
- **多路音视频录制成一个文件**
您可以选择将房间中的多路音视频先进行[云端混流](https://cloud.tencent.com/document/product/647/16827)，然后再将混合后的画面录制下来，这样就可以实现：“一次通话（或一场直播）只生成一个文件”。
...
所以，要开启云端录制，只需要在用户进入房间（enterRoom）时设定当前用的 streamId。如此一来，云端录制服务也会伴随着用户音视频数据的上行一起开始磁盘文件的写入。您可以在调用 TRTCCloud 的 enterRoom 函数时，通过 TRTCParams 中的 `streamId` 参数进行设定 streamId。我们以 iOS 端的 Objective-C 代码为例：
...
<span id="mix"></span>
### 步骤3：控制多路画面的混合方案
默认状态下，一个 TRTC 房间中的每一个主播，都会产生一个独立的录制文件。比如某一个房间中有 userA、userB 和 userC 三个用户，云端默认会录制出 fileA、fileB 和 fileC 三个文件，且三个文件是独立存在的。

在多人会议和在线教育等场景下，我们更希望将一堂课程（或一次会议）只录制出一个文件，并且需要该文件中包含所有用户的画面和声音，这就需要我们在开启录制的同时，使用 [云端混流](https://cloud.tencent.com/document/product/647/16827) 服务将同房间中的 userA、userB 和 userC 的画面和声音混合成一路 （A + B + C），这样录制下来的文件就能符合我们的要求，其录制过程参考文档前半部分提到的 [原理解析](#mixRecord)。
...
我们以 iOS 端的 Objective-C 代码为例，实现一个大小画面上下叠加的画面混合效果：
...
-(void)onUserVideoAvailable:(NSString *)userId available:(BOOL)available {
    ...
    audience.userId = userId;
		audience.rect  = CGRectMake(400, 500, 180, 320); //参数设置仅供参考
    audience.zOrder = 1;
...
### 步骤4：接收录制文件
房间里的最后一路音视频流退出后，腾讯云会结束录制并将文件转存到云点播平台， 该过程大约需要30秒至2分钟（如果您设置了续录时间为300秒，那么等待时间将要比不设置续录的场景多出300秒）。转存完成后，腾讯云会通过您在 [开启云端录制 > 步骤5](#Step5) 中设置的回调地址向您的服务器发送通知。
...
<span id="search"></span>
### 步骤5：查找录制文件
如果您不考虑采用接收回调的方式来接收录制文件，那么您也可以采用主动查找的方式，在点播系统中寻找录制下来的文件。
...
### 步骤6：删除录制文件
腾讯云点播系统提供了一系列 REST API 来管理其上的音视频文件，您可以通过 [删除媒体](https://cloud.tencent.com/document/product/266/31764) 这个 REST API 来删除某个指定的文件。
REST 请求示例：
```
...
**录制费用：转码或转封装产生的计算费用**
由于录制需要进行音视频流的转码或转封装，会产生服务器计算资源的消耗，因此需要按照录制业务对计算资源的占用成本进行收费。计算方法是按照并发录制的路数进行收费，价格为 30元/路/月。所以并发数越高，录制费用越高。

>例如：您目前有 1000 个主播，如果在晚高峰时，最多同时有 500 路主播的音视频流需要录制，那么总录制费用就是：`500 x 30 = 15000 元/月`。如果您在 [设置录制格式](#openstep2) 时同时选择了两种录制文件，录制费用和存储费用都会被 x2，同理，选择三种文件时录制费用和存储费用会 x3。 所以只选择需要一种的文件格式可以大幅节约录制费用。

**存储费用：如将文件存储于腾讯云则会收取**
如果录制出的文件要存放于腾讯云，由于存储本身会产生磁盘资源的消耗，因此需要按照存储的资源占用进行收费。存储的时间越久费用也就越高，因此如无特殊需要，您可以将文件的存储时间设置的短一些来节省费用，或者将文件[存放在自己的服务器](#traceless)上。计费方式上可以选择 [ 视频存储（日结）价格 ](https://cloud.tencent.com/document/product/266/14666#.E8.A7.86.E9.A2.91.E5.AD.98.E5.82.A8) 进行日结计算，也可以购买[存储资源包](https://cloud.tencent.com/document/product/266/14667#1.-.E5.AD.98.E5.82.A8.E8.B5.84.E6.BA.90.E5.8C.85)。

>例如：您通过 [setVideoEncodrParam()](http://doc.qcloudtrtc.com/group__TRTCCloudDef__ios.html#interfaceTRTCVideoEncParam) 设置主播的码率(videoBitrate) 为 1000 kbps，那么每小时大约会产生一个 1000 kbps x 3600 秒 = 0.45GB 大小的视频文件，这个文件每天产生的存储费用约为 0.45GB x 0.0048 元/GB/日 = 0.00216‬元。
...
>例如：如果您通过云端录制产生了一个 1GB 大小的文件，当有 1000 位观众从头到尾完整地观看了视频以后，就大约会产生 1TB 的点播观看流量，那么按照阶梯价格表，1000 位观众就会产生 1000 x 1GB x 0.23元/GB = 230 元的费用；按照流量套餐包则是 175 元。
>
>需要说明的是，如果您选择从腾讯云下载文件到您的服务器上，亦会产生一次很小的点播流量消耗，并且会在您的月度账单中有所体现。
...
>例如：您通过 [setVideoEncodrParam()](http://doc.qcloudtrtc.com/group__TRTCCloudDef__ios.html#interfaceTRTCVideoEncParam) 设置主播的码率(videoBitrate) 为 1500 kbps，分辨率为 720p。如果有一位主播跟观众连麦了一个小时，这段时间您开启了[混流录制](#mix)，那么产生的转码费用为: 0.0325 元/分钟 x 60 分钟 = 1.95元。

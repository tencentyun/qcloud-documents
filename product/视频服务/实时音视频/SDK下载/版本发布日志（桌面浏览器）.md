...
- 默认 video profile 变更为 `480p_2`，在保障画质的情况下，降低上行带宽消耗。具体请参见 [LocalStream.setVideoProfile](https://web.sdk.qcloud.com/trtc/webrtc/doc/zh-cn/LocalStream.html#setVideoProfile)。
- 提升自动恢复播放成功率。
- 提升断网重连后，mute 状态准确性。
- 规避 Mac Safari 15.1 muteVideo 页面崩溃的问题，具体请参见 [webkit bug](https://bugs.webkit.org/show_bug.cgi?id=232006)。

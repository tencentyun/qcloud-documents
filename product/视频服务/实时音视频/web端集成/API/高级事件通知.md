...
| isLocal | Bool | 是否本地流 |
| stream | Stream | 是否本地流 |
| type | 类型 | stream/audio/video (stream 作为 audio 和 video track 的载体，如果类型是 stream，则表示流断开了)|
...
| tinyId | String | 连接所属用户用户名在腾讯云对应的唯一 64 位 id，这里您无需理解这个参数的作用，只需在 startRTC 的时候透传即可。 |

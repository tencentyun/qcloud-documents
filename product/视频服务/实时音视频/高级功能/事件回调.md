...
### 计算签名
签名由 HMAC SHA256 加密算法计算得出，您的事件回调接收服务器收到回调消息后，通过同样的方式计算出签名，相同则说明是腾讯云的实时音视频的事件回调，没有被伪造。回调事件签名计算代码原理及示例请参见 [签名方法v3](https://cloud.tencent.com/document/product/647/37082) ，签名的计算如下所示：
```

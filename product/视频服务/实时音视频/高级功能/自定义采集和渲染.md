...
| rotation | TRTCVideoRotation| 不填写 | 默认不填写。<br>如果需要对画面进行旋转，可以填写取值为0、90、180、270。SDK  会根据这个值将视频顺时针旋转对应角度。例如一个竖屏画面，传入90后，SDK 会旋转成横屏显示。 |
...
| rotation | int| 不填写 |默认不填写。<br>如果需要对画面进行旋转，可以填写取值为0、90、180、270。SDK  会根据这个值将视频顺时针旋转对应角度。例如一个竖屏画面，传入90后，SDK 会旋转成横屏显示。 |

...
[](id:Billing_items)
## 用量统计方式
实时音视频 TRTC 按 [房间](https://cloud.tencent.com/document/product/647/46351#.E6.88.BF.E9.97.B4) 内所有用户产生的 [视频时长](#v_duration) 和 [语音时长](#s_duration) 来统计视频通话服务的用量。
...
假设用户 A、B 、C 一起在 TRTC 房间中持续停留了30分钟。A、B、C  都始终接收对方的视频流。A 、B 、C 实际接收到的视频分辨率如下表所示：
...
- **纯视频时长计费 = 视频时长单价 × 用户视频时长**
- A 产生的用量及费用：
  - A 接收 B 的分辨率30分钟位于高清档； A 接收 C 的分辨率30分钟位于超清档
  - **A 产生的费用 = （A 接收 B 视频时长单价 ×  A 接收 B 视频时长）+（A 接收 C 视频时长单价 × A 接收 C 视频时长）**
  - A 产生的费用为 `高清视频时长单价 × 高清视频时长 + 超清视频时长单价 × 超清视频时长 = 28元/千分钟 × (30分钟 / 1000) + 105元/千分钟 × (30分钟 / 1000）= 3.99元`
- B 产生的用量及费用：
  - B 接收 A 的分辨率30分钟位于标清档；B 接收 C 的分辨率30分钟位于超清档
  - **B 产生的费用 =（B 接收 A 视频时长单价 × B 接收 A 视频时长）+（B 接收 C 视频时长单价 × B 接收 C 视频时长）**
  - B 产生的费用为 `标清视频时长单价 × 标清视频时长 + 超清视频时长单价 × 超清视频时长= 14元/千分钟 × (30分钟 / 1000）+ 105元/千分钟 × (30分钟 / 1000) = 3.57元`
- C 产生的用量及费用：
  - C 接收 A 的分辨率30分钟位于标清档；C 接收 B 的分辨率30分钟位于高清档
  - **C 产生的费用 =（C 接收 A 视频时长单价 ×  C 接收 A 视频时长）+（C 接收 A 视频时长单价 ×  C 接收 B 视频时长）**
  - C 产生的费用为 `标清视频时长单价 × 标清视频时长 + 高清视频时长单价 × 高清视频时长 = 14元/千分钟 × (30分钟 / 1000) + 28元/千分钟 × (30分钟 / 1000）= 1.26元`

则该 TRTC 房间产生的**总费用**为 `用户 A 产生的费用 + 用户 B 产生的费用 + 用户 C 产生的费用 = 8.82元`。

### 语音时长和视频时长混合示例

假设用户 A、B、C 一起在 TRTC 房间中持续停留了30分钟。A、B、C  都始终接收对方的纯音频流/视频流。A 、B 、C 实际接收到的视频分辨率如下表所示：
<table style="width:715px;">
<tr>
<th style ="position:relative;width:150px" >
	<div class="tablestyle"></div>
...
- **混合时长计费 = 视频时长单价 × 用户视频时长 +  语音时长单价 × 用户语音时长**
- A 产生的用量及费用：
  - A 接收 B 纯音频流30分钟，按语音计费； A 接收 C 视频流的分辨率30分钟位于超清档
  - **A 产生的费用 =（A 接收 B 语音时长单价 × A 接收 B 语音时长）+（A 接收 C 视频时长单价 × A 接收 C 视频时长）**
  - A 产生的费用为 `语音时长单价 × 语音时长 + 超清视频时长单价 × 超清视频时长 = 7元/千分钟 × (30分钟 / 1000） + 105元/千分钟 × (30分钟 / 1000)= 3.36元`
- B 产生的用量及费用：
  - B 接收 A 视频流的分辨率30分钟位于标清档；B 接收 C 视频流的分辨率30分钟位于超清档
  - **B 产生的费用 = （B 接收 A 视频时长单价 ×  B 接收 A 视频时长) +（B 接收 C 视频时长单价 ×  A 接收 C 视频时长）**
  - B 产生的费用为 `标清视频时长 x 标清时长 + 超清时长单价 × 超清时长 = 14元/千分钟 × (30分钟 / 1000) + 105元/千分钟 × (30分钟 / 1000）= 3.57元`
- C 产生的用来及费用：
  - C  接收 A 视频流的分辨率30分钟位于标清档； C 接收 B 纯音频流30分钟，按语音计费。
  - **C 产生的费用 = （C 接收 A 视频时长单价 ×  C 接收 A 视频时长）+（C 接收 B 语音时长单价 ×  C 接收 B 语音时长）**
  - C 产生的费用为 `标清视频时长 x 标清时长 + 语音时长单价 × 语音时长 = 14元/千分钟 × (30分钟 / 1000) + 7元/千分钟 × (30分钟 / 1000）= 0.63元`
...
[](id:estimate)
## 用量与费用预估
您可以使用 [TRTC 价格计算器](https://buy.cloud.tencent.com/price/trtc/calculator) 预估自己业务将产生的视频通话用量和费用。
1. 您可以根据业务需求，在【语音通话、视频通话预估用量】中输入【平均每日通话次数】、【平均每次通话人数】和【平均每人通话时间】。
2. 选择使用类型为【标清SD】、【高清HD】或【超清HD+】，可计算得到【每月预估用量】。
![](https://main.qcloudimg.com/raw/e74f76d54fc0b6f9ed525915b9a8183e.png)

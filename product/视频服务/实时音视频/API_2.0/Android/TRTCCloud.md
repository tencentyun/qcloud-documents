...
|-----|------|------|
| context | Context | 上下文  |
| delegate | TRTCCloudDelegate | 事件回调(可通过 addDelegate 额外添加)  |
...
设置回调接口 TRTCCloudDelegate，用户获得来自 TRTCCloud
...
设置驱动回调的线程，默认会采用 Main Thread。 也就是说，如果您不指定 delegateHandler，那么直接在 TRTCCloudDelegate
...
### `addWatermark`
`abstract void addWatermark(Bitmap image, float x, float y, float width)`
...
### `leftMargin`
`float com.tencent.trtc.TRTCCloud.TRTCViewMargin.leftMargin`

距离左边的百分比，取值范围为0-1
### `topMargin`
`float com.tencent.trtc.TRTCCloud.TRTCViewMargin.topMargin`

距离左边的百分比，取值范围为0-1
### `rightMargin`
`float com.tencent.trtc.TRTCCloud.TRTCViewMargin.rightMargin`

距离左边的百分比，取值范围为0-1
### `bottomMargin`
`float com.tencent.trtc.TRTCCloud.TRTCViewMargin.bottomMargin`

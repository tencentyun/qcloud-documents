...
>? TRTCCloud#enterRoom() 在进房失败时会触发这些错误码，您可以通过回调函数 TRTCCloudDelegate#onEnterRoom() 和 TRTCCloudDelegate#OnError() 捕获这些通知。
...
>? TRTCCloud#exitRoom() 在退房失败时会触发这些错误码，您可以通过回调函数 TRTCCloudDelegate#OnError() 捕获这些通知。
...
>? 您可以通过回调函数 TRTCCloudDelegate#OnError() 捕获这些通知。
...
>? 您可以通过回调函数 TRTCCloudDelegate#onStartPublishing() 和 TRTCCloudDelegate#onSetMixTranscodingConfig() 捕获这些通知。
...
>? TRTCCloud#ConnectOtherRoom() 在跨放失败时会触发这些错误码，您可以通过回调函数 TRTCCloudDelegate#onConnectOtherRoom() 捕获这些通知。
...
>? 这些警告码不需要您特别关注，但您也可以考虑根据其中某些感兴趣的警告码，对当前用户进行相应的提示

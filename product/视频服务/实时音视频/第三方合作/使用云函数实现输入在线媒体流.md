...
本文为您介绍如何 [使用 API 网关调用云函数](https://cloud.tencent.com/document/product/583)，将已有的录播视频或者 RTMP 直播流推送到实时音视频 TRTC 房间进行直播。如您需开启推流直播的实时记录，可以选择使用 Redis，API 网关会将进度实时写入 Redis。工作流程如下图所示：

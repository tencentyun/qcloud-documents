...
当调用数美智能音频流内容审核服务接口后，数美通过腾讯实时音视频 TRTC 提供的 Linux SDK 作为哑终端进入指定的房间，作为“观众”拉取音频流并对内容识别与审核，最后通过回调把违规信息发送到指定的 URL。
...
| 数美管理后台账号| 用于登陆数美管理后台。                        |
| 数美管理后台密码 | 用于登陆数美管理后台。                        |
| 数美管理后台地址 |具体值：`https://www.fengkongcloud.com`，用于登陆数美管理后台。                        |

...
const trtcCalling = TRTCCalling.create(options);
```

#### 步骤3：登录
>- sdkAppID: 您从腾讯云申请的 sdkAppID
>- userID: 用户ID
>- userSig: 用户签名，计算方式参见[如何计算userSig](https://cloud.tencent.com/document/product/647/17275)
```javascript
trtcCalling.login({
  sdkAppID,
  userID,
...
>#### 展示远端画面
>- userID: 远端用户id
>- videoViewDomID: 该用户数据将渲染到该 Dom id节点里
```javascript
...
>#### 挂断/拒接
>- status: 表示状态, calling(拨号中) 或者 connected(已接通)
```javascript

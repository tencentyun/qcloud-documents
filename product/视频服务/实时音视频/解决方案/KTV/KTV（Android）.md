## 效果展示
您可以 [下载](https://cloud.tencent.com/document/product/647/17021) 安装我们的 App 体验 KTV 的能力，包括低延时 K 歌、麦位管理、收发礼物、文字聊天等 TRTC 在 KTV 场景下的相关能力。
<table>
     <tr>
...
如需快速接入 KTV 功能，您可以直接基于我们提供的 App 进行修改适配，也可以使用我们提供的 TUIKTV 组件并实现自定义 UI 界面。
...
[](id:ui.step2)
### 步骤2：下载 App 源码
单击进入 [TUIKTV](https://github.com/tencentyun/TUIKTV)，Clone 或者下载源码。
...
[](id:ui.step4)
### 步骤4：运行 App
使用 Android Studio（3.5以上的版本）打开源码工程 `TUIKTV`，单击【运行】即可开始调试本 App。
...
[源码](https://github.com/tencentyun/TUIKTV/tree/main/Android/Source/src/main/java/com/tencent/liteav/trtcKTV) 中的 Source 文件夹包含两个子文件夹 ui 和 model，model 文件夹中包含可重用的开源组件 TRTCKtvRoom，您可以在`TRTCKtvRoom.java`文件中看到该组件提供的接口函数，并使用对应接口实现自定义 UI 界面。
<img src="https://min-cos-1300507594.cos.ap-beijing.myqcloud.com/tmp/ktv/ktv_form.png">

[](id:model.step1)
### 步骤1：集成 SDK
KTV 组件 TRTCKtvRoom 依赖 TRTC SDK 和 IM SDK，您可以按照如下步骤将两个 SDK 集成到项目中。
...
<dx-codeblock>
::: java java
Source/src/main/java/com/tencent/liteav/trtcktv/model
:::
</dx-codeblock>

[](id:model.step4)
### 步骤4：创建并登录组件
1. 调用 `sharedInstance` 接口可以创建一个 TRTCKTV 组件的实例对象。
...
<dx-codeblock>
::: java java
TRTCKTV mTRTCKtvRoom = TRTCKtvRoom.sharedInstance(this);
mTRTCKtvRoom.setDelegate(this);
mTRTCKtvRoom.login(SDKAPPID, userId, userSig, new TRTCKtvRoomCallback.ActionCallback() {
    @Override
...
::: java java
// 1.房主设置昵称和头像
mTRTCKtvRoom.setSelfProfile("my_name", "my_face_url", null);

// 2.房主调用 createRoom 创建房间
final TRTCKtvRoomDef.RoomParam roomParam = new TRTCKtvRoomDef.RoomParam();
...
mTRTCKtvRoom.createRoom(mRoomId, roomParam, new TRTCKtvRoomCallback.ActionCallback() {
    @Override
    public void onCallback(int code, String msg) {
        if (code == 0) {
            //3. 占0号座位
            mTRTCKtvRoom.enterSeat(0, new TRTCKtvRoomCallback.ActionCallback() {
                @Override
                public void onCallback(int code, String msg) {
                    if (code == 0) {
                    }
...
@Override
public void onSeatListChange(final List<TRTCKtvRoomDef.SeatInfo> seatInfoList) {
    // 展示您的麦位列表
}

// 5. 收到 onAnchorEnterSeat 事件通知
@Override
public void onAnchorEnterSeat(TRTCKtvRoomDef.UserInfo userInfo) {
}
:::
</dx-codeblock>
...
mTRTCKtvRoom.getRoomInfoList(roomList, new TRTCKtvRoomCallback.RoomInfoCallback() {
    @Override
    public void onCallback(int code, String msg, List<TRTCKtvRoomDef.RoomInfo> list) {
        if (code == 0) {
            // 此时可以刷新您自己的 UI 房间列表
        }
    }
...
mTRTCKtvRoom.enterRoom(roomId, new TRTCKtvRoomCallback.ActionCallback() {
        @Override
        public void onCallback(int code, String msg) {
            if (code == 0) {
                //进房成功
...
@Override
public void onRoomInfoChange(TRTCKtvRoomDef.RoomInfo roomInfo) {
...
::: java java
// 1: 房主抱人上1号麦位
mTRTCKtvRoom.pickSeat(1, "123", new TRTCKtvRoomCallback.ActionCallback() {
    @Override
    public void onCallback(int code, String msg) {
        // 2.收到 callback 回调
        if (code == 0) {
...
public void onAnchorEnterSeat(int index, TRTCKtvRoomDef.UserInfo user) {
}
:::
</dx-codeblock>
...
::: java java
// 1: 听众主动上2号麦位
mTRTCKtvRoom.enterSeat(2, new TRTCKtvRoomCallback.ActionCallback() {
    @Override
    public void onCallback(int code, String msg) {
        // 2.收到 callback 回调
        if (code == 0) {
...
String inviteId = mTRTCKtvRoom.sendInvitation("ENTER_SEAT", ownerUserId, "1", null);

// 2.收到邀请的同意请求, 正式上麦
@Override
public void onInviteeAccepted(String id, String invitee) {
    if(id.equals(inviteId)) {
        mTRTCKtvRoom.enterSeat(1, null);
    }
}
...
public void onReceiveNewInvitation(final String id, String inviter, String cmd, final String content) {
    if (cmd.equals("ENTER_SEAT")) {
        // 2.房主同意听众请求
         mTRTCKtvRoom.acceptInvitation(id, null);
    }
}
:::
</dx-codeblock>
...
String inviteId = mTRTCKtvRoom.sendInvitation("PICK_SEAT", "123", "2", null);

// 2.收到邀请的同意请求, 正式上麦
@Override
public void onInviteeAccepted(String id, String invitee) {
    if(id.equals(inviteId)) {
        mTRTCKtvRoom.pickSeat(2, null);
    }
}
...
- 通过 `sendRoomTextMsg` 可以发送普通的文本消息，所有在该房间内的主播和听众均可以收到 `onRecvRoomTextMsg` 回调。
 即时通信 IM 后台有默认的敏感词过滤规则，被判定为敏感词的文本消息不会被云端转发。
  ...
  ::: java java
  // 发送端：发送文本消息
  mTRTCKtvRoom.sendRoomTextMsg("Hello Word!", null);
  // 接收端：监听文本消息
  mTRTCKtvRoom.setDelegate(new TRTCKtvRoomDelegate() {
    @Override
    public void onRecvRoomTextMsg(String message, TRTCKtvRoomDef.UserInfo userInfo) {
...
mTRTCKtvRoom.sendRoomCustomMsg("CMD_DANMU", "Hello world", null);
mTRTCKtvRoom.sendRoomCustomMsg("CMD_LIKE", "", null);
// 接收端：监听自定义消息
mTRTCKtvRoom.setDelegate(new TRTCKtvRoomDelegate() {
    @Override
    public void onRecvRoomCustomMsg(String cmd, String message, TRTCKtvRoomDef.UserInfo userInfo) {

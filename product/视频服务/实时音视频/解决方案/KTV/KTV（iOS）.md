...
2. 找到并打开 `TUIKTV/Debug/GenerateTestUserSig.swift` 文件。
...
| 文件或文件夹 | 功能描述 |
|:-------:|:--------|
|TRTCKTVEnteryControl.swift|该文件包含所有 ViewController 的初始化获取方法，您可以通过该实例，快速获取 ViewController 对象。|
| TRTCCreateKTVViewController | 创建 KTV 页面逻辑。 |
| TRTCKTVViewController | 主房间页面，包括房主和听众两种界面。 |
...
[](id:model)
## 实现自定义 UI 界面
[源码](https://github.com/tencentyun/TUIKTV) 中的 `Source` 文件夹包含两个子文件夹 ui 和 model，model 文件夹中包含可重用的开源组件 TRTCKtvRoom，您可以在 `TRTCKtvRoom.h` 文件中看到该组件提供的接口函数，并使用对应接口实现自定义 UI 界面。
...
1. 将工程目录下的 `Source`、`Resources`、`TXAppBasic` 文件夹、`TUIKTV.podspec` 文件拷贝到您的工程目录下。
2. 在您的 `Podfile` 文件中添加以下依赖。之后执行 `pod install` 命令，完成导入。
<dx-codeblock>
::: swift
pod 'TXAppBasic', :path => "TXAppBasic/"
pod 'TXLiteAVSDK_TRTC'
pod 'TUIKTV', :path => "./", :subspecs => ["TRTC"] 
:::
</dx-codeblock>

[](id:model.step4)
### 步骤4：创建并登录组件
1. 调用 TRTCKtvRoom 的 `sharedInstance` 类方法可以创建一个遵守 TRTCKtvRoom 协议的实例对象。也可以使用调用 `shared` 类方法，获取 TRTCKtvRoom 实例对象直接使用，二者在 TRTCKtvRoom 的接口使用上没有任何区别。
...
// 设置 KTV 代理
self.ktv.setDelegate(delegate: ktvDelegate)

// 调用方式如下,闭包内建议使用 weak self 防止循环引用（下面示例代码省略 weak self 示例）
self.ktv.login(sdkAppId: sdkAppID, userId: userId, userSig: userSig) { [weak self] (code, message) in
...
::: swift
// 1.房主设置昵称和头像
self.ktv.setSelfProfile(userName: userName, avatarUrl: avatarURL) { (code, message) in
    // 结果回调           
}
...
self.ktv.createRoom(roomID: yourRoomID, roomParam: param) { (code, message) in
    guard code == 0 else { reutrn }
    // 创建成功
    
    // 3.上麦
    self.ktv.enterSeat(seatIndex: 0) { [weak self] (code, message) in
        guard let `self` = self else { return }
        if code == 0 {
            // 房主占座成功
        } else {
...
self.ktv.getRoomInfoList(roomIdList: roomIdsInt) { (code, message, roomInfos: [RoomInfo]) in
    // 获取结果，此时可以刷新UI
}

// 4.选择 KTV 后，传入 roomId 进入房间
self.ktv.enterRoom(roomID: roomInfo.roomID) { (code, message) in
    // 进入房间结果回调
    if code == 0 {
       // 进房成功
    }
...
::: swift
// case1: 房主抱人上1号麦位
self.ktv.pickSeat(seatIndex: 1, userId: "123") { (code, message) in
    // 结果回调
}
...
::: swift
// case2: 听众主动上2号麦位
ktv.enterSeat(seatIndex: 2) { (code, message) in
    // 上麦结果回调
}
...
let inviteId = self.ktv.sendInvitation(cmd: "ENTER_SEAT", userId: ownerUserId, content: "1") { (code, message) in
    // 发送结果回调
}
// 2.收到邀请的同意请求, 正式上麦
func onInviteeAccepted(identifier: String, invitee: String) {
    if identifier == selfID {
        self.ktv.enterSeat(seatIndex: ) { (code, message) in
            // 上麦结果回调
        }
    }
}
...
func onReceiveNewInvitation(identifier: String, inviter: String, cmd: String, content: String) {
    if cmd == "ENTER_SEAT" {
        // 2.房主同意听众请求
        self.ktv.acceptInvitation(identifier: identifier, callback: nil)
    }
}
:::
</dx-codeblock>
...
let inviteId = self.ktv.sendInvitation(cmd: "PICK_SEAT", userId: ownerUserId, content: "2") { (code, message) in
    // 发送结果回调
}
...
func onInviteeAccepted(identifier: String, invitee: String) {
    if identifier == selfID {
        self.ktv.pickSeat(seatIndex: ) { (code, message) in
            // 上麦结果回调
        }
    }
}
...
- 通过` sendRoomTextMsg` 可以发送普通的文本消息，所有在该房间内的主播和听众均可以收到 `onRecvRoomTextMsg` 回调。
  ...
  ::: Swift Swift
  // 发送端：发送文本消息
  self.ktv.sendRoomTextMsg(message: message) { (code, message) in
...
- 通过 `sendRoomCustomMsg` 可以发送自定义（信令）的消息，所有在该房间内的主播和听众均可以收到 `onRecvRoomCustomMsg` 回调。
 ...
 self.ktv.sendRoomCustomMsg(cmd: “CMD_DANMU”, message: "hello world", callback: nil)
 self.ktv.sendRoomCustomMsg(cmd: "CMD_LIKE", message: "", callback: nil)
 // 接收端：监听自定义消息

...
如果您正在为直播活动寻找一个推拉流的应用，如果您想要办一场讲座且在讲课的同时满足分享PPT的需求，如果您想要满足不同延迟度要求的直播场景，如果您需要在直播的过程中和在线观众进行互动，那么腾讯云快速推拉流场景化解决方案就是为解决您的需求而准备。
...
TUIPusher&TUIPlayer 基于腾讯云实时音视频和即时通讯服务进行开发，因此需要您 [注册腾讯云账号](https://cloud.tencent.com/register?s_url=https%3A%2F%2Fcloud.tencent.com%2Fdocument%2Fproduct%2F647%2F49327) 并开通 [实时音视频](https://console.cloud.tencent.com/trtc) 和 [即时通讯](https://console.cloud.tencent.com/im) 服务。

1. 在 [实时音视频控制台](https://console.cloud.tencent.com/trtc) 点击【应用管理】- 【创建应用】创建新应用

![创建应用](https://qcloudimg.tencent-cloud.cn/raw/6cc2f26bea3cf447ab3cdfa25771d312.png)

2. 在【应用管理】- 【应用信息】中获取 SDKAppID 信息

![获取sdkAppId信息](https://qcloudimg.tencent-cloud.cn/raw/77ea6b78806d1cc4652a913622ebdecb.png)

3. 在【应用管理】- 【快速上手】中获取应用的 secretKey 信息

![获取secretkey](https://qcloudimg.tencent-cloud.cn/raw/a8fb7b57b3d82ab1eef209387a15bebf.png)
...
![配置sdkAppId和secretKey信息](https://qcloudimg.tencent-cloud.cn/raw/9286fcb781fa37179f84e4bdcd85bfae.png)
...
5. 打开 `http://localhost:8080` 和 `http://localhost:8081` 即可体验 TUIPusher 和 TUIPlayer功能；
...
> ！
>
> 完成以上配置，您可以使用 TUIPusher & TUIPlayer 进行超低延时直播，如您需要支持快直播和标准直播，请继续阅读步骤三【旁路直播】。如不需要支持快直播和标准直播，可以步骤三【旁路直播】。

## 步骤三：旁路直播

TUIPusher & TUIPlayer 实现的快直播和标准直播依托于腾讯云的云直播服务，因此支持快直播和标准直播线路需要您开启旁路推流功能。

1.请在 [实时音视频控制台](https://console.cloud.tencent.com/trtc) 中为您正在使用的应用开启旁路推流配置，可按需开启指定流旁路或全局自动旁路。

![启用旁路推流](https://qcloudimg.tencent-cloud.cn/raw/c290e9cc9102c04722848c7d248439a7.png)

2.请在 [域名管理](https://console.cloud.tencent.com/live/domainmanage) 页面添加自有播放域名，请参考 [添加自有域名](https://cloud.tencent.com/document/product/267/20381)。
...
当您将 TUIPusher 及 TUIPlayer 用于生产应用时，在接入 TUIPusher 及 TUIPlayer 之外，您需要：

+ 创建用户管理系统，用于管理产品用户信息，包括但不限于用户Id,  用户名，用户头像等;

+ 创建房间管理系统，用于管理产品直播间信息，包括但不限于直播间Id,  直播间名称，直播间主播信息等;
...
+ 参考 TUIPusher/pusher.vue 及 TUIPlayer/player.vue 页面, 将用户信息、直播间信息、SDKAppId 及 userSig 等账号信息提交到 vu ex 的 store 进行全局存储，您就可以跑通推拉流两个客户端的所有功能。详细业务流程参见下图。

![推拉流应用场景](https://qcloudimg.tencent-cloud.cn/raw/dee6676ff3beeaffb327b9e43569e4ef.png)

## 注意事项

### 1. 平台支持

| 操作系统 | 浏览器类型                   | 浏览器最低版本要求 | TUIPlayer | TUIPusher | TUIPusher屏幕分享            |
| :------- | :--------------------------- | :----------------- | :-------- | :-------- | :--------------------------- |
...
### 2. 域名要求
   ...
   出于对用户安全、隐私等问题的考虑，浏览器限制网页在 https 协议下才能正常使用 TUIPusher & TUIPlayer的全部功能。为确保生产环境用户顺畅接入和体验 TUIPusher&TUIPlayer 的全部功能，请使用 https 协议访问音视频应用页面。

   注：本地开发可以通过 [http://localhost](http://localhost/) 协议进行访问。

   URL域名及协议支持情况请参考如下表格：

| 应用场景     | 协议                                  | TUIPlayer | TUIPusher | TUIPusher屏幕分享 | 备注 |
| :----------- | :------------------------------------ | :-------- | :-------- | :---------------- | :--- |
| 生产环境     | https协议                             | 支持      | 支持      | 支持              | 推荐 |
| 生产环境     | http协议                              | 支持      | 不支持    | 不支持            |      |
| 本地开发环境 | [http://localhost](http://localhost/) | 支持      | 支持      | 支持              | 推荐 |
| 本地开发环境 | [http://127.0.0.1](http://127.0.0.1/) | 支持      | 支持      | 支持              |      |

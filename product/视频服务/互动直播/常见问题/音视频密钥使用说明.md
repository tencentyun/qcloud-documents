...
## 音视频密钥使用说明
### 一、密钥及加密算法
#### 1、密钥： appId 对应音视频密钥，长度 16 字节；官网上可查,如下:
...
##### b）、把密文拼接成一段字符串；
##### c）、用 tea 加密对字符串加密，symmetry_encrypt 函数输出的字符串即为权限加密串(注意:不用要把二进制串转成 16 进制)；
#### 3、密文验证
...
#### 2、加密方式
##### a）、使用 google protobuf 序列化 ConnRoomSig 对象，输出二进制字符串；
##### b）、用 tea 加密对二进制字符串加密，symmetry_encrypt 函数输出的字符串即为加密串；
##### c）、把加密串转换成 16 进制字符串，大小写均可，如：
```
bytes_conn_room_sig: 

Permission key is optional. It is used to encrypt the protocol for audio/video permission when you join an ILVB room.
...
</th><th> <b>Custom/Note</b>
</th></tr>
<tr>
<td> cVer
</td><td> unsigned char/1
</td><td> Enter 0 for version number
</td></tr>
<tr>
<td> wAccountLen
...
</td><td> Expatriation time (current time + validity period) (in sec). 300 seconds is recommended.
</td></tr>
<tr>
<td> dwPrivilegeMap
</td><td> unsigned int/4
...
1.2 Encryption library and example: Attachment ["tea.zip"](http://qzonestyle.gtimg.cn/qzone/vas/opensns/res/doc/tea.zip)
...
(3) Encrypt the string using Tea. The string outputted by symmetry_encrypt function is the encrypted permission string;

## 4 Use of ILVB SDK
When you join the room through SDK, change to specified permission by using the following methods of the API AVMultiRoom:

```
  /*
  @brief Modify your permissions in the room.

  @details You can dynamically modify your uplink/downlink audio and video permissions during chat to achieve permission control and management from third-party aspect.

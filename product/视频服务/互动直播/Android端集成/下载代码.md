...
把随心播代码中的 appid 和 accountType 修改成开发者自己的。<br/>
...
### 引入 SDK
* **aar 方式集成**,(强烈推荐)<br/>
	如果您使用的 Android Studio 开发，那么导入 iliveSDK 非常简单。只需两行代码就可以搞定了 
	...
	(X.X.X 替换成对应版本号 比如 1.0.1)。同步完成之后可以在 build 文件夹中找到 livesdk 和 ilivesdk 文件夹。<br /> 
...
### 删除非 armeabi 架构 so 
* 由于目前只支持 armeabi 和 armeabi-v7a 架构，如果工程(或依赖库)中有多架构，需要在 build.gradle 中添加以下配置<br /> （如果包含子工程子工程也要加）
<pre>
android{
    defaultConfig{
        ndk{

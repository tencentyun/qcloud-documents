## 旁路直播
旁路直播功能可以把互动直播上行的数据转码成通用格式进行推流分发，以方便用户通过 Web 或流媒体播放器观看。<br/><br/>
...
参数名|参数类型|说明
:--:|:--:|:--:
ids|List<Long>|要停止推流的频道 ID 数组

Android 旁路直播功能的详细实现见[新随心播](https://github.com/zhaoyang21cn/ILiveSDK_Android_Demos)
...
视频云 Web 播放器是由腾讯视频云自主开发的一款播放器，同时支持 RTMP 和 HLS。演示地址为<http://live.qcloud.com/dy/test.html>，将启动推流后得到的观看地址输入文本框即可播放。更多详情请参阅[官方文档](http://video.qcloud.com/download/docs/QLIVE_Player_Web_SDK_Developer_Guide.pdf)。简单代码示例如下所示：
...
TcPlayer 是由腾讯视频云自主开发的一款播放器，同时支持 RTMP、FLV、HLS。演示地址为 http://imgcache.qq.com/open/qcloud/video/vcplayer/demo/tcplayer.html 更多详情请参阅官方文档 https://cloud.tencent.com/document/product/267/7479。
...
ffmpeg 是提供跨平台解决方案的开源音视频处理工具，最新 ffmpeg 已自带 rtmp。ffmpeg 下载地址为<http://www.ffmpeg.org>（注：ffmpeg 采用 LGPL 或 GPL 许可证(依据您选择的组件)，如使用，请遵循许可证的规则）代码可以参考 ffplay.c(如需编译 ffplay，需安装 SDL)，其简单开发步骤如下所示：
...
声音播放方式可以用 DirectShow，这里就不展开了（注：上面仅为简略调用步骤示意，实际使用需自行开发，并且还需很多细化工作，例如使用 jitterbuf 来防抖动，音视频同步等）
...
视频上行方网络超时断开后，推流后台会将频道保留 1 小时，这段时间内该用户重新推流所返回的**_观看地址不会改变_**。如果 1 小时内该用户仍未重新推流，则频道将会回收，即该用户重新推流所获得的观看地址会**_重新生成_**。


#### 直播频道的数量限制是与应用相关还是与腾讯云账号相关？

直播频道的数量上限是与腾讯云帐号绑定的，一个帐号下多个应用是共用频道资源的。例如，一个账号频道上限 50，该账号下的应用 A 使用了 30 个，则该账号下的其他应用就只有 20 个频道可用

...
|taskType	|必选	|string 数组|	需要 [服务类型]()|
|cosInfo	|可选|	CosInfo|COS 存储信息，如果使用封面和精彩片段等需要存储 COS 结果的，必须带有 COS 信息。|
|cacheTime	|可选	|int|	=0：不缓存，回调用户后立即删除；<br> &gt;0：结果缓存对应的时间，单位为分钟；<br>其他情况：缓存结果7天，下次同一 url 会直接返回之前的识别结果；|
...
|authType	|可选|	int	|1:key 托管；<br>2:腾讯云 CAM 授权；<br>0、空或其他值:bucket 授权；|
|authContent	|可选|	string	|authType=1:填本服务提供的托管id;<br>authType=2:填腾讯云 CAM 对应的 roleArn；|
...
- CAM 授权有访问频率和时间限制，bucket 授权方式更稳定，建议采用 bucket 授权方式。

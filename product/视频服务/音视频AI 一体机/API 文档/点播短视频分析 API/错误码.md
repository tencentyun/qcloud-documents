...
|-403|	The server is currently overloaded, please try again later.	|服务器当前任务过多，请稍候再试|
...
|-410|	Get COS temporary signature error.	|获取COS临时签名错误|
|-411|	This app id request too frequently.	|该app id访问过于频繁|
|-412|	Video decoding error.	|视频解码错误
|-413|	Failed to get managed COS Key.	|获取托管COS Key失败|
|-500|	Internal server error.	|后端服务异常|
|-501|	Request internal service error.	|请求内部服务错误|
|-502|	Analysis failed.	|识别失败|
|-503|	Result storage to cos failed.|	cos存储结果失败|

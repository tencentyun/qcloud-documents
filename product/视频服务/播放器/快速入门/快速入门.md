...
#### 定制开发
`SuperPlayer`提供了播放器最通用的播放器功能，建议您先阅读[iOS超级播放器]()文档。如果不能满足需求，您可以自行修改播放代码。

如果您对播放的定制不仅限于UI，需要进行更深层次的订制，您可以参考[TXLivePlayer]()和[TXVodPlayer]()了解播放器更底层的用法。

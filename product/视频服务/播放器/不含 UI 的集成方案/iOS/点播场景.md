...
::: fileid 方式
fileid 下载至少需要传入 appId 和 fileId，userName 不传入具体值时，默认为“default”。**注意：加密视频只能通过Fileid下载。**
...
dataSource.quality = TXVodQualityHD;  // 高清
dataSource.userName = @"default" // userName 不传入具体值时，默认为“default”。

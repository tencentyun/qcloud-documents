...
<dx-tabs>
::: 通过 URL 方式
  TXVodPlayer 内部会自动识别播放协议，您只需要将您的播放 URL 传给 startVodPlay(10.7之前使用startPlay) 函数即可。

```objectivec
NSString* url = @"http://1252463788.vod2.myqcloud.com/xxxxx/v.f20.mp4";
[_txVodPlayer startPlay:url]; // 10.7开始startPlay接口废弃，改为startVodPlay
[_txVodPlayer startVodPlay:url];// 10.7开始使用播放器之前，先通过{@link TXLiveBase#setLicenceURL}设置视频播放Licence
```
:::
::: 通过 fileId 方式
```objectivec
...
首次调用 startVodPlay(10.7之前使用startPlay) 之前，支持从指定时间开始播放。
...
软解和硬解的切换需要在切换之前先 **stopPlay**，切换之后再 **startVodPlay**(10.7之前使用startPlay)，否则会产生比较严重的花屏问题。

```objectivec
[_txVodPlayer stopPlay];
_txVodPlayer.enableHWAcceleration = YES;
[_txVodPlayer startPlay:_flvUrl type:_type]; // 10.7开始startPlay接口废弃，改为startVodPlay
[_txVodPlayer startVodPlay:_flvUrl type:_type];// 10.7开始使用播放器之前，先通过{@link TXLiveBase#setLicenceURL}设置视频播放Licence
```
...
```objectivec
// 播放视频 A: 如果将 isAutoPlay 设置为 YES， 那么 startVodPlay(10.7之前使用startPlay)调用会立刻开始视频的加载和播放
NSString* url_A = @"http://1252463788.vod2.myqcloud.com/xxxxx/v.f10.mp4";
_player_A.isAutoPlay = YES;
[_player_A startPlay:url_A]; // 10.7开始startPlay接口废弃，改为startVodPlay
[_player_A startVodPlay:url_A];// 10.7开始使用播放器之前，先通过{@link TXLiveBase#setLicenceURL}设置视频播放Licence
...
[_player_B startPlay:url_B]; // 10.7开始startPlay接口废弃，改为startVodPlay
[_player_B startVodPlay:url_B];// 10.7开始使用播放器之前，先通过{@link TXLiveBase#setLicenceURL}设置视频播放Licence
```

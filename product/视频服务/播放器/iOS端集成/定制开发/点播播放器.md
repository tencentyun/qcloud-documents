...
### step6：结束播放
结束播放时，如果要退出当前的 UI 界面，要记得用 <strong>removeVideoWidget</strong> 销毁 view 控件，否则会产生内存泄露或闪屏问题。
...
## 视频宽高 
- **视频的宽高（分辨率）**：站在 SDK 的角度，如果只是拿到一个 URL 字符串，它是回答不出这个问题的。要知道视频画面的宽和高各是多少个 pixel，SDK 需要先访问云端服务器，直到加载到能够分析出视频画面大小的信息才行，所以对于视频信息而言，SDK 也只能以通知的方式告知您的应用程序。 

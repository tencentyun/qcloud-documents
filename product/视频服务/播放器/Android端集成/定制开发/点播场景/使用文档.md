...
| TXVodConstants.VOD_PLAY_EVT_GET_PLAYINFO_SUCC | 2010 | 获取点播文件信息成功，通过 startPlay(TXPlayerAuthBuilder authBuilder) 接口播放视频会通过此事件回调播放器地址、视频封面等信息 |

获取点播文件信息成功（VOD_PLAY_EVT_GET_PLAYINFO_SUCC）返回的数据解析：

```java
String playUrl = param.getString(TXVodConstants.EVT_PLAY_URL);
String coverUrl = param.getString(TXVodConstants.EVT_PLAY_COVER_URL);
String playName = param.getString(TXVodConstants.EVT_PLAY_NAME);
String duration = param.getString(TXVodConstants.EVT_PLAY_DURATION);
List<String> imageUrls  = param.getStringArrayList(TXVodConstants.EVT_IMAGESPRIT_IMAGEURL_LIST);  //雪碧图下载url
String webVttUrl = param.getString(TXVodConstants.EVT_IMAGESPRIT_WEBVTTURL);  // web vtt描述文件下载URL
```
#### 结束事件
| 事件 ID                 |    数值  |  含义说明                |   
| :-------------------  |:-------- |  :------------------------ | 

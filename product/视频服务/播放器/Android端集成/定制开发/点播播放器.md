...
等到视频 A 播放结束，自动（或者用户手动切换到）视频 B 时，调用 resume 函数即可实现立刻播放。注意：设置了 autoPlay 为 false 之后，调用 resume 之前需要保证视频 A 已准备完成，即需要在监听到视频 A 的 PLAY_EVT_VOD_PLAY_PREPARED （2013 ，播放器已准备完成，可以播放）事件后调用。

...
2. 测试License能否更改Package name 和 BoundId?
   测试License是可以的，在控制台测试License信息右边有编辑按钮，可以点击编辑修改

3. 正式License能否更改Package name 和 BoundId?
  当前版本，正式License不能更改Package name 和 BoundId，后续版本会加以支持

4. License可以同时支持多个APP吗？
  一个License 只能对应一个Package name 和 BoundId，不支持多个APP

...
签名的计算我们有专门的 [文档](https://www.qcloud.com/document/product/266/9221) 进行介绍，文档内容比较详细，但简单概括一下就是如下三个步骤：

- **【步骤一】获取 SecretID+SecretKey**
你可以在 [云 API 密钥](https://console.qcloud.com/capi) 管理里获取或者创建一个 SecretID，并拿到其对应的 SecretKey：
![](//mc.qcloudimg.com/static/img/23f95aaa97adf3eeae3bf90470fe5122/image.png)

- **【步骤二】拼接待加密的明文字符串**
拼接签名明文字符串，形式为HTTP QueryString。格式如下：
```
Original=[SecretID]&f=[FileName]&fs=[FileSha]&t=[currentTime]&e=[expiredTime]&r=[rand]&uid=[uid]
```

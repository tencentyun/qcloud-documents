...
将如下脚本中的 `CLOUD_API_SECRETID`、`CLOUD_API_SECRETKEY` 配置 2.3 中获取到的 SecretId 和 SecretKey。然后登录云主机，直接在云主机上执行修改后的脚本。
 ...
 <font color='red'>注意：请在本地修改以下配置并复制，然后登录云主机在控制台粘贴回车执行。如果您使用图形化界面修改配置，把OutDefine.php内容全部替换为下面两行填写后的define代码即可</font>。
```bash
#!/bin/bash
echo "<?php
...
> 注意：
1、如果服务器没有配置证书，这里的云主机服务器地址需要用http，而不能用https。
2、APP_SVR_URL直接替换成云主机服务器ip，不要加lite根目录，比如：

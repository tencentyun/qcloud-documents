...
- **3.5： 修改云主机配置信息**
1、将如下脚本中的 `appId`、 `SecretId` 和 `SecretKey` 配置 2.3 中获取到的APPID、SecretId 和 SecretKey。然后登录云主机，直接在云主机上执行修改后的脚本。
...
```
2、在云主机输入启动服务命令直接启动服务,服务启动默认端口为: `8001`
启动服务：
```
...
pm2 delete litvideo;
```
3、 查看 **3.4** 中云主机的公网 ip，在浏览器中输入 `http://ip` 查看服务是否启动成功

在 [MobileLine Android 集成手册]() 中，我们根据控制台向导提供了最简单的方式来接入 MobileLine 的各种服务，同时您也可以不完全依赖向导来接入我们的移动分析以及其他服务。
...
每一个 MobileLine 服务都是一个单独的 SDK，其中移动分析服务模块 `com.tencent.tac:tac-core` 是其他所有模块的基础模块。

在您的应用级 build.gradle（通常是 app/build.gradle）添加移动分析服务 Analytics 的依赖：

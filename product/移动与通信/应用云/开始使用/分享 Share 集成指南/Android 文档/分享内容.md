分享 SDK 帮助您快速分享 app 内的内容，我们预设了分享的弹框界面，您可以直接使用，快速集成分享能力。下面的例子展示了如何调用 SDK，分享各种类型的内容，包括文字，图片，链接，文件等。
...
Bitmap musicAlbum = BitmapFactory.decodeResource(getResources(), R.drawable.music_album);
...
urlObject.setMetadata(new ObjectMetadata.Builder().title("WebPage title").description("WebPage description").build());

...
### 1. 注册应用
如果您还没有在 [微博开放平台](http://ope.weibo.com/) 注册您的应用，请先移步注册您的应用，并且获取应用**登录**和**分享**能力。（注意，若是应用没有被微博平台审核成功，则只有注册了此应用的微博账号才能使用分享功能)

### 2. 配置应用

在您的应用模块的 assets 文件夹下，新建一个名为 `tac\_service\_configurations\_weibo.json` 的文件，内容如下：

...
在启动登录的Activity的 onActivityResult 中添加 QQ 登录回调的处理，否则在某些低端机上可能无法正确处理回调。
...
在 app 包名下新建一个 wxapi 的包，然后新建一个名为 WXEntryActivity 的类，该类直接继承此基类即可，不需要增加任何逻辑。
...
然后在 AndroidManifest.xml 中注册该 Activity，设置 export = true。

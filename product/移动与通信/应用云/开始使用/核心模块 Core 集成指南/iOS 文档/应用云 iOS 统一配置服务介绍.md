...
通常我们在集成 SDK 的时候，需要对工程和 SDK 进行一些配置和初始化的操作。为了简化这两部分的工作，更加方便的集成配置 SDK，应用云实现了统一配置服务。
 
应用云支持通过脚本来配置工程需要的参数，同时也支持通过文件和程序配置 SDK。
...
通过这种多文件配置的方式，您可以对应用云的配置文件进行拆分和自定义操作。例如在不改变从官网下载的配置文件的基础上，实现自定义的参数配置。

...
## 创建 string 资源保存厂商通道推送信息

在 ../src/main/res/values 目录下创建 push.xml（文件名可以由您任意取值） 文件，并在其中添加如下 string 资源：

```
<resources>
    <string name="hw_appid">你自己申请的华为 appid</string>
    <string name="xm_appid">你自己申请的小米 appid</string>
    <string name="xm_appkey">你自己申请的小米 appkey</string>
    <string name="mz_appid">你自己申请的魅族 appid</string>
    <string name="mz_appkey">你自己申请的魅族 appkey</string>
</resources>
```

## AndroidManifest.xml 中配置厂商通道推送信息

在 AndroidManifest.xml 中添加应用级的 meta-data：
...
<application
 ...
    <!--添加如下 5 行-->
    <meta-data android:name="com.huawei.hms.client.appid" android:value="@string/hw_appid" />
    <meta-data android:name="com.message.xiaomi.appid" android:value="@string/xm_appid"/>
    <meta-data android:name="com.message.xiaomi.appkey" android:value="@string/xm_appkey"/>
    <meta-data android:name="com.message.meizu.appid" android:value="@string/mz_appid"/>
    <meta-data android:name="com.message.meizu.appkey" android:value="@string/mz_appkey"/>

</application>

```
> 这请通过 `@string/hw_appid` 的方式来引用华为 appid 等参数，不要直接使用 ：
> `<meta-data android:name="com.huawei.hms.client.appid" android:value="huawei_appid" />`，也不要直接使用 manifestPlaceholders 的方式来引入参数，否则数据可能会出错。

|参数|含义|
|:--:|:-|
|com.huawei.hms.client.appid|华为 AppId 对应的键|
|com.message.xiaomi.appid|小米 AppId 对应的键|
|com.message.xiaomi.appkey|小米 AppKey 对应的键|
|com.message.meizu.appid|魅族 AppId 对应的键|
|com.message.meizu.appkey|魅族 AppKey 对应的键|

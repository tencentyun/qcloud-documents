...
Storage 服务依赖于腾讯云 COS 服务，在使用服务时需要对请求进行访问鉴权。通过临时密钥机制，您可以临时授权您的 App 访问您的存储资源，而不会泄露您的永久密钥。密钥的有效期由您指定，过期后自动失效。**通常，我们都不建议您把永久密钥放到客户端代码中。**

本文主要介绍如何在后台快速搭建一个授权服务，通过下发到客户端的临时密钥来对上传或者下载请求进行签名，从而保证您数据的安全性。
...
### （建议）集成 STS（临时密钥服务） SDK

如果您已经有独立的后台服务，我们建议您直接集成我们提供的 STS SDK 到现在的后台服务中。
...
如果您想获取更多关于数据安全的设置，可以参考文章 [数据安全性最佳实践](https://cloud.tencent.com/document/product/666/17921)。

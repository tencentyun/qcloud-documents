...
## 邮箱校验服务
如果您不确定您的邮箱地址是否有效，或无效邮箱地址的比例过高（长期大于5%），将会使您的 IP 声誉，并保持在一个比较差的状态，这样的话可能会导致您的邮件更容易进入垃圾箱。如果您不确定您的邮箱地址的 IP 声誉，可以在发送之前调用该服务以校验邮箱地址是否正确，来保证您的发送效果。此项服务需要单独收费，请 [联系商务](https://cloud.tencent.com/act/event/connect-service) 开通此功能。
### 请求协议格式

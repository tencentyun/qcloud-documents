[](id:senderConfig)
## 发件人配置选项
邮件推送目前提供三种发送电子邮件的方式，包括腾讯云 SES 控制台、腾讯云 SES API 和腾讯云 SES SMTP。您可以使用 SES 命令行界面（SES CLI）或 [SES 开发工具包（SDK）来访问 API](https://cloud.tencent.com/document/product/1288/51062)。或调用 SMTP 接口发送邮件。

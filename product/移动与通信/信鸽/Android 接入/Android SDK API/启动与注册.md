...
注意：这里的帐号可以是邮箱、QQ 号、手机号、用户名等任意类别的业务帐号。目前一个帐号可以绑定最多十五个设备  token，超限后最新的会随机顶掉之前绑定的一个。
***（1）原型***
```
...
|-|-|-|
|context|当前应用上下文对象|不能为 null|
|account|绑定的账号，绑定后可以针对账号发送推送消息；如果要按别名推送，那就需要开发者在调用注册接口时把别名设置在注册请求里面的 account 字段，一台设备只允许有一个别名，但一个别名下可以有最多十五台设备|不能为 null|
|callback|callback 调用，主要包括操作成功和失败的回调|不能为 null|

...
### 一键导入依赖
AndroidStudio 建议采用 jcenter 自动接入，无需再配置文件中配置信鸽各个节点，全部由依赖导入。AndroidStudio 上可以使用 jcenter 远程仓库自动接入，不需要在项目中导入 jar 包和 so 文件；在 AndroidManifest.xml 中不需要配置信鸽相关的内容，jcenter 会自动导入。

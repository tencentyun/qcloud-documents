...
请开发者手动确认应用存储权限，然后查看 SD 卡目录下的 huawei.txt 文件内输出的注册华为失败的错误返回码，
最后再根据 [华为开发文档](http://developer.huawei.com/consumer/cn/service/hms/catalog/huaweipush_agent.html?page=hmssdk_huaweipush_api_reference_errorcode) 对应的错误代码以找到解决方法。
### 代码混淆
~~~
-ignorewarning

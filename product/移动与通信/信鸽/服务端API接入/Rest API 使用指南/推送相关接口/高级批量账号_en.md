If the number of destination accounts of a push is very large (for example, greater than 10000), it is recommended to use the API account_list_multiple.
...
| Parameter Name | Type | Required | Default | Description |
|-|-|-|-|-|
| account_list | string | Yes | None | Json array format, where each element is an account (string type). A maximum of 1000 accounts are allowed for a singe push. For example: ["account 1","account 2","account 3"] |

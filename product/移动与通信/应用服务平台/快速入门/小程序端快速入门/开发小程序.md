...
### QuickStart 项目
在微信开发者工具中新建项目，勾选创建 “云开发 QuickStart 项目”，点击创建即可得到一个展示云开发基础能力的示例小程序。该小程序由三个部分组成：
...
3. `cloudfunctions|(环境名)`：云函数根目录。在`project.config.json`中完成指定之后，云函数的根目录的图标会变成 “云目录图标”，右击根目录，可新建Node.js云函数，即云函数根目录下的第一级目录为云函数目录。在小程序中调用云函数前，需要先将该云函数部署到云端。如果对应云端存在该云函数，则会用特殊的云图标及Nodejs图标标明：

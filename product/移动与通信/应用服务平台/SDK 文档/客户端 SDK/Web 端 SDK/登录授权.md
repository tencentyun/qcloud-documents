...
借助这些临时匿名手段，应用的用户可以无需强制登录也可访问开发者的应用，开发者使用安全规则设置 DB 和 COS 的访问权限，匿名用户的数据会持续保留，直到开发者删除。

当匿名用户想注册应用，可以将匿名用户转成永久用户，匿名用户产生的数据转移到正式用户底下，匿名转为正式用户后将使用自定义登录方式来登录。
...
- 登录腾讯云云开发 [控制台](https://console.cloud.tencent.com/tcb)
- 进入指定云开发环境
- 在【环境-用户管理-登录设置】页面中
- 开启【匿名登录】开关

![云开发匿名登录](https://main.qcloudimg.com/raw/138f785a0b77868a5aac625febe35d20.jpg)

注意：关闭匿名登录后，由于后端同步原因，大约在 60s 后开始生效。

### 使用匿名访问

使用匿名登录来访问，引用客户端 SDK 后，需要按一下步骤配置客户端：
...
1、按照自定义登录的流程搭建获取自定义登录凭证 ticket 的服务，可参考 [自定义登录方式](https://cloud.tencent.com/document/product/876/34660#.E8.87.AA.E5.AE.9A.E4.B9.89.E7.99.BB.E5.BD.95)。
...
安全规则详情可参考 [安全规则使用指南](https://cloud.tencent.com/document/product/876/36414)。

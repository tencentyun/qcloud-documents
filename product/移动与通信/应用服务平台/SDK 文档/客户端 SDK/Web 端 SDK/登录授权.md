...
- 当匿名用户需注册应用，可以将匿名用户转成永久用户，匿名用户产生的数据转移到正式用户底下，匿名转为正式用户后，将使用自定义登录方式来登录。
...
![](https://main.qcloudimg.com/raw/7c294dc221bd091feb533a3599f1a501.png)


>!关闭匿名登录后，由于后端同步原因，大约在60s后开始生效。
...
```
4. 在函数内部可以使用 tcb-admin-node，获取用户信息进而判断当前用户是否为匿名状态，示例代码如下：
```js
const tcb = require('tcb-admin-node');
tcb.init({
  env: 'your-envId'

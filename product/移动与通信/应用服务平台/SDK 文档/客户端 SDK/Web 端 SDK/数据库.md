...
以下对几个特殊的数据类型做个补充说明：
- **地理位置**：详情请参考 [GEO地理位置](#GEO地理位置)。
- **Null**：相当于一个占位符，表示一个字段存在但是值为空。
-  **Date**：用于创建客户端时间，精确到毫秒，可以用 JavaScript 内置 Date 对象创建。如果需要使用服务端时间，应该用 API 中提供的 serverDate 对象来创建一个服务端当前时间的标记。
>?使用客户端时间时，存放 Date 对象和存放毫秒数是不同的。我们的数据库有针对日期类型的优化，建议您使用 Date 或 serverDate 构造时间对象。
...
方法2： collection.doc().set()

通过 `set` 方法新增一个文档，需先取得文档引用再调用 `set` 方法。
...
>!默认取前100条数据，最大取前100条数据。

### 添加查询条件
collection.where()

#### 设置过滤条件
where 可接收对象作为参数，表示筛选出拥有和传入对象相同的 key-value 的文档。比如筛选出所有类型为计算机的、内存为8g的商品。示例代码如下：
...
如果要表达更复杂的查询，可使用高级查询指令，比如筛选出所有内存大于8g的计算机商品。示例代码如下：
```js
...
例如，筛选出内存大于4g小于32g的计算机商品。示例代码如下：
...
表示需满足所有指定条件中的至少一个。如筛选出价格小于4000或在6000-8000之间的计算机。示例代码如下：
...
如果要跨字段 “或” 操作：（如筛选出内存8g或 cpu 3.2 ghz 的计算机）。示例代码如下：

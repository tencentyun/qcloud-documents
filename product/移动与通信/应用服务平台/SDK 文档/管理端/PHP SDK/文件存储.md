...
`$options`         | Array         | 可选参数
 ...
 - `max-keys`      | Number        | 单次返回最大的条目数量，默认值为 `1000`，最大为 `1000`
...
参数名                  |  类型    | 描述
-----------------------|---------------|-------------------
`$key: string`         | String        | `ObjectKey`
`$options`             | Array         | 可选参数
 - `expires`           | String        | 有效期，默认为 `10分钟`，请注意设置合理的有效期，格式为 [strtotime](https://php.net/manual/en/function.strtotime.php) 函数所接受的字符串

本增值服务已整合腾讯云的 [实时音视频](https://cloud.tencent.com/product/trtc) 能力，通过云开发的云函数和数据库的能力，简化配置拉取和房间管理等功能。

## 体验增值服务
![](https://main.qcloudimg.com/raw/5dffe79d2f0b28f7212c17c6a77be62e.png)
...
##  DEMO 接入流程
1. 在小程序的管理后台选择 【设置】> 【基本设置】 > 【服务类目】中添加允许视频直播类的管理。
...
3. 在小程序管理后台选择【开发】> 【开发设置】中，配置以下三个域名：
...
![](https://main.qcloudimg.com/raw/1f70ed8e62c032882e4b6eb6b4da6283.png)

 - 下载 private_key 文件
![](https://main.qcloudimg.com/raw/eadb9d40ef162776f85f41cfb04bc57d.png)
...
6. 请按以下步骤完成操作。
  ...
  - 并在`config`目录下，参照`example.js`文件，新建 `index.js` 文件，并配置好 SDKAppid 和 accountType；
  - 然后上传部署所有的云函数。
...
## 源码介绍
### WebRTC 能力
WebRTC 的能力的体验，主要是围绕源码中 `client/pages/webrtc-room` 里面的 `join-room` 和 `room` 两个目录，一个是手动输入房间号进入房间，另一个是视频房间 `cloud/functions/webrtc-sig-api` 目录，此云函数主要用于填写实时音视频的配置后，进行加密，将加密好的信息传到小程序端，才能正常使用 WebRTC 视频通话能力。

云函数 `webrtc-sig-api` 中，`WebRTCSigApi.js` 是官方提供的 [签名逻辑文件](https://github.com/TencentVideoCloudMLVBDev/usersig_server_source/blob/master/nodejs/WebRTCSigApi.js)，而 `index.js`，调用 `WebRTCSigApi.js` 中的方法，然后将 `privateMapKey`, `userSig` 提供到小程序，小程序端才能正常将直播流唤起。

### 房间管理
房间管理主要通过云开发的云函数和数据库实现。`webrtcRooms` 集合的数据格式如下：
![](https://main.qcloudimg.com/raw/1f06945c2b42e7aa3ee44dd6ba6a7951.png)

...
2. 开通后，进入静态网站页面即可查看静态网站域名。
 - 目前云开发仅提供了最简单的托管能力，并仅支持一个默认的静态网站域名。此域名支持 HTTP 与 HTTPS。
 - 此域名作为云开发提供的默认域名，将有一定的使用限制。当您需要正式对外开放网站时，需要使用您自己注册的域名，绑定使用。自定义绑定域名功能即将上线。
![](https://main.qcloudimg.com/raw/3692cb2180a32f68183110f08a8b33e8.png)
3. 使用 CLI 工具进行静态托管文件管理，目前云开发静态托管服务仅能使用 CLI 工具进行文件管理，详情请参见 [管理托管文件](#gltgwj)。

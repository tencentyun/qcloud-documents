云开发（CloudBase）提供了一个控制台，供开发者对云资源进行创建、管理、配置和统计分析，实现对云资源的可视化管理。目前提供的云资源有数据库、文件存储和云函数。下面将进行 CloudBase 控制台的具体介绍。



## 环境

在云开发里，所有的资源都是通过环境来进行分配和管理的。每个环境都有一个全局唯一的环境 ID，在使用云资源时，都通过这个环境 ID 来进行识别和关联。因此，您需要先创建环境，再对每个环境下的云资源进行配置。

>!目前，在 CloudBase 控制台中创建的环境仅支持开发网站应用。

登录 [ CloudBase 控制台](https://console.cloud.tencent.com/tcb) ，根据需求创建环境，详细创建步骤请参考 [开通云开发](https://cloud.tencent.com/document/product/876/34607#.E5.BC.80.E9.80.9A.E4.BA.91.E5.BC.80.E5.8F.91) 。创建完成后，即可对该环境进行云资源管理。
...
### 概览
概览模块包括**环境详情**、**数据概览&监控**和**关联应用**，方便您查看云资源的总体使用情况。
- 环境详情：查看当前环境的详细信息，包括环境名称、环境 ID、套餐版本和环境来源。
- 数据概览&监控：主要聚焦于 API 今日调用和当前资源用量，此外还包括活跃用户数、API 调用监控和 CDN 下行流量监控。
- 关联应用：让您的网站应用能够正常使用云开发资源。详细步骤请参考 [关联网站应用](https://cloud.tencent.com/document/product/876/34607#.E5.85.B3.E8.81.94.E7.BD.91.E7.AB.99.E5.BA.94.E7.94.A8)。
...
### 用户管理
用户管理模块提供了**应用访问用户**和**安全设置**，帮助您更好地了解网站应用用户的信息，并对指定用户进行管理。
- 应用访问用户：查看网站应用的访问用户信息。
>!访问用户进入应用时，如用户没有授权访问个人信息，则相关字段会展示为“未授权”。在用户管理列表，可搜索和查找复制用户的 OpenID。如果是非小程序来源的云开发环境，则可以搜索用户的 UUId。
- 安全设置包括：
 - 微信平台授权：在您的网站应用中接入微信登录授权。
 - 域名授权：在您的网站应用中使用云开发的身份验证服务。

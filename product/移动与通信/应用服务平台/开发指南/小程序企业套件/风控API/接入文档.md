...
目前 微信安全风险识别能力 需要申请使用，发送邮件到 elontian@tencent.com，填写 公司名、对接人手机号码、小程序名、使用场景说明。我们将在3个工作日内，与您联系接入使用。
...
参数名称|描述|类型|是否必选
|--------|--------|--------|--------|
access_token|string|是|开发者票据
service|string|是|服务 ID，业务风险识别服务ID：wxd8a88093cf541a77
api|string|是|接口名，业务风险识别接口名：weopensecriskservice
data|string|是|服务提供方接口定义的 JSON 格式的数据
client_msg_id|string|是|随机字符串 ID，调用方请求的唯一标识，固定值：client_msg_id_wxd8a88093cf541a77
...
user_access_token|用户票据，仅移动应用必须|string|仅移动应用必须
mchcode|商户号|string|否
phonenum|用户手机号|string|否
certid|证件ID|string|否
wifibssid|用户网络环境WIFI信息|string|否
clientip|用户客户端IP|string|否
deviceid|用户设备信息（安卓：IMEI；iOS：IDFA）|string|否
...
|--------|--------|--------|--------|
code|返回状态码|number|是
msg|返回说明信息，若code码正常则此处为空，若code码为错误则次数说明错误原因|string|是

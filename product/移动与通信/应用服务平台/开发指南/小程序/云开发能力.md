...
```
[初始化文档](../../服务端%20SDK%20文档/Node%20SDK/概览.md)
...
[文件存储文档](../../服务端%20SDK%20文档/Node%20SDK/文件存储.md)
...
[数据库文档](../../服务端%20SDK%20文档/Node%20SDK/数据库.md)
...
[云函数文档](../../服务端%20SDK%20文档/Node%20SDK/云函数.md)

通过本章，相信你已经知道如何在小程序端和服务端使用 SDK 去操作各类云资源。接下来，我们会分别详情讲述云函数、数据库和文件存储详细的使用方法。

## 语法糖
大部份的接口，目前都支持两种写法，分别是Promise 和 Async/Await，本节以  `callFunction` 作为例子，在`云函数`中介绍这两种写法。 Async/Await 本质上是基于 Promise 的一种语法糖，它只是把 Promise 转换成同步的写法而已。
...
下面三章，会详细介绍三大功能：[文件存储](./文件存储/概览.md)、[数据](./数据/概览.md)和[云函数](./云函数/概览.md)，在小程序端、服务端的各种用法。
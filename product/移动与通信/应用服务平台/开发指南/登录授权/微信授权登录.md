...
首先调用 [`Provider.signInWithRedirect()`](https://docs.cloudbase.net/api-reference/web/authentication.html#weixinauthprovider-signinwithredirect)，用户将会跳转到微信 OAuth 授权页面：
...
然后调用 [`Provider.getRedirectResult()`](https://docs.cloudbase.net/api-reference/web/authentication.html#weixinauthprovider-getredirectresult)，获取登录结果：

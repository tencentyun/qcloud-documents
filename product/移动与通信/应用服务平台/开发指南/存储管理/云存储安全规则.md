...
云存储中，可以通过安全规则实现：
1. 对不同类型的文件进行对应的权限控制；
2. 针对某个文件/文件夹进行权限控制，以限定特定文件/文件夹的访问权限。

### 对文件类型进行权限控制
在某个网站应用中，图片属于登录用户可写或者公共用户均可访问，用户文档只有创建者可读写。
<dx-codeblock>
:::  json
{
  "read": "/*.[jpg|png|jpeg]/.test(resource.path) || (/*.doc/.test(resource.path) && resource.openid == auth.uid)",
  "write": "(/*.[jpg|png|jpeg]/.test(resource.path) && auth != null) || (/*.doc/.test(resource.path) && resource.openid == auth.uid)"
}
:::
</dx-codeblock>
...
### 对文件/文件夹访问进行权限控制
在云存储所有的文件中，有2个文件夹，`/uploads`和`/public`，其中`/public`公共用户均可访问，`/uploads`只有创建者可读写。
<dx-codeblock>
:::  json
{

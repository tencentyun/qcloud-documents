...
```
## 其他
云函数中的时区默认是 UTC+0，在函数中使用 date.getHours 等方法获取时间信息的时候会和北京时间有 8 个小时的差异。定义 TZ 环境变量可以定义函数运行时的时区，比如设置 TZ 为 Asia/Shanghai 可以指定函数的时区为北京时间，有效的时区可以参考 https://en.wikipedia.org/wiki/List_of_tz_database_time_zones. 事实上，在服务端处理报错时间 (包括云数据库和云函数) 建议使用时间戳或者时间对象，这样可以避免服务端和客户端时间差异代理的众多问题。

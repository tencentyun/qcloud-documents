...
当小程序端调用云函数时，云函数的传入参数中会被注入小程序端用户的 `openid`，开发者无需校验 `openid` 的正确性，可以直接使用该 `openid`。与 `openid` 一起同时注入云函数的还有小程序的 `appid`。来自小程序端的调用 `openid` 即为小程序的 `openid`，其它端（Web 等）则为 CloudBase 生成的唯一 ID。

...
如果开发者没有详细指定写操作具体类型的规则时，默认读取 `write` 的规则。比如说开发者的安全规则配置如下
...
| in          | 存在集合中                                         | auth.uid in ['zzz','aaa']                            | 用户的 uid 是['zzz','aaa']中的一个                                         |
| !(xx in []) | 不存在集合中，使用 in 的方式描述 `!(a in [1,2,3])` | !(auth.uid in ['zzz','aaa'])                         | 用户的 uid 不是['zzz','aaa']中的任何一个                                   |
...
1. 在未使用变量的情况下，每个 `get` 会产生一次读操作，在使用变量时，每个 get，每个变量值会产生一次读操作，例如：规则 _get(\`database.collection.\${doc.\_id}\`).test_，在查询 `_.or([{_id:1},{_id:2},{_id:3},{_id:4},{_id:5}])}` 下会产生 5 次读取。系统会对同 doc，同 field 的读取进行缓存。

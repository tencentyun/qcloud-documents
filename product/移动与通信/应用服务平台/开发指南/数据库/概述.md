...
数据库 API 分为客户端（小程序端、网页端）和服务端，客户端 API 拥有严格的调用权限控制，开发者可在小程序或者网页内直接调用 API 进行非敏感数据的操作。对于有更高安全要求的数据，可在云函数内通过服务端 API 进行操作。云函数的环境是与客户端完全隔离的，在云函数上可以私密且安全的操作数据库。

...
4. 单击“导入到 IDE”,导入成功并弹出提示弹窗。会在**打开当前控制台的目录**下创建  `page_module_{模块名}` 目录，如遇文件夹冲突则会自动递增。组件导入成功后将自动导入示例页面路径到 `app.json` 内的 [`pages`](https://developers.weixin.qq.com/miniprogram/dev/reference/configuration/app.html#pages) 中。
...
3. 如果有配置不满足需求，并具有通用性，可前往 [github](https://github.com/TencentCloudBase-PageModule) 提 issue，模板开发者将进行跟进处理。如未采纳需求，建议自行对组件进行二次开发。

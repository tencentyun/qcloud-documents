...
### 输出日志
相比原有的 console 对象输出日志，开发者使用云开发自定义日志输出同样提供四个日志等级，例如`log`、`info`、`warn`、`error`。开发者自定义输出的日志内容会自动增加日志中的字段并建立键值索引，详情请参见 [输出日志](https://cloud.tencent.com/document/product/876/38040)。

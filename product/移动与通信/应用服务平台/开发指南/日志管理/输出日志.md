## 操作场景
用户可以使用原有日志输出方式（console对象输出日志），也可以使用 tcb-admin-node 封装的自定义输出日志。
- 使用 console 输出日志：可使用日志等级`log` 、 `info` 、 `warn` 、 `error`不会自动建立 [键值索引](#键值索引)，其中日志内容会封装至日志内容中 msg 字段的值。
- 用户自定义输出日志：需要导入 tcb-admin-node，可使用`log` 、 `info` 、 `warn` 、`error` ，日志内容会自动增加日志字段并建立 [键值索引](#键值索引)。
...
>?`console.log("this is  a log")`日志输出格式同上。
...
>?`console.info("this is a info")`日志输出格式同上。
...
```
>?`console.warn("this is a warn")`日志输出格式同上。
...
>?`console.error("this is a error")`日志输出格式同上。
...
>!
- 用户自定义输出了日志对象内容则会增加自定义的日志字段，并建立 [键值索引](#键值索引)。

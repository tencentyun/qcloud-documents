...
<table>
	...
	<td rowspan="2"><ul style="margin: 0px;">
		<li>函数数量限制150个</li>
		<li>支持固定外网 IP</li>
		<li>资源使用量GBs = 函数配置内存GB × 运行计费时长s。例如，配置为256MB的函数，单次运行了1760ms，计费时长为1800ms，则单次运行的资源使用量为 (256/1024) × (1800/1000) = 0.45GBs。云函数运行时最大可用内存为256MB。在云函数运行日志中展示的运行内存信息，为当次运行时的实际使用内存。实际使用内存可能低于最大可用内存，计费时按配置内存即256MB计算。</li></ul>
	</td>
    </tr>
    <tr>
        <td>外网出流量（GB）</td>
...
- 未使用的资源包可在购买后7天内退款。
- 除云托管资源包外，其他资源包的有限期为3个月（云托管资源包支持多种有效期）。
- 资源包过期后，资源包内的资源自动作废。

...
<table>
    ...
    <tr>
        ...
        <td>资源使用量（GBs）</td>
        <td>0.00011108</td>
				<td rowspan="2"><ul style="margin: 0px;"><li>函数数量限制150个</li><li>支持固定外网 IP</li></ul></td>
    </tr>
    <tr>
        <td>外网出流量（GB）</td>
        <td>0.8</td>
    ...
    <tr>
        ...
        <td>容量（GB/天）</td>
        <td>0.07</td>
	     	<td rowspan="3"><ul style="margin: 0px;"><li>同时连接数1000个</li><li>集合限制800个</li><li>备份保留天数14天</li><li>支持指定回档时间</li></ul></td>
    </tr>
    <tr>
        <td>读操作数（万次）</td>
        <td>0.015</td>
    ...
    <tr>
	 ...
	 <tr>
        ...
        <td>CPU（核 × 小时）</td>
        <td>0.055</td>
        <td> 1（核 × 小时）表示1核的 CPU 连续运行1个小时所消耗的资源量，或2核的 CPU 连续运行30分钟所消耗的资源量。1（核 × 小时） = 60（核 × 分钟） = 360a0（核 × 秒）。</td>
    </tr>
    <tr>
        <td>内存（GiB × 小时）</td>
        <td>0.032</td>
...
- 云开发资源包开始使用后不可退货，过期不可用。未使用的资源包在购买后7天内可通过工单退款。
- 除云托管资源包外，其他资源包有效期均为3个月（云托管资源包有多种有效期可选）。
...
## 微信网关套餐
微信网关，将小程序云开发的私有链路产品化。可帮助企业原有业务进行平滑接入，可有效解决以下问题 ：
- DDoS
- 劫持流量
- 竞争对手扫描扒取数据

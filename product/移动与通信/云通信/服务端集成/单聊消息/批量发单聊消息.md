...
#### 2.6.2 管理员指定某一帐号向目标帐号批量发消息，同时设置离线推送信息
From_Accout为管理员指定的发送方，接收方看到发送者不是管理员，而是From_Account。下述Json请求表达的是dave向帐号bonnie和帐号rong发送一条消息。 bonnie和rong收到消息，看到消息发送方是dave。
注意，若不希望将消息同步至From_Account，则SyncOtherMachine填写2。若希望将消息同步至From_Account，则SyncOtherMachine填写1
```
{

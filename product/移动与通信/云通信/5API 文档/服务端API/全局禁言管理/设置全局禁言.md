...
- 如果帐号被设置单聊消息全局禁言，在单聊消息禁言时间未到期时间内，所有的单聊消息发送失败，发送者接收到错误码20012（详见 [错误码](https://cloud.tencent.com/document/product/269/1671)），不会触发 [发单聊消息之前回调](https://cloud.tencent.com/document/product/269/1632)；禁言时间到期后 IM 云通信后台自动解除单聊消息禁言，解除后所有单聊消息就能发送正常；对于永久单聊消息全局禁言，全局单聊消息禁言时间一直不过期；帐号默认未设置单聊消息全局禁言。
- 如果帐号被设置群组消息全局禁言，在群组消息禁言时间内所有的群组消息发送失败，发送者接收到错误码10017（详见 [错误码](https://cloud.tencent.com/document/product/269/1671)），不会触发 [群内发言之前回调](https://cloud.tencent.com/document/product/269/1619)；禁言时间到期后 IM 通信后台自动解除群组消息禁言，解除后所有群组消息就能发送正常；对于永久群组消息全局禁言，全局群组消息禁言时间一直不过期。帐号默认未设置群组消息全局禁言。

...
>!
>- 互动直播聊天室和在线成员广播大群不支持导入群基础资料，对这两种类型的群组进行操作时会返回 10007 错误；如果需要达到导入群组基础资料的效果，可以通过 [创建群组](/doc/product/269/创建群组) 和 [修改群组基础资料](/doc/product/269/修改群组基础资料) 的方式实现。
...
通过 [REST API在线调试工具](https://avc.cloud.tencent.com/im/APITester/APITester.html#group_open_http_svc/import_group) 调试本接口。

## 参考

- 设置成员未读计数（[v4/group_open_http_svc/set_unread_msg_num](/doc/product/269/1637)）
- 导入群成员（[v4/group_open_http_svc/import_group_member](/doc/product/269/1636)）
- 解散群组（[v4/group_open_http_svc/destroy_group](/doc/product/269/1624)）

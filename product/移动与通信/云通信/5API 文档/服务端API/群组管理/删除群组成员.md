...
|公开群（Public）|是|
|聊天室（ChatRoom）|是|
|互动直播聊天室（AVChatRoom）|否（见说明 1）|
|在线成员广播大群（BChatRoom）|否（见说明 2）|

云通信内置以上五种群组类型，详情请参阅 [群组系统](https://cloud.tencent.com/document/product/269/1502)。

>!
>1. 互动直播聊天室不支持删除群成员，对这种类型的群组进行操作时会返回 10004 错误。如果管理员希望达到删除群成员的效果，可以通过设置 [批量禁言和取消禁言](https://cloud.tencent.com/document/product/269/1627) 的方式实现；

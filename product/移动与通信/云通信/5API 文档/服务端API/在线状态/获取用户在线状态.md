...
| State | String | 返回的用户状态，目前支持的状态有：<br>1."Online"：客户端登录后和云通信后台有长连接；<br>2."PushOnline"：iOS 客户端退到后台或进程被杀或因网络问题掉线，进入 PushOnline 状态，此时仍然可以接收消息离线 [APNS 推送](https://cloud.tencent.com/document/product/269/9154)；<br>3."Offline"：客户端主动退出登录或者客户端自上一次登录起 7 天之内未登录过|

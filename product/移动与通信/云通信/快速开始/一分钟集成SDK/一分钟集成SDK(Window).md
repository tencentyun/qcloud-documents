本文介绍如何快速地将腾讯云的 IM SDK 集成到项目中，只要按照如下步骤进行操作，可以轻松完成 SDK 的集成工作。
...
在官网下载 Windows SDK，解压并打开 IM SDK 文件夹，包含以下几个部分：

| 目录名       | 说明                                             |
| ------------ | ------------------------------------------------ |
| includes     | 接口头文件                                      |
| libs\Debug   | **Debug 模式**，采用/MTd 编译生成的.lib 静态库文件和 dll 动态库文件 |
| libs\Release | **Release 模式**，采用/MT 编译生成的.lib 静态库文件和 dll 动态库文件 |

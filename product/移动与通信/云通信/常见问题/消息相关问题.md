...
如果以上步骤都无法确认问题，建议您获取日志信息，结合 [IMSDK 日志格式](https://cloud.tencent.com/document/product/269/6042) 说明，查看客户端有没有收到消息，如果收到消息，逐步确认具体确错误点。

### 收不到离线推送怎么处理？

- APNs
 - 确认是否按照 [上传证书到控制台](待补超链) 正确上传证书到腾讯云控制台；
 - 确认在登录成功后，是否成功[上传 token 到腾讯云](待补超链)；
 - 确认在上报 token 时，是否上报了正确的证书 id；
 - 确认是否正确上报了 [切前后台事件](待补超链)；
...
红包消息与 @ 消息类似，可以通过 `TIMCustomElem` 来实现。需要应用在 UI 上做相应的特殊处理，比如检查到当前消息为红包消息后，消息展示为红包的样式。
另外，红包消息作为重要消息，最好在发送消息的时候将其设置为高优先级消息，以最大程度保证消息在触达频率限制的情况下仍可以送达（目前群内消息默认限制频率 40 条/s, 单聊消息默认限制频率为 5 条/s）。

关于消息优先级相关的内容可以参考 [消息优先级](超链待补)。

>!红包消息的支付部分功能，需要应用自行集成相应的支付 SDK，IMSDK 暂不提供这部分功能。

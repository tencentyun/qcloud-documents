...
3. 确认该自定义字段的设置请求是否成功；
4. 对于群维度的自定义字段，需要在登录 IMSDK 之前，通过 TIMManager -> setUserConfig -> TIMUserConfig -> TIMGroupInfoOption -> groupCustom 进行相应的配置。
5. 对于群成员维度的自定义字段，需要在登录 IMSDK 之前，通过 TIMManager -> setUserConfig -> TIMUserConfig-> TIMGroupMemberInfoOption -> memberCustom 进行相应的配置。

...
若您需要通过魅族推送进行 IM 消息的推送通知，必须在**用户登录成功后**通过 `TIMManager` 中的 `setOfflinePushToken` 方法将您托管到云通信 IM 控制台生成的 **证书 ID** 及魅族推送服务端生成的 **PushId** 上报到云通信 IM 服务端。
...
> - 若 IM 用户已经 logout 或被云通信 IM 服务端主动下线（例如在其他端登录被踢等情况），则该设备上不会再收到消息推送。

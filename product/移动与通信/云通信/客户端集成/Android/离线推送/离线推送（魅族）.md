...
若您需要通过魅族推送进行 IM 消息的推送通知，必须在 **用户登录成功后** 通过 `TIMManager` 中的 `setOfflinePushToken` 方法将您托管到云通信控制台生成的 **证书 ID** 及魅族推送服务端生成的 **PushId** 上报到云通信服务端。

>!
> 正确上报 PushId 与证书 ID 后，云通信服务才能将用户与对应的设备信息绑定，从而使用魅族推送服务进行推送通知。
...
成功上报证书 ID 及 PushId 后，云通信服务端会在该设备上的 IM 用户 logout 之前、APP 被 kill 之后，将消息通过魅族推送通知到用户端。
...
2. 按照推送的流程，确认魅族推送证书信息是否正确配置在 [云通信控制台](https://console.qcloud.com/avc)中。

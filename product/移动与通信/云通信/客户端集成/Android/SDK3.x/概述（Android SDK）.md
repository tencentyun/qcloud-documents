...
ImSDK3.x 对模块的功能及类的接口进行了优化调整。

为了让 ImSDK 更加的灵活轻便，在 3.x 的时候对 ImSDK 进行了模块功能调整。原来的 `imsdk.jar` 只保留了基本的功能，一些高级功能分离到了相应的扩展包中。调整后原来的 `imsdk.jar` 变成了如下几个包：
+ 基础功能包 —— `imsdk.jar`
...
这里只列出名字变更或者移到其他类的接口，参数变更请参考下载好的 SDK 包中的 API Javadoc 文档。具体见下表：

...
| 包名 | 描述 | 
|---------|---------|
| imsdk_ugc_ext.jar | ImSDK小视频功能扩展Java封装，提供小视频上传、小视频消息收发功能 |
| txrtmpsdk.jar | 小视频功能SDK库Java封装，提供小视频录制、编辑、预览等功能<br>具体功能可以参考[短视频SDK功能列表](/document/product/584/9457)。 |
| cos-sdk-android.1.4.3.6.jar | 对象存储 COS 相关的 jar 包 |
| okhttp-3.2.0.jar | 网络请求库 |
| okio-1.6.0.jar | 网络操作 I/O 库 |
| lib_imcore_ugc_ext_gyp.so | ImSDK小视频功能扩展核心组件 |
| libtxrtmpsdk.so | 直播SDK 核心组件 |

因为小视频扩展功能包只提供了`armeabi`、`armeabi-v7a`和`arm64-v8a`这三种CPU架构的so库，所以在编译的时候，最好通过`abiFilters`把`x86`架构排除掉。
```sh
# build.gradle文件
		...
		for(TIMMessage msg : timMessages) {
			for(int i = 0; i < msg.getElementCount(); i++){
				...
				if(elem.getType() == TIMElemType.UGC){
					...
					//下载小视频消息封面文件，并保存到coverPath指定的路径
					...
					//下载小视频消息视频文件，并保存到videoPath指定的路径

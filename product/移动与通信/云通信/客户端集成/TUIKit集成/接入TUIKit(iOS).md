...
#### 手动集成
##### 1. 从 [Github](https://github.com/tencentyun/TIMSDK) 下载 TUIKit 开发包，其中 TUIKit.framework 和 ImSDK.framework 所在的位置如下：
...
#### 2. 集成 TUIKit.framework 和 ImSDK.framework
...
**4. 重启Xcode 之后，重新打开 TUIKitDemo ，先选择 TUIKit 工程编译生成 TUIKit.framework 。**
![](https://main.qcloudimg.com/raw/3c32f587941aa511fa3d1b93b5c67415.png)

***注意：TUIKit 编译会依赖 ImSDK.framework，参考下图在 TUIKit -> Build Settings -> Framework Search Paths 里面配置 ImSDK.framework 路径寻址，在 TUIKit -> Build Settings -> Header Search Paths 里面配置 ImSDK.framework 头文件路径寻址，注意请不要直接 copy下面的配置，您需要根据自己 ImSDK.framework 的实际位置配置。***
...
***注意：TUIKitDemo 编译会依赖 TUIKit工程，参考下图，在 TUIKitDemo -> Build Settings -> Header Search Paths 配置 TUIKit 的头文件路径寻址，这里的文件寻址方式请选择 recursive。注意请不要直接 copy下面的配置，您需要根据自己 TUIKit 文件夹的实际位置配置。***
...
***最后强调下，无论出于什么样的业务逻辑，整个工程只能链接一份 ImSDK ,否则会导致 api 调用异常，一些客户用我们的 demo测试调用 api 没有问题，用自己的 demo 测试就出现了莫名其妙的错误，大部分都是这个原因导致，一个工程只需要链接一份 ImSDK，如果要在其他位置使用 ImSDK，只需要参考上面的步骤配置下 ImSDK 路径寻址就可以了。***
...
您可以点击会话列表右上角的添加按钮，选择 "发起群聊" 或 "添加会话"，具体步骤如下：
**1. 进入会话列表类 ConversationController ，创建并添加 TUIKit  的列表UI类 TConversationController，并且设置代理监听**
```
- (void)viewDidLoad {
    [super viewDidLoad];
		//初始化 TUIKit 的会话列表UI类
...
```
**2. 选择 "发起群聊" 或 "添加会话" 会触发对应的逻辑回调，您可以在回调里面根据选择的内容跳转到对应的界面类。**
```
...
```
**3. 如果选择发起群聊，会进入发起群聊类 AddGroupController ，在 AddGroupController 创建并添加 TUIKit 的发起群聊UI类TAddGroupController ，并且设置代理监听。如果选择添加会话，会进入添加会话类 AddC2CController ，在 AddC2CController 创建并添加 TUIKit 的添加会话UI类 TAddC2CController ，并且设置代理监听。**
```
//AddGroupController
- (void)viewDidLoad {
    [super viewDidLoad];
...
```
**4. 在发起群聊成功后，会收到 addGroupController 的回调，在回调里面可以获取群组的信息。在添加会话成功后，会收到 addC2CController 的回调，在回调里面可以获取会话信息。**
```
//AddGroupController
...
```
**5. 当点击会话列表的某一个会话的时候，会收到以下回调，这时候创建会话界面类 ChatViewController ，透传 TConversationCellData （包含了消息的Id，消息类型，消息内容等信息）。**
```
...
```
#### 收发消息
**1. 在会话界面，普通文本，表情的收发都会由 TChatController 直接接管，您不需要关注太多细节。相册、视频、文件的发送需要在收到UI触发事件（chatController：didSelectMoreAtIndex）后去对应的系统模块获取。**
```
...
```
**2. 在系统函数回调 imagePickerController 里面获取返回的图片和视频，调用 TChatController 的接口发送。**
```
...
```
**3. 在系统函数 documentPicker 回调里面获取系统返回的文件，调用 TChatController 接口发送。**
```
...
**1. 在会话类 ChatViewController 监听代理 TChatControllerDelegate  ，在下面的回调跳转到对应的消息查看类。**
```
...
```
**2. 跳转到图片、视频、文件查看类后，根据上一步获取的消息 data 信息调用 TUIKit 函数下载对应的图片、视频、文件数据。**
```

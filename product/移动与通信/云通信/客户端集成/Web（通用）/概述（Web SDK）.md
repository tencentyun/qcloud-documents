...
|对象 | 介绍 | 功能 |
|---------|---------|---------|
|webim.SESSION_TYPE  |会话类型，取值范围：<ol><li>webim.SESSION_TYPE.C2C，私聊<li>webim.SESSION_TYPE.GROUP，群聊| 区分消息属于哪种聊天类型 |
|webim.C2C_MSG_SUB_TYPE  | C2C 消息子类型，取值范围：webim.C2C_MSG_SUB_TYPE.COMMON，普通消息 | 区分 C2C 消息类型|
| webim.GROUP_MSG_SUB_TYPE |群消息子类型，取值范围：<ol><li>webim.GROUP_MSG_SUB_TYPE.COMMON，普通消息<li>	webim.GROUP_MSG_SUB_TYPE.LOVEMSG，点赞消息<li>webim.GROUP_MSG_SUB_TYPE.TIP，提示消息<li>webim.GROUP_MSG_SUB_TYPE.REDPACKET，红包消息（优先级最高）|  区分群消息类型，业务可针对不同的消息作出不同的操作。|
| webim.GROUP_TIP_TYPE |群提示消息类型，取值范围：<ol><li>webim. GROUP_TIP_TYPE.JOIN，进群<li>webim. GROUP_TIP_TYPE.QUIT，退群<li>webim. GROUP_TIP_TYPE.KICK，被踢出群<li>webim. GROUP_TIP_TYPE.SET_ADMIN，被设置成管理员<li>webim. GROUP_TIP_TYPE.CANCEL_ADMIN，被取消管理员角色<li>webim. GROUP_TIP_TYPE.MODIFY_GROUP_INFO，修改群资料 <li>webim. GROUP_TIP_TYPE.MODIFY_MEMBER_INFO，修改群成员信息 |  区分群提示消息类型
|webim.GROUP_TIP_MODIFY_GROUP_INFO_TYPE  | 群资料变更类型，取值范围：<ol><li>webim. GROUP_TIP_MODIFY_GROUP_INFO_TYPE.FACE_URL，群头像发生变更<li>webim. GROUP_TIP_MODIFY_GROUP_INFO_TYPE.NAME，群名称发生变更<li>webim. GROUP_TIP_MODIFY_GROUP_INFO_TYPE.OWNER，群主发生变更<li>webim. GROUP_TIP_MODIFY_GROUP_INFO_TYPE.NOTIFICATION，群公告发生变更<li>webim. GROUP_TIP_MODIFY_GROUP_INFO_TYPE.INTRODUCTION，群简介发生变更| 区分群资料变更类型 |
| webim.GROUP_SYSTEM_TYPE | 群系统消息类型，取值范围：<ol><li>webim.GROUP_SYSTEM_TYPE.JOIN_GROUP_REQUEST，申请加群请求（只有管理员会收到）<li>webim.GROUP_SYSTEM_TYPE.JOIN_GROUP_ACCEPT，申请加群被同意（只有申请人能够收到）<li>webim.GROUP_SYSTEM_TYPE.JOIN_GROUP_REFUSE，申请加群被拒绝（只有申请人能够收到）<li>webim.GROUP_SYSTEM_TYPE.KICK，被管理员踢出群（只有被踢者接收到）<li>webim.GROUP_SYSTEM_TYPE.DESTORY，群被解散(全员接收)<li>webim.GROUP_SYSTEM_TYPE.CREATE，创建群（创建者接收, 不展示）<li>webim.GROUP_SYSTEM_TYPE.INVITED_JOIN_GROUP_REQUEST，邀请加群（被邀请者接收）<li>webim.GROUP_SYSTEM_TYPE.QUIT，主动退群（主动退出者接收, 不展示）<li>webim.GROUP_SYSTEM_TYPE.SET_ADMIN，设置管理员（被设置者接收）<li>webim.GROUP_SYSTEM_TYPE.CANCEL_ADMIN，取消管理员（被取消者接收）<li>webim.GROUP_SYSTEM_TYPE.REVOKE，群已被回收（全员接收, 不展示）<li>webim.GROUP_SYSTEM_TYPE.CUSTOM，用户自定义通知（默认全员接收）| 区分群系统消息类型 |
|webim.MSG_ELEMENT_TYPE  | 消息元素类型，取值范围：<ol><li>webim.MSG_ELEMENT_TYPE.TEXT，文本消息<li>webim.MSG_ELEMENT_TYPE.FACE，表情消息<li>webim.MSG_ELEMENT_TYPE.IMAGE，图片消息<li>webim.MSG_ELEMENT_TYPE.SOUND，语音消息<li>webim.MSG_ELEMENT_TYPE.FILE，文件消息<li>webim.MSG_ELEMENT_TYPE.LOCATION，位置消息<li>webim.MSG_ELEMENT_TYPE.CUSTOM，自定义消息<li>webim.MSG_ELEMENT_TYPE.GROUP_TIP，群提示消息（只有群聊天才会出现）|区分消息元素类型  |
|webim.IMAGE_TYPE  | webim.IMAGE_TYPE	图片大小类型，取值范围：<ol><li>webim.IMAGE_TYPE.SMALL，小图<li>webim.IMAGE_TYPE.LARGE，大图<li>webim.IMAGE_TYPE.ORIGIN，原图| 区分图片大小类型 |
...
| webim.BROWSER_INFO | 当前浏览器信息<ol><li>webim.BROWSER_INFO.type，浏览器类型（包括 ‘ie’，’safari’，’chrome’，’firefox’，’opera’，’unknow’）<li>webim.BROWSER_INFO.ver，版本号| 区分浏览器版本 |
| webim.CONNECTION_STATUS | 连接状态<ol><li>webim.CONNECTION_STATUS.ON，连接状态正常，可正常收发消息<li>webim.CONNECTION_STATUS.OFF，连接已断开，当前用户已离线，无法收信息<li>webim.CONNECTION_STATUS.RECONNECT，连接重新建立| 用于区分用户的当前连接状态 |

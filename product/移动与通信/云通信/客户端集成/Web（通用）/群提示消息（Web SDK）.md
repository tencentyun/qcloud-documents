...
**触发时机：**当群成员的群相关资料变更时，会有系统消息发出，可更新相关字段展示，或者选择性把消息展示给用户。此处所说资料变更仅指与群相关的资料变更，例如禁言时间、成员角色变更等，不包括用户昵称等本身资料，目前只支持禁言时间通知。收到的消息 type 为 `WEB_IM_GROUP_TIP_TYPE.MODIFY_MEMBER_INFO`。 

 消息详情请参考 [消息](https://cloud.tencent.com/document/product/269/3662)、[群组消息](https://cloud.tencent.com/document/product/269/3663) 和 [消息格式描述](https://cloud.tencent.com/document/product/269/2720)。
...
| conv_type | TIMConvType | 会话类型。请参考 [TIMConvType](https://cloud.tencent.com/document/product/269/33553#timconvtype)  |
...
- 发送消息时不能发送`kTIMElem_GroupTips`、`kTIMElem_GroupReport`，他们由为后台下发，用于更新（通知）群的信息。可以的发送消息内元素：
  - 文本消息元素  [TextElem](https://cloud.tencent.com/document/product/269/33553#textelem)。
  - 表情消息元素  [FaceElem](https://cloud.tencent.com/document/product/269/33553#faceelem)。
  - 位置消息元素  [LocationElem](https://cloud.tencent.com/document/product/269/33553#locationelem)。
  - 图片消息元素  [ImageElem](https://cloud.tencent.com/document/product/269/33553#imageelem)。
  - 声音消息元素  [SoundElem](https://cloud.tencent.com/document/product/269/33553#soundelem)。
  - 自定义消息元素  [CustomElem](https://cloud.tencent.com/document/product/269/33553#customelem)。
  - 文件消息元素  [FileElem](https://cloud.tencent.com/document/product/269/33553#fileelem)。
  - 视频消息元素  [VideoElem](https://cloud.tencent.com/document/product/269/33553#videoelem)。
...
>?上报此消息已读状态，最好用接收新消息获取的消息数组里面的消息 JSON 或者用消息定位符查找到的消息 JSON，避免重复构造消息 JSON。
...
| cb | TIMCommCallback | 根据消息定位精准查找指定会话的消息成功与否的回调。回调函数定义请参考  [TIMCommCallback](https://cloud.tencent.com/document/product/269/33552#timcommcallback) ，回调参数解析请参考  [TIMMsgFind](https://cloud.tencent.com/document/product/269/33552#timmsgfind)  |
| user_data | const void\* | 用户自定义数据， IM SDK 只负责传回给回调函数 cb ，不做任何处理 |
...
// json_value_msgs.toStyledString().c_str()得到的 json_msg_array JSON 字符串如下
[
   {
      "message_client_time" : 1551446728,
      "message_elem_array" : [
...
| conv_type | TIMConvType | 会话类型。请参考 [TIMConvType](https://cloud.tencent.com/document/product/269/33553#timconvtype)  |
| json_msg_param | const char\* | 消息 JSON 字符串 |
| cb | TIMCommCallback | 保存自定义消息成功与否的回调。调函数定义请参考  [TIMCommCallback](https://cloud.tencent.com/document/product/269/33552#timcommcallback)  |
| user_data | const void\* | 用户自定义数据， IM SDK 只负责传回给回调函数 cb ，不做任何处理 |
...
| cb | TIMCommCallback | 获取指定会话的消息列表成功与否的回调。回调函数定义请参考  [TIMCommCallback](https://cloud.tencent.com/document/product/269/33552#timcommcallback) ，回调参数解析请参考  [TIMMsgGet](https://cloud.tencent.com/document/product/269/33552#timmsgget)  |
| user_data | const void\* | 用户自定义数据， IM SDK 只负责传回给回调函数 cb ，不做任何处理 |
...
**示例： 给 C2C 会话 Windows-02、kTIMConv_C2C 导入消息列表**
...
| cb | TIMCommCallback | 下载成功与否的回调以及下载进度回调。回调函数定义请参考  [TIMCommCallback](https://cloud.tencent.com/document/product/269/33552#timcommcallback) ，回调参数解析请参考  [TIMMsgDownloadElemToPath](https://cloud.tencent.com/document/product/269/33552#timmsgdownloadelemtopath)  |
| user_data | const void\* | 用户自定义数据， IM SDK 只负责传回给回调函数 cb ，不做任何处理 |
...
>?此接口用于下载消息内图片、文件、声音、视频等元素。下载的参数`kTIMMsgDownloadElemParamFlag`、`kTIMMsgDownloadElemParamId`、`kTIMMsgDownloadElemParamBusinessId`、`kTIMMsgDownloadElemParamUrl`均可以在相应元素内找到。其中`kTIMMsgDownloadElemParamType`为下载文件类型  [TIMDownloadType](https://cloud.tencent.com/document/product/269/33553#timdownloadtype)。
...
| cb | TIMCommCallback | 群发消息成功与否的回调。回调函数定义请参考  [TIMCommCallback](https://cloud.tencent.com/document/product/269/33552#timcommcallback) ，回调参数解析请参考  [TIMMsgBatchSend](https://cloud.tencent.com/document/product/269/33552#timmsgbatchsend)  |
| user_data | const void\* | 用户自定义数据， IM SDK 只负责传回给回调函数 cb ，不做任何处理 |

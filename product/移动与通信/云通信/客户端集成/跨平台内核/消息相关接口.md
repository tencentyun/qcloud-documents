消息介绍请参考 [消息](https://cloud.tencent.com/document/product/269/3662)、[群组消息](https://cloud.tencent.com/document/product/269/3663) 和 [消息格式描述](https://cloud.tencent.com/document/product/269/2720)。
...
>?上报此消息已读状态，最好用接收新消息获取的消息数组里面的消息 JSON 或者用消息定位符查找到的消息 JSON，避免重复构造消息 JSON 字符串。
...
| cb | TIMCommCallback | 根据消息定位精准查找指定会话的消息成功与否的回调。回调函数定义请参考 [TIMCommCallback](https://cloud.tencent.com/document/product/269/33552#timcommcallback)，回调参数解析请参考 [TIMMsgFindByMsgLocatorList](https://cloud.tencent.com/document/product/269/33552#timmsgfindbymsglocatorlist)  |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |
...
| cb | TIMCommCallback | 获取指定会话的消息列表成功与否的回调。回调函数定义请参考 [TIMCommCallback](https://cloud.tencent.com/document/product/269/33552#timcommcallback)，回调参数解析请参考 [TIMMsgGetMsgList](https://cloud.tencent.com/document/product/269/33552#timmsggetmsglist)  |
| user_data | const void\* | 用户自定义数据，IM SDK 只负责传回给回调函数 cb，不做任何处理 |

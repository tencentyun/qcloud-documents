...
| 类型 | 含义 |
|-----|-----|
| int | 返回 TIM_SUCC 表示接口调用成功，其他值表示接口调用失败。每个返回值的定义请参考  [TIMResult](https://cloud.tencent.com/document/product/269/33486#timresult)  |
...
| cb | TIMCommCallback | 返回设置配置之后所有配置的回调，此 cb 可为空，表示不获取所有配置信息。回调函数定义请参考  [TIMCommCallback](https://cloud.tencent.com/document/product/269/33486#timcommcallback) ，回调参数解析请参考  [TIMSetConfig](https://cloud.tencent.com/document/product/269/33486#timsetconfig)  |
...
| 类型 | 含义 |
|-----|-----|
| int | 返回 TIM_SUCC 表示接口调用成功（接口只有返回 TIM_SUCC ，回调 cb 才会被调用），其他值表示接口调用失败。每个返回值的定义请参考  [TIMResult](https://cloud.tencent.com/document/product/269/33486#timresult)  |

>?
目前支持设置的配置有代理的 IP 和端口、输出日志的级别、获取群信息/群成员信息的默认选项、是否接受消息已读回执事件等。每项配置可以单独设置、也可以一起配置。详情请参考  [SetConfig](https://cloud.tencent.com/document/product/269/33487#setconfig)。

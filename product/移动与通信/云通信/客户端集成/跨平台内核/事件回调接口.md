...
>?
发送方发送消息，接收方调用接口  [TIMMsgReportReaded](url_for_msg_api#TIMMsgReportReaded)  上报该消息已读，发送方 IM SDK 会通过此接口设置的回调抛出。
...
>?
发送方发送消息，接收方收到消息。此时发送方调用接口  [TIMMsgRevoke](url_for_msg_api#TIMMsgRevoke)  撤回该消息，接收方的IM SDK会通过此接口设置的回调抛出。
...
>?
会话事件包括：会话新增、会话删除、会话更新。任何产生一个新会话的操作都会触发会话新增事件，比如调用接口  [TIMConvCreate](url_for_conv_api#TIMConvCreate)  创建会话，接收到未知会话的第一条消息等。任何已有会话变化的操作都会触发会话更新事件，比如收到会话新消息，消息撤回，已读上报等。调用接口  [TIMConvDelete](url_for_conv_api#TIMConvDelete)  删除会话成功时会触发会话删除事件。
...
>?
1. 当调用接口  [TIMInit](url_for_init_api#TIMInit)  时，IM SDK 会去连接云后台。此接口设置的回调用于监听网络连接的状态。
...
>?
用户票据，可能会存在过期的情况，如果用户票据过期，此接口设置的回调会调用。 [TIMLogin](url_for_login_api#TIMLogin)  也将会返回 70001 错误码。开发者可根据错误码或者票据过期回调进行票据更换。
...
>?
设置日志监听的回调之后，IM SDK内部的日志会回传到此接口设置的回调。客户可以通过接口  [TIMSetConfig](url_for_init_api#TIMSetConfig)  配置哪些日志级别的日志回传到回调函数。

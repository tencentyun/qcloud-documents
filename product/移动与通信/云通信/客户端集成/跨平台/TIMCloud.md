...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMRecvNewMsgCallback | 新消息回调函数，声明 [TIMRecvNewMsgCallback](Url_for_TIMCloudCallback#TIMRecvNewMsgCallback)  |
...
>发送方发送消息，接收方调用接口 [TIMMsgReportReaded](#TIMMsgReportReaded) 上报该消息已读，发送方ImSDK会通过此接口设置的回调抛出。 详情消息属性  [消息是否已读](https://cloud.tencent.com/document/product/269/1587#消息是否已读#消息是否已读) 
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgRevokeCallback | 消息撤回通知回调。回调声明 [TIMMsgRevokeCallback](Url_for_TIMCloudCallback#TIMMsgRevokeCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |

**注释：**

>发送方发送消息，接收方收到消息。此时发送方调用接口 [TIMMsgRevoke](#TIMMsgRevoke) 撤回该消息，接收方的ImSDK会通过此接口设置的回调抛出。
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgElemUploadProgressCallback | 文件上传进度回调。回调声明 [TIMMsgElemUploadProgressCallback](Url_for_TIMCloudCallback#TIMMsgElemUploadProgressCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMGroupTipsEventCallback | 群消息回调。回调声明 [TIMGroupTipsEventCallback](Url_for_TIMCloudCallback#TIMGroupTipsEventCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMConvEventCallback | 会话事件回调。回调声明 [TIMConvEventCallback](Url_for_TIMCloudCallback#TIMConvEventCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |

**注释：**

>会话事件包括：会话新增、会话删除、会话更新。 任何产生一个新会话的操作都会触发会话新增事件，比如调用接口 [TIMConvCreate](#TIMConvCreate) 创建会话，接收到未知会话的第一条消息等。 任何已有会话变化的操作都会触发会话更新事件，比如收到会话新消息，消息撤回，已读上报等。 调用接口 [TIMConvDelete](#TIMConvDelete) 删除会话成功时会触发会话删除事件。
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMNetworkStatusListenerCallback | 连接事件回调。回调声明 [TIMNetworkStatusListenerCallback](Url_for_TIMCloudCallback#TIMNetworkStatusListenerCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |

**注释：**

>当调用接口  [TIMInit](#TIMInit) 时，ImSDK会去连接云后台。此接口设置的回调用于监听网络连接的状态。 网络连接状态包含四个：正在连接、连接失败、连接成功、已连接。 详情 [网络事件通知](https://cloud.tencent.com/document/product/269/1581#网络事件通知#网络事件通知) 
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMKickedOfflineCallback | 踢下线回调。回调声明 [TIMKickedOfflineCallback](Url_for_TIMCloudCallback#TIMKickedOfflineCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMUserSigExpiredCallback | 票据过期回调。回调声明 [TIMUserSigExpiredCallback](Url_for_TIMCloudCallback#TIMUserSigExpiredCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |

**注释：**

>用户票据，可能会存在过期的情况，如果用户票据过期，此接口设置的回调会调用。  [TIMLogin](#TIMLogin) 也将会返回 `70001` 错误码。开发者可根据错误码或者票据过期回调进行票据更换
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMLogCallback | 日志回调。回调声明 [TIMLogCallback](Url_for_TIMCloudCallback#TIMLogCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |

**注释：**

>设置日志监听的回调之后，ImSDK内部的日志会回传到此接口设置的回调。 客户可以通过接口 [TIMSetConfig](#TIMSetConfig) 配置哪些日志级别的日志回传到回调函数。
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMMsgUpdateCallback | 消息更新回调。回调声明 [TIMMsgUpdateCallback](Url_for_TIMCloudCallback#TIMMsgUpdateCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| sdk_app_id | uint64_t | 官网申请的SDKAPPID |
| json_sdk_config | const char* | SDK配置选项Json字符串。 [SdkConfig](Url_for_TIMCloudDef#SdkConfig)  |

**返回值**

| 类型 | 含义 |
|-----|-----|
| int | 返回 `TIM_SUCC` 表示接口调用成功，其他值表示接口调用失败。每个返回值的定义见  [TIMResult](Url_for_TIMCloudCallback#TIMResult)  |
**注释：**

>1. 在使用 `ImSDK` 进一步操作之前，需要先初始化 `ImSDK` 
>2. json_sdk_config可以为NULL或者""空字符串，在此情况下SdkConfig均为默认值。 
>3. json_sdk_config里面的每个Json key都是选填的，详见 [SdkConfig](Url_for_TIMCloudDef#SdkConfig)  
...
| cb | TIMCommCallback | 返回设置配置之后所有配置的回调，此 `cb` 可为空，表示不获取所有配置信息。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMSetConfig](Url_for_TIMCloudCallback#TIMSetConfig)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| 类型 | 含义 |
|-----|-----|
| int | 返回 `TIM_SUCC` 表示接口调用成功（接口只有返回 `TIM_SUCC` ，回调 `cb` 才会被调用），其他值表示接口调用失败。每个返回值的定义见  [TIMResult](Url_for_TIMCloudCallback#TIMResult)  |
**注释：**

>目前支持设置的配置有代理的ip和端口、输出日志的级别、获取群信息/群成员信息的默认选项、是否接受消息已读回执事件等。 每项配置可以单独设置、也可以一起配置。详见  [SetConfig](Url_for_TIMCloudDef#SetConfig) 
...
| cb | TIMCommCallback | 登入成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMCommCallback | 登出成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| conv_type | TIMConvType | 会话类型。 [TIMConvType](Url_for_TIMCloudCallback#TIMConvType)  |
| cb | TIMCommCallback | 创建会话的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMConvCreate](Url_for_TIMCloudCallback#TIMConvCreate)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| conv_id | const char* | 会话的id |
| conv_type | TIMConvType | 会话类型。 [TIMConvType](Url_for_TIMCloudCallback#TIMConvType)  |
| cb | TIMCommCallback | 删除会话成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMCommCallback | 获取会话缓存列表的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMConvGetConvList](Url_for_TIMCloudCallback#TIMConvGetConvList)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 发送新消息成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>3. 可以的发送消息内元素 
>   文本消息元素  [TextElem](Url_for_TIMCloudDef#TextElem)  
>   表情消息元素  [FaceElem](Url_for_TIMCloudDef#FaceElem)  
>   位置消息元素  [LocationElem](Url_for_TIMCloudDef#LocationElem)  
>   图片消息元素  [ImageElem](Url_for_TIMCloudDef#ImageElem)  
>   声音消息元素  [SoundElem](Url_for_TIMCloudDef#SoundElem)  
>   自定义消息元素  [CustomElem](Url_for_TIMCloudDef#CustomElem)  
>   文件消息元素  [FileElem](Url_for_TIMCloudDef#FileElem)  
>   视频消息元素  [VideoElem](Url_for_TIMCloudDef#VideoElem) 
...
| conv_type | TIMConvType | 会话类型。 [TIMConvType](Url_for_TIMCloudCallback#TIMConvType)  |
| json_msg_param | const char* | 消息json字符串 |
| cb | TIMCommCallback | 消息上报已读成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| conv_type | TIMConvType | 会话类型。 [TIMConvType](Url_for_TIMCloudCallback#TIMConvType)  |
| json_msg_param | const char* | 消息json字符串 |
| cb | TIMCommCallback | 消息撤回成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 根据消息定位精准查找指定会话的消息成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMMsgFind](Url_for_TIMCloudCallback#TIMMsgFind)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 导入消息列表到指定会话成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| conv_type | TIMConvType | 会话类型。 [TIMConvType](Url_for_TIMCloudCallback#TIMConvType)  |
| json_msg_param | const char* | 消息json字符串 |
| cb | TIMCommCallback | 保存自定义消息成功与否的回调。调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
```c
TIM_DECL int TIMMsgGetMsgList(const char* conv_id, TIMConvType conv_type, const char* json_msgget_param, TIMCommCallback cb, const void* user_data);
```
...
| cb | TIMCommCallback | 获取指定会话的消息列表成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMMsgGet](Url_for_TIMCloudCallback#TIMMsgGet)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
```c
 ...
 int ret = TIMMsgGetMsgList(json_msgget_param.toStyledString().c_str(), type, json.c_str(), [](int32_t code, const char* desc, const char* json_params, const void* user_data) {
 }, this);
 
 // json_msgget_param.toStyledString().c_str()得到 json_msgget_param Json字符串如下
 {
...
| cb | TIMCommCallback | 删除指定会话的消息成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 下载成功与否的回调以及下载进度回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMMsgDownloadElemToPath](Url_for_TIMCloudCallback#TIMMsgDownloadElemToPath)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>此接口用于下载消息内图片、文件、声音、视频等元素。下载的参数kTIMMsgDownloadElemParamFlag、kTIMMsgDownloadElemParamId、kTIMMsgDownloadElemParamBusinessId、kTIMMsgDownloadElemParamUrl均可以 在相应元素内找到。其中kTIMMsgDownloadElemParamType为下载文件类型 [TIMDownloadType](Url_for_TIMCloudDef#TIMDownloadType) 
...
| cb | TIMCommCallback | 群发消息成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMMsgBatchSend](Url_for_TIMCloudCallback#TIMMsgBatchSend)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 创建群组成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMGroupCreate](Url_for_TIMCloudCallback#TIMGroupCreate)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>2. 创建群参数的Json Key详见 [CreateGroupParam](Url_for_TIMCloudDef#CreateGroupParam) 
...
| cb | TIMCommCallback | 删除群组成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 申请加入群组成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 退出群组成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 邀请加入群组成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMGroupInviteMember](Url_for_TIMCloudCallback#TIMGroupInviteMember)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>   公开群、聊天室邀请用户入群 
>   需要用户同意；直播大群不能邀请用户入群。 
>2. 此接口支持批量邀请成员加入群组。Json Key详见 [GroupInviteMemberParam](Url_for_TIMCloudDef#GroupInviteMemberParam) 
...
| cb | TIMCommCallback | 删除群组成员成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMGroupDeleteMember](Url_for_TIMCloudCallback#TIMGroupDeleteMember)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>   对于直播大群：不能踢人。 
>2. 此接口支持批量删除群成员。Json Key详见 [GroupDeleteMemberParam](Url_for_TIMCloudDef#GroupDeleteMemberParam) 
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| cb | TIMCommCallback | 获取已加入群组列表成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMGroupGetJoinedGroupList](Url_for_TIMCloudCallback#TIMGroupGetJoinedGroupList)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>   此接口可以获取自己所加入的群列表 
>   此接口只能获得加入的部分直播大的列表。 
>2. 此接口用于获取当前用户已加入的群组列表，返回群组的基础信息。具体返回的群组基本信息字段参考 [GroupBaseInfo](Url_for_TIMCloudDef#GroupBaseInfo) 
...
| cb | TIMCommCallback | 获取群组信息列表成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMGroupGetGroupInfoList](Url_for_TIMCloudCallback#TIMGroupGetGroupInfoList)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>此接口用于获取指定群ID列表的群详细信息。具体返回的群组详细信息字段参考 [GroupDetailInfo](Url_for_TIMCloudDef#GroupDetailInfo) 
...
| cb | TIMCommCallback | 设置群信息成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>   对于私有群，任何人可修改群简介。 
>3. `kTIMGroupModifyInfoParamModifyFlag` 可以按位或设置多个值。不同的flag 设置不同的键。详见 [GroupSetInfoParam](Url_for_TIMCloudDef#GroupSetInfoParam) 
...
| cb | TIMCommCallback | 获取群成员信息列表成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMGroupGetMemberInfoList](Url_for_TIMCloudCallback#TIMGroupGetMemberInfoList)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>2. 根据不同的选项，获取群成员信息列表。成员信息的各个字段含义参考 [GroupMemberInfo](Url_for_TIMCloudDef#GroupMemberInfo) 
...
| cb | TIMCommCallback | 设置群成员信息成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>2. `kTIMGroupModifyMemberInfoParamModifyFlag` 可以按位或设置多个值，不同的flag 设置不同的键。详见 [GroupSetMemberInfoParam](Url_for_TIMCloudDef#GroupSetMemberInfoParam) 
...
| cb | TIMCommCallback | 获取群未决信息列表成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback) ，回调参数解析见  [TIMGroupGetPendencyList](Url_for_TIMCloudCallback#TIMGroupGetPendencyList)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>4. 权限说明： 
>   只有审批人有权限拉取相关未决信息。 
>5. kTIMGroupPendencyOptionStartTime设置拉取时间戳,第一次请求填0,后边根据server返回的 [GroupPendencyResult](Url_for_TIMCloudDef#GroupPendencyResult) 键kTIMGroupPendencyResultNextStartTime指定的时间戳进行填写。 
...
| 参数 | 类型 | 含义 |
|-----|-----|-----|
| time_stamp | uint64_t | 已读时间戳(单位秒)。与 [GroupPendency](Url_for_TIMCloudDef#GroupPendency) 键kTIMGroupPendencyAddTime指定的时间比较 |
| cb | TIMCommCallback | 上报群未决信息已读成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
| cb | TIMCommCallback | 处理群未决信息成功与否的回调。回调函数定义见  [TIMCommCallback](Url_for_TIMCloudCallback#TIMCommCallback)  |
| user_data | const void* | 用户自定义数据， `ImSDK` 只负责传回给回调函数 `cb` ，不做任何处理 |
...
>2. 处理未决信息时需要带一个未决信息 [GroupPendency](Url_for_TIMCloudDef#GroupPendency) ，可以在接口TIMGroupGetPendencyList返回的未决信息列表将未决信息保存下来，   在处理未决信息的时候将 [GroupPendency](Url_for_TIMCloudDef#GroupPendency) 传入键kTIMGroupHandlePendencyParamPendency。

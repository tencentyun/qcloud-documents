...
| 群消息发送前回调 | 在把群消息下发给群成员之前，云通信 IM 后台先去 App  后台询问是否允许下发消息，如果不允许，则拒绝下发消息；<br>App  后台在收到回调后，也可对消息内容进行修改并返回给云通信 IM，云通信 IM 将使用修改之后的信息进行下发，详情请参考 [群内发言之前回调](https://cloud.tencent.com/document/product/269/1619)；<br>但是云通信 IM 在发起该回调后，最多只会等待2秒。如2秒内未收到应答，直接将消息下发给群成员，不会进行重试 |

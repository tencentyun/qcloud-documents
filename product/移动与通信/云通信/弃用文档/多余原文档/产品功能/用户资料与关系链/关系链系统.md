...
| 字段名称 | 类型 | 描述 |
|---------|---------|---------|
|Tag_SNS_IM_Group | Array  |好友分组：<br />1. 最多支持 32 个分组；<br />2. 不允许分组名为空；<br />3. 分组名长度不得超过 30 个字节；<br />4. 分组ID的合理取值范围是[1, 32]；<br />5. 同一个好友可以有多个不同的分组。 |
...
| 删除模式 | DeleteType | 描述 |
|---------|---------|---------|
| 单向删除好友 | Delete_Type_Single | 只将To_Account从From_Account 的好友表中删除，但不会将 From_Account 从 To_Account 的好友表中删除 |
|双向删除好友|Delete_Type_Both|将 To_Account 从 From_Account的好友表中删除，同时将 From_Account 从 To_Account 的好友表中删除|

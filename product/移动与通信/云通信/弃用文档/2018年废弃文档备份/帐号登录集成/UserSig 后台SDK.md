开发者可以使用云通信 IM 后台 SDK 及相关工具，生成 `UserSig` 和校验 `UserSig`。云通信团队提供了 6 个包供开发者 [下载](http://share.weiyun.com/2b3abe0e3f185c440cf455647455f661)，内容分别是 Windows 下 64 位预编译文件包、Windows 下32位预编译文件包、Linux 下64位预编译文件包、Linux 下32位预编译文件包、zip 格式的源代码文件和 tar.gz 格式的源代码文件。
...
## Linux 平台
### 工具使用
>!这里讲解的是工具的使用说明，实际应用中需要开发者后台调用云通信 IM 后台 SDK 接口生成 UserSig。

**Linux 下生成 UserSig 和校验 UserSig**
...
sig_file：usersig 文件的路径，如果是生成 usersig，那么会将 usersig 写入这个文件，如果是校验 usersig，那么会从这个文件读取 usersig的内容
sdkappid：创建应用时页面上分配的 sdkappid
identifier：用户标识，即用户 id
```

>!
> 生成的 UserSig 有效期为 180 天，开发者需要在 UserSig 过期前重新生成 UserSig。

### C++ 接口

首先包含 `include/tls_sig_api` 目录下的 `tls_signature.h`。头文件中包含的接口，`tls_gen_signature_ex2` 和 `tls_check_signature_ex2`，前者是生成 UserSig 的接口，后者是校验 UserSig 的接口，详细的参数和返回值说明请参考头文件 `tls_signature.h`。
...
>!
>如果程序有多线程调用云通信 IM 后台 SDK 的用法，请在程序初始化时和结束时分别调用下面的接口：
```
...
### PHP 接口
PHP 实现的方式较为简单，就是调用命令行工具生成 UserSig，工具是 `bin/signature`，PHP 的调用方式如下：
...
### PHP原生接口
在源码包和二进制包中都带有 `php/TLSSig.php` 文件，生成 UserSig 接口 genSig 和校验 UserSig 接口 verifySig 均在其中，注意 PHP 环境需要带 openssl 扩展，否则接口使用会报错，另外只支持 PHP 5.3 及以上的版本。

>!如果上述实现 PHP 环境无法满足要求，比如使用了红帽系（fedora、centos 和 rel 等）的操作系统，可以参考 [腾讯论坛](http://bbs.qcloud.com/thread-22519-1-1.html) 中另一种与 openssl 和系统无关的实现。

## Windows 平台
### 工具使用
>!这里讲解的是工具的使用说明，实际应用中需要开发者在云通信 IM 后台 SDK 接口生成 UserSig。

**Windows 下生成 UserSig 和校验 UserSig**
...
pri_key_file：私钥文件的路径
pub_key_file：公钥文件的路径
sig_file：usersig 文件的路径，如果是生成 usersig，那么会将 usersig 写入这个文件，如果是校验 usersig，那么会从这个文件读取 usersig 的内容
sdkappid：创建应用时页面上分配的 sdkappid
identifier：用户标识，即用户 id
```

### C++ 接口
Windows 下 C++ 接口的使用方式我们采用 vs2012 来举例。首先包含 `include\tls_sig_api` 目录下的 `tls_signature.h`。头文件中包含的接口，`tls_gen_signature_ex2` 和 `tls_check_signature_ex2`，前者是生成 UserSig 的接口，后者是校验 UserSig 的接口，详细的参数和返回值说明请参考头文件 `tls_signature.h`。
然后是链接静态库，在 `lib` 目录下有下列目录：
```
├── jni
├── jsoncpp
...
>!如果程序是多线程调用云通信 IM 后台 SDK，请在程序初始化时和结束时分别调用下面的接口：
...
PHP 实现的方式较为简单，就是调用命令行工具生成 UserSig，工具是 `bin\signature.exe`，PHP 的调用方式如下：
...
>!若上述实现 PHP 环境无法满足要求，比如使用了红帽系（fedora、centos 和 rel 等）的操作系统，可以参考 [腾讯云论坛](http://bbs.qcloud.com/thread-22519-1-1.html) 另一种与 openssl 和系统无关的实现。
...
## 云通信 IM 后台 SDK 下载
下载 [ 云通信IM 后台 SDK ](http://share.weiyun.com/2b3abe0e3f185c440cf455647455f661)。

...
## 步骤
1、控制台：
2、调试smartbox：通过设备SDK的Demo模拟实际的智能货柜（功能可能无法覆盖实际业务场景），让用户通过Demo可快速将SDK与物联网套件打通，能让货柜发布并订阅消息，理解运行机制。
3、模拟业务管理系统控制smartbox：智能货柜系统在完成自身业务流程实现后，最终需要通过物联网套件去将控制指令下发到设备端接收，设备端根据接收后的指令执行物理的开柜门动作。
4、通过规则引擎将smartbox产生的数据实时转发到：
 

## 准备工作
为了通过下面的步骤快速理解该业务场景，需要做好以下准备工作
- 申请加速物联网套件服务
- 拥有一台物理或虚拟的Linux主机环境，可以编译、运行设备端Demo程序 
...
#### 1.3、创建Topic类
在【Topic类管理】点击创建Topic类，分别创建update与cmd两个Topic类
...
```
#### 2.4 修改设备名及证书
需进入/examples/linux/smartbox目录下修改tc_iot_device_config.h文件，将下图的DeviceName，DeviceSecret修改为3.1中创建的设备名及设备证书（注意，一定要修改成你自己创建的Device Secret）

## 场景介绍
智能灯场景是模拟一款智能灯如何通过设备SDK连接到物联网套件，并通过API实现灯的远程控制和智能灯耗电量的获取。本场景主要适用于智能家居类场景，即智能设备主动上报设备数据到云端，消费者通过手机应用如APP或微信控制设备，查询设备数据。
...
#### 控制台查看light状态与上报数据
单击控制台设备管理，Light_GZ_00001处于“上线”状态，表示智能小夜灯成功连接物联网套件。
![enter image description here](https://main.qcloudimg.com/raw/d6c3aafff71ea8d51cc24796704a0830.png)


单击控制台【日志管理】对应light的设备Light_GZ_0001操作列对应的设备数据，light小夜灯第一次上报的数据为下图所示
...
#### 控制台控制light
保持light程序不关闭，进入控制台【设备管理】，单击Light_GZ_00001设备操作列的“下发指令”。
...
将device_switch即智能小夜灯的开关设置为true，表示打开小夜灯。单击更新后，立即进入light程序的运行环境，可以看到light程序已打印了一条红色文本，小夜灯的开关被打开。
...
- 数据历史：可通过数据历史查看light上报到云端的每一笔业务数据
...
#### Python 代码示例
腾讯云 API 提供了 Python、Java、Php 等多种语言的SDK，方便用户调用物联网套件API。本场景模拟Python调用API示例来模拟手机发起控制指令。
...
#### 设备查询相关API
物联网套件的API为使用数据模板类型的智能灯还提供了其他数据查询类API。
- 获取设备历史数据：批量获取设备某一段时间范围的设备上报数据。
- 获取设备当前数据：获取某个设备当前上报到云端的数据。
- 获取设备日志：获取设备与云端的详细通信日志。

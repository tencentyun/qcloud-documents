如果你还不清楚什么是开发环境，可以阅读[开发环境和生产环境](/document/product/619/11446)。

如果你还不清楚腾讯云微信小程序解决方案，可以查看阅读[开通指引](/document/product/619/11447)。

如果阅读以上文章还有以下问题，请往下看相应的解决方法。

如果本文还不能解决你的问题，你可以到腾讯云[问答](https://www.qcloud.com/developer/ask)提问。
...
如果你想使用腾讯云微信小程序一站式解决方案，并使用微信开发者工具一键部署，微信开发者工具打开的目录必须包含 `project.config.json` 文件，文件必须是个 JSON 字符串并包含 `client` 和 `svr` 两个 `key`。实例文件如下：

...
[](id:que8)
### 个人区块怎么去创建？
选中一个模块，点击这里的保存图标，就是保存为个人区块。
<img src = "https://qcloudimg.tencent-cloud.cn/raw/316c7124d8ede57fc711b151ed6bfc6d.png" style = "width:50%">


[](id:que9)
### 微搭代码 组件中绑定 "查看列表 (wedaGetList)" 如何按创建时间降序?
用getrecords。


[](id:que10)
### 可以实现点击图片后隐藏图片吗？
1创建普通变量，变量类型选择boolean，初始值为true
2图片组件配置区-通用配置-条件展示开启，绑定步骤1中的普通变量。
3创建自定义方法，自定义方法中令变量=false
4图片绑定自定义方法，点击时触发。

[](id:que11)
### 如何对地图进行操作标记？
表单地图组件不支持标记，需要用连接器自己封装前端组件，可以看下 [自定义组件](https://cloud.tencent.com/document/product/1301/53287)。 



[](id:que12)
### 个人小程序能使用腾讯位置服务插件吗？
目前微搭平台暂时不能使用插件，已经支持了表单地图定位组件，可以看下是否满足需求。



[](id:que13)
### 目前在列表组件内可以实现不同管理用户查看不同部门的数据吗？
可以的。
...
[](id:que37)
### 地埋位置字段是不是只能用于小程序，不可以用于WEB和H5？
是支持小程序、H5 和 PC 端定位。



[](id:que38)
### 用模板新建的应用，为什么搜索不到【数据容器】呢？
数据容器是个分类。


[](id:que39)
### “引入百度统计”这样需要在index.html  的head 引入脚本 怎么引入呢，可以修改index吗？
可以。需要在lifecycle里写js来动态修改，append一段统计的js。


[](id:que40)
### 微搭现在支持链接蓝牙打印机吗？
需要调用小程序蓝牙能力： [蓝牙微信开放文档](https://developers.weixin.qq.com/miniprogram/dev/framework/device/beacon.html)。 


[](id:que41)
### 时间展示在前端，如何转换为日期格式展示？
在表达式里面写app.utils.formatDate(new Date(forItems.idxx.xxy), 'yyyy-mm-dd')


[](id:que42)
### 在数据模型创建了一个表单，想做一个可以查询这个表单的页面，怎么也操作呢？
页面中实现搜索功能，可以看下这个文档 [搜索功能](https://mp.weixin.qq.com/s/7s3UcnWXw3rllVUR2HUvRg)。 
...
· 在应用内展示图片可以使用“图片”组件
· 需要获得URL的场景可以参考这个文档实现 [参考文档](https://docs.cloudbase.net/lowcode/datasource/usage#appcloudgettempfileurl)。 



[](id:que44)
### 使用的模板页面，删除了协议内容后，无法提交是什么原因？页面也没有提示报错
模板自定义方法中限制了必须同意协议按钮才能进行下一步提交，删除之后confirm这个字段拿不到，所以提交失败。
...
某个分支兼容可能出了问题， 可以选中一下组件 ， 然后刷新，就可以了。

...
[](id:que4)
### 数据模型里面的数据，可以不能外部访问吗？
新建一个自定义应用，通过组件关联数据模型的数据，就可以对外访问了。

## 数据源字段相关
[](id:que5)
### 数据源字段能设置排序吗？
![](https://qcloudimg.tencent-cloud.cn/raw/ad52a4538bf710d4707b5d8aef03517a.png)
不支持。
...
[](id:que17)
### 使用表单容器中自带的绑定数据源新增记录，如何得到新增数据后返回的_id值？
看create 方法有没返回值，有的话， 数据源的success 里， 通过event.detail 拿返回值。
...
可以看下这个文档 [组件与事件绑定](https://cloud.tencent.com/document/product/1301/69302)。
...
[](id:que27)
### 微搭平台云函数怎么写？
可以参考文档： [自定义代码（云函数）](https://cloud.tencent.com/document/product/1301/68440)。

...
<td align="left"><li>编辑器新版本升级。编辑器集成数据模型，立享沉浸式开发体验。<br></li>
<li>组件绑定数据：优化组件获取数据的路径。详见<a src="https://cloud.tencent.com/document/product/1301/69302">组件绑定数据</a><br></li>
<li>数据容器：升级作为核心的数据供给组件，增加获取上下文能力。用于存储、查询、更新/新建数据。简化数据获取操作的复杂度。<a src="https://docs.cloudbase.net/lowcode/components/wedaUI/src/docs/compsdocs/database/DataContainer">数据容器使用说明</a><br></li>
<li>表达式：表达式面板支持公式和获取数据容器字段、变量等数据源。<br></li>
<li>数据校验：组件绑定数据时，增加所需数据类型校验提示。如文本不可绑定数组类型的数据。<br></li>
<li>事件优化：支持可视化配置页面的生命周期事件，如配置页面加载时查询数据。<br></li>
<li>表单组件：表单容器下组件支持数据联动。例如输入框B，可通过<b>绑定字段</b>获取下拉框A已选中的数据标识，实现数据联动。<br></li>
</td>
<td align="left">2022-10</td>
<td align="left"><a href="https://cloud.tencent.com/document/product/1301/69302">组件与数据绑定指引</a></td>
</tr>
</tbody></table>

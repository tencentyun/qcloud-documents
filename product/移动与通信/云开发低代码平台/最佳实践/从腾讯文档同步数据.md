本文主要介绍如何使用自定义 API 实现将数据从腾讯在线文档同步至数据模型。例如，有一个美食地图的文档需要同步至微搭数据模型：
...
接下来我们需要 [创建](https://cloud.tencent.com/document/product/1301/68455) 一个腾讯文档 API ，这个 API 授权的账号为上述在线文档的所有者账号。
![](https://qcloudimg.tencent-cloud.cn/raw/c7d28cc5adce21fc11598f17499e5d44.jpg)

### 步骤3：创建自定义 API 

最后我们 [创建](https://cloud.tencent.com/document/product/1301/68457) 一个自定义 API ，该 API 用来编写同步数据逻辑。
![](https://qcloudimg.tencent-cloud.cn/raw/7b49d544be372ab920ee95e0927407d5.png)

## 自定义 API 方法

我们在自定义 API 中，创建一个**数据同步**的方法，该方法会处理以下两个事件：
- 从腾讯文档读取数据。
- 将数据写入数据模型。
<dx-codeblock>
:::  js
module.exports = async function(params, context) {
  ...
  const { sheetData } = await context.callConnector({
    name: 'txwd_o6j7pzt', // 腾讯文档 API 标识
    methodName: 'getSheets', // 腾讯文档 API 方法标识
    params: {
      bookID,
    },
  });
  ...
  const { rows } = await context.callConnector({
    name: 'txwd_o6j7pzt', // 腾讯文档 API 标识
    methodName: 'getRows', // 腾讯文档 API 方法标识
    params: {
      bookID,
      sheetID,
...
我们可以通过多种方法在应用中 [使用自定义 API ](https://cloud.tencent.com/document/product/1301/68458)，以下我们将以 [模型应用](https://cloud.tencent.com/document/product/1301/67252) 来做举例说明。
...
3. 选择**调用数据源方法**并选择自定义 API 及其方法单击**保存**即可。

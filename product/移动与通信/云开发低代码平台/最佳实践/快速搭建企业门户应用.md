...
9. 添加一个**数据详情--数据列表**组件，选择模板为卡片列表。
![](https://qcloudimg.tencent-cloud.cn/raw/8d8b078bd11807909a264ec045db484e.png)
10. 在右侧对数据列表组件进行数据绑定配置，在右侧数据模型配置中选择前面通过 Excel 生成的数据模型。
![](https://qcloudimg.tencent-cloud.cn/raw/fd6ee0ea57ae517ac7c90a8fe4e33e61.png)
11. 数据配置完成后，我们需要对数据列表的样式进行调整。 在大纲树中选中数据列表下方的普通容器，在组件的样式配置区将边框调整为无。
![](https://qcloudimg.tencent-cloud.cn/raw/9acf092006109020fc7a5b79155bff5d.png)
12. 随后我们对数据列表组件中的图片进行数据绑定，选中**数据列表-普通容器**下的**图片**组件，随后在右侧配置区中单击**数据绑定按钮**，绑定对应的数据字段。
![](https://qcloudimg.tencent-cloud.cn/raw/d4915024822b65efffd2686f4e3d660e.png)
13. 重复上述方法，为文本组件进行数据绑定，绑定完成后效果如下：
![](https://qcloudimg.tencent-cloud.cn/raw/cadf52d3e9135af8d7a91f050eb60211.png)
14. 之后我们修改数据列表的配置，使此处仅展示4条数据，并且分页模式修改为不分页。
![](https://qcloudimg.tencent-cloud.cn/raw/c5fa0da478ff81b193e860aae768b614.png)
15. 之后我们可以使用相同的方式进行"合作伙伴"模块的构建，合作伙伴的相关数据需要我们通过字段进行查找，可以先查看数据模型中的数据明确要筛选的内容。
![](https://qcloudimg.tencent-cloud.cn/raw/a255ecc6222a26c71fb0a6f9531ed2b0.png)
16. 大纲树选中数据列表组件，在右侧配置区域可以看到属性-基础属性-数据筛选，点击，出现下图所示的数据筛选弹窗，配置筛选条件为 category 等于"合作"。
![](https://qcloudimg.tencent-cloud.cn/raw/073238b22e79d9642623da1a3f8ba391.png)
17. 配置完成后点击确定，可以看到列表视图仅会展示合作伙伴的相关数据， 至此我们的企业门户主页创建完成，效果如下图：
![](https://qcloudimg.tencent-cloud.cn/raw/7e67a366f074a91caf3aef4e6589f67c.png)
...
2. 拖入**列表视图**（**列表视图**组件已更新为**数据列表**）组件，选择模板为**图文列表**。
![](https://qcloudimg.tencent-cloud.cn/raw/f5b8343ebd1e0480feb0df7d206c98dd.png)
3. 为**列表视图**（**列表视图**组件已更新为**数据列表**）绑定数据模型后，将列表中的图片、文本依次与数据进行绑定即可完成图文列表页的构建。
...
3. 随后我们在组件区中选择**数据视图组件**（**数据视图组件**已更新为**数据详情组件**），将数据视图（数据详情）组件自带的普通容器删除，并将大纲树的全部组件拖入数据视图组件下。
![](https://qcloudimg.tencent-cloud.cn/raw/be86e1460d39970573029d5105521393.png)
4. 为数据视图（**数据视图组件**已更新为**数据详情组件**）组件绑定数据模型，随后依次为内容详情页面的图片、文本、富文本组件进行数据字段绑定，绑定完成后我们便完成了内容详情页的搭建。
...
1. 选中企业门户主页**列表视图**（**列表视图**组件已更新为**数据列表**）下的普通容器组件，并在右侧组件配置区单击行为配置按钮。
...
5. 之后在**内容详情页**选中**数据视图**（**数据视图组件**已更新为**数据详情**）组件，并单击右侧配置区的**数据筛选**，调起数据筛选弹窗。

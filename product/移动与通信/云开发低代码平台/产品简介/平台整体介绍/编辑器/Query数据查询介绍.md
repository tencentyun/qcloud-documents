...
>
- SQL 的批量操作示例，例如批量查询或批量删除等场景，可通过传入前端数组变量来实现。例如新建了自定义数组变量`$w.page.dataset.state.array`，默认值为`[50,51,52]`，那么 SQL 语句可写作：`SELECT * FROM users WHERE users.id IN ({{$w.page.dataset.state.array]}})`，或者：`SELECT * FROM users WHERE users.id IN ({{$w.page.dataset.state.array.join(",")}})`，上述两种方式均可。

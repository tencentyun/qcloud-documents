腾讯会议提供的 Rest API 是为参与腾讯会议生态系统建设的合作方开发者接入并访问腾讯会议资源提供的一组工具，是访问腾讯会议 SaaS 服务的入口。开发者可以通过腾讯会议 API 进行二次开发，例如创建一个会议，修改会议，查询会议信息等。腾讯云微搭提供了腾讯会议 API，帮助开发者在应用中方便的使用腾讯会议开放能力。

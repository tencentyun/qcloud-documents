...
《腾讯SDK隐私保护指引》主要向第三方开发者和其终端用户说明我们如何处理个人信息（处理包括“收集、存储、使用、加工、传输、提供、公开等”。
...
**我们对个人敏感信息采用“加黑并斜体”的书写方式进行特别提醒**。如对本声明内容有任何疑问、意见或建议，可随时通过本声明 [第十条]() 提供的方式与我们联系。
 
...
| SDK 名称             | 收集的个人信息                                      | 实现的功能目的                                          |
| ---------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| TPNS SDK                     | 厂商、机型（TPNS 直接收集）                                   | 用于判断用户的的手机类型，分别走不同的厂商通道。             |
| TPNS SDK                     | Idfv（仅 iOS）   （TPNS 直接收集）                             | 用于判断是否是真机，在推送时可以过滤掉虚拟机，防止黑产及外部恶意攻击。 |
| TPNS SDK                     | 通知栏开关状态（TPNS 直接收集）                               | 用于判断通知栏开关状态，对于关闭的应用不下发推送。           |
| TPNS SDK                     | 网络状态（TPNS 直接收集）                                     | 用于判设备是否在线，若设备在线则立即推送消息，若设备不在线则缓存消息。 |
| TPNS SDK                     | 第三方版本（TPNS 直接收集）                                   | 支持第三方根据不同版本推送不同的消息内容。                   |
| TPNS SDK                     | 系统语言（TPNS 直接收集）                                     | 支持第三方根据不同系统语言，推送不同的语言内容。             |
| TPNS SDK                     | Android/iOS 系统版本（TPNS 直接收集）                          | 支持第三方根据不同系统版本推送不同的消息内容。               |
| TPNS SDK                     | 时区（TPNS 直接收集）                                         | 支持全球范围推送，规划对不同时区定时推送。                   |
| TPNS SDK                     | 设备分辨率（TPNS 直接收集）                                   | 支持根据不同的分辨率推送不同的图片。                         |
| TPNS SDK                     | 网络类型（例如 Wi-FI、3G、4G）（TPNS 直接收集）                 | 支持 Wi-FI 环境下推送视频   其他环境下推送文字和图片。          |
| TPNS SDK                     | 设备启动时间（TPNS 直接收集）                                 | 通过判断设备是否开机，用于智能推送。设备未开机时缓存消息，设备开机时立即推送消息。 |
| TPNS SDK                     | 设备名称（TPNS 直接收集）                                     | 用于个性化推送，即在推送时可以自动将“设备名称”作为用户昵称，放到消息内容里。 |
| TPNS SDK                     | 国家编码（TPNS 直接收集）                                     | 支持对不同国家推送不同的消息内容。                           |
| TPNS SDK                     | 运营商代号（TPNS 直接收集）                                   | 用于判断网络类型和网络状态，设备可就近接入，缩短推送下发路径，提高推送效率。 |
| TPNS SDK                     | ram、rom（TPNS 直接收集）                                     | 用于判断是否是真机，在推送时可以过滤掉虚拟机，防止黑产及外部恶意攻击。 |
| TPNS SDK                     | SD卡容量（TPNS 直接收集）                                     | 用于判断是否是真机，在推送时可以过滤掉虚拟机，防止黑产及外部恶意攻击。 |
| TPNS SDK                     | 用户账号（第三方开发者主动提供）                             | 支持第三方按用户账号进行推送。                               |
| Bugly SDK（开发者可选）      | cras h时包含本身 SDK 的堆栈（当第三方集成 Bugly   SDK 时，方可获取 crash 信息） | 监控 TPNS 自身 SDK 质量。                                        |
| VIVO PUSH SDK（开发者可选）  | 参考 [vivo 官方隐私协议](https://dev.vivo.com.cn/documentCenter/doc/366#w1-08512613) | TPNS包含各厂商推送，以提升消息抵达率。                       |
| OPPO PUSH SDK（开发者可选）  | 参考 [OPPO 官方隐私协议](https://open.oppomobile.com/wiki/doc#id=10288) | TPNS 包含各厂商推送，以提升消息抵达率。                       |
| 小米推送 SDK（开发者可选）    | 参考[ 小米推送隐私协议](https://dev.mi.com/console/doc/detail?pId=1822)| TPNS 包含各厂商推送，以提升消息抵达率。                       |
| 魅族推送 SDK（开发者可选）    | 参考 [隐私声明]( https://www.meizu.com/legal.html)（魅族通用的隐私声明）    | TPNS 包含各厂商推送，以提升消息抵达率。                       |
| 华为推送 SDK（开发者可选）    | 参考 [华为开发者联盟与隐私的声明](https://developer.huawei.com/consumer/cn/devservice/term)   | TPNS 包含各厂商推送，以提升消息抵达率。                       |
| Google FCM SDK（开发者可选） | 参考 [Google 隐私权和条款](https://policies.google.com/privacy)  | FCM 是海外安卓设备的系统级通道，海外推送会优先走 FCM 通道，例如 FCM 通道推送失败，才会走 TPNS 自建通道进行补推。 |
| APNs（开发者可选）           | 参考 [iOS 的隐私政策]() | APNs通道是苹果手机的系统级通道，TPNS 在对 iOS 设备做消息推送时，会优先走 APNs 通道，例如 APNs 通道推送失败，才会走 TPNS 自建通道进行补推。 |
...
#### 1. 针对第三方开发者
第三方开发者访问、修改和删除个人信息的范围和方式将取决于第三方开发者使用的开发者平台功能设置，也可以按照本声明第十条所述方式与我们联系。
...
## 八、如何处理儿童个人信息
请第三方开发者确保是18周岁（含）以上人士。请第三方开发者理解并知悉，如果第三方是针对儿童用户的，请第三方开发者务必确保第三方终端用户（儿童）的监护人已经阅读并同意了第三方的隐私政策以及《腾讯儿童隐私保护声明》，并且经其同意后提供儿童个人信息给我们，以实现第三方在隐私政策中所述的相关功能。
...
本声明是腾讯各SDK开发者平台服务协议的重要组成部分。我们还会将本声明的旧版本存档，供第三方开发者和/或终端用户查阅。

## 十、如何联系我们

如第三方开发者和/或终端用户对本声明或个人信息相关事宜有疑问，可以通过以下方式与我们取得联系：（i）通过 https://kf.qq.com/与我们联系进行在线咨询；（ii）发送邮件至Dataprivacy@tencent.com；（iii）邮寄信件至：中国广东省深圳市南山区科技中一路腾讯大厦 法务部 数据及隐私保护中心（收）邮编：518057。我们将及时验证请求人身份并尽快审核所涉问题，并于十五个工作日内予以回复。

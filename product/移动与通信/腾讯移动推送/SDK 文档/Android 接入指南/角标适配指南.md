...
|OPPO|支持红点 | 否|<br>圆点展示需由用户在通知设置中手动开启，遵从系统默认逻辑，有通知则展示，无则不展示；</br><br>数值展示只对指定应用开启，如 QQ、微信，需向官方进行权限申请，暂无明确适配说明</br> |
...
|推送形式 | 角标能力 |  实现方式|
|:-:|:--:|:--:|
|华为通道通知 | 推送时自动加1 或 不变 | 通过管理台或 push api 关键字设置 |
...
### 实现方式
#### 通知下发角标设置
通知栏推送下发的角标可通过管理台推送页面或 REST API  实现。
请一定先在管理台华为通道开启及参数配置处填写桌面图标对应的应用入口Activity类，如“com.test.badge.MainActivity”，否则角标设置将不生效，如图所示：

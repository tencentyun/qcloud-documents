...
4.0 版本 SDK 相较 2.6 版本，在接口调用、Push服务开启步骤等方面有较大改变。
腾讯移动推送跟进各厂商通道推送服务的更新进度，提供集成华为推送 4.0 SDK （HMS Push SDK V4） 的插件依赖包供用户选择使用。
>!  
...
3. 在华为推送平台中新建应用，详情参见 [创建应用](https://developer.huawei.com/consumer/cn/doc/distribution/app/agc-create_app)（应用包名需跟您在腾讯移动推送平台填写的一致）。
...
1. 手动从华为仓库下载华为推送服务 4.0 SDK 所需全部依赖包，存放到 app 模块 libs 目录下：   
   1. 包括一个 gradle 脚本 jar 依赖 [agcp-1.2.1.301.jar](https://developer.huawei.com/repo/com/huawei/agconnect/agcp/1.2.1.301/agcp-1.2.1.301.jar)；
   2. 和以下7个程序aar依赖：

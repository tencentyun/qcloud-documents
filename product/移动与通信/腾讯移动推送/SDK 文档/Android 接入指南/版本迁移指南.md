...
针对从信鸽 4.x 版本迁移到 TPNS 腾讯云版本的用户，在版本迁移中变更如下：

## Android Studio 自动集成方式
#### 依赖变更
...
## Eclipse 手动集成方式
####  依赖包变更
请前往 [腾讯移动推送控制台](https://console.cloud.tencent.com/tpns/sdkdownload)，下载安卓 SDK 压缩包、解压并按照以下步骤替换依赖包文件：
- 删除信鸽 4.x 版本所使用的全部 .jar 文件，使用 libs 目录下的所有 .jar 文件替换。
- 删除信鸽 4.x 版本所使用的全部 .so 文件，在 Other-Platform-SO 目录下按照自己当前 .so 支持的平台添加 .so 文件。

#### AndroidManifest 文件变更
在安卓配置文件 AndroidManifest.xml 中，删除所有信鸽 4.x 版本使用组件和权限，删除内容具体如下：
```
<application>
    <!-- 【必须】 信鸽receiver广播接收 -->
...
请参见新版本 [SDK 集成](https://cloud.tencent.com/document/product/548/36652) 重新添加。


<span id="jkbg"></span>
## 接口变更
与4.x对比，部分 API 接口做了以下变更。
...
// 替换为以下对应的厂商依赖。[VERSION] 为当前SDK版本号，版本号可在SDK下载页查看
...
```	
- 手动集成的方式需要替换 .jar 文件
前往 [腾讯移动推送控制台](https://console.cloud.tencent.com/tpns/sdkdownload)，下载安卓 SDK 压缩包、并解压目录 Other-Push-jar 下，找到对应厂商通道所需 .jar 文件，替换信鸽 4.x 版本使用的厂商通道 .jar 文件。

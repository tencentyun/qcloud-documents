...
绑定手机号码：添加或更新手机号码。若原来没有绑定手机号码，则绑定；若原来有，则覆盖（SDK 1.2.5.0+）


>? 如果绑定的手机号码是纯数字，则会被默认当作大陆的手机号码，服务器下发短信时自动增加+86的前缀,如果绑定的手机号码是带了+xx的区号，则按照指定的号码绑定，不做补齐；建议绑定手机号码时，加上+xx的区号，如需删除手机号，调用 delAccountsByKeys (设置 accountTypeSet 为 1002)
>
...
- context： Context 对象。
- phoneNumber：phoneNumber E.164标准，格式为+[国家或地区码][手机号],例如+8613711112222。SDK内部加密传输。
- callback： 绑定手机号操作的回调。

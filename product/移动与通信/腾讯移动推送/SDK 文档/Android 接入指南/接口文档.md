...
>? 如果需要统计由移动推送 TPNS 推送引起的打开 App 操作或获取下发的自定义 key-value，需要开发者在所有（或被打开）的 Activity 的 onResume() 调用以下方法。
>
...
#### 示例代码
```
// 通知点击回调 actionType=2为该消息被清除，actionType=0为该消息被点击
@Override
...
XGPushClickedResult：通知被打开的对象，如果该 activity 是由移动推送 TPNS 的通知引起打开动作的，返回 XGPushClickedResult，否则返回 null。

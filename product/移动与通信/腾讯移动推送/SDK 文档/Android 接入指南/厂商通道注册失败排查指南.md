...
- 方式一：在应用运行日志中通过过滤关键字 `OtherPush`，找到如下类似日志来定位厂商通道注册返回码：
   ```
   // 华为通道
   // 如果过滤关键字 `OtherPush` 找不到返回码，可以过滤关键字 `HMSSDK`，并注意查看onResult或onConnect后的返回码 。
...
- **小米应用配置参数获取方式不正确**
请确认小米推送的应用配置参数是从小米开放平台 [推送运营平台](https://admin.xmpush.xiaomi.com/zh_CN/app/nav) 获取，而非小米开放平台联运平台，否则容易导致小米推送注册过程中返回 22022 包名不匹配问题。

- **vivo部分机型不支持推送服务**

vivo推送仅支持部分较新的机型和对应的系统及以上系统，详情请参考 [vivo推送常见问题汇总](https://dev.vivo.com.cn/documentCenter/doc/156) 二、Q5：vivo推送支持哪些机型和系统版本？

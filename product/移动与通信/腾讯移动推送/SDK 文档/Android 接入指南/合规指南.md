...
| APNs  | 对苹果品牌手机消息推送 | [苹果隐私政策](https://www.apple.com/legal/privacy/en-ww/) | 
...
#### Provider 组件初始化时机配置
TPNS Android SDK 1.3.3.3 起新增以下接口配置是否自动初始化 SDK 的 Provider 组件功能，默认开启。
...
#### 自启动配置
TPNS Android SDK 1.3.4.3 起新增以下接口配置是否开启自启动，默认关闭。
>! 如需开启，请一定在参考 [TPNS隐私保护指引](https://privacy.qq.com/document/preview/8565a4a2d26e480187ed86b0cc81d727) ，在应用自己的隐私政策中披露自启动行为，并确认在用户同意隐私政策后才配置开启。
...
```
>?TPNS SDK 在测试观察部分厂商系统自带的应用启动记录工具时发现，当前 TPNS SDK 版本在某些厂商的部分设备上仍然有被记录存在自启动情况，但属于系统导致的自启动非 SDK 行为。该情况我们已经和厂商直接沟通，其反馈后续的新系统不会再有此问题。如您对此有相关疑问，请参考本文第7点提及的联系方式与我们联系。

#### 关联启动配置
配置是否允许拉起其他集成 TPNS 服务的应用的后台推送进程，默认关闭。
>! 如需开启，请一定在参考 [TPNS隐私保护指引](https://privacy.qq.com/document/preview/8565a4a2d26e480187ed86b0cc81d727) ，在应用自己的隐私政策中披露关联启动行为，并确认在用户同意隐私政策后才配置开启。

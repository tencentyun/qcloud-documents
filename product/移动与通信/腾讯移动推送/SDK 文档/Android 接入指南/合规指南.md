...
[](id:part4)
### 4. SDK 合规接口配置说明
随着国内监管层对用户隐私合规保护要求进一步严格、个人信息保护条目范畴进一步扩大，强烈建议您参考 [Android SDK 发布动态](https://cloud.tencent.com/document/product/548/44520) 升级使用最新版本 SDK。

此外，SDK 提供以下接口配置，建议用户按实际需求配置使用。

#### Povider 组件初始化时机配置
TPNS Android SDK 1.3.3.3 起新增以下接口配置是否自动初始化 SDK 的 Povider 组件功能，默认开启。
>! 如不需要自动开启，请在应用工程 `Application.attachBaseContext()` 方法内调用此接口并传入 false。

```java
/**
     * 配置 SDK 的 Povider 组件功能是否自动初始化，默认开启
     *

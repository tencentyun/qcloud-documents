...
2. 导入华为推送相关依赖，示例代码如下：
```js
 implementation 'com.tencent.tpns:huawei:1.1.3.0-release'//华为推送
```
...
3. 在 `Androidmanifest.xml` 文件中，新增如下配置：
	...
	<!-- 注：华为push 需要的 end -->
	```
4. 自定义类，继承 `com.huawei.hms.support.api.push.PushReceiver`，并在 Androidmanifest.xml 中，配置相关的节点。示例代码如下：
	```java
...
```
5. 在 AndroidManifest.xml 增加 Receiver， 配置如下：
```xml

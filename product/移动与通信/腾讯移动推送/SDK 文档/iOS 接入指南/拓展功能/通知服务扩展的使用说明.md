...
通过 Cocoapods 下载地址：
``` 
pod '移动推送-iOS-Extension', '~> 版本'  // 如果不指定版本，则默认为本地 pod 移动推送-iOS-Extension 最新版本
```
**使用说明：**
...
target ‘XXServiceExtension'do
     platform:ios,'10.0'
     pod '移动推送-iOS-Extension' , '~>1.2.7.2' 
end
```
>? 建议配合 pod '移动推送-iOS' version 1.2.7.2 及以上版本使用。
>
...
```plaintext
移动推送service auto coding done!
New 移动推送Service Extension Success
```  
...
> - 当前 `homebrew` 命令 `new_tpns_svc_ext` 只支持集成通知扩展插件 `移动推送Service`，暂不支持基础推送能力的集成。
>
...
```objective-c
/**
 @brief 移动推送处理富媒体通知和抵达到终端的消息，即消息回执
 @param request 推送请求
 @param accessID 移动推送应用 accessId
 @param accessKey 移动推送应用 accessKey
...
2. 通过 PID 或者 Name 将 UNNotificationServiceExtension 的实现 Target（Demo 中抵达插件为 移动推送Service-Cloud）连接到主 Target。

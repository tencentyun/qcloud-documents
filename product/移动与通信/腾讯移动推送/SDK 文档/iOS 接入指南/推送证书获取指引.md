...
2. 您可以参考如下步骤新建一个 AppID，或者在您原有的 AppID 上增加 `Push Notification` 的 `Service`。需要注意的是，您 App 的 `Bundle ID` 不能使用通配符 `*`，否则将无法使用远程推送服务。
...
3. 勾选【Push Notifications】，开启远程推送服务。
...
2. 您可以看见，在【Apple Push Notification service SSL Certificates】窗口中有两个 `SSL  Certificate` ，分别是用于开发环境（`Development`）和生产环境（`Production`）的远程推送证书，如下图所示：
...
6. 返回 `Apple Developer` 网站刚才的页面，单击【Choose File】上传生成的`*.certSigningRequest`文件。

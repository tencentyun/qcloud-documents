macOS 端实现推送消息的服务涉及三个角色：终端应用（Client App），APNs（Apple Push Notification service），移动推送 TPNS 服务器（XG Provider）。在使用移动推送 TPNS 服务实现给客户端推送消息，需要这三个角色在整个流程中相互配合，任何一个角色出现异常都可能会导致消息无法推送。

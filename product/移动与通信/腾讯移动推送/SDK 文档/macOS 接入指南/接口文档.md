...
>!接口所需参数必须要正确填写，反之信鸽服务将不能正确为应用推送消息。
...
#### 接口说明
调用此接口上报当前 App 角标数到信鸽服务器，客户端配置完成即可使用 macOS 角标自动加1的功能，如需使用此功能，请前往腾讯移动推送控制台，选择创建推送 > 通知栏消息 > 常用设置 > 角标数字，进行设置。
...
#### 接口说明
SDK 的启动方法自动注册设备从 APNs 获取的 Token 到信鸽服务器，注册结果会在 `XGPushDelegate` 以下的回调方法返回。

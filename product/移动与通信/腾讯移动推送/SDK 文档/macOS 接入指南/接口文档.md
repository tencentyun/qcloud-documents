...
## 启动移动推送 TPNS 服务
#### 接口说明
使用在移动推送 TPNS 官网注册的应用的信息，启动移动推送 TPNS 服务。
...
#### 接口说明
终止移动推送 TPNS 服务以后，将无法通过移动推送 TPNS 服务向设备推送消息，如果再次需要接收移动推送 TPNS 服务的消息推送，则必须需要再次调用 `startXGWithAppID:appKey:delegate:` 方法重启移动推送 TPNS 服务。
...
#### 接口说明
调用此接口上报当前 App 角标数到移动推送 TPNS 服务器，客户端配置完成即可使用 macOS 角标自动加1的功能，如需使用此功能，请前往移动推送 TPNS 控制台，选择【创建推送】 > 【通知栏消息】 > 【常用设置】 > 【角标数字】，进行设置。
...
#### 接口说明
SDK 的启动方法自动注册设备从 APNs 获取的 Token 到移动推送 TPNS 服务器，注册结果会在 `XGPushDelegate` 以下的回调方法返回。

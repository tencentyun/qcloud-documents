...
在账号推送的基础上，移动推送 TPNS 提供对某一批具体相同特征的账号推送消息，在 [控制台](#.E6.8E.A7.E5.88.B6.E5.8F.B0.E4.BD.BF.E7.94.A8) 或 [调用 API](#rest-api-.E4.BD.BF.E7.94.A8) 上传号码包文件后，即可对这批账号推送消息。
...
1. 在 [控制台](https://console.cloud.tencent.com) >【消息推送】>【新建推送】>【推送目标】处选择「批量账号」，上传账号号码包文件，并选择推送给「最近」或「所有」绑定该账号的设备，如图：
![](https://main.qcloudimg.com/raw/a4956ed860fffd1e9ab8dff8256df66b.png)
> ?上传号码包要求如下：
>1. 账号包格式及大小： 支持 zip\txt\csv 文件；大小保持在 100Mb 以内。
>2. zip 压缩包中可包含：单个 .txt 或 .csv 文件；（不能嵌套文件夹）。
>3. .`txt` 文件要求：（1）编码为UTF-8；（2）每行一个账号，账号长度限制为 [2, 100]。
>4. .`csv` 文件要求：（1）只能有一列；（2）每行一个账号，账号长度限制为 [2, 100]。

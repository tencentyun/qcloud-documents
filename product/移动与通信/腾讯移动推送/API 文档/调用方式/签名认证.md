...
## 概述
本文主要为您介绍腾讯移动推送鉴权认证方法。
- V3接口历史：采用 AccessId 和 SecretKey 进行 Basic Auth 认证鉴权；密钥容易被获取，安全性不高。
- 新接口鉴权方式：采用 HMACSHA256 算法，根据 SecretKey 生产签名信息。通过校验签名，进行鉴权。

#### 参数说明

| 参数 | 说明 |
| --- | --- |
| AccessId | 信鸽后台分配的应用 ID |
| SecretKey | 信鸽后台分配的密钥，和 AccessId 一一对应 |
| LoginUin | 腾讯云上登录账号 |
...
```
2. 根据密钥通过 HMAC-SHA256 算法，生成十六进制 hash，其中示例对应 `secretKey =&quot;1452fcebae9f3115ba794fb0fff2fd73&quot`。
```
hashcode= hmac-sha256(代签名字符串， secretKey)
得到hashcode="cd20774682bf78bfdb43e17d1d5d56b3e5b789a1670fc1527ef54c65d2d7b76d"
```
3. 对 hashcode 进行 base64 编码，得到签名串如下：
```
得到Sign=Base64(hashcode)
Sign="Y2QyMDc3NDY4MmJmNzhiZmRiNDNlMTdkMWQ1ZDU2YjNlNWI3ODlhMTY3MGZjMTUyN2VmNTRjNjVkMmQ3Yjc2ZA=="
```

...
```
**接口功能**：用户需要对批量账号，通过文件的方式，上传号码包文件。然后对号码包中的文件进行推送。号码包推送接口主要包括号码包上传接口、以及号码包推送接口。
...
| 参数名  | 类型  | 是否必须  | 参数说明  |
| --- | --- | --- | --- |
| file  | form-data  | 是  | 上传文件必须 是 .zip 文件，大小100M 以内每个 .zip 文件内容必须是 txt 文件，不能嵌套文件夹， txt 文件中的每一行代表1个账号，账号长度限制为[2, 100] |
...
| uploadId    | int32   | 是   | 当上传文件成功时，会反馈一个 正整数 uploadId ，代表上传文件 ID，提供给后续号码包接口进行推送   |

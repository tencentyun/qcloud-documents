...
| badge_type           | Integer | aps/android   | 否                             | -1                                   | 用户设置角标数字， 当推送平台是android时，放置在Android结构体中,当推送平台是ios 时,放置在aps 结构体中<li> -1：角标数字不变</li> <li> -2：角标数字自动加1 </li><li> >=0：设置自定义角标数字</li> |
...
| account_tag_operator | String  | 无     | 账号列表推送时可选             | 无                                   | 账号列表和标签进行组合推送：<br><li> 仅支持 AND<li>需要和 [tag_rules 或 tag_list](#audience_type) 同时使用，当 account_tag_operator 为 AND 时，账号和标签计算结果进行“与”计算后再推送，此时如果未填 tag_rules 或 tag_list ，则无计划推送设备数<li>详细使用方法可参考 [账号列表和标签进行与推送示例](#zhanghaobiaoqianshili ) |

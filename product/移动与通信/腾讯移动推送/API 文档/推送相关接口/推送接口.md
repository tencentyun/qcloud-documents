...
| upload_id     | Integer | 是（仅号码包推送\token文件包推送时使用） | 号码包或 token 包的上传 ID，可通过[调用此接口](https://cloud.tencent.com/document/product/548/39065)获取                                             |
...
| package_token_push |  token文件包推送   | 上传 token 文件包推送必填，需要和上传的文件中包含的目标类型对应（token 或 账号）
                                         |

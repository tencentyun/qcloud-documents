...
| token_list   | 设备列表群推 | `token_list`<li>最多1000个token<li>格式 eg：["token1","token2"]<li>token 字符串长度不能超过36 |
| account      | 单账号推送   | `account_list`<li>该参数有多个账号时，仅推送第一个账号<li>格式 eg：["account1"] |
| account_list | 账号列表群推 | `account_list`最多1000个account2. 格式 eg：["account1","account2"] |
...
| custom_content | string | 无    | 否    | 自定义下发的参数                                 |
| xg     | string      | 无    | 否    | 系统保留 key，应避免使用                            |

...
```
- 标签推送：推送给同时设置了 2020年4月21,22,23号 和`男性`标签的设备
...
| custom_content | string | Android   |无    | 否    | 用户自定义的参数， ⚠️需要序列化为 json string <li>若需要进行消息覆盖,则不能使用custome_content 传递自定义参数，需要修改使用intent 方式进行消息覆盖。|
...
| collapse_id  |uint32 |无|仅支持全推、标签推送、号码包推送|若不填此参数,系统会为推送任务默认分配一个ollapse_id|<li>消息覆盖参数,在前一条推送任务已经调度下发后，如果第二条推送任务携带相同的 collapse_id  则会停止前一条推送中尚未下发的 TPNS 通道数据， 同时会覆盖展示第一条推送任务的消息<li>collapse_id 可以从推送记录中查看<li>目前支持TPNS 通道，APNS 通道， 小米通道， 魅族通道，华为通道（V4 版协议）。<li>华为通道 目前消息覆盖只支持 V4 版协议。 因此进行消息覆盖，不能使用custome_content  传递自定义参数，需要修改使用 intent 方式进行消息覆盖。<li>目前 OPPO 通道 VIVO 通道不支持 覆盖消息。当新创建覆盖消息是可通过 force_collapse 字段设置为true 进行处理。|

...
| xg_media_resources    | String  | message       |无    | 否    | 通知栏大图片 url 地址，仅对 TPNS 通道和小米通道生效；<li>**注意：**如需使用小米通道大图通知功能，需先调用小米图片上传接口上传图片文件，获取小米指定的图片地址 pic_url ，再填入 TPNS 推送对应的参数 xg_media_resources 中。参考文档：[小米推送富文本消息](https://dev.mi.com/console/doc/detail?pId=1163#_10_0) - 上传大图 API 部分。 |

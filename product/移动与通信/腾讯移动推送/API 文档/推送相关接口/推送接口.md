...
| custom_content | string | Android   |无    | 否    | 用户自定义的参数， ⚠️需要序列化为 string       |
...
| custom_content | string |ios | 无    | 否    | 自定义下发的参数，需要序列化为 string                                |
...
| custom_content | string | android  | 无    | 否    | 自定义内容                    |
...
| ios    | JSON       |message  | 无    | 是    | ios消息结构体  |
| custom_content | String | ios| 无    | 否    | 自定义下发的参数                                 |
...
| account_push_type  | int  |  无 | 单账号推送时可选         | 0       |<li> 0：往单个账号的最新的 device 上推送信息<li> 1：往单个账号关联的所有 device 设备上推送信息|

...
| 参数名 | 类型      | 是否必须 | 参数说明                                                     |
| ------ | --------- | -------- | ------------------------------------------------------------ |
| file   | form-data | 是       | <li>`Token` 包格式及大小：` .zip `压缩文件； 大小保持在100M<li>压缩包中可含有： 单个`.txt` 或`.csv` 文件， (不能嵌套文件夹)<li>`.txt` 文件要求：<br>（1）编码为 UTF-8<br>（2）每行一个` Token`，有效` Token` 长度为36位<li>`.csv` 文件要求：<br>（1）只能有一列<br>（2）每行一个` Token`，有效 `Token `长度为36位 |

...
| startDate | 是   | string       | 查询截止日期<li>格式：YYYY-MM-DD<li>限制：只能聚合统计最近7天的推送任务 |
| endDate   | 是   | stringstring | 查询截止日期，格式：YYYY-MM-DD                               |
...
| retCode      | int    | 返回状态码                             |
| errMsg       | string | 错误信息                               |
| pushStatData | Json   | 返回结果<br>pushStatData 结构变量见下表 |

...
>?
>数组中“all” 通道对应汇总统计数据。
>-  汇总数据中 verifySvcUv（抵达设备），verifyUv（展示），clickUv（点击），cleanupUv（清除）指标只汇总计算了TPNS通道数据、ROG通道数据、FCM 通道数据。
>-  汇总数据中 pushActiveUv（计划发送）, pushOnlineUv（实际发送）汇总计算了 TPNS 通道 + 厂商通道的数据。
>-  汇总数据中 callbackVerifySvcUv（厂商通道抵达回执）汇总计算了   厂商通道 callbackVerifySvcUv（厂商通道抵达回执）+ TPNS 通道 verifySvcUv（抵达设备）+ ROG 通道 verifySvcUv（抵达设备）+ FCM通道 verifySvcUv（抵达设备）。

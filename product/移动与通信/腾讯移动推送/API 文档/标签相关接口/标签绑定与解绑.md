...
>!
- 单个应用最多可以有10000个自定义`Tag`， 每个设备`Token`最多可绑定100个自定义`Tag`，每个自定义`Tag`可绑定的设备 token 数量无限制。
...
|invalid_tokens|array string|否 | 请求返回错误的token|
|invalid_tags|  array string|否 | 请求返回错误的tags|

...
| tag_list | array | 否 | 标签列表：<li>operator_type =1,2,3,4,6,7,8时必填，=5时忽略<li>operator_type =1,2,3,4,6,7,8时如果该参数包含多个 tag 时，如果只是对单个 tag 操作，则只会设置第一个 tag<li>格式 eg：["tag1","tag2"]<li>列表最大不能超过20个值<li>tag 字符串长度不能超过50 |

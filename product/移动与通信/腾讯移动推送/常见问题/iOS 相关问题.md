...
**推送证书排查**
信鸽服务器在向 APNs 请求消息下发的时候，需要使用两个必需的参数：消息推送证书和设备标识（Device Token），在进行消息推送的时候，请确保消息推送证书是有效的。关于消息推送证书的设置请参见 [iOS 推送证书说明](/ios_access/ios-tui-song-zheng-shu-shuo-ming.md)。
...
>?在通过信鸽管理台上传证书之后，证书的生效之间一般需要5分钟左右。

...
- 除了一种特殊情况外，请求中的 From_Account 和 to_Account 字段值互换，查询结果不变。如果消息是通过 REST API 接口 [单发单聊消息](https://cloud.tencent.com/document/product/269/2282)或 [批量发单聊消息](https://cloud.tencent.com/document/product/269/1612)发送，而且指定了 SyncOtherMachine 的值为1，那么这种情况下，这条消息是否能拉到取决于From_Account 和 to_Account的顺序。举例来说明，假设通过上述的接口指定帐号 A 给帐号 B 发一条消息，同时指定 SyncOtherMachine 的值为1。那么调用本接口时， From_Account 填帐号 A ，to_Account填帐号 B ，就拉不到这条消息，反之则可以拉到。

...
- 若消息通过 [单发单聊消息](https://cloud.tencent.com/document/product/269/2282) 或 [批量发单聊消息](https://cloud.tencent.com/document/product/269/1612) 接口发送，且指定 SyncOtherMachine 值为1，则查询该消息时需指定正确的 From_Account 和 to_Account 字段值，否则无法查询到该消息。其他情况下，请求中的 From_Account 和 to_Account 字段值互换，查询结果不变。
  例如，假设通过 [单发单聊消息](https://cloud.tencent.com/document/product/269/2282) 接口指定帐号 A 给帐号 B 发一条消息，同时指定 SyncOtherMachine 值为1。则调用本接口时， From_Account 必须设置为帐号 A ，to_Account 必须设置为帐号 B 才能查询到该消息。
...
| MsgBody | Object | 消息内容，具体格式请参考 [消息格式描述](https://cloud.tencent.com/document/product/269/2720)（一条消息可包括多种消息元素，MsgBody 为 Array 类型）  |

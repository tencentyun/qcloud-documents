...
| ErrorList | Array | 发消息失败列表，在此列表中的目标帐号，消息发送失败。若消息全部发送成功，则 ErrorList 为空。因此判断回包中是否含 ErrorList ，来判断消息是否全部发送成功。</ul>不存在的目标帐号也会在 ErrorList 中返回，这种情况下该目标帐号的错误码为70107。   |

...
>因Community（社群）人数较多，分页获取方式改用Next分批方法
...
使用Limit 和 Next 两个值用于控制分页拉取：
 ...
 - Next 控制从某一个成员位置拉取后续的信息。第一次请求时，客户端请求参数Next传""；最后一次请求服务端返回的Next为""时，代表拉取结束；
...
| Next | String | 下一次请求应该传的Next值，仅查询 Community（社群）时会返回该字段|

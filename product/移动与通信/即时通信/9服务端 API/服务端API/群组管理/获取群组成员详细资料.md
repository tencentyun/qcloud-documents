...
- **分批获取**
使用Limit 和 Next 两个值用于控制分页拉取：
 - Limit 限制回包中 MemberList 数组中成员的个数，不得超过100。
 - Next 控制从某一个成员位置拉取后续的信息。第一次请求时，客户端请求参数Next传0；最后一次请求服务端返回的Next为0时，代表拉取结束；
 中间的请求，客户端的Next均使用上一次服务端返回的Next。类似redis的scan 游标查询。
 
 例如：假设需要分批拉取，第一次的请求参数应当为：`{"Limit" : 20, "Next" : 0}`，服务端返回 
 ```
{
    "ActionStatus": "OK",
    "ErrorInfo": "ok",
...
```
 ...
 以此类推，直到服务端的应答包返回Next为0，代表无后续成员信息，客户端应结束查询。
...
| Offset | Integer | 选填 |从第几个成员开始获取，如果不填则默认为0，表示从第一个成员开始获取，查询 Community（xxx）时无用 |
| Next | Integer | 选填 |从哪个成员位置开始获取，如果不填则默认为0，表示从最开始成员开始获取，仅查询 Community（xxx）时会使用该字段|
...
| Next | Integer | 下一次请求应该传的Next值，仅查询 Community（xxx）时会返回该字段|

...
|AVChatRoom|支持，但默认不返回此类型群信息。如果指定拉取 AVChatRoom 类型群信息，获得的群信息可能不完整，AVChatRoom 并不存储所有的群成员资料（只存储300人以内的群成员资料），拉取的只是存储了当前用户群成员资料的 AVChatRoom 群。|
...
```
- **指定群组形态**
可以指定所拉取的群组所属的群组类型，例如 Public（陌生人社交群），Private（同新版本 Work，好友工作群）和 ChatRoom（同新版本 Meeting，会议群），如果指定 AVChatRoom（直播群），获得的成员可能不完整（AVChatRoom 并不存储所有的群成员资料，显示的只是存储了群成员资料的 AVChatRoom 的群）。
```
{
    "Member_Account": "leckie",

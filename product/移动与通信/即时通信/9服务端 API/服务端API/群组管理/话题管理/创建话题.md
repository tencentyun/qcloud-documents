...
- **基础形式**
创建话题，其中GroupId必填，代表话题所属的群组。在创建话题之前，群组必须存在且支持创建话题。
```json
{
...
- **自定义话题ID**
为了使得话题 ID 更加简单，腾讯云支持 App 在通过 REST API 创建话题时自定义话题 ID。自定义话题ID必须要以“GroupId+@TOPIC#_+自定义话题ID” 的形式。详情请参阅[自定义话题ID]()
```json
{
    "GroupId": "@TGS#_@TGS#cQVLVHIM62CJ", // 话题所属的群ID（必填）
...
- **仅包含话题维度的自定义信息**
创建话题，并包含话题维度的自定义信息。其中TopicDefinedData 和群共享自定义字段，可以通过 [即时通信 IM 控制台](https://console.cloud.tencent.com/im) 进行配置，详情请参阅 [自定义字段](https://cloud.tencent.com/document/product/269/1502#.E8.87.AA.E5.AE.9A.E4.B9.89.E5.AD.97.E6.AE.B5)
```json
{
    "GroupId": "@TGS#_@TGS#cQVLVHIM62CJ", // 话题所属的群ID（必填）
    "TopicName": "TestTopic", 		// 话题的名称（必填）
...
- **[创建话题之后回调](https://cloud.tencent.com/document/product/269/78206)**

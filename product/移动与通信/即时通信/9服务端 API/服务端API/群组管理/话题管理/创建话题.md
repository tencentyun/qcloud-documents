...
[](id:customiz)
- **自定义话题 ID**
为了使得话题 ID 更加简单，腾讯云支持 App 在通过 REST API 创建话题时自定义话题 ID。自定义话题 ID 必须要以“GroupId+@TOPIC#_+自定义话题 ID” 的形式。详情请参阅 [自定义话题 ID](#customiz)
```json
{
    "GroupId": "@TGS#_@TGS#cQVLVHIM62CJ", // 话题所属的群ID（必填）
...
| TopicId | String | 选填 |为了使得话题 ID 更加简单，便于记忆传播，腾讯云支持 App 在通过 REST API 创建话题时 [自定义话题 ID](#customiz) |

...
| Limit | Integer | 必填 | 单次拉取限制，最大为100 |
| Offset | Integer | 必填 | 分页标识，首次传0，如果封禁的成员数大于100，下一次拉取需要设置为后台回复的NextOffset |
...
```
{
    ...
    "BannedAccountList": [
        {
            "BannedUntil": 1679973861,
            "Member_Account": "charlie"
        }
    ],
    "ErrorCode": 0,
    "ErrorInfo": "",
    "GroupId": "@TGS#1DG6MMMMT",
    "NextOffset": 0
}
```
...
| BannedAccountList | Array | 当前被封禁的群成员列表，其中Member_Account为userid，BannedUntil为封禁到期时间戳 |
| NextOffset | Integer | 分页标识，不为0说明未完全返回，需要将该数值作为下一次请求的Offset |
| GroupId | String | 封禁成员的群 id |

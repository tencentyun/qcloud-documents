...
| 字段 | 类型 | 属性 |说明|
|---------|---------|---------|---------|
| Owner_Account | String | 选填 |群主 ID（需是 [已导入](https://cloud.tencent.com/document/product/269/1608) 的帐号）。填写后自动添加到群成员中；如果不填，群没有群主；AVChatroom群类型，成员使用的时候，必须每次调用进群 (joinGroup) 的操作。 |

...
|AVChatRoom|支持；用户使用的时候必须调用 SDK 接口主动申请进群（包括群主）|
|Community（社群）|支持|
...
>  - 创建此类型群时不能拉人入群，如果创建时指定了拉入的群成员会返回10007错误；用户加入此类型群的唯一方式是主动申请加群；如果指定了群主，群主每次使用都必须先使用 SDK 接口执行进群操作。
>- 如果指定群组类型为非直播群：
...
| 字段 | 类型 | 属性 |说明|
|---------|---------|---------|---------|
| Owner_Account | String | 选填 |群主 ID（需是 [已导入](https://cloud.tencent.com/document/product/269/1608) 的帐号）。填写后自动添加到群成员中；如果不填，群没有群主；AVChatroom 群类型，成员使用的时候，必须每次调用进群 (joinGroup) 的操作。 |

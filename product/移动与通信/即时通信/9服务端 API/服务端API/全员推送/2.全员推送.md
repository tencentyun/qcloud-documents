...
- 支持消息离线存储，不支持漫游。
- 由于全员推送接口下发的帐号量巨大，下发完全部帐号需要一定时间（一般在一分钟内）。
...
| 90004 |JSON 格式请求包体中 MsgSeq 不是 Number 类型，或者其值范围不在0 - 4294967296|
| 90005 |JSON 格式请求包体中 MsgRandom 不是 Number 类型，或者其值范围不在0 - 4294967296|

...
|字段|       类型|       属性|       说明|
|----|-----|------|-----|
|From_Account|       String|       必填|  指定要拉取分组的用户的UserID |
|LastSequence|Integer  |       选填| 上次拉取分组的Sequence, 如果 Sequence 字段的值与后台一致，后台不会返回分组数据, 只有GroupName为空时有效, |
|NeedFriend       |String       |选填| 是否需要拉取分组下的User列表, Need_Friend_Type_Yes: 需要拉取, 不填时默认不拉取, 只有GroupName为空时有效. |
...
|To_Account|       Array|       该分组下的好友的UserID|

...
| ErrorList | Array | 查询状态失败的帐号列表，在此列表中的目标帐号，状态查询失败。若状态全部查询成功，则 ErrorList 为空。因此判断回包中是否含 ErrorList ，来判断状态是否全部查询成功。不存在的目标帐号也会在 ErrorList 中返回，这种情况下该目标帐号的错误码为70107。   |

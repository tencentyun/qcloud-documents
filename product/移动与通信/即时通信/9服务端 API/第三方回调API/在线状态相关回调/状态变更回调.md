...
- 使用终端 SDK 增强版5.6.1200之后的版本，或使用 web SDK x.x.x之后的版本时，因多终端登录或多实例登录被踢之后，只会触发一个 Login + Register 回调。而使用其他 SDK 版本的，当多终端登录或多实例登录被踢之后，会同时触发 Login + Register 回调和 Logout + Unregister 回调。

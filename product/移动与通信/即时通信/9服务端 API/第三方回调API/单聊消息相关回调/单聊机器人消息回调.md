...
-  App 用户通过客户端发送单聊消息，且接收方为机器人账户
-  App 管理员通过 REST API（sendmsg 接口）发送单聊消息，且接收方为机器人账户。

## 回调发生时机

即时通信 IM 后台收到用户发送的单聊消息并将该消息下发给目标用户之后，且该用户判定为机器人账户。
...
| CallbackCommand | 固定为Bot.OnC2CMessage                                       |
...
```
{
    "CallbackCommand": "Bot.OnC2CMessage", // 回调命令
    ...
    "OnlineOnlyFlag":1,//是否仅发送给在线用户标识。1代表仅发送给在线用户，否则为0；
...
| ErrorInfo       | String  | 该条消息下发失败的错误信息，若消息发送成功，则为"send msg succeed"https://cloud.tencent.com/document/product/269/2720) |

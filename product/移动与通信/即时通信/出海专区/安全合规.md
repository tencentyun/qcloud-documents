...
## 简介
应用在出海前须遵守当地数据保护法律法规，确保数据合规性，避免法律风险。 腾讯云即时通信 IM 提供了境外数据中心供选择，数据不回国。即时通信 IM 通过多项安全合规认证，保障您的数据安全。

## 境外数据中心
即时通信 IM 提供了中国、南亚（印度）、东南亚（新加坡）、东北亚（韩国首尔）、欧洲（德国法兰克福）以及北美（美国硅谷）数据存储中心供选择。您的业务数据存储在创建应用时选择的数据中心，每个数据中心均支持全球接入。

如果您的应用在境外上线且用户主要在境外，您可以根据消息传输需求及合规要求，选择适合您业务的境外数据中心。境外数据中心资费请参见 [境外资费](https://cloud.tencent.com/document/product/269/81907)。


## 数据存储
您在使用即时通信 IM 境外数据中心服务时，您的业务数据仅存储您所选的境外数据中心，满足更多国家和地区的数据合规要求。

包括不限于以下数据：
- 用户资料（如昵称、头像、自定义资料字段等）
- 好友关系链
- 群组相关（群资料、群成员等）
...
>?
>- 为提供完整的即时通信 IM 服务，您应用的 SDKAppID 及账单相关数据将存储在中国大陆。

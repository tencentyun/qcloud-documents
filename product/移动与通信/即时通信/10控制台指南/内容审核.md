...
>!开通服务后，默认为您开启图文审核场景，并配置默认审核策略（默认识别并拦截包含较高风险的敏感政治事件/人物内容，如果需要增加对中低风险敏感政治事件/人物、色情、辱骂、违禁等不安全内容的拦截，请参见 [审核策略配置](#step2_3_2) 进行操作），您可通过单击对应模块的**编辑**，分别更改 [**审核场景**](#step2_3_1) 和 [**审核策略**](#step2_3_2)。
...
>!
>- 开通服务后，默认为您开启图文审核场景，并配置默认审核策略（默认识别并拦截包含较高风险的敏感政治事件/人物内容，如果需要增加对中低风险敏感政治事件/人物、色情、辱骂、违禁等不安全内容的拦截，请参见 [审核策略配置](#step2_3_2) 进行操作），您可通过点击对应模块的编辑按钮，分别更改审核场景和审核策略。
...
[](id:step2_3_2)
#### 2. 审核策略配置
每个审核场景及内容类型均已配置默认审核策略，默认审核策略包括对敏感事件、敏感人物、色情、辱骂、违禁等不安全内容的识别。
...
内容回调结果将使用 **后回调** 发送至您在基础配置中配置的 URL 地址，如果您在开启返回审核结果开关前未配置基础回调 URL，请在打开开关时弹出的弹窗中填写基础回调 URL，详情请参见 [基础回调配置](https://cloud.tencent.com/document/product/269/32431#.E5.9F.BA.E7.A1.80.E5.9B.9E.E8.B0.83.E9.85.8D.E7.BD.AE)。

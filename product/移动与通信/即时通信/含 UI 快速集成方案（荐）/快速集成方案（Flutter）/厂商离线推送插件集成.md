...
如果您的应用已经自行完成厂商离线推送，仅需查看本教程 [第一步]([](id:#step_1)) 和 [第五步]([](id:#step_5)，在控制台内录入厂商信息，并在应用登录后，上报证书 ID 即可。
...
```
4. 部分厂商（如OPPO）默认不提供推送权限，需要开发者手动申请。调用`requireNotificationPermission`方法即可。
...
#### 处理单击回调 
1. 此时，填上 [步骤3](#step_3) 初始化时，为 pushClickAction 埋的坑。
...
#### Flutter 通话插件接入
1. 如果你使用到我们的 [tim_ui_kit_calling_plugin](https://pub.dev/packages/tim_ui_kit_calling_plugin) 插件，请将其升级至0.2.0版本以上，即可使用离线推送能力。
...
2. 并在左上角新增`Push Notification`的Capability。

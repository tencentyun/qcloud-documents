...
>? 群组头像默认展示九宫格头像，九宫格失效后展示 `defaultGroupAvatarImage`。如果您不想使用九宫格头像，可以禁用九宫格头像。
...
```objectivec
[TUIChatConfig defaultConfig].backgroudImage = [UIImage imageNamed:@"your chat backgroung image"];
```
设置聊天窗口背景图片效果图如下所示：
<table style="text-align:center;vertical-align:middle;width: 600px">
  <tr>

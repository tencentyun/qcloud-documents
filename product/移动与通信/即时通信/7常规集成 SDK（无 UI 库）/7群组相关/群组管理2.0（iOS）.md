...
|---------|---------|---------|---------|---------|
| 适用场景 | 类似普通微信群，创建后仅支持已在群内的好友邀请加群，且无需被邀请方同意或群主审批。 | 类似 QQ 群，创建后可以申请加入，也可以由群成员邀请入群，但是需要被邀请人同意及群主或管理员审批。 | 创建后可以随意进出，且支持查看入群前消息；适合用于音视频会议场景、在线教育场景等与实时音视频产品结合的场景。 | 创建后可以随意进出，没有群成员数量上限，但不支持历史消息存储；适合与直播产品结合，用于弹幕聊天场景。（注意：直播群有创建数量上限，请参考您的[套餐包配置](https://cloud.tencent.com/document/product/269/11673)） |受所购套餐包版本所限，详情参见[套餐价格](https://cloud.tencent.com/document/product/269/11673)。） |

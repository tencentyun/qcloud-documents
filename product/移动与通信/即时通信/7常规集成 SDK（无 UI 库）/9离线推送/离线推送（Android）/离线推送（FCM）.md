...
1. 请参考《在 Android 上设置 Firebase 云消息传递客户端应用》（地址为https://firebase.google.com/docs/cloud-messaging/android/client）设置 Firebase，集成 FCM SDK，启动应用后获取设备注册令牌 **token**。
2. 请参考《向后台应用发送测试消息》（地址为https://firebase.google.com/docs/cloud-messaging/android/first-message?authuser=0）测试通知消息，确保已成功集成 FCM。
3. 登录 Firebase 控制台（地址为https://console.firebase.google.com），单击您的应用卡片，进入应用配置页面。

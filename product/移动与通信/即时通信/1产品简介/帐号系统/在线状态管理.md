...
>?注意：后台运行状态（PushOnline）只有手机端（Android/iOS）会存在，PC和web不存在该种状态。
...
- 用户使用完 App ，把 App 切后台后进程被手机操作系统 kill 掉，或者用户主动 kill 掉 App 进程。<br>（注意：如果 App 在手机操作系统的保活白名单中，用户把 App 切后台，进程并不会被手机操作系统 kill 掉。此时状态仍然为前台运行状态（Online）。前台运行状态（Online）和后台运行状态（PushOnline）判断的标准之一是 App 进程是否被 kill ，即客户端和即时通信 IM 服务端的 TCP 长连接是否断开。）
- 用户主动关闭客户端网络（如打开手机飞行模式），或者客户端网络完全不可用（如进入完全没有网络信号的隧道）。<br>（注意：在这种特殊情况下，客户端连 TCP 协议的 FIN 包或 RST 包都无法发出，即时通信 IM 服务端需要等待 400 秒后发现心跳包超时，状态才会变成后台运行状态（PushOnline）。）
...
## 状态变更感知的实时性
请参考 [状态变更回调] (https://cloud.tencent.com/document/product/269/2570#.E5.9B.9E.E8.B0.83.E7.9A.84.E5.AE.9E.E6.97.B6.E6.80.A7)。

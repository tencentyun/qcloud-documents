...
### 同时在线
即时通信 IM 支持在 [控制台](https://console.cloud.tencent.com/im) 修改同时在线策略，通过配置可以做到 PC 端和手机端同时在线，或者 PC、iOS 和 Android 都可以同时在线，开启了同时在线登录不同终端不会互踢（但是两个相同终端，如两个 iOS 端登录仍然会互踢）。

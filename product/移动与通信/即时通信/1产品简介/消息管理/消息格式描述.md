...
| AndroidInfo.HuaWeiChannelID | String | 选填 | 华为手机 EMUI 10.0 及以上的通知渠道字段。如果该字段不为空，会覆盖控制台配置的ChannelID值。|
| AndroidInfo.XiaoMiChannelID | String | 选填 | 小米手机 MIUI 10 及以上的通知类别（Channel）适配字段。如果该字段不为空，会覆盖控制台配置的ChannelID值。 |
| AndroidInfo.OPPOChannelID | String | 选填 | OPPO 手机 Android 8.0 及以上的 NotificationChannel 通知适配字段。如果该字段不为空，会覆盖控制台配置的ChannelID值。|

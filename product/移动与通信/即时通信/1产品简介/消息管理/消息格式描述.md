...
若发送自定义消息的同时，设置了 OfflinePushInfo.Desc 字段，此字段会被覆盖。请优先填 OfflinePushInfo.Desc 字段。
...
注意，若发送的消息只有一个 TIMCustomElem 自定义消息元素，该 Desc 字段会覆盖 TIMCustomElem 中的desc字段。如果这两个 Desc 字段均不填，则将收不到该自定义消息的离线推送。|

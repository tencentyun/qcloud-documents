...
即时通信 IM 离线消息目前使用的厂商通道依赖由[移动推送 TPNS](https://cloud.tencent.com/product/tpns) 统一提供和维护，移动推送TPNS提供Push推送一站式解决方案，支持国内主流厂商通道（华为、小米、OPPO、vivo、魅族）、FCM通道、APNs通道一站式托管和集成，同时提供丰富的推送数据分析与精准推送能力。为了避免重复集成推送能力，您的运营消息和IM离线消息均可使用移动推送服务下发。
## iOS APNs 推送
### 推送格式说明

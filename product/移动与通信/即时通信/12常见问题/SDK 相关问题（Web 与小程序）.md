### 1. 用 Taro 开发项目，集成 WebIM v2.x，为什么一直收不到 TIM.EVENT.SDK_READY 事件？
...
### 2. 为什么调用 off 接口取消监听事件后，仍然能监听到 SDK 派发的事件？
...
### 9. WebIM v2.x 和 WebIM v1.7.x 消息是互通的吗？

互通。如果条件允许，建议接入使用最新的 WebIM，获得更好的体验和维护。

### 10. WebIM v2.x 如何兼容 REST API 或 旧版 IM 发送的组合消息？
...
### 11. 为什么 WebIM v2.x 调用 sendMessage发消息成功后，未收到 TIM.EVENT.MESSAGE_RECEIVED 事件？
...
### 12. 通过 getMessageList 拉取到的消息列表和通过监听 TIM.EVENT.MESSAGE_RECEIVED 事件收到的消息合并后，发现有时会消息乱序，为什么？
...
### 13. 调用 createGroup 接口创建音视频聊天室后，收不到消息？
...
### 14. 音视频聊天室怎么没有未读消息计数？为什么拉取不到历史消息？

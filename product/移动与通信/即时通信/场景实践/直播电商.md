直播带货组件是对直播带货场景中使用即时通信 IM 能力的二次封装，在封装基本的收发消息能力的同时，针对直播带货场景封装了点赞、送礼、商品推送、优惠券领取等相关的能力。相关 SDK 下载，请参见 [SDK 下载](https://cloud.tencent.com/document/product/269/36887#TLS)。
...
| userSig  | 即时通信IM用户签名，UserSig为登录腾讯云即时通信 IM SDK 的必要参数，可在业务后台生成后返回给前端使用，生成文档与代码请参见 [生成 UserSig](https://cloud.tencent.com/document/product/269/32688)。 |
| roomID   | 直播间群聊房间 ID，此 ID 为即时通信IM所创建的实时音视频（AVCHATROOM）房间 ID，该房间无加入人数限制可通过 [即时通信 IM 群组管理](https://console.cloud.tencent.com/im-detail/qun-manage) 创建，或通过 [REST API](https://cloud.tencent.com/document/product/269/1519) 创建。 |
...
| [TLS.EVENT.KICKED](#tls.event.kicked)                 | 账号其他地方登录时触发。                                     |

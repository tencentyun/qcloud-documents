...
问：自己发消息时，消息发送状态，Message.nick 与 Message.avatar 字段都为空，该怎么处理才能在界面上正常展示昵称和头像？

答：可通过 getUserInfo 接口，获取自己的用户信息中的 nick 和 avatar 字段作为消息发送发的 nick 和 avatar 字段。

问：TRTC 和直播怎么选择？

答：TRTC 直播延时小，与主播互动实现更加方便，但价格更加贵一些。直播延迟稍大但价格便宜。

...
>?从`kTIMMsgGetMsgListParamLastMsg`指定的消息开始获取本地消息列表，kTIMMsgGetMsgListParamCount  为要获取的消息数目。kTIMMsgGetMsgListParamLastMsg 可以不指定，不指定时表示以会话最新的消息为LastMsg。若指定`kTIMMsgGetMsgListParamIsRamble`为 true 则本地消息获取不够指定数目时，会去获取云端漫游消息。`kTIMMsgGetMsgListParamIsForward`为 true 时表示获取比`kTIMMsgGetMsgListParamLastMsg`新的消息，为 false 时表示获取比`kTIMMsgGetMsgListParamLastMsg`老的消息。

...
| Reason | String | 	App 上下线触发的原因：<br>上线的原因有 Register：App TCP 连接建立；<br>下线的原因有 Unregister：App 用户注销帐号导致 TCP 断开； LinkClose：即时通信 IM 检测到 App TCP 连接断开；TimeOut：即时通信 IM 检测到 App 心跳包超时，认为 TCP 已断开（客户端杀后台或 Crash） |

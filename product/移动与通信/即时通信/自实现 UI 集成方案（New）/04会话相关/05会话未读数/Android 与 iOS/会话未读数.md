...
::: iOS & Mac
```objectivec
[[V2TIMManager sharedInstance] cleanConversationUnreadMessageCount:@"c2c_userID" // "userID" 为待清理单聊会话 ID 
...
### 群聊
您可以通过传入带有 "group_" 前缀的 conversationID 来清理指定群聊会话未读消息数。
...
::: iOS & Mac
```objectivec
[[V2TIMManager sharedInstance] cleanConversationUnreadMessageCount:@"group_groupID" // "groupID" 为待清理的群聊会话 ID
                                                    cleanTimestamp:0
                                                     cleanSequence:123
                                                              succ:^{
    // 清理成功
...
### 所有会话
您可以通过传入空字符串 "" 作为会话 ID，表示清空所有会话的未读消息数。
...
V2TIMManager::GetInstance()->GetMessageManager()->CleanConversationUnreadMessageCount(conversationID, 0, 0, callback);
```
:::
</dx-tabs>

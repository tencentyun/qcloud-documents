...
### 获取未读总数
您可以调用 `ConvGetTotalUnreadMessageCount`([c#](https://comm.qq.com/im/doc/unity/api/ConvApi/ConvGetTotalUnreadMessageCount.html)) 获取所有会话的未读消息总数，获取成功后可以使用其更新 UI。
...
您可以调用 `SetConvTotalUnreadMessageCountChangedCallback`([c#](https://comm.qq.com/im/doc/unity/api/SDKRegisteringCallback/SetConvTotalUnreadMessageCountChangedCallback.html)) 添加会话未读总数变更监听器。添加监听器后，能接收到未读总数变更通知。
...
具体的操作步骤如下文所示。
### 单聊
您可以调用 `MsgReportReaded`([c#](https://comm.qq.com/im/doc/unity/api/MessageApi/MsgReportReaded.html)) 清空指定单聊会话的未读消息数。
...
### 群聊
您可以调用 `MsgSendMessageReadReceipts`([c#](https://comm.qq.com/im/doc/unity/api/MessageApi/MsgSendMessageReadReceipts.html)) 清空指定群聊会话的未读消息数。
...
### 所有会话
您可以调用 `MsgMarkAllMessageAsRead`([c#](https://comm.qq.com/im/doc/unity/api/MessageApi/MsgMarkAllMessageAsRead.html)) 清空所有会话的未读消息数。

...
您可以调用 `pinConversation`([TS](https://comm.qq.com/im/doc/RN/zh/Api/V2TIMConversationManager/pinConversation.html)) 接口，设置是否置顶会话。
...
如果您事先调用了 `addConversationListener`([TS](https://comm.qq.com/im/doc/RN/zh/Api/V2TIMConversationManager/addConversationListener.html)) 添加会话监听器，就可以在 `onConversationChanged` 中获取到 `V2TimConversation` 对象的 `isPinned` 字段值。根据这个字段可以判断会话的置顶状态是否变化。
示例代码如下：

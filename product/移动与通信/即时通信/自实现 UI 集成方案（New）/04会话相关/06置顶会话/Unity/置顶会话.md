...
## 置顶会话
您可以调用 `ConvPinConversation`([c#](https://comm.qq.com/im/doc/unity/api/ConvApi/ConvPinConversation.html)) 接口，设置是否置顶会话。
...
## 会话置顶变更通知
如果您事先调用了 `SetConvEventCallback`([c#](https://comm.qq.com/im/doc/unity/api/SDKRegisteringCallback/SetConvEventCallback.html)) 添加会话监听器，就可以在 `ConvEventCallback` 中获取到 `ConvInfo` 对象的 `conv_is_pinned` 字段值。根据这个字段可以判断会话的置顶状态是否变化。
示例代码如下：

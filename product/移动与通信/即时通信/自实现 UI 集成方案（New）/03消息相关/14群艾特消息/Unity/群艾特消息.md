...
2. 发送方调用 `MsgSendMessage` ([c#](https://comm.qq.com/im/doc/unity/api/MessageApi/MsgSendMessage.html)) 接口创建一条 @ 文本消息，创建消息对象 `Message`，并在其中指定需要 @ 的成员，然后将刚才创建的 @ 消息对象发送出去。
...
1. 在加载和更新会话处，需要调用 `ConvInfo` 的 `conv_group_at_info_array` ([c#](https://comm.qq.com/im/doc/unity/types/ConvAttributes/ConvInfo.html#convgroupatinfoarray)) 接口获取会话的 @ 数据列表。
2. 通过列表中 `GroupAtInfo` 对象的 `conv_group_at_info_at_type` ([c#](https://comm.qq.com/im/doc/unity/types/GroupAttributes/GroupAtInfo.html)) 接口获取 @ 数据类型，并更新到当前会话的 @ 信息。

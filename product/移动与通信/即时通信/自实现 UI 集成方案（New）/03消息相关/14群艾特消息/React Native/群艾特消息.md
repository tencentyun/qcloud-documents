...
2. 发送方调用 `createTextAtMessage` ([TS](https://comm.qq.com/im/doc/RN/zh/Api/V2TIMMessageManager/createTextAtMessage.html)) 接口创建一条 @ 文本消息，拿到消息对象 `V2TIMMessage`，并在其中指定需要 @ 的成员。
3. 发送方调用 `sendMessage` ([TS](https://comm.qq.com/im/doc/RN/zh/Api/V2TIMMessageManager/sendMessage.html)) 接口将刚才创建的 @ 消息对象发送出去。
...
1. 在加载和更新会话处，需要调用 `V2TimConversation` 的 `groupAtInfolist` ([TS](https://comm.qq.com/im/doc/RN/zh/Interface/Message/V2TimConversation.html#groupatinfolist)) 接口获取会话的 @ 数据列表。
2. 通过列表中 `V2TimGroupAtInfo` 对象的 `atType` ([TS](https://comm.qq.com/im/doc/RN/zh/Interface/Group/V2TimGroupAtInfo.html#attype)) 接口获取 @ 数据类型，并更新到当前会话的 @ 信息。

## 功能描述
* 拉取历史消息的接口为 `MsgGetMsgList` ([c#](https://comm.qq.com/im/doc/unity/api/MessageApi/MsgGetMsgList.html))
* 支持单聊、群聊历史消息的拉取。
...
您可以调用接口 `MsgGetMsgList` ([c#](https://comm.qq.com/im/doc/unity/api/MessageApi/MsgGetMsgList.html)) 获取单聊历史消息。
...
您可以调用接口 `MsgGetMsgList` ([c#](https://comm.qq.com/im/doc/unity/api/MessageApi/MsgGetMsgList.html)) 获取群聊历史消息。
在网络正常的情况下会拉取最新的云端数据。如果网络出现异常，SDK 会返回本地存储的历史消息。
...
如果您需要获取最新的群成员信息，您可以使用 `GroupGetMemberInfoList`([c#](https://comm.qq.com/im/doc/unity/api/GroupApi/GroupGetMemberInfoList.html))。

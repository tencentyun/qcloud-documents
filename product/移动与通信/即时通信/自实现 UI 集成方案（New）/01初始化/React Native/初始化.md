...
我们建议您在调用 initSDK 时传入 `V2TimSDKListener`([TS](https://comm.qq.com/im/doc/RN/zh/Interface/Listener/V2TimSDKListener.html)) 接口添加 SDK 事件监听器，在对应回调中做一些逻辑处理。
...
操作完上述步骤后，您可以调用 `initSDK`([TS](https://comm.qq.com/im/doc/RN/zh/Api/V2TIMManager/initSDK.html)) 进行 SDK 初始化。
...
反初始化需要操作一个步骤：调用反初始化接口 `unInitSDK`([TS](https://comm.qq.com/im/doc/RN/zh/Api/V2TIMManager/unInitSDK.html))

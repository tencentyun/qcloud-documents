...
初始化 SDK 前，您需要初始化一个 [`LogLevelEnum`](https://comm.qq.com/im/doc/RN/zh/Enum/LogLevelEnum.html) 对象。该对象用于对 SDK 进行日志级别设置。
...
我们建议您在调用 initSDK 时传入 `V2TimSDKListener`([点击查看详情](https://comm.qq.com/im/doc/RN/zh/Interface/Listener/V2TimSDKListener.html)) 接口添加 SDK 事件监听器，在对应回调中做一些逻辑处理。
...
操作完上述步骤后，您可以调用 `initSDK`([点击查看详情](https://comm.qq.com/im/doc/RN/zh/Api/V2TIMManager/initSDK.html)) 进行 SDK 初始化。
...
反初始化需要操作一个步骤：调用反初始化接口 `unInitSDK`([点击查看详情](https://comm.qq.com/im/doc/RN/zh/Api/V2TIMManager/unInitSDK.html))

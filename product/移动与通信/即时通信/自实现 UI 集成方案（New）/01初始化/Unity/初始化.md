...
### 调用初始化接口
操作完上述步骤后，您可以调用 `Init`([c#](https://comm.qq.com/im/doc/unity/zh/api/IMSDKInit/Init.html)) 进行 SDK 初始化。
...
反初始化需要操作一个步骤：调用反初始化接口 `unInit`([c#](https://comm.qq.com/im/doc/unity/zh/api/IMSDKInit/Uninit.html))

...
群主或管理员可以调用 `GroupModifyMemberInfo` ([c#](https://comm.qq.com/im/sdk/unity_plus/_site/api/com.tencent.imsdk.unity.TencentIMSDK.html#com_tencent_imsdk_unity_TencentIMSDK_GroupModifyMemberInfo_com_tencent_imsdk_unity_types_GroupModifyMemberInfoParam_com_tencent_imsdk_unity_callback_NullValueCallback_)) 接口修改群成员的群名片（`group_modify_member_info_name_card`）、自定义字段（`group_modify_member_info_custom_info`）等与群成员相关的资料。

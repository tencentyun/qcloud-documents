...
1. 创建 `V2TIMGroupInfo` ([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Class/Group/V2TimGroupInfo.html?h=V2TIMGroupInfo)) 对象，其中 `groupType` 设置为 `Community`，`isSupportTopic` 设置为 `true`/`YES`。
2. 调用 `createGroup` ([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Api/V2TIMGroupManager/createGroup.html?h=createGroup))  接口创建群组。
...
### 获取加入的社群列表
调用 `getJoinedCommunityList`([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Api/V2TIMGroupManager/getJoinedCommunityList.html?h=getJoinedCommunityList)) 获取已经加入的支持话题的社群列表。
...
创建话题分两步：
1. 创建 `V2TIMTopicInfo` ([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Class/Topic/V2TimTopicInfo.html)) 对象。
2. 调用 `createTopicInCommunity` ([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Api/V2TIMGroupManager/createTopicInCommunity.html)) 接口创建话题。
...
### 删除话题
调用 `deleteTopicFromCommunity`([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Api/V2TIMGroupManager/deleteTopicFromCommunity.html)) 接口删除话题。
...
1. 创建 `V2TIMTopicInfo` ([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Class/Topic/V2TimTopicInfo.html))  对象，并设置需要修改的字段。
2. 调用  `setTopicInfo` ([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Api/V2TIMGroupManager/setTopicInfo.html))  接口修改话题信息。
...
调用 `getTopicInfoList` ([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Api/V2TIMGroupManager/getTopicInfoList.html))  接口获取话题列表。
...
### 监听话题回调
在 `V2TIMGroupListener` ([dart](https://comm.qq.com/im/doc/flutter/zh/SDKAPI/Class/Listener/V2TimGroupListener.html)) 中增加了话题相关的回调方法，`onTopicCreated`，`onTopicDeleted`，`onTopicInfoChanged`，用来监听话题事件。 

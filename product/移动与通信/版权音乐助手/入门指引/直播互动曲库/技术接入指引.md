...
## 步骤4：API 联调
### 搜索选曲
1. 调用 [搜索直播互动曲库歌曲](https://cloud.tencent.com/document/product/1155/56401) 接口，根据搜索条件返回匹配的歌曲列表。
2. 调用 [查询直播互动曲目详情](https://cloud.tencent.com/document/product/1155/56402) 接口，查询歌曲的详细信息。

### 推荐歌单
1. 调用 [获取直播互动曲库推荐歌单列表](https://cloud.tencent.com/document/product/1155/60295) 接口，获取推荐歌单ID及歌单基础信息。
2. 调用 [获取直播互动曲库推荐歌单详情](https://cloud.tencent.com/document/product/1155/60296) 接口，获取歌曲ID及歌曲基础信息。
3. 调用 [查询直播互动曲目详情](https://cloud.tencent.com/document/product/1155/56402) 接口，获取 SDK 播放所需 PlayToken。
>?
...
### 集成 SDK
- **iOS&Android：**
  1. 集成版权曲库SDK(拷贝TXCopyrightedMedia.framework)到项目工程中并集成。
  2. 如果使用 pod 导入，则在 podfile 里面添加：
```
pod 'TXCopyrightedMedia'
```
>? 如果搜索不到，可能是没有更新到源。建议使用 `https://cdn.cocoapods.org/` 做源。在 `Podfile` 最上面添加即可：
```
...
- **预加载 Music 数据**
	- **接口说明：**预加载 Music 数据，每次播放/重播前都需要调用该接口。
	- **示例代码：**
```Java
...
- **生成 Music URI**
	- **接口说明：**生成 MusicUri，App 客户端在 preloadMusic 成功之后调用，**原唱&伴奏**组合传给 TRTC 进行播放。
	- **示例代码：**
```Java
...
<td>musicUri</td>
<td>String</td>
<td><ul><li>原唱&伴奏：传给 TRTC 播放的 URL，格式 <code>CopyRightMusic://audiotype=****&amp;musicid=****</code><li>歌词：返回歌词的本地路径</ul></td>
</tr>
</tbody></table>
...
- **销毁 ITXCopyrightedMedia 实例**
	- **接口说明：**销毁 ITXCopyrightedMedia 实例。
	- **接口代码：**
```Java

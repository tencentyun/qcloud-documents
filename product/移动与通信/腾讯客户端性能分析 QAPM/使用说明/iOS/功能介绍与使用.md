...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    /********************************  配置卡顿功能  ****************/
#ifdef DEBUG
	/// Blue功能采样率 客户端设置采样率（范围0 - 1),百分之一为0.01。如果不设置则为后台控制采样率（后台默认0.01）。例如百分之一，每次启动App初始化SDK会随机进行开启功能。用户自己设置后则后台无法控制采样率。
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    /********************************  配置VC泄漏功能 ****************/
#ifdef DEBUG
    /// VC泄露检测Yellow功能采样率 客户端设置采样率（范围0 - 1),百分之一为0.01，如果不设置则为后台控制采样率（后台默认0.01）。例如百分之一，每次启动App初始化SDK会随机进行开启功能。用户自己设置后则后台无法控制采样率。
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions { 
	...
	/******************************** 配置Sigkill ****************/ 
#ifdef DEBUG
    /// Sigkill采样率 客户端设置采样率（范围0 - 1),百分之一为0.01，如果不设置则为后台控制采样率（后台默认0.01）。例如百分之一，每次启动App初始化SDK会随机进行开启功能。用户自己设置后则后台无法控制采样率。
...
### 功能配置
```objectivec
    /// 内存触顶率功能采样率 客户端设置采样率（范围0 - 1),百分之一为0.01，如果不设置则为后台控制采样率（后台默认0.01）。例如百分之一，每次启动App初始化SDK会随机进行开启功能。用户自己设置后则后台无法控制采样率。
#ifdef DEBUG
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
#ifdef DEBUG
	/// 大块内存分配功能采样率 客户端设置采样率（范围0 - 1),百分之一为0.01，如果不设置则为后台控制采样率（后台默认0.01）。例如百分之一，每次启动App初始化SDK会随机进行开启功能。用户自己设置后则后台无法控制采样率。
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
#ifdef DEBUG
    /// 资源采集功能 客户端设置采样率（范围0 - 1),百分之一为0.01，如果不设置则为后台控制采样率（后台默认0.01）。例如百分之一，每次启动App初始化SDK会随机进行开启功能。用户自己设置后则后台无法控制采样率。

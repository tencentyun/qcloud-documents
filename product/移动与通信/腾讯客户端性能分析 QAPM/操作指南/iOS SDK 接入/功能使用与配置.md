...
| 函数 | 参数 | 描述 | 返回值 |
|---------|---------|---------|---------|
| beginTrackingWithStage | (NSString *)stage | 开始记录掉帧，建议滑动开始时调用<p>@param stage 用来标识当前页面(一般为当前VC类名） | void |
| stopTrackingWithStage | (NSString *)stage | 结束记录掉帧，建议滑动开始时调用<p>@param stage 用来标识当前页面(一般为当前VC类名） | void |

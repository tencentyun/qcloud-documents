...
3、"sig"字段根据公式sha256(appkey=$appkey&random=$random&msg=$msg&time=$time&mobile=$mobile)生成  
c++示例代码如下：
```
...
string strMsg = "你的验证码是1234"; //msg字段的内容
string strTime = "1457336869"; //unix时间戳
string sig = sha256(appkey=$strAppKey&random=$strRand&msg=$strMsg&time=$strTime&mobile=$strMobile);
```

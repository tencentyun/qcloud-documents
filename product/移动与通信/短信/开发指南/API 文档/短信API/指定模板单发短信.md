...
其中，sdkappid 请填写您在腾讯云上申请到的，random 请填成随机数。
...
| sign   | 否   | string | 短信签名，此处应填写审核通过的签名内容，非签名 ID，如果使用默认签名，该字段可缺省<br>签名信息查看方法：登录 [短信控制台](https://console.cloud.tencent.com/sms/smslist) ，进入目标应用的配置页，选择【国内短信】（或【国际短信】）>【短信内容配置】> 【短信签名】即可查看对应的签名    |
...
>3. "sig" 字段根据公式 `sha256（appkey=$appkey&random=$random&time=$time&mobile=$mobile）`生成，其伪代码如下：
 ```c++

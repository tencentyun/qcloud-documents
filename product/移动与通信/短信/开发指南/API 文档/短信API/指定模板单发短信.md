...
| sign   | 否   | string | 短信签名，此处应填写审核通过的签名内容，而非签名 ID，使用 UTF-8 编码。如果使用默认签名，该字段可缺省<br>签名信息可登录 [短信控制台](https://console.cloud.tencent.com/sms/smslist) 查看    |
| tel    | 是   | object | 国际电话号码，格式依据 [e.164](https://en.wikipedia.org/wiki/E.164) 标准为：`+[国家（或地区）码][手机号]` ，示例如：`+8613711112222`， 其中前面有一个`+`号 ，`86`为国家码，`13711112222`为手机号     |

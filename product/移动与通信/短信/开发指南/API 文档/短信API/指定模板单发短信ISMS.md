...
如果您有多个短信签名，请将需要的短信签名填入`sign`字段，例如您有“腾讯科技”和“腾讯云”两个签名，但想以“腾讯云”签名发送短信，则`sign`字段可赋值为：“腾讯云” 
2. `sig` 字段根据公式 `sha256（appkey=$appkey&random=$random&time=$time&tel=$tel）`生成，其伪代码如下：
```json

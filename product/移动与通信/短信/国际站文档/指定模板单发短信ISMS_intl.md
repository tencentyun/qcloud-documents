...
1. The `tpl_id` field should be entered with the approved template ID.
If the content corresponding to the template ID is "`Your {1} is {2} (valid for {3} minutes). If you are not using our service, ignore the message.`", the message sent is "`[Tencent Cloud] Your verification code is 1234 (valid for 4 minutes). If you are not using our service, ignore the message.`".
If you have more than one SMS signatures, enter the required SMS signature in the `sign` field. For example, if you have two signatures, `Tencent Technology` and `Tencent Cloud`, and you want to send an SMS message with `Tencent Cloud`, the "sign" field can be `Tencent Cloud`.

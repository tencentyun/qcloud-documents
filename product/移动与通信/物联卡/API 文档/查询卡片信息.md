...
>?请求采用 HTTPS 传输，使用 POST 方法。请求包体采用 x-www-form-urlencoded，应答包体采用 JSON 格式，SDKAppID 由腾讯物联卡平台分配，random 为随机整数，不要添加零前缀。如您需在内测阶段调用接口进行测试，请向腾讯云物联卡技术支持（物联卡小助手 QQ：3513545165）申请 SDKAppID 和 Appey。

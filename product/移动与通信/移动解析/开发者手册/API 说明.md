...
>! 如果使用 AES/DES 加密，`cloud.tencent.com` 应为加密后的数据。
...
- **返回格式：** 空
- **格式说明：** 没有记录，则返回空字符串。
>! 如果使用 AES/DES 加密，`cloud.tencent.com` 和 ip 参数中的 `1.2.3.4` 应为加密后的数据。空字符串也是加密后的内容格式。
...
>!如果使用 AES/DES 加密，`cloud.tencent.com` 等域名和 ip 参数中的 `1.2.3.4` 应当为加密后的数据。

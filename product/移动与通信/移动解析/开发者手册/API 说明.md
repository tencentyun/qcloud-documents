...
>- 原移动解析 HTTPDNS 服务地址 `119.29.29.29` 目前可正常使用，但建议您将正式业务迁移至 `119.29.29.99/98`，以获得最佳的接入效果。
...
>- 如果使用 AES/DES 加密方式和 HTTPS 协议进行请求，请求地址为 `119.29.29.99` 并且必须必须传入授权 ID，授权 ID 不需要进行加密。

...
## 背景信息
由于移动解析 HTTPDNS 产品业务扩容及架构升级，2021年04月22日起已对外启用接入 IP `119.29.29.99/98`，原 IP `119.29.29.29` 将于2022年01月01日零时起停止移动解析 HTTPDNS 服务。

## 风险提醒
如果您的正式业务仍在使用 IP `119.29.29.29`，在服务停止后将不能通过接口获得域名解析查询结果。为避免影响业务，请尽快完成接入 IP 切换。
...
2. 开通成功后，您将获得相关的授权信息进行调用服务，此过程不会影响 `119.29.29.29` 的使用。过程中建议您逐渐将请求从 `119.29.29.29` 切换至 `119.29.29.99/98`。详情请查看 [接入移动解析 HTTPDNS](https://cloud.tencent.com/document/product/379/3522)。

...
智营解析 Android SDK 的获取方式 [请点此获取](https://github.com/tencentyun/httpdns-android-sdk)。
...
HttpDNS SDK 监听网络切换广播，在发生网络切换时，清理 HttpDNS 解析结果的缓存。
...
在该情况下，您需要将 HttpDNS 请求使用的 IP 配置到域名白名单中：
...
Retrofit 实际上是一个基于 OkHttp，对接口做了一层封装桥接的 lib。因此只需要仿 OkHttp 的接入方式，定制 Retrofit 中的 OkHttpClient，即可方便地接入 HttpDNS。
示例如下：

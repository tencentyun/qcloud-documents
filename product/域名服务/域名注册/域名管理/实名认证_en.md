According to the identity verification requirement announced by MIIT in 2017, all stock of domain names and newly registered domain names are required to complete identity verification. For now, involved domain names include: .com/.net/.cn/.xyz/.club/.wang. If identity verification is not completed within a specified period of time, the registry will suspend the resolution for your domain name (Serverhold) which means the domain name cannot be accessed. It will be resumed once identity verification is finished. Thus, make sure to complete the verification when you register a new domain name.

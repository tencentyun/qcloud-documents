...
2. 在左侧导航栏中，单击【证书管理】，进入证书列表页面。
3. 在证书列表页面，单击证书管理ID。如下图所示：
...
2.  在 “云解析列表” 中，单击【域名解析列表】，选择【添加解析】。如下图所示：
 ![](https://main.qcloudimg.com/raw/d48a7876869b92fcb35e14e7ca94703d.png)
3. 在弹窗内输入您要解析域名的主域名`domain.com`，并点【确定】。
...
1. **记录类型选择为 TXT**。
2. 主机记录根据证书详情填入，如：`_dnsauth`。
3. 线路类型选择默认。
4. 记录值为系统提供的文本内容，此处为`201712270743...t5bfctnq`，
>!记录值须完整填写。
5. TTL选择默认值 600 秒即可。

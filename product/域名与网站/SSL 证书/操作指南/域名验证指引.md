...
5. 在 “验证域名” 页面，您可以单击【立即验证】检查配置是否成功。
>?
...
2. 在 “我的资料” 管理页面，单击需要验证域名信息的公司名称，即可查看已申请的管理人信息。如下图所示：
3. 单击您需要验证域名信息的管理人姓名，进入您的 “审核信息” 管理页面。如下图所示：
...
5. 在 “验证域名” 页面中，请根据页面提示，添加解析记录。如下图所示：
...
7. 在 “验证域名” 页面中，您可以单击【立即验证】检查添加 CNAME 解析解析记录是否成功。
...
4. 单击【域名信息】页签，选择您需要进行验证的域名，并单击【查看验证】。如下图所示：
![](https://main.qcloudimg.com/raw/12068cd8379d39034a60cab2094a063c.png)
5. 在 “验证域名” 页面中，请根据页面提示，添加文件验证。如下图所示：
![](https://main.qcloudimg.com/raw/416f24505ab89d9f49402bac515ee363.png)
6. 请您登录服务器，并且确保域名已添加 A 记录指向该服务器。
>?若您的域名对应的**域名解析商**在腾讯云，将域名指向您的服务器请参考 [A 记录](https://cloud.tencent.com/document/product/302/3449)。
>
7. 请您在服务器上启动一个 Web 服务（也可在业务运行的 Web 服务上进行配置）并开启监听端口80或443，将文件验证路径地址反向代理为**验证域名（步骤5图例）**中提供的反向代理地址。
...
>?
 ...
 >- 可以支持301/302跳转，跳转次数不超过2次，跳转目的地址跟被检测域名需在同一主域。对于 www 开头的二级域名，例如 `www.domain.com`，除了对该域名进行文件验证，还需对其主域名 `domain.com` 添加文件验证。
 >

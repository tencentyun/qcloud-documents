...
4. 若您的域名对应的**域名解析商**在腾讯云，请您登录 [云解析控制台](https://console.cloud.tencent.com/cns) 为申请证书的域名 `www.domain.com` 添加一条记录类型为 TXT 的 DNS 记录。具体操作请查看 [TXT 记录](https://cloud.tencent.com/document/product/302/12648)。
5. 添加成功后，证书的域名（例如 `www.domain.com`） TXT 记录值的系统会定时检查，若能检测到并且与指定的值匹配，即可完成域名所有权验证。如下图所示：

...
>! 
>- 非 Wotrus 品牌证书（国际标准）与 DNSPod 品牌证书（国密标准（SM2））请尽快添加验证，若您在3天内未添加验证或验证不通过，审核机构将拒绝您此次证书申请。
...
<tr>
<td>自动 DNS 验证</td>
<td>须使用腾讯云 DNS 解析的域名，<br><strong>如使用腾讯云 DNS 解析，推荐使用此种方式（仅 TrustAsia 品牌域名型（DV）SSL 证书（包含免费证书）支持自动 DNS 验证功能）。</strong></td>
<td rowspan="3"><strong>域名所有权验证证书</strong>的首次颁发时长预估为10分钟 - 24小时</td>
</tr>
<tr>
<td>手动 DNS 验证</td>
...
>- Wotrus 品牌证书（国际标准）与 DNSPod 品牌证书（国密标准（SM2））暂不支持自动 DNS 验证功能。
>

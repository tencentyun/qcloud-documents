...
<tr>
<td>自动 DNS 验证</td>
<td>须使用腾讯云解析的域名<br>如使用腾讯云解析，推荐使用此种方式</td>
...
4. 若您的域名对应的**域名解析商**在腾讯云，请您登录 [云解析控制台](https://console.cloud.tencent.com/cns) 为申请证书的域名 `www.domain.com` 添加一条记录类型为 TXT 的 DNS 记录，指定的解析格式为 “`www.domain.com` > TXT > 20181227220956......hj37i4xai8m7uii2a23l”。
5. 切换至 [云解析控制台](https://console.cloud.tencent.com/cns)，在待添加 TXT 解析记录的域名行，单击【解析】。如下图所示：
![解析](https://main.qcloudimg.com/raw/1799b5eaa63550d1d774e67af62eefe7.png)
6. 在域名解析页面，单击【添加记录】，添加新记录行。
关键参数信息如下：
 ...
 - TTL（秒）：选择默认值600秒。
7. 单击【保存】。
8. 在弹出的 “添加成功” 提示框中，单击【我知道了】，记录列表显示新添加的记录信息，即表示添加完成。如下图所示：
...
3. 请您登录服务器，并且确保域名已指向该服务器。具体请您到域名对应的域名解析商处进行设置。

...
5. 找到 `LoadModule ssl_module modules/mod_ssl.so` 和 `Include conf.modules.d/*.conf` 两条配置语句，并检验是否被注释。
...
> - `http-ssl.conf` 配置文件。
> 
> 若以上配置文件中均未找到 `LoadModule ssl_module modules/mod_ssl.so` 和 `Include conf.modules.d/*.conf` 配置语句，请确认是否已经安装 mod_ssl.so 模块。若未安装 mod_ssl.so 模块，您可通过执行 `yum  install mod_ssl` 命令安装 mod_ssl.so 模块。
> - `LoadModule ssl_module modules/mod_ssl.so` 配置语句：用于加载 SSL 模块，一般配置在 `/etc/httpd/conf.modules.d` 目录下的 00-ssl.conf 配置文件中。
> - `Include conf.modules.d/*.conf` 配置语句：用于加载配置 SSL 的配置目录，一般配置在 `/etc/httpd/conf` 目录下的 httpd.conf 配置文件中。
> 

...
解压缩后，分别获得以下文件夹和 CSR 文件：
 - Apache 文件夹：适用于 Apache 服务器，包含了 `1_root_bundle.crt` 证书文件、`2_www.domain.com.crt` 证书文件和 `3_www.domain.com.key` 私钥文件。
 - Nginx 文件夹：适用于 Nginx 服务器，包含了 `1_www.domain.com_bundle.crt` 证书文件和 `2_www.domain.com.key` 私钥文件。
 - IIS 文件夹：适用于 IIS 服务器，包含了 `www.domain.com.pfx` 证书文件和 `keystorePass.txt` 密码文件。若您在申请 SSL 证书时已设置私钥密码，该文件夹下则无 `keystorePass.txt` 文件。
 - Tomcat 文件夹：适用于 Tomcat 服务器，包含了 `www.domain.com.jks` 密钥库和 `keystorePass.txt` 密码文件。若您在申请 SSL 证书时已设置私钥密码，该文件夹下则无 `keystorePass.txt` 文件。
 >! 当您申请 SSL 证书时选择了“粘贴 CSR” 方式，则不提供 Tomcat 证书文件的下载，需要您通过手动转换格式的方式生成密钥库。其操作方法如下：
 > 1. 访问 [转换工具](https://myssl.com/cert_convert.html)。
 > 2. 将 Nginx 文件夹中的证书文件和私钥文件上传至转换工具中，并填写**密钥库密码**，单击【提交】，转换为 jks 格式证书。
 - CSR 文件：即 `www.domain.com.csr` 文件。

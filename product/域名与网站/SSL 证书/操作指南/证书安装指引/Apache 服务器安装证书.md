...
>? 若无 `/etc/httpd/ssl` 目录，可通过`mkdir /etc/httpd/ssl`命令行新建。
3. 关闭 WinSCP 界面。
...
>?Apache 的版本不同，目录结构也会有所区别。具体的则请参阅 Apache 官方的 rewrite 的文档，请您按照需求进行配置 http://httpd.apache.org/docs/2.4/mod/mod_rewrite.html。

...
2. 兼容性 Symantec 优于 Geotrust； Symantec 可兼容市面上所有的浏览器，对移动端的支持也是极好的。
...
6. Symantec 对证书有商业保险赔付保障，金额最高为175万美金，GeoTrust 最高为150万美金。
...
不同品牌的证书在浏览器地址栏、加密强度、赔付保障上均存在差异，最重要的差异点在于根证书。例如，Geotrust 通配符是 Geotrust 根证书签发的，而 Symantec 通配符是 Symantec 根证书签发的。Symantec 根证书可以兼容市面上所有的浏览器，对移动端的支持也是最好的，而 Trustasia 通配符也是赛门铁克的根证书，GlobalSign 通配符是 GlobalSign 的根证书签发的。

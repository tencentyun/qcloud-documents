## 操作场景
Wotrus 品牌证书企业型（OV）与增强型（EV）SSL 证书在购买成功后，需要进行相关材料提交。证书购买流程请参考 [购买流程](https://cloud.tencent.com/document/product/400/7995)。

## 前提条件
1. 登录 [SSL 证书管理控制台](https://console.cloud.tencent.com/ssl)。
2. 在 “我的证书” 页面，选择已购买的证书行，单击【提交资料】。

## 操作步骤
>!不同域名类型的证书，需填写信息略有不同。本操作以企业型（OV）多域名证书为例。
>
### 步骤1：填写域名
根据实际需求，选择 CSR 生成方式（择一）。
- 选择 “在线生成 CSR” 方式，执行 [在线生成 CSR 的操作](#csr1)（**推荐使用在线生成 CSR，由平台生成和管理您的私钥和公钥证书文件，避免私钥文件丢失**）。
- 选择 “粘贴 CSR” 方式，执行 [粘贴 CSR 的操作](#csr2)（**自行上传，不可生成私钥**）。

**在线生成 CSR**<span id="csr1"></span>
1. 填写域名信息。如下图所示：
![](https://main.qcloudimg.com/raw/41158096ac7b1fb92ae5d31b63648dec.png)
主要参数信息如下：
 - **证书绑定域名**：填写绑定证书的域名。
 - **算法选择**：勾选所需证书加密算法。
 - **密钥长度**：勾选所需证书密钥长度。
 - **私钥密码**：可选填，填写后不可更改不可找回，请牢记私钥密码。
>?如需部署腾讯云负载均衡、CDN等云服务、请勿填写私钥密码。
>
2. 填写公司信息。
>?请如实填写公司名称（全称），公司部门，公司所在城市与地址以及公司座机号码。
3. 填写管理人消息。
4. 单击【下一步】，进入 [步骤2](#message)。

**粘贴 CSR**<span id="csr2"></span>
>?不同域名类型的证书，需填写信息略有不同。本操作以多域名证书为例。

1. 将已准备好的 CSR 信息粘贴至文本框中，即可识别您的域名信息，并如实填写公司名称（全称），公司部门，公司所在城市与地址以及公司座机号码等公司相关信息。如下图所示：
![](https://main.qcloudimg.com/raw/ae19357f4bdd39903da3922b46529528.png)
2. 单击【下一步】，进入 [步骤2](#message)。

<span id="message"></span>
### 步骤2：选择域名认证方式
1. 在 “选择验证方式” 页面，选择域名验证方式。如下图所示：
![](https://main.qcloudimg.com/raw/6d1d1e1748c47d1a5722e8ebdc41bfcc.png)
2. 单击【确认】。进入“预审核” 页面。

>?自动 DNS 验证验证方式需满足自动 DNS 验证验证方式验证条件，不满足默认不显示。详情请查看：[域名验证指引](https://cloud.tencent.com/document/product/400/4142)。


### 步骤3：预审核
提交资料并选择域名验证方式后，需等待业务人员预审核您的证书，审核时间预计在10分钟 - 72小时，请您耐心等待。


### 步骤4：验证域名
1. 在 “验证域名” 页面，请根据页面提供信息进行域名验证操作，如您选择的是手动 DNS 验证方式，将会展示如下信息。
 ![](https://main.qcloudimg.com/raw/5340207eca8c0e2cde71a80853d87ca8.png)
您可根据以下操作说明进行验证：
 - **手动 DNS 验证**：域名验证操作请参考：[手动 DNS 验证](https://cloud.tencent.com/document/product/400/4142#.E6.89.8B.E5.8A.A8-dns-.E9.AA.8C.E8.AF.81)。
 - **文件验证**：域名验证操作请参考：[文件验证](https://cloud.tencent.com/document/product/400/4142#.E6.96.87.E4.BB.B6.E9.AA.8C.E8.AF.81)。
2. 域名验证操作完成后，可单击【查看域名验证状态】，查看域名验证操作是否成功。

### 步骤5：人工审核
域名验证操作验证成功后，CA 机构将审核您提交的信息与证书，期间会联系您完成认证，请保持联系方式畅通并耐心等待。

### 步骤6：签发证书
CA 机构认证审核通过后，将会正式签发证书，您可下载证书进行安装。
> ?
> - 需人工审核及域名验证均通过后，才可颁布证书。
> - 提交申请后，需人工审核。人工审核为电话审核，审核过程中将会致电您公司工商登记电话，电话来源地为美国，请您注意接听。

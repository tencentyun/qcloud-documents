...
## 品牌差异
不同品牌的证书在浏览器地址栏、加密强度、赔付保障上均存在差异，最重要的差异点在于根证书。例如，GeoTrust 通配符是 GeoTrust 根证书签发的，而 SecureSite 通配符是 SecureSite 根证书签发的。SecureSite 根证书可以兼容市面上所有的浏览器，对移动端的支持也是最好的，而 Trustasia 通配符也是 SecureSite 的根证书，GlobalSign 通配符是 GlobalSign 的根证书签发的，Wotrus 通配符是 Sectigo 的根证书签发的。

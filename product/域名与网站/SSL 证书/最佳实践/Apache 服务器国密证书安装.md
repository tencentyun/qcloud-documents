...
远程登录 Apache 服务器。例如，使用 [“PuTTY” 工具](https://cloud.tencent.com/document/product/213/35699#.E6.93.8D.E4.BD.9C.E6.AD.A5.E9.AA.A4) 登录。

**1.安装编译工具**
如果您的系统是全新的，请先在服务器上安装 C++ 开发环境，为编译提供环境支持。您可以使用如下命令进行安装。
```
yum install -y gcc yum install-y gcc-c++ 
```

**2.下载并编译安装 apr (以 apr 1.7.0 版本为例)**
您可以通过在服务器上输入以下命令，下载 apr 至服务器并编译安装 ，由于操作系统的版本不同，详细操作步骤略有区别。
...
**2. 下载并编译安装  apr-util (推荐使用 apr-util-1.5 版本，1.6 版本存在兼容性问题，以下以 apr-util-1.5  版本为例)**
...
**4. Apache 服务器安装**
上述三个文件编译安装完成后，下载 Apache 国密版和国密模块至 `/usr/local` 目录下进行编译安装。
...
#进入解压后的pcre-8.43文件夹并指定编译目录路径。
cd httpd-2.4.39_gm/ ./configure --prefix=/usr/local/httpd --enable-so --enable-ssl --enable-cgi --enable-rewrite --enable-modules=most --enable-mpms-shared=all --with-mpm=prefork --with-zlib --with-apr=/usr/local/apr --with-apr-util=/usr/local/apr-util --with-ssl=/usr/local/wotrus_ssl 
#编译安装 Apache
make && make install
```
...
4. 进入`/usr/local/httpd/conf` 目录，编辑`httpd.conf`文件，找到 `#LoadModulessl_modulemodules/mod_ssl.so`文件内容，去掉前面的注释符`#`，添加` Include conf/ssl.conf`文件内容后保存并退出。
...
SSLCertificateChainFile /usr/local/httpd/conf/1_root_sign_bundle.crt
# SM2 证书 encrypt 配置 
SSLCertificateFile /usr/local/httpd/conf/4_cloud.tencent.com_encrypt.crt
SSLCertificateKeyFile /usr/local/httpd/conf/5_cloud.tencent.com.key
SSLCertificateChainFile /usr/local/httpd/conf/2_root_encrypt_bundle.crt
# sign 和 encrypt 配置中的 .key 为同一个 
#请按照以下协议配置
SSLProtocol all-SSLv2-SSLv3 
#请按照以下套件配置，配置加密套件，写法遵循 openssl 标准。
SSLCipherSuite SM2-WITH-SMS4-SM3:ECDH:AESGCM:HIGH:MEDIUM:!RC4:!DH:!MD5:!aNULL:! eNULL SSLHonorCipherOrder on 
...
>?
>以上配置内容仅为参考，具体的 `ServerName`，证书名称，证书目录，`Directory` 等配置请根据实际环境配置。

...
>
2. 请登录 Weblogic 服务管理后台（默认地址：`http://localhost:7001/console`），输入您的用户名及密码，即可进入 WebLogic Server 管理控制台。
3. 单击域配置中的【服务器】。如下图所示：
![](https://main.qcloudimg.com/raw/ffc8a6df5c09dd96680cdcf1bdb241cb.png)
4. 选择您要配置的 Web 服务器名称，以 `AdminiServer` 为例。 
![](https://main.qcloudimg.com/raw/aa01743bf9a8db4e4b56688069bd9906.png)
5. 进入`AdminiServer`的管理设置，勾选【启用 SSL 监听端口】并填写 SSL 监听端口为 `443`，单击【保存】，如下图所示：
![](https://main.qcloudimg.com/raw/c48f73608b825c6fd3d50fe5e2d1ddcb.png)
6. 单击【密钥库】，进行密钥库设置页面，设置完成后并单击【保存】。如下图所示：
...
>
7. 单击【SSL】功能项，进入 SSL 设置页面，设置完成后并单击【保存】。如下图所示：
...
>
8. 修改完以上所有项目并且保存后，会自动激活，不需要进行重启。即可使用 `https://cloud.tencent.com` 进行访问。

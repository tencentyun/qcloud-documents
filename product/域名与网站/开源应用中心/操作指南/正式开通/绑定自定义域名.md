...
>- 若您的 SSL 证书是在非腾讯云平台进行申请。请将您申请的 SSL 上传至腾讯云[证书控制台](https://console.cloud.tencent.com/certoverview)在进行绑定域名操作。详细操作请参见：[上传（托管） SSL 证书指引](https://cloud.tencent.com/document/product/400/48790)。
...
>! 若您的域名在非腾讯云 DNSPod 解析平台进行托管，如阿里云、CloudFlare 等。无需进行**域名所有权 TXT 验证**，只需**配置 CNAME 解析记录**。

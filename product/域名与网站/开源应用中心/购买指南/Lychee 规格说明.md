...
## Lychee 规格说明
若您在腾讯云开源应用中心正式开通 Lychee 应用实例，将默认使用以下规格云资源：
...
>? Lychee 将会创建3个文件存储（CFS）实例，用于存储配置文件，文件资源等。
>
## 费用计算示例
您在腾讯云开源应用中心使用已正式开通的 Lychee 应用实例，某天使用云托管0.5GB流量以及4小时运行时间，文件存储空间为10GiB，则将产生以下费用：
...
- **每天预估总费用**：云托管费用 + 存储空间费用 = 0.5元 + 0.1元 = 0.6元

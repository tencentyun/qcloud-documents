...
## KodBox 规格说明
若您在腾讯云开源应用中心正式开通 KodBox 应用实例，将默认使用以下规格云资源：
...
## 费用计算示例
您在腾讯云开源应用中心使用已正式开通的 KodBox 应用实例，某天使用云托管1GB流量以及6小时运行时间，文件存储空间为10GB，以及在一天的6个小时内，均为2个 CCU 的量，则将产生以下费用：
...
- **每天预估总费用**：云托管费用 + 存储空间费用  = 0.9元 + 0.1元 + 5.2元 = 6.2元

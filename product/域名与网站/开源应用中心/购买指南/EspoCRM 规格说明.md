...
## 费用计算示例
您在腾讯云开源应用中心使用已正式开通的 EspoCRM 应用实例，某天使用云托管1GB流量以及6小时运行时间，文件存储空间为10GB，云原生数据库一天中使用了10GiB的存储空间，以及在一天的6个小时内，累计使用了个10000个 CCU 的量，则将产生以下费用：
...
- **每天预估总费用**：云托管费用 + 存储空间费用 + 云原生数据库费用 = 1.3元 + 0.1元 + 5.2元 = 6.6元

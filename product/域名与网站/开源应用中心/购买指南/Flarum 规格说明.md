...
## Flarum 规格说明
若您在腾讯云开源应用中心正式开通 Flarum 应用实例，将默认使用以下规格云资源：
...
>? Flarum 将会创建3个文件存储（CFS）实例，用于存储日志，文件资源等。
>
## 费用计算示例
您在腾讯云开源应用中心使用已正式开通的 Flarum 应用实例，某天使用云托管0.5GB流量以及4小时运行时间，文件存储空间为1GiB，云原生数据库一天中使用了1GB的存储空间，以及在一天的4个小时内，均为0.5个 CCU 的量，则将产生以下费用：

- **每天预估云托管费用**：1核 × 0.055元/核/小时 × 4小时 + 2GiB × 0.032元/GiB/小时 × 4小时 + 0.8元/GB × 0.5GB = 0.8元（保留小数点后一位）
...
- **每天预估总费用**：云托管费用 + 存储空间费用 + 云原生数据库费用 =  0.8元 + 0.01元 +  0.8元 = 1.61元

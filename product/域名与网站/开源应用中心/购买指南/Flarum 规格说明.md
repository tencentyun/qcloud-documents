...
## 费用计算示例
您在腾讯云开源应用中心使用已正式开通的 Flarum 应用实例，某天使用云托管1GB流量以及6小时运行时间，文件存储空间为10GiB，云原生数据库一天中使用了10GB的存储空间，以及在一天的6个小时内，均为2个 CCU 的量，则将产生以下费用：

- **每天预估云托管费用**：1核 × 0.055元/核/小时 × 6小时 + 2GiB × 0.032元/GiB/小时 × 6小时 + 0.8元/GB × 1GB = 1.5元（保留小数点后一位）
...
- **每天预估总费用**：云托管费用 + 存储空间费用 + 云原生数据库费用 = 1.5元 + 0.1元 + 5.2元 = 6.8元

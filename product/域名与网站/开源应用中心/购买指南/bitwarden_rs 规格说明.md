...
## 费用计算示例
您在腾讯云开源应用中心使用已正式开通的 bitwarden_rs 应用实例，某天使用云托管0.5GB流量以及2小时运行时间，文件存储空间为1GiB，则将产生以下费用：
- **每天预估云托管费用**：0.25核 × 0.055元/核/小时 × 2小时 + 0.5GiB × 0.032元/GiB/小时 × 2小时+ 0.8元/GB × 0.5GB = 0.3元（保留小数点后一位）
- **每天预估存储空间费用**：0.00048611元/GiB/时 × 1GiB × 24小时 = 0.01元（保留小数点后一位）
- **每天预估总费用**：云托管费用 + 存储空间费用 = 0.3元 + 0.01元 = 0.31元

...
## Ghost 规格说明
若您在腾讯云开源应用中心正式开通 Ghost 应用实例，将默认使用以下规格云资源：
...
## 费用计算示例
您在腾讯云开源应用中心使用已正式开通的 Ghost 应用实例，某天使用云托管0.5GB流量以及4小时运行时间，文件存储空间为1GiB，云原生数据库一天中使用了1GB的存储空间，以及在一天的4个小时内，均为0.5个 CCU 的量，则将产生以下费用：

- **每天预估云托管费用**：2核 × 0.055元/核/小时 × 4小时 + 4GiB × 0.032元/GiB/小时 × 4小时 + 0.8元/GB × 1GB = 1.7元（保留小数点后一位）
...
- **每天预估总费用**：云托管费用 + 存储空间费用 + 云原生数据库费用 = 1.7元 + 0.01元 + 0.8元 = 2.51元

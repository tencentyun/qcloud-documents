...
## Joomla 规格说明
若您在腾讯云开源应用中心正式开通 Joomla 应用实例，将默认使用以下规格云资源：
...
## 费用计算示例
您在腾讯云开源应用中心使用已正式开通的 Joomla 应用实例，某天使用云托管1GB流量以及6小时运行时间，文件存储空间为10GiB，云原生数据库一天中使用了10GB的存储空间，以及在一天的6个小时内，累计使用了个10000个 CCU 的量，则将产生以下费用：
...
- **每天预估总费用**：云托管费用 + 存储空间费用 + 云原生数据库费用 = 0.9元 + 0.1元 + 2.1元 = 4.4元

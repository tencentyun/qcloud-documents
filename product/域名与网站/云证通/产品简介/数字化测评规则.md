数字资产测评将针对用户的数字资产数量进行全方位扫描，依照其基线产品（资质证照、知识产权与数字证书）的完整性与多样性、参考非基线产品（域名、解析等其他腾讯云云资源）的使用频次、评估限时任务的完成积极程度等一系列指标表现情况后，依照评分模型计算出用户的数字化测评分数。该分数可在一定程度上反应用户的数字化水平。

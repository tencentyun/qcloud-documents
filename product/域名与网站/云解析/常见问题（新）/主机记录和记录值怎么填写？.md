...
您在 DNS 解析添加了域名，首先需要去万网处将 DNS 地址修改成 DNS 解析提供的 `f1g1ns1.dnspod.net f1g1ns2.dnspod.net` 之后再进行记录值的添加。如不修改 DNS 地址，万网提供了一套 DNS 地址，DNS 解析提供了另一套 DNS 地址，会导致解析混乱，所添加的记录值也不会生效。

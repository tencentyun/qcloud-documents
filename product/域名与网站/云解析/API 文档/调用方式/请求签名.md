...
2. 选择【云产品】>【管理与审计】>【访问密钥】，进入【[API 密钥管理](https://console.cloud.tencent.com/capi)】页面。
...
SecretId： AKIDz8krbsJ****************Px3gnPhESA
SecretKey： Gu5t9xGAR****************CN3Cozk1qA
...
| SecretId | 密钥 ID  | AKIDz8krbsJ****************Px3gnPhESA | 
...
- 若输入参数的 Key 中包含下划线，则需要将其转换为 `.`，但是 Value 中的下划线则不用转换。例如 `Placement_Zone=CN_GUANGZHOU`， 则需要将其转换成 `Placement.Zone=CN_GUANGZHOU`。
...
- **请求路径：** 腾讯云 API 对应产品的请求路径，一般是一个产品对应一个固定路径（DNS 解析请求路径固定为 `/v2/index.php`）。
- **请求字符串：** 即上一步生成的请求字符串。
...
```php
$secretKey = 'Gu5t9xGAR****************CN3Cozk1qA';
$srcStr = 'GETcvm.api.qcloud.com/v2/index.php?Action=DescribeInstances&InstanceIds.0=ins-09dx96dg&Nonce=11886&Region=ap-guangzhou&SecretId=AKIDz8krbsJ****************Px3gnPhESA&SignatureMethod=HmacSHA256&Timestamp=1465185768';
...
```
0EEm/HtGRr/VJXTAD9tYMth1Bzm3l****LHz5RCDv1GdM8s=
```

同理，当您指定签名算法为 **HmacSHA1** 时，生成签名串的代码如下：
```php
$secretKey = 'Gu5t9xGAR****************CN3Cozk1qA';
$srcStr = 'GETcvm.api.qcloud.com/v2/index.php?Action=DescribeInstances&InstanceIds.0=ins-09dx96dg&Nonce=11886&Region=ap-guangzhou&SecretId=AKIDz8krbsJ****************Px3gnPhESA&SignatureMethod=HmacSHA1&Timestamp=1465185768';
...
最终得到的签名串为：
```
nPVnY6njQmwQ****8ciqbPl5Qe+Oru4=
```


##  签名串编码
生成的签名串并不能直接作为请求参数，需要对其进行 URL 编码。
如上一步生成的签名串为 `0EEm/HtGRr/VJXTAD9tYMth1Bzm3****lLHz5RCDv1GdM8s=`，则其编码后为 `0EEm%2FHtGRr%2FVJXTAD9tYMth1Bzm3****lLHz5RCDv1GdM8s%3D`。因此，最终得到的签名串请求参数 (Signature) 为： `0EEm%2FHtGRr%2FVJXTAD9tYMth1Bzm3****lLHz5RCDv1GdM8s%3D`，它将用于生成最终的请求URL。
...
| 4101 | 未被开发商授权访问本接口 | 该子用户未被授权调用此接口。请联系开发商授权，详情请查阅 [授权策略](/doc/product/598/10601)。|
| 4102 | 未被开发商授权访问本接口中所操作的资源 | 请求的资源参数中，存在未被开发商授权访问的资源，请在 message 字段中查看无权查看的资源 ID。</br>请联系开发商授权，详情请查阅 [授权策略](/doc/product/598/10601)。|

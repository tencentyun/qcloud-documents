...
- 云支付另外提供一个工具[SecurityDemo](https://mc.qcloudimg.com/static/archive/9d865835fb378724bed9f8fb0117239e/SecurityDemo.zip),解压后包含两个目录，一个是Demo\_src，为Demo的源码，另外一个是Demo\_tools, 可以体验安全登入的过程。
- 本文档分两部分，一部分是验证接口，另外一部分是安全SDK验证接口
...
如果是首次登入，APP首先调用接口cloud\_pay\_security\_login后，发现需要输入账户登入，则可以生成如下类似的登入页面。需要输入服务商维系支付商户号、子商户的微信支付商户号和门店ID，然后点击登入，生成一个登入二维码，然后店员用手机微信客户端扫描这个二维码，如果这个店员的生成合法，则手机端会显示客户端授权成功，否者失败。然后APP调用接口cloud\_pay\_security\_login\_check 轮询是否登入成功，如果登入成功就进入初始化参数配置页面。

![](https://mc.qcloudimg.com/static/img/1b787fcdaa0ea41cfbf9f807ce4dc309/demo_login.png)

### 初始化参数配置

配置好初始化参数后点击初始化按钮，会调用接口cloud\_pay\_security\_init，然后会在进程的当前目录生成两个文件cloud\_pay\_security\_login\.encrpyt和cloud\_pay\_security\_key\.encrpyt。敏感信息存在文件cloud\_pay\_security\_key\.encrpyt中。

![](https://mc.qcloudimg.com/static/img/2def4922a68200772caffab3d5b792ae/demo_init.png)

### 再次安全登入

如果是再次登入APP，接口cloud\_pay\_security\_login后直接返回一个二维码链接和一个ID。APP需要将这个二维码链接生成一个二维码，店员用手机微信客户端扫描这个二维码即可。

![](https://mc.qcloudimg.com/static/img/d68cb20646da7a0fbea20c4efd6963f1/demo_login_again.png)

如果店员身份合法，然后APP调用接口cloud\_pay\_security\_login\_check 轮询会得到登入成功的结果，然后APP可以进入到初始化界面，并且可以得到初始化配置参数（这个配置项是由接口cloud\_pay\_security\_login\_check返回），如

![](https://mc.qcloudimg.com/static/img/8f2204f4b1ac0acfb29ad643ac85d99c/demo_init_again.png)

...
这个时候，您就可以通过子账号实现不同的人管理不同的服务，来规避以上的问题。默认情况下，子账号没有使用 仿真云集群或者仿真云任务的权限。因此，我们就需要创建策略来允许子账号使用他们所需要的资源或权限。
...
_**注意**：表中未列出的 仿真云 API 操作即表示该 仿真云 API 操作不支持资源级权限。针对不支持资源级权限的 仿真云 API 操作，您仍可以向用户授予使用该操作的权限，但是策略语句的资源元素必须指定为 \*。_

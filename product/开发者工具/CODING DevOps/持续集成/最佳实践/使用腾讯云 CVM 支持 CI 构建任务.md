...
CODING 中提供了内置云服务器用来执行持续集成（CI）中的构建计划，能够胜任大部分构建任务。但如果碰上了大型项目的构建，或者需要在本地服务器生成构建成果，单个计算资源就显得有点捉急了。针对这一部分需求，CODING 现已支持接入第三方计算资源作为构建节点，甚至可以接入多个服务器共同作为构建节点池，打造专属的计算集群。

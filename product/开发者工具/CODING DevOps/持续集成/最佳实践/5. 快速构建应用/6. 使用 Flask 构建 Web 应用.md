...
我们开发时所写的源代码，技术人员可以理解，机器并不理解其意。而若想在计算机上运行一个应用程序，则必须将该程序由**源代码**转换成计算机所能理解的**二进制机器码**。为此，我们需要将程序编译成可执行文件，通过构建来执行更加工程化、结构化、严谨的一系列步骤，并将产出的 [制品](https://en.wikipedia.org/wiki/Artifact_(software_development)) 部署上生产环境。

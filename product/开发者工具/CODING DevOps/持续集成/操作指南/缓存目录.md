...
![CODING 持续集成 缓存目录](https://help-assets.codehub.cn/enterprise/20200408095821.png)

## Docker 环境构建

如果使用 Docker 环境构建，先勾选「缓存目录」，再挂载到 Docker 中即可（一般为 root 用户）。比如下列 `Jenkinsfile` 挂载了 npm 缓存：

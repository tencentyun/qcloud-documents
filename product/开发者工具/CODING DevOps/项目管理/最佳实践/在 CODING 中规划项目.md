...
[](id:manage-demand)
## 收集与管理需求
产品经理将规划上线的功能、用户的反馈以及市场调研的结果整理出来，通过需求管理中需求的形式统一归纳，形成需求池。同时产品负责人对需求池中的需求进行进一步的分析，根据团队习惯将需求分为技术问题、设计问题和产品问题。每一条需求下面会根据需求的复杂程度创建一系列子任务，越重要的需求需要撰写越完整的需求描述。需求收集的需求信息可根据需求进行自定义设置需要反馈的字段，产品经理随着该需求在整个生命周期中阶段的变化，可以在需求中添加**需求类型**、**截止时间**、**预计工时**等信息。并可以为需求添加标签，例如**功能**、**Bug**、**调研**等等。

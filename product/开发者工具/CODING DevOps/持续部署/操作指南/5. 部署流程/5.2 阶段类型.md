...
需要注意的时，如果 Spinnaker 检测到不需要进行新的 Bake，则跳过 Bake 过程。 CODING 部署控制台会根据 Bake 阶段参数（基础操作系统，版本化的软件包等）为每次 Bake 生成唯一的键（key）。如果软件包或 Bake 阶段参数发生了更改，则会触发新的 Bake 操作。 要更改默认行为并在部署流程每次运行时重新 Bake 镜像，请在阶段阶段配置中勾选 `Rebake`。详情请参见 [Hashicorp Packer](https://www.packer.io/intro)。

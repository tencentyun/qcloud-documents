...
| 参数名称 | 是否必选  | 类型 | 描述 |
|---------|---------|---------|---------|
| imageId | 是 | String | 镜像ID，可通过 [DescribeImages](http://www.qcloud.com/document/api/213/1272) 接口返回字段中的 unImgId 获取。|
| uinList.n | 是 | String | 被共享人的账号（此接口支持同时传入多个ID。此参数的具体格式可参考API[简介](https://www.qcloud.com/doc/api/229/568)的 `id.n`一节）。|

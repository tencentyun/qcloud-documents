...
### 构建环境配置
工作流 job 的构建环境配置为1核2G（无法自定义配置）。如果您的构建任务需要较多内存，有可能因为内存溢出而导致构建失败。如果您对构建环境有更高的需求。可以考虑 [开发者平台](https://dev.tencent.com) 中 CODING 企业版产品的持续集成功能。该持续集成有两种模式。常规模式是2核4G环境，云服务器模式是8核16G环境。

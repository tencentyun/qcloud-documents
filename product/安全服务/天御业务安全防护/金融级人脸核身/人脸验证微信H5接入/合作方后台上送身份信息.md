### 1.生成签名
1. **前置条件**：必须按照[SIGN ticket获取](https://www.qcloud.com/document/product/295/10120?!preview&lang=cn).
...
| version | 1.0.0 | 
| api ticket | 合作伙伴服务端缓存的tikcet,注意是SIGN 类型,具体见[Access Token获取规则](https://www.qcloud.com/document/product/295/10118)| 

3.将webankAppId, orderNo, name, idNo, userId, version，ticket（SIGN类型）共七个参数的值进行字典序排序。将排序后的所有参数字符串拼接成一个字符串进行SHA1编码。
...
| sourcePhotoStr | 比对源照片 | BASE64 String | 比对源照片的BASE64 String<br>长度：3145728 | 非必输，<br>**注意**：原始图片不能超过2M,且必须为JPG或PNG格式。<br>**有值**：使用合作伙伴提供的比对源照片进行比对，必须注意照片是正脸可信照片，照片质量由合作方保证。<br>**为空**：根据身份证号+姓名使用权威数据源比对 |

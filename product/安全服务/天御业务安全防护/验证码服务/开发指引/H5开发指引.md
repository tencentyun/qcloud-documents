...
## 3.客户端加载验证码和获取票据接口 ##
[H5客户端-API](/doc/product/295/6616)

## 4.获取验证码票据 ##
用户依据第3步获取的用户验证票据，提交到后台。<br> function cbfn(retJson){<br> 　　if(retJson.ret==0)<br> 　　{<br> 　　　//用户验证成功br> 　　　retJson.ticket://用户票据<br> 
　　}<br> 　　else<br> 　　｛<br> 　　　//用户关闭验证码页面，没有验证<br> 　　｝<br> ｝

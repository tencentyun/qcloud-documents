...
数据库防护服务主要分为接入域名和数据库白名单两大块。本文档介绍了如何去查看数据库白名单的日志，并针对 MySQL 数据库进行测试举例，可供参考。
...
3. 在数据库白名单页面，单击**添加规则**，访问目的选择 [步骤2](#stpe2) 所创建的数据库，其他根据实际需求选择。
...
2. 在数据库白名单页面，可以查看数据库相关的访问控制日志。其中命中白名单策略的会被记录为观察类型，其他的则会被即为阻断类型。
![](https://qcloudimg.tencent-cloud.cn/raw/792ed040603fdd78f0092449610b751b.png)

## 测试
1. 在外网通过数据库连接工具进行连接测试，设置参数如下图所示。
![](https://qcloudimg.tencent-cloud.cn/raw/702845714ebf8e3b1f687c9fa807f38b.png)
...
2. 搭建好测试环境后，通过控制数据库白名单的开关来验证。
 - 开关关闭状态：也就是数据库白名单不生效这种情况。
 ![](https://qcloudimg.tencent-cloud.cn/raw/e4e8a34d0f9ccc5ec69d5708c0861a41.png)
 - 测试结果：数据库访问成功，如下所示。
 ![](https://qcloudimg.tencent-cloud.cn/raw/c73ffe99e0d95b56514c54f37c748d96.png)
 - 开关开启状态：即白名单生效。
 ![](https://qcloudimg.tencent-cloud.cn/raw/292f1ad3a01ed819acde12e5425410c0.png)
 - 此时如果不配置规则来放通相应 IP，访问是不成功的，如将之前创建好的规则删除或者是将开关关闭，此时连接数据库是失败的：
![](https://qcloudimg.tencent-cloud.cn/raw/8ed8e87c372fe3420b46045a1eba3561.png)
 - 如上，将规则开关关闭，测试结果如下，连接过程中报错
![](https://qcloudimg.tencent-cloud.cn/raw/30492655b255a1845fc8773452e9b72a.png)
以上测试证明了数据库白名单的作用，可以有效降低数据库连接域名的暴露面，从而避免数据库被客户进行暴力破解等攻击，保障了数据库安全。

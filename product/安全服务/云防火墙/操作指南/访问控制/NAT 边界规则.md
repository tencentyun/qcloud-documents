...
4. 添加规则，在入向规则页面，可进行规则的配置，以添加入向规则为例进行说明。
   ...
   ii. 在“添加入站规则”的弹窗中，可进行规则的配置。访问目的类型可选 IP 地址，地理位置、云厂商及 [地址模板](https://cloud.tencent.com/document/product/1132/49443#IP)，并根据规则重要性来勾选执行顺序，最后填写访问源 IP，填写访问目的、目的端口、选择协议及策略等信息。
>?
>- 访问目的所在区域： 指云上实例所在的地域

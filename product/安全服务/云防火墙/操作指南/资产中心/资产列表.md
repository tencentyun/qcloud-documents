...
>?防护状态指主机安全产品下的防护状态。若用户未安装主机安全，单击**操作** > **立即安装**，可安装主机安全客户端。若用户已安装主机安全，单击**操作** > **防护详情**，可进入主机安全控制台查看防护详情。
...
## 公网 IP 资产
### 公网 IP 资产统计概况
1. 查看资产统计概况，页面左侧包括公网IP资产、未防护公网IP、风险公网IP、绑定CVM资产、绑定CLB资产、绑定NAT资产，单击数字，页面下方即可筛选对应公网IP资产列表，供用户查看资产详情。
2. 用户可以在页面右侧查看对应时间段内，入/出向峰值带宽、入/出向累计流量、网络攻击告警、攻击拦截、暴露端口、暴露漏洞统计的TOP5资产。
...
>?防护状态指是否使用云防火墙进行相关防护，包括已防护、未防护、未知三种状态。若用户未开启云防火墙，单击**操作** > **开启防护**，可开启互联网边界防火墙开关，为此公网 IP 资产的访问流量进行防护。若用户已启用防火墙开关，单击**操作** > **防护详情**，可跳转至入侵防御模块，查看防火墙防护模式。
2. 单击![](https://qcloudimg.tencent-cloud.cn/raw/bd4277ed0277053370f23db6ef0048db.png)图标，可筛选对应的资产信息，包括公网IP类型、资产归属、地域。
...
>!防护状态指是否使用 Web 应用防火墙（WAF）产品进行防护，包括已防护、未防护两种状态。若用户未开启 WAF 防护，单击**操作** > **开启防护**，可开启 WAF，为此域名资产的访问流量进行防护。若用户已启用 WAF 防护，单击**操作** > **防护详情**，可跳转至 Web 应用防火墙查看防护详情。
>
...
2. 用户可以在页面右侧查看查看对应时间段内资产类型分布信息、以及入/出向峰值带宽、入/出向累计流量、网络攻击告警、攻击拦截、暴露端口、暴露漏洞统计的 TOP5 资产。
...
2. 用户可以在页面右侧查看查看对应时间段内资产类型分布信息、以及数据库访问量统计的 TOP5 资产。
...
3. 在页面下方，包括可以查看该主机资产的风险管理与网络攻击信息。

### 资产风险管理
>!默认显示未处理过的风险信息。
>
#### 端口风险
1. 对于未处理的端口风险，单击**操作** > **封禁端口**，支持下发访问控制规则，拦截目标端口的访问。封禁后，该风险的处理状态置为已封禁。
![](https://qcloudimg.tencent-cloud.cn/raw/210865d0786da0accef779f7ea43c46f.png)
2. 防御处置后的风险可以单击**操作** > **更多 **> **标记处置**，表示该风险已被处理，该风险的处理状态置为标记已处置。
![](https://qcloudimg.tencent-cloud.cn/raw/4513f12aea0514cd170737fe7f25ff52.png)
>!当下次扫描任务中仍然检测到此风险，则处理状态重新变回未处理。
>
3. 当扫描误报产生风险误报时或认为该风险无需处理时，单击**操作** > **更多** > **标记忽略**，可将该风险忽略，后续扫描任务中该风险将被过滤。
...
2. 对于未处理的漏洞风险，单击**操作** > **修复方案**，支持查看腾讯威胁情报提供的漏洞修复方案以及对应的参考链接。
![](https://qcloudimg.tencent-cloud.cn/raw/cc73219331ac4111c259bd6e879a6f1c.png)
3. 标记处置与忽略操作，参考端口风险。
4. 自定义风险列表与导出风险列表操作，参考端口风险。

#### 弱口令风险
1. 弱口令风险显示对应的弱口令类型与服务，用户可以进行标记处置与忽略操作，参考端口风险。
2. 自定义风险列表与导出风险列表操作，参考端口风险。

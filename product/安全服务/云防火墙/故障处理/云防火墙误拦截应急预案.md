本文档将为您介绍当遇到入侵防御的误报，导致大量异常拦截，或因策略变更有误，导致流量出现异常下跌情况时，需要如何处理。
## 现象描述
出现由于入侵防御误报导致的大量异常拦截，或因策略变更有误，导致流量出现异常下跌。
## 解决思路
在确认了是拦截的来源是云防火墙后，可以先止损（如关闭拦截功能）、再排查、最后交由产品安全团队人工处理。
## 处理步骤
### [步骤1：关闭拦截功能](id:step1)
...
1. 进入 [提交工单](https://console.cloud.tencent.com/workorder/category?level1_id=141&level2_id=1290&source=0&data_title=T-Sec-%E4%BA%91%E9%98%B2%E7%81%AB%E5%A2%99&level3_id=1323&queue=3233&scene_code=36970&step=2) 页面，提供 AppID 与误报的 IP 地址给安全团队确认。
2. 安全团队收到反馈后，会在规定时间内快速响应，调整检测规则。


### 步骤3：找到被误拦截的 IP 进行手动处置
...
4. 将误拦截 IP 加入白名单。
	- **方式1**：点击右侧的【忽略】按钮，将IP地址加入到白名单（忽略列表），放行后续的访问。

...
>?
>- 站点名 (UIN)：输入您的申请开通 MFA SaaS 服务时使用的腾讯云账户 UIN。 
>- 密码认证应用名：输入您之前在 MFA SaaS 控制台中创建的使用用户名密码策略的应用名称。
>- 密码认证密钥：输入您之前在 MFA SaaS 控制台中创建的使用用户名密码策略的应用密钥。
>- 动态口令认证应用名：输入您之前在 MFA SaaS 控制台中创建的使用动态口令策略的应用名称。
>- 动态口令认证密钥：输入您之前在 MFA SaaS 控制台中创建的使用用户名密码策略的应用密钥。
>
...
3. 进入用户登录页面后，输入用户名和密码，单击【登录】。
>?用户名密码为之前在 MFA SaaS控制 平台中创建的测试用户信息。此说明中使用的用户名为“demouser1”。 
>
![](https://main.qcloudimg.com/raw/cc32fa043ff80e78a3bcb8c68ef68fab.jpg)
5. 在多因素认证页面，输入 6位动态口令，单击【验证】。如果口令正确，您将会看到登录成功提示。 

本文档将为您介绍如何在凭据管理系统（SSM）中开通密钥管理系统（KMS）服务并进行授权，同时指导您如何在凭据管理系统中，使用密钥管理系统进行加密凭据。
## 背景信息
凭据管理是企业 IT 系统运维安全的核心诉求之一。使用腾讯云凭据管理系统，您可以托管各种类型的凭据，包括但不限于访问密钥、API 密钥、服务器证书、私钥和账号口令等。凭据管理系统使用腾讯云 KMS 托管的密钥对凭据进行加密保护，保证其在服务端的安全性，无需您自行构建和维护密钥管理基础设施，安全便捷。
#### 计费说明

腾讯云 KMS 由用户主密钥（Customer Master Key，简称 CMK）存储费用及 API 调用费用两部分组成：

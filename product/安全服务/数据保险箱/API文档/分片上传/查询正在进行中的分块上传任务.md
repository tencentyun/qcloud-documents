...
| max-count      | 设置最大返回的 multipart 数量，合法取值从1到1000，默认1000。                       | String | 否    |
...
|节点名称（关键字）|父节点|描述|类型|
|:---|:-- |:--|:--|
| Coffer | ListMultipartUploadsResult | 分块上传的目标 Coffer，由用户自定义字符串和系统生成 APPID 数字串由中划线连接而成，例如`examplecoffer-1250000000`。|  String |
| Encoding-Type | ListMultipartUploadsResult | 规定返回值的编码格式，合法值：url。 |  String |
| Marker | ListMultipartUploadsResult| 列出条目从该 Marker 开始。 |  String |
| NextMarker | ListMultipartUploadsResult | 假如返回条目被截断，则返回的 NextMarker 就是下一个条目的起点。 | String |
| MaxCount | ListMultipartUploadsResult | 设置最大返回的 multipart 数量，合法取值从0 - 1000。 |  String |

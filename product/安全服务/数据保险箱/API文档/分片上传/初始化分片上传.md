...
Initiate Multipart Upload 接口请求实现初始化分片上传，成功执行此请求后将返回 UploadId，用于后续的 Upload Part 请求。如果已存在同名的对象，则接口会返回对象已存在的错误。创建成功后，除非取消上传，否则将无法上传或分片上传同名的对象。
...
| 节点名称（关键字） | 父节点                        | 描述                                                         | 类型      |
| :----------------- | :---------------------------- | :----------------------------------------------------------- | :-------- |
| Coffer             | InitiateMultipartUploadResult | 分片上传的目标 Coffer，由用户自定义字符串和系统生成 AppID 数字串组成，例如 examplecoffer-1250000000。 | Container |
...
| ObjectAlreadyExistsOwnedByYou | 指定的对象已存在且由当前帐户创建（也可能是正在上传中或分片上传中）。 | 409 Conflict |
| ErrAssumeRoleFailed | 扮演角色失败，检查是否已授予保险箱角色。 | 409 Conflict |
| ErrKMSFailed | 调用KMS失败。 | 409 Conflict |

...
## 接口描述
- **接口名称**：Initiate Multipart Upload 
- **接口功能**：该接口请求用于实现初始化分片上传，成功执行此请求后将返回 UploadId，用于后续的 [Upload Part](https://cloud.tencent.com/document/product/1232/44742) 请求。如果已存在同名的对象，则接口会返回对象已存在的错误。创建成功后，除非取消上传，否则将无法上传或分片上传同名的对象。
...
| 节点名称（关键字）            | 父节点 | 类型      |描述             | 
| :---------------------------- | :----- | :--------------- | :-------- |
| InitiateMultipartUploadResult | 无     |Container | 说明所有返回信息。 | 

**Container 节点 InitiateMultipartUploadResult 的内容：**

| 节点名称（关键字） | 父节点                        | 类型      |描述                                                         | 
| :----------------- | :---------------------------- | :----------------------------------------------------------- | :-------- |
| Coffer             | InitiateMultipartUploadResult | Container |分片上传的目标 Coffer，由用户自定义字符串和系统生成 AppID 数字串组成，例如 examplecoffer-1250000000。 | 
| Key                | InitiateMultipartUploadResult | Container |Object 的名称。                                                | 
| UploadId           | InitiateMultipartUploadResult |Container | 在后续上传中使用的 ID。        | 
...
| ErrAssumeRoleFailed | 409 Conflict |扮演角色失败，检查是否已授予保险箱角色。 | 
| ErrKMSFailed | 409 Conflict |调用KMS失败。 | 

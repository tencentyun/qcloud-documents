...
本文主要介绍 SCF 和 Tencent CIAM 的集成业务流程，以及 SCF 中集成了 Tencent CIAM 的 Express 框架模版(Auth)的使用，有效帮助用户避免自行开发复杂认证系统所带来的开发成本。用户只需要在 SCF 模版创建并选择 Express 框架模版(Auth)，通过简单的配置操作即可拥有 Tencent CIAM 的认证能力。
...
## 配置流程
用户在 SCF 中通过选择 Express 框架模版(Auth)，可以快速集成 Tencent CIAM 提供的登录认证能力。接下来将为大家介绍如何快速创建、使用和集成 SCF 中的 Express 框架模版(Auth)，并预览集成认证后的效果，只需要以下四步就可以轻松完成：

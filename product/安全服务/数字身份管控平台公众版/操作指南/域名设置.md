## 操作场景
腾讯云证书服务支持快速在云资源中部署数字证书，如果用户需要设置自定义域名，为了将域名生效需要使用 HTTPS 协议，把 HTTPS 中使用的证书配置到我们服务器上。
>?如果不配置，则采用默认域名。


## 前提条件
填写的域名需要在工信部完成备案，否则将无法进行接入，如您的的域名尚未备案，可参考 [备案流程](https://cloud.tencent.com/document/product/243/18909) 进行域名备案。
...
3. 登录 [SSL 证书购买页](https://buy.cloud.tencent.com/ssl?fromSource=ssl)，根据实际需求选择证书种类及证书品牌、域名类型及支持的域名数目、证书年限，单击**立即支付**即可。

...
4. 在编辑页面，配置相关参数，单击**确定**即可。
>?实名认证服务配置支持不同的第三方实名认证服务，通过选择支持的实名认证服务，页面会动态加载该实名认证服务需要的配置信息。
>

### 开通天御实名认证服务
1. 目前数字身份管控平台（公众版）支持天御实名认证服务的身份证二要素（姓名+身份证号码）和运营商三要素（姓名+身份证号码+手机号码），租户如果需要使用这两项服务时，可在控制台单击**立即开通**。
![](https://qcloudimg.tencent-cloud.cn/raw/2f254ebd3ed366cc48d4ea2f599f0e2f.png)
...
>?如果是⼦账号使用腾讯天域实名认证服务，需要登录访问管理控制台，在 [策略页面](https://console.cloud.tencent.com/cam/policy) 为子账号进⾏授权：QcloudFIVFullAccess。详细操作请参见 [授权管理](https://cloud.tencent.com/document/product/598/10602)。
>
...
>?
>- 核验方式：为注册流程选择实名认证核验方式，包括二要素核验（姓名+身份证号码）、三要素核验（姓名+身份证号码+电话号码）。核验方式生效的前提是在模板设置中完成了实名认证服务配置，并且注册流程中的属性中包含实名核验所需要的字段。

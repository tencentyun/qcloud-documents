...
5. 在基本信息页面，会自动生成 Client ID、Client Secret、Token、User URL 和 Group URL 等信息，可参考 [调用方式](https://cloud.tencent.com/document/product/1441/62152) 将以上参数配置到您的客户端程序中进行调用。
>?
>- Client ID：客户端ID，用户获取 accessToken。
>- Client Secret：客户端秘钥，用户获取 accessToken。

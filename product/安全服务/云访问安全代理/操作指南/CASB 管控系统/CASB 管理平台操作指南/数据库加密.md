...
### 数据库加密流程
>?数据库加解密的方式：
>1. 通过 Proxy 登录数据库进行实时加解密。
>2. 通过客户端 Client 方式进行全量加解密。

#### 数据库加解密的前期准备
1. 登录 [云访问安全代理控制台](https://console.cloud.tencent.com/casb) ，然后单击左上角的区域下拉框，切换区域。
2. 创建 CASB 实例，详情请查阅 [**创建CASB实例**]() 。
3. 创建元数据，详情请查阅 [**创建云数据库**]() 或 [**创建自建数据库**]()。
4. 元数据创建完成后，对数据库表结构进行提取，提取完成后，单击 **使用** 操作。
>? 对数据库表结构进行提取和使用，意在将数据库中的表结构同步至CASB系统中。


 5. 登录 [CASB 管理平台]()。
 6. 登录到与 CASB 实例配置同一 VPC 环境的服务器。
 7. 打开浏览器，输入 `https://IP`（其中 IP 为 CASB 管理平台地址），进入登录页面。
 8. 输入账户名 admin 以及在 [CASB 控制台](https://console.cloud.tencent.com/casb) 设置的密码，单击【登录】，即可登录 CASB 管理平台。
  ![](https://main.qcloudimg.com/raw/a78170e0637145171de2b3853c740f45.png)
 9. 在 CASB 管理平台中设置加解密策略。
 10. 在 CASB 管理平台的“策略管理”页面，找到需要加密的应用，在右侧“操作”栏，单击【设置策略】。
 ![](https://main.qcloudimg.com/raw/0e54ac26298cf83ad7c5a3ecdae2c24c.png)
 11. 在该数据源下找到需要加密的表，在右侧“操作”栏，单击【设置策略】，进入表字段页面。
 ![](https://main.qcloudimg.com/raw/03ff86eda8e06e4b2b87a11745da3fa0.png)
 12. 在表字段页面，选中需要加密的字段，在右侧“操作”栏，单击【设置策略】，弹出加密算法窗口。
 ...
 14. （可选）如需还对其他表或字段设置策略，重复上述操作即可。

#### 配置Proxy实现实时加解密
1. 配置 Proxy，绑定元数据。
 >? 绑定的方式：
 >① 在 [CASB 控制台](https://console.cloud.tencent.com/casb) 的左侧导航菜单栏中，单击 **实例** > **实例列表**> 单击 **ID/名称** ，在其 “Proxy资源”界面中选择绑定。
 >② 在 [CASB 控制台](https://console.cloud.tencent.com/casb) 的左侧导航菜单栏中，单击 **元数据管理** > **关系型元数据** > 单击 **元数据ID/名称** ，在其 “元数据详情” 界面中绑定Proxy。
2. 创建代理账号：
在 CASB 控制台的左侧导航栏中，单击  **实例** > **代理账号**，在左上角切换您所需的区域，单击 **新建代理账号**。
...
| 元数据账号   | 选择关联的元数据对应的账号。              |
| 代理账号名   | 创建用于 Proxy 连接时的账号。               |
| 代理账号密码 | 创建用于 Proxy 连接时的密码。               |
| 描述         | 代理账号的用途描述。（可选）              |

3. Proxy 配置完成后，登陆与 Proxy 同 vpc 的 cvm，使用数据库命令行工具连接 Proxy，即可实现实时加解密。
...
2. 下载安装客户端工具
路径：在 CASB 管理平台的右上角，单击【下载客户端工具】，下载客户端工具。（如下图）
![](https://main.qcloudimg.com/raw/13aedcf473a307a20bfe82cc1aa86b7d.png)
3. 将下载后客户端工具 zip 包，上传到数据库 Linux 服务器或可以连接到数据库的 Liunx 服务器上的任意目录下，以 root 用户执行以下安装命令：
```shell
# 切换root用户
~$ su root
# 解压压缩包，视包名有所不同

...
其中：
- len 为明文数据长度。
>?数据是中文时，len乘以3，数据是英文时，len 不变。
- ceil() 为向上取整
- length 为计算得到的密文长度

>?在本次示例中，用户表中的`user_name`作为敏感信息进行加密。根据计算公式，我们需要将`user_name`的字段长度由原来的20个字符长度增加到131个字符长度。
...
# 验证数据库连接
[root@localhost extractor]# java -jar extractor-1.1.6.jar --op=verify --db-type=mysql --host=192.168.xx.xxx --port=3306 --user=root --password=hellocasb --database=ceshi
# 如下是提取过程中打印的日志，可做参考
--op=verify
--db-type=mysql
--host=192.168.xx.xxx 
...
# 提取表结构
[root@localhost extractor]# java -jar extractor-1.1.6-release.jar --op=extract --db-type=mysql --host=192.168.xx.xxx --port=3306 --user=root --password=hellocasb --database=ceshi
# 如下是提取过程中打印的日志，可做参考
--op=extract
--db-type=mysql
--host=192.168.10.130
...
[2020-08-25 09:49:40] 当前占位符#{ip}所指ip为:10.1.1.141
[2020-08-25 09:49:40] [completed] mysql 192.168.xx.xxx:3306 database:ceshi user:root
```
...
$ sh aoe-plugin.sh  /opt/casb/plugin
# 加载环境变量
$ source /etc/profile
```

...
>? 在使用云访问安全代理元数据功能时，会调用 VPC、云产品（MySQL/TDSQL/MariaDB）、CVM 及 LB 的资源，故子账号在进去权限时，需同步关联如下预设策略：
>- QcloudTAGFullAccess	标签（TAG）全读写访问
...
>- QcloudTDSQLFullAccess	TDSQL MySQL 版（TDSQL for MySQL）全读写访问权限

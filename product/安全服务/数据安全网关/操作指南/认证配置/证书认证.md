...
4. 填写匹配起始字符及匹配结束字符
>?匹配字符为个人证书里使用者字段，例如1：使用者处 cn=test，则配置匹配起始字符为 cn=，匹配结束字符为`,`）

![](https://main.qcloudimg.com/raw/70c8807b5dccea8e4233598175e74cdb.png)

5. 上传根证书。
单击【浏览】选择证书文件 > 导入第三方证书 > 单击【上传】
...
#### 登录
1. 打开数据安全网关登录页面。
2. 选择第三方签发证书认证，弹出个人证书（个人证书需要提前将 USBKey 插上或软 key 导入到IE里），选择相应证书进行登录。
![](https://main.qcloudimg.com/raw/dcf4b7f440bc7bd1e6fa1a55b6b8b01f.png)

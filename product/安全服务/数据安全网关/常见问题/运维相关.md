...
#### 运维端登陆
1. 打开远程工具，例如 Xshell，输入远程堡垒命令，格式如：`ssh user@ip`，user 为堡垒机的页面登陆用户，回车后输入密码，如下图：
![](https://main.qcloudimg.com/raw/b3d9503ef4437cbfbccb8f4615835167.png)
2. 登录后展示个人岗位和管理员授权的岗位，如下图所示。根据展示的岗位序号输入序号进入授权列表，例如：输入1，（进入【运维岗】）>再次输入1 ，（选择单点登录资源） >输入s，（选择 ssh 连接方式） 后单点登录到资源192.168.24.46。至此完成客户端 ssh 工具直连单点登录过程。

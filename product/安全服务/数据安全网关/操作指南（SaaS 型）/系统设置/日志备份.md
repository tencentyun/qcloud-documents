## 操作场景
当需要备份堡垒机日志时，可通过日志外发功能将日志进行备份。下面将为您详细介绍如何在堡垒机当中配置日志备份功能。
...
4. 单击**确定**，即可保存设置。启用日志备份之后，将实时对日志记录进行备份，备份的日志可到 T-Sec 安全运营中心 SOC 高级版查询。
>?在 T-Sec 安全运营中心 SOC 高级版当中，堡垒机备份日志类型字段为 Type, 取值 AccessDevice 为资产登录日志, OpUser 为产品登录日志。

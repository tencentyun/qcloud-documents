ZMK、ZPK 和 ZAK 都是在 EVSM 外部存储的，应用密钥是经过 LMK 分组加密保护后存储在外部系统中，只需要同步设备的 DMK 即可。同步 DMK，需要原始初始化时产生 DMK 成份 UKEY。当多 EVSM 备份时，则在第一台设备上完成原始初始化后，对其他的设备进行恢复初始化操作，可完成多台 EVSM 的设备主密钥同步。

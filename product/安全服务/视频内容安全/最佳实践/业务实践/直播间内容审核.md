...
5. 在管理自定义库配置页面，在自定义词库下拉列表中选择是否需要使用自定义词库用于内容审核，如暂无自定义库，可单击**下一步**跳过，或保存当前策略后，前往 [步骤3：配置自定义词库](#step3)。
...
## 步骤4：创建视频内容安全服务 
在完成以上步骤后，可以调用**创建视频审核任务（CreateVideoModerationTask）**接口创建直播间视频审核任务，具体方法如下： 
...
## 步骤5：获取视频内容安全任务结果
在创建视频审核任务之后，可以调用**查看任务详情（DescribeTaskDetail）**接口查询任务的详细信息，具体方法如下： 

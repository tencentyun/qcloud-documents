...
### 1. 接入信息申请分配
合作方按照模板申请接入的 APPID、Secret 和 SDK license。 按照 [登录鉴权流程](https://cloud.tencent.com/document/product/295/10117?!preview&lang=cn) 开始的接入描述获取 Token 并生成签名。
> **注意**：
> APPID 和 Secret 务必保密，一旦泄露，将会导致服务不可用。后果由合作方自行承担。

### 2. 签名鉴权
合作作伙伴服务端根据 [登录鉴权流程](https://cloud.tencent.com/document/product/295/10117?!preview&lang=cn) 开始的接入描述获取 Token 并生成签名。
> **注意**：

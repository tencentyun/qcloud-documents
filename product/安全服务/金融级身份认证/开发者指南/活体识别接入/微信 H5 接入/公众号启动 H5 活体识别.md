...
| version    | 参数值为：1.0.0                     |                                          |
| api ticket | 合作伙伴服务端缓存的 tikcet，注意是 NONCE 类型 | 获取方式见 [NONCE ticket 获取](https://cloud.tencent.com/document/product/655/13816)(所用的userid参数值需要和接口里面的userid值保持一致) |
| nonce      | 随机数：32 位随机串（字母 + 数字组成的随机数）     |合作方自行生成(和接口里面的随机数保持一致)                                  |
...
| 参数          | 说明                                       | 类型     | 长度    | 是否必填                                     |
| ----------- | ---------------------------------------- | ------ | ----- | ---------------------------------------- |
| webankAppId | WebankAppId，由腾讯指定                        | String | 由腾讯指定 |       必填                           |    

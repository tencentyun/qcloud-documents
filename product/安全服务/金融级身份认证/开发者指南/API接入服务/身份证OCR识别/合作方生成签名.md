## 准备步骤
- 前置条件：请合作方确保 NONCE ticket 已经正常获取，获取方式见 [NONCE ticket](https://cloud.tencent.com/document/product/655/13816) 获取 。
...
| nonce      | 随机数</br>32 位随机串（字母 + 数字组成的随机数）    |
| version    | 1.0.0                       |
| api ticket | 合作伙伴服务端缓存的 tikcet，注意是 SIGN 类型 |

## 基本步骤
1. 生成一个 32 位的随机字符串（字母和数字） nonce（登录时也要用到），将 webankAppId、orderNo,version、连同 ticket、nonce 共 5 个参数的值进行字典序排序。

...
**自带比对源比对** 类型，合作方给 SDK 送上自己提供对比源数据进行对比。合作方可以上送两类照片，一类是网纹照，一类是高清照；照片需要转化为经过 base64 编码后的 String 来上送。图片大小不可超过 500k，经过编码后的图片 String 大小不可超过 1MB。上送照片类型与上送照片 String 两者缺一不可，否则将不使用上送的数据源。不自带对比源的合作方可以不上送此两个字段。上送的代码设置如下：

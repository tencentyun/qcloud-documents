...
| version    | 1.0.0                                    |
| api ticket | 合作伙伴服务端缓存的 tikcet，注意是 SIGN 类型，具体参考 [SIGN ticket 获取]() |

合作方需要将 webankAppId, orderNo, name, idNo, userId, version, ticket（SIGN 类型）共七个参数的值进行字典序排序。将排序后的所有参数字符串拼接成一个字符串进行 SHA1 编码，编码后的 40 位字符串作为签名 (sign)。签名算法可参考 [签名算法说明]() 。

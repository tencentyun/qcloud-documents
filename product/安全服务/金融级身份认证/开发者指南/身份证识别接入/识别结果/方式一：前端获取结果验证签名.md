...
1. 合作伙伴 APP 端身份证识别 SDK 返回的带签名结果。
...
合作方需要将 app_id、order_no、连同 ticket（SIGN类型）共三个参数的值进行字典序排序。将排序后的所有参数字符串拼接成一个字符串进行 SHA1 编码，编码后的40位字符串作为签名（**sign**）。签名算法可参考 [签名算法说明](https://cloud.tencent.com/document/product/655/13817) 。

...
### 身份证识别结果类
身份证识别结果, 封装在 EXIDCardResult 类中，通过 WbCloudOcrSDK.getInstance().getResultReturn()
获得，该类属性如下所示：
...
## 接口参数说明
InputData 是用 来 给 SDK 传 递 一 些 必 须 参 数 所 需 要 使 用 的 对 象（WbCloudOcrSdk.init() 的第二个参数），合作方需要往里塞入 SDK 需要的一些数据以便启动 OCR SDK。
...
| clientIp          | 用户 ip 信息                  | String     | 30         | 必填，格式为 “ip=xxx.xxx.xxx.xxx;”<br/>示例：“ip=58.60.124.0” |

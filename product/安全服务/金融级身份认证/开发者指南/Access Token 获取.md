### 注意事项
- 所有场景默认采用 UTF-8 编码。
- Access Token 必须缓存在磁盘，并定时刷新，建议每 1 小时 50 分钟刷新新的 Access Token，原 Access Token 两小时 (7200S) 失效，期间两个 Token 都能使用。
...
- expire_in 为 access_token 的最大生存时间，单位秒，合作伙伴在判定**有效期时以此为准**。

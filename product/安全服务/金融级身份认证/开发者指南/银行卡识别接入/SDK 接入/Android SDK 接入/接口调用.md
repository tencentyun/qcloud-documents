...
### 银行卡识别结果类
银行卡识别结果, 封装在 EXBankCardResult 类中，通过 WbCloudOcrSDK.getInstance().getBankCardResult()
) 获得，该类属性如下所示：
...
```
## 接口参数说明
InputData 是用 来 给 SDK 传 递 一 些 必 须 参 数 所 需 要 使 用的对象（WbCloudOcrSdk.init() 的第二个参数），合作方需要往里塞入 SDK 需要的一些数据以便启动 OCR SDK。
其中 InputData 对象中的各个参数定义如下表，请合作方按下表标准传入对应的数据。

| 参数       | 说明               | 类型 | 长度 | 是否必填                           |
| ----------------- | ------------------------- | ---------- | ---------- | ---------------------------------------- |
| orderNo           | 订单号                       | String     | 32         | 必填，合作方订单的唯一标识                            |
| openApiAppId      | 腾讯服务分配的app_id             | String     | 腾讯服务分配     | 必填，腾讯服务分配的 app_id                         |
...
| clientIp          | 用户 ip 信息                  | String     | 30         | 必填,格式为 “ip=xxx.xxx.xxx.x xx;”<br/>示例：“ip=58.60.124.0” |
| gps               | 用户 GPS 信息                 | String     | 30         | 必 填 , 格 式 为 “lgt=xxx;lat=xxx;”<br/>示例：“ lgt=22.5044;lat=113.9537“ |
...
[上一步：配置流程](https://cloud.tencent.com/document/product/655/14093)

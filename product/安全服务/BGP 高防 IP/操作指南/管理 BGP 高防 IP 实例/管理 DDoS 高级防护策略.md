BGP 高防 IP 提供面向 DDoS 攻击的高级防护策略功能，用户可针对自身业务防护需求对 DDoS 防护策略进行调整和优化。通过黑白名单、禁用协议、禁用端口、报文特征过滤策略、空连接防护以及是否开启水印防护，为业务提供针对性防护。
...
![](https://main.qcloudimg.com/raw/86fedce2e4cc3747d3dd5bb14831ecf7.png)
-  **连接耗尽防护**
...
- **异常连接检测**
 -  **源IP最大异常连接数**：可填范围 0-∞（单位：个）。表示当一个源 IP 符合异常连接行为识别的连接数，超过所指定阈值时，会被认为是异常攻击源，在一定时间内被限制访问。

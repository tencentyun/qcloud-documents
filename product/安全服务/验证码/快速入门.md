...
>?如果您已经具有腾讯云密钥可以跳过这一步。
### **步骤3：客户端接入**
目前验证码支持的 [Web 前端接入](https://cloud.tencent.com/document/product/1110/36841)。
### **步骤4：服务器接入**
服务器通过 HTTPS 接口提供，对环境无要求。请参见 [核查验证码票据结果](https://cloud.tencent.com/document/product/1110/36926)。
### **步骤5：查询调用数据**
登录 [验证码控制台](https://console.cloud.tencent.com/captcha) 通过验证码数据看板，查询调用数据。
### **步骤6：验证码配置**
验证码后台提供验证码配置功能，通过配置中心可以调整验证码安全、外观、告警相关的配置，请登录 [验证码控制台](https://console.cloud.tencent.com/captcha) 进行操作。
验证码前后端调用时序图如下：

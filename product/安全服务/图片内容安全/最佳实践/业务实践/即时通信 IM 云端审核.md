即时通讯 IM 云端审核功能，是天御内容安全产品，在云端进行内容智能识别和拦截的服务实践。
...
>!开通服务后，默认为您开启图文审核场景，并配置默认审核策略，您可通过单击对应模块的**编辑**按钮，分别更改审核场景和审核策略。
...
### 审核策略配置
每个审核场景及内容类型均已配置默认审核策略。如果需要编辑审核策略，请单击对应场景及内容类型右侧的**编辑策略**。

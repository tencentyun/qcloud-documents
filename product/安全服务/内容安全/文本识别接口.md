...
| messageStruct | 必选 | String | 用户产生的内容，详情请参见下方的消息结构体介绍 |
...
消息结构体用于存储用户输入的文本、图片、视频和系统表情等内容，使用 TLV 格式存储，其中 Type 是 4 个字节，Length 是
4 个字节，Value 的值通过 Length 指定，Type 和 Length 都是网络字节序，其中所有的中文字符均为 UTF-8 编码。目前支持的
Type 类型如下：
...
```
1. http://csec.api.qcloud.com/v2/index.php?Action=UgcAntiSpam
2. &accountType=1
3. &appId=100273020

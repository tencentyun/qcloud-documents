...
### 域名回源支持哪些方式？
支持域名回源和 IP 回源，您可以根据需要进行选择和配置。
### 域名接入 WAF 之后，如何绑定 CNAME？
您可以参考 CNAME 文档中的操作说明，在您的 DNS 服务商处绑定 CNAME。
### 域名列表 WAF 开关关闭后，还会记录日志吗？
WAF 的开关关闭后，WAF 所有的防护功能将会关闭，并进入纯流量转发模式，且不会记录日志。
### 域名删除后重新添加， CNAME 会发生变化吗？
域名删除重新添加，CANME 会发生变化，可在 [WAF 控制台域名列表](https://console.cloud.tencent.com/guanjia/waf/config) 中，单击域名，在基础设置中查看。
![](https://main.qcloudimg.com/raw/3f25df1b44fd82c266d453ec9203a8fa.png)

### 域名的 VIP 是否可以更换？
原则上域名的 VIP 不能进行变更，如果 WAF 服务出现异常，我们会及时为您进行 VIP 切换。如遇到异常，您可及时 [提交工单](https://console.cloud.tencent.com/workorder/category?level1_id=141&level2_id=642&source=0&data_title=T-Sec-Web%E5%BA%94%E7%94%A8%E9%98%B2%E7%81%AB%E5%A2%99&level3_id=867&radio_title=%E6%8E%A7%E5%88%B6%E5%8F%B0%E9%97%AE%E9%A2%98&queue=15&scene_code=29995&step=2) 联系我们协助您处理。
### WAF 防护域名的 VIP 因为 DDoS 封堵如何处理？
WAF 的 VIP 已经进行 DDoS 基础防护（默认2G），如果在 DDoS 基础防护中发生封堵，每月拥有三次自助解封机会。如急需恢复业务有如下两种方式：
- 登录 DDoS 防护产品控制台 进行自助解封。
- 购买 DDoS 高防包实例绑定到 WAF 的 VIP。

每月有三次自助解封机会，当月超过三次后将无法进行解封操作。系统将在每月第1天零点时，重置自助解封次数，当月未使用的解封次数，不会累计到次月。
 
### 若已经接入 CDN 域名在 WAF 如何接入？
若已经接入 CND 域名，将 WAF 为域名分配的 CNAME 地址作为 CDN 的源站即可，流量按照用户 > CDN > WAF > 负载均衡 > 源站的架构回源。同时您可以登录 WAF 控制台，在 [添加域名页面](https://console.cloud.tencent.com/guanjia/waf/config/add)，将代理情况勾选为是，此时 WAF 会根据 HTTP 头部中的 XFF 字段，获取客户端的真实 IP，保证 WAF 正常防护。

...
![](https://qcloudimg.tencent-cloud.cn/raw/58b02d8766fd77274ec4bc4f840a5abb.png)
 ...
 - **请求方法**：HTTP 请求方法。
 - **所属域名**：当前 API 的归属域名。
 - **功能场景**：由 API 管控识别出来的相关业务的功能场景、如验证码、回调等，如果识别出的数据不准确， 可以在 [API 详情](#xq)v 中进行相关修正。
 - **涉敏字段**：中由 API 管控识别出来的传输参数上存在敏感字段的字段内容，包括但不限于银行卡号、身份证号等数据字段，如果识别出来的数据类型不准确，可以在 [API 详情](#xq) 中进行相关修正。

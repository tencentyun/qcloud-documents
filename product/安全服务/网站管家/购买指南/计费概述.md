...
| BOT行为管理                                                  | 费用           |
| :----------------------------------------------------------- | :------------- |
| <li>BOT 行为管理拥有威胁情报、前端对抗、AI 评估、智能统计、会话管理等功能。</li><li>可针对访问请求流量进行量化评估恶意风险程度，自动发现恶意  BOT 并自动进行处置。</li><li>新增会话管理，通过配置会话管理，BOT 行为管理可区分识别 IP 下的不同用户，对 IP 下的特定用户进行处置，不影响其他用户。 | 8000元/实例/月 |

>! 
>- BOT 行为管理开启仅针对购买的实例开启。
>- 对历史企业版及以上用户，可开启 BOT 流量分析（不处置 BOT 行为）。

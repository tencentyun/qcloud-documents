...
| 安全日志服务包详情                                                   | 费用      |
| :----------------------------------------------------------- | :-------- |
| 一个日志服务包包含：<li>1T 日志服务存储容量：提供防护域名在用户自定义的日志保存天数内（1～30天）的访问日志查询和下载功能；**如果用户设置的日志保存天数对应的日志量超出 1T 的存储容量，则仅在 1T 的存储容量内提供防护域名的访问日志查询和下载功能。**<li>180天的访问日志存储功能：提供防护域名180天的访问日志存储功能；**如果日志存储在 COS 存储服务中，用户无法在安全日志服务包的控制台查询或下载日志、仅可向腾讯云[ 提交工单](https://console.cloud.tencent.com/workorder/category) 下载数据。** | 500元/月 |

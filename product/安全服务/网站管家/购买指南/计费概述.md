...
### 日志投递服务价格说明
日志投递服务包含：传输计费、投递计费、WAF 记录业务访问日志并写入存储（数据落盘）。按量计费周期为天，即当日所产生用量的费用会在次日00:00点时开始结算。目前支持根据用户用量进行阶梯定价，具体价格如下表所示：

| 日投递量（GB）           | 费用（元/GB） |
| ------------------------ | ------------- |
| 日投递量 <= 30GB         | 0.032         |
| 30GB < 日投递量 <= 100GB | 0.016         |
| 100GB < 日投递量         | 0.01          |

>!
>- 投递日志服务功能记录的日志为未压缩格式，记录值与 CLS 平台不同。

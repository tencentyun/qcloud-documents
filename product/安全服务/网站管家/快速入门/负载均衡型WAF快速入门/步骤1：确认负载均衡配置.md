负载均衡型 WAF 通过添加域名和负载均衡监听器进行绑定，实现对经过负载均衡监听器的 HTTP 或 HTTPS 流量进行检测和拦截。在接入负载均衡型 WAF 前，请确保网站业务已经在腾讯云上，并且使用了腾讯云负载均衡（原应用型负载均衡，网络类型为公网类型）。若您的网站业务不在腾讯云上，建议您使用 SaaS 型 WAF 接入防护。
...
本文以防护`wow.qcloudwaf.com`为例，查看负载均衡监听器配置信息。

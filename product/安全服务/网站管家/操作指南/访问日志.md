## 功能简介
访问日志功能用于记录 Web 应用防火墙防护域名的访问日志信息，在已购买安全日志服务包的日志存储容量范围内提供防护域名在用户自定义的日志保存天数内（1～30天）的访问日志查询和下载功能；**如果用户设置的日志保存天数对应的日志量超出用户购买的存储容量，则仅在用户购买的存储容量内提供防护域名的访问日志查询和下载功能**。

并提供防护域名180天的访问日志存储功能；**如果日志存储在 COS 存储服务中，用户无法在安全日志服务包的控制台查询或下载日志、仅可向腾讯云 [提交工单](https://console.cloud.tencent.com/workorder/category) 下载数据。**
>!

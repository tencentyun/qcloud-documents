...
## 配置案例
1. 登录 [Web 应用防火墙（网站管家）控制台](https://console.cloud.tencent.com/guanjia)，单击【BOT 行为管理】>【BOT 概览】，可通过页面左上方的下拉框选择域名。该页面展示的是所选域名的请求次数统计图和 BOT 类型比例图（以测试域名为例，图中红色标识 BOT 请求，绿色标识总请求，正常请求和 BOT 请求之和）。
####  温馨提示：

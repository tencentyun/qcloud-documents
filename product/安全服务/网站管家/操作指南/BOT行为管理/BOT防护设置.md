## 功能简介
通过 BOT 防护配置，用户可以根据 BOT 会话特征和行为特征设置 BOT 对抗策略，对 BOT 行为进行动作处理。结合 BOT 详情观察结果，根据详情提供的会话状态信息进行精细化策略设置，保护网站核心接口和业务资产安全。BOT 防护设置支持以下两大类防护策略。
### 公开类型
WAF 提供12个已知公开 BOT 大类，超过1000+的 BOT 种类，包括搜索引擎、测速工具、内容聚合、扫描和网页爬虫等类别。用户可以根据需求对公开 BOT 类别设置防护动作，WAF 将对命中公开类型的 BOT 请求进行放行、监控或拦截处理。
### 自定义会话策略
WAF 自定义会话策略提供协议特征、IP 情报特征和自定义会话特征三种类型，每种类型特征提供多种判定维度，您可以根据业务实际情况和 BOT 情况信息，设置自定义会话策略和动作处理。WAF 将命中的自定义防护策略的 BOT 请求进行动作处理，动作处理支持：放行、监控、验证码、重定向和拦截。
...
- **BOT 流量分析开关：**默认关闭，可以根据需要开启，当且仅当域名 WAF 开关为开启时，BOT 流量分析功能生效。
...
**参数说明：**
	...
	- **动作：**公开类型支持的动作类型，默认为监控，支持设置为放行或拦截，可通过右侧操作栏进行设置。监控或拦截结果可在【[攻击日志](https://console.cloud.tencent.com/guanjia/waf/attack)】中查看。
...
![](https://main.qcloudimg.com/raw/d9779127d93f5c8b1640d68a436c72ad.png)
	- **字段说明：**
		...
		- **策略开关：**默认为关闭。
		- **修改时间：**策略最近一次被修改的时间。
		- **操作：**单击【编辑】进行动作设置，动作支持放行、监控、验证和拦截，详情请参见 [动作类型说明](#dzlx)。动作设置后，会话请求命中策略将直接生效，结果可在【[攻击日志](https://console.cloud.tencent.com/guanjia/waf/attack)】中查看。
	- **协议特征策略名称如下：**
<table>
<tr><th>协议特征类别</th><th>策略名称</th></tr>
<tr><td rowspan="7">User-Agent 类别</td><td>User-Agent 为空或不存在。</td></tr>
<tr><td>User-Agent 类别 为 BOT。</td></tr>
...
<tr><td>Cookie 空或不存在。</td></tr>
<tr><td>Connection 空或不存在。</td></tr>
<tr><td>Accept 空或不存在。</td></tr>
<tr><td>Accept-Language 空或不存在。</td></tr>
<tr><td>Accept-Enconding 空或不存在。</td></tr>
...
![](https://main.qcloudimg.com/raw/1453fca7d60c9922a5eea34b4f93264a.png)
	- **字段说明：**
		- **序号：**策略自增序号。
		- **策略名称/描述：**策略名称和描述信息。
		- **匹配条件：**策略匹配条件内容，一条策略最多可以添加是个匹配条件，匹配条件之间是“与”的关系。
...
![](https://main.qcloudimg.com/raw/5d2223f6292b4ed820f014256296bd52.png)
	- **字段说明：**
		- **策略名称：**策略名称字段。
		- **策略描述：**策略描述字段。
		- **策略开关：**策略开关状态，默认开启。
...
<tr><td>拦截</td><td>符合匹配条件的会话请求将执行拦截，可通过攻击日志查看拦截情况，惩罚时长：默认为30分钟，可设置5分钟 - 10080分钟（7天）。</td></tr>
</table>
	- **自定义会话特征匹配条件说明如下：**
	<table>
<tr><th>分类</th><th>过滤条件</th><th>条件说明</th></tr>
...
<tr><td rowspan="5">请求特征</td><td>请求最多的 URL</td><td>请求最多的 URL</td></tr>
<tr><td>URL 重复比</td><td>会话请求中 URL 重复比比例，取值范围0 - 1，根据实际业务情况，进行参数配置，过高或过低为疑似异常（根据实际情况进行判断）。</td></tr>
...
<tr><td>UA 种类</td><td>会话请求中 COOKIE 去重后的数目，过多疑似异常（根据实际情况进行判断），对非代理 IP 有效。</td></tr>
<tr><td>UA 类型</td><td><ul><li>browser   UA类型为浏览器。</li>
<li>mobile   UA 类型为移动端。</li>
<li>gameortv  UA 类型游戏终端或电视终端。</li>
<li>bot  UA 类别为公开 BOT 类型。</li>
<li>bot-unknown  UA 类别为未公开 BOT 类型。</li>
<li>tools   UA 类别为自动化工具。</li>
<li>unknown  UA 类别为未知类型。</li>
<li>scanner  UA 类别为公开扫描器。</li>
<li>frmework  UA 类别为开发框架。</li>
<li>http libray  UA 类别为语言 HTTP 库。</li><ul></td></tr>
<tr><td>UA 随机性指数 </td><td>会话请求中 UA 的随机分布情况，取值范围0  -1，指数越高越异常。参考值阈值超过0.6疑似异常，指数超过0.92基本确定为异常。</td></tr>
...
<tr><td>请求方法 </td><td>会话请求中判断请求使用方法。</td></tr>
<tr><td>请求协议版本比例</td><td>会话请求中判断请求使用的 HTTP 协议版本比例。
</td></tr>
<tr><td>返回状态码比例</td><td>会话请中 WAF 返回给客户状态码比例。
</td></tr>
<tr><td rowspan="7">高级特征</td><td>预测标签</td><td><ul>算法自动预测的疑似行为，不一定所有会话都有预测值，预测标签可能结果如下：
<li>crawler unregular 疑似无规律爬虫。</li>
<li>crawler regular 疑似规律性爬虫。</li>
<li>credential miss user  疑似登录行为，缺失用户值。</li>
<li>credential without user  疑似登录行为，缺失用户参数。</li>
<li>credential only action  疑似登录行为，缺失用户和密码。</li>
<li>credential user password 疑似登录行为，缺失登录动作。</li>
<li>credential cracking 疑似暴力破解。</li>
<li>credential stuffing 疑似撞库攻击。</li>
<li>brush sms 疑似刷短信接口。</li> 
<li>brush captcha 疑似刷验证码接口。</li> 
<li>reg malicious 疑似恶意注册。</li> 
<li>request repeat 疑似接口重复访问。</li> </ul></td></tr>
<tr><td>BOT得分</td><td>BOT 智能分析引擎对会话给出的 BOT 得分，得分越高判定为 BOT 可能性越大，参考值为：5。</td></tr>
...
<tr><td>时序行为异常指数</td><td>一种时序行为异常检测算法，指数越小越异常，参考值阈值小于0.5疑似异常，小于0.24基本确定为异常。</td></tr>

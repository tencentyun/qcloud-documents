...
## 操作指南
登录 [Web 应用防火墙控制台](https://console.cloud.tencent.com/guanjia/waf/api)，在左侧导航栏中，选择【Web 应用防火墙】>【API 安全】，进入 API 安全页面。
###  添加 API
1. 在 [API 安全](https://console.cloud.tencent.com/guanjia/waf/api) 页面，单击【添加】，即可进入 API 安全策略添加页面，您可根据网站的 API 策略，添加对应的 API 安全规则。
![](https://main.qcloudimg.com/raw/fdf5a5ba7ed1b43b31aefac230e88ed0.png)
 **字段说明**：
	- **接口名称**：添加 API 路径，以‘/’开头，注意接口名称+请求方式不能重复。
	- **描述**：选填项，API 策略的相关描述。
	- **接口开关**：API 安全策略开关，默认为关。当开启开关后，才能对相应 API 策略进行解析，执行后续的观察和拦截动作。
	- **请求方式**：目前 API 安全支持四种请求方式：GET、POST、PUT 及 DELETE 方式。
	- **匹配条件**：需要您添加五个参数，分别为参数名称、参数位置、参数类型、是否必填以及备注（可填项），参数位置目前支持 body、query 及 path 三种类型。
	- **执行动作**：目前支持两种动作，观察和拦截模式。
	>?建议您开启 API 安全功能时，先开观察模式，对攻击日志进行查看，观察一段时间后，再开启拦截模式。
2.	添加完成后，单击【确定】，即可将 API 接口添加至 API 安全防护配置列表中。
...
>- 导入创建成功的 API ，可进行编辑，请根据时间情况进行接口确认。
>

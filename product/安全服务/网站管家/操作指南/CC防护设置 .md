...
>?	**封禁访问：**流量触发 CC 规则，若执行动作为封禁访问，则源 IP 所有对网站的访问将被禁止一段时间。
**人机识别：**流量触发 CC 规则，若执行动作为人机识别，人机识别是判定访问源是否为机器访问的一种认证方式，则认证通过的访问源 IP 可以继续正常访问，认证失败的访问源 IP，CC 攻击将会执行封禁访问动作，该 IP 所有对网站的访问将会禁止一段时间。
	- 惩罚时长：30分钟。

...
- CNAME 记录解析：将域名指向另一个域名，再由另一个域名来提供 IP 地址。
 - 记录类型选择 “CNAME”。
 - 记录值填写网站管家 WAF 添加防护域名后的 CNAME 值。
 -  MX 优先级不需要设置。
 -  TTL 设置默认600。
...
## VIP 地址
VIP 地址（Virtual IP Address）指客户添加域名成功后，网站管家 WAF 根据客户添加的域名，自动分配一个对应的 VIP 地址。当客户源站被访问时，VIP 地址成为网站管家 WAF 入口地址，访问流量经过域名解析到 VIP 地址后，会全部导向网站管家 WAF。

## 回源 IP 地址
回源 IP 地址（Return Source IP Address）指客户添加域名成功后，网站管家 WAF 根据客户添加的域名，自动分配多个回源 IP 地址，回源 IP 地址作为网站管家 WAF 的出口 IP，把经过过滤的正常访问流量，导向客户源站。
...
## AI 引擎
[AI 引擎（Artificial Intelligence Engine）](https://cloud.tencent.com/document/product/627/18380)指腾讯云网站管家 WAF 率先应用基于机器学习的 Web 攻击检测技术，通过 AI 引擎的自学习、自进化和自适应能力，最大限度提高已知和未知 Web 威胁的检测率和捕获率，最大限度减少误报，并且灵活适应不断变化的 Web 应用。

...
| /4/w/&lt;LongEdge>/h/&lt;ShortEdge> | 限定缩略图的长边和短边的最小值分别为 LongEdge 和 ShortEdge，进行等比压缩；如果只指定一边，代表另外一边为同样的值<br>当图片长宽相同时，默认长边为图片的高 |
| /5/w/&lt;LongEdge>/h/&lt;ShortEdge> | 限定缩略图的长边和短边的最大值分别为 LongEdge 和 ShortEdge，进行等比压缩，居中裁剪；如果只指定一边，则表示宽高相等的正方形；缩放后其中一边多余的部分会被裁剪掉<br>当图片长宽相同时，默认长边为图片的高 |

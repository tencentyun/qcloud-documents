...
>?
- COS 简单上传文件接口，详细请参见 [COS 文档](https://cloud.tencent.com/document/product/436/7749)。
...
| fileid   | String | 是   | 处理结果的文件路径名称，如以`/`开头，则存入指定文件夹中，否则存入原图文件存储的同目录 |
...
| 参数      | 类型    | 必选   | 描述                                                         |
| --------- | ------- | ------ | ------------------------------------------------------------ |
| cover | Int | 否 | 二维码覆盖功能。   可为0或1。0表示不开启二维码覆盖；1表示开启二维码覆盖。功能开启后，会对识别出的二维码覆盖上马赛克，默认值0 |

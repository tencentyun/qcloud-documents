...
| ------------------  | ------- | -------------------------------------------------------- | --------- | ---- |---| ---- |
| ConcatFragment      | Request.ConcatTemplate | 拼接节点    | Container    | 否   | 无  | 无 |
...
| AudioMix            | Request.ConcatTemplate | 混音参数    | Container    | 否   | 无  | 无 |

Container 类型 ConcatFragment 的具体数据描述如下：

| 节点名称（关键字）     | 父节点  | 描述                                                     | 类型      | 必选 | 默认值       | 限制  |
| ------------------  | ------- | -------------------------------------------------------- | --------- | ---- |---| ---- |
| Url                 | Request.ConcatTemplate.<br/>ConcatFragment | 拼接对象地址(需要Urlencode后传入)   | String    | 是   | 无   | 同bucket对象文件 |
...
| Crf                        | Request.ConcatTemplate.Video | 码率-质量控制因子     | String | 否   | 无           | <li>值范围：(0, 51]<br/><li>如果设置了Crf, 则Bitrate的设置失效<br/><li>当Bitrate为空时,默认为25 |

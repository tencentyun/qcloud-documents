...
| AudioMix           | Request | 混音参数                                                 | Container    | 否   | 1.仅在Audio.Remove为false时生效 |
...
| ----------------- | ------------------------------ | ------------ | ------ | ---- | ------ | ---------------------------- |
| AudioSource       | Request.AudioMix | 混音文件地址  | String | 是   | 无  | 需与Input媒体文件存储于同一bucket |
| MixMode           | Request.AudioMix | 混音模式  | String | 否   | Repeat| 1. Repeat:背景音循环<br/>2. Once:背景音一次播放|
| Replace           | Request.AudioMix | 是否用混音音轨媒体替换Input媒体文件的原音频  | String | 否   | false  | true/false|

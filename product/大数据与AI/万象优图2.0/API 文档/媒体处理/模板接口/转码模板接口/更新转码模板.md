...
| Channels           | Request.Audio | 声道数       | String | 否       | 无     | <li>当 Codec 设置为 aac/flac，支持1、2、4、5、6、8<br/><li>当 Codec 设置为 mp3/opus 时，支持1、2<br/><li>当 Codec 设置为 Vorbis 时，只支持2<br/><li>当 Codec 设置为 amr，只支持1<br/><li>当封装格式为dash时，不支持8 |
...
| StreamSegmentTime           | Request.TransConfig | dash等格式切片时间        | String | 否       | 5            | 1. (0 视频时长] <br/> 2. 单位为秒|
...
| 节点名称（关键字）    | 父节点              | 描述             | 类型   | 必选 | 默认值 | 限制 |
| --------------------- | ------------------- | ---------------- | ------ | ---- | ------ | ------------------------------------------------------------ |
| IsHlsEncrypt          | Request.TransConfig.HlsEncrypt | 是否开启HLS加密 | String | 否   | false        | <li>true/false<br/><li>当Video.Codec为H264或H265并且Container.Format为hls时支持加密 |

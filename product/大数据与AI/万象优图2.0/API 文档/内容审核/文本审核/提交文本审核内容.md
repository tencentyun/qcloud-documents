...
本接口用于提交一个文本审核任务，可审核您的文本内容是否存在敏感违规信息。

>?
> - 当传入的内容为纯文本信息，需要先经过 base64 编码，文本编码前的原文长度不能超过10000个 utf8 编码字符。
> - 当传入的内容为文本文件时，文件大小不能超过1MB。
> - 文本审核支持中文、英文、阿拉伯数字的检测。
> 
...
| Content            | Request.Input | 当传入的内容为纯文本信息，需要先经过base64编码，文本编码前的原文长度不能超过10000个utf8 编码字符。若超出长度限制，接口将会报错。 | String       | 否       |
...
| Content            | Response.JobsDetail | 文本内容的 Base64编码                                         | String    |

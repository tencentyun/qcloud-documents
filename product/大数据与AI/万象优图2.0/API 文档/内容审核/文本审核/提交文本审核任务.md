...
| DetectType         | Request.Conf | 审核的场景类型，有效值：Porn（涉黄）、Ads（广告）、Illegal（违法）、Abuse（谩骂）等，可以传入多种类型，不同类型以逗号分隔，例如：Porn,Ads。<br/>注：不设置时会审核BizType中开启的所有场景。 | String | 否   |
...
| 节点名称（关键字） | 父节点                        | 描述                                                    | 类型    |
| :---------------- | :--------------------------- | :------------------------------------------------------ | :----- |
| TokenId           | Response.JobsDetail.UserInfo | 用户业务 TokenId。                                        | String |
| Nickname          | Response.JobsDetail.UserInfo | 用户业务 Nickname。                                       | String |
| DeviceId          | Response.JobsDetail.UserInfo | 用户业务 DeviceId。                                       | String |
| AppId             | Response.JobsDetail.UserInfo | 用户业务 AppId。                                          | String |
| Room              | Response.JobsDetail.UserInfo | 用户业务 Room。                                           | String |
| IP                | Response.JobsDetail.UserInfo | 用户业务 IP。                                             | String |
| Type              | Response.JobsDetail.UserInfo | 用户业务 Type。                                           | String |

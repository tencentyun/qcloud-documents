...
| AudioSection       | Response.JobsDetail | 该字段用于返回视频中视频声音审核的结果。如果未检测音频，则不返回。<br/>注意：每次查看数据的有效期为2小时，2小时后如还需查看，请重新发起查询请求。 | Container  Array |
...
| ObjectResults      | Response.JobsDetail.Snapshot.*Info | 该字段表示审核到的一些具体结果，例如政治人物名称。注意：该字段仅在 PoliticsInfo 中，有相关违规内容时返回。 | Container Array |

...
```
http://examples-1251000004.picsh.myqcloud.com/sample.jpeg?watermark/2/text/6IW-6K6v5LqRwrfkuIfosaHkvJjlm74=/fill/d2hpdGU=/fontsize/20/dissolve/50/gravity/northeast/dx/20/dy/20/batch/1
```
...
>!
>1. 当 gravity 参数设置为 center 时，dx、dy 参数无效
>2. 当 gravity 参数设置为 north 或 south 时，dx 参数无效（水印会水平居中）
>3. 当 gravity 参数设置为 west 或 east 时，dy 参数无效（水印会垂直居中）

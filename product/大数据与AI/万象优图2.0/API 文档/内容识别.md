...
| 参数名称    | 描述             | 类型  | 必选 |
| ----------- | ----- | ---- | ------------------------------------------------------------ |
|ObjectKey  |	对象文件名，如 picture.jpg|  Sting  |  	是|
|type    |审核类型，拥有 porn（涉黄识别）、terrorist（涉暴恐识别）、politics（涉政识别）、ads（广告识别）四种，用户可选择多种识别类型，如 detect-type=porn,ads 表示对图片进行涉黄及广告审核|  String    |是|

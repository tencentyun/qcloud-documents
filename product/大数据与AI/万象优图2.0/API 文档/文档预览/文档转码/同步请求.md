...
| excelRow            | 值为  `1`  表示将所有列放到 1 页进行排版，默认值为 0 （已支持，但有排版上限问题） | Int  | 否       |
| excelCol            | 值为  `1`  表示将所有行放到 1 页进行排版，默认值为 0（已支持，但有排版上限问题） | Int  | 否       |
| excelPaperSize      | 设置纸张（画布）大小，对应信息为： `0 → A4` 、 `1 → A2` 、 `2 → A0` ，默认 A4 纸张 （需配合  `FitToHeight`  或  `FitToWidth`  一起使用） | Int  | 否       |

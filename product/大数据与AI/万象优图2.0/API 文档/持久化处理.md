...
| rules       | Array | 否    | 处理规则，一条规则对应一个处理结果（目前支持五条规则），不填则不进行图片处理 |
 ...
 | 参数名称   | 类型     | 必选   | 描述                                       |
| ------ | ------ | ---- | ---------------------------------------- |
| bucket | String | 否    | 存储结果的目标 bucket 名称，格式为 BucketName-APPID，如果不指定的话默认保存到当前存储桶 |
...
>!上传时处理支持 COS V5 的分块上传功能，您在使用 COS V5 的 Complete Multipart Upload接口时只需在请求包头部中加入 Pic-Operations 项，即可实现图片处理。

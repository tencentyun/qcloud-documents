...
| SceneChangeInfo    | Request.ConcatTemplate | 转场参数                                                     | Container      | 是   | 无                           | 无                                                           |
...
| StartTime          | Request.ConcatTemplate.<br/>ConcatFragment | 开始时间，单位为秒, 支持float格式，执行精度精确到毫秒，当 Request.ConcatTemplate.DirectConcat 为 true 时不生效    | String | 否   | 无     | 无 |
| EndTime            | Request.ConcatTemplate.<br/>ConcatFragment | 结束时间，单位为秒, 支持float格式，执行精度精确到毫秒，当 Request.ConcatTemplate.DirectConcat 为 true 时不生效     | String | 否   | 无     | 无 |
...
| 节点名称（关键字） | 父节点                                | 描述                                  | 类型   | 必选 |
| ------------------ | ------------------------------------- | ------------------------------------- | ------ | ---- |
| Format             | Request.ConcatTemplate.<br/>Container | 封装格式：mp4，flv，hls，ts, mp3, aac | String | 是   |
...
<span id="SceneChangeInfo"></span>
Container 类型 SceneChangeInfo 的具体数据描述如下：

| 节点名称（关键字） | 父节点                            | 描述       | 类型   | 必选 | 默认值         | 限制                                                         |
| ------------------ | --------------------------------- | ---------- | ------ | ---- | -------------- | ------------------------------------------------------------ |
| Mode               | Request.ConcatTemplate.<br/>SceneChangeInfo | 转场模式   | String | 是   | 无     | <ul  style="margin: 0;"><li>Default：不添加转场特效</li><li>FADE：淡入淡出</li><li>GRADIENT：渐变</li></ul>                                             |
| Time               | Request.ConcatTemplate.<br/>SceneChangeInfo | 转场时长   | String | 否   | 3   | <ul  style="margin: 0;"><li>单位：秒(s)</li><li>取值范围：(0, 5], 支持小数</li></ul> |

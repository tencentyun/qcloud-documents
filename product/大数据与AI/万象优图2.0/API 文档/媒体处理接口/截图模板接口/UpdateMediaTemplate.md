...
| ------------------ | ---------------- | -------- | ------ | -------- | ------------ | ------------------------------------------------------------ |
| Mode               | Request.Snapshot | 截图模式 | String | 是       | Interval     | 1. 值范围：{Interval，Average}<br/> 2. Interval 表示间隔模式，Average 表示平均模式<br/> 3. Interval 模式：Start，TimeInterval，Count 参数生效。当设置 Count，未设置 TimeInterval 时，表示截取所有帧，共 Count 张图片<br/> 4. Average 模式：Start，Count 参数生效。表示从 Start 开始到视频结束，按平均间隔截取共 Count 张图片 |
| Start              | Request.Snapshot | 开始时间 | String | 是       | 0            | 1. 值范围：[0，视频时长] <br/> 2. 单位为秒 <br/> 3. 支持 float 格式，执行精度精确到毫秒 |
| TimeInterval       | Request.Snapshot | 截图频率 | String | 否       | 无           | 1. 值范围：(0，3600] <br/> 2. 单位为秒 <br/> 3. 支持 float 格式，执行精度精确到毫秒 |
| Count              | Request.Snapshot | 截图数量 | String | 是       | 无           | 1. 值范围：(0，10000]                                        |

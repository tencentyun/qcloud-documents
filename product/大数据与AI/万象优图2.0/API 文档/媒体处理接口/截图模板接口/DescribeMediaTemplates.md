...
| Snapshot           | Response.TemplateList | 详细的模版参数                 | Container |

Container 类型 Snapshot 的具体数据描述如下：

| 节点名称（关键字） | 父节点                | 描述     | 类型   | 是否必选 | 默认值       | 限制                                                         |
| ------------------ | --------------------- | -------- | ------ | -------- | ------------ | ------------------------------------------------------------ |
| Mode               | TemplateList.Snapshot | 截图模式 | String | 是       | Interval     | 1. 值范围：{Interval，Average}<br/> 2. Interval 表示间隔模式，Average 表示平均模式<br/> 3. Interval 模式：Start，TimeInterval，Count 参数生效。当设置 Count，未设置 TimeInterval 时，表示截取所有帧，共 Count 张图片<br/> 4. Average 模式：Start，Count 参数生效。表示从 Start 开始到视频结束，按平均间隔截取共 Count 张图片 |
| Start              | TemplateList.Snapshot | 开始时间 | String | 是       | 0            | 1. 值范围：[0，视频时长] <br/> 2. 单位为秒 <br/> 3. 支持 float 格式，执行精度精确到毫秒 |
| TimeInterval       | TemplateList.Snapshot | 截图频率 | String | 否       | 无           | 1. 值范围：(0，3600] <br/> 2. 单位为秒 <br/> 3. 支持 float 格式，执行精度精确到毫秒 |
| Count              | TemplateList.Snapshot | 截图数量 | String | 是       | 无           | 值范围：(0，10000]                                           |

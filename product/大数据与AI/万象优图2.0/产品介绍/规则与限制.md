<table>
   ...
   <tr>
      ...
      <td>总限制</td>
      <td><li>支持格式：目前支持处理 jpg、bmp、gif、png、webp 格式，并且支持 HEIF 格式的解码和处理。<li>体积限制：处理图片原图大小不超过20MB、宽高不超过30000像素且总像素不超过2.5亿像素，处理结果图宽高设置不超过9999像素；针对动图，原图宽 x 高 x 帧数不超过1亿像素。<li>动图帧数限制：GIF 帧数限300帧。</td>
   </tr>
   <tr>

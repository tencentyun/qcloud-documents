腾讯云数据万象 CI 支持多地域存储，不同地区的默认访问域名不同。创建存储桶时选定的地域不可修改，建议根据自己的业务场景选择就近的地域存储，可以提高对象上传、下载速度。
...
## 示例：
> 用户在所属地域广州创建了一个存储桶，存储桶命名中用户自定义字符串部分为 example，系统自动为用户生成的数字串 APPID 为 1234567890，则其数据万象的默认上传访问域名为：`example-1234567890.pic.ap-guangzhou.myqcloud.com`。默认下载访问域名为：`example-1234567890.picgz.myqcloud.com`。

...
### 配置 CDN 加速域名
1. 登录 [万象优图控制台](https://console.cloud.tencent.com/ci) ，进入左侧菜单栏【 Bucket 管理】，单击需要配置域名的存储桶（如 imagetest1），进入存储桶。
...
登录 [万象优图控制台](https://console.cloud.tencent.com/ci) ，进入左侧菜单栏【 Bucket 管理】，单击需要配置域名的存储桶（如 imagetest1 ），单击【域名管理】可进行防盗链设置。

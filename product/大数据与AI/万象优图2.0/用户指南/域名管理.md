...
万象优图 默认域名由腾讯云定义，不可更改。当用户使用腾讯云内部业务使用该域名访问图片资源，请求将通过内网环境发送和接收；当用户是从公网使用该域名访问时，访问将从公网向万象优图 请求文件。CDN 加速域名由腾讯云初始化，可以更改（需要CNAME），访问经由 500+ 的 CDN 加速节点；自定义域名由用户自行进行别名映射，适合需要屏蔽腾讯云地址标识（qcloud）的场景。


## 2. 默认域名

其中万象优图 默认域名由腾讯云定义，不可更改，可以用于腾讯云内部业务请求也适用于公网直接访问。在创建 Bucket 后，腾讯云会自动生成一条默认域名，格式为：
...
进入万象优图管理控制台，点击 Bucket 名称进入管理页面，选择【域名管理】页签，即可以看到默认域名

![默认域名](https://mc.qcloudimg.com/static/img/f44f67ba9f404b38be578ccc76064a80/image.png)

**默认域名-内网跨区域访问： 默认域名可以用于在同区域的不同腾讯云产品之间进行访问。但是，若需要实现内网跨区域的访问， 例如，所在区域为广州的 CVM 需要使用新加坡的万象优图上的数据，则需要使用 VPC 部署专属网络通道，实现高速访问体验。 [点击查看 VPC 相关信息](https://www.qcloud.com/product/vpc.html)**
...
您可以通过默认域名外网访问地址加文件路径访问：testnew-1250000000.image.myqcloud.com/index.htm

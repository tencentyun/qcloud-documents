...
本实践主要介绍如何通过 SDWebImage SDK 加载 TPG、AVIF 图片以及解决以下问题：
1. 降低 DNS 劫持风险
2. 使用 QUIC 协议优化图片下载。
...
pod 'CloudInfinite/TPG' // TPG 解码器，根据实际需求 选择 TPG 或 AVIF
pod 'CloudInfinite/AVIF' // AVIF 解码器
```
#### 第二步：导入头文件
...
```
{
    ...
    // return YES; 表示用 CIImageDownloader ，return NO; 表示不用 CIImageDownloader ，用 SDWebImage 默认下载器。
    ...
    // 可选配置，SDK 内部将图片链接中 format/tpg 或者 format/avif 删除，如果不满足业务，则需自定义 TPG、AVIF 链接转换原图链接策略。
...
[UIView setTPGAVIFImageErrorObserver:^(NSString * _Nonnull url, NSError * _Nonnull error) {
    error 为解码失败的错误信息 可以在这里统一处理 TPG、AVIF 加载失败的错误信息。
}];
```

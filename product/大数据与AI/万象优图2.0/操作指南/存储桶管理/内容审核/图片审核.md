...
开启图片审核功能后，该存储桶中新增的图片将在**上传时**自动进行检测，并支持自动冻结（禁止公有读访问）已检测的违规内容。您也可以通过调用 [内容审核](https://cloud.tencent.com/document/product/460/37318) API 接口，对 COS 中的**存量**图片进行扫描审核。
...
3. 单击【编辑】，将图片审核状态修改为开启，然后按照如下配置项说明进行配置：
   ![](https://main.qcloudimg.com/raw/b23db490a8a8ffa2a504e5f480be9216.png)
...
>
4. 配置完成后，单击保存即可启用该功能。后续上传图片将自动进行审核，并将审核结果展示在 [审核详情](https://cloud.tencent.com/document/product/460/46492) 项中。若需对存量数据进行审核，请查看图片审核 [API 文档](https://cloud.tencent.com/document/product/460/37318)。

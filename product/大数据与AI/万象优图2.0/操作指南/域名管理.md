...
用户可以在创建存储桶时选择开启 CDN 加速，也可以创建存储桶之后开启 CDN 加速。开启之后，外网可以直接访问 CDN 加速域名，例如：
```
...
登录 [数据万象控制台](https://console.cloud.tencent.com/ci) ，进入左侧菜单栏【Bucket 管理】，单击需要配置域名的存储桶（例如 imagetest1 ），单击【域名管理】可进行防盗链设置。

...
内容平台通常使用多终端，因此针对不同用户不同终端需要提供不同格式的媒体文件。数据万象的音视频转码功能提供多种转码参数，覆盖绝大多数转码需求，同时支持媒体文件上传时自动触发转码，从而实现快速适配不同终端的格式需求。

#### 节省空间和流量

针对图片等媒体资源，音视频转码功能可调整码率，并提供多样的压缩功能以提高压缩效率、减小文件体积，实现视频压缩，从而减少卡顿并节省存储空间和流量费用。
...
对于存储在对象存储 COS 上的存量数据，您可通过创建任务来实现音视频转码。音视频转码任务您可选择控制台或 API 进行创建。

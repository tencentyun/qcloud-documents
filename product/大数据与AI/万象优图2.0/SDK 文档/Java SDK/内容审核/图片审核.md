...
| objectKey | Request | COS 存储桶中的图片文件名称，COS 存储桶由Host指定，例如在北京的 examplebucket-1250000000存储桶中的目录 test 下的文件 img.jpg，则Host填写 examplebucket-1250000000.cos.ap-beijing.myqcloud.com，ObjectKey填写 test/img.jpg。 | String    | 否   |
| detectUrl | Request | 您可以通过填写detect-url审核任意公网可访问的图片链接不填写detectUrl时，后台会默认审核ObjectKey填写了detectUrl时，后台会审核detect-url链接，无需再填写objectKey  示例：http://www.example.com/abc.jpg  | String | 否       |

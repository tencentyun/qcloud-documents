...
| [图片批量审核](https://cloud.tencent.com/document/product/460/63594) |  图片批量的扫描 |
...
| detectUrl  | 您可以通过填写`detect-url`审核任意公网可访问的图片链接<li>不填写`detect-url`时，后台会默认审核`ObjectKey`</li><li>填写了`detect-url`时，后台会审核`detect-url`链接，无需再填写`ObjectKey`<br>`detect-url`示例：http://www.example.com/abc.jpg</li> | String | 否       |

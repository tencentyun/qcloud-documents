...
| detect-url  | Query| 您可以通过填写`detect-url`审核任意公网可访问的图片链接<li>不填写`detect-url`时，后台会默认审核`ObjectKey`</li><li>填写了`detect-url`时，后台会审核`detect-url`链接，无需再填写`ObjectKey`<br>`detect-url`示例：http://www.example.com/abc.jpg</li> | String | 否       |
...
| large-image-detect    | Query| 对于超过大小限制的图片是否进行压缩后再审核，取值为： 0（不压缩），1（压缩）。默认为0。
注：压缩最大支持32M的图片，且会收取压缩费用 | String | 否       |

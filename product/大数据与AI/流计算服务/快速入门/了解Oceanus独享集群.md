...
用户可以在 [流计算控制台](https://console.cloud.tencent.com/oceanus/cluster) 创建自己的独享计算集群，集群拥有与其他用户完全独立的资源（CPU、内存、磁盘、网络），以确保 Flink 作业运行时的安全性和隔离性。独享集群以 VPC 的方式进行隔离，因此在独享模式下支持用户自定义 JAR 作业和用户自定义 UDF 函数，满足您的定制化业务需求。

...
1. 登录 [流计算 Oceanus 控制台](https://console.cloud.tencent.com/oceanus)，单击要查看的作业名称，并切换到【监控】页签，即可查看作业的各项关键指标，例如每秒输入输出的数据条数，算子处理耗时，CPU 及内存的使用率等。**BETA 功能** ：在北京、广州等主要地区，用户在监控页还可以查看 JobManager、TaskManager 和 Task 级别的细粒度指标。

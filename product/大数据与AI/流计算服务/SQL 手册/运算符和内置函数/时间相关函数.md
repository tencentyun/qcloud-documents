...
| TO_TIMESTAMP(string, simple_format) | 以 Java 的 SimpleDateFormat 支持的时间格式化模板 simple_format 将 string  字符串格式化为 SCS 支持的时间戳类型（timestamp 类型）
。详见 [类型转换函数](/document/product/849/18079)。|

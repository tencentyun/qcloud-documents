类型转换函数的函数名和功能描述如下：
> ! 如果使用 CAST() 函数，将时间段 INTERVAL 转为数字，则结果会是字面值（可能不符合预期）。例如 `CAST(INTERVAL '1234' MINUTE AS BIGINT)`，结果会是字面值1234，而非时间段表示的毫秒值。
...
| TO_TIMESTAMP(string, simple_format)                          | 以 Java 的 SimpleDateFormat 支持的时间格式化模板 simple_format 将 string 字符串格式化为流计算 Oceanus 内部支持的 timestamp 类型的时间戳。默认以东八区为准。例如 TO_TIMESTAMP(ts, 'yyyy-MM-dd HH:mm:ss')。 |
...
| DATE_FORMAT(timestamp, format)                               | （不建议使用）使用与 MySQL 兼容的模板 format 来格式化时间戳为字符串，与 DATE_FORMAT_SIMPLE 的区别在于采用 MySQL 的格式化语法而非 Java SimpleDateFormat 的格式化语法。使用方式与 MySQL 的 date_parse() 函数相同。例如 DATE_FORMAT(ts, ‘%Y, %d %M’) 返回字符串‘2017, 05 May’。 |

...
| TO_TIMESTAMP(string, simple_format) | 以 Java 的 SimpleDateFormat 支持的时间格式化模板 simple_format 将 string 字符串格式化为流计算 Oceanus 内部支持的 timestamp 类型的时间戳。<br>例如：TO_TIMESTAMP(ts, ‘yyyy-MM-dd HH:mm:ss’)|

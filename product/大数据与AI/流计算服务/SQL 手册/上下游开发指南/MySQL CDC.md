...
### 联合主键设置
如下 DDL 示例中设置了`index1`、`index2`、`index3`、`index4` 字段为联合主键索引，那这4个字段必须定义在 DDL 的最前面，而且顺序要和`PRIMARY KEY`定义保持一致。
```sql
...
### server-id	定义
建议显式的定义`server-id`，避免不同作业读取同一个库可能出现的冲突问题，可以设置为范围值，比如`5400-5405`，也可以是单个值

### 1.13版本支持增量并发处理
增量快照是一种读取表快照的新机制。与旧的快照机制相比，增量快照有很多优点，包括：
-  读取快照时，源可以是并行的。
2. 读取快照时，源可以在块粒度上执行检查点。
3. 读取快照时，源不需要获取全局读锁（FLUSH TABLES WITH read lock）。如果您希望源并行运行，每个并行读取器应该有一个唯一的服务器 id，所以 'server-id' 必须是 '5400-6400' 这样的范围，并且范围必须大于并行度。

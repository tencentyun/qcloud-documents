...
| 1.14      | <li>支持 mysql 版本为 5.7 以上 和 8.x</li><li>默认配置，需要 source 表有 pk。如果 source 表没有 pk，需要 with 参数需要设置 `'scan.incremental.snapshot.enabled' = 'false'`</li><li>如果 mysql 主键类型是 `bigint unsigned`，需在 with 中添加 `'debezium.bigint.unsigned.handling.mode' = 'precise'`</li>               |

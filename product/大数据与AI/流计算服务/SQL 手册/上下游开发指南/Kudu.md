...
| kudu.masters             | 是   | 无     | Kudu 数据库 MasterServer 的连接地址。端口默认为7051。若使用腾讯云的 Kudu 组件，master 地址和端口可以在 [弹性 MapReduce 控制台](https://console.cloud.tencent.com/emr) 的集群列表中单击对应集群 **ID/名称**，在集群详情页，选择**集群服务 > Kudu > 操作 > 查看端口**中找到对应的 master server IP 和端口 |

...
3. 单击**确定** ，完成模型文件的导入。  
4. 上传推理脚本与配置文件   
找到已上传的模型版本，单击 **上传文件** 操作，在弹出的 COS 对话框中，点击左下方【上传文件】，将 mnist-pytorch-infer 包解压后的3个文件依次上传，完成后点击**确定**。  
...
2. 单击 **导入模型** ，在模型导入页面参考如下参数填写。  
	...
	- 模型文件：单击【选择文件】，在弹出的 COS 对话框中，选择需要使用的存储桶，点击左下方【上传文件夹】，将模型包中的 **pytorch > classify文件夹** 上传，上传完成后选中文件夹路径  

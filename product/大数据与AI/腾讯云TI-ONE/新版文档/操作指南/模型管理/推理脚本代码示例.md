...
TI-ONE 的 Detectron2 或者 MMDetection 格式优化后的模型，需要使用 tiacc-inference.load() 函数对模型进行加载，其他格式无需使用 tiacc_inference 的 load，使用原生格式 load 即可。
MMDetection 原生示例：
```
...
```
Detectro2 原生示例(对于 Detectron2 导出的 PyTorch 模型)：
```
import torch
model = torch.load(checkpoint) #.pth模型文件
```
Detectron2 优化后使用示例(对于 Detectron2 导出的 PyTorch 模型):
```
import tiacc_inference
model = tiacc_inference.load('tiacc.pt')  #tiacc.pt是模型优化后生成的新模型
```
Detectro2 原生示例(对于通过 Detectron2.modeling.build_model 构造的模型)：
```
...
Detectron2 优化后使用示例(对于通过 Detectron2.modeling.build_model 构造的模型):
```
import tiacc_inference
model = tiacc_inference.load('tiacc.pt') #tiacc.pt是模型优化后生成的新模型

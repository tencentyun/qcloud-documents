1. 在添加数据源首页单击 Hive 数据库，进入新建页面。新建的页面如下：
 ...
 - 【Krb5 文件路径】Krb5 文件的路径。Linux 环境下 Krb5 文件的名字为` Krb5.conf`；Windows 环境下名字为`Krb5.ini`。Krb5 文件一般会放到一个默认的位置，无需另外配置该项。如果客户未提供此文件，即表示已在默认位置放置此文件。一般情况下，Windows 的默认位置是 `C:\Windows\Krb5.ini` 或者 `C:\winnt\Krb5.ini`；Linux 的默认位置是` /etc/Krb5.conf `或者 `/etc/krb5/krb5.conf`。

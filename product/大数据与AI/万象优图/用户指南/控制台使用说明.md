...
![](//qzonestyle.gtimg.cn/qzone/vas/opensns/res/img/kongzhitaishiyongshuoming-6.png)
...
![](//qzonestyle.gtimg.cn/qzone/vas/opensns/res/img/jingxiangyuanshezhi1.png)

如果开发者设置了自定义域名（参见[域名设置](#2.2.1.-域名设置)），镜像源映射会出现两行，如下图所示：

![](//qzonestyle.gtimg.cn/qzone/vas/opensns/res/img/jingxiangyuanshezhi2.png)
...
![](//qzonestyle.gtimg.cn/qzone/vas/opensns/res/img/jingxiangyuanshezhi3.png)

开发者也可以将自定义域名和图片空间默认域名映射为同一个域名，同样也可以映射为不同的域名。不同的设置，根据域名回源的原则，会产生不同的效果。如下图所示：

![](//qzonestyle.gtimg.cn/qzone/vas/opensns/res/img/jingxiangyuanshezhi4.png)

![](//qzonestyle.gtimg.cn/qzone/vas/opensns/res/img/jingxiangyuanshezhi5.png)

**2.2.2.2 镜像源类型**

请开发者根据自己的场景选择合适的镜像源类型。

>注意：选择存储原图可以使用万象优图的处理能力；选择“存储原图和处理后图片”，不能使用万象优图的处理能力。
...
此种类型适用于镜像源为开发者自己站点的场景。万象优图会存储用户请求的所有图片，而不区分原图和处理过后的图片。
...
如需使用请到[万象优图--服务质量](http://console.qcloud.com/image/service)中查看。

## 4 使用统计

腾讯云•万象优图提供针对单个应用其存储量、下载流量、上传下载删除的次数，wep及水印请求的次数等信息的使用量统计数据，数据会有一定延迟，延迟时间在20分钟以内。

可以在[万象优图--使用统计](http://console.qcloud.com/image/statistics)中查看使用统计数据。

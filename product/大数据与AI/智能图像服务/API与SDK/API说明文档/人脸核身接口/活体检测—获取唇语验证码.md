## 简介
本接口用于获取一个唇语验证字符串，用户录制视频，用于活体检测。
...
`https://service.image.myqcloud.com/face/livegetfour`
...
| Content-Type   | application/json  或者  multipart/form-data | 根据不同接口选择。请注意，实际上 multipart/form-data 后面还需要跟一串 boundary 字段，这由 HTTP 框架/库自动管理，你无需显式设置。例如你使用的是 Apache HttpClient 框架，那么使用 MultipartEntity 构建请求即可，不要使用 setHeader("Content-Type","multipart/form-data") 方法来设置，防止 boundary 缺失导致请求失败。                                 |

...
`https://service.image.myqcloud.com/face/identify`
...
| content-type   | Application/json或者Multipart/form-data | 根据不同接口选择：<br/>1. 使用图片 url，选择 application/json；<br/>2. 使用图片文件，选择 multipart/form-data。  |
| authorization  | 鉴权签名                                  | 多次有效签名，用于鉴权，生成方式见 [鉴权签名方法](/document/product/641/12409) |

### 请求参数

使用 url 则使用 application/json 格式；使用 image 则使用 multipart/form-data 格式。

| 参数名      | 是否必须 | 类型     | 参数说明                                     |
| -------- | ---- | ------ | ---------------------------------------- |
| appid    | 必须   | string | 项目 ID                                    |
| group_id | 必须   | string | 候选人组 id，与group_ids二选一即可                                  |
| group_ids | 可选   | array(string) | 候选人组 id列表，与group_id二选一即可                                  |
| image    | 可选   | binary | 图片内容                                     |
| url      | 可选   | string | image 和 url 只需提供一个；如果都提供，只使用 url |

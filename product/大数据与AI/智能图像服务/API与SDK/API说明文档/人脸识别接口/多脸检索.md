## 简介
对于一张包含多个待识别的人脸的图片，在一个group或多个groups 中识别出最相似的 Person 作为其身份返回，返回的 Top5 中按照相似度从大到小排列。
...
| group_id | 可选   | String | 候选人组 id, group_id 和 group_ids只提供一个即可                                  |
| group_ids | 可选   | Array(String) | 候选人组 ids                                  |
| image    | 可选   | Binary | 图片内容                                     |
| url      | 可选   | String | 图片的 url , image 和 url 只提供一个即可，如果都提供，只使用 url |
| topn    | 可选   | String | 候选人脸数量，一般使用默认值5           |
| min_size    | 可选   | String | 人脸检测最小尺寸，一般使用默认值40                |

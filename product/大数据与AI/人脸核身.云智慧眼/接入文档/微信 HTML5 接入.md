## 原生 HTML5（浮层模式）
微信公众号浮层模式（可选择读数字、光线两种活体检测模式）是基于微信原生的体验，符合以下行业要求的客户可以 [点击申请]()，审核通过后，即可对接浮层模式。
* 政务：政府机构或事业单位
* 金融：银行、保险
* 医疗：公立医疗机构
* 运营商
...
## 通用 HTML5
微信公众号是使用人脸核身·云智慧眼产品的主要场景之一，人脸核身·云智慧眼产品为您提供用于微信公众号的H5接口。
使用流程：
...
1. 前置流程：
   - 申请 SecretId、SecretKey
   - 审批发起的核身请求
2. 客户后端调用实名核身鉴权 API[DetectAuth接口](http://qq.com/) 进行核身流程开启前鉴权，获取业务流水号（BizToken）与微信跳转 URL。
...
4. 人脸核身完成后，流程会回调至客户侧（HTML5类型以回调地址形式返回），此时客户后端即可凭借回调中提供的 BizToken 调用获取实名核身结果信息 [GetDetectInfo接口](http://qq.com/) 去获取本次核身的详细信息。

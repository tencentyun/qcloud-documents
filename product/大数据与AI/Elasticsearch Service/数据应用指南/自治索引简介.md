...
自治索引属性，其中：
		...
		- expire.max_age：时间范围分区过期最长保留时间，单位支持h(小时)、d(天)，最小单位为1小时，不填默认为0，即不删除通过时间过期历史范围分区。
		- expire.max_size：时间范围分区过期最大存储容量，超过则讨论历史时间范围分区吗，单位支持b、kb、mb、gb、tb、pb，不填默认为0，即不淘汰历史范围分区。

我们知道，Elasticsearch 主要用于海量数据的存储和检索，为了保证读写性能，如果将所有数据都放在 SSD 硬盘上，无疑成本会非常高。冷热分离可以用来解决这个问题，它是 Elasticsearch 的经典架构，使用该架构的集群可以设置不同类型的节点，来兼顾性能和容量之间的矛盾，对读写性能要求比较高的热数据（例如7天内的日志）可以在热节点上以 SSD 磁盘存储，对存储量需求比较大但对读写性能要求较低的索引（例如1个月甚至更长时间的日志）可以在冷节点上以 SATA 磁盘存储，既保证了热数据的读写性能，又可以降低存储成本。腾讯云 ES 提供了快速配置构建冷热集群的能力，用户可以在腾讯云官网根据业务需要指定冷热节点规格，分钟级快速建立一个冷热分离架构的 ES 集群。

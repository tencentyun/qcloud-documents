腾讯云 ES 提供了版本升级的功能，支持 ES 从低版本向高版本的升级，以及高级特性的升级，您可以根据业务的需要对集群进行升级操作，实现业务的无缝过度。登录腾讯云 ES 控制台，进入集群详情页，单击右上角「升级」，即可进行升级操作。
...
>?
1. 以上两种类型的升级，一次只能操作一种，不能同时操作。特别地，5.6.4升级到6.4.3 时，可以同时选择升级到基础版。基础版会有 ES 官方高级特性插件 X-Pack 提供的监控和 SQL 等能力，我们也建议您在从5.6.4升级到6.4.3时，也选择升级到基础版。
2. 5.6.4 暂时只支持开源版，同时也不支持升级到基础版和白金版。

## 升级注意事项

### Elasticsearch 5.6.4 与 6.4.3 版本兼容及使用说明

  1. index 多type  
  Elasticsearch 从 6.x 开始，不再支持一个索引多个 type。 从5.6.4升级到6.4.3后，新建多 type 的索引将会报错，原有在 5.6.4 创建的多type索引，不受影响，可以正常写入。
  2. curl 访问集群    
 ...
 3. 配置项兼容   
 Elasticsearch 不同版本之间会存在一些不兼容的配置，如果您有设置，升级后可能会影响集群的使用。腾讯云 ES 升级功能提供了配置项的检查流程，也提供了调整说明，见下文 <a href="#update_check">升级检查</a>
...
- 3.升级过程和集群重启  
  ...
  （1）从开源版升级到基础版，集群需要滚动重启，集群需要满足健康状态是绿色，并且没有只配置了单个副本的索引，才能执行升级操作。升级期间，服务可以正常访问，但性能会有部分影响，建议在集群访问量不大时进行。
  （2）从基础到白金，集群要开启安全权限认证功能，需要全量重启，才能完成操作。全量重启期间，集群会停服，服务不可访问，升级完成后，才能正常操作。

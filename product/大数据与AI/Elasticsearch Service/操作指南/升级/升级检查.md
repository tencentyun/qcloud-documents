Elasticsearch 不同版本之间可能会存在一些不兼容的配置，如果进行版本升级，需要在升级前进行调整，避免升级后，集群使用受到影响。您可以通过升级检查的功能，检查是否有不兼容的配置，并进行调整。以下是腾讯云 ES ，对Elasticsearch 版本升级时检查的配置说明。

>? 升级检查操作入口，在 [控制台]() 详情页，右上角「升级」按钮，操作步骤详见 [升级 ES 集群](https://cloud.tencent.com/document/product/845/36107)。

### Elasticsearch 5.6.4 升级到 6.4.3 

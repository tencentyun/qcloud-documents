...
>! 需特别留意本文中需要人工检查的部分（无法自动检查），并在升级前完成代码兼容性改造，避免升级后造成集群无法正常访问，例如，6.8版本升级到7.5版本后默认无法创建包含 type 的索引。
...
### 需人工自查的不兼容配置
对于超过1024个字段数的索引，查询时如果不指定字段会非常消耗性能。建议设置默认查询字段`index.query.default_field`或者通过设置提高字段限制设置`indices.query.bool.max_clause_count`。
...
| 7 | 索引级别 | 索引设置（index.percolator.map_unmapped_fields_as_string） | WARNING  | 索引设置`index.percolator.map_unmapped_fields_as_string` 改名为 `index.percolator.map_unmapped_fields_as_text`，详见 [Breaking changes in 7.0](https://www.elastic.co/guide/en/elasticsearch/reference/7.0/breaking-changes-7.0.html#_percolator) |
...
| 9 | 索引级别 | 索引设置（index.unassigned.node_left.delayed_timeout）     | WARNING  | 索引设置`index.unassigned.node_left.delayed_timeout`负数值被废弃，详见 [Breaking changes in 7.0](https://www.elastic.co/guide/en/elasticsearch/reference/7.0/breaking-changes-7.0.html#index-unassigned-node-left-delayed-timeout-no-longer-negative) |
| 10 | 索引级别 | 索引设置（index.shard.check_on_startup）                   | WARNING  | 索引设置`index.shard.check_on_startup`的值为`fix`被废弃，详见 [Breaking changes in 7.0](https://www.elastic.co/guide/en/elasticsearch/reference/7.0/breaking-changes-7.0.html#fix-value-for-index-shard-check-on-startup-removed) |

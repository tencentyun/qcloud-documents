...
1. 执行`git clone https://github.com/siddontang/go-mysql-elasticsearch`。
2. 执行`cd go-mysql-elasticsearch/src/github.com/siddontang/go-mysql-elasticsearch`。
3. 执行`vi etc/river.toml` 修改配置文件，同步172.16.0.101:3306数据库中的 webservice.building 表到 ES 集群172.16.32.64:9200的 building index（更详细的配置文件说明请参考 [项目文档](https://github.com/siddontang/go-mysql-elasticsearch)）。
```
...
5. 执行`./bin/go-mysql-elasticsearch -config=./etc/river.toml`。
6. 在控制台输出结果。
```

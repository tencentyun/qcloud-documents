腾讯云 Elasticsearch Service（ 以下简称 ES） 提供了丰富的 Restful 风格的 API，其官方和社区也推出了各个语言版本的 SDK 方便用户使用。
腾讯云 ES 在用户的 VPC 下提供了一个访问 ES 集群的 VIP，通过负载均衡的方式挂载了 ES 集群内的所有节点。这样做的目的，一方面是为了适配集群的弹性伸缩并保证其高可用性，当 ES 集群的节点发生变化时，VIP 会自动更新节点信息；另一方面，简化用户的操作，用户不用再关注集群节点 IP、port 等信息的变更。

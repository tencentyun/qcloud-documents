...
## 客户端
您可通过任意支持发送 HTTP/REST 调用的客户端，通过 REST API 访问集群。例如：curl 工具，Kibana Dev Tool。  
...
Elasticsearch 从5.6.0开始，发布了 Java High Level REST Client。这个官方的 high-level client，可以用于执行 search、index、delete、update and bulk 操作，同 TransportClient 使用的核心 Java classes 一致，并且是为了替代 TransportClient。详情参见 [Java High Level REST Client 说明](https://www.elastic.co/guide/en/elasticsearch/client/index.html)。

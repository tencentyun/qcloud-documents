Elasticsearch 官方推荐使用 **Java REST 客户端** 连接集群并进行数据操作。
Java REST 客户端分为以下两种：
...
使用 TCP 协议连接 ES 集群的 Transport Client 官方已经不再维护，建议使用使用 HTTP 协议连接集群的 Java High Level Client 或者 Java Low Level Client 。可以参考官方文档 [由 TransportClient 迁移至 Java High Level Client](https://www.elastic.co/guide/en/elasticsearch/client/java-rest/5.6/java-rest-high-level-migration.html)

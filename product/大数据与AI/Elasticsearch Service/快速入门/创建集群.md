...
- 地域：目前腾讯云 ES 已上线广州、上海、北京、成都、香港、硅谷、多伦多、上海金融、深圳金融，共计9个地域，其中上海金融和深圳金融需要需 [提交工单](https://console.cloud.tencent.com/workorder/category) 申请。
- 部署方式：单可用区、多可用区，单可用区是 ES 集群部署在一个可用区内；多可用区是可以部署在同一个地域的两个可用区。多可用区的部署方式可以提高集群的容灾能力，保障线上业务的稳定性。
多可用区方案，暂时是内测阶段，可以通过 [提交工单](https://console.cloud.tencent.com/workorder/category) 申请。另外，多可用区的部署方式需要满足该地域下有多个可用区，且有足够的资源。
- 网络：选择当前地域下的 VPC，同一个地域的同一个 VPC，在不同可用区之间是互通的，多可用区部署模式下，也是选择同一个 VPC。
- 可用区、及子网：ES 部署在 VPC 内，为保证在内网能顺利访问 ES 集群，创建 ES 集群时，建议选择已有云上业务的区域位置所在的 VPC。只有在同一个 VPC 下，才能访问 ES 集群。
...
- 高级特性：Elasticsearch 官方商业特性（原 X-Pack 插件），包括数据权限管理、SQL JDBC、告警、机器学习等能力；不同版本包含的功能完整度不同，其中白金版包含所有高级特性，基础版包含一部分高级特性，开源版只有开源部分的功能，不包括高级功能，具体版本包含的功能差异，详见 [高级特性（X-Pack）](https://cloud.tencent.com/document/product/845/34926)。
...
![创建集群](https://main.qcloudimg.com/raw/a5736d5932af279f474c66f708f40749.png)

#### 二、确认及购买
单击【下一步：确认配置信息】，确认配置。
![](https://main.qcloudimg.com/raw/0c1a4edd1f568849d4cd2a26e191df2c.png)
- 包年包月时单击【立即购买】，进入订单页，单击【确认购买】并支付。支付成功后返回 ES 控制台，在集群列表页可以查看到创建的集群。
...
#### 访问集群
ES 提供了多种类型的客户端访问集群，具体请参见 [访问方式概述](https://cloud.tencent.com/document/product/845/19539)。

#### 查看集群监控
ES 提供了丰富的监控指标，可以在集群使用过程中查看集群的状态。具体请参见 [查看监控](https://cloud.tencent.com/document/product/845/16995)。

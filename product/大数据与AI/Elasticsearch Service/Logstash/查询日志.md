本文为您介绍 Logstash 实例运行日志的使用说明。用户可以通过实例的运行日志，了解实例的运行状况，定位问题，辅助实例的应用开发和运维。
...
3. 选择【日志】页，可查看实例的运行日志。

   - 共有三种类型的日志：主日志、慢日志和 GC 日志，日志内容包括日志时间、日志级别，以及具体信息等。
...
![img](https://main.qcloudimg.com/raw/b5ff87193e1525696ad55822e531f5d5.png)
...
通过慢日志可以查看哪些event在管道传输过程中耗时比较久。默认情况，慢日志不开启。开启Logstash实例的慢日志，通过在控制台中修改YML配置完成:
![](https://main.qcloudimg.com/raw/e924bab81d8424b6ffe18954074220c5.png)

点击"修改"按钮，再点击"保存"按钮，确认可以重启实例后则完成开启慢日志。


### GC 日志

Logstash默认开启 GC 日志，典型的GC日志内容如下：

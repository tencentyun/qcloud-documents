...
#### 一、选择实例配置
- 计费模式：支持【包年包月】和【按量计费】。
- 地域：支持国内和境外多个地域，其中上海金融、深圳金融、北京金融需要通过 [在线支持](https://cloud.tencent.com/online-service?from=sales&source=PRESALE) 申请。
...
#### 三、创建完成
支付完成，实例创建成功后，即可跳转到 Elasticsearch Service 控制台查看刚创建的实例，实例将在几分钟内创建完成。

...
在折线图的 API 数据中引用刚刚导出的变量。
>?其他组件 API/SQL 能够使用冒号语法访问刚刚导出的全局变量（这里的变量名是  value），当这个变量的值发生变更时，这个组件的数值会重新拉取。

![](https://main.qcloudimg.com/raw/22f8c0b0585f2222acde23a1367dfae6.png)
至此，当 Tab 列表选中项发生变化，折线图将会使用 Tab 列表选中项的值重新拉取数据。
...
http://v.yuntus.com/cloudv/7dbfdb04a4e400705a27c3ff09078e81#value=tab2
```
>!这里必须要有此全局变量才有用。如果不需要显示 Tab 列表提供给用户进行点击，只想使用 url 联动特性，则可以添加通用文本。
...
然后打开【通用文本】的导出全局变量开关。完成后即可在 url 中传递参数。

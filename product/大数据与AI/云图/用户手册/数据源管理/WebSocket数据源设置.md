...
Push 的方式是实时的，如果您需要实时更新大屏，可以选择将数据源设置为 WebSocket。

下面将介绍 WebSocket 的使用。

## 快速搭建 WebSocket 服务端
这里以 NodeJs 举例，使用到了 ws 这个 WebSocket 库，配合 HTTP 框架 express 进行使用。

使用 NodeJs 运行这段代码，便启动了一个 WebSocket。访问地址是：`ws://127.0.0.1:3000`，代码如下：
```
...
![](https://main.qcloudimg.com/raw/3c0354da6405ff758e7ad396cbcdf600.png)
格式示例如下：
![](https://main.qcloudimg.com/raw/95bea557d0ddf1c134c6a42e52461d11.png)
将“WebSocket 发送数据格式示例”中的内容复制出来，稍微修改一下，使用刚才的代码实现一个消息发送接口。当浏览器访问`http://127.0.0.1:3000/refresh-chart`将会向 WebSocket 客户端发送数据刷新组件。
...
```
现在浏览器每访问一次`http://127.0.0.1:3000/refresh-chart`便会实时刷新柱状图。

因为消息中的 body 是数组，因此可以同时发送多个图表组件的数据，刷新多个图表组件。

>!大屏中只用设置一个图表组件的 WebSocket 数据源，便可以控制大屏中所有的图表组件。图表组件的 ID可以右键编辑界面中的图表组件，单击菜单中的【复制 ID】获取。

## 使用 WebSocket 控制联动
WebSocket 也可以用来控制联动，只需要发送更新全局变量的消息。
![](https://main.qcloudimg.com/raw/f8369158229bee0732a45a93071c2630.png)
如上图，这里要更改全局变量 tabValue 的值为 tab2，向客户端发送以下消息即可（同样，这里可以传入多个字段）：
...
## WebSocket 服务端实现访问秘钥鉴权
当我们实现上述 WebSocket 服务端后，这个时候服务是暴露在公网的，可能被任何人连接。

云图提供了访问秘钥功能，在 WebSocket 连接建立之后会发送带签名的 Connect 消息。

如果在一定时间之内没收到 Connect 消息或者收到 Connect 消息的签名不正确，认为连接的客户端不合法。

以下面的 SecretId, SecretKey 举例

SecretId zUYUtjPu2Kob9jarBhTGxrbkau3FEq6pqxe6

SecretKey xrck1Mgi0IxVjS08B3HsECajO01RYfGW

当访问大屏，大屏连接服务端成功之后，服务端将收到带签名的 Connect 消息：
...
```
{
  ...
  "body": {
    ...
    "TcvSecretId": "zUYUtjPu2Kob9jarBhTGxrbkau3FEq6pqxe6",
...
## WebSocket 服务端实现心跳保活
当大屏 WebSocket 客户端连接上服务端后，客户端和服务端的连接稳定性面临很多问题：
- 无线网络信号突然变差
- 网络发生切换
- 路由器断网
- 网线断了

服务端和客户端都不知道连接变慢或已经的断开状态。
...
如果大屏客户端在发送 Ping 消息 10 秒后没有收到服务回应的 Pong 消息，便认为网络不通，尝试进行重连。
...
```
服务端添加定时器，如果超过 31秒（上图心跳包Ping间隔时间） + 10秒（Ping 消息在网络上传递的最大时间） 时间没有收到消息，则认为客户端已经断开连接，主动将该客户端连接断开。

## 完整服务端代码示例
NodeJs 完整示例代码（ node 8 及以上版本运行通过）：

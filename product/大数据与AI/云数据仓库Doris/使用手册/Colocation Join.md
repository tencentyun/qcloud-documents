Colocation Join 是在 Doris 0.9 版本中引入的新功能。旨在为某些 Join 查询提供本地性优化，来减少数据在节点间的传输耗时，加速查询。最初的设计、实现和效果可以参阅 [ISSUE 245](https://github.com/apache/incubator-doris/issues/245)。
...
可以对一个已经创建的表，修改其 Colocation Group 属性。示例：`ALTER TABLE tbl SET ("colocate_with" = "group2");`。
...
也可以通过以下命令，删除一个表的 Colocation 属性：`ALTER TABLE tbl SET ("colocate_with" = "");`。

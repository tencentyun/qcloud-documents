...
各大主流数据库都会提供ODBC的访问Driver，用户可以参照各数据库官方推荐的方式安装对应的ODBC Driver lib库。
...
例如Mysql中一张表有100万数据，想同步到doris，就可以采用ODBC的方式将数据映射过来，在使用 TODO: 语法参考手册/DDL语法/INSERT语句.md 方式将数据同步到Doris中，如果想同步大批量数据，可以分批次使用 TODO: 语法参考手册/DDL语法/INSERT语句.md 同步（不建议使用）

...
使用Datax导入数据到Doris需要使用DataX doriswriter 插件，这个插件是利用Doris的Stream Load 功能进行数据导入的，需要配合 DataX 服务一起使用。

## 使用手册

DataX doriswriter 插件代码在 [这里](https://github.com/apache/incubator-doris/tree/master/extension/DataX)。这里包含插件代码以及 DataX 项目的开发环境。
...
3.创建datax脚本`import_t_test.json`
...
4.执行datax任务：python datax.py  import_t_test.json。具体用法参考 [datax官网](https://github.com/alibaba/DataX/blob/master/userGuid.md)
...
| OLAP_ERR_TABLE_VERSION_INDEX_MISMATCH_ERROR      | -401   | teblet 版本索引不匹配异常                                     |
...
| OLAP_ERR_TABLE_CREATE_META_ERROR                 | -405   | 无法创建 Tablet（更改schema），Base tablet 不存在 ，会触发此异常 |
...
| OLAP_ERR_ENGINE_LOAD_INDEX_TABLE_ERROR           | -502   | 加载 tablet_meta 失败，cumulative rowset无效的segment group meta，会引发此异常 |

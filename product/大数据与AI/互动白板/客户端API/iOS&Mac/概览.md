...
| startSyncVideoStatus: | 内部启动定时器，定时同步视频状态到远端（仅限于mp4）  |
...
| onTEBBackgroundH5StatusChanged:url:status: | 设置白板背景H5状态改变回调  |
...
| onTEBH5FileStatusChanged:status: | H5文件状态回调  |
...
| TEduBoardContentFitMode | 白板内容自适应模式 内容包括图片、文件、PPT动画  |

...
| 属性 | 类型 | 字段含义 |
| --- | --- | --- |
| TEDU_BOARD_BACKGROUND_H5_STATUS_LOADING | int | H5背景正在加载  |
| TEDU_BOARD_BACKGROUND_H5_STATUS_LOAD_FINISH | int | H5背景加载完成  |
...
### toRGBA
生成16进制格式颜色字符串，格式如rgba(red, green, blue, alpha) 
``` Java
String toRGBA()
```
#### 返回
...
| --- | --- | --- |
| boardId | String | 白板ID  |
| backgroundUrl | String | 背景图或者背景H5页面的URL  |
...
| thumbnailUrl | String | 为文件生成缩略图URL  |
| title | String | 文件标题  |
...
| isStaticPPT | boolean | 指定对于PPT文件是否启用静态转码（转为静态图片），默认转为H5动画（转码耗时较长）  |
...
| url | String | 自定义鼠标指针样式的URL，格式限制参考文档：https:///< developer.mozilla.org/zh-CN/docs/Web/CSS/cursor/urlint offsetX; ///< 自定义鼠标指针样式的横行偏移  |

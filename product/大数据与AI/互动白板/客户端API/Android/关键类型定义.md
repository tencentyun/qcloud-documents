...
| isStaticPPT | boolean | 指定对于PPT文件是否启用静态转码（转为静态图片），默认转为 H5 动画（转码耗时较长）  |
...
| url | String | 自定义鼠标指针样式的 URL，格式限制参考文档：https:///< developer.mozilla.org/zh-CN/docs/Web/CSS/cursor/urlint offsetX; ///< 自定义鼠标指针样式的横行偏移  |

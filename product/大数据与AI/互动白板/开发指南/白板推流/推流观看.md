...
participant Operator as 客户端
participant Business as 业务后台
participant Pusher as 白板推流服务
participant TRTC as 实时音视频服务

Operator ->> Business : 开始上课
Business ->> Pusher : 开始录制
Pusher ->> TRTC : 白板画面
Pusher -->> Business : 推流任务ID
Business -->> Operator : 回包

opt 课间休息
Operator ->> Business: 课间休息
Business ->> Pusher : 停止推流
Pusher ->> Pusher : 停止推流
Pusher -->> Business : 回包
Business -->> Operator : 回包
end
...
opt 恢复上课
Operator ->> Business  : 恢复上课
Business ->> Pusher : 开始推流
Pusher ->> TRTC : 白板画面
Pusher -->> Business : 推流任务ID
Business -->> Operator : 回包
end

Operator ->> Business : 停止上课
Business ->> Pusher : 停止推流
Pusher ->> Pusher : 停止推流
Pusher -->> Business : 回包

...
- 对应云函数的触发器（将在后面详细介绍创建过程，所选地区应于 COS 存储桶所属地域一致）：
![img](https://asrwendangtupian-1256085166.cos.ap-chengdu.myqcloud.com/SCF%2BCOS%2BOFFLINE%E6%96%87%E6%A1%A3%E5%9B%BE%E7%89%87/chufaqinew.png)
...
把请求任务函数复制到对应的函数在线编辑框，请求任务函数内容如下（用户需要进入 [API 密钥管理页面](https://console.cloud.tencent.com/cam/capi) 获取 AppID、SecretId、SecretKey 填入到函数，并修改存储桶 Bucket 的地区和回调 URL，回调 URL 需要在 [配置 API 网关触发器](#callback_url) 后获取）：
...
- 触发方式：COS 触发；
- 触发事件：全部创建；
- 触发规则：前缀 filetrans-raw-1253327908/ （根据用户实际情况填写）；
- 后缀：.wav （根据用户实际情况填写）；

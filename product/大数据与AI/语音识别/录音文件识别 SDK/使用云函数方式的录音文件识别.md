...
1. 创建云函数的服务；
2. 创建生成请求任务的函数，使用 COS 触发器，设置回调 URL，生成录音文件识别任务。
3. 创建接收回调结果的函数，使用 HTTP 触发器，生成回调 URL，用于将录音文件识别结果写回 COS 。

## 创建云函数服务

在[ 云函数控制台 ](https://console.cloud.tencent.com/scf)创建一个云函数服务,按照下图配置对应的** 基础信息 **和** 函数配置 **。
- ** 基础信息**如下：
![image-20191125200040098](https://asrwendangtupian-1256085166.cos.ap-chengdu.myqcloud.com/SCF%2BCOS%2BOFFLINE%E6%96%87%E6%A1%A3%E5%9B%BE%E7%89%87/%E6%96%B0%E5%BB%BA%E5%87%BD%E6%95%B0-%E5%9F%BA%E7%A1%80%E4%BF%A1%E6%81%AF1.png)

- **函数配置**：运行角色选择`QCS_SCFExcuteRole`即可。如果用户想要自己配置角色，可以参考文档：[角色与授权]( https://cloud.tencent.com/document/product/583/32389 )
![image-20191125200110223](https://asrwendangtupian-1256085166.cos.ap-chengdu.myqcloud.com/SCF%2BCOS%2BOFFLINE%E6%96%87%E6%A1%A3%E5%9B%BE%E7%89%87/%E6%96%B0%E5%BB%BA%E5%87%BD%E6%95%B0-%E5%87%BD%E6%95%B0%E9%85%8D%E7%BD%AE.png)

## 生成请求任务

当 COS 上传音频文件时， COS 触发器将会被触发，调用触发器函数，提交录音文件识别请求。

### 编写请求任务函数

在创建的云函数服务下，创建一个函数，用于生成请求任务：
1. 新建一个函数：file_trans
2. 函数入口：index.main_handler
3. 运行环境：Python2.7
4. 函数执行内存：128MB（选择最小值128MB已经够用，这个和云函数的计费相关）
5. 超时时间：3s
请求任务函数内容如下（用户需要进入[API 密钥管理页面](https://console.cloud.tencent.com/cam/capi) 获取 AppID、SecretId、SecretKey填入到函数，并修改存储桶 Bucket 的地区和回调url）：
```python
# -*- coding: utf-8 -*-
...
token = ''
callback_url = 'xxxxx' # 请替换为您在创建接受回调的 HTTP 触发器的回调URL
...
- 运行环境：Python2.7
- 函数执行内存：128MB（选择最小值128MB已经够用，这个和云函数的计费相关）
- 超时时间：3s

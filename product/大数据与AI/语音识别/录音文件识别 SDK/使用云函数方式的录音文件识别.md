...
- 对应云函数的触发器(将在后面详细介绍创建过程)：
![img](https://asrwendangtupian-1256085166.cos.ap-chengdu.myqcloud.com/SCF%2BCOS%2BOFFLINE%E6%96%87%E6%A1%A3%E5%9B%BE%E7%89%87/%E8%A7%A6%E5%8F%91%E5%99%A8%E6%88%AA%E5%9B%BE.png)
...
3. 创建接收回调结果的函数，使用 API 网关触发器，生成回调 URL，用于将录音文件识别结果写回 COS 。
...
请求任务函数内容如下（用户需要进入[ API 密钥管理页面 ](https://console.cloud.tencent.com/cam/capi) 获取 AppID、SecretId、SecretKey填入到函数，并修改存储桶 Bucket 的地区和回调 url ）：
...
- 触发方式：COS触发；
- 触发事件：全部创建；
- 触发规则：前缀filetrans-raw-1253327908/ （根据用户实际情况填写）；
- 后缀：.wav （根据用户实际情况填写）；
...
请求任务函数内容如下（用户需要进入[ API 密钥管理页面 ](https://console.cloud.tencent.com/cam/capi) 获取 AppID、SecretId、SecretKey填入到函数，并修改存储结果的Bucket 的名称与地区）：

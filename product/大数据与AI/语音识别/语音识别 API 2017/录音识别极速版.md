## 1. 接口描述
本接口支持使用者通过HTTP POST方式上传一段音频并在极短时间内同步获取识别结果，满足音视频字幕、准实时质检等场景下对语音文件识别时效性要求。
...
| 响应格式| 统一采用 JSON 格式 |
| 并发限制 | 默认单账号限制并发连接数为3路，如您有提高并发限制的需求，[请提工单](https://console.cloud.tencent.com/workorder/category) 进行咨询。 |
...
用户通过 [签名生成](#sign) 的签名为 HepdTRX6u155qIPKNKC+3U0j1N0=，上传语音数据 @/data/test.wav，请求录音识别极速版服务，结果返回方式为同步返回。
```

...
| 字段名 | 类型 | 描述 |
|---------|---------|---------|
| slice_type | Integer | 识别结果类型，0：一段话开始识别；1：一段话识别中，voice_text_str 为非稳态结果(该段识别结果还可能变化) ；2：一段话识别结束，voice_text_str 为稳态结果(该段识别结果不再变化)。<br>根据发送的音频情况，识别过程中可能返回的 slice_type 序列有：<br>0-1-2：一段话开始识别、识别中(可能有多次1返回)、识别结束。<br>0-2:  一段话开始识别、识别结束。 |

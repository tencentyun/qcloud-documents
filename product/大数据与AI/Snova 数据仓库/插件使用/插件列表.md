## 背景说明
云数据仓库 PostgreSQL 是基于 PostgreSQL 的 MPP 集群架构，因此也兼容部分 PostgreSQL 生态的插件。本文列出了云数据仓库 PostgreSQL 目前支持的插件类型以及使用方法，若您有其它插件的支持需求，可 [提交工单](https://console.cloud.tencent.com/workorder/category) 咨询。

使用云数据仓库 CDW，您需要完成以下操作：

## 一. 创建集群
云数据仓库 CDW 目前处于内测阶段，如需使用，请先填写 [申请](https://cloud.tencent.com/apply/p/9w3pf3gjrtk)。

创建集群前，需要明确数据量、数据所在地域，以及访问集群的网络环境。

目前只支持 VPC 网络，因此在创建集群前需要创建好访问集群的 VPC 网络及其子网。
...
```
CDW 完全兼容 PostgreSQL 8.3.23 协议。使用 psql 连接数据库的基本语法如下：
```
psql  -h 10.0.0.3 -p 5436 -d postgres -U testuser
```

其中 postgres 是 CDW 默认的数据库，testuser 是创建数据库的时候需要用户输入的管理员帐号，5436是数据库默认的端口号，10.0.0.3 是创建完数据库后返回的 vip，该 vip 可以在控制台查询。

## 三. 导入数据
1. 使用 INSERT 导入数据。
您可以通过 INSERT 语句直接向 CDW 写入数据，适用于数据量较小的场景。
   - 使用客户端工具 psql 连接 CDW，并使用标准 INSERT 语法写入数据。
   - 通过 PostgreSQL JDBC 驱动书写应用程序向 CDW 写入数据。 
2. 使用 `\COPY` 命令导入数据。
   您可以使用 `\COPY` 命令将客户端所在主机上的文件导入到 CDW 中，语法可以参见 PostgreSQL 相关 `\COPY` 语法。
3. 从 COS 外表中导入数据。
...
在 CDW 中，语法完全兼容 Greenplum Database 5.x 语法，您可以参考其语法进行数据分析。

...
## 定价详情
CDWPG 的节点类型分为计算密集型、存储密集型和弹性存储型。
...
### **包年包月**
**示例一：**用户在广州二区，购买了一个 CDWPG 集群，集群为2个 nc.large 节点（4核 CPU、16GB内存、160GB SSD 存储），购买时长是1个月，用户所需支付的费用计算如下：
- nc.large 节点：450元/月
- 节点个数：2

总费用为：450 × 2 = 900元/月

**示例二：**用户在广州三区，购买了一个 CDWPG 集群，集群计算节点个数为2个（节点规格是4C16G，节点存储：增强型 SSD，100GB），购买时长是1个月，用户所需支付的费用计算如下：

总费用为：（365 + 100）× 2 = 930元/月

### **按量计费**
**示例一：**用户在广州二区，购买了一个 CDWPG 集群，集群为2个 nc.large 节点（4核 CPU、16GB内存、160GB SSD 存储），选择按量计费，在冻结资金成功后会立刻开通集群，之后每小时作为一个结算周期进行扣款（计费单位为元/秒）：
- nc.large 节点：1.25元/小时
- 节点个数：2

每小时扣款费用：1.25 × 2 = 2.50元/小时

**示例二：**用户在广州三区，购买了一个 CDWPG 集群，集群计算节点个数为2个（节点规格是4C16G，节点存储：增强型SSD, 100GB），选择按量计费，之后每小时作为一个结算周期进行扣款（计费单位为元/秒）：

...
## 背景
尽管 CDWPG 对数据做了主备，但是在某些场景下，仍然需要对重要数据进行全量冷备，例如异常删除数据。由于目前 CDWPG 暂不支持自动冷备数据，因此需要业务侧手动完成相关工作。在 CDWPG 中，数据备份使用 COS 作为存储介质，对 COS 数据操作可参考 [使用外表高速导入或导出 COS 数据](https://cloud.tencent.com/document/product/878/34875)。
...
## 步骤
以下 shell 脚本提供了备份整个 CDWPG 集群数据的功能，用户可根据需要进行扩充，配合 crontab 完成周期冷备的任务，也可直接下载使用 [backup_cdw_v101.sh](https://packagedown-online-1256722404.cos.ap-guangzhou.myqcloud.com/tool/backup_cdw_v101.sh)。
>!

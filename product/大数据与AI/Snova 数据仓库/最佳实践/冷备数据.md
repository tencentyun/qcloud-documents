...
2. 脚本运行会在每个库创建一个COS插件。
3. 脚本运行会对每张需要备份的表创建一张COS外表，备份结束后会进行删除。

## 问题
使用本文提交方法进行数据备份，可能会遇到以下问题：
1. ERROR:  permission denied for external protocol cos
`解决：GRANT ALL ON PROTOCOL cos TO {backup_user}`

2. ERROR:  permission denied for schema {schame_name}
`解决：GRANT ALL ON SCHEMA {schame_name} to {backup_user}`

3. ERROR:  permission denied for relation {table_name}
`解决：GRANT SELECT ON {table_name} to {backup_user}`

## 步骤
以下 shell 脚本提供了备份整个CDW集群数据的功能，用户可根据需要进行扩充，配合 crontab 完成周期冷备的任务，也可直接下载使用 [backup_cdw_v100.sh](https://packagedown-online-1256722404.cos.ap-guangzhou.myqcloud.com/tool/backup_cdw_v100.sh)。
>!

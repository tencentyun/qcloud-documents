...
·         一个索引变得“臃肿”，其中包含很多空的或者近乎为空的页面。数据库中的B-树索引在特定的非常规访问模式下可能会发生这种情况。REINDEX 提供了一种方法来减少索引的空间消耗，即制造一个新版本的索引，其中没有死亡页面。
...
如果怀疑共享全局系统表目录索引已经损坏，它们只能在数据库的utility模式下进行重建索引。损坏一个共享索引的典型症状会出现“index is not a btree”的错误，或者服务器在启动的时候由于依赖在已经损坏的索引上而立即崩溃。在这种情况下，联系Snova客服支持获取帮助。

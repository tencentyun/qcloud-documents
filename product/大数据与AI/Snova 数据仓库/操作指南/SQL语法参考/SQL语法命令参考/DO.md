...
需要执行的程序化语言代码。这必须指定为字符串文本。正如使用 CREATE FUNCTION 命令。建议使用美元引用文字。可选关键字无效。支持这些程序语言：PL/pgSQL（plpgsql），PL/Python （plpythonu），and PL/Perl （plperl 和plperlu）。
...
该 PL/pgSQL 例子决定是否数据库用户是超级用户。在例子中，匿名块检索从临时表的输入值。

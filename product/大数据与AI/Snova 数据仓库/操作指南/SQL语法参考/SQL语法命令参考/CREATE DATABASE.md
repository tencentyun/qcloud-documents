...
默认情况下，将通过克隆标准系统数据库创建新数据库 template1. 可以通过写指定不同的模板 TEMPLATE 名称. 特别是, 通过写 TEMPLATE template0, 用户可以创建一个仅包含由Database预定义的标准对象的干净数据库。 将对用户希望避免复制内容添加到 template1的任何安装对象中非常有用。

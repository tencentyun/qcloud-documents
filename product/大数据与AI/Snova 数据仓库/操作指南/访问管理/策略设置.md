## 概述
访问策略可用于授予访问 CDW 集群的权限。访问策略使用基于 JSON 的访问策略语言。您可以通过访问策略语言授权指定委托人（principal）对指定的 CDW 集群资源执行指定的操作。
...
## 元素用法
### 指定效能
如果没有显式授予（允许）对资源的访问权限，则隐式拒绝访问。同时，也可以显式拒绝（deny）对资源的访问，这样可确保用户无法访问该资源，即使有其他策略授予了访问权限的情况下也无法访问。下面是指定允许访问的示例：
```json
"effect" : "allow"
```

### 指定操作
云数据仓库定义了可在策略中指定一类控制台的操作，指定的操作按照操作性质分为读取部分接口（cdw:Describe\*）和全部接口（cdw:\*）。
...
### 指定条件
访问策略语言可使您在授予权限时指定条件。在 CDW 场景下主要是用于设置标签鉴权，下面是标签策略的一个示例：
```json
 "condition": {
...
```
这个语句含义是策略包含标签 key 为 jing，value 为 jingfddd 的资源。

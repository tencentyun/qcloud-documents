## CDW 预设策略管理
为了方便用户授权子账号资源，CDW 提前设置了两条预设策略，进入 [访问管理](https://console.cloud.tencent.com/cam/policy) 控制台，在页面右上角搜索框中搜索 CDW，可看到如下三条策略： 

| 策略 | 说明 | 
|---------|---------|
| QcloudCDWAccessForVpcLink | 管理 VPC 私有网络的权限| 
| QcloudCDWFullAccess | 授予 CDW 管理的完整权限| 
| QcloudCDWReadOnlyAccess | 授予 CDW 管理的只读权限|

- 设置让用户拥有创建和管理云数据仓库实例的权限，可对此用户使用名称为 QcloudCDWFullAccess 的策略。
![](https://main.qcloudimg.com/raw/8db0f7d3c1559893eb03d48615e56a0c.png)
- 设置让用户只拥有查询云数据仓库集群及相关资源（私有网络、安全组、Monitor）的权限，但不允许该用户拥有创建、删除和修改等操作的权限，可对此用户使用名称为 QcloudCDWReadOnlyAccess 的策略。
![](https://main.qcloudimg.com/raw/d06e519d0ad5262a9c7a6577ce0789b9.png)

## CDW 自定义策略
如果预设策略无法满足需求，可单击【新建自定义策略】创建自定义策略。
![](https://main.qcloudimg.com/raw/8bfa4cbaa3d4d5b7ece06c36efd32e13.png)
创建自定义策略的方法可参考 [策略设置](https://cloud.tencent.com/document/product/878/47410)。

## 策略授权
已设置好的策略可以通过关联用户组或者子用户来授予权限。
![](https://main.qcloudimg.com/raw/dfbfb56c3ba8129ea97dd1f8d3de3a2b.png)

## 自定义策略可授权资源类型
资源级权限指的是能够指定用户对哪些资源具有执行操作的能力。云数据仓库 CDW 部分支持资源级权限，即表示针对支持资源级权限的云数据仓库操作，您可以控制何时允许用户执行操作或是允许用户使用特定资源。访问管理 CAM 中可授权的资源类型如下：

| 资源类型   | 授权策略中的资源描述方法                                     |
| ---------- | ------------------------------------------------------------ |
| 云数据仓库 | `qcs::cdw:$region:$account:cdw-instance/*`<br>`qcs::cdw:$region:$account:cdw-instance/$clusterId` |

下表将介绍当前支持资源级权限的云数据仓库 API 操作，设置策略时，action 填入 API 操作名称就可以对单独 API 进行控制，设置 action 也可以使用\*作为通配符。

...
- **接口级授权：**您可以在 CAM 控制台中按接口维度给子账户授权，子账户将只能使用您已授权的接口。可参考使用示例中授予单个接口权限的操作 [授予单个接口权限](#接口权限)。
- **资源级授权：**您可以在 CAM 控制台中按人员库资源维度给子账户授权，子账户将只能操作您已授权的人员库。可参考 [支持资源级授权的接口列表](#授权)。

## 支持资源级授权的接口列表<span id="授权"></span>

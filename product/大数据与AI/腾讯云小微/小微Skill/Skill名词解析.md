...
### 设备类 Skill
第三方 SmartHome 类厂商期望接入小微平台，使得用户可以通过语音控制其他智能设备，请选用设备类 Skill。如用户对小微音箱说“打开灯”来控制智能灯的打开。目前小微支持灯、空调、窗帘、插座等常用 SmartHome设备的开关和调节。

### 本地Skill
对于已经内置了小微语音交互能力的设备，需要通过小微语音能力来控制设备自身，请选用本地 Skill。如长安汽车已经植入了腾讯云小微，此时需要通过语音来开关天窗，可以建立一个本地 Skill。 本地 Skill 没有 endpoint 地址，所有解析出的意图和槽位将会通过腾讯云小微 SDK 直接传递给设备本身。
...
- 建议使用不少于 3 个字的激活词；
- 除非你是版权拥有者，不能使用众所周知的版权名、人名等，比如“星巴克”、“王力宏”；
- 激活词不能是常用的地名、人名，比如“北京”、“老王”；
- 不能使用常用的词语和短语，比如“吃饭”、“你好”等；
- 不能使用包含小微的唤醒词，比如“嗨小微”、“小微你好”、“你好小微”；
...
### 槽位类型
槽位类型规范了槽位所包含的值，如果你的槽位选择了槽位类型是 T.NUMBER，那就只有数字会被匹配到你的槽位；如果你的槽位选择了槽位类型是 T.LOC_CITY，那就只有城市名会被匹配到你的槽位。每个槽位都需要选择一个槽位类型。
...
在新建槽位时，只需要在槽位类型栏目选择“constellation”就可以使用到刚刚新建的槽位类型。

...
| 指标名称                                   | 指标单位        | 指标含义                                   |
| ------------------------------------------ | --------------- | ------------------------------------------ |
| TotalReadTime                              | ms              | 读操作花费时间(ms)                         |
| TotalWriteTime                             | ms              | 写操作花费时间(ms)                         |
| XceiverCount                               | 个              | Xceiver 数量                             |
| BytesWritten                               | Bytes/s         | 写入 DN 的字节速率                           |
| BytesRead                                  | Bytes/s         | 读取 DN 的字节速率                           |
| RemoteBytesRead                            | Bytes/s         | 远程客户端读取字节速率                     |
| RemoteBytesWritten                         | Bytes/s         | 远程客户端写入字节速率                     |
...
| SendDataPacketTransferNanosNumOps          | 次/s            | 每秒发送数据包次数                         |
| SendDataPacketBlockedOnNetworkNanosNumOps  | 次/s            | 传输数据包 QPS                              |
| SendDataPacketTransferNanosAvgTime         | ms              | 发送数据包平均时间                         |
| SendDataPacketBlockedOnNetworkNanosAvgTime | ms              | 发送数据块平均时间                         |
...
| ReplaceBlockOpNumOps                       | 次/s            | Replace Block操作的 OPS                   |
...
| MemNonHeapMaxM                             | MB              | JVM 配置的 NonHeapMemory 的大小          |
| MemHeapUsedM                               | MB              | JVM 当前已经使用的 HeapMemory 的大小     |
| MemHeapCommittedM                          | MB              | JVM HeapMemory提交大小                   |
| MemHeapMaxM                                | MB              | JVM 配置的 HeapMemory 的大小             |
| MemMaxM                                    | MB              | JVM 运行时的可以使用的最大的内存的大小   |
| ThreadsNew                                 | 个              | 处于新建状态的线程数量                     |
| ThreadsRunnable                            | 个              | 处于可运行状态的线程数量                   |
...
| AllocatedVCores,queue              | 核       | CPU 核数          |
| ReservedVCores,queue               | 核       | CPU 核数          |
| AvailableVCores,queue              | 核       | CPU 核数          |
| PendingVCores,queue                | 核       | CPU 核数          |
| AppsSubmitted,queue                | 个       | 应用总数         |
| AppsRunning,queue                  | 个       | 应用总数         |
| AppsPending,queue                  | 个       | 应用总数         |
| AppsCompleted,queue                | 个       | 应用总数         |
| AppsKilled,queue                   | 个       | 应用总数         |
| AppsFailed,queue                   | 个       | 应用总数         |
| ActiveApplications,queue           | 个       | 应用总数         |
| running_0,queue                    | 个       | 应用总数         |
| running_60,queue                   | 个       | 应用总数         |
| running_300,queue                  | 个       | 应用总数         |
| running_1440,queue                 | 个       | 应用总数         |
| AllocatedMB,queue                  | MB       | 内存大小         |
| AvailableMB,queue                  | MB       | 内存大小         |
| PendingMB,queue                    | MB       | 内存大小         |
| ReservedMB,queue                   | MB       | 内存大小         |
| AllocatedContainers,queue          | 个       | 容器个数         |
| PendingContainers,queue            | 个       | 容器个数         |
| ReservedContainers,queue           | 个       | 容器个数         |
| AggregateContainersAllocated,queue | 个       | 容器分配释放总数 |
| AggregateContainersReleased,queue  | 个       | 容器分配释放总数 |
| ActiveUsers,queue                  | 个       | 用户数           |
...
| RitDuration_num_ops     | 次       | 集群 Assignment 管理器操作 |
...
| ThreadsTerminated                         | 个       | JVM 线程数量   |
| numOpenConnections                        | 个       | RPC 连接数     |
| exceptions.FailedSanityCheckException     | 次       | RPC 异常次数   |
| exceptions.NotServingRegionException      | 次       | RPC 异常次数   |
| exceptions.OutOfOrderScannerNextException | 次       | RPC 异常次数   |
| exceptions.RegionMovedException           | 次       | RPC 异常次数   |
| exceptions.RegionTooBusyException         | 次       | RPC 异常次数   |
| exceptions.UnknownScannerException        | 次       | RPC 异常次数   |
...
| totalRequestCount                         | 个/s     | 读写请求量         |
| readRequestCount                          | 个/s     | 读写请求量         |
| writeRequestCount                         | 个/s     | 读写请求量         |
...
| Active            | GB   | 最近经常被使用的内存总量             |
| Inactive          | GB   | 最近不经常被使用的内存总量           |
| Mlocked           | GB   | 被用户程序锁定而不能被回收的内存总量 |
| Mapped            | GB   | 设备和文件等映射的内存总量           |
...
| AbortOnTimeout             | 次/s     | 因各种计时器（RTO/PTO/keepalive）的重传次数超过上限而关闭连接的 |
...
| TimeWaitOverflow           | 个/s     | 因为超过限制而无法分配的 TIME_WAIT socket 数量             |
...
| Tcp_RtoAlgorithm           | 1/s      | 转发未答复对象的延时的算法的数                               |
| Tcp_RtoMax                 | 1/s      | TCP 延迟重发的最大值                                          |
| Tcp_RtoMin                 | 1/s      | TCP 延迟重发的最小值                                          |
| TcpExt_TCPLostRetransmit   | 次/s     | 丢失重传 SKB 的次数                                            |
| TcpExt_TCPFastRetrans      | 次/s     | 快重传 SKB 次数                                                |
| TcpExt_TCPForwardRetrans   | 次/s     | 一般重传 SKB 次数                                              |
| TcpExt_TCPSlowStartRetrans | 次/s     | 成功慢启动重传 SKB 数量                                        |
| TcpExt_TCPRetransFail      | 次/s     | 尝试重传失败次数                                             |
| Udp_OutDatagrams           | 个/s     | 发送 UDP 数据包文数量                                          |
| Udp_InDatagrams            | 个/s     | 接收 UDP 数据报文数量                                          |
| Udp_InErrors               | 个/s     | 接收 UDP 异常数据报文数量                                      |
| receive_bytes              | MB/s     | 网卡接收数据量                                               |
| transmit_bytes             | MB/s     | 网卡发送数据量                                               |
| receive_drop               | packet/s | 网卡接收丢弃数据量                                           |
| receive_errs               | packet/s | 网卡接收异常数量                                             |
| receive_packets_total      | packet/s | 网卡接收数据包数量                                           |
| transmit_drop              | packet/s | 网卡发送丢弃数据量                                           |
| transmit_errs              | packet/s | 网卡发送异常数据量                                           |
| transmit_packets_total     | packet/s | 网卡发送包数量                                               |
...
| Total        | GB   | 磁盘总空间         |
| UsedPercent  | %    | 磁盘使用率         |
| IFree        | 个   | 磁盘剩余 INODES 数量 |
| ITotal       | 个   | 磁盘 INODES 总数量   |
| IUsedPercent | %    | 磁盘 INODES 使用率   |
...
| maximum     | 个   | 最大文件句柄数量             |
| size_bytes  | GB   | 文件系统总容量               |
| free_bytes  | GB   | 文件系统空余容量             |
| avail_bytes | GB   | 文件系统对非 root 用户有效容量 |
| files       | 个   | 文件 inodes 总数               |
| files_free  | 个   | 空闲文件 inodes 数量           |

...
## 安全组
安全组是一种有状态的包过滤功能的虚拟防火墙，它用于设置单台或多台云服务器（节点）的网络访问控制，是腾讯云提供的重要的网络安全隔离手段。若没有安全组，创建 EMR 集群时会自动帮您新建一个安全组，若安全组数量已达到上限无法新建，可删除部分不再使用的安全组。若已有安全组，可直接在 [私有网络控制台](https:\console.cloud.tencent.com\cvm\securitygroup) 查看并使用已有的安全组。

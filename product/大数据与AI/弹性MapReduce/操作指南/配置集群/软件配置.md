...
## 自定义软件配置
Hadoop、Hive 等软件含有大量配置，通过软件配置功能您可以在新建集群的过程中自主组件配置参数。配置过程需要您按要求提供相应的 json 文件，文件可以由您自定义，也可以将存量集群的 [软件配置参数导出]()，然后以此快速新建一个熟悉的集群。
>?目前仅以下文件中的参数支持自定义配置：
...
**json 文件及说明：**
以假设条件为例，框内应该填入 json 文件（json 内容要求同 [软件配置]()）。
```
[
    {
        "serviceName": "HDFS",

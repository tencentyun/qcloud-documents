...
>- 一旦选择了 EMR 某个版本创建了集群，该集群使用的 EMR 版本和组件版本不会自动升级，例如选 EMR-V2.0.1 版本，那么 Hadoop 就一直保持在 2.7.3，Spark 就一直保持在 2.2.1。后续如果版本升级到了 EMR-V2.1.0，Hadoop 到了 2.8.4，Spark 到了 2.3.2 也不会影响到已经创建出来的集群。只有新的集群才会使用新的镜像。

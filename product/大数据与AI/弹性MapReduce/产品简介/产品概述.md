...
弹性 MapReduce 目前支持多种资源规格，您可以采用标准型、标准网络优化型、内存型、高 IO 型、计算型、计算网络增强型及大数据机型实例作为计算资源。若您需要在黑石物理主机上部署集群，可通过 [在线支持](https://cloud.tencent.com/online-service?from=sales&source=PRESALE) 联系我们。

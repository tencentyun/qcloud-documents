...
|Read Type |	Behavior |
|--|--|
|CACHE_PROMOTE |	如果读取的数据在 Worker 上时，该数据被移动到 Worker 的最高层。如果该数据不在本地 Worker 的 Alluxio 存储中，那么就将一个副本添加到本地 Alluxio Worker 中。 <br>如果</br>`alluxio.user.file.cache.partially.read.block`设置为 true，没有完全读取的数据块也会被`全部`存到 Alluxio 内。相反，一个数据块只有完全被读取时，才能被缓存。
| CACHE	| 如果该数据不在本地 Worker 的 Alluxio 存储中，那么就将一个副本添加到本地 Alluxio Worker 中。<br>如果</br>`alluxio.user.file.cache.partially.read.block`设置为 true，没有完全读取的数据块也会被`全部`存到 Alluxio 内。相反，一个数据块只有完全被读取时，才能被缓存。

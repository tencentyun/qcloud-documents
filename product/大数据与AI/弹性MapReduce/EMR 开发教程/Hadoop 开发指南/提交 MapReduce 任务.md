...
- 确认您已经开通了腾讯云，并且创建了一个 EMR 集群。在创建 EMR 集群的时候在基础配置页面勾选 “开启 COS”，并在下方填写自己的 SecretId 和 SecretKey。SecretId 和 SecretKey 可以在 [API 密钥管理界面](https://console.qcloud.com/cam/capi) 查看。如果还没有密钥，请单击【新建密钥】建立一个新的密钥。
...
## 3. 数据准备
您需要准备统计的文本文件。准备分为两种方式：**数据存储在 HDFS 集群** 和 **数据存储在 COS**。
...
- 在本地通过 COS 的控制台上传，如果数据文件已经在 COS 可以通过如下命令查看：
 ```

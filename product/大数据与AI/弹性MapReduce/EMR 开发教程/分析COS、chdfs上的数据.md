...
## 开发准备
1. 如果任务中需要访问腾讯云对象存储（COS），就需要在 COS 中先 [创建一个存储桶（Bucket）](https://cloud.tencent.com/document/product/436/13309)。如果任务中需要访问腾讯云 CHDFS，需要创建和挂载 CHDFS，具体可以参考 [云 HDFS](https://console.cloud.tencent.com/chdfs/filesystem)。确认您已经开通了腾讯云，并且创建了一个 EMR 集群。

...
在 config.xml 配置文件同级的 conf.d 和 config.d 文件夹下，用户可以新建`*.xml`文件来覆盖 config.xml 文件中的配置（**这里不建议用户手动更改，建议走控制台提供的配置下发功能统一下发配置**）。

例如，在 config.xml 文件同级的目录下新建 config.d 目录：
1. 修改 clickhouse-server 监听的 http port，在 config.xml 文件中默认为9000端口。在 config.d 文件中新建 tcp_port.xml 文件，内容如下：
```
<yandex>
...
```
重启 clickhouse-server，clickhouse-client 执行命令`SELECT * FROM system.metric_log LIMIT 1 FORMAT Vertical;`，可以发现 system.metric_log 表已经自动被创建。

## metrika.xml

ClickHouse 配置提供了“替换”的功能，可以使用**incl**属性将指定文件中的配置替换到 config.xml 配置文件中来，这样使得主配置文件不至于过于冗余，方便维护。在默认的 config.xml 配置中，我们可以看到`<remote_servers>`、`<macros>` 、`<zookeeper>`这三个标签都是有**incl**属性的，可以将 metrika.xml 文件中对应的`<clickhouse_remote_servers>`、、配置加载到 config.xml 文件中。替换文件的路径默认为`/etc/metrika.xml`，可以通过`<include_from>`配置来修改。

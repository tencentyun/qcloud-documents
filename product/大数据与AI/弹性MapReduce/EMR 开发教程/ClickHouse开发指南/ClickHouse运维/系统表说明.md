...
**列信息：**
对于集群的每一个查询，errors_count 都会更新一次，但 estimated_recovery_time 会根据需求重新计算，所以可能会出现这样的情况，当 errors_count 非零、estimated_recovery_time 为零时，下次查询将会把 errors_count 置为零，并且尝试使用副本，就好像没有错误一样。

...
在 EMR 命令行先使用以下指令切换到 Hadoop 用户,并进行目录/usr/local/service/spark ：

`[root@172 ~]# su hadoop`
 [root@172 root]$ cd / usr/local/service/spark
从 [Kafka 官网](http://kafka.apache.org/downloads) 下载安装包，注意选择合适的版本，kafka客户端版和腾讯云ckafka兼容性强，安装对应的kafka客户端版本即可。

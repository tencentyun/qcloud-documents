...
## 标签鉴权操作指引
下文将为您介绍 EMR 标签授权的操作流程，使用标签对 EMR 上的资源进行授权管理前，您需要先对公司的部门或组织做好标签规划；列如下图，分别对部门 A/B/C 规划好对应的用户名、集群、标签等。标签相关操作请参考：[标签](https://cloud.tencent.com/document/product/651) 。
...
- 以子用户身份登陆进行验证；
	此时登陆 EMR，子用户仅能通过标签 tag_A 看见和管理 test_A 集群。
...
## 资源级和接口级鉴权操作指引
下文将为您介绍 EMR 资源级授权的操作流程，使用资源级鉴权对 EMR 具体某功能授权时，您需要先对公司的部门或组织做好规划；列如下图，分别对部门 A/B/C 规划好对应的用户名、接口、集群等；
...
>! 支持资源级鉴权接口可参考**支持资源进授权的 API 列表**，因接口暂时只能采用列表的形式全量罗列，不排除后续接口新增时出现操作报错，如遇到权限报错情况，可根据报错提示缺少对应接口权限进行策略中补充。
...
- 子账号登陆后，调用上述接口出现类似下面需要授权的弹窗，说明鉴权成功。

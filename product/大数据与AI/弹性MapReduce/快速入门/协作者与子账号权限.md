...
- 使用 EMR 产品服务时需要访问云上 CVM、CBS、TencentDB 等服务，在首次进行产品购买时需要赋予 EMR 产品**服务角色 EMR_QCSRole** 并授予策略 **QcloudAccessForEMRRole**（用于申请 CVM、CBS、TencentDB 等基础资源以及对 COS 的读权限）及弹性 MapReduce（EMR）对云资源的访问权限。

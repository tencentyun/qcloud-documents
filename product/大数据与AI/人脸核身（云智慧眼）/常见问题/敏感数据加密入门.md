...
### 操作流程
1. 获取加密密钥，业务后台通过 API 调用 密钥管理系统 KMS [GenerateDataKey](https://cloud.tencent.com/document/product/573/34419) 接口生成数据密钥**明文密钥** Plaintext 和 **密文密钥** CiphertextBlob 。
 GenerateDataKey 接口参数解析：
     - KeyId 统一使用第一步人脸核身控制台生成的 **密钥 ID** 
     - KeySpec 统一选择 **AES_256**
2. 业务方对需要加密的敏感数据 使用 **AES-256-ECB** 加密算法 和 **明文密钥** 将数据进行加密，AES加密可参考 [加密示例](#加密) 中各语言示例。

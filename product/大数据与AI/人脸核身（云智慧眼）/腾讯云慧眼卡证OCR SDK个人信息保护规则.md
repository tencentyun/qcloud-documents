## 更新说明
我们对《腾讯云慧眼身份证 OCR SDK 个人信息保护规则》进行了更新，更新内容主要为：
1. 区分“腾讯云慧眼人脸核身基础版 SDK”、“腾讯云慧眼人脸核身增强版 SDK”和“腾讯云慧眼卡证身份证 OCR SDK”，独立设置《腾讯云慧眼身份证卡证 OCR SDK 个人信息保护规则》；
...
8. “腾讯云慧眼身份证 OCR SDK” 更名为“腾讯云慧眼卡证 OCR SDK”；
9. 因本 SDK 新增银行卡、驾驶证、行驶证三类卡证的识别能力，对应增加银行卡信息、驾驶证信息、行驶证信息的收集及处理说明；
10. iOS 端新增手机型号、系统版本、网络信息收集及处理说明；
11. 更新相机权限的使用目的及拒绝的影响；
12. 更新第三方 bugly SDK 的使用场景；
13. 变更公司注册地址。
14. 进一步细化“故障问题进行排查、分析、解决”的处理目的表述。

## 引言
腾讯云慧眼卡证 OCR SDK 产品（以下简称“SDK 产品”）由腾讯云计算（北京）有限责任公司（以下简称“我们”）开发，公司注册地为北京市海淀区西北旺东路10号院西区9号楼4层101。

《腾讯云慧眼卡证 OCR SDK 个人信息保护规则》（以下简称“本规则”）主要向第三方开发者及其终端用户（以下简称“终端用户”）说明，为了实现 SDK 产品的相关功能，本 SDK 产品将如何处理终端用户的个人信息，“处理”包括收集、存储、使用、加工、传输、提供、公开个人信息等行为。

在注册、接入、使用腾讯云慧眼卡证 OCR SDK 产品和/或相关服务前，请第三方开发者和终端用户务必认真阅读本规则。如您是第三方开发者，您应当将本规则向终端用户公开提供，以便终端用户在使用由您开发并适配、集成和装载本 SDK 产品的产品和/或相关服务前，能够仔细阅读并理解本规则，**如果您不同意本规则的任何内容，应立即停止接入及使用 SDK 产品和/或相关服务，同时，您应仅在获得终端用户的同意后使用本 SDK 产品并处理终端用户的个人信息，在获得终端用户同意前不得启用或初始化本 SDK 产品。**
...
|-|-|-|-|
|身份证信息（身份证照片）<br>注：由开发者决定是否调用该功能模块和是否收集|实名认证|用户在使用第三方开发者应用中与身份验证相关的功能场景|加密传输和必要处理：将身份证照片进行 OCR 技术识别（即文字识别技术），识别出姓名、性别、出生日期、地 址、身份证号码、民族、证件有效期、发证机关、人像照片，并返回给开发者|
|银行卡信息（银行卡照片）<br>注：由开发者决定是否调用该功能模块和是否收集|文字识别|用户在使用第三方开发者应用中与银行卡识别相关的功能场景|加密传输和必要处理：将银行卡照片进行 OCR 技术识别（即文字识别技术），识别出银行卡号、并与银行卡照片一并返回给开发者|
|驾驶证信息（驾驶证照片）<br>注：由开发者决定是否调用该功能模块和是否收集|文字识别|用户在使用第三方开发者应用中与驾驶证识别相关的功能场景|加密传输和必要处理：将驾驶证照片进行 OCR 技术识别（即文字识别技术），识别出姓名、性别、国籍、住址、出生日期、领证日期、准驾车型、起始日期、有效日期、证件红章，并与驾驶证照片一并返回给开发者|
|行驶证信息（行驶证照片）<br>注：由开发者决定是否调用该功能模块和是否收集|文字识别|用户在使用第三方开发者应用中与行驶证识别相关的功能场景|加密传输和必要处理：将行驶证照片进行 OCR 技术识别（即文字识别技术），识别出车牌号码、车辆类型、所属人姓名、住址、使用性质、品牌型号、识别代码、发动机号、证件红章、注册日期、发证日期、创建日期、更新日期、核载人数、核定载质量、档案编号、总质量、检验记录、外廓尺寸、准牵引总质量、准备质量、驾照车牌号并与行驶证正本照片、行驶证副本照片一并返回给开发者|
|Android 端：手机型号、手机品牌、 Android 系统版本、Android 系统 api 等级、厂商系统版本、cpu 架构类 型、设备是否 root、磁盘空间占用大小、sdcard 空间占用大小、内存空间占用大小、网络类型、应用当前正在运行的进程名和 PID|对 SDK 运行时发生的故障问题进行排查，分析 SDK 和设备本身的异常和性能问题，定位和分析异常|定位并解决用户在使用本 SDK 服务时遇到的问题|加密传输和必要处理：用于故障问题的排查、分析、解决|
|iOS 端：手机型号、系统版本、网络信息|对 SDK 运行时发生的故障问题进行排查，分析 SDK 和设备本身的异常和性能问题，定位和分析异常|定位并解决用户在使用本 SDK 服务时遇到的问题|加密传输和必要处理：用于故障问题的排查、分析、解决|

**身份证信息、银行卡信息、驾驶证信息、行驶证信息属于敏感个人信息，**且是我们为第三方开发者提供本服务所必需，如第三方开发者和终端用户选择不提供或不同意我们处理以上敏感个人信息的，将导致第三方开发者和终端用户无法正常使用本 SDK 产品提供的服务。 本 SDK 产品除依据上述情形收集终端用户的个人信息外，还会集成其他可供第三方开发者选择是否接入的其他 SDK 产品，我们将会在本规则第三条说明相应的 SDK 列表。

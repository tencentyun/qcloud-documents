...
|驾驶证信息（驾驶证照片）<br>注：由开发者决定是否调用该功能模块和是否收集|文字识别|用户在使用第三方开发者应用中与驾驶证识别相关的功能场景|加密传输和必要处理：将驾驶证照片进行 OCR 技术识别（即文字识别技术），识别出姓名、性别、国际、住址、出生日期、领证日期、准驾车型、起始日期、有效日期、证件红章，并与驾驶证照片一并返回给开发者|
...
|Android 端：手机型号、手机品牌、 Android 系统版本、Android 系统 api 等级、厂商系统版本、cpu 架构类 型、设备是否 root、磁盘空间占用大小、sdcard 空间占用大小、内存空间占用大小、网络类型、应用当前正在运行的进程名和 PID|本 SDK 运行时对发生的故障问题进行排查、分 析、解决|定位并解决用户在使用本 SDK 服务时遇到的问题|加密传输和必要处理：用于故障问题的排查、分析、解决|
|iOS 端：手机型号、系统版本、网络信息|本 SDK 运行时对发生的故障问题进行排查、分 析、解决|定位并解决用户在使用本 SDK 服务时遇到的问题|加密传输和必要处理：用于故障问题的排查、分析、解决|

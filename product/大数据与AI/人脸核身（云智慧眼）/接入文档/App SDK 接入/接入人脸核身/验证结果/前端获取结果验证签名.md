如果您只需要获取刷脸结果，您可以根据我们刷脸完成后的回调url请求中的参数中的 code 判断是否刷脸通过，code 0表示人脸核身成功，其他错误码标识失败。如果您需要拉取人脸核身的视频和图片用于存证等其他需要，请参看 [服务端获取验证结果](https://cloud.tencent.com/document/product/1007/61302)。（您可以在收到完成刷脸的回调后，再来我们的服务器获取到刷脸视频和照片）为了确保前端 H5 返回的结果真实性且未被篡改，合作伙伴服务端可以根据刷脸完成后的回调 url 请求中的参数中的 newSignature 参数验证结果是否真实有效。

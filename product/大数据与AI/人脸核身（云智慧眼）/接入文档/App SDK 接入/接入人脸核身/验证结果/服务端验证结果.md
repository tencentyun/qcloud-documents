...
- 由于照片和视频信息有可能超过1MB，建议合作伙伴在使用时，获取比对结果用于后续流程处理和存证使用并分开调用，避免网络传输带来的影响。2目前我们的查询接口是异步查询，返回的文件可能会有1/2s的延迟。如果合作方是完成人脸核身流程后马上去拉取视频/照片，建议在查询接口加上一个查询机制：判断图片是否存在，轮询3次，每次2s。

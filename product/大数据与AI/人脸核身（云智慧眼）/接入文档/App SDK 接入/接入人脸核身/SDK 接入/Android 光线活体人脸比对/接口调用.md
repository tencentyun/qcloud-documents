...
WbCloudFaceVeirfyLoginListner loginListerner){
//    ...
}
    /**
     * 登录成功后，调用此函数拉起 sdk 页面。
     * 由 WbCloudFaceVeirfyResultListener 返回刷脸结果。
     */
public void startWbFaceVeirifySdk(Context ctx,
WbCloudFaceVeirfyResultListener listener) {    // ...
}
```
...
- **自带比对源比对类型**
合作方在获取 faceId 的接口里送上自己提供对比源数据，获取 faceId 后送入 SDK 进行对比。合作方可以上送两类照片，一类是网纹照，一类是高清照；设置代码如下： 
 ```

...
| verifyMode        | 人脸核身类型<br/>光线活体：FaceVerifyStatus.Mode.REFLECTION<br/>动作活体是：FaceVerifyStatus.Mode.ACT<br/> | FaceVerifyStatus.Mode | -                  | 是       |
...
### 合作方个性化提示定制
自定义提示语分为短提示（不长于17个字符）和长提示（不长于70个字符）。如果不设置，默认无。如果超过字数限制，将会被截断显示。

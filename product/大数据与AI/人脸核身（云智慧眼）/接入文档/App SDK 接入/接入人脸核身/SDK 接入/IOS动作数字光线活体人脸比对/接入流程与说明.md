## 整体工作流程
人脸识别主要流程包括以下步骤：
1. 登录人脸识别 SDK，即 initSDKinitSDKWitXXX 方法
2. 登录结果返回，在第一步登录成功以后，再调用startWbFaceVeirifySdk 方法拉起人脸识别页面，如果登录失败，会通过 failure block 回调返回一个 WBFaceError 对象。
3. 人脸识别的具体过程，如果成功进入第4步，如果失败进入第5步。

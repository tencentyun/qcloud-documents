...
 
此 SDK 接口中
合作方后台开发需要通过后台接口获取 sign,
然后根据自带比对源接口, 通过后台接口获取 faceId!!!!(native 端无需传入自带比对源图)
...
@param licence 在人脸核身控制台内申请(该 licence 同 app 当前使用的 bundle id绑定)
@param faceId 合作方必须要先在获取faceId的接口里送入用户自带比对源图片信息，得到相应的 faceId 后，再送入sdk, 若为空，则开启仅活体检测模式!!!!
...
 default: YES 默认打开闭眼检测
 */
@property (nonatomic, assign) BOOL detectCloseEyes;

...
- 人脸核身 SDK 将以 AAR 文件的形式提供，即 WbCloudFaceVerifySdk，其中数字活体模式需要配合资源包（WbCloudFaceRes） 一起使用。
- 人脸核身 SDK 同时需要依赖**云公共组件（WbCloudNormal）**，同样也是以 AAR 文件的形式提供，详情请参见 [添加依赖](#yilai)。

### 基本构成

- 人脸核身 SDK 分为动作活体和数字活体两个模式。
  下图为人脸核身 SDK 基本构成：
...
云人脸核身产品的混淆规则分为：云人脸核身 SDK（动作活体/数字活体）的混淆规则、云公共组件的混淆规则、依赖的第三方库混淆规则，下面将为您详细介绍。

### 云人脸核身 SDK（动作活体）的混淆规则

...
2. 根据客户自身需求，按照不同的方式跳转进入腾讯云人脸核身流程。
 - 例如：通过小程序 SDK 接入：客户后端将 BizToken 返回给客户小程序端，然后按照小程序 SDK 接入文档接入即可。
3. 人脸核身完成后，流程会回调至客户侧（HTML5 类型以回调地址形式返回、SDK 类型以回调函数形式返回），客户后端可凭借回调中提供的 BizToken 调用    [GetDetectInfo](https://cloud.tencent.com/document/api/1007/31331) 接口获取本次核身的详细信息。

## SDK 接入
### 开发准备
#### 下载 SDK
本 SDK 暂不支持通过外部 npm 源下载，线下对接时，腾讯云会提供 SDK（包名为`verify_mpsdk`）。

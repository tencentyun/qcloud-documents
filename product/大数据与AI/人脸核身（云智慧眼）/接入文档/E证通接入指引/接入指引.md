...
3. 审核通过后，可以看到商户 ID 和E证通下程序 SDK，后续需要使用商户 ID 获取E证通服务流程唯一标识 EidToken。 
...
[](id:anzhuangsdk)
### 3.2 安装 SDK
商户 ID 申请通过后，在[控制台商户 ID 列表页](https://console.cloud.tencent.com/faceid/access/version?sdkType=eidMpSdk)可以下载E证通小程序 SDK。下载完成后将E证通小程序 SDK 文件夹放在小程序根目录下。

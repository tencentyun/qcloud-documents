## 生成签名
### 前置条件
**请合作方确保 SIGN ticket 已经正常获取，获取方式见 [SIGN ticket 获取](#_SIGN_ticket_获取)**。合作方根据本次人脸核身的如下参数生成签名，需要签名的参数信息如下：
...
| version    | 1.0.0                                                        | -                                                            |
| api ticket | 合作伙伴服务端缓存的 tikcet，注意是 SIGN 类型          | 获取方式见 [SIGN ticket 获取](#_SIGN_ticket_获取)             |

### 基本步骤
1. 将 appId、orderNo、name、idNo、userId、version、ticket（SIGN 类型）共七个参数的值进行字典序排序。
2. 将排序后的所有参数字符串拼接成一个字符串。
3. 将排序后的字符串进行 SHA1 编码，编码后的40位字符串作为签名（sign）。签名算法可参考 [签名算法说明](#_签名算法说明) 。
...
**请求 URL：**
```
[https://miniprogram-kyc.tencentcloudapi.com/api/server/h5/geth5faceid](https://idasc.webank.com/api/server/h5/geth5faceid)?orderNo=xxx
```

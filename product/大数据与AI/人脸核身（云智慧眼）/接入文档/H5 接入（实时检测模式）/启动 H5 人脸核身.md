...
>?App 内调用 H5 人脸核身或者短信链接调用 H5 人脸核身等场景，若当前设备无法正常调用摄像头进行视频录制，前端将返回特定的错误码（如下）告知合作方，合作方拿到错误码后可选择用备用方案核身处理。
...
## 生成签名
### 前置条件
请合作方确保 NONCE ticket 已经正常获取，获取方式见 [NONCE ticket 获取](#_NONCE_ticket_获取)。合作方根据本次人脸核身的如下参数生成签名,需要签名的参数信息如下：
...
| api ticket  | 合作伙伴服务端获取的 tikcet，注意是 NONCE 类型       | 获取方式见 [NONCE ticket 获取](#_NONCE_ticket_获取)（所用的 userid 参数值需要和接口里面的 userId 值保持一致） |
...
4. 将排序后的字符串进行 SHA1 编码，编码后的40位字符串作为签名（sign）。签名算法可参考 [签名算法说明 ](#_签名算法说明)。
...
**请求示例：**
```
https:// miniprogram-kyc.tencentcloudapi.com/api/web/login?webankAppId=appId001&version=1.0.0&nonce=4bu6a5nv9t678m2t9je5819q46y9hf93&orderNo=161709188560917432576916585&h5faceId=wb04f10695c3651ce155fea7070b74c9&url=https%3a%2f%2fcloud.tencent.com&from=browser&userId=23333333333333&sign=5DD4184F4FB26B7B9F6DC3D7D2AB3319E5F7415F&redirectType=1
```

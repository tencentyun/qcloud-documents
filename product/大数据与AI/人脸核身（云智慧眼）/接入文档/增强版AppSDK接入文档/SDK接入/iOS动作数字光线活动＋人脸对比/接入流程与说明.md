...
### 方法功能一：
如果使用活体检测服务+人脸比对服务（身份证的网纹照片进行对比）方式，则需要传入 faceID，其中 faceID 需要合作方后台调用 [合作方后台上传身份信息](https://cloud.tencent.com/document/product/1007/57617) 生成 faceID，其中 userid、nonce、sign、orderNo 要与生成 sign 时，使用的相同。

### 方法功能二：
如果使用活体检测服务+人脸比对服务（合作方提供的比对源图片进行比对），则需要传入 faceID，其中 faceID 需要合作方后台调用 [合作方后台上传身份信息](https://cloud.tencent.com/document/product/1007/57617) 生成 faceID，其中 userid、nonce、sign、orderNo 要与生成 sign 时，使用的相同，在生成 faceID 时传入比对源图片。

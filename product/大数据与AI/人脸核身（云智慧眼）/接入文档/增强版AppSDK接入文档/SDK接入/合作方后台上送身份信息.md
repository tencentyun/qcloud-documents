...
- 前置条件：请合作方确保 NONCE ticket 已经正常获取，获取方式见	[获取 NONCE titcket](https://cloud.tencent.com/document/product/1007/57614)。
...
| version  | 参数值为：1.0.0                                |     -                                                         |
| ticket   | 合作伙伴服务端实时获取的 ticket，注意是 NONCE 类型 | 获取方式请参见 [获取 NONCE ticket](https://cloud.tencent.com/document/product/1007/57614)（所用的 userId 参数值需要与 SDK 里面定义 userId 值保持一致） |
...
<tr><td>orderNo</td><td>	订单号，由合作方上送，每次唯一，不能超过32位	</td><td>String	</td><td>不能超过 32 位	</td><td>是</td></tr>
...
<tr><td>sourcePhotoStr	</td><td>比对源照片，<strong>注意：原始图片不能超过500k，且必须为 JPG 或 PNG 格式。</strong>
<br>参数有值：使用合作伙伴提供的比对源照片进行比对，必须注意是正脸可信照片，照片质量由合作方保证<br>参数为空 ：根据身份证号 + 姓名使用权威数据源比对</td><td>	BASE64String	</td><td>1048576</td><td>	否，非必填</td></tr>
<tr><td>sourcePhotoType</td><td>比对源照片类型<br>参数值为1 时是：水纹正脸照<br>参数值为 2 时是：高清正脸照<br><strong>重要提示：照片上无水波纹的为高清照，请勿传错，否则影响比对准确率。如有疑问，请联系腾讯云技术支持线下确认</strong></td><td>	String</td><td>	1</td><td>	是</td></tr>
...
<tr><td>advMode	</td><td>增强级标准0非增强级 1增强级，默认值为1	</td><td>String	</td><td>1</td><td>否</td></tr>
<tr><td>liveService</td><td>1为单独活体，其他模式可以不传</td><td>	String	</td><td>1</td><td>否</td></tr>
</tbody></table>

>?增强级 SDK 返回参数中，用 faceId 第三位标识是否是增强级，若为1则为增强级。

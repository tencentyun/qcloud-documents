...
- `WbCloudFaceVerifySdk.initAvdSdk()`的第二个参数用来传递数据，可以将参数打包到 `data(Bundle)` 中，必须传递的参数包括以下内容（参数详情请参见 [接口参数说明](#canshu)）：
...
### RiskInfo 对象说明
RiskInfo是 SDK 用来给合作方传递刷脸风险信息的对象，在 WbCloudFaceVerifyLoginListener 回调的 WbFaceVerifyResult 对象中作为参数返回给合作方 App。
...
详情见 [SaaS 错误码风险标签说明](https://cloud.tencent.com/document/product/1007/47912#.E5.A2.9E.E5.BC.BA.E7.BA.A7-riskinfo-.E8.BF.94.E5.9B.9E.E7.A0.81.E8.AF.B4.E6.98.8E.EF.BC.9A)。
...
# 在 MainActivity 中单击某个按钮的代码逻辑：
  ...
  //对 sdk 样式进行设置，默认为黑色模式
  //此处设置为白色模式
  data.putString(WbCloudFaceContant.COLOR_MODE, WbCloudFaceContant.CUSTOMER);
```
...
自定义长提示位于整个验证页面的下方，存在于整个刷脸流程中，不能设置位置和时机。长提示的显示除了设置代码外，还需邮件申请开通，具体 [联系小助手](https://cloud.tencent.com/document/product/1007/56130)。设置代码如下：
...
- SDK 为了进一步确保人脸核身的安全性，有录制用户人脸核身视频做存证。但其实在刷脸中，起到识别作用的并不是视频文件。在 SDK 使用过程中，视频录制在性能不太好的手机上可能会报错，导致人脸核身中断，影响用户体验。
...
### 是否开启闭眼检测
SDK 预检测期间提供闭眼检测功能，默认打开。合作方可以根据自身需求选择是否关闭，设置代码如下：
```
# 在 MainActivity 中单击某个按钮的代码逻辑：
  //先将必填的 InputData 放入 Bundle 中
  ...
  //设置是否打开闭眼检测，默认检测
 //此处设置为不检测
data.putBoolean(WbCloudFaceContant.IS_DETECT_CLOSE_EYES, false);
```

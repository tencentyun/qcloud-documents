...
|version|	默认值：1.0.0	|-|
|api ticket	|合作伙伴服务端缓存的 ticket，注意是 SIGN 类型	|获取方式见 [获取 SIGN ticket](https://cloud.tencent.com/document/product/1007/57613)|
|nonce	|32位随机字符串，字母和数字|	合作方自行生成，不要带有特殊字符|

### 基本步骤
1. 生成一个32位的随机字符串 nonce（其为字母和数字，登录时也要用到）。
2. 将 wbappid、orderNo、version 连同 api ticket、nonce 共5个参数的值进行字典序排序。

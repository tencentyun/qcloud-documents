...
接口请求频率限制：100次/每秒。
腾讯云语音合成技术（TTS）可以将任意文本转化为语音，实现让机器和应用张口说话。 腾讯 TTS 技术可以应用到很多场景，比如，移动 App 语音播报新闻；智能设备语音提醒；依靠网上现有节目或少量录音，快速合成明星语音，降低邀约成本；支持车载导航语音合成的个性化语音播报。**本接口内测期间免费使用**。
...
| Speed	| 否	| Float	| 语速，范围：[-2，2]，分别对应不同语速：<br>-2代表0.6倍<br>-1代表0.8倍<br>0代表1.0倍（默认值）<br>1代表1.2倍<br>2代表1.5倍|
| ProjectId	| 否 | Integer	| 项目 ID，用户自定义，默认为0。|
| VoiceType	| 否	| Integer	| 音色：<br>0：女声1，亲和风格（默认）<br>1：男声1，成熟风格<br>2：男声2，成熟风格 |
| PrimaryLanguage	| 否	| Integer	| 主语言类型：<br>1：中文，最大支持100字符<br>2：英文，最大支持400字符 |
| SampleRate	| 否	| Integer	| 音频采样率，16000：16k，8000：8k，默认为16k |

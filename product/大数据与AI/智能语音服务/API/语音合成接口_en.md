...
| Https Method | Yes | String | HTTPS request method. The request method is POST. |
...
| timestamp | Yes | uint | Current timestamp (in seconds). It is a value that aligns with the UNIX Epoch timestamp specifications. | 
| expired | Yes | uint | Validity period of the signature (in seconds). It is a value that aligns with the UNIX Epoch timestamp specifications. "expired" must be greater than "timestamp" and the interval between "expired" and "timestamp" must be less than 90 days. |
...
```
Taking < appid > = 20170111, < SecretKey >=oaYWFO70LGDmcpfwo8uF1IInayysGtgZ as an example, the combined original signature is:
```

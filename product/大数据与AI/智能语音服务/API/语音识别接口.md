...
| engine_model_type | 否 | String |引擎类型引擎模型类型。8k_0:8k 通用，16k_0:16k通用。|
...
90abclqs"
```
这里以 <appid>=1251000004, <SecretKey>=oaYWFO70LGDmcpfwo8uF1IInayysGtgZ 为例例拼接签名原文，则拼接的签名原文为： 
```
...
| 参数名称 | 类型 | 说明 |
|---------|---------|---------|
| code  | Int | 服务器器错误码，0 为成功。|
| message | String | 服务器器返回的信息。|
| voice_id | Int | 语音 ID。|
| seq | Int | 语音分片的序列号。|
| text | String | 语音转换的结果。|

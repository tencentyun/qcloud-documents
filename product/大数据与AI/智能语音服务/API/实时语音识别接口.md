...
**请求示例**
用户通过 [签名生成](#sign) 的签名为 qPHg2LZkMRZyC5pyVD8egRqxPc0=，上传语音分片 @/mnt/d/1.silk\_down，分片为流式语音的第一个分片（seq=0），采样率为16k，请求实时语音识别服务16k通用（engine\_model\_type = 16k\_0）引擎模型，结果返回方式为同步返回。具体的示例生成方式请参考下面 PHP 代码。

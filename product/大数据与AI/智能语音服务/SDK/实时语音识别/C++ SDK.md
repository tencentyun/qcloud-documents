...
如果不适合客户系统，自行安装方法：
下载speex源码，地址：https://speex.org/downloads/
解压进入源码目录
sudo ./configure
sudo make
sudo make install
...
```
其中v1表示API的版本,v1.0，后面125000001是AppID，各个参数的说明参考上表。

### 2. 接口说明
**2.1 TCloudRealtimeASR::Init**
```
/* 
** 初始化公共请求参数，此类参数较稳定不变
** config 公共请求参数结构体
...
```
**2.4 TCloudRealtimeASR::BuildRequest**
```
/* 
** 构造一个完整的请求
** Output int 返回结果
...
**4.4 传入音频数据**
调用 SetData 接口获取结果  
**4.5 sdk 已提供各个接口源码，用户可根据自身需要进行更改。**
## 快速入门例子

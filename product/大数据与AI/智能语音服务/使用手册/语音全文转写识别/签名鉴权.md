...
生成签名时，用户需要提前获取以下信息：腾讯云应用 ID（appid），腾讯云项目ID（projectid），安全凭证SecretId 和 SecretKey。
...
签名各字段说明和示例数值如下所示。注意，这里只是示例，用户根据自己实际的SecretId和SecretKey生成签名。
...
拼接的签名原文为：
```
POSTaai.qcloud.com/asr/v1/200001?expired=1473148422&nonce=12345&projectid=1000010&secretid=AKIDUfLUEUigQiXqm7CVSspKJnuaiIKtxqAv&source_type=0&template_name=example&timestamp=1473148302&url=http://test.audiourl.com
```
对原文进行加密处理：
```

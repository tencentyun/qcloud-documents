...
### 1.1 RESTful API 请求结构

长语音全文识别的 RESTful API 请求结构如下：
| 参数名称    | 必选   | 类型   | 描述    | 
| ------------- | ---------- | ------------- | ---------- |
...
| engine_model_type | 否  | uint  | 引擎类型，0，8k 通用模型；1，16k 通用模型| 
...
综上，语音全文转写识别请求 API 为：
```
http://aai.qcloud.com/asr/v1/<appid>?engine_model_type=0&expired=1473752807&nonce=44925&projectid=0&res_text_format=0&res_type=1&secretid=<secretid>&source_type=0&sub_service_type=0&timestamp=1473752207&url=<url>
```

## 2 返回结构

### 2.1 RESTful API 返回结果

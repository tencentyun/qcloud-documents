...
### 任务显示“运行中”，但实际上数据没有同步或反复同步重复数据？
**问题描述**：部分实时任务状态显示“运行中”，但是实际上数据并没有同步或反复同步重复的数据。通过“任务运维”界面跳转到任务日志页面，看到任务的日志存在报错信息，而任务一直进入重启状态
**原因分析**：由于 Oceanus 目前采用任务出错时一直重启的策略，因此每次重启后任务均会从上个 checkpoint 位置开始同步。当任务执行到异常的地方又回进入到重启状态并依次循环，因此才出现反复同步重复的数据
**解决方案**：当前版本优先建议订阅实时同步任务任务异常警告通知以监控任务情况，收到异常通知后可根据日志的错误进行排查。任务配置告警方式详情请参见 配置告警。
...
### 自定义字段配置是否支持复杂类型？
当前版本不支持 Array、Map、Struct 等复杂类型，仅支持基本类型。具体支持配置类型请参见【第8节-8.3自定义字段支持类型】
...
**解决方案**：
1.	请检查配置的来源和目标端之间的字段类型是否匹配，具体数据来源和目标端间类型转换请参见【第5节-数据来源配置及类型转换】、【第5节-数据去向配置及类型转换】
2.	若任务为 MySQL-Ckafka 单表同步任务中，请检查是否建立了管理 pkNames_wedata_di 、sqlType_wedata_di 与目标端映射关系。由于当前版本 Ckafka 字段不支持 Array、Map、Struct 等复杂类型，因此无法将 pkNames_wedata_di 、sqlType_wedata_di 参数写入 Ckafka 目标端。具体自定义参数支持类型请参见【第8节-8.3自定义字段支持类型】，数据来源和目标端间类型转换请参见【第5节-5.3数据来源配置及类型转换】、【第5节-5.4数据去向配置及类型转换】


### 腾讯云官网或者控制台提示“服务暂未开通”？
**解决方案**：WeData 目前处于公测阶段，需要在线申请使用 WeData。单击 [前往申请数据开发平台 WeData](https://cloud.tencent.com/apply/p/nsdkvxkgxvb)。

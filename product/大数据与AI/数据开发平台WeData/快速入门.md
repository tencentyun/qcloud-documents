...
3. <span id="jump">创建工作空间</span>
 - 入口
进入 [WeData 数据开发平台控制台](https://console.cloud.tencent.com/wedata/workspace/list) 工作空间列表页面，单击【创建工作空间】。
...
| 角色       | 说明                                                         |
| ---------- | ------------------------------------------------------------ |
| 空间管理员 | 工作空间的管理人员，拥有空间的所有权限，主要负责空间信息、空间公共资源的维护和管理；一个空间可以有多个管理人员，创建该空间的人默认是空间所有者 |
| 数据科学家 | 负责数据的开发、分析等工作，拥有数据开发和运维相关操作权限   |
| 运维工程师 | 负责在运维中心页面管理所有生产环境下的任务运行情况           |
| 普通成员   | 拥有空间所有信息的只读权限，在获得表负责人授权的条件下可进行表数据的查询 |
...
>- 子账号拥有 CAM的WeDataAdministor 策略（该策略权限需由主账号授予）后，即可创建工作空间。子账号创建空间的流程与主账号一致，详见 [创建工作空间](#jump)。
...
![](https://main.qcloudimg.com/raw/70830621bb6ff4070b59e0ad20d774b1.png)

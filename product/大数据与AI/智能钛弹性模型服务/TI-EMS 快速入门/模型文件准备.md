...
2. 在左侧导航栏单击【存储桶列表】，进入存储桶列表页面，单击左上角【创建存储桶】进行操作。详情请参见 [创建存储桶](https://cloud.tencent.com/document/product/436/13309) 。
3. 为了防止跨地域产生的不必要的费用和延时，模型文件所在的储存桶地域需要和服务运行的地域保持一致。例如，您预计将服务部署在广州地域，那么请将您的模型文件上传至广州地域的 COS 存储桶。
4. 单击存储桶名称，进入对应的存储桶，上传模型文件夹。详情请参见 [上传对象](https://cloud.tencent.com/document/product/436/13321) 。

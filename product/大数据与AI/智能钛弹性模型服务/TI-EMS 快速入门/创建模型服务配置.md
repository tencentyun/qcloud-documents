...
假设用户通过主账号（APPID 为1255502019）登录 COS 控制台进行创建存储桶，存储桶的配置信息为：地域选择为北京地域，存储桶名称自定义为ti-ems，模型文件夹为inception（[点击下载模型文件夹](http://ti-ems-1255502019.cosbj.myqcloud.com/tfserving/inception/inception_model.zip)），上传至存储桶ti-ems的`models/tfserving`路径下，如下图所示：

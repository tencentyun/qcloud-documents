...
选择模型资源配置，单击【GPU 配置】，选择2核 CPU，8192MB内存，2TFLOPS 配置项。模型服务配置创建完成之后，单击【确定】，进入模型服务配置页面。
>?TensorRT 镜像因为需要做模型转换，相对 TF-Serving 镜像需要更多 CPU 内存。根据模型不同，建议配置4GB以上 CPU 内存。

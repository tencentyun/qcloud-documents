...
| FailedOperation.ImageDecodeFailed | 图片解码失败。 | 是 |
...
| FailedcaOperation.ImageNoIdCard | 图片中未检测到身份证。 | 是 |
| FailedOperation.ImageNoText | 图片中未检测到文本。 | 是 |
| FailedOperation.IdCardInfoIllegal | 身份证信息不合法（身份证号、姓名字段校验非法等） | 是 |

...
自定义镜像的训练脚本只需要去这些目录下读取训练数据和配置，训练完后将模型文件写入到模型目录，checkpoint和训练日志写入到输出目录中，TI后台会自动将模型和输出目录的文件自动上传到用户指定的COS存储桶中。
...
| 输出目录             | /opt/ml/output/       |  存放训练过程的 checkpoint 及日志文件，训练失败时将把FailReason写进failure文件        |
...
下面的例子实现了一个自定义Tensorflow镜像，使用cifar10数据集作为训练集，例子源码可以在sdk example中找到

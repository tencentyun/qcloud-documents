...
目前有 PyCaffe，PyTorch，TensorFlow 三种框架可以选择，Horovod 和 TensorFlow PS 为分布式训练的不同方式。下面以 TensorFlow 组件为例，介绍如何在平台上运行自己的 TensorFlow 代码。如果有生成模型文件（pb格式），可以通过 COS 或本地上传导入到 TI-ONE 的模型仓库，并部署成在线模型服务。

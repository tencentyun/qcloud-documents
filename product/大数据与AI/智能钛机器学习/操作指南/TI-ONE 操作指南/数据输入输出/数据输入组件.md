TI-ONE 平台提供3种数据源途径：**本地数据**、**库表数据集**和**COS 数据集**。
您可在左侧导航栏的【输入】>【数据源】下，选择需要的数据源组件。

## 本地数据
将本地数据组件拖拽至画布后，您可以将轻量本地文件（不超过256MB）上传到目标 COS 路径，为下游算法提供输入数据。
> ?
- 您需要有上传目标目录的写权限。
- 目标 COS 路径自动生成，如需自定义可修改输入指定路径。
  
![](https://main.qcloudimg.com/raw/1acd1e634c5046b34d4213a281140c11.png)

## 库表数据集
库表管理存储了所有建立数据库连接的信息，建模过程中，您可以选择已创建成功的数据表名称，通过数据所在的测试库选择数据，或者选择手动填写 COS 路径。
![](https://main.qcloudimg.com/raw/17b31b6b72dc14084baf96c9f851b3fb.png)

## COS 数据集

将 COS 数据集组件拖拽至画布中，界面如下图所示。为下游算法提供输入数据，指定COS数据路径即可通过数据流自动传递。 并可通过“是否检查数据”开关来判断是否数据存在以后再启动后续节点处理。
![](https://main.qcloudimg.com/raw/94b9256a462e78a2cddabf3470fb99c0.png)

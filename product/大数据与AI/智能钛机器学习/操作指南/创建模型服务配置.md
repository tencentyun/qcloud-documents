...
**配置模型文件：** 模型文件需要首先上传到 [腾讯云对象存储 COS](https://console.cloud.tencent.com/cos5/bucket)，然后将模型文件所在的路径输入【模型文件】输入框内。模型地址格式为：cos://${bucket_name}-${appid}.cos.{region}.mycloud.com/模型文件夹路径。

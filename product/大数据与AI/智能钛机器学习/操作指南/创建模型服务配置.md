...
**选择运行环境：** 单击【运行环境】，可选择模型运行的公共环境。 TI-EMS针对PMML、Tensorflow和Angel三种模型类型提供了运行环境，可以将三种类型的模型文件加载成一个服务，处理对模型服务进行调用的请求，并按模型计算出结果进行返回。
...
在模型服务列表页面，您可以查看模型服务配置的基本信息，包括运行环境信息、CPU配置信息、GPU配置信息、修改时间等。

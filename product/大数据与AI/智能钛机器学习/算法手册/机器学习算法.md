...
FactorizationMachine（FM）一种基于矩阵分解的机器学习算法。它可对任意的实值向量进行预测。其主要优点包括: 1) 可用于高度稀疏数据场景；2) 具有线性的计算复杂度。
...
- 使用分类模型进行新的数据预测，得到分类结果，分类结果形式为：原始 label 预测 label 预测 score 值。
...
- **训练节点**
  - 算法 IO 参数
    ...
    - 输入数据格式：libsvm 或者 dummy。
      注：带预测的算子模块模型保存路径不需要用户填写，平台会自动生成。

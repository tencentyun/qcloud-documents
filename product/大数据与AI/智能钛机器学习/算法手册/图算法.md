...
- **spark conf 参数**
  - spark.angel.tmp.output.path.prefix：angel临时目录的前缀路径，为 COS 路径。
...
- 学习率衰减系数：学习率衰减参数。
- 评估指标：可选择acc，binary_acc（二分类场景），auc，recall，precision，f1-score，rmse 等。
- 验证周期数：每隔 X 轮验证一次。
...
#### 本算子主要解决的问题是学习网络图中节点的embedding向量

...
在本次任务中，我们利用公开的乳腺癌数据集进行实战学习，具体源数据信息可以[点此查看](<https://archive.ics.uci.edu/ml/datasets/Breast+Cancer+Wisconsin+(Diagnostic)>)。

该乳腺癌数据集共包含569个样本，其中357个阳性（y=1）样本，212个阴性(y=0)样本；每个样本有32个特征，但本次实验中选取其中的10个特征。
...
3. 在【 本地数据】对应右边弹出的设置窗口中选择【算法IO参数】中的"数据文件"上传，点击"数据文件"输入框，在弹出的列表框中选择截图中红框的上传按钮，选择刚刚保存到本地的cancer.csv数据集上传，点击"确定"按钮
...
5. 填写蓝色漏斗参数：（点击蓝色漏斗，在右侧弹出的参数板块填写）
...
4. 在模型运行结束后，右键【BinartEvaluator】，点击【查看结果】>【查看结果】，即可查看该模型评估的混淆矩阵

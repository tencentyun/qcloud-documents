...
1. 本案例的训练样本是图片，因此我们先将训练样本上传至 COS。

  将数据上传至COS有两种方式：

  **方式一：通过控制台上传数据文件**（适合轻量文件上传）

  - 前往 [COS 控制台](https://console.cloud.tencent.com/cos5)，单击左侧导航>存储桶列表。
  ...
  **方式二：通过客户端上传数据文件**（适合大文件上传，支持**批量**上传下载）
  ...
  - 前往[COSBrowser 工具](https://cloud.tencent.com/document/product/436/11366)下载COS客户端

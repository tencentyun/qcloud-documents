...
使用科比职业生涯中投篮命中和投失的历史数据，您能预测哪些投篮能命中篮筐吗？所以在本案例中，您的任务是训练一个模型去准确预测科比的某次投篮是否会投进。本案例非常适合实践分类知识、特征工程和时间序列分析。

## 数据集介绍

该案例中的训练数据包含科比在20年职业生涯中所尝试的每个投篮的具体特征信息：动作、位置、日期等。
...
- classifier.py：分类模型的自定义代码文件，用于模型分类
...
请用户下载好该案例所需全部材料，并保存到本地以便后面搭建工作流需要。

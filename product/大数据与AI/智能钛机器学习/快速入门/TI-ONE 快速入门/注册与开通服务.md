...
## 步骤4 ：创建 COS 存储桶/使用TI-ONE免费存储桶
完成【一键授权】后，在此步骤，您可选择创建个人的付费 COS 存储桶，或选择直接使用 TI-ONE 提供的免费 COS 存储桶（此免费存储空间存储上限为 500 GB，超过 15 天未登陆，免费存储空间数据将被清空。如有大量数据存储需求，请使用个人付费 COS 存储桶）。
...
为方便用户快速上手，TI-ONE 提供免费存储桶供用户体验。用户只需在 [智能钛机器学习平台控制台](https://console.cloud.tencent.com/tione)中【新建工程】时直接使用，无需额外创建或开通。

...
2. 在 Notebook 实例列表页，单击左上角【新增实例】，跳转至创建 Notebook 实例的设置页面。填写说明如下：
   ...
   - **存储大小**：Notebook实例的存储大小(以 GB 为单位)，最小值为 10GB 且为 10 的倍数，注意：请大于当前硬盘值，最大值为 16380GB(约16TB)。
   
   点击“高级设置”（默认不展开）后，可配置：
   
   - **Root 权限**：选择是否赋予 root 权限来访问 Notebook。如果启用 Root 权限，则所有 Notebook 实例用户具有管理员权限，并且可以访问和编辑实例上的所有文件。
   - **生命周期配置**：选择是否使用生命周期脚本。
   - **Git存储：**此为可选项，用户可以前往 Git存储库-新增存储库进行配置。
   ...
   - **总价**：平台根据您选择的配置显示相关价格。
3. 单击【创建】，Notebook 列表中将新增一条实例记录，用户可点击“状态”查看实例创建进程，当实例状态由【创建中】变为【运行中】时，单击【打开】进入 Notebook 实例内部。
4. 进入实例内部后，您可以根据需要设置内核环境。本案例使用 conda_tensorflow_py3。
...
path：结果文件的路径。
bucket：指定存储桶。注意：请指定用户对应地域下的个人COS存储桶，使用示例中的存储桶会导致报错。

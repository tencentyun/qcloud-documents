...
3. #### 准备数据
   ...
   本快速使用指南中，为大家准备了相关材料，请点击下载链接：（该实验选取部分图像作为训练数据）：
...
1. 单击确定开始部署模型。
2. 单击服务中的模型，可以获取服务详情，包括模型指标、服务地址和服务性能。
...
1. 单击【send】，发送调用请求，查看返回结果。
2. 返回结果解读：服务会返回模型对图片的分类结果，结果按照评分返回 top n 的类别，在本例中返回了 top 2 的结果，分别是蒲公英（Dandelion）和向日葵（Sunflower）。Classes中的string_val对应Scores中的float_val，float_val的值代表置信度，值越高则表示该类别越有可能是该图片的正确分类。

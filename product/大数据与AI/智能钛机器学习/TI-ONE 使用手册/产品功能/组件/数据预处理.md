...
![](manual\dataframestyle.png)

从以上代码可以看到，通过csv直接加载得到的dataframe，”age”列是`string`类型，我们可以使用`cast`表达式将其转换为`int`类型。 <img src="manual/pyspark5.png"/>   

基于dataframe，可以使用sql算子灵活方便地处理数据，下面代码显示使用`groupBy`对数据进行分组计算。
<img src="manual/pyspark6.png"/>   

当然，你还可以像如下代码显示一样直接写sql。 <img src="manual/pyspark7.png"/>   

下一章：[算法建模](modelingalgorithm.md)
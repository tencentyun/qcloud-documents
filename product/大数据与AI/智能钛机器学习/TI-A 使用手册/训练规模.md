...
由于spark任务的特殊性，spark任务只支持DIST版本的scaletier参数。如果不设置scaletier，默认会为spark任务设置为一个单driver + 单executor的运算集群进行运算。
...
- pt：即 parameter server type，指定 parameter server 节点的机器类型 (对于spark任务，这个参数会被忽略)。
- wcount：即 worker count，指定 worker/driver 节点的个数。
- pscount：即 parameter server count，指定 parameter server 节点的个数 (对于spark任务，这个参数会被忽略)。

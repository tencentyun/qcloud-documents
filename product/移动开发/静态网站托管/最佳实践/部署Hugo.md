...
云开发 CloudBase 是一款云端一体化的产品方案，采用 serverless 架构，免环境搭建等运维事务，支持一云多端，助力快速构建小程序、Web 应用、移动应用。云开发静态网站托管支持通过云开发 SDK 调用服务端资源，例如云函数、云存储、云数据库等，从而将静态网站扩展为全栈网站。
无论是腾讯云·云开发用户，还是小程序·云开发用户，只需开通按量付费，即可享有云开发静态网站托管服务。本文将为您介绍如何在云开发静态网站托管部署 Hugo。

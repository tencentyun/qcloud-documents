...
通用：适合云服务器、云数据库(MySQL)、云数据库(SQL Server)、云数据库(TDSQL)、CDN流量包、云存储Redis、文智、游戏兼容性测试及服务市场。（其中云服务器、云数据库(MySQL)、云数据库(SQL Server)、云数据库(TDSQL)、云存储Redis可在包年包月产品的购买、续费场景中使用，带宽升级不可使用）

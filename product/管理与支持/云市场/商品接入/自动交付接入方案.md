...
- openId：当服务商的应用接入腾讯云开放后，可获得的腾讯云用户的唯一标识。
...
3. 将三个参数字符串拼接成一个字符串进行 sha256 加密。
...
- 接口名：createInstance
- 接口说明：云市场实例创建即为商品购买，用户购买商品并支付后云市场会通过**实例创建通知接口**通知给商品相应服务商。
...
| openId                  | String  | 是       | 用户在腾讯云开发平台的标识，此标识对于同一服务商是相同的，对于不同服务商是不同的，长度为32位；如果没有接入开发平台，此字段为空 |
...
| productInfo.spec        | String  | 否       | 产品规格，是试用时为空                                       |
| productInfo.timeSpan    | Integer | 否       | 购买时长，是试用时为空                                       |
| productInfo.timeUnit    | String  | 否       | 购买时长单位（y、m、d、h、t 分别代表年、月、日、天、时、次），是试用时为空 |
| extendInfo              | JSON    | 否       | 订单扩展信息，通常情况下为空                                 |
...
```bash
curl -X POST -H 'Content-Type: application/json' 'http://isv/interface?signature=e3k9ierw&timestamp=1483944926&eventId=1780012140' --data '{"action":"createInstance","orderId":"20170109199524"," OpenID ":"xz_D4XL_u7hKY5zt","productId":1024,"requestId":"fab8a029-22fa-41b1-ac08-5cdde878ed04","email":"xinlanli@tencent.com","mobile":"15811517539","productInfo":{"isTrail":"false","productName":"云服务市场测试商品","spec":"普通版","timeSpan":2,"timeUnit":"m"}}'
```
...
接口名：renewInstance
接口说明：用户购买后的所有商品，均以实例（资源）的方式展现展示给用户。云市场会通过**实例续费触发入口**通知给商品相应服务商。该接口需要服务商立即返回响应。
...
```bash
curl -X POST -H 'Content-Type: application/json' 'http://isv/interface?signature=e3k9ierw&timestamp=1483944926&eventId=1780012140' --data '{"action":"renewInstance","orderId":"20170109199524"," OpenID ":"xz_D4XL_u7hKY5zt","productId":1024,"requestId":"3c45e1f3-22b9-4346-9898-4467d3aea000","signId":"kjsadkjhdskjh3k","expiredTime":"2017-02-09 19:59:59"}'
```
...
- 接口名：modifyInstance
- 接口说明：用户将实例从试用版转为正式版时，云市场会通过**实例配置变更**通知给商品相应服务商。
...
| signId             | String   | 是       | 实例标识 ID                                                   |
| spec               | String   | 是       | 实例新规格                                                   |
| timeSpan           | Integer  | 否       | 购买时长，仅在试用版转为正式购买时传递                       |
| timeUnit           | String   | 否       | 购买时长单位（y、m、d、h、t 分别代表年、月、日、天、时、次），是试用时为空 |
| instanceExpireTime | Datetime | 否       | 新的实例到期时间，仅在试用版转为正式购买时传递               |
...
```bash
curl -X POST -H 'Content-Type: application/json' 'http://isv/interface?signature=e3k9ierw&timestamp=1483944926&eventId=1780012140' --data '{"action":"modifyInstance","orderId":"20170109199524"," OpenID ":"xz_D4XL_u7hKY5zt","productId":1024,"requestId":"1d8326b2-9a94-4bf3-91ce-c7a94add99d3","signId":"kjsadkjhdskjh3k","spec":"高级版"}'
```
...
- 接口名：expireInstance。
- 接口说明：实例到期后（用户最后操作后的 instanceExpireTime ），云市场会通过该接口通知服务商。
...
```bash
curl -X POST 'http://isv/interface?signature=e3k9ierw&timestamp=1483944926&eventId=1780012140' --data '{"action":"expireInstance"," OpenID ":"xz_D4XL_u7hKY5zt","productId":1024,"requestId":"ea372177-809d-4722-91d0-d6df4edf7bc9","signId":"kjsadkjhdskjh3k"}'
...
- 接口名：destroyInstance。
- 接口说明：用户退款、实例到期后的七天内如果用户没有进行续费操作，云市场会销毁该实例并通过该接口通知服务商进行实例销毁。用户退款规则为：购买五天内，用户可以自主退款，五天后或用户自主退款次数大于5次，用户则需要提工单申请退款，退款经云市场审核后安排发放，以上退款均为退全款。用户退款按订单退款，一个实例可能关联多个订单（如用户购买一个商品后对其进行了转正式版和续费等操作）。
...
```bash
curl -X POST 'http://isv/interface?signature=e3k9ierw&timestamp=1483944926&eventId=1780012140' --data '{"action":"destroyInstance","orderId":"20170109199524"," OpenID ":"xz_D4XL_u7hKY5zt","productId":1024,"requestId":"80b75030-6571-46a8-87ef-5b414f66dc39","signId":"kjsadkjhdskjh3k"}'
...
#### 在线接口调试
在接口开发过程中，可以使用此在线调试功能，用于模拟各种类型的发货行为；请注意此为调试环境，请勿使用正式环境的接口 URL 或数据库对接。
![](https://main.qcloudimg.com/raw/0d4d28944d68bd040817b54925c1c424.png)

## 接入腾讯云 oAuth 
为了提升用户使用 SaaS 产品的体验，建议服务商将应用接入腾讯云开放平台 oAuth ，实现用户在腾讯云控制台登录后，直接免登访问 SaaS 应用管理后台。

#### 申请接入开放平台
接入 oAuth 需要先申请开放平台 App，服务商需先将下述申请信息发送至 mqcloud@tencent.com。
...
**授权流程说明**
服务商授权过程如下：
![](https://main.qcloudimg.com/raw/485de2257de1ddf2735de2b79ff45194.png)
...
| 参数名                | 类型    | 是否必须 | 描述                                                         |
| --------------------- | ------- | -------- | ------------------------------------------------------------ |
| code                  | Integer | 是       | 错误码，为0时表示请求返回正确。可在 [签名方法](https://www.qcloud.com/document/api/386/6725) 查看更新错误详情 |

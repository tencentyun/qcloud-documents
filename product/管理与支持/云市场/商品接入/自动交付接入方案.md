...
- 接口名：createInstance。
- 接口说明：云市场实例创建即为商品购买，用户购买商品并支付后云市场将通过**实例创建通知接口**通知给商品相应服务商。
...
- 接口名：expireInstance。
- 接口说明：实例到期后（用户最后操作后的 instanceExpireTime ），云市场将通过该接口通知服务商。
...
- 接口名：destroyInstance。
- 接口说明：用户退款、实例到期后的七天内如果用户没有进行续费操作，云市场会销毁该实例并通过该接口通知服务商进行实例销毁。
用户退款规则为：购买五天内，用户可以自主退款，五天后或用户自主退款次数大于5次，用户则需要提工单申请退款，退款经云市场审核后安排发放，以上退款均为退全款。用户退款按订单退款，一个实例可能关联多个订单（例如用户购买一个商品后对其进行了转正式版和续费等操作）。

...
- **账单数据存储**：提供定期将账单数据存储至您制定的COS存储桶的功能，对于账单明细数据量级很大的客户，建议您开通账单数据存储功能，通过存储桶中获取账单文件进行分析。
...
<img src="https://main.qcloudimg.com/raw/85a02b563d9e316c57702dd691abb99d.png" style="width: 60%"/>
 ...
 - 账单存储：用户开通后，每日将更新的数据存入指定的 COS 桶，数据类型分为日账单和月账单两种。
    - 日账单：Day+2后，新增的账单明细会存储到 COS 存储 bucket 中。例如4月6日，将会有增加一个4月1日到 - 4月4日的汇总账单；4月8日，将会增加一个4月1日 - 4月6日的汇总账单。
    - 月账单：每个月2号更新一份完整的月账单明细，如数据量过大，则拆分成多个 CVS 文件。

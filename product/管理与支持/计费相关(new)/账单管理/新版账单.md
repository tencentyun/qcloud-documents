...
- **账单数据存储**：提供定期将账单数据存储至您制定的对象存储（Cloud Object Storage，COS）存储桶的功能，对于账单明细数据量级很大的客户，建议您开通账单数据存储功能，通过存储桶中获取账单文件进行分析。
...
1. 在 [账单概览](https://console.cloud.tencent.com/expense/bill/overview) 页面，将“账单数据存储”设置为 ![](https://main.qcloudimg.com/raw/c41da5513ac80853d157d210f3d7f871.png)。
![](https://main.qcloudimg.com/raw/60272a89120b992fefbefd9956c88462.png)
2. 在账单数据存储开通页面，选择存储桶。
如没有存储桶，可单击【创建存储桶】进行创建。</br>
<img src="https://main.qcloudimg.com/raw/85a02b563d9e316c57702dd691abb99d.png" style="width: 60%"/>

 - 账单存储：用户开通后，每日将更新的数据存入指定的 COS 桶，数据类型分为日账单和月账单两种。
    - 日账单：Day+2后，新增的 bill_details 会存储到 COS 存储 bucket 中。例如4月6日，将会有增加一个4月1日到 - 4月4日的汇总账单；4月8日，将会增加一个4月1日 - 4月6日的汇总账单。
    - 月账单：每个月2号更新一份完整的月账单 bill_details，如数据量过大，则拆分成多个 CVS 文件。
 - 关闭账单存储功能或更换桶：如用户需要关闭账单功能，将“账单数据存储”设置为 ![](https://main.qcloudimg.com/raw/bf25e86868f8baf8bcae95c9d95dafc2.png)，并在弹出的窗口中单击【关闭存储桶】关闭该功能。关闭后，该桶保留旧数据，不再写入新数据。
 - 更换存储桶：用户需要更换桶，将“账单数据存储”设置为 ![](https://main.qcloudimg.com/raw/c41da5513ac80853d157d210f3d7f871.png)，并在弹出的窗口中重新选择存储桶，单击【保存设置】即可。历史数据保留在旧桶，新数据将在 Day+1 之后写入新桶。

...
## 2.服务保证指标
### 2.1. 业务可用性
（1）弹性缓存服务承诺 99.95% 的业务可用性，这意味着您购买的实例理论可用时间高于 30 天×24 小时×60 分钟×99.95%=43178.4分钟，为仍有可能存在 43200-43178.4=21.6 分钟的不可用时间。
...
### 2.8 到期结算和欠费说明
如您选择的数据库实例结算类型为预付费（包年包月），腾讯云在数据库到期后，为您提供7天可使用期限，并在 7 天后关闭数据库服务。您应承担保留期限内产生的云服务费用（若有），并在保留期限届满前结清费用、完成全部数据的迁移。到期或终止后 14 天，腾讯云数据库系统将自动删除您的所有数据。

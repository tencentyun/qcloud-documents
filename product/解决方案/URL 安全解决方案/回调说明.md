腾讯云 URL 安全解决方案采用主动回调的方式返回结果，用户注册回调域名，腾讯云后台天御服务会把被拦截的URL或域名主动回调给用户。
...
天御 URL 安全 [回调示例](http://www.xx.com/callback?data=xx)，[客户预留地址](http://www.xx.com/callback)，data 为回调加密报文。
2. 获取报文

...
√ WProxy配置说明见官网的演示图片。
- 访问外网（免用户名和密码）
√ 服务商在配置收银终端、订单查询终端时，可以点击设置代理，然后填写代理URL，格式为:http://proxy_server_ip:proxy_server_port，然后点击保存即可。
...
√ 访问外网
 - 服务商在配置收银终端、订单查询终端时，可以点击设置代理，然后填写代理URL，格式为:http://proxy_server_ip:proxy_server_port、 代理用户名、密码，然后点击保存即可 。

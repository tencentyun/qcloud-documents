...
- **获取云支付服务商账号**
登录 [云支付控制台](https://cloud.tencent.com/login?s_url=https%3A%2F%2Fconsole.cloud.tencent.com%2Fcpay) ，选择“服务商账户管理（新）”，在服务商账号列表可查看云支付服务商账号。
2. **获取云支付子商户账号**
登录 [云支付控制台](https://cloud.tencent.com/login?s_url=https%3A%2F%2Fconsole.cloud.tencent.com%2Fcpay) ，选择“子商户管理（新）”，在子商户账号列表可查看云支付子商户账号。
3. **获取订单号前缀**
在子商户详情页有两个订单前缀，推荐使用纯数字的8位订单前缀，便于机具扫订单条码退款时很容易扫上。 
4. **获取门店全局 ID**
![](https://main.qcloudimg.com/raw/a3156a34587f749e5d72160181f3bae6.png)
5.  **获取设备 ID**
![](https://main.qcloudimg.com/raw/8385a6e010f6e03bcf33e7615a70a52f.png)
6. **获取店员 ID**
![](https://main.qcloudimg.com/raw/4390d9914e6478578fdd5ce2177a6cae.png)
7. **获取认证密钥和签名私钥**
...
3. 刷卡支付是一个异步操作，提交刷卡支付成功，不代表支付成功，只代表 SDK 已经受理这笔交易，调用方需要调用查单接口去确认支付的结果。
4. 刷卡支付提交后，返回参数里会有一个 json 结构的支付平台，需要调用方解析出来，然后查单的时候用这里解析出来的支付平台。
5. SDK 提供的接口的请求参数和返回参数均是 json 结构。
...
- 业务流程上的日志，可以上传多条，每天日志的需要记录时间点，精确到毫秒，需要调用方自己来完成。
 
#### 流程说明

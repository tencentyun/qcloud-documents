...
**第一步：** 服务商需先在[**微信支付商户平台**](https://pay.weixin.qq.com)，完成微信支付子商户申请

**第二步：** 在[**云支付**](https://console.cloud.tencent.com/cpay)添加微信支付子商户
...
| 参数类型 | 参数说明 | 查看 |
|---------|---------|---------|
| 子商户商户号 | 微信支付商户平台上的微信支付商户号，子商户在申请微信支付后，由微信支付分配的商户收款账号。 | [**点击查看**](https://pay.weixin.qq.com/index.php/core/account/info) |
...
**第五步：** 添加完成后，单击下一步，保存该页面上的**认证密钥**和**签名私钥**到本地电脑上
...
在成功添加微信支付子商户之后，如果需要添加支付宝商户信息，支付宝子商户需要进行扫码授权。具体操作如下所述：

**第一步：** 在子商户详细信息中，可以看到要扫描的支付宝服务商授权码，如下图所示：

![](https://mc.qcloudimg.com/static/img/03b9eaa8ad2f841ce90da92e39c92bd1/123.png)

**第二步：** 使用有支付宝子商户管理权限的支付宝帐号进行扫码授权

* 扫码成功显示如下(**备注：授权成功后，请等一分钟再测试支付**)

![](https://mc.qcloudimg.com/static/img/ca933b53912055a67c3bb52019df0b0c/image.png)

* 如果扫码出现如下结果，有两种情况：
  * 如果您想绑定新的支付宝账户，则需要先删除原来的授权信息(删除方法：编辑>删除“支付宝用户ID”>保存),然后重新授权；
  * 如果您就想使用原有的支付宝账户进行授权，请确定，您使用了正确的商户支付宝。如果确认完成之后依然不对。请参见第1步，删除原有授权之后重新授权。

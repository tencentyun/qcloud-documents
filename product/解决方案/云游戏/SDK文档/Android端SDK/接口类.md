云游戏提供了 Android 端程序包 **com.tencent.tcgsdk.api**，具体接口参数如下文所述。

## 接口总览

| 接口                                                         | 说明                                                        |
| ------------------------------------------------------------ | ----------------------------------------------------------- |
| [ICursorBitmapListener](#icursorbitmaplistener)              | 获取鼠标图片接口                                            |
| [IDesktopListener](#idesktoplistener)                        | 云端画面信息监听器                                          |
| [IGameArchive](#igamearchive)                                | SDK 提供的存档操作                                          |
| [IGameArchive.IArchiveListener](#igamearchive.iarchivelistener) | 加载存档监听器                                              |
| [IGameProcessLaunchListener](#igameprocesslaunchlistener)    | 远端游戏进程监听器                                          |
| [IHitInputBoxListener](#ihitinputboxlistener)                | 远端设备输入框选中状态监听器                                |
| [IJitterListener](#ijitterlistener)                          | 网络抖动状态监听器                                          |
| [ILogger](#ilogger)                                          | 日志回调接口                                                |
| [ILowFPSListener](#ilowfpslistener)                          | 低帧率回调监听器                                            |
| [IOnGameViewTouchListener](#iongameviewtouchlistener)        | 用户在 GameView 触发的动作监听器                            |
| [IRemoteCursorVisibleListener](#iremotecursorvisiblelistener) | 远端光标可见性监听器                                        |
| [IResolutionChangeListener](#iresolutionchangelistener)      | 视频图像宽高变化监听器                                      |
| [ITcgListener](#itcglistener)                                | TcgSdk 生命周期回调，TcgSdk 在调用 start 接口之后进行初始化 |
| [ITcgSdk](#itcgsdk)                                          | SDK 接口定义                                                |
| [ITcgSdk.IAutoLoginWindowStatCallback](#itcgsdk.iautologinwindowstatcallback) | 是否支持自动登录功能回调                                    |
| [ITcgSdk.IRTCResult](#itcgsdk.irtcresult)                    | 云端接口调用结果                                            |
| [ITcgSdk.IRTCTimeoutCallback](#itcgsdk.irtctimeoutcallback)  | 接口调用请求超时                                            |
| [IWanIpChangeListener](#iwanipchangelistener)                | 外网 IP 变化回调监听器                                      |
...
> ? 更多详情请参见 [错误码（TcgErrorType）](https://cloud.tencent.com/document/product/1162/52291?!editLang=zh&!preview)。
...
| [registerRemoteDesktopChangeListener](#unregisterremotedesktopchangelistener) | void  | 注册远端桌面变化监听器                                       |
| [unRegisterRemoteDesktopChangeListener](#unregisterremotedesktopchangelistener) | void  | 注销远端桌面变化监听器                                       |
...
| [sendSeqRawEvents](#sendseqrawevents)                        | void  | 底层发送按键序列的函数，events 为事件数组                    |
...
| callback | ITcgSdk.IRTCResult | 调用结果回调 NOTE<br />该接口依赖于云端配置，若回调不生效需要和云游团队确认配置是否开启 |
...
#### registerRemoteDesktopChangeListener

注册远端桌面变化监听器。

```java
void registerRemoteDesktopChangeListener(IDesktopListener listener)
```
...
| 参数     | 类型             | 说明                   |
| -------- | ---------------- | ---------------------- |
| listener | IDesktopListener | 需要监听桌面变化的回调 |

#### unRegisterRemoteDesktopChangeListener

注销远端桌面变化监听器。

```java
void unRegisterRemoteDesktopChangeListener(IDesktopListener listener)
```
...
| 参数     | 类型             | 说明                   |
| -------- | ---------------- | ---------------------- |
| listener | IDesktopListener | 需要注销的桌面变化回调 |
...
```java
void sendSeqRawEvents(java.util.List<ITcgSdk.Event> events,
                            ITcgSdk.IRTCResult callback)
```
...
| 参数     | 类型                          | 说明             |
| -------- | ----------------------------- | ---------------- |
| events   | java.util.List<ITcgSdk.Event> | 事件数组         |
| callback | ITcgSdk.IRTCResult            | 远端接口调用结果 |

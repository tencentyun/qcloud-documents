...
| 接口名称                                                     | 接口描述                                |
| ------------------------------------------------------------ | --------------------------------------- |
| [ITcgSdk.gameRestart(result)](#ITcgSdk.gameRestart(result))  | 重启当前运行的游戏进程                  |
| [ITcgSdk.getLoginWindowStat(callback)](#ITcgSdk.getLoginWindowStat(callback)) | 查询当前窗口是否支持自动登录功能        |
| [ITcgSdk.getInputMethodStat(listener)](#ITcgSdk.getInputMethodStat(listener)) | 查询云端输入法大小写状态                |
...
| [IMultiPlayer.apply(userID,role,seatIndex,result)](#IMultiPlayer.apply(userID,role,seatIndex,result)) | 申请切换角色和席位                                           |
| [IMultiPlayer.change(userID,role,seatIndex,result)](#IMultiPlayer.apply(userID,role,seatIndex,result)) | 申请切换角色和席位                                           |
| [IMultiPlayer.syncSeatInfo()](#IMultiPlayer.syncSeatInfo())  | 获取所有席位信息（玩家及观察者），结果会通过 onSeatChanged 通知 |
| [IMultiPlayer.registerSeatChangeListener(listener)](#IMultiPlayer.registerSeatChangeListener(listener)) | 注册席位信息监听器                                           |
| [IMultiPlayer.unRegisterSeatChangeListener(listener)](#IMultiPlayer.unRegisterSeatChangeListener(listener)) | 注销席位信息监听器                                           |
...
[](id:ITcgSdk.sendMouseScroll(down,callback))

### ITcgSdk.sendMouseScroll(down,callback)
...
[](idITcgSdk.unRegisterIDleListener(listener))

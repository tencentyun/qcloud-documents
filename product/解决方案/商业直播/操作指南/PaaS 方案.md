...
## 版本支持
- 最新插件版本：1.3.0 。新增小窗功能，包体缩减。需在控制台添加插件，使用小程序直播域名才能调用，无需通过 version 标识方案（即与旧版本不兼容，需要在云直播控制台接入插件）。
- 最低插件版本限制： 1.2.4 。仍支持旧方案，但是新功能暂时不支持，需要通过version 2来标识新方案。
...
| version                   | Number       | 1          | 否   | 1.2.4版本插件参数，此处必填值为：2 旧方案接入：1 或不填。高版本无需填写。 |

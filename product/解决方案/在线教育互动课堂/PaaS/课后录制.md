...
![](https://test-1257240443.cos.ap-shanghai.myqcloud.com/picgo/课后录制流程图.png)

### 2.1 功能开通

若您能还没开通课后录制服务，请按照[服务开通指南](https://gitee.com/vqcloud/TIC/blob/master/Docs/开通服务/服务开通指南.md)发送邮件申请开通，并在申请功能里保留`云端课后录制`，按照文档流程开启实时音视频云端录制以及提供App管理员UserId与UserSig

### 2.2 发起课后录制

在课堂结束后，您的业务服务器可以发起一个对该课堂的课后录制请求，具体的鉴权以及请求方式请参考 `4.1 发起课后录制`。发起请求后，课后录制服务返回一个本次录制任务ID`task_id`，后续查询录制进度需要依赖于这个`task_id`，请妥善存储。

> 由于白板画面的录制实质是通过到IM服务器拉取白板历史信令进行画面重放来实现的，因此在您发起课后录制的时候，课堂ID对应的IM群组必须存在才可进行录制，请务必保证在上完课后**不要解散(销毁)IM群组**。而且，为了避免IM群组中有之前课堂遗留的历史白板信令，每次上课最好使用一个未用过的房间号;
...
| user_id         | string | 视频所属用户的id(白板视频user_id为空)       |
...
| group_id    | string | 白板的IM群组ID，如果不填默认为与房间号相同                                                                                  | 否      |
| mix_stream  | bool   | 是否混流录制，默认为false  | 否      |

> 如果开启混流录制，录制结果就只有一个混流后的视频，混流的布局请参考[云端课后录制混流布局](https://gitee.com/vqcloud/TIC/blob/master/Docs/附录/云端课后录制混流布局.md)

请求示例：
> 请将random/sdkappid/sign/expire_time/room_id/group_id/mix_stream字段替换为实际的值

```
请求URL：https://iclass.api.qcloud.com/record/v1/offline/create?random=526919&sdkappid=1400127115&sign=d33bfea49d7f2795a4829c0d80047a7d
请求Header："Content-Type:application/json"
请求包体：
{
...
| task_id    | string | 课后录制任务ID，查询录制进度以及录制回调都会依赖任务ID |
...
```
请求URL：https://iclass.api.qcloud.com/record/v1/offline/query?random=526919&sdkappid=1400127115&sign=d33bfea49d7f2795a4829c0d80047a7d
请求Header："Content-Type:application/json"
请求包体：
{
    "expire_time":1557202745,
...
| 参数名           | 类型    | 描述                                      |
|:----------------|:-------|:-----------------------------------------|
| video_play_time | int    | 视频播放时间（单位: 毫秒）                  |
| video_size      | int    | 文件大小(字节)                              |
| video_format    | string | 文件格式(目前统一为mp4)                     |
| video_duration  | int    | 文件播放时长(单位: 毫秒)                    |
...
```
请求URL：https://iclass.api.qcloud.com/record/v1/offline/callback?random=526919&sdkappid=1400127115&sign=d33bfea49d7f2795a4829c0d80047a7d
请求Header："Content-Type:application/json"
请求包体：
{
    "expire_time":1557202745,

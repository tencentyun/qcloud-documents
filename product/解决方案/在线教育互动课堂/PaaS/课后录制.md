...
![](https://main.qcloudimg.com/raw/1797c9796f4d91d406b83f9968b9ca10/%E8%AF%BE%E5%90%8E%E5%BD%95%E5%88%B6%E6%B5%81%E7%A8%8B%E5%9B%BE.png)
...
如果您在发起课后录制`4.1 发起课后录制`没设置参数`mix_stream`或者设置参数`mix_stream`为`false`，您将收到**多个视频**以及这些视频相对视频开始播放的时间`video_play_time`，例如以下为一个录制结果JSON串示例：
```json
{
    "room_id":1234,
    "group_id": "1234",
...
| 参数名      | 类型    | 描述                       | 是否必填 | 默认值   |
|:-----------|:-------|:--------------------------|:-------|:--------|
| width      | int    | 实时录制白板宽，默认为1280    | 否      | 1280    |
| height     | int    | 实时录制白板高，默认为960     | 否      | 960     |
| init_param | string | 白板初始化参数，透传到白板SDK | 否      | 空字符串 |

> 如果开启混流录制，录制结果就只有一个混流后的视频，混流的布局请参考[云端课后录制混流布局](SDK文档/云端课后录制混流布局.md)

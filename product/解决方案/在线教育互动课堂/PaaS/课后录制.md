...
> 由于白板画面的录制实质是通过到IM服务器拉取白板历史信令进行画面重放来实现的，因此在您发起课后录制的时候，课堂ID对应的IM群组必须存在才可进行录制，请务必保证在上完课后**不要解散(销毁)IM群组**。而且，为了避免IM群组中有之前课堂遗留的历史白板信令，**每次上课最好使用一个未用过的房间号**。离线录制暂不支持有web端参与互动(操作白板或打开摄像头等)的课堂；

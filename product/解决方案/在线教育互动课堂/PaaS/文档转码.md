...
| is_static_ppt | bool| 可选   | 是否为静态 PPT，默认为 false，当`is_static_ppt`为 false 时，后缀名为`.ppt`或`.pptx`的文档会动态转码成`HTML5`页面，其他格式的文档会静态转码成图片，如果`is_static_ppt`为 true，所有格式的文档会静态转码成图片 |
...
>- 文档转码结果包括`title`/`result_url`/`pages`/`resolution`等参数请配合白板 SDK 的`addTranscodeFile`接口使用 [互动白板接口文档](https://cloud.tencent.com/document/product/680/17884)。
...
7. 建议使用操作系统默认的中英文字体，请勿使用自己安装的字体，否则可能会出现转码失败或者转码 h5 格式异常可参考 [PPT 转 h5 支持字体列表](https://cloud.tencent.com/document/product/680/35951)。

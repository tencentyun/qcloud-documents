...
#### 登录/登出
 初始化完成后，您可以登录（类似 QQ 的用户登录），如果此用已经在其他设备上处于登录状态，先登录的设备会被强制下线收到`onTICForceOffline`回调。在您使用结束后，可以调用登出接口，退出登录状态。
```java
//1. 登录进入
...
更多`TEduBoard`相关内容，请参考 [白板接入文档](README_TEduBoard.md)。

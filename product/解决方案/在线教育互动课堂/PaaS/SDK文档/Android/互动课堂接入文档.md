...
```
在 `Android Studio` 开发环境下，可把TIC sdk源代码直接引入工程中。
...
#### 2.2.1 初始化
 ...
 2. 用户被踢或`userSig`过期会收到相应回调，建议您在登录之前设置`TICStatusListener`监听。
  ...
  ```java
//1、在Application中的onCreate函数中进行TIC初始化，在onTerminate中进行反初始化TIC；
@Override  
...
```
其中 `userId`、`userSig`（[如何获取UserSig？](https://cloud.tencent.com/document/product/647/17275)）为需要您自己填写的参数
...
```
其中 `mRoomId`为需要您自己填写的参数，如果课堂不再使用，可销毁课堂，课堂销毁后，课堂内的成员将会收到`TICEventListener`中的`onTICClassroomDestroy`回调
...
```
其中 `classId`为需要您自己填写的参数，暂时只有房间ID，可根据需要更加的丰富，比如是否打开后置摄像头，`TICClassroomOption`参数说明如下：
...
```
TIC只封装`TXLiteAVSDK_TRTC`进房退房方法，您可以通过`TICManager`获取`TRTC`实例做更多`TXLiteAVSDK_TRTC`的操作。

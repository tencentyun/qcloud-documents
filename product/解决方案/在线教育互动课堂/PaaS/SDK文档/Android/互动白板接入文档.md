...
### 2.2 白板窗口管理
![白板使用流程](https://main.qcloudimg.com/raw/1894831d199868a3cb45fff838512a13.png)

#### 2.2.1 白板控制器创建
 白板实例在TICManager中创建，使用者不需要增加额外创建逻辑；
...
```
#### 2.2.3 获取白板视图
方法是在白板初始化成功后， `onTEBInit` 回调方法内获取白板view加入显示视图中：
```java
...
#### 2.2.4 白板控制器销毁
在Activity的onDestroy函数删除Board的回调和从视图中去掉BoardView;
 ```java
@Override  

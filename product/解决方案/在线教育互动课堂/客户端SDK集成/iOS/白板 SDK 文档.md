...
1. 白板SDK实现文档的原理为多白板（白板组）

2. 每个白板展示一页文档，即每个文档会对应一组白板

3. 每个文档有一个 fid (文件唯一标识)

4. 每个白板有一个 boardID

5. 非文档白板的白板全部合成一个白板组，叫做普通白板，该白板组的 fid 为 #DEFAULT
...
调用`switchFile:`可实现切换文件，该方法接收文件的fid作为参数。切换文件会跳转到对应文件最近展示的页面。

...
| 大账号模式                                 | 私有账号模式                                        |
| ---------------------------------------- | ---------------------------------------- |
| 1.仅需提供 sdkappid 申请开通白名单即可使用，方便快捷| 1.用户须自行开通和配置 COS 服务，并须联系商务开通预览服务|
| 2.SDK 默认使用大账号模式，不需额外代码配置 |2.成功后调用 setCosConfig 接口进行初始化和设置|

* SDK 默认使用大账号模式，如果您是新接入用户，推荐使用该模式。
* 如果您已经或者决定使用私有账号模式，请确保按照 [开通对象存储服务](https://cloud.tencent.com/document/product/680/14782#.E5.BC.80.E9.80.9A.E5.AF.B9.E8.B1.A1.E5.AD.98.E5.82.A8.E6.9C.8D.E5.8A.A1) 指引配置 COS 账号，调用 **setCosConfig** 接口设置配置信息后使用。

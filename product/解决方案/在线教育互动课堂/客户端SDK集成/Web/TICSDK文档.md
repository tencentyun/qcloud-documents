...
> 开发者必须监听 <font color="red">TICSDK.CONSTANT.EVENT.IM.KICKED</font> 事件，防止被踢下线后无感知。
...
roomType | string | 否，默认Public | 创建的IM群组类型
...
```
ticSdk.joinClassroom(roomID, webrtc推流配置, 白板配置, COS配置);
```
...
|参数	| 类型	| 是否必填 | 描述|
|--------- | --------- | ----- | --------- |
|closeLocalMedia | boolean | 否，默认 false | 是否关闭自动推流（如果置为 true，则在完成加入/建房操作后，不会发起本端的推流，如|需|推流，需要由业务主动调推流接口 ）|
...
|peerAddNotify | boolean | 否，默认 false | p2p的建连通知，在建立p2p连接前由业务侧决定是否需要连接。<br/>需要结合[高级事件通知]的[onPeerConnectionAdd]使用 |
...
path | Object | 否，默认/TIC/WEB | 上传到bucket的指定目录下 |
...
TICSDK 支持ppt、pdf、doc 文档上传、并且提供预览服务。
...
receiveUserIdentifier | string | 否 | 接收方的identifier 不填，表示给课堂群组发IM消息
...
调用销毁课堂，则会真正将课堂销毁，清除白板所有数据，解散课堂IM群组；本方法只能由课堂的创建者调用，非创建则调用则不能销毁课堂，并触发销毁课堂失败的事件。调用此方法后，则会触发销毁课堂成功或者销毁课堂失败的事件。

...
- 申请成功后，我们将为您开通除`云端课后录制`以外的`互动白板`、`PPT动画转码`、`云端实时录制`功能。如果申请了`SaaS`服务，我们还会为您开通`SaaS课堂组件`。
...
- 如果您需要开通`云端课后录制`，请提供在`云通信`对应`SDKAppID`下`基础配置`中的`管理员UserID`与`管理员UserSig`，您可以使用[UserSig生成工具](https://sxb.qcloud.com/tools/TLSSig.php)生成有效期较长的UserSig，**为避免过期导致服务不可用，建议有效期50年**。

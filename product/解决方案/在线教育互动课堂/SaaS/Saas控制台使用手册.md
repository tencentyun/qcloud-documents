...
- 机构设置
  修改机构和全局配置信息。
- 用户管理
  在这里（或通过云 API）增加使用互动课堂的学生和老师的账号。
- 课堂管理
  在这里（或通过云 API）进行约课后，互动课堂客户端才能进入课堂。
- 课件管理
  在这里（或通过云 API）把课堂中需要用到的课件提前上传。
...
3. 私钥。如果使用控制台进行约课等行为，管理员必须先填写私钥。详细情况请参考配置页面中的文档链接。
4. 回调 url。当发生老师上下课、实时录制开始结束等事件后，互动课堂 server 会将此信息回调给客户。客户如需要可以进行配置。详细情况请参考配置页面中的文档链接。

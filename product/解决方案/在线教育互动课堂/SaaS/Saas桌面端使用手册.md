...
### 加入课堂
**加入课堂的方式有两种：**
1. 通过管理员分享的上课链接地址打开，点击管理员分享的上课链接地址，会看到下面的界面。
...
而 PPT、PDF、DOC 文件建议是在通过控制台上传。

**这里上传文件有两种方式：**
1. 通过控制台上传
这里可以直接将文件发给管理员。由管理员通过控制台上传并绑定到老师名下（参考 [控制台使用手册](https://cloud.tencent.com/document/product/680/37505)）。老师可以在课件管理页的私人课件页面找到它。
2. 老师在加入课堂后台上传
 1. 加入课堂，打开课件管理页。
...
![](https://main.qcloudimg.com/raw/ac993bebe7dfbe8c58af170da6c20e67.png)
 3. 选择需要上传的课件
点击课件上传后会弹出文件选择对话框，在这里选择需要上传的课件，支持 PPT、PPTX、DOC、DOCX、PDF 格式。
 4. 测试课件
...
## 老师如何使用客户端上课
使用 SaaS 客户端可以使用音视频、IM 聊天、白板、文档课件、播放视频等功能。

### 如何开始上课
老师可以提前一个小时进入课堂进行备课和学生互动。
...
聊天区域由消息列表（1）、消息输入框（2）和【发送】（3）组成，整个聊天区域可以通过（4）显示或隐藏。
自己发送的消息会展示在右边。
...
1. 缩放控制区域。通过缩小/放大按钮可以将文档进行放大以及还原。

...
### 测试延期
可以在 [语音评测离线 SDK 控制台](https://console.cloud.tencent.com/soe/offline/index) 申请延长测试时间。
1. 进入 [语音评测离线 SDK 控制台](https://console.cloud.tencent.com/soe/offline/index) SDK 列表页面，单击**延长测试时间**。
...
### 正式授权	
1. 可通过在 [语音评测离线 SDK 控制台](https://console.cloud.tencent.com/soe/offline/index) 申请测试版本转正式版本，以及正式版本过期后重新申请正式授权，每一次正式授权的时间为一年（按当前 LicenseID 第一次激活算起到次年同日24点）。
2. 发起正式授权方式：在 [语音评测离线 SDK 控制台](https://console.cloud.tencent.com/soe/offline/index) 页面，单击**购买正式授权**或**转为正式授权**，通过接口人将测试版本转为正式版本。目前仅支持 Android 和 iOS 系统，如果有需要其他系统版本，请在控制台向我们发起咨询。

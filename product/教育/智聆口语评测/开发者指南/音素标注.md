## 音素配置
腾讯云智聆口语评测（Smart Oral Evaluation，SOE）支持音素评测，通过使用音素评测，用户可以完成对音标、地名、人名、生僻词、专有名词、自造词等 out-of-vocabulary（OOV）words 的评测，用户可通过上传音素进行自行配置。使用自行配置音素需要将 [textmode](https://cloud.tencent.com/document/product/884/19319) 参数设置为1。传入文本 reftext 需要上传 JSON String，示例可以参考按照调用文本中示例中的规则，按照音素映射表内的对应关系配置 RefText 文本。

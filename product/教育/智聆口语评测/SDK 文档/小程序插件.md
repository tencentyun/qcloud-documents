...
## 常见问题
### 小程序插件多页面示例
参考 demo [soe-sdk-demo.zip](https://drive.weixin.qq.com/s?k=AJEAIQdfAAox0i235zAFgA6wZGACg) 。index.js 的插件初始化是在 onShow 里， test.js 的插件初始化是在 onLoad 里，为了打开两个页面,所以用了两个初始化。插件的初始化可以做一个全局的变量初始化一次。let manager = null; 代码放在了 Page 对象外面，为了清除 manage 信息，防止有其它信息干扰。

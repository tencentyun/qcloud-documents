...
- 用户侧应用系统无法固定部署在与区块链所在地域相同的云上 VPC（私有网络） 中或用户不打算购买腾讯云 CVM。
...
调用方式：在页面上输入 API 密钥以及 [Tbass API（此链接为TbaaS API 请求结构的最新链接）](https://cloud.tencent.com/document/product/663/19457) 的必要参数。
...
## 使用区块链官方 SDK
### 适用场景及要求
- 用户侧应用系统对区块链的调用频率超过笔每秒（TPS）。
- 用户侧已经基于区块链原生的 SDK 进行了相关开发，迁移成本较大。
...
具体操作步骤可参考 [TBaaS 控制台](https://console.cloud.tencent.com/tbaas/fabric/deploy)：区块链网络->访问管理

...
### 数字资产服务
本例使用数字资产服务的资产发行接口 `asset_issue_apply` 为例。
1. <span id="step1"></span>用户需要设置自己的参数并封装成 POST 请求，请求的 URL 为 TBaaS 平台。用户自定义参数如下所示：
```
...
>?已从 [步骤1](step1) 中获取 `sign` 和 `transaction_id`。
>
```
...
### 信息共享服务
本例以信息共享服务的新增信息接口 `Iss_Append` 为例。
1. <span id="step"></span>用户需要设置自己的参数并封装成 POST 请求，请求的 URL 为 TBaaS 平台。用户自定义参数如下所示：
```

...
- approve: 批准函数，该函数具有两个入参，接收者地址`_spender`和token数量`_value`，该函数将接收者`_spender`可以从调用者`msg.sender`处转出的token数量设置为`_value`
- stop: 停止函数，该函数执行后，`transfer`，`transferFrom`和`approve`函数将不能再被执行
- start: 启动函数，该函数执行后，`transfer`，`transferFrom`和`approve`函数将可以再被执行
- setName: 设置token名字函数，该函数具有一个入参`_name`，将token的名字设置为`_name`

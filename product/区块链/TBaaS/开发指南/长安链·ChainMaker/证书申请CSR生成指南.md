...
1. 生成用户证书对应私钥和CSR文件
    ...
    ```
    - **生成用户证书CSR文件**：命令中使用的 `openssl_sign_user.cnf` 文件已包含在下载工具中，根据实际情况填写组织ID和证书标识。
    ```
    openssl req -batch -config openssl_sign_user.cnf -key temp -new -sha256 -out user_ecc_sign.csr
    ```
...
2. 生成用户tls证书对应私钥和CSR文件
    ...
    openssl ecparam -name prime256v1 -genkey -out temp
    ```
    - **生成用户tls证书CSR文件**：命令中使用的 `openssl_tls_user.cnf` 文件已包含在下载工具中，根据实际情况填写组织ID和证书标识。
    ```
    openssl req -batch -config openssl_tls_user.cnf -key temp -new -sha256 -out user_ecc_tls.csr
    ```
...
1. 生成用户证书对应私钥和CSR文件
    ...
    ```
    - **生成用户证书CSR文件**：命令中使用的 `gmssl_sign_user.cnf` 文件已包含在下载工具中，根据实际情况填写组织ID和证书标识。
    ```
    gmssl req -batch -config gmssl_sign_user.cnf -key temp -new -sm3 -out user_sm2_sign.csr
    ```
...
2. 生成用户tls证书对应私钥和CSR文件
    ...
    gmssl ecparam -name sm2p256v1 -genkey -out temp
    ```
    - **生成用户tls证书CSR文件**：命令中使用的 `gmssl_tls_user.cnf` 文件已包含在下载工具中，根据实际情况填写组织ID和证书标识。
    ```
    gmssl req -batch -config gmssl_tls_user.cnf -key temp -new -sm3 -out user_sm2_tls.csr
    ```

...
子账号首次登录时，主账号需要为子账号授予 QcloudMRPStdAccess 权限。添加权限的方式如下：
...
2. 在策略列表中搜索 QcloudMRPStdAccess 策略，选择相应的策略名称后单击**确定**。
>?对于已有的子账号，主账号可以在 [用户列表](https://console.cloud.tencent.com/cam) 页面中找到此账号，并单击**授权**进行权限修改。

## 步骤4：添加项目成员
创建项目完成后，添加项目成员的步骤如下：
1.	具有 QcloudMRPStdAccess 权限的子账号用户可单击 MRP 控制台项目列表中的**申请加入**申请加入项目。子账号用户需填写申请信息。

...
- 对于手动编辑计算文件的用户，MRP 提供多种计算文件的模板供用户使用。用户可单击复制模板，并且粘贴在编辑框中。
...
- 支持的计算文件类型包括：in 文件、势文件、结构文件。
 - in 文件：用户可选择上传或者使用页面右侧提供的模板。用户在页面右侧选择合适的模板，单击**使用此模板**，系统会自动在左侧编辑框中添加一个名称为 in，内容为模板内容的文件。用户可在编辑框中对该文件进行编辑。

...
## 操作步骤
#### 登录迁移服务平台
1.	登录 [迁移服务平台](https://console.cloud.tencent.com/msp) ，在左侧导航栏中单击【对象存储迁移】，进入对象存储迁移页面。
2.	单击【新建任务】，进入对象存储迁移配置页面。
...
3. 设置要迁移的文件来源。
此处迁移源服务提供商应选择腾讯云 COS，并在下方 AccessKey，SecretKey 文本框中输入先前新建用于迁移的腾讯云子用户 SecretId，SecretKey。源对象存储桶列表可在填入密钥后单击下拉框右侧刷新按钮获取。

...
#### 七牛云对象存储 KODO
密钥管理：
1.	登录七牛云控制台，在账号信息下拉菜单中单击进入密钥管理页面。
2.	在页面中单击显示按钮，即可查看七牛云 AK，SK 密钥。

源对象存储桶域名绑定：
1.	登录七牛云控制台，进入源对象存储桶，检查是否已配置融合 CDN 加速域名。
2.	若未配置，单击立即绑定一个域名按钮创建域名。您也可以在七牛云控制台直接进入融合【CDN 产品】>【域名管理】页面，进行添加域名操作。配置时需将七牛云源对象存储桶配置为源站。

...
下文将详细介绍当源对象存储部署在七牛云 KODO 时，如何配置全托管公网迁移任务，实现数据迁移。
...
3. 设置要迁移的文件来源。
此处迁移源服务提供商应选择七牛云 KODO，并在下方 AccessKey，SecretKey 文本框中输入七牛云 AK，SK 密钥。源对象存储桶列表可在填入密钥后单击下拉框右侧刷新按钮获取。
![](https://main.qcloudimg.com/raw/4c56ac67bfc9cd84de5578f9cd0675cf.jpg)
4. 选择文件存储方式。
...
9.	选择要迁移到的目标位置。
在迁移目标信息中，输入用于迁移的腾讯云子用户 SecretId，SecretKey。目标对象存储桶列表可在填入密钥后单击下拉框右侧刷新按钮获取。

“Agent半托管迁移”模式中，客户需要手工在源数据云厂商一侧的服务上部署Agent，Agent通过内网拉取源数据，并推送到腾讯云COS。
如果源数据厂商到腾讯云COS间已经拉通专线，Agent半托管迁移模式不会产生出流量费用。因此建议已经有专线的用户采用此模式进行迁移。
1. 专线准备确认
Agent半托管模式如果是通过专线迁移，需要确保友商云侧主机上使用COS SDK可经过专线访问COS，迁移前请与商务经理确认。 

2. 迁移限速
MSP迁移工具提供了限制QPS（对象存储模式）和带宽限速（URL列表模式）。在使用Agent迁移的情况下，也可以在迁移服务器上进行限速操作，同时在MSP中创建任务时选择“不限速”
...
2.3. 安装iproute工具（默认centos 7.x已安装，此步可跳过）

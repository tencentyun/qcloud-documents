...
## 操作场景
下文将详细介绍当源对象存储部署在金山云 KS3 时，如何配置全托管迁移任务和半托管迁移任务，实现数据迁移。

## 准备工作
#### 金山云对象存储 KS3
创建用于迁移的子用户并授予相关权限：
1. 登录金山云控制台。
2. 在右上角账号信息下拉菜单中，单击进入**访问控制** > **子用户**页面。
3. 创建子用户，勾选编程访问方式。
4. 完成子用户创建并保存生成的 AccessKeyID，AccessKeySecret。
5. 勾选新建的子用户，添加 KS3FullAccess 策略授权。
...
2. 创建用于迁移的子用户并授予相关权限：
	...
	5.	搜索并勾选 QcloudCOSAccessForMSPRole 及 QcloudCOSFullAccess 策略。
	6.	完成子用户创建并保存子用户名，访问登录密码，SecretId，SecretKey。
...
4. 设置要迁移的文件来源。
   此处迁移源服务提供商应选择`金山云KS3`，并在下方 AccessKey，SecretKey 文本框中输入先前新建用于迁移的金山云子账号 AccessKeyID 和 AccessKeySecret。填入密钥后，单击“迁移桶名称”下拉框右侧的**刷新**按钮，即可获取源对象存储桶列表。
   ![](https://qcloudimg.tencent-cloud.cn/raw/7882402677d4b2fc65f0903dc5eda95a.png)

   也可以选择手动输入源桶名称

   ![](https://qcloudimg.tencent-cloud.cn/raw/f088fb1c10d8f0afdc37d86b41fb0c90.png)

...
下文将详细介绍当源对象存储部署在七牛云 KODO 时，如何配置全托管迁移任务和半托管迁移任务，实现数据迁移。


## 准备工作
#### 七牛云对象存储 KODO
密钥管理：
1.	登录七牛云控制台，在账号信息下拉菜单中单击进入**密钥管理**页面。
2.	在页面中单击**显示**，即可查看七牛云 AK，SK 密钥。

源对象存储桶域名绑定（如果需要使用CDN加速域名来下载文件，此项是必须，如果使用 S3 域名下载，则此项非必须）：

1.	登录七牛云控制台，进入源对象存储桶，检查是否已配置融合 CDN 加速域名，
2.	若未配置，单击**立即绑定一个域名**创建域名。您也可以在七牛云控制台直接进入**融合 CDN**产品>**域名管理**页面，进行添加域名操作。配置时需将七牛云源对象存储桶配置为源站。
3.	按照七牛云教程在域名服务商处完成 CNAME 配置。当域名状态显示为成功时即可使用。若对域名绑定配置有疑问可咨询七牛云相关人员。
...
4. 设置要迁移的文件来源。
   ...
   也可以选择手动输入源桶名称
   ![](https://qcloudimg.tencent-cloud.cn/raw/7bf7cf8a3c8b3af16ed8373d9221b399.png)
5. 设置访问模式
   - CDN加速域名访问
     上述桶获取以后，点击此处可以获取在七牛控制台绑定的CDN加速域名列表，点选即可，任务运行会通过CDN加速域名访问待迁移文件。
      ![https://qcloudimg.tencent-cloud.cn/raw/71cfe4258c1ce3686a958e32bd4e68f2.png](https://qcloudimg.tencent-cloud.cn/raw/71cfe4258c1ce3686a958e32bd4e68f2.png)
   - S3兼容接口访问
     通过七牛控制台桶 空间管理->基础配置->S3域名->空间域名 获取到S3兼容访问接口的空间域名，在此处填入，任务运行会通过S3域名直接访问待迁移文件。
     ![](https://qcloudimg.tencent-cloud.cn/raw/70288f733b66044dd4e2b9b83f49016f.png)
6. 选择 Header 方式。

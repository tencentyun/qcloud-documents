...
## 操作场景
下文将详细介绍当源对象存储部署在百度云 BOS 时，如何配置全托管迁移任务和半托管迁移任务，实现数据迁移。

## 准备工作
#### 百度云对象存储 BOS
创建用于迁移的子用户并授予相关权限：
1.	登录百度云控制台。
2.	在右上角账号信息下拉菜单中单击进入多用户访问控制页面。
3.	单击**创建子用户**，勾选编程访问方式。
4.	完成子用户创建并保存生成的 AccessKeyID，AccessKeySecret。
5.	在子用户管理页右侧操作列单击添加权限按钮，勾选并应用 BOSFullControlAccessPolicy 策略。
...
4. 设置要迁移的文件来源。
   此处迁移源服务提供商应选择`百度云BOS`，并在下方 AccessKey，SecretKey 文本框中输入先前新建用于迁移的百度云 ccessKeyID 和 AccessKeySecret。填入密钥后，单击“迁移桶名称”下拉框右侧的**刷新**按钮，即可获取源对象存储桶列表。
   ![](https://qcloudimg.tencent-cloud.cn/raw/a02b0bd85cec95e4580e9a1735f264f6.png)

   也可以选择手动输入源桶名称

   ![](https://qcloudimg.tencent-cloud.cn/raw/10b8858cc7135f64cc63fc415908d93d.png)
...
11. 指定迁移到目标桶的指定目录和同名文件处理方式。
    ...
    >- 若同名文件选择**跳过**，会基于文件最后修改时间 LastModified判断，即

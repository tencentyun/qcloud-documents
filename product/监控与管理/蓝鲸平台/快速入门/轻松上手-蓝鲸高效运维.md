## 用户须知
蓝鲸是一个运维体系解决方案，在 PaaS 平台上有众多 SaaS 产品供你使用，每个 SaaS 产品都具备专属的能力。本文讲解的是高效运维需要协同配合的 SaaS，主要涉及到以下几个产品：

1.【配置平台】：配置业务与机器的关系。为什么要先操作配置平台呢？配置平台是一切操作的基础，因为它配置业务，以及业务下有哪些机器，业务的负责人，方便用户分权限和隔离来管理机器（需要用户自己进入”资源池“，将腾讯云的业务和机器会同步到配置平台）。
2.【安装 Agent 】：Agent 非常小，不影响机器的使用，是命令执行、文件传输、数据上报的基础。
3.【作业平台】：通过 web 端，可以执行脚本，下发文件等。
4.【标准运维】：成熟的流程引擎，支持进行调度编排。
...
添加完成后，在拓扑模型界面即可显示出你所添加的模块，如下图所示。
...
10. 安装完成后，返回主页，可看到该业务的中中转机和 Agen t的概况信息
![](https://mc.qcloudimg.com/static/img/73ee86e7712e31e7293eecf254d49858/12.jpg)
11. 如果您需要使用蓝鲸 Agent 管理 Windows 服务器，您需要在 Windows 服务器上自行安装 cygwin，具体安装步骤可以参考由安装页面给出的提示信息。

## 5. 使用作业平台输出一个 HelloWorld
...
（3）创建好的流程，点击【保存】进行保存。原子里的参数，需要保存为流程里的参数。点击【保存】时会提示，一键设置就完成了。

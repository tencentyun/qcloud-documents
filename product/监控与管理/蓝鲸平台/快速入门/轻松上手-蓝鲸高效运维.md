...
蓝鲸是一个运维体系解决方案，在 PaaS 平台上有众多 SaaS 产品供您使用，每个 SaaS 产品都具备专属的能力。本文讲解的是高效运维需要协同配合的 SaaS，主要涉及到以下几个产品：

- 配置平台：配置业务与机器的关系。为什么要先操作配置平台呢？配置平台是一切操作的基础，因为它配置业务，以及业务下有哪些机器，业务的负责人，方便用户分权限和隔离来管理机器（需要用户自己进入”资源池“，将腾讯云的业务和机器会同步到配置平台）。
- 安装 Agent ：Agent 占用空间小，不影响机器的使用，是命令执行、文件传输、数据上报的基础。
...
（2）单击左上角的【新增业务】，您可以选择建2级或是3级业务，并且填写业务运维人员的信息。
...
步骤1： 单击打开蓝鲸桌面上的【 Agent 安装2.0】应用，如下图所示。
...
步骤7：安装好中转机后，可为其他机器安装 Agent，如下图所示。

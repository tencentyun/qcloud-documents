...
1. 登录需要采集设备指纹的机器，在 root 权限下运行指纹采集工具（getdevUUID），得到长度32位的字符串，即为设备指纹。
2. 通过 [密钥管理系统控制台](https://console.cloud.tencent.com/kms2) 或 [API]() 的方式，将收集到的设备指纹列表上传并绑定到指定的白盒密钥上。

>!
>1. 指纹采集工具（getdevUUID）请直接从 [密钥管理系统控制台](https://console.cloud.tencent.com/kms2) 上下载。
>2. 在 Linux 系统下，指纹采集工具需要在 root 权限下运行。

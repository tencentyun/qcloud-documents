...
![](https://main.qcloudimg.com/raw/f5d40c7b397701fdda63e8236fa26e7b.svg)
...
准备需要使用白盒加密的明文，并对该明文进行 base64 编码。
...
>!返回的字段主要有两个：
> - Iv：随机生成的初始化向量（Initialization Vector），用于增加密文被破解的难度。
> - CipherText：加密后的密文，并进行了 base64 编码。

### 步骤4：白盒解密密钥和 API Key 密文分发

管理员将上述步骤中生成的 DecryptKey，Iv，CipherText 分发给各业务系统的开发或运维人员。其中，DecryptKey 会被部署到相应业务系统的文件中，而 Iv 和 CipherText 会作为 SDK 的传参。
...
2. 将 Iv，CipherText，decrypt_key_bin_dir，decrypt_key.bin 以字符串的形式，作为 SDK 中的 whitebox_decrypt 方法的传入参数。

### 步骤5：部署白盒解密密钥

各业务系统根据自身的编程语言，选择下载相应编程语言的解密 SDK（TODO：下载链接），将 SDK 集成到业务系统中。

### 步骤6：使用 API Key 密文

在业务逻辑中调用 SDK 的解密函数（whitebox_decrypt），传入参数：decrypt_key_bin_dir，decrypt_key.bin，Iv，CipherText，algorithmType 从而获得解密后的明文。其中 algorithmType 是生成密钥时使用的算法类型。取值为0或1。0表示 AES_256，1表示 SM4。

...
从上述流程可以看出，API Key 并没有暴露在开放环境中，在实际的用户业务的服务器上只保存了 API Key 的密文，并且可以针对不同的用户，不同的服务器生成不同的 API Key 的密文。这样即使在某些场景下，API Key 的密文被破解，造成的损失也是局部的，并不会影响到 API Key 继续在其他服务器和场景中使用。例如，针对 GitHub 上源代码的 API Key 泄露事件，如果采用白盒密钥管理的解决方案，就可以彻底杜绝 API Key 的明文暴露在源代码中，从而防止 API Key 被恶意利用。

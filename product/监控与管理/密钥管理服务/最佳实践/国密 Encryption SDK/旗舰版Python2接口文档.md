...
| msgCount     | 是   | int                   | 每个缓存 DataKey 可加密的消息数量，加密的数量达到后，会重新向 KMS 后台请求，生成新的DataKey，设置为0表示没有限制使用次数 |
...
| pubKey   | 是   | str | 公钥内容，数据长度固定为64字节   |
| priKey   | 是   | str | 私钥内容，数据长度固定为32字节   |
| digest   | 是   | str | **HashForSM3WithSM2** 生成的摘要信息内容 |
...
| -------- | ---- | ------ | ---------------------------------------- |
| pubKey   | 是   | str | 公钥内容，数据长度固定为64字节   |
| sig      | 是   | str | **Sm2SignWithDigest** 生成的签名内容                               |
| digest   | 是   | str | **HashForSM3WithSM2** 生成的摘要信息内容 |
...
| -------- | ---- | ------ | -------------------------------------------- |
| source   | 是   | str | 原文数据                                     |
| Key      | 是   | str | 用户自定义的 SM4 密钥，长度固定为128位(16字节) |
| iv       | 是   | str | 初始化向量，固定为128位(16字节)              |

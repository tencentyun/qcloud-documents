...
Grafana 可视化服务是腾讯与 Grafana Lab 联合的发布的原生可视化解决方案， 预制了大量腾讯云相关相关数据源。同时利用 SSO 及网络访问控制，提升了数据安全性。使用托管服务可免除用户搭建、运维和升级等相关工作量。
...
目前我们托管的是 Grafana 社区版，后续我们将会提供付费升级选项。让用户可以升级到 Grafana Enterprise 版本，获得对接 Gitlab、Honeycomb、MongoDB、NewRelic、Salesforc、ServiceNow 和 Snowflake 等数据源，并获得定时报表等企业级能力。

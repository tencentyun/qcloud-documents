...
### 授权场景
当您已注册并登录腾讯云账号后，首次登录 [混沌演练平台控制台](https://console.cloud.tencent.com/cfg) 时，需前往**访问管理**页面对当前账号授予腾讯云混沌演练平台服务操作云服务器（CVM）、负载均衡 (CLB)、腾讯云自动化助手(TAT)、弹性缓存Redis(Redis)、 云数据库MySQL(CDB)、云监控(Monitor)、私有网络(VPC)等云资源的权限。

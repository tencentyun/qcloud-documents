...
2. API 密钥托管「废弃」：需要将用户将 [API 密钥](https://console.cloud.tencent.com/cam/capi)（SecretID 和 SecretKey）托管在 TIC 平台中，TIC 平台对 API 密钥加密存储后，将通过托管的 API 密钥对请求进行签名，实现对云资源的编排操作。API 密钥托管功能仅对存量用户（TIC 平台中存在托管的 API 密钥 ）可见。
...
|            | CFS           | 文件存储                |
|            | COS           | 对象存储                |
| 中间件     | Ckafka        | 消息服务（Ckafka）      |
|            | CMQ           | 消息服务（CMQ）         |
|            | API Gateway   | API 网关                |
| 数据库     | MySQL         | 云数据库（MySQL）       |
|            | MongoDB       | 云数据库（MongoDB）     |
|            | Redis         | 云数据库（Redis）       |
|            | CynosDB       | 云数据库（CynosDB）     |
|            | TDSQL         | 分布式数据库（TDSQL）   |
|            | TcaplusDB     | 游戏数据库（TcaplusDB） |
| CDN与加速  | CDN           | 内容分发网络            |
|            | SCDN          | 安全加速                |
|            | GAAP          | 全球应用加速            |
| 大数据     | Elasticsearch | Elasticsearch           |
| 游戏服务   | GME           | 游戏多媒体引擎          |
| 云通信     | SMS           | 短信                    |
|            | SSL           | SSL证书                 |
| 管理与审计 | CAM           | 访问管理                |
...
> !该列表会持续更新，若您希望将不在上述列表中的云产品纳入到 TIC 中进行编排，请您 [提交工单](https://console.cloud.tencent.com/workorder/category) 告知 TIC 团队，我们将竭诚为您服务。

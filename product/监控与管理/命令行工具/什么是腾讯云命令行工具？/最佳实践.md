...
5. 执行以下命令进行验证。从下图验证结果可以看出，当 Context 为 sa-context 时，默认命名空间为 sa-test， 且对 sa-test 命名空间下拥有 sa-role-test（Role）对象中配置的权限，但在 default 命名空间下不具有任何权限。
...
4. 执行以下命令进行验证。从下图验证结果可以看出，当 Context 为 role_user 时，默认命名空间为 default，且拥有 test-clusterrole 权限对象配置的规则权限。
...
6. 执行以下命令进行验证。从下图验证结果可以看出，在 default2 命名空间下， role_user 同样拥有 test-clusterrole 配置的权限规则，至此通过上述步骤实现了多个命名空间复用集群权限的绑定。
...
此方式主要用于对某个用户绑定所有命名空间下的权限（集群范围），适合集群范围内授权的场景，例如日志收集权限、管理人员权限等，以下介绍在 TKE 中如何使用多个命名空间复用集群权限绑定授权。

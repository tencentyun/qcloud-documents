...
1. 使用以下 Shell 脚本，创建测试命名空间、  `ServiceAccount`  类型的测试用户并设置集群访问凭证（token）认证 ： 
```bash
...
4. 创建一个  `RoleBinding` 对象资源 sa-rb-test.yaml 文件，如下的权限绑定 YAML 表示：添加 ServiceAccount 类型的 sa-acc 用户在 sa-test 命名空间具有 sa-role-test（Role 类型）的权限。
<dx-codeblock>
:::  yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: RoleBinding

...
如上图所示， Kubernetes RBAC 授权主要提供了以下3种常用权限绑定方式，本文将为您分别介绍如何使用这3种权限绑定方式实现对用户的授权管理。
...
### 方式1：作用于单个命名空间权限绑定[](id:way1)

此方式主要用于为某一个用户绑定某一个命名空间下的相关权限，适合需要细化权限的场景，例如开发、测试、运维人员只能在各自的命名空间下对资源操作。以下将为您介绍如何在 TKE 中实现作用于单个命名空间的权限绑定。

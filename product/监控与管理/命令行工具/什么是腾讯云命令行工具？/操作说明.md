...
- TKE 集群版本 ≥ 1.14
- 使用 CBS CSI 组件，才可在 TKE 集群中对 CBS 盘在线扩容和使用快照。
...
- 方式1：重启 Pod 的情况下进行在线扩容
待扩容的云硬盘文件系统未被挂载，能够避免扩容出错以及方式2存在的问题。**推荐使用该方式进行扩容**。
- 方式2：不重启 Pod 的情况下进行在线扩容
在节点上挂载着待扩容的云硬盘的文件系统，如果存在 I/O 进程，将可能出现文件系统扩容错误。
...
```
3. 执行以下命令，可以查看到 PV 和文件系统已经扩容至30G。示例如下：
```plaintext
$ kubectl exec ivantestweb-0 df /usr/share/nginx/html
Filesystem     1K-blocks  Used Available Use% Mounted on
/dev/vdd        30832548 44992  30771172   1% /usr/share/nginx/html
...
- TKE 集群版本 >= 1.18，详见[cbs csi文档](https://github.com/TencentCloud/kubernetes-csi-tencentcloud/blob/master/docs/README_CBS.md "cbs csi文档")
- 仅 cbs csi 插件支持快照，确保csi插件镜像为最新版本

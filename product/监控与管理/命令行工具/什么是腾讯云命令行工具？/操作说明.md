...
腾讯云容器服务 [云原生监控](https://cloud.tencent.com/document/product/457/49889) 兼容 Prometheus 与 Grafana 的 API，同时也兼容主流 prometheus-operator 的 CRD 用法，提供了极大的灵活性与扩展性，可以结合 Prometheus 开源生态的工具解锁更多高级玩法。
...
5. 在【RawJob】页面下单击【新增】，将原生 Prometheus 配置文件中的 Job 配置复制粘贴到此配置窗口中。
...
本文提供以下自建 Prometheus 告警原始配置 YAML 文件为例，介绍如何将其转换为云原生监控上类似的监控配置。[](id:prometheus-native)
<dx-codeblock>
:::  yaml
  - alert: NodeNotReady
...
:::
::: 接入自建\sAlertManager
如需实现更复杂的告警需求，或期望使用内部自建的 AlertManager 进行统一告警，可以选择让云原生监控的告警接入自建的 AlertManager，您只需在 [创建监控实例](https://cloud.tencent.com/document/product/457/49889#.E5.88.9B.E5.BB.BA.E7.9B.91.E6.8E.A7.E5.AE.9E.E4.BE.8B) 时，在高级设置中填入自建 AlertManager 的地址即可，如下图所示：

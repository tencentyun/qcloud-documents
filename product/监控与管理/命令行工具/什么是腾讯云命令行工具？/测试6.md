...
上述K8S 设计逻辑导致用户无法自定义 HPA 的扩缩容灵敏度，而不同的业务场景对于扩容容灵敏度要求可能是不一样的，例如：
...
上述示例表示扩容时立即新增当前9倍数量的副本数，即立即扩容到当前10倍的 Pod 数量，最大不超过 `maxReplicas` 的限制。

例如，开始只有1个 Pod，如果遭遇流量突发，它将以飞快的速度进行扩容，扩容时 Pod 数量变化趋势如下:
...
在流量高峰下降，并发量骤降的场景中，使用默认的缩容策略，几分钟后 Pod 数量也会随之骤降。此时如果 Pod 缩容后突然又来一个流量高峰，虽然可以快速扩容，但扩容的过程需要一定时间，如果流量高峰足够高，在这段时间内可能造成后端处理能力跟不上，将导致部分请求失败。这时候我们可以为 HPA 加上缩容策略，HPA `behavior` 配置示例如下:
...
上述示例中增加了 `scaleDown` 的配置，指定缩容时每10分钟缩掉1个 Pod，大大降低了缩容速度，缩容时的 Pod 数量变化趋势如下:

```
1000 -> … (10 min later) -> 999
```

通过上述配置可以让关键业务在可能有流量突发的情况下保持处理能力，避免流量高峰导致部分请求失败。
...
例如，一开始只有1个 Pod，扩容时它的 Pod 数量变化趋势如下：
```
1 -> 2 -> 3 -> 4
```
...
上述示例表示当负载降下来时，将等待 600s (10 分钟) 再进行缩容，每次只缩容5个 Pod。



### 延长扩容时间窗口

部分应用经常会遇到数据毛刺导致频繁扩容，而扩容出来的 Pod 其实没太大必要，反而浪费资源。例如，数据处理管道的场景，扩容指标是队列中的事件数量， 当队列中堆积了大量事件时，我们希望可以快速扩容，但又不希望太灵敏，因为可能只是短时间内的事件堆积，即使不扩容也可以很快处理掉。

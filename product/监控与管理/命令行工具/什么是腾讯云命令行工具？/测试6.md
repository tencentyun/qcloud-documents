...
部分业务应用经常会遇到数据毛刺导致频繁扩容，在短时间内 Pod 并不需要扩容，扩容的 Pod 反而可能造成资源浪费。例如，数据处理管道的场景，扩容指标是队列中的事件数量， 当队列中堆积了大量事件时，希望在负载持续超过扩容阈值时才快速扩容，因为可能只是短时间内的事件堆积，即使不扩容队列也可以快速处理事件。

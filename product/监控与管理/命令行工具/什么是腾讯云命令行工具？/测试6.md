...
- 对于流量突发的关键业务，需要快速扩容（防止流量瓶颈），但缩容要慢（防止另一个流量高峰）。
...
部分业务应用经常会遇到数据毛刺导致频繁扩容，而扩容出来的 Pod 其实没太大必要，反而浪费业务资源。例如，数据处理管道的场景，扩容指标是队列中的事件数量， 当队列中堆积了大量事件时，希望在负载持续超过扩容阈值是才快速扩容，因为可能只是短时间内的事件堆积，即使不扩容也可以很快处理掉。
...
示例表示扩容时，需要先等待5分钟的时间窗口，如果在这段时间内负载降下即不再扩容，如果负载持续超过扩容阈值才扩容，每次扩容新增20个 Pod。

...
如需要对 PVC 的数据盘创建快照来备份数据，或者将快照备份的数据恢复到新的 PVC 中，可以使用 CBS CSI 插件来实现，本文将介绍如何利用 CBS CSI 插件实现 PVC 的数据备份与恢复。

## 前提条件

- 已创建 [TKE 集群](https://cloud.tencent.com/document/product/457/32189) 或已在腾讯云自建 K8S 集群，集群版本 >= 1.18。
...
1. 使用以下 YAML，创建 `VolumeSnapshotClass` 对象。
```yaml
...
```
2. 创建后执行以下命令检查 VolumeSnapshotClass 状态。示例如下：
```bash
...
1. 使用以下 YAML，创建 `VolumeSnapshot` 对象。本文以 `new-snapshot-demo` 快照名为例创建快照，示例如下：
```yaml
apiVersion: snapshot.storage.k8s.io/v1beta1
kind: VolumeSnapshot
metadata:
...
```
2. 创建后等待几分钟，执行以下命令，查看 volumesnapshot 和 volumesnapshotcontent 对象是否创建成功，若 `READYTOUSE` 为 true，则创建成功。示例如下：
```bash
...
```
3. 执行以下命令，可以获取 volumesnapshotcontent 对象的快照 ID，字段是 `status.snapshotHandle` (snap-rsk8v75j)，可以根据快照 ID 在 [容器服务控制台](https://console.cloud.tencent.com/tke2) 确认快照是否存在。示例如下：
```bash
...
1. 本文以上述步骤 `VolumeSnapshot` 的对象名为 `new-snapshot-demo` 为例，使用以下 YAML 来从快照恢复数据到新的 PVC 中：
```yaml
apiVersion: v1
...
```
2. 执行以下命令，可以查看 PVC 已经创建并绑定 PV，从 PV 中也可以看到对应的 diskid (disk-ju0hw7no)。示例如下：
``` bash

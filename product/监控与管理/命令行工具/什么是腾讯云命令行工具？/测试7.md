...
在 K8S 集群中，部分 CI/CD 流水线业务可能需要使用 Docker 来提供镜像打包服务，一种实现方式是利用宿主机上的 Docker，具体做法是将宿主机上 Docker 的 unix socket (`/var/run/docker.sock`) 作为 hostPath 挂载到 CI/CD 的业务 Pod 中，之后在容器里通过 unix socket 来调用宿主机上的 docker 进行构建。该方式固然简单，且比真正意义上的 Docker in Docker 甚至还能节省资源，但该方式可能会遇到以下问题：

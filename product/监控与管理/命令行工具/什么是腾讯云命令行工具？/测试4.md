## 使用场景
TKE 的集群审计和事件存储为用户配置了丰富的可视化图表，以多个维度对审计日志和集群事件进行呈现，操作简单且足以涵盖绝大多数常见集群运维场景， 发现和定位问题都将事半功倍，提升运维效率，真正将审计和事件数据的价值最大化。
...
```
4. 在左侧“隐藏字段”中选择 “`event.reason`”、“`event.message`”、“`event.involvedObject.name`”、"`event.involvedObject.name`" 进行显示，单击【检索分析】即可开始检索分析日志并返回结果。
5. 将检索结果按照”日志时间“倒序排列，检索结果如下图所示：

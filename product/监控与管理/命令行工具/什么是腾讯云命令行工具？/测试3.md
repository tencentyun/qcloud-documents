...
另外 prometheus-adapter 也同时支持 Custom Metrics API 和 External Metrics API。在实际环境中，使用 Custom Metrics API 即可满足大部分场景。本文将针对 Custom Metrics API 详细介绍如何实现使用自定义指标进行弹性伸缩。
...
您可以通过以下两种方式，配置 Promtheus 采集业务暴露的监控指标。
...
使用 Helm 安装 [prometheus-adapter](https://artifacthub.io/packages/helm/prometheus-community/prometheus-adapter)，安装前请确定并配置自定义指标。按照上文 [示例](example)，在业务中使用 `httpserver_requests_total` 指标来记录 HTTP 请求，因此可以通过如下的 PromQL 计算出每个业务 Pod 的 QPS 监控：
```
...
假如设置每个业务 Pod 的平均 QPS 达到50时，就触发扩容，最小副本为1个，最大副本为1000，则配置示例如下：
``` yaml

...
如果有更复杂的场景需求，例如基于业务单副本 QPS 大小来进行自动扩缩容，可以考虑自行安装 [prometheus-adapter](https://github.com/DirectXMan12/k8s-prometheus-adapter) 来实现，本文将介绍如何在 TKE 上使用自定义指标进行弹性伸缩。
...
另外 prometheus-adapter 也同时支持两种 API ，在实际环境中，使用 Custom Metrics API 即可满足大部分场景。本文针对 Custom Metrics API 详细介绍如何实现使用自定义指标进行弹性伸缩。
...
本文以 golang 业务程序为例，暴露 HTTP 请求的监控指标。示例如下：
...
将业务程序进行容器化并部署到 TKE 集群，通过使用 Deployment 部署。示例如下：
...
该示例程序暴露了 `httpserver_requests_total` 指标，记录 HTTP 的请求，通过该指标可以计算出该业务程序的 QPS 值。


### Prometheus 采集业务监控

配置 Promtheus 采集业务暴露的监控指标。

#### 方式一：配置 Promtheus 采集规则

在 Promtheus 的采集规则配置文件添加采集规则，示例如下：
...
使用 Helm 安装 [prometheus-adapter](https://artifacthub.io/packages/helm/prometheus-community/prometheus-adapter)，安装前请确定并配置自定义指标，按照上述示例，在业务中使用 `httpserver_requests_total` 指标来记录 HTTP 请求，可以通过如下的 PromQL 计算出每个业务 Pod 的 QPS 监控：
...
执行 Helm 命令安装 prometheus-adapter，示例如下：
...
若安装正确，执行以下命令，可以查看 Custom Metrics API 返回配置的 QPS 相关指标。示例如下：
...
假如设置每个业务 Pod 的平均 QPS 达到50，就触发扩容，最小副本为1个，最大副本为1000，配置示例如下：

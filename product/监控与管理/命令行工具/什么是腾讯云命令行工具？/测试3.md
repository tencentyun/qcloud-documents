...
针对更复杂的场景，例如基于业务单副本 QPS 大小来进行自动扩缩容，可以通过安装 [prometheus-adapter](https://github.com/DirectXMan12/k8s-prometheus-adapter) 来实现，本文将介绍如何在 TKE 上使用自定义指标进行弹性伸缩。
...
执行以下 Helm 命令安装 prometheus-adapter。示例如下：
```bash

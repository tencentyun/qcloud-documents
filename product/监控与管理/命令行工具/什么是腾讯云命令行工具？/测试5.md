...
- 传统业务在做容器化改造的过程中，部分服务的代码写死了用固定域名调用内部其它服务，并且不可配，无法使用 Kubernetes 的 Service 名称进行调用。
...
| [方案3](#scheme3) | 可以管理大量的解析记录，记录的管理都在自建 DNS 中，增删记录无需修改 coredns 配置。具体采用哪种方案，请根据自身需求来评估。 | 

>?其中方案1和方案2，每次添加解析记录都需要修改 coredns 配置文件（无需重启）。

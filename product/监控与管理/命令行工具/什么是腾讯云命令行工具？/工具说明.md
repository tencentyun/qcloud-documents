...
>! 请确保您已经按照【[依赖部署](#Dynamic)】>【Prometheus 规则配置】进行监控数据采集规则配置，组件才可以拉取到需要的监控数据，调度策略才会生效。

- 如果您使用自建 Prometheus，直接填入数据查询 URL（HTTP/HTTPS）即可。
- 如果您使用托管 Prometheus, 选择托管实例 ID 即可，我们会自动解析实例对应的数据查询 URL。

...
<tr>
<td>s3Url</td>
<td>对象存储 COS 兼容的 S3 API 访问地址。请注意该访问地址中的域名不是上述创建 COS 存储桶的公网访问域名，须使用格式为 <code>https://cos.&lt;region&gt;.myqcloud.com</code> 的 URL，例如地区是广州的话，参数值为 <code>https://cos.ap-guangzhou.myqcloud.com</code></td>
</tr>
</tbody></table>

另外其他安装参数可以使用命令 `velero install --help` 查看。例如，不备份存储卷数据，可以设置 `--use-volume-snapshots=false` 来关闭存储卷数据快照备份。
...
6. 执行以下命令查看存储位置状态，若显示 “Avaliable”，则说明正常访问腾讯云 COS，如下图所示：
...
2. 登录 minio Web 管理页面，上传用于测试的图片数据， 如下图所示：
![](https://main.qcloudimg.com/raw/9dd7e1f08709292e5c58f5744ffb5f10.png)
3. <span id="velerostep3"></span>使用 Velero 备份，可以直接备份集群中的所有对象，也可以按类型，名称空间和/或标签过滤对象。您可以使用以下命令仅备份 default 命名空间下所有资源。示例如下：
```
...
5. 执行以下命令，删掉 minio 所有资源，包括它的 PVC 持久卷。如下图所示：
![](https://main.qcloudimg.com/raw/23a84feb0f24b45484175278c97c01b9.png)
6. 删掉 minio 资源后，就可以测试使用之前的备份来还原被删除的 minio 资源。执行以下命令，将备份存储位置临时更新为只读模式（这可以防止在还原过程中在备份存储位置中创建或删除备份对象）。
```bash
...
```
	通过命令 `velero restore get` 查看还原任务的状态，若还原状态是 “Completed”，则说明还原任务完成，还原过程如下图：
...
9. 在浏览器上登录 minio 的管理页面，可以查看到之前上传的图片数据，说明持久卷的数据还原成功。如下图所示：
>!这里我们使用的 restic 来备份、还原的持久卷，但 restic 不支持 `hostPath` 类型卷，详情请参见 [restic 限制](https://velero.io/docs/v1.5/restic/#limitations)。
>
![](https://main.qcloudimg.com/raw/1ed0a87a01ece26311c41a026cc8013a.png)
10. 另外在还原完成后，执行以下命令，将备份存储位置恢复为读写模式，以便下次备份任务成功使用。示例如下：
```bash
kubectl patch backupstoragelocation default --namespace velero \
			--type merge \

...
本文为您介绍如何使用云函数 SCF 处理 [视频处理 MPS](https://cloud.tencent.com/document/product/862) 回调消息。其中，MPS 主要用于视频处理任务，SCF 主要提供回调消息处理。
...
1. 登录云函数控制台，选择左侧导航栏中的【[函数服务](https://console.cloud.tencent.com/scf/list)】。
2. 在“函数服务”列表页面上方，选择期望配置 MPS 触发器的函数所在的地域及命名空间。
3. 单击需要配置的函数名，进入该函数的详情页面。
4. 选择左侧菜单栏中的【触发器管理】>【创建触发器】，在弹出的“创建触发器”窗口中添加 MPS 触发器。如下图所示：
...
>?初次创建 MPS 触发器，需单击【SCF_QcsRole】、【MPS_QcsRole】链接对相关服务角色进行授权。
 
 
<span id="step05"></span>

### 测试函数功能
1. 在 [视频处理控制台](https://console.cloud.tencent.com/mps) 执行视频处理工作流。
...
3. 登录 [对象存储控制台](https://console.cloud.tencent.com/cos5)，查看数据转储及加工结果。

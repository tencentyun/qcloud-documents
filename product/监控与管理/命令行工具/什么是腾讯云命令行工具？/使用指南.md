...
已安装 [Nginx-ingress](https://cloud.tencent.com/document/product/457/50503)。
...
Nginx-ingress 扩展组件安装好后，在 `kube-system` 下会有个 Nginx-ingress 的 operator 组件，该组件可以创建多个 nginx ingress 实例，每个 nginx ingress 实例都使用不同的 ingressClass，且使用不同的 CLB 作为流量入口，从而可以实现不同 Ingress 绑定到不同流量入口。


1. 我们可以根据需求，为集群创建多个 nginx ingress 实例，在【组件管理】里找到安装好的 nginx ingress 扩展组件，点进去:
![](https://main.qcloudimg.com/raw/7605a808c6116286593bae4323a04dbf.png)
2. 然后点击 【新增Nginx Ingress实例】:
![](https://main.qcloudimg.com/raw/58750dcc07cf87284573355ed905e0b4.png)
3. 根据需求配置 Nginx Ingress 实例即可，每个实例指定不同的 ingressClass 名称:
![](https://main.qcloudimg.com/raw/f4c859abc629f7a62aa659194da9fb3a.png)
4. 后续在控制台创建 Ingress 时可指定具体的 ingressClass 来将 Ingress 绑定到具体的 Nginx Ingress 实例上:
![](https://main.qcloudimg.com/raw/9c00f36caddb98064359fccdcd61e5d4.png)
5. 若使用 yaml 创建 Ingress，则需指定下 ingressClass 的 annotation (`kubernetes.io/ingress.class`):

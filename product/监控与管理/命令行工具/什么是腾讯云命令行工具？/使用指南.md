...
- 传统业务在做容器化改造的过程中，部分服务的代码配置了用固定域名调用内部其它服务，并且无法修改配置，即无法使用 Kubernetes 的 Service 名称进行调用。
...
1. 查看 forward 配置。forward 默认配置如下所示，指非集群内域名通过 coredns 所在节点上 `/etc/resolv.conf` 中配置的 nameserver 解析。
```yaml
forward . /etc/resolv.conf
```

...
Nginx-ingress 扩展组件安装好后，在 `kube-system` 下会有个 Nginx-ingress 的 operator 组件，该组件可以创建多个 Nginx Ingress 实例，每个 Nginx Ingress 实例都使用不同的 IngressClass，且使用不同的 CLB 作为流量入口，从而实现不同 Ingress 绑定到不同流量入口。可以根据实际需求，为集群创建多个 Nginx Ingress 实例。
...
>?Nginx Ingress 实例详细的创建步骤，请参见 [安装 Nginx-ingress 实例](https://cloud.tencent.com/document/product/457/50503)。
...
2. 【Class】选择上述步骤创建的 Nginx Ingress 实例。
...
创建 Ingress 的具体步骤，请参见[ Ingress 基本功能](https://cloud.tencent.com/document/product/457/31711)。
</dx-alert>

...
目前大多数开源应用的容器镜像（例如 Kubernetes、TensorFlow 等），都托管在海外镜像托管平台（例如 DockerHub、`quay.io` 等），在国内拉取镜像时可能会因为网络问题导致拉取速度慢，甚至无法成功拉取，往往需要手动将镜像 Pull 到本地，再 Push 到自己的镜像仓库进行手动同步，过程极其繁琐且无法覆盖全部仓库及最新镜像版本。
腾讯云 [容器镜像服务 TCR](https://cloud.tencent.com/document/product/1141) 企业版提供主流海外镜像托管平台的加速服务, 可以有效解决海外镜像拉取难导致开源应用无法顺利部署的问题。
...
- 加速服务目前只支持腾讯云 [私有网络 VPC](https://cloud.tencent.com/document/product/215) 访问，公网访问能力暂未开放，相关域名可访问但无法提供实际的加速功能。
...
:::
 ...
 >?您还可以使用 Ansible 来批量修改存量节点的 containerd 配置，详情请参见 [使用 Ansible 批量操作 TKE 节点](https://cloud.tencent.com/document/product/457/48973)。

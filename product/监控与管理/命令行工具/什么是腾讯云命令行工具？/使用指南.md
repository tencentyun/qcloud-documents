...
目前大多数开源应用的容器镜像（例如 Kubernetes、TensorFlow 等），都托管在海外镜像托管平台（例如 DockerHub、`quay.io` 等），在国内拉取镜像时可能存在网络问题导致拉取速度慢、甚至无法成功拉取，常见解决方法为手动将镜像 Pull 到本地，再 Push 到自己的镜像仓库进行手动同步，但该方法过程极其繁琐且无法覆盖全部仓库及最新镜像版本。
腾讯云 [容器镜像服务 TCR](https://cloud.tencent.com/document/product/1141) 企业版提供主流海外镜像托管平台加速服务, 可以有效解决海外镜像拉取难导致开源应用无法顺利部署的问题。本文将介绍 TKE 集群如何通过 TCR 加速服务实现海外镜像拉取加速。
...
:::
::: Containerd
对于运行时为 Containerd 的节点，由于 Containerd 本身支持任意镜像仓库的加速地址配置，可以通过修改 Containerd 配置，实现不更改镜像地址就可自动加速拉取镜像，适用于大量海外镜像的场景，可减少大量地址修改的繁琐步骤。
1. TKE 添加节点或者使用节点池，都可以将节点写入自定义脚本，通过脚本可以统一修改增量节点的 Containerd 配置、添加海外镜像的加速地址。脚本示例如下：
<dx-codeblock>
:::  plaintext

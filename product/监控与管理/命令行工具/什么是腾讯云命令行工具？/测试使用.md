...
Traefik 支持使用 Kubernetes 的 Ingress 资源作为动态配置，可以直接在集群中创建 Ingress 资源来对外暴露集群，需要加上指定的 Ingress class（安装 Traefik 时定义）。示例如下：
...
Traefik 不仅支持标准的 Kubernetes Ingress 资源，也支持 Traefik 特有的 CRD 资源，例如使用最多的是 IngressRoute，可以支持更多 Ingress 不具备的高级功能。示例如下：

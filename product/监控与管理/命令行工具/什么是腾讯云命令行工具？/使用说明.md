...
> ? 
> 1. 博睿探针目前支持了多数主流框架，详情请参见 [博睿探针支持列表](#list)。只有使用这些支持的框架，博睿 smartAgent 才能自动捕获。
...
绑定层以后，探针并不会自动启动，需要在代码入口引入探针。探针运行会占用少量内存，但不会影响您的业务运行。如业务代码本身占用了大量内存，探针将触发熔断机制以保障业务运行。目前提供 Nods.js 和 Python 引入：
...
1. 在 layer 目录下新建 `serverless.yml` 文件，`serverless.yml` 文件如下：
```
component: layer
name: bonree_agent
org: tencent
...
```
2. 统一在 src 目录下创建 Flask 的 `serverless.yml` 文件，`serverless.yml` 文件如下：
```
component: flask
name: bonree_flask
org: tencent
...
└── src
    └── serverless.yml
```
4. 只使用 layer component 便可以完成层的创建。在函数中配置 layers 参数可以完成绑定的操作，您也可以选择在云函数控制台手动绑定层。
5. 在根目录下执行 `sls deploy` 进行应用的部署。

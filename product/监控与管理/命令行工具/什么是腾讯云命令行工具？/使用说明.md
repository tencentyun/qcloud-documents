...
DeScheduler 组件依赖于 Node 当前和过去一段时间的真实负载情况来进行调度决策，需要通过 Prometheus 等监控组件获取系统 Node 真实负载信息。在使用 DeScheduler 组件之前，您可以采用自建 Prometheus 监控或采用 TKE 云原生监控，以下为您详细介绍：
...
> ! 请确保您已经按照【[依赖部署](#DeScheduler)】>【[Prometheus 规则配置](#Prometheus1)】步骤进行监控数据采集规则配置，组件才可以拉取到需要的监控数据，调度策略才会生效。
...
4. 驱逐太多 Pod，导致服务不可用。
   Kubernetes 原生提供 PDB 对象用于防止驱逐接口造成 workload 不可用 Pod 过多，但需要用户创建该 PDB 配置。容器服务 TKE 自研的 DeScheduler 组件加入了兜底措施，即调用驱逐接口前，判断 workload 准备 的 Pod 数是否大于副本数一半，否则不调用驱逐接口。

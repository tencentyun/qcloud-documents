...
在需要被迁移的集群和目标集群上都安装 Velero 实例，并且两个集群的 Velero 实例指向相同的腾讯云 [对象存储 COS](https://cloud.tencent.com/document/product/436) 位置，迁移流程如下：
...
您可以手动执行备份操作，也可以为 Velero 设置定期自动备份，设置方法可以使用命令 `velero schedule -h`  查看。


本示例将以 default 、default2 命名空间的资源情况作比较验证，执行以下命令，可以查看集群 A 中两个命名空间下的 Pods 和 PVC 资源情况。如下图所示：
...
其中，集群 A 中的 minio 对象存储服务使用了持久卷，并且已经上传一些图片数据，如下图所示：

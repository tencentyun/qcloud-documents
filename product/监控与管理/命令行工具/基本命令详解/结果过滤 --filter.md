部分命令行返回结果会比较大，比如返回所有实例列表，而有的返回结果会比较大同时结构还比较复杂，这时候可能不是所有的信息都是你想获取的关键信息，此时可以使用基本命令中的 filter 来过滤掉非关键信息。

下面以 DescribeAvailabilityZones 为例来说明如何使用过滤功能,
配置里设置的是华南地区（广州），API 能力是查看地域下所有可用区。

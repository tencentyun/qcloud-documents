...
| api              | 可选，object，默认为{}。相关的配置： <br> retCodeHandler: Function(data: String, url: String, xhr: Object):{isErr: boolean, code: string}， 返回码上报钩子函数。 会传入接口返回数据，请求 URL 和 XHR 对象。<br> resourceTypeHandler: Function，请求资源类型修正钩子函数 会传入接口 URL，返回值为 <code>static</code> 或 <code>fetch</code>。详情请参见 <span id="jump">[api.retCodeHandler](#exp1)</span>。 |

...
目前RUM提供的 aegis 实例方法如下：  

| 参数 | 用途 |
|---------|---------|
| setConfig | 传入配置对象，包括用户 ID 和 UIN 等信息|
|info|主要上报字段，用于上报白名单日志。<br>下列两种情况日志才会报到后台：1. 打开页面的用户在名单中；2. 对应的页面发生了错误|
|infoAll|主要上报字段，用于上报白名单日志。该上报与 info 唯一的区别： info指定用户上报，|infoAll 所有用户都会上报|
...
2. 通常情况下，我们并不能一开始就获取到用户的 `uin`。若在获取 UIN 的这段时间不进行实例化，这期间发生的错误前端性能监控将无法监听。针对这种情况，我们可以先传入id 进行实例化，引用 setConfig 传入 UIN ，示例如下：

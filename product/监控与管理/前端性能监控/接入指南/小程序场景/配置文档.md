...
| setDataReportConfig | 可选，object，默认为{}。相关的配置：<br> <li>disabled：可选，Boolean，默认 false。是否禁用 setData 数据上报； <br><li> timeThreshold：可选，Number，单位为 ms，默认值为30。上报的耗时阈值，表示仅上报更新耗时超过该阈值的数据；<br> <li>withDataPaths：可选，Boolean，默认为 true。是否上报本次更新的字段信息。|

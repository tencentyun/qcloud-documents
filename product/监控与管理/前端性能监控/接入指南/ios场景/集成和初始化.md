...
1. 把 QAPM.framework 文件放到您的 Xcode 应用内，并在弹框中勾选 Copy items if needed 选项。
2. 选择 **Targets**>**Build Phases** > **Link Binary With Libraries**，添加下列 Xcode 自带的公共包文件：
  -  libc++.dylib（libc++.tbd）
  -  libc.dylib（libz.tbd）
  -  CoreLocation.framework

3. 在应用中的 **Target** >**Build Settings -> other Linker Flags linking** >**Other Linker Flags** 中添加 `-ObjC` 参数。
...
#import <QAPM/QAPM.h> 
```
>?如果是 Swift 应用，请在  `bridging-header.h` 文件中引入 。

...
- 使用 [腾讯云云服务器](https://cloud.tencent.com/product/cvm) 或 [腾讯云云函数](https://cloud.tencent.com/product/scf) 进行数据上报。
...
2. 获取上报 Demo
   ...
   > - Metrics.N ：一组指标和数据。
   > - AnnouceInstance、AnnounceIp：两个参数均可用于上报主机 IP 或实例名，建议使用 AnnouceInstance，AnnounceIp 参数正灰度下线中。如使用CVM或SCF上报数据请参考 [输入参数说明](#.E8.BE.93.E5.85.A5.E5.8F.82.E6.95.B0) 填写此参数。
...
| AnnounceInstance  | 否   | String               | 用于上报实例ID或 IP，最大长度128字节。</br>云服务器请上报其主机IP地址；云函数请以`命名空间\|函数名`的格式上报实例ID，如`Namespace\|MyFunction` |

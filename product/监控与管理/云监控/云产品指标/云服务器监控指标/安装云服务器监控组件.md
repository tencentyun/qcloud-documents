...
## Windows 安装指引
1. 用户 [登录 Windows 实例](https://cloud.tencent.com/document/product/213/5435) 后，在浏览器粘贴下载地址 `http://update2.agent.tencentyun.com/update/windows-stargate-installer.exe`，下载安装包 `windows-stargate-installer.exe`。
2. 运行该程序进行自动化安装。
安装成功时如下图所示：

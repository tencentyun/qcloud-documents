...
| Instances.N.Dimensions.0.Value | clusterId   | 具体的集群 ID，[查询集群列表](https://cloud.tencent.com/document/product/457/31862) 接口中返回的clusterId（集群的ID）字段 | 输入集群具体 ID，例如：cls-xxxxx               |
| Instances.N.Dimensions.1.Name  | serviceName | 服务的维度名称                                               | 输入String类型维度名称：serviceName            |
...
| Instances.N.Dimensions.4.Name  | containerId |                                                              | 输入String类型维度名称：containerId            |
...
&Instances.0.Dimensions.4.Value 为容器ID
...
| 参数名称                       | 维度名称    | 维度解释                                                     | 格式                                           |
| ------------------------------ | ----------- | ------------------------------------------------------------ | ---------------------------------------------- |
| Instances.N.Dimensions.0.Name  | clusterId   | 集群 ID的维度名称                                            | 输入String类型维度名称：clusterId              |
| Instances.N.Dimensions.0.Value | clusterId   | 具体的集群 ID，[查询集群列表](https://cloud.tencent.com/document/product/457/31862) 接口中返回的 clusterId（集群的ID）字段 | 输入集群具体ID，例如 ：disk-test               |
| Instances.N.Dimensions.1.Name  | serviceName | 服务维度名称                                                 | 输入String类型维度名称：serviceName            |
| Instances.N.Dimensions.1.Value | serviceName | 具体的服务名称，[查询服务列表](https://cloud.tencent.com/document/product/457/9440) 接口中返回的serviceName（服务名）字段 | 输入具体服务名称，例如 ：test                  |
| Instances.N.Dimensions.2.Name  | namespace   | 命名空间的维度名称                                           | 输入String类型维度名称：namespace              |
...
| Instances.N.Dimensions.3.Name  | podName     | 实例的维度名称                                               | 输入String类型维度名称：podName                |
...
查询容器服务实例维度监控数据，入参取值例如下：<br>

&Namespace=QCE/DOCKER<br>
&Instances.N.Dimensions.0.Name=clusterId<br>
&Instances.N.Dimensions.0.Value 为集群 ID<br>&Instances.0.Dimensions.1.Name=namespace

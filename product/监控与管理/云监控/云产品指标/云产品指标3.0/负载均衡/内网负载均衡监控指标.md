...
| OutTraffic          | LB 到后端的出带宽          | 在统计粒度内，负载均衡访问后端 RS 所用的带宽。               | Mbps    | 60、300     |
| InTraffic           | LB 到后端的入带宽          | 在统计粒度内，后端 RS 访问负载均衡所用的带宽。               | Mbps    | 60、300     |
| OutPkg              | LB 到后端的出包量          | 在统计粒度内，负载均衡向后端 RS 每秒发送的数据包数量。       | 个/秒   | 60、300     |
| InPkg               | LB 到后端的入包量          | 在统计粒度内，后端 RS 向负载均衡每秒发送的数据包数量。       | 个/秒   | 60、300     |

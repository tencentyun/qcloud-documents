...
| OutTraffic          | LB 到后端的出带宽          | 在统计粒度内，负载均衡访问后端 RS 所用的带宽。               | Mbps    | 10、60、300、3600 |
| OutPkg              | LB 到后端的出包量          | 在统计粒度内，负载均衡向后端 RS 每秒发送的数据包数量。       | 个/秒   | 10、60、300、3600 |
| InPkg               | LB 到后端的入包量          | 在统计粒度内，后端 RS 向负载均衡每秒发送的数据包数量。       | 个/秒   | 10、60、300、3600 |
| ConNum              | LB 到后端的连接数          | 在统计粒度内，从负载均衡到后端 RS 的连接数。                 | 个      | 10、60、300、3600 |
| NewConn             | LB 到后端的新建连接数      | 在统计粒度内，从负载均衡到后端 RS 的新建连接数。             | 个/分钟 | 10、60、300、3600 |
| InTraffic           | LB 到后端的入带宽          | 在统计粒度内，后端 RS 访问负载均衡所用的带宽。               | Mbps    | 10、60、300、3600 |
| AccOuttraffic       | LB 到后端的出流量          | 在统计粒度内，负载均衡访问后端 RS 的流量。                   | MB      | 10、60、300、3600 |
...
| ConnRatio           | SNAT 端口使用率            | 在统计周期内，负载均衡的 SNAT IP 的端口使用率。 <br/>端口使用率 = SNAT 并发连接数<br/> 此指标处于内测阶段，如需使用，请提交 [内测申请](https://cloud.tencent.com/apply/p/z2b346gxli)。 / (SNAT IP 数 × 55000 × RS 个数）。 | %       | 60、300           |
| SnatFail            | SNAT 失败数                | 在统计周期内，负载均衡的 SNAT IP 与后端 RS 每分钟建立连接的失败次数。 <br/> 此指标处于内测阶段，如需使用，请提交 [内测申请](https://cloud.tencent.com/apply/p/z2b346gxli)。| 个/分钟 | 60、300           |

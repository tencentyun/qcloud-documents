...
<ul style="margin:0;list-style-type:disc;"><li>本文档仅包含负载均衡 CLB 上七层监听器（HTTP/HTTPS 协议）的指标，不包含四层监听器（TCP、UDP、TCP SSL、QUIC 等协议）的指标。</li><li>此命名空间的指标统计的是负载均衡访问后端服务的数据。</li></ul>
</dx-alert>
...
| ConNum              | 活跃连接数          | 在统计粒度内，负载均衡或监听器上的连接数                     | 个      | 60、300、3600、86400 |

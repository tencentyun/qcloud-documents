...
此命名空间中的四层监听器指标数据为估算值，请以 [LB_PUBLIC](https://cloud.tencent.com/document/product/248/51898) 命名空间中的指标数据为准。
</dx-alert>
...
| ConNum              | 活跃连接数          | 在统计粒度内，负载均衡或监听器上的连接数。此指标为四层监听器指标，即将下线                     | 个      | 60、300、3600、86400 |
...
| InPkg               | 入包量              | 在统计粒度内，负载均衡每秒接到的请求数据包数量。此指标为四层监听器指标，即将下线               | 个/秒   | 60、300、3600、86400 |
| InTraffic           | 入带宽              | 在统计粒度内，客户端通过外网访问负载均衡所用的带宽。此指标为四层监听器指标，即将下线           | Mbps    | 60、300、3600、86400 |
| NewConn             | 新建连接数          | 在统计粒度内，负载均衡或监听器上新建连接数。此指标为四层监听器指标，即将下线                   | 个/分钟 | 60、300、3600、86400 |
| OutPkg              | 出包量              | 在统计粒度内，负载均衡每秒发出的数据包数量。此指标为四层监听器指标，即将下线                   | 个/秒   | 60、300、3600、86400 |
| OutTraffic          | 出带宽              | 在统计粒度内，负载均衡访问外网所用的带宽。此指标为四层监听器指标，即将下线                     | Mbps    | 60、300、3600、86400 |

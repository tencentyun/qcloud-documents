...
| CpuLoadavg  | CPU 一分钟平均负载 |15分钟内 CPU 平均负载，取`/proc/loadavg`第三列数据，依赖监控组件安装采集 。Windows 机器无此指标 | -    |InstanceId|
|  Cpuloadavg5m | CPU 五分钟平均负载 |1分钟内 CPU 平均负载，取`/proc/loadavg`第一列数据，依赖监控组件安装采集 。Windows 机器无此指标 | -    |InstanceId|
|  Cpuloadavg15m | CPU 十五分钟平均负载 | 5分钟内 CPU 平均负载，取`/proc/loadavg`第二列数据，依赖监控组件安装采集。Windows 机器无此指标  |   -   |InstanceId|

...
| SubMsgBacklog                  | 所选时间范围内，已经生产到TDMQ，但并未被消费的消息数量。消息堆积不宜太多，如有明显增长趋势，请对消费者服务进行扩容以减少堆积量。 | 条    | environmentId、subName、tenantId、topicName | 60s、300s、3600s |
...
| SubUnackedMsg                  | 在所选时间范围内，已发送给消费者消息但是没有接收到确认信息回传的消息数量。如果存在很多这种消息，请检查您的消费者服务是否正常，以及是否使用官方SDK进行消费。 | 条    | environmentId、subName、tenantId、topicName | 60s、300s、3600s |
...
| MsgRateIn               | 消息生产速率                                                 | 条/秒 | environmentId、tenantId、topicName  | 60s、300s、3600s |
| MsgThroughputIn         | 消息生产流量                                                 | B/S   | environmentId、tenantId、topicName  | 60s、300s、3600s |
| InMessagesTotal         | 当前Topic生产消息总数，该指标在发生服务端重启或者切换时会归零 | 条    | environmentId、tenantId、topicName  | 60s、300s、3600s |
| StorageSize             | 积压消息大小                                                 | B     | environmentId、tenantId、topicName  | 60s、300s、3600s |
| TenantInMessagesTotal   | 虚拟集群入消息总数                                           | 条    | tenant                              | 60s、300s、3600s |
| TenantMsgAverageSize    | 租户级别消息平均大小                                         | B     | tenant                              | 60s、300s、3600s |
| TenantRateIn            | 租户级别消息生产速率                                         | 条    | tenant                              | 60s、300s、3600s |
...
| ------------------ | ------------ | ---- | ---------------------------------- | ---------------- |
| CmqQueueMsgBacklog | 消息堆积数量 | 条   | environmentId、tenantId、topicName | 60s、300s、3600s |
| CmqTopicMsgBacklog | 消息堆积数量 | 条   | environmentId、tenantId、topicName | 60s、300s、3600s |
...
| Instances.N.Dimensions.0.Value | environmentId | 具体环境名称               | 输入具体环境名称，例如：default          |
...
| Instances.N.Dimensions.0.Value | subName       | 具体订阅名称               | 输入具体订阅名称，例如：test             |
...
&Instances.N.Dimensions.0.Name = environmentId
&Instances.N.Dimensions.0.Value = 具体环境名称
&Instances.N.Dimensions.1.Name = subName
&Instances.N.Dimensions.1.Value = 具体订阅名称
&Instances.N.Dimensions.2.Name = tenantId 
&Instances.N.Dimensions.2.Value = 具体集群 ID 
&Instances.N.Dimensions.3.Name = topicName 
&Instances.N.Dimensions.3.Value = 具体主题名称

...
| DiskReadTraffic  | 硬盘读流量       | 数据从块存储读取到内存中的速率                               | KB/s | InstanceId  |
| DiskWriteIops    | 硬盘写 IOPS      | 每秒从内存写到块存储中的 IO 次数                             | 次数 | diskId 、InstanceId |
| DiskWriteTraffic | 硬盘写流量       | 数据从内存写入到块存储中的速率                               | KB/s | InstanceId |
| DiskAwait        | 硬盘 IO 等待时间 | 在采样周期内有百分之几的时间 CPU 空闲并且有仍未完成的 I/O 请求 | ms   | InstanceId  |

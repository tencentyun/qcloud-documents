...
| CPU使用率      | CpuUs           | CPU处于非空闲状态的百分比，取 /proc/stat数据计算得出        | 每分钟采集，5分钟粒度数据是按最近5分钟内求平均值 | %     |redis_uuid|
...
| 参数名称               | 维度名称             | 维度解释          | 格式                            |
| ------------------ | ---------------- | ------------- | ----------------------------- |
| Instances.N.Dimensions.0.Name  | redis_uuid              | 实例 ID的维度名称 | 输入String类型维度名称：redis_uuid |
| Instances.N.Dimensions.0.Value | redis_uuid              | 实例具体ID   | 输入具体实例 ID，例如：crs-123456 |

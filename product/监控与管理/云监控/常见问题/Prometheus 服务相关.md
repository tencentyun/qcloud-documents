...
TKE 中 kube-proxy 未指定启动参数 `--metrics-bind-address`，而 metrics 服务默认监听地址为127.0.0.1，这样 Agent 无法根据 POD IP 拉取到 metrics，可通过如下步骤指引进行解决：
1. 登录 [容器服务控制台](https://console.cloud.tencent.com/tke2/cluster?rid=1)，选择对应地域下的容器集群。
...
2. 在安全组搜索框中输入tke-master-security-for-<tke cluster id>，比如集群 ID 是 cls-xxx 那么搜索内容为tke-master-security-for-cls-xxx

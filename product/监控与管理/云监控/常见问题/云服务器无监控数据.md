...
2. 安装监控组件。若提示 OK，则说明安装成功。
   ...
   ```
3. 执行以下命令，脚本将开始检查并修复（检查系统、网络等；提示 OK 则说明检查正常或修复成功）。若问题仍未解决，请 [提交工单](https://console.cloud.tencent.com/workorder/category) 将工具输出的文件 **monitor_agent_admin-\*.zip** 提交给我们，将有专人联系您一同处理。
   ```shell

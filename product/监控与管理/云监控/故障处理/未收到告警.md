...
- 如上图策略1，告警启停按钮为灰色则说明未开启告警策略，单击灰色按>确认开启即可。
- 如上图策略2，告警启停按钮为蓝色则说明告警策略已开启，请按照其它步骤继续排查。
...
如上图显示 ”剩余 0 条可用“ ，则说明 “基础告警” 短信已使用完。每个告警类型、每用户、每月为1000条免费配额，若超过额度需继续使用短信告警，请点击上图【购买短信】链接进行短信增量。
>?如需了解短信收费详细说明，请参考 [购买指南-短信配额](https://cloud.tencent.com/document/product/248/18728#.E7.9F.AD.E4.BF.A1.E9.85.8D.E9.A2.9D) 。
...
3. 查看告警接收对象是否包含未收到告警的用户。若出现“未配置”或没有此用户，请参考  [创建告警接收组]( https://cloud.tencent.com/document/product/248/6217 ) 添加用户到告警接收组。
...
![](https://main.qcloudimg.com/raw/b944265bc92b5a717ce8a38eb1a26b9d.jpg)

若出现“关注公众号”按钮则说明已取消关注。请点击重新关注公众号，才能正常接收微信告警通知。
![](https://main.qcloudimg.com/raw/67ae69d114bda813d67fc1b20034afcb.png)
...
例如：设置指标为 CPU 利用率 、比较关系为 > 、阈值为80% 、统计周期为5分钟 、持续周期为2个周期。 表示：每5分钟收集一次 CPU 利用率数据，若某台云服务器的 CPU 利用率**连续三次**大于80%则触发告警。 以此类推，设置持续周期为N，则需要指标监控数据N+1次达到阈值才会产生告警。

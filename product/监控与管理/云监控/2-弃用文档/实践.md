...
在 [Nginx Ingress on TKE 部署最佳实践](https://mp.weixin.qq.com/s/NAwz4dlsPuJnqfWYBHkfGg) 一文中介绍来 Nginx Ingress 在 TKE 上部署最佳实践，涉及的部署 YAML 已经包含部分性能方面的参数优化。
本文将继续展开介绍 Nginx Ingress 性能调优的方法及其原理的解释，包括内核参数与 Nginx 本身的配置调优，以让 Nginx Ingress 更好的适配高并发的业务场景。
...
在高并发环境下，建议执行以下命令，将 Nginx Ingress 设为65535：
```plaintext
...
Nginx 全局配置通过 configmap 配置（Nginx Ingress Controller 会读取并自动加载配置）：

通过接口回调，您的系统可以直接收到腾讯云的告警通知。接口回调具备将告警信息通过 HTTP 的 POST 请求推送到可访问公网 URL 的功能，您可基于接口回调推送的告警信息做进一步的处理。

## 接口回调使用方法

- 接口回调：用户需要提供能接收 HTTP POST 请求的，公网可访问的 URL 地址，作为回调地址。
- 回调触发：触发逻辑与告警短信、邮件一致，当用户创建的告警策略被触发、告警策略恢复时，均会通过接口回调发送告警消息。接口回调也支持重复告警。
- 绑定接口回调：用户可在 [创建告警策略](https://cloud.tencent.com/document/product/248/6215) 第8步中配置接口回调；也可在告警策略详情页内添加接口回调。一个告警策略组只可绑定一个告警回调 URL。
- 返回内容：向用户绑定的 URL 发出告警信息后，我们需要接收到以下的返回内容，以表明用户已成功接收信息；否则我们将重复发送告警信息，最多发送三次。
  sessionId，用于鉴别回调请求。了解更多 sessionId，可参见 [网络基础 HTTP 会话（session）详解](https://cloud.tencent.com/developer/article/1505726)。
...
接口回调通过 HTTP 的 POST 请求发送 JSON 格式的数据，参数如下：

### 指标告警

#### 指标告警参数示例

>?告警接口回调字段数据类型为 String。

...
- 所有的监控图表都需创建在监控面板之上，故可以将监控面板理解为监控图表的集合。
...
通过在监控面板中创建不同的监控图表，可灵活满足用户在不同监控场景查看资源性能状况的需求。不同的图表类别、图表联动、图表拖拽放大等功能特性，为用户查看监控数据带来更多便利。

### 图表种类

Dashboard 目前提供了折线图、柱状图两种监控图表供用户选择。

- 折线图：最常见、最通用的图表，能满足绝大部分时序性监控数据的查阅需求。
...
3. 完成后单击【确定】，即完成图表创建。
 - 若面板中无图表，新建图表将默认由左上至右下的次序依次建立在面板中。
 - 若面板中已有图表，新建的图表将默认由左上至右下的次序跟随在面板最下方图表后。

>! 
>- 在一次新建图表过程中，支持批量创建同一批对象多个指标的多个图表，为用户免去重复选择监控对象的繁琐流程。
>- 若用户选择的实例数量超出单张曲线图容纳上限，将默认按对象列表顺序批量创建图表，避免用户多次重复新建图表配置。
>- 实例列表支持资源列表的筛选、搜索、一键全选以及按 shift 多选功能。友好的批量操作为用户大批量选取资源带来便利，提升配置效率。

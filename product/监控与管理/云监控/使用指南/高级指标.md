...
>- 可配置指标集的产品：云数据库 MySQL、云托管 CloudBase Run、消息队列 TDMQ、云直播。其它云产品陆续接入中。
...
| 指标英文名          | 指标中文名                | 指标说明                    | 单位 | 维度                          | 统计粒度                 |
| ------------------- | ------------------------- | --------------------------- | ---- | ----------------------------- | ------------------------ |
| CmqBatchonsumeCost  | CMQ 每分钟批量消费平均耗时 | CMQ 每分钟批量消费请求总耗时 | ms   | appid,resourceid,resourcename | 60s、300s、3600s、86400s |
| CmqBatcheleteCost   | CMQ 每分钟批量删除平均耗时 | CMQ 每分钟批量删除请求总耗时 | ms   | appid,resourceid,resourcename | 60s、300s、3600s、86400s |
| CmqBatchProduceCost | CMQ 每分钟批量生产平均耗时 | CMQ 每分钟批量生产请求总耗时 | ms   | appid,resourceid,resourcename | 62s、300s、3600s、86400s |
...
| CmqConsumeCost      | CMQ 每分钟消费平均耗时     | CMQ 每分钟消费请求总耗时     | ms   | appid,resourceid,resourcename | 60s、300s、3600s、86400s |
| CmqDeleteCost       | CMQ 每分钟删除平均耗时     | CMQ 每分钟删除请求总耗时     | ms   | appid,resourceid,resourcename | 60s、300s、3600s、86400s |
| CmqProduceCost      | CMQ 每分钟生产平均耗时     | CMQ 每分钟生产请求总耗时     | ms   | appid,resourceid,resourcename | 60s、300s、3600s、86400s |

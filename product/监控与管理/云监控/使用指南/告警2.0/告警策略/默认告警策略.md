 ...
 [](id:step1)
  ...
  <table>
  	...
  	<tr>
  		...
  		<td>CPU 利用率</td>
  		<td>统计周期为1分钟；阈值为>95%；持续监控数据点为5个监控数据点</td>
  	</tr>
  	<tr>
  		<td>内存利用率</td>
  		<td>统计周期为1分钟；阈值为>95%；持续监控数据点为5个监控数据点</td>	
  	...
  	<tr>
  		...
  		<td>磁盘利用率</td>
  		<td>统计周期为1分钟；阈值为>80%；持续监控数据点为5个监控数据点</td>
  	</tr>
  	<tr>
  		<td>CPU 利用率</td>
  		<td>统计周期为1分钟；阈值为>80%；持续监控数据点为5个监控数据点</td>
      ...
      <tr>
          ...
          <td>指标告警</td>
      	<td>磁盘使用百分比</td>
      	<td>统计周期为1分钟；阈值为>85%；持续监控数据点为5个监控数据点</td>
      </tr>
      <tr>
     ...
     <tr>
     	    <td>平均 CPU 使用率</td>
     	    <td>统计周期为1分钟；阈值为>90%；持续监控数据点为5个监控数据点</td>
     </tr>
     <tr>
     	    <td>平均 JVM 内存使用率</td>
     	    <td>统计周期为1分钟；阈值为>85%；持续监控数据点为5个监控数据点</td>
     ...
     <tr>
     	    <td>集群健康状态</td>
     	    <td>统计周期为1分钟；阈值为>=1；持续监控数据点为5个监控数据点</td>
     </tr>
  </table>

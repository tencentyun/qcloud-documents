...
> ? 
> - 如需查看指标名称，请参考 [云产品监控指标](https://cloud.tencent.com/document/product/248/6140)。
> - 如需获取产品策略类型和命名空间，请参考 [云产品策略类型和命名空间](https://cloud.tencent.com/document/product/248/50397)。

#### 指标告警 dimensions 示例

<dx-fold-block title="云服务器 CVM-基础监控">
```
"dimensions": {
...
<dx-fold-block title="云服务器 CVM-存储监控">
```
"dimensions": {
...
<dx-fold-block title="云数据库 MySQL">
```
"dimensions": {
...
<dx-fold-block title="云数据库 Redis（1分钟）">
```
"dimensions": {
...
<dx-fold-block title="云数据库 Redis（5s-redis节点）">
```
"dimensions": {
...
<dx-fold-block title="云数据库 Redis（5s-实例汇总）">
```
"dimensions": {
		"AppId": "1252068000",    //账户APPID
		"InstanceId":"crs-1amp2588",  // 云数据库 Redis 实例 ID
...
<dx-fold-block title="云数据库 Redis（5s-proxy节点）">
```
"dimensions": {
		"appid": "1252068037",    //账户APPID
...
<dx-fold-block title="负载均衡 -外网监听器">
```
"dimensions": {
		"protocol": "https",   //监听协议
...
<dx-fold-block title="负载均衡 -内网监听器">
```
"dimensions": {
		"protocol": "https",      //监听协议
		"vip": "14.22.4.26",    // 负载均衡 VIP
...
<dx-fold-block title="负载均衡 -服务器端口(传统型内网)">
```
"dimensions": {
	   "protocol": "https",  //监听协议
...
<dx-fold-block title="云数据库 SQL  Server">
```
"dimensions": {
...
<dx-fold-block title="云数据库 MongoDB">
```
"dimensions": {
...
<dx-fold-block title="云数据库 PostgreSQL">
```
dimensions":{
     "uid":"2123"
...
<dx-fold-block title="云数据库 CYNOSDB_MYSQL">
```
dimensions":{
      "appid":"1256754779",
...
<dx-fold-block title="云数据库 TcaplusDB">
```
"dimensions": {
     "ClusterId":"xxx",
     "TableInstanceId":"xxx",
...
<dx-fold-block title="云数据库 TDSQL MySQL">
```
"dimensions": {
     "cluster_name":"xxx",
      "is_master":"xxx",
...
<dx-fold-block title="私有网络-NAT 网关">
```
"dimensions": {
...
<dx-fold-block title="私有网络-VPN 网关">
```
"dimensions": {
    "appid": "12345",
    "vip": "10.0.0.0",
...
<dx-fold-block title="私有网络-VPN 通道">
```
"dimensions": {
...
<dx-fold-block title="私有网络-共享带宽包">
```
"dimensions": {
    "__region__": "xxx",
    "appid": 12345,
...
<dx-fold-block title="内容分发网络 CDN">
```
"dimensions":{
     "appid":"1257137149",
...
<dx-fold-block title="消息队列 Ckafka-Topic">
```
"dimensions":{
      "appid":"1258399706",
...
<dx-fold-block title="消息队列 Ckafka-实例">
```
"dimensions":{
     "appid":"1255817890",
...
<dx-fold-block title="消息队列 Ckafka-ConsumerGroup-Topic">
```
"dimensions":{
     "appid":"1258344866",
...
<dx-fold-block title="消息队列 Ckafka-ConsumerGroup-Partition">
```
"dimensions":{
    "appid":"1258344866",
    "consumer_group":"eslog-group22",
...
<dx-fold-block title="负载均衡-VIP 封堵">
```
"dimensions":{

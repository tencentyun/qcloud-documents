...
如下案例中，策略的含义是允许访问 UIN 为1250000000下面的实例 ID 为 prom-73jingds 的资源，以及绑定了标签键为 testkey，标签值为 testvalue 的资源。
...
```
{
	...
	"statement": [{
		"effect": "allow",
		"action": [
			"name/monitor:DescribeRecordingRules"
		],
		"resource": [
			"qcs::monitor:ap-guangzhou:uin/1250000000:prom-instance/prom-73jingds",
...
```
{
	...
	"statement": [{
		...
		"action": [
			"name/monitorap-guangzhou:TerminatePrometheusInstances"
		]
	}]
}
```

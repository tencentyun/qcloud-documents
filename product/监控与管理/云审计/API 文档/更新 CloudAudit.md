...
## 接口描述
UpdateAudit 用于更新云审计跟踪集，其中可用于更新指定日志文件中的一些设置，比如路径，COS Bucket 等设置，请注意跟踪集的名称是不能修改的。对路径的更改，不需要停止跟踪集服务。对 COS Bucket 的更改，如果该 COS Bucket 之前是跟踪集日志文件的目标，则可以更改成功。否则需要手动授权 COS 存储桶，使得跟踪集具有写权限才行。

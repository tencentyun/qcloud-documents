...
In many scenarios, conditions need to be specified for a created policies to take effect.
Scenario 1: If the access source of a CAM user calling the cloud API needs to be restricted, the IP condition must be added into the existing policy.
...
1. In the following example, a user can only call the cloud API within IP address range `10.217.182.3/24` or `111.21.33.72/24`.
```
{
  "version": "2.0",
  "statement": {
...
```
2. In the following example, the region of VPC must be Shanghai in order for it to be bound to a specified NAT gateway.
```
{
  "version": "2.0",
  "statement": {

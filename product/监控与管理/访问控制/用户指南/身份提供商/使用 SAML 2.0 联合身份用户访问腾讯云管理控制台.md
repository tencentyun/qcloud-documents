...
通过浏览器访问身份提供商的门户网站，并选择跳转到腾讯云管理控制台。
该门户网站可以验证当前用户的身份。
验证成功后，该门户网站会生成一个 SAML 2.0 身份验证响应，其中包括识别用户身份的断言以及用户的相关属性。该门户网站将此响应发送到客户端浏览器。
该客户端浏览器将被重定向到腾讯云单一登录终端节点并发布 SAML 断言。
终端节点将代表用户请求临时安全凭证，并创建一个使用这些凭证的控制台登录 URL。
腾讯云将登录 URL 作为重定向发回客户端。
该客户端浏览器将重定向到腾讯云管理控制台。如果 SAML 2.0 身份验证响应包含映射到多个 CAM 角色的属性，则系统将首先提示用户选择要用于访问控制台的角色。

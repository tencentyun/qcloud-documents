...
## 操作步骤
1. 用户使用前提条件 2 中获取到的访问密钥调用 [AssumeRole](https://cloud.tencent.com/document/product/598/13895) 接口，申请前提条件 1 中预先被创建好的角色 MyRole 的临时密钥。
2. 用户调用 [AssumeRole](https://cloud.tencent.com/document/product/598/13895) 接口成功后，获取到角色 MyRole 的临时密钥。
...
$secretId  = <span class="hljs-string">"AKI***"</span>;       <span class="hljs-comment">//STS返回的临时AK</span>
$secretKey = <span class="hljs-string">"Gu5***PLE"</span>;           <span class="hljs-comment">//STS返回的临时Secret</span>
$token     = <span class="hljs-string">"ADE***fds"</span>; <span class="hljs-comment">//STS返回的安全TOKEN</span>
$param[<span class="hljs-string">"nonce"</span>]     = <span class="hljs-number">11886</span>;      <span class="hljs-comment">//rand();</span>
$param[<span class="hljs-string">"timestamp"</span>] = <span class="hljs-number">1465185768</span>; <span class="hljs-comment">//time();</span>
$param[<span class="hljs-string">"secretId"</span>]  = $secretId;
$param[<span class="hljs-string">"token"</span>]     = $token;
$param[<span class="hljs-string">"action"</span>]    = <span class="hljs-string">"roleLogin"</span>;
ksort($param);
$signStr = <span class="hljs-string">"GETcloud.tencent.com/login/roleAccessCallback?"</span>;
<span class="hljs-keyword">foreach</span> ( $param <span class="hljs-keyword">as</span> $key =&gt; $value ) {
    $signStr = $signStr . $key . <span class="hljs-string">"="</span> . $value . <span class="hljs-string">"&amp;"</span>;
}
$signStr = substr($signStr, <span class="hljs-number">0</span>, <span class="hljs-number">-1</span>);
$signature = base64_encode(hash_hmac(<span class="hljs-string">"sha1"</span>, $signStr, $secretKey, <span class="hljs-keyword">true</span>));
<span class="hljs-keyword">echo</span> $signature.PHP_EOL;
<span class="hljs-meta">?&gt;</span></span></code></pre>
4. 用户拼接完整登录信息以及目的页地址进行登录。

...
公司 A 企业帐号 CompanyExampleA（ownerUin 为 12345），创建⼀个角色并将信任实体设置为公司 B 的企业账号 CompanyExampleB（ownerUin 为 67890）。公司 A（CompanyExampleA）调⽤ CreateRole 接⼝创建⼀个角色名称（roleName）为 DevOpsRole 的角色，公司 A 企业帐号 CompanyExampleA 为创建的角色附加了权限。上述步骤请参阅 [通过 API 创建]()。
企业账号 CompanyExampleB 被授权这个角色后，希望由子帐号 DevB 来完成这项⼯作。公司 B（CompanyExampleB）授权 DevB 可以申请扮演 CompanyExampleA 的角色 DevOpsRole。上述步骤请参阅 [为子账号赋予可扮演角色的策略]() 。

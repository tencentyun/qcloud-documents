在通过 CAM 控制台的 [策略生成器创建策略](https://cloud.tencent.com/document/product/598/37739#.E6.8C.89.E7.AD.96.E7.95.A5.E7.94.9F.E6.88.90.E5.99.A8.E5.88.9B.E5.BB.BA) 时，我们可以根据需要设置策略生效条件。
![](https://qcloudimg.tencent-cloud.cn/raw/c210fbd7ca322fbd3fed2f13a34b5dc7.png)


## 键(key)

腾讯云通用条件键(key) 命名格式：`qcs:<condition-key>`，当前只支持5种条件键，条件键的内容及描述如下：
...
在生效条件(Condition) 中使用条件运算符来将策略中的条件键和值与请求上下文中的值进行匹配。
...
## 映射关系
在生效语句中，可以使用的条件(Condition)取决于选择的键(Key)，键(Key)与条件(Condition)的映射关系如下：

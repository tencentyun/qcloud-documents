...
### 2. 委托人（principal）
描述策略授权的实体。包括用户（主账号、子账号、匿名用户）、用户组，未来会包括角色、联合身份用户等更多实体。仅支持在策略管理 API 中策略语法相关的参数中使用该元素。
...
### 8. 策略样例
该样例描述为：允许属于主账号 ID 1238423下的子账号 ID 3232523以及组 ID 18825， 对北京地域的 cos 存储桶 bucketA 和广州地域的 cos 存储桶 bucketB 下的对象 object2，在访问 IP 为10.121.2.\*网段时，拥有所有 cos 读 API 的权限以及写对象的权限，以及可以发送消息队列的权限。
```
{	 
		"version":"2.0", 

...
> - 请检查tapm.properties中的license_key是否与您创建应用中的看到的License Key一致。
...
3. 修改放在服务器解压的 tapm 目录下 tapm.properties 文件。
   ...
   修改文件中 的license_key、app_name和collector.addresses 配置项，否则探针无法进行数据采集也无法启动探针。对于其他配置项，请根据实际需要进行配置。配置说明如下：
...
4.  登录应用性能监控 TAPM 控制台查看性能数据。


重启5分钟后，当您的Java应用服务有 HTTP 请求进入，性能数据将发送到应用性能监控 TAPM 系统。
...
配置 JAVA_OPTS，在`-javaagent`后加以下三个参数，中间以空格分隔：

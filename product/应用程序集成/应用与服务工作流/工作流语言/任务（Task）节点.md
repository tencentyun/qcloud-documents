...
除了 [常见字段](https://github.com/tencentyun/qcloud-documents/blob/master/product/应用程序集成/应用与服务工作流/工作流语言.md#step3) 之外，Task 节点还包含以下字段：

| 字段                    | 描述                                                         |
| ----------------------- | ------------------------------------------------------------ |
| Resource（必选）        | 定义节点执行的任务资源。目前支持腾讯云API所有服务。详情参考资源定义。 |
...
ASW 通过 Task 节点调用云API 来支持 [全量云 API](https://cloud.tencent.com/api/list) 的服务集成，因此Task 节点支持腾讯云API 包含的所有服务。通过 Resource 与 Parameters 来映射云API 调用所需要的参数字段。规则如下：

1、Resource定义了要调用的服务接口：

```
qrn:qcs:asw:{服务所在区域}:{Appid}:sdk:json:qcloud:{服务名称}:{服务接口}
```

2、Parameters定义的是服务接口对应的参数名称。
...
以下代码定义了 Sum 节点，Sum 节点中调用了sum 函数。由于版本别名和命名空间均为默认值，省略不写如下：
...
以下代码定义了 Mps节点，Mps节点中调用了 [**ProcessMedia 接口**](https://cloud.tencent.com/document/product/862/37578)。Parameters 定义了 ProcessMedia 接口需要的参数。如下：
...
当使用 Task节点调用云服务时，您可以选择以下服务集成模式：

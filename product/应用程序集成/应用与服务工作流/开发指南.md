...
腾讯云状态语言（Tencent Cloud States Language），简称 CSL，是基于 JSON 的结构化语言，用于定义状态机。在本节中，您将学习 TCSL 的基本编写语法，以及如何使用 TCSL 实现状态机的各种功能。
...
Task 节点代表状态机执行的一个工作单元。您状态机中的所有工作由 Task 完成。Task 通过将参数传递指定服务的 API 操作来执行工作。目前只支持云函数服务。您可以在 AWS 控制台使用可视化窗格自动生成 Task 节点，详情参见 [使用状态机编排云函数](https://cloud.tencent.com/document/product/1272/50726)。
...
| ResultPath（可选）| 结果路径，指定输出结果的存储位置，插入到输入数据的指定地址中。更多关于结果路径的信息参见 [状态节点输入与输出]()  | 

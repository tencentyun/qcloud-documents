...
|KeyId|密钥id|CMK 返回给用户的全局唯一标识，注意这不是 CMK 本身，只是一个索引而已|
...
用户可以通过 [KMS SDK](https://cloud.tencent.com/document/product/573/8908) （推荐，目前提供多个语言版本），或直接调用云 API（较为麻烦，建议 SDK 语言之外的用户使用）使用 KMS 的服务：
=======

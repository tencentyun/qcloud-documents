...
- (void)onVideoSizeChanged:(CGSize)videoSize {
    ...
    // 应用画面强制横屏、设置应用画面居中显示类似 UIViewContentModeScaleAspectFit
...
- (void)onVideoShow {
    NSLog(@"SimplePCDemo onVideoShow, 应用开始有画面");
    // 设置正确的鼠标显示与控制模式
...
- 客户端通过 remoteSession 启动应用。
- SDK 启动应用之后会通过 TCGGamePlayerDelegate 把启动过程中的回调告知给客户端。

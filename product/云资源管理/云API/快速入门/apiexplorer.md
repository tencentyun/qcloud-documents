...
![](https://main.qcloudimg.com/raw/6fde9d5e8a195ee8a4abf70c14c62bc6.png)
1. **产品区域**：目前已支持的所有产品。
...
6. **接口所需参数**：仅展示接口所需参数，您可勾选“只看必填参数”进行筛选。具体参数说明可通过选择右侧的【参数说明】进行查看。
7. **功能区**：
...
8. **TCCLI 命令行生成**：可根据用户左侧填写的参数自动生成命令行参数（此处采用复杂类型点连接方式调用的命令行），用户可选择自行复制其内容到 TCCLI 工具中使用。
9. **子功能区**：可切换语言版本，生成对应代码。
10. **SDK 使用说明**：如需了解 SDK 更多信息，例如所需环境及调用示例，可查阅相应的 SDK 使用说明。
11. **响应区**：此处根据7和9的选择不同，展示生成 SDK 调用代码、在线调用结果等响应信息。
...
![](https://main.qcloudimg.com/raw/d85ded7b9ca72162184085166aa71806.png)

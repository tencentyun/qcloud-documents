...
腾讯云 API 全新升级 3.0 ，该版本进行了性能优化且全地域部署、支持就近和按地域接入、访问时延下降显著，接口描述更加详细、错误码描述更加全面、SDK增加接口级注释，让您更加方便快捷的使用腾讯云产品。这里针对GO API调用方式进行简单说明。
...
以云服务器查看可用区列表(DescribeInstances)请求为例，当用户调用这一接口时，其请求参数可能如下:
...
> **再次注意**：此处只到计算签名部分，如需获得最终请求串，需要在参数params里加上下面所得到的签名键值对

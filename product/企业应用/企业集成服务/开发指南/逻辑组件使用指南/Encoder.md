...
![配置界面](D:\iPaaS\03_产品规划\数据集成\img\Encoder\d14d5a30885959254a537d86866cdb63.png)
...
| 参数       | 数据类型 | 描述                                                         | 是否必填 | 默认值 |
| :--------- | :------- | :----------------------------------------------------------- | :------- | ------ |
| 输出持久化 | bool     | 用于指定是否需要对输出的 RecordSet 进行持久化存储，若选择 true，则会在该 RecordSet 第一次计算之后缓存计算结果，后续对该数据的读取将无需重复计算 | 否       | false  |
| 分区数     | int      | 用于指定输出的 RecordSet 的分区数，在实际获取 RecordSet 数据时，有多少个分区就会执行多少次子流，从而为每个分区分别获取数据 | 否       | 1      |
...
| error       | 空                                                           |
| attribute   | 空                                                           |
| variable    | 继承主流中的 Variables 数据，同时新增两个变量，分别为计数器和根信息。若用户使用默认值，可使用表达式 msg.vars.get('counter') 和 msg.vars.get('rootMessage') 访问。在子流中操作 Variables 将不会影响主流中的数据 |
...
![enter image description here](D:\iPaaS\03_产品规划\数据集成\img\Encoder\e1bab0a1028b45d4813f3f74b1f4876f.png)
2. 配置子流，将子流的 payload 设置为需要构造的数据内容：
 - 子流配置：
   ![enter image description here](D:\iPaaS\03_产品规划\数据集成\img\Encoder\79f82b7a1ac071c17a451a47344d7344.png)
...
3. 在下游通过 ForEach 或者 Parallel ForEach 组件遍历输出的 RecordSet，并查看结果。
   ![enter image description here](D:\iPaaS\03_产品规划\数据集成\img\Encoder\7235156e6a9c9e885f966fe4214eed58.png)
4. 通过 debug 模式运行并查看，可以得到如下结果：
   ![enter image description here](D:\iPaaS\03_产品规划\数据集成\img\Encoder\b2a9ddd93e3d1430379de9f730ae38e6.png)

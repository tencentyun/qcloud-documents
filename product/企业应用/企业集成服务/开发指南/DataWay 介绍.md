...
Dataway 表达式是标准的 Python 脚本，在 Dataway 处理过程中，用户可以使用并构造允许的任意类型。作为企业集成服务数据流转的重要入口，dw_process 的返回值类型受到严格限制，其返回的结果中仅支持如下类型，否则（一定）将导致运行时的序列化错误：
...
- **math**，用于数学运算的库，可参考 [Python 官方文档](https://docs.python.org/3.5/library/math.html)。已内置在 Dataway 的处理上下文中，可以直接引用。
    - 目前 Dataway 中支持的 math 模块函数： 
		...
		- math.floor(x)：返回 x 的向下取整，小于或等于 x 的最大整数。如果 x 不是浮点数，则委托 x.floor() 返回 Integral 值。

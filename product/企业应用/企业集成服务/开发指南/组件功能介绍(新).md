...
- `*` 表示任意匹配，如 `hours="*"` 表示每个小时
- `-` 表示范围，如 `weekdays="1-5"` 表示周一至周五
- `,` 表示并列，如 `months="1,3,5,7,8,10,12"` 表示大月
- `/` 表示增量，如 `hours="8/2"` ，表示8点开始，每两个小时
...
Listener组件的使用需要参数配置和Trigger配置两部分配合。
...
```
Listener 需要指定监听域名才能对外提供服务，此处指定的 host 为域名前缀，完整的域名取决于部署集群的配置。如默认集群为 `prefix-$appId.ipaas.myqcloud.com`。
2. basePath 配置
```xml
...
```
Listener 支持 IP 白名单设置，一行一个，CIDR 格式。设置 IP 白名单后，如果请求方IP不在白名单网段内，则返回 `403 Forbidden`。
5. Keep-Alives 设置
```xml
<http:listener-config
    keepAliveTimeout="0"
...
```
Path 参数用于匹配HTTP请求，一个Listener Config可用于多个Trigger，当路径匹配时，才会触发。
Path 支持变量 `{variableName}` 及通配符`*`，例如 `/users/{userId}/*`。当有多个Trigger同时匹配一个请求时，会优先匹配更具体的，同等条件下，会优先匹配第一个。路径变量可通过 `attributes.uriParams.xxx` 获取。
2. 请求方法配置
```xml
...
`<http:response/>`用于指定正常时的响应输出，其中：
	- `statusCode`为HTTP状态码，默认为`200`。
	- `reasonPhrase`为状态描述，默认会根据`statusCode`的值取HTTP规范对应的描述，如`200`对应`OK`、`201`对应`Created`。
	- `compressLevel`为GZIP压缩等级，取值范围为`0`~`9`，默认为`0`，即不开启压缩。
	- `<http:body mimeType="..." encoding="...">`为输出的正文。
	- `<http:headers>#[...]</http:headers>`用于设置Header，结构为dict
	- `<http:add-header name="..." value="...">` 用于快捷设置Header，`name`为Header的键，`value`为Header的值，同一个键可添加多次，如`Set-Cookie`。
	- `<http:set-cookie name="..." value="..." expires="..." path="..." maxAge="..." Secure="true" httpOnly="true"/>`，由于`add-header`中，设置Cookie的逻辑会比较常见，因此提供快捷配置。具体属性含义可参考HTTP规范。

>!其中`Content-Type`和`Content-Length`的值会根据body的内容被优先设置，*`Content-Encoding`*在压缩时也会优先设置，因此可能对应设置的*`Add-Header`*无效。
...
Request 组件的使用需要`参数配置`和`Operation配置`两部分结合。

#### 1. 参数配置

Http Request 的参数配置主要用于设置HTTP Client的通用参数，一般一个Client会用于多个Request请求。典型的配置代码如下：
...
```
请求协议，默认`https`，暂不支持HTTPS参数设置及双向验证。
3. 请求端口
```xml
<http:request-config port="..."/>
```
请求端口，`protocol`为`https`时默认`443`，`protocol`为`http`时默认`80`。
4. basePath
```xml
<http:request-config basePath="/"/>
```
请求的 basePath，用于作为请求 URI 的公共前缀，如 `/api/v2` 的前缀。
5. 重定向设置
```xml
<http:request-config maxRedirectCount="10"/>
```
最大跟进3xx重定向次数，默认为`10`，`0`表示不跟进。
6. 超时设置
```xml
...
<http:headers>#[...]</http:headers>
```
`<http:add-header/>`用于逐一添加Header项， `name`为Header的键，`value`为Header的值
`<http:headers>#[...]</http:/headers>`可按表达式来设置，结构需要为`dict<string, list<string>>`。
4. 请求Cookie
```xml
<http:set-cookie name="..." value="..."/>
```
由于Header中，Cookie的设置较多，因此提供Cookie编辑的选项，以避免手动设置。注意包含`key`为`Cookie`的`add-header`和`set-cookie`同时存在时，`add-header`将被覆盖。
>! 强烈建议：不要手动设置 Cookie。
5. 请求参数
```xml
<http:set-query-param name="..." value="..." />
...
```
`<http:set-query-param/>`用于设置URL参数，`name`为参数名，`value`为参数值，可以为字符串或数组。为数组时，每个元素会以相同的变量名列举。如`a=1&a=2&a=3`。当有两个同名的参数出现时，后面的会覆盖前面的。
...
```
用于设置请求体，支持表达式，默认从message的payload中取。
...
1. 消息的 Attributes
	...
	- `attributes.cookies` 响应的**Cookie**，结果为`dict<string, string>`，由于解析Cookie的逻辑比较常见，因此可直接通过该熟悉读取，而不必手动解析。
2. 消息的 Payload

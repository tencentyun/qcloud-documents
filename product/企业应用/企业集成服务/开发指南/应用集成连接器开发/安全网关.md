...
2. 单击【添加安全网关】，参见 [生成证书](#certificate)上传 Public 证书。
...
- Agent 解压后目录结构如下：
  ...
  - configs 目录中包含 Agent 运行过程中所必须的配置，configs 目录介绍：
    ...
    - secret 目录下存放 Agent 连接 Server 时的 Private 证书，Private 证书如何生成请参见 [生成证书](#certificate)。
...
1. 执行以下命令生成 Private 证书。
```plaintext
openssl genrsa -out private.pem 1024
```
 >!请将生成的 Private证书放置在 `ipaas-private-cloud-agent/configs/secret` 目录下。
2. 执行以下命令，以上述 private.pem 证书为基础生成 Public 证书。当前目录下生成的 public.pem 即为 Public 证书。
```plaintext
openssl rsa -in private.pem -RSAPublicKey_out -out public.pem
```
3. 生成新的 Private 证书时，需替换 `ipaas-private-cloud-agent/configs/secret` 目录下的 private.pem 文件。

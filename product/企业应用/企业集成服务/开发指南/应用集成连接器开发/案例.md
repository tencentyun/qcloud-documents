...
腾讯会议为企业 IT、ISV 系统集成商、SaaS 服务商提供 API 和 SDK，无缝对接企业邮箱、日程、会议室管理系统，实现行业应用、企业办公平台与腾讯会议音视频的连接，让腾讯会议更好地“为你的企业所用”，轻松适配多样场景，会议全流程一键达成，全面助力企业内外部高效沟通协作，打造一体化办公体验，[腾讯会议 API 文档](https://cloud.tencent.com/document/product/1095/42407)。目前，企业集成服务提供的腾讯会议连接器通过封装腾讯会议 API 接口，提供以下八种操作，对应 XML 中的八个 **operation**：
...
**properties** 节点中定义腾讯会议连接器的公共配置，包含调用腾讯会议 REST API 所需的公共参数，每次请求均需携带这些参数，才能正常发起请求，所需公共参数如下：
...
**operations** 节点中定义了腾讯会议连接器提供的8种操作，及所需的参数和输出类型。
...
其中，除了提供可以在页面上展示的8种 **operation** 外，还提供一个私有的 **send-request** 操作用于发送 Http Request 请求，方便在其他 **operation** 关联的 **flow** 中通过如下方式调用。
```xml
...
**send-request** 构造调用腾讯会议REST API鉴权所需的请求头。
...
其他 **operation** 实现类似，都是通过调用 **send-request** 发送 REST API 请求实现对应操作。

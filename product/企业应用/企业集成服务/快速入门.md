...
**相关名词解释**
- **集成流**：集成流是鹊桥 iPaaS 系统中的基本运行单位，通常是为了完成一个特定集成业务场景而设计。例如：组织架构数据从 ERP 系统同步到 HR 系统可以独立为一个集成流。
- **连接器**：连接器在鹊桥 iPaaS 系统中负责与第三方系统进行交互，鹊桥 iPaaS 中将其按照某种通信协议、报文规范或者 API 规范以及相关的交互逻辑统一进行封装，隐藏底层处理逻辑。
- **逻辑组件**：逻辑组件在鹊桥 iPaaS 系统中负责执行集成流中的逻辑处理。


## 前期准备
1. 登录 [企业微信官网](https://work.weixin.qq.com/)，单击【立即注册】进行企业微信注册。
2. 参考 [企业微信官网文档](https://work.weixin.qq.com/api/doc/90000/90135/90665) 获取如下信息：
 - 获取企业 ID。
 ![image-20210328143422018](https://main.qcloudimg.com/raw/d2e3f170f28a2c19441c182e2249f83c/27.png)
 - 获取成员管理密钥和应用管理密钥。
![image-20210328151210839](https://main.qcloudimg.com/raw/19fe13e13eac84a7a11aa721d2047e9d/29.png)
 - 配置对应的企业邮箱账号。
 ![](https://main.qcloudimg.com/raw/777cc38e08745c01d3f8029d084e9336.png)
3. 为方便体验，鹊桥 iPaaS 为用户提供公网数据库 ipaas_demo，其中数据库表 ipaas_email_template 用来存储邮件模版，目前存储的数据如下表格：
<table>
<thead>
<tr>
<th>App（应用名称）</th>
...
4. 参考对应邮箱帮助中心，开通邮箱 POP3/IMAP/SMTP 服务，并记录授权码。例如：QQ 邮箱可参考 [邮箱帮助中心](https://help.lunkr.cn/QQ-IMAP-Login-Tutorial/IMAP-Login-Tutorial-QQ.html)。


## 入门体验
### 集成流设计
1. 登录 [鹊桥 iPaaS 控制台](https://cloud.tencent.com/login?s_url=https%3A%2F%2Fconsole.cloud.tencent.com/eis)。
2. 在项目管理页面单击【添加应用】。
...
7. 填写通用配置，包括监听域名、监听路径、回调事件 Token、回调事件 EncodingAESKey。例如：
 - **监听域名**：qiweirecall
 - **监听路径**：/
 - **回调事件 Token、回调事件 EncodingAESKey：**在 [企业微信](https://work.weixin.qq.com/wework_admin/frame#apps/contactsApi) 后台配置通讯录回调事件时获取，具体步骤请参考 [企业微信 API 官方文档](https://work.weixin.qq.com/api/doc/90000/90135/90967)。
 ![image-20210413212109843](https://main.qcloudimg.com/raw/0fba7589b4561aadf2b2629aca76941a/48.png)
8. 使用逻辑组件 **Choice** 判断验证请求回调（method=="GET"）或者变更事件回调（method=="POST"），具体步骤如下：
 - 单击“+”弹出组件筛选框。
...
9. 使用逻辑组件 **Set Variable** 保存回调事件变更类型、变更对象 userId 等信息，具体步骤如下：
 - 单击 Default 框中的“+”弹出组件筛选框。
 ...
 - 将 payload 中的变更用户 ID（UserId）、变更时间（CreateTime）、变更事件类型（ChangeTpye）用变量名 eventInfo 进行保留。
...
10. 使用 **Database 连接器**，根据回调事件检索对应的邮件模版，具体步骤如下：
 - 单击 Set Variable 后的“+”弹出组件筛选框并选择 Database 连接器。
...
![image-20210413221501222](https://main.qcloudimg.com/raw/1fa47e2b864ba541dd57094f0776798d/57.png)
 - 通用配置：配置查询语句和输入参数，具体步骤如下：
	 - 查询语句填写为：**select * from ipaas_email_template where app='tencent-wechat' and change_type=:changeType**
	 - 单击【添加】即可输入查询参数。
	![image-20210413223726811](https://main.qcloudimg.com/raw/8f53eee04f53997d1c632782a9dad78a/59.png)
	   - 在弹出的参数框中，key 行填写：changeType。
![image-20210413224008630](https://main.qcloudimg.com/raw/b055c1382c43e355f6ca82e0f7a5bc34/60.png)
       - 单击 value 输入框的【f(x)】切换为表达式输入，获取 changeType 表达式。
...
11. 使用逻辑组件 **Set Variable** 保存邮件标题和正文模版，具体步骤如下：
 - 单击 Database 连接器后的“+”弹出组件筛选框并选择 Set Variable 组件。
![image-20210414101817532](https://main.qcloudimg.com/raw/1670ae104338b0581d24377d2ea93a47/64.png)
 - 将 payload 中的邮件标题（email_title）、邮件正文模版（email_body）用变量名 emailInfo 进行保留。
...
12. 使用**企业微信连接器**获取部门成员详情，具体步骤如下：
 - 单击 Set Variable 后的“+”并选择企业微信连接器。
...
:::
 ...
 ![image-20210414110318383](https://main.qcloudimg.com/raw/66d533756135de49b81f6f4a682a7be0/72.png)
	...
	![image-20210414121158667](https://main.qcloudimg.com/raw/d9497cc6f732c7232ef8753f86e2430b/83.png)
	 - 日志级别选择“INFO”，通过表达式获取“成员名称”作为日志内容。
...
![image-20210414110933121](https://main.qcloudimg.com/raw/e3a070dcc02e7d00ccda67e572654210/76.png)
	 - 进行发件人、相关协议等配置。其中 POP 设置和 IMAP 设置可二选一，SMTP 设置为必填，**同时请务必确认对应邮箱已设置开启 POP3/SMTP 或者 IMAP/SMTP 服务，并且基础配置中密码为开启服务时默认的授权码。**
       - 例如：QQ 邮箱授权码：
...
<td>993</td>
       ...
       - 其他邮箱可参考对应邮箱帮助中心。
![image-20210414134630417](https://main.qcloudimg.com/raw/02d8ecba623d65d445e0725d6ccfd98b/77.png)
 - 通用配置：填写 Email 收件人、标题以及正文。
...
### 发布
鹊桥 iPaaS 提供租户级别的多地域部署方式，在完成集成流的设计后，单击右上角【发布】，可以选择部署区域，确认后即可实现应用纬度的一键部署。
...
2. 参考 [企业微信 API 官方文档](https://work.weixin.qq.com/api/doc/90000/90135/90967) 通讯录回调事件，将回调域名：[http://qiweirecall-$appid.ipaas.ap-tianjin.myqcloud.com](http://createrecall-%24appid.ipaas.ap-tianjin.myqcloud.com/) 配置为  URL。
3. 单击通讯录中【添加成员】。
...
## 实践总结
在鹊桥 iPaaS 中通过连接器和逻辑组件构建的集成流，轻松地在企业微信和企业邮箱之间进行数据打通。至此，您已经对鹊桥 iPaaS 有了初步认识，后续您可以通过我们持续更新的教程进一步了解鹊桥 iPaaS 在不同应用场景下的功能和用法，帮助您更快更好地进行企业应用集成～

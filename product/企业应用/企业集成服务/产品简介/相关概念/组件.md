千帆鹊桥iPaaS产品的主要概念包括：深度集成、消息、组件、安全网关等。本文主要介绍组件的相关内容。
...
## 组成部分
### 逻辑组件
逻辑组件是对常见编码逻辑的封装，例如：Set Variable（创建一个变量）、Choice（选择，按条件执行分支）。千帆鹊桥iPaaS将编码时常见的业务逻辑封装成开箱即用的逻辑组件，当业务流程需要添加相应的逻辑时，只需在相应节点添加相关组件并进行配置即可，例如：Set Variable 组件添加后只需填写变量名及变量值；Choice 组件添加后可以增删条件分支，同时可以配置每个分支的执行条件。

## 功能介绍
通过回调通知，您可以在合同状态产生变化以及发生印章、模板、员工相关操作等场景接受到来自电子签的回调通知。如您需要开通此功能，需提供一个能接受 HTTP POST 请求的回调地址给电子签对接人员进行配置，请确保该地址提供的服务可公网访问并能正常处理回调通知。电子签推送的回调内容有进行加密处理，在接受到回调通知后需用配置好的 Key 进行解密。
...
<table>
   ...
   <tr>
      <td>Operate</td>
      <td>string</td>
      <td>动作，仅在非审核结果回调时有值：<br>创建：Create<br>删除：Delete<br>停用：Disable<br>启用：Enable<br>印章授权：Valid<br>解除印章授权：Invalid</td>
   </tr>
   <tr>
   ...
   <tr>
      <td>ReviewStatus</td>
      <td>string</td>
      <td>审核状态，仅在审核结果回调时有值：<br>通过：PASS<br>拒绝：REJECT</td>
   </tr>
   <tr>
      <td>ReviewReason</td>
      <td>string</td>

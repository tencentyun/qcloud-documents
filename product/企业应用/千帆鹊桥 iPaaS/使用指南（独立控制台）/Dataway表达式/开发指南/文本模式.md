...
<dx-accordion>
::: 任意类型（any，默认类型）
可直接输入文本生成字符串，也支持集成流数据面板引用集成流上下文数据。当文本和集成流数据面板引用存在多项时，将以这些数据项为元素，组建列表。
...
可直接输入字面量生成对应数据，也支持集成流数据面板引用，方便用户使用集成流的上下文数据。集成流数据面板引用的数据会转化为字符串，拼接到字面量中。
...
文本模式支持集成流数据面板引用集成流上下文的"数据集成"数据：数据集和单条数据。数据集和单条数据作为数据集成的专有数据类型，需要通过特定组件（"构造数据集"组件等）生成，Dataway 不提供生成方法。

...
腾讯云数据连接器中封装企业微信连接器，可以完成成员管理、消息推送等多种操作，每次操作本质上都是调用 [企业微信 API](https://work.weixin.qq.com/api/doc/90000/90135/90664) 完成的。在调用 API 前，需要获取 access_token。相当于创建一个登录凭证，其他的业务接口都依赖 access_token 鉴权调用者身份。

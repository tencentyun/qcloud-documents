## 2021年10月

<table >
<thead>
<tr>
<th width="20%">动态名称</th>
<th width="50%">动态描述</th>
 <th width="15%">发布时间</th>  
<th width="15%">相关文档</th>
</tr>
</thead>
<tbody><tr>
<td>新功能</td>
<td ><li>提供了团队空间新增创建文件夹功能，团队创建者和团队管理员可创建，删除，重命名文件夹，问卷管理员和团队成员暂时不开放操作文件夹权限。</li>
<li>提供了卷叔填填圈新增老龄答题信息（可填写自己或父母），填写老龄信息将获得更多答题任务和奖励。</li></td>
 <td>2021-10-21</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
</tbody></table>

## 2021年09月

<table >
<thead>
<tr>
<th width="20%">动态名称</th>
<th width="50%">动态描述</th>
 <th width="15%">发布时间</th>  
<th width="15%">相关文档</th>
</tr>
</thead>
<tbody><tr>
<td>新功能</td>
<td >企业微信-成员授权应用安装：
<li>企业创建者-安装应用 -修改为成员授权使用 - 邀请成员一起使用或成员工作台搜索腾讯问卷应用进行安装。</li>
<li>添加未授权应用的白名单协管员时, 发送授权通知,用户授权后,才可正常添加使用。</li></td>
 <td>2021-09-27</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
<tr>
<td>新功能</td>
<td >提供了问卷匿名回收功能。</td>
 <td>2021-09-06</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
</tbody></table>


## 2021年08月

<table >
<thead>
<tr>
<th width="20%">动态名称</th>
<th width="50%">动态描述</th>
 <th width="15%">发布时间</th>  
<th width="15%">相关文档</th>
</tr>
</thead>
<tbody><tr>
<td>新功能</td>
<td >提供了移动端问卷操作栏新增同步腾讯文档入口。</td>
 <td>2021-08-26</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
<tr>
<td>新功能</td>
<td ><li>提供了优化附件存储时长，问卷移动至付费团队版时已有附件改为永久存储。</li>
<li>提供了新增 PC端及移动端支持腾讯文档同步问卷数据。</li>
<li>提供了 URL 预填答案变更为已付费的团队版功能。</li></td>
 <td>2021-08-18</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
<tr>
<td>新功能</td>
<td >新增企业试用版本。</td>
 <td>2021-08-05</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
<tr>
<td>新功能</td>
<td >新增企业微信添加白名单，支持一键推送问卷。</td>
 <td>2021-08-04</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
</tbody></table>

## 2021年07月

<table >
<thead>
<tr>
<th width="20%">动态名称</th>
<th width="50%">动态描述</th>
 <th width="15%">发布时间</th>  
<th width="15%">相关文档</th>
</tr>
</thead>
<tbody>
<tr>
<td>新功能</td>
<td >提供了创建团队手机号验证。</td>
 <td>2021-07-19</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr><tr>
<td>新功能</td>
<td >提供了企业微信支持购买企业版。</td>
 <td>2021-07-14</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
</tbody></table>

## 2021年03月

<table >
<thead>
<tr>
<th width="20%">动态名称</th>
<th width="50%">动态描述</th>
 <th width="15%">发布时间</th>  
<th width="15%">相关文档</th>
</tr>
</thead>
<tbody><tr>
<td>新功能</td>
<td >提供了企业支持改名功能。</td>
 <td>2021-03-19</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
</tbody></table>


## 2021年01月

<table >
<thead>
<tr>
<th width="20%">动态名称</th>
<th width="50%">动态描述</th>
 <th width="15%">发布时间</th>  
<th width="15%">相关文档</th>
</tr>
</thead>
<tbody><tr>
<td>腾讯云发布腾讯问卷产品</td>
<td >提供了问卷设计、投放、分析的所有通用功能，可以满足调查、研究、表单、投票、考试、测评等多场景的使用需要。</td>
 <td>2021-01-19</td> 
<td><a href="https://cloud.tencent.com/document/product/1304/49131">产品功能</a></td>
</tr>
</tbody></table>




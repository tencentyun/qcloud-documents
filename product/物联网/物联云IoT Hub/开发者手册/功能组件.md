...
- SDK 支持 RTOS 移植能力，跨平台移植，框架抽离硬件平台抽象层，可基于不同平台快速、轻松接入物联云。

设备 SDK 支持 TLS(对应MQTT)、DTLS(对应CoAP) 的非对称和对称加密两种鉴权方式，保护设备通信安全：

- 非对称加密
  安全级别高，基于证书、非对称加密算法，适用于硬件规格较高、对功耗不是很敏感的设备。依赖设备证书、私钥，根证书等信息，在物联云创建设备时，会返回相关信息。
- 对称加密
  安全级别普通，基于秘钥、对称加密算法，适用于资源受限、对功耗敏感的设备。依赖于设备 psKey ，在物联云创建设备时，会返回相关信息。

除设备 SDK 接入外，腾讯物联云还提供：

- MQTT over WebSocket 接入，适用于浏览器类应用与物联云平台建立连接与通信能力。
...
对于 MQTT 的数据传输，腾讯物联云支持 QoS=0 或 1，但不支持 QoS=2。基于 MQTT 协议。设备消息支持离线存储。
...
- **设备与云服务互通**
  对于用户需要对设备数据进行进一步处理的场景（如持久化存储、大数据分析），腾讯云一方面提供相应的产品（如云数据库、大数据分析套件），另外也即将支持物联云平台和这些云产品之间的直接打通。

参见 [规则引擎详情](/document/product/634/11917)。

### 8. 消息队列

作为设备的唯一接口，物联云平台支持将设备指定消息写入腾讯云 CMQ、CKafka 消息队列，第三方服务可通过 CMQ、CKafka 的 SDK 接口获取设备消息，从而打通实现与设备的异步消息通信。在此基础上完成后端的数据存储、计算分析或设备控制逻辑。
...
对于物联场景下对设备的管理流接口，提供后台快速、批量操作接口。当前支持 Python、PHP、JAVA 工具包。目前腾讯物联云提供如下云 API
...
物联云平台支持通过 [CAM](https://cloud.tencent.com/product/cam) 安全地访问、使用和管理云账号的资源。通过对子账号与协作者的身份管理和策略管理，来实现物联云资源的隔离与协作。

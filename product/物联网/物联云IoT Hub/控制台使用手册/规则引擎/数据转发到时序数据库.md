...
- 数据字段 ： 类型可选择 CTSDB 里面的 tag 类型或者 field 类型，字段名称输入限制，见 [CTSDB 限制](https://cloud.tencent.com/document/product/652/13611),值有 3 种配置方式：通过 ${} 引用原始消息的字段值；系统函数，见规则引擎函数列表；常量。

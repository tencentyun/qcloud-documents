创建完规则，即可编写 SQL 对某一类 Topic 中数据的处理。物联云控制台提供了 SQL 填写的方式来简化 SQL 语句的生成，在红框中的条件中对 Topic 数据进行筛选。
...
可以监听```house_monitor/thermometer/get```和```house_monitor/door/get``` 等 Topic；
但是不可以监听 ```house_monitor/door/switch/get```，因为```+```只能代表 1 个 Topic 段。

例如： ```house_monitor/#```
可以监听 ```house_monitor/thermometer``` 和```house_monitor/door/switch/get``` 等 Topic；
但是 ```house/#/get``` 是非法的，因为```#```只能放在 Topic 结尾。

...
>?同一产品下的所有设备在生产过程可以烧录统一的产品信息，即产品 ID（ProductId）、产品密钥（ProductSecret）。设备出厂后，通过动态注册的方式获取设备身份信息并保存，然后用申请到的三元组或者四元组信息进行设备认证。若开启动态注册的同时使能自动设备创建，则设备名是可以设备自己生成的，但必须保证同一产品 ID（ProductId）下的设备名不重复，一般取 IMEI 或者 MAC 地址。动态注册功能详情请参见 [设备接入](https://cloud.tencent.com/document/product/634/35270)。

...
>! airConditioner1 不可以通过直接订阅 door1 的 update 消息来完成消息通信，原因请参考 [功能组件](https://cloud.tencent.com/document/product/634/11915#4.-.E6.9D.83.E9.99.90.E7.AE.A1.E7.90.86) 文档中权限管理部分。
...
查询设备详情页面，设备证书、设备私钥用于 MQTT 的 TLS 非对称加密；对称密钥用于对称加密通信（两种通信方式差别请参考 [功能组件 - 设备接入](https://cloud.tencent.com/document/product/634/11915?!preview&!editLang=zh#2.-.E8.AE.BE.E5.A4.87.E6.8E.A5.E5.85.A5)）。
...
具体步骤请参考 [规则引擎详情](/document/product/634/11917)。
...
编辑 C-SDK 程序 samples/scenarized 下的 door_mqtt_sample.c，使用“[创建门产品和设备]()”中获得的设备信息，填写以下配置项（下表物理含义后的编号，与“[创建门产品和设备]()”截图中的序号一一对应）。
...
1. 完成对 CoAP 协议通信初始化参数 init_params 的配置。编辑 C-SDK 程序 samples/scenarized 下的 door_coap_sample.c，使用 [创建门产品和设备]() 中获得的设备信息，填写以下配置项（下表物理含义后的编号，与 [创建门产品和设备]() 截图中的序号一一对应）。

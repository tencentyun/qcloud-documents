...
## 概述
TencentOS tiny 是面向物联网（IOT）领域的操作系统，它由一个精简的实时操作系统（RTOS）内核，以及丰富的物联网组件组成。
...
初级功耗管理方案下，因为还有系统 s ystick 的存在，因此系统进入 idle 任务后，并不会在睡眠模式下停留太久。如需进入到更极致的低功耗状态，需要暂停 systick。

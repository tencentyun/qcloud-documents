...
|`void tos_knl_int_enter(void);`|此函数应该在中断调用函数的最前端被调用|
|`void tos_knl_int_leave(void);`|此函数应该在中断调用函数的尾端被调用|
...
#### 编程实例
对于初级低功耗模式，无需用户编写任何代码，直接通过在 tos_config.h打开TOS_CFG_PMR_MGR_EN 开关即可：`#define TOS_CFG_PWR_MGR_EN		1u`
...
初级功耗管理方案下，因为还有系统s ystick 的存在，因此系统进入 idle 任务后，并不会在睡眠模式下停留太久。如需进入到更极致的低功耗状态，需要暂停 systick。

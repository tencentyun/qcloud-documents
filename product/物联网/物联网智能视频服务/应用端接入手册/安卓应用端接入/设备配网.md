...
SDK 集成了 zxing，开发者可以自定义扫描页面，或使用 com.tencentcs.zxing.CaptureActivity （需要 AndroidManifest.xml 中声明）。

**解析二维码内容**

扫描二维码得到一个字符串，可通过以下接口进行解析：
```java
/**
 * 解析二维码内容
 *
 * @param scanResult 二维码内容字符串
 * @return 解析结果
 */
public static QRCode analyse(String scanResult);
```

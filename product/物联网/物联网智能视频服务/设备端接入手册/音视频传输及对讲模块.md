...
>?该接口仅仅支持单路通道，推荐使用相同功能但支持多通道的 `@iv_avt_get_multiple_av_enc_info_cb`。
>
```
...
>?该接口仅仅支持单路通道，推荐使用相同功能但支持多通道的 `@iv_avt_start_multiple_live_stream_cb`。
>
```
...
>?该接口仅仅支持单路通道，推荐使用相同功能但支持多通道的 `@iv_avt_stop_multiple_live_stream_cb`。
>
```
...
>?该接口仅仅支持单路通道，推荐使用相同功能但支持多通道的 `@iv_avt_send_multiple_stream`。
>
```
...
>?该接口仅仅支持单路通道，推荐使用相同功能但支持多通道的 `@iv_avt_request_iframe_cb`。
...
设置设备对讲时音视频解码信息参数。
>?该接口仅仅支持单路通道，推荐使用相同功能但支持多通道的 `@iv_avt_start_recv_multiple_stream_cb`。
>
```
...
>?该接口仅仅支持单路通道，推荐使用相同功能但支持多通道的 `@iv_avt_recv_multiple_stream_cb`。
>
```
...
发送对讲音视频数据。在观看端发起监控请求时，设备端相应准备工作完成（音视频编码启动）后，取流向观看端端发送数据的接口。不同清晰度视频请求时音频也需要同时发送。适用于多路输出流存在，向指定流通路进行发送。eg：多摄像头、NVR 等。

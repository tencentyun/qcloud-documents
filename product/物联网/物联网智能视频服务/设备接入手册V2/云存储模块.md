...
* int iv_cs_event_start： 触发事件
* int iv_cs_event_stop：停止事件
...
* 用于获取当前设备的云存的套餐信息。
* 当 `timeout_s` 大于0时，该接口为阻塞获取信息，直接从后台获取套餐信息，失败则返回 SDK 中的套餐缓存信息（1小时更新一次），并返回错误码，成功则返回`IV_ERR_NONE`。

...
iv_cs_init
    |
iv_cs_get_balance_info //获取本地套餐信息
    |
...
* 用于触发一个事件，事件编号的合法取值为1 - 16，事件编号的具体含义由用户自定义。
* 用户触发事件以后，SDK 将调用 iv_cs_event_capture_picture_cb 向用户请求截图，同时会调用 iv_cs_push_stream_start_cb 通知用户开始推送音视频流。
* 截图上传完毕后 SDK 调用 iv_cs_event_picture_result_cb 通知用户图片上传结果。
* 在上一事件停止后调用，需与 iv_cs_event_stop_ext 成对调用。
...
* 用于停止一个事件，事件编号的合法取值为1 - 16，事件编号的具体含义由用户自定义。
* 用户停止事件以后，SDK 调用 iv_cs_push_stream_stop_cb 通知用户停止推送音视频流。
* 必须调用 iv_cs_event_start_ext 后才可调，且与 iv_cs_event_start_ext 成对调用。
...
* 用于获取当前设备的云存的套餐信息。
* 当 `timeout_s` 大于0时，该接口为阻塞获取信息，直接从后台获取套餐信息，失败则返回 SDK 中的套餐缓存信息（1小时更新一次），并返回错误码 `IV_ERR_CS_QUERY_SERVICE_TIMEOUT`，成功则返回 `IV_ERR_NONE`。
...
2. u32MaxGopSize 的取值推荐为能保存一个 GOP 的视频和对应的音频。假设帧率为20fps，GOP为80帧，经统计4秒的视频数据约700KB，音频约60KB，则 u32MaxGopSize 可以取 800KB。
3. 如果设备内存紧张可以适当减小 u32MaxGopSize 的取值，若减小至原来的80%（以上述为例即为640KB），但不得小于一个I帧的大小，取值过小会导致云存上传更难以承受网络波动。
4. 用户可以在未收到 iv_cs_push_stream_stop_cb 回调的情况下主动停止推送云存录像，SDK 超过10秒未收到音视频数据会暂停云存录像。
5. 云存使用过程中禁止修改音视频格式，若格式发生变化，则请调用 iv_cs_exit 关闭云存，再调用 iv_cs_init 重新初始化云存并设置新的音视频格式。
6. 内存开销计算：
   - 输入 AAC 格式音频：**内存开销 = u32MaxGopSize**
   - 输入其他格式音频：**内存开销 = u32MaxGopSize + 120KB（单声道） 或 160KB（双声道）**
7. 事件云存触发建议事件至少持续3s及以上，以保证视频片段、图片可及时上传，不会发生丢弃；否则可能会发生视频、图片不上传，只上报事件触发消息。
8. 如果没有套餐，全时云存不会触发推流通知，此时推流也会返回错误码 [IV_ERR_CS_APPLY_NO_SERVICE](https://cloud.tencent.com/document/product/1131/55315)，需要用户定时检查套餐情况。

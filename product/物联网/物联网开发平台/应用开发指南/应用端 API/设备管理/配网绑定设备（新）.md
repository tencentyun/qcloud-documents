...
本接口(AppSigBindDeviceInFamily)提供通过小程序或APP进行WiFi设备配网绑定操作
...
|Signature|String|是|动态签名, 由设备根据配网协议传输到小程序/APP端的签名|
|BindType|String|是|绑定类型，WiFi 绑定 wifi_sign（缺省），蓝牙绑定选择 bluetooth_sigh|

## 3. 输出参数
|名称|类型|描述|
|---|---|---|
|RequestId|String|公共参数，唯一请求ID，与入参相同|
|AppDeviceInfo|Object of [AppDeviceInfo](接口请求域名替换#AppDeviceInfo)|设备信息|

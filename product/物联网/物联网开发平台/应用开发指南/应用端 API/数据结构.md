...
设备地理围栏告警事件。
被如下接口使用：AppGetFenceEventList。

| 名称       | 类型   | 描述                     |
| ---------- | ------ | ------------------------ |
| EventType  | String | 设备地理围栏名称         |
...
| 名称           | 类型   | 描述                           |
| -------------- | ------ | ------------------------------ |
| FenceId        | Int    | 设备地理围栏 ID                 |
| FenceName      | String | 设备地理围栏名称               |
| FenceArea      | String | 设备地理围栏区域范围           |
| FenceDesc      | String | 设备地理围栏描述               |
| AlertCondition | String | 触发条件                       |
| FenceEnable    | Bool   | 启用状态                       |
| Method         | String | 通知方式                       |
| CreateTime     | String | 创建 unix 时间戳（秒级）       |
...
|MsgId|String|消息 ID|
|Category|Int|主类型<li>1：设备<li>2：家庭<li>3：通知|
...
|Repeat|Int|是否循环。<li>0：不需要<li>1：需要|
|Data|String|定时器启动时下发的状态|
|Status|String|开启状态。<li>0：表示关闭<li>1：表示开启。|
...
|AllowEditions|Int|小程序白名单版本。<li>0：表示仅限家庭版<li> 1：表示家庭版和社区版都允许|
|UsingEdition|Int|小程序当前使用版本。<li>0：表示家庭版<li>1：表示社区版|

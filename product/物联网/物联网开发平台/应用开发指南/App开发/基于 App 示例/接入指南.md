...
>!  登录前所使用的API （https://iot.cloud.tencent.com/api/exploreropen/appapi），不建议在设备端调用，需要替换为自建的后台服务，以避免密钥的泄漏。
   ...
   **登录前所使用的 API URL** 在 **self.baseUrl** 配置，**请务必替换成自建的后台服务地址。**
     ...
     - (void)setEnvironment {
         ...
         self.oemTokenApi = @"https://iot.cloud.tencent.com/api/exploreropen/tokenapi"; 
   ...
   **登录前所使用的 API URL** 在 **UNLOGINED_HOST + OEM_APP_API** 配置，**请务必替换成自建的后台服务地址。**
        ```
        /**
         * 接口请求文件
         */
   class HttpRequest private constructor() {
           ...
           const val UNLOGINED_HOST = "https://iot.cloud.tencent.com/api/" // 需要替换为自建后台服务地址
           // 公版&开源体验版使用
           const val OPENSOURCE_APP_API = "studioapp"
           const val OPENSOURCE_TOKEN_API = "studioapp/tokenapi"

           // OEM App 使用
           const val OEM_APP_API = "exploreropen/appapi"  // 需要替换为自建后台服务地址

...
>!  登录前所使用的 API （https://iot.cloud.tencent.com/api/exploreropen/appapi），不建议在设备端调用，需要替换为自建的后台服务，以避免密钥的泄漏。
...
-   App API （api\exploreropen\appapi）请在自建后台进行调用，Token API（api\exploreropen\tokenapi）可安全在设备端调用。

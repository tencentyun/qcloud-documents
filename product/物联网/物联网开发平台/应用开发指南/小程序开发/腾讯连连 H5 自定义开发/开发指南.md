...
- H5 面板所需的参数，可通过 H5 SDK 调用
- H5 面板的标准组件（设备详情页面、模态提示框、顶部提示等），可通过 H5 SDK 调用
- 开发者上传的 JS 文件与 CSS 文件
...
物联网开发平台已提供一个 H5 面板 Demo 供开发者参考，H5 面板 Demo 以开源的方式向开发者开放，便于开发者在开发 H5 面板时进行参考。H5 面板 Demo 项目，详情请参见 [iotexplorer-h5-panel-demo](https://github.com/tencentyun/iotexplorer-h5-panel-demo)。
启动代码示例如下：
```bash
npm install
npm run dev
...
##### 步骤2：安装并配置 whistle
1. 安装 whistle，行执行如下命令：
   ```bash
   npm install -g whistle
   ``` 
2. 启动 whistle，行执行如下命令：
   ```
   w2 start
   ```
...
SDK 已封装检查固件更新的接口与默认的提示交互，开发者在调用 H5 SDK 提供的接口时，可以检查设备当前是否有可升级的固件，以及开始固件升级流程。开发者可以选择使用 H5 SDK 提供的默认固件升级提示，也可以自定义固件升级的提示交互。
...
<span id="h5-device-detail-example">

#### 调用 H5 自定义设备详情

关于 H5 自定义设备详情的接口参数描述，详情请参见 [展示 H5 自定义设备详情视图](#H5面板SDK/sdk-show-device-detail)。
...
H5 面板内无法直接调用小程序的蓝牙接口，H5 SDK中封装了 H5 面板与小程序间的蓝牙协议通信机制。要在 H5 面板使用蓝牙通信能力，必须使用 H5 SDK 提供的蓝牙模块。

关于蓝牙设备的 H5 面板开发，请参见 [蓝牙设备文档](https://github.com/tencentyun/iotexplorer-h5-panel-demo/blob/master/BLUETOOTH-README.md)。

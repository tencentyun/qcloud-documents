...
H5 直出时，需要在服务端拉取面板所需要的所有数据，导致加载速度较慢。为了解决这个问题，我们推出了急速模式，其原理是加载时服务端不去加载数据，而是由 H5 打开后由小程序客户端将本地的数据通过我们特有的链路推送到 H5，可以大大加速加载时间，但是也由于从原本的直出时同步准备好所有数据改成了页面渲染后异步加载数据，所以渲染时依赖直出数据的面板需要做一定的适配才能够正常使用。

...
最后添加宏定义`MBEDTLS_CONFIG_FILE=<qcloud/tls_psk_config.h>`，指定mebedtls库的配置文件：

![](https://main.qcloudimg.com/raw/b06347a849ebda241cd4f1360d601aa2.png)

移植完成，此时编译没有错误，警告可以暂时不管。
...
## 加入示例代码
 ...
 腾讯云C-SDK的测试代码较多，所以直接使用官方仓库中提供的示例文件，在`examples\qcloud_iot_explorer_sdk_data_template`目录下，将此目录复制到工程中，保持原有目录架构不变：
...
>注意不要将data_config.c加入！

修改entry.c中的配置信息：

![](https://main.qcloudimg.com/raw/d1dc0309a19ff454112f64fbc3111da2.png)

示例代码中的任务入口函数为application_entry，接下来我们将task1任务的任务入口函数修改为application_entry，并再次扩大task1的任务栈为4096字节，让示例程序正常运行：
...
在`data_template_sample.c`文件中使能Action：
...
修改`config.h`文件，将设备行为调用关闭，开启设备事件支持：

![](https://main.qcloudimg.com/raw/602e0f0d6d6049f74b30c9b46dd7aa46.png)

修改`data_template_sample.c`文件，关闭设备行为调用示例，开启事件上报示例：

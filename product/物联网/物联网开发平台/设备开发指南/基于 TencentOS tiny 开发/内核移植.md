...
2. 其次，在 board 目录下放入上个步骤中生成的 [裸机工程代码](https://cloud.tencent.com/document/product/1081/47955#.E5.87.86.E5.A4.87.E8.8A.AF.E7.89.87.E5.AF.B9.E5.BA.94.E7.9A.84.E8.A3.B8.E6.9C.BA.E5.B7.A5.E7.A8.8B)，移植的开发板取名为 EVB_MX_Plus_Demo，将裸机代码全部拷贝到该文件夹下即可。
...
TencentOS tiny 的内核代码分为添加 arch 平台代码、添加内核源码、添加 cmsis os 源码及添加 tos_config.h 头文件等步骤。
### 添加 arch 平台代码
- tos_cpu.c 是 TencentOS tiny 的 CPU 适配文件，包括堆栈初始化、中断适配等。
 - 如果您的芯片是 ARM Cortex M 核，例如：M0、M3 、M4、M7 通用核，则该文件无需改动。
 - 如果芯片为其他 IP 核时需要则重新适配。

- port_s.S 文件是 TencentOS tiny 的任务调度汇编代码，主要做弹栈压栈等处理的；port_c.c 适配 systick 等。这两个文件每个 IP 核和编译器都不相同。

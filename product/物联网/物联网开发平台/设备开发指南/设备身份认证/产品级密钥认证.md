...
3. 在产品下创建设备（可选操作）
 ...
 - 若未开启自动创建设备，则云端将在设备注册时校验每一个请求的设备名是否已在云端创建完成，建议您采用设备端可读取的唯一标识作为 Devicename，例如：设备的 IMEI 号、SN 号、MAC 地址等，便于整个流程的顺利完成。
...
4. 设备固件烧录，具体步骤如下：
 ...
 2. 实现 SDK 中关于产品、设备信息读写的 HAL 层函数，包括 ProductID、ProductSecret 和 Devicename 等，并 SDK 中的开启动态注册功能，具体可参见 [设备接入](https://cloud.tencent.com/document/product/1081/39326)。

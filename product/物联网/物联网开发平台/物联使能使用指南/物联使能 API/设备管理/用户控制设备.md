...
本接口（ServiceControlDeviceData）用于用户对绑定的设备发起控制操作。
...
| Data      | String | 返回数据                                                     |
| Result    | String | JSON字符串， 返回下发控制的结果信息，Sent = 1 表示设备已经在线并且订阅了控制下发的 mqtt topic |
...
```json
  {"Response":{"Data":"","Result": "{\"Sent\":1,\"pushResult\":0}","RequestId":"f92406b3-****-4fe8-bc43-45e3d794bb68"}}
```

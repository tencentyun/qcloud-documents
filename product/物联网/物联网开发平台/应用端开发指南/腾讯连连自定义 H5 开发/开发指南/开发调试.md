...
如果您对 whistle 不熟悉，也可以尝试更简单的[免代理模式](#proxy-free-mode)，从而跳过步骤2。
...
开启免代理模式的方式也很简单，就是在开发面板的链接后面加入一个参数：`&sw=true`，假设我们要开发一个设备，它的 productId 为 ABCDEFG, deviceName 为 dev1，那么通过以下链接即可开启免代理模式

```
https://iot.cloud.tencent.com/h5panel/developing?productId=ABCDEFG&deviceName=dev1&sw=true
```
通过加入`sw=true`，页面会开启一个 service-worker, 将`developing.script`, `developing.style`的请求代理到 localhost 开发服务器。
...
而开发服务器打包出来的文件通过 localhost 来访问的，因此我们需要在两者之间进行一个代理，上述的 whistle 就是起着这样的作用。如果不借助 whistle，我们也可以借助浏览器自带的 service-worker 完成这样的工作，这就是免代理模式。进一步讲，所谓免代理模式只是避免了安装 whistle 这样的工具，而是通过浏览器自身的能力(service-worker)实现了代理。

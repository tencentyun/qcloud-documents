...
| FEATURE_AUTH_MODE                | MQTT/CoAP 接入认证方式，使用证书认证：CERT；使用密钥认证：KEY，默认 CERT。 |
...
| FEATURE_LOG_UPLOAD_ENABLED       | 是否打开日志上报云端功能，默认开启。                         |
| FEATURE_EVENT_POST_ENABLED       | 是否打开事件上报功能，默认开启。                             |
| FEATURE_MULTITHREAD_TEST_ENABLED | 是否编译Linux多线程测试例程，默认开启。                      |
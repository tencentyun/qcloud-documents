...
本文对 C SDK 的编译方式和编译配置选项进行说明，并介绍了 Linux 和 Windows 开发环境下的编译环境搭建以及编译示例。
...
| 名称                             | cmake 值            | 说明                                                         |
| :------------------------------- | ------------- | ------------------------------------------------------------ |
| BUILD_TYPE                       | release/debug |<li> release：不启用 IOT_DEBUG 信息，编译输出到 release 目录下。<li> debug：启用 IOT_DEBUG 信息，编译输出到 debug 目录下。 |

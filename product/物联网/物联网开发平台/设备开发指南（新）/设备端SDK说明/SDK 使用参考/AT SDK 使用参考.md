...
|开发平台|文档|
|---|---|
|MCU+定制AT模组（蜂窝类）|[MCU+定制MQTT AT模组（蜂窝类）接入指引](设备开发指南\直连设备开发\资源受限类设备MCU+定制MQTT AT模组（蜂窝类）接入指引)|
|MCU+定制AT模组（wifi类）|[MCU+定制MQTT AT模组（wifi 类）接入指引](设备开发指南\直连设备开发\资源受限类设备MCU+定制MQTT AT模组（wifi 类）接入指引)|

## SDK 接口说明

关于 SDK 的更多使用方式及接口了解，请参见 qcloud_iot_api_export.h 文件。

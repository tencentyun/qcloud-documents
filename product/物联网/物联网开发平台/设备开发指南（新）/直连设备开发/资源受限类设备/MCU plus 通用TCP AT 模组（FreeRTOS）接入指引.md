...
MCU+通用TCP AT模组（FreeRTOS）接入腾讯云物联网开发平台可以分为以下4个步骤：
...
请先参见 [C SDK 移植接入指引](设备开发指南\设备端SDK说明\SDK 使用参考\C SDK 使用参考\AT SDK 使用参考) 进行移植。
...
请参见[C SDK 使用参考](设备开发指南\设备端SDK说明\SDK 使用参考\C SDK 使用参考\AT SDK 使用参考)。

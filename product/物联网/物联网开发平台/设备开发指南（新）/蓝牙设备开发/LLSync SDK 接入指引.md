...
1. 用户需要在物联网开发平台创建设备，定义数据模版。请参见[物联网开发平台产品开发手册](https://cloud.tencent.com/document/product/1081/34738)

2. 将定义好的数据模版拷贝到本地，通过 LLSync SDK 提供的代码生成脚本生成对应的 C 代码
...
|  7   |       log接口       |  在`ble_qiot_configh`中适配log接口   |
|  8   |     定时器接口      |             定时器类接口             |

1. 设备三元信息在物联网开发平台创建设备后获取，用户需要自行存储到设备。
...
|  3   |  ble_send_indication  |  向UUID FFE3写入通知/指示   |
...
用户需要根据自身需求在`ble_qiot_config.h`中对SDK做简单配置后使用。

1. SDK需要存储内部数据到设备内，存储地址在`ble_qiot_config.h`中使用`BLE_QIOT_RECORD_FLASH_ADDR`定义，请用户根据实际情况修改。
2. 用户需要明确定义设备大小端，在`ble_qio_config.h`中定义`__BYTE_ORDER__`。
3. SDK支持两种广播模式，可以选择默认开启广播或按键开始广播。定义`BLE_QIOT_BUTTON_BROADCAST`为1，当设备未绑定时需要按键才开始广播(需要用户在按键动作中添加广播功能)，设备绑定时开机默认广播。定义`BLE_QIOT_BUTTON_BROADCAST`为0时，设备开机默认广播。

##### 例程代码抽取

SDK 已经适配了`nordic 52832` 并提供了例程，存储在`samples/nrf52832`目录下，用户可以通过脚本将指定平台的例程代码抽取出来。
...
|  5   | ble_qiot_advertising_start | 开始广播，广播包内容请参考[LLSync协议说明](https://main.qcloudimg.com/raw/28854b0c56ed0cfd2e06904550d90243.pdf) |
...
|  9   |   ble_gap_disconnect_cb    |                   蓝牙连接端口后调用此接口                   |

## LLSync 协议

请点击下载 [LLSync协议说明](https://main.qcloudimg.com/raw/28854b0c56ed0cfd2e06904550d90243.pdf)

## SDK使用参考

请参见 [LLSync SDK使用参考](设备开发指南\蓝牙设备开发\LLSync SDK使用参考)

...
版权保护服务由腾讯云数据万象提供，原图保护是数据万象提供的源文件保护服务，能够阻止用户对源文件的请求，该功能需要与数据万象的样式功能配合使用，了解样式功能请参见 [样式设置](https://cloud.tencent.com/document/product/460/6936)。开启原图保护功能后，对应存储桶中的图片文件仅能以带样式的 URL 进行访问。
...
2. 单击【存储桶配置】页签，进入存储桶配置页面。
3. 下拉页面找到【原图保护】配置项，单击【编辑】并打开状态按钮。

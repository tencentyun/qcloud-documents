...
文本处理是针对存放在腾讯云对象存储上的文件数据提供的增值服务。
...
文本处理服务支持对象存储 COS 源的 CDN 域名进行内容识别。用户只需在 [数据万象](https://console.cloud.tencent.com/ci) 或 [对象存储](https://console.cloud.tencent.com/cos5) 控制台开启 CDN 加速，或通过 [CDN 控制台](https://console.cloud.tencent.com/cdn) 将加速域名的源站设置为 COS 源或数据万象的域名（使用自有源将万象域名填入）即可。

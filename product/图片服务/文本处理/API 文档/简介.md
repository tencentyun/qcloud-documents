腾讯云文本处理 TP 基于 [数据万象 CI](https://cloud.tencent.com/document/product/460)，使用 XML API，这是一种轻量级的、无连接状态的接口，调用此接口您可以直接通过 HTTP/HTTPS 发出请求和接受响应，实现与腾讯云文本处理后台的交互操作。
...
|名称|	描述|
|---|---|
| APPID	|开发者访问文本处理 TP 服务时拥有的用户维度唯一资源标识，用以标识资源，可在 [API 密钥管理](https://console.cloud.tencent.com/capi) 页面获取|
...
| Bucket | 存储桶，文本处理 TP 中用于存储数据的容器。有关存储桶的进一步说明，请参见 [存储桶概述](https://cloud.tencent.com/document/product/436/13312) 文档|
| Object | 对象，文本处理 TP 中存储的具体文件，是存储的基本实体 |
...
4. 编写一个请求签名算法程序（或使用任何一种服务端 SDK），详情请参见 [请求签名](https://cloud.tencent.com/document/product/1247/45327) 文档。
5. 计算签名，调用 API 执行操作。

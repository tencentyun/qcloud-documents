...
![](https://main.qcloudimg.com/raw/1263feea6b1c96f6a1155065875dd3aa.png)

## 新建 IVR
我们以创建系统默认绑定的“电话体验 IVR”为例介绍 IVR 各模块的功能与连接。
1. 在管理中心左侧导航栏选择【电话客服】-【IVR 管理】，单击【新建】创建 IVR。
![](https://main.qcloudimg.com/raw/ef8fdcc28eb4f6cae02594eaba646266.png)
...
>
（5）添加【分支】模块，可支持用户电话按键转接至指定的技能组或播放指定的提示音。若无需导航功能可跳过该步骤，前往步骤（6）。


| 关键参数 | 说明 | 示例 |
|---------|---------|---------|
| 播放声音 | 指引用户进行相应按键的提示语。 | 转人工坐席接听请按1。 |
| 超时播放 | 用户回复按键超时的提示音。 | 您的回复已超时，请挂断重新拨打。 |
| 错误播放 | 用户按键错误时的提示语。 | 输入错误，请重新输入。错误3次，电话将被自动挂断。 |
| 最大错误次数 | 用户可以输入的最大错误次数，如输入错误超过指定值后播放错误提示音。 | 3 |
| 超时时间 | 用户按键超过设定的值后将播放超时提示音。 | 60 |
| 按键 | 匹配用户的电话按键值。 | 范围为0-9，*号键与#号键。 |
...
| 关键参数 | 说明 | 填写示例 |
|---------|---------|---------|
| 技能组名称 | 可以选择对应分支需要接待的技能组。 | 选择“电话体验技能组”。 |
| 转接人工过程放音 | 客户选择转人工后，在转接过程中的放音。 | 正在为您转接，请稍候。 |
| 转接人工过程循环音乐 | 播放等待音乐。 | 可选择系统提供的“等待音”音频。 |
| 转接人工失败放音 | 当所有人工坐席不在线时播放的提示音。 | 当前非工作时间，欢迎您于工作时间再次联系。 |
| 接通后用户侧播报音 | 人工坐席接听后播报给客户侧的提示音。`${StaffNo}`代表客服工号。 | 客服`${StaffNo}`号为您服务。 |

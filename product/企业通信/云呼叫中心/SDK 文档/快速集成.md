## 快速集成
>?本章节中所描述的**获取 SDK 登录 Token** 的方式仅用于调试，正式环境集成 SDK 请参见 SDK 最佳实践。
...
- 由于腾讯云呼叫中心嵌入实例需要保证登录态, 以便和腾讯云底层协议之间互信调用。请确保浏览器允许访问第三方 Cookie。否则SDK将不能正常登录。腾讯云呼叫中心只使用 Cookie 保存登录态，腾讯云呼叫中心不会做任何跟踪。
- 浏览器版本支持 Chrome 56+，Edge 80+。

## 获取密钥
获取腾讯云主账号访问密钥或者腾讯云子账号访问密钥文档请参考 [主账号访问密钥管理](https://cloud.tencent.com/document/product/598/40488)。
>?生产环境中密钥请妥善保存，避免暴露。
...
## 获取 SDK 登录 Token
获取 Token 接口的相关说明，请查阅 [创建 SDK 登录 Token](https://cloud.tencent.com/document/api/679/49227)。除了文档中使用 SDK 或者接口的方式获取之外，通过 [API Explorer](https://console.cloud.tencent.com/api/explorer?Product=ccc&Version=2020-02-10&Action=CreateSDKLoginToken) 可以更快速获取 Token，并用于调试。

进入 API Explorer 界面之后，使用上图步骤获取到的密钥发送请求，SdkAppId 可以通过查看控制台实例列表看到，一般为140开头，请不要使用体验环境实例的 SdkAppId。
...
效果如下：
![](https://main.qcloudimg.com/raw/a79b7d2b6026356b0b34dc860aad8840.png)
...
呼出事件流程：
![](https://main.qcloudimg.com/raw/9032aeedb6dbaf971eca13dbcbd54384.png)
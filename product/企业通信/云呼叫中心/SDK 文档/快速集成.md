>?本章节中所描述的**获取 SDK 登录 Token** 的方式仅用于调试，正式环境集成 SDK 请参见 [SDK 最佳实践](https://cloud.tencent.com/document/product/679/61238)。
...
进入 API Explorer 界面之后，使用上图步骤获取到的密钥发送请求，SdkAppId 可以通过查看控制台实例列表看到，一般为140开头，请避免使用体验环境实例的 SdkAppId。

...
2. 从上文 JSON 结构中可知，键名为 `0` 和 `2` 的字段由于 `-` 和空格的影响，还存在连接的耦合数据。因此再分别对这两个键进行 `-` 和空格进行 **分隔符** 拆分。 拆分后的
   JSON 结果如下所示：
...
1. 使用 **JSONPATH** 语句处理第一层嵌套的 JSON 结构 `$.kubernetes`，解析模式选择 `JSON`。首先将嵌套 JSON 结构转换为单层 JSON
   结构。测试后结果如下所示：
...
2. 依次处理第二层嵌套结构 `$.kubernetes.annotations` 和 `$.kubernetes.labels`。在处理链中使用 `Map`
   方式选中这两个名称，即可将嵌套格式转换成单层 JSON 格式。处理后如下所示：
...
3. 最后修改相应映射字段的 `key` 为所需名称，并且删去不需要的字段。此时点击 **添加处理链** 后，打开 **处理上层所有结果** 按钮，整理优化后可以参考如下所示：
...
有时 JSON 格式在传输过程中，由于格式或性能等需要，需要转义成字符串格式的形式，此处把这种格式叫做 **Raw JSON**。需要在数据处理中重新将字符串反序列化成 JSON 格式。 此处以
MongoDB 中的 Raw JSON 格式为例，大致结构如下所示：
...
2. 点击 **添加处理链**，使用 MAP 方式选中 key，解析方式选择 `JSON`。这样数据处理就能将 RAW JSON 自动转换成 JSON 形式。解析后如下所示：
...
3. 最后点击 **添加处理链** 后，打开 **处理上层所有结果** 按钮，修改相应映射字段的 `key` 为所需名称，并且删去不需要的字段。整理优化后可以参考如下所示：
...
> ?
> 目前 RAW JSON 只支持解析 MAP 类型的数据。
>
> 当最外层为 List 类型时，例如 `"[\"test1\",\"test2\"]"`，或者 `"[{\"key\":\"value\"}]"`，由于无法解析合适的键值，因此将会提示解析失败。
>

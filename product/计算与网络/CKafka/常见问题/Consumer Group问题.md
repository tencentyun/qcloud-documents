...
- 当Topic设置了ACL以后，在某些实例会出现无法看到消费者组的详情，如果出现无法看详情，可以先检查一下是否有ACL，如果有的话，可以提工单进行处理。
...
1. 一个消费者可以同时订阅多个 Topic。
2. 一个 Topic 里面包含了1到多个分区。
3. 一个分区只能被一个消费者消费。

所以，一个消费组里面，消费者数量上限 = topic1的分区数 + topic2的分区数 +...... + topicN的分区数。

来看一下消费者是怎么定义。消费者在代码层面指的是一个Consumer的对象，一个机器上可以有多个消费者， 比如起多个线程，一个线程里面有一个Consumer，以此类推，如下面代码所示：
...
初始的消费者数量可以根据客户端的资源情况进行初始的部署，然后配置消费者组的堆积告警，如果出现堆积的时候再扩容消费者。配置告警方式参考[配置告警](https://cloud.tencent.com/document/product/597/57244)，配置页面如图所示。
...
其中，1和3两种情况是无法避免rebalance的。正常情况，rebalance在30s内能完成。如果出现长期的 rebalance，需要提单进行处理。

如果是由于心跳超时或者两次poll时间间隔太大导致的问题。可以通过调整如下参数：
...
如果出现一个消费者订阅了很多的 Topic，可以尝试减少 Group 订阅 Topic 的数量。

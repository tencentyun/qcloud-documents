...
3. 升配过程中耗时过长，由于迁移的机器配置的接受最大的消息字节数是1MB，而需要迁移的 broker 配置的是8MB。会导致 broker 无法接收超大消息迁移，从而产生较长的迁移数据耗时，请咨询腾讯云 [在线客服](https://cloud.tencent.com/online-service?from=doc_1149) 处理。
4. 新旧集群升配或迁移过程中，broker ip更新发生异常，导致新集群的 broker ip 拉取数据失败。通过 [查看监控](https://cloud.tencent.com/document/product/597/12167) 可观察到持续一段时间无监控数据，请咨询腾讯云 [在线客服](https://cloud.tencent.com/online-service?from=doc_1149) 处理。

...
4. 在编辑 Topic 窗口中，选择分区数和副本数等信息。
   ...
   - 标签：设置资源标签，关于标签的详细介绍请参考 [标签管理](https://cloud.tencent.com/document/product/597/33355)。
   - 预设 ACL 策略：勾选提前设置好的 ACL 策略，关于 ACL 策略详情请参考[配置 ACL 策略](https://cloud.tencent.com/document/product/597/31528)。
5. 单击**提交**完成 Topic 创建。
...
2. 在实例详情页，单击 **topic管理**标签页，在操作栏单击**发送消息**。
...
3. 单击**确认**，发送消息。在消息发送成功弹窗中点击**消息查询**可以查看刚刚发送的消息。
...
1. 在实例列表页，单击目标实例的“ID/名称”，进入实例详情页。
2. 在实例详情页，单击 **topic管理**标签页，在操作栏单击**生产端连接关系**，查看与 Topic 连接的生产者列表信息。

...
针对业务连续服务、数据可靠性有强需求或是监管需要的场景，CKafka 专业版提供数据同步功能，帮助用户提升业务连续服务的能力，同时提升数据的可靠性。数据同步功能支持数据同步和跨地域容灾两种任务类型，区别如下：
...
**前提条件**：在创建数据同步任务之前，您需要提前创建好目标实例与 Topic。
...
**前提条件**：在实现跨地域容灾之前，您需要选择一个不同于生产环境的地域作为容灾目标地域，在该地域上创建一个和源实例相同规格的 CKafka 实例。具体操作参考 [创建实例](https://cloud.tencent.com/document/product/597/53207)。

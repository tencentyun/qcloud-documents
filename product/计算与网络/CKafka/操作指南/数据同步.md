...
>?当前处于灰度测试阶段，Connector 作为数据传递的管道，本身的数据可靠性依赖上下游的数据可靠性，请参考 [CKafka 数据可靠性说明]( https://cloud.tencent.com/document/product/597/36186)，如对数据可靠性有更高的要求，请通过其他方式进行数据的多次备份。请配置 [监控告警](https://console.cloud.tencent.com/monitor/policylist) 对数据同步情况实时掌握。特别地请注意CKafka Connector 的数据同步流量会占用一定比例的实例峰值吞吐流量的配额。
...
例如 CVM 8核16G 服务器2台主备容灾。产生的CVM基础费用如下

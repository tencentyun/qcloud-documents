消息服务 CKafka 支持用户存储消息的能力，您可以将消息存储到 COS 中，并下载分析。

## 开启消息到COS的功能
在 **topic 管理** 页面，开启访问功能。

针对不同的 topic，选取相应的 COS 中 bucket，则请求消息会自动在 bucket 下创建 instance id+topic id 为名称的文件夹进行存储。选取完成后，单击 bucket 地址可以直接跳转到文件下载页面。

此外，可以根据消息量的大小，选取汇聚消息的时间间隔，时间间隔为 5~60 分钟不等。
...
- 当前 CKafka 消息到COS服务`免费`，COS 存储可享受一定 [免费额度](https://cloud.tencent.com/document/product/436/6240)，提供 50G 免费存储空间。如您的消息量级较大，请及时清理数据。

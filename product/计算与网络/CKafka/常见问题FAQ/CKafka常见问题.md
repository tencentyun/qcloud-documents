...
强烈建议topic创建时选择双副本或三副本存储数据，保障数据可靠性。当前CKafka已经禁止单副本topic的创建，如您账户下有单副本的topic，建议按如下步骤迁移：
1. 创建新的topic，选择相同的分区，选取双副本
2. 生产消息到新的topic中，存量的单副本topic继续被消费
3. 消费完毕后修改消费者配置，订阅新的topic进行消费

...
### CKafka 是否会丢失消息？
1. 开源的 Apache Kafka 不保证不丢消息；CKafka 针对可用性做了优化，腾讯云承诺 CKafka 的可用性超 99.95%，在极端情况下可能丢失少量消息。
2. CKafka 可以通过开启 ACK 或采用多副本的形式，尽量保障不丢失和少丢失消息。
3. 变更集群或升级过程对客户透明，秒级响应，后续可以保障不丢失消息。
4. CKafka 面向的使用场景主要是需要高吞吐、高性能的大数据处理场景，允许消息丢失；若需保障完全不丢失消息，且对性能要求不是非常高的场景，推荐使用 CMQ。

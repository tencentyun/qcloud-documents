...
CKafka兼容0.9系列以及0.10系列的生产/消费接口，因此低版本（如0.8版本）的自建Kafka接入需要进行相应改造才能接入。接下来将分别从生产和消费端说明0.8版本Kafka和高版本的区别，以及改造方式。

## 1. Kafka Producer 兼容说明

### 1.1 概述
Kafka 0.8.1版本中，Producer API被重写。该客户端被官方推荐，其拥有更好的性能以及更多的功能，后续社区将维护新版本的Producer API。

### 1.2 新旧版本Producer API对比
1）	新Producer API写法DEMO
```
...
### 1.3 Ckafka兼容性说明
对于Ckafka而言，0.8.x版本的Producer API都可以顺利接入Ckafka无需改造，我们推荐与社区一样使用新的Kafka Producer Api，其拥有更多配置，功能更加完善。

## 2. Kafka Consumer兼容说明
### 2.1 概述
开源Apache Kafka 0.8版本中提供了两种消费者API，分别为
1. High Level Consumer API （屏蔽配置细节）
2. Simple Consumer API （配置细节支持指定）
Kafka 0.9.x 版本引入了 New Consumer，其融合了Old Consumer两种Consumer API的特性，减轻了zookeeper的负载，因此下文给出了0.8版本Consumer转换为0.9版本New Consumer的方式。

### 2.2 新旧版本Consumer API对比

#### 2.2.1 0.8版本Consumer API介绍
...
#### 2.2.4 Old Consumer 转成 New Consumer
##### 1. New Consumer
```
...
### 2.3 Ckafka兼容性说明
Ckafka与开源社区高版本的Kafka一致，支持重写后的New Consumer API，屏蔽了Consumer客户端与Zookeeper的交互（Zookeeper不再向用户暴露）。New Consumer解决原有与Zookeeper直接交互的 Herd Effect和Split Brain 问题，以及融合了原有Old Consumer的特性，使消费环节更加可靠。

...
## 查看实例
1. 在 CKafka 实例列表页，单击目标实例的“ID/名称”，进入实例详情页。
2. 在实例详情页，单击【基本信息】标签页，可查看实例的配置信息、接入方式、消息保留以及自动创建Topic。
3. 自动创建Topic，此项配置开启将会在服务器上启用主题的自动创建，若使用或获取不存在的主题元数据时将自动使用配置的副本数和分区数创建它。自动创建的Topic也可以在Topic管理中查看。
注意：自动创建的Topic，总数量也根据实例规格有不同的限制
具体参见[计费说明](https://cloud.tencent.com/document/product/597/11745)。

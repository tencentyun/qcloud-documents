...
3. 分别进入 VPC1（广州） 和 VPC2（上海）的各子网路由表中，查看路由情况，云联网路由会默认下发到子网路由表，但不一定生效。若路由下发未生效，需要手动操作停用或启用路由完成最终迁移，共有如下四种迁移场景：
>? VPC1（广州） 和 VPC2（上海）可分别根据需求选择如下任意场景进行操作。
>
 - **场景一**：云联网自动下发的路由与对等连接未产生冲突，则云联网路由生效，迁移步骤如下：
 ...
 - **场景二**：对等连接路由包含云联网自动下发的路由，则云联网路由默认无效。由于路由根据最长掩码匹配原则转发，手动开启云联网路由即可使其生效。
   ![](https://main.qcloudimg.com/raw/6fecb22a1dbbaeb4be60cd05e6adf6f3.png)
   具体迁移步骤如下：
     1. **启用**下一跳为云联网的路由策略（若“启用路由”状态为关闭，需设置为开启）。
     2. 查看对等连接网络流量监控数据，具体操作请参见 [查看网络流量监控数据](https://cloud.tencent.com/document/product/553/18842)。
     >?若无流量从对等连接通过，即可删除对等连接。
 - **场景三**：云联网自动下发的路由包含对等连接的路由，则云联网路由默认无效。由于路由根据最长掩码匹配原则转发，您需要先调整对等连接路由策略目的段，再手动开启云联网路由即可使其生效。具体操作步骤如下：
      1. **启用**下一跳为云联网的路由策略（若“启用路由”状态为关闭，需设置为开启）。
      2. 更改对等连接的路由策略，将目的端网段调整为包含原目的端网段，且子网掩码小于云联网默认下发路由的网段。例如将“10.0.0.0/24”调整为“10.0.0.0/16”。
      ...
      >?若无流量从对等连接通过，即可删除对等连接。
 - **场景四**：云联网自动下发的路由等于对等连接的路由，则云联网路由默认无效，且无法直接启用。由于路由根据最长掩码匹配原则转发，您需要先调整对等连接路由策略目的段，并重新将 VPC 关联至云联网，然后手动开启云联网路由即可使其生效。
![](https://main.qcloudimg.com/raw/e237b4320919d8b68f2b89eb0d685623.png)
	1. 更改对等连接的路由策略，将目的端网段调整为包含原目的端网段，且子网掩码小于云联网默认下发路由的网段。例如将“10.0.0.0/24”调整为“10.0.0.0/16”。
	2. 将 VPC 与云联网解绑并重新关联，具体操作请参见 [解除网络实例关联](https://cloud.tencent.com/document/product/877/18757) 和 [关联网络实例](https://cloud.tencent.com/document/product/877/18747)。
	3. **启用**下一跳为云联网的路由策略（若“启用路由”状态为关闭，需设置为开启）。
   调整后，流量将优先从下一跳为云联网的路由转发，原下一跳为对等连接的路由策略失效。
	4. 查看对等连接网络流量监控数据，具体操作请参见 [查看网络流量监控数据](https://cloud.tencent.com/document/product/553/18842)。
    >?若无流量从对等连接通过，即可删除对等连接。

## 快速开始
本文介绍如何使用批量计算控制台提交一个作业，完成3ds Max 2018图片渲染，并导出渲染图片。具体操作步骤如下。
### 制作自定义镜像
...
- 渲染素材较小时，可以将全部素材压缩成gzip包，[上传至对象存储](https://cloud.tencent.com/document/product/436/6233)。
- 渲染素材较大时，可以存放在文件存储上。

### 创建任务模板
1. 登录批量计算[控制台](https://console.cloud.tencent.com/batch/task)，单击左侧导航栏“任务模板”选项，选择目标地域后，点击【新建】按钮。
2. 配置基本信息。
  * 名称：rendering
  * 描述：3ds Max 2018 Demo
  * 资源配置：S1.LARGE8（4核8G），更多配置点击“云主机详细配置”
  * 资源数量：并发渲染数，比如 1 台
  * 超时时间：默认值
  * 重试次数：默认值
  * 镜像：自定义镜像标识符，比如img-i64lx84h
...
4. 配置存储映射。
 ...
  * 输出路径映射-COS CFS路径：格式参考 [COS、CFS路径填写](https://cloud.tencent.com/document/product/599/13996)
...
3. 选中“任务流”左侧 “rendering” 任务，移动鼠标将任务放置到右侧画布中。
...
这个是一个简单的渲染例子，它是一个单实例的作业，仅仅是向用户展示最基本的能力，您可以根据控制台使用指南继续测试 Batch 更高阶的能力。

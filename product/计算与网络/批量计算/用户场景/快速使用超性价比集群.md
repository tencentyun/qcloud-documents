## 如何快速创建集群？
使用批量计算的计算环境能力，可以轻松高效的维护资源集群，Batch 的计算环境可以简单的对应常规的集群概念，下面的例子介绍如何使用计算环境能力来快速创建/销毁一个超性价比资源集群，目前计算环境仅支持命令行调用，请先参照开始准备安装命令行工具。
...
下面的例子在广州二区快速创建包含 10台 BS1.LARGE8（批量计算通用型 CPU 4核 内存 16GB）类型集群
...
qcloudcli batch CreateComputeEnv --Version 2017-03-12 --ComputeEnv '{
    ...
    "EnvData": {                            // 具体配置（可参照 CVM 创建实例说明）
        ...
        "ImageId": "img-i91njcmx",          // 计算环境内CVM 镜像 ID（可替换成自定义镜像）
        "LoginSettings": {

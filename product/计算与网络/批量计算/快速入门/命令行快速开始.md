...
请参照 [命令行工具 - 前置准备](https://www.qcloud.com/document/product/599/10548) 里第三章节 **『准备 COS 目录』**，创建对应 COS Bucket 和 子文件夹。
...
``* 3. 详细 Job 配置说明请参考 `` [作业配置说明](https://www.qcloud.com/document/product/599/11040) ``。``

```
qcloudcli batch SubmitJob --Version 2017-03-12  --Job '{"JobName": "TestJob",  "JobDescription": "for test", "Priority": "1", "Tasks": [{"TaskName": "Task1",  "TaskInstanceNum": 1,  "Application": {"DeliveryForm": "LOCAL", "Command":  "python -c \"fib=lambda n:1 if n<=2 else fib(n-1)+fib(n-2); print(fib(20))\" "},  "ComputeEnv": {"EnvType":  "MANAGED", "EnvData": {"InstanceType": "S1.SMALL1",  "ImageId": "img-i91njcmx","SystemDisk": {"DiskType": "CLOUD_BASIC",  "DiskSize": 50 }, "DataDisks": [{"DiskType":  "CLOUD_BASIC", "DiskSize": 50 } ] }  }, "RedirectInfo": {"StdoutRedirectPath": "待替换", "StderrRedirectPath":   "待替换"}, "MaxRetryCount":  1, "Authentications": [{"Scene": "COS", "SecretId":   "待替换", "SecretKey":  "待替换"} ] } ] }' --Placement '{"Zone": "ap-guangzhou-2"}'
```
...
* **更简单的操作方法**：Batch 的能力强大，配置项较多，通过脚本来调用会更加简便快捷，从 [前置准备>>>](https://www.qcloud.com/document/product/599/10548) 和 [1_简单开始>>>](https://www.qcloud.com/document/product/599/10551) 开始尝试这种方式。

* **执行远程代码包**：Batch 提供**自定义镜像 + 远程代码包 + 命令行**的方式，在技术上全方位的覆盖您的业务需要，详细查看 [2_执行远程代码包>>>](https://www.qcloud.com/document/product/599/10552)。

* **远程存储映射**：Batch 在存储访问上进行优化，将对远程存储服务的访问简化为对本地文件系统操作，详细查看 [3_远程存储映射>>>](https://www.qcloud.com/document/product/599/10983)。

## 操作场景
本文介绍如何使用命令行来提交一个简单的作业，文中示例为使用 Python 实现的对斐波那契数列求和，Python 代码由任务中 Application 参数的 Command 字段指定，返回结果直接输出到任务配置的 stdout 输出地址。
...
```
中指定在广州二区申请资源，您可以根据 TCCLI 中配置的默认地域，选择相应的可用区并申请资源。
...
```
- 执行以下命令，通过 DescribeJob 来查看刚才提交的任务信息。
```
...
* **执行远程代码包**：Batch 提供**自定义镜像 + 远程代码包 + 命令行**的方式，在技术上全方位的覆盖您的业务需要，详情请参考 [执行远程代码包](https://cloud.tencent.com/document/product/599/10552)。

...
### 硬盘有分区情况
#### 确认分区情况
首先执行 `fdisk -l /dev/vdb` 确认磁盘分区为 `GPT` 格式，其中 `/dev/vdb` 是要进行扩容盘的盘符：
![](//mc.qcloudimg.com/static/img/b8af55b537f0e2b87644aacf837c082f/image.png)

如上图所示，磁盘`vdb`大小为`4295G`，有一个磁盘分区`vdb1`，分区格式为` GPT ` 格式，执行命令`df  -h  |  grep  /dev/vdb1 `查看分区上的文件系统大小：
![](//mc.qcloudimg.com/static/img/b96c54176bfd8638a0eac9e89e90ed95/image.png)

可见整块磁盘大小为`4T`，磁盘分区`vdb1`为`2T`，此时需要将磁盘分区从` 2T` 扩容到 `4T`。

首先须确保磁盘处于`umount` 状态，执行 `umount /dev/vdb1`命令：
![](//mc.qcloudimg.com/static/img/6ed11b4fa2bda5b1d58db97b5b09f1d9/image.png)
#### 扩容已有分区
执行 `parted /dev/vdb`命令：
![](//mc.qcloudimg.com/static/img/c8426e4dbc1808c306323f47d07ba05c/image.png)

输入 `print`：
![](//mc.qcloudimg.com/static/img/8130c693b7bb5ae2a16c01aca36ce551/image.png)

确认磁盘存在一个磁盘分区，分区号为` 1`，分区大小为` 2T`，分区已有文件系统，类型为 `xfs`。

在原有分区 `/dev/vdb1` 上进行扩容分区大小，需要先将原分区删除，然后再新建分区，删除原分区前需要做如下操作：
执行 `unit s`，以扇区为单位显示磁盘分区起始位置：
![](//mc.qcloudimg.com/static/img/616d77de8865194261943f0a0e14683c/image.png)

如上图所示，执行 `unit s` 前，分区起始位置 `Start` 对应值为 `17.4kb`，执行 `unit s` 后，分区起始位置以扇区为单位显示，`Start` 对应值为 34。必须记住原分区的 `Start` 值，删除原分区并新建分区后，`Start` 值必须与原有分区 `Start`值保持一样，否则数据将会丢失。

执行 `rm [磁盘分区号]`，`Number` 所对应的值即为磁盘分区号，上图所示 `Number` 值为 `1`，这里执行 `rm 1` 即可。完成后执行 `print` 确认磁盘分区已删除：
![](//mc.qcloudimg.com/static/img/81bc35ad7a3628db8b9f9901be86050d/image.png)

执行 `mkpart [磁盘分区名]  [分区start 位置]  [分区end 位置]`命令，其中：
- 磁盘分区名：自行命名，简单字符串描述即可
- 分区start 位置：必须与原有分区 `Start` 值保持一致，本例为 `34`
- 分区end 位置：新分区的终止位置。比如：想把新分区扩容到 4T，那 `end` 值为 `4096G`

本例输入 `mkpart  gpt2  34  4096G`命令后，输入 `Ignore`，新分区就创建好了。再执行 `print`，确认已成功创建新分区，最后执行`quit` 退出 `parted` 命令：

![](//mc.qcloudimg.com/static/img/019fe2fd4a484d18bdc044fb6b9cea98/image.png)

执行 `mount  /dev/vdb1  [分区挂载点目录]`重新挂载：
![](//mc.qcloudimg.com/static/img/b7532f9c67f793690ca0f81cca7638c1/image.png)
#### 扩容文件系统
接下来扩容文件系统，执行 `xfs_growfs  /dev/vdb1` 扩容新分区上的文件系统大小，执行此命令将会自动扩展`XFS`文件系统到磁盘分区最大可用大小。执行 `df -h` 确认文件系统已扩容到 `4T`，最后可以到挂载点目录下确认原有数据是否完整：：
![](//mc.qcloudimg.com/static/img/c848a060d35db37d03d7a8e12ffff82c/image.png)

### 硬盘无分区情况
 如果硬盘没有做分区直接格式化为`xfs`文件系统时，运行 `xfs_growfs  /dev/vdb` 命令将直接把文件系统扩容至最大容量。

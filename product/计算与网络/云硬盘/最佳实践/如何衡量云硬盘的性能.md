...
## 测试场景
本文使用 FIO 来测试磁盘性能，包括压力测试和验证，建议在测试过程中配合使用 libaio 的 I/O 引擎。
请您自行安装 FIO 和 libaio。

>!
- 请不要在**系统盘**上进行 FIO 测试，避免损坏系统重要文件。
- 请确保被测数据盘上**无数据**，否则将会破坏原有数据。
- 需确认`/etc/fstab`文件配置项中**没有**被测硬盘的挂载配置，否则将导致云服务器启动失败。

测试对象建议：
- 建议在空闲的数据盘上进行 FIO 测试，并在测试完后重新制作被测硬盘的文件系统。
- 测试硬盘性能时，推荐直接测试裸数据盘（如 /dev/vdb），避免底层文件系统元数据损坏导致数据损坏。
...
```
测试结果如下图所示：
![](https://main.qcloudimg.com/raw/208d43390192b455be6531589e7011c3.png)

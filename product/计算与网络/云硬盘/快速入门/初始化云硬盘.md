...
>- 本文以使用 EXT4 文件系统为例。
>- Linux 云服务器重启或开机后，不会自动挂载数据盘。您可参考 [步骤9](#Step09) -  [步骤15](#Step15) 设置磁盘开机自动挂载。
>
...
9. [](id:Step09)您可通过以下步骤，设置磁盘开机自动挂载。
...
10. 执行以下命令，备份 `/etc/fstab`  文件。以备份到  /home 目录下为例：
```
cp -r /etc/fstab /home
```
11. 执行以下命令，使用 VI 编辑器打开 `/etc/fstab` 文件。
```
vi /etc/fstab
```
12. 按 **i**，进入编辑模式。
13. 将光标移至文件末尾，按 **Enter**，添加如下内容。
```
...
```
14. 按 **Esc**，输入 **:wq**，按 **Enter**。
保存设置并退出编辑器。
15. [](id:Step15)执行以下命令，检查 **/etc/fstab** 文件是否写入成功。
```
mount -a 

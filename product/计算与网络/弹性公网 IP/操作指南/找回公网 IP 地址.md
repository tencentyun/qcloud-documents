若您误操作释放或退还了公网 IP 地址（包含 EIP 和普通公网 IP），可以在 EIP 控制台找回，找回后的公网 IP 为 EIP。

## 使用限制
找回公网 IP 地址具有以下使用限制：
- 仅支持找回您已使用过、且当前未分配给其他用户的公网 IP 地址。
- 每个账户单个地域申请找回 IP 不超过3次/月。
...
- 对于带宽上移账户，找回的 EIP 默认计费模式为按流量计费，带宽上限为 5 Mbps，计费模式和带宽上限可在重新申请成功后修改。
...
4. 在弹性公网 IP 的 [列表页](https://console.cloud.tencent.com/cvm/eip?rid=1) 中，查看已找回的公网 IP（找回后的公网 IP 为 EIP）。

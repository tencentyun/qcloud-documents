...
EIP 的使用生命周期包括申请 EIP、绑定云资源、解绑云资源和释放 EIP。
![](https://main.qcloudimg.com/raw/6394a72972c3924ce1ab4f326994e808.png)

1. [申请 EIP](#step1)：根据账户类型和实际使用需求，申请 EIP 资源。
2. [EIP 绑定云资源](#step2)：将 EIP 绑定到 CVM 实例上，通过 EIP 与公网通信。
3. [EIP 解绑云资源](#step3)：将 EIP 解绑云资源后，您可以将该 EIP 与其他云资源绑定。未绑定云资源的 EIP 会产生 IP 资源费。
4. [释放 EIP](#step4)：若不再需要使用该 EIP，可将其释放，释放后不再产生费用。

## 操作步骤
<span id="step1"></span>
### 申请 EIP
1. 登录 [EIP 控制台](https://console.cloud.tencent.com/cvm/eip)。
2. “弹性公网IP”页面顶部选择【地域】，然后单击【申请】。
3. 在“申请弹性公网IP”对话框中，按照您的账户类型，进行以下操作：
<div class="codetab">
...
<li>高级选项：如需添加标签可在此进行添加，可通过标签进行权限管理。</li>
</ul></p>
</div>
...
>?建议您及时为处于未绑定状态的 EIP 绑定云资源，节省 IP 资源费。IP 资源费按小时计费，精确到秒级，不足一小时，按闲置时间占比收取费用，因此请及时 [EIP 绑定云资源](#step2)。
>
![](https://main.qcloudimg.com/raw/9492216f61d10704015dac0cf217bd01.png)

### <span id="step2" />EIP 绑定云资源
1. 登录 [EIP 控制台](https://console.cloud.tencent.com/cvm/eip)，并选择 EIP 所在地域。
2. 目标 EIP 右侧操作列中，选择【更多】>【绑定】。
...
## EIP 解绑云资源
1. 登录 [EIP 控制台](https://console.cloud.tencent.com/cvm/eip)，并选择 EIP 所在地域。
2. 目标 EIP 右侧操作列中，选择【更多】>【解绑】。
3. 在“解绑弹性公网IP”窗口中，确认解绑信息，并单击【确定】。
...
## 释放 EIP
1. 登录 [EIP 控制台](https://console.cloud.tencent.com/cvm/eip)，并选择 EIP 所在地域。
2. 目标 EIP 右侧操作列中，选择【更多】>【释放】。
3. “确定释放所选弹性公网IP”窗口中，勾选【确定释放以上IP】，单击【释放】。

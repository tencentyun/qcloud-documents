## 操作场景
当用户在云函数中访问数据库、微信公众号的 API 接口或其他第三方的服务时，可以开启云函数的固定公网出口 IP 功能，实现云函数网络配置的控制与管理。

云函数的固定公网出口 IP 功能具有以下特点：
- 当云函数启用固定公网出口 IP 功能后，该云函数将会获得一个随机分配的弹性公网 IP。该云函数访问公网所使用的流量，将会统一基于该弹性公网（EIP）进行转发。
- 当在云函数同时开启公网访问、内网访问时，访问公网的流量会基于 EIP 转发，访问内网的流量会基于私有网络转发。

## 使用限制
- 弹性公网 IP 会基于命名空间共享。
 - 同一命名空间下，已开启固定公网出口 IP 功能的云函数将共享弹性 IP。
 - 同一命名空间下的云函数需更换固定出口 IP 时，所有云函数需关闭固定公网出口 IP 功能。再次开启此功能时，会随机产生一个新的弹性公网 IP。
- 弹性公网 IP 会基于私有网络的子网共享。
 某个云函数配置了私有网络，且同时开启了固定公网出口 IP 功能，则该云函数会获得一个随机分配的弹性公网 IP。同一私有网络子网下的云函数在开启固定出口 IP 功能时，会共享此固定出口 IP。
	
您可参考下表，理解弹性公网 IP 和云函数的绑定关系：
<table>
	<tr>
	<th>网络配置 </th>
	<th>命名空间 A/函数 a</th>

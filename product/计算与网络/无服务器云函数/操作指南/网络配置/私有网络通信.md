...
腾讯云云函数默认部署在公共网络中。若果您想使云函数具备以下功能：
- 内网服务访问：访问内网中的资源。例如 TencentDB、Redis、Kafka 等，并在访问时确保数据安全，连接安全。
- 访问控制：公网访问可控制出口地址唯一。可通过开启 “固定公网出口 IP” 功能或者给 VPC 配置 NAT 网关两种方式实现。

请参考本文对云函数进行配置，获取在 VPC 网络中的通信能力。
...
## 注意事项
在进行私有网络配置时，需注意以下几点：
- 部署在 VPC 中的云函数默认隔离外网。如果您想让云函数同时具备内网访问和外网访问能力，可通过以下两种方式实现：
 - 配置云函数公网访问能力，请参考 [固定公网出口 IP](https://cloud.tencent.com/document/product/583/38198)。
 - VPC 添加 NAT 网关，请参考 [私有网络中配置 NAT](https://cloud.tencent.com/document/product/583/19704)。
- 云函数当前不支持对接到基础网络里的资源。
...
### 使用 VPC 网络
在云函数完成内网访问配置，并开始使用 VPC 网络时，云函数将从当前独立的网络环境切换至配置的 VPC 中。云函数启动时，将占用用户 VPC 子网中的 IP 地址作为云函数运行环境的 IP 地址。为了降低云函数对子网的 IP 地址占用，运行中的云函数实例会共享一组 proxy 对，并根据网络带宽使用率对 proxy 对进行扩缩容。
...
在云函数完成内网访问配置后，若需要在 VPC 网络中仍然使用域名方式访问网络内的自建服务，通常需要使用自定义 `name server` 实现域名解析。

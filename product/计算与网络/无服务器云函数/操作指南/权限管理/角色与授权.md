...
运行角色用于授权函数代码在运行时有读取，操作资源。函数在创建时为用户提供了“SCF默认运行角色”，又称“QCS_SCFExcuteRole”。
...
您除了可以直接使用默认运行角色外，也可以自行创建运行角色。

1. 打开访问管理控制台的["角色"页面](https://console.cloud.tencent.com/cam/role)，选择“新建角色”
2. 在弹出的“选择角色页面”选择“腾讯云产品服务”，以授权云服务通过角色使用您的云资源
3. 在支持角色的服务中勾选“无服务器云函数”，选择“下一步”
4. 在“配置角色策略”页选择您需要的策略，选择“下一步”
5. 添加“角色名称”、“角色描述”即可创建完成新角色

除此之外，您还可以在创建函数的流程中新建运行角色。设定角色载体为无服务器云函数，并完成策略配置后，即可通过函数创建或修改配置时使用自定义角色。通过使用自定义角色，您可以更灵活且自主的控制您的函数代码在运行时获取的权限和操作的资源。

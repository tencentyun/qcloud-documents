...
此角色当前所拥有 QcloudAccessForScfRole 策略，用以：
- 配置 COS 对象存储触发器时向 Bucket 配置中写入触发配置信息。 
- 读取 COS 对象存储 Bucket 中的触发器配置信息。 
- 在使用 COS 对象存储更新代码时，从 Bucket 完成代码 zip 包的读取操作。 
- 配置 API 网关触发器时，完成 API 网关的服务、API创建，以及服务发布等操作。 

>! 此角色曾经所拥有以下策略（已被 QcloudAccessForScfRole）替代以提供更精细化的授权：
...
SCF 提供预设策略，可用于授权访问云函数和其他关联资源。云函数会根据需要更新预设策略，以确保您有权在云函数发布新功能后可以访问它们。您也可以创建自己的策略来限制用户权限的范围。

QcloudSCFFullAccess – 授予操作云函数和其他相关资源的全部访问权限。

QcloudSCFReadOnlyAccess – 授予操作云函数和其他相关资源的只读访问权限。
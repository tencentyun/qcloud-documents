...
您在创建云函数（SCF）时，可能会需要操作部分其他产品的权限，例如 COS 触发器创建和删除所需的 COS 权限、API 网关触发器创建和删除所需的 API 网关权限、COS 代码文件的 zip 包读取的权限等。
...
>!
>- 此角色曾经所拥有的以下策略，已被 `QcloudAccessForScfRole` 替代，以提供更精细化的授权：
>  `QcloudCOSFullAccess`、`QcloudCOSBucketConfigWrite`、`QcloudCOSBucketConfigRead`、 `QcloudCOSDataReadOnly`、 `QcloudAPIGWFullAccess`。
...
![](https://main.qcloudimg.com/raw/c998191d9265790f1ae99ce472a31b49.png)
>!如果您当前是子用户/协作者，则需要主账号按照以上三个步骤进行授权。授权完成后，主账号和子用户登录均可以使用云函数服务。
>
...
请参考 [授权操作指南](<https://cloud.tencent.com/document/product/598/17289>) 配置以下两条预设策略来快速授权：
<table>
	<tr>
	<th>预设策略</th><th>功能</th>
	</tr>
...
SCF 于2019年11月完善了预设策略权限，针对预设策略 `QcloudSCFFullAccess` 和 `QcloudSCFReadOnlyAccess` 完成如下修改：

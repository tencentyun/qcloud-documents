...
腾讯云无服务器云函数 SCF 通过使用访问管理（Cloud Access Management，CAM）的角色能力，完成了服务和用户资源间的权限打通。通过使用配置角色，云函数服务可以在配置流程中访问用户资源；通过使用运行角色，云函数服务能为运行代码完成角色临时授权的申请，便于代码通过角色的授权机制实现权限打通和资源访问。

关于角色和授权的详细内容请见 [角色与授权](https://cloud.tencent.com/document/product/583/32389) 部分。

...
> ! 目前，SCF 云函数只支持读取 bootstrap 或 scf_bootstrap 作为启动文件名称
...
#### 常见错误定位
执行文件 bootstrap 作为容器启动命令，必须保证容器可以正常启动运行，执行代码逻辑，因此，请确保您的启动命令写法正确，如遇到 `405` 错误码信息，通常为执行文件无法正常运行导致，请确保您的启动文件写法正确。

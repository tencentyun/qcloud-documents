...
## Serverless Framework 命令行部署
>?在使用 Serverless Framework 命令行工具之前，可以通过 [命令行安装](https://cloud.tencent.com/document/product/1154/42990) 方法完成命令行的安装和配置。
>
您可通过 SLS 命令行执行 `sls deploy` 命令，即可部署函数。

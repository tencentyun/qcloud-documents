...
### 更多创建方式
您可根据实际需求，选择更多方式创建函数：
- 使用 Serverless Cloud Framework CLI 命令行工具，可参考 [使用 CLI 创建函数](https://cloud.tencent.com/document/product/583/37510)，以及函数的调用和部署。
...
### Serverless Cloud Framework CLI 命令行更新函数配置
        ...
        parameters:
          ...
          endpoints:
            - path: /index
              method: GET
```
2. 修改完成后，通过 Serverless Cloud Framework CLI，执行 `sls deploy` 命令部署函数。
...
### Serverless Cloud Framework CLI 更新函数代码
在本地修改函数代码后，通过 Serverless Cloud Framework CLI 执行 `sls deploy` 命令，即可部署函数并完成代码更新。

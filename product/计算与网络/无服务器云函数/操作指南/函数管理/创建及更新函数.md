        ...
        parameters:
          ...
          endpoints:
            - path: /index
              method: GET
```
2. 修改完成后，使用 Serverless Framework CLI 命令行执行 `sls deploy` 命令部署函数。
...
### Serverless Franekwork CLI 命令行更新函数代码
在本地修改函数代码后，通过 Serverless Franekwork CLI 执行 `sls deploy` 命令即可部署函数并完成代码更新.

同时，serverless framework 的开发模式支持函数的同步更新，详情请查看[开发模式与云端调试](https://cloud.tencent.com/document/product/1154/43220)

...
如需修改函数配置，可直接修改 `template.yaml` 文件，然后执行 `scf deploy` 命令部署函数。更多关于部署函数请参见 [打包部署](<https://cloud.tencent.com/document/product/583/33451>)。
>!
...
1. 通过 [云函数控制台](https://console.cloud.tencent.com/scf)，单击左侧导航栏的【函数服务】。
2. 在主界面上方选择期望更新的函数所在地域。在列表页中单击期望更新的函数，进入函数详情页面。
...
### SCF CLI 命令行更新函数代码
在本地修改函数代码后，执行 `scf deploy` 命令即可部署函数并完成代码更新，更多关于部署函数请参见 [打包部署](<https://cloud.tencent.com/document/product/583/33451>)。

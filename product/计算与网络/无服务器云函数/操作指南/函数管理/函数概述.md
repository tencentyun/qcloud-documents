...
两段式的执行方法，前一段指向代码包中不包含后缀的文件名，后一段指向文件中的入口函数名。需要确保代码包中的文件名后缀与语言环境匹配，如 Python 环境为 .py 文件，Node.js 环境为 .js 文件。 更多执行方法相关说明，请见执行方法详情说明

HTTP Service通过执行入口文件来启动您的服务。
例如您的入口文件为index.js，那么HTTP Service会用 node index.js命令来执行启动。

需要注意的是，您的服务需要监听环境变量TENCENTCLOUD_SERVER_PORT所指定的端口。
...
- 函数类型：SCF 支持 `event` 函数和 `Web` 函数两种函数类型。
  ...
  - `event` 函数由指定格式的事件触发，比如定时触发事件、COS 触发事件等，事件结构详情请查看 [触发器](https://cloud.tencent.com/document/product/583/9705)。
...
- [异步调用配置](https://cloud.tencent.com/document/product/583/41138)：通过该配置设置异步调用场景下的重试策略，还可以配置 [死信队列](https://cloud.tencent.com/document/product/583/41138#死信队列文档) 收集错误事件信息、分析失败原因。

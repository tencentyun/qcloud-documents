...
2. 在主界面上方选择期望创建函数的地域。
3. 点击【新建】按钮，进入函数创建流程。
4. 可选择使用【空白函数】或选择使用【函数模板】来新建函数。
5. 如果使用【空白函数】，通过填写必填的函数名称、运行环境来完成函数的创建。
6. 如果使用【函数模板】，通过填写必选的函数名称即可创建函数，运行环境等配置依赖函数模板中的配置来创建。

## 通过腾讯云命令行创建函数

在使用腾讯云命令行前，可以通过[命令行安装及配置](https://cloud.tencent.com/document/product/440/6176)方法完成命令行的安装和配置。

通过命令行中的 `tccli scf CreateFunction` 命令即可完成函数创建。

### 通过本地 zip 包完成函数创建

如下示例为通过本地 zip 包完成函数创建。

首先将命名为 hello.zip 的 zip 包转变为 base64 编码并 urlencode ：
...
由于通过 COS 创建函数，需要将相应 Bucket 授权给 SCF 云函数平台，以便云函数平台可以通过授权访问到代码文件并可以下载到平台。

您可以通过控制台进行一次函数创建并使用 Bucket 上传代码，会自动在前台完成授权操作。如果未通过控制台创建过函数，你可以自行设置 Bucket 授权以便云函数平台可以访问到代码文件，详细操作可见权限管理。

...
在创建别名或进行流量配置调整时，可通过配置指向两个版本，实现流量在版本间按照一定的规则路由。目前支持 [按权重随机路由](#RandomRoutingByWeight) 和 [按规则路由](#RoutingByRule) 两种路由方案，请按需并参考以下步骤进行函数的流量路由配置。


## 操作步骤
### 按权重随机路由<span id="RandomRoutingByWeight"></span>
使用按权重配置随机路由时，在选择两个版本后，可以分别为两个版本配置百分比的权重。则针对此别名进行调用时，将按百分比在两个版本间随机路由。本文以在创建别名时配置为例，步骤如下：
...
使用按规则配置路由时，在选择两个版本后，可以针对其中一个版本配置命中规则，则另一个版本将在请求未命中规则时被调用。目前的规则语法包括以下三部分：
* [匹配 Key](#matchKey)
* [匹配方法](#matchMethod)
...
#### 匹配方法<span id="matchExpression"></span>
匹配时通过方法与表达式进行对比，目前支持的匹配方法有 `exact`，`range`。
* `exact`：精确匹配，在使用 `exact` 方法时，匹配表达式需为字符串。通过匹配 key 读取到的值与表达式精确相等时，即为命中规则。
* `range`：范围匹配，在使用 `range` 方法时，匹配表达式需为 (a,b) 或 [a，b] 的写法，其中a，b 要求为整数。通过匹配 key 读取到的值为整数，且在表达式定义的区间中时，即为命中规则。

#### 匹配表达式<span id="matchMethod"></span>
匹配设定值即为命中，表达式写法可参见 [匹配方法](matchExpression) 说明。



####  示例
您可结合以下示例，并参考 [操作步骤](#Steps) 开始配置：

...
在创建别名或进行流量配置调整时，可通过控制台控制流量指向两个函数版本，实现流量在版本间按照一定的规则路由。目前支持**按权重随机路由**和**按规则路由**两种路由方案：
 - 当您希望任意两个版本按设定的百分比进行随机路由时，可进行 [按权重随机路由](#RandomRoutingByWeight) 操作。
 - 当您只想对一个版本配置命中规则，另一个版本不配置或者设置为未命中时，可进行 [按规则路由](#RoutingByRule) 操作。
...
主要参数信息如下：
 - **路由方法**：选择【按规则路由】。
 - **版本权重设置**：请结合以下示例，按需配置：
    1. 选择需配置命中规则的版本，例如版本2，匹配规则为 `invoke.headers.User exact Bob`。选择不需要配置命中规则或配置为未命中的版本，例如版本1。如下图所示：
![](https://main.qcloudimg.com/raw/d8b267b2c9d0234354d645c59672d464.png)
在实际请求时，如果在通过 `invoke` 接口调用函数别名时，设置了 `routingKey` 参数为 `{"User":"Bob"}`，则此次执行将使用版本2的代码和配置完成。如果未设置 `routingKey` 参数或 `routingKey` 为其他值，则将使用版本1的代码和配置完成执行。
    2. 选择需配置命中规则的版本，例如版本3，匹配规则为 `invoke.headers.userHash range [1,50]`。选择不需要配置命中规则或配置为未命中的版本，例如版本2。如下图所示：
    ![](https://main.qcloudimg.com/raw/6be8c469faf1dd13b4a2988ef27c5c08.png)
		在实际请求时，如果在通过 `invoke` 接口调用函数别名时，设置了 `routingKey` 参数为 `{"userHash":30}`，此次执行将使用版本3的代码和配置完成。如果未设置 `routingKey` 参数或 `routingKey` 为其他值，例如 `{"userHash":80}`，则将使用版本2的代码和配置完成执行。

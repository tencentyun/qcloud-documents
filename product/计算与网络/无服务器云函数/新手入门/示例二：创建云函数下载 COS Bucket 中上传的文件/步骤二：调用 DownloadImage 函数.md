...
## 使用 COS 上传文件测试
1. 登录腾讯云控制台，导航至【对象存储服务】，在 Bucket 列表中选择第一步中创建的`TestBucket`Bucket，单击【上传文件】按钮，上传一张[示例图片:testimage.jpeg](https://mc.qcloudimg.com/static/img/e683e8627510ec92344329c5219c1939/testimage.jpeg)（您可将此示例图片下载到本地并用作测试）。

2. 导航至【无服务器云函数】，选择刚刚创建的`DownloadImage`。

3. 单击【日志】选项卡，观察是否有刚刚上传图片后的函数运行日志。日志中应有本地下载下来的图片的日志，类似：

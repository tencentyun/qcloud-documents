...
6) 点击【下一步】按钮，进入函数代码编辑页，默认选择【在线编辑】，并在【模版】中选择 `cos-get-object-python` 模版。此时，执行方法和代码将填入模版的默认值：
...
region = 'gz' #替换为当前地域，广州：gz、上海：sh、北京：bj
...
8) 点击底部【完成】按钮，此时控制台会自动生成代码程序包并上传至 SCF 平台以创建云函数。您可以点击云函数列表页中刚刚创建的`GetObjectType`函数进入云函数详情页。

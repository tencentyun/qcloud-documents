...
<tr>
<td>Command</td>
<td>填写容器的启动命令。<br>参数传入规范，建议以“双引号 + 逗号”做为参数的分割标识，填写容器的启动命令，例如 <code>"nginx"</code>。<br>该参数为可选参数，如果不填写，则默认使用镜像中的  Entrypoint / CMD。</td>
</tr>
<tr>
<td>Args</td>
<td>填写容器的启动参数。<br>参数传入规范，建议以“双引号+逗号”做为参数的分割标识，例如 <code>"-args","value"</code>。该参数为可选参数，如果不填写，则默认使用镜像中的 CMD。</td>
</tr>
</tbody></table>
4. 单击【完成】，即可创建函数。

...
在使用如下测试事件进行测试运行时，您可在日志服务的字段日志中检索 `key1` 和 `key2` 两个字段。您也可以在日志服务中启用键值索引，并使用 `key1`、`key2` 作为关键字进行检索，详情可参见 [日志服务索引配置](https://cloud.tencent.com/document/product/614/50922)。

...
### 进入开发模式[](id:joinDev)
在项目下执行 `serverless-cloud-framework dev` 命令，可以进入项目的开发模式。示例如下：
>!目前 `serverless-cloud-framework dev`  仅支持 Node.js 10.15 及 12.16 运行环境。
>
```plaintext
$ serverless-cloud-framework dev

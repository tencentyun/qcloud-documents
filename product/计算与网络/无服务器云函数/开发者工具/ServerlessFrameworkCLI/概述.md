...
### 云函数 SCF 组件
通过使用 Serverless Framework 的云函数 SCF 组件，您可快速打包部署您的云函数项目。该组件通过使用 [Tencent Serverless Framework](https://github.com/serverless/components/tree/cloud)，基于云上 Serverless 服务（云函数及触发器等），可实现“0”配置、便捷开发、极速部署云函数，同时支持丰富的配置扩展，提供了目前最易用、低成本且弹性伸缩的云函数的开发、配置及部署能力。


## 快速入门
您可通过 [使用 CLI 创建函数](https://cloud.tencent.com/document/product/583/37510)，快速上手 Serverless Framework CLI。

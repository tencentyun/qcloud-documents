## 简介
在 [函数操作](https://cloud.tencent.com/document/product/583/45352) 中介绍了如何使用 Serverless Framework 创建云函数。对于 Serverless Framework 框架本身而言，即通过云函数组件部署了一个 Serverless 单实例应用。一个 Serverless 应用可以由单个或者多个实例构成，每个组件部署对应一个实例。每个实例都会涉及一个 serverless.yml 文件，该文件定义了组件的参数，这些参数在部署时用于生成实例的信息。例如，`region` 定义了资源的所在区。

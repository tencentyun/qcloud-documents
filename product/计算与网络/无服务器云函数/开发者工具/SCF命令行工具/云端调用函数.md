...
| type      | -t   | 否   | 支持异步（async）和同步（sync）两种调用方式，默认为同步          | async                |



## 使用示例
例如，初始化配置为 ap-shanghai 区域，在default 命名空间下已运行了 hello_world 函数。
...
- 可以在本地编写测试模板，测试模板规范请参见 [触发器事件消息结构汇总](<https://cloud.tencent.com/document/product/583/31927>)，本文使用测试文件 `event.json`，内容如下：
>!为避免因编码问题导致调试异常，故本地测试文件需要确认为 UTF8 编码。
>
```json
{
  "test1": "001",
...
```
执行以下命令，用指定测试模板 event.json 调用云端函数：
<pre>

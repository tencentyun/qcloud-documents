...
2. 分别获取账号的 APPID，SecretId，SecretKey 以及产品期望所属的地域。您可以参考 [安装配置](https://cloud.tencent.com/document/product/583/33449#.E9.85.8D.E7.BD.AE-scf-cli) 文档以获取上述信息。
...
通过本地调用，您可以在本地使用模拟事件触发函数执行，实时调试函数代码。当前 `native invoke`  仅支持 Node.js 及 Python 语言。其他语言的函数本地调用方法可以参考下方`local invoke` 。
在通过 `scf init` 初始化得到的示例项目中，均带有已准备好的代码文件及模板配置文件。

执行 `scf native invoke -t template.yaml --no-event `命令，启动函数在本地运行并返回 Hello World：

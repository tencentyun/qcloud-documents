...
>- 本文中 TCSAM 版本为 2018-11-11。TCSAM 根据 [Apache 2.0 许可证](http://www.apache.org/licenses/LICENSE-2.0.html) 授权。 
...
属性名称 | 类型 | 描述
---|:---:|---
StageName | `String` |发布阶段的名称，API 网关用作调用统一资源标识符（URI）中的第一个路径段。可选值为：`test`、`prepub`、`release`。默认如果为新 API 服务时为 `release`，已有 API 服务时为 `test`。

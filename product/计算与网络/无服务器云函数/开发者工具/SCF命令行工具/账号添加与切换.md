...
### 使用示例
- 执行 `scf configure add` 命令，将以交互的方式添加账号信息。输出结果如下：
```bash
$ scf configure add 
...
```
- 执行 `scf configure add -a xxxx -si xxxx -sk xxxxx -r ap-shanghai -uc y ` 命令，添加账号信息。输出结果如下：
>?您可参考 [参数说明](#para)，填写相关参数信息。
>
```bash
...
### 使用示例
- 执行  `scf configure change` 命令，并输入您想切换的账号数字，即可进行切换。输出结果如下：
```bash

...
- 通过执行 `scf configure set` 命令，可将获取的配置信息配置到 SCF CLI 中，也可修改某个配置项的信息。 例如，用户希望在**广州**地区使用云函数，并获取到账号 ID 为 1253970223，SecretId 和 SecretKey 分别为 AKIxxxxxxxxxx，uxxlxxxxxxxx。您可以选择以下命令，完成 SCF CLI 的配置。
> !

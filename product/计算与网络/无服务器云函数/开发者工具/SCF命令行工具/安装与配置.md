...
python --version
```
Python 版本须为 2.7 或 3.6 +，如果 Python 版本不符，您可以参考 [Python 安装教程](#python)，安装 2.7 或 3.6+ 的 Python 版本。
2. 执行以下命令，安装 SCF CLI。
```bash
$ pip install scf
```
...
- SecretID 及 SecretKey：指云 API 的密钥 ID 和密钥 Key。您可以通过登录【访问管理控制台】，选择【云 API 密钥】>【[API 密钥管理](https://console.cloud.tencent.com/cam/capi)】，获取相关密钥或创建相关密钥。
...
- 通过执行 `scf configure set` 命令，可将获取的配置信息配置到 SCF CLI 中，也可修改某个配置项的信息。 
  例如，用户希望在**广州**地区使用云函数，并获取到账号 ID 为1253970223，SecretId 和 SecretKey 分别为 AKIxxxxxxxxxx，uxxlxxxxxxxx。您可以选择以下命令，完成 SCF CLI 的配置。
>!using-cos 是指在部署时是否通过 COS 部署，使用 COS 部署速度会快很多，建议开启该设置。在部署频次和包非常大时（如上万次部署或数G的包，详情可参考 [COS 免费额度](https://cloud.tencent.com/document/product/436/6240)），可能会产生 COS 计费。
>
- 执行以下命令，并按照提示输入 [获取配置信息](#get) 中您的配置信息
```
$ scf configure set
[-] appid = 1255721742
[-] region = ap-guangzhou
...
using-cos = True (By default, it is deployed by COS.)
```
您也可以在 `~/.tcli_config.ini ` 中看到所有的已配置信息。

## 升级 SCF CLI
如果您已经安装过 SCF CLI，可以通过如下步骤，将 SCF CLI 升级至最新版本。
1. 执行以下命令，进行 SCF CLI 的升级。
```
$ pip install -U scf
```

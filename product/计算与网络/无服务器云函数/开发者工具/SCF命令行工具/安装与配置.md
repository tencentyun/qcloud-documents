...
```
返回类似如下信息，则表示安装成功。
>!SCF CLI 已于2019.07.22更新版本0.1.1 ，此次更新为不兼容更新，您可以参考下方 [升级 SCF CLI](#Updatescf) 方式，升级至最新版本。
> 
```
SCF CLI, version 0.0.1
```
如果是 Ubuntu 环境，执行上述命令后，提示类似“找不到 scf 命令”的错误，则需要执行以下命令将 SCF 安装路径加入 PATH 环境变量：
```bash
...
>!SCF CLI 已于2019.07.22更新版本0.1.1 ，此次更新为不兼容更新，您可以参考本步骤，升级至最新版本。
>

...
- SCF CLI：VS Code 插件借助于 SCF 命令行工具进行本地函数的创建，触发和调试。
>?SCF CLI 可通过执行`pip install scf`命令进行组件的安装，详情请参见 [CLI 详细安装教程](<https://cloud.tencent.com/document/product/583/33449>)。
...
### 安装插件
1. 运行 VS Code IDE。
2. 单击左侧导航栏中的<img src="https://main.qcloudimg.com/raw/825509ea850983496eae3a1387e767a0.png" style="margin:-3px 0;">，打开 VS Code 插件市场。
...
>!如果您已经在 SCF CLI 中配置了账户信息，则不需要再次配置，即可自动获取本机存储的账户信息。
> 
2. 单击创建一个腾讯云用户凭证。如下图所示：
![Alt text](https://main.qcloudimg.com/raw/fca11ef6e54287f2ad400d34123872c9.png)
3. 根据提示依次输入账号的 APPID，SecretId 及 SecretKey 信息，作为插件调用云API 时的认证信息。认证成功后选择您希望部署函数的地域。配置信息获取途径如下：
...
>
2. 根据提示依次选择函数所在的命名空间，运行时 runtime，输入函数名。函数信息录入成功后即开始创建。
3. 函数创建成功后，列表页中会展示新建的本地函数（以`vsCode` 函数为例）。如下图所示：
![](https://main.qcloudimg.com/raw/62a0f773ab07b9a1feaf8fb4d7afd232.png)


### 本地触发函数项目
单击左侧列表中的本地函数，选择【本地调用】，即可对函数进行本地测试（当前仅支持 Python 及 Node.js 函数的本地触发能力）。如下图所示：

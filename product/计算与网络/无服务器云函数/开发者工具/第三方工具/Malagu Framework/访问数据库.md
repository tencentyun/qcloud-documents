...
Malagu 框架根据装饰器的配置，在方法调用前开启事务（也可能不开启），并将 EntityManager 托管在 OrmContext 上下文中，通过  OrmContext 取到框架帮助开启过事务的 EntityManager，其中 Repository 是通过托管的 EntityManager 创建。为正确的取到 EntityManager，请确保装饰器配置的名称与通过 OrmContext 要获取的 EntityMananger 名称保持一致，不指定名称，则默认为 default 。

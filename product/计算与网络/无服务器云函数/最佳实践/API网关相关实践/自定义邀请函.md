...
<tr>
<td>target_path</td><td>目标存储桶路径。用于存放邀请函的存储桶目录路径，请前往【<a herf="https://console.cloud.tencent.com/cos5/bucket">存储桶列表</a>】>【进入“对应的存储桶”】，查看对应的目录路径。例如目录为 example，则此处的目标路径即为 /example。若在存储桶在未创建目录，那么此处输入 “/” 即可。</td>
</tr>
</table>
3. 单击【保存】即可完成函数配置。

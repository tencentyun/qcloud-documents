...
云函数在较多使用场景是和 API 网关组合使用实现 REST API，此步骤结合特定业务场景说明如何获取已捕获异常的日志。
...
def main_handler(event, context):
    ...
    if event["pathParameters"]["user_type"] == "teacher":
        ...
        if event["pathParameters"]["action"] == "put":
            return teacher_put()
```
假设函数在执行写文件步骤时，IO 异常已被捕获。故执行结束后无法获取文件，但函数调用结果为成功，API 请求返回值为 null。

若此时使用旧版架构的日志功能，则需逐条查看日志。可根据代码捕获异常时打印的 Error 信息，使用高级日志功能直接检索该关键词，步骤如下：
...
至此已成功获取已捕获异常的日志，您也可使用其他捕获时关键词，通过上述方法直接检索即可获取。对于日志输出方法无特殊要求，使用运行时原生日志即可。



### 查找指定函数运行时间的请求日志
高级日志功能支持使用运行时间进行检索，如您收到云函数告警消息或通过监控图标查看到某个函数的运行时间异常，请按照以下步骤进行日志检索：
...
4. 可基于上述结果，组合使用时间范围条件及关键词检索，进一步检索。
...
>- 若需查找运行时间为150ms - 300ms之间的日志，可使用 `SCF_Duration in (150 300]`。
>

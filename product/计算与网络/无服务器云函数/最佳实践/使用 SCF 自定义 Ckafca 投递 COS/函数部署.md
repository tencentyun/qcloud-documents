...
在开始部署云函数前，您需要完成以下操作：
1. 在待部署的电脑中，安装 Python2.7 环境和 Pip 工具。
...
> ? 如果您已下载示例代码的压缩包，可直接执行 [步骤4](#step4)。

1. 在本地创建待放置函数代码的文件夹，并在该目录下，执行以下命令安装 pytz 库：
```
pip install pytz -t .
```
...
9. 上传已压缩的 zip 包，将**“执行方法”为：Ckafka_SCF_COS.main_handler**，单击保存。如下图所示：
...
主要参数信息如下：
 ...
 - Topic：选择需要对接的 Topic。
 - 最大批量消息数：请根据单条消息的大小和实际业务量进行配置，默认消息总数的大小要小于 1 MB。单次触发云函数运行时最大可处理的消息条数。例如，配置500，则最多会有500条消息会触发一次云函数。Ckafca 生产的消息过多时，消费组将自动触发多个函数实例运行，加速消息消费速度。
2. 单击【保存】，即完成操作。

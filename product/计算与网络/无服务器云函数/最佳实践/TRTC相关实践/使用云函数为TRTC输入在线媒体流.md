...
本文为您介绍如何 [使用API网关调用云函数](https://cloud.tencent.com/product/serverless-catalog)，将已有的录播视频或者 RTMP 直播流推送到实时音视频 TRTC 房间进行直播。如您需开启推流直播的实时记录，可以选择使用 Redis，API 网关会将进度实时写入 Redis。工作流程如下图所示：
...
对于游戏、秀场、球赛这类直播 节目，可以通过云函数将RTMP 直播流推送到实时音视频 TRTC 房间进行直播。在同一个TRTC房间下的用户可以一边观看直播，一边发起互动，提升直播节目观影体验。

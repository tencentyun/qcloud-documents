...
2. [](id:step2)选择左侧导航栏中的【服务】，并在“服务”页面上方切换地域为【北京】。
...
6. [](id:step6)单击【新建】进入“新建API” 页面，在 “前端配置” 步骤中，参考以下主要参数信息进行创建 API：
 - **API名称**：自定义 API 名称。
 - **路径为**  `/article`
 - **请求方法**： GET。
...
9. [](id:step9)再次在【管理API】选项卡中单击【新建】创建 API，路径为 `/article/{articleId}`，请求方法为 GET，选择【免鉴权】，参数配置中输入名称为 `articleId` 的参数，参数位置为 Path，类型为 int，默认值为 1，单击【下一步】。

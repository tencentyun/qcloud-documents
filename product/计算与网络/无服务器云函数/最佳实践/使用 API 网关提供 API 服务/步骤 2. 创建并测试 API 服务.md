...
9. <span id="step9">再次在【API管理】选项卡中单击【新建】创建 API，路径为 `/article/{articleId}`，请求方法为 GET，选择【免鉴权】，参数配置中输入名称为 `articleId` 的参数，参数位置为 Path，类型为 int，默认值为 1，单击【下一步】。</span>
10. 后端类型选择为【cloud function】，选择函数为 [步骤一](https://cloud.tencent.com/document/product/583/13198) 中已创建的 `blogArticle` 函数，单击【完成】。

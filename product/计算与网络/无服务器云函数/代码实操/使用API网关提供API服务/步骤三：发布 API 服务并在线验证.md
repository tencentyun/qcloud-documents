如果您完成了步骤二：创建并测试 API 服务，且测试结果符合预期，则接下来我们可以对外发布此服务并从浏览器发起请求来验证接口的工作情况。

1. 在 API 网关控制台的【服务】列表页中，找到我们在步骤二创建的 blogAPI 服务，并点击服务操作中的【发布】按钮。

2. 在发布服务的弹窗中，发布环境选择 `发布`，备注内填入 `发布API`，点击【提交】。

通过发布动作，我们完成了 API 服务的发布，使得 API 可以被外部所访问到，接下来我们通过浏览器发起请求来查看 API 是否能正确响应。
...
至此我们完成了通过 SCF 云函数实现服务，通过 API 对外提供服务。后续我们可以继续修改代码，增加功能并增加 API 规则，使其完善成为一个更丰富的应用模块。
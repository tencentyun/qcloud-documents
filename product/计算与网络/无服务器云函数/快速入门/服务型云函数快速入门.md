## 操作场景
如果您初次使用云函数，可参考 [ 使用 Hello World 函数](https://cloud.tencent.com/document/product/583/9202)、[部署 API 后台](https://cloud.tencent.com/document/product/583/32955)，帮助您快速入门云函数。

本文旨在通过 Express、Koa 和 Egg 的 Hello World 示例介绍 HTTP Service 服务型云函数使用方法，帮助您了解服务型云函数的基本使用流程和 SCF 控制台的基本操作。例如创建服务、上传代码、测试访问和查看运行日志和监控。

您可结合示例步骤进行项目开发，服务创建及访问验证。本文示例步骤如下：
...
## 前提条件
 ...
 - 已开通 HTTP Service 功能。
 HTTP Service 目前为内测发布功能，可通过 [提交工单](https://cloud.tencent.com/apply/p/tagk8e0x19f) 申请使用。
...
6. 单击访问路径，浏览器窗口显示 `Hello World!`，则表示 helloexpress 应用部署成功。
7. 您可以单击【运行日志】和【监控信息】来查看日志和监控。

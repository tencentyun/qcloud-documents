...
为提升函数上传效率，我们提供了默认使用 COS 上传函数的方式。在 SCF CLI 中开启“使用 COS 上传函数”功能，即可提升80%的上传效率，开启后可领取 COS 代金券。
...
>
1. 执行以下命令，初始化示例项目 hello_world。然后进入函数目录执行 `scf deploy` ：
```
scf init
```

<span id="cd"></span>
2. 执行以下命令，根据您的实际情况进入对应的项目目录。
```
$ cd hello_world/
```

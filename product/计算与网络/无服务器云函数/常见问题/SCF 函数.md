...
无服务器云函数上运行的代码以 “SCF 函数” 的形式上传。每个函数都有相关的配置信息，如其名称、描述、资源要求等。代码必须以 “无状态” 样式编写，即应假设与底层计算基础设施无密切关系。本地文件系统访问、子进程等严格控制在云函数的生命周期内，且任何持续状态都应存储在外部可用的存储如 COS 或 CDB。SCF 函数可包含外部库，甚至是本机库。

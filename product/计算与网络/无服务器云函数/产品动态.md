...
<tr>
<td>云函数预置并发功能正式发布</td>
<td>云函数支持预置并发功能，通过配置预置并发，可预先进行计算资源的准备，降低冷启动、运行环境初始化及业务代码初始化引起的耗时。预置并发功能在2021年11月01日正式发布上线。上线后会有如下调整及能力新增：
<li>预置并发闲置费用会正式启动收费，配置预置并发后对已经配置并启动、但未使用的实例收取闲置费用。如无需使用预置并发，请注意取消预置并发配置，避免产生额外费用。</li>
<li>预置并发支持定时预置功能，可以根据业务情况设置定时对预置配额进行升降配置，降低过多的闲置费用。</li>

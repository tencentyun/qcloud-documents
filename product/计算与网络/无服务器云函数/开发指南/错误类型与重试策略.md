...
不提供在别名上配置DLQ的额外入口。这里的考量是：从长期来看，我们会弱化除"默认流量"外的其他别名，所以不再给别名增加额外的配置项；
所以别名的DLQ会跟随主版本DLQ

主版本DLQ即为用户在控制台创建别名时第一个选择并配置的DLQ。
* 目前主版本在控制台的展现比较弱，和俊哥讨论后，俊哥会安排交互优化。

async ->region invoke时，如果访问的是别名就走别名的DLQ，访问的是版本的就走版本的DLQ。
async -> region invoke直接因为各种异常（网络，机器问题等）访问失败也是要丢DLQ的（如果用户未配置DLQ才丢弃）
...
死信队列投递至 CMQ 时会把属性信息与 Event 封装到一起 Json 请求体中，例：
...
3. 配置死信队列。
 ...
 >?函数别名的死信队列会跟随主版本的死信队列，主版本死信队列即为在控制台创建别名时第一个选择并配置的死信队列。
 
### 死信队列监控
 
在使用死信队列过程中，可能会发生权限错误、资源误配或消息的总大小超过目标队列或主题的限制大小，这种情况下会导致死信投递失败。可在云函数监控信息中查询到该监控项参数，死信队列投递失败次数/次（DeadLetterErrors）。

![](https://main.qcloudimg.com/raw/3d329edb69f44ac105718ad474d444df.png)

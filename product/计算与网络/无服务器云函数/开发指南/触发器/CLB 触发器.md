...
| X-Real-Port | 记录在 API 网关中配置过的 Path 参数以及实际取值。（可选，CLB 自定义配置） |
| X-Vip | CLB 负载均衡的 VIP 地址（可选，CLB 自定义配置） |
| X-Vport | CLB 负载均衡的 Vport（可选，CLB 自定义配置）  |
| X-Url | 请求 CLB 负载均衡的 PATH（可选，CLB 自定义配置） |
| X-Method |请求  CLB 负载均衡的 method（可选，CLB 自定义配置） |

...
- **异步调用的事件大小限制**：128KB，详情请参见 [限制说明](https://cloud.tencent.com/document/product/583/11637)。如果日志主题的消息较大，例如单条消息通过后台组件压缩后依然达到64KB以上，那么由于异步调用的128KB限制，传递给云函数的事件结构中只会有一条消息，而不是用户配置的最长等待时间的聚合。

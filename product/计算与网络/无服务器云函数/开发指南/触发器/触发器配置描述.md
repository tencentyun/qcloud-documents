...
### API 请求说明
使用 API 请求创建 Ckafka 触发器时，TriggerName 字段需定义为要转储的 instanceId ，topicName
...
```
>!访问域名请在COS控制台 存储桶列表-基础配置-基本信息 中查看
...
| filterKey | String  | 否   | 当`filterType`为`1`时表示消息过滤标签，当`filterType`为`2`时表示Binding Key |
...
使用 API 请求创建 CMQ 触发器，TriggerName 字段需定义为CMQ Topic，示例如下：
```json

...
API 网关触发器使用的 API 配置，如下表所示：

| 名称                 | 类型   | 必选 | 描述                                                         |
| -------------------- | ------ | ---- | ------------------------------------------------------------ |
| AuthRequired         | String | 否   | 是否需要鉴权，可选 TRUE 或者 FALSE                              |
| RequestConfig        | ApigwApiRequestConfing       | 否   | 请求后端 API 的配置 |
...
| 名称   | 类型   | 必选 | 描述                                                         |
| ------ | ------ | ---- | ------------------------------------------------------------ |
| Prefix | String | 是   | 过滤文件的前缀规则  |
| Suffix | String | 是   | 过滤文件的后缀规则，必须以`.`开头。                          |


### COS 事件冲突规则
- 核心思想：一个事件最多触发一次函数调用。
- 若已设置 `cos:ObjectCreated:*` 事件，后续再绑定任何以 `cos:ObjectCreated` 作为开头的事件都会失败。
- 前缀及后缀均匹配才有效，且当前缀和后缀都冲突的时候，后面的绑定会失败。

### 示例
```json
{"event":"cos:ObjectCreated:*","filter":{"Prefix":"scf","Suffix":".log"}}
```
>! JSON 字符串需要是紧凑的，中间不能有空格。

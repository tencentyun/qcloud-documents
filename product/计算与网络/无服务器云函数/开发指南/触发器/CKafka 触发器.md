...
>?
>
>对于运行错误（含用户代码错误和Runtime错误），CKafka触发器会按照您配置的重试次数进行重试。默认重试10000次。
>
...
- **起始位置**：触发器消费消息的起始位置，默认从最新位置开始消费。支持三种配置：最新、最开始、按指定时间点。
- **重试次数**：函数发生运行错误（含用户代码错误和Runtime错误）时的最大重试次数。

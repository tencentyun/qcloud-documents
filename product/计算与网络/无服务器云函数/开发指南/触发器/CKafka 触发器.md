...
- **超时时间**：目前云函数后台的消费模块的超时时间为60秒，避免时延过长才进行消费。例如，Ckafka Topic 的消息写入很少，消费模块在60内没有凑够最大批量消息数的消息，则依然会发起函数调用。

## CKafka 消费及消息传递

由于 CKafka 消息无主动推送能力，需要消费方通过拉取的方式，拉取到消息并进行消费。因此，在配置 CKafka 触发器后，云函数后台会通过启动 CKafka 消费模块，作为消费者，并在 CKafka 中创立独立的消费组进行消费。

云函数后台的消费模块在消费到消息后，会根据一定的超时时间、累积消息数量、大小及最大批量消息数等信息，组合为事件结构并发起函数调用（同步调用）。

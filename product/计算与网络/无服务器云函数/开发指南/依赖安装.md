...
## 安装依赖库
您可以将 SCF 代码所的依赖库保存在代码包中，上传至云端以供 SCF 使用。SCF 已支持的运行时及使用方法如下：


### Node.js 运行时
**依赖管理工具**：Node.js 可以通过 npm 包管理器进行依赖管理。
**使用方法**：使用时需要在 `package.json` 中配置依赖信息，通过 `npm instal`l 命令安装依赖库至 `node_modules` 文件夹，上传代码库时请将依赖库一同打包上传。

### Python 运行时
**依赖管理工具**：Python 可以通过 pip 包管理器进行依赖管理。
**使用方法**：使用时需要在 `requirements.txt` 中配置依赖信息，通过 `pip install -t .` 命令安装依赖包，上传代码库时请将依赖库一同打包上传。

### Java 运行时
**依赖管理工具**：Java 可以通过 maven 包管理器进行依赖管理。
**使用方法**：使用时需要在 `pom.xml` 中配置依赖信息。然后可以通过 `maven install` 命令安装依赖包，上传代码库时请将依赖库一同打包上传。

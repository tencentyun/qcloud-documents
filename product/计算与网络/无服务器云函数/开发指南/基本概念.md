...
## 执行方法
SCF 平台在调用云函数时，会首先寻找执行方法作为入口，开始执行您的代码，您需以**文件名.执行方法名**的形式进行设置。
例如，用户设置的执行方法为 `index.handler`，则 SCF 平台会首先寻找代码程序包中的 `index` 文件，并找到该文件中的 `handler` 方法开始执行。

在编写执行方法时您需遵循平台特定的编程模型，如下所示：
```
def method_name(event,context): 
    ...
    return some_value
```
该模型中指定固定的事件数据 event 和环境数据 context 作为入参。在执行方法中，您需对参数进行处理，并且可任意调用代码中的任何其他方法。
...
<table>
  ...
  例如，COS 触发函数时会将 Bucket 及文件的具体信息以 <a href="https://cloud.tencent.com/document/product/583/9707#cos-.E8.A7.A6.E5.8F.91.E5.99.A8.E7.9A.84.E4.BA.8B.E4.BB.B6.E6.B6.88.E6.81.AF.E7.BB.93.E6.9E.84">JSON 格式</a> 传递给 event 参数。</li>
	...
	<tr>
		...
		<td>
				<ul>
			<li>SCF 平台将 context 入参传递给执行方法，代码通过此 context 入参对象，将能了解到运行环境及当前请求的相关内容。</li>
			<li>context 结构内容将会随着 SCF 平台的开发迭代而增加更多内容。</li>
				</ul>
		</td>
		<td>
...
在了解了 event 入参和 context 入参的基本用法后，编写函数代码时您还需注意以下几点：
<ul>
	<li>为保证针对各开发语言和环境的统一性，event 入参和 context 入参均使用 JSON 数据格式统一封装。</li>
	<li>不同触发器在触发函数时，所传递的数据结构有所不同，详情请参见 <a href="https://cloud.tencent.com/document/product/583/9705">函数触发器说明</a>。</li>
	<li>通过 API 方法触发云函数时，您可以自行定义传递给云函数的入参。</li>
...
<table>
	...
	<td>
		<ul class="params">
		<li>通过 API 网关、云 API 中 RequestResponse 触发的方式为同步触发。</li>
		<li>在函数执行期间，SCF 平台不会返回同步触发云函数的请求。</li>
...
<tr>
<td>Python</td>
<td>通常为简单数据结构或 <code>dict</code> 数据结构</td>
</tr>
<tr>
<td>Node.js</td>
<td>通常为 <code>JSON Object</code></td>
</tr>
<tr>
<td>PHP</td>
...
为保证针对各开发语言和环境的统一性，函数返回值会使用 **JSON 数据格式统一封装**。SCF 平台在获取到例如以上运行环境函数的返回值后，将会对返回的数据结构进行 JSON  化，并返回 JSON 内容到调用方。
>!
>- 需确保函数的返回值均可 JSON 化，若返回值为对象且不具备 JSON 化方法，将会导致 SCF 平台在 JSON 化操作时失败并报错。
>- 返回值也无需在 return 前自行 JSON 化，否则会导致输出的字符串会进行再次 JSON 化。
...
<table>
	...
	<td>
	若此函数在运行过程中出现异常，将返回以下错误信息，SCF 平台会将此错误信息记录到函数日志中。<pre>File "/var/user/index.py", line 2, in always_failed_handler
...
def create_user(event):
	...
	<td>
		您可以通过此方式在代码中自行定义如何处理可能出现的错误，保障应用程序的健壮性和可扩展型。
	</td>
	</tr>
	<tr>
	<td>使用 Try 语句捕获错误</td>
	...
	<td>
	您可以通过此方式针对不同的错误类型采用不同的处理方式。
	</td>
	</tr>
</table>

### 返回错误信息
当用户的代码逻辑中未包含异常及错误处理时，SCF 平台会在用户使用时尽可能的捕获错误。例如，用户函数在运行过程中突然退出，当出现此类平台也无法捕获错误的情况时，平台将会返回一个通用的错误信息。您可根据下表中的常见错误信息分析原因：
...
### 日志组成
函数运行日志信息包含函数名、启动时间、执行时间、计费时间、内存实际用量、返回码、返回值、代码日志、执行状态。
...
def my_logging_handler(event):
    ...
    return 'Hello World!'</pre>
		您可以前往控制台日志模块或通过 <a href="https://cloud.tencent.com/document/product/583/18583">获取函数运行日志</a> API 来查看代码中的日志信息。
</td>
</tr>
<tr>
<td>使用 print 语句</td>
...
<table>
	...
	<tr>
	<td>SCL CLI 命令行工具</td>
	<td>执行 `scf logs` 命令，详情请参见<a href="https://cloud.tencent.com/document/product/583/36352"> 日志查看</a>。</td>
	</tr>
	<tr>
	<td>SCF 控制台</td>
	<td>在函数代码页面，单击【测试】按钮同步调用函数，执行完成后会直接在控制台展示本次调用的日志。</td>
	</tr>
	<tr>
	<td>触发器调用</td>

...
<tr>
<td>Python</td>
<td>通常为简单数据结构或 `dict` 数据结构</td>
</tr>
<tr>
<td>Node.js</td>
<td>通常为 `JSON Object`</td>
</tr>
<tr>
<td>PHP</td>
<td>`Array` 结构</td>
...
为保证针对各开发语言和环境的统一性，函数返回值会使用 **JSON 数据格式统一封装**。SCF 平台在获取到例如以上运行环境函数的返回值后，将会对返回的数据结构进行 JSON  化，并返回 JSON 内容到调用方。您在编写代码时需注意以下几点：
...
def create_user(event):
	...
	<td>
	您可以通过此方式以不同的处理方式处理不同的错误类型。
	</td>
	</tr>
</table>

### 返回错误信息
当用户的代码逻辑中未包含异常及错误处理时，SCF 平台会在用户使用时尽可能的捕获错误。当出现例如用户函数在运行过程中突然退出，此类平台也无法捕获错误的情况时，平台将会返回一个通用的错误信息。您可根据下表中的常见错误信息分析原因：

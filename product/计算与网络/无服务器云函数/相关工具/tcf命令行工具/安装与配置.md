## 操作场景
SCF CLI 在安装前依赖 Python 运行环境，pip 包管理工具，以及使用本地调试时的 Docker 工具。我们将按如下步骤完成依赖安装、scf cli 安装及配置：
1. [安装 Python 及 pip](#InstallPythonAndPip)
2. [安装 Docker（可选）](#InstallDocker)
3. [安装 scf cli](#Installscf)
3. [升级 scf cli](#Updatescf)
4. [配置 scf cli](#Configscf)
...
```
3. 针对 Linux 或 Mac 操作系统，可以执行以下命令，启动 scf 命令的自动补全功能。
```bash
...
例如，用户希望在广州区使用云函数，并获取到账号 ID 为 1253970223，SecretId 和 SecretKey 分别为 AKIxxxxxxxxxx，uxxlxxxxxxxx。您可以通过执行以下命令，完成 scf cli的配置：
```bash

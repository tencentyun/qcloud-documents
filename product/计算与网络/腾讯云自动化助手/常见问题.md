### 什么是自动化助手？
腾讯云自动化助手（TencentCloud Automation Tools，TAT）是云服务器原生的运维部署工具，支持可视化控制台和 API 操作。通过自动化助手，您无需远程连接实例即可批量执行 Shell 命令。

### 如何使用自动化助手？
您可以通过轻量应用服务器控制台创建及执行命令，或通过 [CreateCommand]() 接口使用自动化助手。
...
### 如何查看命令执行状态？
您可以选择以下任一方式查看命令执行状态：
- 通过 [轻量应用服务器控制台](https://console.cloud.tencent.com/lighthouse/instance/index)。
- 通过 [DescribeInvocationTasks]() 接口。
- 登录实例，并执行 `cd /usr/local/qcloud/tat_agent/log/` 命令，查看 agent 的运行日志。
...
您可以选择以下任一方式查看客户端状态：
- 通过 [轻量应用服务器控制台](https://console.cloud.tencent.com/lighthouse/instance/index)。
- 通过 [DescribeAutomationAgentStatus]() 接口。
...
可能是由于 PATH 环境变量设置导致，请确保您需要执行的命令路径已经加入环境变量。

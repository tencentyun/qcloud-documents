## 什么是腾讯云自动化助手？
腾讯云自动化助手（TencentCloud Automation Tools，TAT）是轻量应用服务器 Lighthouse 的原生运维部署工具。您无需远程连接实例，自动化助手即可自动批量执行 Shell 命令，完成运行自动化运维脚本、轮询进程、安装/卸载软件、更新应用及安装补丁等任务。具备以下功能：

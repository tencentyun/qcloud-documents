## 什么是访问管理？
你的团队在使用黑石物理服务器、黑石弹性公网IP、黑石私有网络，黑石负载均衡时，不同云资源可能由团队内不同的人员管理，那这时间你可能需要

- 每位人员都有权限访问腾讯云控制台或者调用云 API
...
当你在使用腾讯云控制台或者 API 时，遇到以下提示，则说明你没有被授予操作权限。请联系 root 账号管理员或者有 CAM 管理权限的人员为你的 CAM 账号关联相应策略。调用任一黑石 API 都要求通过 CAM 鉴权。 故你需要把用到的 API和 实例Id 都添加到策略中，否则该提示会频繁出现。 <br/>
...
> 编辑策略时，复制提示中的 operation 以及 resource ，并黏贴到策略的 action 和 resource 字段即可 <br/>
> 或者使用预设策略，但授权的粒度太粗了

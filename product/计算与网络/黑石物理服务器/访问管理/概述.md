...
# 访问管理的用途
你的团队在使用黑石物理服务器、黑石弹性公网IP、黑石私有网络，黑石负载均衡时，不同云资源可能由团队内不同的同事管理，那这时间你可能需要
>
...
是 root 账号添加的，用于管理云资源的账号，且拥有自己的登录密码或者密钥(SecretId和SecretKey)。 CAM 用户可以登录控制台，但默认没有任何权限。在 CAM 的设计里，为 CAM 用户关联策略，即是授权，策略则包含了权限声明。

### 云资源
即 root 账号购买的云产品实例，比如 黑石服务器、黑石负载均衡等实例。
...
<td>关联后，获得所有黑石所有产品（cpm,bmeip,bmlb,bmvpc等）的增、删、改、查操作</td>
</tr>

<tr>
<td>QcloudBMReadOnlyAccess</td>
<td>关联后，只能查询黑石所有产品（cpm,bmeip,bmlb,bmvpc等）列表及基本信息</td>
</tr>
...
<td>关联后，获得所有黑石弹性公网 IP 的增、删、改、查操作</td>
</tr>

<tr>
<td>	QcloudBMEIPReadOnlyAccess</td>
<td>关联后，只能查询黑石弹性公网 IP列表及基本信息</td>
</tr>

<tr>
<td>	QcloudBMLBFullAccess</td>
<td>关联后，获得所有黑石负载均衡的增、删、改、查操作</td>
</tr>

<tr>
<td>QcloudBMLBReadOnlyAccess</td>
<td>关联后，只能查询黑石负载均衡列表及基本信息</td>
</tr>

<tr>
<td>	QcloudBMVPCFullAccess</td>
<td>关联后，获得所有黑石私有网络的增、删、改、查操作</td>
</tr>

<tr>
<td>	QcloudBMVPCReadOnlyAccess</td>
<td>关联后，只能查询黑石私有网络列表及基本信息</td>
</tr>
...
当你在使用腾讯云控制台或者 API 时，遇到以下提示，则说明你没有被授予操作权限。请联系 root 账号管理员或者有 CAM 管理权限的同事为你的 CAM 账号关联相应策略。
...
> 复制提示中的 operation 以及 resource ，并黏贴到策略的 action 和 resource 字段即可 <br/>
> 调用任一黑石 API 都要求通过 CAM 鉴权。 故你需要把用到的 API和 实例Id 都添加到策略中，否则该提示会频繁出现。 <br/>
> 或者使用预设策略，但缺点是授权的粒度太粗了

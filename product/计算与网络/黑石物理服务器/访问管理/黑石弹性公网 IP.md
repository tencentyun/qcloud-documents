...
## 预设策略
预设策略，能帮助你快速授权，而不需要编写策略，缺点是授权的精度会粗些，以下是黑石弹性公网IP 的两个预设策略，分别为：
<table >
 <tr>
    <th>预设策略名</th>
    <th>授权范围描述</th>
...
以下表格，罗列了在配置 黑石弹性公网IP 的策略时，需要用到的 action 、 resource、condition。<br/>
...
1. 场景1： 授权 释放 eip-adt6pq7f
2. 场景2： 授权 绑定 vpc-34cxlz7z 和 vpc-muinpf9p 里内所有的 物理服务器和EIP 
...
1. 当调用 EipBmDelete 时，CAM 会判断传入的 EipId 是否为 eip-adt6pq7f，【是】则鉴权通过；【否】则鉴权失败
...
评估逻辑：
1. 当调用 BindEip 时，CAM 会对传入的 instanceId和EipID 做鉴权，发现满足 resource（*）的要求。
2. 但要求 instanceId和EipID 在 vpc-34cxlz7z 或者 vpc-muinpf9p 里，【是】则鉴权通过；【否】则鉴权失败

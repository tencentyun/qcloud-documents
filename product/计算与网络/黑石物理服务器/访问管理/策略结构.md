...
在 CAM 策略语句中，您可以从支持 CAM 的任何服务中指定任意的 API 操作。对于 bm，请使用以 name/bm: 为前缀的 API 。例如： name/bm:StartDevice 或者 name/bm:ReloadDeviceOs 。
如果您要在单个语句中指定多个操作的时候，请使用逗号将它们隔开，如下所示:
```
"action":["name/bm:action1","name/bm:action2"]
```
您也可以使用通配符指定多项操作。例如，您可以指定名字以单词" Describe "开头的所有操作，如下所示：
```
"action":["name/bm:Describe*"]
```
...
**service_type**：产品简称，如 bm。
**region**：地域信息，如 ap-beijing。
**account**：资源拥有者的根帐号信息，如 uin/164256472。
**resource**：各产品的具体资源详情，如 instance/instance_id1 或者 instance/*。

例如，您可以使用特定实例 (cpm-34xw9x3t) 在语句中指定它，如下所示：
```
"resource":[ "qcs::bm:ap-beijing:uin/164256472:instance/cpm-34xw9x3t"]
```
您还可以使用 * 通配符指定属于特定账户的所有实例，如下所示：
```
"resource":[ "qcs::bm:ap-beijing:uin/164256472:instance/*"]
```
...
| 资源 | 授权策略中的资源描述方法 |
|-------|-------|
|实例|  qcs::bm:$region:$account:instance/$instanceId|

 


### BM 的条件密钥
在策略语句中，您可以选择性指定控制策略生效的条件。每个条件都包含一个或多个密钥值对。条件密钥不区分大小写。

- 如果您指定了多个条件或在单一条件中指定了多个密钥，我们将通过逻辑 AND 操作对其进行评估。
- 如果您在单一条件中指定了一个具有多个值的密钥，我们将通过逻辑 OR 操作对其进行评估。必须匹配所有条件才能授予权限。
下表描述了 BM 用于特定于服务的条件键。

|条件键|参考类型|键值对|
|---|---|---|
|bm:unVpcId|String|bm:unVpcId=vpc-xxxxx <br/>vpc-xxxx为黑石私有网络的唯一ID|
|bm:unSubnetId|String|bm:unSubnetId=subnet-xxxxx <br/>subnet-xxxx为黑石私有网络子网的唯一ID|

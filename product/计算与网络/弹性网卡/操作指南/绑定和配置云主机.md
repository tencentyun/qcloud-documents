...
[CentOS  配置弹性网卡](#centos)

[Ubuntu  配置弹性网卡](#ubuntu)
...
6. 根据业务实际情况配置路由策略。
  按照上述步骤配置好后，Linux 镜像依旧默认都从主网卡发包。您可通过策略路由来指定报文从某个网卡进，并从该网卡返回。
 1. 创建两张路由表。
   ```plaintext
 ...
 2. 给两个路由表添加默认路由。
        ...
        ```
   > !上述两个命令中，`192.168.1.1`要分别替换成主网卡所属子网的网关，以及辅助网卡所属子网的网关。具体网关，请参考 [查看网关](#.E6.9F.A5.E7.9C.8B.E7.BD.91.E5.85.B3) 。
 3. 配置策略路由。
    ```plaintext
...
1. 以管理员身份登录云服务器，执行如下命令，进入`/etc/network/`文件夹。
   ```plaintext
   cd /etc/network/
   ```
2. 修改配置文件 interfaces。
  1. 输入如下命令修改配置文件内容。
   ```plaintext
      vim interfaces
      ```
  2. 按 “i” 切换至编辑模式，追加如下格式的信息（实际请按照网卡的 IP 信息填写）。
    ```plaintext
      auto eth1
...
5. 根据业务实际情况配置路由策略。
      ...
      2. 给两个路由表添加默认路由。
         ...
         ```
 > !上述两个命令中，`172.21.48.1`要分别替换成主网卡所属子网的网关，以及辅助网卡所属子网的网关。如不清楚具体网关，请参考 [查看网关](#.E6.9F.A5.E7.9C.8B.E7.BD.91.E5.85.B3) 。

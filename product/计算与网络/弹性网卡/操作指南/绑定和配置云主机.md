...
6. 根据业务实际情况配置路由策略。
 ...
 1. 创建两张路由表。
   ```plaintext
     echo "10 t1" >> /etc/iproute2/rt_tables # 此处10为自定义的路由ID，t1为自定义路由表的名称，请根据实际填写
     echo "20 t2" >> /etc/iproute2/rt_tables # 此处20为自定义的路由ID，t2为自定义路由表的名称，请根据实际填写
   ```
 2. 给两个路由表添加默认路由。
> !具体网关，请参考 [查看网关](#.E6.9F.A5.E7.9C.8B.E7.BD.91.E5.85.B3) 。
> 
		```plaintext
     	   ip route add default dev eth0 via 192.168.1.1 table 10 # 此处192.168.1.1需替换为主网卡所属子网的网关，10需替换为自定义的路由ID，请根据实际填写
     	   ip route add default dev eth1 via 192.168.1.1 table 20 # 此处192.168.1.1需替换为辅助网卡所属子网的网关，20需替换为自定义的路由ID，请根据实际填写
 3. 配置策略路由。
> !
>
...
>
>   ```plaintext
>    ip rule add from 192.168.1.5 table 10 # 此处IP需替换为主网卡的IP，请根据实际填写
>    ip rule add from 192.168.1.62 table 20 # 此处IP需替换为辅助网卡的IP，请根据实际填写
>   ```
...
> 
1. 以管理员身份登录云服务器，执行如下命令，查看需配置（未显示 IP）的网卡信息，如图所示，需配置的网卡名称为 `eth1`：![image-20200904180708563](../../../../../../../../../../../Application Support/typora-user-images/image-20200904180708563.png)
...
4. 重启网卡 eth1。
 5. 执行如下命令切换至 root 用户，并安装 ifupdown。
     ```plaintext
      sudo su
...
> ?如不清楚具体网关，请参考 [查看网关](#.E6.9F.A5.E7.9C.8B.E7.BD.91.E5.85.B3) 。
>
>     ```plaintext
>     ip route add default dev eth0 via 172.21.48.1 table 10 # 此处172.21.48.1需替换为主网卡所属子网的网关，请根据实际填写
>     ip route add default dev eth1 via 172.21.48.1 table 20 # 此处172.21.48.1需替换为辅助网卡所属子网的网关，请根据实际填写
>     ```
>     3. 执行如下命令，配置策略路由。
> !
...
>
>      ```plaintext
>     ip rule add from 172.21.48.11 table 10 # 此处IP需替换为主网卡的IP，请根据实际填写
>     ip rule add from 172.21.48.3 table 20 # 此处IP需替换为辅助网卡的IP，请根据实际填写
>     ```
> <span id="windows">

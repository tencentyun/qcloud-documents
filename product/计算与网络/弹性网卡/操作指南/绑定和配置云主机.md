...
6. 根据业务实际情况配置路由策略。
  按照上述步骤配置好后，Linux 镜像还是默认都从主网卡发包。您可通过策略路由来指定报文从哪个网卡进，并从该网卡返回。
...
2. 修改配置文件 interfaces。
   ...
   2. 按 “i” 切换至编辑模式，追加如下格式的信息（实际请按照网卡的IP信息填写）。
...
4. 检查和确认 IP 配置正确。
      ...
      2. 确认辅助网卡和辅助网卡上的 IP 可见，如下图所示。
         ...
         ![image-20200827115538818](../../../../../../../Library/Application Support/typora-user-images/image-20200827115538818.png)

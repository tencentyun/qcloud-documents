...
6. **根据业务实际情况配置路由**
虽然上述步骤把辅助网卡配置好了，但 Linux 镜像还是默认都从主网卡发包。如果您希望报文根据业务场景要求走不同的网卡，需要在主机配置对应的路由策略。
...
ip rule add from 192.168.1.26/32 table 20
```
注：上述两个命令中，IP要分别替换成主网卡上的IP，以及辅助网卡上的IP。
至此配置完成，可分别ping两个网卡上的IP地址，都ping通即可。注意网络重启后要重新配置路由。

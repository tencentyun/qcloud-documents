...
> 
 3. 配置策略路由。
   ...
   ```
	...
	> + 网络重启后要重新配置路由，如需将路由配置持久化，可执行 [步骤7](#ste7_centos7)。

7. <span id="step7_centos7">将路由配置持久化写入文件，可使重启网络后路由不丢失。
   >?系统不同版本，将路由配置写入文件的方式可能略有差异，此处以 CentOS 7 举例，CentOS7 的永久静态路由需要写到”/etc/sysconfig/network-scripts/route-网卡名“文件中。
   >
   1. 在”/etc/sysconfig/network-scripts/“下，新建文件名为”route-网卡名“的文件，本例为 route-eth1。
   2. 执行如下命令，进入网卡配置文件。
...
ip route add default dev eth1 via 172.21.48.1 table 20
 ...
 3. 执行如下命令，配置策略路由。
	...
	> + 网络重启后要重新配置路由，如需将路由配置持久化，可执行 [步骤7](#step7_ubuntu)。

7. 将路由配置持久化写入文件，可使重启网络后路由不丢失。[](id:step7_ubuntu)
   
  1. 执行如下命令进入"rc.local"文件。
```plaintext
	vi /etc/rc.local
```

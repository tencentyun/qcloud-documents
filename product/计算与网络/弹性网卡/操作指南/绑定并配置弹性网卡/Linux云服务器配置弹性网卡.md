...
### 方式一：工具配置
>!
>+ 该方式适用于CentOS 8.0、7.8、7.6、7.5、7.4和7.2版本。
>+  “nic-hotplug.tgz”工具将在绑定弹性网卡或云服务器重启时被触发，自动创建网卡配置文件，并下发弹性网卡的策略路由。因此当云服务器有存量弹性网卡时，请务必确保存量弹性网卡的策略路由均已正确配置，再执行工具配置新弹性网卡；如可以接受云服务器重启对业务的影响，也可在配置完成后，参考[步骤5](#step5)如下操作重启云服务器使工具对所有网卡均生效。
#### 操作步骤
...
./install.sh
```
4.  参考 [绑定弹性网卡](https://cloud.tencent.com/document/product/576/18535)，绑定弹性网卡。
5. <span id="step5">（可选）如有存量网卡，重启云服务器后，所有网卡的策略路由将自动下发正常。
  ```
reboot
```

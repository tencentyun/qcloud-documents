...
日峰值计费价格
 ...
 <th><center>规格（Mbps）</center> </th>

 <th><center>中国大陆（不含香港）地域间互通价格</center> </th>

 <th><center>计费单位</center> </th>
 ...
 <td rowspan=5><center>元/Mbps/天</center> </td>
...
>!
- 如果您需要中国大陆与其它地域互通数据，请您咨询您的商务经理。
- 账单系统中的对等连接描述为：跨地域互通（大陆）账单——对等连接两端都在中国内地（大陆）地域的账单。
...
月95计费价格
 ...
 <td><center>(20,50]Mbps</center> </td>
...
月95带宽峰值：因为每5分钟产生一个统计点，所以每天统计点数为288个（60min * 24 / 5min），14天内所有统计点数为4032个（14天 * 288个/天 ），4032个统计点的带宽值从高到低排列，去掉最高的5%的点（4032个 * 0.05 = 201.6个），则取第202个点的带宽值为当月的月95带宽峰值，记为 Max95，这里假设为60M。

阶梯单价： 取120M所处阶梯单价：220元/Mbps/月。

6月费用 = Max95 \* 有效天占比 \* 阶梯单价 = 60 \*（14 / 30）\* 220 = 6160元。

## 运营活动
日峰值计费可享有以下活动，有效期至：2018.12.31。
>? 即2019.1.1起，对等连接将按实际使用带宽计费。

1. VIP 客户

   - 每条大陆地域对等连接享有100Mbps免费带宽（系统自动发放）。

   - 每条非大陆地域对等连接享有20Mbps免费带宽（系统自动发放）。

2. 非 VIP 客户 （2017年申请通过该活动）
每条大陆地域对等连接享有10Mbps免费带宽（系统自动发放）。

...
容器镜像类似虚拟机的快照，但更轻量，可以理解是容器的静态形式。镜像定义了容器运行的一切文件和依赖关系，保证了容器运行的一致性。
...
如果您需要更完善的 Kubernetes 容器管理服务，推荐您使用 [容器服务 CCS](https://cloud.tencent.com/product/ccs)。

如果您需要管理容器镜像，编排 devops 工作流，推荐您使用 [Tencent Hub](https://cloud.tencent.com/product/thub)。

如果您需要用虚拟机和容器部署不同场景的业务，推荐您使用 [云服务器 CVM](https://cloud.tencent.com/product/cvm)。

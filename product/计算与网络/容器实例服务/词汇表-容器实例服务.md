...
## A-J 
### 重启策略 
never：容器实例退出时不会自动重启； on-failure： 当容器实例以非0状态码退出时重启；always：不管容器实例退出状态码是什么始终重启。
### 地域 
...
### 容器状态 
Waiting：创建中；Running：运行中；Terminated：已结束，需要根据 ExitCode 判断结束原因。
### 私有网络 ID
私有网络是自定义的逻辑隔离网络空间。
### 私有网络子网 ID
子网 是 VPC 内的 IP 地址块，私有网络中的所有云资源都必须部署在子网内。
### 实例状态 
Pending：创建中；Running：运行中；Successed：运行成功，已结束；Failed：运行失败，已结束；Unknown：状态异常。

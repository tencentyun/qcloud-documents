...
VPN 连接是通过公网加密通道，将对端 IDC 与私有网络（Virtual Private Cloud，VPC）连接，或与云联网（Cloud Connected Network，CCN）连接的一种方式。建立 VPN 连接后，您在路由表中配置相关路由策略后，可以帮您实现通信。如下图所示，腾讯云 VPN 连接分为如下组成部分：

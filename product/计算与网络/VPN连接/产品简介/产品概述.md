...
## SSL VPN
>?SSL VPN 目前内测中，如需体验，请提交 [内测申请](https://cloud.tencent.com/apply/p/xhfe94i9u8f)。
>
腾讯云 SSL VPN 连接分为如下组成部分:
- SSL VPN 网关：创建的 SSL VPN 网关。当您需要与单个 VPC 通信时，可通过 SSL VPN 网关接入 VPC。
- SSL 服务端：VPN 网关中用于提供 SSL 服务的服务模块，主要实现数据包的封装与解封装。
- SSL 客户端：可以通过任 OpenVPN 的客户端进行访问。

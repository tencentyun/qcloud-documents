...
ip address 120.235.225.76 255.255.255.252  # 配置 VPN 通道本端公网 IP 地址。
```
3. 配置 isakmp 策略。
```
...
4. 配置预共享密码。
```
tunnel-group 159.75.41.242 type ipsec-l2l  # 创建一个ipsec隧道组，type 为点到点。
tunnel-group 159.75.41.242 ipsec-attributes  # 配置隧道组属性，并指定预共享密钥。
...
5. 配置 ACL。
```
access-list INTERESTING extended permit ip 172.30.0.0 255.255.0.0 10.1.1.0 255.255.255.0  # 配置 ACL 抓取 VPN 通道上的数据流。
```
6. 配置 IPsec 策略。
```
crypto map CMAP 1 match address INTERESTING  # 调用 ACL，使满足 ACL 的源网段或者目地网段的数据包在 VPN 通道上流通。
crypto map CMAP 1 set peer 159.75.41.242  # 将被 IPsec 保护的流量转发到的对端 VPN 公网地址，本文此处为腾讯云 VPN 公网地址。
...
8. 配置静态路由。
```
route outside 10.1.1.0 255.255.255.0 159.75.41.242 1  # 将待加密保护的数据网段引向 IPsec 隧道，且配置下一跳为 VPN 隧道对端公网 IP。
```
9. 测试 VPN 连通性。
...
nameif outside  # 定义端口的安全域名。
security-level 0  # 定义端口的安全域等级。
ip address 120.235.225.76 255.255.255.252  # 配置 VPN 通道本端的公网 IP 地址。
```
3. 配置 isakmp 策略。
```
 crypto ikev1 policy 10  # 定义 ikev1 第一阶段协商使用参数，序号为10，序号越小越优先，范围为1-65535。
...
tunnel destination 159.75.41.242  # 配置对端 VPN 的公网 IP 地址，本处为腾讯云 VPN 公网 IP 地址。
tunnel mode ipsec ipv4  # 配置 tunnel口 使用的协议。
tunnel protection ipsec profile PROFILE1  # 调用 IPsec 策略对经过 tunnel 口的数据进行保护。
```
8. 配置静态路由。
```
route vti 10.1.1.0 255.255.255.0 159.75.41.242   # 将待加密保护的数据包引到 tunnel 口。
```
9. 测试 VPN 连通性。
执行 Ping 命令测试 VPN 的连通性。
![](https://main.qcloudimg.com/raw/94c1317f84d1ee2440f7d0efcf914ced.png)
...
```
tunnel-group 159.75.41.242 type ipsec-l2l  # 创建一个ipsec隧道组，type 为点到点。
tunnel-group 159.75.41.242 general-attributes default-group-policy group_policy  # 调用上一步定义的组策略。
tunnel-group 159.75.41.242 ipsec-attributes  # 配置隧道组的属性，并指定预共享密钥。
...
nameif outside  # 定义端口的安全域名。
security-level 0 # 定义端口的安全域等级。
ip address 120.235.225.76 255.255.255.252  # 配置对接腾讯云 VPN 公网 IP 地址。
interface Tunnel100
nameif vti
ip address 172.30.3.2 255.255.255.0 # 该 IP 地址用于激活 Tunnel 口。
```
3. 配置 isakmp 策略。
```

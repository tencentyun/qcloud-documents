...
+ VPC/IDC 侧的安全策略未放通对应源、目的 IP
+ VPC/IDC 侧的内网服务器操作系统的防火墙未放行对端网段
+ VPC/IDC 侧的 SPD 策略未包含该源、目的 IP
...
2. [](id:step2)检查通信是否恢复正常，即登录 VPC/IDC 中的一台服务器，ping 对端服务器内网 IP。[](id:step2)
    + 是 => 通信正常，结束
    + 否 => 请执行 [步骤3](#step3)
...
![](https://main.qcloudimg.com/raw/b452e9b1e2047e8d20e817215253b636.png)
 ...
 display   current-configuration   configuration security-policy
  + 是 => 请执行 [步骤5](#step5)
  + 否 => 请放通安全组/网 ACL/IDC 侧安全设备需要互通的内网地址段，再执行 [步骤4](#step4)
4. [](id:step4)检查通信是否恢复正常，即登录 VPC/IDC 中的一台服务器，ping 对端服务器内网 IP。
    + 是 => 通信正常，结束
    + 否 => 请执行 [步骤5](#step5)
5. [](id:step5)分别检查 VPC 中云服务器和 IDC 侧内网机器操作系统自带防火墙是否有放通对端网段的策略。
   linux 机器查看防火墙：iptables  --list
   Windows 机器查看防火墙：控制面板\系统和安全\Windows 防火墙\允许的应用
   + 是 => 请执行 [步骤7](#step7)
...
8. [](id:step8)检查通信是否恢复正常，即登录 VPC/IDC 中的一台服务器，ping 对端服务器内网 IP。
   + 是 => 通信正常，结束
   + 否 => 请执行 [步骤9](#step9) 
9. [](id:step9)请收集以上检查信息提交工单或联系设备厂商跟进处理。

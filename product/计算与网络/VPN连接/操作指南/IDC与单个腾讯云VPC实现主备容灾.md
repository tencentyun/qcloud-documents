## 容灾方案
![](https://main.qcloudimg.com/raw/5b6a7e473329a9bb8bae9af04ee12cdc.png)
...
![1.png](/download/attachments/864485923/1.png?version=3&modificationDate=1626701935314&api=v2)
  - 网关名称：填写自定义网关名称。
  - 关联网络：选择创建好的 VPC 网络。
  - 带宽上限：依据实际需求选择带宽。
...
## 步骤二：创建 VPN 对端网关[创建对端网关](id:step02)
在腾讯云侧创建对端网关 A 和 B。
1. 在左侧导航栏选择【 VPN 链接】>【对端网关】。
2.  在“对端网关”管理页面，选择地域，单击【+新建】。
3. 填写对端网关名称，公网 IP 填写对端 IDC 侧的 VPN 网关设备的静态公网 IP ，根据需要设置标签。
![1.png](/download/attachments/864485923/1.png?version=4&modificationDate=1626745514862&api=v2)
 - 名称：填写对端网关名称。
 - 公网IP：填写 IDC 侧 VPN 网关所在的 公网 IP 地址。
4.单击【创建】。
5. 在腾讯云侧创建对端网关 B，请重复对端网关 A 的创建步骤1～步骤4。

## 步骤三：创建 VPN 通道（主备）
VPN 网关和对端网关创建完成后，需要创建两条 VPN 网关与 IDC 侧相连的 VPN 通道，一条作为主通道，一条作为备用通道。

### 创建主用通道 A
1. 在左侧导航栏选择【 VPN 链接】>【 VPN 通道】。
2.  在“对端网关”管理页面，选择地域，单击【+新建】。
3. 填写对端网关名称，公网 IP 填写对端 IDC 侧的 VPN 网关设备的静态公网 IP ，根据需要设置标签。
![1.png](/download/attachments/864485923/1.png?version=4&modificationDate=1626745514862&api=v2)
 - 名称：填写对端网关名称。
...
###  创建备用通道 B
重复主用通道的创建步骤1～步骤4，其中配置的 IDC 侧公网 IP 网段，不能与主用通道对应的 IDC 侧公网 IP 网段相同，请配置为 IDC 侧另外一个 VPN 网关的 IP 网段。 
...
## 步骤五 配置 VPN 网关路由
截止至步骤 4，已经将主备 VPN 通道配置成功，需要在 VPN 控制台配置 VPN 网关至 VPN 通道的路由。
1. 在左侧导航栏选择【 VPN 链接】>【 VPN 网关】，并在右侧 VPN 网关列表中找到步骤一创建的 VPN 网关 A，并单击其名称。 
2. 在 VPN 网关 A 详情页签，单击【路由表】页签，并单击【新增路由】。
![1.png](/download/attachments/864485923/1.png?version=13&modificationDate=1628494908625&api=v2)
3.在【新建路由】页面配置 VPN 网关 A 至 VPN 通道 B 和 VPN 通道 C 的路由策略。
![1.png](/download/attachments/864485923/1.png?version=14&modificationDate=1628495289892&api=v2)

| 配置项 | 说明 |
| ------ | ------ |
| 目的端 | 填写待访问的对端网络的网段，即 IDC侧提供对外访问的网段。 |
| 下一跳类型 | 系统自动填充【 VPN 通道】。 |
| 下一跳 | 选择创建好的 VPN 通道。 |
| 权重 | <ul><li>VPN 通道 B 填写 0。</li><li>VPN 通道 C  填写 100。</li></ul> <br>0 表示优先级高，100表示优先级低。|
4. 单击【确定】
...
### 主用通道 B 健康检查配置
1. 在左侧导航栏选择【 VPN 链接】>【 VPN 通道】，并在右侧 VPN 通道列表中找到创建好的 VPN 通道，然后单击 VPN 通道名称。 
2.在通道【基本信息】页签单击【编辑】。
![1.png](/download/attachments/864485923/1.png?version=6&modificationDate=1626767017409&api=v2)
3.打开健康检查开关，输入【健康检查本端地址】和【健康检查对端地址】，并单击【保存】。
![1.png](/download/attachments/864485923/1.png?version=7&modificationDate=1626767173420&api=v2)
>?
...
![1.png](/download/attachments/864485923/1.png?version=11&modificationDate=1626786030688&api=v2)
3. 单击【+新增路由策略】。

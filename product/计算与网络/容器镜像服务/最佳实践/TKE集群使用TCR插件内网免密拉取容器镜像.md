...
为保障用户数据安全，新建的 TCR 实例默认拒绝全部来源的访问。为允许指定 TKE 集群可访问 TCR 实例拉取镜像，需将集群所在的 VPC 关联至 TCR 实例，并配置相应的内网域名解析。具体请参考 [内网访问控制](https://cloud.tencent.com/document/product/1141/41838)。
...
![](https://main.qcloudimg.com/raw/34fa53f8b3a3b6c04f7470ea84613d7b.png)
    ...
    - **免密拉取配置**：建议采用默认配置。插件将自动在集群的命名空间内注入所选 TCR 实例的访问凭证，新建工作负载时默认引用对应的访问凭证。
    - **内网访问配置**：可选。如果已在 TCR 侧配置基于 VPCDNS 的自动解析，可无需启动本插件的内网访问解析功能。若当前集群所在地域暂未支持 VPCDNS，可在新建内网访问链路后，勾选本插件的内网解析功能，插件将自动配置集群节点服务器上的 host，实现 TCR 实例域名的内网解析。
1. 单击【确定】返回组件选择界面。
2. 在组件选择界面单击【完成】，开始为集群安装 TCR 扩展组件。
3. 组件安装完成后，集群将具备内网免密拉取该关联实例内镜像的能力，无需额外配置。如下图所示：
...
1. 完成其他参数设置后，单击【创建workload】，查看该工作负载的部署进度。

...
登录成功后，您可在本地构建新的容器镜像或从 DockerHub 上获取一个公开镜像用于测试。
...
TCR 企业版实例支持网络访问控制，您可根据 EKS 集群的网络配置，选择通过公网或内网访问指定实例，拉取容器镜像。若 EKS 集群与 TCR 实例部署在同一地域，建议通过内网访问方式拉取容器镜像，该方式可提升拉取速度，并节约公网流量成本。下文将介绍通过内网访问的方式，若要通过外网访问，请参考 [通过 NAT 网关访问外网](https://cloud.tencent.com/document/product/457/48710)。
...
从 TCR 实例中拉取容器镜像需要首先使用凭证信息登录至实例。请参考 [获取实例访问凭证](https://cloud.tencent.com/document/product/1141/41829)，保存该实例的长期访问凭证，用于之后配置部署 TCR 镜像。

...
其中，`$region` 描述地域信息，如 ap-guangzhou 即代表广州地域，具体地域列表及缩写请参考 [地域和可用区](https://cloud.tencent.com/document/product/213/6091)，值为空的时候表示所有地域；`$account` 描述资源拥有者的主账号信息，表示为 uin/${uin}，如 uin/12345678，值为空的时候表示创建策略的 CAM 用户所属的主账号。
关于授权策略中的资源描述方式，具体可参考 [资源描述方式](https://cloud.tencent.com/document/product/598/10606)。

...
- 已拥有域名。
  域名，即网域名称（Domain Name），由一串用点分隔的字符组成。在容器镜像服务企业版中用于访问实例服务，且直接影响镜像仓库的访问地址。您可通过腾讯云 [域名服务](https://console.cloud.tencent.com/domain) 注册域名，详情可参见 [域名注册](https://cloud.tencent.com/document/product/242/9595)。
...
- 已开通腾讯云 DNSPod 服务。
  腾讯云 DNSPod 服务可以将自定义域名的访问路由到容器镜像服务企业版实例的相应 IP 地址，详情请参见 [DNS 解析](https://cloud.tencent.com/product/cns) 和 [Private DNS 解析](https://cloud.tencent.com/product/privatedns) 。
...
### 更新域名证书
如因证书过期或升级证书等原因需要更新自定义域名所绑定的证书，单击【更新证书】重新选择 SSL 证书。更新证书操作需要重新下发 SSL 证书，此期间自定义域名仍可正常访问。

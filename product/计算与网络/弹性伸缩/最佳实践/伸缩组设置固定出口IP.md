...
###方案简述
1. 通过负载均衡接收外部请求
2. 将机器放到VPC的子网中，将路由表指向NAT网关，所有统一经NAT网关的外网IP发出
3. 伸缩组的网络属性设为该子网，这样扩容出来的机器都会统一用NAT网关主动外访
...
2) 点击【新建】，填写子网络名称、CIDR、可用区和关联路由表。然后点击【创建】来确认。

3）新建完后，您可以购买机器到这个子网中了
...
- 为 NAT 网关分配弹性 IP，这个IP就是以后您机器外访的固定IP
...
- 选择“网络”和“子网”，指向刚才设置好的VPC和子网**（重要）**。如下图：

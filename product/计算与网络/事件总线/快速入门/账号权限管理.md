## 简介
腾讯云事件总线通过[访问管理（Cloud Access Management，CAM）](https://cloud.tencent.com/document/product/598)来实现权限管理。CAM 是腾讯云提供的权限及访问管理服务，主要用于帮助客户安全管理腾讯云账户下的资源的访问权限。用户可以通过 CAM 创建、管理和销毁用户（组），并使用身份管理和策略管理控制其他用户使用腾讯云资源的权限。

## 主账号授权
主账号第一次使用事件总线服务时，根据 CAM 要求，您需要开通事件总线服务角色**EB_QCSRole**，并授权服务角色相关权限来调用其它服务。进入控制台，根据提示完成授权即可：
![](https://qcloudimg.tencent-cloud.cn/raw/008e5af6fa3bbbd079d80b5d2829ebe4.png)
![](https://qcloudimg.tencent-cloud.cn/raw/22b196c10807f022e5055044c7d188ce.png)

## 子账号授权步骤
>! 注意：子账号使用前，主账户需要在[CAM 角色管理控制台](https://console.cloud.tencent.com/cam/role)先确认是否已经成功创建 `EB_QCSRole`，如果没有，请按照上述授权操作完成授权，否则子用户无法正常使用 EB 控制台和通过 EB 调用其他云上资源。
...
3. 选择「按策略语法创建」--「空白模版」，自定义名称后，输入下列语法内容：
```json
{
    "version": "2.0",
    "statement": [
...
```
![](https://qcloudimg.tencent-cloud.cn/raw/45ee6038c3d9d5d3de0d47d59eb9601a.png)

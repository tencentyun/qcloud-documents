若您在混合云连接时，专线两端 IP 发生冲突，您可以通过 NAT 型的 VPC 专线网关配置网络地址转换功能来解决，即在 IP 数据包通过路由器或防火墙时重新配置报文头部的源 IP 地址、目的 IP地址、端口等信息。本文将介绍如何配置 IP 转换和 IP 端口转换，并提供相关示例帮助您了解。

## 前提条件
您已创建 NAT 型专线网关，具体操作请参见 [创建 VPC 专线网关]()。
...
- 原 IP 和映射 IP 具有唯一映射关系，即私有网络内1个 IP 只能唯一映射为1个 IP。
...
- 原 IP 端口和映射 IP 端口具有唯一映射关系，即私有网络内同一 IP 端口只能唯一映射为一个 IP 端口。

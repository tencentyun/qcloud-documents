...
- 专线网关的 IDC 方向的路由
  - 若专线通道为 BGP 模式，则专线网关通过 BGP 协议学到 IDC 方向的路由策略。
  - 若专线通道为静态模式，则专线网关通过在专用通道 IDC 网段录入路由信息，学习到 IDC 方向的路由策略。
- 专线网关的云联网方向路由
...
3. 在专线网关详情页面单击【路由表】页签。查看专线网关 IDC 方向和云联网方向的路由表，若需下载路由表信息，请单击<img src="https://main.qcloudimg.com/raw/5be52268cd6656b7fccb91180c187035.svg" style="zoom:10%;" />。若路由表中同时存在多个路由策略时，转发优先级如下：
 ...
 - 云联网方向
    - 若目的网段 CIDR 不同，则按最长掩码匹配原则进行路由转发。
    - 若目的网段 CIDR 相同，则 AS-Path 较短的优先转发。
    - 若 AS-Path 相同，云联网接收所有路由。

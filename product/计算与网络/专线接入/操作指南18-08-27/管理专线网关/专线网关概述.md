...
## 高可用概述[](id:dc-dsr)
DSR（Disaggregated Software-Defined Router，面向大规模云网络服务的分布式路由系统），是腾讯基于 SDN、NFV 和微服务技术自助研发的新一代软件路由系统，用于替代传统的商业路由器，广泛部署的腾讯超大规模、高性能、高弹性的云网络系统。
 腾讯云专线继承了腾讯云 DSR 高可用特性，在腾讯云侧2个边界 IP 地址分别作用于两个不同的 DSR 集群上，这样 IDC 侧本地路由器通过 BGP 协议分别与两个 DSR 集群分别建立了 BGP 邻居关系，有效的保证了 DSR 集群升级或者单集群故障时业务的高可用，避免因单 BGP 邻居中断导致路由收敛而对业务产生的影响。 
![](https://main.qcloudimg.com/raw/e1223de6586b26115d721c55245544f5.png)

1. 腾讯云侧：DSR 通过 BGP 协议学习腾讯云到用户 IDC 的路由，下一跳为用户本地路由器。
2. 用户 IDC 侧：用户本地路由器通过 BGP 协议学习 IDC 到腾讯云 VPC 的路由，下一跳为 2个 DSR 集群的 IP 地址。

...
<img width="80%" src="https://main.qcloudimg.com/raw/937d386542e93d30398c8e96a9eaf047.png" style="zoom:67%;" />
- 若专用通道为 BGP 模式，IDC 到腾讯云 VPC 方向的路由目的网段，为本地路由器通过 BGP 协议学习到的 VPC CIDR（172.21.0.0/16）。
<img width="80%" src="https://main.qcloudimg.com/raw/95f042fe8da5c77b6d58e1f4799946bb.png" style="zoom:67%;" />
...
- 当专用通道 A 和专用通道 B 均为 BGP 模式时，本地路由器向专线网关 A 发送的路由目的网段为通过 BGP 协议学习的子网 CIDR（172.21.0.0/20、172.21.16.0/20），向专线网关 B 发送的路由目的网段为通过 BGP 协议学习的 VPC CIDR（172.21.0.0/16）。由于本地路由器按最长掩码匹配原则进行转发，因此流量将全部转发至专线网关 A。当专用通道 A 故障时，IDC 侧去往专线网关A 的路由条目消失，上云流量才会转发至专线网关 B。
>? 您可以提工单申请更改专线网关 B 的对外发布路由规则）。
>

...
4. 在此，您可[ 配置 IP 转换](#.E9.85.8D.E7.BD.AE-ip-.E8.BD.AC.E6.8D.A2) 和 [配置 IP 端口转换](#.E9.85.8D.E7.BD.AE-ip-.E7.AB.AF.E5.8F.A3.E8.BD.AC.E6.8D.A2)。

## 配置 IP 转换
### 配置本端 IP 转换
在专线网关详情页中，选择【本端 IP 转换】选项卡，进行本端 IP 转换配置：
#### 1. 配置本端 IP 转换规则
- **新增**
 1. 在 IP 映射页左上角，单击【新增】。
 ![](https://main.qcloudimg.com/raw/272179c4b42889d1135b425b1d258262.png)
 2. 在弹框中，输入原 IP、映射 IP 及备注，单击【确定】即可。
...
>
- **修改**
在 IP 映射页中，单击 IP 转换规则所在行右侧的【修改 IP 映射】，即可编辑本端 IP 转换规则的原 IP、映射 IP 和备注，单击【确认】 后，IP 转换生效。
...
![](https://main.qcloudimg.com/raw/1e00b3d213e89f180fe178fa18f5b1b5.png)
...
- **删除**
 1. 在 IP 映射页中，单击 IP 映射规则所在行右侧的【编辑 ACL 规则】，进入 ACL 规则编辑状态，单击规则右侧的【删除】，并保存操作，即可删除该 ACL 规则。
![](https://main.qcloudimg.com/raw/928b23767dbe3d0a227100870fc9683a.png)
...
#### 规则限制
 ...
 - 映射 IP 唯一不可以重复，即不支持多个私有网络IP映射为同1个 IP。
 - 原目的 IP 不支持广播地址（255.255.255.255）、D 类地址（224.0.0.0 - 239.255.255.255）、E 类地址（240.0.0.0 - 255.255.255.254）。
 - 专线网关的本端IP转换规则数上限为100条，每个本端 IP 转换最大支持20条 ACL 规则（如有需要提升配额，请提交 [工单申请](https://console.cloud.tencent.com/workorder/category)）。

### 配置对端 IP 转换
在专线网关详情页中，选择【对端 IP 转换】选项卡，进行对端 IP 转换配置：
 - **新增**
  1. 在 IP 映射页左上角，单击【新增】。
...
![](https://main.qcloudimg.com/raw/362404885ef7e18b2ae2d3af7883f779.png)
 - **修改**：单击 IP 转换规则所在行右侧的【修改 IP 映射】，即可修改对端 IP 转换规则的原 IP、映射 IP 和备注，单击【确定】后 IP 转换生效。
...
#### 规则限制
 ...
 - 原目的 IP 不支持广播地址（255.255.255.255）、D 类地址（224.0.0.0 - 239.255.255.255）、E 类地址（240.0.0.0 - 255.255.255.254）。
 - 专线网关的对端 IP 转换规则数上限为100条（如有需要提升配额，请提交 [工单申请](https://console.cloud.tencent.com/workorder/category)）。

## 配置 IP 端口转换
### 配置本端源 IP 端口转换
在专线网关详情页中，选择【本端源 IP 端口转换】选项卡，进行本端源 IP 端口转换配置：
#### 1. 配置本端 IP 端口转换地址池
- **新增**
...
#### 规则限制
 ...
 - 本端源 IP 端口转换最大支持100个 IP 地址池，每个地址池支持最大20条 ACL 规则（如有需要提升配额，请提交 [工单申请](https://console.cloud.tencent.com/workorder/category)）。
...
#### 规则限制
 ...
 - 本端目的 IP 端口转换最大支持100个 IP 端口映射（如有需要提升配额，请提交 [工单申请](https://console.cloud.tencent.com/workorder/category)）。

...
- IP A 对专线对端的主动访问网络包源 IP 将自动修改为 `10.100.0.3`。
...
- IP D `10.0.0.3`主动访问私有网络的网络包源 IP ，将自动修改为 IP C `172.16.0.3`。
...
完成配置后，私有网络 C 内主动访问银行 A、B 的网络请求，会对应的根据 ACL 规则分别转换为对应映射 IP 池的随机端口，访问对应的专用通道。
...
#### 配置示例
私有网络 C 的网段为 `172.16.0.0/16`，仅希望开放部分端口给专线对端的第三方银行 A 和 B 主动访问，其中银行 A 对端网段为`10.0.0.0/28`；银行 B 对端网段为 `10.1.0.0/28`，则可以按照下面方案配置 A、B 两条本端目的 IP 端口映射：
- 映射 A：原 IP 端口 `172.16.0.1:80`，映射后 IP 端口 `10.0.0.1:80`。
- 映射 B：原 IP 端口 `172.16.0.0:8080`，映射后 IP 端口 `10.0.0.1:8080`。

完成配置后，专线对端可以主动访问 `10.0.0.1:80`、`10.0.0.1:8080` 端口，实现对私有网络内`172.16.0.1:80`、`172.16.0.0:8080` 两个端口的主动访问。

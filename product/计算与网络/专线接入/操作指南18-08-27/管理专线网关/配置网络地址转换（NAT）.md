...
6. 在弹框中，输入原 IP、映射 IP 及备注（可选），单击【确定】即可。
![](https://main.qcloudimg.com/raw/42172589f8ebd012cc9f2a6a3eec556a.png)
7. （可选）新增本端 IP 映射，默认添加了 ALL PASS 的 ACL 规则，即本端 IP 转换对所有专用通道生效，您可以编辑本端 IP 转换的 ACL 规则，以改变本端 IP 转换的适用范围。
>?
...
8. （可选）如果您需修改本端 IP 映射，可在 IP 映射页中，单击 IP 映射所在行右侧的【修改 IP 映射】，即可修改本端 IP 映射的原 IP、映射 IP 和备注（可选），单击【确认】 后，IP 映射生效。
...
7. （可选）如果您需修改对端 IP 映射，可在 IP 映射页中，单击 IP 映射所在行右侧的【修改 IP 映射】，即可修改对端 IP 映射的原 IP、映射 IP 和备注（可选），单击【确定】后，对端 IP 映射生效。
...
6. 在弹框中，输入映射 IP 池（支持 IP 或 IP 段，IP 段格式为 “A - B”）和备注（可选），单击【确定】即可。
...
6. 在弹框中，选择协议，输入原 IP 端口、映射后 IP 端口及备注（可选），单击【确定】即可。

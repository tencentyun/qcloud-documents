...
## 排查数据链路层故障[](id:2)
1. 若 VLAN ID 不为0时，请确保运营商专线、腾讯云边界设备和本地 IDC 接入设备（包括 IDC 边界设备自身）之间的任何二层/三层设备，对您的 VLAN 标签启用了 VLAN 中继，即识别您的 VLAN 标签并允许其通过。
2. 若 VLAN ID 不为0时，请确保腾讯云边界设备和您本地 IDC 边界设备（包括 IDC 边界设备自身）之间的任何二层/三层设备，对该 VLAN 进行正确的中继，且不存在 VLAN 转换的情况。
3. 请检查是否正确配置互联网 IP。确保本地 IDC 接入设备上 ARP 表项中，存在专线网关上互联 IP 对应的表项，且保持稳定，无 MAC 地址 flapping 记录。

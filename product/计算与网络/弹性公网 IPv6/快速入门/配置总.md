...
```
 ...
 - 若返回`inet6`相关内容，表示实例已成功开启 IPv6 服务，您可以跳过步骤2继续操作。
...
5. 按 “i” 切换至编辑模式，对如上两个文件中的 IPv6 相关参数均设置为0：
     ```bash
     #net.ipv6.conf.all.disable_ipv6 = 1
     #net.ipv6.conf.default.disable_ipv6 = 1
     #net.ipv6.conf.lo.disable_ipv6 = 1    
     ...
     net.ipv6.conf.lo.disable_ipv6 = 0
     ```
6. 修改完成后，按 “Esc”，输入`:wq`并回车，保存文件并返回，重启实例。
 ...
 - Tlinux 1/2、CentOS 6/7 的操作步骤如下：
    ...
    3. 按 “i” 切换至编辑模式，在文件中根据实际信息添加如下配置：
        ...
        - 多 IPv6 地址：
          ...
          IPV6_DEFAULTGW=<IPv6网关>
          ```
     4. 按 “Esc”，输入:wq并回车，保存文件返回后，重启网络服务，执行`service network restart`或`systemctl restart network`即可。
   ...
   - Debian 7/8/9 和 Ubuntu 14/16 的操作步骤如下：
    ...
    3. 按 “i” 切换至编辑模式，在文件中根据实际信息添加如下配置：
        ...
        - 多 IPv6 地址：
          ...
          gateway <IPv6网关>
          ```
     4. 按 “Esc”，输入:wq并回车，保存文件返回后，重启网络服务，执行`service network restart`或`systemctl restart networking`即可。
...
>?仅添加 addresses 和 gateway6。
     3. 按 “Esc”，输入:wq并回车，保存文件返回后，执行如下命令使配置生效。
        ```
        netplan apply
        ```
  ...
  - OpenSUSE 42 和 SUSE 操作步骤如下：
    ...
    **手动方式：**
    ...
    5. 按 “i” 切换至编辑模式，添加配置项：
       ...
       default <IPv6网关> - -
       ```
    6. 按 “Esc”，输入:wq并回车，保存文件返回后，重启网络服务，执行`service network restart`或`systemctl restart networking`即可。
  ...
  - CoreOS 17 的操作步骤如下：
	...
	3. 按 “i” 切换至编辑模式，在文件中根据实际信息添加如下配置：
       ...
       - 多 IPv6 地址：
         ...
         Gateway=<IPv6网关>
         ```
    4. 按 “Esc”，输入:wq并回车，保存文件返回后，重启网络服务，执行`systemctl restart systemd-networkd`即可。      
  ...
  - FreeBSD 11 的操作步骤如下：
    ...
    2. 执行如下命令，打开网卡配置文件（`vtnet0`为网卡标识符，您需要修改成实际的标识符）。
     ...
     4. 按 “Esc”，输入:wq并回车，保存文件返回后，重启网络服务，执行`/etc/netstart restart`即可。

### SSH 支持 IPv6 配置
>?如果需要使用 IPv6 地址远程连接，则需要开启 SSH 的 IPv6 支持。

操作如下：
1. 执行如下命令，打开 SSH 的配置文件。`/etc/ssh/sshd_config`。
```
/etc/ssh/sshd_config
```

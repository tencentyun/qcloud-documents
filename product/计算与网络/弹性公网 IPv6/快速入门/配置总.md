 ...
 - Tlinux 1/2、CentOS 6/7 的操作步骤如下：
    ...
    3. 按 “i” 切换至编辑模式，在文件中根据实际信息添加如下配置：
        ...
        - 多 IPv6 地址：
          ...
          IPV6_DEFAULTGW=<IPv6网关>
          ```
     4. 按 “Esc”，输入`:wq`并回车，保存文件返回后，重启网络服务，执行`service network restart`或`systemctl restart network`即可。
  ...
  - CoreOS 17 的操作步骤如下：
	...
	3. 按 “i” 切换至编辑模式，在文件中根据实际信息添加如下配置：
       ...
       - 多 IPv6 地址：
         ...
         Gateway=<IPv6网关>
         ```
    4. 按 “Esc”，输入`:wq`并回车，保存文件返回后，重启网络服务，执行`systemctl restart systemd-networkd`即可。      

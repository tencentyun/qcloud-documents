...
## 前提条件
按照以下条件准备好边缘节点，可使用通过 [云服务器控制台](https://console.cloud.tencent.com/cvm) 或 [边缘计算机器控制台](https://console.cloud.tencent.com/ecm/instance) 已购买服务器、其他平台或自建机房的服务器。
...
## 操作步骤
1. 登陆腾讯云容器服务控制台，选择左侧导航栏中的【[边缘集群](https://console.cloud.tencent.com/tke2/edge)】。
...
5. 在弹出的“添加节点”窗口中，按照指引进行配置，得到节点侧初始化脚本。
   1. 在“配置”步骤中，获取初始配置并修改成合适的值：
...
![](https://main.qcloudimg.com/raw/07bd1050fa0ea9677f2fed954a74666d.png)
   3. 在“安装”步骤中，保存已生成的节点初始化脚本内容，本文以保存至 `script.sh` 文件为例。
4. 登录已准备好的服务器，并切换至 `root` 账户。
5. 依次执行以下命令，执行初始化节点脚本。
```
chmod +x script.sh
```
```
./script.sh
```
6. 脚本执行成功后，前往“节点列表”页面并刷新，即可查看已新增的节点。如下图所示：
...
1. 在“边缘集群”列表页面，选择需关闭外网访问集群所在行右侧的【查看集群凭证】。如下图所示：
![](https://main.qcloudimg.com/raw/72942559f6fcb4b1e573320381090b68.png)
2. 在弹出的“集群凭证”窗口中，单击“外网访问”中的<img src="https://main.qcloudimg.com/raw/84c11b68fbbd46c46c2cae68d45baee2.png" style="margin:-6px 0px">，即可关闭该集群外网访问。

当您使用腾讯云容器服务 TKE 组建 Kubernetes 集群时，会面对多种配置选项，难以进行选择。本文介绍其中主要的功能选型，进行对比并给出选型建议。您可参考本文，选择更适用于您业务的配置选型。

## Kubernetes 版本
Kubernetes 版本迭代较快，新版本通常包含许多 bug 修复和新功能，而旧版本会逐渐淘汰。建议您在创建集群时，选择当前 TKE 支持的最新版本，后续版本支持可通过 Master 和节点版本升级实现。
...
- 若后期部分业务需使用 VPC-CNI 模式，可在 GlobalRouter 集群中再开启 VPC-CNI 支持，也就是 GlobalRouter 与 VPC-CNI 混用，仅部分业务使用 VPC-CNI 模式。
...
- **Docker 作为运行时架构**：
![](https://main.qcloudimg.com/raw/d859ce047bff74bcd7147f2236eeecbe.jpg)
	- 调用链如下：
...
![](https://main.qcloudimg.com/raw/f5f15ac8ce703e7cb49f910cd528e00e.jpg)
...
Service 转发原理图如下所示：
![](https://main.qcloudimg.com/raw/429df8fdb2a12b16403000b35ba73f1f.jpg)
...
### 选型建议
对稳定性要求极高且 Service 数量小于2000时，建议选择 iptables，其余场景首选 ipvs。
...
### 选型建议
建议通常情况下选择托管集群，如需完全掌握 Master 时，例如对 Master 进行个性化定制实现高级功能，则可选择使用独立集群。


## 节点操作系统
TKE 支持 Ubuntu 和 CentOS 两类发行版，带 `TKE-Optimized` 后缀的操作系统使用了 TKE 定制优化版的内核，其余的操作系统使用了 Linux 社区官方开源内核。如下图所示：
...
## 使用启动脚本
### 组件自定义参数
>?如需使用该功能，请通过 [工单申请](https://console.cloud.tencent.com/workorder/category)。
>

...
上述三种计费模式中，由于用户购买的是一个整体实例，因此按照实例计费，不针对单独一个资源计费。通过使用 Kubecost，可以参照模型分析资源类型分摊成本，每种资源分摊多少成本。

例如，一个云厂商提供的某种 Node（虚拟机、物理机），CPU 1核(C) 内存 Mem(G) 按照20元/月计算, 对于不清楚具体某种资源如何计价的云厂商，只知道整个 Node 如何计费，但是这其中并不清楚分摊到每种资源（CPU/GPU/Mem/PV/Network） 是如何计费。

当前分摊模型比较简单，我们还需知道每种资源的基本价格，例如 CPU 如何收费、Mem 如何收费，这类资源价格目前可以按照云厂商说明的基础价格来定，或者自己去配置对应的比例，例如 1C:1G 的价格比是3:1，CPU/Mem=3:1，则 CPU 按照15元/月、Mem 按照5元/月进行分摊成本，加起来即为整个 Node 的价格20元/月。具体计算公式如下：
...
成本加权平均评估效率，由于每一种资源的成本权重不一样（成本权重即为不同类型的资源，售卖的价格也不一样。例如 CPU 和 GPU 等计算资源价格相对较高，Mem 价格相对较低，而 Disk 价格则更低），相同的资源利用效率，不同资源对成本的贡献度也不一样。

例如，100%的 Disk 利用率，但是由于磁盘相对便宜，对最终的成本控制的贡献就较低。但如果是 CPU 资源，即使资源利用率为30%，但由于 CPU 资源价格相对昂贵，最终可能对成本起到关键作用，因此需要用加权平均来评估成本效率，例如：

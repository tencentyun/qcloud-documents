...
##  JMX Exporter 简介
Java Management Extensions，JMX 是管理 Java 的一种扩展框架，JMX Exporter 基于此框架读取 JVM 的运行时状态。

JMX Exporter 利用 Java 的 JMX 机制来读取 JVM 运行时的监控数据，然后将其转换为 Prometheus 可辨识的 metrics 格式，以便让 Prometheus 对其进行监控采集。


## 操作步骤

### 使用 JMX Exporter 暴露 JVM 监控指标
JMX Exporter 提供**启动独立进程**及 **JVM 进程内启动（in-process）**两种使用方式：
...
```
>!更多配置项请参考官方文档 [Prometheusr](https://prometheus.io/docs/introduction/overview/)。
>
2. 准备 jar 包文件，可前往 [jmx_exporter](https://github.com/prometheus/jmx_exporter) 的 Github 页面找到最新的 jar 包下载地址。执行以下命令，下载到当前目录。
``` bash
...
```
至此已完成镜像打包，您还可利用 docker 多阶段构建，省掉手动下载 jar 包的步骤。Dockerfile 示例如下：
``` dockerfile
...
* 该示例使用 8088 端口暴露 JVM 的监控指标，您可按需自行更改。
...
``` yaml
    - job_name: tomcat
      ...
      - action: keep
        ...
        regex: jmx-metrics
```
若已安装 prometheus-operator，则可通过创建 ServiceMonitor 的 CRD 对象来配置 Prometheus。示例如下：
``` yaml

...
Tencnet Linux 内核中实现了类似 LXCFS 特性，用户无需在节点部署 LXCFS 文件系统，也无需修改 POD spec。只需在节点开启一个全局开关 `sysctl -w kernel.stats_isolated=1`，容器中读取 `/proc/cpuinfo`，`/proc/meminfo` 等文件获取的信息即已经容器隔离。
另外，考虑到有些特殊容器，例如节点监控组件，可能需要读取节点级的信息。为了解决这个问题，专门增加了容器级的开关`kernel.container_stats_isolated`。在主机级开关开启的情况下，只需要在容器的启动脚本里面，关闭容器级的开关 `sysctl -w kernel.container_stats_isolated=0`，在此容器中读取 `/proc/cpuinfo`，`/proc/meminfo` 等文件获取的就是主机的信息了。

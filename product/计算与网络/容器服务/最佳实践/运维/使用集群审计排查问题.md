## 操作场景
由于人为误操作、应用出现 bug 或恶意程序调用 apiserver 接口，集群资源会被删除或修改。通过开启集群审计，记录 apiserver 的接口调用，就可以根据条件检索和分析审计日志找出问题原因。
...
## 前提条件
登录 [容器服务控制台](https://console.cloud.tencent.com/tke2)，开启集群审计功能。
...
#### 场景示例1
如节点被封锁后，需查出操作者，可执行以下命令，进行检索：
```
objectRef.resource:nodes AND requestObject:unschedulable
```
单击【版面设置】，在“版面设置”页面中设置显示 `user.username`, `requestObject` 和 `objectRef.name` 三个字段，分别表示做操作的用户、请求内容以及节点名称。查询结果如下图所示：
![](https://main.qcloudimg.com/raw/00577669ac93fd7a176cdbb46ba51319.png)
由图可见，是 `10001****958` 这个子账号在 `2020-10-09 16:13:22` 的时候对 `main.63u5qua9.0` 这台节点进行了封锁操作。可以根据账号 ID 在【访问管理】>【[用户列表](https://console.cloud.tencent.com/cam)】中找到关于此子账号的详细信息。


#### 场景示例2
如果某个工作负载被删除，需查出操作者，可执行以下命令，进行检索：
```
...
为避免恶意程序或 bug 导致对 apiserver 请求频率过高，使得 apiserver/etcd 负载过高，影响正常请求，apiserver 有默认请求频率限制保护。如发生限频，可通过审计找到发出大量请求的客户端。
如需通过 userAgent 分析统计请求的客户端，首先要修改日志主题的键值索引，为 userAgent 字段开启统计。如下图所示：
<img style="width:80%" src="https://main.qcloudimg.com/raw/9e2579381976f94156a7d425a490e971.png" data-nonescope="true">
执行以下命令，对每种客户端请求 apiserver 的 QPS 大小进行统计：
```
...
```
切换到图标分析，选择折线图，X 轴用 time，Y 轴用 qps，聚合列使用 userAgent。如下图所示：
...
```
同理，如果要使用其它字段来区分要统计的客户端，可以根据需求灵活修改 SQL，比如使用 user.username 来区分。SQL 语句可参考如下示例：
```

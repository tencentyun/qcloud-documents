...
如果您的域名使用腾讯云 [DNSPod](https://docs.dnspod.cn/) 管理，想在 Kubernetes 上为域名自动签发免费证书，可以使用 cert-manager 来实现。
...
在 DNSPod 控制台，在 [密钥管理](https://console.dnspod.cn/account/token) 中创建密钥，然后复制自动生成的 `ID` 和 `Token` 并保存下来，以备后面的步骤使用。如下图所示：

![](https://main.qcloudimg.com/raw/2c7f32cae8693fd855b835dfd1f9d532.png)

### 2. 安装 cert-manager

阅读前面推荐的 [使用 cert-manager 签发免费证书 ](https://cloud.tencent.com/document/product/457/49368) 文章，安装 cert-manager。

### 3. 安装 cert-manager-webhook-dnspod

下面使用 HELM 来安装下 cert-manager-webhook-dnspod ，首先准备下 HELM 配置文件 (`dnspod-webhook-values.yaml`):
...
使用用下面的 YAML 文件创建 `Certificate` 对象来签发免费证书:
...
证书签发成功后会保存到指定的 Secret 中，下面给出一些使用示例。

...
为了避免此类情况发生，我们要有合理的副本数量，还需要让不同副本调度到不同的节点，分散开以避免单点故障。可以利用反亲和性来实现，示例如下：
```
affinity:
 podAntiAffinity:
...
如果您有更高的要求，例如避免调度到同一个可用区的节点，实现异地多活，则可以使用 `failure-domain.beta.kubernetes.io/zone`。但通常情况下，同一个集群的节点都在一个地域，如果节点跨地域，即使使用专线，时延也会很大。
如果无法避免调度到同一个地域的节点，则可以使用 `failure-domain.beta.kubernetes.io/region`。
...
可以看出，以上过程是先删除再创建，并非滚动更新。因此在更新过程中，如果一个服务的所有副本都在被驱逐的节点上，则可能导致该服务不可用。由于节点被驱逐导致服务不可用的情况会有以下两种：
...
转发规则更新不及时这一现象的出现，主要是由于 Kubernetes 的设计理念中各个组件的逻辑是解耦的，它们各自使用 Controller 模式，listAndWatch 感兴趣的资源并做出相应的行为，使得从 Pod 创建或销毁到 Endpoint 更新再到节点上的转发规则更新的整个过程是异步的。
...
- <span id="CaseTwo"></span>情况二：Pod 被销毁，但是从 Endpoint Controller watch 到变化，更新 Service 对应的 Endpoint 以及 kube-proxy 更新节点转发规则这期间是异步的，存在时间差，在这个时间差内 Pod 可能已经完全被销毁了，但转发规则还未更新，就会造成新的请求依旧还能被转发到已经被销毁的 Pod，导致连接被拒绝。
...
在现网运营中，很多场景为了提高效率一般都采用建立长连接的方式来请求服务端。我们发现在客户端以长连接的方式请求服务端的场景下，Kubernetes 的自动扩容会失效。原因是客户端长连接一直保留在老的 Pod 容器中，而新扩容的 Pod 未接收到新的连接请求，导致 Kubernetes 按照步长扩容第一批 Pod 后就停止了扩容操作，而新扩容的 Pod 未能承载请求，出现服务过载的情况，自动扩容失去了意义。

解决方法是将长连接转换为短连接。我们参考了 nginx keepalive 的设计，nginx 中的 `keepalive_requests` 配置项设定了一个 TCP 连接能处理的最大请求数，达到设定值（例如1000）之后服务端会在 HTTP 的 Header 头标记 `“Connection:close”`，通知客户端处理完当前的请求后关闭连接，新的请求则需要重新建立 TCP 连接。所以在此过程中不会出现请求失败，同时实现了将长连接按需转换为短连接的目的。通过这个办法客户端和云 Kubernetes 服务端在处理完每一批请求后，会不断的更新 TCP 连接，使自动扩容的新 Pod 能接收到新的连接请求，从而解决了自动扩容失效的问题。 

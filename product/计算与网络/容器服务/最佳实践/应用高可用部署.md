...
可以看出，以上过程是先删除再创建，并非滚动更新。因此在更新过程中，如果一个服务的所有副本都在被驱逐的节点上，则可能导致该服务不可用。由于节点被驱逐导致服务不可用的情况有以下两种：
...
在现网运营中，很多场景为了提高效率一般都采用建立长连接的方式请求服务端。而在客户端以长连接的方式请求服务端的场景下，Kubernetes 的自动扩容会失效。原因是客户端长连接一直保留在之前的 Pod 容器中，而新扩容的 Pod 未接收到新的连接请求，导致 Kubernetes 按照步长扩容第一批 Pod 后就停止了扩容操作，而新扩容的 Pod 未能承载请求，出现服务过载的情况，自动扩容失去了意义。

解决方法是将长连接转换为短连接，可参考 nginx keepalive 的设计。nginx 中的 `keepalive_requests` 配置项设定了 TCP 连接能处理的最大请求数，达到设定值（例如1000）后服务端会在 HTTP 的 Header 中标记 `“Connection:close”`，通知客户端处理完当前的请求后关闭连接，新的请求则需要重新建立 TCP 连接。在此过程中不会出现请求失败，同时实现了将长连接按需转换为短连接。通过此方式，客户端和云 Kubernetes 服务端在处理完每一批请求后，会不断的更新 TCP 连接，使自动扩容的新 Pod 能接收到新的连接请求，解决自动扩容失效的问题。 

In order to make better use of Kubernetes orchestration engine, and combine it with Tencent Cloud basic services at IaaS layer more effectively, Tencent KTE extends the Kubernetes native orchestration syntax. The following describes the extended syntax.

## Extended Syntax - Use of CBS Disk

In KTE, a CBS disk can be directly mounted to a service pod. You can proceed as follows:
...
In basic syntax, you can set the service access method by setting `type` field in `Service`. In KTE definition, `service.kubernetes.io/qcloud-loadbalancer-internal-subnetid` is defined in `annotations` to specify whether it is VPC-based load balancer access or public network-based load balancer access.

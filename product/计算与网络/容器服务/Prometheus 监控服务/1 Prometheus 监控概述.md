...
腾讯云Prometheus 监控作为超轻量、高可用、无侵入式的监控系统。
...
Prometheus 监控支持多集群监控、支持同一 VPC 网络中非集群内业务的监控、支持超大集群的监控并实时进行监控组件的扩缩容，保障高可用的监控服务。

在用户关联集群后，Prometheus 监控将默认添加社区主流的采集配置，用户在不做任何个性化配置的基础上能够开箱即用。

...
边缘集群中已经提交了一个 **nvidia-device-plugin** 的 DaemonSet，镜像版本 `nvidia-device-plugin:v0.9.0`，边缘节点要是需要，可执行如下命令边便会自动部署到相应节点：
...
#### 1. 添加带 **NVIDIA 系列 **GPU 的 CVM 节点

目前支持的 **NVIDIA 系列 **GPU 版本：
...
>    注意：添加带GPU的CVM系统版本是[边缘节点系统](#createCVMNode) 的子集，要是边缘节点的系统不支持，添加节点可能会失败，请遵守[边缘节点的系统](#createCVMNode) 要求。

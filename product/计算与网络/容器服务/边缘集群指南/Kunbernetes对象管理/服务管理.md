...
Service 支持以下类型：
- 仅在集群内访问：将提供一个可以被集群内其他服务或容器访问的入口，支持 TCP/UDP 协议。例如，MySQL 数据库类的服务可以该类型，来保证服务网络隔离性。详情请参见 [Kubernetes Service](https://kubernetes.io/docs/concepts/services-networking/service/)。

...
## 成本大师组件的内存消耗
成本大师 - 成本洞察模块因为包含大量的用量数据和成本数据，因此需要占用较多的内存资源。下表中：Workload、Namespace 等其他维度未标出，维度越高，数据条数越少（例如 Cluster 维度，只有一条数据），对整体影响越小，因此仅以 Pod 和 Contianer 作为计算参考。下面是按照一个集群里面的 Pod 和 Container 的数量，预估成本大师 - 成本洞察组件消耗的内存量，真实值可能上下浮动。

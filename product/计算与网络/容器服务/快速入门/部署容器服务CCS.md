...
5. 填写云主机配置并单击 【完成】。
 - **操作系统**：选择操作系统。
 - **系统盘**：提供本地硬盘、普通云硬盘、SSD云硬盘三种服务选择。本地硬盘固定为 50GB，系统盘不支持更换介质，使用本地硬盘的服务器暂不支持升级CPU/内存/硬盘；普通云硬盘适用于大文件顺序读写场景，如日志流水、流媒体业务，数据仓储等；SSD云硬盘的后端使用NVMe固态存储，适用于核心数据库业务、大型OLTP业务，以及对IO性能有极高的要求的业务。

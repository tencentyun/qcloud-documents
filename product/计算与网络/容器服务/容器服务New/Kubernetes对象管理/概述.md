...
您可以通过控制台直接操作原生 Kubernetes 对象，例如 Deployment、deamonset等。
...
- 工作负载
    + Deployment：管理根据调度规则在集群内部署的 Pod。
    + StatefulSet：管理所有应用程序的工作负载 API 对象。
...
- 存储
    ...
    + Persisten Volumes（PV）：Kubernetes 集群中配置的一块存储。
    + Persisten Volumes Claim（PVC）：请求存储的声明。如果把 PV 比作 Pod，那么 PVC 相当于工作负载。
    + Storage Classes：用于描述存储的类型。 创建 PVC 时，通过 Storage Classes 创建指定类型的存储，即存储的模板。

Kubernetes 对象还包括 Namespaces、HPA、Resource Quotas等数十种，您可以根据业务需要使用不同的 Kubernetes 对象。 不同版本的Kubernetes 可使用的对象也不相同，更多说明可登录 [Kubernetes 官方网站](https://kubernetes.io/docs/concepts/) 查询。

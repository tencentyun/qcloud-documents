...
这里使用了tutum/wordpress Docker镜像来创建一个公开访问的Wordpress网站

创建单实例版的wordpress仅供测试使用，该镜像中包含了wordpress所有的运行环境，直接拉取创建服务即可，但使用单实例版的wordpress不能保证数据的持久化存储，建议您使用自建的mysql或使用腾讯云数据库CDB来保存您的数据，可见使用[CDB版的wordpress搭建](https://www.qcloud.com/document/product/457/7447)

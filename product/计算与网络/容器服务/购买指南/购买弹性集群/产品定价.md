## 计费项说明
 ...
 TKE Serverless 集群的容器（Serverless 集群 Pod 或容器实例）资源类型分为 CPU 和 GPU 两类。
...
## 计费模式说明[](id:ModeDescription)
 TKE Serverless 集群提供三种类型的计费模式：预留券、按量计费和竞价模式，分别适用于不同场景下的用户需求。
下表列出了三种计费模式的区别：
...
为降低客户成本，TKE Serverless 集群提供了几种类型的折扣，包括小规格资源折扣和预留券时长折扣。具体说明如下，其中所有优惠对于 [TKE Serverless 集群](https://cloud.tencent.com/document/product/457/39804)、[容器实例](https://cloud.tencent.com/document/product/457/57339)、[按量计费超级节点](https://cloud.tencent.com/document/product/457/53027) 均适用。

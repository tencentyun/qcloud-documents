在应用模板中我们使用 Nginx 单服务应用作为应用模板的基础示例。在应用中我们同样使用 Nginx 单服务应用作为示例。

## 步骤一： 创建 Nginx 单服务的应用的模板

如果不存在 Nginx 单服务应用的应用模板，则需要创建对应的应用模板，具体的创建过程可以可以参考 [应用模板示例-Nginx单服务应用][1]。

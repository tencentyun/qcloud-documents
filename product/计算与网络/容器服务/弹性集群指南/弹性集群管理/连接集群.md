...
>!请根据实际需求，将命令行中的 `v1.8.13` 替换成业务所需的 Kubectl 版本。
>
 - **Mac OS X 系统**
...
```
 - **Windows 系统**
   1. 按 **Windows + R** 键，打开运行窗口。
   2. 输入 **cmd** 后按 **Enter**。
   3. 在打开的 cmd 窗口中执行以下命令，获取 Kubectl 工具。
```shell
...
5. <span id="info"></span>在弹出的“集群凭证”窗口中，查看用户名、密码和证书信息。
您可以根据实际需求，单击【复制】或【下载】将集群 CA 证书保存到本地。
6. 在“集群凭证”窗口中，根据实际需求开启外网或内网访问地址，并选择以下方式连接集群：

...
此外，普通 Kubernetes 集群 ClusterIP 类型的 Service 依赖节点转发流量，弹性集群为了兼容 ClusterIP 类型的 Service ，需用户单独制定一个 VPC 子网作为集群 Service CIDR。所有 ClusterIP Service 会在此子网中创建内网负载均衡（免费）实现，并占用子网一个 IP，请确保子网具备充足剩余 IP。

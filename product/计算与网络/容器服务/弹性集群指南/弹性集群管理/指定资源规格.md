...
弹性容器服务支持对工作负载设置的 Request 及 Limit 进行计算，自动判断 Pod 运行所需的资源量。Pod 资源类型不同，计算方法也有一定差异。请参考 [CPU Pod 规格计算方法](#CPUpod) 及 [GPU Pod 规格计算方法](#GPUpod)，进一步了解您实际使用 Pod 资源规格的计算方法。

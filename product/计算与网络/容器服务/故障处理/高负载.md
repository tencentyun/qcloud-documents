本文档介绍如何 TKE 集群中通过工具定位是否因高负载造成的不可用。当遇到此类问题时，建议您首先按照以下步骤进行排查，若确认检查项无误后仍不能正常访问，请您及时 [提交工单](https://console.cloud.tencent.com/workorder/category?level1_id=6&level2_id=350&source=0&data_title=%E5%AE%B9%E5%99%A8%E6%9C%8D%E5%8A%A1TKE&step=1) 与我们联系。
...
## 问题定位及解决思路
有时在低 cpu ‘us’ \(user\) 、高 cpu ‘id’ \(idle\) 的条件下，仍会出现节点负载很高的情况。而出现此问题通常是文件 IO 性能达到瓶颈导致 IO Wait 过多，从而使得节点整体负载升高，影响其它进程的性能。
...
## 其他原因
节点上部署了其他非 K8S 管理的服务可能造成节点高负载问题。例如，不建议在节点上安装不被 K8S 所管理的数据库。

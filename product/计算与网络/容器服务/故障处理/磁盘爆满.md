本文档介绍 TKE 集群中多场景下可能发生的磁盘爆满问题，并给出对应的排查思路及解决方案。当遇到此类问题时，建议您首先按照下文中的检查建议进行排查，若确认检查项无误后仍不能正常访问，请您及时 [提交工单](https://console.cloud.tencent.com/workorder/category?level1_id=6&level2_id=350&source=0&data_title=%E5%AE%B9%E5%99%A8%E6%9C%8D%E5%8A%A1TKE&step=1) 与我们联系。

## 可能原因
Kubelet 支持 gc 和驱逐机制，可通过 `--image-gc-high-threshold`、`--image-gc-low-threshold`、`--eviction-hard`、`--eviction-soft` 及 `--eviction-minimum-reclaim` 等参数进行控制以实现磁盘空间的释放。

通常来说，上述配置正确的情况下，磁盘不会出现爆满问题。但当配置不正确，或者节点上有其它非 K8S 管理的进程在不断写数据到磁盘，从而占用大量空间时将导致磁盘爆满。

磁盘爆满将影响 K8S 运行，主要涉及 kubelet 和容器运行时两个关键组件。执行以下命令，查看 kubelet 和容器运行时所使用的目录是否存在于该磁盘。
```
df
```
请对应实际结果，选择以下方式进行问题精确定位：
...
如果容器运行时使用的目录所在磁盘空间爆满，将可能会造成容器运行时无响应。

例如，此时的容器运行时为 docker，则执行 docker 相关的命令将会一直 hang 住， kubelet 日志也将看到 PLEG unhealthy，而 CRI 调用 timeout 也将导致容器无法创建或销毁，外在现象通常表现为 Pod 一直 ContainerCreating 或一直 Terminating。
...
#### Pod 事件
Kubelet 使用的目录所在磁盘空间爆满（通常是系统盘），新建 Pod 因 mkdir 都将创建失败，导致 Sandbox 都无法创建成功，则 Pod 通常会出现以下类似事件：
...
当容器运行时为 docker 时发生磁盘爆满问题，dockerd 也会因此无法正常响应，在停止时会卡住，从而导致无法直接重启 dockerd 来释放空间。需要先手动清理部分文件腾出空间以确保 dockerd 能够停止并重启。此时，磁盘爆满恢复步骤如下：
...
```
>!
>- 删除文件时，建议使用 `cat /dev/null >` 方式进行删除，不建议使用 `rm`。使用 `rm` 方式删除的文件，不能够被 docker 进程释放掉，进而该文件所占用的空间也就不会被释放。
...
## 如何规避磁盘爆满 ？
需确保 kubelet 的 gc 和驱逐相关参数进行配置正确。若配置无问题，即便磁盘达到爆满地步，此时事发节点上的 Pod 也已自动驱逐到其它节点上，不存在 Pod 一直 ContainerCreating 或 Terminating 的问题。

...
>?不管是 cgroup 限制，还是因为节点机器本身资源不够导致的进程停止，都可以从系统日志中找到记录。方法如下：
...
Linux 程序被外界中断时会发送中断信号，程序退出时的状态码为中断信号值加128。例如， `SIGKILL` 的中断信号值为 9，那么程序退出状态码则为9 + 128 = 137。更多标准信号值参考如下表：

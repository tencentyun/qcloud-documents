本文档介绍可能导致 Pod 处于 CrashLoopBackOff 状态的几种情形，以及如何通过排查步骤定位异常原因。在确定引发 Pod 异常的原因后，您可调整对应配置进行解决。
...
节点上所有 Pod 的实际内存占用总量不会超过  `/sys/fs/cgroup/memory/kubepods` 中 cgroup 的限制，这个限制的值等于 `"capacity" - "kube-reserved" - "system-reserved"`。通常情况下，如果预留空间设置合理，且节点上其它非容器进程（非 kubelet、dockerd、kube-proxy 及 sshd 等）内存占用没有超过 kubelet 配置的预留空间，是不会发生系统 OOM 的。为确保不再发生此类问题，您可以根据实际需求对预留空间进行合理的调整。

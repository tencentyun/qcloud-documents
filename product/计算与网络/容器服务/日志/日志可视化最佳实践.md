...
## ELK 部署指导（请保证集群有足够的可用资源）

本篇使用腾讯云容器服务提供的基础模版，以部署一套从 Kafka 读取日志数据的 ELK 为例。
...
## 第三步：创建 Elasticsearch 索引

使用 Kibana 进行日志检索前，需要先创建索引，另要保证 Elasticsearch 内有相应 pattern 的数据。

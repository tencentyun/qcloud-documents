...
- 采集容器标准输出日志[链接至 采集容器标准输出日志.md]
- 采集主机文件日志[链接至 采集主机日志文件.md]
- 采集容器内文件日志[链接至 采集容器内路径日志.md]
- 配置所采集日志的消费端[链接至 配置日志的接收端.md]

## 最佳实践

基于日志收集功能，使用 Logstash 和 Elasticsearch 进行集群服务日志的可视化检索。

为提供日志的可视化能力，建议用户使用 logstash 消费 kafka 的日志数据，并将日志数据发送到 elasticsearch 集群，此处[链接至 日志可视化最佳实践.md]提供 elasticsearch 和 logstash 集群的搭建模板。

...
## 服务资源限制推荐
CCS会根据你的容器一定时间段内历史的实际负载来推荐cpu和mem的request与limit值，使用推荐值会保证的你容器更加平稳的运行，大大减小出现异常的概率。

**推荐算法**：
  我们首先会取出过去7天容器分钟级别的实际负载，并辅以百分位统计第95%的值来最终确定推荐的Request,Limit为Request的2倍。
```
Request = Percentile(实际负载[7d],0.95)
Limit = Request * 2
```
  如果当前的样本数量（实际负载）不满足推荐计算的数量要求，我们会相应的扩大样本取值范围，尝试重新计算，例如：去掉namespace，serviceName等筛选条件。如经过多次计算后同样未能得到有效值，则推荐值为空。

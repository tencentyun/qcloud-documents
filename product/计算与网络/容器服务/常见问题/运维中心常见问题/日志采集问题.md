...
- 若采用环境变量的方式开启 TKE Serverless 日志采集，并选择角色授权，在**创建角色时需要选择 CVM 载体**，详情请参见 [TKE Serverless 日志采集](https://cloud.tencent.com/document/product/457/47200)。
...
当客户程序的日志数据跨占多行时（例如 Java 程序日志），不能使用换行符 `\n` 作为日志的结束标识符。为了能让日志系统明确区分开每条日志，需要配置具有首行正则表达式的 configmap，当某行日志匹配了预先设置的正则表达式，则认为是一条日志的开头，而在下一个行首出现则作为该条日志的结束标识符。详情可参见 [TKE Serverless 日志采集实现多行日志合并](https://cloud.tencent.com/document/product/457/54714)。
...
- 开启 TKE Serverless 日志采集，详情可参见 [TKE Serverless 日志采集](https://cloud.tencent.com/document/product/457/47200)。

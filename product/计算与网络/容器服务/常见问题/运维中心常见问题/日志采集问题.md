...
当客户程序的日志数据跨占多行时（例如 Java 程序日志），不可以换行符 `\n` 作为日志的结束标识符。为了能让日志系统明确区分开每条日志，需要配置具有首行正则表达式的 configmap 去进行匹配，当某行日志匹配了预先设置的正则表达式，则认为是一条日志的开头，而在下一个行首出现则作为该条日志的结束标识符。详情可参见 [EKS 日志采集实现多行日志合并](https://cloud.tencent.com/document/product/457/54714)。

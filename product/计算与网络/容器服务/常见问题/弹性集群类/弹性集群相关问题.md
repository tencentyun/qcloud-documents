...
::: 为什么\sPod\s规格与填写的\sRequest/Limit\s不一致？
...
[](id:test) 在创建弹性集群 Pod 时，若未指定安全组，则默认使用 default 安全组，您也可以通过 `Annotation eks.tke.cloud.tencent.com/security-group-id : 安全组 ID` 为 Pod 指定安全组，请确保同地域已存在该安全组 ID。关于此 Annotation 的详细说明请参加 [Annotation 说明](https://cloud.tencent.com/document/product/457/44173)。
:::
::: 如何设置容器终止消息？
...
:::
::: 挂载自建的\snfs\s时，事件报\sOperation\snot\spermitted

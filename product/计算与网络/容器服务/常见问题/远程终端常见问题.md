...
如果您在使用 `apt-get` 安装软件速度过慢，有可能是因为机器访问国外软件源速度过慢引起。我们根据不同操作系统的容器为您提供对应的提速方案，您可对应操作系统参考以下方案进行提速：
...
直接修改源地址为临时解决方案，当容器被重新调度后，您所作的修改将会失效，建议您在创建镜像时解决该问题。具体的操作方法如下：

修改创建容器镜像的 Dockerfile，在 Dockerfile 的 RUN 字段中，对应操作系统添加信息，直接修改源地址。
例如，在一个基于 Ubuntu 系统的镜像的 Dockerfile 中，加入以下内容：
```shell
...
ENDOF
```
>?您可参考此步骤，在创建镜像时修改 CentOS、Debian 系统容器镜像的 Dockerfile。

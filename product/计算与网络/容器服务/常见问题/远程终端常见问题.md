...
如果您在使用 `apt-get` 安装软件速度过慢，有可能是因为机器访问国外软件源速度过慢引起。我们根据不同操作系统的容器为您提供对应的提速方案，您可对应所使用的操作系统参考以下方案进行提速：
...
修改创建容器镜像的 Dockerfile，在 Dockerfile 的 RUN 字段中，对应操作系统添加 [解决方案](#solve) 中提供的信息修改源地址。例如，在一个基于 Ubuntu 系统的镜像的 Dockerfile 中，加入以下内容：
```shell

...
该服务模式下，Kubernetes 集群的 Master 和 Etcd 会由腾讯云技术团队集中管理和维护。您只需要购置集群，运行负载所需的工作节点即可，不需要关心集群的管理和维护。

### 说明

- Master、Etcd 的托管当前免费，但是您需要为集群的工作节点、持久化存储以及服务绑定的负载均衡等服务付费。
- 因为 Master、Etcd 不属于用户资源，所以您在该模式下无法自主修改 Master 和 Etcd 的部署规模和服务参数。如果您有修改的需求，请使用 [Master 独立部署模式](#IndependentDeploy)。
...
腾讯云 TKE 容器服务还为您提供集群完全自主可控的 Master 独立部署模式。选择该模式，Kubernetes 集群的 Master 和 Etcd 会部署在您购置的 CVM 上，并且您拥有 Kubernetes 集群的所有管理和操作权限。
>! 
> - 该模式仅适用于 Kubernetes 1.10.x以上版本。

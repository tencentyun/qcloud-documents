...
主要设置参数如下：
 ...
 > - 更多 Elasticsearch 服务设置请参见 [修改 Elasticsearch 上报配置](#Elasticsearch)。
...
#### 设置 Elasticsearch 鉴权信息
假设需配置 Elasticsearch 的鉴权，可登录集群，访问 kube-system 下的 fluentd-config，修改其中的 fluentd.conf，增加 user 和 password 字段，详细步骤如下：
1. 登录集群节点。
...
4. 执行以下命令，即可完成鉴权配置。
 ```
 kubectl delete po -n kube-system $(kubectl get pods -n kube-system -l qcloud-app=tke-persistent-event | awk 'NR==2{print $1}’)
 ```  

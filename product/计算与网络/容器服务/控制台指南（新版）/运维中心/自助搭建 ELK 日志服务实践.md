## 操作场景
通常我们习惯登录服务器使用 Linux 命令查看应用日志。但此种方式面对大规模分布式应用，一个服务的负载分布在多个服务器上时，效率较为低下，由此产生了日志统一收集展示的需求。

使用容器部署服务时，情况就更复杂。相比普通的分布式应用，容器在上层编排系统的管理下，和服务器的关联关系并不固定。此时，如果没有统一收集展示日志的服务，应用程序的日志将很难阅览。

腾讯云容器服务已经集成了腾讯云日志服务，支持配置收集规则将容器服务日志自动收集上报。当然，我们也支持用户使用 ELK （Elasticsearch＋Logstash＋Kibana）自建可视化日志收集服务。本文档使用腾讯云容器服务提供的基础模版，以部署一套从 Kafka 读取日志数据的 ELK 为例，介绍如何在容器服务里部署 ELK。
...
此步骤直接使用容器服务提供的 ELK 基础模板搭建 Elasticsearch 集群和 Kibana。
...
2. <span id="step2"></span>选择【服务】>【Service】，进入 Service 详情页，可查看已创建的 Elasticsearch 的服务 IP。如下图所示：
...
2. 选择【服务】>【Service】，进入 Service 详情页，可查看已创建的 Kibana 服务负载均衡 IP。如下图所示：
![](https://main.qcloudimg.com/raw/45d534ce91f072c963fc27ea1f9d803f.png)
2. 访问其公网负载均衡 IP ，即可打开 Kibana dashboard 进行日志查阅。如下图所示：
![](https://mc.qcloudimg.com/static/img/a233130efb256ef5836b294e9ec65a35/ccs-log-visual.jpeg)
3. 使用 Kibana 进行日志检索前，需确保 Elasticsearch 内有相应 index pattern 的数据。如下图所示：

## 简介
集群审计功能是基于 [Kubernetes Audit](https://kubernetes.io/docs/tasks/debug-application-cluster/audit) 使用 kube-apiserver 产生的可配置策略的 JSON 结构日志，日志按时间顺序记录对 kube-apiserver 的访问事件。例如，每个用户、管理员或系统组件影响集群的活动。

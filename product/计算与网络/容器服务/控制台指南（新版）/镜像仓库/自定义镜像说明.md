...
- TKE 后期计划调整镜像逻辑，会提前通过站内信、短信、邮件的方式进行通知，请放心使用。
...
<table>
	...
	<tr>
	<td>制作前</td>
	<td>如需使用自定义镜像功能，请 <a href="https://console.cloud.tencent.com/workorder/category/?level1_id=6&level2_id=350&source=undefined&data_title=%E5%AE%B9%E5%99%A8%E6%9C%8D%E5%8A%A1TKE&step=1">提交工单</a> 申请。</td>
	</tr>
	<tr>
	<td>制作时</td>
  <td>
	<ul class="params">
	<li>目前仅支持同类型的操作系统镜像的制作。例如，Centos > CentOS 类的自定义镜像。</li>
	<li>如果您使用自定义镜像功能，请使用 TKE 提供的基础镜像来制作自定义镜像。否则完全自定义的镜像在 TKE 控制台上会无法识别。</li>
	<li>若在自定义镜像中对 <code>/etc/resolv.conf</code> 文件设置文件保护（<code>chattr +i /etc/resolv.conf</code>），则会导致 <code>clount-init</code> 失败。由于 TKE 依赖于 <code>cloud-init</code> 成功状态，最终会导致节点加入集群失败。</li>
	<li>由于 <code>rc.local</code> 和 <code>container_cluster_agent</code> 无法保证执行顺序，会导致用户 <code>start_init.sh</code> 脚本拷贝的数据丢失。不建议您把 <code>start_init.sh</code> 放在 <code>rc.local</code> 中执行，建议在 <code>user-data</code> 中执行。</li>
	<li>在自定义镜像中添加个人 yum 源时，若放置在不合适的目录下（例如 <code>/etc/yum.repo.d/</code>），则会引起 <code>container-cluster-agent</code> 在执行 <code>yum install</code> 操作时报错，从而跳过该步骤，导致 agent 安装 yum 源失败。</li>
	</ul>
	</td>
	</tr>
	<tr>
  ...
  <td>
	<ul class="params">
	<li>操作系统属于集群级别。集群在进行新增节点、伸缩组、添加已有节点、节点升级时，均会使用已设置的操作系统。</li>
	<li>修改操作系统只影响后续新增的节点或重装的节点，对存量的节点操作系统无影响。</li>
	<li>TKE 后期计划调整镜像逻辑，会提前通过站内信、短信、邮件的方式进行通知，请放心使用。
镜像逻辑变化会造成用原有自定义镜像新建节点失败，您需要重新制作自定义镜像。如集群有使用伸缩组，需调整启动配置镜像 ID。</li>
<li>曾经制作过镜像的节点上如果保留了 <code>/var/lib/cloud</code> 目录，那么 <code>/var/lib/cloud/instances/ins-1cgoe1y9/sem</code> 目录下的 <code>config_scripts_user</code> 文件会影响 <code>cloud-init</code> 服务无法正常执行，会导致该节点加入 TKE 集群内时修改的节点主机名无法生效。</li>
	</ul>
	</td>
	</tr>
</table>

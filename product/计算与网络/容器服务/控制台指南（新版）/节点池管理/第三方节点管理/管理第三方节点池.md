...
![](https://main.qcloudimg.com/raw/276bdc724b566a88aa7b9daa5f1eba32.png)
 ...
 - **容器网络插件**：针对于云下第三方节点的网络插件，目前支持 **Cilium VXLan** 和 **Cilium BGP** 两种类型。云上和云下必须是同一类型的网络插件，全是 Underlay 或者全是 Overlay。如云上网络插件是 Cilium-Overlay，则这里默认是 Cilium-Overlay；如云上是 VPC-CNI-单网卡多IP模式（Underlay），则这里默认是Cilium BGP。

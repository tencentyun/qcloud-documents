...
- Pod 创建时从节点已绑定的可用 IP 池中随机分配 1个可用 IP。
...
- 集群每新增一个节点，将申请一张弹性网卡。不会提前绑定任何辅助 IP，但会在网络组件内给这个节点预留网卡 IP 配额数量的 IP。
- 新建一个使用 VPC-CNI 模式的 Pod 时，IPAMD 组件会依据节点绑定网卡所在的子网分配 1个 IP，然后才会即时申请绑定该辅助 IP 到相应节点的网卡上。

...
如上图所示：
1. TKE 组件在每个节点维护一个可弹性伸缩的网卡池。已绑定的网卡/ IP 数量将被维持在 Pod 数量+最小预绑定数量 及 Pod 数量+最大预绑定数量 之间:
...
2. 独占网卡的 Pod 创建时，从节点可用网卡池中随机分配一个可用网卡。
3. 独占网卡的 Pod 销毁时，网卡释放回节点的网卡池，留给下一个 Pod 使用，不会在 VPC 侧释放（删除）。
4. 节点删除时，将释放（删除）所有已绑定的网卡。

#### 固定 IP 模式

1. TKE 不会为每个节点维护网卡池，网卡不会预绑定到节点上。
2. 独占网卡的 Pod 创建时，直接绑定一张网卡到节点上，给这个 Pod 使用。
3. 非固定 IP 的独占网卡 Pod 销毁时，直接在 VPC 侧删除释放该 Pod 使用的网卡，固定 IP 的 Pod 销毁时，网卡仅做解绑，不会删除释放。
4. 节点删除时，将释放（删除）所有已绑定的网卡。

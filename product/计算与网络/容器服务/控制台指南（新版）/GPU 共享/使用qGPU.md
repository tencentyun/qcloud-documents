...
- **OS 支持**：推荐使用TencentOS Server 3.1 (TK4) ，稳定高效。不推荐使用市场镜像，公共镜像更稳定、高效、更易维护。
<table>
<thead>
  <tr>
    <th>镜像 ID</th>
...
- **GPU卡架构:**  支持 Volta（如 **V100**）、Turing（如 **T4**）、Ampere（如 **A100、A10**)。
- **驱动版本：** 支持驱动版本由镜像和机型所共同决定，具体可参考下方操作步骤。
  - 为保证兼容性，强烈建议用户在节点上安装 NVIDIA 驱动，无需在 POD 内部重复安装。
...
### 二、开启集群qgpu共享开关
1. 单击目标集群 ID，进入集群详情页。
2. 开启qgpu共享开关，开启后，**集群中所有新增GPU节点**默认开启GPU共享能力。您可以在下一步：准备GPU资源中通过Label控制是否开启隔离能力。如下图所示：
![](https://qcloudimg.tencent-cloud.cn/raw/9c377771dd99b99cc6a457834dc7663f.jpg)

### 三、准备 GPU 资源
1. 单击**新建节点池**，选中支持镜像，例如**TencentOS Server 3.1 (TK4)** 并设置相关驱动。
设置好镜像，选好机型后, 可以根据需求选择 GPU 驱动的版本、CUDA 版本、cuDNN 版本。

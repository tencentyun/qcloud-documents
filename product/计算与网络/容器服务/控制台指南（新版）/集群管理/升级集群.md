...
### 升级 Master Kubernetes 版本<span id="Master"></span>
>! **目前已支持托管集群、独立集群 Master 版本升级**，且升级需要花费5 - 10分钟，在此期间您将无法操作您的集群。

#### Master 大版本与小版本升级说明
- 目前Master升级已支持大版本升级（比如从1.14升级到1.16）、小版本升级（比如从1.14.3 升级到1.14.6，或者从v1.18.4-tke.5 升级到 v1.18.4-tke.6，强烈建议您升级前先查阅对应的功能发布记录：
...
- 当大版本升级（如1.12升级到1.14），若您已设置自定义参数，需要您重新设置新版本的自定义参数。原参数不保留。详情见 [自定义 kubernetes 组件启动参数](https://cloud.tencent.com/document/product/457/47775)。
...
#### Master升级技术原理
master节点升级主要分为3个步骤：前置组件升级、master节点组件升级、后置组件升级。

1. 升级前置操作：将会升级前置依赖的组件，如监控组件等，做一些前置依赖的操作防止兼容性问题导致组件异常。
2. master节点组件升级：将按组件顺序，对所有Master的对应组件进行升级，所有Master的某个组件升级完成后再进行下一个组件的升级。
    TKE会先升级kube-apiserver，后升级kube-controller-manager和kube-scheduler，最后升级kubelet，具体步骤如下：
   - 重新生成kube-apiserver组件静态Pod对应的yaml文件内容
   - 检查当前kube-apiserver pod是否健康，kubernetes版本是否正常
   - 同理，依次升级kube-controller-manager和kube-scheduler
   - 升级kubelet，并检查所在master节点是否ready
3. 升级后置操作：
	- 按需升级后置依赖组件，如kube-proxy（并将其滚动更新策略改为on delete）、cluster-autoscaler组件等
...
6. 您可以在集群管理页对应的集群状态处查看升级进度，您可以在升级进度弹窗中查看当前升级进展、Master节点升级进度（托管集群不显示具体Matser节点列表）、升级持续时间：
![](https://main.qcloudimg.com/raw/6eac3875e41ab25a7e0673556eef0cfd.png)
...
### 升级 Node Kubernetes 版本<span id="Node"></span>
...
#### 重装滚动升级执行原理<span id="chongzhuang"></span>
...
![](https://main.qcloudimg.com/raw/8e43b217faf23a042abdb319a54da9ff.png)

#### 原地滚动升级执行原理<span id="yuandi"></span>
...
8. 查看节点升级进度， 直至所有节点升级完成。![](https://main.qcloudimg.com/raw/8e43b217faf23a042abdb319a54da9ff.png)

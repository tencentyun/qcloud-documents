...
VPC-CNI 模式是腾讯云容器服务支持的扩展网络模式，利用腾讯云的多弹性网卡能力，为集群内的 Pod 分配 VPC 内的 IP 地址。 由腾讯云 VPC 功能负责路由，可实现 Pod 和 Node 的控制面和数据面完全在同一网络层面，该模式下的Pod能够复用腾讯云 VPC 所有产品特性。
...
- 当前 VPC-CNI 模式的子网不能与其他云上资源共用（如 CVM、CLB 等）。
...
4. 单击【基本信息】。在 VPC-CNI 字段中点击开启，选择子网，并确认使用限制。如下图所示：
...
2. 在左侧导航栏中，单击【集群】。
3. 选择需要查看的集群ID/名称，进入该集群的管理页面。
4. 点击【基本信息】。 在 VPC-CNI 字段中点击关闭。（仅支持在集群内不存在任何 VPC-CNI 模式的 Pod 时关闭）

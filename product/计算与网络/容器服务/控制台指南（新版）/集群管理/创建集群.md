...
![](https://qcloudimg.tencent-cloud.cn/raw/4133a1ff88bbc430e5c76fa86f657dcf.png)
 ...
 - **所在地域**：建议您根据所在地理位置选择靠近的地域，可降低访问延迟，提高下载速度。详情请参见 [地域和可用区](https://cloud.tencent.com/document/product/457/44787)。
 - **集群网络**：为集群内主机分配在节点网络地址范围内的 IP 地址。详情请参见 [容器及节点网络设置](https://cloud.tencent.com/document/product/457/9083)。
 - **容器网络插件**：提供 GlobalRouter 模式、VPC-CNI 模式和 Cilium-Overlay 模式。详情请参见 [如何选择容器网络模式](https://cloud.tencent.com/document/product/457/50353#.E5.A6.82.E4.BD.95.E9.80.89.E6.8B.A9.E5.AE.B9.E5.99.A8.E7.BD.91.E7.BB.9C.E6.A8.A1.E5.BC.8F)。
 - **容器网络**：为集群内容器分配在容器网络地址范围内的 IP 地址。详情请参见 [容器及节点网络设置](https://cloud.tencent.com/document/product/457/9083)。
 - **镜像提供方**：支持公共镜像、自定义镜像和市场镜像三种类型的镜像。详情请参见 [镜像概述](https://cloud.tencent.com/document/product/457/68289)。
 - **操作系统**：根据实际需求进行选择。
 ...
 - **高级设置**（可选）：
    ...
    - **自定义参数**：指定自定义参数来配置集群。详情请参见 [自定义 Kubernetes 组件启动参数](https://cloud.tencent.com/document/product/457/47775)。
    - **运行时版本**：选择容器运行时组件的版本。
4. 单击**下一步**。
...
- **Worker 配置**：当**节点来源**选择**新增节点**、**集群类型**选择**托管集群**时，该模块下所有设置项为默认项，您可根据实际需求进行更改。
   ...
   - **机型**：选择大于 CPU 4核的机型，具体选择方案请参看 [实例规格](https://cloud.tencent.com/document/product/213/11518) 和 [快速入门 Linux 云服务器](https://cloud.tencent.com/document/product/213/2936)。
      ...
      - **主机名**：操作系统内部的计算机名（`kubectl get nodes` 命令展示的 node name）, 该属性为集群属性。主机名有如下两种命名模式：
        ...
        - **手动命名**：支持批量连续命名或指定模式串命名。仅支持小写字母、数字、连字符 "-" 、点号 "." ，符号不能用于开头或结尾且不能连续使用，更多命名规则指引请查看 [批量连续命名或指定模式串命名](https://cloud.tencent.com/document/product/213/34343)。
...
![](https://qcloudimg.tencent-cloud.cn/raw/f0526d569bb777c8a3644596e35f03d0.png)
 - **qGPU共享**：开启后，集群所有增量 GPU 节点默认开启 GPU 共享能力。您可以通过 Label 控制是否开启隔离能力。请注意，需要安装组件方可正常使用GPU共享调度能力，详情请参见 [如何使用 GPU 共享](https://cloud.tencent.com/document/product/457/65734)。
...
![](https://qcloudimg.tencent-cloud.cn/raw/b6f6f385e472a4330e6ba685a1e05cba.png)
 - **组件**：组件包含存储、监控、镜像等，您可按需选择。详情请参见 [扩展组件概述 ](https://cloud.tencent.com/document/product/457/39048)。
 - **Prometheus 监控服务**：开启后，您可以按照实际需求灵活配置数据采集规则，并按需要配置告警规则，配置完成后即可打开 Grafana 查看监控数据。详情请参见 [Prometheus 监控服务](https://cloud.tencent.com/document/product/457/71896)。
 - **日志服务**：默认开通集群审计服务。详情请参见 [集群审计](https://cloud.tencent.com/document/product/457/48346)。
2. 单击**下一步**，检查并确认配置信息。


### 5. 信息确认
在“信息确认”页面，确认集群的已选配置信息和费用，勾选**我已阅读并同意 [容器服务服务等级协议](https://cloud.tencent.com/document/product/457/36510)**。单击**完成**即可创建一个集群。
...
![](https://qcloudimg.tencent-cloud.cn/raw/07dc13e3de935fbfd41d09cd067a2d55.png)

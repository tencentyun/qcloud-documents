...
- 集群的操作系统属于集群级别，决定集群后续新增节点、伸缩组、添加已有节点、集群节点升级的后的操作系统。
...
6. 在 “选择机型” 中，判断是否使用已有云主机创建集群。如下图所示：
![](https://main.qcloudimg.com/raw/139922aac72cecba2256cb7ab6fe086c.png)
 - **勾选 “使用已有云主机创建集群”**，执行 [使用已有云主机创建集群的操作](#UseExistingCVMCreateCluster)。
 - **未勾选 “使用已有云主机创建集群”**，执行 [不使用已有云主机创建集群的操作](#NotUseExistingCVMCreateCluster)。
...
![](https://main.qcloudimg.com/raw/17aa0e61bc92ba7aea3169fc654030ba.png)
 主要参数信息如下：
 ...
 -  **Master&Etcd机型**：根据如下要求，选择云主机。
    - 选择的云主机配置需要大于 CPU 4核且要求选择>=3台云主机。
    - 云主机不能是其他 TKE 集群的节点。
    - 云主机的状态须为【运行中】。
    - 可跨可用区选择。 
 - **Node机型**：当 “**Node**” 选择为 “**新增**” 时，可选。您可以选择已有的 CVM 作为 Node 节点，也可以在集群创建完成后再添加 Node 节点。
...
9. 在 “选择机型” 中，选择部署模式和机型。如下图所示：
 ![选择机型](https://main.qcloudimg.com/raw/2d532ee8f64fd9a98ed21d0d0d167948.png)
 主要参数信息如下：
 - **Master**：Master 的部署方法决定了您集群的管理模式，我们提供了两种集群托管模式选择。参阅 [集群概述](https://cloud.tencent.com/document/product/457/32187)。
 - **Node**：Node 配置的是集群运行服务真正使用的工作节点。您可以在创建集群时购置 CVM 作为 Node 节点，也可以在集群创建完成后再添加 Node 节点。
...
11. 在 “云主机配置” 中，配置云服务器的其他配置。如下图所示：
![云主机配置](https://main.qcloudimg.com/raw/10610b39dc40ff67fdbecc922132acae.png)
主要参数信息如下：
 - **数据盘挂载**：默认勾选。

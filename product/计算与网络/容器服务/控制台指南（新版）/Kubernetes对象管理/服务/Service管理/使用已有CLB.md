...
腾讯云容器服务 TKE 具备通过 `service.kubernetes.io/tke-existed-lbid: <LoadBalanceId>` 注解实现使用已有负载均衡的功能，您可使用该注解指定集群 Service 资源关联的负载均衡实例。还具备 **Service 负载均衡复用**功能，即指定多个 Service 使用同一个已有负载均衡，您可参考本文进行设置。

## 使用已有负载均衡的同步行为
* 使用已有负载均衡时，指定 Service 的网络类型的注解不生效。
* 当一个Service不再使用已有负载均衡时，该 Service 描述的对应监听器会删除，该负载均衡将保留。
...
-  **复用**负载均衡时：
  ...
  - 删除 Service，复用负载均衡绑定的后端云服务器需要自行解绑，同时会保留一个 tag `tke-clusterId: cls-xxxx`，需自行清理。
...
>- 请注意 Service 的类型，需要设置为 `LoadBalancer` 类型。
...
Kubernetes 官方在 Service 的设计中具有限制：一个 Service 下暴露的多个端口协议必须相同。有许多游戏场景的客户，有在同一个端口同时暴露 TCP 和 UDP 服务的需求，腾讯云负载均衡服务支持在同一个端口上同时监听 UDP 和 TCP 协议。此需求可以通过 Service 负载均衡复用来解决。
请参考以下 Service 配置，`game-service` 被描述为两个 Service 资源，描述的内容除了监听的协议以外基本相同。两个 Service 都通过注解指定使用了已有负载均衡 `lb-6swtxxxx`。通过 kubectl 将以上资源应用到集群中，就可以实现在同一个负载均衡的端口上暴露多种协议的目的。
```
apiVersion: v1
kind: Service
metadata:

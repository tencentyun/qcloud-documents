...
WorkingDir，即指定当前的工作目录。如果不存在，则自动创建。如果没有指定，则使用容器运行时的默认值。如果镜像中如果没指定 workdir，且在控制台未指定，则 workdir 默认为 “/”。

## 命令和参数的使用

单击 [详情](https://cloud.tencent.com/document/product/457/9883)，了解如何将 docker run 命令适配到腾讯云容器服务。
 

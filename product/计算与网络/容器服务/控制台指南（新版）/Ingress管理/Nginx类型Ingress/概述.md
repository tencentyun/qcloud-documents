...
- **Nginx-ingress 实例**：一个集群中可部署多个 Nginx-ingress（如一个用于公网，一个用于内网），在 Kubernetes 中对应一个 CRD，创建一个 Nginx-ingress 实例会在集群中自动创建 Nginx-ingress-controller、service、configmap 等 Kubernetes 资源。

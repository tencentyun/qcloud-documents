...
- 使用已有的 CLB 时：
  ...
  - 默认 CLB 的转发规则的限制是 50 个，如果您 Ingress 的转发规则超过50时，可通过 [在线咨询](https://cloud.tencent.com/online-service?from=doc_457) 提升负载均衡 CLB 的配额。
  - Ingress 和 CLB 之间配置的管理和同步是由以 CLB ID 为名字的 LoadBalancerResource 类型的资源对象，请勿对该 CRD 进行任何操作，否则容易导致 Service 失效。

...
开通容器服务后，腾讯云会授予您的账户 `TKE_QCSRole` 角色的权限。该角色为容器服务角色，在某些场景下需进行预设策略关联操作，完成操作后，以下策略会出现在该角色的已授权策略列表中：
...
`IPAMDofTKE_QCSRole` 角色为容器服务的 IPAMD 支持服务角色。被授予该角色的权限后，在某些场景下需进行预设策略关联操作，完成操作后，以下策略会出现在该角色的已授权策略列表中：

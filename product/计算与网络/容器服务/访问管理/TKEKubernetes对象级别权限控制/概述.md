...
### 认证方式
Kubernetes APIServer 支持丰富多样的认证策略，例如 x509 证书、bearer token、basic auth。其中仅 bearer token 单个认证策略就支持多种 token 认证方式，例如指定 known-token csv 文件的 beaer token、serviceaccount token、OIDC token、webhook token server 等。

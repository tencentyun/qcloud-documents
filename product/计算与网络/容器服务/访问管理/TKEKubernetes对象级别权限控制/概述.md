腾讯云容器服务 TKE 提供了对接 Kubernetes RBAC 的授权模式，便于对子账户进行细粒度的访问权限控制。该授权模式下，可通过容器服务控制台及 kubectl 两种方式进行集群内资源访问。如下图所示：
![](https://main.qcloudimg.com/raw/664f3ac1ec693eab06123e61e3d05b93.png)
...
### 认证方式
Kubernetes APIServer 支持丰富多样的认证策略，例如 x509 证书、bearer token、basic auth。其中，仅 bearer token 单个认证策略就支持指定 known-token csv 文件的 beaer token、serviceaccount token、OIDC token、webhook token server 等多种 token 认证方式。

TKE 分析了实现复杂性及多种场景等因素，选择使用 x509 证书认证。其优势如下：
- 用户理解成本低。
- 对于存量集群无需进行复杂变更 。
- 按照 User 及 Group 进行划分，后续扩展性好。

TKE 基于 x509 证书认证实现了以下功能：
- 每个子账户都拥有单独的属于自己的客户端证书，用于访问 Kubernetes APIServer
...
通过 TKE 提供的授权管理功能，您可以进行更细粒度的权限控制。例如，仅赋予某个子账号只读权限或仅赋予某个子账号下的某个命名空间读写权限等。可参考以下方式，对子账号进行更细粒度的权限控制：
- [使用预设身份授权]()
- [自定义策略授权]()

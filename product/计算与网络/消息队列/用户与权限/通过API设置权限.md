...
使用子账户的账户及密码登录，在控制台的【云产品】- 【[云API密钥](https://console.qcloud.com/capi)】，找到子用户的密钥。密钥用于生成签名，认证通过后可访问相关腾讯云资源。
...
3. 资源(resource)描述格式通常采用六段式，格式为"qcs:project:serviceType:region:account:resource",
	...
	- resource由name/value构成。name表示业务对资源的定义。如cos是用prefix描述，cdn用host描述等。"*"表示所有资源，归一化为"*/*"的形式。不可以为空。

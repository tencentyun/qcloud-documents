...
控制台会调用cmq api接口，因此子账号需要在控制台查看队列、主题、订阅信息时需要对控制台调用的cmq api接口授予权限，否则子账号查看控制台会报没有权限的错误， 这里说明不同的控制台界面所需要授权的接口：
1. 控制台查看队列列表： 授权ListQueue接口

2. 控制台查看主题列表： 授权ListTopic接口

3. 控制台查看订阅列表： 授权ListSubscriptionByTopic接口

子账号也需要控制台访问cmq。需要授予对应的接口权限， **注意 子账号控制台查看监控数据需要对云监控接口进行cam授权**。

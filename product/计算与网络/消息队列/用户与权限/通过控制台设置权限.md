本文以指定某用户拥有 CMQ 队列模型的**消费消息及批量消费消息 写权限**为例，演示如何设置 CMQ 权限。CMQ的CAM权限管理能力，目前处于灰度阶段，可联系我们的一线技术支持，或从工单申请该能力。

从原分项目切换到CAM时，需要注意：原协作者账户能登录cmq控制台（用的是根账户秘钥）。但目前接入cam后，默认是没权限的（登录控制台用的是子账户的秘钥），要通过CAM授权，才有访问权限。

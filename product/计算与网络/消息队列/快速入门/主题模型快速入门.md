...
## 4.路由匹配
Binding key 、Routing key是组合使用的，兼容rabbitmq topic匹配模式。发消息时配的 Routing key 是客户端发消息带的， 必须是字符串，不能有匹配符。创建订阅关系时配的 Binding key 是 topic 和 订阅者 的绑定关系。
...
3、rabbitmq的特例：routing_key为空字符串时，不匹配\* ，但可以匹配#

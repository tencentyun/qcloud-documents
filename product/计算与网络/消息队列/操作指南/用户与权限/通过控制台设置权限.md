...
3. 在选择服务和操作页面，补充以下信息.
  ...
  - 服务（必选）：选择队列模型 (cmqqueue)（没有找到请确认是否已经开通 CMQ 服务）。

  - 操作（必选）：选择您要授权的操作。

  - 资源（必填）：填入您要授权的资源的资源六段式。CMQ 的资源六段式描述，例如`qcs::cmqqueue:bj:uin/1238423:queueName/uin/3232/myqueue`。详情参考[接入CAM的 API 授权详情](#接入CAM的 API 授权详情)
    - 第一段为固定格式 qcs；
    - 第二段为空；

当消息生产者本地事务处理成功与消息发送成功不一致时，传统的处理方式无法解决该问题，事务消息实现了消息生产者本地事务与消息发送的原子性，保证了消息生产者本地事务处理成功与消息发送成功的最终一致性。用户实现类似 X/Open XA 的分布事务功能，通过 CMQ 事务消息能达到分布式事务的最终一致。

前提条件
该功能目前处于灰度测试阶段，暂时支持广州、上海、印度地域，其他地域在逐步支持中，如需试用请通过 提交工单（https://console.cloud.tencent.com/workorder/category?level1_id=6&level2_id=335&source=0&data_title=%E6%B6%88%E6%81%AF%E9%98%9F%E5%88%97CMQ/CKAFKA/IoT%20MQ&step=1） 的方式开通白名单。

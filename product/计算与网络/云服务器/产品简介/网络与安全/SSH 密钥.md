...
## SSH 密钥概述
腾讯云建议您使用 SSH 密钥对登录 Linux 实例。

SSH 密钥对是通过加密算法生成的一对密钥，包括公有密钥（公钥）和私有密钥（私钥）。腾讯云创建的 SSH 密钥对采用 RSA 2048位的加密方式生成。

SSH 密钥对成功生成后，腾讯云仅存储公钥。对于 Linux 实例，公钥内容存储在 `~/.ssh/authorized_keys` 文件中。而您需要下载并妥善保管私钥。私钥仅一次下载机会，腾讯云不会保存您的私钥。拥有您的私钥的任何人都可以解密您的登录信息，因此将私钥保存在一个安全的位置是非常重要的。

您可以通过密钥对安全地与云服务器进行连接，使用密钥对登录云服务器比使用常规密码更加安全。只需要在创建实例时指定密钥对，或在实例创建后绑定密钥对，便可使用私钥登录 Linux 实例，无需输入密码。

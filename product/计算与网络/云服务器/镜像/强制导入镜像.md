当用户的linux镜像因为某些原因无法[安装cloudinit](https://cloud.tencent.com/document/product/213/12587)时，可以使用`强制导入镜像`功能完成镜像的导入。此时，腾讯云无法对用户的虚拟机进行初始化配置，因此需要用户设置脚本自行根据腾讯云提供的配置文件对虚拟机进行配置。
## 限制条件及配置方法
### 申请权限

...
### 镜像导入配置方法
用户强制导入的镜像并没有使用cloudinit，因此无法自动进行配置。腾讯云提供了包含了配置信息的cdrom设备供用户自行配置，用户需要挂载cdrom，然后读取`mount_point/os_action/os.conf`的信息进行配置。如果用户有使用其他配置数据、UserData的需要，可以直接读取`mount_point/`下的文件。
...
* 该脚本需要设置为开机启动才能正确配置，请根据操作系统的类型实现该要求。(例如放置到/etc/init.d/目录下 然后执行一下命令。）

```
echo "chmod +x /etc/init.d/os_config.sh" >> /etc/rc.d/rc.local
echo "/etc/init.d/os_config.sh > /tmp/os_config.log 2>&1" >> /etc/rc.d/rc.local
echo "rm -f /etc/init.d/os_config.sh" >> /etc/rc.d/rc.local
```

* 用户需要自行保证脚本执行正确，如果镜像导入后遇到无法ssh连接实例，没有网络连接等问题首先尝试通过控制台连接到实例，重新执行脚本排查问题，如仍然无法处理，请联系客服。

以下为`os_config.sh`脚本示例，用户可根据实际情况修改脚本。
...
mount_cdrom() {
	...
	if [[ -f $mount_path/os_action/os.conf ]]; then
		. $mount_path/os_action/os.conf
		if [[ -n $password ]]; then
			passwd_file=$(mktemp /mnt/pass.XXXX)
			passwd_line=$(grep password $mount_path/os_action/os.conf)

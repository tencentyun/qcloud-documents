...
### 镜像导入配置方法
用户强制导入的镜像并没有使用cloudinit，因此无法自动进行配置。腾讯云提供了包含了配置信息的cdrom设备供用户自行配置，用户需要挂载cdrom，然后读取`mount_point/qcloud_init/os.conf`的信息进行配置。如果用户有使用其他配置数据、UserData的需要，可以直接读取`mount_point/`

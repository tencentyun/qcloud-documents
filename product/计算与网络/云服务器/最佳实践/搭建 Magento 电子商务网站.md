## 操作场景
Magento 是一个使用 PHP 语言开发的开源电子商务平台，是国际电子商务解决方案之一。本文介绍通过在腾讯云云服务器（CVM）上安装 Magento 镜像来搭建个人电子商务网站。
...
3. 在“选择镜像”窗口的左侧搜索框中，输入 Magento 并单击。如下图所示：
>?单击镜像名可查看镜像详情。
>
...
```
3. 输入账户名为 `root` 默认密码为 `123456`，并单击【执行】。如下图所示：
...
5. <span id="password"></span>在弹出的“修改密码”窗口中，自行设置或自动生成密码后，单击【执行】。如下图所示：
>?本文使用系统自动生成密码，请记录您的数据库密码。
>
...
6. <span id="admin"></span>将 Magento 管理员登录地址修改为 `admin` ，单击【Next】。如下图所示：
>?请记录您的商务网站地址及管理员登录地址。
>
![](https://main.qcloudimg.com/raw/b7b0c3e65c324f58c65e9d3b735b8275.png)
7. 根据您的事情情况设置语言、时区等信息，并单击【Next】。本文设置如下图所示：
...
10. 成功安装后请巨鹿页面上的信息，并单击【Launch Magento Admin】。如下图所示：

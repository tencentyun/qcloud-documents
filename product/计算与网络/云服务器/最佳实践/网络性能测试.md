...
推荐使用两台相同配置的 CVM 进行测试，避免性能测试结果出现偏差，其中一台作为测试机，另一台作为陪练机。本示例中指定 10.0.0.1 与 10.0.0.2 进行测试。
...
实例：
```
iperf -c 10.0.0.1 -b 2G -t 300 -P 8
```
...
```
陪练机器应该启动多个 netperf 实例（经验上值总 netperf 实例数至少需要 300 以上），以达到 TCP-RR 极限。
实例：
```

...
本文档主要介绍如何快速使用Windows系统的云服务器实例的相关功能，引导新手快速了解腾讯云云服务器的创建和配置。
...
- 基础型：适合有一定访问量的网站或应用。如：企业官网等简单的Web应用。
- 普及型：适合常使用云计算等一定计算量的需求。如：并发适中的App或普通数据处理。
...
### 确定付费方式
腾讯云提供包年包月和按量付费两种付费模式。详见[计费模式说明](https://www.qcloud.com/document/product/213/2180)
...
5\. 选择镜像提供方。
对于刚开始使用腾讯云的用户，推荐选择公共镜像，其中包含了正版Windows操作系统，后续运行环境自行搭建。

6\. 选择操作系统：选择Windows Server
...
云服务器创建好后将会收到站内信，内容包括实例名称、公网 IP 地址、内网 IP 地址、登录名、初始登录密码等信息。您可以使用这些信息登录和管理实例，也请尽快更改您的Windows登录密码保障主机安全性。

站内信的查看请见下一步骤。

## 步骤三：登录Windows云服务器
-------------------
本部分操作介绍登录Windows云服务器的常用方法，不同情况下可以使用不同的登录方式，此处介绍 控制台登录 与 本地Windows机器登录 ，更多登录方式请见[登录 Windows 实例](https://www.qcloud.com/document/product/213/5435)。
...
### 本地Windows机器登录云服务器
 >注： 仅适用于购买了公网带宽/流量的云服务器，可以从您的本地Windows机器登录Windows云服务器（远程连接MSTSC）。
 
 1. 在本地Windows机器上，点击【开始】- 【运行】，输入"mstsc"命令，打开远程桌面连接对话框。

 2. 在输入框输入Windows服务器的公网IP（登录[云服务器控制台](https://console.qcloud.com/cvm)可查看云服务器的公网IP），点击【连接】：
	![](//mc.qcloudimg.com/static/img/4ee2695da27bb3e58d9bf929802f6105/image.png)

 3. 新打开的界面中输入帐号（Administrator）和站内信中的初始密码（或您修改后的密码），点击密码输入框后的箭头图标：

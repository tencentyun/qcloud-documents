...
>! 大于2TB的云硬盘请使用 GPT 方式进行搭载数据盘操作。详情请参见 [使用 GPT 分区表分区并格式化](/doc/product/213/2043) 。
 
### 数据盘分区
...
>? 以创建1个分区为例，开发者也可以根据自己的需求创建多个分区。
>
![](//mc.qcloudimg.com/static/img/8a9c8ff4db5a7e4622bf2968d0309129/image.png)
 	1. 输入`fdisk /dev/vdb`（对数据盘进行分区），按 **Enter**。
 	2. 输入`n`（新建分区），按 **Enter**。
 	3. 输入`p`（新建扩展分区），按 **Enter**。
 	4. 输入`1`（使用第1个主分区），按 **Enter**。
 	5. 按 **Enter**（使用默认配置）。
 	6. 再次按 **Enter**（使用默认配置）。
 	7. 输入`wq`（保存分区表），按 **Enter**，开始分区。
...
2. **挂载分区**
执行以下命令，创建 mydata 目录并将分区挂载在该目录下：
```
mkdir /mydata
...
df -h
```
出现如下图框选的 vdb1 信息则说明挂载成功，即可以查看到数据盘。

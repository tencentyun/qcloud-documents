...
1. **选择计费模式**：腾讯云提供【包年包月】和【按量付费】两种计费模式，请根据实际需求进行选择。
2. **选择地域和可用区**：建议选择与您的客户最近的地域。如果您需要购买多台云服务器，建议选择不同可用区，实现容灾效果。
3. **选择网络**：腾讯云提供【基础网络】或【私有网络】两种网络。2017年6月13日后，新注册的账号已不支持基础网络，推荐您使用私有网络。
...
4. **选择实例**：根据底层硬件的不同，腾讯云目前提供了**系列1**和**系列2**（下文称为**上一代实例**和**当前一代实例**）两种不同的实例系列。
 - 上一代实例：标准型 S1，高 IO 型 I1，内存型 M1
 - 当前一代实例：[标准型 S2](https://cloud.tencent.com/document/product/213/11518#S2)，[高 IO 型 I2](https://cloud.tencent.com/document/product/213/11518#I2)，[内存型 M2](https://cloud.tencent.com/document/product/213/11518#M2)，[计算型 C2](https://cloud.tencent.com/document/product/213/11518#C2)，[GPU 型 G2](https://cloud.tencent.com/document/product/560/11625)，[FPGA 型 FX2](https://cloud.tencent.com/document/product/565/10417) 等。为获得最佳性能，建议使用当前一代实例。
>! 不同的地域与可用区下的系列、机型会有所不同。更多实例详情请参见 [实例规格](https://cloud.tencent.com/document/product/213/11518)。
>
5. **选择镜像**：
 - 镜像提供方：腾讯云提供公共镜像、自定义镜像、共享镜像、服务市场，您可参考 [镜像类型](https://cloud.tencent.com/document/product/213/4941) 进行选择。
对于刚开始使用腾讯云的用户，推荐选择公共镜像。
 - 操作系统及版本：腾讯云提供了 CentOS、CoreOS、Debian、FreeBSD、openSUSE、SUSE、Ubuntu 等操作系统，请根据实际需求进行选择。
6. **设置系统盘**：腾讯云提供云硬盘和本地硬盘两种类型（均默认为50GB），请根据实际需求进行设置。
 - [云硬盘](https://cloud.tencent.com/document/product/362/2353)：采用一盘三备的分布式存储方式，数据可靠性高。
 - 本地硬盘：处在云服务器所在的物理机上的存储设备，可以获得较低的时延，但存在单点丢失风险。
7. （可选）**添加数据盘**：包含本地盘数据盘和云硬盘数据盘两种类型，请根据实际需求进行添加。
8. **设置公网带宽**：腾讯云提供【按带宽计费】和【按使用流量】两种网络计费方式，请根据实际需求进行选择。
  - 按带宽计费：选择固定带宽，超过本带宽时将丢包。适合网络波动较小的场景。
  - 按使用流量计费：按实际使用流量收费。可限制峰值带宽避免意外流量带来的费用，当瞬时带宽超过该值时将丢包。适合网络波动较大的场景。
9. 单击【下一步：设置主机】，进入设置主机页面。
...
3. **设置登录方式**：
 - 设置密码：自定义输入登录实例的密码。
 - 立即关联密钥：关联 SSH 密钥。如您没有密钥或现有的密钥不合适，可以单击【现在创建】创建，参考 [创建密钥](/doc/product/213/%E5%AF%86%E9%92%A5%E6%93%8D%E4%BD%9C%E6%8C%87%E5%8D%97#1.-创建密钥) 指南。更多 SSH 密钥请参见 [SSH 密钥](http://cloud.tencent.com/doc/product/213/SSH%E5%AF%86%E9%92%A5)。
 - 自动生成密码：自动生成的密码将会以 [站内信](https://console.cloud.tencent.com/message) 方式发送。
4. **设置自动续费**：仅限包年包月的云服务器。
5. （可选）**单击【高级设置】**，根据实际需求对实例做更多配置。
![高级设置](https://main.qcloudimg.com/raw/0f3b93ce6f5c1b81dfa54ea092763eed.png)
其关键参数信息如下：
...
1. 核对购买的云服务器信息。
2. 根据实际需求，选择购买实例的数量，以及购买时长（仅限包年包月的云服务器）。
3. 勾选【同意《退款规则》】（仅限包年包月的云服务器）。
4. 单击【立即购买】，完成支付。当您付款完成后，即可进入 [云服务器控制台](https://console.cloud.tencent.com/cvm) 查收您的云服务器。

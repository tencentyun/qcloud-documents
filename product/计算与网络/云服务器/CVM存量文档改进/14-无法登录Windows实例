本文主要介绍无法连接 Windows 实例时的问题排查方法和几种主要可能的原因，您可以通过逐一排查的方法定位并解决问题。
...
**处理步骤**：
1. 通过 [VNC 登录](#VNC) 登录实例。
2. 参考 [Windows 系统 CPU 与内存占用率高导致无法登录](https://cloud.tencent.com/document/product/213/10233)，在 “任务管理器” 中定位高负载的进程。

<span id="RemotePortConfiguration"></span>
### 远程端口配置异常

**故障现象**：远程无法连接，远程访问端口非默认端口、被修改或者 3389 端口没打开。
...
**处理步骤**：推荐通过安装安全更新的方式解决，具体可参考 [Windows 实例远程登录时身份验证错误](https://cloud.tencent.com/document/product/213/30813)。

## 其它解决方案
通过上述排查后，仍然不能连接 Windows 实例，请您保存自助诊断结果，[提交工单](https://console.cloud.tencent.com/workorder/category) 进行故障排查和咨询。

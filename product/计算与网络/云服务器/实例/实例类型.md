...
如需购买，请点击 [这里](http://cloud.tencent.com/event/cvm2.html)。
...
#### 标准型 S2
系列 2 的标准型 S2 机器采用主频 2.4 GHz 的 Intel E5-Xeon Broadwell（v4） CPU处理器和 DDR4 内存，数据盘可搭配本地盘、普通云盘和SSD云盘等不同类型（具体可能随硬件规格而不同）。
...
#### 高 IO 型 I2 
系列 2 的高 IO 型 I2 机器采用主频 2.4 GHz 的 Intel E5-Xeon Broadwell（v4） CPU处理器和 DDR4 内存，其中所有配置的系统盘均为SSD本地盘。搭载网络增强，包转发能力高达30w。

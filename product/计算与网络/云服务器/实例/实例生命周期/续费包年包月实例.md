不同于按量计费类型的实例，包年包月类型实例无法主动销毁，而会在到期后一段时间内被系统回收。您可以在任意时刻对包年包月实例进行续费或设置自动续费，以防止实例到期后被系统销毁而导致的数据丢失和服务中断。
...
2) 对需要续费的包年包月 CVM 实例，在右侧操作栏中，点击【续费】。
...
1) 登录[腾讯云控制台]( https://console.qcloud.com)，将鼠标移动到右上角您的账户名称处，在菜单中选择【续费】。

2) 对需要续费的包年包月 CVM 实例，在右侧操作栏中，点击【设为自动续费】。

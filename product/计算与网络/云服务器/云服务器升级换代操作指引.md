...
### 置放群组准备
建议您在迁移前创建置放群组，为存量实例和增量实例赋予容灾和高可用性能力，配置建议如下：
...
- 置放层级，根据业务特性，依据充分使用资源的原则，依次选择物理机层级、交换机层级、机架层级。
...
2. 在弹出的“机型升级换代”窗口中，仔细查阅活动须知，并单击**下一步**。
3. 在“确认升级”中，查看可升级换代实例的目标配置及可用区，勾选需迁移的实例。
4. 勾选“同意强制关机”后，单击**开始升级**即可。
您可在“机型升级换代”窗口中查看进度，升级成功后单击**完成**即可。

## 步骤4：验证升级
您可根据从以下两方面进行验证迁移是否成功。请 [联系我们](https://cloud.tencent.com/document/product/213/58600) 进行解决。
- **系统验证**：检查并确认系统运行状态正常。
- **业务确认**：检查业务状态是否恢复正常。

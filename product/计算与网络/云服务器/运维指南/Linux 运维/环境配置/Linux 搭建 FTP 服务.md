...
>!FTP 可通过主动模式和被动模式与客户端机器进行连接，并传输数据。由于大多数客户端机器的防火墙设置和无法获取真实 IP 等原因，建议您选择**被动模式**搭建 FTP 服务。
>
...
### 步骤5：验证 FTP 服务
您可通过 FTP 客户端软件、浏览器、文件资源管理器验证 FTP 服务，本文以客户端文件资源管理器为例。

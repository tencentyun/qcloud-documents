...
如果需要停止是僵尸进程，则需要杀掉进程的父进程，需执行的命令为： kill -9 ppid（ppid 为父进程 ID 号，可以通过 ps -o ppid PID 查找，例如 ps -o ppid 32535）。 
...
如果是纯 Linux shell 出现乱码，请使用 export 命令查看用户环境变量，LANG，LC_CTYPE等环境变量设置。
...
登录 Linux 服务器执行 rm 命令删除文件后，用 df 命令查看硬盘空间时，可能会发现删除文件后可用的硬盘空间没有增加。其原因为，当通过 rm 命令删除文件时，有其它进程正在访问该文件，若通过执行 df 命令进行查看，删除的文件占用的空间将为没有立即释放的状态。

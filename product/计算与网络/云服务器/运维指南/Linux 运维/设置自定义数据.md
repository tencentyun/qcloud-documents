...
用户在创建云服务器时，您可以通过指定**自定义数据**配置实例。当服务器**首次启动**时，自定义数据将以文本方式传递到云服务器中，并执行该文本。如果您一次购买多台云服务器，自定义数据会在所有的云服务器首次启动时运行。
本文以 Linux 云服务器首次启动时，通过传递 Shell 格式的脚本，实现输出 "Hello Tencent Cloud" 为例。
...
参考以下内容，创建 Shell 脚本文件。
例如，按照以下内容，创建一个文件名称为 script_text 的 Shell 脚本文件。
```
#!/bin/bash
...
例如，对 script_text 脚本文件进行 Base64 编码，返回 `IyEvYmluL2Jhc2gKCmVjaG8gIldlbGNvbWUgVG8gVGVuY2VudCBDbG91ZC4iCg==` 编码结果。

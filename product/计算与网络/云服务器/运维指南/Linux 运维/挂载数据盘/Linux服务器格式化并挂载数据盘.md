...
出现如下信息则说明挂载成功，即Linux已经挂载数据盘:
```
Filesystem      Size  Used Avail Use% Mounted on
/dev/vdb        50G  53M   47G   1% /data
```

### 步骤三：添加数据盘挂载信息至`/etc/fstab`，实现开机自动挂载
如果希望云服务器在重启或开机时能自动挂载数据盘，必须将数据盘挂载信息添加到`/etc/fstab`中。如果没有添加，则云服务器重启或开机后，不会自动挂载数据盘。
...
在腾讯云的Linux服务器上，您可以运行以下Shell脚本替换以上的全部操作来实现自动地格式化并挂载数据盘：
```
#!/bin/bash
type=ext4
mount_dir=/data

...
### 检查本地客户端
1. 本地客户端访问 [华佗诊断分析系统](https://ping.huatuo.qq.com/)，测试本地访问各域名的速度。
...
1. 本地客户端 ping 服务器公网 IP，确认是否存在丢包或延时高的情况。
 - 若存在丢包或时延高的情况，请使用 MTR 进行诊断。具体参考 [服务器网络延迟和丢包处理](https://cloud.tencent.com/document/product/213/14638)。
...
您也可以直接使用公网 IP 访问对应页面，排查是否 DNS 的问题导致访问慢。

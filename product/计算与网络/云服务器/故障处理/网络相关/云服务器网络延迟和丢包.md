...
<dx-tabs>
::: WinMTR 的介绍和使用（Windows 操作系统）[](id:MTRofWindows)
...
:::
::: MTR 的介绍和使用（Linux 操作系统）[](id:MTRofLinux)
...
2. [](id:step02)往上查看报告结果，定位第一次丢包的节点。
 - 如果丢包发生在目的服务器，则可能是目的服务器的网络配置不当引起，请检查目的服务器的防火墙配置。
 - 如果丢包开始于前三跳，一般为本地运营商网络问题，建议检查访问其他网址是否存在相同情况。如果存在相同情况，请反馈给您的运营商进行处理。
 - 如果有频繁丢包的情况，确实为网络不稳定的场景，则请 [提交工单](https://console.cloud.tencent.com/workorder/category) 进行咨询，并附上测试截图，以便工程师进行定位。

...
在云服务器 iptables 未设置相关规则的情况下，可能是 iptables policy 设置相关规则引到达云服务器的包都被丢弃。处理步骤如下：

1. [](id:Step1)执行以下命令，查看 iptables policy 规则。
```
iptables -L | grep policy 
```
iptables policy 规则默认为 ACCEPT。若 INPUT 链 policy 非 ACCEPT，则会导致所有到服务器的包都被丢弃。返回如下结果，表示进入云服务器的包都会被 drop。
```

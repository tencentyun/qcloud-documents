...
### 检查及调整访问策略设置
Linux 中可以通过 `/etc/hosts.allow` 和 `/etc/hosts.deny` 文件设置访问策略，两个文件分别对应允许和阻止的策略。例如，可以在 `hosts.allow` 文件中设置信任主机规则，在 `hosts.deny` 文件中拒绝所有其他主机。本文以 `hosts.deny` 为例，阻止策略配置如下：
```
...
请检查 `/etc/hosts.deny` 文件及 `/etc/hosts.allow` 文件。并根据检查结果选择以下处理方式：
 - 配置有误，请您按需进行修改，更改后即时生效。
 - 未配置或配置无误，请进行下一步。
...
```
 - 若 SSH 连接确实被阻止，请通过对应软件白名单等相关策略自行设置。
 - 若 SSH 连接未被阻止，请进行下一步。

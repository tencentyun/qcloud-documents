...
3. 可能导致无法正常登录的原因是配置文件 `/etc/security/limits.conf` 中关于 root 用户最大能打开的文件描述符个数配置错误，正确的配置应满足 `soft nofile ≤ hard nofile ≤ fs.nr_open` 关系。查看参数 `soft nofile`、`hard nofile` 及 `fs.nr_open` 值是否符合规范：
...
本示例中获取的配置结果因不满足  `soft nofile ≤ hard nofile ≤ fs.nr_open` 关系，导致无法成功登录。
...
2. 修改 `/etc/security/limits.conf` 文件，在文件末尾添加或修改如下配置： 
 - `root soft nofile`：100001
 - `root hard nofile`：100002
3. 修改 `/etc/sysctl.conf` 文件，在文件末尾添加或修改如下配置：

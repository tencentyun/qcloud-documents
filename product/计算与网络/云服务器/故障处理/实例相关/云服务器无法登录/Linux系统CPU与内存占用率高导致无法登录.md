...
CPU 或内存使用率过高，引起服务响应速度变慢、服务器登录卡顿等问题。
...
- S：进程状态。
- %CPU：更新时间间隔内进程所使用的 CPU 的百分比。
- %MEM：更新时间间隔内进程所使用的内存的百分比。
- TIME+：进程使用的 CPU 时间，精确到 0.01s。
- COMMAND：进程名称。

## 可能原因
引起 CPU 或内存使用率过高的原因可能由硬件因素、系统进程、业务进程或者木马病毒等因素导致。您可以使用 [腾讯云可观测平台](https://cloud.tencent.com/document/product/248/13466)，创建 CPU 或内存使用率阈值告警，当 CPU 或内存使用率超过阈值时，将及时通知到您。

...
<tbody>
  <tr>
    ...
    <td ><a href="#tcpPacketLoss"> 通过 VNC 方式登录云服务器</a></td>
  </tr>
  <tr>
    <td>未配置允许远程登录的权限</td>
...
3. 在打开的**允许通过远程桌面服务登录** 属性窗口中，检查允许通过远程桌面服务登录的用户列表是否存在需要登录的帐户。如下图所示：
...
3. 在打开的**拒绝通过远程桌面服务登录**属性窗口中，检查拒绝通过远程桌面服务登录的用户列表是否存在需要登录的帐户。

## 可能原因
当您的 Linux 云服务器实例出现由内存问题引发的故障，可能是由于实例内存使用率高等问题引起，您可参考本文进行问题分析并查找对应的解决方法。

## 解决思路
1. 参考 [内存使用率高问题处理](#ProcessingSteps)，判断问题是否由内存使用率高引起。
2. 参考 [其他内存问题处理](#OtherProcessingSteps)，定位问题原因。

## 处理步骤
### 内存使用率高问题处理[](id:ProcessingSteps)
1. 参考 [相关操作](#RelatedOperations)，查看内存使用率是否较高。
 - 内存使用率高，则执行下一步。
 - 内存使用率正常，则请参考 [其他内存问题处理](#OtherProcessingSteps)，进一步定位问题原因。
2. 在系统内部执行 `top` 命令后按 **M**，查看 “RES” 及 “SHR” 列是否有进程占用内存过大。
  - 否，则执行下一步。
  - 是，则
3. 执行以下命令，查看共享内存占用是否较多。
```
cat /proc/meminfo | grep -i shmem
```
 - 否，则执行下一步。
 - 是，则
4. 执行如下命令，查看是否不可回收的 slab 占用内存较高。
```
cat /proc/meminfo | grep -i SUnreclaim
```
 - 否，则执行下一步。
 - 是，则执行 `slabtop` 命令定位具体占用内存高的 slab。
5.  执行以下命令，查看是否存在内存大页。
```
cat /proc/meminfo | grep -iE "HugePages_Total|Hugepagesize"
```
 - `HugePages_Total` 输出为0，则进行下一步。
...
### 其他内存问题处理[](id:OtherProcessingSteps)
如通过以上处理步骤无法处理问题，或您使用云服务器时出现以下类型的错误信息，则可以参考以下解决方案：

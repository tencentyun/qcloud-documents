...
### 普通云硬盘、高性能云硬盘和 SSD 云硬盘有哪些异同点？

- 普通云硬盘：是腾讯云提供的上一代云硬盘类型，适用于数据不经常访问的低 I/O 负载的业务场景。
- 高性能云硬盘：是腾讯云推出的混合型存储类型，通过 Cache 机制提供接近固态存储的高性能存储能力，同时采用三副本的分布式机制保障数据可靠性。高性能云硬盘适用于高数据可靠性要求、普通中度性能要求的中小型应用和 MySQL、SQL Server、PostgreSQL 等中小型关系数据库应用的场景。
- SSD 云硬盘：是基于全 NVMe SSD 存储介质，采用三副本的分布式机制，提供低时延、高随机 IOPS、高吞吐量的 I/O 能力及数据安全性高达99.9999999%的高性能存储。SSD 云硬盘适用于对 I/O 性能有较高要求的场景。

这三种云硬盘的收费根据地域不同有所区别，您可根据应用程序要求和费用预算选择您所需的云硬盘类型。详细费用，请参考 [云硬盘价格总览](https://cloud.tencent.com/document/product/362/2413)。

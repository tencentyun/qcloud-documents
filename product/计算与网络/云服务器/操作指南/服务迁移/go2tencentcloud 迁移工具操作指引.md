## 概述
[在线迁移](https://cloud.tencent.com/document/product/213/32961) 是指在系统不停机的情况下，将服务器或虚拟机上的系统、服务程序等从自建机房（IDC）或云平台等源环境迁移同步至腾讯云。
...
<table>
	...
	<tr><td>在线迁移指引</td><td>指引文档，迁移前请详细阅读。</td></tr>
	<tr><td>go2tencentcloud_x64</td><td>64位 Linux 系统的迁移工具可执行程序。</td></tr>
	<tr><td>go2tencentcloud_x32</td><td>32位 Linux 系统的迁移工具可执行程序。</td></tr>
	<tr><td>user.json</td><td>迁移时源端主机和目标云服务器的配置文件，请按《在线迁移指引》中的说明修改配置。</td></tr>
	<tr><td>client.json</td><td>迁移工具的配置文件，请按《在线迁移指引》中的说明修改配置。</td></tr>
	<tr><td>rsync\_excludes\_linux.txt</td><td>rsync 配置文件，排除 Linux 系统下不需要迁移的文件目录。</td></tr>
</table>

>?  不能删除配置文件，并请将配置文件存放在和 go2tencentcloud 可执行程序同级目录下。 
>

- <span id="userJsonState">user.json 文件参数详细说明：</span>
<table>
...
>? 请将对应参数值替换为您实际的配置参数。
>
- <span id="clientJsonState">client.json 文件部分参数说明：</span>
<table>
	<tr><th>参数名称</th><th>类型</th><th>是否必填</th><th>说明</th></tr>
...
请按照 [user.json 文件参数详细说明](#userJsonState) 配置必填项和所需项的值。
2. 在 client.json 文件配置迁移模式和其他项。
将 client.json 文件里的`Client.Net.Mode`项设置为`0`，即进行 [默认模式](#DefaultMode) 迁移。此外，如果还需要进行其他项设置，请按照 [client.json 文件部分参数说明](#clientJsonState) 进行配置。
...
```
工具运行后，迁移主要分为初始化远程实例、传输数据和释放远程实例三个阶段。每个阶段均会产生一些子任务去执行相关操作，部分耗时的子任务还将具有默认的最大超时时间。传输数据耗时受源端数据大小，网络带宽等因素影响，请耐心等待迁移流程的完成。迁移工具支持数据传输的断点续传。
...
将 client.json 文件里的`Client.Net.Mode`项设置为`1`，即进行 [内网迁移模式：场景1](#Scenario1) 的迁移。此外，如果还需要进行其他项设置，请按照 [client.json 文件部分参数说明](#clientJsonState) 进行配置。
...
例如，在一台可以访问公网的主机上执行以下命令运行工具，进行初始化远程实例的迁移。
```
sudo ./go2tencentcloud_x64
```
...
待 [步骤5](#Scenario1_step05)（即初始化远程实例）完成后，需先将初始化远程实例的整个工具目录拷贝至待迁移的源端主机，再运行工具进行传输数据的迁移。
...
待 [步骤6](#Scenario1_step06)（即传输数据）完成后，需先将传输数据的整个工具目录拷贝至刚才初始化远程实例的主机，再运行工具进行传输数据的迁移。
例如，执行以下命令运行工具，进行传输数据的迁移。
```
sudo ./go2tencentcloud_x64
```
...
2. 在 user.json 文件配置迁移源端主机和目标云服务器。
 请按照 [user.json 文件参数详细说明](#userJsonState) 配置必填项和所需项的值。
3. 在 client.json 文件配置迁移模式和其他项。
 将 client.json 文件里的`Client.Net.Mode`项设置为`2`，即进行 [内网迁移模式：场景2](#Scenario2) 的迁移。此外，如果还需要进行其他项设置，请按照 [client.json 文件部分参数说明](#clientJsonState) 进行配置。
4. 排除源端主机上不需迁移的文件或目录。（可选）
在 Linux 源端主机迁移时，如果需要排除一些不需迁移的文件或目录，可以将这些不需要迁移的文件或目录追加至[rsync_excludes_linux.txt 文件](#_linuxTxtState) 中。
5. 运行工具。
例如，在64位 Linux 源端主机下，以 root 权限执行以下命令运行工具。
...
如果您的网络代理还需要认证，请在`Client.Net.Proxy.User`和`Client.Net.Proxy.Password`项填写网络代理的用户名和密码；如果不需要认证，则不填。此外，如果您还需要进行其他项设置，请按照 [client.json 文件部分参数说明](#clientJsonState) 进行配置。
4. 排除源端主机上不需迁移的文件或目录。（可选）
在 Linux 源端主机迁移时，如果需要排除一些不需迁移的文件或目录，可以将这些不需要迁移的文件或目录追加至[rsync_excludes_linux.txt 文件](#_linuxTxtState) 中。
5. 运行工具。
例如，在64位 Linux 源端主机下，以 root 权限执行以下命令运行工具。

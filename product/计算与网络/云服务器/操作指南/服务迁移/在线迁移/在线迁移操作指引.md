...
<td>是</td>
<td>
  设置linux块级迁移数据的迁移方式类型。
    <ul>
    <li><b>Linux文件级迁移</b>：迁移颗粒度为文件级，高兼容性，相对较低的传输效率。
    </li>
    <li><b>Linux块级迁移</b>：迁移颗粒度为磁盘逻辑存储单位“块”级，高传输效率，相对较低的兼容性。
    </li>
		  <li><b>Windows块级迁移</b>：迁移颗粒度为磁盘逻辑存储单位“块”级，Windows迁移默认采用块级迁移，具备高兼容性和高传输效率。
    </li>
  </ul>
</td>
</tr>	

...
服务迁移是腾讯云为方便企业用户上云而研发的迁移平台。该迁移平台可以将源端主机的操作系统、应用程序和应用数据等迁移至腾讯云云服务器或云硬盘，以实现企业上云、跨云平台迁移、跨账号/区域迁移或部署混合云等业务需求。
...
- 准备需要迁入的云服务器（CVM）或云硬盘。[点此前往购买 >>](https://buy.cloud.tencent.com/cvm?tab=custom&step=1&regionId=8)

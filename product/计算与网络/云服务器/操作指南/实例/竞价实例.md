...
目前已上线的竞价实例有**云 API** 和**批量计算控制台**两种使用方式，本文档指导您如何通过云 API 和批量计算控制台管理竞价实例。

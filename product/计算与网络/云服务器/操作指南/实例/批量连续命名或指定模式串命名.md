...
- 当您需要购买 n 个实例并希望生成类似为 “CVM+序号” 的实例名称（或主机名称）时（即实例名称为 CVM1、CVM2、CVM3 等实例），您可通过 [后缀数字自动升序](#AutoAscending) 实现。
- 当您需要创建 n 个实例并指定实例名称（或主机名称）带有序号且序号从 x 开始递增时，您可通过 [指定单个模式串](#SpecifySingleString) 实现。
- 当您希望创建 n 个有多个前缀且每个前缀均指定序号的实例名称（或主机名称）时，您可通过 [指定多个模式串](#SpecifyMultipleStrings) 实现。
...
>? 以下操作步骤以设置实例名称为例，根据设置名称的类型不同，详细操作步骤略有区别。
>

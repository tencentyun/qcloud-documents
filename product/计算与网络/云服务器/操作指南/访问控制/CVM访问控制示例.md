...
若您想要让用户能够查看 CVM 控制台中的云硬盘信息，创建云硬盘，使用云硬盘，可将以下操作添加到您策略中，再将该策略关联到该用户。
...
若您想要让用户能够查看并使用 CVM 控制台中的弹性 IP 地址，可将以下操作添加到您策略中，再将该策略关联到该用户。
...
以下策略允许用户查看弹性 IP 地址并可先将其分配给实例并与之相关联。用户不可以修改弹性 IP 地址的属性、取消弹性 IP 地址的关联或释放弹性 IP 地址。
```
{
    "version": "2.0",
    "statement": [

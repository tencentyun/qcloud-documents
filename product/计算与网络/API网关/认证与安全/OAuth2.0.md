...
```
#### 使用授权码的方式获取token
  以上示例中可以看到，获取token时并没有使用授权码进行获取，为了确保指定的用户才能获取到token，根据授权流程，需要有一个步骤需去资源拥有者那里获取授权码，从server.py文件中可以看到，可以第一步请求code路径获取授权码，而同时获取token的时候，需要对发放的code进行登记，判断是否合法。

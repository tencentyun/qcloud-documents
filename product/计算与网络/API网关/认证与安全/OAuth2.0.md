...
- JWT（JSON Web Token）：是一个定义一种紧凑的，自包含的并且提供防篡改机制的传递数据的方式的标准协议。
**API 网关校验的是 JWT 标准生成id_token。**

![图片描述](http://file.tapd.oa.com//tfl/captures/2019-07/tapd_20392312_base64_1563876189_51.png)

## API 网关 OAuth2.0 操作方法
API 网关已经对外提供了 OAuth 功能，实践一个 Auth 的 Demo 
需要一个 Client 和 AS，可能需要 RS。 
功能需求有两个
- 授权 API 调用
 - 请求 Token
 - 刷新 Token
- 业务 API 调用（需要一个 RS Demo）直接用 Mock
- Client 收发请求，postman 等模拟

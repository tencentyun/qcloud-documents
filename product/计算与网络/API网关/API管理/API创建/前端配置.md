...
```
>? 您可以通过编写 SCF 云函数来实现 Web 后端服务，并通过 API 网关对外提供服务。API 网关会将请求内容以参数形式传递给函数，并将函数返回作为响应返回给请求方。详细内容请参考[API 网关触发器概述](https://cloud.tencent.com/document/product/583/12513)。

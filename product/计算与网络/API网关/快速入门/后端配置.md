...
1. 前端类型是 HTTP 时，后端配置包括 [对接 HTTP](#http)、[对接 SCF](#scf) 和 [对接 mock](#mock) 。
2. 前端类型是 WEBSOCKET 时，后端配置包括 [对接 WEBSOCKET](#websocket)和[对接 SCF](#scf) 。
...
## 对接 WEBSOCKET
当用户的业务使用 WEBSOCKET 时，后端服务可选择对接 WEBSOCKET。对接 WEBSOCKET 的配置方式与对接[对接 HTTP](#http)基本相同，但后端域名以`ws://`或`wss://`开头，不包括后面的路径。

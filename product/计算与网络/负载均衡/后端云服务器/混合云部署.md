...
## 方案优势
- 快速搭建混合云，无缝连接云上云下，负载均衡可将请求同时转发至云上 VPC 内服务器和云下 IDC 机房内服务器。
- 复用腾讯云的高质量公网接入能力。
...
- 目前仅广州、深圳、上海、济南、杭州、合肥、北京、天津、成都、重庆、香港、新加坡、硅谷地域支持该功能。
- TCP 和 TCP SSL 监听器需在 RS 上通过通用 TOA 获取源 IP，详情请参见 [混合云部署场景下通过 TOA 获取客户端真实 IP](https://cloud.tencent.com/document/product/214/68105)。

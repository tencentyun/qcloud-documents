...
请先 [配置访问日志到 CLS](https://cloud.tencent.com/document/product/214/41379) 后，关闭 [配置访问日志到 COS ](https://cloud.tencent.com/document/product/214/10329)。

### 配置访问日志到 COS 与到 CLS 有什么差异？
相比于直接配置访问日志到 COS，配置访问日志到 CLS 可以提供：
...
- 在配置访问日志到 COS 正式下线前，可以并存，访问日志会同时写入 CLS 和 COS。
- 在配置访问日志到 COS 正式下线后，无法再直接写入 COS。

配置访问日志到 CLS 后，可应用 CLS 的功能将部分日志存入COS，例如，将最新日志（3天内）存入 CLS、方便查询，将历史日志（3天之前）通过 CLS 转存至 COS。

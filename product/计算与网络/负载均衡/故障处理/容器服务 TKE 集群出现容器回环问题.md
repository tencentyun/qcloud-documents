## 现象描述
容器服务 TKE 集群内部无法通过内网 CLB 访问同一 Node 下的 Pod，出现容器回环问题。

## 可能原因
K8s 服务需要同时对集群内和集群外暴露。其中集群内通过集群内部负载均衡（IPVS）实现，集群外通过内网负载均衡 CLB 实现。IPVS 会把内网 CLB 的 IP 地址绑定在本地的一个接口上，这样集群内访问内网 CLB 的地址实际上用的是集群内的IPVS负载均衡。
而在容器服务 TKE 中，内网 CLB 使用了 CLB 的 VIP 地址作为健康检查源 IP，这与原生的 K8s 实现方式 IPVS 绑定的地址冲突，导致内网 CLB 健康检查失败。
为了解决上述健康检查的问题，容器服务 TKE 更改了 kube-proxy 的代码，不再在接口上绑定内网 CLB 的地址。这样内网 CLB 的健康检查不会有问题，但集群内这部分负载均衡流量会强制通过负载均衡 CLB 服务。这就导致了容器服务 TKE 场景的回环问题。
...
5. 在“健康检查”页签中，健康检查源 IP 选择**100.64网段**，单击**下一步**后再单击提交**提交**。

...
3. 在“记录管理”页签，单击【添加记录】为域名添加 A 记录，在添加记录区域，填写以下参数：
  ...
  - 线路类型：默认。
  - 记录值：单击【关联云资源】，在“关联云资源”对话框勾刚才创建的 `clb-test`。
  - TTL：设置为默认值“600s”。
4. 添加完毕后，单击【保存】。

DNS 解析 DNSPod ，然后将该记录在 Internet 上传播需要一段时间。为测试域名是否解析正常，可以在添加完解析记录十分钟左右后，在浏览器中输入绑定后的 CNAME 域名（如本例中的`www.example.com`）来验证负载均衡是否生效。

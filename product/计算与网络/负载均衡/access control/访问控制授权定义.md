...
所有`$loadbalancerid`应为某个loadbalancer的id，或者“*”； 以此类推。
...
| ModifyLBHealth	|  修改负载均衡健康检查阀值 |  `qcs::clb:$region:$account:clb/$loadbalancerid` |
| DescribeLBHealth	|  查询LB健康检查阀值 |  `qcs::clb:$region:$account:clb/*` |
#### 负载均衡监听器相关：
| API操作 | 资源描述 | 接口说明 |
| :-------- | :---------| :------ |
...
|ManualRewrite	|用户自定义重定向|	`qcs::clb:$region:$account:clb/$loadbalancerid` 
`qcs::clb:$region:$account:listener/$sourceloadbalancerlistenerid` `qcs::clb:$region:$account:listener/$targetloadbalancerlistenerid` |

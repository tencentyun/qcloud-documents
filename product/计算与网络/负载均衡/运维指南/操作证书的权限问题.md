## 操作场景
由于2020年3月23日后，CLB 的证书相关操作均接入 CAM 鉴权。因此当使用子用户进行 CLB 的证书相关操作时，若提示“该操作需要授权，请联系您的开发商为您添加权限”，可根据如下操作，给子用户授予证书相关权限。

## 前提条件
登录的账号需为主账号或有 CAM 相关权限的子用户。


## 操作步骤
请选择如下任意一种方式，进行授予证书相关权限操作。
>? 若使用子用户进行授予证书相关权限操作时，提示“暂无API权限(cam:AttachUserPolicies)，请联系开发商授权”，即代表该子用户无 CAM 相关权限，需使用主账号进行授予证书相关权限操作。

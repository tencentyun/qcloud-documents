若您想要查看健康检查日志，则需先将日志存储到日志服务 CLS 中，然后在 CLS 中进行查看。负载均衡支持配置健康检查日志存储到日志服务 CLS 中，能够进行分钟粒度的日志上报和在线多规则检索，帮助您排查健康检查异常的原因，快速定位问题。
...
- 当前负载均衡仅七层协议（HTTP/HTTPS）支持配置健康检查日志到 CLS，四层协议（TCP/UDP/TCP SSL）暂不支持。
...
## 步骤一：创建日志集和日志主题[](id:step1)

若您需要配置健康检查日志到日志服务 CLS 中，则需先创建日志集和日志主题。
若已有日志集和日志主题，则可直接跳转至 [步骤二](#step2) 开始操作。
...
>?检索语法详情请参见 [语法与规则](https://cloud.tencent.com/document/product/614/16982)。
...
<tr><td>status</td><td>当前健康检查状态。true：表示健康。false：表示异常。</td><td>text</td></tr>
<tr><td>domain</td><td>健康检查域名。若监听器为四层监听器，则无健康检查域名，此参数为空。</td><td>text</td></tr>
<tr><td>status</td><td>健康检查 URL。若监听器为四层监听器，则无健康检查 URL，此参数为空。</td><td>text</td></tr>
</tbody></table>

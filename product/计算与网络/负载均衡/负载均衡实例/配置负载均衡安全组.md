创建 CLB 后，您可以配置 CLB 的安全组来进行公网流量隔离，例如，将测试流量和线上流量隔离。本文将介绍 CLB 的安全组的功能和使用方法。

## 使用限制
- 每个 CLB 最多绑定5个安全组。
- 安全组规则数有限制，在65535内时有效，再增加则无效。
- 可使用安全组的 CLB 实例类型如下：
<table>
<thead>
<tr>
...
2. <span id ="create"></span>配置 CLB 安全组
 1. 配置负载均衡安全组规则
...
> 
 2. 将安全组绑定 CLB 
   在负载均衡详情页中，选择【安全组】标签页，已绑定安全组可以选择对应的安全组绑定到 CLB 上。
...
3. 配置 CVM 安全组
  ...
  2. 将安全组绑定 CVM 
   单击 CLB 绑定的云服务器 ID，进入云服务器详情页，选择【安全组】标签页，配置云服务器的安全组规则。
...
1. 创建 CLB 实例并绑定 CLB 安全组，详情请参见 [配置 CLB 安全组](#create)。
2. 在 CLB 实例列表页面，单击目标 CLB 实例。
3. 在 CLB 详情页面，选择【安全组】标签页。
4. 在“安全组”页面，单击<img src="https://main.qcloudimg.com/raw/5ba06490364505efc4d698e3adb1064e.png" style="margin:-3px 0px">，启用默认放通。
5. 启用默认放通功能后，将仅验证如下**规则预览**中的安全组规则。

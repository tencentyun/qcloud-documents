...
|  client_body_timeout | 60s |  [30-120]s | 获取 Client 请求 Body 的超时时间，不是获取整个 Body 的持续时间，而是指空闲一段时间没有传输数据的超时时间，超时返回 408 |
|  client_max_body_size | 60M |[1-256]M| 客户端请求 Body 的最大值，上传业务可能需要修改，超出后会返回 413 |
...
|  proxy_buffers | 8 8k |[8-16] [8-16]k|缓冲区数量和缓冲区大小|

## CLB个性化配置示例

在负载均衡控制台，左侧单击个性化配置页面。单击【新建】，创建个性化配置文件。填写相应的配置项，并且以`;`结尾。
![](https://main.qcloudimg.com/raw/f107a33279a5d5adfcf575bbba3c95d3.jpg)
创建个性化配置后，可以单击【绑定至实例】选择绑定到同地域的LB实例。

...
|ssl_early_sni|off|on，off| <ul><li>on 表示支持根据客户端 SNI 查找 server 配置，协商 SSL 握手协议版本号。</li><li>off 表示使用默认 server 配置，协商 SSL 握手协议版本号。</li> </ul>支持国密算法后，在同一监听器下有不同域名配置不同 SSL 协议（TLS1.2、TLS1.3、国密）、不同证书（国密证书和非国密证书）的场景，开启此参数可确保握手选择的 SSL 协议版本和配置的证书完全一致，否则有可能会握手失败。  |

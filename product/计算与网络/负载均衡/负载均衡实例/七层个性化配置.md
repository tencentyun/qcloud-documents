...
|  proxy_set_header   |X-Real-Port $remote_port|X-Real-Port $remote_port，X-clb-stgw-vip $server_addr，Stgw-request-id $stgw_request_id|X-Real-Port $remote_port表示客户端端口，X-clb-stgw-vip $server_addr 和 Stgw-request-id $stgw_request_id表示 CLB 的 VIP。|
...
|  proxy_pass_header | - |-|将禁止转发的header设置为允许转发。|

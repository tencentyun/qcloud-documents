负载均衡支持用户配置七层（HTTP/HTTPS）访问日志（Access Log），您可以通过负载均衡访问日志了解客户端请求、辅助排查问题、分析梳理访问数据等。您可以将访问日志存储到 COS 中，并下载分析。
当前公网七层 CLB 日志支持的地域包括广州、上海、北京、香港、上海金融和北京金融。

## 开启访问日志
1. 在“负载均衡实例详情”页面，开启访问日志。
![](https://main.qcloudimg.com/raw/7c74d0e549df7c8bf9c510cbe8ae5a39.png)
...
>?CLB 的日志主要用于故障排查，帮助业务快速定位问题。日志功能包括日志上报、日志存储和查询：
- CLB 的日志上报，提供尽力而为服务（Best Effor Service），优先保障业务转发，再保障日志上报。
- CLB 的日志存储和查询，按当前使用的存储服务来提供 SLA。

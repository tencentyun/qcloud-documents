...
- IPv6 具有更大的地址空间。当前 IPv4 最大的问题就在于网络地址资源有限，严重制约了互联网的应用和发展。 IPv6 采用 128位的地址，而 IPv4 采用的是 32 位。因此 IPv6 支持 2<sup>128</sup>（约 3.4 × 10<sup>38</sup>）个地址，比 IPv4 增加了约 7.9 × 10<sup>28</sup> 个地址。使用 IPv6，可以让全世界的每一粒沙子都能分配到一个 IP 地址。
...
- 业务平滑过渡。业务仅需改造客户端，无需改造后端服务，便可平滑接入 IPv6。IPv6 负载均衡支持来自 IPv6 客户端的访问，并将 IPv6 报文转换成 IPv4 报文，后端云服务器上的应用程序无需感知 IPv6，仍以原有形式部署和工作。
...
> 4. 互联网 IPv6 网络大环境还处于建设初期，当前可能存在部分线路访问不通，如有请填写 [工单](https://console.cloud.tencent.com/workorder/category) 反馈，同时在内测期间，不提供 SLA 保障。

## 操作指南
### 创建 IPv6 负载均衡
1. 登录 [负载均衡购买页](https://buy.cloud.tencent.com/lb)。
2. 在购买页，实例类型选择 **应用型 CLB**，IP 类型选择 **IPv6**，其他配置和普通实例配置相同。
...
### 使用 IPV6 负载均衡
在 [负载均衡管理控制台](https://console.cloud.tencent.com/loadbalance/index?rid=1&forward=1) 单击负载均衡实例 ID 进入 **LB 详情**，在 **监听器管理** 页面配置监听器、转发规则、绑定云服务器，详情请参考 [应用型 LB 快速入门](https://cloud.tencent.com/document/product/214/8975)。

CLB supports request log storage, so you can store request logs in COS for download and analysis. Layer-7 LB log feature is released and now supports Guangzhou, Shanghai, Beijing, and Finance Zones. Welcome to use it.

### Enabling Log Feature
Enable log access feature on the **Load Balancer Instance Details** page.

![](https://mc.qcloudimg.com/static/img/17014eeb67628fa78ffe04e2d7a58d8d/log1.png)

Select the appropriate bucket in the COS, and the request log will be stored into the folder lb-id created under the bucket automatically. After the bucket is selected, click the bucket address and redirect to the log download page.
...
| 18 | upstream_header_time | Time spent for receiving all http headers from rs |

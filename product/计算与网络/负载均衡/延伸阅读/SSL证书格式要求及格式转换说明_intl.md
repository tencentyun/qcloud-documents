## How to apply for commonly-used certificates
...
- Generate certificate request file: `openssl req -new -key privateKey.pem -out server.csr`, where `server.csr` is your certificate request file which is used to apply for a certificate
...
- Concatenation rule: The server certificate comes first, followed by the intermediate certificate. No blank line is allowed in between. Note: Generally, a CA will provide instructions when issuing certificates. Make sure to carefully read the instructions.

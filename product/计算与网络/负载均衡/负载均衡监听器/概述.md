...
<td>在同一个负载均衡实例内<li>监听端口不可重复，如不可以同时创建监听器 TCP:80 和监听器 HTTP:80。</li><li>仅 TCP 和 UDP 协议的端口可重复，如可以同时创建监听器 TCP:80 和监听器 UDP:80。</li><br><br>服务端口可以在同一个负载均衡实例内重复。<br>例如，监听器 HTTP:80 和监听器 HTTPS:443 可以同时绑定同一台云服务器的同一个端口。</td>
</tr>
</tbody></table>

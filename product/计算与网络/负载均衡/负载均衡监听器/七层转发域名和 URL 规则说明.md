...
![](https://main.qcloudimg.com/raw/dc6821617984fb466abb12d8f80897c0.png)
...
- 转发域名长度限制：1 - 80个字符。
- 不能以 `_` 开头。
- 通配的域名，目前仅支持 `*.example.com` 或者 `www.example.*`的形式，即 `*` 在开头或结尾，且单个域名中仅支持 `*` 出现一次。
- 转发域名支持正则表达式，正则表达式的域名：
  - 支持的字符集为：`S` `a-z` `0-9` `.` `-` `?` `=` `~` `_` `-` `+` <code>\\</code> `^` `*` `!` `$` `&` `|` `(` `)` `[` `]`。
  - 需以 `~` 开头，且 `~` 仅能出现一次。
  - 负载均衡支持的正则域名举例如下：`~^www\d+\.example\.com$`。。
- 非正则表达式的域名，支持的字符集为：`a-z` `0-9` `.` `-` `_`。
...
- 转发 URL 支持正则表达式：
  ...
  - 正则表达式的 URL 举例如下：`~* .png$`。
- 非正则表达式的 URL ，必须以 `/` 开头，支持的字符集为：`a-z` `A-Z` `0-9` `.` `-` `_` `/` `=` `?`。

### 转发 URL 路径匹配规则
![](https://main.qcloudimg.com/raw/701a33ec53590cc14e39161701a4b244.png)
...
- 健康检查域名配置支持的字符集为：`a-z` `0-9` `.` `-`。

### 健康检查路径配置规则
健康检查路径是七层负载均衡探测后端服务健康状态的 URL 路径。
- 健康检查路径长度限制：1 - 200个字符。
- 健康检查路径默认为 `/`，且必须以 `/` 开头。
- 健康检查路径不支持正则表达，建议指定某个固定 URL 路径（静态页面）进行健康检查。

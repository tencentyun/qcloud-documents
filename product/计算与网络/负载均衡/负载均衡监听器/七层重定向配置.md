...
使用场景：强制 HTTPS 跳转，即HTTP 强转 HTTPS。PC、手机浏览器等以 HTTP 请求访问 Web 服务，CLB 会将所有`HTTP:80`的请求重定向至`HTTPS:443`进行转发。
2. 自定义重定向
您可以配置一对一重定向，如在某个 CLB 实例中，配置`监听器1 / 域名1 / URL1`重定向至`监听器2 / 域名2 / URL2`.
使用场景：单路径的重定向。如 Web 业务需要临时下线（如电商售罄、页面维护，更新升级时），此时需将原有页面重定向至新页面。如果不做重定向，用户的收藏和搜索引擎数据库中的旧地址只能让访客得到一个 `404/503` 错误信息页面，降低了用户体验度，导致访问流量白白丧失。
...
已配置 HTTPS:443 监听器。
### 配置步骤
1. 请在 [腾讯云负载均衡控制台](https://console.cloud.tencent.com/clb) 完成 CLB 的 [HTTPS 监听器的配置](https://cloud.tencent.com/document/product/214/36385)，搭建`https://example.com`的 Web 环境。
...
假定业务使用 forsale 页面来做运营活动，现在活动结束需要将活动页面`https://www.example.com/forsale`重定向至新主页`https://www.new.com/index`。
### 适用前提
- 已配置 HTTPS 监听器。
...
1. 请在 [腾讯云负载均衡控制台](https://console.cloud.tencent.com/clb) 完成 CLB 的 [HTTPS 监听器的配置](https://cloud.tencent.com/document/product/214/36385)，搭建`https://example.com`的 Web 环境。
2. 完成 HTTPS 配置后的结果如下图所示。
![](/tfl/captures/2019-08/tapd_10114221_base64_1566829017_47.png)
3. 在 CLB 实例详情的“重定向配置”标签页中，单击【新建重定向配置】。
![](https://main.qcloudimg.com/raw/32adf5abcf5569e6986f2af645ae2eac.png)
4. 选择【手动重定向配置】，选择原访问的前端协议端口`HTTPS:443`和域名`https://www.example.com/forsale`，选择重定向后的前端协议端口`HTTPS:443`和域名`https://www.new.com/index`。
![](/tfl/captures/2019-08/tapd_10114221_base64_1566829118_8.png)
5. 单击【下一步：配置路径】，原访问路径选择`/forsale`，重定向后的访问路径选择`/index`。
![](/tfl/captures/2019-08/tapd_10114221_base64_1566829162_81.png)
5. 单击【提交】即可完成配置。
![](https://main.qcloudimg.com/raw/ba728da9f4e0ecd9070f2f7a23763bd8.png)
6. 完成重定向配置后的结果如下图所示，可以看到`HTTP:443`监听器中，`https://www.example.com/forsale`已重定向至`https://www.new.com/index`。
![](/tfl/captures/2019-08/tapd_10114221_base64_1566829407_92.png)

## 注意事项
- 会话保持：如果 client 端访问了`example.com/bbs/test/123.html` ，且后端 CVM 开启了会话保持。当启用重定向后，将流量导到`example.com/bbs/test/456.html`时，原会话保持机制将失效。

...
<tr>
<td>服务端口（后端端口）</td>
<td>服务端口是后端服务器提供服务的端口，接收并处理来自负载均衡的流量。<br/>在一个负载均衡实例中，同一个负载均衡监听端口可以将流量转发到多个后端服务器的多个端口上。</td>
<td><ul>在同一个负载均衡实例内：<li>不同监听协议的服务端口可以重复。例如，监听器 HTTP:80 和监听器 HTTPS:443 可以同时绑定同一台后端服务器的同一个端口。</li><li>同一种监听协议下，同一个后端服务端口只能被一个监听器绑定，即四元组（VIP、监听协议、后端服务内网 IP、后端服务端口）需要唯一。</li></ul></td>
</tr>
</tbody></table>

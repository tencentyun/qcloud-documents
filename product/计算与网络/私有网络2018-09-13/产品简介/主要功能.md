...
| 连接方式 | 说明 |
|---------|---------|
|[弹性 IP（EIP）](https://cloud.tencent.com/document/product/215/20080) | 弹性 IP（Elastic IP，EIP）是可以独立申请的公网 IP 地址，支持与实例（如：云服务器、NAT 网关）的动态绑定和解绑。<br/>典型应用场景：<br/>1. 保留 IP。国内的  IP 和 DNS 之间需要域名备案。<br/>2. 屏蔽实例故障。例如：动态 DNS 映射把 DNS 名称映射到 IP 地址，传播这个映射变化到整个 Internet 可能需花费 24 小时，而弹性 IP 实现了 IP 从一个云服务器到另一个云服务器的漂移。在任何云服务器出现故障时，只需启动另一个实例并重新映射它，从而快速屏蔽实例故障 。|

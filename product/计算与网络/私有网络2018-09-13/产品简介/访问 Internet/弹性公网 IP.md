## 什么是弹性公网 IP？
弹性公网 IP（Elastic IP，EIP）简称弹性 IP 地址或弹性 IP，是可以独立申请的公网  IP 地址，支持与 CVM / NAT 网关实例的动态绑定和解绑。如图 1 所示，您可以将其与账户中的云服务器（或 NAT 网关实例）绑定或者解绑，起到屏蔽实例故障的作用。例如：动态 DNS 映射把 DNS 名称映射到 IP 地址，传播这个映射变化到整个 Internet 可能需花费 24 小时，而弹性 IP 实现了 IP 从一个云服务器到另一个云服务器的漂移。在任何云服务器出现故障时，只需启动另一个实例并重新映射它，从而快速响应实例故障。
您的弹性 IP 地址与腾讯云账户相关联，而不是与某个 CVM 实例相关联，且在您选择显式释放该地址，或您的账户欠费超过  7 天前，它会一直与您的腾讯云账户保持关联。
<div align=center>
![](//mccdn.qcloud.com/static/img/3dbf04b5fb9bff4bef632074ecf767da/image.png)
</div>
<p align=center>图1：弹性 IP 示意图</p>

## 使用约束
### 注意事项
弹性公网 IP 地址同时适用于基础网络和私有网络的 CVM 实例，和私有网络中的 NAT 网关实例，支持动态绑定和解绑。
关于弹性公网 IP，您需要注意以下几点：
- 弹性 IP 与 CVM / NAT 网关是 1 对 1 的关系。
如图 1 所示，1 个弹性公网 IP 同一时间只能绑定同一地域内的 1 个 CVM / NAT 网关实例，1 个 CVM 实例同一时间只能绑定 1 个弹性公网 IP，1 个 NAT 网关同一时间最多支持绑定 10 个弹性公网 IP。
...
此种方案中：
云服务器访问 Internet 的流量会通过内网转发至 NAT 网关，不受云服务器购买时的公网带宽上限限制，NAT 网关产生的网络流量费用也不会占用云服务器的公网带宽出口。
...
> 注意：如果您的账号开通了带宽包共享带宽功能，则 NAT 网关产生的出流量按照带宽包整体结算（不再重复收取 0.8 元 / GB 的网络流量费）。建议您限制 NAT 网关的出带宽，避免因 NAT 网关出带宽过高产生高额的带宽包费用。

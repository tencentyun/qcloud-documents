## 操作场景
当您在腾讯云 VPC 中的部分 CVM 没有公网 IP，但需要访问公网时，可以利用带有公网 IP 的 CVM(如果购买时未申请公网IP，可以申请和绑定弹性公网 IP)，将这些 CVM 作为其它没有公网 IP 的 CVM 访问 Internet 的公网出口网关。公网出口网关 CVM 将对出网流量进行源地址转换，所有其他 CVM 访问公网的流量经过公网出口网关 CVM 后，源 IP 都被转换为公网出口网关 CVM 的公网 IP 地址，如下图所示：
...
### 步骤1：绑定弹性公网 IP（可选）
>?
>- 如果用作代理网关的 CVM 已经有公网 IP 地址，请跳过此步骤，完成后续步骤。
>- 代理网关 CVM 只能转发非所在子网的路由转发请求，因此，代理网关 CVM 不能与需要借助代理网关访问公网的  CVM 处于同一个子网下。
...
### 步骤3：配置普通子网路由表
配置普通子网的路由表，配置默认路由走代理网关 CVM，使得普通子网内 CVM 能通过代理网关的路由转发能力访问公网，详细操作步骤，请参见 [配置普通子网路由表](https://cloud.tencent.com/document/product/215/20139)。
步骤2创建路由表后会提示关联子网操作，直接关联即可：
![](https://main.qcloudimg.com/raw/2b88219349bc95197a6472a195af61df.png)
### 步骤4：配置代理网关
1. 登录代理网关 CVM，开启网络转发，NAT 代理功能，以及相关参数优化。
...
3. 完成上述配置后，重启代理网关 CVM 使配置生效，并在无公网 IP 的 CVM 上，测试是否能够成功访问公网。

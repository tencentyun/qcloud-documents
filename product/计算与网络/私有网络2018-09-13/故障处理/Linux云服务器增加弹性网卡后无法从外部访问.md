...
## 可能原因
云服务器绑定弹性网卡后，如需从外部访问，需要为弹性网卡绑定正确的弹性公网 IP（EIP），并配置网卡策略路由，该问题可从如下原因逐个排查定位：
+ 弹性网卡未正确绑定到云服务器上
...
4. 执行如下命令，查看弹性网卡的内网 IP 和 MAC 地址与步骤2记录的是否一致。
   ``` 
ip address
```
...
5. 执行如下命令重新修改辅助网卡的配置文件。[](id:s5)
   **说明**：此处以 Centos7.8 为例，Centos7 系统网卡配置文件一般存放在"/etc/sysconfig/network-scripts"目录，保存在“ ifcfg-网卡名称“ 文件。
   1. 进入配置文件
   ``` 
...
### 步骤四、检查是否将每个网卡的默认网关配置到策略路由表中[](id:step4)
...
### 步骤六、验证问题是否解决
在外网 ping 辅助网卡的 EIP，查看问题是否解决。
  - 如返回如下报文，表示已 ping 通，问题解决。
  - 如依然无法 ping 通，请记录问题信息，并联系 [在线支持]() 进一步定位处理。

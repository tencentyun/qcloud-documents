## 现象描述
某用户云服务器主网卡无公网IP，主要用于内网通信，因有公网业务的需求，故绑定了一个弹性网卡（辅助网卡），并在弹性网卡上绑定了弹性公网IP，但无法从外部正常访问。
![](https://qcloudimg.tencent-cloud.cn/raw/fed18ecde25765b8227a9d3ec509e305.png)
## 可能原因
云服务器绑定弹性网卡后，如需从外部访问，需要为弹性网卡绑定正确的弹性公网IP（EIP），并配置网卡策略路由，该问题可从如下原因逐个排查定位：
+ 弹性网卡未正确绑定到云服务器上
+ 弹性网卡内网IP未正确配置到弹性网卡上
+ 未配置策略路由表
+ 策略路由表中未添加网卡的默认网关
...
1. 登录 [云服务器控制台](https://console.cloud.tencent.com/cvm/instance/index?rid=16) ，单击云服务器实例ID，进入实例详情页。
![](https://qcloudimg.tencent-cloud.cn/raw/a4f18b41f8b9f84df5a5d587931a5374.png)
2. 单击“弹性网卡”页签，查看云服务器是否有弹性网卡，以及是否绑定了EIP。
  + 如有弹性网卡，且正确绑定了EIP，如下图所示，则继续排查 [步骤二](#step2)。
...
### <span id="step2">步骤二、检查弹性网卡内网IP是否正确配置到弹性网卡中
1. 单击云服务器详情页“弹性网卡”页签下的弹性网卡ID，进入弹性网卡详情页。
  	![](https://qcloudimg.tencent-cloud.cn/raw/5ea336de830e69b40e810261b85a014e.png)
2. 记录弹性网卡的MAC地址，并单击“IPv4 地址管理”页签，记录弹性网卡的内网IP地址。
  + **MAC地址**：
...
4. 执行如下命令，查看弹性网卡的内网IP和MAC地址与步骤2记录的是否一致。
   ``` 
ip address
```
...
5. <span id="s5">执行如下命令重新修改辅助网卡的配置文件。
   **说明**：此处以Centos7.8为例，Centos7系统网卡配置文件一般存放在"/etc/sysconfig/network-scripts"目录，保存在“ ifcfg-网卡名称“ 文件。
   1. 进入配置文件
   ``` 
 cd /etc/sysconfig/network-scripts/   #进入配置文件所在目录
 vim ifcfg-eth1  #编辑辅助网卡配置文件
```
 2. 按`i`开始编辑。
  ```
DEVICE='eth1'
HWADDR=20:90:6F:63:98:CC
NM_CONTROLLED='yes'
ONBOOT='yes'
IPADDR='10.0.0.14'  # 此处填写弹性网卡上的内网IP地址，请根据实际填写   
NETMASK='255.255.255.0'  # 此处填写子网掩码，请根据实际填写
GATEWAY='10.0.0.1    # 默认以eth0作为默认网关，如果需要使用弹性网卡作为主网关，可以在此处填写弹性网卡的默认网关
```
...
```
 5. 再次执行`ip address`，确认内网IP已经配置到弹性网卡上，如下图所示。
...
### <span =id=step4>步骤四、检查是否将每个网卡的默认网关配置到策略路由表中
>?配置好策略路由表后，需要针对每个网卡配置网卡的默认路由到策略路由表中。

1. 在云服务器登录界面，执行如下命令，可看到默认路由表中已经有辅助网卡的内网路由信息。
...
```
>!
>+ 每个table都需要检查，因为不同的table设置了不同网卡的默认路由。
>+ 请一定确认网关与网卡名对应一致，此处经常出现将eth0的网关配置到eth1上而导致配置问题，因此请务必做好检查。
>+ 此处配置为临时路由，重启网络后路由会消失，如需配置永久路由，请参见 [配置永久路由 ]()。

 配置后可查询到两个路由表中配置的默认路由条目：
 ![](https://qcloudimg.tencent-cloud.cn/raw/09d21f25ad332cc8e107126558de6d2e.png)
### <span id="step5">步骤五、检查是否配置了正确的策略路由规则
>?配置好默认路由后，目前系统已经知道哪个路由表走哪个网关出去，但还不知道某个网卡来的流量，要到哪个路由表上，因此需要查看是否为每个网卡配置了策略规则。

1. 执行 `ip rule list` 命令，查看是否有策略路由规则。
  - 如仅有0、32766、32767三条默认规则，表示辅助网卡未配策略路由规则，请执行步骤2配置。
  - 如已配置如红框所示的策略规则，且仍然无法ping通，请联系 [在线支持]() 。
...
### 步骤六、验证问题是否解决
1. 在外网ping 辅助网卡的EIP，查看问题是否解决。
  - 如返回如下报文，表示已ping通，问题解决。
  - 如依然无法ping通，请记录问题信息，并联系 [在线支持]() 进一步定位处理。

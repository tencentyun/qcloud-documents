# 通过EIP访问公网
EIP是私有网络内无公网访问能力的云资源访问公网的一种常用方式。本章节介绍云服务器绑定EIP访问公网的操作指导，其它云资源操作类似。
## 操作场景
如果您VPC中的云服务器资源没有公网IP，但是需要访问公网进行业务交互，您可以为其绑定EIP，实现与公网的通信。
...
1. 登录云服务器。
2. 测试公网连通性，例如执行`ping www.qq.com`，可看到有数据返回，表示该CVM可以访问公网。

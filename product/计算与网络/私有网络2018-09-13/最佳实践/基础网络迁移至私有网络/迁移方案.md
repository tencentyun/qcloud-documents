...
##  网络迁移中的混访方案
混访是指云资源在网络切换过程中可同时被基础网络和私有网络访问，目前支持混访能力的网络实例有：云数据库 MySQL、MariaDB、TDSQL、Redis、MongoDB，以及对象存储 COS 等，例如：
...
在基础网络业务迁移至私有网络的过程中，如果有基础网络与私有网络通信的需求，那么可结合基础网络互通、终端连接功能配合实现，例如：

当您购买 CVM 等实例时，需选择对应实例的所属私有网络，提供选择的私有网络有如下两种类型：
- 系统默认的私有网络，即系统为您在相应地域创建默认私有网络，与自行创建的私有网络功能完全一致，且默认私有网络不会占用您在某个地域下的配额，可以帮助您更快速地部署业务，节省时间。
- 自行创建的私有网络，即您在私有网络控制台中自行规划网段创建的私有网络。

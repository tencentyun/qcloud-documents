...
+ 在私有网络 VPC 中，同样通过在云服务器中部署 Keepalived 来实现高可用主备集群。与物理网络不同的是，出于安全考虑（如 ARP 欺骗等），通常不支持云主机通过 ARP 宣告普通内网 IP，该 VIP 必须为从腾讯云申请的高可用虚拟 IP (HAVIP) ，且该 VIP 具有子网属性，只能在同一子网下的机器间宣告绑定。
...
1. CVM1 和 CVM2 均安装keepalived软件，配置 HAVIP 为 VRRP VIP，并设置主备服务器的优先级（priority 值），值越大优先级越高。

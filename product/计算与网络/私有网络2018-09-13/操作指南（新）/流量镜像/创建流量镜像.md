...
2. 执⾏如下操作，验证采集端的流量是否正常镜像到接收端。
	...
	**Destination 数据包:** 
	...
	3. 使⽤终端模拟⼯具（例如 SecureCRT 等）登录接收端服务器，将[ 步骤2 ](#buzhou2)中保存的⽂件导出到本地。
```plaintext
...
3. 如获取到数据包异常或⽆法正常获取到数据包，请[ 联系我们 ](https://cloud.tencent.com/act/event/Online_service?from=doc_215)或[ 提交工单](https://console.cloud.tencent.com/workorder/category)。

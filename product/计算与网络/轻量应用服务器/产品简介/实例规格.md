...
### 中国大陆地域
<table>
	<tr><th>套餐类型</th><th style="width:18%">支持操作系统类型</th><th>CPU（核）</th><th>内存（G）</th><th>系统盘-SSD（GB）</th><th>带宽（Mbps）</th><th>每月流量包（GB）</th></tr>
...
## 出网带宽上限说明（下行带宽）
轻量应用服务器套餐中配置的公网网络带宽默认为出网带宽上限，即从轻量应用服务器流出的带宽。

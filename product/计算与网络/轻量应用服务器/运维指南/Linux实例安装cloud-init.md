...
Cloud-init 主要提供实例首次初始化时自定义配置的能力。本文档介绍如何在轻量应用服务器 Linux 实例上安装 cloud-init 服务。安装 cloud-init 推荐以下两种方式：
...
- 在正常安装的情况下，cloud-init-17.1 版本与腾讯云的兼容性最佳，可以保证使用该镜像创建的轻量应用服务器的所有配置项都可以正常初始化。建议选择 **cloud-init-17.1.tar.gz** 安装版本。您也可以 [点此获取](https://launchpad.net/cloud-init/+download) 其他版本的 cloud-init 源码包。本文以 cloud-init-17.1 版本为例。

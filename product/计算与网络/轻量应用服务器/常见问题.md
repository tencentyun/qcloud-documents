...
[](id:Q38)
### 轻量应用服务器如何设置流量包告警？
可通过 [云监控](https://cloud.tencent.com/document/product/248/13466) 自定义配置轻量应用服务器实例流量指标的阈值告警，您可通过该功能实时掌握实例流量包使用情况。云监控将会根据您设置的告警规则，及时通过微信、邮件、短信、电话等渠道通知您采取对应措施。创建告警策略具体步骤请参见 [新建告警策略](https://cloud.tencent.com/document/product/248/50398)。
...
[](id:Q22)
### 轻量应用服务器的购买配额是多少？
默认情况下，每个账户在单地域可创建10台轻量应用服务器实例。如果业务需求需要提升配额，可通过 [在线支持](https://cloud.tencent.com/online-service?from=connect-us) 申请。腾讯云会结合您的申请原因进行评估，通过后即可享受对应购买配额。

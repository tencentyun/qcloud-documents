...
请确保您的自定义镜像所在地域下有可用实例，及您的账户未处于欠费状态。自定义镜像配额及计费说明如下：
- 每个存在实例（不包含待回收实例）的地域提供5个免费自定义镜像配额，超过5个后将会产生计费。
- 当您的账号处于欠费状态时，账号下已有的自定义镜像（包含免费配额内的镜像）都将进入“待回收”状态。若自定义镜像进入“待回收”状态后7天内（包括第7天），账号未进行充值，则自定义镜像将会自动释放。

本文旨在介绍如何快速使用轻量级数据库服务，帮助用户快速了解轻量级数据库服务使用的全流程，从数据库的创建到基本使用，您需要完成如下操作。

## 创建数据库
1. 登录 [轻量级数据库服务购买页](https://buy.cloud.tencent.com/lighthousedb)，根据实际需求选择各项配置信息，确认无误后，单击【立即购买】。
 - **地域**：选择您业务需要部署  的地域。建议您选择与云服务器同一个地域，不同地域的云产品内网不通，购买后不能更换。
 - **类型**：目前支持 MySQL 5.7。
...
## 连接数据库
### 连接方式
- **内网地址连接**：通过内网地址连接轻量级数据库服务，使用云服务器 CVM 直接连接数据库的内网地址，这种连接方式使用内网高速网络，延迟低。
...
- **外网地址连接**：无法通过内网连接时，可通过外网地址连接轻量级数据库服务。外网地址需 [手动开启](#waiwang)，可在 [数据库列表](https://console.cloud.tencent.com/lighthouse/db/index) 的数据库详情页查看，不需要时也可关闭。
...
### 通过 DMC 平台连接
1. 登录 [轻量级数据库服务控制台](https://console.cloud.tencent.com/lighthouse/db/index)，在数据库列表，单击【登录】。
...
### 附录1：无法连接数据库原因
#### VPC 有误
若 CVM 和数据库的 VPC 不一致，则 CVM 无法直接通过内网连接数据库，请使用与数据库同一 VPC 的 CVM 进行连接，或在两个 VPC 网络之间建立 [云联网](https://cloud.tencent.com/document/product/877)。

...
- **内网地址连接**：通过内网地址连接轻量数据库服务，使用轻量应用服务器直接连接数据库的内网地址，这种连接方式使用内网高速网络，延迟低。
 - 轻量应用服务器和数据库须是同一账号，且同一个[ VPC](https://cloud.tencent.com/document/product/215/20046) 内（保障同一个地域），或同在基础网络内。
 - 内网地址系统默认提供，可在 [数据库概要页](https://console.cloud.tencent.com/lighthouse/db/index) 查看。
>?对于不同的 VPC 下（包括同账号/不同账号，同地域/不同地域）的轻量应用服务器和数据库，内网连接方式请参见 [云联网](https://cloud.tencent.com/document/product/877)。
...
### 附录1：无法连接数据库原因
#### VPC 有误
若轻量应用服务器和数据库的 VPC 不一致，则轻量应用服务器无法直接通过内网连接数据库，请使用与数据库同一 VPC 的轻量应用服务器进行连接，或在两个 VPC 网络之间建立 [云联网](https://cloud.tencent.com/document/product/877)。

...
1. [](id:Step1)进入实例详情页，选择**防火墙**页签，参考 [配置轻量应用服务器网络防火墙](#configFirewall) 步骤放通 `TCP:6443` 及 `UDP:8472` 端口。
...
- Node 节点需与 Master 节点内网互通。
  ...
  - 实例间的网络访问受到防火墙的控制，`TCP:6443` 及 `UDP:8472` 端口默认未在防火墙中放通，您需按照 [步骤1](#Step1) 放通端口。
</dx-alert>
示例命令如下：
```

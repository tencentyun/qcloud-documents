...
> ! 
> - 宝塔 Linux 面板的默认端口为8888。
> - 如果您在登录面板后修改了端口，或需要放通指定端口，则请在防火墙中同步更新或增加对应端口，并参考 [修改安全设置](#updateSafe) 放通操作系统端口。
>
...
在宝塔 Linux 面板的【安全】中，您可以按需对服务器进行安全设置。包含启动/禁用 SSH 、禁用 Ping 、配置服务器放行端口、配置屏蔽 IP 等。另外，您还可以查看面板操作日志。

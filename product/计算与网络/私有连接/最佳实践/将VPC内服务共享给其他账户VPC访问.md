...
## 场景示例
本文以下图业务场景为例。某公司业务部署在VPC2，现需要将该业务共享给公司内其他部门的其他账户访问。为避免公网访问带来的潜在安全风险，使用腾讯云私有连接Private Link来实现VPC1 到VPC2 的安全内网访问方案。
...
参数说明如下：
 ...
 + 自动接受：指定终端节点服务是否自动接受终端节点发起的连接请求，默认为否，此处和白名单策略结合使用，本例保持为否，不自动接受时，需将服务使用方账号添加在白名单中，且服务提供方接受连接后，方可接通。
...
2. 单击【新建】，在弹出的新建终端节点界面，配置相关参数。
 ...
 参数说明如下：
    ...
    + 对端账户类型：选择需要访问的 VPC 的所属账户，【我的账户】说明是同账户 VPC 的访问，【其他账户】说明是跨账户 VPC 的访问，本例选择【其他账户】，请输入对端账户 ID。

...
### CIDR
[CIDR（Classless Inter-Domain Routing）](https://cloud.tencent.com/doc/product/215/4927#cidr)即无类别域间路由，由您指定的独立网络空间地址块，通过 IP 和掩码结合，实现对网络的整体划分。以`10.1.0.0/16`为例，其中`10.1.0.0`为网络块的 IP，`16`为网络块的掩码。。通过设定掩码的大小，可以调整网络块的大小设定。网络块包括的 IP 数 = 2 ^( 32-掩码)，因而`10 .1.0.0/16`网络块最多包含 65536 个 IP 地址。

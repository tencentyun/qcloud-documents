...
### CIDR
CIDR（Classless Inter-Domain Routing）无类别域间路由，由您指定的独立网络空间地址块，通过 IP 和掩码结合，实现对网络的整体划分。以 10.1.0.0/16 为例，斜杠左边为网络块的 IP，斜杠右边为网络块的掩码。通过设定掩码的大小，可以调整网络块的大小设定。网络块包括的 IP 数 = 2 ^( 32-掩码)，因而 10 .1.0.0/16 网络块最多包含 65536 个 IP 地址。
查看<a href="https://cloud.tencent.com/doc/product/215/4927#cidr" target="_blank"> CIDR 详情</a>。
...
### 地域
<a href="https://cloud.tencent.com/doc/product/215/4927#.E5.9C.B0.E5.9F.9F.EF.BC.88region.EF.BC.89" target="_blank">地域</a>（Region）是腾讯云托管机房分布地区，地域下有不同的可用区。
...
### 可用区
<a href="https://cloud.tencent.com/doc/product/215/4927#.E5.8F.AF.E7.94.A8.E5.8C.BA.EF.BC.88zone.EF.BC.89" target="_blank">可用区</a>（Zone）一地域下（如广州），电力和网络互相独立的物理区域（一般是一个物理机房），如广州一区，划分可用区的目是能够保证可用区间的故障隔离（大型灾害故障除外）。
...
### 网络访问控制列表
<a href="https://cloud.tencent.com/doc/product/215/5132" target="_blank">网络访问控制列表</a>（Access control list，ACL）是一个子网级别的无状态可选安全层，可作为防火墙以控制进出子网的数据流，可以精确到协议和端口维度。除此之外，网络 ACL 可以限制网络流量，提高网络性能。

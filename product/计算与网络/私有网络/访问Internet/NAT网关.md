...
|公网IP	|最多绑定3个弹性IP|	1个弹性IP or 普通公网IP|
|公网限速	|无|	取决于云服务器限速|
|最大连接数	|1000w|	50w|
...
- 大容量：最大支持 1000W 并发连接、5Gbps 带宽和 3 个弹性 IP，满足大规模用户诉求。
- 双机热备高可用：单机故障自动切换，相比于公网网关的手动切换，实现了自动容灾，保障99.99%的服务可用性。
- 省成本：提供高、中、低三种配置，用户可按需购买，弹性计费，更省成本。

## 配置类型
NAT 网关支持绑定 3 个弹性IP，同时提供了三种配置类型，最大满足 5Gbps 突增流量和1000w 并发连接数。
...
## 关键特性
NAT网关主要有以下几点关键特性：
- DNAT：目的网络地址转换，用于 VPC 内的云服务资源访问互联网。
...
| 类型 | 国内 |香港（暂未发布） | 北美（暂未发布） |
|---------|---------|---------|---------|
| 小型 | 0.5元/h | 0.75元/h |0.8元/h |
| 中型 | 1.5元/h| 2.25元/h |2.4元/h |
| 小型 | 5元/h | 7.5元/h |8元/h |

 >注：欠费逻辑：与按量计费主机保持一致，[点击查看私有网络价格总览](https://www.qcloud.com/doc/product/215/3079)
...
### 使用 NAT 网关访问 Internet
您需要完成以下两个步骤创建 NAT 网关和配置访问 Internet：
#### 第一步：创建NAT网关

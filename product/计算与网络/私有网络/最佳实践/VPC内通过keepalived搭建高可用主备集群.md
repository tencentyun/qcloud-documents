...
## 本文小引
为了更清晰地阐述keepalived 如何在腾讯云主机上实践，本文：
- 首先对keepalived简述，并说明其在云主机的应用与物理网络的区别。
- 然后开始阐述如何通过何种步骤，达到两种使用模式：1) 无常主模式，即双机选举主设备的优先级相同；2) 常主常备模式，即需要让其中一台设备在无故障时尽量当主的场景。  常主常备模式较无常主模式增加了主备倒换次数，推荐使用无常主模式（非常主常备模式）
- 本文通过给出若干keepalived配置和脚本文件 + 不同场景配置方法的形式，帮助用户在云主机上作本次实践。
...
3.  编辑使用 keepalived  的 notify 机制，调用云 API 进行主备切换。
4.  编辑使用 keepalived 的 track_script 机制，周期性执行检查脚本增强可用性。
5.  给 VIP 分配外网 IP。**（可选）**

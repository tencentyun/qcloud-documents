...
4. 部署成功后，单击 `apigw` 部分输出 URL，根据指引完成账号密码配置，即可开始使用您的 Wordpress 应用。
...
在 [网关控制台](https://console.cloud.tencent.com/apigateway/service?rid=1) 修改自定义映射，如下图所示：
...
1. 修改 layer 代码。将 etc 文件夹中的 php.ini 文件移到 etc/php.d 文件夹，也可以直接使用我们提供的 [压缩包](https://github.com/serverless-components/tencent-wordpress/blob/master/src/fixtures/layer/wp-layer.zip)。
...
目前 API 网关 base 64 转码会将用户本身代码大小扩大1.5倍左右，因此上传的文件大小，建议文件大小控制在**3.5MB**以内。

...
1. [安装 Serverlesss Cloud Framework](https://cloud.tencent.com/document/product/1154/42990)。
...
1. 登录 [访问管理控制台](https://console.cloud.tencent.com/cam/role)，单击**新建角色**。
2. 角色载体选择**腾讯云产品服务**。
3. 在“输入角色载体信息”步骤中勾选**云函数（scf）**，并单击**下一步**。
4. 在“配置角色策略”步骤中，您可以直接选择 `QcloudCOSFullAccess` 对象存储（COS）全读写访问权限，如果需要更细粒度的权限配置，请根据实际情况配置选择。单击**下一步**。

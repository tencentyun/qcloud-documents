...
1. 所有资源要在同一个 VPC 内部，无需使用 vpc 的资源最好在同一个区域。
2. 该服务是 aPaas 的后台服务，需要结合 aPaas 的客户端代码一起使用。
...
* 若 Token 已过期，返回 UserTokenExpired (203)。
* 若 Token 不正确，返回 UserTokenInvalid (204)。
* 注意：由于涉及数据安全，该接口不返回 phone 和 email 字段。
...
* 用户若掉线，会根据不同的业务定义，更新相关的状态，比如 KTV 会在房主和主播掉线后，将房间设置为不可见。

...
>? 注意：旧版本命令为`sls deploy --inputs.key=value`，serverless cli v3.2.3 后命令统一格式为 `sls deploy --inputs key=value` ，旧版本命令在新版本serverless cli中不可用，升级serverles cli的用户请使用新版本命令。

## 操作场景
通过 Serverless Framework 的本地调试能力，支持在本地的模拟环境中运行代码，发送模拟测试事件，并获取到函数代码的运行日志等信息。
...
>!
>- 当前命令仅支持 Node.js 和 Python runtime。为保证部署云端和本地运行的结果一致，建议本地安装的 runtime 版本和云端版本保持一致。例如，在云端使用 Node.js 12.x，则本机建议也安装 Node.js 12.x 版本。
>- 目前只有 SCF 组件支持本地调试。

...
开发模式是为处于开发状态下的项目可以更便捷的进行代码编写、开发调试而设计的。处于开发模式中，用户可以进行持续的进行开发-调试的过程，而尽量不受打包、更新等其他工作的干扰。

### 进入开发模式

通过在项目下执行 `serverless dev`命令，可以进入项目的开发模式。
...
8. 通过 DevTools 的右侧栏，可以控制中断的程序继续执行、单步执行、步入步出的操作，也可以直接查看当前变量，或设定要跟踪查看的变量。DevTools 的进一步使用可以查看[DevTools说明文档](https://developers.google.com/web/tools/chrome-devtools/)。

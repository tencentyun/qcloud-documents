...
Serverless Components V2 是 Serverless Framework 重磅推出的基础设施编排能力，支持开发者通过 Serverless Components 构建，组合并部署 Serverless 应用。Serverless Components 既可以提供基础设施服务（例如，对象存储 COS），也可以提供更高阶的场景——这也是组件最擅长的部分。例如以下的场景：
...
目前，**Serverless Framework 1.67.2 版本及以上**支持 V2 版本 Component。
 ...
 > 注意：如果使用 Node.js 进行开发时，建议您使用 Node.js10.0 及以上版本，否则 Component V2 部署有可能报错

## 版本特性
Component V2 支持以下特性：

- **降低门槛：** 对于新用户而言，只需要在终端输入 serverless 命令，即可按照引导快速部署一个 Express 或 静态网站应用。
...
关于 Component V2 更多配置信息，您可以[点此查看](https://github.com/serverless/components/blob/master/README.cn.md)

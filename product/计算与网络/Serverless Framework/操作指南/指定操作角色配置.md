除了默认角色 SLS_QcsRole外，主账户还可自行创建多个角色，并将它们分配给每个子用户，各子用户根据自身业务需求只具有相应角色所赋予的策略，达到权限收缩的目的，流程图如下：
...
1. 创建子账户配置角色，并为该角色赋予相应策略。以部署api网关触发的scf函数为例，主账户在cam控制台 “角色‘标签下创建新的配置角色 test-role1，并为该角色赋予相应的访问策略，配置完成后角色页面：
    ...
    "statement": [
        {
            ...
            "resource": [
                #填入项目资源名称（例："qcs::sls:ap-shanghai::appname/*"）
            ],
            "effect": "allow"
        }
    ]

...
- **指定区域和环境**
执行以下命令，将会指定目标部署stage（pro）和 region（ap-shanghai），部署包会生成在您项目下的 .serverless 目录。
```
...
- **指定目录**
执行以下命令，将会指定目标部署stage（dev）和 region（ap-guangzhou），部署包会生成在目录`/path/to/package/directory` 。
```

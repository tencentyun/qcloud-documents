...
在 Serverless Framework CLI 运行`serverless deploy `命令进行项目部署，如果当前目录下未配置账号授权信息，则会在 Serverless Framework CLI 弹出二维码，直接使用**微信**扫描进行授权部署。
...
如您希望配置持久的环境变量/密钥信息，不需要每次都进行扫码部署，也可以在项目目录下创建`.env`文件，SecretId 和 SecretKey 信息并保存。
```ini
# .env

...
### 1. 授予子账户 Serverless Framework 所有资源的操作权限

在[用户列表](https://console.cloud.tencent.com/cam/user)中，选取对应子账户，点击【关联策略】，点击【从策略列表中选取策略关联】，搜索并关联 `QcloudSLSFullAccess`，策略语法如下：
...
### 2. 授予子账户 Serverless Framework 特定资源的操作权限

如果希望子账户仅对 Serverless Framework 某些特定资源有操作权限，在[用户列表](https://console.cloud.tencent.com/cam/user)中，选取对应子账户，点击【关联策略】，点击【从策略列表中选取策略关联】并点击【新建自定义策略】，创建一条自定义策略并关联到用户，参考策略语法如下：
...
配置完毕后，则子账户仅对 `${appname}` 和 `${stagename}` 下的 Serverless 应用具有操作权限。
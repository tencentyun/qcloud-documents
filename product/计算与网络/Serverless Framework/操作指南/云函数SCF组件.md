...
- 已安装 Serverless Cloud Framework（参考 [安装 Serverless Cloud Framework](https://cloud.tencent.com/document/product/1154/42990)）
...
- 方式二：直接使用 `scf init` 命令创建。快速创建一个 nodejs 的 SCF 示例：
```
scf init scf-nodejs
```
...
|组件名| 是否必选| 说明 |
|---------|---------|---------|
| component | 必填 | component 的名称，使用`scf registry`查询您可以引入的组件。  |
...
在 `serverless.yml`文件所在的目录下运行 `scf dev` 可以实时输出云端日志，每次部署完毕后，对项目进行访问，即可在命令行中实时输出调用日志，便于查看业务情况和排障。nodejs 支持开启开发调试能力，将会对本地代码的改动进行检测和自动上传。详情请参考 [开发与调试](https://cloud.tencent.com/document/product/1154/43220)。
...
云函数组件提供了组件级的命令。对于`scf deploy`部署成功的云函数，可以通过以下命令操作云函数。

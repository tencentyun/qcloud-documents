...
## 前提条件
已 [安装 Serverless Cloud Framework](https://cloud.tencent.com/document/product/1154/42990)。
...
>?`scf init` 支持快速初始化多个项目模板，请通过 `scf registry`查看所有支持的项目模板。

### 构建多组件应用
Serverless Cloud Framework 提供了多个基础资源组件，用户可以通过不同组件的结合使用，快速完成云端资源的创建与部署，无需在控制台手动操作（参考 [基础组件列表与配置方式](https://cloud.tencent.com/document/product/1154/51106)）。

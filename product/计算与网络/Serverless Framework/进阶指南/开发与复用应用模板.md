...
## 前提条件
已 [安装 Serverless Cloud Framework](https://cloud.tencent.com/document/product/1154/42990)，并保证您的 Serverless Cloud Framework 不低于1.0.2版本：
...
点击 `apigw` 输出的 URL，即可访问您已经创建好的应用，执行 `scf info`，可以查看部署的实例状态，执行 `scf remove`，可以快速移除应用。

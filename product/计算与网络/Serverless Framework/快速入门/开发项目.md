...
Serverless应用的灰度发布支持两种灰度发布：**默认别名**与**自定义别名**。更多详情参考[Serverless灰度发布](https://cloud.tencent.com/document/product/1154/46330)

- 默认别名：灰度发布配置简单，但只能在最后一次发布的函数版本和$latest版本间进行流量规则设置。目前适用于**云函数组件及涉及云函的相关组件**。
- 自定义别名：灰度发布配置灵活，可以在两个任意函数版本间进行流量规则设置。目前只适用于**云函数组件**。

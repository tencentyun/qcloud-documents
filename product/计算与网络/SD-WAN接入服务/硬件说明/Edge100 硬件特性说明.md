本文将为您介绍 Edge-100 设备的硬件特性。


## 基本信息

| 属性         | 规格                                                         |
| ------------ | ------------------------------------------------------------ |
| 产品         | Edge-100                                                     |
| LAN 口       | 3个 GE 电口。                                                  |
| WAN 口       | 2个 GE 电口。                                                  |
| LTE 模组      | <ul><li>LTE FDD：B1、B3、 B5、 B8。</li><li>LTE TDD：B38、B39、B40、B41。</li><li>WCDMA：B1、B5、B8。</li><li>TD SCDMA：B34、B39。</li><li>GSM：B3、B8。</li><li>CDMA EVDO/1x：800M。</li>  </ul>               |
| Wi-Fi        | Wi-Fi 模组 IEEE 802.11 b/g/n，2x2，miniPCIe 接口。                |
| RJ45         | 一个 RJ45 标准串口，不带指示灯。                                |
| USB          | 一个 USB 2.0 Type-A 接口。                                     |
| 指示灯       | <ul><li>电口 RJ-45 自带 LED 指示灯。</li><li>面板绿色指示灯 SYS、POWER、WIFI、LT、蜂窝信号强度1/2/3档 LED，CLOUD 。</li></ul> |
| 天线         | 缺省配置4根天线，其中2根 Wi-Fi 天线，2根 LTE 天线。 |
| 电源         | 12V电源适配器规格，AC 100~240V @50 - 60 Hz。 |
| 工作环境     | <ul><li>工作环境温度0°C - 45°C。</li><li>存储温度-40°C - +70°C。</li><li>工作海拔高度小于5000m。</li><li>工作相对湿度5% - 95%，非凝露。</li> </ul> |

## 指示灯说明
### 指示灯布局


| 指示灯 | 说明                                       |
| ------ | ------------------------------------------ |
| PWR    | 电源指示灯                                 |
| SYS    | 系统指示灯                                 |
| WIFI   | 无线 WIFI 指示灯                             |
| LTE    | 4G LTE 指示灯（包含额外三个信号强度指示灯） |
| CLOUD  | 云连接（IPSec VPN）指示灯                  |

### 指示灯亮灯方式

共有如下几种亮灯方式：

| 亮灯方式 | 用途                             |
| -------- | -------------------------------- |
| 熄灭     | 一般为不工作。                     |
| 长亮     | 一般为功能正常。                   |
| 闪硕     | 0.5s亮0.5s灭，一般为功能存在异常。 |

不同指示灯类型会有不同用途，具体定义如下：

#### PWR 支持的亮灯方式

| 亮灯方式 | 用途       | 软件处理方式           |
| -------- | ---------- | ---------------------- |
| 熄灭     | 电源未接通 | 硬件连接，无需软件处理。 |
| 长亮     | 电源接通   | 硬件连接，无需软件处理。 |

#### SYS 支持的亮灯方式

| 亮灯方式 | 用途                 | 软件处理方式                                                 |
| -------- | -------------------- | ------------------------------------------------------------ |
| 熄灭     | 系统硬件故障或复位   | 从系统上电，到 Linux 正常启动前，保持熄灭。无需额外处理（uboot 启动时默认熄灭） |
| 长亮     | 正常连上控制器并上线 | 正常情况。                                                     |
| 闪硕     | 没有连上控制器       | 直到 Agent 连接上控制器后变为长亮。                            |

#### WIFI 支持的亮灯方式

| 亮灯方式 | 用途       | 软件处理方式                                      |
| -------- | ---------- | ------------------------------------------------- |
| 熄灭     | Wi-Fi 未启动 | Wi-Fi 未启动时，设置为熄灭（Linux 启动时默认熄灭）。 |
| 长亮     | Wi-Fi 启动   | Wi-Fi 启动成功时，设置为长亮。                      |

#### LTE 支持的亮灯方式

>?由 LTE 模块进行设置，软件无法控制。

| 亮灯方式 | 用途          | 软件处理方式                                   |
| -------- | ------------- | ---------------------------------------------- |
| 熄灭     | LTE 未启动     | LTE 未插卡，设置为熄灭（Linux 启动时默认熄灭）。 |
| 闪烁     | LTE 注册网络中 | LTE 插卡，但未注册到网络。                      |
| 长亮     | LTE 正常工作   | LTE 插卡时，并成功注册到网络。                  |

#### LTE 信号强度指示灯支持的亮灯方式

>?有三格信号指示灯，由小到大依次标记为 LTE0，LTE1，LTE2，且 LTE 信号强度区分4级。

| 信号强度 | 亮灯           | 软件处理方式                                                 |
| -------- | -------------- | ------------------------------------------------------------ |
| 无信号   | 三盏灯都熄灭   | LTE 未启动、SIM 卡拔掉以及无信号时，设置为熄灭（Linux 启动时默认熄灭）。 |
| 很弱     | 仅 LTE0 长亮     | 监控 LTE 信号质量，信号很弱时设置。                                |
| 中等     | LTE0，LTE1 长亮 | 监控 LTE 信号质量，信号中等时设置。                                |
| 强       | 三盏灯长亮     | 监控 LTE 信号质量，信号强时设置。                                  |

#### CLOUD 支持的亮灯方式

| 亮灯方式 | 用途                   | 软件处理方式                                           |
| -------- | ---------------------- | ------------------------------------------------------ |
| 熄灭     | VPN 未启动或没有 VPN 配置 | IPSec 服务未启动，或者没有配置（Linux 启动时默认熄灭）。 |
| 长亮     | VPN 隧道建立成功        | 存在 IPSec 配置，并且已经建立成功。                      |
| 闪烁     | VPN 隧道未建立          | 存在 IPSec 配置，但隧道未建立成功。                      |

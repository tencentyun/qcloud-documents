本篇文档为您介绍腾讯云 SD-WAN 接入服务的典型使用场景之一，Edge 设备作为线下分支 PC、手机等客户端访问远端云服务器内网流量的转发设备。Edge 设备采用旁挂模式双边接入三层交换机，无需改变企业现有网络架构，内网带宽可达到 Edge 设备的最大规格。
...
>
## 操作场景
Edge 设备的 WAN1 侧和 LAN1 侧分别和三层交换机连接。线下分支可通过 Edge 设备与云上网络实现互通，其中：
- 线下分支 PC 所在网段为172.16.2.3/24
- Edge 设备 WAN 口的网段为192.168.0.2/24
- 私有网络的云服务器所在网段为172.16.0.3/24
- 三层交换机与 Edge 设备连接一侧的网段为192.168.0.1/24

>?
>- 若远端云服务器为另外一个 Edge 设备，且另外一个 Edge 设备的架构与当前 Edge 设备架构相同（Edge 设备以旁挂的方式双边接入三层交换机），则按照本篇所介绍的方式配置另一个 Edge 设备即可，请注意不可与当前 Edge 设备使用同一网段。
>- 架构图中的两个三层交换机可以是一台物理交换机的两个接口。
>

![](https://main.qcloudimg.com/raw/76a2118d800ac00f75e3b7333cf4e29e.png)
...
在三层交换机上进行如下配置：
- 新增静态路由，其目的网段为远端私有网络云服务器所在网段，即172.16.0.3/24

本地配置 Edge 设备时，除了可以通过管理控制台对设备进行配置，还可以登录 SD-WAN 接入服务本地 Web 控制台进行配置。本文将介绍如何登录本地 Web 控制台配置 Edge 设备。
...
## 步骤2：配置 Edge 设备
1. 启动 Edge 设备。
2. 在本地 PC 端浏览器访问本地 Web 控制台，即 `192.168.3.1` ，初次登录本地 Web 控制台时，需要设置登录密码。

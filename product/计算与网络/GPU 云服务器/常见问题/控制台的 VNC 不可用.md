...
## 解决方式
1. 若实例有公网 IP，则可参考 [使用 RDP 文件登录 Windows 实例](https://cloud.tencent.com/document/product/213/5435) 登录实例。
2. 针对安装图形驱动的 GPU 实例，可在该实例中手动安装 VNC Server，用户即可在本地通过 VNC Client 进行登录。
请自行获取 VNC Server/Client 安装包。
3. 检查已安装的第三方软件，分析其可能导致无法通过 VNC 方式登录实例的原因。
建议卸载该第三方软件，或重装系统。

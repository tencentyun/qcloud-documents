...
#### 创建集群
1. 登录云服务器控制台，选择左侧导航栏中的【[高性能计算集群](https://console.cloud.tencent.com/cvm/hcc/index?rid=1)】。
...
#### 创建 HCCG5v 实例
>?购买 HCCG5v 实例需通过申请，详情请参见 [高性能计算集群申请
](https://cloud.tencent.com/apply/p/pzkkscpz7km)。
>
...
```
按照界面提示输入提示信息，及 `conda` 安装目录。请将 conda 安装在 `/cfs/ti-deepspeed-demo` 目录下，安装至该目录则仅需安装一次软件依赖，不需要在每个节点上重复安装。
如下图所示，本文以 `/cfs/ti-deepspeed-demo/miniconda3` 目录为例：
...
#### 配置机器免密登录及安装 openmpi
由于百亿/千亿模型最佳实践均为8机64卡训练，按照一般分布式训练要求，机器需要提前做好免密登录，并安装好 openmpi。

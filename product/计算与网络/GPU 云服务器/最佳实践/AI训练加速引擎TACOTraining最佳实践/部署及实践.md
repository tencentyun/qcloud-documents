...
- 目前 TACO-Training 的三个加速组件已集成至同一个 Docker 镜像中，后续会提供单独的 whl 安装包，可结合您的实际开发环境集成组件。拉取镜像地址如下：
```bash
...
1. 参考 [购买 NVIDIA GPU 实例](https://cloud.tencent.com/document/product/560/30211) 创建实例。其中：
 ...
 - **镜像**：请选择 CentOS 7.8或 Ubuntu 18.04及以上版本。
CUDA 驱动及 cuDNN 的自动安装非本次部署的必选项，您可根据实际需求勾选“镜像”中的“后台自动安装GPU驱动”，使用自动安装功能安装 GPU 驱动。如下图所示：
![](https://main.qcloudimg.com/raw/529a71ea600327be582567a24ed52f97.png)

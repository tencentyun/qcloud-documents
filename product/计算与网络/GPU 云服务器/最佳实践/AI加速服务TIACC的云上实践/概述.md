...
### 接入便捷
训练加速和推理加速已支持原生的 Pytorch 框架，后续会支持 TensorFlow 等框架，用户可直接在原生框架下使用 TI-ACC 的加速能力，无需进行额外的模型格式转换等适配工作。
- 训练加速中的通信加速能力通过兼容原生的 DDP 工具，使用户无需修改原生的使用代码即可直接使用，数据 IO 优化、自适应 FP16均通过封装好的简单函数/类进行提供，仅需增加几行代码便可使用。
...
## 开始使用
在推荐、CV、NLP 等模型训练和推理场景中，都可以使用到 TI-ACC 的训练和推理加速能力。目前通过容器方式提供给用户使用，具体使用方式请参见 [推理加速部署及实践](https://cloud.tencent.com/document/product/560/64709) 及 [训练加速部署及实践](https://cloud.tencent.com/document/product/560/64710)。

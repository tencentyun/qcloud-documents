...
### 获取推理结果
本文硬件环境以 GPU 实例 GN7.2XLARGE32 为例，获取的推理加速实测数据如下表所示：
<table>
<tr>
<th>模型</th>
<th>batch</th>
...
<tr>
<td>input_model</td>
<td>多种<br>(STRING+torch.jit.ScriptModule 类型)</td>
<td>是</td>
<td>输入待优化的原始模型。Pytorch 模型格式支持以下格式：
...
<tr>
<td>test_data</td>
<td>多种<br>(LIST[Tuple[torch.tensor, ]])</td>
<td>否</td>
<td>
用于模型推理速度对比的测试数据。<br>
用户给出则使用给出的测试数据。如果用户不给出，则 TIACC 自动提供测试数据。对于不同类型的模型，其测试数据格式存在差异，PyTorch 模型的测试数据为若干组输入Tensor Tuple，类型为 LIST[Tuple[torch.tensor, ]]。
</td>
<td>None</td>
<td>/</td>
</tr>
...
<td>软件环境。包括模型训练框架+版本、CUDA+版本、cudnn+版本、tensorRT+版本。</td>
<td>

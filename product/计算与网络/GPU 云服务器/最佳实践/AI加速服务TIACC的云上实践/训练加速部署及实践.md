...
```plaintext
# 免密处理
# 把 publickey 添加到 ~/.ssh/authorized_keys，请注意本机和对端的 key 均需添加至 authorized_keys
ssh-keygen
```
```plaintext
# 确认 ssh 可以互通
...
- **输入参数**：
 ...
 - label_str2int：非必填，类别名（str）到类别 id（int）的映射关系文件。若不输入，则会自动生成。您可查看 [label_str2int 文件示例](https://docs.qq.com/scenario/link.html?url=https%3A%2F%2Ftiacc-1308240844.cos.ap-nanjing.myqcloud.com%2Flabel_str2int.pkl&pid=300000000$BFnZBDMjUakM&cid=159214213345)。
- **使用示例**：
```plaintext
...
<td>否</td>
<td>数据预处理变换方式。</td>
<td><pre style="color:white">'transforms.Compose([         

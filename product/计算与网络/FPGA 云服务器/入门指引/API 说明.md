...
- **[IN]fc_net_file**: fc_net 的文件路径，fc_net 是 net_file 中 ”pool5” 层后的各层单独列为一个文件，并增加1个 input layer 作为第一层， 参数设置为(批量大小x256x6x6)。
...
- **[INOUT]scores**: 接收打分结果的buffer 指针，指向 float 类型。数目不得低于分类标签的数目,分值的顺序与 label_file 里的标签顺序一致；

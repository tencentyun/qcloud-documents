VirtualService 定义了指定 hosts 的一系列路由规则和流量操作（权重路由，故障注入...），其中每一条路由规则都定义了指定流量协议的匹配规则，如流量匹配，则被路由至指定的服务或服务的版本。VirtualService 配置主要包含以下部分：
...
| `spec.http.mirrorPercent` | `uint32 ` | 定义流量镜像的复制百分比，缺省时复制 100% 的流量。最大值为 100 |

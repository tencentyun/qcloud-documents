...
**接入层**使用腾讯云 CLB 或其他云厂商 LB 产品结合 K8s ingress 和 service 实现流量接入和转发，也支持配置 nginx 实现。例如 [腾讯云负载均衡 CLB](https://cloud.tencent.com/product/clb) 。
...
3. 单击**创建**按钮完成创建，查看 qcbm-subscription 分发策略创建成功。
...
3. 单击**创建**按钮完成创建，命名空间 qcbm 成功创建并分发至指定集群上。
:::
::: 方式 2：使用 YAML 部署
1. 登录 [云原生分布式云中心控制台](https://console.cloud.tencent.com/tdcc)，进入**分布式应用管理** > **应用管理**页面。
2. 单击 **YAML 创建资源**，输入以下 YAML 配置。
3. 单击**创建**按钮完成创建，修改分发策略选择 qcbm-subscription。
```yaml
  apiVersion: v1
  kind: Namespace
  metadata:
...
3. 单击**创建**按钮完成创建，查看配置成功的创建并分发至指定集群上。
:::
::: 方式 2：使用 YAML 部署
1. 登录 [云原生分布式云中心控制台](https://console.cloud.tencent.com/tdcc)，进入**分布式应用管理** > **应用管理**页面。
2. 单击 **YAML 创建资源**，输入以下 YAML 配置。
...
3. 单击**确定**按钮，分布式云中心将自动将所有 QCBM 应用分发至两个集群上。单击名称进入详情页面可查看详情信息和拓扑图。
...
```
3. 单击**确定**按钮保存，分布式云中心将根据该差异化策略自动调整对应集群下的配置。

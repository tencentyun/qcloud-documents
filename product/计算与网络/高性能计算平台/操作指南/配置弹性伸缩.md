...
弹性伸缩 AS 将根据您的工作负载，弹性伸缩当前队列中的子机数量。若您在创建集群时使用了自动挂载文件存储 CFS 功能，则可参考本文步骤进行配置，使新扩容的节点自动挂载 CFS。
...
### 制作自定义镜像
使用弹性伸缩需要您通过已创建固定集群的计算节点制作自定义镜像。创建自定义镜像前，请您了解 [注意事项](https://cloud.tencent.com/document/product/213/4942#.E6.B3.A8.E6.84.8F.E4.BA.8B.E9.A1.B9)。步骤如下：
...
### 创建启动配置
参考 [创建启动配置](https://cloud.tencent.com/document/product/377/8544) 进行操作，您需注意以下事项：
- 启动配置需与集群在同一可用区、同一私有网络 VPC、同一子网下。
- “镜像”请选择已制作的自定义镜像。


### 创建伸缩组策略
参考 [创建伸缩组](https://cloud.tencent.com/document/product/377/3580) 进行操作，您需注意以下事项：

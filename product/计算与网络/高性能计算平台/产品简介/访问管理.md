...
<dx-tabs>
::: 支持资源级授权的 API 列表
下表列出高性能计算平台支持资源级权限操作的各个 API，以及每个操作支持的资源路径。
...
| API 接口：action | API描述  | 资源路径：resource                         |
| :--------------- | -------- | ------------------------------------------ |
...
<table><thead><tr><th width="50%">API 接口：action</th><th>API描述</th></tr></thead><tbody><tr><td>CreateCluster</td><td>创建集群</td></tr></tbody></table>

...
>?NAT 网关支持 TCP、UDP 和 ICMP 协议（仅支持云服务器向外 ping），而 GRE 隧道和 IPSec 使用的 ESP、AH 无法使用 NAT 网关，且暂不支持 ALG 相关技术。这是由 NAT 网关本身的特性决定的，与服务提供商无关。当前互联网大部分应用都是 TCP 应用，TCP 和 UDP 应用总和约占互联网应用类型的99%。

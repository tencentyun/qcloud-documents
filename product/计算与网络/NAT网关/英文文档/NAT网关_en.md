...
- Highly available master/slave hot backup: It supports automatic switch when a single server suffers a failure to allow automatic disaster recovery and 99.99% service availability, superior to the manual switch of a public network gateway.
- Cost effectiveness: Three configuration types, high, medium and low, are available for users to choose as needed, offering flexibility in billing and high cost effectiveness.
...
- Monitoring details display: All the key metrics for private IPs flowing to the NAT gateway are displayed, to help you implement rapid troubleshooting and locate exceptional traffic. The monitoring details data can be kept for 7 days. 
- Refined gateway traffic control: The bandwidth between a private IP and the NAT gateway can be restricted to guarantee key businesses.
...
- It has full-time and full-traffic gateway traffic analysis capability, to help minimize cloud network cost. By means of qos, it can restrict the bandwidth of non-key businesses to minimize cost with limited network budget.
...
- Security groups cannot be associated with the NAT gateway, but can be used for the instances within the private subnet in order to control the traffic that flows in and out of these instances.
...
| Maximum port forwarding entries per NAT gateway | 200 |

## Billing Method
Charges for an NAT gateway device include two parts: gateway rental fee (by hour) and traffic fee for accessing the Internet. The traffic fee can be charged using the "Bill-by-Traffic" method for CVM network charges. The billing modes for the NAT gateway are as follows:

| Type | Mainland China | Singapore/Silicon Valley/South Korean/Frankfurt/Hong Kong | Toronto |
|---------|---------|---------|---------|
...
| Large | 5 CNY/hr | 7.5 CNY/hr | 8 CNY/hr |
 ...
 - Arrear logic: Consistent with the Postpaid method for CVM. [Click to view VPC Price Overview](https://cloud.tencent.com/doc/product/215/3079).
 - As the NAT gateway features master/slave hot backup, the system sends a 5 KB detection packet to the master and slave servers of the NAT gateway every three seconds. This results in a traffic of 0.2747 GB each day, incurring a daily charge of 0.2197 CNY, 0.2747 CNY and 0.1373 CNY for Mainland China, Hong Kong and North America respectively.
...
If you want to allow the resources within the subnet of a VPC to access the Internet through a NAT gateway, you need not only to create the NAT gateway, but also to configure the routing rules in the routing table associated with the subnet that needs route forwarding.

## Quick Start
You can access the Internet through a NAT gateway by following the steps below:
...
4. After the creation of the NAT gateway is completed, you need to configure the routing rules on the Routing Table page of the VPC Console to direct the subnet traffic to the NAT gateway.
>**Note:**
The rental fee for 1 hour will be frozen during the creation of NAT gateway.
...
3. Click "New line", fill in the "Destination" field, select "NAT Gateway" in "Next Hop Type", and select the ID of the created NAT gateway.
...
1. Log in to [Tencent Cloud Console](https://console.cloud.tencent.com/), click "Virtual Private Cloud" in the navigation bar to go to the [VPC Console](https://console.cloud.tencent.com/vpc/vpc?rid=8), and then select "NAT Gateway".
2. In the NAT gateway list page, click the ID of the NAT gateway, whose port forwarding rule needs to be queried, to go to its details page, and select "Port Forwarding".
...
You can modify the attributes of an created NAT gateway.
1. Log in to [Tencent Cloud Console](https://console.cloud.tencent.com/), click "Virtual Private Cloud" in the navigation bar to go to the [VPC Console](https://console.cloud.tencent.com/vpc/vpc?rid=8), and then select "NAT Gateway".
...
### Managing EIPs Bound to NAT Gateway
1. Log in to [Tencent Cloud Console](https://console.cloud.tencent.com/), click "Virtual Private Cloud" in the navigation bar to go to the [VPC Console](https://console.cloud.tencent.com/vpc/vpc?rid=8), and then select "NAT Gateway".
2. In the NAT gateway list page, click the ID of the NAT gateway to go to its details page.
...
A NAT gateway can be deleted when it is not needed. The routing table and routing rules containing the NAT gateway will be deleted with the NAT gateway. Upon the deletion, the request forwarded over Internet is interrupted immediately. Be prepared for the network interruption in advance.
1. Log in to [Tencent Cloud Console](https://console.cloud.tencent.com/), click "Virtual Private Cloud" in the navigation bar to go to the [VPC Console](https://console.cloud.tencent.com/vpc/vpc?rid=8), and then select "NAT Gateway".
2. Select the NAT gateway to be deleted, click "Delete" button and confirm the deletion, and then the NAT gateway is deleted.

### Enabling Gateway Traffic Control Details
After it is enabled, you can view the metrics of IP traffic passing through an NAT gateway over the last 7 days, and can also set the outbound bandwidth from an IP to a specific NAT gateway.
1. Log in to [Tencent Cloud Console](https://console.cloud.tencent.com/), click "Virtual Private Cloud" in the navigation bar to go to the [VPC Console](https://console.cloud.tencent.com/vpc/vpc?rid=8), and then select "NAT Gateway".
2. In the NAT gateway list page, click the ID of the NAT gateway to go to its details page.
3. Click the "Monitor" tab, and enable the switch of "Gateway Traffic Control Details" on the upper right corner.
After the Gateway Traffic Control Details is enabled, it takes 5 to 6 days to collect and publish data. And then you can view the monitoring details table below the monitoring chart.
...
After Gateway Traffic Control Details is enabled, you can set the outbound bandwidth from an IP to a specific NAT gateway.
1. Log in to [Tencent Cloud Console](https://console.cloud.tencent.com/), click "Virtual Private Cloud" in the navigation bar to go to the [VPC Console](https://console.cloud.tencent.com/vpc/vpc?rid=8), and then select "NAT Gateway".
2. In the NAT gateway list page, click the ID of the NAT gateway to go to its details page.
...
### Binding Dayu Anti-DDoS Packages
...
3. Click "OK" to associate the high defense package with this NAT gateway.

## API Overview
You can use APIs to configure and manage your NAT gateway. For more information on other VPC resources, please see [API Overview](https://cloud.tencent.com/doc/api/245/909).
...
| Bind EIP for NAT gateway | [EipBindNatGateway](https://cloud.tencent.com/doc/api/245/4093) | Bind EIP for an NAT gateway. |
| Unbind EIP for NAT gateway | [EipUnBindNatGateway](https://cloud.tencent.com/doc/api/245/4092) | Unbind an EIP for an NAT gateway. |
| Upgrade NAT gateway specifications | [UpgradeNatGateway](https://cloud.tencent.com/doc/api/245/4090) | Upgrade the specifications of an NAT gateway. |

...
使用归档存储服务 CAS 时，发起的每一次 HTTP 的请求都要求为**签名请求**。签名请求根据腾讯云提供的密钥，并结合请求的内容计算出一串密文，通过 HTTP 请求传入。

- 签名请求：通过在 HTTP Headers 中的 Authorization 字段的方式传入加密后的签名信息，归档存储服务端将会判断该签名请求是否被许可。
...
Host: example.cn-southwest.myqcloud.com

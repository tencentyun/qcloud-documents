...
1. 当请求创建的bucket已经存在，并且请求创建的用户就是拥有者时，会返回409 Conflict，错误码为：BucketAlreadyExists。

2. 如果bucket的命名不规范，则会返回400 Bad Request消息。但是这里的具体错误码不定，可能返回的错误码有：InvalidBucketName、InvalidRequest等。具体原因可参考message的描述。

3. 如果bucket设置的ACL不正确，也会导致创建bucket失败，同时会返回“Failed to set access control authority for the bucket”的错误信息。具体错误原因，可根据返回的错误码参考[Put Bucket ACL](https://cloud.tencent.com/document/product/436/7737) 相关的文档。

4. 创建bucket的时候，如果没有指定访问权限，则默认使用私有读写(private)权限。

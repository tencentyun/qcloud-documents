...
| CIParam              | Request.Snapshot | 截图图片处理参数  | String    | 否   | 无  | 1. 参考 [缩放](https://cloud.tencent.com/document/product/460/36540) 文档 <br/> 2. 例如：imageMogr2/format/png |
...
| BlackLevel           | Request.Snapshot | 截图黑屏检测参数  | String    | 否   | 空  | 1. 当 IsCheckBlack=true 时有效<br/>2. 值参考范围[30，100]，表示黑色像素的占比值，值越小，黑色占比越小 <br/> 3. Start>0 ，参数设置无效，不做过滤黑屏 <br/> 3. Start =0 参数有效，截帧的开始时间为第一帧非黑屏开始  |
| PixelBlackThreshold  | Request.Snapshot | 截图黑屏检测参数  | String    | 否   | 空  | 1. 当 IsCheckBlack=true 时有效<br/>2.判断像素点是否为黑色点的阈值，取值范围：[0，255]  |
...
| Padding              | Request.Snapshot.SpriteSnapshotConfig | 雪碧图内边距大小  | String    | 否   | 0  | 1. 值范围：[0，4096]<br/> 2. 单位：px<br/> |
| Margin              | Request.Snapshot.SpriteSnapshotConfig | 雪碧图外边距大小  | String    | 否   | 0  | 1. 值范围：[0，4096]<br/> 2. 单位：px<br/> |
...
| Columns              | Request.Snapshot.SpriteSnapshotConfig | 雪碧图列数  | String    | 是   | 0  | 1. 值范围：[0，10000]<br/> |
| Lines              | Request.Snapshot.SpriteSnapshotConfig | 雪碧图行数  | String    | 是   | 0  | 1. 值范围：[0，10000]<br/> |

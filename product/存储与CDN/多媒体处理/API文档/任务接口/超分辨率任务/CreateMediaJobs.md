...
| 节点名称（关键字） | 父节点            | 描述                                                         | 类型      | 必选 |
| ------------------ | ----------------- | ------------------------------------------------------------ | --------- | ---- |
| SuperResolution    | Request.Operation | 指定超级分辨率模板参数                                         | Container | 否   |
...
| TranscodeTemplateId| Request.Operation | 指定的转码模版 ID，优先使用模板 ID，不能与 Transcode 同时为空         | String  | 否|
...
| 节点名称（关键字） | 父节点                         | 描述                                   |
| ------------------ | :----------------------------- | -------------------------------------- |
| Resolution         | Request.Operation.SuperResolution | 同创建超级分辨率模板 CreateMediaTemplate 接口中的 Request.Resolution | 
| EnableScaleUp      | Request.Operation.SuperResolution | 同创建超级分辨率模板 CreateMediaTemplate 接口中的 Request.EnableScaleUp|
...
| TranscodeTemplateId| Response.JobsDetail.Operation  | 指定的转码模版ID，优先使用模板 ID，不能与 Transcode 同时为空         | String  | 否|
| Watermark           | Response.JobsDetail.Operation | 指定水印模板参数，同创建水印模板 CreateMediaTemplate 接口的Request.Watermark，最多传3个 | Container | 否 |
| WatermarkTemplateId | Response.JobsDetail.Operation | 指定的水印模版 ID，可以传多个水印模板 ID，最多传3个，优先使用模板 ID          | String    | 否 |无|

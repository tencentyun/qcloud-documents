...
```
4. 请检查确认 LogListener 中密钥及 IP 标识配置信息是否有误，配置文件位于 loglistener 目录下的 etc 目录（若在`/usr/local`路径下安装了 loglistener，则配置文件位于`/usr/local/loglistener/etc/loglistener.conf`）。

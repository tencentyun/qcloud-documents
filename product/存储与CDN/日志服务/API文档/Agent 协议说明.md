...
```
POST /agent/heartbeat HTTP/1.1Host: <Region>.cls.myqcloud.comAuthorization: <AuthorizationString>{"total_size": 121212112}
```
...
| 字段名     | 类型 | 是否必选 | 含义                |
| ---------- | ---- | -------- | ------------------- |
| total_size | long | 是       | agent 采集的数据大小 |
...
上报当前机器采集到的经过处理的结构化日志。

#### 请求行

```
POST /agent/structuredlog
```

...
<tr>
  <td rowspan="2">仪表盘</td>
	  <td>每个账号最多可创建100个仪表盘</td>
</tr>
<tr>
	<td>每个仪表盘最多可包含30张分析图表</td>
</tr>
<tr>
...
<table>
	...
	<tr><td>单个统计字段值（Value）最大长度</td><td>最大长度为32KB，超出部分将无法写入 es</td><td>若单词大小超过32k，无法写入 ES，不便排查丢弃的日志内容</td></tr>
	<tr><td>非聚类分析（不含 group by 子句）结果条数</td><td>每次分析返回结果条数最大1000条</td><td>limit 默认100条，最大1000条</td></tr>
	<tr><td>聚类分析（含 group by 子句）结果条数</td><td>	每次分析返回结果条数最大2000条</td><td>limit 默认100条，最大2000条</td></tr>

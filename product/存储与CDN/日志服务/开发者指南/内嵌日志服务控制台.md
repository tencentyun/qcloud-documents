...
2021-07-15T10:00:00.000,2021-07-15T12:30:00.000</td></tr>
	...
	<tr><td>filter</td><td>否</td><td>String</td><td>过滤条件，采用 base64Url 编码，详见过滤条件参数说明</td></tr>
...
filter 参数用于生成下图中查询语句输入框底部的过滤条件，适合将比较固定的检索条件。
...
```
其中：
- key 为键值检索中的字段名称，如果需要全文检索，key 为 $ReservedClsSearchFilterFullTextKey。
...
<table>
	...
	<tr><td>全文检索-包含</td><td>INCLUDE_WITHOUT_KEY</td><td>[{"key":"$ReservedClsSearchFilterFullTextKey","grammarName":"INCLUDE_WITHOUT_KEY","values":[{"values":["test3"]}]}]</td><td>"test3"</td></tr>
	<tr><td>全文检索-不包含</td><td>EXCLUDE_WITHOUT_KEY</td><td>[{"key":"$ReservedClsSearchFilterFullTextKey","grammarName":"EXCLUDE_WITHOUT_KEY","values":[{"values":["test3"]}]}]</td><td>NOT "test3"</td></tr>

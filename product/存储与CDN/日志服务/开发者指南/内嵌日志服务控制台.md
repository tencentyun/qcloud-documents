...
5. 拼接日志服务免登录目标地址 `s_url`（可选, 获取后填写在免登陆项目的配置文件中，访问免登录服务后将自动跳转到此地址）
...
4. 使用获取到的秘钥，调用 [STS AssumeRole](https://cloud.tencent.com/document/product/598/33164) 接口，申请扮演角色的临时秘钥。

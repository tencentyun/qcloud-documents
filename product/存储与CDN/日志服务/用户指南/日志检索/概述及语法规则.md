...
| *          | 通配符查询，匹配零个、单个、多个字符，例如 `host:www.test*.com`<br />不支持前缀模糊查询，详情请参见 [模糊查询](https://cloud.tencent.com/document/product/614/47044#.E6.A8.A1.E7.B3.8A.E6.9F.A5.E8.AF.A2) <br />还可以通过 `key:*` 的方式查询字段（key）存在的日志，等价于 `_exists_:key` |
...
| [HAVING]()                                                   | 用于对分组聚合后的数据进行过滤，与WHERE的区别在于其作用于分组（GROUP BY）之后，排序（ORDER BY）之前，而WHERE作用于聚合前的原始数据，例如`level:* | select count(*) as logCounts,level group by level having count(*) > 100` |

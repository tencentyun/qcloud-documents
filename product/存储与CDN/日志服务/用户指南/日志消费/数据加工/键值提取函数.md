...
| ------------ | ------------------- | ----------- | ------------- | ---- -------- | ------------------------ |
| field     | 待提取的字段名      | string     | y       | -   | 用户日志中已存在的字段名 |
| output  | 单个字段名或以英文半角逗号拼接的多个新字段名       | string     | y        | -      | -   |
| sep       | 分隔符  | string            | n            | ,          | 任意单字符               |
| quote    | 将值包括起来的字符    | string            | n            | -           | -              |
| restrict          | 默认为 False。当提取的值个数与用户输入的目标字段数不一致时：</br>True：忽略，不进行任何提取处理 </br>False：尽量匹配前几个字段 | bool     | n        | False      | -              |
| mode     | 新字段的写入模式。默认强制覆盖        | string        | n          | overwrite  | -              |

...
收到告警通知时，通知内容将自动替换为如下的值，代表触发告警时最近的一条详细日志：
```
详细日志：
...
- **说明**：数组结构，`{{.QueryResult[0]}}` 代表第1个执行语句的详细日志，`{{.QueryResult[0]}}` 代表第2个执行语句的详细日志，以此类推。每个执行语句最多包含最近的10条详细日志。
- **变量值示例**：
```
[
	[{
...
:::
::: {{.[](id:AnalysisResult)}}[](id:AnalysisResult)

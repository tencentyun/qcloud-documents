## 操作场景
在本文档示例中，我们用到了日志服务（CLS），云函数（SCF），消息队列（Ckafka）。其中，CLS 主要用于日志采集，SCF 主要提供数据加工的节点计算能力，Ckafka 主要提供数据流终端转储能力。数据处理流程图参考 [函数处理概述](#概述URL)。
...
5. 切换至对象存储 Ckafka 控制台，查看数据转储及加工结果。

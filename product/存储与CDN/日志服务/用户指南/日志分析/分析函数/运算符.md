...
| ALL   |  语法：`x expression operator ALL ( subquery ) `  </br>x 满足所有条件时，返回 true，operator 支持`<、>、<=、>=、=、<>、!=`。   | 21 < ALL (VALUES 19, 20, 21) * \| SELECT 200 = ALL(SELECT status)   |
| ANY / SOME   | 	语法：`x expression operator ANY ( subquery )` 或 `x expression operator SOME ( subquery )`  </br>x 满足任意一个条件时，返回 true，operator 支持`<、>、<=、>=、=、<>、!=`。    | 'hello' = ANY (VALUES 'hello', 'world') * \| SELECT 200 = ANY(SELECT status)   |

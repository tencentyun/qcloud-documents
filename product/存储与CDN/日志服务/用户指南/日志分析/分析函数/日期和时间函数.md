日志服务提供时间函数、日期函数、时间阶段函数、时间间隔函数和时序补全函数，支持对日志中的日期和时间进行格式转换，分组聚合等处理。
...
按照一秒的时间粒度进行数据补全，查询和分析语句如下：
```
* | select time_series(__TIMESTAMP__, '1s', '%H:%i:%s', '0') as t, count(*) group by t order by t
```
![image-20210715154838801](https://main.qcloudimg.com/raw/5151467b8e37f846b98c9d33ca89db33.png)

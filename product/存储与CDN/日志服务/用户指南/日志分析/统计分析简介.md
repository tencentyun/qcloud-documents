日志服务提供 SQL 的统计计算功能，对采集的日志进行数据分析并支持以图表的形式展示分析结果。目前支持列表，折线图，柱状图，饼图，等多种类型的统计图表，用户可以根据自己的分析需求选择合适的图表类型展示分析结果。

## 分析语法简介

>  注意：
>  使用统计分析功能，必须在索引配置中，配置待统计键名（key）的键值索引，并打开统计开关。
>
...
- SQL 语句必须以`SELECT`关键词开头。
- 与标准的 SQL 语法相比，CLS 分析语句中，不需要设置 `from`子句，默认从当前日志主题中分析数据。
- SQL 语句不需要在末尾加分号表示结束。
- SQL 语句对大小写不敏感。
...
| 指标                                         | 限制说明                                                     | 备注                        |
| -------------------------------------------- | ------------------------------------------------------------ | --------------------------- |
| 单个统计键名（key）对应的值（Value）最大长度 | 最大长度为32KB，超出部分将被截断                             | -                           |

...
您需要先指定一个key，然后通过设置过滤规则对其进行正则提取过滤，用"()"捕获需要和 value 进行匹配的对象，并在value 处填上所要匹配的值。系统会先根据投递规则中的正则表达式进行一次匹配，提取出捕获组"()"的内容，然后和value 值进行比较，当捕获出来的内容=value时，则该条日志数据将会投递。
投递过滤规则最多支持5条，多条规则之间是“与”逻辑，即每条规则都满足的情况下才能投递成功。
**示例1：**
指定字段名为 status，该键值对形如 status:404，若要投递 status 字段为404的日志，则过滤规则为：`(.*)`。
![](http://file.tapd.oa.com//tfl/captures/2019-07/tapd_10114221_base64_1563158934_86.png)
**示例2：**
:指定字段名为 http_host，该键值对形如http_host:172.16.19.20，若要投递http_host字段为172.16开头的日志，则过滤规则为：`^(\d+\.\d+)\..*`
7. 单击【确定】，即可看到投递状态已开启。

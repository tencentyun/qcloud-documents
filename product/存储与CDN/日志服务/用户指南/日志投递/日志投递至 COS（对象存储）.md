...
<font color=red>权限说明：设置日志投递将授权日志服务对您指定的bucket有写的操作权限，若您以协作者身份配置日志投递，请先确认您的主账号下的给存储桶已经授权日志服务有写操作权限，否则将投递失败。</font>
...
- 目录前缀（非必选）：日志服务支持您自定目录前缀，目前投递至COS存储桶中的分区为`COS-BUCKET/{prefix}/{logset}/{topic}/{Y}/{m}/{d}/{time}_{random}_{index}.gz` 我们将每天为您建议一个目录，路径中的 time 指此日志文件中包含的实际的日志时间，random_index是一个随机数

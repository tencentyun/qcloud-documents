图表自定义跳转功能，支持点击图表中的数值跳转到设定的URL地址，URL地址可以引用变量。
...
<img src="https://qcloudimg.tencent-cloud.cn/raw/2d54d1d55bf45c8af9f12ad001fe19dc.png" width="65%">
...
- `${__Fields.具体字段}`  同行该字段的值。如下图所示，单击 **8.4s** 触发跳转链接，链接中嵌入了 **${__Fields.protocol_type}** 变量，将会引用被点击值同行该字段的值，即 protocol_type 的值**http2**。
...
场景：现已经统计了服务端IP的错误率的TOP情况，当出现错误率较高的IP时，需要跳转到检索分析页面，查看该 IP 的 status 状态码为 4XX 的日志。

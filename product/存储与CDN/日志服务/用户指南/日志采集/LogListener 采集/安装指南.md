...
以解压路径 `/usr/local/`为例：
先下载LogListener，然后将下载的安装包解压，安装解压路径为 `/usr/local/` ，进入 Loglistener 安装目录`/loglistener/tools` 执行以下命令进行安装，命令如下：
```shell
wget https://main.qcloudimg.com/raw/8656fcadd12ab9689674df09b510b52b/loglistener.2.2.2.tar.gz && tar -zxvf loglistener.2.2.2.tar.gz -C /usr/local/ && cd /usr/local/loglistener/tools && ./install.sh [SecretId] [SecretKey] [region]
```
...
下载并安装成功如下图：
![](https://main.qcloudimg.com/raw/aa69b977801c012199c5165db44b6ecf.png)

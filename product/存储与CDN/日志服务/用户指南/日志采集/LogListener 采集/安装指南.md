...
以解压路径 `/usr/local/`为例：
先下载LogListener，然后将下载的安装包解压，解压路径为 `/usr/local/` ，进入 Loglistener 安装目录`/loglistener/tools` ，命令如下：
...
进入 Loglistener 安装目录`/loglistener/tools`，以 root 权限执行以下命令，安装Loglistener：

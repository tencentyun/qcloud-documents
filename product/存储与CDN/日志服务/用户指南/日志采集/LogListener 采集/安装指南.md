...
以解压路径`/usr/local/`为例：
下载 LogListener，然后解压下载的安装包，解压路径为`/usr/local/` ，进入 Loglistener 安装目录`/loglistener/tools` ，执行命令如下：
```shell
...
进入 Loglistener 安装目录`/loglistener/tools`，以 root 权限执行以下命令，安装 Loglistener：

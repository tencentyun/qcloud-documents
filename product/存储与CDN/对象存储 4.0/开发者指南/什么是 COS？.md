...
对象存储（Cloud Object Storage，简称 COS）是面向企业和个人开发者提供的高可用，高稳定，强安全的云端存储服务。您可以将任意数量和形式的非结构化数据放入 COS，并在其中实现数据的管理和处理。COS 支持标准的 Restful API 接口，您可以快速上手使用。对象存储支持按量计费的后付费模式，无最低使用限制。此外，COS 在中国大陆地域推出了可预付费优惠购买的资源包，用于抵扣实际使用量。
...
[创建请求](#创建请求概述)|讲述使用 REST API 、使用 SDK 创建请求。

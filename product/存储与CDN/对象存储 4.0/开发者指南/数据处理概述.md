...
腾讯云 COS 集成了数据万象（Cloud Infinite，CI）专业的一体化媒体解决方案，涵盖图片上传、下载、存储、处理、识别等功能，详见如下表。您可以通过 COS 的上传和处理接口来进行媒体数据的处理操作。
...
您可以使用对象存储控制台进行图片处理设置，详情请参见 [开启图片处理](补充对应链接)。

> !当前对象存储控制台仅支持图片处理功能，仅支持北京、上海、广州、成都、重庆、南京地域。

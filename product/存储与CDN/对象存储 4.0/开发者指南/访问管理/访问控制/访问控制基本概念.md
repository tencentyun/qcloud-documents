**在默认情况下，腾讯云对象存储 COS 中的资源（存储桶和对象）都是私有的**。只有腾讯云主账号（资源拥有者）才可以访问、修改存储桶和对象，其他用户（子账号、匿名用户等），在未授权的情况下都无法直接通过 URL 访问对象。
...
- 使用腾讯云 [访问管理 CAM](https://cloud.tencent.com/document/product/598/10583) 账号创建存储桶或上传对象时，其父级主账号就是资源拥有者。
...
- 以用户为出发点：用户策略（CAM 策略），将权限关联在用户上，填写策略时不需要填写用户，需要指定资源、操作、条件等，在 [访问管理 CAM ](https://console.cloud.tencent.com/cam)配置。
...
以下是一个许可匿名用户访问位于广州的存储桶 examplebucket-1250000000 中所有对象的策略，无需签名校验即可下载存储桶中的所有对象（GetObject），即任何知晓 URL 的匿名用户都可以下载对象（类似于公有读方式）：

...
以下存储桶策略示例表示用户必须在 10.217.182.3/24 或者 111.21.33.72/24 网段才能完成授权`cos:PutObject`操作。其中，
...
- 条件操作符含有`_if_exist`，如`string_equal_if_exist`，当请求不带对应的请求头/请求参数时，默认命中条件，即为`True`。
...
存储桶策略中的特殊字符，都需要经过 urlencode。例如，您在使用`cos:content-type`条件键时，"image/jpeg" 必须经过 urlencode 转换为 "image%2Fjpeg"，再填入存储桶策略中。

...
| Replicationstatus   | 用于标记对象复制中源端文件和副本文件的状态。源端文件标记：PENDING（待复制）、COMPLETED（复制完成）、FAILED（复制失败）；副本文件标记：REPLICA（已完成复制，生成副本文件）。有关更多信息，请参见 [复制行为说明](https://cloud.tencent.com/document/product/436/19923)  |
| Tag | 对象的标签 | 

...
3. 执行成功会返回 DeleteObjectsResult 对象，包含所有已删除的对象键。如果部分成功部分失败（如对该对象没有删除权限），则返回 MultiObjectDeleteException 类。其他失败导致的异常返回异常类(CosClientException/CosServiceException)，请参照SDK异常类说明。

...
>?在高带宽或弱网络环境中，当您的对象较大（例如超过100MB）时，即使小于5GB，我们仍建议您优先使用分块上传的方式。因为分块上传可以并行传输多个分块，在高带宽情况可以有效利用带宽资源。而在弱网络环境中，单一分块上传失败不会影响其他已上传的分块，通过简单的重试即可重传失败的分块，因此可以提高整体的上传成功率。有关移动端在弱网络环境的上传，可以参见 [弱网分块续传实践](https://cloud.tencent.com/document/product/436/34352)。

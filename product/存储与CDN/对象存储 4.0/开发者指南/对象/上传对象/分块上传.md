...
String bucketName = "mybucket-1251668577";
String key = "aaa/bbb.txt";
// uploadid(通过initiateMultipartUpload或者 ListMultipartUploads 获取)
...
（3） `CompleteMultipartUploadRequest`是完成分块请求，需要传入之前上传的所有分块的 partEtag。通过调用completeMultipartUpload 完成分块上传。示例代码如下：
...
String bucketName = "mybucket-1251668577";
String key = "aaa/bbb.txt";
// uploadid（通过 initiateMultipartUpload 或者 ListMultipartUploads 获取）
String uploadId = "1512380198aecfed004aeaaca195d08232f718f7b52a91b8f6e9d36c7dfead2b3d1c917a6f";
...
对象存储 COS 的 PHP SDK 中提供了此方法，可参考 [PHP SDK 对象操作](https://cloud.tencent.com/document/product/436/34282) 文档。
...
可以通过 abort_multipart_upload() 方法来终止一个分块上传，需要提供存储桶名、对象键名称以及 UploadId：
```python
...
可以通过 list_multipart_uploads() 方法来列出某个存储桶中所有未完成的分块上传，需要提供存储桶名：
```python

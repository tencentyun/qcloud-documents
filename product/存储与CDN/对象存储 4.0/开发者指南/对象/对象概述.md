...
对象（Object）是对象存储的基本单元，可理解为任何格式类型的数据，例如图片、文档和音视频文件等。存储桶（Bucket）是对象的载体，每个存储桶可容纳任意数量的对象。对象在 COS 中可指定为不同的存储类型，详情请参见 [存储类型概述](https://cloud.tencent.com/document/product/436/33417)。

...
腾讯云对象存储 COS 是一个使用 HTTP/HTTPS 协议访问的 Web 存储服务，您可以使用 REST API 或 [COS SDK](//cloud.tencent.com/document/product/436/6474) 来访问 COS。

通常情况下，当您发起请求，需要经过认证和鉴权后，才可以对资源进行操作。因此根据身份是否可识别，访问 COS 的请求分两种类型：匿名请求和签名请求。
...
如果您使用 COS SDK 访问，只需配置好您的安全凭证即可发起请求；如果您使用 REST API 访问，则需要参阅 [请求签名](https://cloud.tencent.com/document/api/436/7778) 文档自行计算请求签名或通过 [COS 签名工具](https://cloud.tencent.com/document/product/436/30442) 直接生成。
...
由于角色是一个虚拟身份，因此角色不具备永久密钥，腾讯云的 CAM 提供了一套 STS API 用于生成临时安全凭证。使用方法和示例详情请参阅 [使用角色](https://cloud.tencent.com/document/product/598/19419) 文档，或查看生成临时安全凭证的方式请参阅 [STS API ](https://cloud.tencent.com/document/product/598/13895) 文档。临时安全凭证通常只包括 **有限策略**（操作、资源、条件）和 **有限时间**（有效起止时间），因此生成的临时安全凭证可自行分发或直接使用。
...
接口还会通过 `expiration` 字段返回临时安全凭证的有效时间，这意味着您只能在此时间内使用这组安全凭证来发起请求。
...
COS 推荐使用虚拟托管型域名访问存储桶，在发起 HTTP 请求时直接通过 `Host` 头部带入需要访问的存储桶，形如 `<BucketName-APPID>.cos.<Region>.myqcloud.com`。使用虚拟托管型的域名实现了类似虚拟服务器「根目录」的功能，可用于托管类似 favicon.ico、robots.txt、crossdomain.xml 的文件，这些文件是很多应用程序在识别托管网站时，会默认从虚拟服务器「根目录」位置检索的内容。

您也可以使用路径型请求来访问存储桶，形如使用 `cos.<region>.myqcloud.com/<BucketName-APPID>/` 的路径来访问存储桶。相应的，请求 Host 和签名需要使用 `cos.<region>.myqcloud.com`，我们的 SDK 默认不支持此种访问方式。
...
静态网站域名形如：`<BucketName-APPID>.cos-website.<Region>.myqcloud.com`，您也可以通过控制台的存储桶「基础配置 - 静态网站配置」模块获取此域名。

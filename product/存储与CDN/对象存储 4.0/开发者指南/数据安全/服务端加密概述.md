...
## 加密方式
腾讯云 COS 支持三种不同的服务端加密方式：SSE-COS、SSE-KMS 和 SSE-C。用户可以自行选择合适的加密方式对存放到 COS 中的数据进行加密。
...
>!
>- 当使用 POST 操作上传对象时，需在表单字段中提供相同的信息，而不是提供 x-cos-server-side-encryption 头部。更多详情，请参阅 [POST Object](https://cloud.tencent.com/document/product/436/14690)。
...
腾讯云 COS 提供 REST API 与服务端加密相关的接口，在创建对象时可以通过提供 x-cos-server-side-encryption 头部来请求服务端加密。当用户执行以下 PUT、分块上传和 Copy 操作时，需要提供 x-cos-server-side-encryption 头部。当使用 POST 操作上传对象时，请在表单字段中提供相同的信息，不需提供该请求头部，详情请参阅 [POST Object](https://cloud.tencent.com/document/product/436/14690)。以下操作支持此头部：
...
#### 使用 REST API
当用户执行以下 PUT、分块上传、POST 和 Copy 操作时，需要提供 x-cos-server-side-encryption 头部。
...
#### 使用 REST API
当用户执行以下 GET、HEAD、 PUT、POST 和分块上传操作时，需要提供 x-cos-server-side-encryption 头部。

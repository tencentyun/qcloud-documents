...
以下是一个许可匿名用户访问位于广州的存储桶 `testbucket-125125125` 中所有对象的策略，无需签名校验即可下载存储桶中的所有对象（GetObject），即任何知晓 URL 的匿名用户都可以下载到对象（类似于公有读方式）：
  ...
  "Statement": [
    {
      ...
      "Resource": ["qcs::cos:ap-guangzhou:uid/125125125:testbucket-125125125/*"]
    }
  ],
  "Version": "2.0"
}
...
以下是一个存储桶 ACL 的示例，描述了存储桶拥有者（用户 UIN：12345）的完全控制权限：
...
以下是一个对象 ACL 的示例，描述了对象拥有者（用户 UIN：12345）的完整控制权限，并授予了所有人可读（匿名用户公有读）的权限：
...
以下是一个授权位于广州的存储桶 `testbucket-125125125` 所有 COS 操作的策略，您需要将策略保存后再关联到 CAM 子用户、用户组或角色方可生效：
  ...
  "Statement": [
    {
      ...
      "Action": ["cos:*"],
      "Resource": [
          "qcs::cos:ap-guangzhou:uid/125125125:testbucket-125125125/*",
          "qcs::cos:ap-guangzhou:uid/125125125:testbucket-125125125/"
      ]
    }
  ],

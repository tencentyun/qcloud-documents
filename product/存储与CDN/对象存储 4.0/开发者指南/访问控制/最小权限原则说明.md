...
您可以使用预签名 URL 的方式将签名提交给第三方，以供完成临时的上传操作。收到有效预签名 URL 的任何人都可以上传对象。

临时密钥和永久密钥都可以用于生成预签名 URL，但强烈建议您使用临时密钥计算预签名，并遵循最小权限指引原则（这里加个链接）生成临时密钥；尽量不要使用权限过大的永久密钥，以免造成安全风险。

#### 授权示例

#### 授予用户使用临时密钥生成的预签名进行下载

...
一种方式是 uin 方式，即主账号的 UIN 账号，表示为 `uin/${OwnerUin}`，如 uin/100000000001。
另外一种方式是 uid 方式，即主账号的 APPID，表示为 `uid/${appid}`，如 uid/1250000000。目前 COS 的资源拥有者统一使用 uid 的方式表述，即主账号的开发商 APPID。|是

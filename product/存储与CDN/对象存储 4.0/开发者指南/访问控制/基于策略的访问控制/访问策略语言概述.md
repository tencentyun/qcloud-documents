...
- **委托人（principal）**：描述策略授权的实体。例如主账号、子账号、所有用户或者云服务。该元素对于存储桶访问策略有效，对用户访问策略则不应添加。
...
授予所有用户权限：（请注意，对指定账户的授权将优先于对所有用户的授权，详情请参阅 [访问策略评估流程](https://cloud.tencent.com/document/product/436/41516)）
...
允许所有用户（包括 CAM 用户和匿名用户），在访问来源 IP 为 101.226.\*\*\*.185/101.226.\*\*\*.186 时，对华南地区存储桶 examplebucket-1250000000 中的对象，执行 GET Object 和 HEAD Object 操作。更多案例请参阅 [权限设置相关案例](https://cloud.tencent.com/document/product/436/12514)。

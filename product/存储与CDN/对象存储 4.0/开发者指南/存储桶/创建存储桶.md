...
- Region：选择您的存储桶所属地域，一旦存储桶创建完成，地域将无法修改，前往查看 COS 支持的 [可用地域](https://cloud.tencent.com/document/product/436/6224)。

## 使用方法
### 使用对象存储控制台
您如需使用对象存储控制台创建存储桶，请参阅 [创建存储桶](https://cloud.tencent.com/document/product/436/13309) 控制台指南文档。

### 使用 REST API
您可以直接使用 REST API 发起一个创建存储桶的请求，请参阅 [Put Bucket](https://cloud.tencent.com/document/product/436/7738) API 文档。
### 使用 C++ SDK

对象存储 COS 的 C++ SDK 中提供了此方法，请参阅 [C++ SDK 接口文档 Put Bucket 部分](https://cloud.tencent.com/document/product/436/12302#put-bucket)。
...
1. 准备好签名服务器，提供 auth.php 接口给前端获取签名，可参考 [后端签名例子](https://github.com/tencentyun/cos-js-sdk-v5/tree/master/server) 。
...
对象存储 COS 的 PHP SDK 中提供了此方法，可参考 [PHP SDK 存储桶操作](https://cloud.tencent.com/document/product/436/34277) 文档。
...
```php
try {
    $result = $cosClient->createBucket(array('Bucket' => 'examplebucket-1250000000'));
    print_r($result);
    } catch (\Exception $e) {
    echo "$e\n";
}

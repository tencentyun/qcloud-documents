...
```shell
目标存储桶/{对象前缀}/{y}/{m}/{d}/{time}_{random}_{index}.gz
```
...
## 启用日志管理
### 使用控制台
用户可以通过控制台快速开启日志管理功能。操作指引请参阅 [设置日志管理](https://cloud.tencent.com/document/product/436/17040) 控制台指南。
...
#### 3. 创建日志角色
调用接口，开启日志管理功能，具体接口信息参见 [PUT Bucket logging](https://cloud.tencent.com/document/product/436/17054)，其中，要求存放日志的目标存储桶和源存储桶在同一地域。

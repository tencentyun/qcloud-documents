...
PUT Bucket Versioning 接口实现启用或者暂停存储桶的多版本功能。
### 细节分析
1. 如果您从未在存储桶上启用过多版本，则 GET versioning 请求不返回版本状态值。
2. 开启多版本功能后，只能暂停，不能关闭。
3. 设置多版本状态值为 Enabled 或者 Suspended，表示开启多版本和暂停多版本。
4. 设置存储桶的多版本功能，您需要有存储桶的写权限。
...
| 节点名称（关键字）                | 父节点               | 描述    | 类型   |
| --------------------------------------- | --------------------- | --------- | ------- |
| VersioningConfiguration |        无                                   |说明多版本的具体信息    | Container    |
...
| 错误码             | HTTP状态码         |描述                                       | 
| -------------- | --------------------------------------- | -------------- |
| InvalidArgument | 400 Bad Request |如果开启多版本的 xml body 为空，会返回 InvalidArgument  | 
| InvalidDigest   |400 Bad Request | 1. 携带的 Content-MD5 和服务端计算的请求 body 的不一致；<br>2. 开启多版本的状态只有 Enabled 和 Suspended 两个合法值，如果写了其他状态，会返回 InvalidArgument | 

...
SQL 语法介绍请参见 [SQL 命令]()、[SQL 函数]()、[运算符]()、[数据类型]()。

为了保证功能的正常使用和后续扩展，日志检索分析功能指定了保留字段，包含函数名称、数据类型、运算符等字段，详情请参见 [保留字段]()。
...
对于筛选类语句，支持生成 [日志趋势图](日志检索分析文档中的第五步)。支持生成趋势图的筛选类语句必须满足以下条件：

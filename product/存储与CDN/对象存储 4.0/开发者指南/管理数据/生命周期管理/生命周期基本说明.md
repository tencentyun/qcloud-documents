...
热数据往往在上传后，短时间内被大量访问而热度升高，一段时间后热度逐渐降低或者不再需要被实时访问。您可以通过生命周期规则将30天前的数据沉降至低频存储，进一步可以将60天前的数据沉降至归档存储。
...
如需转换为 ARCHIVE 存储类型，则仅在归档存储 CAS 提供的地域支持转换为 ARCHIVE 类型。
目前支持的地域：北京 ap-beijing、上海 ap-shanghai、广州 ap-guangzhou、成都 ap-chengdu、重庆 ap-chongqing。

#### 单向原则

对象存储发生的存储类型转换是单向的，只允许从 STANDARD → STANDARD_IA → ARCHIVE，也支持跳级沉降（例如 STANDARD → ARCHIVE），但是不支持反向。您只能通过调用 [PUT Object - Copy](https://cloud.tencent.com/document/product/436/10881)（针对非 ARCHIVE 类型），或 [POST Object restore ](https://cloud.tencent.com/document/product/436/12633)（仅适用于 ARCHIVE 类型）来转换较冷存储类型至较热的存储类型。
...
请注意 STANDARD_IA 类型需存储至少30天、ARCHIVE 类型需存储至少60天，执行转换存储类型或删除才不会产生额外的存储费用。腾讯云 COS 不会检查少于30/60天的生命周期配置，因此对于正确的配置都将按照您的要求执行。

例如，一个 STANDARD_IA 对象在未存满30天时被执行沉降转换，将导致对象在当天产生 ARCHIVE 存储费用的同时，STANDARD_IA 类型仍将计费至第30天止。

例如，一个 ARCHIVE 对象在未存满60天时被执行过期删除，将导致对象持续以 ARCHIVE 类型计费至第60天止。

#### 不受大小限制

在 STANDARD_IA、ARCHIVE 类型分别制定了对象最小占用空间限制，例如在 STANDARD_IA 中上传小于64KB的文件，将按照64KB计算。腾讯云 COS 不会检查文件的大小，将无条件按照指定的规则，执行对象的转换操作。 

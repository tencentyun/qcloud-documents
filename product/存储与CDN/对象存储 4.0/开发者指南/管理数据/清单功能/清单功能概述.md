...
清单是一种帮助用户管理存储桶中对象的功能，将有计划地取代对象存储同步 List API 操作。对象存储 COS 可根据用户的清单任务配置，每天或者每周定时扫描用户存储桶内指定的对象，并输出一份清单报告，以 CSV 格式的文件存储到用户指定的存储桶中。文件中会列出存储的对象及其对应的元数据，并根据用户配置信息，记录用户所需的对象属性信息。
...
1. [指定源存储桶待分析对象信息](#step1)。
2. [配置清单报告投递信息](#step2)，包括投递的目标存储桶，清单报告格式等。
3. [其他配置项](#step3)。
...
您需要告知 COS 按照何种频率导出清单报告，并将清单报告投递至哪个存储桶中，并决定是否需要对清单报告进行加密，需要配置信息如下：

- 选择清单导出频率：每日或者每周。您可以通过此配置告知 COS 应该按照何种频率执行清单功能。
- 选择清单加密：不加密或者 SSE-COS。如您选择了 SSE-COS 加密，则我们将会在投递时对生成的清单报告进行加密。
- 配置清单的输出位置：您需要指定清单报告需要投递的存储桶。

> !目标存储桶必须和源存储桶位于同一地域，可以和源存储桶相同。

<span id="step3"></span>
### 其他配置项

为了保证 COS 能将清单报告投递到目标存储桶中，您需要为目标存储桶添加存储桶策略（Bucket Policy），必须在目标存储桶上创建存储桶策略，授予对象存储将对象写入（PUT）目标存储桶的权限。
相关策略如下：

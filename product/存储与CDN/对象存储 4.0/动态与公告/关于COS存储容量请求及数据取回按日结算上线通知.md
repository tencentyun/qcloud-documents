...
| ------------ | -------- | ---------------------------------------- | -------------------------------- |
| 存储容量     | 元/GB/月 | 存储容量中“元/GB/月”中的“月”指的是 30 天，与计费周期有关。按日结算价格需要换算，换算公式为：日单价 = 月单价/30。 | 比如：北京地域的标准存储容量按月单价为0.118 元/GB/月，则按日单价为 0.118 / 30 = 0.00393333 元/GB/日。 |
...
>因此，在每个月用量不变的情况下，存储容量当月总费账单用会产生波动。比如，在2月会按照28或29天计费，在4月会按照30天计费，在3月会按照31天计费；请求、数据取回当月总费用账单不会产生波动。

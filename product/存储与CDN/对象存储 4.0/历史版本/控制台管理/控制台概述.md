...
Object是腾讯云 COS 中存储的具体内容，使用 key-value 存储方式，专门用于存储任意多的数据。Object 必须存储在一个或多个存储桶中，由以下基本内容组成：

- 键（Key）：Object 名称。在 Bucket 中唯一标识一个 Object，每个  Bucket 都支持无限个数的 Object。


- 值（Value）：根据您指定的 Key 可将本地任意类型的文件上传至 Bucket 中。单文件上传最大支持 64 GB，单文件存储最大支持 64 GB，。


- 元数据（Metadata）：一组键值对，可用于存储有关 Object 的信息。

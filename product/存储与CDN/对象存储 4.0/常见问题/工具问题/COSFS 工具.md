...
### 在系统日志 /var/log/messages 日志中，看到有记录大量 404 错误码，正常吗？
cosfs 内部逻辑中，会 Head 请求去判断父目录和文件是否存在，出现 404 并不意味着程序运行出错。

### 为什么服务器上看到的文件大小为 0 ？
一般，往 cosfs 中写入数据时，会在服务器上创建一个0大小的文件，并且将数据写入本地缓存文件，当关闭本地文件时，才会把缓存文件中的数据上传到服务器中。

...
### 为什么 COS 上看到的文件大小为0 ？
一般情况下，往 COSFS 中写入数据时，会首先在 COS 上创建一个0大小的文件，并将数据写入本地缓存文件；在写入的过程中，该挂载点 ls 的结果会显示文件大小的变化；当关闭文件时，COSFS 会把写入到本地缓存文件中的数据上传到 COS 中。如果上传失败，您可能只能看到一个0大小的文件，该情况下可尝试重新拷贝失败的文件。

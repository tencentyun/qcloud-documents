...
A：现网4.0版本接口即2016年9月之后，用户接入COS使用的API，上传域名为[Region].file.myqcloud.com。现网4.0版本接口日后将保持维护状态，可以正常使用但是不发展新特性。其与标准XML的API 底层架构相同，数据互通，可以交叉使用，但是接口不兼容，域名不一致。

**Q：XML API与3.0的JSON格式API有什么关系？**

A：现网3.0版本接口即2016年10月之前，用户接入COS使用的API，上传域名为web.file.myqcloud.com。对于使用现网3.0版本接口的用户，COS将主动帮助用户缓慢迁移数据，转发请求，客户无需变更。其与标准XML的API 底层架构不同，数据不互通，接口不兼容。

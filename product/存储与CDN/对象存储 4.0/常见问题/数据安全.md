...
最长2小时，即7200秒。有关临时密钥的介绍请参阅 [临时密钥生成及使用指引](https://cloud.tencent.com/document/product/436/14048)。

### 密钥相关信息如 APPID、SecretId 等信息泄露了，如何处理？

用户可删除已泄露的密钥，并新建一个密钥。详情请参阅 [密钥管理](https://cloud.tencent.com/document/product/436/10074)。

### COS 支持文件加密吗？

支持在服务端进行文件加密。详情请参阅 [服务端加密](https://cloud.tencent.com/document/product/436/18145)。
...
请求量大不会影响，但频率过快会影响。详情请参阅 [请求速率与性能优化](https://cloud.tencent.com/document/product/436/13653)。
...
2. 确保上述信息正确的前提下，请检查是否使用子账号操作，若使用子账号请检查主账号是否已对子账号授权。否则，请先登录主账号对子账号授权。
   授权详情请参阅 [访问管理权限设置相关案例](https://cloud.tencent.com/document/product/436/12514)。
...
### 使用默认域名访问公有读私有写的存储桶时，会返回该存储桶的全部文件信息，如何隐藏文件列表信息？
...
>
>!请将"qcs::cos:ap-beijing:uid/125000000:testbucket-125000000/*"中的相关信息进行以下替换：
>- “ap-beijing”替换为您的存储桶所在地域；
>- “125000000”替换为您的 APPID 信息；
>- “testbucket-125000000”替换为您的存储桶名称。
>
>其中，APPID 为存储桶名称的后半部分，您可以在 [对象存储控制台](https://console.cloud.tencent.com/cos5/bucket) 查看存储桶名称。

### 如何授权协作者访问指定存储桶？

协作者账号是一类特殊的子账号，详情请参阅 [访问策略语言概述](https://cloud.tencent.com/document/product/436/18023)。
...
COS 的 ACL + Policy 数量限制是1000条，当设置的 相关 ACL 或 Policy 策略大于1000条时，会出现此报错，因此建议删除无用的 ACL 或 Policy 策略。
...
详情请参阅 [授权子账号访问 COS ](https://cloud.tencent.com/document/product/436/11714)，创建子账号并对其授权。

### 如何对私有读写的文件生成具有时效性的访问链接？

详情请参阅 [临时密钥生成及使用指引](https://cloud.tencent.com/document/product/436/14048) 文档，设定密钥有效时间。
...
若希望子账号只有特定存储桶的操作权限，可以使用子账号添加路径。详情请参阅 [访问存储桶列表](https://cloud.tencent.com/document/product/436/17061)。

### 如何使用 A 账号对 B 账号授权 A 账号下存储桶的写权限？

详情请参阅 [ACL 访问控制实践](https://cloud.tencent.com/document/product/436/12470) 和 [CAM 访问管理实践](https://cloud.tencent.com/document/product/436/12469) 进行授权。

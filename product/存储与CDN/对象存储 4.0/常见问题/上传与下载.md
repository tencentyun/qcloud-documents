...
中国大陆公有云地域默认带宽为15Gbit/s、其他地域为10Gbit/s。如果带宽达到该阈值，请求会触发流控。如果有更高带宽需求，请通过 [在线支持](https://cloud.tencent.com/online-service) 或 [提交工单](https://console.cloud.tencent.com/workorder/category) 联系我们。
...
>!腾讯云黑石机器内网 IP 地址和 CVM 的 IP 地址存在差异，一般形如`9.*.*.*`或`10.*.*.*`，如您存在疑问可通过 [在线支持](https://cloud.tencent.com/online-service) 或 [提交工单](https://console.cloud.tencent.com/workorder/category) 咨询。

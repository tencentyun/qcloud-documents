想要充分利用 COS，您需要了解一些相关的基本概念和术语。以下概念和术语是按照您最有可能遇到它们的顺序排列的。
### APPID
APPID 是腾讯云账户的账户标识，用于关联云资源。在用户成功申请腾讯云账号后，系统自动为用户分配一个 APPID，一个APPID下可以创建多个项目。
### 项目
项目为一个虚拟概念，用户可以在一个账户（APPID）下面建立多个项目，每个项目中管理不同的资源。用户可以从控制台访问 [项目管理](https://console.qcloud.com/project)。
### SecretId 

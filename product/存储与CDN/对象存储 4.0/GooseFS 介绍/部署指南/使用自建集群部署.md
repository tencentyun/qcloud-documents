...
例如，可以挂载一个 COS 路径为 GooseFS 根路径：goosefs.master.mount.table.root.ufs=cosn://bucket-1250000000/goosefs/。
...
上述配置选项中，`goosefs.master.hostname` 配置每个 Master 对外服务的 ip 或 hostname，并且确保客户端和 Worker 节点都能够访问到。`goosefs.master.mount.table.root.ufs` 则设置为挂载到 GooseFS 根目录的底层存储URI。而 `goosefs.master.embedded.journal.address` 则设置为所有备节点的 `ip:embedded_journal_port` 或 `host:embedded_journal_port`。其中，embedded_journal_port 默认为9202。
...
- Zookeeper集群，GooseFS Master 使用 Zookeeper 进行 Leader 选取，GooseFS 的客户端和 Worker 节点则通过 Zookeeper 查询主 Master 节点；
...
然后，使用 `./bin/goosefs copyDir conf/` 来将配置同步分发到集群中的所有节点；

最后，启动集群 `./bin/goosefs-start.sh all` 即可。

## 腾讯云 EMR 环境部署

目前，GooseFS 已经集成到了腾讯云 EMR 环境中，将会在最新的 EMR 版本中发布。届时，用户无需针对腾讯云 EMR 环境做单独部署，直接可以像使用其他 EMR 组件一样使用 GooseFS。

下文将针对，未集成 GooseFS 的腾讯云 EMR 存量集群介绍如何部署配置 GooseFS 的 EMR 环境。

首先，参照集群模式部署运行章节的内容，选择生产环境合适的部署架构，完成集群部署；

然后，需要针对 EMR 支持组件来配置，本文以 Hadoop MapReduce、Spark 以及 Flink 对 GooseFS 的支持来讲解。

### Hadoop MapReduce 支持

为了使得 Hadoop 的 MapReduce 作业能够读写GooseFS中的数据，需要在hadoop-env.sh中将GooseFS Client的依赖路径添加到HADOOP_CLASSPATH，这个操作可以在EMR的控制台上完成：

![截屏2021-04-27 下午1 30 04](https://user-images.githubusercontent.com/4209464/123776817-39558500-d902-11eb-849c-215a3f2593b2.png)

同时，还需要配置在 core-site.xml 中配置 GooseFS 的HCFS实现，同样这个操作也可以在 EMR 的控制台上完成：

![截屏2021-04-27 下午2 21 28](https://user-images.githubusercontent.com/4209464/123777065-6d30aa80-d902-11eb-9c30-56db4f415c94.png)

![截屏2021-04-27 下午2 22 19](https://user-images.githubusercontent.com/4209464/123777188-85082e80-d902-11eb-8c57-dbb113ce47f8.png)
...
![截屏2021-04-27 下午3 33 18](https://user-images.githubusercontent.com/4209464/123777623-f811a500-d902-11eb-97c6-1f1d2c626b8b.png)

### Flink 支持

腾讯云EMR的flink采用的是 Flink on YARN 的部署模式，因此原则上只要确保 ${FLINK_HOME}/flink-conf.yaml 中正确设置 `fs.hdfs.hadoopconf` 到 hadoop 的配置路径下即可，腾讯云EMR集群中一般为/usr/local/service/hadoop/etc/hadoop。

其他无需设置，直接使用 Flink on YARN 的方式提交 Flink 作业即可，作业中需要访问 GooseFS 的路径为 gfs://master:port/<path>.  注意：flink 访问 goosefs 时必须指定master和port。

### Hive、Impala、HBase、Sqoop 以及 Oozie 支持

当配置 Hadoop MapReduce 的环境支持以后，Hive、Impala、HBase等组件无需单独配置支持，即可正常使用。
...
当时执行 goosefs-start.sh ALL 脚本启动 GooseFS 以后，集群中包含如下进程：
  - GooseFSMaster：默认RPC端口号为9200，Web端口为9201
  - GooseFSWorker：默认RPC端口号为9203，Web端口为9204
  - GooseFSJobMaster：默认RPC端口为9205，Web端口为9206
  - GooseFSProxy：默认Web端口为9211
  - GooseFSJobWorker：默认RPC端口为9208，Web端口为9210

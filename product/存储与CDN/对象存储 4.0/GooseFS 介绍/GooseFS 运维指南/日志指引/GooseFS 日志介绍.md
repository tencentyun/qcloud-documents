...
>!
- .log 文件是滚动存储的，您可以将其备份到 UFS，例如对象存储 COS 中；.out 文件不滚动存储，如果需要清理 .out 文件需要手动发起删除操作。
...
GooseFS 支持配置 GC 事件日志，FUSE 接口日志，RPC 调用日志，UFS 操作日志，以及进行日志分割和日志筛选等操作。以下会介绍部分常用高级配置的使用方式。

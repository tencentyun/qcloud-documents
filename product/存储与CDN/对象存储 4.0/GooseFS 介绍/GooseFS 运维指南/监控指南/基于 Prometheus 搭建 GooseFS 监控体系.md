...
```
2. master 和 worker 的 Prometheus 的监控指标可用如下的命令查看：
```plaintext
...
```
2. 打开登录页面 http://134.xxx.xx.247:3000 ，username 和 password 都是 admin，首次登录后修改密码。
3. 进入页面后，添加 Prometheus 的 Datasource：
```plaintext
localhost:9101
```
4. 导入 Goosefs 的 Grafana 模板，选择 json 导入（[点此下载 json](https://iwiki.woa.com/download/attachments/585423153/goosefs-prometheus-grafana-monitor-cache-hits.json?version=3&modificationDate=1618327767333&api=v2)），并选择上面创建的 Datasource。

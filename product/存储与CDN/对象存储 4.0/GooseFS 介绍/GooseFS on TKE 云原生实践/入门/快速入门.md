...
i. 创建一个 resource.yaml 文件，里面包含如下两部分：
- 包含数据集及 ufs 的 dataset 信息，并创建一个 Dataset CRD 对象，其中描述了数据集的来源，例如示例中的 test-bucket。
...
以上通过一个简单的例子完成 GooseFS on Fluid 的入门体验和学习，并最后进行环境的清理，更多 Fluid GooseFSRuntime 的功能介绍可参见 [功能列表](https://cloud.tencent.com/document/product/436/59501)。

...
### 设置步骤
#### 默认 CDN 加速
1. 单击存储桶详情界面上方的【域名管理】，单击【编辑】，把默认加速域名的当前状态设置为开启。源站类型通常默认为 XML 节点，如果作为源站的存储桶开启了静态网站，并且希望为静态网站加速，则选择为静态网站节点，如下图所示：
...
#### 开启鉴权
1. 当存储桶为私有读时，需要开启**回源鉴权**（在已添加 CDN 服务授权前提下）。登录 [对象存储控制台](https://console.cloud.tencent.com/cos5) ，进入左侧菜单栏【存储桶列表】，单击需要配置域名的存储桶（如 example），进入存储桶配置界面，如下图所示： 
![](https://main.qcloudimg.com/raw/688c1dc518cb417461bc9d4645e4c41c.png)
2. 单击存储桶详情界面上方的【域名管理】，单击【编辑】，选择开启【回源鉴权】，并单击【保存】即可开启 CDN 加速，如下图所示：
...
3. 单击【保存】之后，“默认加速域名”下会出现 CDN 鉴权状态提示，可通过单击页面上【鉴权配置】开始配置，如下图所示：

...
存储桶列表指使用子账号（或协作者账号）访问受限的存储桶列表。子账号默认没有拉取存储桶列表数据的权限，因此使用子账号登录[对象存储控制台](https://console.cloud.tencent.com/cos)，在存储桶列表中无法访问存储桶列表和统计数据（见下示例图），如需访问存储桶列表，可通过添加 **访问路径列表** 或子账号 **获取存储桶列表权限** 的方式，才可进行其他操作。

...
3. 在弹出的创建存储桶对话框中，配置如下信息：
 ...
 - **多 AZ 特性**：多 AZ 特性是存储桶的一个标识，当您打开多 AZ 配置后，您的数据将只能以标准存储（多 AZ）类型存储在同个地域的不同机房中，具有同城容灾的作用。有关多 AZ 配置的描述，可参见 [多 AZ 特性概述](https://cloud.tencent.com/document/product/436/40548) 文档。
 >!存储桶多 AZ 配置开启后，无法进行修改，请谨慎配置；您已创建的存储桶时默认未开启多 AZ 配置，仅新创建存储桶时可设置开启。

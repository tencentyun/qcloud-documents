当前使用归档存储有两种模式：使用归档存储 CAS（Cloud Archive Storage）、通过生命周期沉降使用对象存储COS（Cloud Object Storage）中归档存储 。后续这两种模式将进行归一，我们将不再提供归档存储 CAS，只保留后者。与此同时，我们将在对象存储 COS 中实现直传归档，即在 COS 中直接上传存储类型为归档存储的对象。

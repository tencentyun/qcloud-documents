...
```
> **注意：**
无论是在 Linux 还是 Windows 环境下，都可以通过以上的方法安装或更新
...
```
### 指定 Bucket 的命令
通过`-b <bucket> 指定bucket`可指定特定 Bucket。Bucket 的命名规则为`{name}-{appid}` ，此处填写的存储桶名称必须为此格式。
```
...
> - `GRANT_READ / GRANT_WRITE / GRANT_FULL_CONTORL`代表被赋权的账号。

- 若赋权根帐号，请使用 rootid 的形式；
- 若赋权子账户，请使用 rootid/subid 的形式；
- 若需要对所有人赋权，请使用 anyone 的形式。
- 同时赋权的多个帐号用逗号`,`隔开。

...
jar 包的 [MD5 校验值](https://cos-sdk-archive-1253960454.file.myqcloud.com/cos-distcp/cos-distcp-1.2-md5.txt)
...
COSDistcp 基于 MapReduce 框架实现，在 Mapper 中对文件进行分组，在 Reducer 进程中使用多线程对文件进行拷贝、压缩、数据校验和文件属性保留等工作。当文件迁移或校验失败的时候，任务可能会执行失败。当您的源端有文件新增或文件内容发生变化时，您可通过 skipMode 和 diffMode 模式，通过对比文件的长度或 CRC 校验值，实现文件的增量迁移。
...
|         --skipMode=MODE          | 拷贝文件前，校验源文件和目的文件的 是否相同，相同则跳过，可选 none（不校验）、length （长度）、checksum（CRC值）和 length-checksum（长度 + CRC 值）<br/>示例：--skipMode=length |  none  |   否   |
|         --checkMode=MODE         | 当文件拷贝完成的时候，校验源文件和目的文件是否相同，不同则停止拷贝，可选none（不校验）、 length （长度）、checksum（CRC值）和 length-checksum（长度 + CRC 值）<br/>示例：--checkMode=length-checksum |  none  |   否   |
...
|      --preserveStatus=VALUE      | 指定是否将源文件的 user、group、permission、xattr 和 timestamps 元信息拷贝到目的文件，可选值为 ugpxt（单词 user, group, permission, XATTR, timestamps 的首字母）<br/>示例：--preserveStatus=ugpt | 无 |   否   |
...
```
以上命令中， ${version} 为COSDistcp 版本号，例如 1.0 版本的COSDistcp  jar 包名为 cos-distcp-1.0.jar
...
以参数`--skipMode`执行命令。跳过具有源和目的具有相同长度文件的拷贝：
```plaintext
hadoop jar cos-distcp-${version}.jar  --src /data/warehouse --dest cosn://examplebucket-1250000000/data/warehouse  --skipMode=length
```

`--skipMode`选项在拷贝文件前，校验源文件和目的文件是否相同，相同则跳过，可选 none（不校验）、length （长度）、checksum（CRC值）和 length-checksum（长度 + CRC 值）。如源和目标文件系统的校验和算法不同，则会读取源端文件计算新的校验和。如果您的源是 HDFS，您可以通过如下方式，确定 HDFS 源是否支持 COMPOSITE-CRC32C 校验算法：
...
以参数`--checkMode`执行命令，文件拷贝完成时，校验源文件和目标文校验和是否一致。

从非 COS 文件系统同步到 COS 时，如果源的 CRC  算法和 Hadoop-COS 的 CRC 算法不一致，则拷贝的同时计算 CRC，并在拷贝完成后，获取目的 COS 文件的 CRC，和计算得到的源文件 CRC 对比校验：
...
以参数`--preserveStatus`执行命令，将源文件或源目录的 user、group、permission 和 timestamps（modification time 和 access time）拷贝到目的文件或目的目录，示例如下：
```plaintext

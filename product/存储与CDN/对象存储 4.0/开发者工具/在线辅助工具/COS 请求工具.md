...
COS 请求工具是腾讯云对象存储 COS 为您提供的 Web 端调试工具，集成在云 API3.0平台上，可用于 API 接口的调试工作。您只需在工具页面上输入指定的参数，即可得到请求之后的结果、响应头等相关信息，供您调试、参考。
...
COS 请求工具的整体页面，从左至右依次是产品栏，接口栏，参数栏和结果栏；您可以在不同的栏目执行相应的操作，最终在结果栏发送请求并获取响应结果和相关的过程参数信息。
...
> **说明：**
> COS 请求工具集成在云 API3.0平台，该平台上搭载着其他腾讯云产品的 API 调试工具，您同样可以根据您的需要在该平台上选择其他产品并调试相应的接口。

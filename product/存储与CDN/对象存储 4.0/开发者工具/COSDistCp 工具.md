...
COSDistCp 基于 MapReduce 框架实现，在 Mapper 中对文件进行分组，在 Reducer 进程中使用多线程对文件进行拷贝、压缩、数据校验和文件属性保留等工作。COSDistCp 默认会覆盖目标目录中的文件。由于网络及其他因素导致的迁移任务失败，可能只会拷贝部分源文件以及源文件的部分内容，此时，您可通过 skipMode 和 diffMode 模式，通过对比文件的长度或 CRC 校验值，实现文件的增量迁移和覆盖。

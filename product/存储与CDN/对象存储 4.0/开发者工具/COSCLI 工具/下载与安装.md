...
COSCLI 提供 Windows、Mac、Linux 操作系统的二进制包，通过简单的安装和配置后即可使用。
...
1. [下载 Windows 版本 COSCLI](https://github.com/tencentyun/coscli/releases/download/v0.9-beta/coscli-windows.exe)。
2. 将已下载的 COSCLI 可执行文件移动至`C:\Users\<用户名>`目录下。
3. 按`win+r`键打开`运行`程序。
4. 在对话框中，输入`cmd`，按 `Enter` 打开命令行窗口。
4. 在命令行窗口中，输入`coscli --help`，若打印出以下信息则代表安装成功：
...
1. 运行以下命令下载 COSCLI：
```bash
wget https://github.com/tencentyun/coscli/releases/download/v0.9-beta/coscli-mac
```
2. 运行以下命令修改文件执行权限：
```bash
chmod 755 coscli
...
1. 单击 [下载 Linux 版本 COSCLI](https://github.com/tencentyun/coscli/releases/download/v0.9-beta/coscli-linux)，或运行以下命令下载 COSCLI：
```bash
wget https://github.com/tencentyun/coscli/releases/download/v0.9-beta/coscli-linux
```
2. 运行以下命令修改文件执行权限：
```bash
chmod 755 coscli
...
>? 更多有关配置文件的操作，请参考 [config 命令](#config)。

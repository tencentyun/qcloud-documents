> 注意：为方便用户使用，迁移工具升级为 [COS Migration 工具](https://cloud.tencent.com/document/product/436/15392)，原迁移工具即将下线，对您造成的不便我们深表歉意！
...
## 常见问题
1. pip 命令不存在。 
使用 `apt install python-pip` 或者 `yum install python-pip` 命令安装 pip。
2. 使用 pip 安装迁移工具不成功。

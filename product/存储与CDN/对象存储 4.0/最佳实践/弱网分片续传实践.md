...
本文主要介绍基于腾讯云对象存储（COS）服务，移动端在弱网情况下，如何根据实际情况动态调节上传分片大小，来提高上传成功率。

>! 该功能目前处于灰度测试阶段，如需支持该功能，请 [提交工单](https://console.cloud.tencent.com/workorder/category) 申请开通白名单。

## 实践须知

1. COS 目前支持最小分片大小为100KB。
2. 分片越小，意味着请求数也多。目前根据测试统计，100KB分片大小的请求耗时是1MB分片大小的2倍左右。
3. 在正常网络下发起请求，建议设置分片大小为1MB及以上。
4. 在网络信号弱等网络环境下上传对象容易导致上传失败，可以考虑将分片大小设置为100KB，再重新上传。
5. 分片大小设置越小，将来下载该文件时可能比较缓慢。
    ...
    public void upload(final String srcPath, final String cosPath, final String uploadId, long sliceSize){
        ...
        uploadTask.setCosXmlResultListener(new CosXmlResultListener() {
            ...
            public void onFail(CosXmlRequest request, CosXmlClientException exception, CosXmlServiceException serviceException) {
                ...
                if(exception != null){
                    /** 若是因为网络导致失败， 则可尝试将分片大小设置为100KB再运行一遍*/
...
```objective-c
    ...
    [put setFinishBlock:^(QCloudUploadObjectResult *result, NSError *error) {
        /**
         ...
         2. 根据需要通过 QCloudCOSXMLUploadObjectRequest 的 sliceSize 重新设置分片大小
         3. 开始上传
         */
        if (error.code == -1001) {

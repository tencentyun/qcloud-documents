...
对于使用第三方云平台存储的用户，对象存储 COS 支持以下迁移方式，帮助用户将第三方云平台的存储数据快速迁移至对象存储 COS。

- 线上迁移：[COS Migration](#cos)、[迁移服务平台 MSP](#msp)。

这两种迁移方式都支持查看数据迁移进度、文件一致性校验、失败重传、断点续传等功能，能够满足用户数据基本的迁移需求。但这两种迁移方式在交互形式和功能特点等方面又有所差别，如下表所示。用户可根据以下差异对比，选择最适合的一种方式进行数据迁移。
...
- 在配置文件 config.ini 中 [common] 分节，修改 smallFileThreshold 小文件阈值参数，大于等于这个阈值使用分块上传，默认设置为5MB。
- 分别修改大文件并发度 bigFileExecutorNum 和小文件并发度 smallFileExecutorNum ，如果是通过外网来连接 COS，且带宽较小，请减小该并发度。小文件与大文件的并发度分别默认为64和8。

2. 调整工具执行时间和设立带宽限制，保证自身业务运行不受迁移数据带宽占用影响。

- 在配置文件 config.ini 中 [common] 分节，修改参数 executeTimeWindow，该参数定义迁移工具每天执行的时间段，其他时间则会进入休眠状态，休眠状态暂停迁移并会保留迁移进度，直到下一个时间窗口自动继续执行。

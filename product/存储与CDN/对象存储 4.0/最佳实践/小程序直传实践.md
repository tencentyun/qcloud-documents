...
后缀式请求格式需要注意，签名时使用的路径要用以存储桶名称的作为前缀的路径，例如`/examplebucket-1250000000/`。
...
var uploadFile = function () {
    ...
    if (ForcePathStyle) {
        prefix = 'https://cos.' + Region + '.myqcloud.com/' + Bucket + '/'; // 后缀式请求在签名时域名使用地域域名，而不是存储桶域名，具体说明见本文上述“3.后缀式请求”
    }
    ...
    var uploadFile = function (filePath) {
        ...
        if (ForcePathStyle) {
            signPathname = '/' + Bucket + '/';  // 后缀式请求在签名时用的路径，要包含存储桶名称，具体说明见本文上述“3.后缀式请求”
        }

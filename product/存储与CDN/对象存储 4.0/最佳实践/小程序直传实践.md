...
COS API 一般的请求格式都类似 `POST http://examplebucket-1250000000.cos.ap-beijing.myqcloud.com/`，请求的域名是存储桶域名。这样如果在小程序里用到多个存储桶，就要配置这个存储桶域名作为白名单域名。解决方法如下：
...
后缀式请求格式需要注意，签名时用的路径要用以存储桶名称的作为前缀的路径。比如 `/examplebucket-1250000000/`
...
var uploadFile = function () {
    ...
    if (ForcePathStyle) {
        prefix = 'https://cos.' + Region + '.myqcloud.com/' + Bucket + '/'; // 后缀式请求在签名时域名使用地域域名，而不是存储桶域名，具体看上文“后缀式请求”
    }
    ...
    var uploadFile = function (filePath) {
        ...
        if (ForcePathStyle) {
            signPathname = '/' + Bucket + '/'; // 后缀式请求在签名时用的路径，要包含存储桶名称，具体看上文“后缀式请求”
        }

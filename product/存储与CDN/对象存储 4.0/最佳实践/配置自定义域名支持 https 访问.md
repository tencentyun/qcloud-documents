...
- 开启 CDN 加速，指引参考 [ HTTPS 配置](/document/product/228/6295)。
- 关闭 CDN 加速，指引参考 
## 配置自定义域名支持 HTTPS 访问
本章节主要以示例的形式介绍在 COS 中通过反向代理配置自定义域名支持 https 访问的操作步骤。本示例将实现直接通过自定义域名`https://test.cos.com`访问用户 APPID 为 12345678 、所属地域为华南、名称为 testhttps 的存储桶（不开启 CDN 加速），具体操作步骤如下：
...
```
3. 设置完成后，直接打开文件会报错：未命中 refer 白名单。
![图片3](//mc.qcloudimg.com/static/img/005099e6a30398c600bb945b6b1c34e7/image.png)
但是通过自定义域名走代理访问，可以正常打开网页：
![图片4](//mc.qcloudimg.com/static/img/92a5f605f73a971e9c7188eeb93e398c/image.png)

...
- 如果用户需要检验上传到 COS 的对象与本地对象是否一致，可以在上传时将 HTTP 请求的 [Content-MD5](https://cloud.tencent.com/document/product/436/7728) 字段设置为经过 Base64 编码的对象内容 MD5 校验值，此时 COS 服务器会检验用户上传的对象，只有当 COS 服务器接收到的对象 MD5 校验值与用户设置的 Content-MD5 一致时，对象才能上传成功。

...
3. 如果使用 PutObject 接口上传文件，将 sessionToken 和计算得到的签名 authorization， 分别放到发请求时 header 的 authorization 和 x-cos-security-token 字段里。
如果使用 PostObject 接口上传文件，则将 sessionToken 和计算得到的签名 authorization，分别放到发请求时表单的 Signature 和 x-cos-security-token 字段里。

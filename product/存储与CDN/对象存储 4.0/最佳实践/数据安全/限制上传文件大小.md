对象存储（Cloud Object Storage，COS）可以在上传对象时，对上传的文件类型、大小进行限制。限制上传的文件类型，可以保证您的存储桶中不会有不符合预期的文件；限制上传的文件大小，可以方便您更加灵活管理存储空间，避免上传过大、过小文件浪费存储空间与网络带宽。下面列举了两个案例介绍如何精细化地控制上传文件的大小。
...
在实际使用中，请替换为您自己的存储桶，并且使用有操作存储桶权限的账号密钥操作存储桶。
...
可以通过存储桶策略中指定文件上传类型，存储桶策略语法示例可以参考文档 [存储桶策略示例](https://cloud.tencent.com/document/product/436/18031)。
  ...
  "Statement": [
    {
      ...
      "Resource": [
        "qcs::cos:ap-nanjing:uid/1259733965:aidata-1259733965/*"
      ],
      "Condition": {

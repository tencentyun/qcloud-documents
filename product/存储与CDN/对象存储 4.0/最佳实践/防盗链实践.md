...
腾讯云对象存储支持防盗链配置，建议您通过控制台的防盗链设置黑/白名单，来进行安全防护。本文为您详细介绍如何在对象存储配置防盗链。

<span id="fenxi"></span>

## 盗链案例分析

- 用户 A 在 COS 上传了图片资源`1.jpg`，得到可访问链接`https://examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/1.jpg`，并且在他自己的网页`https://a.com/a.html`嵌入了该图片，能正常访问。
- 用户 B 在`a.com`上看到了图片，也在他自己的网页`https://b.com/b.html`嵌入了`1.jpg`的链接，B 的网页也能正常显示图片。
...
例如，下图是在`https://127.0.0.1/test/test.html`嵌入了`1.jpg`，访问`test.html`时就带有 Referer 指向访问来源：
...
我们使用上文中的 [盗链案例分析](#fenxi) 来举例，介绍用户 A 如何通过防盗链设置防止用户 B 盗链图片：
...
2. 开启了防盗链配置之后：
 - 访问`https://a.com/a.html` 图片显示正常。
 - 访问`https://b.com/b.html`  图片无法显示，表现如下图。

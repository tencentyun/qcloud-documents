...
在添加服务授权弹窗中，单击【确定】。
![](https://main.qcloudimg.com/raw/3c80c1de65447704db0551b7e3ad7248.png)

4）请稍等几分钟，等待域名部署上线完成后。然后复制对应的 CNAME 记录，再进行 [步骤 3.2](#步骤3.2)。
...
- `http://www.example.com/test.html`（不存在的文件） ——返回404提示。如果您需要自定义错误文档，可在 [步骤2.2](#step2.2) 配置静态网站中添加设置【错误文档】，当访问不存在的文件时将显示该指定的错误文档。

> ?
> - 在某些情况下，您可能需要清除浏览器缓存才能看到预期结果。
> - 错误文档可以放在子目录下，如 pages/404.html，但不支持针对不同的子目录配置不同的错误文档，每个存储桶只能配置一个错误文档。

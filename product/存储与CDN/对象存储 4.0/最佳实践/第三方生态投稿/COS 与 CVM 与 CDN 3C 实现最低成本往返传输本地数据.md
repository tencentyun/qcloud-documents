...
- 如果是按流量计费的带宽，以成都区为例，每 G 0.8元的费用，若每天300G 则每天为240元，费用较高。
...
### 生态
- COS，CDN，CVM 统一使用腾讯云的产品，在同一个账户下控制台可以直接管理，免去了切换平台切换账号之间的麻烦。
...
>?相较于 COS 在欧洲地区的0.5-0.9元/GB，CDN 只需要0.31元/GB，每 GB 省下了最多0.5元，成本直接减半。上例中300G 数据通过 COS 直接下载的价格为150元，而通过 CDN 只需要93元。
...
通过上述数据综合得出，CDN 的费用是最低最划算的，单价在所列举的产品中均为最低单价。
...
若系统为 Windows，建议使用浏览器或 COSCMD 工具访问对象。

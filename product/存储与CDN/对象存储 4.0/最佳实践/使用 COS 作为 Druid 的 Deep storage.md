...
下载 HADOOP-COS 后，将 dep 目录下对应需要的版本`hadoop-cos-2.x.x.jar`拷贝到 Druid 安装路径`extensions/druid-hdfs-storage`以及`hadoop-dependencies/hadoop-client/2.x.x`下。因为Druid访问COS利用HDFS的plugin，所以版本需要跟druid里面HDFS的plugin的版本一致。

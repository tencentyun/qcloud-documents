...
### 策略示例
该样例描述为：允许属于主账号 ID 为100000000001（APPID 为1250000000）下的子账号 ID 100000000011以及组 ID 18825， 对北京地域的存储桶 examplebucket-bj 和广州地域的存储桶 examplebucket-gz 下的对象 exampleobject，在访问 IP 为 `10.*.*.10/24` 网段时，拥有所有 COS **读 API** 的权限以及**写对象**的权限，以及可以发送消息队列的权限。

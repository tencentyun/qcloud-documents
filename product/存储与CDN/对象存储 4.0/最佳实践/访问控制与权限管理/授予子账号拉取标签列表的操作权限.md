...
（3）您可以根据业务需要勾选相应的操作（Action），授予子账号 GetResourceTags、GetResourcesByTags 和 GetTags 操作权限，如下图所示。
![](https://main.qcloudimg.com/raw/7d4580cc9b99df4f1ca351283b4f3618.jpg)
（4）根据实际环境填写资源（Resource），详情请参见 [资源描述方式](https://cloud.tencent.com/document/product/598/10606)。
（5）单击【添加声明】，您可在下方查看已添加的策略配置。
（6）单击【下一步】，输入策略名称。
（7）单击【创建策略】，即可创建策略。
 - **通过策略语法**
（1）进入 [访问管理](https://console.cloud.tencent.com/cam/policy) 策略配置页面。
...
3. 子账号登录控制台，在 [存储桶列表](https://console.cloud.tencent.com/cos5/bucket) 页面，选择**标签**并输入**标签键**进行搜索，即可查询到带有此相同标签的存储桶列表，如下图所示。

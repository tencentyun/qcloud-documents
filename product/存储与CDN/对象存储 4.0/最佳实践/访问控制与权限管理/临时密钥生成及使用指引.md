...
```
{
    ...
    'statement': [{
        ...
        'resource': [
            ...
            'qcs::cos:ap-guangzhou:uid/1250000000:test-1250000000/allowDir/*',
        ]
    }]
}
```
...
这个场景在移动应用中非常常见，数据允许创建自己的用户访问，而其他用户无法访问。例如一个相册应用允许用户管理自己上传的图片，而其他用户没有访问权限。您可以通过以下设置，达到用户级别的数据安全。

第一步，将数据设置为私有写。

第二步，数据按照用户存储。文件在上传时，将用户唯一 ID 作为文件路径前缀的一部分。举例：假如用户A唯一标识是 userID123456，那么他上传的文件 file1 存放在 pictures/userID123456/file1。

第三步，按照用户 ID 来设置 policy 的资源路径前缀。举例：假如您的存储桶地域是上海 ap-shanghai，您的腾讯云 APPID 是 12345678，用户A唯一标识是 userID123456，当他访问时，Policy 设置为：
...
public class Demo {
    public static void main(String[] args) {
        ...
        String policy = "{\"statement\": [{\"action\": [\"name/cos:GetObject\",\"name/cos:PutObject\"],\"effect\": \"allow\",\"resource\":[\"qcs::cos:ap-beijing:uid/1250000000:sevenyou-1250000000/*\"]}],\"version\": \"2.0\"}";
...
"statement": [
    {
        ...
        "effect": "allow",
        "resource": [
            "qcs::cos:cn-east:uid/1250000000:sevenyou-1250000000/*"
        ]
    }
],
"version": "2.0"

...
>! 目前已支持根据当前 PHP 版本下载 guzzle6.x 或 guzzle7.x。guzzle7.x 版本支持 laravel8 框架。当本地 PHP 版本 >= 7.2.5时自动下载 guzzle7.x 版本，反之下载 guzzle6.x 版本。若有问题，也可通过 Composer 方式`composer require --dev qcloud/cos-sdk-v5:dev-guzzle7` 安装 guzzle7 对应分支应用。
>
...
>! `Source code`压缩包为 Github 默认打包的代码包，里面不包含`vendor`目录。请注意下载release包（cos-sdk-v5.tar.gz包）而不是Source包，也不要直接clone整个仓库。
>

...
上传对象到指定的存储桶中（PUT Object），最大支持 5G 大小的文件，如果需要上传超过 5G 的文件，建议使用高级接口中的复合上传接口。
...
| Bucket | String | 存储桶名称，格式：BucketName-APPID                         | 无 |
| Key | String | 对象键（Key）是对象在存储桶中的唯一标识。例如，在对象的访问域名 examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg 中，对象键为 doc/pic.jpg    | 无 |
| CopySource | String | 描述拷贝源文件的路径，包含 Appid、Bucket、Key、Region，例如，examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg | 无 |
...
| Key      | String | 对象键（Key）是对象在存储桶中的唯一标识。例如，在对象的访问域名 examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg 中，对象键为 doc/pic.jpg |    Object    |
...
| UploadId | String | 对象的分块上传ID | Upload |
...
| ID          | String | 权限所有者ID                                                 | Grantee / Owner |

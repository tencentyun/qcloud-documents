...
try {
    $result = $cosClient->copyObject(array(
        'Bucket' => 'examplebucket-1250000000', //格式：BucketName-APPID
        'Key' => 'exampleobject',
        'CopySource' => 'examplebucket2-1250000000.cos.ap-guangzhou.myqcloud.com/exampleobject?versionId=MTg0NDUxNzYzMDc0NDMzNDExOTc',
    )); 
    // 请求成功
    print_r($result);
} catch (\Exception $e) {
...
##### 示例二：上传归档对象
[//]: # (.cssg-snippet-transfer-upload-object)```php
try {
    $result = $cosClient->Upload(
        $bucket = 'examplebucket-1250000000', //格式：BucketName-APPID
        $key = 'exampleobject',

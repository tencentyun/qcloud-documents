...
try {
    $result = $cosClient->uploadPart(array(
        ...
        'Key' => 'exampleobject', 
        'Body' => 'string',
        'UploadId' => 'NWNhNDY0YzFfMmZiNTM1MGFfNTM2YV8xYjliMTg', //UploadId为对象分块上传的Id，在分块上传初始化的返回参数里获得 
        'PartNumber' => integer, //PartNumber为分块的序列号，cos会根据携带序列号合并分块
        /*
        'ContentMD5' => 'string',
        'ContentLength' => integer,
        */
...
| Bucket | String | 存储桶名称，格式：BucketName-APPID                         | 是 |
| Key | String | 对象键 | 是 |
| UploadId | String |  UploadId为对象分块上传的Id，在分块上传初始化的返回参数里获得 | 是 |
| Body | File/String | 上传的内容 | 是 |
| PartNumber | Int | PartNumber为分块的序列号，cos会根据携带序列号合并分块 | 是 |
| ContentLength | Int | 设置传输长度 | 否 |
| ContentMD5 | String | 设置上传文件的 MD5 值用于校验 | 否 |

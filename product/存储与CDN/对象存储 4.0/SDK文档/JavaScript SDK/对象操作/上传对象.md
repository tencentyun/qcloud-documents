...
| - VersionId  | 在开启过版本控制的存储桶中上传对象返回对象的版本 ID，存储桶从未开启则不返回该参数。需要Expose-Headers里设置VersionId字段，[参考文档]（https://cloud.tencent.com/document/product/436/13318） | String |
...
#### 一般不需要关注这些方法，sliceUploadFile已经封装了以下分块操作，直接调用即可。更推荐使用高级上传uploadFile方法。

...
dependencies {
	...
    // 增加这行
    compile 'com.tencent.qcloud:cosxml:5.4.18'
}
```
...
dependencies {
	...
    // 增加这行
    compile 'com.tencent.qcloud:cosxml-lite:5.4.18'
}
```

4、因在 SDK 中引入了腾讯移动分析（mta）和 SDK 本地日志显示功能，若是想关闭该功能，则在应用的根目录下的 build.gradle 中添加依赖：
...
dependencies {
    ...
    // 增加这行
   compile ('com.tencent.qcloud:cosxml:5.4.18'){
        exclude group:'com.tencent.qcloud', module: 'logUtils' // 关闭sdk本地日志显示功能
...
dependencies {
	...
    // 增加这行
    compile ('com.tencent.qcloud:cosxml-lite:5.4.17'){
        exclude group:'com.tencent.qcloud', module: 'logUtils' // 关闭sdk本地日志显示功能
        exclude group:'com.tencent.qcloud', module: 'mtaUtils' //关闭mta上报功能
    }
}
...
在终端直接通过永久密钥来进行身份认证会存在泄漏密钥的风险，COS 终端 SDK（Android/IOS）均支持通过临时密钥来授权请求，您只需要搭建一个返回临时密钥的服务，即可给终端 COS 请求进行授权，我们强烈建议您使用这种方式，具体请参考 [快速搭建移动应用直传服务](https://cloud.tencent.com/document/product/436/9068)。

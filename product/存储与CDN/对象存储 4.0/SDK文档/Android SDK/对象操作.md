...
// 如果是第一次调用，您不用设置 marker 参数，COS 会从头开始的列出对象，
// 如果想列出下一页对象，则需要将 marker 设置为上次列出对象时返回的 
// GetBucketResult.listBucket.nextMarker 值，如果返回的
// GetBucketResult.listBucket.isTruncated 为 false，则说明
// 您已经列出了所有满足条件的对象
...
do {
    ...
    // commonPrefixs 表示表示被 delimiter 截断的路径, 如 delimter 设置为/, commonPrefixs 则表示子目录的路径

...
try
{
	...
	PostObjectRequest request = new PostObjectRequest(bucket, cosPath, srcPath);
	//设置签名有效时长
...
String sourceCosPath = "exampleobject"; //源对象键
//构造源对象属性
...
String bucket = "examplebucket-1250000000"; //存储桶，格式：BucketName-APPID
String cosPath = "copy_exampleobject"; //对象在存储桶中的位置，即称对象键.
...
try
{
	...
	OptionObjectRequest request = new OptionObjectRequest(bucket, cosPath, origin, accessMthod);
	//设置签名有效时长
	request.SetSign(TimeUtils.GetCurrentTime(TimeUnit.SECONDS), 600);
	//执行请求

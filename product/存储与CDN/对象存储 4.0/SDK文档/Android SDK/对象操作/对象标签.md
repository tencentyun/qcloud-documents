...
本文档提供关于对象标签的相关的 API 概览以及 SDK 示例代码。

| API                                                          | 操作名                       | 操作描述               |
| :----------------------------------------------------------- | :--------------------------- | :--------------------- |
| [PUT Object](https://cloud.tencent.com/document/product/436/7749) | 简单上传对象 | 上传一个对象至存储桶     |
| [PUT Object tagging](https://cloud.tencent.com/document/product/436/42997) | 添加对象标签                 | 为已存在的对象添加标签 |

## 上传时添加标签

上传对象时，可以通过为请求添加特定的 Header 来给对象打标签，比如设置 x-cos-tagging 的值为 Key1=Value1&Key2=Value2，标签集合中的 Key 和 Value 必须先进行 URL 编码。
...
putObjectTaggingRequest.addTag("Key2", "你好");

try {
	PutObjectTaggingResult result = cosXmlService.putObjectTagging(request);

...
将本地文件上传到 COS，适用于图片类小文件(20M以下)上传，最大支持 5 GB(含), 5 GB 以上必须使用分块上传或 [UploadService 上传](#upload_service)。如果COS上已存在对象, 则会进行覆盖。简单上传接口无法进行暂停和续传，一旦在上传过程中出现异常情况导致失败，那么您需要重新上传。
具体步骤如下：
...
`UploadService` 是一个已经封装过的上传类，它会根据您上传文件的大小（大于或者等于 2M 的文件使用分片上传，否则使用简单上传）来自动选择使用简单上传或者分片上传，它允许您开始、暂停、恢复和取消上传，并且上传大文件（大于等于 2M）时会支持断点续传，我们推荐您使用这种方式来上传文件，示例代码如下：
 ...
 * etag： 是此分片上传成功后，返回的此分片的 MD5 + 分片号
 */
String srcPath = "本地文件的绝对路径";
...
uploadPartRequest.setProgressListener(new CosXmlProgressListener() {
    @Override
    public void onProgress(long progress, long max) {
        // todo do something to update your progress
    }
});

对象存储服务 XML Android SDK 操作成功会返回各个 API 对应的类型，失败会抛 CosXmlClientException 和 CosXmlServiceException。其中 CosXmlClientException 是指客户端异常，如参数为空，网络连接失败等； CosXmlServiceException 是指服务端处理一些不符合要求的客户端请求所返回的错误，如访问不存在的文件，没有访问文件的权限等。详细请查看 [SDK异常说明](#sdk_exception)。
...
getObjectRequest.setSign(600); //设置签名sign有效期
...
copyObjectRequest.setSign(600); //设置签名sign有效期
...
## 生成预签名链接
### 示例1 获取下载预签名的链接
调用CosXmlService的getAccessUrlWithSign（CosXmlRequest）方法获取对应下载的预签名链接

String getAccessUrlWithSign(CosXmlRequest cosXmlRequest) throws CosXmlClientException

```java
try{
String urlWithSign = cosXml.getAccessUrlWithSign(putObjectRequest);
}catch(Exception ex){
...
### 示例2 获取任何请求的预签名链接
调用GenerateGetObjectURLUtils中提供的方法。

#### 参数说明
| 参数名称   | 参数描述   |类型 | 必填 | 
| -------- | --------------- | -- | ----------- |
| isHttps    | true：生成https链接； false: 生成http链接   |boolean           | 是  |
| appid    | 对象存储的服务 APPID|String           | 是  |
| bucket    | 存储桶名称(cos v5 的 bucket格式为：xxx-appid, 如 test-1253960454)   |String           | 是  |
| region    | 存储桶所在的地域                                                   |String           | 是  |
| cosPath    | 对象键，即存储到 COS 上的绝对路径   |String           | 是  |
| httpMethod    | 请求方法，如 put  |String           | 是  |
| headers    | 签名中需要验证的header, 不验证填写 null   |Map&lt;String, String>           | 否  |
| queryParameters    | 签名中需要验证的url中的请求参数, 不验证填写 null   |Map&lt;String, String>           | 否  |
| signTime    | 签名有效期，格式 startTimeSecond; endTimeSecond   |String           | 是  |
| keyTime    | key有效期，格式 startTimeSecond; endTimeSecond   |String           | 是  |
| secretId    | 密钥Id   |String           | 是  |
| signKey    |  密钥Key经过hamc之后的结果  |String           | 是  |
| signKey    |签名的有效期，单位为秒   | String           | 是  | 
| duration    | 签名有效期    |long         | 是  | 
| qCloudAPI   | 密钥接口      |QCloudAPI           | 否  | 
...
```java
try{
//secretKey 和 keyTime 经过 hamc获得 signKey并进行Hex
byte[] byteKey = secretKey.getBytes(Charset.forName("UTF-8"));
SecretKey hmacKey = new SecretKeySpec(byteKey, "HmacSHA1");
Mac mac = Mac.getInstance("HmacSHA1");
mac.init(hmacKey);
String signKey = StringUtils.toHexString(mac.doFinal(keyTime.getBytes(Charset.forName("UTF-8"))));

String sign = GenerateGetObjectURLUtils.getSign("get", "/1.txt", null, null, "1542594285;1542594885","1542594285;1542594885", "secretId", signKey);
}catch(Exception ex){
Log.d("TEST", ex.getMessage());
}
```
...
```java
try{
String urlWithSign = GenerateGetObjectURLUtils.getRequestUrlWithSign(false, "put", null, null, "appid","bucket", "region", "cosPath", 600, new GenerateGetObjectURLUtils.QCloudAPI() {
	@Override
...
客户端异常，用于指因为客户端原因导致无法和服务端完成正常的交互而导致的失败， 如客户端无法连接到服务端，无法解析服务端返回的数据， 读取本地文件发生 IO 异常等。CosXmlClientException 集成自 Exception, 使用方法同 Exception， 同时添加一个额外的成员errorCode，如下:
|成员|描述|类型|
| ---- | ---- | ---- |

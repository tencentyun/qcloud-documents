对象存储服务 XML Android SDK 操作成功会返回各个 API 对应的类型，失败会抛 CosXmlClientException 和 CosXmlServiceException。其中 CosXmlClientException 是指客户端异常，如参数为空，网络连接失败等； CosXmlServiceException 是指服务端处理一些不符合要求的客户端请求所返回的错误，如访问不存在的文件，没有访问文件的权限等。详细请查看 [SDK 异常说明](#sdk_exception)。
...
## 生成预签名链接
### 示例1 获取下载预签名的链接
调用 CosXmlService 的 getAccessUrlWithSign(CosXmlRequest) 方法获取对应下载的预签名链接
...
### 示例2 获取任何请求的预签名链接
调用 GenerateGetObjectURLUtils 中提供的方法。

#### 参数说明
| 参数名称   | 参数描述   |类型 | 必填 | 
| -------- | --------------- | -- | ----------- |
| isHttps    | true：生成 HTTPS 链接； false：生成 HTTP 链接   |boolean           | 是  |
| appid    | 对象存储的服务 APPID|String           | 是  |
| bucket    | 存储桶名称 (cos v5 的 bucket格式为：xxx-appid，如 test-1253960454)   |String           | 是  |
| region    | 存储桶所在的地域                                                   |String           | 是  |
| cosPath    | 对象键，即存储到 cos 上的绝对路径   |String           | 是  |
| httpMethod    | 请求方法，如 put  |String           | 是  |
| headers    | 签名中需要验证的 header， 不验证填写 null   |Map&lt;String, String>           | 否  |
| queryParameters    | 签名中需要验证的 url 中的请求参数，不验证填写 null   |Map&lt;String, String>           | 否  |
...
| signKey    |  密钥 Key 经过 hamc 之后的结果  |String           | 是  |

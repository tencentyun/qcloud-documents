...
- 对象存储 COS 的 XML C++ SDK 源码下载地址：
  linux版本: [ XML linux C++ SDK ](https://github.com/tencentyun/cos-cpp-sdk-v5 "cos-cpp-sdk-v5下载地址")， windows版本: [ XML  windows C++ SDK ](https://github.com/tencentyun/cos-cpp-sdk-v5 "cos-cpp-sdk-v5下载地址")。
...
在Windows环境下，安装visual studio开发环境。

### 安装 CMake 工具

从[CMake官网](https://cmake.org/download/)下载Windows版本的CMake编译工具，并将{CMake的安装路径}/bin配置在Windows系统中配置系统环境变量Path中。

### 安装OpenSSL

1.https://slproweb.com/products/Win32OpenSSL.html 直接选择window版本的.exe文件进行安装。

2.在Windows系统中系统环境变量增加OPENSSL_ROOT_DIR变量，并设置为{OpenSSL的安装路径}。例如：

 设置系统环境变量：OPENSSL_ROOT_DIR=D:\OpenSSL-Win64。

3.将{OpenSSL的安装路径}/bin配置在Windows的系统环境变量Path中。

### 安装Poco

从 [Poco 官网](https://pocoproject.org/download.html) 获取并安装 Poco 的库和头文件（下载 complete 版本）。

1.打开Windows的命令行，cd到Poco的源码目录下，执行 mkdir {文件夹名}} 命令，创建文件夹。

2.在Windows的命令行，执行cd  {文件夹名} 命令后，执行 cmake .. 命令。

3.然后使用VS打开解决方案，进行编译。

### 安装 Boost 

在[Boost官网](https://www.boost.org/)下载Boost源码

1.打开Windows的命令行，cd到Boost的源码目录下。

2.在Boost源码目录下，执行bootstrap命令。

3.随后在Windows的命令行下，执行b2编译命令。

注意:

visual studio  下有四种代码生成方式，对应boost在编译时不同的选择命令。
...
1.下载[Jsoncpp源码](https://github.com/open-source-parsers/jsoncpp)，win32下建议选择[老版本](https://github.com/open-source-parsers/jsoncpp/tree/0.y.z)。

2.打开Windows的命令行，cd到poco的源码目录下，执行 mkdir {文件夹名}} 命令，创建文件夹。

3.在Windows的命令行下，执行cd  {文件夹名}命令后，执行 cmake .. 命令。

4.使用visual studio 打开解决方案，进行编译。

5.编译完成后，将编译得到jsonlib拷贝到cos cpp sdk安装目录下lib文件夹中。

### 编译 COS CPP SDK 

下载 [XML Windows C++ SDK 源码](https://github.com/tencentyun/cos-cpp-sdk-v5/tree/windows_dev) 集成到您的开发环境，然后进行编译：

2.打开Windows的命令行，cd到C++ SDK的源码目录下，执行mkdir {文件夹名}命令，创建文件夹。

3.修改${cos-cpp-sdk} 安装目录下的CMakeLists.txt文件，修改例子如下：

```
# include directories
INCLUDE_DIRECTORIES(./include)
INCLUDE_DIRECTORIES({OpenSSL安装目录}/include)
INCLUDE_DIRECTORIES({Boost安装目录}/boost_1_68_0)
...
link_directories({Boost安装目录}/boost_1_68_0/stage/lib)

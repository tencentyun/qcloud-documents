...
Linux 版本: [ XML linux C++ SDK ](https://github.com/tencentyun/cos-cpp-sdk-v5 "cos-cpp-sdk-v5下载地址")， Windows 版本: [ XML  windows C++ SDK ](https://github.com/tencentyun/cos-cpp-sdk-v5 "cos-cpp-sdk-v5下载地址")。
...
> ?[示例 Demo](https://github.com/tencentyun/cos-cpp-sdk-v5/blob/master/demo/cos_demo.cpp) 提供常见 API 例子。生成的 cos_demo 可以直接运行，生成的静态库名称为：libcossdk.a。
> 生成的 libcossdk.a 放到您自己的工程里 lib 路径下，include 目录拷贝到自己的工程的 include 路径下。
...
在 Windows 环境下，安装 visual studio 开发环境。

#### 2. 安装 CMake 工具

从 [CMake官网](https://cmake.org/download/) 下载 Windows 版本的 CMake 编译工具，并将`{CMake的安装路径}/bin`，配置在 Windows 系统中配置系统环境变量 Path 中。

#### 3. 安装 OpenSSL

（1）单击 [这里](https://slproweb.com/products/Win32OpenSSL.html)， 选择 Windows 版本的`.exe`文件进行安装。
...
（1）打开 Windows 的命令行，cd 到 Poco 的源码目录下，执行`mkdir {文件夹名}}`命令，创建文件夹。
（2）在 Windows 的命令行，执行`cd  {文件夹名}`命令后，执行`cmake ..`命令。
（3）然后使用 VS 打开解决方案，进行编译。
...
#### 注意事项
visual studio  下有四种代码生成方式，对应 boost 在编译时不同的选择命令。
...
（1）下载 [Jsoncpp源码](https://github.com/open-source-parsers/jsoncpp)，Win32 下建议选择[低版本](https://github.com/open-source-parsers/jsoncpp/tree/0.y.z)。
（2）打开 Windows 的命令行，cd 到 poco 的源码目录下，执行`mkdir {文件夹名}}`命令，创建文件夹。
（3）在 Windows 的命令行下，执行`cd  {文件夹名}`命令后，执行`cmake .. `命令。
（4）使用 visual studio 打开解决方案，进行编译。
（5）编译完成后，将编译得到 jsonlib 拷贝到 COS CPP SDK 安装目录下 lib 文件夹中。

#### 7. 编译 COS CPP SDK 

（1）下载 [XML Windows C++ SDK 源码](https://github.com/tencentyun/cos-cpp-sdk-v5/tree/windows_dev) 集成到您的开发环境，然后进行编译：
（2）打开 Windows 的命令行，cd 到 C++ SDK 的源码目录下，执行`mkdir {文件夹名}`命令，创建文件夹。

...
1. 推荐在第三方 [网站](https://slproweb.com/products/Win32OpenSSL.html) 下选择 Windows 版本的 .exe 文件进行安装，也可以在 [OpenSSL官网](https://www.openssl.org/) 下载 OpenSSL 源码进行编译安装。
...
3. 使用 visual studio 2017 打开解决方案，进行编译。
...
```
4. 在 Windows 的命令行下，执行`cd examplefolder`  （examplefolder 为您的自定义文件夹名） 命令后，执行`cmake ..`命令。
5. 使用 visual studio 2017 打开解决方案，进行编译。

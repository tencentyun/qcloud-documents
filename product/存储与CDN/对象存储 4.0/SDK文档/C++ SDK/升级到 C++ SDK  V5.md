如果您细心对比过 CPP SDK V4 和 V5 的文档，您会发现并不是一个简单的增量更新。CPP SDK V5 不仅在架构、可用性和安全性上有了非常大的提升，而且在易用性、健壮性和传输性能上也做了非常大的改进。如果您想要升级到 CPP SDK V5，请参考下面的指引，完成 SDK 的升级工作。
...
| 功能           |                              V5                              |                              V4                              |
| -------------- | :----------------------------------------------------------: | :----------------------------------------------------------: |
| 文件上传       | 支持本地文件、字节流、输入流上传<br>覆盖同名文件 | 支持本地文件上传,大于8M的文件需将文件内容进行分片上传<br>需要手动设置同名文件是否覆盖 |
...
您也可以在这里 [COS CPP SDK 快速入门](https://cloud.tencent.com/document/product/436/12301)中获取 V5 SDK 的具体安装流程。

### 更改 SDK 配置文件

V4 和 V5 的初始化过程相同，但对应配置文件 "config.json" 内容不同。
在 V5 配置文件中，发生了一些变化：
* 删除 V4 中"APPID"配置项。
...
在初始化时，请将存储桶所在区域简称设置到配置文件的 `Region` 中：

### 更改 API

升级到 SDK V5 之后，一些操作的 API 发生了变化，请您根据实际需求进行相应的更改。我们同时做了封装让 SDK 更加易用，具体请参考我们的示例和 [接口文档](https://cloud.tencent.com/document/product/436/12302)。
...
在 COS V5 SDK中，我们封装了多线程分块上传和下载操作，为`MultiUploadObjectReq`和`MultiGetObjectReq`接口，对 API 设计和传输性能都能做了优化。
...
具体请参考我们的 [CPP SDK 接口文档](https://cloud.tencent.com/document/product/436/12302)。

...
示例3：从输入流上传(需提前告知输入流的长度，否则可能导致 oom)
<dx-codeblock>
:::  java
// Bucket的命名格式为 BucketName-APPID ，此处填写的存储桶名称必须为此格式
String bucketName = "examplebucket-1250000000";
...
putObjectResult = cosClient.putObject(bucketName, key, emptyContent, objectMetadata);
etag = putObjectResult.getETag();
// 关闭输入流...
:::
</dx-codeblock>
...
通过对简单操作的组合，完成一些对文件系统里的基本操作的模拟。

### 删除文件夹

COS 上的文件夹概念是以 '/' 分隔对象名，形成类似文件系统的路径，从而模拟出来的。所以删除文件夹的操作，在 COS
上相当于删除一批有着同样前缀的对象。例如：文件夹 'prefix/' ，代表的是以 'prefix/' 为前缀的所有对象，所以删除 'prefix/'
意味着删除以 'prefix/' 为前缀的所有对象。

目前 COS JAVA SDK 没有提供一个接口去支持这样的操作，但是可以通过基本操作的组合，达到同样的效果。

**删除文件夹**等于**查询对象列表**加上**批量删除对象**。
...
COS 上的对象通过 'bucket+key' 这个名字来标识，移动对象也就意味着对这个对象改名字，COS Java SDK 目前没有提供给对象
改名字的单独接口，但是可以通过基本操作，达到这个效果。

例如 'mybucket' 这个桶中的 'mykey' 这个对象要移动到 'mybucket' 这个桶的 'prefix/mykey'，就可以先复制一个 'prefix/mykey'
这个对象，然后删除 'mykey' 这个对象。

同样的，如果想把 'mykey' 这个对象移动到 'myanothorbucket' 这个桶里，也可以先复制对象到新桶，然后删除掉原来的对象。

**移动对象**等于**复制对象**加上**删除对象**。
...
public void copyDiffRegion(String srcRegion, String srcBucket, String srcKey,
    ...
    copyObjectRequest = new CopyObjectRequest(srcBucketRegion, srcBucket, srcKey, destBucketName, destKey);
    copyObjectResult = cosClient.copyObject(copyObjectRequest);
}

// 要移动的对象信息
String srcRegion = "ap-shanghai";
String srcBucket = "mybucket";
String srcKey = "mykey";

// 移动后的对象的信息
String destRegion = "ap-shanghai";
String destBucket = "myanotherbucket";
String destKey = "mynewkey";
...
public Download download(final GetObjectRequest getObjectRequest, final File file,
        ...
        int multiThreadThresholdMB, int partSizeMB) {
```
...
| multiThreadThresholdMB  | 断点续传下载使用多线程下载的最小文件大小  | int              | 20                            |
| partSizeMB              | 断点续传下载使用的分块大小              | int              | 8                             |
...
下载COS上有公共前缀的一批对象（虚拟文件夹）到指定的本地目录

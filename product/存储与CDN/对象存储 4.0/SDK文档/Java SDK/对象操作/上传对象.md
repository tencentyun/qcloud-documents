...
// 这里创建一个 ByteArrayInputStream 来作为示例，实际中这里应该是你要上传的 InputStream 类型的流
...
>？
> - 支持递归上传目录，但是如果太大会有上传比较慢，阻塞太久的情况。如果要递归上传目录，则建议只针对那些不是很大（比如有 1024 以上个文件）的目录。大目录可以分成小目录多调用几次。
>
...
>?
> - 如果要上传的文件，或者流，是可以完整获取的，推荐使用高级接口
>
...
0. 如果没有记录 UploadId，则要先查询分块上传任务（List Multipart Uploads），得到对应文件的 UploadId
...
0. 如果没有记录 UploadId，则查询分块上传任务（List Multipart Uploads），得到对应文件的 UploadId
1. 终止分块上传并删除已上传分块（Abort Multipart Upload）

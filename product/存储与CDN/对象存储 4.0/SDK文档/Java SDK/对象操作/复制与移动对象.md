...
| API                                                          | 操作名         | 操作描述                             |
| [List Multipart Uploads](https://cloud.tencent.com/document/product/436/7736) | 查询分块复制   | 查询正在进行中的分块复制信息         |
| [Initiate Multipart Upload](https://cloud.tencent.com/document/product/436/7746) | 初始化分块复制 | 初始化分块复制操作     |
| [Upload Part - Copy](https://cloud.tencent.com/document/product/436/8287) | 复制分块       | 将其他对象复制为一个分块             |
| [List Parts](https://cloud.tencent.com/document/product/436/7747) | 查询已复制块   | 查询特定分块复制操作中的已复制的块   |
| [Complete Multipart Upload](https://cloud.tencent.com/document/product/436/7742) | 完成分块复制   | 完成整个文件的分块复制              |
| [Abort Multipart Upload](https://cloud.tencent.com/document/product/436/7740) | 终止分块复制   | 终止一个分块复制操作并删除已复制的块 |
...
0. 如果没有记录 UploadId，则要先查询分块复制任务（List Multipart Uploads），得到对应文件的 UploadId
1. 使用 UploadId 列出已复制的分块（List Parts）
2. 使用 UploadId 复制剩余的分块（Upload Part）
3. 完成分块复制（Complete Multipart Upload）

#### 终止分块复制的流程

0. 如果没有记录 UploadId，则查询分块复制任务（List Multipart Uploads），得到对应文件的 UploadId
1. 终止分块复制并删除已复制分块（Abort Multipart Upload）

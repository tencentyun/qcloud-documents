...
#### 使用永久云API秘钥信息初始化
云 API 秘钥为永久秘钥，通过在腾讯云 [CAM 控制台](https://console.qcloud.com/cam/capi) 获取生成，适用于大部分的应用场景。

JAVA SDK 云 API 秘钥的初始化方法如下所示。
```java
...
通过向腾讯云 CAM 服务申请临时密钥，关于临时秘钥生成和使用可参考 [临时密钥生成及使用指引](https://cloud.tencent.com/document/product/436/14048)。

JAVA SDK 支持传入临时秘钥来初始化客户端，临时秘钥的初始化方法如下所示。
```java

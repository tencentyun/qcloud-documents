...
>- COS Java SDK 中的常见类所在包分别为：
 ...
 - 高级API接口在 com.qcloud.cos.transfer.* 子包下。
...
### 下载对象
上传对象后，您可以用同样的 key，调用 GetObject 接口将对象下载到本地，也可以生成预签名链接（下载请指定 method 为 GET，详情请参阅 [预签名 URL 文档](#)），分享到其他终端来进行下载。但如果您的文件设置了私有读权限，那么请注意预签名链接的有效期。
将文件下载到本地指定路径，参考示例如下：

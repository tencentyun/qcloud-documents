...
|  成员名 | 设置方法            | 描述                                                         | 类型    |
| ------------ | ------------------- | ------------------------------------------------------------ | ------- |
| region   | 构造函数或 set 方法 | 存储桶所在的区域, COS 地域的简称请参阅 https://cloud.tencent.com/document/product/436/6224 | Region  |
| httpProtocol       | set 方法 |  请求所使用的协议，默认使用 http 协议与 COS 交互| HttpProtocol  |
| signExpired      | set 方法 | 请求签名的有效时间，默认为 1 小时   | int |
| connectionTimeout      | set 方法 | 连接 COS 服务的超时时间，默认为 30s        | int |
| socketTimeout      | set 方法 |  客户端读取数据的超时时间, 默认为 30s         | int |

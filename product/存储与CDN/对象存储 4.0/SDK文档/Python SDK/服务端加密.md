...
由腾讯云 COS 托管主密钥和管理数据。COS 会帮助您在数据写入数据中心时自动加密，并在您取用该数据时自动解密。目前支持使用 COS 主密钥对数据进行 AES-256 加密。SDK 通过调用 `put_object` 来完成。

[//]: # (.cssg-snippet-put-object-sse-cos)
```py
...
由腾讯云 KMS 托管主密钥和管理数据。COS 会帮助您在数据写入数据中心时自动加密，并在您取用该数据时自动解密。目前支持使用 KMS 主密钥对数据进行加密。SDK 通过调用 `put_object` 来完成。
...
response = client.put_object(
    ...
    SSEKMSKeyId = 'ExampleKMSkeyId' #KMSKeyId可以在这里获取 https://console.cloud.tencent.com/kms2
)
```

...
| 节点名称（关键字） | 父节点             | 描述     | 类型      |
| :----------------- | :----------------- | :------- | :-------- |
| Stream             | MediaInfo | 流信息   | Container |
| Format             | MediaInfo | 格式信息 | Container |

Container 节点 Stream 的内容：

| 节点名称（关键字） | 父节点                     | 描述     | 类型      |
| :----------------- | :------------------------- | :------- | :-------- |
| Video              | MediaInfo.Stream | 视频信息 | Container |
| Audio              | MediaInfo.Stream | 音频信息 | Container |
| Subtitle           | MediaInfo.Stream | 字幕信息 | Container |

Container 节点 Format 的内容（查询视频信息时，可能部分字段未返回）：

| 节点名称（关键字） | 父节点                     | 描述                                        | 类型   |
| :----------------- | :------------------------- | :------------------------------------------ | :----- |
| NumStream          | MediaInfo.Format | Stream（包含 Video、Audio、Subtitle）的数量 | Int    |
| NumProgram         | MediaInfo.Format | 节目的数量                                  | Int    |
| FormatName         | MediaInfo.Format | 容器格式名字                                | String |
| FormatLongName     | MediaInfo.Format | 容器格式的详细名字                          | String |
| StartTime          | MediaInfo.Format | 起始时间，单位为秒                          | Float  |
| Duration           | MediaInfo.Format | 时长，单位为秒                              | Float  |
| Bitrate            | MediaInfo.Format | 比特率，单位为 kbps                         | Int    |
| Size               | MediaInfo.Format | 大小，单位为 Byte                           | Int    |

Container 节点 Video 的内容（查询视频信息时，可能部分字段未返回）：

| 节点名称（关键字） | 父节点                           | 描述                        | 类型   |
| :----------------- | :------------------------------- | :-------------------------- | :----- |
| Index              | MediaInfo.Stream.Video | 该流的编号                  | Int    |
...
| Height             | MediaInfo.Stream.Video | 视频高，单位 px             | Int    |
| Width              | MediaInfo.Stream.Video | 视频宽，单位 px             | Int    |
| HasBFrame          | MediaInfo.Stream.Video | 是否有B帧。1表示有，0表示无 | Int    |
| RefFrames          | MediaInfo.Stream.Video | 视频编码的参考帧个数        | Int    |
...
| Level              | MediaInfo.Stream.Video | 视频编码等级                | Int    |
| Fps                | MediaInfo.Stream.Video | 视频帧率                    | Int    |
| AvgFps             | MediaInfo.Stream.Video | 平均帧率                    | String |
| Timebase           | MediaInfo.Stream.Video | 时基                        | String |
| StartTime          | MediaInfo.Stream.Video | 视频开始时间，单位为秒      | Float  |
| Duration           | MediaInfo.Stream.Video | 视频时长，单位为秒          | Float  |
| Bitrate            | MediaInfo.Stream.Video | 比特率，单位为 kbps         | Float  |
| NumFrames          | MediaInfo.Stream.Video | 总帧数                      | Int    |
...
| 节点名称（关键字） | 父节点                           | 描述                 | 类型   |
| :----------------- | :------------------------------- | :------------------- | :----- |
| Index              | MediaInfo.Stream.Audio | 该流的编号           | Int    |
...
| SampleRate         | MediaInfo.Stream.Audio | 采样率               | Int    |
| Channel            | MediaInfo.Stream.Audio | 通道数量             | Int    |
| ChannelLayout      | MediaInfo.Stream.Audio | 通道格式             | String |
| Timebase           | MediaInfo.Stream.Audio | 时基                 | String |
| StartTime          | MediaInfo.Stream.Audio | 音频开始时间，单位秒 | Float  |
| Duration           | MediaInfo.Stream.Audio | 音频时长，单位秒     | Float  |
| Bitrate            | MediaInfo.Stream.Audio | 比特率，单位 kbps    | Float  |
...
| 节点名称（关键字） | 父节点                              | 描述                     | 类型   |
| :----------------- | :---------------------------------- | :----------------------- | :----- |
| Index              | MediaInfo.Stream.Subtitle | 该流的编号               | Int    |

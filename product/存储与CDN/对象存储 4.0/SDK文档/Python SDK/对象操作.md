...
| IsTruncated   |  表示返回的对象否被截断  | String|
| EncodingType   | 默认不编码，规定返回值的编码方式，可选值：url  | String  | 否|
|Version |包含所有多版本对象元信息的 list，包括 'ETag'，'StorageClass'，'Key'，'VersionId'，'IsLatest'，'Owner'，'LastModified'，'Size' 等信息|List|
|DeleteMarker|包含所有delete marker 对象元信息的 list，包括 'Key'，'VersionId'，'IsLatest'，'Owner'，'LastModified' 等信息|List|
|CommonPrefixes |所有以 Prefix 开头，以 Delimiter 结尾的对象被归到同一类|List|
...
|  x-cos-version-id   |  开启多版本后，文件的版本号 | String  |
...
response = client.head_object(
    ...
    Key='exampleobject',
    VersionId='string',
    IfModifiedSince='Mon, 05 Aug 2019 17:56:27 GMT',
)
```
#### 参数说明
...
| -------------- | -------------- |---------- | ----------- |
  ...
  | Key   |  对象键（Key）是对象在存储桶中的唯一标识。例如，在对象的访问域名 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg` 中，对象键为 doc/pic.jpg  |String  | 是 |
  | VersionId   | 开启多版本后，指定文件的具体版本  | String  | 否 | 
  | IfModifiedSince   | 在指定时间后被修改才返回，时间格式为GMT  | String  | 否 | 
...
response = client.get_object(
    ...
    IfUnmodifiedSince='Mon, 05 Aug 2019 17:56:27 GMT',
...
| -------------- | -------------- |---------- | ----------- |
 ...
 |  IfUnmodifiedSince  |  文件修改时间早于或等于指定时间才返回，时间格式为GMT  | String  | 否|
...
<table>
   ...
   <tr>
      <td>Body</td>
      <td>下载文件的内容，get_raw_stream() 方法可以得到一个文件流，get_stream_to_file(local_file_path) 方法可以将文件内容下载到指定本地文件中</td>
      <td>StreamBody</td>
   </tr>
   <tr>
      <td>ETag</td>
...
| -------------- | -------------- |---------- | ----------- |
 |  Bucket  | 存储桶名称，由 BucketName-APPID 构成 | String|  是 |
 |  Key  | 对象键（Key）是对象在存储桶中的唯一标识。例如，在对象的访问域名 `examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com/doc/pic.jpg` 中，对象键为 doc/pic.jpg | String| 是 | 
 |  CopySource  | 描述拷贝源文件的路径，包含 Bucket、Key、Region、VesionId |  Dict | 是 |
...
| VersionId | 开启多版本后，目的文件的版本号 | String |
| x-cos-copy-source-version-id | 源文件的版本号 | String | 
...
| 参数名称   | 参数描述   |类型 | 
| -------------- | -------------- |---------- | 
| x-cos-version-id | 删除文件的版本号 | String |
| x-cos-delete-marker | 标识删除的文件是否为delete marker | String | 
...
| PartNumber  |标识上传分块的序号 |  Int |  是|
| UploadId  | 标识分块上传的 ID | String |  是|
|  CopySource  | 描述拷贝源文件的路径，包含 Bucket、Key、Region、VersionId |  Dict | 是 |

...
region = 'ap-beijing'      # 替换为用户的 region，已创建桶归属的 region 可以在控制台查看，https://console.cloud.tencent.com/cos5/bucket
                           # COS支持的所有region列表参见 https://cloud.tencent.com/document/product/436/6224
...
tmp_secret_id = 'TmpSecretId'     # 临时密钥的 SecretId，临时密钥生成和使用指引参见 https://cloud.tencent.com/document/product/436/14048
tmp_secret_key = 'TmpSecretKey'   # 临时密钥的 SecretKey，临时密钥生成和使用指引参见 https://cloud.tencent.com/document/product/436/14048
...
token = None               # 如果使用永久密钥不需要填入 token，如果使用临时密钥需要填入，临时密钥生成和使用指引参见https://cloud.tencent.com/document/product/436/14048
scheme = 'https'           # 指定使用 http/https 协议来访问 COS，默认为 https，可不填
...
通过密钥轮转对象实例初始化时，SDK 会从用户传入的 CredentialInstance 对象获取密钥，用户可将密钥轮转逻辑封装到该对象中，暴露 `secret_id()`、`secret_key()`和`token()` 接口供 SDK 调用，进一步防止密钥泄漏。
...
config = CosConfig(Region=region, CredentialInstance=credential, Scheme=scheme)
client = CosS3Client(config)
```

...
### 初始化
下面介绍几种初始化Python SDK Client的方式，可以根据具体场景选择其中一种。

#### 通过COS默认域名初始化
通过COS默认域名初始化Client，需要传入region名称，SDK会以{bucket-appid}.cos.{region}.myqcloud.com的域名形式访问COS。
...
>! 正常情况下一个region只需要生成一个CosS3Client实例，然后循环上传或下载对象，不能每次访问都生成CosS3Client实例，否则python进程会占用过多的连接和线程。

>? 关于临时密钥如何生成和使用，请参见 [临时密钥生成及使用指引](https://cloud.tencent.com/document/product/436/14048)。
>

#### 通过COS默认域名和代理初始化
当需要通过代理访问COS时配置，否则跳过。
...
#### 通过自定义Endpoint初始化（全球加速域名）
通过自定义的Endpoint初始化CosS3Client，SDK会以{bucket-appid}.{endpoint}的域名形式访问COS。
比如Endpoint配置为COS全球加速域名时，SDK会以{bucket-appid}.cos.accelerate.myqcloud.com的域名形式访问COS。
...
#### 通过自定义域名初始化
通过自定义域名初始化CosS3Client，SDK会使用配置的域名直接访问COS，bucket和region都不会出现在访问域名中。

...
上传一个 Object（文件/对象）至存储桶（PUT Object）。对于大文件，推荐使用高级接口 MultiUpload 方法。
#### 方法原型
...
- 删除已上传分块。
- 分块上传对象，推荐用户使用 高级接口 MultiUpload 方法
...
上传接口根据用户文件的长度，自动切分数据， 降低用户的使用门槛。用户不用关心分块上传的每个步骤。

#### 方法原型

```go
func (s *ObjectService) MultiUpload(ctx context.Context, key string, filepath string, opt *MultiUploadOptions) (*CompleteMultipartUploadResult, *Response, error)
```
...
v, resp, err := c.Object.MultiUpload(
...
| opt            | 对象属性                                                     | Struct | 是   |
| OptIni         | 设置对象属性和ACL，见InitiateMultipartUploadOptions          | Struct | 否   |
| PartSize       | 块大小，如果用户不指定或者指定partSize < 0，由SDK自动切分    | int    | 否   |

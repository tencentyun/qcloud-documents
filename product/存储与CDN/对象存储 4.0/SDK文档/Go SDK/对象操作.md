...
上传由 '/' 分隔的对象名，自动创建包含文件的文件夹。想要在此文件夹中添加新文件时，只需要在将文件上传至COS时，填写的 Key 以此目录为前缀即可。
```go
dir := "exampledir/"
...
```
#### 请求示例3 获取下载进度
Go SDK通过回调的方式获取下载进度，用户需要自行实现cos.ProgressListener接口，接口定义如下
```go
const (
    // 数据开始传输
...
分块下载接口根据用户对象的长度，自动使用Range下载数据，可以实现并发下载。

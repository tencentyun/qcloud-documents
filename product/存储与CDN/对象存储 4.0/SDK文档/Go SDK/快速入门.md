...
### 环境依赖
- Golang：用于下载和安装 Go 编译运行环境，前往 [下载 Golang ](https://golang.org/doc/install/source?spm=a2c4g.11186623.2.11.168ec486fb1eQK)。
...
client := cos.NewClient(b, &http.Client{
	Transport: &cos.AuthorizationTransport{
		// 填写 SecretID 和 SecretKey 有两种方式：
		// 1.设置环境变量（Getenv 为获取环境变量函数），如下：
		SecretID: os.Getenv("COS_ENV_SECRETID"),                         
		SecretKey: os.Getenv("COS_ENV_SECRETKEY"),
		// 2.直接填写账号和密钥, 如下：
...
client := cos.NewClient(b, &http.Client{
	Transport: &cos.AuthorizationTransport{
		...
		SecretKey: "COS_SECRETKEY",
		// 设置临时密钥token
        	SessionToken：os.Getenv("COS_SECRETTOKEN"),
	},                                 
})
```
...
func main() {
	...
	err = c.Object.GetToFile(context.Background(), name, "example", nil)
	if err != nil {
		panic(err)
	}
}

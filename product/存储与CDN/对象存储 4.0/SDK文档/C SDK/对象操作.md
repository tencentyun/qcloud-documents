...
| ------------ | ------------------------------------------------------------ | ------ |
| options      | COS 请求选项                                                 | Struct |
| src_bucket       | 源存储桶名称，存储桶的命名格式为 BucketName-APPID，此处填写的存储桶名称必须为此格式                             | String |
| src_object       | 源Object 名称                                                 | String |
| src_endpoint     | 源endpoint 域名                                                 | String |
| dest_bucket      | 目的存储桶名称，存储桶的命名格式为 BucketName-APPID，此处填写的存储桶名称必须为此格式                           | String |
| dest_object      | 目的Object 名称                | String |
| headers          | 请求添加的头域                  | Struct |
| copy_object_param | 对象拷贝参数，包括etag和last_modify                                        | Struct |
| resp_headers     | 返回 HTTP 响应消息的头域                                                 | Struct |
...
注意：标准存储可以转换到归档存储和深度归档存储，如果希望将归档存储或深度归档存储的对象转换为其他存储类型，需要首先使用cos_post_object_restore()将归档存储或深度归档存储的对象回热，才能使用该接口请求转换存储类型；关于存储类型详细说明请参见 [存储类型概述](https://cloud.tencent.com/document/product/436/33417)。

...
cos_str_set(&options->config->access_key_id, "<用户的SecretId>");         //用户从 sts服务获取到的 临时SecretId
cos_str_set(&options->config->access_key_secret, "<用户的SecretKey>");    //用户从 sts服务获取到的 临时SecretKey
cos_str_set(&options->config->sts_token, "<用户的StsToken>");             //用户从 sts服务获取到的 临时token

对象存储服务 XML C# SDK 操作成功会返回各个 API 对应的类型，失败会抛 CosClientException
 和 CosServerException。其中 CosClientException 是指客户端异常，如参数为空，网络连接失败等； CosServerException 是指服务端处理一些不符合要求的客户端请求所返回的错误，如访问不存在的文件，没有访问文件的权限等。详细请查看 [SDK 异常说明](#COS_CLIENT_EXCEPTION)。
...
*操作失败会抛出* 
[*CosClientException*](COS_EXCEPTION) *或* [*CosServerException*](COS_SERVER_EXCEPTION) *异常*
...
获取指定存储桶的跨域访问配置 (CORS) (Delete Bucket CORS).
...
2、根据uploadId，进行分片复制;
3、完成所有分片复制，详细请查看 [CompleteMultiUpload](COMPLETE_MULTI_UPLOAD) .
...
### <span id="COS_CLIENT_EXCEPTION"> CosXmlClientException </span>
...
### <span id="COS_SERVER_EXCEPTION"> CosXmlServiceException </span>

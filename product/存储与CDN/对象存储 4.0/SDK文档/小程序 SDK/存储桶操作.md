...
GET Service 接口是用来查询请求者名下的所有存储桶列表或特定地域下的存储桶列表（Bucket list）。了解更多请参阅 [GET Service](https://cloud.tencent.com/document/product/436/8291)。
...
| 参数名 | 参数描述                                                     | 类型   | 必填 |
| ------ | ------------------------------------------------------------ | ------ | ---- |
| Region | Bucket 所在地域。枚举值请参阅 [地域和访问域名](https://cloud.tencent.com/document/product/436/6224) | String | 否 |
...
PUT Bucket 接口请求可以在指定账号下创建一个存储桶，创建 存储桶的用户默认成为存储桶的持有者。了解更多请参阅 [PUT Bucket 接口说明](https://cloud.tencent.com/document/product/436/7738)。
...
GET Bucket acl 接口用来查询 Bucket 的 ACL(access control list)， 即存储桶（Bucket）的访问权限控制列表。 此 API 接口只有 Bucket 的持有者有权限操作。了解更多请参阅 [Get Bucket ACL 接口说明](https://cloud.tencent.com/document/product/436/7733)。

...
小程序里请求 COS 需要到[微信小程序管理界面](https://mp.weixin.qq.com)，在“开发”->“开发设置”，配置域名白名单，。

SDK 用到了两个接口：
1. cos.postObject 使用 wx.uploadFile 方法
2. 其他方法使用 wx.request 方法

要在对应白名单里需要配置 COS 域名。配置的域名格式：
1. 如果标准请求可以配置 Bucket 域名作为白名单域名，例如：
`examplebucket-1250000000.cos.ap-guangzhou.myqcloud.com`
2. 如果小程序使用的存储桶多，可以选择后缀式请求 COS，只需要在 SDK 实例化时传入 `ForcePathStyle: true`，这种方式需要配置园区域名作为白名单，例如:

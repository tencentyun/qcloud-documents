...
> 3. 从result文件夹下将打出产物拖到工程中，然后按照上面手动集成的方式集成即可。
...
SDK在发请求的时候需要获取临时密钥计算签名，因此需要您实现`QCloudSignatureProvider`来获取密钥，并将密钥通过block回调给SDK。建议把初始化过程放在 `AppDelegate`或者 **程序单例** 中。
...
SDK提供了一个 `QCloudCredentailFenceQueue` 的脚手架，实现对临时密钥的缓存与复用。脚手架在密钥过期之后会重新该调用协议的方法来重新获取新的密钥，直到该密钥过期时间大于当前时间。
>! 脚手架只是通过密钥的过期时间是否大于当前时间来判断密钥是否能复用的，如果您申请密钥时有设置复杂的策略不建议使用脚手架工具

...
- SDK 常见问题请参考：[iOS SDK 常见问题](https://cloud.tencent.com/document/product/436/50747)。
...
>! SDK提供了打包脚本，支持根据业务需求自行打包(该打包脚本依赖于cocoapods，请先确保您的开发环境安装了cocoapods)，打包步骤如下

> 1. 下载源码：git clone https://github.com/tencentyun/qcloud-sdk-ios
> 2. 运行打包脚本: source package.sh
> 3. 将打出产物拖到工程中，然后按照上面手动集成的方式集成即可。
>>   1. iOS：将ios文件夹下的产物拖入项目
>>   2. macOS:将osx文件夹下的产物拖入项目
...
SDK在发请求的时候需要获取临时密钥计算签名，因此需要您实现 `QCloudSignatureProvider` 协议，在该协议中获取密钥之后将密钥通过参数`continueBlock` 回调给SDK。
...
SDK提供了一个 `QCloudCredentailFenceQueue` 的脚手架，实现对临时密钥的缓存与复用。脚手架在密钥过期之后会重新该调用协议的方法来重新获取新的密钥，直到该密钥过期时间大于设备的当前时间。
>! 脚手架只是通过密钥的过期时间是否大于设备的当前时间来判断密钥是否能复用的，如果您申请密钥时有设置复杂的策略不建议使用脚手架工具。

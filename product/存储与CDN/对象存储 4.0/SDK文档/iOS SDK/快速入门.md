...
#### 后台上传
如果要开启后台传输，需要在实例化云服务配置对象 QCloudServiceConfiguration的时候设置其backgroundIn4GEnable为YES，除此之外还必须设置后台传输的唯一标识backgroundIdentifier，如果想要在4G网络下也支持后台传输，需要设置backgroundIn4GEnable为YES。
##### 示例
```objective-c
QCloudServiceConfiguration* configuration = [QCloudServiceConfiguration new];

...
本文档提供关于如何使用在COS 上移动对象：

COS 上的对象通过 'bucket+key' 这个名字来标识，移动对象也就意味着对这个对象改名字，COS iOS SDK 目前没有提供给对象
改名字的单独接口，但是可以通过基本操作，达到这个效果。

例如 'sourcebucket' 这个桶中的 'exampleobject' 这个对象要移动到 'examplebucket' 这个桶的下，就可以先复制一个 'exampleobject'
这个对象，然后删除 'exampleobject' 这个对象。

...
[put setFinishBlock:^(id result, NSError *error) {
    ...
    // 本地的文件crc64 若返回的crc64与本地计算一致，则表示本地的文件与远端文件完整一致；
...
[request setFinishBlock:^(QCloudUploadPartResult* outputObject, NSError *error) {
    ...
    // 保存起来用于最好完成上传时使用
    self.parts = @[part];
    // 上传当前分块完成后，服务端返回的 crc64
    result.crc64
    // 本地当前分块的crc64 若返回的crc64与本地计算一致，则表示当前分块上传完成且完整一致；
    uint64_t localCrc64 = 当前分块.data.qcloud_crc64();
...
[completeRequst setFinishBlock:^(QCloudUploadObjectResult * _Nonnull result,
    ...
    // 服务端文件的crc64
    result.crc64
    // 本地的文件crc64 若返回的crc64与本地计算一致，则表示本地文件与远端文件完整一致；
    uint64_t localCrc64 = [本地文件data qcloud_crc64];
}];
...
complete.setFinish { (result, error) in
    if let result = result {
        ...
        result.crc64
        // 本地的文件crc64 若返回的crc64与本地计算一致，则表示本地文件与远端文件完整一致；
        int localCrc64 = 本地文件data.qcloud_crc64();
    } else {
        print(error!);
    }
}

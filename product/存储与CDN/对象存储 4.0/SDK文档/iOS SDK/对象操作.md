...
// 构建请求
 let uploadRequest = QCloudCOSXMLUploadObjectRequest.init();
uploadRequest.body = self.uploadFilePath as AnyObject
uploadRequest.bucket = "example-1250000000";
...
NSError* error;
//这里是主动调用取消，并且产生 resumetData 的例子
let resumedData =  self.uploadRequest.cancel(byProductingResumeData: &error) as QCloudCOSXMLUploadObjectResumeData;
if  resumedData != nil {
      let resumeUploadRequest = QCloudCOSXMLUploadObjectRequest<AnyObject>.init(request: resumedData as Data?);
 }
//生成的用于恢复上传的请求可以直接上传
QCloudCOSXMLServiceConfiguration.shared.currentTransferManagerService().uploadObject(resumeUploadRequest);
```
...
copyRequest.bucket = "example-1250000000";//目的<BucketName-APPID>，需要是公有读或者在当前账号有权限
...
getBucketReq.bucket = "example-1250000000";
...
putObject.bucket = "example-1250000000";
putObject.body =  filePath;//只支持Data、以及文件的本地路径
...
headObject.bucket = "example-1250000000";
...
getObject.bucket = "example-1250000000";
...
optionsObject.bucket = "example-1250000000";
...
putObjectCopy.bucket = "example-1250000000";
...
deleteObject.bucket = "example-1250000000";
...
[delteRequest setFinishBlock:^(QCloudDeleteResult* result, NSError *error) {
    //可以从 result 获取删除的对象
}];
...
mutipleDel.bucket = "example-1250000000";
...
listParts.bucket = "example-1250000000";
...
uploadPart.bucket = "example-1250000000";
...
restore.finishBlock = {(outputObject,error)in
     if error != nil{
          print(error!)
     }else{
           //可以从 outputObject 中获取 response 中 etag 或者自定义头部等信息

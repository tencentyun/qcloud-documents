...
可以参考快速入门中的 [上传对象](https://cloud.tencent.com/document/product/436/11280#.E4.B8.8A.E4.BC.A0.E6.96.87.E4.BB.B6) 环节。

### 上传对象时的断点续传

对于大于1M的对象，SDK 都会以分片上传的方式进行上传，即将文件切分成多个1M大小的分块，然后并行地（最大并行数为4）进行上传。对于每个已经完成上传的分片，后台服务器都会将其保存起来，这是上传时候断点续传的基础。    

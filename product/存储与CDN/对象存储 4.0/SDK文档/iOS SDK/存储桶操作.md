...
putBucketReq.bucket = "example-1250000000";
...
headBucketReq.bucket = "example-1250000000";
...
deleteBucketReq.bucket = @"example-1250000000";  //存储桶名称，命名格式：BucketName-APPID
...
putBucketACLReq.bucket = @"example-1250000000";
let appid =  "1131975903";//授予全新的账号ID
...
getBucketACLReq.bucket = "example-1250000000";
getBucketACLReq.setFinish { (result, error) in;
   if error != nil{
       print(error!);
   }else{
     //result中包含了 Bucket 的 ACL 信息。
       print(result!);
   }
 }

如果您细心对比过 iOS SDK V4 和 V5 的文档，您会发现并不是一个简单的增量更新。iOS SDK V5 不仅在架构、可用性和安全性上有了非常大的提升，而且在易用性、健壮性和传输性能上也做了非常大的改进。如果您想要升级到 iOS SDK V5，请参考下面的指引，一步步完成 SDK 的升级工作。
...
在 SDK V5 中，我们封装了上传、下载和复制操作，命名为 `QCloudCOSTransferMangerService`，对 API 设计和传输性能都做了优化，建议您直接使用。`QCloudCOSTransferMangerService`的主要特性有：

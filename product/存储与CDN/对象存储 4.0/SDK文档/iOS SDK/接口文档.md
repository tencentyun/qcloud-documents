...
### 加入自定义头部
如果有需要加入自定义头部的需求，例如因为使用服务端加密或者其它需求等，可以使用自定义头部。支持加入自定义头部的类，都会有 customHeaders 这个属性：
```objective-c
@property (strong, nonatomic) NSDictionary* customHeaders;
```
对于所有该属性内的键值对，都会以对应的形式加入到构建请求的 HTTP 头部中。示例可以参考最后的服务器端加密（Server side encryption）说明一节。
## Service 操作
...
| 参数名称   | 描述                                       |类型         | 必填 |
| ------ | ---------- | ---- | ---------------------------------------- |
| Object  | 上传文件（对象）的文件名，也是对象的 key。 对象键（Key）是对象在存储桶中的唯一标识。例如，在对象的访问域名 bucket1-1250000000.cos.ap-guangzhou.myqcloud.com/doc1/pic1.jpg 中，对象键为 doc1/pic1.jpg。更详细的描述可以参考[对象描述](https://cloud.tencent.com/document/product/436/13324)            |NSString * | 是    |
|bucket|存储桶名,可在[COSV5控制台](https://console.cloud.tencent.com/cos5/bucket)上面看到，格式为&lt;bucketName&gt;-&lt;APPID&gt; ,例如 testBucket-1253653367|NSString * |是|
...
COS 支持以下几种服务器端加密(Server side encryption)的方式，
### 服务端加密 - 无需解密的加密
该特性的作用是 object 在上传到 COS 后会进行服务器端加密，然后再存储。下载时候会拿出原始没有解密的数据，进行解密后返回。这个过程对客户端是透明的，无需关心具体的加密过程。
如果需要使用该特性，那么在上传 object 时，需要在上传HTTP请求中加入一个额外的头部，key 为 x-cos-server-side-encryption, 值为AES256。在下载时候直接照常 GetObject 即可。

利用 SDK 中自定义头部的功能可以实现这一需求。只需要在上传请求中加入自定义头部即可：
...
```
### 服务端加密 - SSE-C
该请求必须使用 SSL 保护数据内容，或必须签名,cos 会记录并在下载时匹配密钥。
该请求返回的 ETag 和源文件的 ETag 不同。

注意：如果要对使用SSE-C加密过的文件进行操作
   1. 运行的服务必须开启https，设置QCloudCOSXMLEndPoint的useHTTPS为yes即可
   2. 如果request设置了enableMD5Verification（是否开启对文件的校验）的开关,请设置该request的enableMD5Verification为NO
   
如果需要使用该特性，那么在上传 object 时，需要在上传请求中加入以下3个头部：

|     头部名称      | 描述                     | 必填        |
| ----------------------------- | ----------------------------------------- | -- |
| x-cos-server-side-encryption-customer-algorithm |  使用 SSE-C 则此头部必填 AES256 | Y |
| x-cos-server-side-encryption-customer-key |  提供一个 256 位的 Base64 后的密钥 | Y |
| x-cos-server-side-encryption-customer-key-MD5 |  提供 Base64 后密钥的 MD5 校验值 | Y |
...
```objective-c
    ...
    NSData *data = [@"123456qwertyuioplkjhgfdsazxcvbnm" dataUsingEncoding:NSUTF8StringEncoding];
    ...
    uploadObjectRequest.customHeaders = @{@"x-cos-server-side-encryption-customer-algorithm":@"AES256",
                                          @"x-cos-server-side-encryption-customer-key":excryptAES256Key,
                                          @"x-cos-server-side-encryption-customer-key-MD5":base64md5key
                                          };
    [uploadObjectRequest setFinishBlock:^(QCloudUploadObjectResult *result, NSError *error) {
        if(!error){
	    //得到响应头
            NSDictionary *responseParamas = (NSDictionary *)result.__originHTTPURLResponse__.allHeaderFields;
            //该值要保存，如果对源文件使用SSE-C进行了加密，那么在以后下载、copy、headObject的时候要对源文件解密需要在customHeader中携带该值
            NSString *value = responseParamas[@"x-cos-server-side-encryption-customer-key-MD5"];
        }
    }];
    //该request运行的service需要开启https
    [[QCloudCOSTransferMangerService costransfermangerServiceForKey:kHTTPServiceKey] UploadObject:uploadObjectRequest];
```
下载数据在自定义头部中必须携带下面3个头部，cos需要使用其对加密的文件进行解密

|     头部名称      | 描述                     | 必填        |
| ----------------------------- | ----------------------------------------- | -- |
| x-cos-server-side-encryption-customer-algorithm |  使用 SSE-C 则此头部必填 AES256 | Y |
| x-cos-server-side-encryption-customer-key |  提供一个 256 位的 Base64 后的密钥 | Y |
| x-cos-server-side-encryption-customer-key-MD5 |  加密后的 MD5，非源文件 MD5，从上传成功的响应头部可以得到 | Y |

```objective-c
QCloudGetObjectRequest* getObjectRequest = [[QCloudGetObjectRequest alloc] init];
            getObjectRequest.bucket = self.bucket;
            getObjectRequest.object = @"SSE-part-Upload";
            getObjectRequest.customHeaders = @{@"x-cos-server-side-encryption-customer-algorithm":@"AES256",
                                               @"x-cos-server-side-encryption-customer-key":excryptAES256Key,
                                               @"x-cos-server-side-encryption-customer-key-MD5":value
                                               };
            [getObjectRequest setFinishBlock:^(id outputObject, NSError *error) {
            ...
            [Expectation fulfill];
```
copy使用SSE-C加密过文件的时候，需要在自定义头部中加入下面3个值

|     头部名称      | 描述                     | 必填        |
| ----------------------------- | ----------------------------------------- | -- |
| x-cos-copy-source-server-side-encryption-customer-algorithm |  使用 SSE-C 则此头部必填 AES256 | Y |
| x-cos-copy-source-server-side-encryption-customer-key |  提供一个 256 位的 Base64 后的密钥 | Y |
| x-cos-copy-source-server-side-encryption-customer-key-MD5 |  加密后的 MD5，非源文件 MD5(copy源文件的reponse headers中会返回) | Y |

```objective-c
    QCloudCOSXMLCopyObjectRequest* request = [[QCloudCOSXMLCopyObjectRequest alloc] init];
        request.customHeaders = @{@"x-cos-copy-source-server-side-encryption-customer-algorithm":@"AES256",
                                  @"x-cos-copy-source-server-side-encryption-customer-key":excryptAES256Key,
                                  @"x-cos-copy-source-server-side-encryption-customer-key-MD5":@"pG5+osMhdqcA9NgNE4bybA=="
                                  };
        ...
        request.sourceAPPID = @"sourceAPPID"
...
如果headObject 的源文件是经过SSE-C加密的，需要在请求中加入以下3个头部：
...
```objective-c
    QCloudHeadObjectRequest* headObjectRequest = [[QCloudHeadObjectRequest alloc] init];
            headObjectRequest.customHeaders =  @{@"x-cos-server-side-encryption-customer-algorithm":@"AES256",
                                                 @"x-cos-server-side-encryption-customer-key":excryptAES256Key,
                                                 @"x-cos-server-side-encryption-customer-key-MD5":value
                                                 };

...
cos.headObject({
    ...
    // data.headers 没有 x-cos-object-type 字段需要配置 expose-headers，参考文档：https://cloud.tencent.com/document/product/436/13318
...
cos.headObject({
    ...
    if (err) return console.log(err);
    // 首先取到要追加的文件当前长度，即需要上送的 Position
    ...
    function(err, data) {
        if (err) return console.log(err);
        // 也可以取到下一次上传的 position 继续追加上传
        // data.headers 没有 x-cos-next-append-position 字段需要配置 expose-headers，参考文档：https://cloud.tencent.com/document/product/436/13318

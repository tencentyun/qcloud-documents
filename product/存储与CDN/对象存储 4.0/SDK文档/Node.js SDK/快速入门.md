...
3. 使用永久密钥初始化。
将 SecretId、SecretKey、Bucket 和 Region 修改为你实际环境下的值，测试上传文件，请参考以下示例代码。
```javascript
// 引入模块
...
4. 使用临时密钥初始化。临时密钥生成和使用请参考 [临时密钥生成及使用指引](https://cloud.tencent.com/document/product/436/14048)。
	Node.js SDK 还支持通过传入临时密钥进行初始化，请参考以下示例代码。
```
// 使用临时密钥创建实例
var cos = new COS({

...
2. When Local DNS resolves www.test.com, it will find that CNAME [www.test.\*.\*cdntip.com](www.test.\*.\*cdntip.com) has been configured, so the resolution request will be sent to Tencent DNS (GSLB). GSLB is a scheduling system independently developed by Tencent Cloud and will allocate the best node IP to the request;

## Product Overview
Tencent Cloud Dynamic Site Accelerator (DSA) is used to accelerate the transmission of data from a user to the origin server using the linkage optimization technology independently developed by Tencent Cloud through high-quality nodes across the globe.
...
6. The node detects the optimal path from internal network to origin server with the intelligent detection algorithm, and sends the request to the origin server through this path;
7. Upon receiving the request, the origin server sends the dynamic data to the DSA node based on the request;
8. DSA sends the result from the origin server to the user transparently via the optimal internal linkage.

您可以对已经接入 DSA 的域名进行 HTTPS 证书配置。DSA 支持配置您已有的证书，或腾讯云 [SSL 证书管理](https://console.qcloud.com/ssl) 控制台中托管或颁发的证书。
...
## 配置证书
登录 [DSA 控制台](https://console.qcloud.com/dsa)，在左侧菜单栏中单击【证书管理】进入 **证书管理** 页面。页面中单击【配置证书】进入 **配置证书** 页面。

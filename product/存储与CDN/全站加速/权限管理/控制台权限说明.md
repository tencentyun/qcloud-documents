腾讯云提供的访问管理 Web 服务，用于帮助客户安全地管理腾讯云账户的访问权限，资源管理和使用权限。通过访问管理，您可以创建、管理和销毁用户（组），并通过身份管理和策略管理控制哪些人可以使用哪些腾讯云资源。ECDN 支持资源级授权，通过指定 Action 和 Resource 创建自定义策略，可直接调用 API 接口对加速域名等资源进行操作。控制台功能点与 Action 映射关系见下文说明。

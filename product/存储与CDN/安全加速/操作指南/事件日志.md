...
| waf_attack_payload  | WAF日志特有。Web 攻击请求的攻击内容                          | /cache.txt?id=1 or 5=5                                       |
| waf_attack_type     | WAF日志特有。Web 攻击请求的攻击类型                          | xss                                                          |
| waf_attack_location | WAF日志特有。Web 攻击请求发生的位置，例如请求参数、URI、IP 等 | REQUEST_URI_RAW                                              |
| cookie              | WAF日志特有。请求 Cookie 头部信息                            | isQcloudUser=false; language=zh                              |
| req_header          | WAF日志特有。请求头内容                                      | 略                                                           |

...
| 请求参数     | 描述    | 是否必选      |
|  -----|  ---| -----|
|LibraryId|    媒体库 ID，在媒体托管控制台创建媒体库后获取，请参阅请参阅 [创建媒体库]()  |  是  |
...
|Period|令牌有效时长及每次使用令牌后自动续期的有效时长，可选参数，单位为秒，有效值为正整数，传入其他值将使用默认值 86400（24 小时），传入小于 1200 的值将自动使用最小值 1200（20 分钟）| 否 | 
...
|Grant|授予的权限，如为空则只拥有读取权限，可指定此参数在只读基础上同时附加下述多个权限，并使用英文逗号(,)分隔| 否|
    ...
    |create_directory|拥有创建目录或相簿权限|否|

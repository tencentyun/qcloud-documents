...
[标签](https://tcloud-doc.isd.com/document/product/651) 是腾讯云提供的用于标识云上资源的标记，是一个键-值对（Key-Value）。标签可以帮助您从各种维度（例如业务，用途，负责人等）方便的对云资源进行分类管理。需要注意的是，腾讯云不会使用您设定的标签，标签仅用于您对腾讯云资源的管理。本文档指导您对文件存储资源进行编辑标签的操作。

...
吞吐型是采用分层架构的并行文件存储，可实现更灵活的带宽扩展，提供SMB协议的访问方式。满足用户小容量、大带宽的存储需求。提供三副本强一致架构能力，每一份写入文件系统的数据确保成功落盘，并位于不同机架的三台独立物理服务器，同时提供接入机热迁移的机制，保障数据的可靠性和服务的高可用。适用于渲染、游戏战斗服务器、非线性编辑等读密集型场景。


<table> 
    <tr align="center">
        <td width="5%" >产品大类</td>
        <td width="10%" colspan="2">标准型</td>
        <td width="10%" colspan="2">性能型</td>
        <td width="10%" colspan="2">吞吐型</td>
    <tr align="center">
        <td width="5" >产品名称</td>
        <td width="10%" >通用标准型</td>
        <td width="10%" >Turbo 标准型</td>
        <td width="10%" >通用性能型</td>
        <td width="10%">Turbo 性能型 </td>
        <td width="10%">吞吐型 </td>
    </tr>
    <tr align="center">
        <td>产品定位</td>

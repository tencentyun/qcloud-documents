...
| 操作类型 | 说明                                                         |
| -------- | ------------------------------------------------------------ |
| 设置     | 变更指定响应头部参数的取值为设置后的值。<br/>若设置的头部不存在，则会增加该头部。<br/>若存在多个重复的头部参数，则会全部变更，同时合并为一个头部。<br/> 即当配置规则为【设置 x-cdn: value1】，若请求中包含有多个 x-cdn 头部，则多个头部均会变更，合并为一个头部 x-cdn: value1。<br/>设置同一头部参数时，多条规则支持调整优先级，底部优先级高于顶部。|

...
- 文件 Content-Type 类型的内容默认为 text/html, text/xml, text/plain, text/css, text/javascript, application/json, application/javascript, application/x-javascript, application/rss+xml, application/xmltext, image/svg+xml, image/tiff，可按需自行配置，不可超过100组，不同组内容用 “;” 分隔。

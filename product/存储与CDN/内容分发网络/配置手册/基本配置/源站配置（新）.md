...
**回源 HOST**
即回源域名，CDN 节点在回源时，访问的源站 IP 地址下具体的站点域名。默认为当前加速域名，若接入泛域名，则默认为泛域名，且实际回源 Host 为访问域名。您可根据实际业务情况自行修改（注：COS 源时不可修改）。具体配置示例说明可见 [回源域名配置](https://cloud.tencent.com/document/product/228/41334#exp)。

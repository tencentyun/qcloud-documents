...
- 待重写回源 URL：以/开头，支持全路径匹配（例如：/test/a.jpg）和通配符 `*` 匹配（例如：/test/\*/\*.jpg）。
- 目标回源 Host：默认为当前域名，可修改为其他域名，不包含 `http://` 或 `https://` 头。
- 目标回源 Path：以/开头（例如：/newtest/b.jpg），通配符 `*` 可通过 `$n` 捕获（n=1,2,3...，例如：/newtest/$1/$2.jpg）。

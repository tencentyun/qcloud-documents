...
> + 若您看到的与下文不符，可参考 [IP 访问限频配置（旧）](https://cloud.tencent.com/document/product/228/6420)。

## 配置场景
若您希望对业务资源的访问来源进行控制，腾讯云 CDN 为您提供了 IP 访问限频配置。通过对用户端 IP 在每一个节点每一秒钟访问次数进行限制，可进行高频 CC 攻击抵御、防恶意用户盗刷等场景。

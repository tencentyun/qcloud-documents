...
5. 确保可通过`http://example.com/verification.html`访问至到该文件后，即可单击**验证**按钮进行验证。如果文件内的记录值与我们提供的记录值是一致的，即可验证通过；如果验证失败，请确保上诉文件链接可访问，并且您上传的文件为正确文件，可通过访问文件的链接与所下载的文件进行比对是否一致；

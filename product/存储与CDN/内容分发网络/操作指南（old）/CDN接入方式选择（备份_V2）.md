...
3）如果您在腾讯云开通的有面向对象存储（COS），可以考虑使用“COS源”方式接入，（参考CDN接入COS源）。腾讯云COS为您提供公共了免费50G存储空间进行数据托管，CDN回源到COS空间。
...
是否选择使用ftp托管源接入，可参考CDN接入方式选择

接入流程如下：

1） 点击“添加域名”
![](//mccdn.qcloud.com/img569b96c15e43a.png)
2） 选择ftp托管源接入
![](//mccdn.qcloud.com/img569b96cc53bd2.png)
3） 填入有效信息：
![](//mccdn.qcloud.com/img569b96d5e27f5.png)
注意：
a) ftp托管源方式下，您必须提供一个访问域名，使用CDN域名直接访问资源，会影响高级配置、统计分析等高级功能
b) 您的访问域名必须是备案过的域名，否则审核不通过。
c) ftp上传地址是您托管文件的空间地址，您需要用云服务帐号和密码登录，如未设置密码，需要到腾讯云官网管理中心设置。
d) 完成如下接入流程后，上传到FTP文件可以通过您的访问域名进行访问，通过CDN进行访问加速。FTP上传常用的工具请参考常用FTP客户端使用指引
e) FTP上传的限制：每个QQ号下的所有ftp托管接入的域名，共享6G的存储空间，文件个数不能超过10万个。
4）点击下一步，进入“填写配置信息”页面，可在页面上选择所需的基础配置，具体配置详情可在下方配置说明当中查看。
![](//mccdn.qcloud.com/img569b96e4c11ca.png)
5） 点击下一步，进入“确认信息”页面，确认接入域名的基本信息和配置信息，若需修改可点击上一步，若无需修改，可直接点击“提交”。
![](//mccdn.qcloud.com/img569b96f2d1ff4.png)
6）点击“提交”后，进入到“完成”页面，此时域名已经添加完成，还需进入到“接入管理”查看添加域名当前的状态。
![](//mccdn.qcloud.com/img569b970e80e0e.png)
7）添加完成时，域名处于审核状态，腾讯云的同学会对您的域名进行进一步审核，审核时间约10分钟。有疑问请联系客服或者大客户经理。

8） 审核通过后，您的域名进入开启状态，此时会分配对应的CDN域名。
![](//mccdn.qcloud.com/img569b971a2a459.png)
**9） 最后一步，您需要到接入域名的DNS服务商（如dnspod）处，把域名cname到cdn域名，生效后即可完成接入。**[详细操作指引请点击这里](http://bbs.qcloud.com/forum.php?mod=viewthread&tid=5090&extra=page=1)

10） 整个流程完成后，您就可以通过添加加速的域名来访问CDN了。
[常用FTP客户端使用指引](http://cdn.qcloud.com/res/ftp%E5%AE%A2%E6%88%B7%E7%AB%AF%E4%BD%BF%E7%94%A8%E4%B8%BE%E4%BE%8B.docx)

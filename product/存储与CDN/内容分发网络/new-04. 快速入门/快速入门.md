...
2.您需要到接入域名的 DNS 服务商处完成 CNAME 配置，配置方法请查阅 [CNAME配置](https://www.qcloud.com/doc/product/228/3121)。

3.验证CNAME是否生效：不同的DNS服务商，CNAME生效的时间略有不同，一般会在半个小时之内生效。您也可以通过 PING 或 DIG 的方式来查询CNAME是否生效：
- Linux下， dig 您的加速域名后看到后缀为 ```.cdn.dnsv1.com``` 的 CNAME 记录，则表示域名 CNAME 已生效。
- Windows下， ping 您的加速域名后转到后缀为 ```.cdntip.com``` 或 ```.tcdn.qq.com``` 的域名，则表示域名 CNAME 已生效。
...
> 若 X < 500 Mbps，则账单费用为 X &times; 1.1；
> 若 500 Mbps <= X < 5000 Mbps，则账单费用为 X &times; 1.0；
> 若 5000 Mbps <= X < 50000Mbps，则账单费用为 X &times; 0.9；
...
> 名词解释
> 日95带宽值：每五分钟一个带宽统计点，一天共288个，从大到小排序，去掉最高5%的点，剩余最大值即为当天的日95带宽；
> 带宽计费有效天：若当日95带宽值 > 1Kbps，则作为带宽计费有效天，若 <= 1Kbps，则该天无效，不纳入月度带宽计算。

+ **日峰带宽取月均**：CDN 每日带宽统计点共 288 个，对每一个有效天的峰值取平均，得到计费带宽，再根据商务签订的合同价格计算费用；

> 计算示例：
> 客户从 2017 年 2 月 15 日 正式开始计费，签订的合同价格为：P 元/Mbps/月
> 有效天：2 月的总天数为 28 天，假设每天产生的消耗 > 1Kbps，2 月的有效天为 14 天
> 这 14 天的每一天的 288 个统计点最大值为：Max_1、Max_2、Max_3... Max_14，则计费带宽为 Average(Max_1, Max_2, ..., Max_14)，2月的费用为：Average(Max_1, Max_2, ..., Max_14)  x  P  x  14 / 28
...
> 则计费带宽为这14天的所有统计点 14 \* 288个，去掉最高的 5% 的点，剩余统计点中最高的为 Max95，Max95即为计费带宽，2月的费用为：Max95 x P x 14 / 28

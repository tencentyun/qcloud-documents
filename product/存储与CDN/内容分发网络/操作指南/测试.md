...
<thead>
	<tr>
		...
		<th scope="col" style="width: 30%;text-align: center;font-weight:700;">描述</th>
		<th scope="col" style="width: 34%;text-align: center;font-weight:700;">适用场景</th>
...
<tbody>
	<tr>
		...
		<td>按每日的峰值带宽计费。每5分钟统计一个带宽数据，每日得到288个值，取其中的最大值。<br>付费方式：后付费按日结算<br>计费规则：采用阶梯到达模式<br>进制换算规则： 1Mbps = 1000Kbps</td>
	...
	<td colspan="1" rowspan="2" >流量计费</td>
		...
		<td>按每日从腾讯云 CDN 节点流出的实际流量计费。<br>付费方式：后付费按日结算<br>计费规则：月度阶梯累进结算<br>进制换算规则： 1Mbps = 1000Kbps</td>
	...
	<tr>
		...
		<td>付费方式：预付费模式，支持自动续订。<br>计费规则：可选择1个月或6个月有效期的流量包，流量包有效期内可叠加。<br>进制换算规则： 1GB = 1000MB</td>
...
<tbody>
	<tr>
		...
		<td>CDN 每日带宽统计点共288个，对每一个有效天的峰值取平均，得到计费带宽，再根据商务签订的合同价格计算费用。<br>进制换算规则： 1Mbps = 1000Kbps</td>
	...
	<tr>
		...
		<td>CDN 每日带宽统计点共288个，从当月1号起，每一个有效日（产生消耗）的所有统计点进行排序，去掉前5%的统计点，剩下的最大的统计点，即为计费带宽，再根据合同价格计算费用。<br>进制换算规则：
1Mbps = 1000Kbps</td>
		<td style="text-align: center;">参见 <a href="#m4">计费详情</a></td>
	</tr>
	<tr>
		<td style="text-align: center; ">月流量</td>
		<td>月度累计使用的所有流量和，根据合同价格计算费用。<br>进制换算规则：1GB = 1000MB</td>

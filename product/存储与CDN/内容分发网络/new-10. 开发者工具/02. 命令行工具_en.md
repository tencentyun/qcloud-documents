...
The cache expiration configuration of one domain is consisted of several entries of cache expiration configurations. Each entry is divided into three parameters: cache type,matching rule, and configured expiration time (in seconds). CDN provides four types:
...
+ 3: Full-path file. This means matching based on home page or matching a specified file. Examples: /index.html; test/*.jpg.

...
您可以在【访问控制】中找到 **过滤参数配置** 模块，设置是否过滤参数：
![](https://mc.qcloudimg.com/static/img/8eb651e5ced8a8285422be3ecfbcc346/access.png)
...
3. 假设用户再次请求 URL 为： ```http://www.test.com/1.jpg?version=1.2 ``` 的资源，此时由于过滤参数已经关闭，则无法全路径匹配内容，因此会再次回源进行资源的拉取。
...
3. 假设用户再次请求 URL 为： ```http://www.test.com/1.jpg?version=1.2``` 的资源，此时在节点实际查找的资源为 ```http://www.test.com/1.jpg```，节点已经缓存，因此会直接命中资源，并返回给用户。

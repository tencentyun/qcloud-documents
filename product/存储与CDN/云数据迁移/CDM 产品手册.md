...
- 交换机：客户提供用于连接外部设备（例如CDM-L80）的交换机  。
...
#### 2.4 填写 IP 等信息并保存
执行过程中会弹出vi编辑窗口，按照提示填写IP地址，网关和掩码信息，填写完毕之后保存。
...
例如，编号为 ABC123 的迁移设备对应的 IP 地址是`192.168.1.100，192.168.1.101`，将迁移服务器的 nfs 共享目录 `/dataseal`挂载到存储挂载点上的 `/mydata100 /mydata101`目录下，挂载点需依次执行命令： 
```
mount -t nfs 192.168.1.100:/dataseal /mydata100```和 ```mount -t nfs 192.168.1.101:/dataseal /mydata101
```

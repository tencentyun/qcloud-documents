智能视图计算平台（Intelligent Viewdata Computing）基于腾讯云 [对象存储](https://cloud.tencent.com/document/product/436)，使用 XML API，这是一种轻量级的、无连接状态的接口，调用此接口您可以直接通过 HTTP/HTTPS 发出请求和接受响应，实现与后台的交互操作。
...
|名称|	描述|
|---|---|
| APPID	|开发者访问智能视图计算平台服务时拥有的用户维度唯一资源标识，用以标识资源，可在 [API 密钥管理](https://console.cloud.tencent.com/capi) 页面获取|
...
| Bucket | 存储桶，智能视图计算平台中用于存储数据的容器。有关存储桶的进一步说明，请参见 [存储桶概述](https://cloud.tencent.com/document/product/436/13312) 文档|
| Object | 对象，智能视图计算平台中存储的具体文件，是存储的基本实体 |
...
要使用腾讯云智能视图计算平台，需要先执行以下步骤：

1. 在腾讯云 [智能视图计算平台控制台](https://console.cloud.tencent.com/iss) 开通服务，并创建项目。

...
如果创建的是 "通用型网关"，则需要从本地磁盘、DAS 或 SAN 存储中为网关 VM 分配磁盘。
...
如果创建的是 "通用型网关"，则购买完云服务器后，需要回到[CVM 控制台](https://console.cloud.tencent.com/cvm)。 在 CVM 控制台新建至少 2 块 10GB 以上的云硬盘并挂载到该主机上（网关正常运行至少需要2块以上磁盘，请根据业务需要选择缓存/上传缓冲磁盘大小，磁盘后期还可根据需要自行添加）。<span style="color: red;"> 注意，为了保障网关的读写性能，缓存磁盘的容量必须为上传缓冲磁盘容量的 1.5 倍或更高。</span>

...
文件网关 | 4核 CPU/16GB 内存/4Mbps 带宽 | 8核 CPU/32GB 内存/10Mbps 带宽 | 最少2块10G以上磁盘
...
根据您的业务使用场景，在页面中选择 "卷网关" 或 "NFS 文件网关" 或者 "SMB 文件网关"。

- 卷网关：提供 iSCSI 访问接口，本地缓存频繁访问的热数据，全部数据存储于 COS。
- NFS 文件网关：提供 NFS v3.0 / v4.0 访问协议，本地缓存频繁访问的热数据，全部数据存储于 COS。
- SMB 文件网关：提供 CIFS / SMB v2.0 / v3.0 访问协议，本地缓存频繁访问的热数据，全部数据存储于 COS。
- 磁带网关： 提供 iSCSI 访问接口，模拟虚拟磁带库（VTL），将归档存储作为最终的后端存储，为用户提供更持久、更低廉的存储。


其中，NFS 文件网关和 SMB 文件网关 除了 "通用型" 还提供 "高 IO 型" 性能的版本 （卷网关默认为 "通用型"，暂不提供高 IO 型） 。

- 通用型：适合于使用公网来上传及下载的场景。
- 高 IO 型：适合于高出口带宽或者开通专线的场景。注意：高 IO 网关主机内存最低配置为64GB。

![](https://mc.qcloudimg.com/static/img/072000dcd814d9d681b641915092838f/image.png)
...
如果创建的是 "通用型网关"，则需要从本地磁盘、DAS 或 SAN 存储中为网关 VM 分配磁盘。<p style="color: red;"> 缓存区:上传缓冲区至少要达到3:2，否则网关将无法正常运行。<p>
...
如果创建的是 "通用型网关"，则购买完云服务器后，需要回到[CVM 控制台](https://console.cloud.tencent.com/cvm)。 在 CVM 控制台新建至少 2 块 10GB 以上的云硬盘并挂载到该主机上（网关正常运行至少需要2块以上磁盘，请根据业务需要选择缓存/上传缓冲磁盘大小，磁盘后期还可根据需要自行添加）。<span style="color: red;"> 注意，为了保障网关的读写性能，缓存磁盘的容量必须为上传缓冲磁盘容量的 1.5 倍。</span>
...
对于通用型的卷网关、NFS 文件网关、CIFS/SMB 文件网关，在此处可以选择数据写入模式（高IO型网关无该设置项），即，
...
<p style="color: red;"> 注意： 网关正常运行，**至少需要配置一个 "上传缓冲区" 和 一个 "缓存"，且缓存磁盘容量必须是上传缓冲区容量的 1.5 倍**。 若您没有在创建时为网关分配本地磁盘，网关将处于 "待配置" 状态，需要等待配置本地磁盘后才能正常运行，请参照 [管理磁盘配置](https://cloud.tencent.com/document/product/581/9485#.E7.AE.A1.E7.90.86.E6.9C.AC.E5.9C.B0.E7.A3.81.E7.9B.985) 进行配置。<p> 

    
### 高 IO 型网关
无需配置本地磁盘，直接点击【完成】按钮即可结束创建向导。    

...
存储网关 CSG 提供两种计费模式，包年包月（预付费）以及按量计费（后付费）。您可以根据需求进行选择，详情请参见 [计费方式](https://cloud.tencent.com/document/product/581/45754)。
...
> !当前版本不支持对包年包月网关进行退费，请谨慎删除资源，以免造成不必要的损失，我们将在下一版本加入退费规则。

此外由于 CSG 的存储基于 [对象存储 COS](https://cloud.tencent.com/document/product/436)，所以存储部分的费用会在 COS 的账单中给出。具体费用可参见 [COS 产品定价](https://buy.cloud.tencent.com/price/cos)。

## 计费项

- 对于包年包月的存储网关 CSG，我们只根据您所选购买时长一次性收取费用，没有额外的计费项。

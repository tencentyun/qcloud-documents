...
您可以从各种维度（例如，业务、用途、负责人等）使用标签对存储网关（Cloud Storage Gateway，CSG）的资源进行分类管理，标签可以非常方便地帮助您筛选出您所需要的资源。标签键值对会严格按字符串进行解析匹配，腾讯云不会使用您设定的标签，标签仅用于您对资源的管理。更多相关使用限制请参见 [使用限制](https://cloud.tencent.com/document/product/651/13354)。

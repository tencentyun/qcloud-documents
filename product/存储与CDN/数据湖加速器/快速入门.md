...
1. 新建本地目录并进入该目录下(您也可以按需选择其他目录)，从官方仓库下载 GooseFS 安装包到本地。从官方仓库下载链接：[goosefs-1.3.0-bin.tar.gz](https://downloads.tencentgoosefs.cn/goosefs/1.3.0/release/goosefs-1.3.0-bin.tar.gz)。
```
$ cd /usr/local
$ mkdir /service
$ cd /service
$ wget https://downloads.tencentgoosefs.cn/goosefs/1.3.0/release/goosefs-1.3.0-bin.tar.gz
```
...
```
> 注意：配置`goosefs.worker.tieredstore.level0.dirs.path`该路径时，需要先新建好这一路径。

## 启用 GooseFS

1. 启用 GooseFS 前，需要进入到 GooseFS 目录下执行启动指令：
```shell
$ cd /usr/local/service/goosefs-1.3.0

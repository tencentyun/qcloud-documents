...
> 写缓存策略设置后如果需要变更，需要审慎评估缓存数据的重要性，如果数据重要，建议先保证缓存数据已经持久化，否则缓存数据可能出现丢失情况。比如将写缓存从`MUST_CACHE`变更到`CACHE_THROUGH`后，如果未调用`persist`指令持久化数据，那么即将淘汰的数据无法写入到底层，会出现丢失的情况。

...
**内容分发网络 CDN**：内容分发网络 CDN 和云解析服务将共同作用，使您的域名和网站内容实现绑定，同时为您的静态网站加速，降低访问延迟，提高可用性。

**云解析**：您可以通过云解析将您的域名和网站内容绑定在一起，实现使用自定义域名访问您的静态网站的目的。

实践过程中，域名系统 (DNS)、CNAME 记录将很有用。对这些概念的详细说明不在本指南的范围内。（但是您可以通过术语表进行了解。）
...
### 步骤 4：开通 COS 的静态网站功能
绑定自定义域名之后，您还需要开通 COS 的静态网站托管功能，以实现使用自定义域名，通过浏览器直接访问您的网站的目的。

在 COS 控制台，点击您的网站对应的 Bucket。进入到 Bucket 后，点击 基础配置 ，找到静态网站，点击`编辑`按钮开通 COS 的静态网站托管功能并保存，如下图所示。开启静态网站功能的具体配置，请参考[配置静态访问](https://www.qcloud.com/document/product/436/6249)。
![开通静态网站托管](//mc.qcloudimg.com/static/img/03ccad699598098f5eb57c6760b4818d/image.png)

### 步骤 5：测试

若要验证该网站可以在您的浏览器直接访问，请尝试以下 URL：

http://www.example.com ——显示 www.example.com 存储桶中的索引文档（index.html）。
http://www.example.com/test.html （不存在该文件） ——显示 www.example.com 存储桶中的错误文档（error.html）。

...
// 2 设置 Bucket 的区域, 详细信息请参见 可用地域(https://www.qcloud.com/document/product/436/6224) 文档
...
另外在分块文件上传过程中还可以获取已上传分块(listParts)和终止分块上传(abortMultipartUpload)。
...
|  headerMap   | Map&lt;String, String&gt;  |  无   |               要签发的 HTTP 头部               |
...
strBuilder.append("http://").append(bucketName).append("-").append(appid).append(".")
     .append(Region.CN_NORTH.getRegionName()).append(".myqcloud.com")

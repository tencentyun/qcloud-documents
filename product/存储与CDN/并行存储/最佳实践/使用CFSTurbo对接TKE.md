## 操作场景
本文为您介绍如何使用并行存储对接容器服务（Tencent Kubernetes Engine，TKE）集群。


## 前提条件

- 容器服务的宿主机节点满足 Turbo 系列兼容的操作系统。
- 已在所有容器服务节点安装 Turbo 的私有客户端，推荐使用 pshell 工具进行批量操作。
相关的操作系统兼容列表及私有客户端安装方式，可参考 [在 Linux 客户端上使用 CFS Turbo 文件系统](https://cloud.tencent.com/document/product/1546/68682) 文档。
...
6. [](id:step6)在“集群APIServer信息”栏中，将内网访问设置为 <img src="https://main.qcloudimg.com/raw/557ab76a34a9a1af96a81237e12882c3.png" style="margin:-3px 0px"/>，单击 <img src="https://main.qcloudimg.com/raw/6603ab4f907562addb1c01596c6296cd.png" style="margin:-3px 0px"/>，复制配置 host 的命令。
...
2. 进入 `kubernetes-csi-tencentcloud/deploy/cfsturbo/kubernetes/` 目录，分别将 csi-node-rbac.yaml、csi-node.yaml 和 csidriver.yaml 文件上传至可访问容器服务集群的 CVM 管理节点中。
...
```
其中，host为：10.0.1.16，表示 Fsid 为：d3dcc487。
修改完成的脚本示例如下：
```
apiVersion: v1
...
若状态（STATUS）持续为 ContainerCreating，即表示创建失败。您可在容器服务控制台的事件中，查看失败原因。

...
本文为您介绍如何利用云函数，通过 API 网关调用，将已有的录播视频或者 RTMP 直播流推送到 TRTC 房间进行直播。如您需开启推流直播的实时记录，可以选择使用 Redis，则会将进度实时写入 Redis。工作流程如下图所示：
...
>? 
>- 如果 redis 值为 false，直接从 videoSrc 视频源拉流下来进行直播推流，直播流将永远都是从最新开始。
>- 如果 redis 值为 true，对于同一个 videoSrc 视频源，API 网关将先在 redis 中查询是否有上一次直播流推流记录：
>  - 若存在记录，则恢复上一次推流。
>  - 若无记录，则从新开始推流。直播推流进度通过回调实时写入 redis。
...
5. 切换至 [实时音视频控制台](https://console.cloud.tencent.com/trtc/monitor)，在“监控仪表盘”页面单击房间 ID，查看推流监控详情信息。如下图所示：
	 ![](https://main.qcloudimg.com/raw/dd7bfa35793bc3248ad6d0ae7f1e3ccf.png)
6. 如果在推流过程中想要停止推流，可以调用 [终止异步函数接口](https://cloud.tencent.com/document/api/583/52500) InvokeRequestId 参数（**必须开启状态追踪**）。

...
<dx-tabs>
::: 方式1：使用 YAML
下面是为 PiggyMetrics 创建 Secret 的 yaml。使用 yaml 时需要注意一点：Secret 的 value 需要是 base64 编码后的字符串。
<dx-codeblock>
:::  yaml
# 创建 Secret
apiVersion: v1
...
下面是 account-service Deployment 的 YAML 参数说明：
...
PiggyMetrics 的前端页面和 gateway 打包在一块，需要对外提供服务，因而指定 LoadBalancer 类型的 ServiceType。TKE 对 LoadBalancer 模式进行了扩展，通过 Annotation 注解配置 Service，可实现更丰富的负载均衡能力。

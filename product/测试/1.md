...
本最佳实践以 Q 云书城（QCBM：Q Cloud Book Mall）项目为例，详细介绍 dubbo 应用托管到 TKE 的整个过程。QCBM 首页如下图展所示：
...
Q云书城的部署和代码托管在 coding 上，详情请前往[ QCBM](https://github.com/TencentCloud/container-demo/tree/main/dubbo-on-tke) 查看。
...
由于 QCBM 只是个 Dubbo 容器化的 demo 项目，因而个人版完全满足需求。但对于企业用户，推荐使用 [企业版 TCR](https://console.cloud.tencent.com/tcr)。有关镜像仓库的具体操作，请参见 [镜像仓库基本操作](https://cloud.tencent.com/document/product/1141/41811) 文档。
...
- 其中命名 namespace 是为了方便镜像管理使用，可以按项目取名，本文采用 qcbm 表示 Q云书城 项目下的所有镜像。
...
1. 在 [容器服务控制台](https://console.cloud.tencent.com/tke2) 。
2. 选择【镜像仓库】>【个人版】> 【命名空间】，新建一个命名空间 QCBM。QCBM 项目所有的镜像都存放于该命名空间下。
...
上传镜像需要执行以下两个步骤：登录腾讯云 registry 和 上传镜像，下面详细介绍 qcbm 镜像的上传过程。


1. 执行以下命令登录腾讯 云registry。
```sh
...
1. 创建完成后，在容器服务控制台的 [集群管理](https://console.cloud.tencent.com/tke2/cluster) 页面可以看到新建的集群信息。这里我们新建的集群名称为 qcbm-k8s-demo。
![](https://main.qcloudimg.com/raw/bd56002ab91548603dcb3e3e107511a2.png)
2. 选择集群 qcbm-k8s-demo 点击进入 **基本信息** 页面，可以看到整个集群的配置信息

## 1. 接口描述

接口请求域名： mrs.tencentcloudapi.com 。

图片转结构化对象
<div class="rno-api-explorer">
<div class="rno-api-explorer-inner">
        <div class="rno-api-explorer-hd">
            <div class="rno-api-explorer-title">
                推荐使用 API Explorer
            </div>
            <a href="https://console.cloud.tencent.com/api/explorer?Product=mrs&Version=2020-09-10&Action=ImageToObject" class="rno-api-explorer-btn" hotrep="doc.api.explorerbtn" target="_blank"><i class="rno-icon-explorer"></i>点击调试</a>
        </div>
        <div class="rno-api-explorer-body">
            <div class="rno-api-explorer-cont">
API Explorer 提供了在线调用、签名验证、SDK 代码生成和快速检索接口等能力。您可查看每次调用的请求内容和返回结果以及自动生成 SDK 调用示例。
            </div>
        </div>
    </div>
</div>


## 2. 输入参数

以下请求参数列表仅列出了接口请求参数和部分公共参数，完整公共参数列表见 [公共请求参数](https://tcloud-dev.oa.com/document/product/1428/15692?!preview&preview_docmenu=1&lang=cn&!document=1)。

| 参数名称        | 必选 | 类型                                                         | 描述                                                         |
| :-------------- | :--- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| Action          | 是   | String                                                       | 公共参数，本接口取值：ImageToObject。                        |
| Version         | 是   | String                                                       | 公共参数，本接口取值：2020-09-10。                           |
| Region          | 是   | String                                                       | 公共参数，详见产品支持的 [地域列表](https://tcloud-dev.oa.com/document/product/1428/15692?!preview&preview_docmenu=1&lang=cn&!document=1#.E5.9C.B0.E5.9F.9F.E5.88.97.E8.A1.A8)，本接口仅支持其中的: ap-shanghai |
| ImageInfoList.N | 是   | Array of [ImageInfo](https://tcloud-dev.oa.com/document/product/1428/46912?!preview&preview_docmenu=1&lang=cn&!document=1#ImageInfo) | 图片列表                                                     |
| HandleParam     | 是   | [HandleParam](https://tcloud-dev.oa.com/document/product/1428/46912?!preview&preview_docmenu=1&lang=cn&!document=1#HandleParam) | 图片处理参数                                                 |
| Type            | 是   | Integer                                                      | 图片类别                                                     |
| BusinessId      | 是   | String                                                       | 业务Id                                                       |
| IsUsedClassify  | 是   | Boolean                                                      | 是否使用分类引擎                                             |

## 3. 输出参数

| 参数名称  | 类型                                                         | 描述                                                         |
| :-------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| Template  | [Template](https://tcloud-dev.oa.com/document/product/1428/46912?!preview&preview_docmenu=1&lang=cn&!document=1#Template) | 报告结构化结果 注意：此字段可能返回 null，表示取不到有效值。 |
| RequestId | String                                                       | 唯一请求 ID，每次请求都会返回。定位问题时需要提供该次请求的 RequestId。 |

## 4. 示例
### 输入示例
<dx-codeblock>
:::  url
https://mrs.tencentcloudapi.com/?Action=ImageToObject
&Type=0&BusinessId=10005&IsUsedClassify=true&HandleParam.OcrEngineType=2&HandleParam.IsReturnText=false&HandleParam.RotateTheAngle=0.0&HandleParam.AutoFitDirection=false&HandleParam.AutoOptimizeCoordinate=true&HandleParam.IsScale=false&HandleParam.ImageOriginalSize=310006&HandleParam.ScaleTargetSize=300000&ImageInfoList.0.Id=1&ImageInfoList.0.Url=http://wenzhentest-1252949230.cos.ap-guangzhou.myqcloud.com/8e0ee7d44576f057f7866d3f5f10xxxx&ImageInfoList.0.Base64=
&<公共请求参数>
:::
</dx-codeblock>

### 输出示例
<dx-codeblock>
:::  json
{
    "Response": {
        "RequestId": "f9be6dc1-0cf2-4b0a-b57b-e7d3662fada8",
        "Template": {
            "PatientInfo": {
                "Name": "",
                "Sex": "",
                "Age": "",
                "Phone": "",
                "Address": "",
                "IdCard": ""
            },
            "ReportInfo": {
                "Hospital": "华中科技大学同济医学院附属协和医院",
                "DepartmentName": "",
                "BillingTime": "",
                "ReportTime": "",
                "InspectTime": "2020/2/9",
                "CheckNum": "",
                "ImageNum": "",
                "RadiationNum": "",
                "TestNum": "",
                "OutpatientNum": "",
                "PathologyNum": "",
                "InHospitalNum": "",
                "SampleNum": "",
                "SampleType": "",
                "MedicalRecordNum": "",
                "ReportName": "CT检查报告单",
                "UltraNum": ""
            },
            "Check": {
                "Desc": {
                    "Text": "右肺上叶后段、下叶背段见少许极淡薄磨玻璃密度影，两肺门影不大;气管和支气管大分支内腔通畅;纵隔内未见明显增大淋巴结。",
                    "TuberInfos": [
                        {
                            "Part": {
                                "Desc": "部位",
                                "Src": "下叶背段",
                                "IndexRow": 0,
                                "IndexCol": [
                                    7
                                ],
                                "PartValue": "",
                                "PartDirection": "右方",
                                "TissueValue": "下叶",
                                "TissueDirection": "",
                                "Upper": "支气管和肺",
                                "Value": "右方肺下叶"
                            },
                            "Edge": null,
                            "Shape": null,
                            "Type": {
                                "Desc": "类型",
                                "Src": "极淡薄磨玻璃密度影",
                                "IndexRow": 0,
                                "IndexCol": [
                                    14
                                ],
                                "Value": "磨玻璃影",
                                "Positive": "有",
                                "Type": ""
                            },
                            "CDFI": null,
                            "SizeList": null,
                            "Metabolism": null,
                            "LymphGland": null,
                            "LymphDoor": null,
                            "SkinMedulla": null,
                            "InnerEcho": null,
                            "RearEcho": null,
                            "Envelope": null,
                            "Calcification": null,
                            "Enhancement": null,
                            "AspectRatio": null
                        },
                        {
                            "Part": {
                                "Desc": "部位",
                                "Src": "纵隔",
                                "IndexRow": 0,
                                "IndexCol": [
                                    45
                                ],
                                "PartValue": "",
                                "PartDirection": "",
                                "TissueValue": "",
                                "TissueDirection": "",
                                "Upper": "心脏、纵隔和胸膜",
                                "Value": "纵隔"
                            },
                            "Edge": null,
                            "Shape": null,
                            "Type": {
                                "Desc": "类型",
                                "Src": "增大淋巴结",
                                "IndexRow": 0,
                                "IndexCol": [
                                    52
                                ],
                                "Value": "淋巴结",
                                "Positive": "无",
                                "Type": ""
                            },
                            "CDFI": null,
                            "SizeList": null,
                            "Metabolism": null,
                            "LymphGland": null,
                            "LymphDoor": null,
                            "SkinMedulla": null,
                            "InnerEcho": null,
                            "RearEcho": null,
                            "Envelope": null,
                            "Calcification": null,
                            "Enhancement": null,
                            "AspectRatio": null
                        }
                    ]
                },
                "Summary": {
                    "SymptomInfos": [
                        {
                            "Grade": null,
                            "Part": {
                                "Desc": "部位",
                                "Src": "右肺上、下叶",
                                "IndexRow": 0,
                                "IndexCol": [
                                    0
                                ],
                                "PartValue": "",
                                "PartDirection": "右方",
                                "TissueValue": "下叶",
                                "TissueDirection": "",
                                "Upper": "支气管和肺",
                                "Value": "右方肺上方下叶"
                            },
                            "Symptom": {
                                "Desc": "病变",
                                "Src": "磨玻璃密度影",
                                "IndexRow": 0,
                                "IndexCol": [
                                    8
                                ],
                                "Value": "磨玻璃影",
                                "Positive": "",
                                "Type": "占位"
                            },
                            "Attrs": null
                        }
                    ],
                    "Text": "右肺上、下叶少许磨玻璃密度影，考虑少许炎症;建议病原学进一步检查。\n请结合临床及随访复查。"
                },
                "InspectionReport": {
                    "Desc": {
                        "Text": "右肺上叶后段、下叶背段见少许极淡薄磨玻璃密度影，两肺门影不大;气管和支气管大分支内腔通畅;纵隔内未见明显增大淋巴结。",
                        "TuberInfos": [
                            {
                                "Part": {
                                    "Src": "下叶背段"
                                }
                            },
                            {
                                "Part": {
                                    "Src": "纵隔"
                                }
                            }
                        ]
                    },
                    "Summary": {
                        "Text": "右肺上、下叶少许磨玻璃密度影，考虑少许炎症;建议病原学进一步检查。\n请结合临床及随访复查。"
                    }
                }
            },
            "Pathology": null,
            "Surgery": null,
            "DiagCert": null,
            "MedicalDoc": null,
            "DetailInfo": null,
            "Type": "check"
        }
    }
}

:::
</dx-codeblock>





## 5. 开发者资源

### 腾讯云 API 平台

[腾讯云 API 平台](https://cloud.tencent.com/api) 是综合 API 文档、错误码、API Explorer 及 SDK 等资源的统一查询平台，方便您从同一入口查询及使用腾讯云提供的所有 API 服务。

### API Inspector

用户可通过 [API Inspector](https://cloud.tencent.com/document/product/1278/49361) 查看控制台每一步操作关联的 API 调用情况，并自动生成各语言版本的 API 代码，也可前往 [API Explorer](https://cloud.tencent.com/document/product/1278/46697) 进行在线调试。

### SDK

云 API 3.0 提供了配套的开发工具集（SDK），支持多种编程语言，能更方便的调用 API。

- [Tencent Cloud SDK 3.0 for Python](https://github.com/TencentCloud/tencentcloud-sdk-python)
- [Tencent Cloud SDK 3.0 for Java](https://github.com/TencentCloud/tencentcloud-sdk-java)
- [Tencent Cloud SDK 3.0 for PHP](https://github.com/TencentCloud/tencentcloud-sdk-php)
- [Tencent Cloud SDK 3.0 for Go](https://github.com/TencentCloud/tencentcloud-sdk-go)
- [Tencent Cloud SDK 3.0 for NodeJS](https://github.com/TencentCloud/tencentcloud-sdk-nodejs)
- [Tencent Cloud SDK 3.0 for .NET](https://github.com/TencentCloud/tencentcloud-sdk-dotnet)
- [Tencent Cloud SDK 3.0 for C++](https://github.com/TencentCloud/tencentcloud-sdk-cpp)
- [Tencent Cloud SDK 3.0 for Ruby](https://github.com/TencentCloud/tencentcloud-sdk-ruby)

### 命令行工具

- [Tencent Cloud CLI 3.0](https://cloud.tencent.com/document/product/440/6176)

## 6. 错误码

以下仅列出了接口业务逻辑相关的错误码，其他错误码详见 [公共错误码](https://tcloud-dev.oa.com/document/product/1428/15694?!preview&preview_docmenu=1&lang=cn&!document=1#.E5.85.AC.E5.85.B1.E9.94.99.E8.AF.AF.E7.A0.81)。

| 错误码                                      | 描述                        |
| :------------------------------------------ | :-------------------------- |
| FailedOperation.PushUsageMessageError       | 用量消息推送失败。          |
| FailedOperation.ServiceNotOpen              | 服务未开通。                |
| FailedOperation.UnknownError                | 未知错误。                  |
| InternalError                               | 内部错误。                  |
| InternalError.ImageOcrError                 | 图片OCR错误。               |
| InternalError.ImageProcessError             | 图片处理错误。              |
| InternalError.ServerTimeOutError            | 服务调用超时。              |
| InternalError.StructionError                | 报告结构化错误。            |
| InternalError.TextClassifyError             | 报告分类错误。              |
| InvalidParameter                            | 参数错误。                  |
| InvalidParameter.AutoFitDirection           | AutoFitDirection参数错误。  |
| InvalidParameter.ImageInfoList              | ImageInfoList参数错误。     |
| InvalidParameter.ImageOriginalSize          | ImageOriginalSize参数错误。 |
| InvalidParameter.InvalidAction              | 请求Action无效。            |
| InvalidParameter.OcrEngineType              | OcrEngineType参数错误。     |
| InvalidParameter.RotateTheAngle             | RotateTheAngle参数错误。    |
| InvalidParameter.Text                       | Text参数错误。              |
| InvalidParameterValue.ImageCodeInvalid      | 图片base64不合法。          |
| InvalidParameterValue.ImageURLInvalid       | 图片链接不可用。            |
| UnauthorizedOperation.PermissionDeniedError | 接口无权限。                |
| UnsupportedOperation.UnSupportThisType      | 当前报告类型不支持。        |

## 简介
腾讯会议 Postman模板调试工具是为方便开发者进行腾讯会议 Rest API 接口联调而提供第三方开发调试工具。目前该工具支持OAuth鉴权与接口调试。下文将描述如何利用 Postman 工具调试 REST API。

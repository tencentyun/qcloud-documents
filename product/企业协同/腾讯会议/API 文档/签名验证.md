...
|X-TC-Registered|Integer|否|该字段为非必填字段，表示是否启用了企业用户管理功能。请求头不带该字段，缺省认为未启用企业用户管理功能。请求头带该字段，则：<li>取值为0：未启用企业用户管理功能。 <li>取值为1：启用企业用户管理功能。用户调用会议管理 API 前，须先调用企业用户管理功能 API 创建用户。|

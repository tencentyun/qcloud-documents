...
<td>signature</td>
<td>String</td>
<td>加密签名，signature 的计算结合了企业填的 token、timstamp、nonce、check_str，签名计算方法请参考 <a href="https://tcloud-doc.isd.com/document/product/1095/51612。">签名校验。</a></td>
</tr>
</tbody></table>
...
2. 根据已有的 token，结合上述 timestamp、nonce、check_str 参数计算签名，并与 signature 参数对比是否一致，一致表示调用合法，签名计算方法请参考 [签名校验](https://tcloud-doc.isd.com/document/product/1095/51612)；
...
<td>signature</td>
<td>String</td>
<td>加密签名，signature 的计算结合了开发者填的 token、timstamp、nonce、消息体，签名计算方法请参考 <a href="https://tcloud-doc.isd.com/document/product/1095/51612。">签名校验。</a></td></td>
</tr>
</tbody></table>
...
3. 正确响应本次请求：
 ...
 - 当响应的 HTTP 头部返回200且响应内容为 successfully received callback 字符串（不能加引号、换行符），则表示回调成功，其他情况一律当做失败并也会在1分、3分、6分后重新发起请求，重试三次。

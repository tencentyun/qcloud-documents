...
| allow_in_before_host          | 否   | boolean | 允许成员在主持人进会前加入会议 true：允许 false：不允许默认值 true。 |
| auto_in_waiting_room          | 否   | boolean | 开启等候室true：开启 false：不开启默认值 false。                 |
| allow_screen_shared_watermark | 否   | boolean | 开启屏幕共享水印 true：开启false：不开启默认值 false。           |
| only_enterprise_user_allowed  | 否   | boolean | 是否仅企业内部成员可入会<br> true：仅企业内部用户可入会 <br>false：所有人可入会默认值 false。 |

## 接口描述
**描述**：快速创建/预定一个会议。其中，企业 secert 鉴权用户可创建该用户所属企业下的会议；OAuth2.0 鉴权用户只能创建该企业下 OAuth2.0 应用的会议。当您想实时监测会议创建状况时，您可以通过订阅 [会议创建的事件](https://cloud.tencent.com/document/product/1095/51614)，接收事件通知。
**调用方式**：POST
**接口请求域名**：
```Plaintext
...
|meeting_room_info_list    | 否   | Object       | 会议室列表。                                                     |

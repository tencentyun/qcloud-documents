## 接口描述
**描述**：快速创建/预定一个会议。其中，企业 secert 鉴权用户可创建该用户所属企业下的会议；OAuth2.0 鉴权用户只能创建该企业下 OAuth2.0 应用的会议。用户必须是注册用户，请求头部 X-TC-Registered 字段必须传入为1。当您想实时监测会议创建状况时，您可以通过订阅 [会议创建](https://cloud.tencent.com/document/product/1095/51614) 的事件，接收事件通知。
**调用方式**：POST
**接口请求域名**：
```Plaintext

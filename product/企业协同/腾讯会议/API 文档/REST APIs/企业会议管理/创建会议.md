...
|invitees| 否|[UserObj 数组](#UserObj) |会议邀请的参会者，可为空。<br>注意：仅腾讯会议商业版和企业版可指定主持人。|
...
| auto_record_type  | 否   |String | 自动会议录制类型。该参数依赖企业账户设置，当企业强制设置后，该参数按照企业配置生效，且仅客户端2.7及以上版本可生效。<br>none：禁用，表示不开启自动会议录制。<br>local：本地录制，表示主持人入会后自动开启本地录制。<br>cloud：云录制，表示主持人入会后自动开启云录制。|
...
|participants|UserObj 数组 |邀请的参会者用户 ID（企业用户为企业自定义账号；个人用户为用户名、手机号等；oauth2.0 鉴权用户为 openId）。<br>注意：仅腾讯会议商业版和企业版可指定主持人。|
|user_non_registered|String 数组 |邀请的参会者中未注册用户。<br>注意：仅腾讯会议商业版和企业版可指定主持人。|

...
|hosts| 否 |[UserObj 数组](#UserObj) |会议指定主持人的用户 ID，如果无指定，主持人将被设定为参数 userid 的用户，即 API 调用者。|
...
| auto_record_type  | 否   |String | 自动会议录制类型。该参数依赖企业账户设置，当企业强制设置后，该参数按照企业配置生效。<br>none：禁用，表示不开启自动会议录制。<br>local：本地录制，表示主持人入会后自动开启本地录制。<br>cloud：云录制，表示主持人入会后自动开启云录制。|
...
|hosts| UserObj 数组 |会议主持人的用户 ID，如果无指定，主持人将被设定为上文参数 userid 的用户，即 API 调用者（企业内部请使用企业唯一用户标识；OAuth2.0 鉴权用户请使用 openId）。|
|participants|UserObj 数组 |邀请的参会者用户 ID（企业用户为企业自定义账号；个人用户为用户名、手机号等；oauth2.0 鉴权用户为 openId）。|

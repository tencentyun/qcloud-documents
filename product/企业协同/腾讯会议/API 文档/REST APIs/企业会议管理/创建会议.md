...
| 参数名称 | 必选 | 参数类型| 参数描述|
|---------|---------|---------|---------|
| userid| 是 | String |调用方用于标识用户的唯一 ID（企业内部请使用企业唯一用户标识；OAuth2.0 鉴权用户请使用 openId）。<br>企业唯一用户标识说明：<br>1. 企业对接 SSO 时使用的员工唯一标识 ID；<br>2. 企业调用创建用户接口时传递的 userid 参数。 |
...
|invitees| 否|[UserObj 数组](#UserObj) |调用方用于标识用户的唯一 ID，仅支持邀请与会议创建者同企业的成员（企业内部请使用企业唯一用户标识；OAuth2.0 鉴权用户请使用 openId），该会议将添加至邀请成员的会议列表中。<br>企业唯一用户标识说明：<li>企业对接 SSO 时使用的员工唯一标识 ID。<li>企业调用创建用户接口时传递的 userid 参数。<br>注意：仅腾讯会议商业版和企业版可邀请参会者，邀请者列表仅支持300人；需要邀请超过300人的场景请调用 [设置会议邀请成员](https://cloud.tencent.com/document/product/1095/63659) 接口。</li>|

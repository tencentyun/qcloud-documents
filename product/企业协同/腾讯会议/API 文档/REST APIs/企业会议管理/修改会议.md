...
|invitees|否 |UserObj 数组 |邀请的参会者，可为空（企业内部请使用企业唯一用户标识；OAuth2.0 鉴权用户请使用 openId）。<br>注意：仅腾讯会议商业版和企业版可邀请参会者。|
|start_time|否 |String  |会议开始时间戳（单位秒）。|
|end_time|否 |String  |会议结束时间戳（单位秒）。|
|password（\*） |否 |String  |会议密码，可不填。如果将字段值改为空字符串""，则表示取消会议密码。|
...
| auto_record_type | 否   |String     |  自动录制类型，该参数依赖企业账户设置，当企业强制设置后，该参数按照企业配置生效，且仅客户端2.7及以上版本可生效。快速会议只能使用账户统一配置，该参数设置无效。 <br>none：禁用 <br>local：本地录制 <br>cloud：云录制<br> |

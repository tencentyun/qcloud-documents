...
1：操作者必须是会议的创建者、主持人或者联席主持人才可以设置。<br>2：使用 OAuth 2.0 鉴权方式时，调用该接口需要权限项：MANAGE_MEETING 查看和管理您的会议。
</dx-alert>
- **调用方式**：PUT
- **接口请求域名**：
```Plaintext
...
| video | 否   | Boolean | 	是否关闭视频：<br>false：关闭视频（默认值）。<br>true：开启视频， 仅支持 MRA 设备。                |

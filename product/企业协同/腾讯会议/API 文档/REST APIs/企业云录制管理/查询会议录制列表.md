...
**接口请求域名**：
```Plaintext
https://api.meeting.qq.com/v1/records?page_size={page_size}&page_index={page_index}&meeting_id={meeting_id}&meeting_code={meeting_code}&user_id={user_id}&from={from}&to={to}
```
...
| user_id       | 是   | String   | 用户 ID，当会议 ID 和会议 code 均为空时，表示查询用户所有会议的录制列表。 |

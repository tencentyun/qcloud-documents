...
**接口请求域名**：
```Plaintext
https://api.meeting.qq.com/v1/records?pagesize={page_size}&page_index={page_index}&meeting_id={meeting_id}=meeting_code={meeting_code}&user_id={user_id}&from={from}&to={to}
```

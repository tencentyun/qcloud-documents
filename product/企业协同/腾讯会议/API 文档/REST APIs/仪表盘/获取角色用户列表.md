...
**接口请求域名：**
```plaintext
https://api.meeting.qq.com/v1/users/list?page={page}&page_size={page_size}&role_code={role_code}
```
...
| 参数名称    | 必选 | 参数类型        | 参数描述       |
| ----------- | ---- | --------------- | -------------- |
| userid      | 是   | String          | 企业用户 userid。 |
| user_name   | 是   | String          | 企业用户 name。   |
| area        | 是   | String          | 手机区号。       |
| update_time | 是   | String          | 更新时间。       |
| avatar_url  | 否   | String          | 用户图像地址。   |
| role_name   | 否   | String          | 角色名称。       |
| role_code   | 否   | String          | 角色类型。       |
| phone       | 是   | String          | 手机号。         |
| email       | 否   | String          | 邮箱。           |
| status      | 否   | Integer         | 账号状态。       |
| departments | 否   | department_item | 部门对象列表。   |
...
| --------- | ---- | -------- | ---------------------------------------------- |
| id        | 是   | String   | 部门 ID。                                         |
| name      | 是   | String   | 部门 name。                                       |
| full_name | 是   | String   | 部门全称（包含全部上级部门名称，不包含公司名称）。 |
| is_main   | 是   | boolean  | 是否主部门：<br> true：是 <br>false：否                      |

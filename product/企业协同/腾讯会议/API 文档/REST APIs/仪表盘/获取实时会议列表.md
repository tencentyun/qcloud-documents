## 接口描述
**描述：**获取账户中正在进行中的会议列表，目前暂不支持 OAuth2.0 鉴权访问。
**请求方法：**GET
**接口请求域名：**
```plaintext
https://api.meeting.qq.com/v1/corp/meetings
```
...
| userid            | 是   | String   | 创建者 userid。                                 |
| nick_name         | 否   | String   | 创建者昵称。                                   |
| start_time        | 是   | Integer  | 会议开始时间，单位秒。                       |
| end_time          | 是   | Integer  | 会议结束时间，单位秒。                       |
| online_member_num | 否   | Integer  | 在线人数。                                     |
| meeting_state     | 否   | Integer  | 会议状态：<br>1：结束会议<br>2：实时会议<br>3：即将开始的会议 |
| meeting_type      | 否   | Integer  | 会议类型。                                     |
...
```plaintext
GET /v1/corp/meetings?meeting_state=2&page_size=20&page=1
```

...
|X-TC-Registered|Integer|否|当未使用企业通讯录创建会议时，该字段为非必填字段，创建的会议将不出现在用户的会议列表中，可使用会议链接或会议号入会；<br>当启用企业通讯录创建会议时，该字段为必填字段，传入值为1，可指定主持人、可邀请参会成员，创建的会议可出现在用户的会议列表中。<br>启用企业通讯录说明：<br>1. 通过 SSO 接入腾讯会议帐号体系；<br>2. 通过调用接口创建企业用户；<br>3. 通过企业管理后台添加或批量导入企业用户。|

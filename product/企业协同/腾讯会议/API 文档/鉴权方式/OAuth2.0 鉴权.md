...
| X-TC-Registered | Integer  | 否       | 非必填字段，表示是否启用了腾讯会议的企业用户管理功能。请求头不带该字段或者该字段值为0，表示未启用企业用户管理功能。用户使用未注册的 userid 创建的会议，在会议客户端中无法看到会议列表，但可以正常使用会议短链接或会议号加入会议。<br>以下两种场景，请求头必须带该字段且值为1<br><li> 1. 企业用户通过 SSO 接入腾讯会议账号体系；<br><li>2. 企业用户通过腾讯会议企业用户管理创建用户。 |

一个完整的腾讯云 API 请求需要两类请求参数：公共请求参数和接口请求参数。下面将为您介绍腾讯云 API 请求需要用到的6个公共请求参数。
...
|SecretId	| 在 [云API密钥](https://console.cloud.tencent.com/cam/capi) 上申请的标识身份的 SecretId，一个 SecretId 对应唯一的 SecretKey , 而 SecretKey 会用来生成请求签名 Signature<br>详情请参见 [签名方法](https://cloud.tencent.com/document/product/215/1693) 	| String	| 是|
|Signature	| 请求签名，用来验证此次请求的合法性，需要用户根据实际的输入参数计算得出<br>计算方法详情请参见 章节	| String	| 是|

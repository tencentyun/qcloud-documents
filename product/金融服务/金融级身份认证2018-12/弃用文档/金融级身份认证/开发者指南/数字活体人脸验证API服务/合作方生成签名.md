## 准备步骤
- 前置条件：请合作方确保 SIGN ticket 已经正常获取，获取方式见 [SIGN ticket](https://cloud.tencent.com/document/product/655/31956) 获取 。
- 合作方根据本次人脸比对的如下参数生成签名，需要签名的参数信息如下：
...
## 基本步骤
1. 生成一个32位的随机字符串（字母和数字） nonce（登录时也要用到），将 webankAppId、orderNo、version、ticket、nonce 共5个参数的值进行字典序排序。

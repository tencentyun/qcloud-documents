...
|url|人脸核身完成后回调第三方 URL<br/>H5 人活体检测完成后的回调 URL，需要第三方提供完整 URL，且做 URL Encode<br/>完整 URL Encode 示例<br/>原URL：`https://idaop.webank.com`<br/>Encode 后：`http%3A%2F%2Fidaop.webank.com`|String|-|是|
|resultType|是否显示结果页面<br/>值为1：直接跳转回第三方的小程序<br/>null 或其他值：跳转刷脸小程序提供的结果页面|String|-|否|
|userId|用户 ID ，用户的唯一标识（不要带有特殊字符）|String|-|是|
|sign|使用本文生成的签名|String|40|是|
|mode|刷脸模式<br/>默认值为空：使用活体+身份验证模式<br/>定值为 ‘digitlive’：使用单活体模式<br/>|String|-|否|

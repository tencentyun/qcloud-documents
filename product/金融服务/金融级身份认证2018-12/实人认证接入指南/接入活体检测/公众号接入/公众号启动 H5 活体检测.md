...
| version    | 参数值为：1.0.0                     | -                                   |
| api ticket | 合作伙伴服务端缓存的 tikcet，注意是 NONCE 类型 | 获取方式请参见 [NONCE ticket 获取](https://cloud.tencent.com/document/product/655/31957)（所用的 userId 参数值需要与接口里的 userId 值保持一致） |
...
## 启动 H5 活体检测
合作方公众号上送 sign，后台校验 sign 通过之后重定向到 H5 活体检测。
...
<tr><td>url</td><td>活体检测完成后回调第三方 URL<br/>H5 活体检测完成后的回调 URL，需要第三方提供完整 URL，且做 URL Encode<br/>完整 URL Encode 示例：<ul><li>原URL：`https://cloud.tencent.com`</li><li>Encode 后：`https%3a%2f%2fcloud.tencent.com`</li></ul></td><td>String</td><td>-</td><td >是</td></tr>

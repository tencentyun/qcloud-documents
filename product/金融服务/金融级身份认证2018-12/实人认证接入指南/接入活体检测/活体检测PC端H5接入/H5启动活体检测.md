...
## 前提条件
活体检测需要录制视频，因此要求浏览器支持视频录制功能，目前适用的浏览器及版本如下所示：
- 火狐浏览器：适用于火狐浏览器29及以上版本。
- Chrome 浏览器：适用于 chrome 浏览器53及以上版本。

## 生成签名
### 准备步骤
- 前置条件：请合作方确保 NONCE ticket 已经正常获取，获取方式见NONCE ticket 获取 。
...
- **计算 SHA1 得到签名：**
```
BADF4F8B38DF09506CEBFF3347A7ACD908A43BF1该字串就是最终生成的签名（40 位），不区分大小写。
```

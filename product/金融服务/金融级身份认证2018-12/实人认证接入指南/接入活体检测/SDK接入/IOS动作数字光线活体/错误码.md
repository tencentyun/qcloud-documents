SDK 在登录以及返回人脸服务结果时，如果发生错误或者识别失败会返回 WBFaceError 对象，该对象的结构如下, 并且在我们判断具体错误时, 需要先根据 domain 字段判断错误发生在 sdk 服务运行中的位置, 然后根据code判断具体的错误：
```
...
@end
```
## 若Domain为：WBFaceErrorDomainInputParams
...
## Domain为：WBFaceErrorDomainLoginNetwork
...
## Domain为：WBFaceErrorDomainLoginServer

| 错误码 | 描述 | 详细实际原因 |
| -------------- | ------------------- | ---------------------- |
| 其他错误码     | 透传后台错误码      | 例如签名问题等等       |

## Domain为：WBFaceErrorDomainGetInfo
...
## Domain为：WBFaceErrorDomainNativeProcess
...
## Domain为：WBFaceErrorDomainCompareNetwork
...
## Domain为：WBFaceErrorDomainCompareServer

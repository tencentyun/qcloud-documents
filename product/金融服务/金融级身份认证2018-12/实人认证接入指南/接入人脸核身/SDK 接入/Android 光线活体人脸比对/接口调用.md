...
InputData 是用来给 SDK 传递一些必须参数所需要使用的对象（`WbCloudFaceVerifySdk.init()`的第二个参数），合作方需要往里塞入 SDK 需要的一些数据，以便启动人脸核身 SDK。InputData 对象中的各个参数定义如下表，请合作方按下表标准传入对应的数据。
...
| verifyMode        | 人脸核身类型<br/>动作活体： FaceVerifyStatus.Mode.MIDDLE<br/>数字活体：FaceVerifyStatus.Mode.ADVANCED<br/>光线活体：FaceVerifyStatus.Mode.REFLECT | FaceVerifyStatus.Mode | -                  | 是       |
...
- 如果合作方对人脸核身安全有进一步的更加严格的要求，可以考虑打开这一选项。**但打开这个字段可能导致某些低性能手机上用户人脸核身不能进行，请慎重考虑。**设置代码如下：

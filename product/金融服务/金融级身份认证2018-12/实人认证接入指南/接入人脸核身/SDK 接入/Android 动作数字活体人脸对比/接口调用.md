...
```
### 是否对录制视频进行检查
- SDK 为了进一步确保刷脸的安全性，不论是简单还是中级模式都有录制用户刷脸视频做存证。但其实在简单模式与中级模式中，起到识别作用的并不是视频文件。在 SDK 使用过程中，视频录制在性能不太好的手机上可能会报错，导致刷脸中断，影响用户体验。
- 为了减少录制视频导致的刷脸中断问题，SDK 默认设置对录制的视频不作检测。
- 如果合作方对刷脸安全有进一步的更加严格的要求，可以考虑打开这一选项。**但打开这个字段可能导致某些低性能手机上用户刷脸无法进行，请慎重考虑。**设置代码如下：
```
# 在 MainActivity 中单击某个按钮的代码逻辑：
  //先将必填的 InputData 放入 Bundle 中
  data.putSerializable(WbCloudFaceVerifySdk.INPUT_DATA, inputData);

...
## 入口方法说明
- 活体检测 + 人脸比对服务（身份证的网纹照片进行对比）

```
/**
 接口服务包含: 活体检测 + 人脸比对(身份证的网纹照片进行对比)
...
## 个性化参数设置
SDK 登录接口 startXXXX 方法中需要传入 WBFaceVerifySDKConfig 字段，通过该对象可以配置 SDK 中其他基础配置：包括设置是否展示成功/结果页面，主题风格，资源路径等等，具体参考头文件。

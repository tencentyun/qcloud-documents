为了确保前端 SDK 和 H5 返回的结果真实性且未被篡改，合作伙伴服务端可以验证结果，我们提供**[前端获取结果验证签名](https://cloud.tencent.com/document/product/655/32051)**和 **服务端验证结果** 两种结果验证方式，合作伙伴可以根据需求自行选择。
服务端验证结果方式用于：
...
<tr><td >code</td><td >String</td><td >0：身份验证成功且认证为同一人</td></tr>
...
>- 由于照片和视频信息有可能超过1M，建议合作伙伴在使用时，获取比对结果用于后续流程处理和存证使用并分开调用，避免网络传输带来的影响。

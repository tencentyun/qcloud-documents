## 生成签名
### 准备步骤
**前置条件：请合作方确保 SIGN ticket 已经正常获取，获取方式见SIGN ticket 获取 。**
...
sourcePhotoStr | 比对源照片，**注意：原始图片不能超过 500k，且必须为 JPG 或 PNG 格式。**<br>**参数有值：**使用合作伙伴提供的比对源照片进行比对，必须注意照片是正脸可信照片，照片质量由合作方保证。 <br>**参数为空：**根据身份证号 + 姓名使用权威数据源比对。 | BASE64String | 1048576 | 否
sourcePhotoType | 比对源照片类型<br>参数值为1 时：水纹正脸照。<br>参数值为 2 时：高清正脸照 | String | 1 | 是

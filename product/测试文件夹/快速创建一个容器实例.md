...
单击进入 [腾讯云容器实例控制台]() 中，按照界面提示为容器实例授权。（如果您已为容器实例授权，请跳过该步骤。）
...
<div style="background-color:#00A4FF; width: 170px; height: 35px; line-height:35px; text-align:center;"><a href="https://console.cloud.tencent.com/tke2/cluster/create?rid=1" target="_blank"  style="color: white; font-size:13px;">点此进入快速创建容器实例页面</a></div><br>
...
在 [容器实例控制台]()，在需要查看事件的实例右侧，单击事件；或者单击实例 ID，单击事件 Tab 查看。如下图所示：
...
## 步骤5：查看容器日志
在 [容器实例控制台]()，在需要查看日志的实例右侧，单击日志；或者单击实例 ID，单击日志 Tab 查看。
...
这里只可查看容器标准输出日志，关于标准输出日志及容器文件日志的采集，详情请参见 [配置日志采集]()。

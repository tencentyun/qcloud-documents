...
Kubernetes 作为 IaaS 和 PaaS 的中间层，当前对于资源的计费依托于 IaaS 层的资源计费。一个 Kubernetes 的集群，按照其购买的节点实例 CVM 计费。而用户则使用的是 Pod，一个 Pod 如何承担成本，如何评估集群的成本，做成本预测和优化建议。
Kubecost 是一款成本分析工具，可以为集群成本提供洞察、分析、推荐和建议。作为您优化集群成本的财务分析师，Kubecost 可以为您提供全面的成本分析报告。
...
当前云厂商的 Node 计费模式一般是**包年包月（Month）**、**按量计费（Hour）**和**竞价实例**。当 Kubecost 计算 Pod 成本时，即使是相同请求量的 Container，在不同类型的 Node 上成本消耗也不一致。

上述三种计费模式中，由于用户购买的是一个整体实例，因此按照实例计费，不针对单独一个资源计费。通过使用 Kubecost，可以参照模型分析资源类型分摊成本，每种资源分摊多少成本。

例如，一个云厂商提供的某种 Node（虚拟机、物理机），CPU 1核(C) 内存 Mem(G) 按照 $20/month 计算, 对于不清楚具体某种资源怎么计价的云厂商，只知道整个 Node 如何计费，但是这其中并不清楚分摊到每种资源（CPU/GPU/Mem/PV/Network） 是如何计费。
总的来说，当前分摊模型比较简单，下面的公式，还需要知道每种资源的基本价格，例如 CPU 如何收费，Mem 如何收费，这个目前可以按照云厂商说明的基础价格来定，或者自己去配置对应的比例，例如 1C : 1G 的价格比是 3：1；CPU/Mem=3:1; 则 CPU 按照 15$/month, Mem 按照 5$/month 分摊成本；加起来是整个Node 的 20$/month，计算公式如下：
...
成本加权平均评估效率，由于每一种资源的成本权重不一样（成本权重就是不同类型的资源，售卖的价格也不一样。例如 CPU 和 GPU 等计算资源价格相对较高，Mem 价格相对较低，Disk 价格相对更低），相同的资源利用效率，不同资源对成本的贡献度不一样。

例如，100%的 Disk 利用率，但是由于磁盘很便宜，对最终的成本控制的贡献就较低。但是如果是 CPU，即使资源利用率为30%，但由于价格昂贵，最终可能对成本起到关键作用，所以需要用做一个加权平均来评估成本效率，例如：
...
哪些业务需要优化，优化成本结构，查找 TOP 10 的某种资源浪费，例如资源的 Usage 和 Request 差别较大，可以根据应用监控画像给出推荐的 Request，最后计算出每种资源可节省的成本。
...
如果有上述需求和问题，都可以使用 Kubecost 来分析自己的集群成本趋势。
...
1. Kubecost 主要给出 cost 分析，包括 Service、Application、Pod、Workload 等各种标签类型维度分析。
2. 资源分配和使用。
3. 包含一个集群健康检查的功能，类似集群巡检和健康检查。
...
2. **聚合：**
   Cost Allocation 可以查看所有原生 Kubernetes 概念的分配支出，例如 NS、Label、Service。 Cost Allocation 还允许将成本分配给 team、product/project、department、or environment 等组织的概念。
3. **时间窗口：**
...
5. **Allocate idle costs（分配空闲成本）：**
   Allocate idle costs 按比例将空闲或空闲集群成本分配给租户。具体来说，这适用于已配置但未被租户完全使用或请求的资源。例如，如果您的集群的利用率仅为25%，以资源的最大使用量和 Request 来衡量，则 Allocate idle costs 会按比例增加每个 pod/NS/Deployment的成本到原来的4倍。
6. **图表选择：**
...
以 Request 推荐为例，如下面第二张图所示，这里提供了三种不同等级的推荐值，并且推荐值与给定的时间窗口也存在关系：
...
1. 登录集群。
2. 单机 service 集群 >【服务和路由】>【Service】进入 Service 页面。
3. 找到您想要更改的 Service，在其右侧操作栏下单击【更改访问方式】， service cost-analyzer-cost-analyzer 的访问方式为 负载均衡方式，会得到一个公网 IPv4 地址，这样可以在公网访问。

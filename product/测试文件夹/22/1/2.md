...
1. 确认配置参数，创建 value.yaml。
   adapter 连接的 prometheus 信息，主要是 URL 和端口，默认值分别是`http://prometheus.default.svc`和9090，当使用云原生监控时，对应的信息需要修改成 [云原生监控控制台](https://console.cloud.tencent.com/tke2/prometheus) **基本信息**中提供的数据查询地址，如下图所示：

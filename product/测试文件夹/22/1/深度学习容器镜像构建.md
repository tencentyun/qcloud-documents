...
所有能对接 alertmanager Webhook 的开源组件都可以对接云原生监控。目前只支持将告警投递到内网地址（该地址需要和当前实例在同个私有网络中，或可被当前内网访问），无法发送到公网，因此无法直接将告警投递到钉钉/企业微信渠道，需要通过开源组件实现中转，本文将以配置云原生监控告警发送到企业微信为例介绍相关配置流程。

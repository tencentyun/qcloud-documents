...
在用户的开发运维过程中，存在需要同时用到多个容器镜像仓库的场景，这些仓库可能是跨主账号、跨地域、跨国、跨平台。用户可以通过手动完成实例间的推送、分发任务，但是会存在运维成本较高、同步不及时、不便于管理等问题。
...
2. 接着在这两个实例里分别 [创建并管理复制实例](https://cloud.tencent.com/document/product/1141/52095#.E5.88.9B.E5.BB.BA.E5.B9.B6.E7.AE.A1.E7.90.86.E5.A4.8D.E5.88.B6.E5.AE.9E.E4.BE.8B) ，分别实现国内和国外的单点上传、多地域实时复制、就近内网拉取。
...
![](https://main.qcloudimg.com/raw/489c53984776e1772c8a27000ca0b098.svg)
![](https://main.qcloudimg.com/raw/8afdd8a50db4193b2122e90a5dc987e9.svg)
...
:::
::: 一键迁移模式
使用 image-transfer 的一键迁移模式可以实现从 TCR 个人版迁移至 TCR 企业版，请直接参考 [个人版迁移至企业版完全指南](https://cloud.tencent.com/document/product/1141/52292)，指南包含利用开源工具 [image-transfer](https://github.com/tkestack/image-transfer) 进行一键数据全量迁移，以及在 TCR 控制台进行业务迁移两大部分，助力您实现业务的平滑迁移。
...
如下图所示，以「将阿里云 ACR 的镜像实时同步至腾讯云 TCR」为例，用户首先在 Harbor 里配置 Pull-based 的复制策略，将 ACR 的镜像实时拉取至 Harbor 中，作为中转；接着再在 Harbor 里配置 Push-based 的复制策略，将 Harbor 内来自 ACR 的镜像实时推送至 TCR 中。
![](https://main.qcloudimg.com/raw/ef0f61933aa3bd15b2321a6768bd0dfe.svg)
...
![](https://main.qcloudimg.com/raw/3c6df1cb41d2b1deb074d04425a7d765.svg)

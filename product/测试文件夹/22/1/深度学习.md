...
本文将再上一篇文档 [构建容器镜像](https://cloud.tencent.com/document/product/457/60220) 基础上继续操作，利用自建集群，在 EKS 上运行深度学习任务。
自建镜像已经上传到镜像仓库中：`ccr.ccs.tencentyun.com/carltk/tensorflow-model`，无需重新构建，可以直接拉取使用。
...
- 因为节点自带 GPU 资源，因此无需 Annotations 和 Resources。在实践操作汇总，您可以保留 Annotations 部分，TKE 不会处理这部分。Resources 则建议注释掉，因为在某些情况下可能会导致不合理的资源需求。

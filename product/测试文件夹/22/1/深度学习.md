...
下面给出控制台的部署范例，如图：
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16278873918955.png)
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16278771798268.png)
需注意：[](id:precautions)
- 因为数据集可能需要联网下载，所以需要配置对集群的外网访问。详情请参见 [容器访问公网问题]((3)%E5%B8%B8%E8%A7%81%E9%97%AE%E9%A2%98.md)。
- 选择 GPU 型号后，在填写 request 和 limit 时需要给容器分配符合 [资源规格](https://cloud.tencent.com/document/product/457/39808) 的 CPU 和内存，实际填写并不严格要求精确到个位。
  在控制台中配置，也可以选择删除默认配置以留空，即为“不限制”，也会有对应的计费规格；更推荐这种做法。
- 容器运行命令 comman 继承 docker 的 CMD 字段，而 CMD 指令首选 exec 形式，不调用 shell 命令。这意味着不会发生正常的 shell 处理。所以我们命令需要 shell 形式运行，就需要在前面添加 `"sh","-c"`。
  在控制台输入多个命令和参数时，每个命令单独一行（以换行为准）。
:::
::: Kubectl 操作指引
可以使用 yaml 文件创建任务，然后通过 `kubectl create -f [yaml_name]` 完成部署。范例 gpu_pod.yaml 如下：
...
- 在 yaml 里需使用 annotations 声明资源分配，详细见[Annotation 说明](https://cloud.tencent.com/document/product/457/44173)。同样需要注意的是不同 GPU 对应不同的 CPU 、内存选项，最好按需填写。
- 此处数据卷使用的是 NFS 。如果不想用 NFS ，选择其他数据卷进行持久化存储，参考[其他存储卷使用说明](https://cloud.tencent.com/document/product/457/31713)。
- annotation 可以**只保留**`eks.tke.cloud.tencent.com/gpu-type` ，**而不需要其他项**。如果写上了`/gpu-count`，那么`cpu`和`mem`也要一起写上（注：这里**不推荐加上其他项**；不加没影响，加了如果没有按规格填写可能会报OOM的错误）。
- 在 GPU 的调度中，对`nvidia.com/gpu`而言，**limits 的是必须且仅需填写的**。如果只写 annotation ，会出现找不到卡的报错。如果只填 limits ，该值将被作为 request 。如果也写上 request ，二者值必须相等。详细参考 K8s 文档[调度 GPUs](https://kubernetes.io/zh/docs/tasks/manage-gpus/scheduling-gpus/)（注：同样**不推荐在 request 和 limits 中加上 cpu 和 memory 的设置**，理由同上）。
:::
</dx-tabs>
...
在【Pod 管理】查看事件
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16278754147175.png)


在【Pod 管理】查看日志

![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16278777015753.png)
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16278777585607.png)



#### 命令行查看

使用命令`kubectl describe pod [name] ` 查看事件
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16278999713005.png)
使用命令`kubectl logs -f [pod_name]` 持续输出日志
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16278999954545.png)
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16279000438914.png)


因为 EKS 即用即消的特性，导致如果想查看日志，必须当且仅当 Pod 还在 Running 时查看。解决方法请参见 [日志采集问题]((3)%E5%B8%B8%E8%A7%81%E9%97%AE%E9%A2%98.md)。

##### 查看存储

如果已经按照前面的配置 NFS ，此时可以前往挂载点，查看 NFS 内存储：

![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16279007403880.png)
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16279008152135.png)
![](https://git.woa.com/carltkcai/eks-gpu-doc/raw/master/image/16279008745454.png)

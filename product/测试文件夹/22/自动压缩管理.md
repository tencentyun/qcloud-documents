## 操作场景
集群运行过程中，对 etcd 键值的所有更新操作都将被记录。为防止集群性能下降或存储空间耗尽，云原生 etcd 支持通过自动压缩功能实现对数据的定时清理，目前支持**周期性压缩**和**根据 revision 压缩**两种压缩模式。本文将为您介绍如何为 etcd 集群设置数据自动压缩。
...
2. 进入“etcd 集群列表”页面，在页面上方选择相关地域，在下方集群列表中选择需要操作的 etcd 集群。

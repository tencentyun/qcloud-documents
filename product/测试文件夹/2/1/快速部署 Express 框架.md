...
本篇文档将为您指导如何通过 Web Function，将您的本地 Express 项目快速部署到云端。
...
3. 选择使用【模版创建】来新建函数，在搜索框里筛选 `WebFunc`，筛选所有 Web 函数模版，选择【Express 框架模版】，单击【下一步】。如下图所示：
...
6. 您可以通过 API 网关生成的访问路径 URL 进行访问，查看您部署的 Express 项目。单击左侧菜单栏中的【触发管理】，查看访问路径。如下图所示：
![](https://main.qcloudimg.com/raw/2f0518e802213e84250e66f768d32d99.png)
如下为在浏览器通过访问路径 URL 访问服务 Express 示例图：
...
1. 执行以下命令安装 Express 框架和express-generator 脚手架，初始化 Express 示例项目。
```shell
...
具体步骤如下：
1. 已知在 Express 示例项目中，通过 `./bin/www` 设置监听地址与端口，打开该文件可以发现，可以通过环境变量设置指定监听端口，否则将自动监听 `3000`。如下图所示：
...
3. 本地配置完成后，执行以下命令启动文件（如下命令为在该文件 `scf_bootstrap` 目录下执行时示例），确保您的服务可以本地正常启动。
<dx-codeblock>
:::  shell
./scf_bootstrap
:::
...
6. 选择使用【自定义创建】来新建函数，根据页面提示配置相关选项。如下图所示：
	- **函数类型**：选择 “Web 函数”。
	- **函数名称**：填写您自己的函数名称。
	- **地域**：填写您的函数部署地域，默认为广州。
...
![](https://main.qcloudimg.com/raw/97529e56867dd8fd0205535d1021f920.png)
7. 单击【完成】即可创建函数。函数创建完成后，您可在“函数管理”页面，查看 Web 函数的基本信息。
...
#### 开发管理
部署完成后，即可在 SCF 控制台快速访问并测试您的 Web 服务，并且体验云函数多项特色功能如层绑定、日志管理等，享受 Serverless 架构带来的低成本、弹性扩缩容等优势。如下图所示：
![](https://main.qcloudimg.com/raw/60776abd2d9d72ac6768087cfb9bf58c.png)

...
本文将为您指导，如何通过 `SCF Web Function`，快速部署您的 Flask 业务上云。
...
3. 选择使用【模版创建】来新建函数，在搜索框里筛选 `WebFunc`，筛选所有 Web 函数模版，选择【Flask 框架模版】，单击【下一步】。如下图所示：
...
如下为在浏览器通过访问路径 URL 访问服务 Flask 示例图：
...
4. 打开浏览器里访问 `http://127.0.0.1:5000`，即可在本地完成 Express 示例项目的访问。如下图所示：
![](https://main.qcloudimg.com/raw/4230ec2b5e5160f25ebd44a8a401524f.png)


#### 部署上云

接下来执行以下步骤，对本地已创建完成的项目进行简单修改，使其可以通过 Web Function 快速部署，对于 Flask，具体改造步骤如下：


1. **安装依赖包**
 1. 由于 SCF 云上标准环境内未提供 Flask 依赖库，此处您必须将依赖文件安装完成后，与项目代码一起打包上传。新建 `requirements.txt` 文件，文件内容如下：
```txt
#requirements.txt
Flask==1.0.2
werkzeug==0.16.0
...
```
2. **修改监听地址与端口**
在 Web 函数内，限制了监听端口必须为 `9000`，因此需要对监听地址端口进行修改，修改为 `0.0.0.0:9000`，如下图所示：
...
4. 本地配置完成后，执行以下命令启动文件（如下命令为在该文件 scf_bootstrap 目录下执行时示例），确保您的服务可以本地正常启动。
<dx-codeblock>
:::  shell
./scf_bootstrap
:::
